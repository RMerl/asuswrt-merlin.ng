 ...
 *
 * $Id: hnddma_priv.h 793358 2020-11-24 11:25:24Z $
 */
...
typedef struct dma_info {
	...
	struct dma_info	*sts_di;	/* dma link to Status fifo (Rx fifo #3) */
	bool		sts_xfer_phyrxs;	/** PhyRx Status xfer is enabled for this FIFO */
	union {
		struct sts_buff_pool *sts_mempool;	/**< STS_FIFO_RXEN: sts mempool header */
		void	*phyrxsts_memory;		/**< PhyRx Status ring memory */
	};
...
#define dma64_txcommit_local(di) \
	W_REG((di)->osh, &(di)->d64txregs->ptr, \
	    (uint32)((di)->xmtptrbase + I2B((di)->txout, (di)->dd64_size)))
