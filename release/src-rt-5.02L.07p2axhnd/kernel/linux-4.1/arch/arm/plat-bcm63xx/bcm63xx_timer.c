 ...
 * = 21474836us */
#define PERIPH_TIMER_PERIOD_MAX	(TIMER_CNT_MAX/(perclk_freq/1000)) 
...
static void __init periph_timer_clocksource_init(void)
{
	 ...
	 * register function */
	clocksource_register_khz(&bcm63xx_clocksource, perclk_freq);

        /* register with sched clock */
	sched_clock_register(bcm63xx_sched_clock_read, 30, perclk_freq*1000);
}
...
static void __init bcm63xx_timer_of_register(struct device_node *np)
{
	struct clk* clk;
	...
	if( !IS_ERR_OR_NULL(clk) ) {
		...
		pr_err("bcm63xx_timer_of_register failed to get clk %p, use default perclk rate %d\n", clk, perclk_freq);
...
CLOCKSOURCE_OF_DECLARE(bcm63xx, "brcm,periph-timer", bcm63xx_timer_of_register);
