...
{ \
	...
	{ \
		...
		{ \
			if (FWINV((result & info->vlanmask##INDEX) < (info->vlantag##INDEX[0] & info->vlanmask##INDEX) || \
				(result & info->vlanmask##INDEX) > (info->vlantag##INDEX[1] & info->vlanmask##INDEX), EBT_SKBVLAN_VLAN_TAG_##INDEX)) \
				return false; \
		} \
		else \
		{ \
	...
	{ \
		result = (skb->vlan_tpid << 16) | (skb->vlan_header[0] >> 16); \
		if (FWINV((result & info->vlanmask##INDEX) < (info->vlantag##INDEX[0] & info->vlanmask##INDEX) || \
				(result & info->vlanmask##INDEX) > (info->vlantag##INDEX[1] & info->vlanmask##INDEX), EBT_SKBVLAN_VLAN_TAG_##INDEX)) \
				return false; \
	} \
...
static bool getvlantag(const struct sk_buff *skb, __u32 expect_tag, __u8 layer, __u32 * vlantag_result)
{
	...
	for (i = 0; (i < skb->vlan_count) && (i < SKB_VLAN_MAX_TAGS); i++)
		{
			if (i == 0)
				vlantag = (skb->vlan_tpid << 16) | (skb->vlan_header[0] >> 16);
			else
				vlantag = (skb->vlan_header[i - 1] << 16) | (skb->vlan_header[i] >> 16);
