 ...
 *
 * $Id: blanket.h 776955 2019-07-15 05:31:14Z $
 *
 *  @author
 *  @bug No known bugs.
 */
...
typedef struct blanket_he_caps {
	uint16 he_bw80_txmcsmap;	/* HE 80Mhz tx mcs map */
	uint16 he_bw80_rxmcsmap;	/* HE 80Mhz rx mcs map */
	uint16 he_bw160_txmcsmap;	/* HE 160Mhz tx mcs map */
	uint16 he_bw160_rxmcsmap;	/* HE 160Mhz rx mcs map */
	uint16 he_bw80p80_txmcsmap;	/* HE 80p80Mhz tx mcs map */
	uint16 he_bw80p80_rxmcsmap;	/* HE 80p80Mhz rx mcs map */
	uint8 caps_ex;			/* HE extended capabilities. */
	uint8 caps;			/* HE capabilities. */
	uint8 valid;			/* non zero value indicates valid he caps */
} blanket_he_caps_t;
...
typedef struct blanket_ap_caps {
	...
	blanket_he_caps_t he_caps;		/* HE capabilities */
} blanket_ap_caps_t;
 ...
 * @return		status of the call. 0 Success. Non Zero Failure
 */
int blanket_get_he_cap(char *ifname, wl_bss_info_t *in_bi, blanket_he_caps_t *out_he_caps);
