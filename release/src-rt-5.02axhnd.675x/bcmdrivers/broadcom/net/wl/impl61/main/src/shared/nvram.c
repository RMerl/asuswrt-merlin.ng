...
BCMINITFN(nvram_rehash)(struct nvram_header *header)
{
	...
	unsigned long l_nvram_space = MAX_NVRAM_SPACE;

	if (nvram_space_str)
		l_nvram_space =  bcm_strtoul(nvram_space_str, NULL, 0);

	if (l_nvram_space < DEF_NVRAM_SPACE)
		l_nvram_space = DEF_NVRAM_SPACE;
	...
	name = (char *) &header[1];
	end = (char *) header + l_nvram_space - 2;
	end[0] = end[1] = '\0';
...
BCMINITFN(_nvram_commit)(struct nvram_header *header)
{
	...
	bzero(ptr, l_nvram_space - sizeof(struct nvram_header));
	...
	for (i = 0; i < NVRAM_HASH_TABLE_SIZE; i++) {
		for (t = curr_nvram_hash[i]; t; t = t->next) {
			if ((ptr + strlen(t->name) + 1 + strlen(t->value) + 1) > end)
				break;
			ptr += snprintf(ptr, l_nvram_space - sizeof(struct nvram_header),
			                "%s=%s", t->name, t->value) + 1;
		}
	}
...
BCMINITFN(_nvram_init)(void *sih, int idx)
{
	...
	if (!(header = (struct nvram_header *) MALLOC_NOPERSIST(si_osh(sih), MAX_NVRAM_SPACE))) {
