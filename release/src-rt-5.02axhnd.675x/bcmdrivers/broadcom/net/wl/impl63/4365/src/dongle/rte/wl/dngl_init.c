 ...
 *
 * $Id: dngl_init.c 672151 2016-11-24 11:30:07Z $
 */
...
BCMATTACHFN(c_init)(void)
{
	...
	event_log_init(sih);
	  /* BUS LOG */
	event_log_set_init(sih, EVENT_LOG_SET_BUS, EVENT_LOG_BUS_BLOCK_SIZE);
	event_log_set_destination_set(EVENT_LOG_SET_BUS, SET_DESTINATION_HOST);
	event_log_set_expand(sih, EVENT_LOG_SET_BUS, EVENT_LOG_BUS_BLOCK_SIZE);

	  /* ERROR LOG  for directing errors to proper log set buffers */
	event_log_set_init(sih, EVENT_LOG_SET_ERROR, EVENT_LOG_ERROR_BLOCK_SIZE);
	event_log_set_destination_set(EVENT_LOG_SET_ERROR, SET_DESTINATION_HOST);
	event_log_set_expand(sih, EVENT_LOG_SET_ERROR, EVENT_LOG_ERROR_BLOCK_SIZE);

#if defined(EVENT_MEM_ALLOC)
	/* Memory allocator */
	event_log_set_init(sih, EVENT_LOG_SET_MEM_API, EVENT_LOG_MEM_API_BLOCK_SIZE);
#endif // endif
	...
	/* ROM printf to be logged by default. Since its a post printf hook
	 * prints are enabled by default
	 */
	event_log_tag_start(EVENT_LOG_TAG_ROM_PRINTF, EVENT_LOG_SET_ERROR,
		EVENT_LOG_TAG_FLAG_LOG|EVENT_LOG_TAG_FLAG_PRINT);
#endif // endif

#ifdef MEM_API_TRACE_BY_DEFAULT
	event_log_tag_start(EVENT_LOG_TAG_MEM_ALLOC_SUCC, EVENT_LOG_SET_MEM_API,
		EVENT_LOG_TAG_FLAG_LOG);
	event_log_tag_start(EVENT_LOG_TAG_MEM_ALLOC_FAIL, EVENT_LOG_SET_MEM_API,
		EVENT_LOG_TAG_FLAG_LOG);
	event_log_tag_start(EVENT_LOG_TAG_MEM_FREE, EVENT_LOG_SET_MEM_API,
		EVENT_LOG_TAG_FLAG_LOG);
#endif /* MEM_API_TRACE_BY_DEFAULT */
	event_log_tag_start(EVENT_LOG_TAG_HNDRTE_EVENT_ERROR, EVENT_LOG_SET_ERROR,
		EVENT_LOG_TAG_FLAG_LOG);

#endif /* EVENT_LOG_COMPILE */
#if defined(EVENT_LOG_COMPILE) && defined(ECOUNTERS)
	ecounters_init(sih);
	event_log_tag_start(EVENT_LOG_TAG_ECOUNTERS_ERROR, EVENT_LOG_SET_ERROR,
		EVENT_LOG_TAG_FLAG_PRINT|EVENT_LOG_TAG_FLAG_LOG);
#endif // endif
