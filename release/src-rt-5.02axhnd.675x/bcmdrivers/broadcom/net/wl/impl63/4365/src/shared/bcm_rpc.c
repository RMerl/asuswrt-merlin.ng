 ...
 *
 * $Id: bcm_rpc.c 708017 2017-06-29 14:11:45Z $
 */
...
#ifdef WLC_LOW
#if !defined(_RTE_)
#error "RPC only supports _RTE_ in LOW driver"
#endif // endif
#endif /* WLC_LOW */
...
#define RPC_RETURN_WAIT_TIMEOUT_MSEC	800 /* NDIS OIDs timeout in 1 second.
					     ...
					     */
#else
#define RPC_RETURN_WAIT_TIMEOUT_MSEC	3200 /* XXX WLRPC_WLC_BMAC_RATE_SHM_OFFSET_ID fails
					      * on 5357/43236 HSIC when set to 800ms.
					      */
#endif // endif
...
#ifdef WLC_LOW
static void bcm_rpc_dump_state(void *arg, int argc, char *argv[]);
#else
...
#else
static void bcm_rpc_dump_pktlog_low(void *arg, int argc, char *argv[]);
#endif // endif
#endif	/* BCMDBG_RPC */
...
bcm_rpc_up(struct rpc_info *rpci)
{
	...
	bcm_rpc_pktlog_init(rpci);
#ifdef WLC_LOW
	hnd_cons_add_cmd("rpcpktdump", bcm_rpc_dump_pktlog_low, rpci);
#endif // endif
#endif /* BCMDBG_RPC */
#ifdef WLC_LOW
	hnd_cons_add_cmd("rpcdump", bcm_rpc_dump_state, rpci);
#endif // endif
	return 0;
}
...
#else
		bcm_rpc_dump_state(rpci, 0, NULL);
#endif // endif
		RPC_OSL_LOCK(rpci->rpc_osh);
		rpci->state = DISCONNECTED;
		RPC_OSL_UNLOCK(rpci->rpc_osh);
...
#if !defined(USBAP)
#if defined(BCM_RPC_NOCOPY) || defined(BCM_RPC_RXNOCOPY) || defined(BCM_RPC_ROC)
			/* XXX Make pkts native for PKTALLOCED check in wlc_down(),
			 * Maybe we should not PKTFRMNATIVE in bulk_recv and only do
			 * PKTFRMNATIVE for MGN pkts in bcm_rpc_buf_recv_high().
			 */
...
#else
static void
bcm_rpc_dump_pktlog_low(void *arg, int argc, char *argv[])
{
	rpc_info_t *rpci;
...
#ifdef WLC_LOW
static void
bcm_rpc_dump_state(void *arg, int argc, char *argv[])
#else
static void
