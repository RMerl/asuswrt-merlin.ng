 ...
 *
 * $Id: bcm_rpc_tp_dbus.c 708017 2017-06-29 14:11:45Z $
 */
...
/*
 * RPC TP CANNOT DROP any RX frame since it's fatal for wl driver not to complete a function call
 * RPCRXLOWAT will trigger rx flowcontrol. Use high and low watermarks to avoid ping-pong
 *
 * XXX This number needs to be revisited. But make it a large number now, assuming
 *     host has lots of memory.
 *   experiment shows in 130Mbps TCP dongle->host, the max buf is around 220
 *   This is to meter TCP traffic since UDP can go unbounded
 *
 *   NOTE: The total rx pkt should smaller than NTXD in wl driver because
 *    if too many RPC rx data pkts are queued up and not dispatched or sent up quickly in rpcq
 *    wl driver will have to drop them(overflow the rxPKT pool) in wlc_rpc_high_dispatch()
 *    and hurt throughput
 */
/* XXX Not that RPC flowcontrol is not available for BCM_RPC_RXNOCOPY since the packet allocation
 * is done via PKTGET, which doesn't keep track of buf_cnt_inuse used in RPC flowcontrol.
 */
#define RPCNUMBUF	\
...
void
bcm_rpc_tp_set_agg_size(rpc_tp_info_t *rpc_th, uint agg_size)
{
	rpc_th->host_rpc_agg_size = agg_size;
}

uint
bcm_rpc_tp_get_agg_size(rpc_tp_info_t *rpc_th)
{
	return rpc_th->host_rpc_agg_size;
}

void
bcm_rpc_tp_set_ampdu_mpdu(rpc_tp_info_t *rpc_th, uint ampdu_mpdu)
{
	rpc_th->ampdu_mpdu = ampdu_mpdu;
}

uint
bcm_rpc_tp_get_ampdu_mpdu(rpc_tp_info_t *rpc_th)
{
	return rpc_th->ampdu_mpdu;
}
...
bcm_rpc_tp_buf_send_internal(rpc_tp_info_t * rpcb, rpc_buf_t *b)
{
	...
	/* XXX: PR59343: some USB host controllers have trouble to send ZLP with
	 * multiple of 512 byte frames. add padding to help host not to send ZLP.
	 */
