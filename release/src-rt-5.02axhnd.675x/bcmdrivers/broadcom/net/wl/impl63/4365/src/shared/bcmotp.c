 ...
 *
 * $Id: bcmotp.c 716876 2017-08-22 04:02:59Z $
 */
...
BCMNMIATTACHFN(ipxotp_write_region)(void *oh, int region, uint16 *data, uint wlen, uint flags)
{
	...
	case OTP_HW_RGN:
		...
		/* The OTPGU_HWP_OFF bit has been set on ATE for 4365b1 (chiprev=3), but not for c0,
		 * we need to additionally check the Hardware/Software Boundary for 4365b1 to see if
		 * the Hardware/Software Boundary is configured
		 */
		if (BCM4365_CHIP(sih->chip) && CHIPREV(sih->chiprev) == 3 &&
			rewrite) {
			if (ipxotp_otpr(oi, &otpregs, oi->otpgu_base + OTPGU_HSB_OFF) == 0) {
				rewrite = FALSE;
			}
		}
		base = oi->hwbase;
...
BCMNMIATTACHFN(otp_init)(si_t *sih)
{
	...
	event_log_tag_start(EVENT_LOG_TAG_OTP_ERROR, EVENT_LOG_SET_ERROR,
