...
#ifndef D11_EXT_BUFFER_LEN
#define D11_EXT_BUFFER_LEN	(64)
#endif // endif
...
#define LFRAG_BUF_POOL_NUM	2	/* D3_BUFFER and D11_BUFFER pools */

#ifdef HOST_HDR_FETCH
lfrag_buf_pool_t *d11_ext_lfrag_buf_pool = NULL;
#endif // endif
...
_pktpool_get(pktpool_t *pktp, void *bufp)
{
	...
	else if (BCMDHDHDR_ENAB() && BCMLFRAG_ENAB() && (pktp->type == lbuf_frag) && bufp) {
		void *buf;
...
BCMATTACHFN(hnd_pktpool_init)(osl_t *osh)
{
...
#if defined(BCM_DHDHDR) && defined(DONGLEBUILD)
	if (BCMDHDHDR_ENAB()) {
		pool_num = LFRAG_BUF_POOL_NUM;
#ifdef HOST_HDR_FETCH
		if (HOST_HDR_FETCH_ENAB()) {
			/* one more pool for D11_EXT_BUFFER */
			pool_num++;
		}
#endif // endif
		d3_lfrag_buf_pool = MALLOCZ(osh, sizeof(lfrag_buf_pool_t) * pool_num);
...
#ifdef HOST_HDR_FETCH
		if (HOST_HDR_FETCH_ENAB()) {
			d11_ext_lfrag_buf_pool = d11_lfrag_buf_pool + 1;
		}
#endif // endif
	}
#endif /* BCM_DHDHDR && DONGLEBUILD */
	...
	if (BCMDHDHDR_ENAB()) {
		...
		n = 1;
		metadata_len = AMSDUFRAG_ENAB() ? LFRAG_METADATA_LEN_AMSDUFRAG : LFRAG_METADATA_LEN;
		if (pktpool_init(osh, pktpool_shared_lfrag, &n, metadata_len,
...
#ifdef HOST_HDR_FETCH
		if (HOST_HDR_FETCH_ENAB()) {
			/* D11_BUFFER_EXT pool initialization */
			n = 1;
			if (lfbufpool_init(osh, d11_ext_lfrag_buf_pool, &n, PKTFRAGSZ) == BCME_ERROR) {
				ASSERT(0);
				goto error6;
			}
			lfbufpool_setmaxlen(d11_ext_lfrag_buf_pool, D11_EXT_BUFFER_LEN);
		}
#endif /* HOST_HDR_FETCH */
	} else
#endif /* BCM_DHDHDR && DONGLEBUILD */
...
#ifdef SHARED_FRAG_POOL_LEN_SHRINK
		pktpool_setmaxlen(pktpool_shared_lfrag, SHARED_FRAG_POOL_LEN_SHRINK);
#else
		pktpool_setmaxlen(pktpool_shared_lfrag, SHARED_FRAG_POOL_LEN);
#endif /* SHARED_FRAG_POOL_LEN_SHRINK */
	}
#endif /* BCMFRAGPOOL && !BCMFRAGPOOL_DISABLED */
#if defined(BCMRXFRAGPOOL) && !defined(BCMRXFRAGPOOL_DISABLED)
	n = 1;
...
#ifdef HOST_HDR_FETCH
		if (d11_ext_lfrag_buf_pool)
			lfbufpool_deinit(osh, d11_ext_lfrag_buf_pool);
#endif // endif
	}
#endif /* BCM_DHDHDR && DONGLEBUILD */
error5:
