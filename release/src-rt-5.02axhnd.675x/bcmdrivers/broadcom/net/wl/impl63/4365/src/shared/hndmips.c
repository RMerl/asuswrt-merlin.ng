 ...
 *
 * $Id: hndmips.c 708017 2017-06-29 14:11:45Z $
 */
...
BCMATTACHFN(si_mips_init)(si_t *sih, uint shirqmap)
{
	...
	case BCM4706_CHIP_ID:
		...
		si_setirq(sih, 0, CC_CORE_ID, 0);
		/* XXX. Make sure to assign proper interrupt number.
		si_setirq(sih, 0, ALTA_CORE_ID, 0);
		*/
...
do_router_coma(si_t *sih, void *dmem, int delay)
{
	...
	/* XXX, below "Set switching freq ..." code cause the 5357C0 board
	 * reboot immediately when delay is zero.  For now disable it
	 * before we find the reason.
	 */
	if (((CHIPID(sih->chip)) != BCM53572_CHIP_ID)) {
