 ...
 *
 * $Id: hndpci.c 708017 2017-06-29 14:11:45Z $
 */
...
extpci_write_config(si_t *sih, uint bus, uint dev, uint func, uint off, void *buf, int len)
{
	...
	if (is_hostbridge && reg == NULL) {
		...
		/* XXX Do a dummy read back of the config register. If this
		 * read back is not present the config write is resulting in
		 * a bus error or some times hang. The root cause of the issue
		 * isn't known but the dummy read back war seem to solve the
		 * hang/crash issue.
		 */
...
BCMATTACHFN(hndpci_init_pci)(si_t *sih, uint coreunit)
{
	...
	if (!host) {
		...
		if ((chip == BCM4716_CHIP_ID) || (chip == BCM4748_CHIP_ID) ||
			...
			/* XXX: This should be done with an ai_xxx interface that
			 * gives us wrappers other than the controlling wrapper
			 * for each core, but we don't have one yet. Further, if we
			 * did, it would be wrong on 4716 since the erom has the
			 * wrong address for the slave wrapper in the PCIe core.
			 * So we just hardwire it here.
			 */
