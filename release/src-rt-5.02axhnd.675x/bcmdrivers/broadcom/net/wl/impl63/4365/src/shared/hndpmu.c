 ...
 *
 * $Id: hndpmu.c 716876 2017-08-22 04:02:59Z $
 */
...
/** XXX
 * this function is not called in PHOENIX2_BRANCH_6_10 nor AARDVARK_BRANCH_6_30.
 * Dead code ?
 */
uint32
...
si_pmu_res_masks(si_t *sih, uint32 *pmin, uint32 *pmax)
{
	...
	case BCM4358_CHIP_ID:
		...
		if (SR_ENAB() && sr_isenab(sih)) {
			...
			/* XXX: JIRA:HW4350-352:
			* This fix is the VDDIO misconnect issue.
			* Before board rework, the WAR solution is for the
			* firmware to set the PMU min-rsrc = 0x407.
			* This is applicable to only 4354a1 PCIe WLCSP pkg.
			*/
...
BCMATTACHFN(si_pmu_res_init)(si_t *sih, osl_t *osh)
{
	...
	if ((CHIPID(sih->chip) == BCM43602_CHIP_ID) && SR_ENAB() && sr_isenab(sih)) {
...
BCMINITFN(si_pmu2_alpclk0)(si_t *sih, osl_t *osh, pmuregs_t *pmu)
{
	...
	/* XXX STALE:
	 * uint32 prescale = (pll0 & PMU15_PLL_PC0_PRESCALE_MASK) >> PMU15_PLL_PC0_PRESCALE_SHIFT;
	 */
...
BCMATTACHFN(si_pmu1_pllinit0)(si_t *sih, osl_t *osh, pmuregs_t *pmu, uint32 xtal)
{
	...
	*/
	...
	if ((CHIPID(sih->chip) == BCM4329_CHIP_ID) && (CHIPREV(sih->chiprev) == 0)) {
		 ...
		 * Higher MAC clock frequency would take the ratio higher
		 * and lower MAC clk frequency would cause high uCode latencies
		 */
...
#ifdef ATE_BUILD
static void
BCMATTACHFN(si_set_bb_vcofreq_ate)(si_t *sih, osl_t *osh, pmuregs_t *pmu)
{
	uint32 oldpmucontrol;

	/* max_res_mask, min_res_mask -> 0x3fff */
	W_REG(osh, &pmu->max_res_mask, 0x3fff);
	W_REG(osh, &pmu->min_res_mask, 0x3fff);

	/* make sure resource wake up on 0x3fff */
	si_pmu_wait_for_steady_state(sih, osh, pmu);
	SPINWAIT(!(R_REG(osh, &pmu->res_state) & 0x3fff),
		PMU_MAX_TRANSITION_DLY);
	/* make sure bit13 is enabled */
	ASSERT(R_REG(osh, &pmu->res_state) & 0x2000);

	/* set ndiv_frac = 408643 */
	W_REG(osh, &pmu->pllcontrol_addr, 0x03);
	W_REG(osh, &pmu->pllcontrol_data, 0x63c43);

	/*
	Set  ch0_mdiv = 12(dot11mac_backendclk = 162.59MHz,
	macphyclk(if pmu_chip_cntrl[120] = 1'b0) = 80.75MHz),
	# ch1_mdiv =3(dot11acphy_adc_test_clk),
	# ch2_mdiv = 10(dot11mac_clk = 195.11MHz),
	# ch3_mdiv =12(dot11mac_avbclk)
	*/
	W_REG(osh, &pmu->pllcontrol_addr, 0x04);
	W_REG(osh, &pmu->pllcontrol_data, 0x0c0a0306);

	/* set areset[bit3] to 1, and dreset[bit4] to 1 */
	W_REG(osh, &pmu->pllcontrol_addr, 0x0);
	W_REG(osh, &pmu->pllcontrol_data, 0x53607e0c);

	/* Set pllCtrlUpdate */
	oldpmucontrol = R_REG(osh, &pmu->pmucontrol);
	W_REG(osh, &pmu->pmucontrol, oldpmucontrol | (0x1 << 10));

	OSL_DELAY(10);

	/* set areset[bit3] to 0, and dreset[bit4] to 1 */
	W_REG(osh, &pmu->pllcontrol_addr, 0x0);
	W_REG(osh, &pmu->pllcontrol_data, 0x53607e08);
	...
	/* set areset[bit3] to 1, and dreset[bit4] to 0 */
	W_REG(osh, &pmu->pllcontrol_addr, 0x0);
	W_REG(osh, &pmu->pllcontrol_data, 0x53600000);

	/* Set pllCtrlUpdate */
	oldpmucontrol = R_REG(osh, &pmu->pmucontrol);
	W_REG(osh, &pmu->pmucontrol, oldpmucontrol | (0x1 << 10));

	PMU_ERROR(("%s: set BBPLL to 162.xxx MHz\n", __FUNCTION__));

} /* si_set_bb_vcofreq_ate */
#endif /* ATE_BUILD */
...
BCMATTACHFN(si_set_clocks_4365_c0)(si_t *sih, osl_t *osh, pmuregs_t *pmu)
{
	...
	W_REG(osh, &pmu->chipcontrol_addr, 3);
	val = R_REG(osh, &pmu->chipcontrol_data);
	val &= ~(0xf | (0xf << 11) | (0xf << 15) | (0xf << 19));
	val |= (0x2 | (0x1 << 11) | (0x2 << 15) | (0x4 << 19));
	val |= (0x1 << 8);
	W_REG(osh, &pmu->chipcontrol_data, val);
	val &= ~(0x1 << 8);
	W_REG(osh, &pmu->chipcontrol_data, val);
...
BCMATTACHFN(si_set_clocks_4365)(si_t *sih, osl_t *osh, pmuregs_t *pmu)
{
	...
	W_REG(osh, &pmu->pllcontrol_addr, 9);
	W_REG(osh, &pmu->pllcontrol_data, vco_val);

	/* Set ch0_mdiv = 2, ch1_mdiv =4, ch2_mdiv = 8 */
	W_REG(osh, &pmu->pllcontrol_addr, 0xb);
	W_REG(osh, &pmu->pllcontrol_data, 0x00080402);

	/* set areset[bit3] to 1, and dreset[bit4] to 1 */
	W_REG(osh, &pmu->pllcontrol_addr, 0x7);
	W_REG(osh, &pmu->pllcontrol_data, 0x53600e0c);
	/* Set pllCtrlUpdate */
	oldpmucontrol = R_REG(osh, &pmu->pmucontrol);
	...
	/* set areset[bit3] to 0, and dreset[bit4] to 1 */
	W_REG(osh, &pmu->pllcontrol_addr, 0x7);
	W_REG(osh, &pmu->pllcontrol_data, 0x53600e08);
	/* Set pllCtrlUpdate */
	oldpmucontrol = R_REG(osh, &pmu->pmucontrol);
	W_REG(osh, &pmu->pmucontrol, oldpmucontrol | (1<<10));
	OSL_DELAY(50);
	...
	if (ca7_regs) {
		/* Configured CPU/CCI/Backplane clocks */
...
BCMATTACHFN(si_set_bb_vcofreq_4365)(si_t *sih, osl_t *osh, pmuregs_t *pmu)
{
	...
	W_REG(osh, &pmu->pllcontrol_addr, 9);
	W_REG(osh, &pmu->pllcontrol_data, 0x201);
...
BCMATTACHFN(si_pmu_pll_init)(si_t *sih, osl_t *osh, uint xtalfreq)
{
	...
	case BCM43231_CHIP_ID:
	case BCM4342_CHIP_ID: {
		/* XXX PR52182 WAR for 4322A0.
		 * set PMU SB PLL control register 10 to 0x380005c0 for better jitter performance
		 */
		if (CHIPREV(sih->chiprev) == 0) {
	...
	case BCM43666_CHIP_ID:
		...
		if (sih->buscorerev == 15) {
			si_set_bb_vcofreq_4365(sih, osh, pmu);
		}
#endif // endif

#ifdef ATE_BUILD
		si_set_bb_vcofreq_ate(sih, osh, pmu);
#else
...
#ifdef BCMUSBDEV_ENABLED
	/*
	 * PR99735: If the PLL is turned off and on for updating the spur mode, there is a
	 * Zero Length packet seen over the USB/HSIC bus which causes the USB/HSIC host
	 * controller crash. So do not enable spur mode for USB/HSIC. This has a potential fix in
	 * 4324B0 and hence the revision check is added here.
	 */
...
si_pmu_spuravoid_pllupdate(si_t *sih, pmuregs_t *pmu, osl_t *osh, uint8 spuravoid)
{
	...
	uint32 *pllctrl_addr = (uint32 *)&pmu->pllcontrol_addr;
	uint32 *pllctrl_data = (uint32 *)&pmu->pllcontrol_data;
...
BCMATTACHFN(si_pmu_set_avs_4349_grp)(si_t *sih)
{
	...
	if (getintvar(NULL, rstr_avs_enab)) {
		...
		if ((avs_status & AVS_STATUS_ENABLED_FLAG) &&
			...
			/* TODO: JIRA SW4349-458. The AVS equation is not ready at the moment. Need
			   to correct the below 2 statements once the equation is ready.
			 */
...
si_pmu_set_ulbmode(si_t *sih, osl_t *osh, uint8 pmu_ulb_bw)
{
	...
	switch (CHIPID(sih->chip)) {
		...
		case BCM43666_CHIP_ID:
		{
			...
			si_pmu_pllcontrol(sih, PMU1_PLL0_PLLCTL1, ~0, pll_reg1 | (0x3f<<9));

			/* Set the m0div and m2div value for 5/10/20MHz mode */
			si_pmu_pllcontrol(sih, PMU1_PLL0_PLLCTL4, ~0, mdiv);

			/* set areset[bit3] to 1, and dreset[bit4] to 1 */
			si_pmu_pllcontrol(sih, PMU1_PLL0_PLLCTL0, ~0, pll_reg0 | (1<<3) | (1<<4));
			/* Updating PMU to load the PLL */
			si_pmu_pllupd(sih);
			OSL_DELAY(10);

			/* set areset[bit3] to 0, and dreset[bit4] to 1 */
			si_pmu_pllcontrol(sih, PMU1_PLL0_PLLCTL0, ~0,
					(pll_reg0 & ~(1<<3)) | (1<<4));
			/* Updating PMU to load the PLL */
			si_pmu_pllupd(sih);
			OSL_DELAY(50);
