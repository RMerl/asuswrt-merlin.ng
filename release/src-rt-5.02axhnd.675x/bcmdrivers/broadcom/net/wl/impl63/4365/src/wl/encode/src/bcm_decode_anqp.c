...
#if (defined(WL_MBO) && !defined(WL_MBO_DISABLED)) || !defined(BCM_DECODE_NO_ANQP)
		case ANQP_ID_QUERY_LIST:
			anqp->anqpQueryListLength = dataLength;
			anqp->anqpQueryListBuffer = dataPtr;
			break;
#endif /* MBO && !MBO_DISABLED && !BCM_DECODE_NO_ANQP */
#ifndef BCM_DECODE_NO_ANQP
	...
	char *realmName, uint8 eapMethod, uint8 credential)
{
	...
	for (i = 0; i < realmList->realmCount; i++) {
		...
		strncpy((char *)realm, (char *)data->realm, sizeof(realm) - 1);
...
int bcm_decode_anqp_is_domain_name(bcm_decode_anqp_domain_name_list_t *list, char *domain)
{
	int i;
