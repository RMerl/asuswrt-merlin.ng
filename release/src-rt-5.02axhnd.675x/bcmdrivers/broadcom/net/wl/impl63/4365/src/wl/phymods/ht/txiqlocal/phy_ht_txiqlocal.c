...
BCMATTACHFN(phy_ht_txiqlocal_register_impl)(phy_info_t *pi, phy_ht_info_t *aci,
	phy_txiqlocal_info_t *cmn_info)
{
	phy_ht_txiqlocal_info_t *ac_info;
	phy_type_txiqlocal_fns_t fns;

	PHY_CAL(("%s\n", __FUNCTION__));

	/* allocate all storage together */
	if ((ac_info = phy_malloc(pi, sizeof(phy_ht_txiqlocal_info_t))) == NULL) {
		PHY_ERROR(("%s: phy_malloc failed\n", __FUNCTION__));
		goto fail;
	}

	/* Initialize params */
	bzero(ac_info, sizeof(phy_ht_txiqlocal_info_t));
	ac_info->pi = pi;
	ac_info->aci = aci;
...
fail:
	if (ac_info != NULL)
		phy_mfree(pi, ac_info, sizeof(phy_ht_txiqlocal_info_t));
	return NULL;
}

void
BCMATTACHFN(phy_ht_txiqlocal_unregister_impl)(phy_ht_txiqlocal_info_t *ac_info)
{
