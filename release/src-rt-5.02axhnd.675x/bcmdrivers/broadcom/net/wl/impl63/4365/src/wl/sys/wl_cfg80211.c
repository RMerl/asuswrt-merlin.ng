 ...
 *
 * $Id: wl_cfg80211.c 786802 2020-05-08 12:21:49Z $
 */
/* */
#include <typedefs.h>
#include <linuxver.h>
...
#ifdef WL_CFG80211_NIC
/*
 * TODO: Implement these 3 after the ucode that supports the P2P is ready
 */
int
wl_cfg80211_register_if(struct bcm_cfg80211 *cfg, int ifidx, struct net_device *ndev)
{
	WL_ERR(("Enter, not implemented\n"));
	return 0;
}

int
wl_cfg80211_remove_if(struct bcm_cfg80211 *cfg, int ifidx, struct net_device *ndev)
{
	WL_ERR(("Enter, not implemented\n"));
	return 0;
}
...
struct net_device *
wl_cfg80211_allocate_if(struct bcm_cfg80211 *cfg, int ifidx, char *name,
				...
				if (((wpa2_ie = bcm_parse_tlvs((u8 *)sme->ie, sme->ie_len,
					...
					if (rsn_cap[0] & RSN_CAP_MFPC) {
						...
						if ((sme->crypto.n_akm_suites == 1) &&
							...
							u32 wpa_auth;
							if ((err = wldev_iovar_getint_bsscfg(dev,
							    "wpa_auth", &wpa_auth, bssidx)) < 0) {
							    return -1;
							}
							if (sme->crypto.akm_suites[0] ==
							    WL_AKM_SUITE_SHA256_PSK)
...
#ifdef WL_CFG80211_NIC
	if (wl_cfg80211_bcn_bringup_ap(wiphy, dev, &ies, dev_role, bssidx) < 0) {
#else
...
static s32 wl_event_handler(void *data)
{
	...
	while (down_interruptible (&tsk->sema) == 0) {
		...
		while ((e = wl_deq_event(cfg))) {
			...
			p2p_disc_bssidx = cfg->p2p ? wl_to_p2p_bss_bssidx(cfg,
				P2PAPI_BSSCFG_DEVICE) : 0;

			if ((p2p_disc_bssidx > 0) && (e->emsg.bsscfgidx == p2p_disc_bssidx)) {
				/* P2P Discovery Interface. Put the events on to the
				 * p2p0 interface
				 */
				wdev = bcmcfg_to_p2p_wdev(cfg);
			} else if (cfg->bss_cfgdev && (e->emsg.bsscfgidx == cfg->cfgdev_bssidx)) {
				/* Dual STA / AP / AIBSS case */
				wdev = (ndev = cfgdev_to_wlc_ndev(cfg->bss_cfgdev, cfg)) ?
					ndev->ieee80211_ptr : NULL;
			} else {
				/* STA case / P2P Group Interface */
#ifdef WL_CFG80211_NIC
				ndev = bcmcfg_to_prmry_ndev(cfg);
#endif /* WL_CFG80211_NIC */
				if (ndev)
			...
			if (!wdev) {
				WL_ERR((" Event not mapped to the Interface correctly \n"));
...
#ifdef WL_CFG80211_NIC
				/* TODO:
				 * Currently the 4365/4366 PM support is TBD
				 * Workaround to prevent AXI error
				 * Remove this after PM is supported
				 */
				wlc_rev_info_t revinfo;

				wldev_ioctl(iter->ndev, WLC_GET_REVINFO, &revinfo,
					sizeof(revinfo), false);
				if (revinfo.chipnum == 0x4365) {
					WL_ERR(("!! TODO: 4365/4366 Not support WLC_SET_PM\n"));
					continue;
				}
#endif /* WL_CFG80211_NIC */
