 ...
 *
 * $Id: wlc_pub.h 778298 2019-08-29 00:43:36Z $
 */
...
struct wlc_bss_info {
	...
	uint8		ht_ccfs2;
	uint8		bcnflags;	/* additional flags for beacon */
};
...
#define MU_FEATURES_AUTO	(1 << 15)
...
#endif // endif
	int		_mutx_max;		/* Max number of MU clients; configured by NVRAM:
						 * at boot:
						 * -1: auto/not specified
						 *  0: No MUTX, 2: MU-2, 4: MU-4, 8: MU-8
						 */
    ...
    bool        rxdma_stuck; /* RXDMA stuck */
    uint16      rxdma_stuck_cnt; /* DMA stuck count */
    uint16      rxin_last;   /* Last count of DMA rxin */
    uint        rxfifo2_cnt;   /* no of frames recvd over rxfifo2 */
    bool        trig_rxstuck;   /* trigger rxstuck condition */
	uint		rxstuck_tmo;   /* rx stuck timeout */
} wlc_pub_t;

#define MUTX_MAX_AUTO			(-1)
#define MUTX_MAX_0			0
#define MUTX_MAX_2			2
#define MUTX_MAX_4			4
#define MUTX_MAX_8			8
...
typedef struct wlc_pub_cmn {
	...
	bool		_mbo;			/* set to True for MBO feature enable */
 ...
 *
 * ***********************************************************************************************
 * please add new elements  at the end of the structure. As some of the existing fields are
 * accessed using the address + offset in pcie module. as this structure
 * is not shared with pcie
 *
 * snr is accessed with offset MON_PKTTAG_NOISE_IDX(11)
 * rssi is accessed with offset MON_PKTTAG_RSSI_IDX(12)
 *
 * ***********************************************************************************************
 */
...
#if defined(WLAWDL) && defined(MACOSX)
#define WLF2_PCB3_AWDL_TX	1	/* overloaded with WLF2_PCB3_WLFC, wlc_awdl_tx_complete */
#endif /* WLAWDL */
...
/* AWDL Support */
	...
	#define AWDL_SUPPORT(pub)	((pub)->_awdl_support)
	#define AWDL_ENAB(pub)		((pub)->_awdl)
#elif defined(WLAWDL_DISABLED)
	#define AWDL_SUPPORT(pub)	(0)
	#define AWDL_ENAB(pub)		(0)
#else
	#define AWDL_SUPPORT(pub)	(1)
	#define AWDL_ENAB(pub)		((pub)->_awdl)
#endif // endif
#else
#define AWDL_SUPPORT(pub)		(0)
...
/* SLAVE RADAR Support */
#if defined(BAND5G) && (defined(SLAVE_RADAR) || defined(CLIENT_CSA))
 ...
 * feature, WL_ENAB_RUNTIME_CHECK is defined.
 */
#if defined(SRHWVSDB)
	#if defined(WL_ENAB_RUNTIME_CHECK) || !defined(DONGLEBUILD)
