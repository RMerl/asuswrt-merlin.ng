...
ap_eap_sm_process_protocol(char *eap_msg)
{
	...
	/* skip 2 bytes length info, if any. */
	if (wpsEapHdr->flags & EAP_WPS_FLAGS_LF) {
		in_data += EAP_WPS_LF_OFFSET;
		in_len -= EAP_WPS_LF_OFFSET;
	}
...
ap_eap_sm_process_frag(char *eap_msg)
{
	...
	int wps_data_received = 0;

	TUTRACE((TUTRACE_INFO, "Receive a EAP WPS fragment packet\n"));

	if (apEapState->total_received) {
		wps_data_received = apEapState->total_received - sizeof(WpsEapHdr);
	}
	...
	if (apEapState->total_received == 0) {
		/* Make sure it's a valid fragmentation. */
		if (wps_len < EAP_WPS_LF_OFFSET) {
			TUTRACE((TUTRACE_ERR, "Received invalid WPS data (len %d) in "
				"first fragmentation\n", wps_len));
			return EAP_FAILURE;
		}
	...
	/* frags_received ignore length field 2 bytes copy, so reset WpsEapHdr flags */
	wpsEapHdr->flags &= ~(EAP_WPS_FLAGS_MF | EAP_WPS_FLAGS_LF);
...
static int
ap_eap_sm_isAuthorizedMac(unsigned char *ether_shost,
	unsigned char *authorizedMacs, int authorizedMacs_len)
{
	uint8 wc_mac[] = {0xff, 0xff, 0xff, 0xff, 0xff, 0xff}; /* wildcard mac */
	int i, ret = WPS_ERR_GENERIC;
	uint8 *amac;

	for (i = 0; i < authorizedMacs_len; i += SIZE_MAC_ADDR)
	{
		amac = &authorizedMacs[i];
		if ((memcmp(authorizedMacs, ether_shost, SIZE_MAC_ADDR) == 0) ||
			(memcmp(authorizedMacs, wc_mac, SIZE_MAC_ADDR) == 0)) {
			ret = WPS_SUCCESS;
			break;
		}
	}
...
uint32
ap_eap_sm_process_sta_msg(char *msg, int msg_len, char *authorizedMacs, int authorizedMacs_len)
{
	...
	if (apEap_wps_version2 && authorizedMacs_len >= SIZE_MAC_ADDR) {
		if ((ret = ap_eap_sm_isAuthorizedMac(eapolHdr->eth.ether_shost,
			authorizedMacs, authorizedMacs_len) != WPS_SUCCESS)) {
			TUTRACE((TUTRACE_ERR,  "Not a AuthorizedMAC\n"));
			return ret;
		}
	}
	...
	if (IS_EAP_TYPE_WPS(wpsEapHdr) && REASSEMBLING(apEapState, wpsEapHdr)) {
		if (apEapState->total_bytes_to_recv == 0) {
			...
			total_wps_data_len = WpsNtohs((uint8 *)total_bytes_to_recv);
			if (total_wps_data_len > (WPS_EAP_DATA_MAX_LENGTH - sizeof(WpsEapHdr)))
				return EAP_FAILURE;

			/* Save total EAP message length */
			apEapState->total_bytes_to_recv = total_wps_data_len;
	...
	unsigned int (*send_data)(char *, uint32), int eap_frag_threshold, int wps_delay_deauth_ms)
{
	...
	apEapState->wps_delay_deauth_ms = wps_delay_deauth_ms;
...
ap_eap_sm_Failure(int deauthFlag)
{
	...
	if (deauthFlag)
		WpsSleepMs(apEapState->wps_delay_deauth_ms);
