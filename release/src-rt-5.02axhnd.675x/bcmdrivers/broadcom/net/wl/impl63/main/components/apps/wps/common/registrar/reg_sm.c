...
reg_sm_handleMessage(RegSM *r, BufferObj *msg, BufferObj *outmsg)
{
	...
	case M6:
		...
		/* AirPort WAR */
		if (!r->reg_info->enrollee->b_ap &&
		    strlen(r->reg_info->dev_info->nwKey) != 64 &&
		    !strncmp(r->reg_info->enrollee->manufacturer, "Apple ", 6) &&
		    !strncmp(r->reg_info->enrollee->modelName, "AirPort", 7)) {
			if (wps_createM8StaPskKey(r->g_mc) != WPS_SUCCESS) {
...
reg_proto_process_m1(RegData *regInfo, BufferObj *msg)
{
	...
	if (IS_OOB_DEVPWID(regInfo->registrar->devPwdId)) {
		...
		if (sha2(HASH_SHA256, NULL, 0, regInfo->pke, SIZE_PUB_KEY, DHKey,
			SHA2_SHA256_DIGEST_LEN) != 0) {
...
reg_proto_create_m2(RegData *regInfo, BufferObj *msg)
{
	...
	if (sha2(HASH_SHA256, NULL, 0, secret, secretLen, DHKey, SHA2_SHA256_DIGEST_LEN) != 0) {
	...
	(void)hmac_sha2(HASH_SHA256, DHKey, SIZE_256_BITS, NULL, 0,
		buf2->pBase, buf2->m_dataLength, kdk, SIZE_256_BITS);
...
#ifdef WFA_WPS_20_NFC_TESTBED_DEBUG
{
	...
	/* Public Key Hash check */
	if (sha2(HASH_SHA256, NULL, 0, regInfo->pkr, SIZE_PUB_KEY, DHKey,
		SHA2_SHA256_DIGEST_LEN) != 0) {
		TUTRACE((TUTRACE_ERR, "Public key hash: SHA256 calculation failed\n"));
	}
	...
	(void)hmac_sha2(HASH_SHA256, regInfo->authKey->pBase, SIZE_256_BITS, NULL, 0,
		buffobj_GetBuf(hmacData), buffobj_Length(hmacData), hmac, SIZE_256_BITS);
...
reg_proto_process_m3(RegData *regInfo, BufferObj *msg)
{
	...
	*/
	...
	tlv_err |= tlv_dserialize(&m.registrarNonce, WPS_ID_REGISTRAR_NONCE,
		msg, SIZE_128_BITS, 0);
	tlv_err |= tlv_dserialize(&m.eHash1, WPS_ID_E_HASH1, msg, SIZE_256_BITS, 0);
	tlv_err |= tlv_dserialize(&m.eHash2, WPS_ID_E_HASH2, msg, SIZE_256_BITS, 0);
	if (tlv_err) {
		err = RPROT_ERR_INVALID_VALUE;
		goto error;
	}
...
reg_proto_create_m4(RegData *regInfo, BufferObj *msg)
{
	 ...
	 */
	(void)hmac_sha2(HASH_SHA256, regInfo->authKey->pBase, SIZE_256_BITS, NULL, 0,
		pwdPtr, (pwdLen/2)+(pwdLen%2), hashBuf, SIZE_256_BITS);
	...
	(void)hmac_sha2(HASH_SHA256, regInfo->authKey->pBase, SIZE_256_BITS, NULL, 0,
		pwdPtr+(pwdLen/2)+(pwdLen%2), pwdLen/2, hashBuf, SIZE_256_BITS);
	...
	RAND_bytes(regInfo->rs2, SIZE_128_BITS);
	{
		...
		(void)hmac_sha2(HASH_SHA256, regInfo->authKey->pBase, SIZE_256_BITS, NULL, 0,
			rhashBuf->pBase, rhashBuf->m_dataLength, hashBuf, SIZE_256_BITS);
	...
	/* Calculate the hmac */
	{
		...
		(void)hmac_sha2(HASH_SHA256, regInfo->authKey->pBase, SIZE_256_BITS, NULL, 0,
			hmacData->pBase, hmacData->m_dataLength, hmac, SIZE_256_BITS);
		tlv_serialize(WPS_ID_AUTHENTICATOR, msg, hmac, SIZE_64_BITS);
	}
	/* Store the outgoing message */
	buffobj_Reset(regInfo->outMsg);
...
reg_proto_process_m5(RegData *regInfo, BufferObj *msg)
{
	...
	tlv_err |= tlv_dserialize(&m.registrarNonce, WPS_ID_REGISTRAR_NONCE, msg, SIZE_128_BITS, 0);
	if (tlv_err) {
		err = RPROT_ERR_INVALID_VALUE;
		goto error;
	}
	...
	/* extract encrypted settings */
	{
		...
		if (reg_msg_nonce_parse(&eNonce, WPS_ID_E_SNONCE1, plainText, regInfo->authKey)) {
			err = RPROT_ERR_REQD_TLV_MISSING;
			goto error;
		}
...

	{
		...
		/* 3. generate the mac */
		(void)hmac_sha2(HASH_SHA256, regInfo->authKey->pBase, SIZE_256_BITS, NULL, 0,
			ehashBuf->pBase, ehashBuf->m_dataLength, hashBuf, SIZE_256_BITS);
...
reg_proto_process_m7(RegData *regInfo, BufferObj *msg, void **encrSettings)
{
	...
	if (WPS_ID_X509_CERT_REQ == buffobj_NextType(msg)) {
		tlv_err |= tlv_dserialize(&m.x509CertReq, WPS_ID_X509_CERT_REQ, msg, 0, 0);
		if (tlv_err) {
			err = RPROT_ERR_INVALID_VALUE;
			goto error;
		}
