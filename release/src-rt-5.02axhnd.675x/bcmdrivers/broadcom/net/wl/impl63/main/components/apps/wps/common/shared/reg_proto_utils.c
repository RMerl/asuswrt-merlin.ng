 ...
 *
 * $Id: reg_proto_utils.c 740784 2018-01-13 10:16:22Z $
 */
...
reg_proto_generate_sha256hash(BufferObj *inBuf, BufferObj *outBuf)
{
	...
	if (sha2(HASH_SHA256, NULL, 0, inBuf->pBase, inBuf->m_dataLength, Hash,
...
reg_proto_derivekey(BufferObj *KDK, BufferObj *prsnlString, uint32 keyBits, BufferObj *key)
{
	...
	for (i = 0; i < iterations; i++) {
		...
		*(uint32 *)inPtr = WpsHtonl(i+1) ; /* i should start at 1 */
		(void)hmac_sha2(HASH_SHA256, KDK->pBase, SIZE_256_BITS, NULL, 0, input->pBase,
			input->m_dataLength, hmac, SIZE_256_BITS);
		hmacLen = SIZE_256_BITS;
...
reg_proto_validate_mac(BufferObj *data, uint8 *hmac, BufferObj *key)
{
	...
	(void)hmac_sha2(HASH_SHA256, key->pBase, SIZE_256_BITS, NULL, 0, data->pBase,
		data->m_dataLength, dataMac, SIZE_256_BITS);
	...
	BufferObj *cipherText, BufferObj *iv)
{
	...
	/* Extra AES_BLOCK_SZ is for PAD_LEN_PADDING case */
	outBuf = (uint8 *)malloc(plainText->m_dataLength + AES_BLOCK_SZ);
	if (!outBuf) {
		TUTRACE((TUTRACE_ERR, "outBuf: memory allocation error, %d bytes\n",
			plainText->m_dataLength + AES_BLOCK_SZ));
		buffobj_del(buf);
		return;
	}
	...
	BufferObj *authKey, BufferObj *plainText)
{
	...
	outBuf = (uint8 *)malloc(cipherText->m_dataLength);
	if (!outBuf) {
		TUTRACE((TUTRACE_ERR, "outBuf: memory allocation error, %d bytes\n",
			cipherText->m_dataLength));
		return;
	}
...
reg_proto_generate_priv_key(unsigned char *priv_key, unsigned char *pub_key_hash)
{
	...
	if (sha2(HASH_SHA256, NULL, 0, pub_key, SIZE_PUB_KEY, hash, SHA2_SHA256_DIGEST_LEN) != 0) {
		TUTRACE((TUTRACE_ERR, "RPROTO: SHA256 calculation failed\n"));
		goto error;
	}
