...
tlv_credentialParseAKey(CTlvCredential *tlv,  BufferObj *theBuf, bool allocate)
{
	...
	tlv_err |= tlv_dserialize(&tlv->nwKey, WPS_ID_NW_KEY, theBuf, SIZE_64_BYTES, allocate);
	tlv_err |= tlv_dserialize(&tlv->macAddr, WPS_ID_MAC_ADDR, theBuf, SIZE_MAC_ADDR, allocate);

	/* Parse optional attributes */
	if (WPS_ID_EAP_TYPE == buffobj_NextType(theBuf))
		tlv_err |= tlv_dserialize(&tlv->eapType, WPS_ID_EAP_TYPE, theBuf,
				SIZE_8_BYTES, allocate);

	if (WPS_ID_EAP_IDENTITY == buffobj_NextType(theBuf))
		tlv_err |= tlv_dserialize(&tlv->eapIdentity, WPS_ID_EAP_IDENTITY,
				theBuf, 0, allocate);

	if (WPS_ID_KEY_PROVIDED_AUTO == buffobj_NextType(theBuf))
		tlv_err |= tlv_dserialize(&tlv->keyProvidedAuto, WPS_ID_KEY_PROVIDED_AUTO,
				theBuf, 0, 0);

	if (WPS_ID_8021X_ENABLED == buffobj_NextType(theBuf))
		tlv_err |= tlv_dserialize(&tlv->oneXEnabled, WPS_ID_8021X_ENABLED, theBuf, 0, 0);
	...
	while (buffobj_NextType(theBuf) != 0) { /* more attributes */
		...
		switch (nextType) {
		case WPS_ID_KEY_LIFETIME:
			tlv_err |= tlv_dserialize(&tlv->keyLifetime, WPS_ID_KEY_LIFETIME,
					theBuf, 0, 0);
			if (tlv_err)
				goto tlv_error;
			break;
		case WPS_ID_REKEY_KEY:
			tlv_err |= tlv_dserialize(&tlv->rekeyKey, WPS_ID_REKEY_KEY, theBuf,
					0, allocate);
			if (tlv_err)
				goto tlv_error;
			break;
		case WPS_ID_X509_CERT:
			tlv_err |= tlv_dserialize(&tlv->x509Cert, WPS_ID_X509_CERT, theBuf,
					0, allocate);
			if (tlv_err)
				goto tlv_error;
			break;
		case WPS_ID_WEP_TRANSMIT_KEY:
			tlv_err |= tlv_dserialize(&tlv->WEPKeyIndex, WPS_ID_WEP_TRANSMIT_KEY,
					theBuf, 0, allocate);
			if (tlv_err)
				goto tlv_error;
			break;
		case WPS_ID_NW_KEY_INDEX:
			return 0; /* we've reached the end of this network key */
		...
		default: /* skip over other unknown attributes */
			...
			if (buffobj_Pos(theBuf) - tlv->base.m_pos >= tlv->base.m_len) {
				 ...
				 */
				return 0; /* we've reached the end of the Credential */
			}
			break;
		}
	}
...
tlv_credentialParse(CTlvCredential *tlv, BufferObj *theBuf, bool allocate)
{
	...
	tlv_err |= tlv_dserialize(&tlv->nwIndex, WPS_ID_NW_INDEX, theBuf, 0, 0);
	tlv_err |= tlv_dserialize(&tlv->ssid, WPS_ID_SSID, theBuf, SIZE_32_BYTES, allocate);
	tlv_err |= tlv_dserialize(&tlv->authType, WPS_ID_AUTH_TYPE, theBuf, 0, 0);
	tlv_err |= tlv_dserialize(&tlv->encrType, WPS_ID_ENCR_TYPE, theBuf, 0, 0);
	...
	*/
	...
	while (WPS_ID_NW_KEY_INDEX == buffobj_NextType(theBuf)) {
		if (tlv_dserialize(&tlv->nwKeyIndex, WPS_ID_NW_KEY_INDEX, theBuf, 0, 0)) {
			TUTRACE((TUTRACE_ERR, "De-serialize error!!\n"));
			return -1;
		}
		if (tlv_credentialParseAKey(tlv, theBuf, allocate))
			return -1;
