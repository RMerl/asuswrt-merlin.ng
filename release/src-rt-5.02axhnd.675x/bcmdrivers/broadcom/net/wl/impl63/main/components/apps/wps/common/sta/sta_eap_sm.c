 ...
 *
 * $Id: sta_eap_sm.c 735710 2017-12-12 06:04:13Z $
 */
...
wps_eap_process_frag(char *eap_msg)
{
	...
	if (staEapState->total_received) {
		wps_data_received = staEapState->total_received - sizeof(WpsEapHdr);
	}
...
wps_process_ap_msg(char *eapol_msg, int eapol_len)
{
	...
	if (IS_EAP_TYPE_WPS(wpsEapHdr) && REASSEMBLING(staEapState, wpsEapHdr)) {
		if (staEapState->total_bytes_to_recv == 0) {
			...
			staEapState->total_bytes_to_recv = total_wps_data_len;
