 ...
 *
 * $Id: aes.h 732060 2017-11-15 15:26:12Z $
 */
...
/* AES_CCM_AUTH_LEN can be 8 or 16 (for 802.11/CCMP) - CCM mode SP800-38C allows others */

#define AES_CCM_AUTH_FLAGS(_mac_len) \
	(4*((_mac_len)-2) + (AES_CCM_LEN_FIELD_LEN-1))
...
#define AES_CCMP_FC_HTC_MASK		~(FC_ORDER)
...
struct dot11_header;
void aes_ccmp_cal_params(const struct dot11_header *h,
	bool legacy, uint8 nonce_1st_byte,
...
int aes_ccm_mac(unsigned int *rk, const size_t key_len, const uint8 *nonce,
                          ...
                          const uint8 *ptxt, uint8 *mac, const size_t mac_len);
int aes_ccm_encrypt(unsigned int *rk, const size_t key_len, const uint8 *nonce,
                              const size_t aad_len, const uint8 *aad, const size_t data_len,
                              const uint8 *ptxt, uint8 *ctxt, uint8 *mac, const size_t mac_len);
int aes_ccm_decrypt(unsigned int *rk, const size_t key_len, const uint8 *nonce,
                              const size_t aad_len, const uint8 *aad, const size_t data_len,
                              const uint8 *ctxt, uint8 *ptxt, const size_t mac_len);
...
void aes_cmac(const int nrounds, const uint32* rK, const uint8 *K1,
	const uint8 *K2, bcm_const_xlvp_t *pfxs, int npfxs,
	const size_t data_len, const uint8 *data, uint8 *mac, const size_t mac_len);
void aes_cmac_calc(bcm_const_xlvp_t *pfxs, int npfxs,
	const uint8 *data, const size_t data_length,
