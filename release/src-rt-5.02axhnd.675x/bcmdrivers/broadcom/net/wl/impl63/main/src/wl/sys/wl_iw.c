 ...
 *
 * $Id: wl_iw.c 764377 2018-05-25 03:02:33Z $
 */
...
/* XXX: Is this limit really needed?
 * Any way, make it large enough to accomodate medium sized IOCTL
 */
...
)
{
	...
	for (i = 0; i < list->count && i < IW_MAX_AP; i++) {
		...
		/* XXX: JIRA:SW4349-173 80p80 Support Required
		   The code is not yet ready for 80p80 due to iw interface, but the code should
		   do something to present good info if iw is used over 80p80. Maybe it could
		   take the primary 80 of an 80+80 channel.
		*/
...
#endif /* defined (BCMSUP_PSK) || defined(BCMDONGLEHOST) */
...
#endif	/* defined(STA) || defined(BCMDONGLEHOST) */
	{
		return FALSE;
	}
}
	...
	uint16 type, uint16 len)
{
	...
	switch (type) {
		case WL_CNT_XTLV_SLICE_IDX: {
			WL_TRACE(("Slice_index: %d\n", *data));
			break;
		}
		case WL_CNT_XTLV_WLC_RINIT_RSN: {
			/* reinit reason counters */
			const reinit_rsns_t *cnt = (const reinit_rsns_t *)data;
			const uint32 *val = cnt->rsn;
			uint maxoffset = len/sizeof(uint32);

			if (len > sizeof(reinit_rsns_t)) {
				printf("counter structure length invalid! %d > %d\n",
					len, (int)sizeof(reinit_rsns_t));
				break;
			}

			if (cnt->rsn[0] != INVALID_CNT_VAL) {
				int i, pos = 0;
				char tmpbuf[WL_REINIT_RC_LAST * 10];
				for (i = 0; i < WL_REINIT_RC_LAST && i < maxoffset; i++) {
					pos += sprintf(tmpbuf + pos, "%d(%d) ", i, val[i]);
				}
				WL_TRACE(("reinitreason_counts: %s", tmpbuf));
			}
			break;
		}
		...
		case WL_CNT_XTLV_GE64_UCODEX_V1: {
			const wl_cnt_ge64mcxst_v1_t *cnt = (const wl_cnt_ge64mcxst_v1_t *)data;
			if (len != sizeof(wl_cnt_ge64mcxst_v1_t)) {
				printf("counter structure length mismatch! %d != %d\n",
					len, (int)sizeof(wl_cnt_ge64mcxst_v1_t));
				break;
			}
			WL_TRACE(("wl_iw_get_wireless_stats counters macxsusp %u "
				"m2vmsg %u v2mmsg %u mboxout %u musnd %u sfb2v %u\n",
				cnt->macxsusp, cnt->m2vmsg, cnt->v2mmsg,
				cnt->mboxout, cnt->musnd, cnt->sfb2v));
			BCM_REFERENCE(cnt);
			break;
		}
		default:
