 ...
 *
 * $Id: wl_cfgvendor.c 776807 2019-07-10 10:55:33Z $
 */
	...
	struct wireless_dev *wdev, const void  *data, int len)
{
	...
	if (wl_dev_set_rssi_monitor_cfg(bcmcfg_to_prmry_ndev(cfg),
	...
	struct wireless_dev *wdev, const void  *data, int len)
{
	...
	const wl_cnt_ge40mcst_v1_t *macstat_cnt;
	scb_val_t scbval;
	char *output = NULL;
	char *outdata = NULL;
	...
	if ((macstat_cnt = (const wl_cnt_ge40mcst_v1_t *)
			bcm_get_data_from_xtlv_buf(((wl_cnt_info_t *)iovar_buf)->data,
			((wl_cnt_info_t *)iovar_buf)->datalen,
			WL_CNT_XTLV_CNTV_LE10_UCODE, NULL,
			BCM_XTLV_OPTION_ALIGN32)) == NULL) {
		macstat_cnt = (const wl_cnt_ge40mcst_v1_t *)
				bcm_get_data_from_xtlv_buf(((wl_cnt_info_t *)iovar_buf)->data,
				((wl_cnt_info_t *)iovar_buf)->datalen,
