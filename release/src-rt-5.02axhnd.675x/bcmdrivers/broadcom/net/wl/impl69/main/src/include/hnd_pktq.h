 ...
 *
 * $Id: hnd_pktq.h 790156 2020-08-18 07:20:44Z $
 */
...
struct pqp_req; /**< PQP: Packet Queue Pager handler paired with a pktq_prec */

typedef struct pktq_prec {
	union {
		void *head;     /**< first packet to dequeue */
		struct pqp_req *pqp_req; /**< PQP: pqp request paired with pktq_prec */
	};
	union {
		void *tail;     /**< last packet to dequeue */
		struct {
			uint16 head;     /**< PQP: head packet page index in host bm */
			uint16 tail;     /**< PQP: tail packet page index in host bm */
		} pqp_hbm;           /**< PQP: host buffer memory page */
		uint32 pqp_hbm_page; /**< PQP: packet queue page indices in host bm */
	};
