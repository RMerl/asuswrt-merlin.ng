 ...
 * $Id: bcmwifi_radiotap.c 782713 2020-01-02 13:29:10Z $
 */
...
{
	switch (CHSPEC_BAND(rxsts->chanspec)) {
	case WL_CHANSPEC_BAND_2G:
		return wf_channel2mhz(wf_chspec_ctlchan(rxsts->chanspec), WF_CHAN_FACTOR_2_4_G);
	case WL_CHANSPEC_BAND_5G:
		return wf_channel2mhz(wf_chspec_ctlchan(rxsts->chanspec), WF_CHAN_FACTOR_5_G);
	case WL_CHANSPEC_BAND_6G:
		return wf_channel2mhz(wf_chspec_ctlchan(rxsts->chanspec), WF_CHAN_FACTOR_6_G);
	default:
		break;
	}
	/* is assert needed? */
...
wl_radiotap_rx_channel_flags(wl_rxsts_t *rxsts)
{
	...
	case WL_CHANSPEC_BAND_6G:
		/* no radiotap flag defined for 6G */
