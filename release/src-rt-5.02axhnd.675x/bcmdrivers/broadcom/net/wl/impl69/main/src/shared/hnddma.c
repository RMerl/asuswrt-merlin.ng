...
dma_txcommitted(hnddma_t *dmah)
{
	...
	ptr = B2I(R_REG(di->osh, &di->d64txregs->ptr), di->dd64_size);
...
dma_update_rxfill(hnddma_t *dmah)
{
	...
	ad = (uint16)B2I(((R_REG(di->osh, &di->d64rxregs->status1) & D64_RS1_AD_MASK) -
		di->rcvptrbase) & D64_RS1_AD_MASK, di->dd64_size);
	rxout = di->rxout;
...
dma_txrewind(hnddma_t *dmah)
{
	...
	act = (uint)(R_REG(di->osh, &di->d64txregs->status1) & D64_XS1_AD_MASK);
	act = (act - di->xmtptrbase) & D64_XS0_CD_MASK;
	start = (uint16)B2I(act, di->dd64_size);
	...
	for (i = start; i != end; i = NEXTTXD(i)) {
		...
		if (flags & CTRL_EOF) {
			...
			W_REG(di->osh,
			      &di->d64txregs->ptr,
			      (uint32)(di->xmtptrbase + I2B(NEXTTXD(i), di->dd64_size)));
...
BCMATTACHFN_DMA_ATTACH(dma64_alloc)(dma_info_t *di, uint direction)
{
	...
	size = (direction == DMA_TX) ? (di->ntxd * di->dd64_size) : (di->nrxd * di->dd64_size);
...
dma_txrotate(hnddma_t *dmah)
{
	...
	ad = B2I((((R_REG(di->osh, &di->d64txregs->status1) & D64_XS1_AD_MASK)
		- di->xmtptrbase) & D64_XS1_AD_MASK), di->dd64_size);
	rot = TXD(ad - di->txin);
...
static void *
dma_hme_ringalloc(dma_common_t *dmac, uint32 boundary, uint size, uint16 *alignbits,
	uint *alloced, dmaaddr_t *descpa)
{
	...
	desc_strtaddr = ROUNDUP(dmac->hmecurr, alignbytes);
	if (desc_strtaddr + size > PHYSADDR64LO(dmac->hmeaddr) + dmac->hmelen) {
		return NULL;
	}
	...
	/* Decide the address */
	dmac->hmecurr = desc_strtaddr + size;
	PHYSADDRLOSET(*descpa, desc_strtaddr);
	PHYSADDRHISET(*descpa, PHYSADDR64HI(dmac->hmeaddr));
	*alloced = size;
...
void
dma_common_hme_upd(dma_common_t *dmac, dma64addr_t hmeaddr, uint hmelen)
{
	dmac->hmeaddr = hmeaddr;
	dmac->hmelen = hmelen;
	dmac->hmecurr = PHYSADDR64LO(hmeaddr);
}
...
dma64_hme_alloc(hnddma_t *dmah, uint *flags)
{
	...
	*flags = 0;
	...
	if (DMA_HWA_HME_TX(di)) {
		...
		if ((va = dma_hme_ringalloc(di->dmacommon,
			(di->d64_xs0_cd_mask == 0x1fff) ? D64RINGBOUNDARY : D64RINGBOUNDARY_LARGE,
			size, &align_bits, &alloced, &paorig)) == NULL) {
