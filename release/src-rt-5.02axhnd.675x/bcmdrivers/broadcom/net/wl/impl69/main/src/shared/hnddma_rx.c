 ...
 *
 * $Id: hnddma_rx.c 793358 2020-11-24 11:25:24Z $
 */
...
void
dma_sts_set_phyrxsts_memory(hnddma_t *dmah, void *memory)
{
	dma_info_t *di = DI_INFO(dmah);

	ASSERT(di);
	ASSERT(di->phyrxsts_memory == NULL);
	ASSERT(memory != NULL);
	di->phyrxsts_memory = memory;
} /* dma_sts_phyrxsts_memory_set() */
...
dma_sts_rxinit(hnddma_t *dmah)
{
	...
	rxout = 0;
	phyrxsts_buf = (uint8 *)di->phyrxsts_memory;
	memory_size = di->rxbufsize * di->nrxd;
...
#else /* ! BCM_SECURE_DMA */
#if !defined(OSL_CACHE_COHERENT)
	pa = DMA_MAP(di->osh, phyrxsts_buf, memory_size, DMA_RX, NULL, NULL);
#else /* OSL_CACHE_COHERENT */
	pa = (dmaaddr_t)VIRT_TO_PHYS(phyrxsts_buf);
#endif  /* OSL_CACHE_COHERENT */
#endif /* ! BCM_SECURE_DMA */

	BCM_REFERENCE(memory_size);

	/* PhyRx Status buffers are always 8 byte aligned */
	ASSERT(ISALIGNED(PHYSADDRLO(pa), D11PHYRXSTS_GE129_ALIGN_BYTES));

	for (iter = 0; iter < (di->nrxd - 1); iter++) {
		...
		phyrxsts_buf = (uint8 *)phyrxsts_buf + di->rxbufsize;
		PHYSADDRLOSET(pa,  PHYSADDRLO(pa) + di->rxbufsize);
		rxout = NEXTRXD(rxout);
	}

	/* Set "end of descriptor table" and post buffers to last descriptor */
	flags |= D64_CTRL1_EOT;
	dma64_dd_upd(di, di->rxd64, pa, rxout, &flags, di->rxbufsize);
	...
	OR_REG(di->osh, &di->d64rxregs->control, D64_RC_WC);
 ...
 * - D64_RC_WCPD is set for Rx-FIFO-3 so need to reset and readback PhyRx Status len
 *   to protect against INT racing ahead of DMA transfers.
 */
bool BCMFASTPATH
dma_sts_rxfill(hnddma_t *dmah)
{
	...
	W_REG(di->osh, &di->d64rxregs->ptr, (uint32)(di->rcvptrbase + I2B(rxout, di->dd64_size)));
	di->rxout = rxout;
...
dma_sts_rx(hnddma_t *dmah)
{
	...
	di->rs0cd = (uint16)B2I(((R_REG(di->osh, &di->d64rxregs->status0) & D64_RS0_CD_MASK) -
		di->rcvptrbase) & D64_RS0_CD_MASK, di->dd64_size);
...
dma_sts_rxfill(hnddma_t *dmah)
{
...
#else
		ULONGTOPHYSADDR((unsigned long)VIRT_TO_PHYS(STSBUF_DATA(p)), pa);
#endif  /* ! OSL_CACHE_COHERENT */
#endif  /* ! BCM_SECURE_DMA */
	...
	W_REG(di->osh, &di->d64rxregs->ptr,
		(uint32)(di->rcvptrbase + I2B(rxout, di->dd64_size)));
...
#else
			ULONGTOPHYSADDR((unsigned long)VIRT_TO_PHYS(PKTDATA(di->osh, p)), pa);
#endif // endif
		}
...
dma_splitrxfill(dma_info_t *di)
{
	...
	/* update the chip lastdscr pointer */
	W_REG(di->osh, &di->d64rxregs->ptr,
		di->rcvptrbase + I2B(rxout1, di->dd64_size));

	W_REG(di->linked_di->osh, &di->linked_di->d64rxregs->ptr,
		di->linked_di->rcvptrbase + I2B(rxout2, di->dd64_size));
