...
#if defined(WIRELESS)
         /* When new push button is used */
        unsigned short btnIdx=0;
        if(btnGetSesBtnIdx(&btnIdx) == BP_SUCCESS) {
                registerPushButtonReleaseNotifyHook(btnIdx, sesBtn_releaseAction, SHORT_PUSH_BUTTON_PERIOD,NULL);
                registerPushButtonReleaseNotifyHook(btnIdx, sesBtn_releaseAction, LONG_PUSH_BUTTON_PERIOD,NULL);
        } 
#endif
    }
#if defined(WIRELESS)
...
#else
static void sesBtn_timer_handler(struct timer_list *arg)
#endif
{
...
#if defined(CONFIG_BCM94908) || defined(CONFIG_BCM96858) || defined(CONFIG_BCM963158) || defined(CONFIG_BCM96856)
...
#else
		timer_setup(&sesBtn_timer, sesBtn_timer_handler, 0);
#endif
