...
#define IPROC_L2CC_REG_BASE		(0x8001D000)
#else
...
{
	volatile l2cc_reg_struct *l2cc = (l2cc_reg_struct *)IPROC_L2CC_REG_BASE;

	/* Invalidate the entire L2 cache */
	l2cc->inv_way = 0x0000FFFF;
	while(l2cc->inv_way);
	while(l2cc->cache_sync);
	l2cc->cache_sync = 0;
	while(l2cc->cache_sync);
}

void
l2cache_clean_invalidate(void) 
{
	volatile l2cc_reg_struct *l2cc = (l2cc_reg_struct *)IPROC_L2CC_REG_BASE;

	if (l2cc->control & 0x01) {
		/* clean invalidate the entire L2 cache */
		l2cc->clean_inv_way = 0x0000FFFF;
		while(l2cc->clean_inv_way);
		while(l2cc->cache_sync);
		l2cc->cache_sync = 0;
		while(l2cc->cache_sync);
	}
...
l2cache_clean(void) 
{
	...
	if (l2cc->control & 0x01) {
		...
		l2cc->clean_way = 0x0000FFFF;
		while(l2cc->clean_way);
		while(l2cc->cache_sync);
		l2cc->cache_sync = 0;
		while(l2cc->cache_sync);
	}
...
void
l2cache_clean_range(unsigned long start, unsigned long end)
{
	volatile l2cc_reg_struct *l2cc = (l2cc_reg_struct *)IPROC_L2CC_REG_BASE;
...
void
l2cache_invalid_range(unsigned long start, unsigned long end)
{
	volatile l2cc_reg_struct *l2cc = (l2cc_reg_struct *)IPROC_L2CC_REG_BASE;
...
void
l2cache_clean_invalid_range(unsigned long start, unsigned long end)
{
	volatile l2cc_reg_struct *l2cc = (l2cc_reg_struct *)IPROC_L2CC_REG_BASE;
	...
	if (l2cc->control & 0x01) {
		start &= ~(CACHE_LINE_SIZE - 1);
		while (start < end) {
			l2cc->clean_inv_pa = start;
			start += CACHE_LINE_SIZE;
		}
		while(l2cc->clean_inv_pa & 0x1);
		l2cc->cache_sync = 0;
		while(l2cc->cache_sync);
	}
}
