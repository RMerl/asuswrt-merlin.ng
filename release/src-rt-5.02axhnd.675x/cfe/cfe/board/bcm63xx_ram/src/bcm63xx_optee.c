...
int optee_init(struct optee_boot_param_t *params)
{
#if defined(CONFIG_ARM64)
    ...
    if((params->ddk_len > OPTEE_KEY_SIZE) || (params->fek_len > OPTEE_KEY_SIZE) || (params->fek_enc_len > OPTEE_KEY_SIZE)){
        printf ("ERROR: Invalid key for OPTEE\n");
       return -1;
    }
