...
static int loaded_os_imageNum = 0;
...
int emmc_boot_os_image(int imageNum)
{
    ...
    /* If image number is not specified, we use the image that the current running cferam was loaded from */
    if( !imageNum )
        imageNum = (int) CFE_RAM_ROM_PARMS_GET_ROOTFS;

    /* Set flag so that all subsequent file loads are done from the specified image */
    loaded_os_imageNum = imageNum;
...
defined(_BCM94908_) || defined(_BCM96858_) || defined(_BCM96856_) || defined(_BCM947622_) 
...
int emmc_flash_image( PFILE_TAG pTag, uint8_t *imagePtr ) 
{
    ...
    if (cfeSize)
    {
        /* Erase entire CFEROM partition before flashing new one */
        printf("...Erasing %s\n", part_name);
        status = emmc_erase_partition(part_name);
               ...
               unsigned char** file, unsigned int* file_size )
{
    ...
    /* If we have already loaded an OS into memory, load all subsequent
     * files from the image that the OS binary was loaded from
     */ 
    if( loaded_os_imageNum )
        imageNum = loaded_os_imageNum;
    else
    {
        /* Load files from the image that the current running cferam was loaded from */
        imageNum = (int) CFE_RAM_ROM_PARMS_GET_ROOTFS;
    }
