...
#elif defined(_BCM94908_) || defined(_BCM963158_) || defined(_BCM96846_) || defined(_BCM96856_) || defined(_BCM963178_)  || defined(_BCM947622_) || defined(_BCM96878_)
...
#else
#if defined(_BCM963148_) || defined(_BCM963138_) || defined(_BCM94908_) || defined(_BCM96858_) || defined(_BCM963158_) \
...
/* if pureUBI check if we need to boot the other image */
    if ( (rfs_info[0].type == UBI_IMAGE) && (rfs_info[1].type == UBI_IMAGE) && 
        BCM_MBOX_INACTIVE_IMAGE_GET() ) {
        boot_prev = 1;
    }
        ...
        if (rfsi->type == JFFS2_IMAGE)
        {
            ...
            if (media_params->boot_secure) {
                ...
                if (res > SEC_S_MODULUS) { 
                    if (cfe_sec_validate_hashblock(pHashes, res, 
                           media_params)) {
                        goto err_crit; 
                    }
...
#endif
                pucDest = (uint8_t *)BTRM_INT_MEM_ENCR_COMP_CFE_RAM_ADDR;
#endif
            } else {
               /* Reading loading address from the image on flash six argument
                 from ther end of function eq 4 */
                parse_ubi(0, buf, rfsi->start_blk, rfsi->end_blk, len, 
                    VOLID_UBIFILES, media_params->boot_file_name, 
                    (char *)&pucEntry, 0, 4, read_blk, 0, 0, 0, 0);
                pucDest = pucEntry - 12;
            } 

            res = parse_ubi(0, buf, rfsi->start_blk, rfsi->end_blk, len, 
                  VOLID_UBIFILES, media_params->boot_file_name, (char *)pucDest, 0, 0, read_blk, 
                  0, 0, 0, 0);
            if (!res) {
                res = -1;
                goto err;
...
#ifdef BOARD_SEC_ARCH
        if (media_params->boot_secure) {
            cfe_sec_err_t rc = cfe_sec_load_sbi(pucDest, secCfeRamSize, 
			    media_params->boot_file_flags, (uint8_t*)media_params->boot_file_hash, 
                            &pucEntry);
            if (rc) {
               goto err_crit;
            }
