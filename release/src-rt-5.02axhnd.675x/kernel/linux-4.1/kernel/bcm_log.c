        ...
        switch (unitSize) {
          case UNIT_SIZE_BYTES:
          {
            if ( bSpiRead )
            {
               ...
               bcm_printk("%02X ", (unsigned char)temp);
            }
            else
          ...
          case UNIT_SIZE_HALFWORDS:
          {
            ...
            {
               spiFns.kerSysSlaveRead(spiDev, (uintptr_t)curPtr, &temp, unitSize);
               bcm_printk("%04X ", (unsigned short)temp);
            }
            else
            {
          ...
          case UNIT_SIZE_WORDS:
          {
            ...
            {
               spiFns.kerSysSlaveRead(spiDev, (uintptr_t)curPtr, &temp, unitSize);
               bcm_printk("%08X ", (unsigned int)temp);
            }
            else
            {
               ...
               cmdValid = 0;
                ...
                if (!ishex(arg[3])) {
                    ...
                    if ((length = str2val(arg[4])) > 32) {
                        ...
                        {
                            bcm_printk ("Spi device already reserved, clkspeed parameter ignored\n");
                        }
                        spiFns.syncTrans(txbuf, rxbuf, 0, length, busnum, chipsel);
                        bcm_printk ("Transmitted:\n");
