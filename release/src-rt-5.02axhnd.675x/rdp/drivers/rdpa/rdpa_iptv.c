...
static int iptv_attr_channel_req_delete(struct bdmf_object *mo, struct bdmf_attr *ad, bdmf_index index)
{
    ...
    if (ports == rdpa_if_id(key->port) &&  /* This is the last port */
       (!rdpa_if_is_wlan(key->port) || (rdpa_if_is_wlan(key->port) && wlan_mcast_get_num_of_ssids(key->port, wlan_mcast) == 0))) /* This is the last LAN port with/witout SSID */
#else
    if (ports == rdpa_if_id(req.mcast_result.egress_port))
#endif    
    {
    ...
    uint32_t size)
{
    ...
    /* zero result since rdd stores only crc of ipv6 addresses */
    if (channel->key.mcast_group.l3.gr_ip.family == bdmf_ip_family_ipv6)
    {
        channel->key.mcast_group.l3.gr_ip.addr.ipv4 = 0;
        channel->key.mcast_group.l3.src_ip.addr.ipv4 = 0;
    }
