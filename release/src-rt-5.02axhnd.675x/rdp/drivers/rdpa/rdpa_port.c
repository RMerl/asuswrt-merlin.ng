...
static int port_post_init(struct bdmf_object *mo)
{
    ...
    void *us_tm_ddr_queues_base_virt_addr;
    uint32_t hi, lo, i;
#endif
    ...
    if (rdpa_if_is_wan(port->index) && port->wan_type == rdpa_wan_gbe) {
        for (i = 0; i < 9; i++) {
            us_tm_ddr_queues_base_virt_addr = (void *)rdp_mm_aligned_alloc(8192 * 16, &us_tm_ddr_queues_base_phy_addr);

            bdmf_trace("VIRT ptr %d: %p\n", i, us_tm_ddr_queues_base_virt_addr);

            GET_ADDR_HIGH_LOW(hi, lo, us_tm_ddr_queues_base_phy_addr);

            bdmf_trace("egress_tm_post_init_ex() %d: Hi = 0x%08x  Lo = 0x%08x\n", i, hi, lo);

            RDD_DDR_QUEUES_BASE_PTR_LOW_WRITE_G(lo, RDD_US_TM_DDR_QUEUES_BASE_PTR_ADDRESS_ARR, i);
            RDD_DDR_QUEUES_BASE_PTR_HIGH_WRITE_G(hi, RDD_US_TM_DDR_QUEUES_BASE_PTR_ADDRESS_ARR, i);
        }
    }
