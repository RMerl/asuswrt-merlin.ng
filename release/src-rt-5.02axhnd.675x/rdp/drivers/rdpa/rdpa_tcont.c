...
#ifdef DQM_SW_WORKAROUND
static void *us_tm_ddr_queues_base_virt_addr[RDPA_MAX_TCONT];
#endif
...
static int tcont_post_init(struct bdmf_object *mo)
{
    tcont_drv_priv_t *priv = (tcont_drv_priv_t *)bdmf_obj_data(mo);
#ifdef DQM_SW_WORKAROUND
    bdmf_phys_addr_t us_tm_ddr_queues_base_phy_addr = 0;
    uint32_t hi, lo;
#endif
    int i, rc = 0;
...
#ifdef DQM_SW_WORKAROUND
    us_tm_ddr_queues_base_virt_addr[priv->channel] = (void *)rdp_mm_aligned_alloc(8192 * 16, &us_tm_ddr_queues_base_phy_addr);

    bdmf_trace("VIRT ptr %ld: %p\n", priv->channel, us_tm_ddr_queues_base_virt_addr[priv->channel]);

    GET_ADDR_HIGH_LOW(hi, lo, us_tm_ddr_queues_base_phy_addr);

    bdmf_trace("egress_tm_post_init_ex() %ld: Hi = 0x%08x  Lo = 0x%08x\n", priv->channel, hi, lo);

    RDD_DDR_QUEUES_BASE_PTR_LOW_WRITE_G(lo, RDD_US_TM_DDR_QUEUES_BASE_PTR_ADDRESS_ARR, priv->channel);
    RDD_DDR_QUEUES_BASE_PTR_HIGH_WRITE_G(hi, RDD_US_TM_DDR_QUEUES_BASE_PTR_ADDRESS_ARR, priv->channel);
#endif
...
#ifdef DQM_SW_WORKAROUND
    rdp_mm_aligned_free(us_tm_ddr_queues_base_virt_addr[priv->channel], 8192 * 16);

    RDD_DDR_QUEUES_BASE_PTR_LOW_WRITE_G(0, RDD_US_TM_DDR_QUEUES_BASE_PTR_ADDRESS_ARR, priv->channel);
    RDD_DDR_QUEUES_BASE_PTR_HIGH_WRITE_G(0, RDD_US_TM_DDR_QUEUES_BASE_PTR_ADDRESS_ARR, priv->channel);

    us_tm_ddr_queues_base_virt_addr[priv->channel] = NULL;
#endif
