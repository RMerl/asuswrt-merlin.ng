...
static int _cfg_us_xtmflow_hw(bdmf_boolean cfg_xtmflow, bdmf_index xtmflow,
    bdmf_object_handle xtmchannel, uint16_t fstat, bdmf_boolean calc_crc,
    int ptm_bonding);
...
static int xtmflow_post_init(struct bdmf_object *mo)
{
    ...
    {
        rc = _cfg_us_xtmflow_hw(1, priv->index, us_cfg->xtmchannel, priv->fstat, 0, priv->ptm_bonding);
        if (rc < 0)
...
static void xtmflow_destroy(struct bdmf_object *mo)
{
    ...
    if (priv->us_cfg.xtmchannel != NULL)
    {
        _cfg_us_xtmflow_hw(0, priv->index, us_cfg->xtmchannel, priv->fstat, 0, priv->ptm_bonding);
    }
}
/*
 * private enum tables
    ...
    int ptm_bonding)
{
    ...
    if (cfg_xtmflow)
    {
        ...
        rc = rdpa_us_wan_flow_config(xtmflow, (int)priv->channel,
            fstat, calc_crc, ptm_bonding, 0, 0, cfg_xtmflow);
    }
    else
    {
        rc = rdpa_us_wan_flow_config(xtmflow, 0, 0, 0, 0, 0, 0, cfg_xtmflow);
    }
    ...
    bdmf_index index, const void *val, uint32_t size)
{
    ...
    {
#ifndef BDMF_SYSTEM_SIM
       rc = rdpa_us_wan_flow_config(priv->index, 0, 0, 0, 0, 0, 0, 0);
       if (rc)
    ...
    else
    {
        ...
        if (mo->state == bdmf_state_active)
        {
            ...
            rc = _cfg_us_xtmflow_hw(1, priv->index, cfg->xtmchannel, priv->fstat, 0, priv->ptm_bonding);
            if (rc < 0)
                return rc;
        }
        priv->us_cfg = *cfg;
