...
static rdpa_tm_mbr_profile mbr_profile[QM_MBR_PROFILE__NUM_OF];
    ...
    bdmf_boolean is_wait)
{
    ...
    if (egress_tm_get_queue_dir(qentry->dir_channel) == rdpa_dir_us)
        return BDMF_ERR_OK;
#endif
    ...
    bdmf_boolean is_new, rdd_queue_profile_id_t *profile)
{
    ...
    /* first remove the old configuration if exists */
    if ((!is_new) && (old_queue_cfg->reserved_packet_buffers != 0))
    {
        rc = rdpa_rdd_tm_queue_mbr_profile_free(mo, dir, old_queue_cfg);
        if (rc)
            return rc;
    }
    ...
    for (i = 1; i < QM_MBR_PROFILE__NUM_OF; i++)
    {
        ...
        if (mbr_profile[i].token_threshold/QM_MBR_PROFILE_RESOLUTION == new_queue_cfg->reserved_packet_buffers/QM_MBR_PROFILE_RESOLUTION)
        {
            new_profile_index = i;
            break;
        }
...
/* Unreference queue minimum buffer reservation profile */
    ...
    const rdpa_tm_queue_cfg_t *queue_cfg)
{
    ...
    /* find exist profile. Profile 0 must remain zero (irrelevant for 6858/46, default for 6856) */
    for (i = 1; i < QM_MBR_PROFILE__NUM_OF; i++)
    {
        if (dir && (mbr_profile[i].attached_us_q_num != 0) &&
            (mbr_profile[i].token_threshold/QM_MBR_PROFILE_RESOLUTION == queue_cfg->reserved_packet_buffers/QM_MBR_PROFILE_RESOLUTION))
        {
            break;
        }
        if ((!dir) && (mbr_profile[i].attached_ds_q_num != 0) &&
            (mbr_profile[i].token_threshold/QM_MBR_PROFILE_RESOLUTION == queue_cfg->reserved_packet_buffers/QM_MBR_PROFILE_RESOLUTION))
        {
            break;
        }
    }
    if (i == QM_MBR_PROFILE__NUM_OF)
    {
        BDMF_TRACE_DBG_OBJ(mo, "Can't find existing minimum buffer reservation profile with"
            "reserved_packet_buffers=%d\n", queue_cfg->reserved_packet_buffers);
        return BDMF_ERR_INTERNAL;
    }

    /* update number of queues */
    if (dir)
        mbr_profile[i].attached_us_q_num--;
    else
        mbr_profile[i].attached_ds_q_num--;

    /* no queues using this profile - remove it */
    if ((mbr_profile[i].attached_us_q_num + mbr_profile[i].attached_ds_q_num) == 0)
    {
        mbr_profile[i].token_threshold = 0;
        rc = drv_qm_fpm_buffer_reservation_profile_cfg(i, 0);
    }

    /* update UG thresholds accordingly */
    rc = rc ? rc : _rdpa_mbr_ug_thresholds_cfg(dir);

    BDMF_TRACE_DBG_OBJ(mo, "removed queue from minimum buffer reservation profile dir %s index %d. "
        "remaining attached ds queues %d us queues %d\n", dir ? "us" : "ds", i,
        mbr_profile[i].attached_ds_q_num, mbr_profile[i].attached_us_q_num);
