    ...
    rdd_ic_context_t *context)
{
    ...
    if (!(context->cntr_disable))
    {
        rc = rdpa_cntr_id_alloc(CNTR_GROUP_TCAM_DEF, &context->cntr_id);
        if (rc || context->cntr_id == TCAM_DEF_CNTR_GROUP_INVLID_CNTR)
        {
            BDMF_TRACE_INFO("ingress class counter generation failed, not enough counters, rule will be generated without counter\n");
            context->cntr_disable = 1;
            if (!rc)
                rdpa_cntr_id_dealloc(CNTR_GROUP_TCAM_DEF, DEF_FLOW_CNTR_SUB_GROUP_ID, context->cntr_id); /*release counter*/
    ...
    rdd_ic_context_t *context)
{
    ...
    if ((context->cntr_id == TCAM_DEF_CNTR_GROUP_INVLID_CNTR) && (!(context->cntr_disable)))
    {
        ...
        if (rc || context->cntr_id == TCAM_DEF_CNTR_GROUP_INVLID_CNTR)
        {
            BDMF_TRACE_INFO("ingress class counter generation failed, not enough counters, rule will be changed without counter\n");
            if (!rc)
                rdpa_cntr_id_dealloc(CNTR_GROUP_TCAM_DEF, DEF_FLOW_CNTR_SUB_GROUP_ID, context->cntr_id); /*release counter*/
    ...
    bdmf_boolean iptv, bdmf_boolean is_init, rdpa_ic_type ic_type, bdmf_boolean skip_tm)
{
    ...
    BDMF_TRACE_INFO("set context->rate_controller_id=%d, context->priority=%d context->cntr_disable=%d\n", (int)rc_id, (int)context->priority, (int)context->cntr_disable);
...
bdmf_error_t ingress_class_attr_flow_stat_read_ex(rdpa_traffic_dir dir, uint16_t index, uint32_t *cntr)
{
    ...
    *cntr = accumulative_tcam_cntrs[context.cntr_id];
    return rc;
}
