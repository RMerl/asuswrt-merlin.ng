...
static inline void _init_rdd_mirroring_cfg(port_drv_priv_t *port, rdd_mirroring_cfg_t *rdd_mirroring_cfg)
{
    ...
    {
        rdd_mirroring_cfg->src_tx_bbh_id = rdpa_emac_to_bbh_id_e[rdpa_if_to_port_emac_map[port->index]];
    }
    else
    {
...
static void _tx_mirror_set_copy_to_ddr(port_drv_priv_t *port, bdmf_boolean force_copy)
{
    ...
    for (qid = 0; qid < num_queues; qid++)
    {
        ...
        if (!rc)
        {
            ...
            if (force_copy)
                force_copy_ddr_on_queue(rdpa_queue, force_copy);
            else /* return previous mirror configuration */
            {
                force_copy_ddr_on_queue(rdpa_queue, mirroring_cfg.tx_mirror_copy_to_ddr_prev[port->index]);
...
int port_post_init_ex(struct bdmf_object *mo)
{
    ...
    if (rdpa_if_is_wan(port->index))
    {
        if (rdpa_wan_if_to_wan_type(port->index) == rdpa_wan_gbe)
        {
          ...
          RDD_BBH_QUEUE_DESCRIPTOR_INGRESS_COUNTER_READ_G(ingress_counter, RDD_DS_TM_BBH_QUEUE_TABLE_ADDRESS_ARR, port->cfg.emac);
#if defined(BCM6858)
          RDD_BBH_TX_EGRESS_COUNTER_ENTRY_COUNTER_READ_G(egress_counter, RDD_DS_TM_BBH_TX_EGRESS_COUNTER_TABLE_ADDRESS_ARR, port->cfg.emac);
#else
          RDD_BYTE_1_BITS_READ_G(egress_counter, RDD_DS_TM_BBH_TX_EGRESS_COUNTER_TABLE_ADDRESS_ARR, port->cfg.emac);
#endif
        }
        else
        {
          /* should be good enough for PON and AE, as only 1st value gets overwrtten */
          RDD_BYTE_1_BITS_READ_G(ingress_counter, RDD_BACKUP_BBH_INGRESS_COUNTERS_TABLE_ADDRESS_ARR, 0);
          RDD_BYTE_1_BITS_READ_G(egress_counter, RDD_BACKUP_BBH_EGRESS_COUNTERS_TABLE_ADDRESS_ARR, 0);
        }
        
#if defined(XRDP_BBH_PER_LAN_PORT)
        RDD_BBH_QUEUE_DESCRIPTOR_INGRESS_COUNTER_WRITE_G(ingress_counter, RDD_US_TM_BBH_QUEUE_TABLE_ADDRESS_ARR, 0);
        RDD_BYTE_1_BITS_WRITE_G(egress_counter, RDD_US_TM_WAN_0_BBH_TX_EGRESS_COUNTER_TABLE_ADDRESS_ARR, 0);
#else
        RDD_BBH_QUEUE_DESCRIPTOR_INGRESS_COUNTER_WRITE_G(ingress_counter, RDD_US_TM_BBH_QUEUE_TABLE_ADDRESS_ARR, port->cfg.emac);
        RDD_BYTE_1_BITS_WRITE_G(egress_counter, RDD_US_TM_WAN_0_BBH_TX_EGRESS_COUNTER_TABLE_ADDRESS_ARR, port->cfg.emac);
#endif
    }
#endif
...
void port_destroy_ex(struct bdmf_object *mo)
{
    ...
    rdd_ag_processing_vport_cfg_ex_table_set(rdd_vport, 0, 0, INVALID_PROFILE_IDX, 0, 0, 0, 0, 0, 0, 0, 0);
...
#if defined(XRDP_BBH_PER_LAN_PORT)
      RDD_BBH_QUEUE_DESCRIPTOR_INGRESS_COUNTER_READ_G(ingress_counter, RDD_US_TM_BBH_QUEUE_TABLE_ADDRESS_ARR, 0);
      RDD_BYTE_1_BITS_READ_G(egress_counter, RDD_US_TM_WAN_0_BBH_TX_EGRESS_COUNTER_TABLE_ADDRESS_ARR, 0);
#else
      RDD_BBH_QUEUE_DESCRIPTOR_INGRESS_COUNTER_READ_G(ingress_counter, RDD_US_TM_BBH_QUEUE_TABLE_ADDRESS_ARR, port->cfg.emac);
      RDD_BYTE_1_BITS_READ_G(egress_counter, RDD_US_TM_WAN_0_BBH_TX_EGRESS_COUNTER_TABLE_ADDRESS_ARR, port->cfg.emac);
#endif
