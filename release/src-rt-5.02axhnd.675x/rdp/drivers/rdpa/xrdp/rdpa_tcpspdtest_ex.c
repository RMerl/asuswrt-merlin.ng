...
#if defined(BCM6858)
    /* 4 comes for LAN3 */
    RDD_TCPSPDTEST_STREAM_TCPSPDTEST_ENGINE_CONN_INFO_WAN_FLOW_WRITE_G(0x84, RDD_TCPSPDTEST_STREAM_TABLE_ADDRESS_ARR, 2);
    /* 0xa8 comes for a QM queue that configured for LAN3 egress_tm object (egress_tm/dir=ds,index=2). */
    RDD_TCPSPDTEST_STREAM_TCPSPDTEST_ENGINE_CONN_INFO_TX_QM_QUEUE_WRITE_G(0xa8, RDD_TCPSPDTEST_STREAM_TABLE_ADDRESS_ARR, 2);
#endif
#if defined(BCM6846)
    /* 4 comes for LAN3 */
    RDD_TCPSPDTEST_STREAM_TCPSPDTEST_ENGINE_CONN_INFO_WAN_FLOW_WRITE_G(0x84, RDD_TCPSPDTEST_STREAM_TABLE_ADDRESS_ARR, 2);
    /* 0x36 comes for a QM queue that configured for LAN3 egress_tm object (egress_tm/dir=ds,index=2). */
    RDD_TCPSPDTEST_STREAM_TCPSPDTEST_ENGINE_CONN_INFO_TX_QM_QUEUE_WRITE_G(0x36, RDD_TCPSPDTEST_STREAM_TABLE_ADDRESS_ARR, 2);
#endif
...
#else
    spdt_tx_fpm_ug_budget_set(0); /* We transmit to LAN, UG = DS */
#endif
    ...
    rdpa_tcpspdtest_engine_conn_info_t *conn_info)
{
    ...
    cpu_drv_priv_t *cpu_data = (cpu_drv_priv_t *)rdpa_cpu_data_get(rdpa_cpu_host);
#endif
    uint32_t fpm_ug;
    ...
    if (rdpa_if_is_wan(conn_info->port))
    {
        /* Upstream */
        fpm_ug = 1;
        rdd_conn_info.wan_flow = conn_info->wan_flow;
    }
    else
    {
        /* Downstream */
        fpm_ug = 0;
        rdd_conn_info.wan_flow = 0;
    }
#ifdef CONFIG_BCM_SPDT_FC_SUPPORT
    rdd_conn_info.wan_flow = BB_ID_CPU0; 
#endif
    spdt_tx_fpm_ug_budget_set(fpm_ug);

    rdd_conn_info.l2_hdr_len = conn_info->l2_hdr_len;
    rdd_conn_info.l3_hdr_len = conn_info->l3_hdr_len;
    rdd_conn_info.l3_protocol = conn_info->l3_protocol;
    tx_hdr_len = conn_info->l2_hdr_len + conn_info->l3_hdr_len + conn_info->l4_hdr_len;
    rdd_conn_info.tx_hdr_len = (RDPA_TCPSPDTEST_MIN_TX_PD_LEN <= tx_hdr_len ? tx_hdr_len :
        RDPA_TCPSPDTEST_MIN_TX_PD_LEN);
#ifdef CONFIG_BCM_SPDT_FC_SUPPORT
    rdd_conn_info.cpu_rx_rdd_queue = conn_info->cpu_rx_rdd_queue;
#else
    rdd_conn_info.cpu_rx_rdd_queue = cpu_data->rxq_to_rdd_rxq[conn_info->cpu_rx_rdd_queue];
#endif

#ifdef CONFIG_BCM_SPDT_FC_SUPPORT
    rdd_conn_info.tx_qm_queue = 0; /* Send to dispatcher */
#else
    rdd_conn_info.tx_qm_queue = conn_info->tx_qm_queue;
#endif
    ...
    return rdd_tcpspdtest_engine_conn_info_set(&rdd_conn_info, (uint8_t)index);
}
