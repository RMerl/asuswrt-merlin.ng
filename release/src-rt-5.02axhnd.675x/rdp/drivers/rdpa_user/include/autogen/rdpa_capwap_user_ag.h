...
// :>
/*
 * capwap object user header file.
 * This header file is generated automatically. Do not edit!
 */
#ifndef _RDPA_AG_CAPWAP_USR_H_
#define _RDPA_AG_CAPWAP_USR_H_
 ...
 */
static inline bdmf_type_handle rdpa_capwap_drv(void)
{
	rdpa_ioctl_cmd_t pa = {0};
	int fd, ret = 0;

	pa.cmd = RDPA_CAPWAP_DRV;
	...
	ret = ioctl(fd, RDPA_CAPWAP_IOCTL, &pa);
	if (ret)
	{
 ...
 */
static inline int rdpa_capwap_get(bdmf_object_handle *capwap_obj)
{
	rdpa_ioctl_cmd_t pa = {0};
	int fd, ret = 0;

	pa.cmd = RDPA_CAPWAP_GET;
	fd = open(RDPA_USR_DEV_NAME, O_RDWR);
	if (fd < 0)
	{
		rdpa_usr_error("%s: %s\n", RDPA_USR_DEV_NAME, strerror(errno));
...
static inline int rdpa_capwap_clear_stats_set(bdmf_object_handle mo_, bdmf_boolean clear_stats_)
{
	...
	pa.mo = mo_;
	pa.parm = (uint64_t)(long)clear_stats_;
	pa.cmd = RDPA_CAPWAP_CLEAR_STATS_SET;
