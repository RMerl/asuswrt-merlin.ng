...
static int drv_bbh_tx_lan_queue_cfg_get(uint8_t bbh_id, pd_queue_cfg_t *lan_queue_cfg)
{
    ...
    {
        rc = ag_drv_bbh_tx_common_configurations_q2rnr_get(bbh_id, i, &lan_queue_cfg->queue_to_rnr[i].q0, &lan_queue_cfg->queue_to_rnr[i].q1);
...
static int drv_bbh_tx_cli_occupancy_get(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    ...
    /* read the pd_full_for_wakeup_array_sel */
    rc = rc ? rc : drv_bbh_tx_read_indirect_debug_register(bbh_id, PDFFWKPSEL, 0, (uint32_t*)&almostFull[0]);
    rc = rc ? rc : drv_bbh_tx_read_indirect_debug_register(bbh_id, PDFFWKPSEL, 1, (uint32_t*)&almostFull[4]);
    ...
    if(IS_WAN_TX_PORT(bbh_id))
    {
        ...
        if (config.mac_type == MAC_TYPE_EPON)
        {
            ...
            {
                RDD_BYTE_1_BITS_READ_G(egress_cnt[i], RDD_BBH_TX_EPON_EGRESS_COUNTER_TABLE_ADDRESS_ARR, i);
                RDD_BYTE_1_BITS_READ_G(ingress_cnt[i], RDD_BBH_TX_EPON_INGRESS_COUNTER_TABLE_ADDRESS_ARR, i);
            }
#endif
        }
        else
        {
            for(i = 0 ;i < (TX_QEUEU_PAIRS * 2); i++)
            {
                ...
                {
                    RDD_BYTE_1_BITS_READ_G(egress_cnt[i], RDD_US_TM_PON_BBH_TX_EGRESS_COUNTER_TABLE_ADDRESS_ARR, i - RDD_US_CHANNEL_OFFSET_TCONT);
                }
                else if ((i >= RDD_US_CHANNEL_OFFSET_DSL) && (i <= RDD_US_CHANNEL_OFFSET_DSL_END))
                {
                    RDD_BYTE_1_BITS_READ_G(egress_cnt[i], RDD_US_TM_DSL_BBH_TX_EGRESS_COUNTER_TABLE_ADDRESS_ARR, i - RDD_US_CHANNEL_OFFSET_DSL);
                }
                else if ((i >= RDD_US_CHANNEL_OFFSET_AE10) && (i <= RDD_US_CHANNEL_OFFSET_AE10_END))
                {
                    RDD_BYTE_1_BITS_READ_G(egress_cnt[i], RDD_US_TM_AE10_BBH_TX_EGRESS_COUNTER_TABLE_ADDRESS_ARR, i - RDD_US_CHANNEL_OFFSET_AE10);
                }
                else /* if ((i >= RDD_US_CHANNEL_OFFSET_AE2P5) && (i <= RDD_US_CHANNEL_OFFSET_AE2P5_END)) */
                {
                    RDD_BYTE_1_BITS_READ_G(egress_cnt[i], RDD_US_TM_AE2P5_BBH_TX_EGRESS_COUNTER_TABLE_ADDRESS_ARR, i - RDD_US_CHANNEL_OFFSET_AE2P5);
                }
#else
                RDD_BYTE_1_BITS_READ_G(egress_cnt[i], RDD_US_TM_BBH_TX_EGRESS_COUNTER_TABLE_ADDRESS_ARR, i);
#endif
                RDD_BBH_QUEUE_DESCRIPTOR_INGRESS_COUNTER_READ_G(ingress_cnt[i], RDD_US_TM_BBH_QUEUE_TABLE_ADDRESS_ARR,i);
            }
        }
        for(i = 0 ;i < (TX_QEUEU_PAIRS ); i++)
        {
            fifo_size[i *2]       = config.wan_queue_cfg.pd_fifo_size[i].size0;
            fifo_size[(i *2) + 1] = config.wan_queue_cfg.pd_fifo_size[i].size1;
        }
        for(i = 0 ;i < (TX_QEUEU_PAIRS * 2); i++)
        {
            bdmf_session_print(session, "BBH_TX queue(%03d): ingress=%03d, egress=%03d, occupancy=%03d, fifo size=%03d almost_full=%d\n\r", i, ingress_cnt[i], egress_cnt[i], ingress_cnt[i] - egress_cnt[i], fifo_size[i], (almostFull[i/8] & (1 << (i % 8))) != 0);
        }
        /*now calculate ghost */
    ...
    else
    {
       ...
       {
#ifdef BCM6858
           RDD_BBH_TX_EGRESS_COUNTER_ENTRY_COUNTER_READ_G(egress_cnt[i], RDD_DS_TM_BBH_TX_EGRESS_COUNTER_TABLE_ADDRESS_ARR, i);
#else
           RDD_BYTE_1_BITS_READ_G(egress_cnt[i], RDD_DS_TM_BBH_TX_EGRESS_COUNTER_TABLE_ADDRESS_ARR, i);
#endif
           RDD_BBH_QUEUE_DESCRIPTOR_INGRESS_COUNTER_READ_G(ingress_cnt[i], RDD_DS_TM_BBH_QUEUE_TABLE_ADDRESS_ARR,i);
       }
       for(i = 0 ;i < (TX_LAN_QUEUE_PAIRS); i++)
       {
           fifo_size[i *2]       = config.lan_queue_cfg.pd_fifo_size[i].size0;
           fifo_size[(i *2) + 1] = config.lan_queue_cfg.pd_fifo_size[i].size1;
       }
       for(i = 0 ;i < (number_of_lans); i++)
       {
           bdmf_session_print(session, "BBH_TX queue(%03d): ingress=%03d, egress=%03d, occupancy=%03d, fifo size=%03d almost_full=%d\n\r", i, ingress_cnt[i], egress_cnt[i], ingress_cnt[i] - egress_cnt[i], fifo_size[i], (almostFull[i/8] & (1 << (i % 8))) != 0);
...
int drv_bbh_tx_config_clock_autogate(bdmf_boolean auto_gate, uint8_t timer_val)
{
