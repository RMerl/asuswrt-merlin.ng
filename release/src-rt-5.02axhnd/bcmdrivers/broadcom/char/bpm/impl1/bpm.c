...
static int bpm_alloc_mult_buf_ex( uint32_t num, void ** buf_p , uint32_t prio)
{
    ...
    if ((buf_avail_cnt < num) || (((buf_avail_cnt  - num) < BPM_LOW_ALLOC_THRESH) && (prio != BPM_HIGH_PRIO_ALLOC))) 
    {
        bpm_pg->buf_fail_cnt++;
        ret = BPM_ERROR;
        goto exit_func;
...
static inline void _skb_zero( struct sk_buff *skb )
{
	unsigned long long *cb; /* 8B assigns */

    skb->prev     = NULL;
    skb->next     = NULL;
    skb->priority = 0;

    /* Clear some key fields of the SKB as they may have stale data
     * from a previous incarnation of the recycled BPM skbuff
     */
	cb = (unsigned long long *) (&skb->cb[0]);
	...
	*(cb + 6) = 0ULL; *(cb + 7) = 0ULL; /* + 16 Bytes = 64 Bytes */
	...
	cb = (unsigned long long *)(&skb->wl_pktc[0]);
	*(cb + 0) = 0ULL; *(cb + 1) = 0ULL;  /*   16 Bytes = 20 Bytes */
	skb->wl_pktc[4] = 0;                 /* +  4 Bytes = 20 Bytes */
#endif
...
static void bpm_attach_skb( void *skbp, void *buf, uint32_t datalen )
{
	...
	/* _skb_zero(skb); not reqd in WFD|LAN|XTM transmit to WLAN path */
...
static void * bpm_alloc_buf_skb_attach( uint32_t datalen )
{
	...
	_skb_zero(skb); /* clear pkttag, reqd in WLAN rx path */
...
static void bpm_dump_status(void)
{
    ...
    printk( "\n------------------------ BPM Status -----------------------\n" );
    printk( " tot_buf avail   cur  no_buf_err  cum_alloc   cum_free\n" );
    printk( "%7u %5u %5u %10u %10u %10u",
