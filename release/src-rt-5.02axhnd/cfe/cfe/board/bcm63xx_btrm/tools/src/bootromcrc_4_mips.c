...
#define BOOTROM_DEPTH    (0x10000/BOOTROM_WIDTH)
#define BOOTROM_CRC        (0xD42CF71BBA3FB265ull)
// #define BOOTROM_CRC        (0x947933A84D237BAAull)
	...
	for( wordnum=0; wordnum<BOOTROM_DEPTH; wordnum++ ) {
		if( wordnum < (BOOTROM_DEPTH - 1) ) {
			...
			// reverse bytes (endianess)
			newd64 = 0;
			for ( j=56; j>=0; j-=8 )
                	{
                   		val = (uint8_t)(d64 & 0xff);
                   		newd64 = newd64 | ((uint64_t)(val) << j);
                   		d64 = d64 >> 8;
                	}
			d64 = newd64;
		...
		// printf("d64[%3lld]=>0x%016llx, crc64=0x%016llx\n", wordnum, d64, crc64);
	}
...
void usage(char *name){
	...
	printf("Input file is padded to 64K. Rom is assumed 64-bit wide.     \n");
	printf("CRC is inserted into the \"last\" word of the 64K.         \n\n");
...
int main (int argc, char *argv[]){
	...
	if (obin) {
		...
		if (ofp) {
			...
			num=fwrite(data,4,64/4*1024,ofp);
	...
	if (ocode) {
		...
		if (ofp) 
		{
                    for( word=0; word<BOOTROM_DEPTH; word+=1 ) 
		    {
			  ...
			  // d64i=d64 = ((uint64_t)data[2*word + 1] << 32) | ((uint64_t)data[2*word] );
			  d64 = ((uint64_t)data[2*word + 1] << 32) | ((uint64_t)data[2*word] );
