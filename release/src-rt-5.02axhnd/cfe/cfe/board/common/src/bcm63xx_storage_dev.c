...
#if (INC_EMMC_FLASH_DRIVER==1)
extern int cferom_emmc_read_full_blocks( int handle_not_used, unsigned long long byte_offset, unsigned char * dest_buf, int length );
                           ...
                           int numbytes)
{
        ...
        unsigned int address =  sector*EMMC_DFLT_BLOCK_SIZE+offset+_obj.boot_offs;
        int res = cferom_emmc_read_full_blocks(0, address&(~(EMMC_DFLT_BLOCK_SIZE-1)), buffer, (address+numbytes) - (address&~(EMMC_DFLT_BLOCK_SIZE-1)));
...
static inline void _init_flash_device(flash_dev_t * info)
{
       ...
       //cferom_emmc_read_full_blocks(0, 0, &_obj.img_hdr, TAG_LEN);
       _obj.boot_offs = TAG_LEN;
