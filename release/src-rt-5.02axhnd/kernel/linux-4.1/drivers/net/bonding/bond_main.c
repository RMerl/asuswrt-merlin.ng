...
#define BCM_MAX_SUPPORTED_BOND  (4)
static struct bonding *g_bonding_groups[BCM_MAX_SUPPORTED_BOND];
#endif
...
#if defined(CONFIG_BCM_KF_KBONDING) && defined(CONFIG_BCM_KERNEL_BONDING)
    /* Clear and recompute the IFF_WANDEV flag for the bond device */
    bond_dev->priv_flags &= ~IFF_WANDEV;
#endif
	bond_for_each_slave(bond, slave, iter) {
...
#if defined(CONFIG_BCM_KF_KBONDING) && defined(CONFIG_BCM_KERNEL_BONDING)
        /* If any of the slave devices has the IFF_WANDEV flag set, inherit
           the flag for the bond device */
        if (slave->dev->priv_flags & IFF_WANDEV) 
        {
            bond_dev->priv_flags |= IFF_WANDEV;
        }
#endif
...
int bond_create(struct net *net, const char *name)
{
	...
	if (res < 0)
        ...
        g_bonding_groups[master_id] = bond;
		master_id++;
	}
#endif
	return res;
...
{
	BCM_BondingMasterInfo *info = ctxt;
    ...
    {
        int idx = 0;
        for (idx=0; idx < max_bonds; idx++) 
        {
            struct slave *slave;
            struct list_head *iter;

            bond = g_bonding_groups[idx];
            bond_for_each_slave(bond, slave, iter) 
            {
                if (slave->dev == dev) 
                {
                    info->mstr_dev = bond->dev;
                    return bond->master_id;
                }
            }
        }
...
static inline int bcm_bond_is_dev_in_bond_slave_path(struct net_device *dev, 
                                                     struct bonding *bond)
{
    struct slave *slave;
    struct list_head *iter;
    ...
    bond_for_each_slave(bond, slave, iter) 
    {
        ...
        do
        {
            ...
            tempdev = netdev_path_next_dev(tempdev);
        }while(tempdev);
    }
    return 0;
}
...
/* Callback function through bcm_log function pointer;
   Given a bond and slave/root device ; confirms if slave belong to same group
   If bond dev is NULL, then check if the slave belongs to any bonding group
   and return the bond dev */
int bcm_bond_is_dev_in_slave_path(void *ctxt)
{
    ...
    if (*bond_dev == NULL) /* check if slave_dev is part of any bond group */
    {
        ...
        for (idx = 0; idx < max_bonds; idx++)
        {
            ...
            if (bond) 
            {
                if (bcm_bond_is_dev_in_bond_slave_path(slave_dev, bond))
                {
                    *bond_dev = bond->dev;
                    return 1;
                }
    ...
    else if ( netif_is_bond_master(*bond_dev) )
    {
        bond = netdev_priv(*bond_dev);
...
static int __init bonding_init(void)
{
    ...
    if (max_bonds > BCM_MAX_SUPPORTED_BOND) {
        pr_err("Error: Invalid max_bonds %d n",max_bonds);
        res = -EINVAL;
        goto out;
    }
#endif /* defined(CONFIG_BCM_KF_KBONDING) && defined(CONFIG_BCM_KERNEL_BONDING) */
	...
	bcmFun_reg(BCM_FUN_ID_BOND_GET_MSTR_INFO, bcm_bond_get_mstr_id);
	bcmFun_reg(BCM_FUN_ID_BOND_CLR_SLAVE_STAT, bcm_bond_clr_slave_stat);
	bcmFun_reg(BCM_FUN_ID_BOND_IS_DEV_IN_SLAVE_PATH, bcm_bond_is_dev_in_slave_path);
...
static void __exit bonding_exit(void)
{
	...
	bcmFun_dereg(BCM_FUN_ID_BOND_GET_MSTR_INFO);
	bcmFun_dereg(BCM_FUN_ID_BOND_CLR_SLAVE_STAT);
	bcmFun_dereg(BCM_FUN_ID_BOND_IS_DEV_IN_SLAVE_PATH);
