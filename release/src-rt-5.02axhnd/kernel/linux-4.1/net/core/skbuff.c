...
static void skb_release_data(struct sk_buff *skb)
{
	...
	int i;
	bool chk_invalid, chk_invalid_skb, chk_invalid_shinfo;
	...
	chk_invalid_skb = !virt_addr_valid(skb);
	chk_invalid_shinfo = !virt_addr_valid(shinfo);

	if (chk_invalid_skb) {
		printk("skb invalid check fail\n");
		return;
	}

	if (chk_invalid_shinfo) {
		printk("shinfo invalid check fail\n");
		return;
	}

	for (i = 0; i < shinfo->nr_frags; i++) {
		chk_invalid = chk_invalid || !virt_addr_valid(&shinfo->frags[i]) ||
			!(skb_frag_page(&shinfo->frags[i]) >= virt_to_page(PAGE_OFFSET) &&
				skb_frag_page(&shinfo->frags[i]) <= virt_to_page(high_memory));
		if (chk_invalid) {
			printk("invalid check shinfo frag fail\n");
			return;
		}
	}
