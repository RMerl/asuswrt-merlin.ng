...
static bool brcm_i2s_wr_reg(struct device *dev, unsigned int reg)
{
	 switch (reg) 
	 {
	   ...
	   case I2S_IRQ_CTL:
	   case I2S_IRQ_EN:
	   case I2S_IRQ_IFF_THLD:
	   case I2S_IRQ_OFF_THLD:
	   case I2S_DESC_IFF_ADDR:
	   case I2S_DESC_IFF_LEN:
		   return true;
	   default:
		   return false;
	}
...
static bool brcm_i2s_rd_reg(struct device *dev, unsigned int reg)
{
   switch (reg) 
   {
     ...
     case I2S_IRQ_CTL:
	   ...
	   case I2S_DESC_IFF_ADDR:
	   case I2S_DESC_IFF_LEN:
	   case I2S_DESC_OFF_ADDR:
	   case I2S_DESC_OFF_LEN:
		   return true;
	   default:
		   return false;
	}
...
static int bcm63xx_i2s_set_fmt(struct snd_soc_dai *cpu_dai, unsigned int fmt)
{
   ...
   switch (fmt & SND_SOC_DAIFMT_INV_MASK) 
   {
     case SND_SOC_DAIFMT_NB_NF:
        ...
        regmap_update_bits(regmap_i2s, I2S_CFG, I2S_SCLK_POLARITY | I2S_LRCK_POLARITY, 0 );
		    break;
     default:
        return -EINVAL;
   }
				...
				struct snd_soc_dai *dai)
{
   switch (params_format(params)) 
     ...
     case SNDRV_PCM_FORMAT_S32_LE:
       regmap_update_bits(regmap_i2s, I2S_CFG, I2S_SCLKS_PER_1FS_DIV32_MASK, I2S_64BITS_PERFRAME );
			      ...
			      struct snd_soc_dai *dai)
{
   ...
   if (!cpu_dai->active)
   {
      regmap_write(regmap_i2s, I2S_CFG, 0);// clear cfg reg 
      ...
      regmap_update_bits(regmap_i2s, I2S_CFG, I2S_OUT_R | I2S_DATA_ALIGNMENT |I2S_DATA_ENABLE | I2S_CLOCK_ENABLE,
                                     I2S_OUT_R | I2S_DATA_ALIGNMENT |I2S_DATA_ENABLE | I2S_CLOCK_ENABLE ); 

      regmap_write(regmap_i2s, I2S_IRQ_CTL, 0);// clear irq reg 
      regmap_write(regmap_i2s, I2S_IRQ_EN,  0);// clear irq en

      regmap_write(regmap_i2s, I2S_IRQ_IFF_THLD, 0);// clear 
      regmap_write(regmap_i2s, I2S_IRQ_OFF_THLD, 0);// clear  
      
      /* Enable off interrupts - interrupt when output fifo level is over 0 */
      regmap_update_bits(regmap_i2s, I2S_IRQ_EN,  I2S_DESC_INTR_TYPE_SEL | I2S_DESC_OFF_INTR_EN,
                                 ...
                                 struct snd_soc_dai *dai)
{
   ...
   regmap_update_bits(regmap_i2s, I2S_CFG, I2S_ENABLE, 0 );      
   /* Clear and disable I2S interrupts ( by writing 0 ) */
   regmap_update_bits(regmap_i2s, I2S_IRQ_CTL, I2S_INTR_MASK, 0 ); 
   regmap_write(regmap_i2s, I2S_IRQ_EN, 0);// clear cfg reg 
   return ;
}

static int bcm63xx_i2s_prepare(struct snd_pcm_substream *substream,
                               struct snd_soc_dai *dai)
{
   return 0;
}
