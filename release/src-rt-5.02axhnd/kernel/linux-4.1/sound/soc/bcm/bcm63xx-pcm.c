...
static int bcm63xx_pcm_trigger(struct snd_pcm_substream *substream, int cmd)
{
   ...
   switch (cmd) 
   {
     case SNDRV_PCM_TRIGGER_START:
       regmap_update_bits(regmap_i2s, I2S_IRQ_EN,  I2S_DESC_OFF_INTR_EN,I2S_DESC_OFF_INTR_EN ); 
       regmap_update_bits(regmap_i2s, I2S_CFG,     I2S_ENABLE ,         I2S_ENABLE);
     break;
...
static int bcm63xx_pcm_prepare(struct snd_pcm_substream *substream)
{
   ...
   regmap_write(regmap_i2s, I2S_DESC_IFF_LEN,  pdma_desc->dma_len); // push to descriptor fifo
   regmap_write(regmap_i2s, I2S_DESC_IFF_ADDR,  pdma_desc->dma_addr); // push to descriptor fifo
...
static irqreturn_t i2s_dma_isr(int irq, void *dev_id)
{
   ...
   int ret = regmap_read(regmap_i2s, I2S_IRQ_CTL, &int_status);
   if( ret )
   {
   ...
   {	
   	  ...
   	  while( offlevel )
   	  {
         regmap_read(regmap_i2s, I2S_DESC_OFF_ADDR, &val_1);
         regmap_read(regmap_i2s, I2S_DESC_OFF_LEN,  &val_2);
      ...
      while( availdepth )
      {
         ...
         regmap_write(regmap_i2s, I2S_DESC_IFF_LEN,  prtd->periodsize_inbyte);//push to hw 
         regmap_write(regmap_i2s, I2S_DESC_IFF_ADDR,  pdma_desc->dma_addr);
   ...
   {
   	  regmap_update_bits(regmap_i2s, I2S_IRQ_CTL,I2S_DESC_OFF_OVERRUN_INTR , 0 );
   }
   else if( int_status & I2S_DESC_IFF_INTR ) 
   {
   	  regmap_update_bits(regmap_i2s, I2S_IRQ_CTL,I2S_DESC_IFF_INTR , 0 );
   }   
   else
   {
