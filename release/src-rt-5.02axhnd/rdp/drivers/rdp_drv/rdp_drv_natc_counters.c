...
int drv_natc_counters_get_counter_values(uint32_t sub_table_id, uint32_t counter_key, uint64_t *packet, uint64_t *bytes)
{
    ...
    drv_natc_key_idx_get(natc_counters, (uint8_t*)&key_entry, &hash_index, &entry_index);
    return drv_natc_entry_counters_get(natc_counters, entry_index, packet, bytes);
}
...
int drv_natc_counters_add_new_counter(uint32_t sub_table_id, uint32_t *entry_index, uint32_t counter_key)
{
    ...
    rc = drv_natc_key_result_entry_add(natc_counters, keyword, (uint8_t *)&context[0], entry_index);
...
#endif
    drv_natc_key_idx_get(natc_counters, (uint8_t*)&key_entry, &hash_index, &entry_index);
    rc = drv_natc_entry_delete(natc_counters, entry_index, 1, 1);
    return rc;
}
