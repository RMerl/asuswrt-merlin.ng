...
/* del_cmd_mode:  - DEL Command DDR-bin matching mode enableThis bit is used for NAT0, but not us */
/*               ed for NAT1, NAT2 and NAT3.0h: DEL command deletes the cache entry with matching */
/*                key1h: DEL command deletes the cache entry with matching key and matching DDR b */
/*               innumber specified in DEL_CMD_DDR_BIN field                                      */
/* cache_flush:  - Cache Flush enableThis bit is used as CACHE_FLUSH for NAT0.This bit is used as */
/*               both DEL_CMD_MODE and CACHE_FLUSH for NAT1, NAT2 and NAT3.For NAT1, NAT2 and NAT */
/*              3, this bit is used as CACHE_FLUSH for Lookup command.For NAT1, NAT2 and NAT3, th */
/*              is bit is used as DEL_CMD_MODE for Delete command.When set, LOOKUP command is use */
/*              d to flush counters from cache into DDR.This command does not use key to lookup t */
/*              he cache entry.  Instead it usescache index number located in 10-MSB bits of key  */
/*              specified in NAT_KEY_RESULT register.For 16 bytes key, the cache index will be lo */
/*              cated in{NAT_KEY_RESULT[15], NAT_KEY_RESULT[14][7:6]} (15th byte of NAT_KEY_RESUL */
/*              T register andbits 7:6 of 14th byte of NAT_KEY_RESULT register).For 32 bytes key, */
/*               the cache index will be located in{NAT_KEY_RESULT[31], NAT_KEY_RESULT[30][7:6]}  */
/*              (31th byte of NAT_KEY_RESULT register andbits 7:6 of 30th byte of NAT_KEY_RESULT  */
/*              register).0h: LOOKUP command is used as normal lookup command.1h: LOOKUP command  */
/*              is used as cache flush command.                                                   */
...
/*          only)Hash (debug command)Hashes are stored in different set of COMMAND_STATUS registe */
/*          r (i.e.Hashes for HASH command issued using NAT0 register are returnedat NAT1 KEY_RES */
/*          ULT registers; hashes for NAT1 HASH commandare returned at NAT0 KEY_RESULT; hashes fo */
/*          r NAT2 HASH command arereturned at NAT3 KEY_RESULT; hashes for NAT3 HASH command arer */
/*          eturned at NAT2 KEY_RESULT register).Internal Cache command (debug command)Do not use */
/*           this command                                                                         */
/**************************************************************************************************/
typedef struct
{
