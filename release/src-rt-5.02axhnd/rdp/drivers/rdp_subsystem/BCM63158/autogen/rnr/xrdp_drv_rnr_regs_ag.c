...
#ifdef VALIDATE_PARMS
    if((rnr_id >= BLOCK_ADDR_COUNT) ||
       (micro_sec_val >= _10BITS_MAX_VAL_))
    {
        bdmf_trace("ERROR driver %s:%u| err=%s (%d)\n", __FILE__, __LINE__, bdmf_strerror(BDMF_ERR_RANGE), BDMF_ERR_RANGE);
        return BDMF_ERR_RANGE;
    }
...
bdmf_error_t ag_drv_rnr_regs_cfg_metal_fix_reg_set(uint8_t rnr_id, uint32_t metal_fix)
{
    uint32_t reg_cfg_metal_fix_reg=0;
    ...
    reg_cfg_metal_fix_reg = RU_FIELD_SET(rnr_id, RNR_REGS, CFG_METAL_FIX_REG, METAL_FIX, reg_cfg_metal_fix_reg, metal_fix);

    RU_REG_WRITE(rnr_id, RNR_REGS, CFG_METAL_FIX_REG, reg_cfg_metal_fix_reg);

    return BDMF_ERR_OK;
}

bdmf_error_t ag_drv_rnr_regs_cfg_metal_fix_reg_get(uint8_t rnr_id, uint32_t *metal_fix)
{
    uint32_t reg_cfg_metal_fix_reg;
    ...
    RU_REG_READ(rnr_id, RNR_REGS, CFG_METAL_FIX_REG, reg_cfg_metal_fix_reg);

    *metal_fix = RU_FIELD_GET(rnr_id, RNR_REGS, CFG_METAL_FIX_REG, METAL_FIX, reg_cfg_metal_fix_reg);
...
{
    uint32_t reg_dbg_design_dbg_ctrl=0;

#ifdef VALIDATE_PARMS
    if(!dbg_design_dbg_ctrl)
    {
        bdmf_trace("ERROR driver %s:%u| err=%s (%d)\n", __FILE__, __LINE__, bdmf_strerror(BDMF_ERR_PARM), BDMF_ERR_PARM);
        return BDMF_ERR_PARM;
    }
    if((rnr_id >= BLOCK_ADDR_COUNT) ||
       (dbg_design_dbg_ctrl->dbg_sel >= _5BITS_MAX_VAL_) ||
       (dbg_design_dbg_ctrl->main_dis_per_sched >= _1BITS_MAX_VAL_) ||
       (dbg_design_dbg_ctrl->pico_dis_per_sched >= _1BITS_MAX_VAL_) ||
       (dbg_design_dbg_ctrl->main_fw_self_is_sync >= _1BITS_MAX_VAL_) ||
       (dbg_design_dbg_ctrl->pico_fw_self_is_sync >= _1BITS_MAX_VAL_))
    {
        bdmf_trace("ERROR driver %s:%u| err=%s (%d)\n", __FILE__, __LINE__, bdmf_strerror(BDMF_ERR_RANGE), BDMF_ERR_RANGE);
        return BDMF_ERR_RANGE;
    }
    ...
    reg_dbg_design_dbg_ctrl = RU_FIELD_SET(rnr_id, RNR_REGS, DBG_DESIGN_DBG_CTRL, DBG_SEL, reg_dbg_design_dbg_ctrl, dbg_design_dbg_ctrl->dbg_sel);
    reg_dbg_design_dbg_ctrl = RU_FIELD_SET(rnr_id, RNR_REGS, DBG_DESIGN_DBG_CTRL, MAIN_DIS_PER_SCHED, reg_dbg_design_dbg_ctrl, dbg_design_dbg_ctrl->main_dis_per_sched);
    reg_dbg_design_dbg_ctrl = RU_FIELD_SET(rnr_id, RNR_REGS, DBG_DESIGN_DBG_CTRL, PICO_DIS_PER_SCHED, reg_dbg_design_dbg_ctrl, dbg_design_dbg_ctrl->pico_dis_per_sched);
    reg_dbg_design_dbg_ctrl = RU_FIELD_SET(rnr_id, RNR_REGS, DBG_DESIGN_DBG_CTRL, MAIN_FW_SELF_IS_SYNC, reg_dbg_design_dbg_ctrl, dbg_design_dbg_ctrl->main_fw_self_is_sync);
    reg_dbg_design_dbg_ctrl = RU_FIELD_SET(rnr_id, RNR_REGS, DBG_DESIGN_DBG_CTRL, PICO_FW_SELF_IS_SYNC, reg_dbg_design_dbg_ctrl, dbg_design_dbg_ctrl->pico_fw_self_is_sync);

    RU_REG_WRITE(rnr_id, RNR_REGS, DBG_DESIGN_DBG_CTRL, reg_dbg_design_dbg_ctrl);
...
{
    uint32_t reg_dbg_design_dbg_ctrl;
    ...
    RU_REG_READ(rnr_id, RNR_REGS, DBG_DESIGN_DBG_CTRL, reg_dbg_design_dbg_ctrl);

    dbg_design_dbg_ctrl->dbg_sel = RU_FIELD_GET(rnr_id, RNR_REGS, DBG_DESIGN_DBG_CTRL, DBG_SEL, reg_dbg_design_dbg_ctrl);
    dbg_design_dbg_ctrl->main_dis_per_sched = RU_FIELD_GET(rnr_id, RNR_REGS, DBG_DESIGN_DBG_CTRL, MAIN_DIS_PER_SCHED, reg_dbg_design_dbg_ctrl);
    dbg_design_dbg_ctrl->pico_dis_per_sched = RU_FIELD_GET(rnr_id, RNR_REGS, DBG_DESIGN_DBG_CTRL, PICO_DIS_PER_SCHED, reg_dbg_design_dbg_ctrl);
    dbg_design_dbg_ctrl->main_fw_self_is_sync = RU_FIELD_GET(rnr_id, RNR_REGS, DBG_DESIGN_DBG_CTRL, MAIN_FW_SELF_IS_SYNC, reg_dbg_design_dbg_ctrl);
    dbg_design_dbg_ctrl->pico_fw_self_is_sync = RU_FIELD_GET(rnr_id, RNR_REGS, DBG_DESIGN_DBG_CTRL, PICO_FW_SELF_IS_SYNC, reg_dbg_design_dbg_ctrl);
...
{
    uint32_t reg_dbg_design_dbg_data;
    ...
    RU_REG_READ(rnr_id, RNR_REGS, DBG_DESIGN_DBG_DATA, reg_dbg_design_dbg_data);

    *dbg_data = RU_FIELD_GET(rnr_id, RNR_REGS, DBG_DESIGN_DBG_DATA, DBG_DATA, reg_dbg_design_dbg_data);
...
typedef enum
{
    ...
    bdmf_address_dbg_design_dbg_ctrl,
    bdmf_address_dbg_design_dbg_data,
}
bdmf_address;
...
static int bcm_rnr_regs_cli_set(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    ...
    case cli_rnr_regs_cfg_profiling_cfg_0:
        ...
        break;
    case cli_rnr_regs_cfg_metal_fix_reg:
        err = ag_drv_rnr_regs_cfg_metal_fix_reg_set(parm[1].value.unumber, parm[2].value.unumber);
        break;
    case cli_rnr_regs_dbg_design_dbg_ctrl:
    {
        rnr_regs_dbg_design_dbg_ctrl dbg_design_dbg_ctrl = { .dbg_sel=parm[2].value.unumber, .main_dis_per_sched=parm[3].value.unumber, .pico_dis_per_sched=parm[4].value.unumber, .main_fw_self_is_sync=parm[5].value.unumber, .pico_fw_self_is_sync=parm[6].value.unumber};
        err = ag_drv_rnr_regs_dbg_design_dbg_ctrl_set(parm[1].value.unumber, &dbg_design_dbg_ctrl);
        break;
    }
    default:
...
int bcm_rnr_regs_cli_get(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    ...
    {
        uint32_t metal_fix;
        err = ag_drv_rnr_regs_cfg_metal_fix_reg_get(parm[1].value.unumber, &metal_fix);
        bdmf_session_print(session, "metal_fix = %u (0x%x)\n", metal_fix, metal_fix);
        break;
    }
    case cli_rnr_regs_dbg_design_dbg_ctrl:
    {
        rnr_regs_dbg_design_dbg_ctrl dbg_design_dbg_ctrl;
        err = ag_drv_rnr_regs_dbg_design_dbg_ctrl_get(parm[1].value.unumber, &dbg_design_dbg_ctrl);
        bdmf_session_print(session, "dbg_sel = %u (0x%x)\n", dbg_design_dbg_ctrl.dbg_sel, dbg_design_dbg_ctrl.dbg_sel);
        bdmf_session_print(session, "main_dis_per_sched = %u (0x%x)\n", dbg_design_dbg_ctrl.main_dis_per_sched, dbg_design_dbg_ctrl.main_dis_per_sched);
        bdmf_session_print(session, "pico_dis_per_sched = %u (0x%x)\n", dbg_design_dbg_ctrl.pico_dis_per_sched, dbg_design_dbg_ctrl.pico_dis_per_sched);
        bdmf_session_print(session, "main_fw_self_is_sync = %u (0x%x)\n", dbg_design_dbg_ctrl.main_fw_self_is_sync, dbg_design_dbg_ctrl.main_fw_self_is_sync);
        bdmf_session_print(session, "pico_fw_self_is_sync = %u (0x%x)\n", dbg_design_dbg_ctrl.pico_fw_self_is_sync, dbg_design_dbg_ctrl.pico_fw_self_is_sync);
        break;
    }
    case cli_rnr_regs_dbg_design_dbg_data:
    {
        uint32_t dbg_data;
        err = ag_drv_rnr_regs_dbg_design_dbg_data_get(parm[1].value.unumber, &dbg_data);
        bdmf_session_print(session, "dbg_data = %u (0x%x)\n", dbg_data, dbg_data);
        break;
    }
    default:
        err = BDMF_ERR_NOT_SUPPORTED;
...
static int bcm_rnr_regs_cli_test(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    ...
    {
        uint16_t micro_sec_val=gtmv(m, 10);
        ...
        if(err || micro_sec_val!=gtmv(m, 10))
            return err ? err : BDMF_ERR_IO;
    }
    {
    ...
    {
        uint32_t metal_fix=gtmv(m, 32);
        if(!err) bdmf_session_print(session, "ag_drv_rnr_regs_cfg_metal_fix_reg_set(%u %u)\n", rnr_id, metal_fix);
        if(!err) ag_drv_rnr_regs_cfg_metal_fix_reg_set(rnr_id, metal_fix);
        if(!err) ag_drv_rnr_regs_cfg_metal_fix_reg_get( rnr_id, &metal_fix);
        if(!err) bdmf_session_print(session, "ag_drv_rnr_regs_cfg_metal_fix_reg_get(%u %u)\n", rnr_id, metal_fix);
        if(err || metal_fix!=gtmv(m, 32))
            return err ? err : BDMF_ERR_IO;
    }
    {
        rnr_regs_dbg_design_dbg_ctrl dbg_design_dbg_ctrl = {.dbg_sel=gtmv(m, 5), .main_dis_per_sched=gtmv(m, 1), .pico_dis_per_sched=gtmv(m, 1), .main_fw_self_is_sync=gtmv(m, 1), .pico_fw_self_is_sync=gtmv(m, 1)};
        if(!err) bdmf_session_print(session, "ag_drv_rnr_regs_dbg_design_dbg_ctrl_set(%u %u %u %u %u %u)\n", rnr_id, dbg_design_dbg_ctrl.dbg_sel, dbg_design_dbg_ctrl.main_dis_per_sched, dbg_design_dbg_ctrl.pico_dis_per_sched, dbg_design_dbg_ctrl.main_fw_self_is_sync, dbg_design_dbg_ctrl.pico_fw_self_is_sync);
        if(!err) ag_drv_rnr_regs_dbg_design_dbg_ctrl_set(rnr_id, &dbg_design_dbg_ctrl);
        if(!err) ag_drv_rnr_regs_dbg_design_dbg_ctrl_get( rnr_id, &dbg_design_dbg_ctrl);
        if(!err) bdmf_session_print(session, "ag_drv_rnr_regs_dbg_design_dbg_ctrl_get(%u %u %u %u %u %u)\n", rnr_id, dbg_design_dbg_ctrl.dbg_sel, dbg_design_dbg_ctrl.main_dis_per_sched, dbg_design_dbg_ctrl.pico_dis_per_sched, dbg_design_dbg_ctrl.main_fw_self_is_sync, dbg_design_dbg_ctrl.pico_fw_self_is_sync);
        if(err || dbg_design_dbg_ctrl.dbg_sel!=gtmv(m, 5) || dbg_design_dbg_ctrl.main_dis_per_sched!=gtmv(m, 1) || dbg_design_dbg_ctrl.pico_dis_per_sched!=gtmv(m, 1) || dbg_design_dbg_ctrl.main_fw_self_is_sync!=gtmv(m, 1) || dbg_design_dbg_ctrl.pico_fw_self_is_sync!=gtmv(m, 1))
            return err ? err : BDMF_ERR_IO;
    }
    {
        uint32_t dbg_data=gtmv(m, 32);
        if(!err) ag_drv_rnr_regs_dbg_design_dbg_data_get( rnr_id, &dbg_data);
        if(!err) bdmf_session_print(session, "ag_drv_rnr_regs_dbg_design_dbg_data_get(%u %u)\n", rnr_id, dbg_data);
    }
    return err;
}
...
static int bcm_rnr_regs_cli_address(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    ...
    case bdmf_address_dbg_design_dbg_ctrl : reg = &RU_REG(RNR_REGS, DBG_DESIGN_DBG_CTRL); blk = &RU_BLK(RNR_REGS); break;
    case bdmf_address_dbg_design_dbg_data : reg = &RU_REG(RNR_REGS, DBG_DESIGN_DBG_DATA); blk = &RU_BLK(RNR_REGS); break;
    default :
...
bdmfmon_handle_t ag_drv_rnr_regs_cli_init(bdmfmon_handle_t driver_dir)
{
...

    {
        ...
        static bdmfmon_cmd_parm_t set_cfg_metal_fix_reg[]={
            BDMFMON_MAKE_PARM_ENUM("rnr_id", "rnr_id", rnr_id_enum_table, 0),
            BDMFMON_MAKE_PARM("metal_fix", "metal_fix", BDMFMON_PARM_NUMBER, 0),
            BDMFMON_PARM_LIST_TERMINATOR
        };
        static bdmfmon_cmd_parm_t set_dbg_design_dbg_ctrl[]={
            BDMFMON_MAKE_PARM_ENUM("rnr_id", "rnr_id", rnr_id_enum_table, 0),
            BDMFMON_MAKE_PARM("dbg_sel", "dbg_sel", BDMFMON_PARM_NUMBER, 0),
            BDMFMON_MAKE_PARM("main_dis_per_sched", "main_dis_per_sched", BDMFMON_PARM_NUMBER, 0),
            BDMFMON_MAKE_PARM("pico_dis_per_sched", "pico_dis_per_sched", BDMFMON_PARM_NUMBER, 0),
            BDMFMON_MAKE_PARM("main_fw_self_is_sync", "main_fw_self_is_sync", BDMFMON_PARM_NUMBER, 0),
            BDMFMON_MAKE_PARM("pico_fw_self_is_sync", "pico_fw_self_is_sync", BDMFMON_PARM_NUMBER, 0),
            BDMFMON_PARM_LIST_TERMINATOR
        };
        static bdmfmon_enum_val_t selector_table[] = {
            ...
            { .name="cfg_metal_fix_reg", .val=cli_rnr_regs_cfg_metal_fix_reg, .parms=set_cfg_metal_fix_reg },
            { .name="dbg_design_dbg_ctrl", .val=cli_rnr_regs_dbg_design_dbg_ctrl, .parms=set_dbg_design_dbg_ctrl },
            BDMFMON_ENUM_LAST
        };
    ...
    }
    {
        ...
        static bdmfmon_enum_val_t selector_table[] = {
            ...
            { .name="cfg_metal_fix_reg", .val=cli_rnr_regs_cfg_metal_fix_reg, .parms=set_default },
            { .name="dbg_design_dbg_ctrl", .val=cli_rnr_regs_dbg_design_dbg_ctrl, .parms=set_default },
            { .name="dbg_design_dbg_data", .val=cli_rnr_regs_dbg_design_dbg_data, .parms=set_default },
            BDMFMON_ENUM_LAST
        };
    ...
    }
    {
        static bdmfmon_enum_val_t enum_table_address[] = {
            ...
            { .name="DBG_DESIGN_DBG_CTRL" , .val=bdmf_address_dbg_design_dbg_ctrl },
            { .name="DBG_DESIGN_DBG_DATA" , .val=bdmf_address_dbg_design_dbg_data },
            BDMFMON_ENUM_LAST
        };
