...
static int bcm_rnr_quad_cli_set(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    ...
    {
        rnr_quad_general_config_dma_arb_cfg general_config_dma_arb_cfg = { .use_fifo_for_ddr_only=parm[2].value.unumber, .token_arbiter_is_rr=parm[3].value.unumber, .chicken_no_flowctrl=parm[4].value.unumber, .ddr_congest_threshold=parm[5].value.unumber, .psram_congest_threshold=parm[6].value.unumber};
        err = ag_drv_rnr_quad_general_config_dma_arb_cfg_set(parm[1].value.unumber, &general_config_dma_arb_cfg);
        break;
    }
...
int bcm_rnr_quad_cli_get(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    ...
    case cli_rnr_quad_general_config_dma_arb_cfg:
    {
        ...
        err = ag_drv_rnr_quad_general_config_dma_arb_cfg_get(parm[1].value.unumber, &general_config_dma_arb_cfg);
...
static int bcm_rnr_quad_cli_test(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    ...
    {
        rnr_quad_general_config_dma_arb_cfg general_config_dma_arb_cfg = {.use_fifo_for_ddr_only=gtmv(m, 1), .token_arbiter_is_rr=gtmv(m, 1), .chicken_no_flowctrl=gtmv(m, 1), .ddr_congest_threshold=gtmv(m, 5), .psram_congest_threshold=gtmv(m, 5)};
        if(!err) bdmf_session_print(session, "ag_drv_rnr_quad_general_config_dma_arb_cfg_set(%u %u %u %u %u %u)\n", quad_idx, general_config_dma_arb_cfg.use_fifo_for_ddr_only, general_config_dma_arb_cfg.token_arbiter_is_rr, general_config_dma_arb_cfg.chicken_no_flowctrl, general_config_dma_arb_cfg.ddr_congest_threshold, general_config_dma_arb_cfg.psram_congest_threshold);
        if(!err) ag_drv_rnr_quad_general_config_dma_arb_cfg_set(quad_idx, &general_config_dma_arb_cfg);
        if(!err) ag_drv_rnr_quad_general_config_dma_arb_cfg_get( quad_idx, &general_config_dma_arb_cfg);
        if(!err) bdmf_session_print(session, "ag_drv_rnr_quad_general_config_dma_arb_cfg_get(%u %u %u %u %u %u)\n", quad_idx, general_config_dma_arb_cfg.use_fifo_for_ddr_only, general_config_dma_arb_cfg.token_arbiter_is_rr, general_config_dma_arb_cfg.chicken_no_flowctrl, general_config_dma_arb_cfg.ddr_congest_threshold, general_config_dma_arb_cfg.psram_congest_threshold);
        if(err || general_config_dma_arb_cfg.use_fifo_for_ddr_only!=gtmv(m, 1) || general_config_dma_arb_cfg.token_arbiter_is_rr!=gtmv(m, 1) || general_config_dma_arb_cfg.chicken_no_flowctrl!=gtmv(m, 1) || general_config_dma_arb_cfg.ddr_congest_threshold!=gtmv(m, 5) || general_config_dma_arb_cfg.psram_congest_threshold!=gtmv(m, 5))
            return err ? err : BDMF_ERR_IO;
    }
    {
...
bdmfmon_handle_t ag_drv_rnr_quad_cli_init(bdmfmon_handle_t driver_dir)
{
...

    {
        ...
        static bdmfmon_cmd_parm_t set_general_config_dma_arb_cfg[]={
            ...
            BDMFMON_MAKE_PARM("ddr_congest_threshold", "ddr_congest_threshold", BDMFMON_PARM_NUMBER, 0),
            BDMFMON_MAKE_PARM("psram_congest_threshold", "psram_congest_threshold", BDMFMON_PARM_NUMBER, 0),
            BDMFMON_PARM_LIST_TERMINATOR
        };
