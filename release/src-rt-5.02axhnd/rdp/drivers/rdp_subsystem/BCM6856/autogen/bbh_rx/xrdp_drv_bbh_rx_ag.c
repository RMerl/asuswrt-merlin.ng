...
bdmf_error_t ag_drv_bbh_rx_multicast_en_set(uint8_t bbh_id, bdmf_boolean multen)
{
    ...
    reg_general_configuration_exclqcfg = RU_FIELD_SET(bbh_id, BBH_RX, GENERAL_CONFIGURATION_EXCLQCFG, MULTEN, reg_general_configuration_exclqcfg, multen);
...
bdmf_error_t ag_drv_bbh_rx_multicast_en_get(uint8_t bbh_id, bdmf_boolean *multen)
{
    ...
    *multen = RU_FIELD_GET(bbh_id, BBH_RX, GENERAL_CONFIGURATION_EXCLQCFG, MULTEN, reg_general_configuration_exclqcfg);
...
bdmf_error_t ag_drv_bbh_rx_oam_en_set(uint8_t bbh_id, bdmf_boolean oamen)
{
    ...
    reg_general_configuration_exclqcfg = RU_FIELD_SET(bbh_id, BBH_RX, GENERAL_CONFIGURATION_EXCLQCFG, OAMEN, reg_general_configuration_exclqcfg, oamen);
...
bdmf_error_t ag_drv_bbh_rx_oam_en_get(uint8_t bbh_id, bdmf_boolean *oamen)
{
    ...
    *oamen = RU_FIELD_GET(bbh_id, BBH_RX, GENERAL_CONFIGURATION_EXCLQCFG, OAMEN, reg_general_configuration_exclqcfg);
...
bdmf_error_t ag_drv_bbh_rx_disable_normal_check_set(uint8_t bbh_id, bdmf_boolean disnormalcheck)
{
...
#ifdef VALIDATE_PARMS
    if((bbh_id >= BLOCK_ADDR_COUNT) ||
       (disnormalcheck >= _1BITS_MAX_VAL_))
    {
        bdmf_trace("ERROR driver %s:%u| err=%s (%d)\n", __FILE__, __LINE__, bdmf_strerror(BDMF_ERR_RANGE), BDMF_ERR_RANGE);
        return BDMF_ERR_RANGE;
    }
    ...
    reg_general_configuration_exclqcfg = RU_FIELD_SET(bbh_id, BBH_RX, GENERAL_CONFIGURATION_EXCLQCFG, DISNORMALCHECK, reg_general_configuration_exclqcfg, disnormalcheck);
...
bdmf_error_t ag_drv_bbh_rx_disable_normal_check_get(uint8_t bbh_id, bdmf_boolean *disnormalcheck)
{
    ...
    *disnormalcheck = RU_FIELD_GET(bbh_id, BBH_RX, GENERAL_CONFIGURATION_EXCLQCFG, DISNORMALCHECK, reg_general_configuration_exclqcfg);
...
static int bcm_bbh_rx_cli_set(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    ...
    case cli_bbh_rx_ctrl_en:
        ...
        break;
    case cli_bbh_rx_multicast_en:
        err = ag_drv_bbh_rx_multicast_en_set(parm[1].value.unumber, parm[2].value.unumber);
        break;
    case cli_bbh_rx_oam_en:
        err = ag_drv_bbh_rx_oam_en_set(parm[1].value.unumber, parm[2].value.unumber);
        break;
    ...
    case cli_bbh_rx_exc_en:
        ...
        break;
    case cli_bbh_rx_disable_normal_check:
        err = ag_drv_bbh_rx_disable_normal_check_set(parm[1].value.unumber, parm[2].value.unumber);
        break;
...
int bcm_bbh_rx_cli_get(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    ...
    {
        bdmf_boolean multen;
        err = ag_drv_bbh_rx_multicast_en_get(parm[1].value.unumber, &multen);
        bdmf_session_print(session, "multen = %u (0x%x)\n", multen, multen);
        break;
    }
    case cli_bbh_rx_oam_en:
    {
        bdmf_boolean oamen;
        err = ag_drv_bbh_rx_oam_en_get(parm[1].value.unumber, &oamen);
        bdmf_session_print(session, "oamen = %u (0x%x)\n", oamen, oamen);
        break;
    }
    case cli_bbh_rx_pattern_en:
    {
    ...
    case cli_bbh_rx_disable_normal_check:
    {
        bdmf_boolean disnormalcheck;
        err = ag_drv_bbh_rx_disable_normal_check_get(parm[1].value.unumber, &disnormalcheck);
        bdmf_session_print(session, "disnormalcheck = %u (0x%x)\n", disnormalcheck, disnormalcheck);
        break;
    }
    case cli_bbh_rx_pattern_recog:
    {
...
static int bcm_bbh_rx_cli_test(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    ...
    {
        bdmf_boolean multen=gtmv(m, 1);
        if(!err) bdmf_session_print(session, "ag_drv_bbh_rx_multicast_en_set(%u %u)\n", bbh_id, multen);
        if(!err) ag_drv_bbh_rx_multicast_en_set(bbh_id, multen);
        if(!err) ag_drv_bbh_rx_multicast_en_get( bbh_id, &multen);
        if(!err) bdmf_session_print(session, "ag_drv_bbh_rx_multicast_en_get(%u %u)\n", bbh_id, multen);
        if(err || multen!=gtmv(m, 1))
            return err ? err : BDMF_ERR_IO;
    }
    {
        bdmf_boolean oamen=gtmv(m, 1);
        if(!err) bdmf_session_print(session, "ag_drv_bbh_rx_oam_en_set(%u %u)\n", bbh_id, oamen);
        if(!err) ag_drv_bbh_rx_oam_en_set(bbh_id, oamen);
        if(!err) ag_drv_bbh_rx_oam_en_get( bbh_id, &oamen);
        if(!err) bdmf_session_print(session, "ag_drv_bbh_rx_oam_en_get(%u %u)\n", bbh_id, oamen);
        if(err || oamen!=gtmv(m, 1))
            return err ? err : BDMF_ERR_IO;
    }
    {
    ...
    {
        bdmf_boolean disnormalcheck=gtmv(m, 1);
        if(!err) bdmf_session_print(session, "ag_drv_bbh_rx_disable_normal_check_set(%u %u)\n", bbh_id, disnormalcheck);
        if(!err) ag_drv_bbh_rx_disable_normal_check_set(bbh_id, disnormalcheck);
        if(!err) ag_drv_bbh_rx_disable_normal_check_get( bbh_id, &disnormalcheck);
        if(!err) bdmf_session_print(session, "ag_drv_bbh_rx_disable_normal_check_get(%u %u)\n", bbh_id, disnormalcheck);
        if(err || disnormalcheck!=gtmv(m, 1))
            return err ? err : BDMF_ERR_IO;
    }
    {
...
bdmfmon_handle_t ag_drv_bbh_rx_cli_init(bdmfmon_handle_t driver_dir)
{
...

    {
        ...
        static bdmfmon_cmd_parm_t set_multicast_en[]={
            BDMFMON_MAKE_PARM_ENUM("bbh_id", "bbh_id", bbh_id_enum_table, 0),
            BDMFMON_MAKE_PARM("multen", "multen", BDMFMON_PARM_NUMBER, 0),
            BDMFMON_PARM_LIST_TERMINATOR
        };
        static bdmfmon_cmd_parm_t set_oam_en[]={
            BDMFMON_MAKE_PARM_ENUM("bbh_id", "bbh_id", bbh_id_enum_table, 0),
            BDMFMON_MAKE_PARM("oamen", "oamen", BDMFMON_PARM_NUMBER, 0),
            BDMFMON_PARM_LIST_TERMINATOR
        };
        ...
        static bdmfmon_cmd_parm_t set_disable_normal_check[]={
            BDMFMON_MAKE_PARM_ENUM("bbh_id", "bbh_id", bbh_id_enum_table, 0),
            BDMFMON_MAKE_PARM("disnormalcheck", "disnormalcheck", BDMFMON_PARM_NUMBER, 0),
            BDMFMON_PARM_LIST_TERMINATOR
        };
        ...
        static bdmfmon_enum_val_t selector_table[] = {
            ...
            { .name="multicast_en", .val=cli_bbh_rx_multicast_en, .parms=set_multicast_en },
            { .name="oam_en", .val=cli_bbh_rx_oam_en, .parms=set_oam_en },
            { .name="pattern_en", .val=cli_bbh_rx_pattern_en, .parms=set_pattern_en },
            { .name="exc_en", .val=cli_bbh_rx_exc_en, .parms=set_exc_en },
            { .name="disable_normal_check", .val=cli_bbh_rx_disable_normal_check, .parms=set_disable_normal_check },
    ...
    }
    {
        ...
        static bdmfmon_enum_val_t selector_table[] = {
            ...
            { .name="multicast_en", .val=cli_bbh_rx_multicast_en, .parms=set_default },
            { .name="oam_en", .val=cli_bbh_rx_oam_en, .parms=set_default },
            { .name="pattern_en", .val=cli_bbh_rx_pattern_en, .parms=set_default },
            { .name="exc_en", .val=cli_bbh_rx_exc_en, .parms=set_default },
            { .name="disable_normal_check", .val=cli_bbh_rx_disable_normal_check, .parms=set_default },
