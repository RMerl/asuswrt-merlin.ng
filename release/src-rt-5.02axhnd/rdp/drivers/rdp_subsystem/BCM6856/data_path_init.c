...
static int bbh_tx_init(void)
{
    ...
    if (p_dpi_cfg->bbh_id_gbe_wan != BBH_ID_NULL)
    {
        ...
        config.rnr_cfg[1].tcont_addr = IMAGE_3_US_TM_BBH_TX_WAKE_UP_DATA_TABLE_ADDRESS >> 3;
...
static int dispatcher_reorder_init(void)
{
    ...
    /* VIQ for CPU_TX egress - 9 */
    dispatcher_reorder_viq_init(&cfg, &ingress_congs_init, &egress_congs_init, get_runner_idx(cpu_tx_runner_image),
    	(IMAGE_2_CPU_TX_EGRESS_DISPATCHER_CREDIT_TABLE_ADDRESS >> 3 | IMAGE_2_IMAGE_2_CPU_TX_EGRESS_THREAD_NUMBER << 12),
    ...
    /* VIQ for CPU_TX forward - 10 */
    dispatcher_reorder_viq_init(&cfg, &ingress_congs_init, &egress_congs_init, get_runner_idx(cpu_tx_runner_image),
    	(IMAGE_2_CPU_TX_INGRESS_DISPATCHER_CREDIT_TABLE_ADDRESS >> 3 | IMAGE_2_IMAGE_2_CPU_TX_INGRESS_THREAD_NUMBER << 12),
    ...
    dispatcher_reorder_viq_init(&cfg, &ingress_congs_init, &egress_congs_init, get_runner_idx(dhd_tx_post_runner_image),
    	(IMAGE_3_DHD_MCAST_DISPATCHER_CREDIT_TABLE_ADDRESS >> 3 | IMAGE_3_US_TM_DHD_MCAST_THREAD_NUMBER << 12), dsptchr_viq_dest_reor,
