...
{
    uint32_t reg_cfg_profiling_sts=0;

#ifdef VALIDATE_PARMS
    if((rnr_id >= BLOCK_ADDR_COUNT) ||
       (profiling_active >= _1BITS_MAX_VAL_))
    {
    ...
    reg_cfg_profiling_sts = RU_FIELD_SET(rnr_id, RNR_REGS, CFG_PROFILING_STS, PROFILING_ACTIVE, reg_cfg_profiling_sts, profiling_active);

    RU_REG_WRITE(rnr_id, RNR_REGS, CFG_PROFILING_STS, reg_cfg_profiling_sts);
...
bdmf_error_t ag_drv_rnr_regs_reset_trace_fifo_get(uint8_t rnr_id, bdmf_boolean *profiling_active)
{
    ...
    *profiling_active = RU_FIELD_GET(rnr_id, RNR_REGS, CFG_PROFILING_STS, PROFILING_ACTIVE, reg_cfg_profiling_sts);
...
bdmf_error_t ag_drv_rnr_regs_clear_trace_fifo_overrun_set(uint8_t rnr_id, bdmf_boolean trace_fifo_overrun)
{
...
#ifdef VALIDATE_PARMS
    if((rnr_id >= BLOCK_ADDR_COUNT) ||
       (trace_fifo_overrun >= _1BITS_MAX_VAL_))
    {
        bdmf_trace("ERROR driver %s:%u| err=%s (%d)\n", __FILE__, __LINE__, bdmf_strerror(BDMF_ERR_RANGE), BDMF_ERR_RANGE);
        return BDMF_ERR_RANGE;
    }
    ...
    reg_cfg_profiling_sts = RU_FIELD_SET(rnr_id, RNR_REGS, CFG_PROFILING_STS, TRACE_FIFO_OVERRUN, reg_cfg_profiling_sts, trace_fifo_overrun);
...
#ifdef VALIDATE_PARMS
    if(!disable_dma_old_flow_control)
    {
        bdmf_trace("ERROR driver %s:%u| err=%s (%d)\n", __FILE__, __LINE__, bdmf_strerror(BDMF_ERR_PARM), BDMF_ERR_PARM);
        return BDMF_ERR_PARM;
    }
...
bdmf_error_t ag_drv_rnr_regs_cfg_stall_cnt1_get(uint8_t rnr_id, uint16_t *ld_stall_cnt, uint16_t *acc_stall_cnt)
{
    ...
    *ld_stall_cnt = RU_FIELD_GET(rnr_id, RNR_REGS, CFG_STALL_CNT1, LD_STALL_CNT, reg_cfg_stall_cnt1);
    *acc_stall_cnt = RU_FIELD_GET(rnr_id, RNR_REGS, CFG_STALL_CNT1, ACC_STALL_CNT, reg_cfg_stall_cnt1);
...
bdmf_error_t ag_drv_rnr_regs_cfg_stall_cnt2_get(uint8_t rnr_id, uint16_t *ldio_stall_cnt, uint16_t *store_stall_cnt)
{
...
#ifdef VALIDATE_PARMS
    if(!ldio_stall_cnt || !store_stall_cnt)
    {
        bdmf_trace("ERROR driver %s:%u| err=%s (%d)\n", __FILE__, __LINE__, bdmf_strerror(BDMF_ERR_PARM), BDMF_ERR_PARM);
        return BDMF_ERR_PARM;
    }
    ...
    *ldio_stall_cnt = RU_FIELD_GET(rnr_id, RNR_REGS, CFG_STALL_CNT2, LDIO_STALL_CNT, reg_cfg_stall_cnt2);
    *store_stall_cnt = RU_FIELD_GET(rnr_id, RNR_REGS, CFG_STALL_CNT2, STORE_STALL_CNT, reg_cfg_stall_cnt2);
...
bdmf_error_t ag_drv_rnr_regs_cfg_idle_cnt1_get(uint8_t rnr_id, uint32_t *idle_cnt)
{
    ...
    *idle_cnt = RU_FIELD_GET(rnr_id, RNR_REGS, CFG_IDLE_CNT1, IDLE_CNT, reg_cfg_idle_cnt1);
...
bdmf_error_t ag_drv_rnr_regs_cfg_jmp_cnt_get(uint8_t rnr_id, uint16_t *untaken_jmp_cnt, uint16_t *taken_jmp_cnt)
{
...
#ifdef VALIDATE_PARMS
    if(!untaken_jmp_cnt || !taken_jmp_cnt)
    {
        bdmf_trace("ERROR driver %s:%u| err=%s (%d)\n", __FILE__, __LINE__, bdmf_strerror(BDMF_ERR_PARM), BDMF_ERR_PARM);
        return BDMF_ERR_PARM;
    }
    ...
    *untaken_jmp_cnt = RU_FIELD_GET(rnr_id, RNR_REGS, CFG_JMP_CNT, UNTAKEN_JMP_CNT, reg_cfg_jmp_cnt);
    *taken_jmp_cnt = RU_FIELD_GET(rnr_id, RNR_REGS, CFG_JMP_CNT, TAKEN_JMP_CNT, reg_cfg_jmp_cnt);
...
static int bcm_rnr_regs_cli_set(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    ...
    case cli_rnr_regs_cfg_gen_cfg:
        err = ag_drv_rnr_regs_cfg_gen_cfg_set(parm[1].value.unumber, parm[2].value.unumber);
        break;
...
int bcm_rnr_regs_cli_get(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    ...
    case cli_rnr_regs_reset_trace_fifo:
    {
        ...
        err = ag_drv_rnr_regs_reset_trace_fifo_get(parm[1].value.unumber, &profiling_active);
    ...
    case cli_rnr_regs_clear_trace_fifo_overrun:
    {
        ...
        err = ag_drv_rnr_regs_clear_trace_fifo_overrun_get(parm[1].value.unumber, &trace_fifo_overrun);
    ...
    case cli_rnr_regs_cfg_gen_cfg:
    {
        ...
        err = ag_drv_rnr_regs_cfg_gen_cfg_get(parm[1].value.unumber, &disable_dma_old_flow_control);
    ...
    case cli_rnr_regs_cfg_stall_cnt1:
    {
        uint16_t ld_stall_cnt;
        uint16_t acc_stall_cnt;
        err = ag_drv_rnr_regs_cfg_stall_cnt1_get(parm[1].value.unumber, &ld_stall_cnt, &acc_stall_cnt);
        bdmf_session_print(session, "ld_stall_cnt = %u (0x%x)\n", ld_stall_cnt, ld_stall_cnt);
        bdmf_session_print(session, "acc_stall_cnt = %u (0x%x)\n", acc_stall_cnt, acc_stall_cnt);
        break;
    }
    case cli_rnr_regs_cfg_stall_cnt2:
    {
        uint16_t ldio_stall_cnt;
        uint16_t store_stall_cnt;
        err = ag_drv_rnr_regs_cfg_stall_cnt2_get(parm[1].value.unumber, &ldio_stall_cnt, &store_stall_cnt);
        bdmf_session_print(session, "ldio_stall_cnt = %u (0x%x)\n", ldio_stall_cnt, ldio_stall_cnt);
        bdmf_session_print(session, "store_stall_cnt = %u (0x%x)\n", store_stall_cnt, store_stall_cnt);
        break;
    }
    case cli_rnr_regs_cfg_idle_cnt1:
    {
        uint32_t idle_cnt;
        err = ag_drv_rnr_regs_cfg_idle_cnt1_get(parm[1].value.unumber, &idle_cnt);
        bdmf_session_print(session, "idle_cnt = %u (0x%x)\n", idle_cnt, idle_cnt);
        break;
    }
    case cli_rnr_regs_cfg_jmp_cnt:
    {
        uint16_t untaken_jmp_cnt;
        uint16_t taken_jmp_cnt;
        err = ag_drv_rnr_regs_cfg_jmp_cnt_get(parm[1].value.unumber, &untaken_jmp_cnt, &taken_jmp_cnt);
        bdmf_session_print(session, "untaken_jmp_cnt = %u (0x%x)\n", untaken_jmp_cnt, untaken_jmp_cnt);
        bdmf_session_print(session, "taken_jmp_cnt = %u (0x%x)\n", taken_jmp_cnt, taken_jmp_cnt);
        break;
    }
    case cli_rnr_regs_dbg_design_dbg_ctrl:
    {
...
static int bcm_rnr_regs_cli_test(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    ...
    {
        bdmf_boolean profiling_active=gtmv(m, 1);
        if(!err) bdmf_session_print(session, "ag_drv_rnr_regs_reset_trace_fifo_set(%u %u)\n", rnr_id, profiling_active);
        if(!err) ag_drv_rnr_regs_reset_trace_fifo_set(rnr_id, profiling_active);
        if(!err) ag_drv_rnr_regs_reset_trace_fifo_get( rnr_id, &profiling_active);
        if(!err) bdmf_session_print(session, "ag_drv_rnr_regs_reset_trace_fifo_get(%u %u)\n", rnr_id, profiling_active);
        if(err || profiling_active!=gtmv(m, 1))
    ...
    }
    {
        ...
        if(!err) ag_drv_rnr_regs_clear_trace_fifo_overrun_set(rnr_id, trace_fifo_overrun);
        if(!err) ag_drv_rnr_regs_clear_trace_fifo_overrun_get( rnr_id, &trace_fifo_overrun);
        if(!err) bdmf_session_print(session, "ag_drv_rnr_regs_clear_trace_fifo_overrun_get(%u %u)\n", rnr_id, trace_fifo_overrun);
        if(err || trace_fifo_overrun!=gtmv(m, 1))
            return err ? err : BDMF_ERR_IO;
    }
    {
    ...
    }
    {
        ...
        if(!err) bdmf_session_print(session, "ag_drv_rnr_regs_cfg_gen_cfg_set(%u %u)\n", rnr_id, disable_dma_old_flow_control);
        if(!err) ag_drv_rnr_regs_cfg_gen_cfg_set(rnr_id, disable_dma_old_flow_control);
        if(!err) ag_drv_rnr_regs_cfg_gen_cfg_get( rnr_id, &disable_dma_old_flow_control);
        if(!err) bdmf_session_print(session, "ag_drv_rnr_regs_cfg_gen_cfg_get(%u %u)\n", rnr_id, disable_dma_old_flow_control);
        if(err || disable_dma_old_flow_control!=gtmv(m, 1))
            return err ? err : BDMF_ERR_IO;
    }
    {
    ...
    {
        uint16_t ld_stall_cnt=gtmv(m, 16);
        uint16_t acc_stall_cnt=gtmv(m, 16);
        if(!err) ag_drv_rnr_regs_cfg_stall_cnt1_get( rnr_id, &ld_stall_cnt, &acc_stall_cnt);
        if(!err) bdmf_session_print(session, "ag_drv_rnr_regs_cfg_stall_cnt1_get(%u %u %u)\n", rnr_id, ld_stall_cnt, acc_stall_cnt);
    }
    {
        uint16_t ldio_stall_cnt=gtmv(m, 16);
        uint16_t store_stall_cnt=gtmv(m, 16);
        if(!err) ag_drv_rnr_regs_cfg_stall_cnt2_get( rnr_id, &ldio_stall_cnt, &store_stall_cnt);
        if(!err) bdmf_session_print(session, "ag_drv_rnr_regs_cfg_stall_cnt2_get(%u %u %u)\n", rnr_id, ldio_stall_cnt, store_stall_cnt);
    }
    {
        uint32_t idle_cnt=gtmv(m, 24);
        if(!err) ag_drv_rnr_regs_cfg_idle_cnt1_get( rnr_id, &idle_cnt);
        if(!err) bdmf_session_print(session, "ag_drv_rnr_regs_cfg_idle_cnt1_get(%u %u)\n", rnr_id, idle_cnt);
    }
    {
        uint16_t untaken_jmp_cnt=gtmv(m, 16);
        uint16_t taken_jmp_cnt=gtmv(m, 16);
        if(!err) ag_drv_rnr_regs_cfg_jmp_cnt_get( rnr_id, &untaken_jmp_cnt, &taken_jmp_cnt);
        if(!err) bdmf_session_print(session, "ag_drv_rnr_regs_cfg_jmp_cnt_get(%u %u %u)\n", rnr_id, untaken_jmp_cnt, taken_jmp_cnt);
    }
    {
...
bdmfmon_handle_t ag_drv_rnr_regs_cli_init(bdmfmon_handle_t driver_dir)
{
...

    {
        static bdmfmon_cmd_parm_t set_reset_trace_fifo[]={
            ...
            BDMFMON_MAKE_PARM("profiling_active", "profiling_active", BDMFMON_PARM_NUMBER, 0),
            BDMFMON_PARM_LIST_TERMINATOR
        };
        static bdmfmon_cmd_parm_t set_clear_trace_fifo_overrun[]={
            BDMFMON_MAKE_PARM_ENUM("rnr_id", "rnr_id", rnr_id_enum_table, 0),
            BDMFMON_MAKE_PARM("trace_fifo_overrun", "trace_fifo_overrun", BDMFMON_PARM_NUMBER, 0),
            BDMFMON_PARM_LIST_TERMINATOR
        };
    ...
    }
    {
        ...
        static bdmfmon_enum_val_t selector_table[] = {
            ...
            { .name="cfg_stall_cnt1", .val=cli_rnr_regs_cfg_stall_cnt1, .parms=set_default },
            { .name="cfg_stall_cnt2", .val=cli_rnr_regs_cfg_stall_cnt2, .parms=set_default },
            { .name="cfg_idle_cnt1", .val=cli_rnr_regs_cfg_idle_cnt1, .parms=set_default },
            { .name="cfg_jmp_cnt", .val=cli_rnr_regs_cfg_jmp_cnt, .parms=set_default },
