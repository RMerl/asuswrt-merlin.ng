...
static inline int rdp_cpu_inc_read_idx(uint32_t ring_id, uint8_t type, uint16_t delta)
{
    if (type == rdpa_ring_recycle)
    {
        ...
        *ring_descr->read_idx = swap2bytes((swap2bytes(*ring_descr->read_idx) + delta) & (ring_descr->num_of_entries - 1));
    }
    else
...
int alloc_and_assign_packet_to_feed_ring(void)
{
    RING_DESCTIPTOR *feed_ring_descr = &host_ring[FEED_RING_ID];
    uint32_t read_idx = feed_ring_descr->shadow_read_idx;
    uint32_t write_idx = feed_ring_descr->shadow_write_idx;
...
/* cntr todo: seems not in use - delete?*/
inline int rdp_cpu_rx_get_queue_discard(uint32_t ring_id, uint16_t* num_of_packets)
{
    ...
    {
        rc = drv_cntr_various_counter_get(COUNTER_CPU_RX_FEED_RING_CONGESTION, num_of_packets);
    }
    else
    {
...
int rdp_cpu_ring_buffers_init(RING_DESCTIPTOR *ring_descr, uint32_t ring_id)
{
    uint32_t i;
