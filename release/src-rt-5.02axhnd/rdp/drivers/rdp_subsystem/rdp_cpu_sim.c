...
int cpu_runner_sim_connect(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    ...
    while (bind(g_runner_sim_srv_socket, (struct sockaddr *)&connection_address, sizeof(struct sockaddr_in)) == INVALID_SOCKET)
    {
    ...
    if (listen(g_runner_sim_srv_socket, 1) == INVALID_SOCKET)
    {
        bdmf_session_print(session, "Listen error occured with errno=%d\n", errno);
        goto exit;
    }
