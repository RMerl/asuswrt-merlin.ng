...
static int drv_bbh_tx_cli_occupancy_get(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    ...
    rc = rc ? rc : drv_bbh_tx_read_indirect_debug_register(bbh_id, PDFULLSEL, 0, (uint32_t*)&full[0]);
    rc = rc ? rc : drv_bbh_tx_read_indirect_debug_register(bbh_id, PDFULLSEL, 1, (uint32_t*)&full[4]);
    rc = rc ? rc : drv_bbh_tx_read_indirect_debug_register(bbh_id, PDEMPTYSEL, 0, (uint32_t*)&empty[0]);
    rc = rc ? rc : drv_bbh_tx_read_indirect_debug_register(bbh_id, PDEMPTYSEL, 1, (uint32_t*)&empty[4]);
    ...
    if(IS_WAN_TX_PORT(bbh_id))
    {
        ...
        for(i = 0 ;i < (TX_QEUEU_PAIRS * 2); i++)
        {
            bdmf_session_print(session, "BBH_TX queue(%03d): ingress=%03d, egress=%03d, occupancy=%03d, fifo size=%03d, almost_full=%d, full=%d, empty=%d\n\r", i, ingress_cnt[i],
                    egress_cnt[i], ingress_cnt[i] - egress_cnt[i], fifo_size[i], (almostFull[i/8] & (1 << (i % 8))) != 0, (full[i/8] & (1 << (i % 8))) != 0, (empty[i/8] & (1 << (i % 8))) != 0);
        }
        /*now calculate ghost */
    ...
    else
    {
       ...
       for(i = 0 ;i < (number_of_lans); i++)
       {
           bdmf_session_print(session, "BBH_TX queue(%03d): ingress=%03d, egress=%03d, occupancy=%03d, fifo size=%03d, almost_full=%d, full=%d, empty=%d\n\r", i, ingress_cnt[i], egress_cnt[i], ingress_cnt[i] - egress_cnt[i], fifo_size[i],
                   (almostFull[i/8] & (1 << (i % 8))) != 0, (full[i/8] & (1 << (i % 8))) != 0, (empty[i/8] & (1 << (i % 8))) != 0);
       }
    }
    return rc;
}
