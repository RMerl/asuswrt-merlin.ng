...
#include "merlin_serdes_dependencies.c"
...
err_code_t merlin_mptwo_display_eye_scan(merlin_access_t *ma) {
    uint32_t   stripe[64];
    uint16_t   status = 0;
    int8_t     y;
    ...
    {   err_code_t err_code = merlin_mptwo_meas_eye_scan_start(ma,EYE_SCAN_HORIZ);
        if (err_code) {
            EFUN((merlin_mptwo_meas_eye_scan_done(ma), err_code));
        }
    }
    ...
    for (y = 31;y>=-31;y=y-1)
    {
        {   err_code_t err_code = merlin_mptwo_read_eye_scan_stripe(ma,&stripe[0], &status);
            if (err_code) {
                EFUN((merlin_mptwo_meas_eye_scan_done(ma), err_code));
            }
        }
        EFUN(merlin_mptwo_display_eye_scan_stripe(ma,y,&stripe[0]));
        EFUN_PRINTF(("\n"));
    }
    /* stop acquisition */
    EFUN(merlin_mptwo_meas_eye_scan_done(ma));
...
err_code_t merlin_mptwo_meas_lowber_eye(merlin_access_t *ma, const struct merlin_mptwo_eyescan_options_st eyescan_options, uint32_t *buffer) {
  int8_t y,x;
  int16_t i;
  uint16_t status;
  uint32_t errors = 0;
  ...
  for (y = eyescan_options.vert_max;y>=eyescan_options.vert_min;y=y-eyescan_options.vstep) {
    for (x=eyescan_options.horz_min;x<=eyescan_options.horz_max;x=x+eyescan_options.hstep) {
      /* acquire sample */
        EFUN(merlin_mptwo_pmd_uc_cmd_with_data(ma,CMD_DIAG_EN,CMD_UC_DIAG_GET_EYE_SAMPLE,((uint16_t)x)<<8 | (uint8_t)y,200));
...
#else
        EFUN(merlin_mptwo_poll_diag_done(ma,&status, (((uint32_t)timeout)<<7)*10 + 20000));
#endif
        {
            ...
            if(osr_mode.tx_rx == MERLIN_MPTWO_OSX1) {
                EFUN(merlin_mptwo_prbs_err_count_ll(ma,&errors));
...
err_code_t merlin_mptwo_display_lowber_eye(merlin_access_t *ma, const struct merlin_mptwo_eyescan_options_st eyescan_options, uint32_t *buffer) {
    int8_t x,y,z;
    ...
    j = 0;
    for (y = eyescan_options.vert_max;y>=eyescan_options.vert_min;y=y-eyescan_options.vstep) {
      ESTM_PRINTF(("%6dmV : ", _ladder_setting_to_mV(ma,y, rd_p1_thresh_sel())));
...
err_code_t merlin_mptwo_meas_eye_scan_start(merlin_access_t *ma, uint8_t direction) {
    uint8_t lock;
    ...
    if(direction == EYE_SCAN_VERTICAL) {
        EFUN(merlin_mptwo_pmd_uc_diag_cmd(ma,CMD_UC_DIAG_START_VSCAN_EYE,200));
    } else {
        EFUN(merlin_mptwo_pmd_uc_diag_cmd(ma,CMD_UC_DIAG_START_HSCAN_EYE,200));
    }
    return(ERR_CODE_NONE);
}

err_code_t merlin_mptwo_read_eye_scan_stripe(merlin_access_t *ma, uint32_t *buffer,uint16_t *status){
    uint32_t val[2] = {0,0};
    uint16_t sts = 0;
    int8_t   i;
    ...
    *status = 0;
    for(i=0;i<32;i++) {
        {   err_code_t err_code = merlin_mptwo_poll_diag_eye_data(ma,&val[0], &sts, 200);
            *status |= sts & 0xF000;
            if (err_code) {
                return (err_code);
            }
...
err_code_t merlin_mptwo_display_eye_scan_stripe(merlin_access_t *ma, int8_t y,uint32_t *buffer) {
    ...
    level = _ladder_setting_to_mV(ma,y,data_thresh);
...
err_code_t merlin_mptwo_meas_eye_density_data(merlin_access_t *ma, const struct merlin_mptwo_eyescan_options_st eyescan_options, int32_t *buffer,uint16_t *buffer_size) {
  ...
  EFUN(merlin_mptwo_pmd_uc_diag_cmd(ma,CMD_UC_DIAG_DENSITY,2000));
  i = 0;hzcnt=0;
  ESTM_PRINTF(("Calculating %d\n", rd_cnt_d_minus_m1()));
  for (y = eyescan_options.vert_max;y>=eyescan_options.vert_min;y=y-eyescan_options.vstep) {
    EFUN(_set_p1_threshold(ma,y));
    EFUN(_move_clkp1_offset(ma,eyescan_options.horz_min-1));   /* Walk back 32 steps */
    EFUN(_move_clkp1_offset(ma,1));                            /* Walk forward one (net -31). This sets up the registers for 1 write increments */
    hzcnt = eyescan_options.horz_min;
    for (x=eyescan_options.horz_min;x<=eyescan_options.horz_max;x=x+eyescan_options.hstep) {
         EFUN(_trnsum_clear_and_enable(ma));
         EFUN(merlin_mptwo_poll_dsc_state_equals_uc_tune(ma,2000));
    ...
    EFUN(_move_clkp1_offset(ma,-hzcnt));     /* Walk back center */
    EFUN_PRINTF(("\n"));
  }
  EFUN_PRINTF(("\n"));
  *buffer_size = i;
...
err_code_t merlin_mptwo_display_eye_density_data(merlin_access_t *ma, const struct merlin_mptwo_eyescan_options_st eyescan_options, int32_t *buffer,uint16_t buffer_size) {
    ...
    for (y = eyescan_options.vert_max-1;y>=eyescan_options.vert_min;y=y-eyescan_options.vstep) {
        EFUN_PRINTF(("%6dmV : ",(_ladder_setting_to_mV(ma,y, range) + _ladder_setting_to_mV(ma,y+1, range))/2));
        for (z = -31; z < eyescan_options.horz_min; ++z) {
            EFUN_PRINTF((" "));    /* add leading spaces if any */
        }
        for (x=eyescan_options.horz_min;x<=eyescan_options.horz_max;x=x+eyescan_options.hstep) {
...
err_code_t merlin_mptwo_read_eye_scan_status(merlin_access_t *ma, uint16_t *status) {
...
err_code_t merlin_mptwo_meas_eye_scan_done(merlin_access_t *ma) {
  EFUN(merlin_mptwo_pmd_uc_diag_cmd(ma,CMD_UC_DIAG_DISABLE,200));
  return(ERR_CODE_NONE);
}


err_code_t merlin_mptwo_start_ber_scan_test(merlin_access_t *ma, uint8_t ber_scan_mode, uint8_t timer_control, uint8_t max_error_control) {
    uint8_t lock,sts;
    ESTM(lock = rd_pmd_rx_lock());
    if(lock == 0) {
    ...
    EFUN(merlin_mptwo_pmd_uc_cmd(ma,CMD_CAPTURE_BER_START, ber_scan_mode,500));
    return(ERR_CODE_NONE);
}

err_code_t merlin_mptwo_read_ber_scan_data(merlin_access_t *ma, uint32_t *errors, uint32_t *timer_values, uint8_t *cnt, uint32_t timeout) {
 ...
 ESTM(sts = rdv_usr_diag_status());
    ...
    *cnt = (sts & 0x00FF)/3;
    for(i=0;i < *cnt;i++) {
        ...
        EFUN(merlin_mptwo_pmd_uc_cmd(ma,CMD_READ_DIAG_DATA_WORD, 0, timeout));
        ...
        EFUN(merlin_mptwo_pmd_uc_cmd(ma,CMD_READ_DIAG_DATA_BYTE, 0, timeout));
...
err_code_t merlin_mptwo_eye_margin_proj(merlin_access_t *ma, USR_DOUBLE rate, uint8_t ber_scan_mode, uint8_t timer_control, uint8_t max_error_control) {
    ...
    if(verbose > 2) EFUN_PRINTF(("offset_start = %d:%dmV\n",offset_start,_ladder_setting_to_mV(ma,(int8_t)offset_start,0)));
    ...
    EFUN(merlin_mptwo_read_ber_scan_data(ma,&errs[0], &time[0], &cnt, 2000));

    if(verbose > 2) EFUN_PRINTF(("read done cnt=%d\n",cnt));

    EFUN(merlin_mptwo_pmd_uc_cmd(ma,CMD_CAPTURE_BER_END,0x00,50));
    ...
    do {
        if(heye == 1) {
            ...
            if(intrusive) {
                ...
                margins[i] = direction*_ladder_setting_to_mV(ma,offset[i], range250);
            }
#else
            margins[i] = info_out->margins[i];
#endif
...
err_code_t merlin_mptwo_display_lane_state(merlin_access_t *ma) {
  err_code_t err_code = _merlin_mptwo_display_lane_state_no_newline(ma);
  EFUN_PRINTF(("\n"));
  return (err_code);
}

err_code_t merlin_mptwo_get_eye_margin_est(merlin_access_t *ma, int *left_eye_mUI, int *right_eye_mUI, int *upper_eye_mV, int *lower_eye_mV) {
  ...
  ESTM(*upper_eye_mV = _eye_to_mV(ma,rdv_usr_sts_veye_upper(), ladder_range));
  ESTM(*lower_eye_mV = _eye_to_mV(ma,rdv_usr_sts_veye_lower(), ladder_range));
  return (ERR_CODE_NONE);
}
...
err_code_t merlin_mptwo_display_core_state_line(merlin_access_t *ma) {
  err_code_t err_code = _merlin_mptwo_display_core_state_no_newline(ma);
  EFUN_PRINTF(("\n"));
  return (err_code);
}
...
err_code_t merlin_mptwo_display_core_config(merlin_access_t *ma) {
    EFUN_PRINTF(("\n\n***********************************\n"  ));
...
err_code_t merlin_mptwo_display_lane_config(merlin_access_t *ma) {
    ...
    EFUN_PRINTF((    "*************************************\n\n"                                        ));
    EFUN(                                                         merlin_mptwo_get_uc_lane_cfg(ma,&lane_cfg)      );
...
err_code_t merlin_mptwo_display_core_state(merlin_access_t *ma) {
    EFUN(merlin_mptwo_display_core_state_hdr());
    EFUN(merlin_mptwo_display_core_state_line(ma));
...
err_code_t merlin_mptwo_display_lane_debug_status(merlin_access_t *ma) {
    /* startup */
    ...
    EFUN(merlin_mptwo_get_usr_ctrl_disable_startup(ma,&ds));
    ...
    EFUN(                                                            merlin_mptwo_get_usr_ctrl_disable_startup_dfe(ma,&dsd) );
    ...
    EFUN(                                                           merlin_mptwo_get_usr_ctrl_disable_steady_state(ma,&dss) );
    ...
    EFUN(                                                      merlin_mptwo_get_usr_ctrl_disable_steady_state_dfe(ma,&dssd) );
...
err_code_t merlin_mptwo_stop_uc_lane(merlin_access_t *ma, uint8_t enable) {
  ...
  if (enable) {
    return(merlin_mptwo_pmd_uc_control(ma,CMD_UC_CTRL_STOP_GRACEFULLY,100));
  }
  else {
    return(merlin_mptwo_pmd_uc_control(ma,CMD_UC_CTRL_RESUME,50));
  }
}
...
err_code_t merlin_mptwo_stop_rx_adaptation(merlin_access_t *ma, uint8_t enable) {
...
err_code_t merlin_mptwo_request_stop_rx_adaptation(merlin_access_t *ma) {

  return(merlin_mptwo_pmd_uc_control_return_immediate(ma,CMD_UC_CTRL_STOP_GRACEFULLY));
}
...
err_code_t merlin_mptwo_ucode_crc_verify(merlin_access_t *ma, uint16_t ucode_len,uint16_t expected_crc_value) {
    uint16_t calc_crc;

    EFUN(merlin_mptwo_pmd_uc_cmd_with_data(ma, CMD_CALC_CRC,0,ucode_len,200));
...
err_code_t merlin_mptwo_start_ucode_crc_calc(merlin_access_t *ma, uint16_t ucode_len) {

    EFUN(merlin_mptwo_pmd_uc_cmd_with_data_return_immediate(ma, CMD_CALC_CRC, 0, ucode_len)); /* Invoke Calculate CRC command and return control immediately */
    return(ERR_CODE_NONE);
}

err_code_t merlin_mptwo_check_ucode_crc(merlin_access_t *ma, uint16_t expected_crc_value, uint32_t timeout_ms) {
    uint16_t calc_crc;
    err_code_t err_code;

    err_code = merlin_mptwo_poll_uc_dsc_ready_for_cmd_equals_1(ma, timeout_ms);     /* Poll for uc_dsc_ready_for_cmd = 1 to indicate merlin_mptwo ready for command */
    if (err_code) {
...
err_code_t merlin_mptwo_pmd_uc_cmd_return_immediate(merlin_access_t *ma, enum srds_pmd_uc_cmd_enum cmd, uint8_t supp_info) {
  ...
  err_code = merlin_mptwo_poll_uc_dsc_ready_for_cmd_equals_1(ma,1); /* Poll for uc_dsc_ready_for_cmd = 1 to indicate merlin_mptwo ready for command */
  if (err_code) {
...
/*EFUN(wr_uc_dsc_ready_for_cmd(0x0)  ); */                     /* Issue command, by clearing "ready for command" field */
  ...
  EFUN(merlin_mptwo_pmd_wr_reg(ma,DSC_A_DSC_UC_CTRL, cmddata));         /* This address is same for Eagle, and all Merlin */
...
err_code_t merlin_mptwo_pmd_uc_cmd(merlin_access_t *ma, enum srds_pmd_uc_cmd_enum cmd, uint8_t supp_info, uint32_t timeout_ms) {
  ...
  EFUN(merlin_mptwo_pmd_uc_cmd_return_immediate(ma,cmd, supp_info));    /* Invoke merlin_mptwo_uc_cmd and return control immediately */

  EFUN(merlin_mptwo_poll_uc_dsc_ready_for_cmd_equals_1(ma,timeout_ms)); /* Poll for uc_dsc_ready_for_cmd = 1 to indicate merlin_mptwo ready for command */
...
err_code_t merlin_mptwo_pmd_uc_cmd_with_data(merlin_access_t *ma, enum srds_pmd_uc_cmd_enum cmd, uint8_t supp_info, uint16_t data, uint32_t timeout_ms) {
    ...
    EFUN(merlin_mptwo_pmd_uc_cmd_with_data_return_immediate(ma,cmd, supp_info, data)); /* Invoke merlin_mptwo_uc_cmd and return control immediately */
...
uint8_t merlin_mptwo_rdbl_uc_var(merlin_access_t *ma, err_code_t *err_code_p, uint16_t addr) {
    uint8_t rddata;
    ...
    EPSTM(rddata = merlin_mptwo_rdb_uc_ram(ma,err_code_p, (LANE_VAR_RAM_BASE+addr+(merlin_mptwo_get_lane(ma)*LANE_VAR_RAM_SIZE)))); /* Use Micro register interface for reading RAM */

    return (rddata);
}

/* Micro RAM Lane Byte Signed Read */
int8_t merlin_mptwo_rdbls_uc_var(merlin_access_t *ma, err_code_t *err_code_p, uint16_t addr) {
  return ((int8_t) merlin_mptwo_rdbl_uc_var(ma,err_code_p, addr));
}

/* Micro RAM Lane Word Read */
uint16_t merlin_mptwo_rdwl_uc_var(merlin_access_t *ma, err_code_t *err_code_p, uint16_t addr) {
  uint16_t rddata;
  ...
  EPSTM(rddata = merlin_mptwo_rdw_uc_ram(ma, err_code_p, (LANE_VAR_RAM_BASE+addr+(merlin_mptwo_get_lane(ma)*LANE_VAR_RAM_SIZE)))); /* Use Micro register interface for reading RAM */
...
int16_t merlin_mptwo_rdwls_uc_var(merlin_access_t *ma, err_code_t *err_code_p, uint16_t addr) {
  return ((int16_t) merlin_mptwo_rdwl_uc_var(ma,err_code_p, addr));
}
...
err_code_t merlin_mptwo_wrbl_uc_var(merlin_access_t *ma, uint16_t addr, uint8_t wr_val) {
    ...
    if(addr < LANE_VAR_RAM_SIZE) {
        return (merlin_mptwo_wrb_uc_ram(ma, (LANE_VAR_RAM_BASE+addr+(merlin_mptwo_get_lane(ma)*LANE_VAR_RAM_SIZE)), wr_val));    /* Use Micro register interface for writing RAM */
    } else {
...
err_code_t merlin_mptwo_wrbls_uc_var(merlin_access_t *ma, uint16_t addr, int8_t wr_val) {
  return (merlin_mptwo_wrbl_uc_var(ma,addr, wr_val));
}
...
err_code_t merlin_mptwo_wrwl_uc_var(merlin_access_t *ma, uint16_t addr, uint16_t wr_val) {
    ...
    if(addr < LANE_VAR_RAM_SIZE) {
        return (merlin_mptwo_wrw_uc_ram(ma, (LANE_VAR_RAM_BASE+addr+(merlin_mptwo_get_lane(ma)*LANE_VAR_RAM_SIZE)), wr_val));    /* Use Micro register interface for writing RAM */
    } else {
        return(_error(ERR_CODE_INVALID_RAM_ADDR));
    }
}
...
err_code_t merlin_mptwo_wrwls_uc_var(merlin_access_t *ma, uint16_t addr, int16_t wr_val) {
  return (merlin_mptwo_wrwl_uc_var(ma,addr,wr_val));
}
...
uint8_t merlin_mptwo_rdbc_uc_var(merlin_access_t *ma, err_code_t *err_code_p, uint8_t addr) {
  ...
  EPSTM(rddata = merlin_mptwo_rdb_uc_ram(ma, err_code_p, (CORE_VAR_RAM_BASE+addr)));                      /* Use Micro register interface for reading RAM */
  return (rddata);
}

/* Micro RAM Core Byte Signed Read */
int8_t merlin_mptwo_rdbcs_uc_var(merlin_access_t *ma, err_code_t *err_code_p, uint8_t addr) {
  return ((int8_t) merlin_mptwo_rdbc_uc_var(ma,err_code_p, addr));
}

/* Micro RAM Core Word Read */
uint16_t merlin_mptwo_rdwc_uc_var(merlin_access_t *ma, err_code_t *err_code_p, uint8_t addr) {
  ...
  EPSTM(rddata = merlin_mptwo_rdw_uc_ram(ma, err_code_p, (CORE_VAR_RAM_BASE+addr)));                  /* Use Micro register interface for reading RAM */
  return (rddata);
}

/* Micro RAM Core Word Signed Read */
int16_t merlin_mptwo_rdwcs_uc_var(merlin_access_t *ma, err_code_t *err_code_p, uint8_t addr) {
  return ((int16_t) merlin_mptwo_rdwc_uc_var(ma,err_code_p, addr));
}
...
err_code_t merlin_mptwo_wrbc_uc_var(merlin_access_t *ma, uint8_t addr, uint8_t wr_val) {
    ...
    return (merlin_mptwo_wrb_uc_ram(ma, (CORE_VAR_RAM_BASE+addr), wr_val));                                /* Use Micro register interface for writing RAM */
    } else {
        return(_error(ERR_CODE_INVALID_RAM_ADDR));
    }
}
...
err_code_t merlin_mptwo_wrbcs_uc_var(merlin_access_t *ma, uint8_t addr, int8_t wr_val) {
  return (merlin_mptwo_wrbc_uc_var(ma,addr, wr_val));
}
...
err_code_t merlin_mptwo_wrwc_uc_var(merlin_access_t *ma, uint8_t addr, uint16_t wr_val) {
    ...
    if(addr < CORE_VAR_RAM_SIZE) {
        return (merlin_mptwo_wrw_uc_ram(ma, (CORE_VAR_RAM_BASE+addr), wr_val));                                 /* Use Micro register interface for writing RAM */
    } else {
        return(_error(ERR_CODE_INVALID_RAM_ADDR));
    }
}
...
err_code_t merlin_mptwo_wrwcs_uc_var(merlin_access_t *ma, uint8_t addr, int16_t wr_val) {
  return(merlin_mptwo_wrwc_uc_var(ma,addr,wr_val));
}
...
err_code_t merlin_mptwo_set_uc_core_config(merlin_access_t *ma, struct merlin_mptwo_uc_core_config_st struct_val) {
    {   
#ifndef ATE_LOG
        uint8_t reset_state;
        ESTM(reset_state = rdc_core_dp_reset_state());
        if(reset_state < 7) {
            EFUN_PRINTF(("ERROR: merlin_mptwo_set_uc_core_config(..) called without core_dp_s_rstb=0 Lane=%d reset_state=%d\n",merlin_mptwo_get_lane(ma),reset_state));
...
err_code_t merlin_mptwo_set_uc_lane_cfg(merlin_access_t *ma, struct merlin_mptwo_uc_lane_config_st struct_val) {
#ifndef ATE_LOG
    uint8_t reset_state;
...
err_code_t merlin_mptwo_get_uc_core_config(merlin_access_t *ma, struct merlin_mptwo_uc_core_config_st *get_val)
{
...
err_code_t merlin_mptwo_get_uc_lane_cfg(merlin_access_t *ma, struct merlin_mptwo_uc_lane_config_st *get_val) {
...
err_code_t merlin_mptwo_poll_diag_eye_data(merlin_access_t *ma, uint32_t *data,uint16_t *status, uint32_t timeout_ms) {
 ...
 for(loop=0;loop < 100; loop++) {
     ...
     if(((*status & 0x00FF) > 2) || ((*status & 0x8000) > 0)) {
        EFUN(merlin_mptwo_pmd_uc_cmd(ma,CMD_READ_DIAG_DATA_WORD, 0, 200));
...
err_code_t merlin_mptwo_poll_uc_dsc_ready_for_cmd_equals_1(merlin_access_t *ma, uint32_t timeout_ms) {
  ...
  for (loop = 0; loop < 100; loop++) {
    uint16_t rddata;
    EFUN(merlin_mptwo_pmd_rdt_reg(ma,DSC_A_DSC_UC_CTRL, &rddata));
...
err_code_t merlin_mptwo_tx_ull_config(merlin_access_t *ma, uint8_t enable) {
    EFUN(merlin_mptwo_tx_rptr_mode_timing(ma,enable)); /* Enable jitter filter enable */
    EFUN(_ull_tx_mode_setup(ma,enable));
    return (ERR_CODE_NONE);
}

/* Configure ULL mode for RX */
err_code_t merlin_mptwo_rx_ull_config(merlin_access_t *ma, uint8_t enable) {
    EFUN(merlin_mptwo_rx_rptr_mode_timing(ma,enable));
    EFUN(_ull_rx_mode_setup(ma,enable));
    return (ERR_CODE_NONE);
}

/* TX PI setup for Repeater Mode */
err_code_t merlin_mptwo_tx_rptr_mode_timing(merlin_access_t *ma, uint8_t enable) {
...
err_code_t merlin_mptwo_rx_rptr_mode_timing(merlin_access_t *ma, uint8_t enable) {
    (void)enable;
...
err_code_t merlin_mptwo_init_pram_for_uc_load(merlin_access_t *ma, uint16_t ucode_len) {
    uint8_t result;
...
err_code_t merlin_mptwo_finish_pram_load(merlin_access_t *ma) {
...
err_code_t merlin_mptwo_reg_dump(merlin_access_t *ma) {
  ...
  for (addr = 0x0; addr < 0x10; addr++) {
    ...
    EFUN(merlin_mptwo_pmd_rdt_reg(ma,addr,&rddata));
...
err_code_t merlin_mptwo_uc_core_var_dump(merlin_access_t *ma) {
  ...
  for (addr = 0x0; addr < CORE_VAR_RAM_SIZE; addr++) {
    ...
    ESTM_PRINTF(("%02x ", merlin_mptwo_rdbc_uc_var(ma,__ERR, addr)));
  }
  return (ERR_CODE_NONE);
}
...
err_code_t merlin_mptwo_uc_lane_var_dump(merlin_access_t *ma) {
  ...
  EFUN_PRINTF(("\n**** SERDES UC LANE %d RAM VARIABLE DUMP ****",merlin_mptwo_get_lane(ma)));
  ...
  if (rx_lock == 1) {
    ...
    for (addr = 0x0; addr < LANE_VAR_RAM_SIZE; addr++) {
      ...
      ESTM_PRINTF(("%02x ", merlin_mptwo_rdbl_uc_var(ma,__ERR, addr)));
    }
...
err_code_t merlin_mptwo_display_diag_data(merlin_access_t *ma, uint16_t  diag_level) {
    ...
    ESTM_PRINTF(("Core = %d; LANE = %d\n",merlin_mptwo_get_core(ma),merlin_mptwo_get_lane(ma)));
    ...
    if (rx_lock == 1) {
        ...
        EFUN(merlin_mptwo_display_lane_state(ma));
    ...
    if(diag_level & SRDS_DIAG_EVENT) {
        ...
        EFUN(merlin_mptwo_read_event_log(ma,trace_mem,EVENT_LOG_HEX));
    }
    if(diag_level & SRDS_DIAG_EYE) {
        EFUN(merlin_mptwo_display_eye_scan(ma));
    }
    if(diag_level & SRDS_DIAG_REG_CORE) {
        EFUN(merlin_mptwo_reg_dump(ma));
    }
    ...
    if(diag_level & SRDS_DIAG_UC_CORE) {
        EFUN(merlin_mptwo_uc_core_var_dump(ma));
    }
    if(diag_level & SRDS_DIAG_UC_LANE) {
        EFUN(merlin_mptwo_uc_lane_var_dump(ma));
    }
    if(diag_level & SRDS_DIAG_LANE_DEBUG) {
        EFUN(merlin_mptwo_display_lane_debug_status(ma));
    }
    if(diag_level & SRDS_DIAG_BER_VERT) {
        ...
        EFUN(_merlin_mptwo_display_ber_scan_data(ma,ber_mode, timer_control,  err_threshold));
...
/* Required Diagnostic Functions */
err_code_t merlin_mptwo_diag_access(merlin_access_t *ma, enum srds_diag_access_enum type, uint16_t addr, uint16_t data, uint16_t param) {
    ...
    case SRDS_REG_READ:      {
                             ...
                             if(data > 1) {
                                 ...
                                 for (i = 0; i < data; i++) {
                                     ...
                                     EFUN(merlin_mptwo_pmd_rdt_reg(ma,i+addr,&tmp));
                                     ESTM_PRINTF(("%04x ", tmp));
                                 }
                                 EFUN_PRINTF(("\n"));
                             } else {
                                 EFUN(merlin_mptwo_pmd_rdt_reg(ma,addr,&tmp));
    ...
    case SRDS_REG_RMW:       {
                             ...
                             EFUN(merlin_mptwo_pmd_wr_reg(ma,addr,tmp2));
    ...
    case SRDS_CORE_RAM_READ_BYTE: {
                             if(data > 1) {
                                 ...
                                 for (i = 0; i < (uint8_t)data; i++) {
                                     ...
                                     ESTM_PRINTF(("%02x ", merlin_mptwo_rdbc_uc_var(ma,__ERR, i+(uint8_t)addr)));
                                 }
                                 EFUN_PRINTF(("\n"));
                             } else {
                                 ESTM_PRINTF(("Core RAM Read byte: x%04x = x%02x\n",(uint8_t)addr,merlin_mptwo_rdbc_uc_var(ma,__ERR, (uint8_t)addr)));
                             }
                             } break;
    case SRDS_LANE_RAM_READ_BYTE: {
                             if(data > 1) {
                                 ...
                                 for (i = 0; i < data; i++) {
                                     ...
                                     ESTM_PRINTF(("%02x ", merlin_mptwo_rdbl_uc_var(ma,__ERR,i+addr)));
                                 }
                                 EFUN_PRINTF(("\n"));
                             } else {
                                 ESTM_PRINTF(("Lane RAM Read byte: x%04x = x%02x\n",addr,merlin_mptwo_rdbl_uc_var(ma,__ERR,addr)));
                             }
                             } break;
    case SRDS_CORE_RAM_READ_WORD: {
                             if(data > 1) {
                                 ...
                                 for (i = 0; i < (uint8_t)data; i+=2) {
                                     ...
                                     ESTM_PRINTF(("%04x ", merlin_mptwo_rdwc_uc_var(ma,__ERR, i+(uint8_t)addr)));
                                 }
                                 EFUN_PRINTF(("\n"));
                             } else {
                                 ESTM_PRINTF(("Core RAM Read word: x%04x = x%04x\n",addr,merlin_mptwo_rdwc_uc_var(ma,__ERR, (uint8_t)addr)));
                             }
                             } break;
    case SRDS_LANE_RAM_READ_WORD: {
                             if(data > 1) {
                                 ...
                                 for (i = 0; i < data; i+=2) {
                                     ...
                                     ESTM_PRINTF(("%04x ", merlin_mptwo_rdwl_uc_var(ma,__ERR,i+addr)));
                                 }
                                 EFUN_PRINTF(("\n"));
                             } else {
                                 ESTM_PRINTF(("Lane RAM Read word: x%04x = x%04x\n",addr,merlin_mptwo_rdwl_uc_var(ma,__ERR, addr)));
                             }
                             } break;
    case SRDS_CORE_RAM_RMW_BYTE:  {
                             uint8_t tmp, tmp2;
                             ESTM(tmp = merlin_mptwo_rdbc_uc_var(ma,__ERR, (uint8_t)addr));
                             tmp2 = (tmp & (uint8_t)~param) | ((uint8_t)param & data);
                             EFUN(merlin_mptwo_wrbc_uc_var(ma,(uint8_t)addr,tmp2));
                             EFUN_PRINTF(("Core RAM RMW byte: x%04x = x%02x -> x%02x\n",addr,tmp,tmp2));
                             } break;
    case SRDS_LANE_RAM_RMW_BYTE:  {
                             uint8_t tmp, tmp2;
                             ESTM(tmp = merlin_mptwo_rdbl_uc_var(ma,__ERR,addr));
                             tmp2 = (tmp & (uint8_t)~param) | ((uint8_t)param & data);
                             EFUN(merlin_mptwo_wrbl_uc_var(ma,addr,tmp2));
                             EFUN_PRINTF(("Lane RAM RMW byte: x%04x = x%02x -> x%02x\n",addr,tmp,tmp2));
                             } break;
    case SRDS_CORE_RAM_RMW_WORD:  {
                             uint16_t tmp, tmp2;
                             ESTM(tmp = merlin_mptwo_rdwc_uc_var(ma,__ERR, (uint8_t)addr));
                             tmp2 = (tmp & ~param) | (param & data);
                             EFUN(merlin_mptwo_wrwc_uc_var(ma,(uint8_t)addr,tmp2));
                             EFUN_PRINTF(("Core RAM RMW word: x%04x = x%04x -> x%04x\n",addr,tmp,tmp2));
                             } break;
    case SRDS_LANE_RAM_RMW_WORD:  {
                             uint16_t tmp, tmp2;
                             ESTM(tmp = merlin_mptwo_rdwl_uc_var(ma,__ERR,addr));
                             tmp2 = (tmp & ~param) | (param & data);
                             EFUN(merlin_mptwo_wrwl_uc_var(ma,addr,tmp2));
    ...
    case SRDS_GLOB_RAM_READ_BYTE: {
                             if(data > 1) {
                                 ...
                                 for (i = 0; i < data; i++) {
                                     ...
                                     ESTM_PRINTF(("%02x ", _merlin_mptwo_rdb_uc_var(ma,__ERR,i+addr)));
                                 }
                                 EFUN_PRINTF(("\n"));
                             } else {
                                 ESTM_PRINTF(("Glob RAM Read byte: x%04x = x%02x\n",addr,_merlin_mptwo_rdb_uc_var(ma,__ERR,addr)));
                             }
                             } break;
    case SRDS_GLOB_RAM_RMW_BYTE:  {
                             uint8_t tmp, tmp2;
                             ESTM(tmp = _merlin_mptwo_rdb_uc_var(ma,__ERR,addr));
                             tmp2 = (tmp & (uint8_t)~param) | ((uint8_t)param & data);
                             EFUN(_merlin_mptwo_wrb_uc_var(ma,addr,tmp2));
    ...
    case SRDS_GLOB_RAM_READ_WORD: {
                             if(data > 1) {
                                 ...
                                 for (i = 0; i < data; i+=2) {
                                     ...
                                     ESTM_PRINTF(("%04x ", _merlin_mptwo_rdw_uc_var(ma,__ERR,i+addr)));
                                 }
                                 EFUN_PRINTF(("\n"));
                             } else {
                                 ESTM_PRINTF(("Glob RAM Read word: x%04x = x%04x\n",addr,_merlin_mptwo_rdw_uc_var(ma,__ERR,addr)));
                             }
                             } break;
    case SRDS_GLOB_RAM_RMW_WORD:  {
                             uint16_t tmp, tmp2;
                             ESTM(tmp = _merlin_mptwo_rdw_uc_var(ma,__ERR,addr));
                             tmp2 = (tmp & ~param) | (param & data);
                             EFUN(_merlin_mptwo_wrw_uc_var(ma,addr,tmp2));
                             EFUN_PRINTF(("Glob RAM RMW word: x%04x = x%04x -> x%04x\n",addr,tmp,tmp2));
                             } break;
    case SRDS_UC_CMD:        {
                             uint16_t tmp;
                             EFUN(merlin_mptwo_pmd_uc_cmd_with_data(ma,(enum srds_pmd_uc_cmd_enum)addr, (uint8_t)param, data, 100));
                             ESTM(tmp = rd_uc_dsc_data());
                             EFUN_PRINTF(("uC Command: cmd=x%02x supp=x%02x data=x%04x returned=x%04x\n",addr,param,data,tmp));
                             } break;
    case SRDS_BER_PROJ_DATA: {
                             /* display ber projections for all channels */
                             EFUN(_merlin_mptwo_display_ber_scan_data(ma,(uint8_t)addr, (uint8_t)data,  (uint8_t)(param>>4)));
                             } break;
...
err_code_t merlin_mptwo_read_event_log(merlin_access_t *ma, uint8_t *trace_mem,enum srds_event_log_display_mode_enum display_mode) {
    ...
    EFUN(merlin_mptwo_event_log_stop(ma));

    EFUN(merlin_mptwo_event_log_readmem(ma,trace_mem));

    EFUN(merlin_mptwo_event_log_display(ma,trace_mem, display_mode));
...
err_code_t merlin_mptwo_event_log_stop(merlin_access_t *ma) {
    ...
    EFUN(merlin_mptwo_pmd_uc_cmd(ma,CMD_EVENT_LOG_READ, CMD_EVENT_LOG_READ_START, 10));
...
err_code_t merlin_mptwo_event_log_readmem(merlin_access_t *ma, uint8_t *trace_mem) {
    ...
    do {
        /* Read Next */
        EFUN(merlin_mptwo_pmd_uc_cmd(ma,CMD_EVENT_LOG_READ, CMD_EVENT_LOG_READ_NEXT, 10));
    ...
    EFUN(merlin_mptwo_pmd_uc_cmd(ma,CMD_EVENT_LOG_READ, CMD_EVENT_LOG_READ_DONE, 10));
...
err_code_t merlin_mptwo_display_state (merlin_access_t *ma) {
    EFUN(merlin_mptwo_display_core_state(ma));
    EFUN(merlin_mptwo_display_lane_state_hdr());
    EFUN(merlin_mptwo_display_lane_state(ma));
...
err_code_t merlin_mptwo_display_config (merlin_access_t *ma) {
    EFUN(merlin_mptwo_display_core_config(ma));
    EFUN(merlin_mptwo_display_lane_config(ma));
    return(ERR_CODE_NONE);
}
...
err_code_t merlin_mptwo_force_die_temperature (merlin_access_t *ma, int16_t die_temp) {
...
err_code_t merlin_mptwo_read_die_temperature (merlin_access_t *ma, int16_t *die_temp) {

    uint16_t die_temp_sensor_reading;

    EFUN(merlin_mptwo_pmd_uc_cmd(ma,CMD_UC_DBG,CMD_UC_DBG_DIE_TEMP,100));
...
err_code_t merlin_mptwo_read_die_temperature_double (merlin_access_t *ma, USR_DOUBLE *die_temp) {
#ifdef SERDES_API_FLOATING_POINT
    /* Formula for PVTMON: T = 410.04-0.48705*reg10bit (from PVTMON Analog Module Specification v5.0 section 6.2) */
    uint16_t die_temp_sensor_reading;

    EFUN(merlin_mptwo_pmd_uc_cmd(ma,CMD_UC_DBG,CMD_UC_DBG_DIE_TEMP,50));
    ESTM(die_temp_sensor_reading=rd_uc_dsc_data());
