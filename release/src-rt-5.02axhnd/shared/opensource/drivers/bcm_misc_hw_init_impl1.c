    ...
    uint32_t *mc_base_addr, uint32_t *mc_base_addr_phys, uint32_t *tm_size, uint32_t *mc_size)
{
    ...
    rc = BcmMemReserveGetByName(TM_BASE_ADDR_STR, (void**)tm_base_addr, tm_size);
    if (rc)
    {
    ...
    else
    {
        ...
        else
        {
            ...
            *tm_base_addr_phys = (uint32_t)virt_to_phys((void*)*tm_base_addr);

            printk("tm_base_addr 0x%p, size %u, tm_base_addr_phys 0x%p\n",
                (void*)*tm_base_addr, *tm_size, (void*)*tm_base_addr_phys);

            *mc_base_addr_phys = (uint32_t)virt_to_phys((void*)*mc_base_addr);
