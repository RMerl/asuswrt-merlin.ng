...
static int merlin_direct_lane_init(E_MERLIN_LANE lane_id, LPORT_PORT_MUX_SELECT mux_sel)
{
    ...
    case PORT_SGMII_AN_MASTER:
    case PORT_SGMII_AN_SLAVE:
        tx_cfg = (merlin_tx_cfg_t){ 0, 40, 0, 0, 0 };
        break;
    case PORT_SGMII_AN_IEEE_CL37:
    case PORT_SGMII_AN_USER_CL37:
        tx_cfg = (merlin_tx_cfg_t){ 2, 44, 11, 2, 0 };
        break;
    case PORT_HSGMII:
        tx_cfg = (merlin_tx_cfg_t){ 0, 60, 0, 0, 4 };
        break;
    case PORT_SFI:
        tx_cfg = (merlin_tx_cfg_t){ 2, 44, 11, 2, 0 };
        break;
    case PORT_XFI:
        tx_cfg = (merlin_tx_cfg_t){ 0, 46, 8, 0, 0 };
        break;
    default:
       return -1;
    }
...
static int merlin_direct_tx_cfg_set(E_MERLIN_LANE lane_index, merlin_tx_cfg_t *tx_cfg)
{
    ...
    else if ((main > 60) || (main < 0))
        MERLIN_ERR("ERR_CODE_TXFIR_MAIN_INVALID\n");

    else if (((post2 != 0) && (post1 > 18)) || (post1 < 0))
        MERLIN_ERR("ERR_CODE_TXFIR_POST1_INVALID\n");

    else if (((post2 == 0) && (post1 > 23)) || (post1 < 0))
        MERLIN_ERR("ERR_CODE_TXFIR_POST1_INVALID\n");

    else if (post2 > 5)
        MERLIN_ERR("ERR_CODE_TXFIR_POST2_INVALID\n");

    else if (main < (pre + post1 + post2 + 1))
        MERLIN_ERR("ERR_CODE_TXFIR_V2_LIMIT\n");

    else if ((pre + main + post1 + post2) > 60)
        MERLIN_ERR("ERR_CODE_TXFIR_SUM_LIMIT\n");

    else if ((post2 != 0) && ((pre + post1) > 22))
        MERLIN_ERR("ERR_CODE_TXFIR_PRE_POST1_SUM_LIMIT\n");

    else if ((post2 == 0) && ((pre + post1) > 27))
        MERLIN_ERR("ERR_CODE_TXFIR_PRE_POST1_SUM_LIMIT\n");

    else if ((hpf < 0) || hpf > 15)
        MERLIN_ERR("ERR_CODE_HPF_VALUE\n");
    ...
    MERLIN_WRITE_REG(1, lane_index, 0xd0a5, MASK_BIT(15), 15, (post2 == 0));
    MERLIN_WRITE_REG(1, lane_index, 0xd0a5, MASK_BIT(14), 14, (pre > 0));
    MERLIN_WRITE_REG(1, lane_index, 0xd0a5, MASK_BIT(13), 13, (post1 > 0));
    MERLIN_WRITE_REG(1, lane_index, 0xd0a5, MASK_BIT(12), 12, (post2 > 0));
    MERLIN_WRITE_REG(1, lane_index, 0xd110, MASK_BITS_BETWEEN(0, 3), 0, (uint8_t)pre);
    MERLIN_WRITE_REG(1, lane_index, 0xd111, MASK_BITS_BETWEEN(0, 5), 0, (uint8_t)main);
    MERLIN_WRITE_REG(1, lane_index, 0xd110, MASK_BITS_BETWEEN(5, 9), 5, (uint8_t)post1);
    MERLIN_WRITE_REG(1, lane_index, 0xd111, MASK_BITS_BETWEEN(7, 10), 7, (uint8_t)post2);
    MERLIN_WRITE_REG(1, lane_index, 0xd0a2, MASK_BITS_BETWEEN(0, 3), 0, (uint8_t)hpf);
    ...
    MERLIN_WRITE_REG(1, lane_index, 0xd0a3, MASK_BITS_BETWEEN(2, 5), 2, reg_val);
...
static int merlin_direct_tx_cfg_get(E_MERLIN_LANE lane_index, merlin_tx_cfg_t *tx_cfg)
{
    ...
    MERLIN_READ_REG(1, lane_index, 0xd0a5, MASK_BIT(15), 15, &reg_val);
    MERLIN_INFO("\nconvert post2 slices to post1 %d\n", reg_val);
    MERLIN_READ_REG(1, lane_index, 0xd0a5, MASK_BIT(14), 14, &reg_val);
    MERLIN_INFO("pre   en %d\n", reg_val);
    MERLIN_READ_REG(1, lane_index, 0xd0a5, MASK_BIT(13), 13, &reg_val);
    MERLIN_INFO("post1 en %d\n", reg_val);
    MERLIN_READ_REG(1, lane_index, 0xd0a5, MASK_BIT(12), 12, &reg_val);
    MERLIN_INFO("post2 en %d\n", reg_val);
    MERLIN_READ_REG(1, lane_index, 0xd0a3, MASK_BITS_BETWEEN(2, 5), 2, &reg_val);
    MERLIN_INFO("refcalshunt %d\n", reg_val);
