...
typedef struct
{
   ...
   UINT16  ownerId;               /**< Instance Id of the controller execution unit who created this node. */ 
} MdmNodeAttributes;
...
/** Return TRUE if the oid object is an instance node.
 *
 * @param oid (IN) Mdm Object Id
 * @return TRUE if the oid object is an instance node.
 *         FALSE otherwise
 */
UBOOL8 cmsMdm_isInstanceNode(MdmObjectId oid);

/** Return the generic fullpath given the MdmPathDescriptor.
 *
 * @param pathDesc (IN) MdmPathDescriptor to convert.
 * @param genericFullpath (OUT) This will be a generic object or parameter name.
 *                       the user is responsible for freeing the fullpath.
 * @return CmsRet enum.
 */
CmsRet cmsMdm_pathDescriptorToGenericFullPath(const MdmPathDescriptor *pathDesc,
                                              char **genericFullpath);

/** Return the path descriptor of a subtree root node given the
 *  path descriptor of a member of the subtree.
 *
 * @param pathDesc (IN) MdmPathDescriptor of the subtree member.
 * @param rootPathDesc (OUT) MdmPathDescriptor of the subtree root node.
 * @return CmsRet enum.
 */
CmsRet cmsMdm_getSubTreeRootPath(const MdmPathDescriptor *pathDesc,
                                 MdmPathDescriptor       *rootPathDesc);

/** Get the next generic parameter node of this object node.
 *
 ...
 * @return CmsRet enum.
 */
CmsRet cmsMdm_getNextGenericParamName(MdmPathDescriptor *pathDesc);
 ...
 * @param rootGenericPath (IN) The generic fullpath of the subtree root node.
 * @param nextPath (IN/OUT) On input, the generic fullpath of the current object node.
 *                          object node. On return, the generic fullpath of the next
 *                          object node in the subtree.
 * @return CmsRet enum.
 */
CmsRet cmsMdm_getNextGenericObjPathInSubtree(const char *rootGenericPath,
