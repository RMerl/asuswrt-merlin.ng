...
typedef enum 
{
   ...
   CMS_MSG_REQUEST_EE_STATE_CHANGE              = 0x10002512, /**< request Execution Environment state change */
   CMS_MSG_RESPONSE_EE_STATE_CHANGE             = 0x10002513, /**< response Execution Environment state change */
...
/** Data body for CMS_MSG_REQUEST_EE_STATE_CHANGE message type.
 *
 */
typedef struct
{
   char operation[BUFLEN_32];   /**< Required: Install, Update, Uninstall, install_at_bootup */
   char URL[BUFLEN_1024];       /**< Optional: Location of the Exec Env to be installed/update */
   char username[BUFLEN_256];   /**< Optional username for file server */
   char password[BUFLEN_256];   /**< Optional password for file server */
   char version[BUFLEN_32];     /**< Required for update or uninstall: Version of the Exec Env */
   char fullPath[BUFLEN_256];   /**< Required for update or uninstall: FullPath to the Exec Env */
   UINT16 reqId;                /**< Optional: track req number */
} EErequestStateChangedMsgBody;


/** Data body for CMS_MSG_RESPONSE_EE_STATE_CHANGE message type.
 *
 */
typedef struct
{
   char operation[BUFLEN_32];   /**< Install, Update, Uninstall, install_at_bootup */
   char version[BUFLEN_32];     /**< Version of the Exec Env */
   char fullPath[BUFLEN_256];   /**< FullPath to the Exec Env */
   int faultCode;               /**< Return error code */
   UINT16 reqId;                /**< Optional: track req number */
} EEresponseStateChangedMsgBody;
