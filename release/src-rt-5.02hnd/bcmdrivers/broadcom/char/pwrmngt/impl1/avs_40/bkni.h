...
static inline uint32_t BREG_Read32_Single(void * c, uint32_t x)
{
   reg_read_count++;
   return(*(volatile uint32_t *)(x));
}
...
#else
// One Single read at init time followed by double reads protects the AVS
// register interface from getting out of sync after a soft reset
static inline uint32_t BREG_Read32(void * c, uint32_t x)
{
   *(volatile uint32_t *)(x);
   return(*(volatile uint32_t *)(x));
}
#define BREG_Read32_Single(c,x) (*(volatile uint32_t *)(x))
