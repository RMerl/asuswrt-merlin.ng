...
#endif
#ifndef PKTQ_MAX_SW
#define PKTQ_MAX_SW             32	/* Maximum software */
#endif
...
struct swpktq {
	PKTQ_COMMON
	struct pktq_prec q[PKTQ_MAX_SW];
	HND_PKTQ_MUTEX_DECL(mutex)
};
...
extern bool pktq_swfull(struct swpktq *pq, int prec);
#else
#define pktq_pavail(pq, prec)	((pq)->q[prec].max - (pq)->q[prec].len)
#define pktq_pfull(pq, prec)	((pq)->q[prec].len >= (pq)->q[prec].max)
#define pktq_swfull(pq, prec)	pktq_pfull(pq, prec)
...
extern bool pktq_sw_full(struct swpktq *pq);
#else
#define pktq_avail(pq)		((int)((pq)->max - (pq)->len))
#define pktq_full(pq)		((pq)->len >= (pq)->max)
#define pktq_sw_full(pq)	pktq_full(pq)
#endif	/* HND_PKTQ_THREAD_SAFE */
...
extern void *pktq_swdeq(struct swpktq *pq, int prec);
extern void *pktq_swenq(struct swpktq *pq, int prec, void *p);
extern void *pktq_swenq_head(struct swpktq *pq, int prec, void *p);
extern void pktq_swflush(osl_t *osh, struct swpktq *pq, int prec, bool dir, ifpkt_cb_t fn, int arg);
extern void pktq_sw_append(struct swpktq *pq, int prec, struct spktq *list);
extern void *pktq_sw_deq(struct swpktq *pq, int *prec_out);
extern bool pktq_sw_init(struct swpktq *pq, int num_prec, int max_len);
extern void pktq_sw_prepend(struct swpktq *pq, int prec, struct spktq *list);
