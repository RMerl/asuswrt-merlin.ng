...
void
ai_d11rsdb_core1_alt_reg_clk_en(si_t *sih)
{
	si_info_t *sii = SI_INFO(sih);
	aidmp_t *smacai = NULL;
	uint origidx = 0;
	...
	/* turn on mac core 1 ioctrl to enable core 1 radio reg accesses */
	bits = R_REG(sii->osh, &smacai->ioctrl);
	W_REG(sii->osh, &smacai->ioctrl, (bits | (SICF_CLOCK_EN)));
	OSL_DELAY(100);
	dummy = R_REG(sii->osh, &smacai->ioctrl);
	BCM_REFERENCE(dummy);
	ASSERT(dummy & SICF_CLOCK_EN);

	si_setcoreidx(sih, origidx);

}
void
ai_d11rsdb_core1_alt_reg_clk_dis(si_t *sih)
{
	si_info_t *sii = SI_INFO(sih);
	aidmp_t *smacai = NULL;
	uint origidx = 0;
	...
	BCM_REFERENCE(sii);
	BCM_REFERENCE(smacai);
	BCM_REFERENCE(origidx);
	BCM_REFERENCE(bits);
	BCM_REFERENCE(dummy);
	...
	/* turn off mac core 1 ioctrl to disable core 1 radio reg accesses */
	bits = R_REG(sii->osh, &smacai->ioctrl);
