 ...
 *
 * $Id: siutils.c 646989 2016-07-01 12:35:27Z $
 */
...
static int gcicoreidx = SI_CC_IDX;
	...
	uint *origidx, void *regs)
{
	...
	if (sii->pub.cccaps & CC_CAP_PMU) {
		if (AOB_ENAB(&sii->pub)) {
			...
			gcicoreidx = si_findcoreidx(&sii->pub, GCI_CORE_ID, 0);
...
si_gci_direct(si_t *sih, uint offset, uint32 mask, uint32 val)
{
	...
	return si_corereg(sih, gcicoreidx, offset, mask, val);
}
...
si_gci_indirect(si_t *sih, uint regidx, uint offset, uint32 mask, uint32 val)
{
	...
	si_corereg(sih, gcicoreidx, OFFSETOF(chipcregs_t, gci_indirect_addr), ~0, regidx);
	return si_corereg(sih, gcicoreidx, offset, mask, val);
}
...
{
	/* gci_input[] */
	return si_corereg(sih, gcicoreidx, OFFSETOF(chipcregs_t, gci_input[reg]), 0, 0);
}
...
{
	/* gci_output[] */
	return si_corereg(sih, gcicoreidx, OFFSETOF(chipcregs_t, gci_output[reg]), mask, val);
}
...
si_gci_gpio_chipcontrol(si_t *sih, uint8 gci_gpio, uint8 opt)
{
	...
	si_corereg(sih, gcicoreidx, OFFSETOF(chipcregs_t, gci_indirect_addr), ~0, ring_idx);
	si_corereg(sih, gcicoreidx, OFFSETOF(chipcregs_t, gci_gpioctl),
...
si_gci_gpio_reg(si_t *sih, uint8 gci_gpio, uint8 mask, uint8 value, uint32 reg_offset)
{
	...
	if (mask || value) {
		/* set operation */
		si_corereg(sih, gcicoreidx, reg_offset, GCIMASK_4B(pos), GCIPOSVAL_4B(value, pos));
	}
	val_32 = si_corereg(sih, gcicoreidx, reg_offset, 0, 0);
...
{
	si_corereg(sih, gcicoreidx, OFFSETOF(chipcregs_t, gci_wakemask),
...
si_gci_gpioint_handler_process(si_t *sih)
{
	...
	si_corereg(sih, gcicoreidx, OFFSETOF(chipcregs_t, gci_indirect_addr), ~0, 0);
	gpio_status[0] = si_corereg(sih, gcicoreidx, OFFSETOF(chipcregs_t, gci_gpiostatus), 0, 0);
	si_corereg(sih, gcicoreidx, OFFSETOF(chipcregs_t, gci_gpiostatus), ~0, ~0);

	si_corereg(sih, gcicoreidx, OFFSETOF(chipcregs_t, gci_indirect_addr), ~0, 1);
	gpio_status[1] = si_corereg(sih, gcicoreidx, OFFSETOF(chipcregs_t, gci_gpiostatus), 0, 0);
	si_corereg(sih, gcicoreidx, OFFSETOF(chipcregs_t, gci_gpiostatus), ~0, ~0);
...
si_gci_handler_process(si_t *sih)
{
	...
	gci_intstatus = si_corereg(sih, gcicoreidx, OFFSETOF(chipcregs_t, gci_intstat), 0, 0);
...
si_gci_seci_init(si_t *sih)
{
	...
	si_gci_indirect(sih, 0,
		OFFSETOF(chipcregs_t, gci_chipctrl), ALLONES_32, 0x0080000); //0x200

	/* config GPIO pins 5/6 as SECI_IN/SECI_OUT */
	si_gci_indirect(sih, 0,
		OFFSETOF(chipcregs_t, gci_seciin_ctrl), ALLONES_32, 0x161); //0x218
	si_gci_indirect(sih, 0,
		OFFSETOF(chipcregs_t, gci_seciout_ctrl), ALLONES_32, 0x10051); //0x21c

	si_gci_direct(sih, OFFSETOF(chipcregs_t, gci_seciout_txen_txbr), ALLONES_32, 0x01); //0x224

	si_gci_indirect(sih, 1,
		OFFSETOF(chipcregs_t, gci_gpioctl), ALLONES_32, 0x00010280); //0x044

	/* baudrate:4Mbps at 40MHz xtal, escseq:0xdb, high baudrate, enable seci_tx/rx */
	si_gci_direct(sih, OFFSETOF(chipcregs_t, gci_secibauddiv), ALLONES_32, 0xF6); //0x1e0
	si_gci_direct(sih, OFFSETOF(chipcregs_t, gci_baudadj), ALLONES_32, 0xFF); //0x1f8
	si_gci_direct(sih, OFFSETOF(chipcregs_t, gci_secifcr), ALLONES_32, 0x00); //0x1e4
	si_gci_direct(sih, OFFSETOF(chipcregs_t, gci_secimcr), ALLONES_32, 0x08); //0x1ec
	si_gci_direct(sih, OFFSETOF(chipcregs_t, gci_secilcr), ALLONES_32, 0xA8); //0x1e8
	si_gci_direct(sih, OFFSETOF(chipcregs_t, gci_uartescval), ALLONES_32, 0xDB); //0x1d0

	/* WLAN rx offset assignment */
	/* WLCX: RX offset assignment from WLAN core to WLAN core (faked as BT TX) */
	si_gci_indirect(sih, 0,
		OFFSETOF(chipcregs_t, gci_secif0rx_offset), ALLONES_32, 0x13121110); //0x1bc
	si_gci_indirect(sih, 1,
		OFFSETOF(chipcregs_t, gci_secif0rx_offset), ALLONES_32, 0x17161514);
	si_gci_indirect(sih, 2,
		OFFSETOF(chipcregs_t, gci_secif0rx_offset), ALLONES_32, 0x1b1a1918);

	/* Atlas/GMAC3 configuration for SECI */
	si_gci_direct(sih, OFFSETOF(chipcregs_t, gci_miscctl), ALLONES_32, 0xFFFF); //0xc54

	/* first 12 nibbles configured for format-0 */
	/* note: we can only select 1st 12 nibbles of each IP for format_0 */
	si_gci_indirect(sih, 0,  OFFSETOF(chipcregs_t, gci_seciusef0tx_reg), //0x1b4
	                ALLONES_32, 0xFFF); // first 12 nibbles

	si_gci_indirect(sih, 0,  OFFSETOF(chipcregs_t, gci_secitx_datatag),
			ALLONES_32, 0x0F0); // gci_secitx_datatag(nibbles 4 to 7 tagged)
	si_gci_indirect(sih, 0,  OFFSETOF(chipcregs_t, gci_secirx_datatag),
	                ALLONES_32, 0x0F0); // gci_secirx_datatag(nibbles 4 to 7 tagged)

	/* TX offset assignment (wlan to bt) */
	si_gci_indirect(sih, 0,
		OFFSETOF(chipcregs_t, gci_secif0tx_offset), 0xFFFFFFFF, 0x76543210); //0x1b8
	si_gci_indirect(sih, 1,
		OFFSETOF(chipcregs_t, gci_secif0tx_offset), 0xFFFFFFFF, 0x0000ba98);

	// HW ECI bus directly driven from IP
	si_gci_direct(sih, OFFSETOF(chipcregs_t, gci_control_0), ALLONES_32, 0x00000000);
	si_gci_direct(sih, OFFSETOF(chipcregs_t, gci_control_1), ALLONES_32, 0x00000000);
}
...
si_gci_chipcontrol(si_t *sih, uint reg, uint32 mask, uint32 val)
{
	...
	si_corereg(sih, gcicoreidx, OFFSETOF(chipcregs_t, gci_indirect_addr), ~0, reg);
	return si_corereg(sih, gcicoreidx, OFFSETOF(chipcregs_t, gci_chipctrl), mask, val);
#else /* NFLASH_SUPPORT */
	ASSERT(0);
	return ALLONES_32;
#endif
...
si_gci_chipstatus(si_t *sih, uint reg)
{
	...
	return si_corereg(sih, gcicoreidx, OFFSETOF(chipcregs_t, gci_chipsts), 0, 0);
#else /* NFLASH_SUPPORT */
	ASSERT(0);
	return ALLONES_32;
#endif
...
BCMATTACHFN(si_gci_chipctrl_overrides)(osl_t *osh, si_t *sih, char *pvars)
{
	...
	/* because NFLASH and GCI clashes in 0xC00 */
#ifndef NFLASH_SUPPORT
	cap1 = si_corereg(sih, gcicoreidx, OFFSETOF(chipcregs_t, gci_corecaps1), 0, 0);
#else /* NFLASH_SUPPORT */
	ASSERT(0);
#endif
	...
	*/
	...
	if (bustype == PCI_BUS) {
		if ((CHIPID(sih->chip) == BCM4331_CHIP_ID) ||
			...
			/* backplane timeout will be disabled after watchdog, re-enable again */
			si_slave_wrapper_add(sih);
#endif
		}
...
#ifdef ATE_BUILD
uint32
si_gpio_intmask_enable(si_t *sih)
{
	si_info_t *sii = SI_INFO(sih);
	uint32 gpiointmask;
	...
	gpiointmask = R_REG(sii->osh, &cc->intmask);
	if (!(gpiointmask & CI_GPIO)) {
		W_REG(sii->osh, &cc->intmask, CI_GPIO | gpiointmask);
		gpiointmask = R_REG(sii->osh, &cc->intmask);
	}
	return gpiointmask;
}
...
int BCMATTACHFN(si_set_sromctl)(si_t *sih, uint32 value)
{
	...
	if (si_corerev(sih) >= 32) {
		/* SpromCtrl is only available if CoreCapabilities.SpromSupported */
