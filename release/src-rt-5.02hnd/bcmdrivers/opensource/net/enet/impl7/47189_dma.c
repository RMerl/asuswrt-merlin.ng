...
extern int bcmPktDma_EthTxEnable_Iudma(BcmPktDma_LocalEthTxDma * txdma);
...
static int alloc_txdma_bds(enetx_port_t *port)
{
    ...
    txdma->txBdsBase = (volatile DmaDesc *)
            bcmPktDma_EthAllocTxBds(&port->dev->dev, 0, nr_tx_bds, &phy_addr);
...
static int init_tx_channel(enetx_port_t *port)
{
    ...
    bcmPktDma_EthTxEnable_Iudma(txdma);
...
static int init_rx_channel(enetx_port_t *port)
{
    ...
    gbpm_resv_rx_buf(GBPM_PORT_ETH, 0, rxdma->pktDmaRxInfo.numRxBds,
