...
#include "rom_parms.h"
#include "memsys_top_api_impl2.h"
#include "bcm_sec.h"
...
static int memc_setup_scrambler(void);
#endif
...
static mcbindex* memc_get_mcb(uint32_t memcfg)
{
    ...
    unsigned int is_4Gb_ddr = ((MISC->miscStrapBus & MISC_STRAP_DDR_DENSITY_MASK) >> MISC_STRAP_DDR_DENSITY_SHIFT) ? 0 : 1;
    unsigned int mcb_index = is_4Gb_ddr + 2*is_32b_ddr;
    mcbindex* pRet = &MCB[mcb_index];
#endif
#endif
    return pRet;
}
...
static int pre_shmoo(memsys_top_params_t *params, void *args)
{
    ...
    print_log("[0x%08x] : 0x%08x\n\r", &(MEMC->PhyByteLane0Control.VDL_CTRL_RD[1]), MEMC->PhyByteLane0Control.VDL_CTRL_RD[1]);
    print_log("[0x%08x] : 0x%08x\n\r", &(MEMC->PhyByteLane1Control.VDL_CTRL_RD[1]), MEMC->PhyByteLane1Control.VDL_CTRL_RD[1]);
    print_log("[0x%08x] : 0x%08x\n\r", &(MEMC->PhyByteLane2Control.VDL_CTRL_RD[1]), MEMC->PhyByteLane2Control.VDL_CTRL_RD[1]);
    print_log("[0x%08x] : 0x%08x\n\r", &(MEMC->PhyByteLane3Control.VDL_CTRL_RD[1]), MEMC->PhyByteLane3Control.VDL_CTRL_RD[1]);
...
//*****************************************
int ddr_init(unsigned long mcb_selector)
{
    mcbindex* pMCB = NULL;
    uint32_t ret;
