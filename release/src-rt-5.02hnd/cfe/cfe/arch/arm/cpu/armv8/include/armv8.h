...
/* For ARMv8 CFE, MMU setup the 33 bit linear virtual to physical address mapping.
   ...
   as physical address. Uncached address is (cached address|0x100000000). Physical
   address is still 32 bit in 4908. This is to mimic the K0 K1 segment behavior in
   the MIPS to keep the cfe code compatiable with ARM environment. 
*/

#define cache_to_uncache(va) ((uint64_t)(va) | 0x100000000L)
#define uncache_to_cache(va) ((uint64_t)(va) & ~(0x100000000L))

/* There is no k0 and K1 conception in arm. Definition them here for code compatiblity only*/
#define PHYS_TO_K0(pa)	((uint64_t)(pa))
#define PHYS_TO_K1(pa)	cache_to_uncache((uint64_t)(pa))
#define K0_TO_PHYS(va)  (uint32_t)((va))
#define K1_TO_PHYS(va)	(uint32_t)uncache_to_cache((va))
