...
void set_reserved_memory(void)
{
    ...
    int rdp_size[2], dhd_size[3], i, rdp_def_size, chk_dt_dhd;
    uint8_t bp_dhd_size;
    ...
    if (!dtb_getprop_reg(DT_RSVD_NODE_STR, dt_node_name,
                          &adsl_base_addr,  &adsl_rsv_size)) {
        if (adsl_rsv_size && adsl_rsv_size < mem_end) {
              rsrv_mem_required += adsl_rsv_size;
              mem_end -= adsl_rsv_size;
              dtb_set_reserved_memory(dt_node_name, mem_end, adsl_rsv_size);
        } else {
              dtb_del_reserved_memory(dt_node_name);        
        } 
    }
    ...
    for( i = 0; i < 2; i++ ) {
        ...
        if (!dtb_getprop_reg(DT_RSVD_NODE_STR, dt_node_name,
            ...
            if( dt_rdp_size > rdp_size[i] ) {
                ...
                printf("Please update nvram setting in CFE if possible!\n");
                rdp_size[i] = dt_rdp_size;
            }
        }

        /* Make sure the final values are larger than minimum required 
         * in case both device tree and nvram value are smaller than default
         */
        if( i == 0 )
            rdp_def_size = RDP_PARAM1_DEF_DDR_SIZE;
        else
            rdp_def_size = RDP_PARAM2_DEF_DDR_SIZE;
        if (rdp_size[i] < rdp_def_size )
            rdp_size[i] = rdp_def_size;
    }
    ...
    for( i = 0; i < 2; i++ ) {
        sprintf(dt_node_name, "%s%s", DT_RSVD_PREFIX_STR, rdp_str[i]);
        if (!dtb_set_reserved_memory(dt_node_name, ALIGN_FLR(SZ_2M, mem_end - rdp_size[i]), rdp_size[i])) {
    ...
    for( i = 0; i < 3; i++ ) {
        ...
        if( dhd_size[i] != 0 ) {
            sprintf(dt_node_name, "%s%s%d", DT_RSVD_PREFIX_STR, "dhd", i);
            if (!dtb_set_reserved_memory(dt_node_name, ALIGN_FLR(SZ_2M, mem_end - dhd_size[i]), dhd_size[i])) {
    ...
    if (!dtb_getprop_reg(DT_RSVD_NODE_STR, dt_node_name,
        ...
        if (plc_rsv_size && plc_rsv_size < mem_end) {
              ...
              dtb_set_reserved_memory(dt_node_name, mem_end, plc_rsv_size);
        } else {
              dtb_del_reserved_memory(dt_node_name);        
        }
    }
...
void dtb_install(void)
{
    ...
    dtb_set_memory_size(cfe_get_sdram_size());
    ...
    if( dtb_get_cpu_rel_addr(&rel_addr, 1) == 0 )
    {
    ...
    dtb_set_nr_cpus();
#endif
    if (dtb_set_blparms(blparms_buf, blparms_len)) {
...
#if defined(CFG_DT)
extern unsigned long fmw_arg0;
    if (dtb_get_prepare_status() == DTB_PREPARE_NONE) {
        dtb_prepare(DTB_PREPARE_FDT_DEF, NULL, 0);
    }
    dtb_install();
    if (loadaddr) {
        fmw_arg0 = (unsigned long)dtb_get_fdt();
    }  
#elif defined(CFG_ATAG)
    atag_install();
#else
