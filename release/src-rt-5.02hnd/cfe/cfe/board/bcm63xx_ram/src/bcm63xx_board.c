...
#if defined(_BCM96838_) || defined(_BCM963138_) || defined(_BCM963148_) || defined(_BCM96848_) || defined(_BCM94908_) || defined(_BCM96858_) || defined(_BCM968360_)
...
static PARAMETER_SETTING gBoardParam[] =
{
    ...
    {"Base MAC Address                  :", MAC_ADDR_PROMPT, "", "", 17,
        parseMacAddr, TRUE},
    {"PSI Size (1-128) KBytes           :", PSI_SIZE_PROMPT, "", "", 3,
        parsePsiSize, TRUE},
...
static PARAMETER_SETTING gAllocBoardParam[] =
{
    {"RNR_TBLS memory allocation (MB)         :", NULL,
        "", "", 2, parseParam2Size, TRUE},
...
static int parseParam2Size(char *ctStr)
{
    ...
    if (percent >= 0 && percent <= MAX_RDP_PARAM2_SIZE)
        return 0;
    else
        return 1;
}
...
static void getAllocBoardParam(void)
{
    ...
    sprintf(gAllocBoardParam[PARAM_IDX_ALLOC_RDP_PARAM2].parameter,"%d", NVRAM.allocs.alloc_rdp.param2_size);
...
void displayBoardParam(void)
{
    ...
    if((flash_get_flash_type() == FLASH_IFC_NAND) || (flash_get_flash_type() == FLASH_IFC_SPINAND)) {
        ...
        if(NVRAM.part_info[3].size == 0xffff ) {
            printf("%s %dMB  \n", gPartitionsParam[3].promptName, NAND_DATA_SIZE_KB/1024);
        }
        else {
