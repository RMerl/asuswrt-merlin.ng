...
#if defined (_BCM96838_) || defined(_BCM96858_) || defined(_BCM968360_)
#include "clk_rst.h"
#endif

#if defined(_BCM963148_) || defined(_BCM94908_) || defined(_BCM96858_) || defined(_BCM968360_)
#include "pmc_drv.h"
#endif
...
#if defined (_BCM96328_) || defined (_BCM96362_) ||  defined (_BCM963268_) ||     defined (_BCM96318_) ||      defined (_BCM96838_) ||       defined (_BCM960333_) ||        defined (_BCM963381_) || defined (_BCM96848_) || defined (_BCM96858_) || defined(_BCM968360_)
static int cfe_bus_speed = 0;
static int cfe_ddr_speed = 0;
#endif
#if defined (_BCM96838_) || defined (_BCM96848_) || defined (_BCM96858_) || defined(_BCM968360_)
...
const uint32 ddr_speed_table[2] = {
    533, 800
};
const uint32 bus_speed_table[2] = {
    214, 428
...
#if defined(_BCM96318_) || defined (_BCM960333_) || (_BCM963381_)
	int bSDR = 0;
...
#endif
#if defined (_BCM96328_) || defined (_BCM96362_) ||  _BCM963268_ ||     defined (_BCM96838_) || defined (_BCM96848_) || defined (_BCM96858_) || defined (_BCM968360_)
...
#elif defined (_BCM963268_) ||  defined (_BCM96838_)
...
#if !defined(_BCM963138_) && !defined(_BCM963148_) && !defined(_BCM94908_) && !defined(_BCM96858_) && !defined(_BCM947189_) && !defined(_BCM968360_)
...
void cfe_main(int a,int b)
{
...
#if !defined (_BCM960333_) && !defined (_BCM96318_) && !defined(_BCM96838_) && !defined(_BCM947189_)
    xprintf("Boot Strap Register:  0x%x\n", MISC->miscStrapBus);
#else
#if defined (_BCM960333_) || defined (_BCM96318_)
    ...
    {
#if defined (_BCM963138_) || defined(_BCM963148_) || defined(_BCM947189_)
...
void static calculateCpuSpeed(void)
{
    ...
    cfe_bus_speed = ((ddr_speed_table[sdr_freq_strap]+1)/2) * 1000000;
...
//    ddr_freq_straps |= (MEMC->PhyControl.STRAP_CONTROL & PHY_CONTROL_REGS_STRAP_STATUS_SPEED_HI_MASK) >> (PHY_CONTROL_REGS_STRAP_STATUS_SPEED_HI_STRT-2);
    ...
    /* we need to initializa PMC before running cfe_set_cpu_freq */
    pmc_init();
    ...
    else
    {
        ...
        if ( !bcm_otp_get_cpu_clk(&clk_index) )
            cfe_cpu_speed = cpu_speed_table[clk_index];
        else
            cfe_cpu_speed = 0;
    }
    pmc_initmode();
...
#else
    unsigned int clk_index;
// FIXMET: unmark when pmc driver is added
//    get_rdp_freq(&cfe_rdp_speed);
    if ( !bcm_otp_get_cpu_clk(&clk_index) )
        cfe_cpu_speed = cpu_speed_table[clk_index];
    else
        cfe_cpu_speed = 0;
#endif
    cfe_ddr_speed = ddr_speed_table[(MISC->miscStrapBus & MISC_STRAP_DDR_FREQ_MASK) >> MISC_STRAP_DDR_FREQ_SHIFT] * 1000000;
    cfe_bus_speed = bus_speed_table[1] * 1000000;
#elif defined (_BCM963381_)
...
#if defined(_BCM96328_) || defined (_BCM96362_) || defined(_BCM96828_)

    mem_totalsize = (DDR->CSEND << 14);
