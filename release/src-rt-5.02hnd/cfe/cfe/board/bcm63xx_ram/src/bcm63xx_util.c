...
#if defined(_BCM963138_) || defined(_BCM963148_) || defined(_BCM96858_) || defined(_BCM94908_) || defined(_BCM968360_)
#include "pmc_drv.h"
#include "BPCM.h"
#endif
#include "bcm_ubi.h"
...
static int checkChipId(int tagChipId, char *sig2)
{
    ...
    {
        if ((chipId == 0x6811) || (chipId == 0x6815) || (chipId == 0x6817)) {
            chipId = 0x6818;
        } else {
            chipId = 0x6816;
            ...
            if ((revId & 0xf0) == 0xA0)
            {
                chipId = 0x6818;  /* FIXME - do we need additional checks */
            }
        }
    }

    /* Force BCM6369 to be BCM6368) */
    if( (chipId & 0xfffe) == 0x6368 )
        chipId = 0x6368;
    ...
    /* Force BCM6828 variants to be BCM6828) */
    if (( chipId == 0x6821 )
     ||  (chipId == 0x6822 ))
        chipId = 0x6828;

    /* Force BCM6319 to be BCM6318 */
    if (chipId == 0x6319)
         chipId = 0x6318;
...
int getDtbFromTag(unsigned char** dtbBuf, unsigned int* size)
{
    ...
    unsigned char* buf = (unsigned char*) cfe_get_mempool_ptr(); 
    unsigned int avail_size = (unsigned int) cfe_get_mempool_size(); 
    ...
    if (avail_size < dtbSize) {
        return -3;
    }
    pDtbFlashAddr = (uint32_t*)(uintptr_t)(atoi(pTag->dtbAddress) + BOOT_OFFSET + IMAGE_OFFSET);
    ret = kerSysReadFromFlash( buf, (uintptr_t)pDtbFlashAddr, dtbSize);
    if( ret != dtbSize )  {
        return -4;
    }
    /* Check dtb CRC */
    ...
    if( dtbCrc )
    {
        ...
        ulCrc = getCrc32((unsigned char *) buf, dtbSize, ulCrc);      
        if( ulCrc != dtbCrc)
        {
...
PFILE_TAG getTagFromPartition(int imageNumber)
{
    ...
    defined(_BCM963381_) || defined(_BCM963138_) || defined(_BCM963148_) || \
    ...
    /* For these targets, the secure boot could be mfg mode or field mode. */
	   ...
	   if (bcm_otp_is_boot_mfg_secure())
            ...
            NVRAM.ulNandPartSizeKb[rootfs] < ((num_blks * len) / 1024) )
        {
            ...
            {
                if( (read_len = flash_read_buf(i, 0, buf, 512)) > 0 )
                {
...
int findBootImageDirEntry(int state)
{
        ...
        NVRAM.ulNandPartOfsKb[NP_DATA] < ((num_blks * len) / 1024) )
    {
        ...
        end_blk =
            start_blk + (NVRAM.ulNandPartSizeKb[NP_DATA] / (len / 1024));
        ...
        for( i = start_blk, done = 0; (i < end_blk) && (done == 0); i++ )
        {
            if( (read_len = flash_read_buf(i, 0, buf, 512)) > 0 )
            {
                ...
                {
                    KFREE(buf);
                    return (state == -1) ? BOOT_SET_NEW_IMAGE : 0; // if pureUBI boot latest image as cferom determined the cferam to boot, else show success that the boot_state was set correctly (this is now handled in Linux)
                }
                else
                    read_len = 0;
            }
                            ...
                            sizeof(struct jffs2_unknown_node) - 4, 0) )
                    {
                            ...
                            je32_to_cpu(pdir->ino) != 0 )
                        {
                            if( state == -1 )
                                 ...
                                 */
                                ret = (int) pdir->name[fname_cmp_len];
                            else
                            {
...
int findBootBlock()
{
     ...
     struct ubi_ec_hdr *ec;
     int read_len;
     unsigned char *buf = KMALLOC(1024, sizeof(void*));
     if (!buf) {
         return -1;
     }
     start_blk = 1;
    ...
    /* Find the directory entry. */
     ...
     {
         if( flash_dev_specific_cmd(CHECK_BAD_BLOCK, &i, NULL) == 0 && (read_len = flash_read_buf(i, 0, buf, 512)) > 0 )
         {
             pdir = (struct jffs2_raw_dirent *) buf;
             ec = (struct ubi_ec_hdr *) buf;
...
PFILE_TAG getBootImageTag(void)
{
    ...
    if(( flash_get_flash_type() !=  FLASH_IFC_NAND ) && ( flash_get_flash_type() !=  FLASH_IFC_SPINAND ))
    {
        ...
        if( pTag1 && pTag2 )
        {
            ...
            if( kerSysGetBootImageState() == BOOT_SET_NEW_IMAGE_ONCE )
    ...
    if( seq != -1 )
    {
        ...
        for(p = imagePtr; (p < imagePtr+imageSize) && (done == 0); p += len)
        {
            ...
            { // will only need to search through the blocks once since the image is built with volumes in order
                ...
                printf("Setting UBI image sequence number to %d and committing image\n", seq);
                ...
                for (i = 0; i < 2; i++)
                {
                    ...
                    if (parse_ubi(imagePtr, buffer, (p - imagePtr) / len, imageSize / len, len, try, fname, seqstr, 0, 0, readBlk, writeBlk, 0, 0, 0) == 3)
                    {
...
// FIXMET: Add for 68360 when pmc driver enabled */
...
int getBootLine(int setdef)
{
    ...
    if( rc == 0 ) {
        if( (i = kerSysGetBootImageState()) != -1 ) {
            if( i == BOOT_SET_NEW_IMAGE_ONCE )
                i = BOOT_SET_NEW_IMAGE;
            gBootParam[PARAM_IDX_BOOT_IMAGE].parameter[0] = (char) (i & 0xff);
        } else {
            kerSysSetBootImageState((int)
                gBootParam[PARAM_IDX_BOOT_IMAGE].parameter[0]);
        }

#if !defined(_BCM947189_)
        if (cfe_mailbox_status_isset()) {
           unsigned int ver = cfe_mailbox_message_get(); 
...
{
    unsigned char* afe = KMALLOC(sizeof(NVRAM.afeId), sizeof(void*));
    if (!afe) {
        return -1;
    }
...
int cfe_set_cpu_freq(int freqMHz)
{
    ...
    ReadBPCMRegister(PMB_ADDR_B15_PLL, PLLBPCMRegOffset(ch01_cfg), &pll_ch01_cfg.Reg32);
    cfe_usleep(1);
    if (pll_ch01_cfg.Bits.mdiv0 != mdiv0) {
        pll_ch01_cfg.Bits.mdiv0 = mdiv0;
        WriteBPCMRegister(PMB_ADDR_B15_PLL, PLLBPCMRegOffset(ch01_cfg), pll_ch01_cfg.Reg32);
        cfe_usleep(1);
    ...
    /* poll for PLL LOCK before we set Post Reset */
    do {
        ReadBPCMRegister(PMB_ADDR_B15_PLL, PLLBPCMRegOffset(stat), &stat_reg.Reg32);
...
unsigned long convert_to_data_partition_entry_to_bytes(uint16 size)
{
...
static int ubi_find_file(unsigned char* blk_buf,  
                        ...
                        const char* fname, unsigned char** dst, unsigned int *dst_size)
{
        int res = -1;
        void* dst_buf = NULL;
        ...
        res = parse_ubi(0, blk_buf, blk, blk + bcnt, blk_sz, VOLID_UBIFILES, 
                    (char*)fname, (char*)dst_buf, 0, 0, read_blk_cb, 0, 0, 0, 0); 
        if ( res ) {
           *dst = dst_buf;
           *dst_size = res;
        }
...
{ 
    ...
    switch(cfe_fs_get_dentry_type(de, blk_sz, blk, bcnt, &bstart))
    {
             ...
             case CFE_FS_IMAGE_UBI:
                  res = ubi_find_file(de, bstart, bcnt, blk_sz, fname, dst, dsize);
                  break;
             default:
                  break;
    }
               ...
               unsigned char** file, unsigned int* file_size)
{
     ...
     if (!get_rootfs_offset(bootInfo.bootPartition, &bstart, &bcnt)) {
         printf("Error: can't find rootfs partition\n");
         return -1;
     }

     if (cfe_fs_find_file(fname, fnsize, bstart, bcnt, file, file_size, 0)) {
         printf("Error locating %s image\n",fname);
         return -1;
     }
