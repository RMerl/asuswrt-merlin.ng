...
static void reset_rx_dma(bcmenet_softc *softc);
...
static void eth_configure_phy(int ethcore, uint8_t phyaddress, int link_cfg);
                                 ...
                                 unsigned long probe_b, void *probe_ptr)
{
    ...
    if (softc == NULL) {
        ...
        /*
         * softc->dmaPort will codify the Ethernet core number (0,1). Passed in
         * probe_a from cfe_add_device(&bcm47189_enet, [0], 0, 0);
         */
        softc->dmaPort = probe_a;
...
static int bcm47189_ether_inpstat(cfe_devctx_t *ctx, iocb_inpstat_t *inpstat)
{
    ...
    /* Check for a Rx FIFO overflow condition. Reset RX DMA channel if needed */
    if (misc_regs->intstatus & I_RFO) {
        reset_rx_dma(softc);
    }
...
static int bcm47189_ether_read(cfe_devctx_t *ctx, iocb_buffer_t *buffer)
{
    ...
    uint8_t *srcptr;
    volatile DmaDesc *CurrentBdPtr;
    uint8_t *nextbd = (uint8_t *)((softc->rxDma->addrlow & 0xffff0000) |
                                  (softc->rxDma->status0 & 0xFFFF));

    /*
     * nextbd contains the address of the next BD the DMA will use for a RX data
     * transfer (status0), so the last received buffer is pointed to by the
     * previous BD in the ring.
     */
    if ((nextbd - sizeof(DmaDesc)) < (uint8_t *)softc->rxDma->addrlow)
        /* status0 points to the first BD, we want the last one then */
        CurrentBdPtr = (DmaDesc *)K1_TO_PHYS((uint32_t)softc->rxLastBdPtr);
    else
        CurrentBdPtr = (DmaDesc*)(nextbd - sizeof(DmaDesc));
    ...
    /* Make the next BD in the RX ring available */
    if (softc->rxDma->ptr == ((uint32_t)(softc->rxLastBdPtr) & 0xFFFF)) {
        softc->rxDma->ptr = K1_TO_PHYS((uint32_t)softc->rxBds);
    }
    else {
...
static int ether_gphy_reset(bcmenet_softc* softc)
{
    ...
    if (BpGetPhyResetGpio(softc->dmaPort,/* 0,*/ &reset) != BP_SUCCESS)
    {
...
static void init_dma(bcmenet_softc *softc)
{
    ...
    /*
     * Post one RX descriptor to begin with. CFE is expected to read one buffer
     * at a time. After a buffer has been received and read, a new BD in the
     * ring will be made available for the RX DMA.
     *
     *     (1)   (2)                 (3)
     *      |     |                   |
     *      v     v                   v
     ...
     *      ---------------------------------
     *
     * addrlow always points to the start of the ring (1).
     * The first available descriptor is the first descriptor in the ring so,
     * initially, ptr points to the descriptor _after_ the fist one (2).
     *
     * After a buffer has been received, (status0 & DMA_CURRENT_DESCR) advances
     * to the address of the next available BD. The DMA will keep transferring
     * buffers until (status0 & DMA_CURRENT_DESCR) reaches ptr.
     *
     * When (status0 & DMA_CURRENT_DESCR) reaches ptr and both are at (3), ptr
     * has to be set at (1) in order for the DMA to use the last BD (EOT) and
     * wrap around.
     *
     * This means that, while the ring is 16 BDs long, the DMA will be allowed
     * to receive only one packet at a time. Receiving more than one packet will
     * trigger a FIFO overflow error and a channel reset will be necessary.
     */
    softc->rxDma->ptr = (uint32)K1_TO_PHYS((uintptr_t)(&softc->rxBds[1]));
}


/*
 * Reset the DMA RX channel (softc->rxDma). After reset, ptr will be one
 * descriptor ahead, allowing one packet reception.
 */
static void reset_rx_dma(bcmenet_softc *softc)
{
    /* Disable, wait for status0.state == 0 and enable */
    softc->rxDma->control &= ~DMA_EN;
    while (softc->rxDma->status0 & DMA_STATE_MASK) ;
    softc->rxDma->control |= DMA_EN;

    /* Advance rxDma->ptr to the next BD */
    if (softc->rxDma->ptr == ((uint32_t)(softc->rxLastBdPtr) & 0xFFFF)) {
        softc->rxDma->ptr = K1_TO_PHYS((uint32_t)softc->rxBds);
    }
    else {
...
static void gphy_init(int ethcore, ETHERNET_MAC_INFO EnetInfo)
{
    ...
    eth_configure_phy(ethcore, phyaddr, 0);
}
