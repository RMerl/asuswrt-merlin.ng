...
static int bcm6xxx_impl2_ether_open(cfe_devctx_t *ctx)
{
    ...
    rc = rdp_cpu_ring_create_ring(BL_LILAC_RDD_CPU_RX_QUEUE_0, rdpa_ring_data, 32,&ring_head, BCM_PKTBUF_SIZE, NULL);
    if(rc)
    {
    ...
    rc = rdd_ring_init(BL_LILAC_RDD_CPU_RX_QUEUE_0, 0, VIRT_TO_PHYS(ring_head),
                        32,sizeof(RDD_CPU_RX_DESCRIPTOR_DTS),BL_LILAC_RDD_CPU_RX_QUEUE_0,0);
    if(rc)
    {
