...
uint32_t getNumBootBlks(const NVRAM_DATA* pNvramData)
{
    ...
    unsigned long data_partition_size = NAND_DATA_SIZE_KB;
...
int validateNandPartTbl(uint32_t frcUpdt, uint32_t numBtBlks)
{
    ...
    int i, res = 1;
    unsigned long extra=0, data_partition_size=NAND_DATA_SIZE_KB;
     ...
     * */
    if((((NVRAM.ulNandPartSizeKb[NP_DATA] != data_partition_size) && (NAND_DATA_SIZE_KB != NVRAM.ulNandPartSizeKb[NP_DATA])) ||
...
#if defined(_BCM96838_) || defined(_BCM963268_) || defined(_BCM963381_) || defined(_BCM963138_) || defined (_BCM963148_) || defined(_BCM94908_) || defined(_BCM96858_) || defined(_BCM968360_)
	...
	ulRootfsSizeKb = ((NVRAM.ulNandPartOfsKb[NP_DATA] - extra -
        ...
        if( (rootfs == NP_ROOTFS_2) && ulOldRfs2SizeKb &&
            ...
            ulOldDataSizeKb && (ulOldDataSizeKb < NAND_DATA_SIZE_KB) &&
            (ulOldRfs2OfsKb > NVRAM.ulNandPartOfsKb[NP_ROOTFS_2]) )
        {
            ...
            int total_blks =
                blk_start + (NAND_DATA_SIZE_KB - ulOldDataSizeKb) / blk_size;
...
int kerSysSetBootImageState( int state )
{
    /* Rewrites the directory entry with the specified boot state value. */
...
int kerSysGetBootImageState( void )
{
    ...
    /* Get the current boot state stored in a file system directory entry. */
