...
int cfe_mailbox_status_isset(void)
{
    return  (((CFE_MAILBOX_STATUS&CFE_MAILBOX_STATUS_MASK)>>
                  CFE_MAILBOX_STATUS_SHIFT)&CFE_MAILBOX_STATUS_VAL) == CFE_MAILBOX_STATUS_VAL;
}
...
/*
* 31-28 dst id if permissible
* 
* 28-0 payload or all 32 bits  
*/
unsigned int cfe_mailbox_message_get(void)
{
    return  (CFE_MAILBOX_MSG&CFE_MAILBOX_MSG_MASK) >> CFE_MAILBOX_MSG_SHIFT;
}

void cfe_mailbox_message_set(unsigned int val)
{
    CFE_MAILBOX_MSG = 
            (CFE_MAILBOX_MSG&(~CFE_MAILBOX_MSG_MASK))|((val<< CFE_MAILBOX_MSG_SHIFT)&CFE_MAILBOX_MSG_MASK);
}
/*
