...
#include "bcm_common.h"
    ...
    (defined(_BCM963138_) || defined(_BCM963148_) || \
     ...
     defined(_BCM963158_) || defined(_BCM968360_))
...
#if (! defined(_BCM963158_)) && (! defined(_BCM968360_))
    *(uint32_t *)(PKA_BASE + PKA_PERM) = 0;
#endif
#endif

#if (INC_BTRM_BUILD==1) && \
    (defined(_BCM94908_) || defined(_BCM96858_) || defined(_BCM963158_) || defined(_BCM968360_))
    uint32_t binLen = 0;
 ...
 */
    ...
    (! defined(_BCM968360_)) && (! defined(_BCM963158_)) && (INC_BTRM_BOOT==0)
...
uint8_t const* authenticate(uint8_t const* signedObject, int signedSize, uint8_t const* key)
{
    ...
    int const objectSize = signedSize - SEC_S_MODULUS;
    
    // Verify that the signature was made from this object
    SecStatus const rc = Sec_verify((uint32_t*)signedObject, SEC_S_MODULUS,  // The signature we are checking
                                    (uint32_t*)key,          SEC_S_MODULUS,  // The public key we are using to validate the authentication
                                    object,                  objectSize);    // The object that was signed (without the concatenated signature)  
