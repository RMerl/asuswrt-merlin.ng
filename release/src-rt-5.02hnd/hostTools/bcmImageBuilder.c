...
static void build_image(int littleendian, char *chipid, char *board, unsigned int blksize, 
    char *imageversion, char *outputfile, char *cfefile, char *rootfsfile, char *kernelfile, char *dtbfile, int includecfe)
{
    FILE *stream_cfe = NULL;
    FILE *stream_rootfs = NULL;
    FILE *stream_kernel = NULL;
    FILE *stream_dtb = NULL;
    FILE *stream_output = NULL;
    unsigned int total_size, cfe_size, rootfs_size, kernel_size, dtb_size, count, numWritten;
    unsigned int cfeaddr = IMAGE_BASE;
    unsigned int fskerneladdr;
    FILE_TAG tag;
    UINT32 imageCrc, tagCrc, fsCrc, kernelCrc, dtbCrc;
    ...
    if (strlen(rootfsfile) != 0 && strlen(kernelfile) != 0 && strlen(dtbfile) != 0) {
        ...
        if ((stream_dtb = fopen(dtbfile, "rb")) == NULL) {
    ...
    total_size = cfe_size  = rootfs_size = kernel_size = dtb_size = count = 0;
    ...
    if (stream_rootfs && stream_kernel && stream_dtb) {
        if (!includecfe) {
        ...
        while (!feof( stream_dtb )) {
            count = fread( buffer, sizeof(byte), BUF_SIZE, stream_dtb );
            if (ferror( stream_dtb)) {
                perror( "bcmImageBuilder: Read error" );
                exit(1);
            }
            imageCrc = getCrc32(buffer, count, imageCrc);
            dtbCrc = getCrc32(buffer, count, dtbCrc);
            /* Total up actual bytes read */
            total_size += count;
            dtb_size += count;
        }
    ...
    dtbCrc = BCM_HOST_TO_TARGET32(dtbCrc);
    ...
    memcpy(tag.imageValidationToken + (CRC_LEN * 4), (byte*)&dtbCrc, CRC_LEN);
...
int main (int argc, char **argv)
{
    ...
    int option_index = 0;
    char cfefile[256], rootfsfile[256], kernelfile[256], dtbfile[256], outputfile[256];
    ...
    progname = argv[0];
    chipid[0] = boardid[0] = cfefile[0] = kernelfile[0] = rootfsfile[0] = dtbfile[0] = outputfile[0] = '\0';
    ...
    build_image(littleendian, chipid, boardid, blksize, imageversion, outputfile, cfefile, rootfsfile, kernelfile, dtbfile, includecfe);
