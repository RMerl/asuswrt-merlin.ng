...
struct sk_buff *bcm_fkb_to_skb_tcp(FkBuff_t *fkb, struct net_device *dev)
{
	struct sk_buff *skb;
	...
	if (!skb) {
		...
		/* TODO check if we can use skb_dst_set_noref as blog
		 * holds reference */
...
int bcm_tcp_v4_recv(pNBuff_t pNBuff, struct net_device *dev)
{
	...
	if (IS_FKBUFF_PTR(pNBuff)) {
		...
		/* TODO check if we can use skb_dst_set_noref as
		 * blog holds reference */
		dst_hold(fkb->dst_entry);
		skb_dst_set(skb, fkb->dst_entry);
