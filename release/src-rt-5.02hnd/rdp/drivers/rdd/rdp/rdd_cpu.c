...
void rdd_cpu_rx_init(void)
{
    ...
    for (cpu_reason = rdpa_cpu_rx_reason_direct_queue_0, cpu_queue = CPU_RX_QUEUE_ID_0;
        cpu_reason <= rdpa_cpu_rx_reason_direct_queue_7; cpu_reason++, cpu_queue++)
    {
...
int rdd_cpu_rx_meter_drop_counter_get(rdd_cpu_rx_meter cpu_meter, rdpa_traffic_dir direction, uint16_t *drop_counter)
{
    int rdd_error;

    rdd_error = rdd_2_bytes_counter_get(CPU_RX_METERS_DROPPED_PACKETS_GROUP, direction * CPU_RX_METERS_DROPPED_PACKETS_UPSTREAM_OFFSET + cpu_meter, drop_counter);
