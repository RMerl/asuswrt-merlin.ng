...
#ifdef CM3390
    if (ctx->conn_dir == rdpa_dir_ds)
        ctx->l2_hdr_offset -= RDD_DS_IH_PACKET_HEADROOM_OFFSET;
    else
        ctx->l2_hdr_offset -= RDD_US_IH_PACKET_HEADROOM_OFFSET;
#endif
...
void rdd_fc_context_entry_write(rdd_fc_context_t *ctx, RDD_FLOW_CACHE_CONTEXT_ENTRY_DTS *entry)
{
    ...
    RDD_FLOW_CACHE_CONTEXT_ENTRY_CPU_REASON_WRITE(ctx->trap_reason - rdpa_cpu_rx_reason_connection_trap_0, entry);
...
#ifdef CM3390
    if (ctx->conn_dir == rdpa_dir_ds)
        l2_hdr_offset = ctx->l2_hdr_offset + RDD_DS_IH_PACKET_HEADROOM_OFFSET;
    else
        l2_hdr_offset = ctx->l2_hdr_offset + RDD_US_IH_PACKET_HEADROOM_OFFSET;
#else
...
void rdd_ds_lite_tunnel_cfg(bdmf_ipv6_t *ipv6_src_ip, bdmf_ipv6_t *ipv6_dst_ip)
{
    ...
    dual_stack_lite_entry = (RDD_DUAL_STACK_LITE_ENTRY_DTS *)(DEVICE_ADDRESS(RUNNER_PRIVATE_1_OFFSET) + DUAL_STACK_LITE_TABLE_ADDRESS);
...
#ifdef CM3390
void rdd_cmim_pass_through_cfg(bdmf_boolean control)
{
    ...
    RDD_SYSTEM_CONFIGURATION_CMIM_PASS_THROUGH_MODE_WRITE(control, system_cfg);
}
...
void rdd_full_flow_cache_cfg(bdmf_boolean control)
{
    ...
    RDD_SYSTEM_CONFIGURATION_FULL_FLOW_CACHE_MODE_WRITE(control, system_cfg);
}
#endif
