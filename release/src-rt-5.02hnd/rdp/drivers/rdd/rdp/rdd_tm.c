...
#else
       (RDD_FREE_PACKET_DESCRIPTORS_POOL_DESCRIPTOR_ENTRY_DTS *)(DEVICE_ADDRESS(RUNNER_PRIVATE_1_OFFSET) +
#endif
...
int rdd_wan_tx_init(void)
{
    ...
    bbh_tx_wan_channel_index = (uint32_t *)(DEVICE_ADDRESS(RUNNER_PRIVATE_1_OFFSET) +
        BYOI_BBH_TX_WAN_CHANNEL_INDEX_ADDRESS);

    MWRITE_32(bbh_tx_wan_channel_index, RDD_WAN_CHANNEL_1);
#else
    bbh_tx_wan_channel_index = (uint32_t *)(DEVICE_ADDRESS(RUNNER_PRIVATE_1_OFFSET) +
...
void rdd_eth_tx_init(void)
{
    ...
    RDD_FREE_PACKET_DESCRIPTORS_POOL_DESCRIPTOR_ENTRY_DTS *free_packet_descriptors_pool_descriptor;
#if defined(CM3390) || defined(DSL_63138) || defined(DSL_63148) || defined(WL4908)
...
#endif
#ifdef CM3390
    RDD_WAN_CHANNEL_0_7_DESCRIPTOR_WAN_CHANNEL_TYPE_WRITE(wan_channel, wan_channel_descriptor);
#endif
...
#else
    if (wan_channel == RDD_WAN_CHANNEL_0)
    {
        RDD_WAN_CHANNEL_0_7_DESCRIPTOR_BBH_DESTINATION_WRITE((BBH_PERIPHERAL_WAN_TX + (wan_channel << 7)),
            wan_channel_descriptor);
    }
...
#else
    rdd_error = _rdd_cpu_message_send(RDD_CPU_MESSAGE_FLUSH_WAN_TX_QUEUE, RDD_CLUSTER_1,
        ((wan_tx_pointers_entry_ptr->wan_tx_queue_ptr - WAN_TX_QUEUES_TABLE_ADDRESS) / sizeof(RDD_WAN_TX_QUEUE_DESCRIPTOR_DTS)), 0, 0, is_wait);
#endif
    bdmf_fastlock_unlock_irq(&int_lock_irq, flags);
...
#else
    rdd_error = _rdd_cpu_message_send(RDD_CPU_MESSAGE_FLUSH_LAN_TX_QUEUE, RDD_CLUSTER_1, port, queue_id, 0, is_wait);
#endif
...
#else
    rdd_error = BDMF_ERR_PARM;  /* error - not supported for CM3390 */
#endif
