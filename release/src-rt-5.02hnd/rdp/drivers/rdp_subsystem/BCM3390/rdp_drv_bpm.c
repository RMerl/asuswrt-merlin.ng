...
{
#ifdef __KERNEL__
    g_bpm_virt_base = (uint32_t)ioremap(FPM_BPM_PHYS_ADDR,FPM_BPM_PHYS_SIZE);
    printk("FPM_BPM phy_addr 0x%x,virt_addr 0x%x\n",FPM_BPM_PHYS_ADDR,g_bpm_virt_base);
#endif
}
/******************************************************************************/
/*                                                                            */
...
/*                                                                             */
/*******************************************************************************/
DRV_BPM_ERROR fi_bl_drv_bpm_init(DRV_BPM_GLOBAL_CONFIGURATION *xi_global_configuration, DRV_BPM_USER_GROUPS_THRESHOLDS *xi_ug_configuration,
    uint16_t xi_replay_address, E_DRV_BPM_SPARE_MESSAGE_FORMAT xi_bpm_spare_message_format)
{
    ...
    swaped = swap4bytes(*(uint32_t*)&bpm_init);

    BPM_MODULE_REGS_RAM_INIT_WRITE(swaped);

    BPM_MODULE_REGS_BPM_RADDR0_READ(raddr0);

    raddr0.emac0_rx_raddr = 0x3e;
    raddr0.gpon_rx_raddr = 0x02;
    raddr0.runa_raddr = 0x04;
    raddr0.runb_raddr = 0x0;
    swaped = swap4bytes(*(uint32_t*)&raddr0);

    BPM_MODULE_REGS_BPM_RADDR0_WRITE(swaped);

    BPM_MODULE_REGS_BPM_RADDR1_READ(raddr1);

    raddr1.emac1_rx_raddr = 0x1e;
    raddr1.emac2_rx_raddr = 0x2e;
    raddr1.emac3_rx_raddr = 0x0;
    raddr1.emac4_rx_raddr = 0x22;
    swaped = swap4bytes(*(uint32_t*)&raddr1);

    BPM_MODULE_REGS_BPM_RADDR1_WRITE(swaped);
...
/*******************************************************************************/
DRV_BPM_ERROR fi_bl_drv_bpm_sp_enable(DRV_BPM_SP_USR xi_source_port, E_DRV_BPM_ENABLE xi_enable)
{
...
/*                                                                             */
/*******************************************************************************/
DRV_BPM_ERROR fi_bl_drv_bpm_set_global_threshold(DRV_BPM_GLOBAL_THRESHOLD xi_global_threshold, uint32_t xi_global_hysteresis)
{
    uint32_t swapped;
...
DRV_BPM_ERROR fi_bl_drv_bpm_set_runner_msg_ctrl(DRV_BPM_RUNNER_MSG_CTRL_PARAMS *xi_runner_msg_ctrl_params)
{
    ...
    swaped = swap4bytes(*(uint32_t*)&runner_message_control);

    BPM_MODULE_REGS_BPM_RNR_MSG_CTRL_WRITE(swaped);
    ...
    swaped = swap4bytes(*(uint32_t*)&bpm_rnr_rply_ta_register);

    BPM_MODULE_REGS_BPM_RNR_RPLY_TA_WRITE(swaped);
