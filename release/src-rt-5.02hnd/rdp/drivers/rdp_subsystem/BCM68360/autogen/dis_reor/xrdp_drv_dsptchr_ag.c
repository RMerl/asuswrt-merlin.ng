...
bdmf_error_t ag_drv_dsptchr_reorder_cfg_dsptchr_reordr_cfg_set(bdmf_boolean disp_enable, bdmf_boolean rdy, bdmf_boolean reordr_par_mod, bdmf_boolean per_q_egrs_congst_en)
{
    ...
    if((disp_enable >= _1BITS_MAX_VAL_) ||
       (rdy >= _1BITS_MAX_VAL_) ||
       (reordr_par_mod >= _1BITS_MAX_VAL_) ||
       (per_q_egrs_congst_en >= _1BITS_MAX_VAL_))
    {
        bdmf_trace("ERROR driver %s:%u| err=%s (%d)\n", __FILE__, __LINE__, bdmf_strerror(BDMF_ERR_RANGE), BDMF_ERR_RANGE);
        return BDMF_ERR_RANGE;
    }
    ...
    reg_reorder_cfg_dsptchr_reordr_cfg = RU_FIELD_SET(0, DSPTCHR, REORDER_CFG_DSPTCHR_REORDR_CFG, REORDR_PAR_MOD, reg_reorder_cfg_dsptchr_reordr_cfg, reordr_par_mod);
    reg_reorder_cfg_dsptchr_reordr_cfg = RU_FIELD_SET(0, DSPTCHR, REORDER_CFG_DSPTCHR_REORDR_CFG, PER_Q_EGRS_CONGST_EN, reg_reorder_cfg_dsptchr_reordr_cfg, per_q_egrs_congst_en);
...
bdmf_error_t ag_drv_dsptchr_reorder_cfg_dsptchr_reordr_cfg_get(bdmf_boolean *disp_enable, bdmf_boolean *rdy, bdmf_boolean *reordr_par_mod, bdmf_boolean *per_q_egrs_congst_en)
{
    uint32_t reg_reorder_cfg_dsptchr_reordr_cfg;

#ifdef VALIDATE_PARMS
    if(!disp_enable || !rdy || !reordr_par_mod || !per_q_egrs_congst_en)
    {
        bdmf_trace("ERROR driver %s:%u| err=%s (%d)\n", __FILE__, __LINE__, bdmf_strerror(BDMF_ERR_PARM), BDMF_ERR_PARM);
        return BDMF_ERR_PARM;
    }
    ...
    *reordr_par_mod = RU_FIELD_GET(0, DSPTCHR, REORDER_CFG_DSPTCHR_REORDR_CFG, REORDR_PAR_MOD, reg_reorder_cfg_dsptchr_reordr_cfg);
    *per_q_egrs_congst_en = RU_FIELD_GET(0, DSPTCHR, REORDER_CFG_DSPTCHR_REORDR_CFG, PER_Q_EGRS_CONGST_EN, reg_reorder_cfg_dsptchr_reordr_cfg);
...
bdmf_error_t ag_drv_dsptchr_congestion_total_egrs_congstn_set(uint16_t frst_lvl, uint16_t scnd_lvl, uint8_t hyst_thrs)
{
    uint32_t reg_congestion_total_egrs_congstn=0;

#ifdef VALIDATE_PARMS
    if((frst_lvl >= _12BITS_MAX_VAL_) ||
       (scnd_lvl >= _12BITS_MAX_VAL_))
    {
        bdmf_trace("ERROR driver %s:%u| err=%s (%d)\n", __FILE__, __LINE__, bdmf_strerror(BDMF_ERR_RANGE), BDMF_ERR_RANGE);
        return BDMF_ERR_RANGE;
    }
    ...
    reg_congestion_total_egrs_congstn = RU_FIELD_SET(0, DSPTCHR, CONGESTION_TOTAL_EGRS_CONGSTN, FRST_LVL, reg_congestion_total_egrs_congstn, frst_lvl);
    reg_congestion_total_egrs_congstn = RU_FIELD_SET(0, DSPTCHR, CONGESTION_TOTAL_EGRS_CONGSTN, SCND_LVL, reg_congestion_total_egrs_congstn, scnd_lvl);
    reg_congestion_total_egrs_congstn = RU_FIELD_SET(0, DSPTCHR, CONGESTION_TOTAL_EGRS_CONGSTN, HYST_THRS, reg_congestion_total_egrs_congstn, hyst_thrs);

    RU_REG_WRITE(0, DSPTCHR, CONGESTION_TOTAL_EGRS_CONGSTN, reg_congestion_total_egrs_congstn);

    return BDMF_ERR_OK;
}

bdmf_error_t ag_drv_dsptchr_congestion_total_egrs_congstn_get(uint16_t *frst_lvl, uint16_t *scnd_lvl, uint8_t *hyst_thrs)
{
    uint32_t reg_congestion_total_egrs_congstn;

#ifdef VALIDATE_PARMS
    if(!frst_lvl || !scnd_lvl || !hyst_thrs)
    {
        bdmf_trace("ERROR driver %s:%u| err=%s (%d)\n", __FILE__, __LINE__, bdmf_strerror(BDMF_ERR_PARM), BDMF_ERR_PARM);
        return BDMF_ERR_PARM;
    }
    ...
    RU_REG_READ(0, DSPTCHR, CONGESTION_TOTAL_EGRS_CONGSTN, reg_congestion_total_egrs_congstn);

    *frst_lvl = RU_FIELD_GET(0, DSPTCHR, CONGESTION_TOTAL_EGRS_CONGSTN, FRST_LVL, reg_congestion_total_egrs_congstn);
    *scnd_lvl = RU_FIELD_GET(0, DSPTCHR, CONGESTION_TOTAL_EGRS_CONGSTN, SCND_LVL, reg_congestion_total_egrs_congstn);
    *hyst_thrs = RU_FIELD_GET(0, DSPTCHR, CONGESTION_TOTAL_EGRS_CONGSTN, HYST_THRS, reg_congestion_total_egrs_congstn);
...
bdmf_error_t ag_drv_dsptchr_congestion_congstn_status_get(dsptchr_congestion_congstn_status *congestion_congstn_status)
{
    ...
    congestion_congstn_status->ingrs_congstn = RU_FIELD_GET(0, DSPTCHR, CONGESTION_CONGSTN_STATUS, INGRS_CONGSTN, reg_congestion_congstn_status);
    congestion_congstn_status->egrs_congstn = RU_FIELD_GET(0, DSPTCHR, CONGESTION_CONGSTN_STATUS, EGRS_CONGSTN, reg_congestion_congstn_status);
    congestion_congstn_status->per_q_egrs_congstn = RU_FIELD_GET(0, DSPTCHR, CONGESTION_CONGSTN_STATUS, PER_Q_EGRS_CONGSTN, reg_congestion_congstn_status);
...
bdmf_error_t ag_drv_dsptchr_egrs_queues_per_q_egrs_size_get(uint16_t q_idx, bdmf_boolean *set_delay)
{
    ...
    RU_REG_RAM_READ(0, q_idx / 32, DSPTCHR, EGRS_QUEUES_PER_Q_EGRS_SIZE, reg_egrs_queues_per_q_egrs_size);

    *set_delay = FIELD_GET(reg_egrs_queues_per_q_egrs_size, (q_idx % 32) *1, 0x1);
...
static int bcm_dsptchr_cli_set(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    ...
    case cli_dsptchr_reorder_cfg_dsptchr_reordr_cfg:
        err = ag_drv_dsptchr_reorder_cfg_dsptchr_reordr_cfg_set(parm[1].value.unumber, parm[2].value.unumber, parm[3].value.unumber, parm[4].value.unumber);
        break;
    ...
    case cli_dsptchr_congestion_total_egrs_congstn:
        err = ag_drv_dsptchr_congestion_total_egrs_congstn_set(parm[1].value.unumber, parm[2].value.unumber, parm[3].value.unumber);
        break;
...
int bcm_dsptchr_cli_get(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    ...
    case cli_dsptchr_congestion_total_egrs_congstn:
    {
        ...
        uint16_t scnd_lvl;
        uint8_t hyst_thrs;
        err = ag_drv_dsptchr_congestion_total_egrs_congstn_get(&frst_lvl, &scnd_lvl, &hyst_thrs);
        bdmf_session_print(session, "frst_lvl = %u (0x%x)\n", frst_lvl, frst_lvl);
        bdmf_session_print(session, "scnd_lvl = %u (0x%x)\n", scnd_lvl, scnd_lvl);
        bdmf_session_print(session, "hyst_thrs = %u (0x%x)\n", hyst_thrs, hyst_thrs);
        break;
    ...
    case cli_dsptchr_congestion_congstn_status:
    {
        ...
        bdmf_session_print(session, "ingrs_congstn = %u (0x%x)\n", congestion_congstn_status.ingrs_congstn, congestion_congstn_status.ingrs_congstn);
        bdmf_session_print(session, "egrs_congstn = %u (0x%x)\n", congestion_congstn_status.egrs_congstn, congestion_congstn_status.egrs_congstn);
        bdmf_session_print(session, "per_q_egrs_congstn = %u (0x%x)\n", congestion_congstn_status.per_q_egrs_congstn, congestion_congstn_status.per_q_egrs_congstn);
    ...
    {
        bdmf_boolean set_delay;
        err = ag_drv_dsptchr_egrs_queues_per_q_egrs_size_get(parm[1].value.unumber, &set_delay);
        bdmf_session_print(session, "set_delay = %u (0x%x)\n", set_delay, set_delay);
        break;
    }
    case cli_dsptchr_wakeup_control_wkup_req:
...
static int bcm_dsptchr_cli_test(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    ...
    }
    {
        ...
        bdmf_boolean reordr_par_mod=gtmv(m, 1);
        bdmf_boolean per_q_egrs_congst_en=gtmv(m, 1);
        if(!err) bdmf_session_print(session, "ag_drv_dsptchr_reorder_cfg_dsptchr_reordr_cfg_set( %u %u %u %u)\n", disp_enable, rdy, reordr_par_mod, per_q_egrs_congst_en);
        if(!err) ag_drv_dsptchr_reorder_cfg_dsptchr_reordr_cfg_set(disp_enable, rdy, reordr_par_mod, per_q_egrs_congst_en);
        if(!err) ag_drv_dsptchr_reorder_cfg_dsptchr_reordr_cfg_get( &disp_enable, &rdy, &reordr_par_mod, &per_q_egrs_congst_en);
        if(!err) bdmf_session_print(session, "ag_drv_dsptchr_reorder_cfg_dsptchr_reordr_cfg_get( %u %u %u %u)\n", disp_enable, rdy, reordr_par_mod, per_q_egrs_congst_en);
        if(err || disp_enable!=gtmv(m, 1) || rdy!=gtmv(m, 1) || reordr_par_mod!=gtmv(m, 1) || per_q_egrs_congst_en!=gtmv(m, 1))
            return err ? err : BDMF_ERR_IO;
    }
    {
    ...
    {
        uint16_t frst_lvl=gtmv(m, 12);
        uint16_t scnd_lvl=gtmv(m, 12);
        uint8_t hyst_thrs=gtmv(m, 8);
        if(!err) bdmf_session_print(session, "ag_drv_dsptchr_congestion_total_egrs_congstn_set( %u %u %u)\n", frst_lvl, scnd_lvl, hyst_thrs);
        if(!err) ag_drv_dsptchr_congestion_total_egrs_congstn_set(frst_lvl, scnd_lvl, hyst_thrs);
        if(!err) ag_drv_dsptchr_congestion_total_egrs_congstn_get( &frst_lvl, &scnd_lvl, &hyst_thrs);
        if(!err) bdmf_session_print(session, "ag_drv_dsptchr_congestion_total_egrs_congstn_get( %u %u %u)\n", frst_lvl, scnd_lvl, hyst_thrs);
        if(err || frst_lvl!=gtmv(m, 12) || scnd_lvl!=gtmv(m, 12) || hyst_thrs!=gtmv(m, 8))
            return err ? err : BDMF_ERR_IO;
    }
    {
        uint8_t viq_idx=gtmv(m, 5);
    ...
    {
        dsptchr_congestion_congstn_status congestion_congstn_status = {.ingrs_congstn=gtmv(m, 2), .egrs_congstn=gtmv(m, 2), .per_q_egrs_congstn=gtmv(m, 2), .sbpm_congstn=gtmv(m, 2), .glbl_congstn=gtmv(m, 2)};
        if(!err) ag_drv_dsptchr_congestion_congstn_status_get( &congestion_congstn_status);
        if(!err) bdmf_session_print(session, "ag_drv_dsptchr_congestion_congstn_status_get( %u %u %u %u %u)\n", congestion_congstn_status.ingrs_congstn, congestion_congstn_status.egrs_congstn, congestion_congstn_status.per_q_egrs_congstn, congestion_congstn_status.sbpm_congstn, congestion_congstn_status.glbl_congstn);
    }
    {
        uint8_t q_idx=gtmv(m, 5);
    ...
    }
    {
        ...
        bdmf_boolean set_delay=gtmv(m, 1);
        if(!err) ag_drv_dsptchr_egrs_queues_per_q_egrs_size_get( q_idx, &set_delay);
...
bdmfmon_handle_t ag_drv_dsptchr_cli_init(bdmfmon_handle_t driver_dir)
{
...

    {
        ...
        static bdmfmon_cmd_parm_t set_congestion_total_egrs_congstn[]={
            BDMFMON_MAKE_PARM("frst_lvl", "frst_lvl", BDMFMON_PARM_NUMBER, 0),
            BDMFMON_MAKE_PARM("scnd_lvl", "scnd_lvl", BDMFMON_PARM_NUMBER, 0),
            BDMFMON_MAKE_PARM("hyst_thrs", "hyst_thrs", BDMFMON_PARM_NUMBER, 0),
            BDMFMON_PARM_LIST_TERMINATOR
        ...
        static bdmfmon_enum_val_t selector_table[] = {
            ...
            { .name="congestion_total_egrs_congstn", .val=cli_dsptchr_congestion_total_egrs_congstn, .parms=set_congestion_total_egrs_congstn },
