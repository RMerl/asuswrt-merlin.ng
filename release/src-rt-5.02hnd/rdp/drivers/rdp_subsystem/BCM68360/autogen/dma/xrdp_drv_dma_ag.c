...
bdmf_error_t ag_drv_dma_config_dbg_sel_set(uint8_t dma_id, uint8_t sel)
{
    ...
    reg_config_dbg_sel = RU_FIELD_SET(dma_id, DMA, CONFIG_DBG_SEL, SEL, reg_config_dbg_sel, sel);
...
bdmf_error_t ag_drv_dma_config_dbg_sel_get(uint8_t dma_id, uint8_t *sel)
{
    ...
    *sel = RU_FIELD_GET(dma_id, DMA, CONFIG_DBG_SEL, SEL, reg_config_dbg_sel);
...
int bcm_dma_cli_get(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    ...
    {
        uint8_t sel;
        err = ag_drv_dma_config_dbg_sel_get(parm[1].value.unumber, &sel);
...
static int bcm_dma_cli_test(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    ...
    }
    {
        ...
        if(!err) ag_drv_dma_config_dbg_sel_set(dma_id, sel);
        if(!err) ag_drv_dma_config_dbg_sel_get( dma_id, &sel);
