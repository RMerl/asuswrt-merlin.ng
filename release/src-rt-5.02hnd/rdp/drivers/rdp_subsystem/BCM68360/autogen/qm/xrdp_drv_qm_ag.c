...
bdmf_error_t ag_drv_qm_drop_counters_ctrl_set(const qm_drop_counters_ctrl *drop_counters_ctrl)
{
    ...
    if((drop_counters_ctrl->read_clear_pkts >= _1BITS_MAX_VAL_) ||
       ...
       (drop_counters_ctrl->close_aggregation_on_timeout_d >= _1BITS_MAX_VAL_) ||
       ...
       (drop_counters_ctrl->ghost_rpt_update_after_close_a >= _1BITS_MAX_VAL_) ||
    ...
    reg_global_cfg_qm_general_ctrl = RU_FIELD_SET(0, QM, GLOBAL_CFG_QM_GENERAL_CTRL, CLOSE_AGGREGATION_ON_TIMEOUT_D, reg_global_cfg_qm_general_ctrl, drop_counters_ctrl->close_aggregation_on_timeout_d);
    ...
    reg_global_cfg_qm_general_ctrl = RU_FIELD_SET(0, QM, GLOBAL_CFG_QM_GENERAL_CTRL, GHOST_RPT_UPDATE_AFTER_CLOSE_A, reg_global_cfg_qm_general_ctrl, drop_counters_ctrl->ghost_rpt_update_after_close_a);
...
bdmf_error_t ag_drv_qm_drop_counters_ctrl_get(qm_drop_counters_ctrl *drop_counters_ctrl)
{
    ...
    drop_counters_ctrl->close_aggregation_on_timeout_d = RU_FIELD_GET(0, QM, GLOBAL_CFG_QM_GENERAL_CTRL, CLOSE_AGGREGATION_ON_TIMEOUT_D, reg_global_cfg_qm_general_ctrl);
    ...
    drop_counters_ctrl->ghost_rpt_update_after_close_a = RU_FIELD_GET(0, QM, GLOBAL_CFG_QM_GENERAL_CTRL, GHOST_RPT_UPDATE_AFTER_CLOSE_A, reg_global_cfg_qm_general_ctrl);
...
bdmf_error_t ag_drv_qm_dma_qm_dma_config_dbg_sel_set(uint8_t sel)
{
    ...
    reg_dma_qm_dma_config_dbg_sel = RU_FIELD_SET(0, QM, DMA_QM_DMA_CONFIG_DBG_SEL, SEL, reg_dma_qm_dma_config_dbg_sel, sel);
...
bdmf_error_t ag_drv_qm_dma_qm_dma_config_dbg_sel_get(uint8_t *sel)
{
    ...
    *sel = RU_FIELD_GET(0, QM, DMA_QM_DMA_CONFIG_DBG_SEL, SEL, reg_dma_qm_dma_config_dbg_sel);
...
int bcm_qm_cli_get(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    ...
    case cli_qm_drop_counters_ctrl:
    {
        ...
        bdmf_session_print(session, "close_aggregation_on_timeout_d = %u (0x%x)\n", drop_counters_ctrl.close_aggregation_on_timeout_d, drop_counters_ctrl.close_aggregation_on_timeout_d);
        ...
        bdmf_session_print(session, "ghost_rpt_update_after_close_a = %u (0x%x)\n", drop_counters_ctrl.ghost_rpt_update_after_close_a, drop_counters_ctrl.ghost_rpt_update_after_close_a);
    ...
    {
        uint8_t sel;
        err = ag_drv_qm_dma_qm_dma_config_dbg_sel_get(&sel);
...
static int bcm_qm_cli_test(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    ...
    {
        qm_ddr_cong_ctrl ddr_cong_ctrl = {.ddr_byte_congestion_drop_enabl=gtmv(m, 1), .ddr_bytes_lower_thr=gtmv(m, 30), .ddr_bytes_mid_thr=gtmv(m, 30), .ddr_bytes_higher_thr=gtmv(m, 30), .ddr_pd_congestion_drop_enable=gtmv(m, 1), .ddr_pipe_lower_thr=gtmv(m, 8), .ddr_pipe_higher_thr=gtmv(m, 8)};
        ...
        if(err || ddr_cong_ctrl.ddr_byte_congestion_drop_enabl!=gtmv(m, 1) || ddr_cong_ctrl.ddr_bytes_lower_thr!=gtmv(m, 30) || ddr_cong_ctrl.ddr_bytes_mid_thr!=gtmv(m, 30) || ddr_cong_ctrl.ddr_bytes_higher_thr!=gtmv(m, 30) || ddr_cong_ctrl.ddr_pd_congestion_drop_enable!=gtmv(m, 1) || ddr_cong_ctrl.ddr_pipe_lower_thr!=gtmv(m, 8) || ddr_cong_ctrl.ddr_pipe_higher_thr!=gtmv(m, 8))
    ...
    {
        uint8_t res_thr_global=gtmv(m, 8);
        ...
        if(err || res_thr_global!=gtmv(m, 8))
            return err ? err : BDMF_ERR_IO;
    }
    {
    ...
    {
        uint8_t sel=gtmv(m, 2);
        if(!err) bdmf_session_print(session, "ag_drv_qm_dma_qm_dma_config_dbg_sel_set( %u)\n", sel);
        if(!err) ag_drv_qm_dma_qm_dma_config_dbg_sel_set(sel);
        if(!err) ag_drv_qm_dma_qm_dma_config_dbg_sel_get( &sel);
...
bdmfmon_handle_t ag_drv_qm_cli_init(bdmfmon_handle_t driver_dir)
{
...

    {
        ...
        static bdmfmon_cmd_parm_t set_drop_counters_ctrl[]={
            ...
            BDMFMON_MAKE_PARM("close_aggregation_on_timeout_d", "close_aggregation_on_timeout_d", BDMFMON_PARM_NUMBER, 0),
            ...
            BDMFMON_MAKE_PARM("ghost_rpt_update_after_close_a", "ghost_rpt_update_after_close_a", BDMFMON_PARM_NUMBER, 0),
