...
bdmf_error_t ag_drv_hash_context_ram_context_3b_data_set(uint16_t ctx_idx, uint32_t bits)
{
    uint32_t reg_context_ram_context_3b_data=0;
    ...
    reg_context_ram_context_3b_data = RU_FIELD_SET(0, HASH, CONTEXT_RAM_CONTEXT_3B_data, DATA, reg_context_ram_context_3b_data, bits);

    RU_REG_RAM_WRITE(0, ctx_idx, HASH, CONTEXT_RAM_CONTEXT_3B_data, reg_context_ram_context_3b_data);

    return BDMF_ERR_OK;
}

bdmf_error_t ag_drv_hash_context_ram_context_3b_data_get(uint16_t ctx_idx, uint32_t *bits)
{
    uint32_t reg_context_ram_context_3b_data;
    ...
    RU_REG_RAM_READ(0, ctx_idx, HASH, CONTEXT_RAM_CONTEXT_3B_data, reg_context_ram_context_3b_data);

    *bits = RU_FIELD_GET(0, HASH, CONTEXT_RAM_CONTEXT_3B_data, DATA, reg_context_ram_context_3b_data);
...
typedef enum
{
    ...
    bdmf_address_context_ram_context_3b_data,
...
static int bcm_hash_cli_set(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    ...
    case cli_hash_aging_ram_aging:
        ...
        break;
    case cli_hash_context_ram_context_3b_data:
        err = ag_drv_hash_context_ram_context_3b_data_set(parm[1].value.unumber, parm[2].value.unumber);
        break;
...
int bcm_hash_cli_get(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    ...
    {
        uint32_t bits;
        err = ag_drv_hash_context_ram_context_3b_data_get(parm[1].value.unumber, &bits);
...
static int bcm_hash_cli_test(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    ...
    {
        uint16_t ctx_idx=gtmv(m, 1);
        uint32_t bits=gtmv(m, 24);
        if(!err) bdmf_session_print(session, "ag_drv_hash_context_ram_context_3b_data_set( %u %u)\n", ctx_idx, bits);
        if(!err) ag_drv_hash_context_ram_context_3b_data_set(ctx_idx, bits);
        if(!err) ag_drv_hash_context_ram_context_3b_data_get( ctx_idx, &bits);
        if(!err) bdmf_session_print(session, "ag_drv_hash_context_ram_context_3b_data_get( %u %u)\n", ctx_idx, bits);
...
static int bcm_hash_cli_address(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    ...
    case bdmf_address_context_ram_context_3b_data : reg = &RU_REG(HASH, CONTEXT_RAM_CONTEXT_3B_data); blk = &RU_BLK(HASH); break;
...
bdmfmon_handle_t ag_drv_hash_cli_init(bdmfmon_handle_t driver_dir)
{
...

    {
        ...
        static bdmfmon_cmd_parm_t set_context_ram_context_3b_data[]={
        ...
        static bdmfmon_enum_val_t selector_table[] = {
            ...
            { .name="context_ram_context_3b_data", .val=cli_hash_context_ram_context_3b_data, .parms=set_context_ram_context_3b_data },
    ...
    }
    {
        static bdmfmon_enum_val_t enum_table_address[] = {
            ...
            { .name="CONTEXT_RAM_CONTEXT_3B_data" , .val=bdmf_address_context_ram_context_3b_data },
