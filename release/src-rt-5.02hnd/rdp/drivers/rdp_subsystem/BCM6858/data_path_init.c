...
bbh_id_e g_dma_to_bbh_x[profiles_num][NUM_OF_DMA][NUM_OF_PERIPHERALS_PER_DMA] = {
         {{BBH_ID_XLMAC1_1_RGMII, BBH_ID_XLMAC1_2_RGMII, BBH_ID_PON, BBH_ID_NULL, BBH_ID_NULL, BBH_ID_NULL},
          {BBH_ID_XLMAC0_0_10G, BBH_ID_XLMAC0_1_2p5G, BBH_ID_XLMAC0_2_1G, BBH_ID_XLMAC0_3_1G, BBH_ID_XLMAC1_0_RGMII, BBH_ID_NULL}},
         {{BBH_ID_XLMAC1_1_RGMII, BBH_ID_XLMAC1_2_RGMII, BBH_ID_PON, BBH_ID_NULL, BBH_ID_NULL, BBH_ID_NULL},
          {BBH_ID_XLMAC0_0_10G, BBH_ID_XLMAC0_1_2p5G, BBH_ID_XLMAC0_2_1G, BBH_ID_XLMAC0_3_1G, BBH_ID_XLMAC1_0_RGMII, BBH_ID_NULL}},
         {{BBH_ID_XLMAC1_1_RGMII, BBH_ID_XLMAC1_2_RGMII, BBH_ID_PON, BBH_ID_NULL, BBH_ID_NULL, BBH_ID_NULL},
          {BBH_ID_XLMAC0_0_10G, BBH_ID_XLMAC0_1_2p5G, BBH_ID_XLMAC0_2_1G, BBH_ID_XLMAC0_3_1G, BBH_ID_XLMAC1_0_RGMII, BBH_ID_NULL}} };
...
uint8_t g_bbh_buff_num[profiles_num][NUM_OF_DMA][NUM_OF_PERIPHERALS_PER_DMA] = { 
         /* PON + No XFI port */ {{4, 4, 24}, {8, 6, 6, 6, 6}},
         /* PON + XFI port0   */ {{4, 4, 24}, {10, 6, 6, 6, 4}},
         /* PON + XFI port4   */ {{4, 4, 24}, {6, 6, 6, 6, 8}} };
...
static int bbh_profiles_init(void)
{
    ...
    for (bbh_id = 0; bbh_id < BBH_ID_NUM; bbh_id++)
    {
        ...
        bbh_tx_ddr_cfg->bufsize = BUF_512;
...
static int bbh_rx_cfg(bbh_id_e bbh_id)
{
    ...
    if (!IS_WAN_PORT(bbh_id))
    {
        ...
        cfg.crc_omit_dis = 1;
#else
        if ((int)p_dpi_cfg->system_port == (int)bbh_id)
            cfg.crc_omit_dis = 1;
#endif
    }
    else
...
static int bbh_tx_pon_init(int wantype)
{
    ...
    if (wantype == MAC_TYPE_EPON || wantype == MAC_TYPE_XEPON)
    {
        ...
        else /* wantype == MAC_TYPE_XEPON */
        {
            ...
            config.sts_rnr_cfg[0].tcont_addr = IMAGE_3_BBH_TX_EPON_QUEUE_ID_TABLE_ADDRESS >> 3;
...
static int bbh_tx_init(void)
{
    ...
    for (bbh_id = BBH_ID_FIRST; bbh_id < BBH_ID_PON; bbh_id++)
    {
        ...
        /* cores which sending PDs */
        config.mac_type = MAC_TYPE_XLMAC;
        config.rnr_cfg[0].skb_addr = 0;
        ...
        else
        {
            ...
            config.rnr_cfg[0].ptr_addr = (IMAGE_0_BBH_TX_EGRESS_COUNTER_TABLE_ADDRESS + (bbh_id * sizeof(RDD_BBH_TX_EGRESS_COUNTER_ENTRY_DTS))) >> 3;
#ifndef G9991
            config.rnr_cfg[0].task_number = (IMAGE_0_DS_TM_LAN0_THREAD_NUMBER + bbh_id);
#else
            config.rnr_cfg[0].task_number = (IMAGE_0_DS_TM_FRAG0_THREAD_NUMBER + bbh_id);
#endif
        }
...
static int dispatcher_reorder_init(void)
{
    ...
    uint16_t common_max_limit = DIS_REOR_LINKED_LIST_BUFFER_NUM 
        ...
        + (DSPTCHR_NORMAL_GUARANTEED_BUFFERS - DSPTCHR_EXCLUSIVE_WAN_GUARANTEED_BUFFERS) * BBH_ID_NUM /*Guaranteed for PON exclusive VIQ*/
    ...
    dispatcher_reorder_viq_init(&cfg, &congs_init, get_runner_idx(cpu_tx_runner_image), (IMAGE_2_CPU_TX_EGRESS_DISPATCHER_CREDIT_TABLE_ADDRESS >> 3 |
        IMAGE_2_CPU_IF_2_CPU_TX_EGRESS_THREAD_NUMBER << 12), dsptchr_viq_dest_reor, dsptchr_viq_non_delayed, DISP_REOR_VIQ_CPU_TX_EGRESS,
    ...
    /* VIQ for TX Mirroring - Initialized for US - 21 */
...
#ifdef G9991
    /* VIQ for g9991 reassembly0 - 22 */
    dispatcher_reorder_viq_init(&cfg, &congs_init, get_runner_idx(reassembly_runner_image), (IMAGE_1_REASSEMBLY0_DISPATCHER_CREDIT_TABLE_ADDRESS >> 3) |
        (IMAGE_1_REASSEMBLY_G9991_0_THREAD_NUMBER << 12), dsptchr_viq_dest_disp, dsptchr_viq_delayed, DISP_REOR_VIQ_G9991_REASS0,
        DSPTCHR_NORMAL_GUARANTEED_BUFFERS, common_max_limit, 0);

    /* VIQ for g9991 reassembly1 - 23 */
    dispatcher_reorder_viq_init(&cfg, &congs_init, get_runner_idx(reassembly_runner_image), (IMAGE_1_REASSEMBLY1_DISPATCHER_CREDIT_TABLE_ADDRESS >> 3) |
    ...
    for (core_index = 0; core_index < GROUPED_EN_SEGMENTS_NUM; core_index++)
    {
        ...
        if (rdp_core_to_image_map[core_index] == wan_direct_runner_image)
        {
            dispatcher_reorder_rnr_group_init(&cfg, 1, core_index, (1<< IMAGE_2_CPU_IF_2_WAN_DIRECT_THREAD_NUMBER), (IMAGE_2_DIRECT_PROCESSING_PD_TABLE_ADDRESS >> 3), 0);
        }
        /* setting group 2 - reassembly0 */
        if (rdp_core_to_image_map[core_index] == reassembly_runner_image)
        {
            dispatcher_reorder_rnr_group_init(&cfg, 2, core_index, (1<< IMAGE_1_REASSEMBLY_G9991_0_THREAD_NUMBER), (IMAGE_1_G9991_PD_TABLE_ADDRESS >> 3), (sizeof(RDD_PROCESSING_RX_DESCRIPTOR_DTS) >> 3));
        }
        /* setting group 3 - reassembly1 */
        if (rdp_core_to_image_map[core_index] == reassembly_runner_image)
        {
            dispatcher_reorder_rnr_group_init(&cfg, 3, core_index, (1<< IMAGE_1_REASSEMBLY_G9991_1_THREAD_NUMBER), (IMAGE_1_G9991_PD_TABLE_ADDRESS >> 3), (sizeof(RDD_PROCESSING_RX_DESCRIPTOR_DTS) >> 3));
        }
        /* setting group 4 - g9991 flow control */
        if (rdp_core_to_image_map[core_index] == flow_control_runner_image)
        {
            dispatcher_reorder_rnr_group_init(&cfg, 4, core_index, (1<< IMAGE_0_DS_TM_FLOW_CONTROL_THREAD_NUMBER), (IMAGE_0_G9991_FLOW_CONTROL_PD_ADDRESS >> 3), 0);
        }
        /* setting group 5 - system port */
        if (rdp_core_to_image_map[core_index] == wan_direct_runner_image)
        {
            dispatcher_reorder_rnr_group_init(&cfg, 5, core_index, (1<< IMAGE_2_CPU_IF_2_DIRECT_SYSTEM_PORT_THREAD_NUMBER), ((IMAGE_2_DIRECT_PROCESSING_PD_TABLE_ADDRESS +
                sizeof(RDD_PROCESSING_RX_DESCRIPTOR_DTS)) >> 3), 0);
        }
#endif
    }
...
#ifndef _CFE_
    cfg.rnr_grp_num = 2;
    cfg.dsptchr_rnr_group_list[0].queues_mask = 0x7FFF | /* all bbh viq except wan-bbh exclusive viq */
    ...
    cfg.rnr_grp_num = 5;
    cfg.dsptchr_rnr_group_list[0].queues_mask = 0x80 | /* wan-bbh normal viq (DS) */
        (1 << DISP_REOR_VIQ_G9991_REASS0) | (1 << DISP_REOR_VIQ_G9991_REASS1) | (1 << DISP_REOR_VIQ_CPU_TX_FORWARD);
    cfg.dsptchr_rnr_group_list[1].queues_mask = 0x8000;                                                                         /* wan-bbh exclusive viq */
    cfg.dsptchr_rnr_group_list[2].queues_mask = p_dpi_cfg->g9991_port_vec & (~(p_dpi_cfg->g9991_port_vec - 1));                 /* reassembly0 */
    cfg.dsptchr_rnr_group_list[3].queues_mask = p_dpi_cfg->g9991_port_vec & (p_dpi_cfg->g9991_port_vec - 1);                    /* reassembly1 */
    cfg.dsptchr_rnr_group_list[4].queues_mask = 0x7F00;                                                                         /* lan-bbh exclusive viq (flow control)*/
    cfg.dsptchr_rnr_group_list[5].queues_mask = system_port_mask;                                                               /* system port */
#endif
...
static int qm_wantype_init(int wantype)
{
    ...
    else
    {
        ...
        if (wantype == MAC_TYPE_XEPON)
        {
            ...
            queue_cfg.fec_enable = 1;                   /* TRUE=enable FEC */
...
static int qm_init(void)
{
    ...
    init_cfg.fpm_ug_thr[1].higher_thr = 40*1024; /* US */
    ...
    init_cfg.fpm_ug_thr[3].higher_thr = 8*1024; /* WLAN */
...
#ifdef BCM6858_A0
    queue_cfg.wred_profile = 2;                         /* WRED profile */
#endif	
    ...
    {
        if ((idx == QM_QUEUE_DROP) || (idx == QM_QUEUE_CPU_RX))
            continue;
        rc = rc ? rc : drv_qm_queue_config(idx, &queue_cfg);
        rc = rc ? rc : drv_qm_queue_enable(idx);
    }
...
uint32_t data_path_init_gbe(rdpa_emac wan_emac)
{
    ...
    /* QM mactype init */
    rc = qm_wantype_init(MAC_TYPE_XLMAC);

    /* RUNNER mactype init */
    rc = rc ? rc : rdd_scheduling_us_wan(MAC_TYPE_XLMAC, wan_emac);
