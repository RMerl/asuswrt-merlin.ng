...
#ifdef BCM6858_A0
    rc = rc ? rc : ag_drv_bbh_rx_general_configuration_g9991en_set(bbh_id, config->gint_en);
#else
...
#ifdef BCM6858_A0
    rc = rc ? rc : ag_drv_bbh_rx_general_configuration_g9991en_get(bbh_id, &config->gint_en);
#else
...
static inline int drv_bbh_rx_pkt_size_cfg_print(bdmf_session_handle session, bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    ...
    parm[0].value.unumber = cli_bbh_rx_min_pkt_sel_flows_0_31;
    for (i = 0; i < 32; i++)
    {
    ...
    parm[0].value.unumber = cli_bbh_rx_max_pkt_sel_flows_0_31;
    for (i = 0; i < 32; i++)
    {
        bdmf_session_print(session, "[%d]:", i);
        parm[2].value.unumber = i;
