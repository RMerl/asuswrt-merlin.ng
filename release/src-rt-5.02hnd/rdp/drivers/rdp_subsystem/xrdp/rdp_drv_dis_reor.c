...
int drv_dis_reor_queues_init()
{
    ...
    congs_init.frst_lvl = 500;
    congs_init.hyst_thrs = (DIS_REOR_LINKED_LIST_BUFFER_NUM / 4) - 1;

    return rc ? rc : ag_drv_dsptchr_congestion_glbl_congstn_set(0, &congs_init);
};
...
int drv_dis_reor_cfg(dsptchr_config *cfg)
{
    ...
    g_prcs_rnr_num = cfg->prcs_rnr_num;
    for (i = 0; i < g_prcs_rnr_num; i++)
    {
...
int drv_dis_reor_cli_config_get(bdmf_session_handle session, bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    ...
    for (core_idx = 0; core_idx < g_prcs_rnr_num; core_idx++)
    {
...
{
    int rc, time_out = POLLING_TIME_OUT;
    ...
    /* get the number of active viq */
    rc = _rdp_drv_dis_reor_queue_num_get(&viq_num);

    /* wait for all the disp-reor buffers to return to the fll */
    while ((!rc) && (time_out > 0) && (available_buffers < (DIS_REOR_LINKED_LIST_BUFFER_NUM - viq_num)))
    {
        rc = ag_drv_dsptchr_flldes_bufavail_get(&available_buffers);
        time_out--;
    }

    if (rc || (time_out <= 0) || (available_buffers < (DIS_REOR_LINKED_LIST_BUFFER_NUM - viq_num)))
...
int drv_dis_reor_restore_isolate_set(uint8_t rnr_grp_idx)
{
    ...
    /* wait for all the disp-reor buffers to return to the fll */
    while ((rc == BDMF_ERR_OK) &&
        (time_out > 0) && (available_buffers < (DIS_REOR_LINKED_LIST_BUFFER_NUM - viq_num)))
    {
        rc = ag_drv_dsptchr_flldes_bufavail_get(&available_buffers);
        time_out--;
    }
