...
bdmf_error_t drv_qm_init(const qm_init_cfg *init_cfg)
{
    ...
    /* Init DMA */
#ifdef BCM68360
	ag_drv_qm_dma_qm_dma_config_num_of_writes_set(0, QM_DMA_NUM_READS);
...
#ifdef BCM6858_A0
        drop_ctrl.close_aggregation_on_empty_que = init_cfg->is_close_agg_disable;
#endif
    ...
    /* Clear counters SRAM: 2 counters per queue */
    ...
    for (i = 0; i < QM_NUM_EPON_QUEUES; i++)
    {
...
int drv_qm_cli_sanity_check(bdmf_session_handle session, bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    ...
    if (!rc)
    {
        ...
        if (intr_ctrl_isr.qm_target_mem_force_copy_contr)
            bdmf_session_print(session, "QM: qm_target_mem_force_copy_contr\n");
