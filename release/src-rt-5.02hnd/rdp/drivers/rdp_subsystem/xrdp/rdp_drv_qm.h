...
static inline bdmf_error_t drv_qm_cpu_tx(uint32_t *cpu_tx_descriptor, uint16_t qm_queue_num, uint8_t no_lock)
{
    /*atomic binary operation must be on unsigned long aligned pointers*/
    static uint64_t qm_interface __attribute__((aligned(8))) = 0;
    uint8_t local_qm_interface = 0;
    ...
    local_qm_interface = qm_interface;
