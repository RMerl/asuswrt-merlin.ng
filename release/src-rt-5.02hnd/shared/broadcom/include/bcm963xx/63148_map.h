 ...
 */
typedef struct Sotp {
	uint32 OTPProgCtrl;	/* 0x00 */
	uint32 OTPWData0;	/* 0x04 */
	uint32 OTPWData1;	/* 0x08 */
	uint32 OTPAddr;		/* 0x0c */
	uint32 OTPCtrl0;	/* 0x10 */
	uint32 unused0;		/* 0x14 */
	uint32 OTPStat;		/* 0x18 */
	uint32 OTPRData0;	/* 0x1c */
	uint32 OTPRData1;	/* 0x20 */
	uint32 unused1[3];	/* 0x24-0x2f */
	uint32 OTPECCCnt;	/* 0x30 */
	uint32 OTPBadAddr;	/* 0x34 */
	uint32 OTPDebug0;	/* 0x38 */
	uint32 unused2[5];	/* 0x3c-0x4f */
	uint32 SRState[8];	/* 0x50-0x6c */
	uint32 perm;		/* 0x70 */

} Sotp;

#define SOTP ((volatile Sotp * const) SOTP_BASE)
...
#define OTP_GET_USER_BIT(x)      (*((unsigned int*)&(JTAGOTP->status3) + (x)/32) >> ((x) % 32) & 1)
