...
#define PCS_MDIO_PHYID 2
    ...
    if ((wan_reg & 0x3) == 0x2)
    {
        if (pBoardInfo[0].sw.phy_id[DRV_BBH_EMAC_5] & PHY_EXTERNAL)
        {
            phy_dev_t *phy_dev;

            phy_dev = phy_dev_add(PHY_TYPE_PCS, PCS_MDIO_PHYID, NULL);
            phy_dev->phy_drv->bus_drv = bus_drv_get(BUS_TYPE_6838_AE);
            phy_dev->mii_type = PHY_MII_TYPE_SGMII;
