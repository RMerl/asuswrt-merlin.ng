...
static intr_handler_s *id_to_handler[LPORT_INTR_LAST][ENTITY_ID_COUNT] = 
{
    ...
    [LPORT_PORT_LINK] = {&port0_link_h, &port1_link_h, &port2_link_h, &port3_link_h, &port4_link_h, &port5_link_h,
        &port6_link_h},
...
void lport_intr_unreqister(LPORT_INTR_ID intr_id, uint32_t entity_id)
{
    intr_handler_s *intr_handler;
