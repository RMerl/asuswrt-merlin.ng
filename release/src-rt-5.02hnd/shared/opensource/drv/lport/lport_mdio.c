...
int lport_mdio22_wr(uint16_t phyid,uint16_t addr, uint16_t data)
{
    ...
    rc = ag_drv_lport_mdio_cfg_get(&supress_preamble,&mdio_clk_divider,&mdio_clause);

    mdio_clause = MDIO_CL22;

    rc = rc ? rc: ag_drv_lport_mdio_cfg_set(supress_preamble,mdio_clk_divider,mdio_clause);
...
int lport_mdio45_rd(uint16_t phyid, uint16_t devid, uint16_t addr, uint16_t *data)
{
    ...
    rc = ag_drv_lport_mdio_cfg_get(&supress_preamble, &mdio_clk_divider,
        &mdio_clause);
    ...
    rc = rc ?
        rc :
        ag_drv_lport_mdio_cfg_set(supress_preamble, mdio_clk_divider,
            mdio_clause);
    ...
    uint16_t data)
{
    ...
    MDIO_CNTX_UNLOCK;
    /* XXX workaround to allow mdio clause 45 to be written on the bus */
    if (phyid != 0)
        lport_mdio45_rd(phyid, devid, addr, &val);
    return rc;
}
#ifndef _CFE_
