...
static int lport_proc_cmd_init(int argc, char *argv[])
{
    ...
    for (i = 0; i <  LPORT_NUM_OF_PORTS; i++)
        switch (argv[1][i])
        {
        case 'S': init_params.prt_mux_sel[i] = PORT_SGMII_SLAVE; break;
        case 'B': init_params.prt_mux_sel[i] = PORT_SGMII_1000BASE_X; break;
    ...
    if (!(res = lport_init_driver(&init_params)))
        pr_info("XLMAC init done\n");
    else
        pr_err("XLMAC init failed; err %d\n", res);
