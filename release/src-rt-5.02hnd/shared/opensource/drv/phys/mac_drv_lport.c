...
static int port_lport_init(mac_dev_t *mac_dev)
{
    ...
    int mux_sel = (LPORT_PORT_MUX_SELECT)mac_dev->priv;
    ...
    if (mux_sel == PORT_RGMII)
    {
        lport_rgmii_cfg_s rgmii_cfg;

        if (lport_get_rgmii_cfg(port_id, &rgmii_cfg))
        {
            printk("Failed to get RGMII configuration for lport mac %d\n", port_id);
            return -1;
        }
        ...
        rgmii_cfg.valid = 1;
        rgmii_cfg.idelay_dis = 1;

        if (lport_set_rgmii_cfg(port_id, &rgmii_cfg))
        {
            printk("Failed to set RGMII configuration for lport mac %d\n", port_id);
            return -1;
        }
    }
