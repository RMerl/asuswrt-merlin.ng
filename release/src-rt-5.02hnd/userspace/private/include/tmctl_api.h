...
#elif defined(CHIP_6858)
#define TMCTL_DEF_ETH_Q_SZ_US     (768)
#define TMCTL_DEF_ETH_Q_SZ_DS     (48)
...
typedef struct
{
   ...
   BOOL     queueShaper; /* Boolean to indicate whether queue rate
                          * shaping is supported by this port.
                          */
   uint32_t cfgFlags;    /* Port TM initialization config flags.
...
typedef struct
{
   int shapingRate;           /* Shaping rate in kbps. 0 implies no shaping. */
   int shapingBurstSize;      /* Shaping burst size in bytes */
   int minRate;               /* Minimum rate in kbps. 0 implies no shaping. */
 ...
 * ----------------------------------------------------------------------------
 */
#define MAX_TMCTL_QUEUES   8

typedef struct
{
   int              numQueues;   /* Number of queues configured */
   tmctl_queueCfg_t qcfg[MAX_TMCTL_QUEUES];
