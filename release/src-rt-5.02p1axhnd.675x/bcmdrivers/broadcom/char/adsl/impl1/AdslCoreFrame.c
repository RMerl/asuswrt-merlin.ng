...
Public void * AdslCoreFrameAllocMemForFrames(uint frameNum)
{
	void	**pFreeFrameList;
	uchar	*frPtr;
	uint	i;
...
Public void * AdslCoreFrameAllocMemForBuffers(void **ppMemPool, uint bufNum, uint memSize)
{
	void	**pFreeBufList;
	uchar	*frPtr;
	uint	i;
...
Public void AdslCoreFrameFreeMemForBuffers(void *hMem, uint memSize, void *pMemPool)
{
	free (pMemPool);
}

Public dslFrameBuffer * AdslCoreFrameAllocBuffer(void *handle, void *pMem, uint length)
{
	dslFrameBuffer	*pBuf;
...
Public uint AdslCoreFrame2Id(void *handle, dslFrame *pFrame)
{
	return ((uchar *)pFrame - (uchar *)handle - sizeof (void *)) / sizeof(dslFrame);
}

Public void * AdslCoreFrameId2Frame(void *handle, uint frameId)
{
