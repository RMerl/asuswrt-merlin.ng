...
typedef enum ShaperIdEnum
{
    ...
    DRV_EPON_SHAPER6,
    DRV_EPON_SHAPER7,
    DRV_EPON_SHAPER_MAX = DRV_EPON_SHAPER7
} ShaperIdEnum;
...
static int bcm_epn_tx_l1s_shp_cli_set(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    ...
    {
    case BDMF_config:
        err = ag_drv_epn_tx_l1s_shp_config_set(parm[1].value.unumber, parm[2].value.unumber, parm[3].value.unumber);
        break;
    case BDMF_que_en:
...
static int bcm_epn_tx_l1s_shp_cli_get(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    ...
    case BDMF_config:
    {
        ...
        err = ag_drv_epn_tx_l1s_shp_config_get(parm[1].value.unumber, &cfgshprate, &cfgshpbstsize);
...
static int bcm_epn_tx_l1s_shp_cli_test(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
...

    {
        ...
        if(!err) bdmf_session_print(session, "ag_drv_epn_tx_l1s_shp_config_set(%u %u %u)\n", shaper_idx, cfgshprate, cfgshpbstsize);
        if(!err) ag_drv_epn_tx_l1s_shp_config_set(shaper_idx, cfgshprate, cfgshpbstsize);
        if(!err) ag_drv_epn_tx_l1s_shp_config_get( shaper_idx, &cfgshprate, &cfgshpbstsize);
        if(!err) bdmf_session_print(session, "ag_drv_epn_tx_l1s_shp_config_get(%u %u %u)\n", shaper_idx, cfgshprate, cfgshpbstsize);
        if(err || cfgshprate!=gtmv(m, 23) || cfgshpbstsize!=gtmv(m, 8))
            return err ? err : BDMF_ERR_IO;
    }
    {
        uint8_t cfgshpen=gtmv(m, 8);
