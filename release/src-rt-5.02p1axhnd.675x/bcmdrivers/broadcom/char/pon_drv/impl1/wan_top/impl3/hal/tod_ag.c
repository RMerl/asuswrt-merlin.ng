...
int ag_drv_tod_config_0_set(const tod_config_0 *config_0)
{
    ...
    if((config_0->cfg_ts48_read >= _1BITS_MAX_VAL_) ||
...
int ag_drv_tod_config_3_set(uint8_t cfg_ts48_fifo_dis, uint8_t cfg_ts48_fifo_ld_rate, uint16_t cfg_ref_offset)
{
...
#ifdef VALIDATE_PARMS
    if((cfg_ts48_fifo_dis >= _1BITS_MAX_VAL_) ||
       (cfg_ts48_fifo_ld_rate >= _5BITS_MAX_VAL_) ||
       (cfg_ref_offset >= _10BITS_MAX_VAL_))
    {
        pr_err("ERROR driver %s:%u|(%d)\n", __FILE__, __LINE__, 0);
        return 0;
    }
    ...
    reg_config_3 = RU_FIELD_SET(0, TOD, CONFIG_3, CFG_TS48_FIFO_DIS, reg_config_3, cfg_ts48_fifo_dis);
    reg_config_3 = RU_FIELD_SET(0, TOD, CONFIG_3, CFG_TS48_FIFO_LD_RATE, reg_config_3, cfg_ts48_fifo_ld_rate);
...
int ag_drv_tod_config_3_get(uint8_t *cfg_ts48_fifo_dis, uint8_t *cfg_ts48_fifo_ld_rate, uint16_t *cfg_ref_offset)
{
...
#ifdef VALIDATE_PARMS
    if(!cfg_ts48_fifo_dis || !cfg_ts48_fifo_ld_rate || !cfg_ref_offset)
    {
        pr_err("ERROR driver %s:%u|(%d)\n", __FILE__, __LINE__, 2);
        return 2;
    }
    ...
    *cfg_ts48_fifo_dis = RU_FIELD_GET(0, TOD, CONFIG_3, CFG_TS48_FIFO_DIS, reg_config_3);
    *cfg_ts48_fifo_ld_rate = RU_FIELD_GET(0, TOD, CONFIG_3, CFG_TS48_FIFO_LD_RATE, reg_config_3);
