...
wpsap_process_msg(wpsap_wksp_t *wps_wksp, char *buf, uint32 len, uint32 msgtype)
{
	...
	case TRANSPORT_TYPE_EAP:
		...
		retVal = ap_eap_sm_process_sta_msg(buf, len,
			wps_wksp->authorizedMacs, wps_wksp->authorizedMacs_len);
...
wpsap_readConfigure(wpsap_wksp_t *wps_wksp, DevInfo *ap_devinfo)
{
	...
	/* delay deauth after EAP-FAIL */
	value = wps_safe_get_conf("wps_delay_deauth_ms");
	if (strcmp(value, ""))
		wps_wksp->wps_delay_deauth_ms = atoi(value);
...
wpsap_wksp_init(wpsap_wksp_t *ap_wksp)
{
	...
	if (ap_eap_sm_init(ap_wksp->mc, (char *)ap_wksp->mac_sta, wps_eapol_parse_msg,
		wps_eapol_send_data, ap_wksp->eap_frag_threshold, ap_wksp->wps_delay_deauth_ms) !=
			WPS_SUCCESS) {
