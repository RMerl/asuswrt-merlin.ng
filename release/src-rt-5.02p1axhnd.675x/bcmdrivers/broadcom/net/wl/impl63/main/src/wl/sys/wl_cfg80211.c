...
#ifdef BCA_HNDROUTER
		/* Gas comeback req and resp action frames will be handled by passpoint app */
		if ((dev->ieee80211_ptr->iftype == NL80211_IFTYPE_AP) &&
			(action == GAS_COMEBACK_REQUEST_ACTION_FRAME ||
			action == GAS_COMEBACK_RESPONSE_ACTION_FRAME)) {
			return false;
		}
#endif	/* BCA_HNDROUTER */
	...
	if (((event == WLC_E_ASSOC_IND) || (event == WLC_E_REASSOC_IND) ||
		...
		*/
		...
		cfg80211_new_sta(ndev, e->addr.octet, &sinfo, GFP_ATOMIC);
			...
			((event == WLC_E_DEAUTH) &&
#ifdef WL_HAPD_WDS
			(((reason == DOT11_RC_INACTIVITY) || (reason == DOT11_RC_AUTH_INVAL)) ||
#else
	...
	if ((event == WLC_E_LINK) && (strstr(e->ifname, "wds") != NULL) &&
		...
		if (flags & WLC_EVENT_MSG_LINK) {
			...
			if (bss_security->security_mode == TRUE) {
				...
				if ((rsnie = bcm_parse_tlvs(cfg->ioctl_buf, WLC_IOCTL_MEDLEN,
						DOT11_MNG_RSN_ID)) != NULL) {
...
wl_notify_wds_connect_status(struct bcm_cfg80211 *cfg,
	bcm_struct_cfgdev *cfgdev, const u8 *bssid)
{
	struct net_device *ndev = NULL, *primary_ndev = NULL;
	s32 err = BCME_OK;
	wl_event_msg_t e;
	ndev = cfgdev_to_wlc_ndev(cfgdev, cfg);
