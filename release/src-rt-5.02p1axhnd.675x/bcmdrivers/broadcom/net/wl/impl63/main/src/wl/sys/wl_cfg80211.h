...
#define WLAN_AKM_SUITE_SAE_FBT		0x09AC0F00
...
typedef struct wl_cfg80211_wds_list {
	struct ether_addr BSSID;
	wl_bss_info_t *bi;
} wl_cfg80211_wds_list;
typedef struct wl_cfg80211_wds_info {
	wl_cfg80211_wds_list wl_wds_list[WDS_MAX];
	uint8 wds_count;
	/* XXX: Scan partial results are getting in different ndev
	 * To pass it to the right ndev, use it
	*/
	struct net_device *ndev;
} wl_cfg80211_wds_info;
#endif /* WL_HAPD_WDS */
...
#if defined(STB) && !defined(STBAP)
#if (LINUX_VERSION_CODE > KERNEL_VERSION(2, 6, 39))
	enum wl_cfg80211_power_state cfg_power_state; /* Cfg powersave state */
	struct wowlan_info wowlan_info;   /* Wowlan information */
#endif /* KERNEL_VERSION(2, 6, 39) */
#endif /* STB && STBAP */
	struct wl_core_priv *wlcore;
#if defined(WL_HAPD_WDS)
	wl_cfg80211_wds_info *wds_mac_info;
#endif /* WL_HAPD_WDS */
};
...
extern s32 wl_cfg80211_set_wowlan_triggers(struct net_device *dev, s32 wowlan_trigger);
extern s32 wl_cfg80211_get_wowlan_triggers(struct net_device *dev);
#endif /* STB && STBAP */
