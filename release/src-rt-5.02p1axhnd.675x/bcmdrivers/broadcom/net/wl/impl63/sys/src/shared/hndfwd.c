 ...
 * $Id: hndfwd.c 714391 2017-08-04 07:32:52Z $
 *
...
static const char * _fwder_mode_g[FWDER_MAX_MODE] = { "NIC", "DGL", "UNDEF" };
...
#define _FWDER_CPUMAP_INI_(ix)                                                 \
	{ .mode = FWDER_UNDEF_MODE, .chan = FWDER_UNDEF_CHAN,                        \
...
fwder_assign(fwder_mode_t mode, int radio_unit)
{
	...
	if (fwder_cpumap_g[radio_unit].mode == FWDER_UNDEF_MODE) {
		/* If unknown mode, don't assert, just warn. And,
		 * update the mode based on the incoming request.
		 * This will happen only first time after nvram erase.
		 */
		FWDER_ERROR(("%s: Warning! radio <%d> mode unknown. Inheriting driver mode <%d>\n",
			__FUNCTION__, radio_unit, mode));
		fwder_cpumap_g[radio_unit].mode = mode;
	} else {
...
fwder_affinity(fwder_dir_t dir, int fwder_unit, int irq)
{
	...
	if (dir == FWDER_UPSTREAM) { /* Radio Interface */
		...
		if (cpumap->irq == 0) {
			/* If irq# is not set, let's use the irq# passed by the caller. */
			cpumap->irq = irq;
		} else {
			FWDER_ASSERT((cpumap->unit == fwder_unit) && (cpumap->irq == irq));
		}

		/* Traverse previously assigned cpumap and do sanity check to make sure
		 * irq assignment does not conflict with cpu map.
		 */
		for (radio = 0; radio < radio_unit; radio++) {
			if (fwder_cpumap_g[radio].irq == cpumap->irq) {
				if (fwder_cpumap_g[radio].cpu != cpumap->cpu) {
					FWDER_ERROR(("ERROR %s: unit:irq:cpu<%d:%d:%d> "
							"mismatch <%d:%d:%d>\n", __FUNCTION__,
							radio,
							fwder_cpumap_g[radio].irq,
							fwder_cpumap_g[radio].cpu,
							radio_unit, cpumap->irq, cpumap->cpu));
					ASSERT(0);
					return FWDER_FAILURE;
				}
			}
...
_fwder_cpumap_parse(const char *fwder_cpumap_nvar_str)
{
	...
	while ((cpumap_per_str = bcmstrtok((char **)&cpumap_all_str, " ", NULL))
		...
		cpumap.mode = (mode == 'd') ? FWDER_DNG_MODE :
			(mode == 'n') ? FWDER_NIC_MODE : FWDER_UNDEF_MODE;

		if (cpumap.mode == FWDER_UNDEF_MODE) {
			/* Let's check if the radio_dmode is set in nvram and use that */
			char nvname[32];
			const char *fwder_mode;
			int i;

			snprintf(nvname, sizeof(nvname), "wlradio_dmode_%d", radio_unit);
			fwder_mode = getvar(NULL, nvname);
			if (fwder_mode) {
				for (i = 0; i < FWDER_MAX_RADIO; i++) {
					if (strcmp(__fwder_mode(i), fwder_mode) == 0) {
						cpumap.mode = i;
						FWDER_TRACE(("%s: override mode<%s> on radio<%d>\n",
							__FUNCTION__, fwder_mode, radio_unit));
						break;
					}
				}
			}
...
fwder_mode_t
fwder_getmode(uint8 unit)
{
	return fwder_cpumap_g[unit].mode;
}
