...
wl_stats () {
    ...
    display_cmd_op "RADAR STATUS: wl -i $IFNAME radar_status" "$WLCMD -i $IFNAME radar_status"
    ...
    display_cmd_op "ESCAN start: wl -i $IFNAME escan" "$WLCMD -i $IFNAME escan"
    display_cmd_op "Wait 1 second: sleep 1" "sleep 1"
    ...
    display_cmd_op "RRM NGR LIST: wl -i $IFNAME rrm_nbr_list" "$WLCMD -i $IFNAME rrm_nbr_list"
    display_cmd_op "NGR DISCOVERY CAP: wl -i $IFNAME nbr_discovery_cap" "$WLCMD -i $IFNAME nbr_discovery_cap"
    display_cmd_op "OCE ENABLE: wl -i $IFNAME oce enable" "$WLCMD -i $IFNAME oce enable"
}
...
ceventc_dump() {
	...
	echo "================================="
	display_cmd_op "CEVENTCDUMP: ceventc -i $IFNAME dump" "ceventc -i $IFNAME dump"
	## Flush ceventc log
