 ...
 *
 * $Id: hnddma_tx.c 789674 2020-08-04 20:55:39Z $
 */
...
#else  /* ! BCM_SECURE_DMA */
#if defined(OSL_CACHE_COHERENT)
		ULONGTOPHYSADDR((unsigned long)VIRT_TO_PHYS(data), pa);
#else
...
dma_txdesc(hnddma_t *dmah, dma64dd_t *dd, bool commit)
{
	...
	if (commit) {
		W_REG(di->osh, &di->d64txregs->ptr,
		    (uint32)(di->xmtptrbase + I2B(txout, di->dd64_size)));
	}
		...
		void **list_head, void **list_tail, txd_range_t range)
{
	...
	if (range == HNDDMA_RANGE_ALL)
	{
		...
		if (range == HNDDMA_RANGE_TRANSFERED) {
			...
			active_desc = B2I(active_desc, di->dd64_size);
			if (end != active_desc) {
