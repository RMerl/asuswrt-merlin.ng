...
extern cntr_group_cfg_t cntr_group_cfg[CNTR_GROUP_GROUPS_NUMBER];
...
bdmf_error_t drv_cntr_group_init(bdmf_boolean is_gateway, bdmf_boolean vlan_stats_enable)
{
    ...
    for (cntr_group_index = 0; cntr_group_index < CNTR_GROUP_GROUPS_NUMBER; cntr_group_index++)
    {
...
bdmf_error_t drv_cntr_various_counter_get(uint32_t cntr_id, uint16_t *cntr)
{
    ...
    rc = drv_cntr_counter_read(CNTR_GROUP_VARIOUS, cntr_id, cntr_arr);
    if (rc)
        return rc;
