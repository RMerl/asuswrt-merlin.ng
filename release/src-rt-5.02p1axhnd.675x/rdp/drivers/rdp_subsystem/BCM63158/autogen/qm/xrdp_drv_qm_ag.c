...
bdmf_error_t ag_drv_qm_flow_ctrl_wred_source_set(uint32_t idx, uint32_t src)
{
    uint32_t reg_flow_ctrl_wred_source=0;
    ...
    reg_flow_ctrl_wred_source = RU_FIELD_SET(0, QM, FLOW_CTRL_WRED_SOURCE, SRC, reg_flow_ctrl_wred_source, src);

    RU_REG_RAM_WRITE(0, idx, QM, FLOW_CTRL_WRED_SOURCE, reg_flow_ctrl_wred_source);

    return BDMF_ERR_OK;
}

bdmf_error_t ag_drv_qm_flow_ctrl_wred_source_get(uint32_t idx, uint32_t *src)
{
    uint32_t reg_flow_ctrl_wred_source;
    ...
    RU_REG_RAM_READ(0, idx, QM, FLOW_CTRL_WRED_SOURCE, reg_flow_ctrl_wred_source);

    *src = RU_FIELD_GET(0, QM, FLOW_CTRL_WRED_SOURCE, SRC, reg_flow_ctrl_wred_source);

    return BDMF_ERR_OK;
}

bdmf_error_t ag_drv_qm_flow_ctrl_qm_flow_ctrl_rnr_cfg_set(uint8_t rnr_bb_id, uint8_t rnr_task, bdmf_boolean rnr_enable)
{
    ...
    reg_flow_ctrl_qm_flow_ctrl_rnr_cfg = RU_FIELD_SET(0, QM, FLOW_CTRL_QM_FLOW_CTRL_RNR_CFG, RNR_BB_ID, reg_flow_ctrl_qm_flow_ctrl_rnr_cfg, rnr_bb_id);
    reg_flow_ctrl_qm_flow_ctrl_rnr_cfg = RU_FIELD_SET(0, QM, FLOW_CTRL_QM_FLOW_CTRL_RNR_CFG, RNR_TASK, reg_flow_ctrl_qm_flow_ctrl_rnr_cfg, rnr_task);
    reg_flow_ctrl_qm_flow_ctrl_rnr_cfg = RU_FIELD_SET(0, QM, FLOW_CTRL_QM_FLOW_CTRL_RNR_CFG, RNR_ENABLE, reg_flow_ctrl_qm_flow_ctrl_rnr_cfg, rnr_enable);
...
bdmf_error_t ag_drv_qm_flow_ctrl_qm_flow_ctrl_rnr_cfg_get(uint8_t *rnr_bb_id, uint8_t *rnr_task, bdmf_boolean *rnr_enable)
{
    ...
    *rnr_bb_id = RU_FIELD_GET(0, QM, FLOW_CTRL_QM_FLOW_CTRL_RNR_CFG, RNR_BB_ID, reg_flow_ctrl_qm_flow_ctrl_rnr_cfg);
    *rnr_task = RU_FIELD_GET(0, QM, FLOW_CTRL_QM_FLOW_CTRL_RNR_CFG, RNR_TASK, reg_flow_ctrl_qm_flow_ctrl_rnr_cfg);
    *rnr_enable = RU_FIELD_GET(0, QM, FLOW_CTRL_QM_FLOW_CTRL_RNR_CFG, RNR_ENABLE, reg_flow_ctrl_qm_flow_ctrl_rnr_cfg);
...
typedef enum
{
    ...
    bdmf_address_flow_ctrl_wred_source,
...
static int bcm_qm_cli_set(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    ...
    case cli_qm_flow_ctrl_wred_source:
        err = ag_drv_qm_flow_ctrl_wred_source_set(parm[1].value.unumber, parm[2].value.unumber);
        break;
    case cli_qm_flow_ctrl_qm_flow_ctrl_rnr_cfg:
        err = ag_drv_qm_flow_ctrl_qm_flow_ctrl_rnr_cfg_set(parm[1].value.unumber, parm[2].value.unumber, parm[3].value.unumber);
        break;
...
int bcm_qm_cli_get(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    ...
    {
        uint32_t src;
        err = ag_drv_qm_flow_ctrl_wred_source_get(parm[1].value.unumber, &src);
        bdmf_session_print(session, "src = %u (0x%x)\n", src, src);
        break;
    }
    case cli_qm_flow_ctrl_qm_flow_ctrl_rnr_cfg:
    {
        ...
        err = ag_drv_qm_flow_ctrl_qm_flow_ctrl_rnr_cfg_get(&rnr_bb_id, &rnr_task, &rnr_enable);
        bdmf_session_print(session, "rnr_bb_id = %u (0x%x)\n", rnr_bb_id, rnr_bb_id);
        bdmf_session_print(session, "rnr_task = %u (0x%x)\n", rnr_task, rnr_task);
...
static int bcm_qm_cli_test(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    ...
    }
    {
        ...
        if(err || iem!=gtmv(m, 22))
            return err ? err : BDMF_ERR_IO;
    }
    {
        uint32_t ist=gtmv(m, 22);
    ...
    }
    {
        ...
        uint32_t src=gtmv(m, 32);
        if(!err) bdmf_session_print(session, "ag_drv_qm_flow_ctrl_wred_source_set( %u %u)\n", idx, src);
        if(!err) ag_drv_qm_flow_ctrl_wred_source_set(idx, src);
        if(!err) ag_drv_qm_flow_ctrl_wred_source_get( idx, &src);
        if(!err) bdmf_session_print(session, "ag_drv_qm_flow_ctrl_wred_source_get( %u %u)\n", idx, src);
        if(err || src!=gtmv(m, 32))
            return err ? err : BDMF_ERR_IO;
    }
    {
        ...
        if(!err) bdmf_session_print(session, "ag_drv_qm_flow_ctrl_qm_flow_ctrl_rnr_cfg_set( %u %u %u)\n", rnr_bb_id, rnr_task, rnr_enable);
        if(!err) ag_drv_qm_flow_ctrl_qm_flow_ctrl_rnr_cfg_set(rnr_bb_id, rnr_task, rnr_enable);
        if(!err) ag_drv_qm_flow_ctrl_qm_flow_ctrl_rnr_cfg_get( &rnr_bb_id, &rnr_task, &rnr_enable);
        if(!err) bdmf_session_print(session, "ag_drv_qm_flow_ctrl_qm_flow_ctrl_rnr_cfg_get( %u %u %u)\n", rnr_bb_id, rnr_task, rnr_enable);
...
static int bcm_qm_cli_address(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    ...
    case bdmf_address_flow_ctrl_wred_source : reg = &RU_REG(QM, FLOW_CTRL_WRED_SOURCE); blk = &RU_BLK(QM); break;
...
bdmfmon_handle_t ag_drv_qm_cli_init(bdmfmon_handle_t driver_dir)
{
...

    {
        ...
        static bdmfmon_cmd_parm_t set_flow_ctrl_wred_source[]={
            BDMFMON_MAKE_PARM("idx", "idx", BDMFMON_PARM_NUMBER, 0),
            BDMFMON_MAKE_PARM("src", "src", BDMFMON_PARM_NUMBER, 0),
            BDMFMON_PARM_LIST_TERMINATOR
        };
        ...
        static bdmfmon_enum_val_t selector_table[] = {
            ...
            { .name="flow_ctrl_wred_source", .val=cli_qm_flow_ctrl_wred_source, .parms=set_flow_ctrl_wred_source },
    ...
    }
    {
        static bdmfmon_enum_val_t enum_table_address[] = {
            ...
            { .name="FLOW_CTRL_WRED_SOURCE" , .val=bdmf_address_flow_ctrl_wred_source },
