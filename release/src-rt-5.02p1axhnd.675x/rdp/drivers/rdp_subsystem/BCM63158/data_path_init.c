...
#if !defined(RDD_DHD_BACKUP_QUEUE_MEM_SIZE) && defined(_CFE_)
...
/* FIXME! double check if it overlaps with other memory */
#define NATC_DDR_RES_MCAST_WL_BASE 0x00300000
#define NATC_DDR_KEY_MCAST_WL_BASE 0x00380000
#define NATC_DDR_RES_MCAST_CNTR_BASE 0x00400000
#define NATC_DDR_KEY_MCAST_CNTR_BASE 0x00480000
#endif
...
bbh_dma_tx_profiles_init_exit:
        ...
        bbh_tx_ddr_cfg->hnsize0 = 0x40;
        bbh_tx_ddr_cfg->hnsize1 = 0x40;
    }
...
static int bbh_rx_cfg(bbh_id_e bbh_id)
{
    ...
    {
        case BBH_ID_PON:
            cfg.excl_viq = DISP_REOR_VIQ_BBH_PON_RX_EXCL;
            cfg.excl_cfg.exc_en = 1;
            break;
        case BBH_ID_AE10:
        case BBH_ID_AE2P5:
            cfg.excl_viq = (bbh_id == BBH_ID_AE10) ? DISP_REOR_VIQ_BBH_AE10_RX_EXCL : DISP_REOR_VIQ_BBH_AE2P5_RX_EXCL;
            break;
        default:
            cfg.excl_viq = bbh_id;
    }
...
{
    bdmf_error_t rc = BDMF_ERR_OK;
    bbh_rx_flow_ctrl_drops_config  flow_ctrl;
    ...
    /* Set flow ctrl towards SAR & DSL in case of runner congestion scenarios
    ** instead of dropping the data in XRDP.
    ** Useful in case of G.inp/G.fast bursty scenarios when runner is congested so as to utilize the internal
    ** buffers in SAR and DSL and in some selective cases, the backpressure can be signalled towards CO through PHY protocol mechanisms.
    **/

    flow_ctrl.dispdropdis = 1; /* Disable dropping packets due to no space in the Dispatcher. */
    flow_ctrl.sdmadropdis = 1; /* Disable dropping packets due to no space in the SDMA. */
    flow_ctrl.sbpmdropdis = 1; /* Disable dropping packets due to no space in the SBPM. */
    rc = rc ? rc : ag_drv_bbh_rx_flow_ctrl_drops_config_set(BBH_ID_DSL,  &flow_ctrl);
...
static int sbpm_init(void)
{
    ...
    thr_ug.excl_high_thr = SBPM_UG0_EXCL_HIGH_THRESHOLD;
...
static int bbh_tx_lan_init(void)
{
    ...
    config.rnr_cfg[0].skb_addr = IMAGE_0_DS_TM_BBH_TX_ABS_COUNTER_TABLE_ADDRESS >> 3;
...
static int bbh_tx_wan_init(void)
{
    ...
    bbh_id_e bbh_id;
    uint16_t thread_number, tcont_addr, ptr_addr, skb_addr;
    ...
    for (bbh_id = BBH_TX_ID_AE10; bbh_id < BBH_TX_ID_NUM; bbh_id++)
    {
        ...
        if (bbh_id == BBH_TX_ID_AE2P5)
        {
            ...
            skb_addr = IMAGE_3_US_TM_AE2P5_BBH_TX_ABS_COUNTER_TABLE_ADDRESS;
        }
        else if (bbh_id == BBH_TX_ID_AE10)
        {
            ...
            skb_addr = IMAGE_3_US_TM_AE10_BBH_TX_ABS_COUNTER_TABLE_ADDRESS;
...
static int bbh_tx_pon_init(int wantype)
{
    ...
    /* cores which sending PDs */
    config.rnr_cfg[0].rnr_src_id = get_runner_idx(us_tm_runner_image);
    config.rnr_cfg[0].skb_addr = IMAGE_3_US_TM_PON_BBH_TX_ABS_COUNTER_TABLE_ADDRESS >> 3;
    config.rnr_cfg[1].skb_addr = IMAGE_3_US_TM_PON_BBH_TX_ABS_COUNTER_TABLE_ADDRESS >> 3;
    config.src_id.stsrnrsrc = get_runner_idx(us_tm_runner_image);

    if (wantype == MAC_TYPE_EPON || wantype == MAC_TYPE_XEPON)
    {
        /* EPON mode requires 2 level scheduling, so it has to have 2 additional tasks
         * 1 more set of "update_fifo + tx_task" to deal with this 2 level scheduling. */
        if (wantype == MAC_TYPE_EPON)
        {
            config.rnr_cfg[0].tcont_addr = IMAGE_3_US_TM_EPON_BBH_TX_QUEUE_ID_TABLE_ADDRESS >> 3;
            ...
            config.sts_rnr_cfg[0].tcont_addr = IMAGE_3_US_TM_PON_BBH_TX_QUEUE_ID_TABLE_ADDRESS >> 3;
            config.sts_rnr_cfg[0].task_number = IMAGE_3_US_TM_WAN_PON_THREAD_NUMBER;
            config.sts_rnr_cfg[0].ptr_addr = IMAGE_3_US_TM_PON_BBH_TX_EGRESS_COUNTER_TABLE_ADDRESS >> 3;
        ...
        else /* wantype == MAC_TYPE_XEPON */
        {
            ...
            config.sts_rnr_cfg[0].tcont_addr = IMAGE_3_US_TM_EPON_BBH_TX_QUEUE_ID_TABLE_ADDRESS >> 3;
        ...
        /* TODO! PON team has defined different PD FIFO/Threshold for different PON.
         * Also configure pd_bytes_threshold.  If this change is required, we need to
         * work on PD memory reservation. */
    ...
    else /* GPON/XGPON */
    {
        ...
        /* TODO! PON team has defined different PD FIFO/Threshold for GPON vs XGPON.
         * Also configure pd_bytes_threshold */
        config.wan_queue_cfg.pd_fifo_base = gpon_pd_fifo_base;
        config.wan_queue_cfg.pd_fifo_size = g_gpon_pd_fifo_size;
        config.wan_queue_cfg.pd_wkup_threshold = g_gpon_pd_wkup_threshold;
...
static int runner_init(int is_basic)
{
    ...
    /* image 2, core 2 */
    ag_drv_rnr_regs_cfg_sch_cfg_set(get_runner_idx(image_2_runner_image), DRV_RNR_4SP_12RR);
    /* image 3, core 1 */
    ag_drv_rnr_regs_cfg_sch_cfg_set(get_runner_idx(image_3_runner_image), DRV_RNR_4SP_12RR);
    /* image 4, core 4 */
...
static int dispatcher_reorder_init(void)
{
    ...
    /* normal VIQs for bbh-rx LAN0 */
    dispatcher_reorder_viq_init(&cfg, &ingress_congs_init, &egress_congs_init, BB_ID_RX_BBH_0, dsptchr_viq_bbh_target_addr_normal,
        dsptchr_viq_dest_disp, dsptchr_viq_delayed, DISP_REOR_VIQ_BBH_LAN_RX0_NORMAL, DSPTCHR_GUARANTEED_MAX_LIMIT_PER_NORMAL_VIQ,
        DSPTCHR_COMMON_MAX_LIMIT_PER_VIQ, 1);

    /* normal VIQs for bbh-rx LAN1 */
    dispatcher_reorder_viq_init(&cfg, &ingress_congs_init, &egress_congs_init, BB_ID_RX_BBH_1, dsptchr_viq_bbh_target_addr_normal,
        dsptchr_viq_dest_disp, dsptchr_viq_delayed, DISP_REOR_VIQ_BBH_LAN_RX1_NORMAL, DSPTCHR_GUARANTEED_MAX_LIMIT_PER_NORMAL_VIQ,
        DSPTCHR_COMMON_MAX_LIMIT_PER_VIQ, 1);

    /* normal VIQs for bbh-rx LAN2 */
    dispatcher_reorder_viq_init(&cfg, &ingress_congs_init, &egress_congs_init, BB_ID_RX_BBH_2, dsptchr_viq_bbh_target_addr_normal,
        dsptchr_viq_dest_disp, dsptchr_viq_delayed, DISP_REOR_VIQ_BBH_LAN_RX2_NORMAL, DSPTCHR_GUARANTEED_MAX_LIMIT_PER_NORMAL_VIQ,
        DSPTCHR_COMMON_MAX_LIMIT_PER_VIQ, 1);

    /* normal VIQs for bbh-rx PON */
    dispatcher_reorder_viq_init(&cfg, &ingress_congs_init, &egress_congs_init, BB_ID_RX_PON, dsptchr_viq_bbh_target_addr_normal,
        dsptchr_viq_dest_disp, dsptchr_viq_delayed, DISP_REOR_VIQ_BBH_PON_RX_NORMAL, DSPTCHR_GUARANTEED_MAX_LIMIT_PER_NORMAL_VIQ,
        DSPTCHR_COMMON_MAX_LIMIT_PER_VIQ, 1);

    /* normal VIQs for bbh-rx AE10 */
    dispatcher_reorder_viq_init(&cfg, &ingress_congs_init, &egress_congs_init, BB_ID_RX_10G, dsptchr_viq_bbh_target_addr_normal,
        dsptchr_viq_dest_disp, dsptchr_viq_delayed, DISP_REOR_VIQ_BBH_AE10_RX_NORMAL, DSPTCHR_GUARANTEED_MAX_LIMIT_PER_NORMAL_VIQ,
        DSPTCHR_COMMON_MAX_LIMIT_PER_VIQ, 1);

    /* normal VIQs for bbh-rx AE2P5 */
    dispatcher_reorder_viq_init(&cfg, &ingress_congs_init, &egress_congs_init, BB_ID_RX_2P5, dsptchr_viq_bbh_target_addr_normal,
        dsptchr_viq_dest_disp, dsptchr_viq_delayed, DISP_REOR_VIQ_BBH_AE2P5_RX_NORMAL, DSPTCHR_GUARANTEED_MAX_LIMIT_PER_NORMAL_VIQ,
        DSPTCHR_COMMON_MAX_LIMIT_PER_VIQ, 1);

    /* normal VIQs for bbh-rx DSL */
    dispatcher_reorder_viq_init(&cfg, &ingress_congs_init, &egress_congs_init, BB_ID_RX_DSL, dsptchr_viq_bbh_target_addr_normal,
        dsptchr_viq_dest_disp, dsptchr_viq_delayed, DISP_REOR_VIQ_BBH_DSL_RX_NORMAL, DSPTCHR_GUARANTEED_MAX_LIMIT_PER_NORMAL_VIQ,
        DSPTCHR_COMMON_MAX_LIMIT_PER_VIQ, 1);

    /* All normal VIQs for BBH_RX should be added before exclusive VIQ, because the dispatcher_runner_group setting assume normal
     * VIQs for BBH_RX are grouped contiguously.
     */

    /* exclusive VIQs for bbh-rx PON */
    dispatcher_reorder_viq_init(&cfg, &ingress_congs_init, &egress_congs_init, BB_ID_RX_PON, dsptchr_viq_bbh_target_addr_excl,
        dsptchr_viq_dest_disp, dsptchr_viq_delayed, DISP_REOR_VIQ_BBH_PON_RX_EXCL, DSPTCHR_GUARANTEED_MAX_LIMIT_PER_EXCLUS_VIQ,
        DSPTCHR_COMMON_MAX_LIMIT_PER_VIQ, 1);

    /* exclusive VIQs for bbh-rx AE10 */
    dispatcher_reorder_viq_init(&cfg, &ingress_congs_init, &egress_congs_init, BB_ID_RX_10G, dsptchr_viq_bbh_target_addr_excl,
        dsptchr_viq_dest_disp, dsptchr_viq_delayed, DISP_REOR_VIQ_BBH_AE10_RX_EXCL, DSPTCHR_GUARANTEED_MAX_LIMIT_PER_EXCLUS_VIQ,
        DSPTCHR_COMMON_MAX_LIMIT_PER_VIQ, 1);

    /* exclusive VIQs for bbh-rx AE2P5 */
    dispatcher_reorder_viq_init(&cfg, &ingress_congs_init, &egress_congs_init, BB_ID_RX_2P5, dsptchr_viq_bbh_target_addr_excl,
        dsptchr_viq_dest_disp, dsptchr_viq_delayed, DISP_REOR_VIQ_BBH_AE2P5_RX_EXCL, DSPTCHR_GUARANTEED_MAX_LIMIT_PER_EXCLUS_VIQ,
        DSPTCHR_COMMON_MAX_LIMIT_PER_VIQ, 1);
    ...
    dispatcher_reorder_viq_init(&cfg, &ingress_congs_init, &egress_congs_init, get_runner_idx(cpu_tx_runner_image),
        (IMAGE_2_CPU_TX_INGRESS_DISPATCHER_CREDIT_TABLE_ADDRESS >> 3) | (IMAGE_2_IMAGE_2_CPU_TX_1_THREAD_NUMBER << 12), dsptchr_viq_dest_disp,
    ...
    dispatcher_reorder_viq_init(&cfg, &ingress_congs_init, &egress_congs_init, get_runner_idx(us_tm_runner_image),
        (IMAGE_3_US_TM_FLUSH_DISPATCHER_CREDIT_TABLE_ADDRESS >> 3) | (IMAGE_3_US_TM_FLUSH_THREAD_NUMBER << 12), dsptchr_viq_dest_reor, dsptchr_viq_delayed,
        DISP_REOR_VIQ_US_TM_FLUSH, DSPTCHR_GUARANTEED_MAX_LIMIT_PER_NORMAL_VIQ, DSPTCHR_COMMON_MAX_LIMIT_PER_VIQ, 0);

    /* VIQ for flush DS */
    dispatcher_reorder_viq_init(&cfg, &ingress_congs_init, &egress_congs_init, get_runner_idx(ds_tm_runner_image),
        (IMAGE_0_DS_TM_FLUSH_DISPATCHER_CREDIT_TABLE_ADDRESS >> 3) | (IMAGE_0_DS_TM_FLUSH_THREAD_NUMBER << 12), dsptchr_viq_dest_reor, dsptchr_viq_delayed,
        DISP_REOR_VIQ_DS_TM_FLUSH, DSPTCHR_GUARANTEED_MAX_LIMIT_PER_NORMAL_VIQ, DSPTCHR_COMMON_MAX_LIMIT_PER_VIQ, 0);

    /* VIQ for TX Mirroring - Initialized for US */
    /* FIXME!! see if we want to do TX mirror for other WAN.. right now set it to 2P5 only */
    dispatcher_reorder_viq_init(&cfg, &ingress_congs_init, &egress_congs_init, get_runner_idx(us_tm_runner_image),
        ((IMAGE_3_TX_MIRRORING_DISPATCHER_CREDIT_TABLE_ADDRESS >> 3) | (IMAGE_3_US_TM_WAN_AE2P5_THREAD_NUMBER << 12)), dsptchr_viq_dest_reor,
    ...
    /* VIQ for WAN TX DDR READ DDR prefetch */
    dispatcher_reorder_viq_init(&cfg, &ingress_congs_init, &egress_congs_init, get_runner_idx(us_tm_runner_image),
        (IMAGE_3_WAN_TX_DDR_READ_DISPATCHER_CREDIT_TABLE_ADDRESS >> 3) | (IMAGE_3_US_TM_WAN_TX_DDR_READ_THREAD_NUMBER << 12), dsptchr_viq_dest_reor,
        dsptchr_viq_delayed, DISP_REOR_VIQ_WAN_TX_DDR_READ, DSPTCHR_GUARANTEED_MAX_LIMIT_PER_NORMAL_VIQ, DSPTCHR_COMMON_MAX_LIMIT_PER_VIQ, 0);

    /* VIQ for WAN TX PSRAM WRITE DDR prefetch */
    dispatcher_reorder_viq_init(&cfg, &ingress_congs_init, &egress_congs_init, get_runner_idx(us_tm_runner_image),
        (IMAGE_3_WAN_TX_PSRAM_WRITE_DISPATCHER_CREDIT_TABLE_ADDRESS >> 3) | (IMAGE_3_US_TM_WAN_TX_PSRAM_WRITE_THREAD_NUMBER << 12), dsptchr_viq_dest_reor,
        dsptchr_viq_delayed, DISP_REOR_VIQ_WAN_TX_PSRAM_WRITE, DSPTCHR_GUARANTEED_MAX_LIMIT_PER_NORMAL_VIQ, DSPTCHR_COMMON_MAX_LIMIT_PER_VIQ, 0);

#ifdef CONFIG_DHD_RUNNER
    /* VIQ for DHD_RX_COMPLETE_0 */
    dispatcher_reorder_viq_init(&cfg, &ingress_congs_init, &egress_congs_init, get_runner_idx(dhd_complete_runner_image),
        (IMAGE_5_DHD_RX_COMPLETE_0_DISPATCHER_CREDIT_TABLE_ADDRESS >> 3) | (IMAGE_5_PROCESSING_DHD_RX_COMPLETE_0_THREAD_NUMBER << 12),
    ...
    /* VIQ for DHD_RX_COMPLETE_1 */
    dispatcher_reorder_viq_init(&cfg, &ingress_congs_init, &egress_congs_init, get_runner_idx(dhd_complete_runner_image),
        (IMAGE_5_DHD_RX_COMPLETE_1_DISPATCHER_CREDIT_TABLE_ADDRESS >> 3) | (IMAGE_5_PROCESSING_DHD_RX_COMPLETE_1_THREAD_NUMBER << 12),
        dsptchr_viq_dest_disp, dsptchr_viq_delayed, DISP_REOR_VIQ_DHD_RX_COMPLETE_1, DSPTCHR_GUARANTEED_MAX_LIMIT_PER_NORMAL_VIQ,
        DSPTCHR_COMMON_MAX_LIMIT_PER_VIQ, 0);

    /* VIQ for DHD_RX_COMPLETE_2 */
    dispatcher_reorder_viq_init(&cfg, &ingress_congs_init, &egress_congs_init, get_runner_idx(dhd_complete_runner_image),
        (IMAGE_5_DHD_RX_COMPLETE_2_DISPATCHER_CREDIT_TABLE_ADDRESS >> 3) | (IMAGE_5_PROCESSING_DHD_RX_COMPLETE_2_THREAD_NUMBER << 12),
    ...
    dispatcher_reorder_viq_init(&cfg, &ingress_congs_init, &egress_congs_init, get_runner_idx(dhd_tx_post_runner_image),
        (IMAGE_4_DHD_MCAST_DISPATCHER_CREDIT_TABLE_ADDRESS >> 3) | (IMAGE_4_IMAGE_4_DHD_MCAST_THREAD_NUMBER << 12), dsptchr_viq_dest_reor,
    ...
    /* Common VIQ for DHD Mirroring */
    dispatcher_reorder_viq_init(&cfg, &ingress_congs_init, &egress_congs_init, get_runner_idx(dhd_tx_post_runner_image),
	    (IMAGE_4_DHD_MIRRORING_DISPATCHER_CREDIT_TABLE_ADDRESS >> 3 | IMAGE_4_IMAGE_4_DHD_TX_POST_0_THREAD_NUMBER << 12),
		dsptchr_viq_dest_reor, dsptchr_viq_delayed, DISP_REOR_VIQ_DHD_MIRRORING, DSPTCHR_GUARANTEED_MAX_LIMIT_PER_NORMAL_VIQ,
		DSPTCHR_COMMON_MAX_LIMIT_PER_VIQ, 0);

    /* VIQ for DHD_TX_COMPLETE_0 */
    dispatcher_reorder_viq_init(&cfg, &ingress_congs_init, &egress_congs_init, get_runner_idx(dhd_complete_runner_image),
        (IMAGE_5_DHD_TX_COMPLETE_0_DISPATCHER_CREDIT_TABLE_ADDRESS >> 3) | (IMAGE_5_PROCESSING_DHD_TX_COMPLETE_0_THREAD_NUMBER << 12),
        dsptchr_viq_dest_reor, dsptchr_viq_delayed, DISP_REOR_VIQ_DHD_TX_COMPLETE_0, DSPTCHR_GUARANTEED_MAX_LIMIT_PER_NORMAL_VIQ,
        DSPTCHR_COMMON_MAX_LIMIT_PER_VIQ, 0);

    /* VIQ for DHD_TX_COMPLETE_1 */
    dispatcher_reorder_viq_init(&cfg, &ingress_congs_init, &egress_congs_init, get_runner_idx(dhd_complete_runner_image),
        (IMAGE_5_DHD_TX_COMPLETE_1_DISPATCHER_CREDIT_TABLE_ADDRESS >> 3) | (IMAGE_5_PROCESSING_DHD_TX_COMPLETE_1_THREAD_NUMBER << 12),
        dsptchr_viq_dest_reor, dsptchr_viq_delayed, DISP_REOR_VIQ_DHD_TX_COMPLETE_1, DSPTCHR_GUARANTEED_MAX_LIMIT_PER_NORMAL_VIQ,
        DSPTCHR_COMMON_MAX_LIMIT_PER_VIQ, 0);

    /* VIQ for DHD_TX_COMPLETE_2 */
    dispatcher_reorder_viq_init(&cfg, &ingress_congs_init, &egress_congs_init, get_runner_idx(dhd_complete_runner_image),
        (IMAGE_5_DHD_TX_COMPLETE_2_DISPATCHER_CREDIT_TABLE_ADDRESS >> 3) | (IMAGE_5_PROCESSING_DHD_TX_COMPLETE_2_THREAD_NUMBER << 12),
    ...
    {
        /* VIQ for SERVICE QUEUES */
        dispatcher_reorder_viq_init(&cfg, &ingress_congs_init, &egress_congs_init, get_runner_idx(service_queues_runner_image),
            (IMAGE_0_SERVICE_QUEUES_DISPATCHER_CREDIT_TABLE_ADDRESS >> 3 | IMAGE_0_DS_TM_SERVICE_QUEUES_TX_THREAD_NUMBER << 12),
    ...
    /* VIQ for epon tm  */
    dispatcher_reorder_viq_init(&cfg, &ingress_congs_init, &egress_congs_init, get_runner_idx(us_tm_runner_image),
        (IMAGE_3_DISPATCHER_CREDIT_TABLE_ADDRESS >> 3) | (IMAGE_3_US_TM_WAN_PON_THREAD_NUMBER << 12), dsptchr_viq_dest_reor,
...
#endif
    cfg.dsptchr_rnr_group_list[1].queues_mask = (1 << DISP_REOR_VIQ_BBH_PON_RX_EXCL);
    cfg.dsptchr_rnr_group_list[1].queues_mask |= (1 << DISP_REOR_VIQ_BBH_AE10_RX_EXCL) | (1 << DISP_REOR_VIQ_BBH_AE2P5_RX_EXCL);
...
typedef struct {
    ...
    uint32_t key_mask;
    key_len_t key_len;
    ddr_nat_table_size_t tbl_size;
    ddr_bins_per_bucket_t bins;
    total_length_t total_entry_len;
    void *tbl_base_addr;
}natc_table_cfg_t;


#ifdef XRDP_EMULATION
static void natc_init_table_addr(natc_config_t *cfg, rdd_natc_tbl_idx tbl_idx, void *key, void *res)
{
    cfg->tbl_cfg[tbl_idx].vir_addr.key = key;
    cfg->tbl_cfg[tbl_idx].vir_addr.res = res;
    cfg->tbl_cfg[tbl_idx].phy_addr.key = (bdmf_phys_addr_t)key;
    cfg->tbl_cfg[tbl_idx].phy_addr.res = (bdmf_phys_addr_t)res;
}
#endif

/* Function to build/fill the natc table configuation parameters */
static void natc_init_build_table_cfg(natc_config_t *cfg, rdd_natc_tbl_idx tbl_idx, natc_table_cfg_t *tbl_cfg)
{
    ...
    cfg->tbl_cfg[tbl_idx].mask = tbl_cfg->key_mask;
...
static int natc_init(void)
{
    int rc = BDMF_ERR_OK;
    int tbl_idx;
    uint32_t natc_tbls_avail_mem;
    void *tbl_start_addr;
    natc_config_t cfg;
    natc_table_cfg_t tbl_cfg;
    ...
    natc_ddr_cfg_total_len total_len =
                                    {.total_len_tbl0 = total_len_144B,
                                     .total_len_tbl1 = total_len_144B,
                                     .total_len_tbl2 = total_len_80B,
                                     .total_len_tbl3 = total_len_80B,
                                     .total_len_tbl4 = total_len_def,
                                     .total_len_tbl5 = total_len_def,
                                     .total_len_tbl6 = total_len_def,
                                     .total_len_tbl7 = total_len_def};

    /* Memory for DHD-Offload Backup Queues comes from Runner-Tables-Memory */
    natc_tbls_avail_mem = p_dpi_cfg->rnr_tables_buf_size - RDD_DHD_BACKUP_QUEUE_MEM_SIZE;

    if (natc_tbl_database_get_tbl_size(&ddr_size, &total_len, NATC_DEFAULT_BIN_SIZE, natc_tbls_num, natc_tbls_avail_mem))
    {
    ...
    /* Assign the total number of tables in use */
    cfg.tbl_num = natc_tbls_num;
    /* Assign the start address for the first table */
    tbl_start_addr = p_dpi_cfg->rdp_ddr_rnr_tables_base_virt;

    for (tbl_idx = 0; tbl_idx < natc_tbls_num; tbl_idx++)
    {
        /* Setup default configuration */
        tbl_cfg.key_len = key_len_16B;
#if defined(USE_NATC_VAR_CONTEXT_LEN)
        tbl_cfg.var_cntx = 1;
#else
        tbl_cfg.var_cntx = 0;
#endif
        tbl_cfg.non_cacheable = 0;
        tbl_cfg.key_mask = NATC_16BYTE_KEY_MASK;
        tbl_cfg.tbl_size = _natc_tbl_ddr_size_enum(tbl_idx, &ddr_size);
        tbl_cfg.bins = NATC_DEFAULT_BIN_SIZE;
        tbl_cfg.total_entry_len = _natc_tbl_entry_size_enum(tbl_idx, &total_len);
        tbl_cfg.tbl_base_addr = tbl_start_addr;

        /* Table specific exceptions */
        switch (tbl_idx)
        {
            case NATC_TBL_IDX_DS:
            case NATC_TBL_IDX_US:
                break;

            case NATC_TBL_IDX_COUNTERS:
                tbl_cfg.var_cntx = 0;
                tbl_cfg.non_cacheable = 1;
                break;

            case NATC_TBL_IDX_MCAST_WHITELIST:
                tbl_cfg.var_cntx = 0;
                /* masking first 3 byte */
                tbl_cfg.key_mask = NATC_16BYTE_KEY_MASK_FOR_MCAST_WHITELIST;
                /* Disable caching capability in this NAT Cache table */
                tbl_cfg.non_cacheable = 1;
                break;

            default:
                /* Error */
                BDMF_TRACE_ERR("Invalid NATC table %d\n",tbl_idx);
                return BDMF_ERR_INTERNAL;
                break;
        }
        /* Build the table configuration */
        natc_init_build_table_cfg(&cfg, tbl_idx, &tbl_cfg);
        /* Update table start address for next table */
        tbl_start_addr = (uint8_t*)tbl_start_addr + cfg.tbl_cfg[tbl_idx].key_tbl_size + cfg.tbl_cfg[tbl_idx].res_tbl_size;

#ifdef XRDP_EMULATION
        switch (tbl_idx)
        {
            case NATC_TBL_IDX_DS:
                natc_init_table_addr(&cfg, tbl_idx, NATC_DDR_KEY_DS_BASE, NATC_DDR_RES_DS_BASE);
                break;

            case NATC_TBL_IDX_US:
                natc_init_table_addr(&cfg, tbl_idx, NATC_DDR_KEY_US_BASE, NATC_DDR_RES_US_BASE);
                break;

            case NATC_TBL_IDX_COUNTERS:
                natc_init_table_addr(&cfg, tbl_idx, NATC_DDR_KEY_MCAST_CNTR_BASE, NATC_DDR_RES_MCAST_CNTR_BASE);
                break;

            case NATC_TBL_IDX_MCAST_WHITELIST:
                natc_init_table_addr(&cfg, tbl_idx, NATC_DDR_KEY_MCAST_WL_BASE, NATC_DDR_RES_MCAST_WL_BASE);
                break;
        ...
        bdmf_trace("Flow table[%d]:\t\tkey(size@phy_addr) = %d@0x%p, res(size@phy_addr) = %d@0x%p\n",
                   tbl_idx, cfg.tbl_cfg[tbl_idx].key_tbl_size,
                   (void *)cfg.tbl_cfg[tbl_idx].phy_addr.key,
                   cfg.tbl_cfg[tbl_idx].res_tbl_size,
                   (void *)cfg.tbl_cfg[tbl_idx].phy_addr.res);
    }

    /* Need to configure the physical address of whitelist table in runner SRAM */
    rdd_multicast_whitelist_context_table_addr_set(0x0, cfg.tbl_cfg[NATC_TBL_IDX_MCAST_WHITELIST].phy_addr.res);

    /* marks end of NATC memory */
    natc_mem_end_addr = tbl_start_addr;
    natc_mem_available_size = p_dpi_cfg->rnr_tables_buf_size - (int)(natc_mem_end_addr - p_dpi_cfg->rdp_ddr_rnr_tables_base_virt);

    bdmf_trace("%s:%d:natc_mem_available_size = %d\n", __func__, __LINE__, natc_mem_available_size);
#ifndef XRDP_EMULATION
    if (natc_mem_available_size < 0)
        return BDMF_ERR_NOMEM;
#endif
    ...
    rc = rc ? rc : ag_drv_natc_key_mask_tbl_key_mask_set(NATC_TBL_IDX_DS, NATC_16BYTE_KEY_MASK);
    rc = rc ? rc : ag_drv_natc_key_mask_tbl_key_mask_set(NATC_TBL_IDX_US, NATC_16BYTE_KEY_MASK);
    rc = rc ? rc : ag_drv_natc_key_mask_tbl_key_mask_set(NATC_TBL_IDX_COUNTERS, NATC_16BYTE_KEY_MASK);
    rc = rc ? rc : ag_drv_natc_key_mask_tbl_key_mask_set(NATC_TBL_IDX_MCAST_WHITELIST, NATC_16BYTE_KEY_MASK_FOR_MCAST_WHITELIST);
...
static int qm_wantype_init(int wantype)
{
    ...
    if (wantype == MAC_TYPE_EPON)
    {
        /* setting up the EPON 2nd stage queues */
...
static int qm_adjust_reservation(void)
{
    uint32_t idx = 0;
    bdmf_error_t rc = BDMF_ERR_OK;
    qm_q_context queue_cfg = {};
    ...
    /* change the existing EPON queue configuration to match with default US
     * queue configuraiton */
    ...
    rc = rc ? rc : drv_qm_merge_us_epon_queue_reservation();
    rc = rc ? rc : rdd_us_queue_end_update_context();

    /* update QM Runner Group CFG for group#1 */
    rc = rc ? rc : ag_drv_qm_rnr_group_cfg_get(qm_rnr_group_1, &rnr_group_cfg);
    rnr_group_cfg.end_queue = drv_qm_get_us_end();
    rc = rc ? rc : ag_drv_qm_rnr_group_cfg_set(qm_rnr_group_1, &rnr_group_cfg);

#ifdef EPON
    /* disable QM Runner Group CFG for group#2 */
    rc = rc ? rc : ag_drv_qm_rnr_group_cfg_get(qm_rnr_group_2, &rnr_group_cfg);
    rnr_group_cfg.rnr_enable = 0;
...
static int qm_init(void)
{
    ...
    qm_init_cfg init_cfg =
    {
        ...
        .ddr_sop_offset = {RDD_DDR_SOP_OFFSET0, RDD_DDR_SOP_OFFSET1}      /* DDR SoP offsets profile0 - everything except multicast , profile1 - for multicast, */
    };
    ...
    /* setup 1 queue, if there is change in US QM Queue configuration, please
     * apply the same to the queue configuration in "qm_adjust_reservation"  */
    queue_cfg.wred_profile = 1;                 /* WRED profile */
    queue_cfg.copy_dec_profile = QM_COPY_DEC_PROFILE_LOW; /* Copy decision profile */
    queue_cfg.fpm_ug = FPM_US_UG;               /* FPM UG */
    queue_cfg.ddr_copy_disable = 0;             /* TRUE=never copy to DDR */
...
#ifdef EPON
    /* us - group 2 (epon update fifo task) */
    ...
    rnr_group_cfg.rnr_task = IMAGE_3_US_TM_EPON_UPDATE_FIFO_THREAD_NUMBER;
    ...
    rnr_group_cfg.pd_fifo_base = (IMAGE_4_DHD_MCAST_PD_FIFO_TABLE_ADDRESS >> 3);
    rnr_group_cfg.rnr_task = IMAGE_4_IMAGE_4_DHD_MCAST_THREAD_NUMBER;
    rnr_group_cfg.upd_fifo_base = (IMAGE_4_DHD_MCAST_UPDATE_FIFO_TABLE_ADDRESS >> 3);
    rnr_group_cfg.pd_fifo_size = qm_pd_fifo_size_2;
    rnr_group_cfg.upd_fifo_size = qm_update_fifo_size_8;
    rnr_group_cfg.rnr_bb_id = get_runner_idx(dhd_tx_post_runner_image);
    rnr_group_cfg.rnr_enable = 1;
    ...
    if (p_dpi_cfg->number_of_service_queues)
    {
        ...
        rnr_group_cfg.rnr_task = IMAGE_0_DS_TM_SERVICE_QUEUES_UPDATE_FIFO_THREAD_NUMBER;
        rnr_group_cfg.rnr_enable = 1;
...
uint32_t data_path_init_fiber(int wan_bbh)
{
    ...
    if (wan_bbh == MAC_TYPE_EPON)
        ...
        rc = rc ? rc : qm_adjust_reservation();
