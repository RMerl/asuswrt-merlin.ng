...
/* In case XRDP_CFE builds don't include rdd_dhd_helper.h */
#if !defined(RDD_DHD_BACKUP_QUEUE_MEM_SIZE)
#define RDD_DHD_BACKUP_QUEUE_MEM_SIZE   (0)
#endif

/* Reserved memory for VLAN actions table */
#define NATC_MEMORY_RESERVED  (8*1024*80)
...
bbh_id_e g_dma_to_bbh_rx_x[NUM_OF_DMA][NUM_OF_PERIPHERALS_PER_DMA] = {
          {BBH_ID_0, BBH_ID_1, BBH_ID_2, BBH_ID_3, BBH_ID_4, BBH_ID_5,BBH_ID_NULL},
          {BBH_ID_PON,BBH_ID_NULL,BBH_ID_NULL,BBH_ID_NULL,BBH_ID_NULL,BBH_ID_NULL,BBH_ID_NULL} };
          

/* in TX, only 2 BBHs approaches the DMA (BBH-Tx LAN -> DMA0,   BBH-Tx-WAN -> DMA1) */
bbh_to_dma_x_t  g_bbh_tx_to_dma_x[BBH_TX_ID_NUM] = {
          {BBH_ID_0, DMA0_ID},
          {BBH_ID_PON, DMA1_ID}};
...
uint8_t g_bbh_rx_buff_num[NUM_OF_DMA][NUM_OF_PERIPHERALS_PER_DMA] = {{5, 5, 5, 5, 5, 7},{32}};
uint8_t g_bbh_tx_buff_num[NUM_OF_DMA][NUM_OF_PERIPHERALS_PER_DMA] = {{15},{15}};  // in tx, each DMA has 1 bbh that approach it

/* this array, per DMA, is needed because there are 2 BBHtx, that each approaches one DMA and one SDMA (only).
   The DMA know that for Tx, the FIRST  bb_id is either the TX_LAN  or  TX_PON
   (the rest of the sources are not important, since they will not access the DMA, anyway)
   this array is similar to g_dma_to_bbh_tx_x & g_dma_to_bbh_tx_x  that also has entry per DMA, telling it which BBH approaches it */
bb_source_t g_dma_bb_source[DMA_NUM][BBH_ID_NUM] = {  /* DAM0, DMA1, SDMA0, SDMA1.   each entry is {rx,  tx} */
          {{BB_ID_RX_BBH_0, BB_ID_TX_LAN},
           ...
           {BB_ID_RX_BBH_5, 0},
           {BB_ID_RX_PON,   0}},
          {{BB_ID_RX_BBH_0, BB_ID_TX_PON_ETH_PD},  // this is US DMA
           {BB_ID_RX_BBH_1, 0},
           {BB_ID_RX_BBH_2, 0},
           {BB_ID_RX_BBH_3, 0},
           {BB_ID_RX_BBH_4, 0},
          ...
          {{BB_ID_RX_BBH_0, BB_ID_TX_LAN},
           ...
           {BB_ID_RX_BBH_5, 0},
           {BB_ID_RX_PON,   0}},
          {{BB_ID_RX_BBH_0, BB_ID_TX_PON_ETH_PD},  // this is US SDMA
           {BB_ID_RX_BBH_1, 0},
           {BB_ID_RX_BBH_2, 0},
           {BB_ID_RX_BBH_3, 0},
           {BB_ID_RX_BBH_4, 0},
...
uint8_t g_max_otf_reads[BBH_ID_NUM] = 
         {MAX_OTF_READ_REQUEST_DEFAULT_DMA0, MAX_OTF_READ_REQUEST_DEFAULT_DMA0,
          MAX_OTF_READ_REQUEST_DEFAULT_DMA0, MAX_OTF_READ_REQUEST_DEFAULT_DMA0,
          MAX_OTF_READ_REQUEST_DEFAULT_DMA0, MAX_OTF_READ_REQUEST_DEFAULT_DMA0,
          MAX_OTF_READ_REQUEST_DEFAULT_DMA1};
...
static uint8_t calculate_buffers_offset_rx(uint8_t dma_num, uint8_t periphreal_num)
{
    uint8_t j, offset = 0;

    for (j = 0; j < periphreal_num; j++)
    {
       offset = offset + g_bbh_rx_buff_num[dma_num][j];
    }
    return offset;
}

static uint8_t calculate_buffers_offset_tx(uint8_t dma_num, uint8_t periphreal_num)
{
    uint8_t j, offset = 0;

    for (j = 0; j < periphreal_num; j++)
    {
       offset = offset + g_bbh_tx_buff_num[dma_num][j];
    }
    return offset;
}
static void bbh_rx_profiles_init(void)
{
    ...
    for (bbh_id = 0; bbh_id < BBH_ID_NUM; bbh_id++)
    {
        ...
        for (i = 0; i < NUM_OF_DMA; i++)
        {
            ...
            {
                if (g_bbh_rx_to_dma_x[bbh_id].bbh_id == g_dma_to_bbh_rx_x[i][j]) 
				{
...
bbh_rx_profiles_init_exit:
        ...
        bbh_rx_sdma_cfg->sdma_bb_id = (g_bbh_rx_to_dma_x[bbh_id].dma_id == DMA0_ID)? BB_ID_SDMA0 : BB_ID_SDMA1;
        bbh_rx_sdma_cfg->first_chunk_idx = calculate_buffers_offset_rx(i, j) * BBH_RX_TOTAL_BUFFER_NUM / TOTAL_BUFFERS_WEIGHT;
        bbh_rx_sdma_cfg->sdma_chunks = g_bbh_rx_buff_num[i][j] * BBH_RX_TOTAL_BUFFER_NUM / TOTAL_BUFFERS_WEIGHT;
    }
}
    ...
    for (bbh_id = BBH_TX_ID_FIRST; bbh_id < BBH_TX_ID_NUM; bbh_id++)
    {
        ...
        for (i = 0; i < NUM_OF_DMA; i++)
        {
            for (j = 0; j < NUM_OF_PERIPHERALS_PER_DMA; j++)
            {
                ...
                {
                    goto bbh_tx_profiles_init_exit;
                }
            }
        }
        continue;
         
bbh_tx_profiles_init_exit:


        bbh_tx_dma_cfg = &(g_bbh_tx_dma_profile->bbh_tx_dma_cfg[bbh_id]);
        bbh_tx_dma_cfg->dmasrc =  (g_bbh_tx_to_dma_x[bbh_id].dma_id == DMA0_ID)? BB_ID_DMA0 : BB_ID_DMA1;
        bbh_tx_dma_cfg->descbase = calculate_buffers_offset_tx(i, j) * BBH_TX_TOTAL_BUFFER_NUM / TOTAL_BUFFERS_WEIGHT;
        bbh_tx_dma_cfg->descsize = g_bbh_tx_buff_num[i][j] * BBH_TX_TOTAL_BUFFER_NUM / TOTAL_BUFFERS_WEIGHT;

        bbh_tx_sdma_cfg = &(g_bbh_tx_sdma_profile->bbh_tx_sdma_cfg[bbh_id]);
        bbh_tx_sdma_cfg->sdmasrc = (g_bbh_tx_to_dma_x[bbh_id].dma_id == DMA0_ID)? BB_ID_SDMA0 :BB_ID_SDMA1 ;
        bbh_tx_sdma_cfg->descbase = calculate_buffers_offset_tx(i, j) * BBH_TX_TOTAL_BUFFER_NUM / TOTAL_BUFFERS_WEIGHT;
        bbh_tx_sdma_cfg->descsize = g_bbh_tx_buff_num[i][j] * BBH_TX_TOTAL_BUFFER_NUM / TOTAL_BUFFERS_WEIGHT;
...
static int bbh_rx_pon_init(int wan_bbh)
{
    ...
    {
        rc = rc ? rc : ag_drv_bbh_rx_disable_normal_check_set(BBH_ID_PON, 1);
...
static int cnpl_init(bdmf_boolean is_gateway, bdmf_boolean vlan_stats_enable)
{
    ...
    /* disable color aware */
    rc = rc ? rc : ag_drv_cnpl_misc_col_awr_en_set(0);
...
static int bbh_tx_pon_init(int wantype)
{
    ...
    if (wantype == MAC_TYPE_EPON || wantype == MAC_TYPE_XEPON)
    {
        ...
        if (wantype == MAC_TYPE_EPON)
        {
            ...
            config.sts_rnr_cfg[0].tcont_addr = IMAGE_3_US_TM_WAN_0_BBH_TX_WAKE_UP_DATA_TABLE_ADDRESS >> 3;
            config.sts_rnr_cfg[0].task_number = IMAGE_3_US_TM_WAN_THREAD_NUMBER;
            config.sts_rnr_cfg[0].ptr_addr = IMAGE_3_US_TM_WAN_0_BBH_TX_EGRESS_COUNTER_TABLE_ADDRESS >> 3;
        ...
        else /* wantype == MAC_TYPE_XEPON */
        {
            ...
            config.rnr_cfg[0].tcont_addr = IMAGE_3_US_TM_WAN_0_BBH_TX_WAKE_UP_DATA_TABLE_ADDRESS >> 3;
            config.rnr_cfg[0].task_number = IMAGE_3_US_TM_WAN_THREAD_NUMBER;
            config.rnr_cfg[0].ptr_addr = IMAGE_3_US_TM_WAN_0_BBH_TX_EGRESS_COUNTER_TABLE_ADDRESS >> 3;
        ...
        rc = rc ? rc : ag_drv_bbh_tx_sdma_epon_urgent_set(BBH_TX_WAN_ID, EPON_URGENT_REQUEST_ENABLE);
    ...
    fpm_base_phys_addr = p_dpi_cfg->rdp_ddr_pkt_base_phys;
    ...
    config.rnr_cfg[0].skb_addr = IMAGE_3_US_TM_CPU_TX_ABS_COUNTERS_ADDRESS >> 3;
    config.rnr_cfg[1].skb_addr = IMAGE_3_US_TM_CPU_TX_ABS_COUNTERS_ADDRESS >> 3;
...
static int bbh_tx_init(void)
{
    ...
    config.rnr_cfg[1].tcont_addr = IMAGE_3_US_TM_WAN_0_BBH_TX_WAKE_UP_DATA_TABLE_ADDRESS >> 3;
    config.rnr_cfg[1].ptr_addr = IMAGE_3_US_TM_WAN_0_BBH_TX_EGRESS_COUNTER_TABLE_ADDRESS >> 3;
    ...
    if (p_dpi_cfg->bbh_id_gbe_wan != BBH_ID_NULL)
    {
        if (p_dpi_cfg->bbh_id_gbe_wan & 0x1)
            config.lan_queue_cfg.queue_to_rnr[(p_dpi_cfg->bbh_id_gbe_wan >> 1)].q1 = RNR1;
        else
            config.lan_queue_cfg.queue_to_rnr[(p_dpi_cfg->bbh_id_gbe_wan >> 1)].q0 = RNR1;
    ...
    config.rnr_cfg[0].skb_addr = IMAGE_0_DS_TM_CPU_TX_ABS_COUNTERS_ADDRESS >> 3;
    config.rnr_cfg[1].skb_addr = IMAGE_0_DS_TM_CPU_TX_ABS_COUNTERS_ADDRESS >> 3;
...
int bbh_tx_reconfig(bbh_id_e bbh_id_gbe_wan, bbh_id_e bbh_id)
{
    bbh_tx_config config;
    bdmf_error_t rc = BDMF_ERR_OK;
    bdmf_boolean lan_queue_to_rnr_q0, lan_queue_to_rnr_q1;
    
    
    p_dpi_cfg->bbh_id_gbe_wan = bbh_id_gbe_wan;
    
    xrdp_memset(&config, 0, sizeof(bbh_tx_config));
  
    drv_bbh_tx_common_configurations_q2rnr_get_lan(BBH_TX_DS_ID, (bbh_id >> 1), 
        &lan_queue_to_rnr_q0, &lan_queue_to_rnr_q1);
    
    rc = rc ? rc : ag_drv_bbh_tx_common_configurations_perqtask_get(BBH_TX_DS_ID, &config.per_q_task);

    /* ---- set new config  */
    if (bbh_id_gbe_wan == bbh_id)
    {
        if (bbh_id & 0x1)
            lan_queue_to_rnr_q1 = RNR1;
        else
            lan_queue_to_rnr_q0 = RNR1;
            
        *(((uint8_t *)&config.per_q_task) + bbh_id) = IMAGE_3_US_TM_WAN_THREAD_NUMBER;
    }
    else
    {
        if (bbh_id & 0x1)
            lan_queue_to_rnr_q1 = RNR0;
        else
            lan_queue_to_rnr_q0 = RNR0;
            
        *(((uint8_t *)&config.per_q_task) + bbh_id) = IMAGE_0_DS_TM_TX_TASK_THREAD_NUMBER;
    }

    rc = rc ? rc : drv_bbh_tx_common_configurations_q2rnr_set_lan(BBH_TX_DS_ID, (bbh_id >> 1), 
          lan_queue_to_rnr_q0, lan_queue_to_rnr_q1);
          
    rc = rc ? rc : ag_drv_bbh_tx_common_configurations_perqtask_set(BBH_TX_DS_ID, &config.per_q_task);


    if (rc)
        BDMF_TRACE_ERR("Failed to reconfigure bbh_tx driver");
...
static int runner_init(int is_basic)
{
    ...
    ag_drv_rnr_regs_cfg_sch_cfg_set(get_runner_idx(image_2_runner_image), DRV_RNR_8SP_8RR);
    ...
    ag_drv_rnr_regs_cfg_sch_cfg_set(get_runner_idx(image_6_runner_image), DRV_RNR_8SP_8RR);
    ag_drv_rnr_regs_cfg_sch_cfg_set(get_runner_idx(image_7_runner_image), DRV_RNR_8SP_8RR);
    ...
    rdd_init_params.is_gateway = p_dpi_cfg->is_gateway;
...
static int dispatcher_reorder_init(void)
{
    dsptchr_config cfg = {};
    bdmf_error_t rc = BDMF_ERR_OK;
    uint8_t core_index, i, number_of_cores;
    ...
    /* normal VIQs for bbh-rx - 5*/
    dispatcher_reorder_viq_init(&cfg, &ingress_congs_init, &egress_congs_init, BB_ID_RX_BBH_5, dsptchr_viq_bbh_target_addr_normal,
    ...
    /* normal BB_ID_RX_PON for bbh-rx WAN - 6*/
    dispatcher_reorder_viq_init(&cfg, &ingress_congs_init, &egress_congs_init, BB_ID_RX_PON, dsptchr_viq_bbh_target_addr_normal, dsptchr_viq_dest_disp,
        dsptchr_viq_delayed, DISP_REOR_VIQ_BBH_RX6_NORMAL, DSPTCHR_GUARANTEED_MAX_LIMIT_PER_NORMAL_VIQ, DSPTCHR_COMMON_MAX_LIMIT_PER_VIQ, 1);
        
    /* exclusive BB_ID_RX_PON for bbh-rx WAN - 7*/
    dispatcher_reorder_viq_init(&cfg, &ingress_congs_init, &egress_congs_init, BB_ID_RX_PON, dsptchr_viq_bbh_target_addr_excl, dsptchr_viq_dest_disp,
        dsptchr_viq_delayed, DISP_REOR_VIQ_BBH_WAN_EXCL, DSPTCHR_GUARANTEED_MAX_LIMIT_PER_EXCLUS_VIQ, DSPTCHR_COMMON_MAX_LIMIT_PER_VIQ, 1);
    ...
    /* VIQ for cpu_tx_task (egress) 0, 1 - 9 */
    dispatcher_reorder_viq_init(&cfg, &ingress_congs_init, &egress_congs_init, get_runner_idx(cpu_tx_runner_image),
        (IMAGE_2_CPU_TX_EGRESS_DISPATCHER_CREDIT_TABLE_ADDRESS >> 3 | IMAGE_2_IMAGE_2_CPU_TX_0_THREAD_NUMBER << 12),
    ...
    /* VIQ for cpu_tx_task 0, 1 (ingress) - 10 */
    dispatcher_reorder_viq_init(&cfg, &ingress_congs_init, &egress_congs_init, get_runner_idx(cpu_tx_runner_image),
        (IMAGE_2_CPU_TX_INGRESS_DISPATCHER_CREDIT_TABLE_ADDRESS >> 3 | IMAGE_2_IMAGE_2_CPU_TX_1_THREAD_NUMBER << 12),
        dsptchr_viq_dest_disp, dsptchr_viq_delayed, DISP_REOR_VIQ_CPU_TX_FORWARD, DSPTCHR_GUARANTEED_MAX_LIMIT_PER_NORMAL_VIQ,
        DSPTCHR_COMMON_MAX_LIMIT_PER_VIQ, 0);

    /* VIQ for flush US - 11 */
    dispatcher_reorder_viq_init(&cfg, &ingress_congs_init, &egress_congs_init, get_runner_idx(us_tm_runner_image),
    	(IMAGE_3_US_TM_FLUSH_DISPATCHER_CREDIT_TABLE_ADDRESS >> 3) | (IMAGE_3_US_TM_FLUSH_THREAD_NUMBER << 12), dsptchr_viq_dest_reor,
		dsptchr_viq_delayed, DISP_REOR_VIQ_US_TM_FLUSH, DSPTCHR_GUARANTEED_MAX_LIMIT_PER_NORMAL_VIQ, DSPTCHR_COMMON_MAX_LIMIT_PER_VIQ, 0);

    /* VIQ for flush DS - 12 */
    dispatcher_reorder_viq_init(&cfg, &ingress_congs_init, &egress_congs_init, get_runner_idx(ds_tm_runner_image),
    	(IMAGE_0_DS_TM_FLUSH_DISPATCHER_CREDIT_TABLE_ADDRESS >> 3) | (IMAGE_0_DS_TM_FLUSH_THREAD_NUMBER << 12), dsptchr_viq_dest_reor,
		dsptchr_viq_delayed, DISP_REOR_VIQ_DS_TM_FLUSH, DSPTCHR_GUARANTEED_MAX_LIMIT_PER_NORMAL_VIQ, DSPTCHR_COMMON_MAX_LIMIT_PER_VIQ, 0);

    /* VIQ for TX Mirroring - Initialized for US - 13 */
    dispatcher_reorder_viq_init(&cfg, &ingress_congs_init, &egress_congs_init, get_runner_idx(us_tm_runner_image),
    ...
    /* VIQ for CPU_RX_copy - 14 */
    dispatcher_reorder_viq_init(&cfg, &ingress_congs_init, &egress_congs_init, get_runner_idx(cpu_rx_runner_image),
    	(IMAGE_1_CPU_RX_COPY_DISPATCHER_CREDIT_TABLE_ADDRESS >> 3 | IMAGE_1_IMAGE_1_CPU_RX_COPY_THREAD_NUMBER << 12), dsptchr_viq_dest_reor,
    ...
    /* Another VIQ for WAN loopback - 15 */
    dispatcher_reorder_viq_init(&cfg, &ingress_congs_init, &egress_congs_init, get_runner_idx(cpu_rx_runner_image),
    	(IMAGE_1_WAN_LOOPBACK_DISPATCHER_CREDIT_TABLE_ADDRESS >> 3 | IMAGE_1_IMAGE_1_CPU_RX_COPY_THREAD_NUMBER << 12),
		dsptchr_viq_dest_disp, dsptchr_viq_delayed, DISP_REOR_VIQ_WAN_LOOPBACK, DSPTCHR_GUARANTEED_MAX_LIMIT_PER_NORMAL_VIQ,
		DSPTCHR_COMMON_MAX_LIMIT_PER_VIQ, 0);

#ifdef CONFIG_DHD_RUNNER
    /* VIQ for DHD_RX_COMPLETE_0 - 15 */
    dispatcher_reorder_viq_init(&cfg, &ingress_congs_init, &egress_congs_init, get_runner_idx(dhd_complete_runner_image),
    	(IMAGE_4_DHD_RX_COMPLETE_0_DISPATCHER_CREDIT_TABLE_ADDRESS >> 3 | IMAGE_4_IMAGE_4_DHD_RX_COMPLETE_0_THREAD_NUMBER << 12),
		dsptchr_viq_dest_disp, dsptchr_viq_delayed, DISP_REOR_VIQ_DHD_RX_COMPLETE_0, DSPTCHR_GUARANTEED_MAX_LIMIT_PER_NORMAL_VIQ,
		DSPTCHR_COMMON_MAX_LIMIT_PER_VIQ, 0);

    /* VIQ for DHD_RX_COMPLETE_1 - 16 */
    dispatcher_reorder_viq_init(&cfg, &ingress_congs_init, &egress_congs_init, get_runner_idx(dhd_complete_runner_image),
    	(IMAGE_4_DHD_RX_COMPLETE_1_DISPATCHER_CREDIT_TABLE_ADDRESS >> 3 | IMAGE_4_IMAGE_4_DHD_RX_COMPLETE_1_THREAD_NUMBER << 12),
		dsptchr_viq_dest_disp, dsptchr_viq_delayed, DISP_REOR_VIQ_DHD_RX_COMPLETE_1, DSPTCHR_GUARANTEED_MAX_LIMIT_PER_NORMAL_VIQ,
		DSPTCHR_COMMON_MAX_LIMIT_PER_VIQ, 0);

    /* VIQ for DHD_RX_COMPLETE_2 - 17 */
    dispatcher_reorder_viq_init(&cfg, &ingress_congs_init, &egress_congs_init, get_runner_idx(dhd_complete_runner_image),
    	(IMAGE_4_DHD_RX_COMPLETE_2_DISPATCHER_CREDIT_TABLE_ADDRESS >> 3 | IMAGE_4_IMAGE_4_DHD_RX_COMPLETE_2_THREAD_NUMBER << 12),
		dsptchr_viq_dest_disp, dsptchr_viq_delayed, DISP_REOR_VIQ_DHD_RX_COMPLETE_2, DSPTCHR_GUARANTEED_MAX_LIMIT_PER_NORMAL_VIQ,
		DSPTCHR_COMMON_MAX_LIMIT_PER_VIQ, 0);

    /* VIQ for DHD Mcast - 18 */
    dispatcher_reorder_viq_init(&cfg, &ingress_congs_init, &egress_congs_init, get_runner_idx(dhd_tx_post_runner_image),
    	(IMAGE_6_DHD_MCAST_DISPATCHER_CREDIT_TABLE_ADDRESS >> 3 | IMAGE_6_IMAGE_6_DHD_MCAST_THREAD_NUMBER << 12), dsptchr_viq_dest_reor,
		...
		/* Common VIQ for DHD Mirroring - 19 */
    dispatcher_reorder_viq_init(&cfg, &ingress_congs_init, &egress_congs_init, get_runner_idx(dhd_tx_post_runner_image),
	    (IMAGE_6_DHD_MIRRORING_DISPATCHER_CREDIT_TABLE_ADDRESS >> 3 | IMAGE_6_IMAGE_6_DHD_TX_POST_0_THREAD_NUMBER << 12),
		dsptchr_viq_dest_reor, dsptchr_viq_delayed, DISP_REOR_VIQ_DHD_MIRRORING, DSPTCHR_GUARANTEED_MAX_LIMIT_PER_NORMAL_VIQ,
		DSPTCHR_COMMON_MAX_LIMIT_PER_VIQ, 0);
		
    /* VIQ for DHD_TX_COMPLETE_0 - 20 */
    dispatcher_reorder_viq_init(&cfg, &ingress_congs_init, &egress_congs_init, get_runner_idx(dhd_complete_runner_image),
    	(IMAGE_4_DHD_TX_COMPLETE_0_DISPATCHER_CREDIT_TABLE_ADDRESS >> 3 | IMAGE_4_IMAGE_4_DHD_TX_COMPLETE_0_THREAD_NUMBER << 12),
		dsptchr_viq_dest_reor, dsptchr_viq_delayed, DISP_REOR_VIQ_DHD_TX_COMPLETE_0, DSPTCHR_GUARANTEED_MAX_LIMIT_PER_NORMAL_VIQ,
		DSPTCHR_COMMON_MAX_LIMIT_PER_VIQ, 0);

    /* VIQ for DHD_TX_COMPLETE_1 - 21 */
    dispatcher_reorder_viq_init(&cfg, &ingress_congs_init, &egress_congs_init, get_runner_idx(dhd_complete_runner_image),
    	(IMAGE_4_DHD_TX_COMPLETE_1_DISPATCHER_CREDIT_TABLE_ADDRESS >> 3 | IMAGE_4_IMAGE_4_DHD_TX_COMPLETE_1_THREAD_NUMBER << 12),
		dsptchr_viq_dest_reor, dsptchr_viq_delayed, DISP_REOR_VIQ_DHD_TX_COMPLETE_1, DSPTCHR_GUARANTEED_MAX_LIMIT_PER_NORMAL_VIQ,
		DSPTCHR_COMMON_MAX_LIMIT_PER_VIQ, 0);

    /* VIQ for DHD_TX_COMPLETE_2 - 22 */
    dispatcher_reorder_viq_init(&cfg, &ingress_congs_init, &egress_congs_init, get_runner_idx(dhd_complete_runner_image),
    	(IMAGE_4_DHD_TX_COMPLETE_2_DISPATCHER_CREDIT_TABLE_ADDRESS >> 3 | IMAGE_4_IMAGE_4_DHD_TX_COMPLETE_2_THREAD_NUMBER << 12),
    ...
    dispatcher_reorder_viq_init(&cfg, &ingress_congs_init, &egress_congs_init, get_runner_idx(spdsvc_gen_runner_image),
	    (IMAGE_1_SPDSVC_GEN_DISPATCHER_CREDIT_TABLE_ADDRESS >> 3 | IMAGE_1_IMAGE_1_SPDSVC_GEN_THREAD_NUMBER << 12),
		dsptchr_viq_dest_reor, dsptchr_viq_delayed, DISP_REOR_VIQ_SPDSVC, DSPTCHR_GUARANTEED_MAX_LIMIT_PER_NORMAL_VIQ,
		DSPTCHR_COMMON_MAX_LIMIT_PER_VIQ, 0);
#endif

    /* VIQ for SERVICE QUEUES - 24 */
    if  (p_dpi_cfg->number_of_service_queues)
    {
        dispatcher_reorder_viq_init(&cfg, &ingress_congs_init, &egress_congs_init, get_runner_idx(service_queues_runner_image), 
           (IMAGE_5_SERVICE_QUEUES_DISPATCHER_CREDIT_TABLE_ADDRESS >> 3 | IMAGE_5_PROCESSING_SERVICE_QUEUES_TX_THREAD_NUMBER << 12), 
           dsptchr_viq_dest_reor, dsptchr_viq_delayed, DISP_REOR_VIQ_SERVICE_QUEUES, DSPTCHR_GUARANTEED_MAX_LIMIT_PER_NORMAL_VIQ,
           DSPTCHR_COMMON_MAX_LIMIT_PER_VIQ, 0);
    }

    /* VIQ for mirroring TX - 25 */
    dispatcher_reorder_viq_init(&cfg, &ingress_congs_init, &egress_congs_init, get_runner_idx(ds_tm_runner_image),
    	(IMAGE_0_TX_MIRRORING_DISPATCHER_CREDIT_TABLE_ADDRESS >> 3) | (IMAGE_0_DS_TM_TX_TASK_THREAD_NUMBER << 12), dsptchr_viq_dest_reor,
        dsptchr_viq_delayed, DISP_REOR_VIQ_TX_MIRRORING_DS, DSPTCHR_GUARANTEED_MAX_LIMIT_PER_NORMAL_VIQ,
        DSPTCHR_COMMON_MAX_LIMIT_PER_VIQ, 0);

    /* VIQ for COMMON_REPROCESSING - 26, currently in use for TCP/UDP Speed test only */
    ...
    if (rdpa_is_bcm6836_chip())
    	number_of_cores = GROUPED_EN_SEGMENTS_NUM - 2;
    else
    	number_of_cores = GROUPED_EN_SEGMENTS_NUM;
    ...
    for (core_index = 0; core_index < number_of_cores; core_index++)
    {
        ...
        if (IS_PROCESSING_RUNNER_IMAGE(core_index))
        {
            ...
            dispatcher_reorder_rnr_group_init(&cfg, 0, core_index, 0xFF00, offsetof(RDD_PACKET_BUFFER_DTS, pd) >> 3, sizeof(RDD_PACKET_BUFFER_DTS) >> 3);
        }

        /* setting group 1 - wan direct, no processing in same core -> offset=0 */
        if (rdp_core_to_image_map[core_index] == wan_direct_runner_image)
        {
            dispatcher_reorder_rnr_group_init(&cfg, 1, core_index, (1<< IMAGE_3_US_TM_WAN_DIRECT_THREAD_NUMBER), (IMAGE_3_DIRECT_PROCESSING_PD_TABLE_ADDRESS >> 3), 0);
        }
    }
...
#endif
static int natc_tbl_database_get_tbl_size(natc_ddr_cfg_natc_ddr_size *ddr_size, natc_ddr_cfg_total_len *entry_len_array, 
                                          uint32_t bins, int num_tbls, uint32_t mem_avail)
{
    /* natc_ddr_cfg_natc_ddr_size struct = tbl0 ... tbl7 */
    natc_ddr_cfg_natc_ddr_size ddr_size_db[] = { 
                                      { .ddr_size_tbl0 = ddr_size_256k,.ddr_size_tbl1 = ddr_size_256k, .ddr_size_tbl2 = ddr_size_8k, .ddr_size_tbl3 = ddr_size_64k, .ddr_size_tbl4 = ddr_size_def, .ddr_size_tbl5 = ddr_size_def, .ddr_size_tbl6 = ddr_size_def, .ddr_size_tbl7 = ddr_size_def },
                                      { .ddr_size_tbl0 = ddr_size_128k,.ddr_size_tbl1 = ddr_size_128k, .ddr_size_tbl2 = ddr_size_8k, .ddr_size_tbl3 = ddr_size_32k, .ddr_size_tbl4 = ddr_size_def, .ddr_size_tbl5 = ddr_size_def, .ddr_size_tbl6 = ddr_size_def, .ddr_size_tbl7 = ddr_size_def },
                                      { .ddr_size_tbl0 = ddr_size_64k,. ddr_size_tbl1 = ddr_size_64k,  .ddr_size_tbl2 = ddr_size_8k, .ddr_size_tbl3 = ddr_size_16k, .ddr_size_tbl4 = ddr_size_def, .ddr_size_tbl5 = ddr_size_def, .ddr_size_tbl6 = ddr_size_def, .ddr_size_tbl7 = ddr_size_def },
                                      /* DEFAULT configuration */
                                      { .ddr_size_tbl0 = ddr_size_32k,. ddr_size_tbl1 = ddr_size_32k,  .ddr_size_tbl2 = ddr_size_8k, .ddr_size_tbl3 = ddr_size_8k, .ddr_size_tbl4 = ddr_size_def, .ddr_size_tbl5 = ddr_size_def, .ddr_size_tbl6 = ddr_size_def, .ddr_size_tbl7 = ddr_size_def },
                                      { .ddr_size_tbl0 = ddr_size_16k,. ddr_size_tbl1 = ddr_size_16k,  .ddr_size_tbl2 = ddr_size_8k, .ddr_size_tbl3 = ddr_size_8k, .ddr_size_tbl4 = ddr_size_def, .ddr_size_tbl5 = ddr_size_def, .ddr_size_tbl6 = ddr_size_def, .ddr_size_tbl7 = ddr_size_def },
                                      { .ddr_size_tbl0 = ddr_size_8k,.  ddr_size_tbl1 = ddr_size_8k,   .ddr_size_tbl2 = ddr_size_8k, .ddr_size_tbl3 = ddr_size_8k, .ddr_size_tbl4 = ddr_size_def, .ddr_size_tbl5 = ddr_size_def, .ddr_size_tbl6 = ddr_size_def, .ddr_size_tbl7 = ddr_size_def },
                                    };
    ...
    for (idx=0; idx < ARRAY_SIZE(ddr_size_db); idx++)
    {
        ...
        for (tbl_idx = 0; tbl_idx < num_tbls; tbl_idx++)
        {
            ...
            total_mem_reqd += _calc_natc_tbl_size(total_key_ctxt_len, bins, tbl_ddr_size_enum);
        }
...
static int natc_init(void)
{
    int rc = BDMF_ERR_OK;
    int i;
    natc_ddr_cfg_natc_ddr_size ddr_size; /* Filled and returned from ddr_size database */
    /* natc_ddr_cfg_total_len struct = tbl7 ... tbl0 */
    natc_ddr_cfg_total_len total_len =  {.total_len_tbl0 = total_len_80B, .total_len_tbl1 = total_len_80B, .total_len_tbl2 = total_len_80B, .total_len_tbl3 = total_len_def, .total_len_tbl4 = total_len_def, .total_len_tbl5 = total_len_def, .total_len_tbl6 = total_len_def, .total_len_tbl7 = total_len_def};
    natc_config_t cfg;
    uint32_t natc_tbls_total_avail_mem;
    ...
    /* Memory for DHD-Offload Backup Queues comes from Runner-Tables-Memory */
    natc_tbls_total_avail_mem = p_dpi_cfg->rnr_tables_buf_size - RDD_DHD_BACKUP_QUEUE_MEM_SIZE - NATC_MEMORY_RESERVED;

    if (natc_tbl_database_get_tbl_size(&ddr_size, &total_len, NATC_DEFAULT_BIN_SIZE, natc_tbls_num, natc_tbls_total_avail_mem))
    {
        return BDMF_ERR_NOMEM;
    }

    drv_natc_cfg_build_tables(&cfg, &ddr_size, &total_len, NATC_TABLE_BASE_SIZE_SIZE, p_dpi_cfg->rdp_ddr_rnr_tables_base_virt, p_dpi_cfg->rdp_ddr_rnr_tables_base_phys);
    ...
    rc = rc ? rc : ag_drv_natc_ddr_cfg_ddr_bins_per_bucket_0_set(NATC_DEFAULT_BIN_SIZE, NATC_DEFAULT_BIN_SIZE, NATC_DEFAULT_BIN_SIZE, NATC_DEFAULT_BIN_SIZE);
    rc = rc ? rc : ag_drv_natc_ddr_cfg_ddr_bins_per_bucket_1_set(NATC_DEFAULT_BIN_SIZE, NATC_DEFAULT_BIN_SIZE, NATC_DEFAULT_BIN_SIZE, NATC_DEFAULT_BIN_SIZE);
...
static int dqm_init(void)
{
    ...
    switch (buf_size)
    {
        ...
        case BUF_2K:
        case BUF_4K:
            rc = ag_drv_dqm_fpm_addr_set(xrdp_virt2phys(&RU_BLK(FPM), 0) + RU_REG_OFFSET(FPM, POOL3_ALLOC_DEALLOC));
           break;
    }
...
static int qm_init(void)
{
    ...
    qm_init_cfg init_cfg =
    {
        ...
        .ddr_sop_offset = {RDD_DDR_SOP_OFFSET0, RDD_DDR_SOP_OFFSET1}     /* DDR SoP offsets profile0 - everything except multicast , profile1 - for multicast, + TCP speed test */
    };
    ...
    ag_drv_qm_wred_profile_cfg_set(0, &wred_profile_cfg);
#endif
    ...
    queue_cfg.aggregation_disable = 1;   /* TRUE=Disable aggregation */
    for (idx = QM_QUEUE_MAX_DYNAMIC_QUANTITY; idx < QM_QUEUE_LAST+1; idx++)
    {
       ...
       if (idx == QM_QUEUE_PKTGEN_REPROCESSING)
	        continue;
#endif
        rc = rc ? rc : drv_qm_queue_config(idx, &queue_cfg);
        rc = rc ? rc : drv_qm_queue_enable(idx);
...
#if defined(CONFIG_BCM_SPDSVC_SUPPORT) || defined(CONFIG_BCM_TCPSPDTEST_SUPPORT) || defined(CONFIG_BCM_UDPSPDTEST_SUPPORT)
    queue_cfg.fpm_ug = FPM_ALL_PASS_UG; /* Unlimited FPM UG, as we assume that pktgen tasks won't work in parallel with
                                           other tasks */
    queue_cfg.copy_to_ddr = 1;
    queue_cfg.aggregation_disable = 1;
    queue_cfg.wred_profile = QM_WRED_PROFILE_CPU_RX; /* Re-use CPU RX profile's thesholds in order not to occupy
                                                        separate profile */
    rc = rc ? rc : drv_qm_queue_config(QM_QUEUE_PKTGEN_REPROCESSING, &queue_cfg);
    rc = rc ? rc : drv_qm_queue_enable(QM_QUEUE_PKTGEN_REPROCESSING);
#endif
    ...
    rnr_group_cfg.pd_fifo_base = (IMAGE_6_DHD_TX_POST_PD_FIFO_TABLE_ADDRESS >> 3);
    rnr_group_cfg.rnr_task = IMAGE_6_IMAGE_6_DHD_TX_POST_UPDATE_FIFO_THREAD_NUMBER;
    rnr_group_cfg.upd_fifo_base = (IMAGE_6_DHD_TX_POST_UPDATE_FIFO_TABLE_ADDRESS >> 3);
    rnr_group_cfg.pd_fifo_size = qm_pd_fifo_size_2;
    rnr_group_cfg.upd_fifo_size = qm_update_fifo_size_8;
    ...
    rnr_group_cfg.pd_fifo_base = (IMAGE_6_DHD_MCAST_PD_FIFO_TABLE_ADDRESS >> 3);
    rnr_group_cfg.upd_fifo_base = (IMAGE_6_DHD_MCAST_UPDATE_FIFO_TABLE_ADDRESS >> 3);
    rnr_group_cfg.pd_fifo_size = qm_pd_fifo_size_2;
    rnr_group_cfg.upd_fifo_size = qm_update_fifo_size_8;

    rnr_group_cfg.rnr_task = IMAGE_6_IMAGE_6_DHD_MCAST_THREAD_NUMBER;
    ...
    if (p_dpi_cfg->number_of_service_queues)
    {
        ...
        rnr_group_cfg.pd_fifo_base = (IMAGE_5_SERVICE_QUEUES_PD_FIFO_TABLE_ADDRESS >> 3);
        rnr_group_cfg.pd_fifo_size = qm_pd_fifo_size_2;
        rnr_group_cfg.upd_fifo_base = (IMAGE_5_SERVICE_QUEUES_UPDATE_FIFO_TABLE_ADDRESS >> 3);
        rnr_group_cfg.upd_fifo_size = qm_update_fifo_size_8;
        rnr_group_cfg.rnr_bb_id = get_runner_idx(service_queues_runner_image);
        rnr_group_cfg.rnr_task = IMAGE_5_PROCESSING_SERVICE_QUEUES_UPDATE_FIFO_THREAD_NUMBER;
        rnr_group_cfg.rnr_enable = 1;
    ...
    rnr_group_cfg.end_queue = QM_QUEUE_PKTGEN_REPROCESSING;
    rnr_group_cfg.pd_fifo_base = (IMAGE_1_COMMON_REPROCESSING_PD_FIFO_TABLE_ADDRESS >> 3);
    rnr_group_cfg.rnr_task = IMAGE_1_IMAGE_1_COMMON_REPROCESSING_THREAD_NUMBER;
    rnr_group_cfg.upd_fifo_base = (IMAGE_1_COMMON_REPROCESSING_UPDATE_FIFO_TABLE_ADDRESS >> 3);
    rnr_group_cfg.pd_fifo_size = qm_pd_fifo_size_2;
    rnr_group_cfg.upd_fifo_size = qm_update_fifo_size_8;
    rnr_group_cfg.rnr_bb_id = get_runner_idx(common_reprocessing_runner_image);
    rnr_group_cfg.rnr_enable = 1;
...
uint32_t data_path_init_fiber(int wan_bbh)
{
    ...
    if (wan_bbh == MAC_TYPE_EPON)
    {
        ...
        rc = rc ? rc : rdd_ag_us_tm_wan_0_bb_destination_table_set(BB_ID_TX_PON_ETH_STAT);
    }
    else
        rc = rc ? rc : rdd_ag_us_tm_wan_0_bb_destination_table_set(BB_ID_TX_PON_ETH_PD);

    /* configure bbh-tx fifo size */
    if (wan_bbh == MAC_TYPE_EPON)
        rc = rc ? rc : rdd_ag_us_tm_wan_0_bbh_tx_fifo_size_set(BBH_TX_EPON_PD_FIFO_SIZE_0_7);
    else if (wan_bbh == MAC_TYPE_XEPON)
        rc = rc ? rc : rdd_ag_us_tm_wan_0_bbh_tx_fifo_size_set(BBH_TX_XEPON_PD_FIFO_SIZE_0_7);
    else
    {
        rc = rc ? rc : rdd_ag_us_tm_wan_0_bbh_tx_fifo_size_set(BBH_TX_GPON_PD_FIFO_SIZE_0_7);
    }
    if (wan_bbh == MAC_TYPE_EPON || wan_bbh == MAC_TYPE_GPON)
        rc = rc ? rc : rdd_ag_us_tm_bbh_tx_us_wan_0_fifo_bytes_threshold_set(BBH_TX_FIFO_BYTES_WAN_1G_LIMIT);
    else
        rc = rc ? rc : rdd_ag_us_tm_bbh_tx_us_wan_0_fifo_bytes_threshold_set(BBH_TX_FIFO_BYTES_WAN_10G_LIMIT);
    if (rc)
    {
...
uint32_t data_path_init_gbe(rdpa_emac wan_emac)
{
    ...
    /* RUNNER mactype init */
    rc = rc ? rc : rdd_ag_us_tm_wan_0_bb_destination_table_set(BB_ID_TX_LAN | (wan_emac << 6));

    /* configure bbh-tx fifo size */
    rc = rc ? rc : rdd_ag_us_tm_wan_0_bbh_tx_fifo_size_set(BBH_TX_DS_PD_FIFO_SIZE_0);
    rc = rc ? rc : rdd_ag_us_tm_bbh_tx_us_wan_0_fifo_bytes_threshold_set(BBH_TX_FIFO_BYTES_LAN_LIMIT);
    if (rc)
...
static int _data_path_init(dpi_params_t *dpi_params, int is_basic)
{
    ...
    rc = fpm_init();
    rc = rc ? rc : qm_init();
    /* cnpl_init should be before runner_init as runner_init uses counter dynamics offset */
