...
const ru_field_rec RNR_QUAD_PARSER_CORE_CONFIGURATION_ENG_CFG_FIELD =
{
    ...
    "eng_cnfg[3]  - enables MAC_SPOOF exception bit"
    "eng_cnfg[8]  - Free"
    "eng_cnfg[9]  - Free"
    "eng_cnfg[10] - Free"
    "eng_cnfg[11] - Free"
...
#if RU_INCLUDE_DESC
    "EXCEPTION_EN",
    "bit 0 - enable excpetion for IP header length error"
    "bit 1 - enable excpetion for checksum error"
    "bit 2 - enable exception for ip version error"
    "bit 3 - enable exception for ip_frag"
    "exception_en[4] - disables ip hdr length error check - 0"
    "exception_en[5] - disable ip_ver_err check -> ip versions is always 4  - 0"
    "exception_en[6] - enables checking if ip_version matches the IP version according to L2 - 0"
    "exception_en[7] - defines the protocol of ppp_code_1 (1 - IPv6, 0 - IPv4)"
    "exception_en[8] - ipv6 route with non zero segment orred with ip_hdr_len_err"
    "exception_en[9] - enable detection of ipv6_hop_by_hop  not directly after ipv6 header (ginger identifies also non trivial options)"
    "//                   the exception is orred with cksum"
    "// exception_en[14:10] - enable excepetion for"
    "bit 10 - ip_length_error"
    "bit 11 - l4 & not 5 tupple valid"
    "bit 12 - error"
    "bit 13 - DHCP identified"
    "bit 14 - MAC SPOOF is used from eng_cnfg[3]"
    ""
    ""
    ""
    "",
