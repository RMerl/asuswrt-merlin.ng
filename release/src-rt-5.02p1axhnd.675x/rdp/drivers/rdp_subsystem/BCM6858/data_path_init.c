...
#ifdef G9991
        rc = rc ? rc : ag_drv_bbh_rx_flow_ctrl_timer_set(bbh_id, (uint32_t)((BBH_FREQUENCY/1000000)*20));   /* 20usec */
#endif
        if (rc)
        {
...
static int bbh_tx_pon_init(int wantype)
{
    ...
    if (wantype == MAC_TYPE_EPON || wantype == MAC_TYPE_XEPON)
    {
        ...
        if (wantype == MAC_TYPE_EPON)
        {
            ...
            config.sts_rnr_cfg[0].task_number = IMAGE_3_US_TM_WAN_0_THREAD_NUMBER;
        ...
        else /* wantype == MAC_TYPE_XEPON */
        {
            ...
            config.rnr_cfg[0].skb_addr = 0;
            config.rnr_cfg[0].task_number = IMAGE_3_US_TM_WAN_0_THREAD_NUMBER;
...
{
    if ((p_dpi_cfg->dpu_split_scheduling_mode) && (bbh_id == BBH_ID_2 || bbh_id == BBH_ID_3))
...
static int bbh_tx_init(void)
{
    ...
    for (bbh_id = BBH_ID_FIRST; bbh_id <= BBH_ID_LAST_XLMAC; bbh_id++)
    {
        ...
        if (p_dpi_cfg->bbh_id_gbe_wan == bbh_id)
        {
            ...
            config.rnr_cfg[0].tcont_addr = IMAGE_3_US_TM_WAN_1_BBH_TX_WAKE_UP_DATA_TABLE_ADDRESS >> 3;
            config.rnr_cfg[0].ptr_addr = IMAGE_3_US_TM_WAN_1_BBH_TX_EGRESS_COUNTER_TABLE_ADDRESS >> 3;
            config.rnr_cfg[0].task_number = IMAGE_3_US_TM_WAN_1_THREAD_NUMBER;
#else
            config.rnr_cfg[0].tcont_addr = IMAGE_3_US_TM_WAN_0_BBH_TX_WAKE_UP_DATA_TABLE_ADDRESS >> 3;
            config.rnr_cfg[0].ptr_addr = IMAGE_3_US_TM_WAN_0_BBH_TX_EGRESS_COUNTER_TABLE_ADDRESS >> 3;
            config.rnr_cfg[0].task_number = IMAGE_3_US_TM_WAN_0_THREAD_NUMBER;
...
#else
            config.rnr_cfg[0].tcont_addr = (IMAGE_0_DS_TM_BBH_TX_WAKE_UP_DATA_TABLE_ADDRESS +
                (bbh_id_to_rdpa_emac[bbh_id] * sizeof(RDD_BBH_TX_WAKE_UP_DATA_ENTRY_DTS))) >> 3;
                /* system port */
                if (bbh_id == BBH_ID_5)
        ...
        /* pd_prefetch_byte_threshold feature enabled */
#ifndef G9991
        config.lan_queue_cfg.pd_bytes_threshold_en = 0;
#else
        config.lan_queue_cfg.pd_bytes_threshold_en = 1;
#endif
...
int bbh_tx_reconfig(bbh_id_e bbh_id_gbe_wan, bbh_id_e bbh_id)
{
    ...
    {
        /* update destination for wan */
        wan_emac = bbh_id_to_rdpa_emac[bbh_id_gbe_wan];
        rc = rc ? rc : rdd_ag_us_tm_wan_0_bb_destination_table_set(rdpa_emac_to_bb_id_tx[wan_emac]);
    ...
    /* ---- set new config  */
    rc = rc ? rc : ag_drv_bbh_tx_rnr_src_id_set(bbh_id, config.rnr_cfg[0].rnr_src_id,  
            config.rnr_cfg[1].rnr_src_id);
...
static int runner_init(int is_basic)
{
    ...
    if (!rdd_init_params.is_basic)
    {
        ...
        iptv_hw_config.hn_size0 = g_bbh_tx_ddr_profile->bbh_tx_ddr_cfg[BBH_ID_1].hnsize0;
        iptv_hw_config.hn_size1 = g_bbh_tx_ddr_profile->bbh_tx_ddr_cfg[BBH_ID_1].hnsize1;
    }
...
static int dispatcher_reorder_init(void)
{
    ...
    dispatcher_reorder_viq_init(&cfg, &ingress_congs_init, &egress_congs_init, BB_ID_RX_BBH_6, dsptchr_viq_bbh_target_addr_normal,
    ...
    dispatcher_reorder_viq_init(&cfg, &ingress_congs_init, &egress_congs_init, BB_ID_RX_BBH_7, dsptchr_viq_bbh_target_addr_normal,
...
#ifdef G9991
    dispatcher_reorder_viq_init(&cfg, &ingress_congs_init, &egress_congs_init, BB_ID_RX_BBH_4, dsptchr_viq_bbh_target_addr_excl,
    ...
    dispatcher_reorder_viq_init(&cfg, &ingress_congs_init, &egress_congs_init, BB_ID_RX_BBH_1, dsptchr_viq_bbh_target_addr_excl,
    ...
    dispatcher_reorder_viq_init(&cfg, &ingress_congs_init, &egress_congs_init, BB_ID_RX_BBH_2, dsptchr_viq_bbh_target_addr_excl,
    ...
    dispatcher_reorder_viq_init(&cfg, &ingress_congs_init, &egress_congs_init, BB_ID_RX_BBH_3, dsptchr_viq_bbh_target_addr_excl,
    ...
    dispatcher_reorder_viq_init(&cfg, &ingress_congs_init, &egress_congs_init, BB_ID_RX_BBH_0, dsptchr_viq_bbh_target_addr_excl,
    ...
    dispatcher_reorder_viq_init(&cfg, &ingress_congs_init, &egress_congs_init, BB_ID_RX_BBH_5, dsptchr_viq_bbh_target_addr_excl,
    ...
    dispatcher_reorder_viq_init(&cfg, &ingress_congs_init, &egress_congs_init, BB_ID_RX_BBH_6, dsptchr_viq_bbh_target_addr_excl,
    ...
    dispatcher_reorder_viq_init(&cfg, &ingress_congs_init, &egress_congs_init, BB_ID_RX_BBH_7, dsptchr_viq_bbh_target_addr_excl,
    ...
    dispatcher_reorder_viq_init(&cfg, &ingress_congs_init, &egress_congs_init, get_runner_idx(us_tm_runner_image),
	    ((IMAGE_3_DISPATCHER_CREDIT_TABLE_ADDRESS >> 3) | (IMAGE_3_US_TM_WAN_0_THREAD_NUMBER << 12)), dsptchr_viq_dest_reor,
    ...
    dispatcher_reorder_viq_init(&cfg, &ingress_congs_init, &egress_congs_init, get_runner_idx(cpu_tx_runner_image),
        (IMAGE_2_CPU_TX_EGRESS_DISPATCHER_CREDIT_TABLE_ADDRESS >> 3 | IMAGE_2_CPU_IF_2_CPU_TX_0_THREAD_NUMBER << 12),
    ...
    dispatcher_reorder_viq_init(&cfg, &ingress_congs_init, &egress_congs_init, get_runner_idx(cpu_tx_runner_image),
        (IMAGE_2_CPU_TX_INGRESS_DISPATCHER_CREDIT_TABLE_ADDRESS >> 3 | IMAGE_2_CPU_IF_2_CPU_TX_1_THREAD_NUMBER << 12),
    ...
    /* VIQ for flush DS  */
    dispatcher_reorder_viq_init(&cfg, &ingress_congs_init, &egress_congs_init, get_ds_tm_rnr_id(BBH_ID_0),
...
#ifdef G9991
    /* VIQ for flush DS */
    dispatcher_reorder_viq_init(&cfg, &ingress_congs_init, &egress_congs_init, get_ds_tm_rnr_id(BBH_ID_6),
	    (IMAGE_0_DS_TM_FLUSH_DISPATCHER_CREDIT_TABLE_ADDRESS >> 3) | (IMAGE_0_DS_TM_FLUSH_THREAD_NUMBER << 12),
    ...
    dispatcher_reorder_viq_init(&cfg, &ingress_congs_init, &egress_congs_init, get_runner_idx(us_tm_runner_image),
        ((IMAGE_3_TX_MIRRORING_DISPATCHER_CREDIT_TABLE_ADDRESS >> 3) | (IMAGE_3_US_TM_WAN_0_THREAD_NUMBER << 12)),
    ...
    /* Common VIQ for DHD Mirroring - 20 */
    dispatcher_reorder_viq_init(&cfg, &ingress_congs_init, &egress_congs_init, get_runner_idx(dhd_tx_post_runner_image),
	    (IMAGE_1_DHD_MIRRORING_DISPATCHER_CREDIT_TABLE_ADDRESS >> 3 | IMAGE_1_CPU_IF_1_DHD_TX_POST_0_THREAD_NUMBER << 12),
		dsptchr_viq_dest_reor, dsptchr_viq_delayed, DISP_REOR_VIQ_DHD_MIRRORING, DSPTCHR_GUARANTEED_MAX_LIMIT_PER_NORMAL_VIQ,
		DSPTCHR_COMMON_MAX_LIMIT_PER_VIQ, 0);
		
    /* VIQ for DHD_TX_COMPLETE_0 - 25 */
    dispatcher_reorder_viq_init(&cfg, &ingress_congs_init, &egress_congs_init, get_runner_idx(dhd_complete_runner_image),
    ...
    /* VIQ for TX Mirroring - Initialized for DS - 29 (TX_MIRRORING NOT SUPPORTED in G9991 project)*/
...
#else
        /* setting group 1 - wan direct, no processing or reassembly in same core -> offset=0 */
    ...
    cfg.dsptchr_rnr_group_list[0].queues_mask =
        (1 << DISP_REOR_VIQ_BBH_RX8_NORMAL) |                                        /* PON normal    */
        (1 << DISP_REOR_VIQ_BBH_RX5_NORMAL) |                                        /* System port   */
        (1 << DISP_REOR_VIQ_G9991_REASS0) | (1 << DISP_REOR_VIQ_G9991_REASS1) |      /* Upstream -    */
        (1 << DISP_REOR_VIQ_G9991_REASS2) | (1 << DISP_REOR_VIQ_G9991_REASS3) |      /* reprocessing  */
        (1 << DISP_REOR_VIQ_CPU_TX_FORWARD) | (1 << DISP_REOR_VIQ_WAN_LOOPBACK);

    cfg.dsptchr_rnr_group_list[1].queues_mask = (1 << DISP_REOR_VIQ_BBH_RX8_EXCL);   /* PON exclusive */
    cfg.dsptchr_rnr_group_list[2].queues_mask = (1 << DISP_REOR_VIQ_BBH_RX0_NORMAL); /* EMAC_0 */
    cfg.dsptchr_rnr_group_list[3].queues_mask = (1 << DISP_REOR_VIQ_BBH_RX1_NORMAL); /* EMAC_1 */
    cfg.dsptchr_rnr_group_list[4].queues_mask = (1 << DISP_REOR_VIQ_BBH_RX2_NORMAL); /* EMAC_2 */
    cfg.dsptchr_rnr_group_list[5].queues_mask = (1 << DISP_REOR_VIQ_BBH_RX3_NORMAL); /* EMAC_3 */
                                          ...
                                          uint32_t bins, int num_tbls, uint32_t mem_avail)
{
    ...
    /* natc_ddr_cfg_natc_ddr_size struct = tbl0 ... tbl7 */
    natc_ddr_cfg_natc_ddr_size ddr_size_db[] = { 
                                      { .ddr_size_tbl0 = ddr_size_256k,.ddr_size_tbl1 = ddr_size_8k, .ddr_size_tbl2 = ddr_size_def, .ddr_size_tbl3 = ddr_size_def, .ddr_size_tbl4 = ddr_size_def, .ddr_size_tbl5 = ddr_size_def, .ddr_size_tbl6 = ddr_size_def, .ddr_size_tbl7 = ddr_size_def },
                                      /* Below DEFAULT configuration */
                                      { .ddr_size_tbl0 = ddr_size_128k,.ddr_size_tbl1 = ddr_size_8k, .ddr_size_tbl2 = ddr_size_def, .ddr_size_tbl3 = ddr_size_def, .ddr_size_tbl4 = ddr_size_def, .ddr_size_tbl5 = ddr_size_def, .ddr_size_tbl6 = ddr_size_def, .ddr_size_tbl7 = ddr_size_def },
                                      { .ddr_size_tbl0 = ddr_size_64k,. ddr_size_tbl1 = ddr_size_8k, .ddr_size_tbl2 = ddr_size_def, .ddr_size_tbl3 = ddr_size_def, .ddr_size_tbl4 = ddr_size_def, .ddr_size_tbl5 = ddr_size_def, .ddr_size_tbl6 = ddr_size_def, .ddr_size_tbl7 = ddr_size_def },
                                      { .ddr_size_tbl0 = ddr_size_32k,. ddr_size_tbl1 = ddr_size_8k, .ddr_size_tbl2 = ddr_size_def, .ddr_size_tbl3 = ddr_size_def, .ddr_size_tbl4 = ddr_size_def, .ddr_size_tbl5 = ddr_size_def, .ddr_size_tbl6 = ddr_size_def, .ddr_size_tbl7 = ddr_size_def },
                                      { .ddr_size_tbl0 = ddr_size_16k,. ddr_size_tbl1 = ddr_size_8k, .ddr_size_tbl2 = ddr_size_def, .ddr_size_tbl3 = ddr_size_def, .ddr_size_tbl4 = ddr_size_def, .ddr_size_tbl5 = ddr_size_def, .ddr_size_tbl6 = ddr_size_def, .ddr_size_tbl7 = ddr_size_def },
                                      { .ddr_size_tbl0 = ddr_size_8k,.  ddr_size_tbl1 = ddr_size_8k, .ddr_size_tbl2 = ddr_size_def, .ddr_size_tbl3 = ddr_size_def, .ddr_size_tbl4 = ddr_size_def, .ddr_size_tbl5 = ddr_size_def, .ddr_size_tbl6 = ddr_size_def, .ddr_size_tbl7 = ddr_size_def },
                                    };
#endif
...
static int natc_init(void)
{
    ...
    natc_ddr_cfg_total_len total_len =  {.total_len_tbl0 = total_len_80B, .total_len_tbl1 = total_len_80B, .total_len_tbl2 = total_len_80B, .total_len_tbl3 = total_len_80B, .total_len_tbl4 = total_len_def, .total_len_tbl5 = total_len_def, .total_len_tbl6 = total_len_def, .total_len_tbl7 = total_len_def};
    natc_config_t cfg;
...
static int qm_init(void)
{
    ...
    if (p_dpi_cfg->dpu_split_scheduling_mode)
    {
        ...
        rnr_group_cfg.upd_fifo_size = qm_update_fifo_size_8;
        rnr_group_cfg.rnr_bb_id = get_ds_tm_rnr_id(BBH_ID_0);
        ...
        rnr_group_cfg.upd_fifo_base = (IMAGE_0_UPDATE_FIFO_TABLE_ADDRESS >> 3);
        rnr_group_cfg.upd_fifo_size = qm_update_fifo_size_8;
        rnr_group_cfg.rnr_bb_id = get_ds_tm_rnr_id(BBH_ID_2);
        rnr_group_cfg.rnr_task = IMAGE_0_DS_TM_UPDATE_FIFO_THREAD_NUMBER;
        rnr_group_cfg.rnr_enable = 1;
    ...
    rnr_group_cfg.rnr_task = IMAGE_1_CPU_IF_1_DHD_MCAST_THREAD_NUMBER;
    ...
    rnr_group_cfg.rnr_task = IMAGE_1_CPU_IF_1_COMMON_REPROCESSING_THREAD_NUMBER;
...
uint32_t data_path_init_gbe(rdpa_emac wan_emac)
{
    ...
    /* RUNNER mactype init */
#if defined(CONFIG_MULTI_WAN_SUPPORT)
    rc = rdd_ag_us_tm_wan_1_bb_destination_table_set(rdpa_emac_to_bb_id_tx[wan_emac]);

    /* configure bbh-tx fifo size */
    rc = rc ? rc : rdd_ag_us_tm_wan_1_bbh_tx_fifo_size_set(BBH_TX_DS_PD_FIFO_SIZE_0);
    rc = rc ? rc : rdd_ag_us_tm_bbh_tx_us_wan_1_fifo_bytes_threshold_set(BBH_TX_FIFO_BYTES_LAN_LIMIT);
#else
    /* QM mactype init */
...
int data_path_init(dpi_params_t *dpi_params)
{
	...
	if ((rc = drv_rnr_quad_ubus_decode_wnd_cfg(DECODE_WIN1, (uint32_t)fpm_pool_phys_addr, log2_32(fpm_pool_size), UBUS_PORT_ID_BIU, CACHE_BIT_OFF)))
	{
