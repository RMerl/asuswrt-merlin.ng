...
bdmf_error_t ag_drv_dma_config_target_mem_set(uint8_t dma_id, uint8_t word_index, bdmf_boolean rxtmem, bdmf_boolean txtmem)
{
    ...
    RU_REG_RAM_WRITE(dma_id, word_index, DMA, CONFIG_TARGET_MEM, reg_config_target_mem);
...
bdmf_error_t ag_drv_dma_config_target_mem_get(uint8_t dma_id, uint8_t word_index, bdmf_boolean *rxtmem, bdmf_boolean *txtmem)
{
    ...
    RU_REG_RAM_READ(dma_id, word_index, DMA, CONFIG_TARGET_MEM, reg_config_target_mem);
...
bdmf_error_t ag_drv_dma_config_ubus_credits_set(uint8_t dma_id, uint8_t ddr, uint8_t sram, bdmf_boolean ddr_set, bdmf_boolean sram_set)
{
    ...
    if((dma_id >= BLOCK_ADDR_COUNT) ||
       (ddr >= _5BITS_MAX_VAL_) ||
       (sram >= _5BITS_MAX_VAL_) ||
       (ddr_set >= _1BITS_MAX_VAL_) ||
       (sram_set >= _1BITS_MAX_VAL_))
    {
        bdmf_trace("ERROR driver %s:%u| err=%s (%d)\n", __FILE__, __LINE__, bdmf_strerror(BDMF_ERR_RANGE), BDMF_ERR_RANGE);
        return BDMF_ERR_RANGE;
    }
    ...
    reg_config_ubus_credits = RU_FIELD_SET(dma_id, DMA, CONFIG_UBUS_CREDITS, DDR, reg_config_ubus_credits, ddr);
    reg_config_ubus_credits = RU_FIELD_SET(dma_id, DMA, CONFIG_UBUS_CREDITS, SRAM, reg_config_ubus_credits, sram);
    reg_config_ubus_credits = RU_FIELD_SET(dma_id, DMA, CONFIG_UBUS_CREDITS, DDR_SET, reg_config_ubus_credits, ddr_set);
    reg_config_ubus_credits = RU_FIELD_SET(dma_id, DMA, CONFIG_UBUS_CREDITS, SRAM_SET, reg_config_ubus_credits, sram_set);
...
bdmf_error_t ag_drv_dma_config_ubus_credits_get(uint8_t dma_id, uint8_t *ddr, uint8_t *sram, bdmf_boolean *ddr_set, bdmf_boolean *sram_set)
{
    ...
    *ddr = RU_FIELD_GET(dma_id, DMA, CONFIG_UBUS_CREDITS, DDR, reg_config_ubus_credits);
    *sram = RU_FIELD_GET(dma_id, DMA, CONFIG_UBUS_CREDITS, SRAM, reg_config_ubus_credits);
    *ddr_set = RU_FIELD_GET(dma_id, DMA, CONFIG_UBUS_CREDITS, DDR_SET, reg_config_ubus_credits);
    *sram_set = RU_FIELD_GET(dma_id, DMA, CONFIG_UBUS_CREDITS, SRAM_SET, reg_config_ubus_credits);
...
static int bcm_dma_cli_set(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    ...
    case cli_dma_config_target_mem:
        err = ag_drv_dma_config_target_mem_set(parm[1].value.unumber, parm[2].value.unumber, parm[3].value.unumber, parm[4].value.unumber);
        break;
    ...
    case cli_dma_config_ubus_dpids:
        err = ag_drv_dma_config_ubus_dpids_set(parm[1].value.unumber, parm[2].value.unumber, parm[3].value.unumber);
        break;
    case cli_dma_config_max_otf:
        err = ag_drv_dma_config_max_otf_set(parm[1].value.unumber, parm[2].value.unumber, parm[3].value.unumber);
        break;
    case cli_dma_config_ubus_credits:
        err = ag_drv_dma_config_ubus_credits_set(parm[1].value.unumber, parm[2].value.unumber, parm[3].value.unumber, parm[4].value.unumber, parm[5].value.unumber);
        break;
    case cli_dma_config_psram_base:
        err = ag_drv_dma_config_psram_base_set(parm[1].value.unumber, parm[2].value.unumber);
        break;
    case cli_dma_config_ddr_base:
        err = ag_drv_dma_config_ddr_base_set(parm[1].value.unumber, parm[2].value.unumber);
        break;
    case cli_dma_debug_dbg_sel:
        err = ag_drv_dma_debug_dbg_sel_set(parm[1].value.unumber, parm[2].value.unumber);
        break;
    default:
...
int bcm_dma_cli_get(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    ...
    case cli_dma_config_target_mem:
    {
        bdmf_boolean rxtmem;
        bdmf_boolean txtmem;
        err = ag_drv_dma_config_target_mem_get(parm[1].value.unumber, parm[2].value.unumber, &rxtmem, &txtmem);
    ...
    case cli_dma_config_ubus_dpids:
    {
        uint8_t ddr;
        uint8_t sram;
        err = ag_drv_dma_config_ubus_dpids_get(parm[1].value.unumber, &ddr, &sram);
    ...
    case cli_dma_config_max_otf:
    {
        uint16_t max_ddr;
        uint16_t max_sram;
        err = ag_drv_dma_config_max_otf_get(parm[1].value.unumber, &max_ddr, &max_sram);
    ...
    case cli_dma_config_ubus_credits:
    {
        ...
        err = ag_drv_dma_config_ubus_credits_get(parm[1].value.unumber, &ddr, &sram, &ddr_set, &sram_set);
        bdmf_session_print(session, "ddr = %u (0x%x)\n", ddr, ddr);
        bdmf_session_print(session, "sram = %u (0x%x)\n", sram, sram);
    ...
    {
        uint32_t base;
        err = ag_drv_dma_config_psram_base_get(parm[1].value.unumber, &base);
    ...
    {
        uint32_t base;
        err = ag_drv_dma_config_ddr_base_get(parm[1].value.unumber, &base);
        bdmf_session_print(session, "base = %u (0x%x)\n", base, base);
        break;
    }
    ...
    case cli_dma_debug_ubuscrdt:
    {
        uint8_t ddr;
        uint8_t sram;
        err = ag_drv_dma_debug_ubuscrdt_get(parm[1].value.unumber, &ddr, &sram);
        bdmf_session_print(session, "ddr = %u (0x%x)\n", ddr, ddr);
        bdmf_session_print(session, "sram = %u (0x%x)\n", sram, sram);
        break;
    }
    case cli_dma_debug_ubusbytes:
    {
        uint16_t ddr;
        uint16_t sram;
        err = ag_drv_dma_debug_ubusbytes_get(parm[1].value.unumber, &ddr, &sram);
        bdmf_session_print(session, "ddr = %u (0x%x)\n", ddr, ddr);
        bdmf_session_print(session, "sram = %u (0x%x)\n", sram, sram);
        break;
    }
    ...
    {
        uint8_t otf;
        err = ag_drv_dma_debug_on_the_fly_get(parm[1].value.unumber, &otf);
    ...
    {
        uint8_t dbgsel;
        err = ag_drv_dma_debug_dbg_sel_get(parm[1].value.unumber, &dbgsel);
    ...
    {
        uint32_t dbg;
        err = ag_drv_dma_debug_debugout_get(parm[1].value.unumber, &dbg);
...
static int bcm_dma_cli_test(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    ...
    {
        uint8_t word_index=gtmv(m, 3);
        bdmf_boolean rxtmem=gtmv(m, 1);
        bdmf_boolean txtmem=gtmv(m, 1);
        if(!err) bdmf_session_print(session, "ag_drv_dma_config_target_mem_set(%u %u %u %u)\n", dma_id, word_index, rxtmem, txtmem);
        if(!err) ag_drv_dma_config_target_mem_set(dma_id, word_index, rxtmem, txtmem);
        if(!err) ag_drv_dma_config_target_mem_get( dma_id, word_index, &rxtmem, &txtmem);
        if(!err) bdmf_session_print(session, "ag_drv_dma_config_target_mem_get(%u %u %u %u)\n", dma_id, word_index, rxtmem, txtmem);
        if(err || rxtmem!=gtmv(m, 1) || txtmem!=gtmv(m, 1))
            return err ? err : BDMF_ERR_IO;
    }
    {
    ...
    }
    {
        ...
        if(!err) bdmf_session_print(session, "ag_drv_dma_config_ubus_dpids_set(%u %u %u)\n", dma_id, ddr, sram);
        if(!err) ag_drv_dma_config_ubus_dpids_set(dma_id, ddr, sram);
        if(!err) ag_drv_dma_config_ubus_dpids_get( dma_id, &ddr, &sram);
        if(!err) bdmf_session_print(session, "ag_drv_dma_config_ubus_dpids_get(%u %u %u)\n", dma_id, ddr, sram);
        if(err || ddr!=gtmv(m, 8) || sram!=gtmv(m, 8))
            return err ? err : BDMF_ERR_IO;
    }
    {
        uint16_t max_ddr=gtmv(m, 16);
        uint16_t max_sram=gtmv(m, 16);
        if(!err) bdmf_session_print(session, "ag_drv_dma_config_max_otf_set(%u %u %u)\n", dma_id, max_ddr, max_sram);
        if(!err) ag_drv_dma_config_max_otf_set(dma_id, max_ddr, max_sram);
        if(!err) ag_drv_dma_config_max_otf_get( dma_id, &max_ddr, &max_sram);
        if(!err) bdmf_session_print(session, "ag_drv_dma_config_max_otf_get(%u %u %u)\n", dma_id, max_ddr, max_sram);
        if(err || max_ddr!=gtmv(m, 16) || max_sram!=gtmv(m, 16))
            return err ? err : BDMF_ERR_IO;
    }
    {
        uint8_t ddr=gtmv(m, 5);
        uint8_t sram=gtmv(m, 5);
        bdmf_boolean ddr_set=gtmv(m, 1);
        bdmf_boolean sram_set=gtmv(m, 1);
        if(!err) bdmf_session_print(session, "ag_drv_dma_config_ubus_credits_set(%u %u %u %u %u)\n", dma_id, ddr, sram, ddr_set, sram_set);
        if(!err) ag_drv_dma_config_ubus_credits_set(dma_id, ddr, sram, ddr_set, sram_set);
        if(!err) ag_drv_dma_config_ubus_credits_get( dma_id, &ddr, &sram, &ddr_set, &sram_set);
        if(!err) bdmf_session_print(session, "ag_drv_dma_config_ubus_credits_get(%u %u %u %u %u)\n", dma_id, ddr, sram, ddr_set, sram_set);
        if(err || ddr!=gtmv(m, 5) || sram!=gtmv(m, 5) || ddr_set!=gtmv(m, 1) || sram_set!=gtmv(m, 1))
            return err ? err : BDMF_ERR_IO;
    }
    {
        uint32_t base=gtmv(m, 32);
        if(!err) bdmf_session_print(session, "ag_drv_dma_config_psram_base_set(%u %u)\n", dma_id, base);
        if(!err) ag_drv_dma_config_psram_base_set(dma_id, base);
        if(!err) ag_drv_dma_config_psram_base_get( dma_id, &base);
        if(!err) bdmf_session_print(session, "ag_drv_dma_config_psram_base_get(%u %u)\n", dma_id, base);
        if(err || base!=gtmv(m, 32))
            return err ? err : BDMF_ERR_IO;
    }
    {
        uint32_t base=gtmv(m, 32);
        if(!err) bdmf_session_print(session, "ag_drv_dma_config_ddr_base_set(%u %u)\n", dma_id, base);
        if(!err) ag_drv_dma_config_ddr_base_set(dma_id, base);
        if(!err) ag_drv_dma_config_ddr_base_get( dma_id, &base);
        if(!err) bdmf_session_print(session, "ag_drv_dma_config_ddr_base_get(%u %u)\n", dma_id, base);
        if(err || base!=gtmv(m, 32))
            return err ? err : BDMF_ERR_IO;
    }
    {
    ...
    }
    {
        uint8_t ddr=gtmv(m, 5);
        uint8_t sram=gtmv(m, 5);
        if(!err) ag_drv_dma_debug_ubuscrdt_get( dma_id, &ddr, &sram);
        if(!err) bdmf_session_print(session, "ag_drv_dma_debug_ubuscrdt_get(%u %u %u)\n", dma_id, ddr, sram);
    }
    {
        uint16_t ddr=gtmv(m, 16);
        uint16_t sram=gtmv(m, 16);
        if(!err) ag_drv_dma_debug_ubusbytes_get( dma_id, &ddr, &sram);
        if(!err) bdmf_session_print(session, "ag_drv_dma_debug_ubusbytes_get(%u %u %u)\n", dma_id, ddr, sram);
    }
    {
        uint8_t otf=gtmv(m, 6);
        if(!err) ag_drv_dma_debug_on_the_fly_get( dma_id, &otf);
        if(!err) bdmf_session_print(session, "ag_drv_dma_debug_on_the_fly_get(%u %u)\n", dma_id, otf);
    }
    {
        uint8_t dbgsel=gtmv(m, 3);
        if(!err) bdmf_session_print(session, "ag_drv_dma_debug_dbg_sel_set(%u %u)\n", dma_id, dbgsel);
        if(!err) ag_drv_dma_debug_dbg_sel_set(dma_id, dbgsel);
        if(!err) ag_drv_dma_debug_dbg_sel_get( dma_id, &dbgsel);
        if(!err) bdmf_session_print(session, "ag_drv_dma_debug_dbg_sel_get(%u %u)\n", dma_id, dbgsel);
        if(err || dbgsel!=gtmv(m, 3))
            return err ? err : BDMF_ERR_IO;
    }
    {
        uint32_t dbg=gtmv(m, 32);
        if(!err) ag_drv_dma_debug_debugout_get( dma_id, &dbg);
        if(!err) bdmf_session_print(session, "ag_drv_dma_debug_debugout_get(%u %u)\n", dma_id, dbg);
    }
    {
...
bdmfmon_handle_t ag_drv_dma_cli_init(bdmfmon_handle_t driver_dir)
{
...

    {
        ...
        static bdmfmon_cmd_parm_t set_config_target_mem[]={
            ...
            BDMFMON_MAKE_PARM("rxtmem", "rxtmem", BDMFMON_PARM_NUMBER, 0),
            BDMFMON_MAKE_PARM("txtmem", "txtmem", BDMFMON_PARM_NUMBER, 0),
            BDMFMON_PARM_LIST_TERMINATOR
        };
        ...
        static bdmfmon_cmd_parm_t set_config_ubus_dpids[]={
            ...
            BDMFMON_MAKE_PARM("sram", "sram", BDMFMON_PARM_NUMBER, 0),
            BDMFMON_PARM_LIST_TERMINATOR
        };
        static bdmfmon_cmd_parm_t set_config_max_otf[]={
            BDMFMON_MAKE_PARM_ENUM("dma_id", "dma_id", dma_id_enum_table, 0),
            BDMFMON_MAKE_PARM("max_ddr", "max_ddr", BDMFMON_PARM_NUMBER, 0),
            BDMFMON_MAKE_PARM("max_sram", "max_sram", BDMFMON_PARM_NUMBER, 0),
            BDMFMON_PARM_LIST_TERMINATOR
        };
        static bdmfmon_cmd_parm_t set_config_ubus_credits[]={
            ...
            BDMFMON_MAKE_PARM("ddr", "ddr", BDMFMON_PARM_NUMBER, 0),
            BDMFMON_MAKE_PARM("sram", "sram", BDMFMON_PARM_NUMBER, 0),
            BDMFMON_MAKE_PARM("ddr_set", "ddr_set", BDMFMON_PARM_NUMBER, 0),
            BDMFMON_MAKE_PARM("sram_set", "sram_set", BDMFMON_PARM_NUMBER, 0),
            BDMFMON_PARM_LIST_TERMINATOR
        };
    ...
    }
    {
        ...
        static bdmfmon_enum_val_t selector_table[] = {
            ...
            { .name="config_ubus_dpids", .val=cli_dma_config_ubus_dpids, .parms=set_default },
            { .name="config_max_otf", .val=cli_dma_config_max_otf, .parms=set_default },
            { .name="config_ubus_credits", .val=cli_dma_config_ubus_credits, .parms=set_default },
            { .name="config_psram_base", .val=cli_dma_config_psram_base, .parms=set_default },
            { .name="config_ddr_base", .val=cli_dma_config_ddr_base, .parms=set_default },
            ...
            { .name="debug_ubuscrdt", .val=cli_dma_debug_ubuscrdt, .parms=set_default },
            { .name="debug_ubusbytes", .val=cli_dma_debug_ubusbytes, .parms=set_default },
            { .name="debug_on_the_fly", .val=cli_dma_debug_on_the_fly, .parms=set_default },
            { .name="debug_dbg_sel", .val=cli_dma_debug_dbg_sel, .parms=set_default },
            { .name="debug_debugout", .val=cli_dma_debug_debugout, .parms=set_default },
