...
static int bcm_natc_ctrs_cli_set(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    ...
    case cli_natc_ctrs_natc_ctrs:
    {
        ...
        err = ag_drv_natc_ctrs_natc_ctrs_set(&natc_ctrs);
        break;
    }
    default:
...
int bcm_natc_ctrs_cli_get(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    ...
    case cli_natc_ctrs_natc_ctrs:
    {
        ...
        err = ag_drv_natc_ctrs_natc_ctrs_get(&natc_ctrs);
...
static int bcm_natc_ctrs_cli_test(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
...

    {
        ...
        if(!err) bdmf_session_print(session, "ag_drv_natc_ctrs_natc_ctrs_set( %u %u %u %u %u)\n", natc_ctrs.cache_hit_count, natc_ctrs.cache_miss_count, natc_ctrs.ddr_request_count, natc_ctrs.ddr_evict_count, natc_ctrs.ddr_block_count);
        if(!err) ag_drv_natc_ctrs_natc_ctrs_set(&natc_ctrs);
        if(!err) ag_drv_natc_ctrs_natc_ctrs_get( &natc_ctrs);
        if(!err) bdmf_session_print(session, "ag_drv_natc_ctrs_natc_ctrs_get( %u %u %u %u %u)\n", natc_ctrs.cache_hit_count, natc_ctrs.cache_miss_count, natc_ctrs.ddr_request_count, natc_ctrs.ddr_evict_count, natc_ctrs.ddr_block_count);
