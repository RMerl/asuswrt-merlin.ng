...
static int bcm_rnr_cntxt_cli_set(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    ...
    case cli_rnr_cntxt_mem_entry:
        err = ag_drv_rnr_cntxt_mem_entry_set(parm[1].value.unumber, parm[2].value.unumber, parm[3].value.unumber);
        break;
    default:
...
int bcm_rnr_cntxt_cli_get(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    ...
    case cli_rnr_cntxt_mem_entry:
    {
        ...
        err = ag_drv_rnr_cntxt_mem_entry_get(parm[1].value.unumber, parm[2].value.unumber, &context_entry);
...
static int bcm_rnr_cntxt_cli_test(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
...

    {
        ...
        if(!err) bdmf_session_print(session, "ag_drv_rnr_cntxt_mem_entry_set(%u %u %u)\n", rnr_id, word_index, context_entry);
        if(!err) ag_drv_rnr_cntxt_mem_entry_set(rnr_id, word_index, context_entry);
        if(!err) ag_drv_rnr_cntxt_mem_entry_get( rnr_id, word_index, &context_entry);
        if(!err) bdmf_session_print(session, "ag_drv_rnr_cntxt_mem_entry_get(%u %u %u)\n", rnr_id, word_index, context_entry);
        if(err || context_entry!=gtmv(m, 32))
            return err ? err : BDMF_ERR_IO;
    }
    return err;
}

static int bcm_rnr_cntxt_cli_address(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    uint32_t i;
    uint32_t j;
...
bdmfmon_handle_t ag_drv_rnr_cntxt_cli_init(bdmfmon_handle_t driver_dir)
{
    bdmfmon_handle_t dir;

    if ((dir = bdmfmon_dir_find(driver_dir, "rnr_cntxt"))!=NULL)
        return dir;
    dir = bdmfmon_dir_add(driver_dir, "rnr_cntxt", "rnr_cntxt", BDMF_ACCESS_ADMIN, NULL);

    {
        static bdmfmon_cmd_parm_t set_mem_entry[]={
            ...
            BDMFMON_MAKE_PARM("context_entry", "context_entry", BDMFMON_PARM_NUMBER, 0),
        ...
        BDMFMON_MAKE_CMD(dir, "set", "set", bcm_rnr_cntxt_cli_set,
    ...
    }
    {
        ...
        BDMFMON_MAKE_CMD(dir, "test", "test", bcm_rnr_cntxt_cli_test,
    ...
    }
    {
        ...
        BDMFMON_MAKE_CMD(dir, "address", "address", bcm_rnr_cntxt_cli_address,
