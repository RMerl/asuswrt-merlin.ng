...
{
#ifndef RDP_SIM
    volatile uint32_t* p = ((volatile uint32_t*) UBUS_SYS_MODULE_BASE);
#endif
    ...
    p[DECODE_CFG_REG_MST_PORT_NODE_B53_Window0_31_0] = (0x0082A000);
    p[DECODE_CFG_REG_MST_PORT_NODE_B53_Window0_63_32] = (0x0082A000);
    p[DECODE_CFG_REG_MST_PORT_NODE_B53_Window0_95_64] = (0x00031509);
    p[DECODE_CFG_REG_MST_PORT_NODE_B53_Window1_31_0] = (0x00821000);
    p[DECODE_CFG_REG_MST_PORT_NODE_B53_Window1_63_32] = (0x00821000);
    p[DECODE_CFG_REG_MST_PORT_NODE_B53_Window1_95_64] = (0x00031409);  // 1M instead of 246K
#endif
...
static int bbh_tx_pon_init(int wantype)
{
    ...
    else
    {
        ...
        config.rnr_cfg[0].tcont_addr = IMAGE_0_US_TM_WAN_0_BBH_TX_WAKE_UP_DATA_TABLE_ADDRESS >> 3;
        config.rnr_cfg[0].task_number = IMAGE_0_TM_WAN_THREAD_NUMBER;
        config.rnr_cfg[0].ptr_addr = IMAGE_0_US_TM_WAN_0_BBH_TX_EGRESS_COUNTER_TABLE_ADDRESS >> 3;
    ...
    config.sts_rnr_cfg[0].tcont_addr = IMAGE_0_US_TM_WAN_0_BBH_TX_WAKE_UP_DATA_TABLE_ADDRESS >> 3;
    config.sts_rnr_cfg[0].task_number = IMAGE_0_TM_WAN_THREAD_NUMBER;
    config.sts_rnr_cfg[0].ptr_addr = IMAGE_0_US_TM_WAN_0_BBH_TX_EGRESS_COUNTER_TABLE_ADDRESS >> 3;
    ...
    config.wan_queue_cfg.queue_profile = g_wan_queue_profile;
    config.wan_queue_cfg.qm_q = g_wan_qm_q;
    ...
    config.rnr_cfg[0].skb_addr = IMAGE_0_US_TM_CPU_TX_ABS_COUNTERS_ADDRESS >> 3;
    config.rnr_cfg[1].skb_addr = IMAGE_0_US_TM_CPU_TX_ABS_COUNTERS_ADDRESS >> 3;
...
static int bbh_tx_init(void)
{
    ...
    config.lan_queue_cfg.qm_q                   = g_lan_qm_q;
        ...
        config.rnr_cfg[1].tcont_addr = IMAGE_0_US_TM_WAN_0_BBH_TX_WAKE_UP_DATA_TABLE_ADDRESS >> 3;
        config.rnr_cfg[1].ptr_addr = IMAGE_0_US_TM_WAN_0_BBH_TX_EGRESS_COUNTER_TABLE_ADDRESS >> 3;
...
int bbh_tx_reconfig(bbh_id_e bbh_id_gbe_wan, bbh_id_e bbh_id)
{
    ...
    if (bbh_id_gbe_wan == bbh_id)
    {
        ...
        *(((uint8_t *)&config.per_q_task) + bbh_id) = IMAGE_0_TM_WAN_THREAD_NUMBER;
    }
    else
    {
        if (bbh_id & 0x1)
        ...
        *(((uint8_t *)&config.per_q_task) + bbh_id) = IMAGE_0_TM_TX_TASK_DS_THREAD_NUMBER;
    }
...
static int runner_init(int is_basic)
{
    ...
    /* in bring up disable C code hard codded to avoid mistakes */
    rdd_init_params.fw_clang_dis = /*p_dpi_cfg->fw_clang_dis;*/ 1;
...
static int dispatcher_reorder_init(void)
{
    ...
    /* Only one cpu_tx task is required for 6878 chip, VIQ - 9 can be freed */
    dispatcher_reorder_viq_init(&cfg, &ingress_congs_init, &egress_congs_init, get_runner_idx(cpu_tx_runner_image),
        (IMAGE_2_CPU_TX_EGRESS_DISPATCHER_CREDIT_TABLE_ADDRESS >> 3 | IMAGE_2_IMAGE_2_CPU_TX_0_THREAD_NUMBER << 12), dsptchr_viq_dest_reor, 
    ...
    dispatcher_reorder_viq_init(&cfg, &ingress_congs_init, &egress_congs_init, get_runner_idx(cpu_tx_runner_image), 
        (IMAGE_2_CPU_TX_INGRESS_DISPATCHER_CREDIT_TABLE_ADDRESS >> 3 | IMAGE_2_IMAGE_2_CPU_TX_0_THREAD_NUMBER << 12), dsptchr_viq_dest_disp,
    ...
    /* Common VIQ for DHD Mirroring - 16 */
    dispatcher_reorder_viq_init(&cfg, &ingress_congs_init, &egress_congs_init, get_runner_idx(dhd_tx_post_runner_image),
	    (IMAGE_2_DHD_MIRRORING_DISPATCHER_CREDIT_TABLE_ADDRESS >> 3 | IMAGE_2_IMAGE_2_DHD_TX_POST_0_THREAD_NUMBER << 12),
    ...
    for (core_index = 0; core_index < GROUPED_EN_SEGMENTS_NUM; core_index++)
    {
        ...
        {
            dispatcher_reorder_rnr_group_init(&cfg, 0, core_index, 0x3F00, offsetof(RDD_PACKET_BUFFER_DTS, pd) >> 3, sizeof(RDD_PACKET_BUFFER_DTS) >> 3);
        }
        ...
        {
            dispatcher_reorder_rnr_group_init(&cfg, 1, core_index, (1<< IMAGE_0_TM_WAN_DIRECT_THREAD_NUMBER), (IMAGE_0_DIRECT_PROCESSING_PD_TABLE_ADDRESS >> 3), 0);
        }
    }
...
static int dqm_init(void)
{
    ...
    switch (buf_size)
    {
        ...
        case BUF_512:
            rc = ag_drv_dqm_fpm_addr_set(xrdp_virt2phys(&RU_BLK(FPM), 0) + RU_REG_OFFSET(FPM, POOL2_ALLOC_DEALLOC));
            break;
        ...
        case BUF_2K:
        case BUF_4K:
            rc = ag_drv_dqm_fpm_addr_set(xrdp_virt2phys(&RU_BLK(FPM), 0) + RU_REG_OFFSET(FPM, POOL4_ALLOC_DEALLOC));
           break;
    }
...
#if defined(RDP_SIM)
    rc = rc ? rc : ag_drv_qm_qm_pd_cong_ctrl_set(DEF_QM_PD_CONGESTION_CONTROL_THRESHOLD_NORMAL);
#else
    chip_rev = UtilGetChipRev();
    if (chip_rev == 0xA0)
    {
    	rc = rc ? rc : ag_drv_qm_qm_pd_cong_ctrl_set(DEF_QM_PD_CONGESTION_CONTROL_THRESHOLD_DQM_SW_WORKAROUND_A0);
        bdmf_trace("chip revision is = 0x%x, pd_cong_ctrl value = %d\n", chip_rev, DEF_QM_PD_CONGESTION_CONTROL_THRESHOLD_DQM_SW_WORKAROUND_A0);
    }
    else
    {
    	rc = rc ? rc : ag_drv_qm_qm_pd_cong_ctrl_set(DEF_QM_PD_CONGESTION_CONTROL_THRESHOLD_NORMAL);
    	bdmf_trace("chip revision is = 0x%x, pd_cong_ctrl value =  %d\n", chip_rev, DEF_QM_PD_CONGESTION_CONTROL_THRESHOLD_NORMAL);
    }
#endif
...
/* -------------------------------- */
    ...
    rnr_group_cfg.rnr_task = IMAGE_1_PROCESSING_COMMON_REPROCESSING_THREAD_NUMBER;
    ...
    rc = rc ? rc :ag_drv_qm_global_cfg2_bbhtx_fifo_addr_set(QM_BBHTX_REQ_ADDR, QM_BBHTX_REQ_OTF);
