...
static void f_configure_runner(S_DPI_CFG *pCfg)
{
    ...
    ddr_runner_base_ptr = (uint8_t *)DEVICE_ADDRESS((uint32_t)pCfg->runner_tm_base_addr);
#ifndef LEGACY_RDP
...
#endif
    rdd_init_params.extra_ddr_pool_ptr = (uint8_t*)DEVICE_ADDRESS((uint32_t)pCfg->runner_mc_base_addr);
...
uint32_t data_path_init(S_DPI_CFG *pCfg)
{
    ...
    pCfg->runner_tm_base_addr_phys = K1_TO_PHYS(pCfg->runner_tm_base_addr);
    pCfg->runner_mc_base_addr_phys = K1_TO_PHYS(pCfg->runner_mc_base_addr);
#endif
    ...
    ddr_tm_base_address_phys = (void*)pCfg->runner_tm_base_addr_phys;
    ddr_multicast_base_address_phys = (void*)pCfg->runner_mc_base_addr_phys;

    /*init dma arrays*/
    f_initialize_bbh_dma_sdma_related_arrays(0);

    /*init runner,load microcode and structures*/
    f_configure_runner(pCfg);
