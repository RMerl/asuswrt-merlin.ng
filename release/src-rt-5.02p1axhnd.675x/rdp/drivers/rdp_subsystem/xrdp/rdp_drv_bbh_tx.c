...
#if defined(DUAL_ISSUE)
    queue_to_rnr_t lan_queue_profile[TX_LAN_QUEUE_PAIRS] = {};
    queue_to_rnr_t wan_queue_profile[TX_QEUEU_PAIRS] = {};
    queue_to_rnr_t lan_qm_q[TX_LAN_QUEUE_PAIRS] = {};
    queue_to_rnr_t wan_qm_q[TX_QEUEU_PAIRS] = {};
#endif
...
#if defined(DUAL_ISSUE)
    config.wan_queue_cfg.queue_profile = wan_queue_profile;
    config.lan_queue_cfg.queue_profile = lan_queue_profile;
    config.wan_queue_cfg.qm_q = wan_qm_q;
    config.lan_queue_cfg.qm_q = lan_qm_q;
#endif
...
static void __drv_bbh_tx_cli_occupancy_dump(bdmf_session_handle session, int q_id, bbh_tx_occupancy_info_t *o, int is_lan)
{
...
/* There is HW bug in BCM68460_A0->BBH_TX.BBH_TX.bbhtx_debug_counters.SWRDEN where only first bit is available */
#ifndef BCM6846
    ...
    if (q_id > 0 && is_lan)
        bdmf_session_print(session, "BBH_TX queue(%03d): ingress=%03d, egress=%03d, "
                "occupancy=%03d, fifo size=%03d, almost_full=N/A, full=N/A, empty=N/A\n",
                q_id, o->ingress_cnt[q_id], o->egress_cnt[q_id],
                o->ingress_cnt[q_id] - o->egress_cnt[q_id], o->fifo_size[q_id]);
    else
#endif
        bdmf_session_print(session, "BBH_TX queue(%03d): ingress=%03d, egress=%03d, "
...
static int drv_bbh_tx_cli_occupancy_get(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    ...
    if(IS_WAN_TX_PORT(bbh_id))
    {
        ...
        for(i = 0 ;i < (TX_QEUEU_PAIRS * 2); i++)
        {
            __drv_bbh_tx_cli_occupancy_dump(session, i, &o, 0);
        }
        /* now calculate ghost */
    ...
    else
    {
       ...
       for (i = 0; i < (number_of_lans); i++)
       {
           __drv_bbh_tx_cli_occupancy_dump(session, i, &o, 1);
       }
    }
    return rc;
}
