...
bdmf_error_t drv_qm_init(const qm_init_cfg *init_cfg)
{
...
#if defined(BCM6858) || defined(BCM6856) || defined(BCM6878)
        drop_ctrl.qm_residue_per_queue = 1;
#endif        
#if !defined(BCM6846) && !defined(BCM6856) && !defined(BCM6878) && !defined(BCM63158_B0)
    ...
    /* Initialize all minimum buffer reservation profiles */
#if defined(BCM63158_B0) || defined(BCM6856) || defined(DUAL_ISSUE)
    for (i = 0; i < QM_MBR_PROFILE__NUM_OF; i++)
    {
        ag_drv_qm_fpm_buffer_reservation_data_set(i, 0);
    }
#endif
...
/* Configure queue */
bdmf_error_t drv_qm_queue_config(rdp_qm_queue_idx_t q_idx, qm_q_context *cfg)
{
...
bdmf_error_t set_fpm_budget(int resource_num, int add, uint32_t reserved_packet_buffer)
{
    ...
    if (resource_num == FPM_RES_MBR)
    {
        return 0; /* No use for MBR besides 6856 and after */
    }
#endif
...
bdmf_error_t drv_qm_merge_us_epon_queue_reservation(void)
{
    /* merge of US and EPON queue, since EPON is not used */
    if ((g_queue_locations.queue_us_end + 1) == g_queue_locations.queue_us_epon_start)
    {
        g_queue_locations.queue_us_end = g_queue_locations.queue_us_epon_end;
        g_queue_locations.queue_us_epon_start = QM_ILLEGAL_QUEUE;
        g_queue_locations.queue_us_epon_end = QM_ILLEGAL_QUEUE;
        drv_qm_update_queue_tables();

        bdmf_trace("%s:%d:changed US QUEUE reservation: us_start = %d, us_end = %d, "
                   "epon_start = %d, epon_end = %d\n", __func__, __LINE__,
                   g_queue_locations.queue_us_start,
                   g_queue_locations.queue_us_end,
                   g_queue_locations.queue_us_epon_start,
                   g_queue_locations.queue_us_epon_end);
    }
    return BDMF_ERR_OK;
...
static int _drv_qm_cli_print_not_empty_queues(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    ...
    for (idx = 0; idx < QM_NUM_QUEUES; idx++)
    {
        ...
        rc = rc ? rc : ag_drv_qm_total_valid_cnt_get((idx * (sizeof(RDD_QM_QUEUE_COUNTER_DATA_DTS) / 4)), &(packets));
        rc = rc ? rc : ag_drv_qm_total_valid_cnt_get((idx * (sizeof(RDD_QM_QUEUE_COUNTER_DATA_DTS) / 4)) + 1, &(bytes));
        packets += open_aggr;
        if (packets || bytes)
        {
...
static int _drv_qm_read_print_pd(bdmf_session_handle session, bdmf_error_t(*get_captured_pd)(uint32_t, uint32_t *))
{
    uint8_t idx;
    uint32_t pd[4];
    bdmf_error_t rc = BDMF_ERR_OK;
    ...
    for (idx = 0; idx < 4 ; idx++)
    {
        rc = rc ? rc : get_captured_pd(3-idx, &pd[idx]);

        bdmf_session_print(session, "%3d:%2d 0x%08x\n", (32*(3-idx)+31), 32*(3-idx), pd[idx]);

        /* Change endianes */
        pd[idx] = __swap4bytes(pd[idx]);
    }
    if (rc)
        return rc;

    /* Parse and print PD */
    dump_RDD_PROCESSING_TX_DESCRIPTOR(session, (unsigned char *)pd);
    
    return rc;
}

static int _drv_qm_cli_print_captured_pd(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    bdmf_error_t rc;

    bdmf_session_print(session, "\nQM last ingress processed PD captured:\n");

    rc = _drv_qm_read_print_pd(session, &ag_drv_qm_qm_ingress_processed_pd_capture_get);

    bdmf_session_print(session, "\nQM last illegal PD captured:\n");

    rc = rc ? rc : _drv_qm_read_print_pd(session, &ag_drv_qm_qm_illegal_pd_capture_get);

    bdmf_session_print(session, "\nQM last copy machine processed PD captured:\n");

    rc = rc ? rc : _drv_qm_read_print_pd(session, &ag_drv_qm_qm_cm_processed_pd_capture_get);
...
int drv_qm_cli_debug_get(bdmf_session_handle session, bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    ...
    static uint32_t qm_debug[] = {cli_qm_debug_counters, cli_qm_debug_info, cli_qm_good_lvl1_pkts_cnt, cli_qm_good_lvl1_bytes_cnt, cli_qm_good_lvl2_pkts_cnt,
                                  ...
                                  cli_qm_fpm_congestion_drop_cnt,cli_qm_ddr_pd_congestion_drop_cnt, cli_qm_ddr_byte_congestion_drop_cnt, 
                                  cli_qm_qm_pd_congestion_drop_cnt, cli_qm_qm_abs_requeue_cnt, cli_qm_ingress_stat, cli_qm_egress_stat, cli_qm_cm_stat, 
                                  cli_qm_fpm_prefetch_stat, cli_qm_qm_connect_ack_counter, cli_qm_qm_ddr_wr_reply_counter, cli_qm_qm_ddr_pipe_byte_counter, 
                                  cli_qm_qm_abs_requeue_valid_counter};
#else

    static uint32_t qm_debug[] = {cli_qm_good_lvl1_pkts_cnt, cli_qm_good_lvl1_bytes_cnt, cli_qm_good_lvl2_pkts_cnt,
                                  cli_qm_good_lvl2_bytes_cnt, cli_qm_copied_pkts_cnt, cli_qm_copied_bytes_cnt, cli_qm_agg_pkts_cnt, cli_qm_agg_bytes_cnt,
                                  cli_qm_agg_1_pkts_cnt, cli_qm_agg_2_pkts_cnt, cli_qm_agg_3_pkts_cnt, cli_qm_agg_4_pkts_cnt,cli_qm_wred_drop_cnt,
                                  cli_qm_fpm_congestion_drop_cnt,cli_qm_ddr_pd_congestion_drop_cnt, cli_qm_ddr_byte_congestion_drop_cnt,
                                  cli_qm_qm_pd_congestion_drop_cnt, cli_qm_qm_abs_requeue_cnt, cli_qm_ingress_stat, cli_qm_egress_stat, cli_qm_cm_stat,
    ...
    static uint32_t fpm_ug_debug[] = {cli_qm_fpm_grp_drop_cnt};
    static uint32_t fpm_pool_debug[] = {cli_qm_fpm_pool_drop_cnt};

    /* get qm debug */
    bdmf_session_print(session, "\nQM debug information:\n");
    HAL_CLI_PRINT_LIST(session, qm, qm_debug);
    
    bdmf_session_print(session, "\nQM fpm UG debug information:\n");
    HAL_CLI_PRINT_NUM_OF_LIST(session, qm, fpm_ug_debug, NUM_OF_FPM_UG);
    
    bdmf_session_print(session, "\nQM fpm pool debug information:\n");
    HAL_CLI_PRINT_NUM_OF_LIST(session, qm, fpm_pool_debug, NUM_OF_FPM_POOLS);
...
void drv_qm_cli_init(bdmfmon_handle_t driver_dir)
{
    ...
    BDMFMON_MAKE_CMD_NOPARM(qm_dir, "print_captured_pd", "print packet descriptors captures by QM", _drv_qm_cli_print_captured_pd);
