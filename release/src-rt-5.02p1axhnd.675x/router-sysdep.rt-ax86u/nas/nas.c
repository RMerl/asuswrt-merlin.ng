 ...
 *
 * $Id: nas.c 764796 2018-06-04 09:12:53Z $
 */
          ...
          unsigned char *key, int key_len, int index, int unicast)
{
	...
	*/
	...
	if (send_key) {
		...
		hmac_sha2(HASH_MD5, send_key, send_key_len, NULL, 0,
			&eapol->version, packet.iov_len - OFFSETOF(eapol_header_t, version),
			body->signature, EAPOL_KEY_SIG_LEN);
	}
	...
	case EAP_PACKET:
		...
		if (ntohs(eapol->length) >= (EAP_HEADER_LEN + 1)) {
			...
			default:
				/* Bogus packet */
				/*
				* Don't check it at all to work around some weired protocol
				* in which the peer does not always send responses and the AS
				* does not always send requests. They could send both.
				*/
...
driver_message_dispatch(nas_t *nas, bcm_event_t *dpkt)
{
	...
	nas_sta_t *sta;
#ifdef WLHOSTFBT
	uint8 *event_data = (uint8 *)(event + 1);
	wl_event_fbt_t *fbt_event;
#endif /* WLHOSTFBT */
#ifdef BCMDBG
	char eabuf[ETHER_ADDR_STR_LEN];
#endif // endif
	...
	sta = lookup_sta(nas, (struct ether_addr *)addr,
		(type == WLC_E_DISASSOC_IND || type == WLC_E_DISASSOC || type == WLC_E_DEAUTH_IND ||
...
#ifdef WLHOSTFBT
	case WLC_E_FBT:
		fbt_event = (wl_event_fbt_t *)event_data;
		/* Event data is sent in little endian, so do the conversion accordingly */
		uint16 len = host_to_le16(fbt_event->length);
		uint16 sub_type = host_to_le16(fbt_event->type);
		uint16 offset = host_to_le16(fbt_event->data_offset);
		uint16 data_len = len - offset;
		uint8 *data = (uint8*)fbt_event + offset;
		dbg(nas, "Received FBT Subtype = %d", sub_type);

		if (sub_type == WL_E_FBT_TYPE_FBT_OTD_AUTH) {
			dbg(nas, "FT OTD Authentication Request received by AP");
			wpa_ft_process_auth_action(nas->wpa, data, data_len);
		}
		if (sub_type == WL_E_FBT_TYPE_FBT_OTA_AUTH) {
			dbg(nas, "FT OTA Authentication Request received by AP");
			wpa_ft_process_auth(nas->wpa, sta, data, data_len);
		}
		break;
#endif /* WLHOSTFBT */
...
#ifdef WLHOSTFBT
/* Watchdog for NAS FBT to push PMKR1 */
static void
nas_watchdog(bcm_timer_id td, nas_t *nas)
{
	...
	fbt_aps = nvram_safe_get(strcat_r(wpa->fbt_info.prefix, "fbt_aps", data));
