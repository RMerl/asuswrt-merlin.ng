 ...
 * $Id: nas_radius.c 774063 2019-04-09 06:11:02Z $
 */
...
radius_dispatch(nas_t *nas, radius_header_t *response)
{
	...
	while (left >= 2) {
		...
		case RD_TP_MESSAGE_AUTHENTICATOR:
			...
			hmac_sha2(HASH_MD5, nas->secret.data, nas->secret.length, NULL, 0,
				(unsigned char *) response, ntohs(response->length), cur, 16);
	...
	case RADIUS_ACCESS_ACCEPT:
		...
		if (mppe_recv) {
			...
			if (nas_set_key(nas, &sta->ea, &mppe_recv[1], length, index,
				1, 0, 0, 0) < 0) {
...
radius_forward(nas_t *nas, nas_sta_t *sta, eap_header_t *eap)
{
	...
	/* NAS Port */
	val = htonl((uint32) pae_hash(&sta->ea));
	...
	hmac_sha2(HASH_MD5, nas->secret.data, nas->secret.length, NULL, 0,
		(unsigned char *)request, ntohs(request->length), ptr, 16);
