 ...
 *
 * $Id: nas_wpa.c 771158 2019-01-16 09:15:26Z $
 */
...
#ifdef WLHOSTFBT
static bool wpa_insert_pmk_r1name(wpa_t *wpa, nas_sta_t *sta,
		eapol_header_t *eapol, uint16 *data_len);
static bool wpa_insert_mdie(wpa_t *wpa, nas_sta_t *sta, eapol_header_t *eapol, uint16 *data_len);
static bool wpa_insert_ftie(wpa_t *wpa, nas_sta_t *sta, eapol_header_t *eapol, uint16 *data_len,
		const unsigned char *anonce, const unsigned char *snonce,
		const unsigned char *subelem, size_t subelem_len);
#endif /* WLHOSTFBT */

#ifdef MFP
#define MFP2 1
void wpa_gen_igtk(wpa_t *wpa);
static void wpa_initialize_ipn(wpa_t *wpa);
#endif // endif

/* Definitions for interval timer periods */
#define RETRY_SECS	3 /* more tolerate peer late resp */
...
#ifdef BCM_CEVENT
static void wpa_send_cevent_a2c(wpa_t *wpa, nas_sta_t *sta, uint32 a2c_subtype, uint32 flags);
#endif /* BCM_CEVENT */

/* Default values of RSNE fields */
wpa_suite_mcast_t def_mcast = {{0x00, 0x0f, 0xac}, 0x04};
wpa_suite_ucast_t def_ucast = {.count = {0x01, 0x00}, .list[0] = {{0x00, 0x0f, 0xac}, 0x04}};
wpa_suite_auth_key_mgmt_t def_akm = {.count = {0x01, 0x00}, .list[0] = {{0x00, 0x0f, 0xac}, 0x01}};
wpa_rsncap_t def_rsn_cap = {0x00};

/* Set an iterval timer. */
itimer_status_t
wpa_set_itimer(bcm_timer_module_id module, bcm_timer_id *td,
               bcm_timer_cb handler, uintptr_t handler_param, int secs, int msecs)
{
...
#ifdef BCM_CEVENT
		dbg(nas, "NAS_TIMEOUT. Sending cevent from NAS. Reason %s",
				((reason == DOT11_RC_GTK_UPDATE_TIMEOUT) ?
				 "DOT11_RC_GTK_UPDATE_TIMEOUT" : "DOT11_RC_4WH_TIMEOUT"));
		wpa_send_cevent_a2c(wpa, sta, CEVENT_A2C_ST_NAS_TIMEOUT, 0);
#endif /* BCM_CEVENT */
...
{
	hmac_sha2_ctx_t ctx;
	const uint8 data[] = "PMK Name";

#if defined(MFP) || defined(HSPOT_OSEN)
	if (sta->key_auth_type == KEYAUTH_SHA256) {
		if (hmac_sha2_init(&ctx, HASH_SHA256, (const uint8 *)sta->suppl.pmk,
			sta->suppl.pmk_len)) {
			dbg(wpa->nas, "hmac_sha2_init fail!");
			return;
		}
	}
	else
	...
	{
		if (hmac_sha2_init(&ctx, HASH_SHA1, (const uint8 *)sta->suppl.pmk,
			sta->suppl.pmk_len)) {
			dbg(wpa->nas, "hmac_sha2_init fail!");
			return;
		}
	...
	hmac_sha2_update(&ctx, data, sizeof(data) - 1);
	hmac_sha2_update(&ctx, (const uint8 *)&wpa->nas->ea, ETHER_ADDR_LEN);
	hmac_sha2_update(&ctx, (const uint8 *)&sta->ea, ETHER_ADDR_LEN);
	hmac_sha2_final(&ctx, sta->suppl.pmkid, WPA2_PMKID_LEN);
...
wpa_calc_ptk(wpa_t *wpa, nas_sta_t *sta)
{
	...
	pfx[npfx++].data = (const uint8 *) wpa_array_cmp(MIN_ARRAY,
		(const uint8 *)&wpa->nas->ea, (const uint8 *)&sta->ea, ETHER_ADDR_LEN);
	...
	pfx[npfx++].data = (const uint8 *)wpa_array_cmp(MIN_ARRAY,
		(const uint8 *)&sta->suppl.snonce, (const uint8 *)&sta->suppl.anonce,
...
#ifdef WLHOSTFBT
	if (CHECK_FBT(sta->mode)) {
		wpa_auth_derive_ptk_ft(wpa, sta,
				sta->suppl.eapol_mic_key, sta->suppl.ptk_len);
	}
	else
#endif /* WLHOSTFBT */
#if defined(MFP) || defined(HSPOT_OSEN)
	if (sta->key_auth_type == KEYAUTH_SHA256) {
		hmac_sha2_n(HASH_SHA256, sta->suppl.pmk, sta->suppl.pmk_len, pfx, npfx, NULL, 0,
			sta->suppl.eapol_mic_key, sta->suppl.ptk_len);
	}
	else
#endif /* MFP || HSPOT_OSEN */
	{
		hmac_sha2_n(HASH_SHA1, sta->suppl.pmk, sta->suppl.pmk_len, pfx, npfx, NULL, 0,
			sta->suppl.eapol_mic_key, sta->suppl.ptk_len);
	}
...
wpa_gen_gtk(wpa_t *wpa, nas_sta_t *sta)
{
	...
	wpa_generate_rand_nonce(&wpa->nas->ea, wpa->gnonce, sizeof(wpa->gnonce));
	bcopy((char*)&wpa->gnonce, (char*)&data[data_len], NONCE_LEN);
	data_len += NONCE_LEN;

	/* generate the GTK */
	pfx.len = sizeof(prefix) - 1;
	pfx.data = (const uint8 *)prefix;
	(void)hmac_sha2_n(HASH_SHA1, wpa->gmk, sizeof(wpa->gmk),
		&pfx, 1, data, data_len, wpa->gtk, wpa->gtk_len);
...
#define CATHY_RETRY_SET_KEY 5

#ifdef CATHY_RETRY_SET_KEY
void
wpa_plumb_gtk(wpa_t *wpa, int primary)
{
	uint16 lo;
	uint32 hi;
	int retry = CATHY_RETRY_SET_KEY;
	int ret;
...
retry_set_key:
	/* install the key */
	if ((ret = nas_set_key(wpa->nas, NULL, wpa->gtk, wpa->gtk_len,
                        wpa->gtk_index, primary, hi, lo, 0)) < 0) {
		if (retry > 0) {
			retry--;
			err(wpa->nas, "retry_set_key retry %d!! ret %d\n",
				retry, ret);
			goto retry_set_key;
		}
		else {
...
wpa_set_gtk_timer(wpa_t *wpa)
{
	...
	if (wpa->gtk_rekey_secs) {
		...
		ts = wpa_set_itimer(wpa->nas->timer, &wpa->gtk_rekey_timer,
		                    ...
		                    (uintptr_t)wpa, wpa->gtk_rekey_secs, 0);
		if (ts != ITIMER_OK)
...
#if defined(MFP) || defined(WLHOSTFBT)
...
#ifdef BCM_CEVENT
/* send cevent in bcm event on installation of ptk/gtk on AP */
static void
wpa_send_cevent_a2c(wpa_t *wpa, nas_sta_t *sta, uint32 a2c_subtype, uint32 flags)
{
	struct iovec frags;
	struct timespec tp;
	uint64 time_in_ms = 0;
	...
	struct nas *nas = NULL;
	struct ether_addr *lan_ea = NULL;
	struct ether_addr *sta_ea = NULL;
	bcm_event_t *be = (bcm_event_t *)buffer;
	wl_cevent_t *ce = (wl_cevent_t *)(be + 1);

	if ((wpa == NULL) || (sta == NULL)) {
		dbg(wpa->nas, "wpa/sta is NULL\n");
		return;
	}
	...
	if ((lan_ea == NULL) || (sta_ea == NULL)) {
		dbg(wpa->nas, "lan_ea/sta_ea is NULL\n");
		return;
	}

	if (clock_gettime(CLOCK_REALTIME, &tp) < 0) {
		dbg(nas, "clock_gettime failed\n");
		return;
	}
	...
	ce->length = sizeof(*ce); /* No data */
	ce->type = CEVENT_TYPE_A2C;
	ce->subtype = a2c_subtype;
	ce->flags = flags; /* No data, hence no eth header in data */
	ce->data_offset = sizeof(*ce); /* However, no data */
	...
	be->event.datalen = hton32(ce->length);
	bcopy(sta_ea, &(be->event.addr), sizeof(*sta_ea)); /* station address */
	memcpy(&(be->event.ifname), nas->interface, BCM_MSG_IFNAME_MAX);
	be->event.ifidx = 0;
	...
	frags.iov_len = sizeof(*be) + sizeof(*ce);

	/* send event to cevent_app through socket */
	nas_send_packet(nas, &frags, 1);
...
wpa_insert_gtk(wpa_t *wpa, nas_sta_t *sta, eapol_header_t *eapol, uint16 *data_len)
{
	...
	/* Do not generate GTK for WDS link */
	if (!(wpa->nas->flags & NAS_FLAG_WDS)) {
		/* make sure we have a GTK plumbed */
		if (!(wpa->nas->flags & NAS_FLAG_GTK_PLUMBED)) {
			wpa_init_gtk(wpa, sta);
#ifdef BCM_CEVENT
			dbg(wpa->nas, "GTK Installed. Sending CEVENT from NAS");
			wpa_send_cevent_a2c(wpa, sta, CEVENT_A2C_ST_GTK_INSTALL, 0);
#endif /* BCM_CEVENGT */
		...
		if (wpa->gtk_per_sta) {
			bcopy(&sta->ea, gtk, ETHER_ADDR_LEN);
			bcopy(wpa->gtk+ETHER_ADDR_LEN, gtk+ETHER_ADDR_LEN,
					TKIP_TK_LEN-ETHER_ADDR_LEN);
		} else {
			bcopy(wpa->gtk, gtk, TKIP_TK_LEN);
		}
#endif // endif
...
wpa_extract_gtk(wpa_t *wpa, nas_sta_t *sta, eapol_header_t *eapol)
{
	...
	if (!(key_info & WPA_KEY_MIC)) {
		dbg(wpa->nas, "ignoring the gtk as MIC bit is not set.");
		return (FALSE);
	}

	/* FIXME: need to validate the all the key data encap fits within the
	 * received frame.  don't let the reported encap length be an buffer overflow
	 * problem
	 */
	/* look for a GTK */
...
static void
wpa_countermeasures_cb(bcm_timer_id td, nas_t *nas)
{
...
void wpa_reset_countermeasures(wpa_t *wpa)
{
	if (wpa && wpa->countermeasures_timer)
		wpa_countermeasures_cb(wpa->countermeasures_timer, wpa->nas);
}
 ...
 */
static size_t
wpa_do_mic(nas_sta_t *sta, int kdver, eapol_header_t *eapol, uint8* out_mic)
{
	size_t mic_len;
	int data_len;
	int err = BCME_OK;
...
#ifdef HSPOT_OSEN
	if (kdver == WPA_KEY_DESC_OSEN)
		kdver = WPA_KEY_DESC_V3;
#endif /* HSPOT_OSEN */
	...
	switch (kdver) {
	case WPA_KEY_DESC_V1:
		err = hmac_sha2(HASH_MD5, (const uint8 *)sta->suppl.eapol_mic_key, WPA_MIC_KEY_LEN,
			NULL, 0, (const uint8 *)&eapol->version, data_len,
			out_mic, EAPOL_WPA_KEY_MIC_LEN);
		break;
	case WPA_KEY_DESC_V2:
		err = hmac_sha2(HASH_SHA1, (const uint8 *)sta->suppl.eapol_mic_key, WPA_MIC_KEY_LEN,
			NULL, 0, (const uint8 *)&eapol->version, data_len,
			out_mic, EAPOL_WPA_KEY_MIC_LEN);
		break;
#if defined(MFP) || defined(WLHOSTFBT)
	case WPA_KEY_DESC_V3:
		aes_cmac_calc(NULL, 0, (const uint8 *)&eapol->version, (const size_t)data_len,
			(const uint8 *)sta->suppl.eapol_mic_key, EAPOL_WPA_KEY_MIC_LEN,
			out_mic, AES_BLOCK_SZ);
		break;
#endif /* MFP || WLHOSTFBT */
	default:
...
static void wpa_send_mic_failure(wpa_t *wpa, nas_sta_t *sta)
{
	...
	wpa_do_mic(sta, sta->suppl.desc, eapol, wpa_key->mic);
...
wpa_mic_error(wpa_t *wpa, nas_sta_t *msta, bool from_driver, bool pairwise)
{
	...
	if ((nas->MIC_failures != 0) && (nas->MIC_countermeasures == 0) &&
		...
		ts = wpa_set_itimer(wpa->nas->timer, &wpa->countermeasures_timer,
		                    ...
		                    (uintptr_t) nas, WPA_TKIP_CM_BLOCK, 0);
		if (ts != ITIMER_OK)
...
/*
* FIXME: this routine should go away once we have a way to share the code
* with wlc/wl in which the exact same thing is done.
*
* wsec - crypto bitvec of WEP_ENABLED, TKIP_ENABLED, and AES_ENABLED.
*	 it is all supported cryptos on authenticator/AP, or
*	 it is selected cryptos (1uc/1mc) on supplicant/STA.
* algo - key algo CRYPTO_ALGO_WEPXXXX when WEP_ENABLED is set in wsec.
* mode - see nas_mode_t
*/
static int
	...
	uint8 *buf, uint16 *len)
{
	...
	*len = 0;
	...
	if (!wds && !sup) {
		...
		if (sta_mode & (WPA2 | WPA2_PSK | WPA | WPA_PSK | WPA2_FT)) {
			/* get wpa ie */
	...
	if ((mode & (WPA2 | WPA2_PSK | WPA2_FT)) && (sta_mode & (WPA2 | WPA2_PSK | WPA2_FT))) {
...
wpa_send_eapol(wpa_t *wpa, nas_sta_t *sta)
{
	...
	nas_t *nas = wpa->nas;
	uint16 data_len = 0;
	bool insert_pmkid = FALSE;
#ifdef BCMDBG
	char eabuf[ETHER_ADDR_STR_LEN];
#endif // endif
#ifdef WLHOSTFBT
	uint32 reassoc_deadline;
#endif /* WLHOSTFBT */
	...
	if ((sta->mode & (WPA2_PSK | WPA2 | WPA2_FT)))
	...
	case WPA_PTKSTART:
		...
		/* Insert PMKID in M1 for WPA2 (PSK/1x/FT) */
		insert_pmkid = ((sta->mode & (WPA2_PSK | WPA2 | WPA2_FT)) != 0);
#ifdef HSPOT_OSEN
		/* No PMKID in M1 for OSEN-AUTH */
		if (sta->flags & STA_FLAG_OSEN_AUTH) {
			insert_pmkid = FALSE;
		}
#endif /* HSPOT_OSEN */
		/* No PMKID in M1 for PSK if STA didn't send a valid PMKID in assoc request */
		if ((sta->mode & (WPA2_PSK | WPA_PSK | WPA2_FT)) &&
			!sta->suppl.assoc_req_has_valid_pmkid) {
			insert_pmkid = FALSE;
		}
	...
	case WPA_PTKINITNEGOTIATING:
...
#ifdef WLHOSTFBT
		if (sta->mode & WPA2_FT) {
			if (!wpa_insert_pmk_r1name(wpa, sta, eapol, &data_len)) {
				err(nas, "insertion of pmk-r1name failed");
				nas_handle_error(nas, 1);
			}
			if (!wpa_insert_mdie(wpa, sta, eapol, &data_len)) {
				err(nas, "insertion of mdie failed");
				nas_handle_error(nas, 1);
			}
		}
#endif /* WLHOSTFBT */
		...
		tie_t *tie_info;
		if (sta->mode & WPA2_FT) {
			if (!wpa_insert_ftie(wpa, sta, eapol, &data_len, NULL, NULL, NULL, 0)) {
				err(nas, "insertion of ftie failed");
				nas_handle_error(nas, 1);
			}

			nas_get_reassoc_timer(nas, &reassoc_deadline);
			wpa->fbt_info.reassociation_deadline = reassoc_deadline;
			/* TIE[ReassociationDeadline] (TU) */
			tie_info = (tie_t *) (body->data + data_len);
			tie_info->id = DOT11_MNG_FT_TI_ID;
			tie_info->len = REASSOC_DEADLINE_LEN;
			tie_info->type = WLAN_TIMEOUT_REASSOC_DEADLINE;
			WPA_PUT_LE32(tie_info->data, wpa->fbt_info.reassociation_deadline);
			data_len += TLV_HDR_LEN + REASSOC_DEADLINE_LEN;

			wpa->fbt_info.r0_key_lifetime = KEY_LIFETIME;
			/* TIE[KeyLifetime] (Sec) */
			tie_info = (tie_t *) (body->data + data_len);
			tie_info->id = DOT11_MNG_FT_TI_ID;
			tie_info->len = KEY_LIFETIME_LEN;
			tie_info->type = WLAN_TIMEOUT_KEY_LIFETIME;
			WPA_PUT_LE32(tie_info->data, wpa->fbt_info.r0_key_lifetime);
			data_len += TLV_HDR_LEN + KEY_LIFETIME_LEN;
		}
#endif /* WLHOSTFBT */
	...
	case WPA_SUP_STAKEYSTARTP:
		...
		case WPA_SUP_STAKEYSTARTP:
			...
			case EAPOL_SUP_PK_MSG1:
				/* Copy the WPA IE sent in the assoc req : XXX currently self built
				 */
				...
				body->data_len = htons(data_len);
				/* set the nonce */
				memcpy(body->nonce, sta->suppl.snonce, NONCE_LEN);
		...
		default:
			...
			/*
			* FIXME: For some reason, the supplicant did not get driver
			* assoc indication, or the WPA IE from driver assoc indication
			* was not up to date with what AP has. We need to fix it in
			* the right place either in the driver or the supplicant init
			* procedures. Here is just a temporary WAR:
			*
			* 	Disassoc and retry join.
			*/
			if (!(nas->flags & NAS_FLAG_WDS)) {
				nas_disassoc(nas);
				nas_sleep_ms(500);
	...
	if (mic_length)
		wpa_do_mic(sta, sta->suppl.desc, eapol, body->mic);

	if (sta->suppl.state == WPA_PTKSTART) {
#ifdef BCM_CEVENT
		dbg(wpa->nas, "M1 sent. Sending CEVENT from NAS");
		wpa_send_cevent_a2c(wpa, sta, CEVENT_A2C_ST_M1_TX, CEVENT_FRAME_DIR_TX);
#endif /* BCM_CEVENT */
	} else if (sta->suppl.state == WPA_PTKINITNEGOTIATING) {
#ifdef BCM_CEVENT
		dbg(wpa->nas, "M3 sent. Sending CEVENT from NAS");
		wpa_send_cevent_a2c(wpa, sta, CEVENT_A2C_ST_M3_TX, CEVENT_FRAME_DIR_TX);
#endif /* BCM_CEVENT */
	}
	/* send the pkt */
	nas_eapol_send_packet(nas, &frags, 1);
	...
	uint32 *pmkc, nas_sta_t *sta)
{
	...
	def_suite.mcast = def_mcast;
	def_suite.ucast = def_ucast;
	def_suite.akm = def_akm;
	def_suite.rsn_cap = def_rsn_cap;
	...
	case DOT11_MNG_RSN_ID: {
		...
		if (len == 0) {
			/* Since rest of the fields are optional in RSNE, use default
			* values for them.
			*/
			dbg(wpa->nas, "Use default mcast, ucast, akm suites and rsn cap");
			mcast = (wpa_suite_mcast_t *)(&def_suite.mcast);
			len += WPA_SUITE_LEN + (WPA_IE_SUITE_COUNT_LEN + WPA_SUITE_LEN) +
				(WPA_IE_SUITE_COUNT_LEN + WPA_SUITE_LEN) + WPA_CAP_LEN;
		} else {
	...
	if (len == 0) {
		 ...
		 * values for them.
		 */
		dbg(wpa->nas, "Use default ucast, akm suites and rsn cap");
		ucast = (wpa_suite_ucast_t *)(&def_suite.ucast);
	...
	if (len == 0) {
		 ...
		 * values for them.
		 */
		dbg(wpa->nas, "Use default akm suite and rsn cap");
		mgmt = (wpa_suite_auth_key_mgmt_t *)(&def_suite.akm);
		len += (WPA_IE_SUITE_COUNT_LEN + WPA_SUITE_LEN) + WPA_CAP_LEN;
	} else {
	...
	if (len == 0) {
		 ...
		 * values for them.
		 */
		dbg(wpa->nas, "Use default rsn cap");
		cap = (uint8 *)(&def_suite.rsn_cap);
		len += WPA_CAP_LEN;
	} else {
...
#endif // endif
				akm_type = mgmt->list[i].type;
#ifdef WLHOSTFBT
				if (!CHECK_FBT(sta->mode)) {
					/* downgrade to non-FBT variant */
					if (akm_type == RSN_AKM_FBT_1X) {
						akm_type = RSN_AKM_UNSPECIFIED;
					} else if (akm_type == RSN_AKM_FBT_PSK) {
						akm_type = RSN_AKM_PSK;
					}
				}
#endif /* WLHOSTFBT */
				mode &= ~wpa_auth2mode(akm2auth(akm_type));
			}
		}
	}
...
wpa_set_suppl(wpa_t *wpa, nas_sta_t *sta, uint32 mode, uint32 wsec, uint32 algo)
{
	...
	if (WSEC_AES_ENABLED(wsec)) {
		...
		sta->suppl.tk_len = AES_TK_LEN;
#ifdef WLHOSTFBT
		/* Set key descriptor version to 3 for FBT case */
		if (mode & WPA2_FT) {
			sta->suppl.desc = WPA_KEY_DESC_V3;
		}
		else
...
wpa_driver_assoc_msg(wpa_t *wpa, bcm_event_t *dpkt, nas_sta_t *sta)
{
	...
	if (nas->flags & NAS_FLAG_AUTHENTICATOR) {
		...
		sta->suppl.assoc_req_has_valid_pmkid = (pmkc == 1);
	}
#ifdef BCMSUPPL
	...
	if ((sta->mode & WPA2) || (sta->mode & WPA2_PSK)) {
		...
		nas_get_stainfo(nas, (char *)&sta->ea, sizeof(struct ether_addr), sta_info_buf,
		                300);
		sta_info = (sta_info_v5_t *)sta_info_buf;
	...
	if (nas->flags & NAS_FLAG_SUPPLICANT) {
		/* Reset the GTK */
		dbg(wpa->nas, "Reset previos GTK");
		memset(wpa->gtk, 0, wpa->gtk_len);
		/* fill the snonce */
		wpa_generate_rand_nonce(&nas->ea, sta->suppl.snonce, sizeof(sta->suppl.snonce));
		dbg(nas, "done");
		return 0;
	}
#endif /* BCMSUPPL */
	...
	else if (sta->mode & (WPA2_PSK | WPA2_FT)) {
		/*
...
{
	wl_event_msg_t *event = &(dpkt->event);
	uint32 len = ntoh32(event->datalen);
...
wpa_ptkinitnegotiating(wpa_t *wpa, nas_sta_t *sta, eapol_header_t *eapol)
{
...
#ifdef WLHOSTFBT
	if (CHECK_FBT(sta->mode)) {
		if ((wpa_ft_validate_ies(wpa, body->data, ntohs(body->data_len),
				&parse, &rsnie_data) != 0) ||
				(parse.r1kh_id == NULL)) {
			dbg(wpa->nas, "FT: Validation of FT IEs failed");
			return 0;
		}

		mdie = (struct rsn_mdie *) parse.mdie;
		wpaie_len = parse.rsn_len;

		/* MDIE info */
		memcpy(wpa->fbt_info.mobility_domain, mdie->mobility_domain,
		...
		nas_get_fbt_r0kh(wpa->nas, buf, sizeof(buf));
		...
		dbg(wpa->nas, "FT: Mobility domain");
		dump(wpa->nas, wpa->fbt_info.mobility_domain, MOBILITY_DOMAIN_ID_LEN);
		dbg(wpa->nas, "FT: R1KH");
		dump(wpa->nas, wpa->fbt_info.r1_key_holder, FT_R1KH_ID_LEN);
		dbg(wpa->nas, "FT: R0KH");
		dump(wpa->nas, wpa->fbt_info.r0_key_holder, wpa->fbt_info.r0_key_holder_len);
	}
	else
#endif /* WLHOSTFBT */
		wpaie_len = ntohs(body->data_len);

	/* In case of FBT, EAPOL key 2 msg will have PMKID, MDIE, FTIE too, apart from RSNIE.
	 * So, We can not compare key 2 RSN IE with assoc request RSN IE
	 */
	if (!CHECK_FBT(sta->mode)) {
		/* Check the WPA IE */
	...
	if (wpa_check_mic(sta, eapol) == FALSE) {
		...
		ts = wpa_set_itimer(wpa->nas->timer, &sta->td, (bcm_timer_cb)wpa_retransmission,
				(uintptr_t)sta, sta->wpa_msg_timeout_s,
				sta->wpa_msg_timeout_ms);
		if (ts != ITIMER_OK)
...
#ifdef BCM_CEVENT
	dbg(nas, "PTK Installed. Sending CEVENT from NAS\n");
	wpa_send_cevent_a2c(wpa, sta, CEVENT_A2C_ST_PTK_INSTALL, 0);
#endif /* BCM_CEVENT */
...
wpa_rekeyneg(wpa_t *wpa, nas_sta_t *sta, eapol_header_t *eapol)
{
	...
	if (wpa->ptk_rekey_secs) {
		itimer_status_t ts = wpa_set_itimer(wpa->nas->timer, &wpa->ptk_rekey_timer,
		                                    ...
		                                    (uintptr_t)wpa, wpa->ptk_rekey_secs, 0);
		if (ts != ITIMER_OK)
...
eapol_sup_process_key(wpa_t *wpa, eapol_header_t *eapol, nas_sta_t *sta)
{
	...
	bool wds = wpa->nas->flags & NAS_FLAG_WDS;
	...
	/* check data length */
	if (ntohs(eapol->length) < OFFSETOF(eapol_wpa_key_header_t, data)) {
		err(wpa->nas, "too short - no data len");
		return EAPOL_SUP_PK_UNKNOWN;
	}

	if (ntohs(eapol->length) < (OFFSETOF(eapol_wpa_key_header_t, data) + data_len)) {
		err(wpa->nas, "not enough data");
		return EAPOL_SUP_PK_UNKNOWN;
	}
	...
	/* FIXME: need to handle group vs. pairwise replies better.  this
	 * state change is only here because we don't have a better way
	 * to propagate reply-specific info into wpa_send_eapol()
	 */
	if (key_info & WPA_KEY_PAIRWISE) {
	...
	/* FIXME: do we need to handle receipt of pairwise when expecting group, and
	 * group when expecting pairwise? e.g what if we're expecting frame 3 of
	 * pairwise exchange, and get frame 1 of group key update?
	 */
	if (key_info & WPA_KEY_PAIRWISE) {
		...
		if (state != EAPOL_SUP_PK_MICFAILED)  {
			...
			if (key_info & WPA_KEY_INSTALL) {
				...
				if ((sta->mode & (WPA2 | WPA2_PSK))) {
					...
					/* Do not update GTK for WDS link */
					if (!wds) {
						memset(t_gtk, 0, sizeof(t_gtk));
						memcpy(t_gtk, wpa->gtk, wpa->gtk_len);
						if (!(UpdateGTK =
							wpa_extract_gtk(wpa, sta, eapol))) {
							err(wpa->nas, "extraction of gtk from"
								" eapol message failed");
							return EAPOL_SUP_PK_UNKNOWN;
						}
...
/*		Send EAPOL(0, 1, 0, 0, 0, K, 0, TSNonce, 0, MIC(TPTK), 0) */
	...
	if (UpdatePTK == TRUE) {
		...
		/*
		  * FIXME: In 4-way handshaking the last EAPOL-KEY will send
		  * after set key (due to EAPOL-KEY may queue in bridge),
		  * than driver will encrypt EAPOL-KEY as wep packet.
		  * We need to fix it in the right place maybe in the driver
		  * Here is just a temporary WAR:
		  */
		nas_sleep_ms(100);
	...
	if ((nas->flags & NAS_FLAG_WDS) &&
		...
		if (sta->suppl.state == WPA_AUTHENTICATION2 ||
			...
			*/
			if (!sta->wds_td) {
				/*
				* FIXME: are we going to do this forever
				* in case authenticator and supplicant
				* don't have common cipher?
				*/
				wpa_start(wpa, sta);
			}
		}
...
#ifdef BCM_CEVENT
		dbg(wpa->nas, "M2 received. Sending CEVENT from NAS");
		wpa_send_cevent_a2c(wpa, sta, CEVENT_A2C_ST_M2_RX, CEVENT_FRAME_DIR_RX);
#endif /* BCM_CEVENT */
...
#ifdef BCM_CEVENT
		dbg(wpa->nas, "M4 received. Sending CEVENT from NAS");
		wpa_send_cevent_a2c(wpa, sta, CEVENT_A2C_ST_M4_RX, CEVENT_FRAME_DIR_RX);
#endif /* BCM_CEVENT */
...
initialize_global_key_counter(wpa_t *wpa)
{
	...
	(void)hmac_sha2_n(HASH_SHA256, buff, (int)sizeof(buff), &pfx, 1,
		(uint8 *)&wpa->nas->ea, ETHER_ADDR_LEN, wpa->global_key_counter,
		KEY_COUNTER_LEN);
	memcpy(wpa->initial_gkc, wpa->global_key_counter, KEY_COUNTER_LEN);
}

/* generates given size random nonce */
void
wpa_generate_rand_nonce(struct ether_addr *ea, uint8 *pnonce, size_t size)
{
	unsigned char buff[32], prf_buff[KEY_COUNTER_LEN];
	unsigned char prefix[] = "Random Counter";
	bcm_const_xlvp_t pfx;

	if ((pnonce == NULL) || (ea == NULL)) {
		return;
	}
	nas_rand128(&buff[0]);
	nas_rand128(&buff[16]);
	...
	(void)hmac_sha2_n(HASH_SHA256, buff, (int)sizeof(buff), &pfx, 1,
		(uint8 *)ea, ETHER_ADDR_LEN, prf_buff,
		KEY_COUNTER_LEN);
	memcpy(pnonce, prf_buff, size);
}
...
wpa_initialize_ipn(wpa_t *wpa)
{
	...
	(void)hmac_sha2_n(HASH_SHA256, buff, (int)sizeof(buff), &pfx, 1,
		(uint8 *)&wpa->nas->ea, ETHER_ADDR_LEN, buff,
		sizeof(wpa->igtk.ipn_lo) + sizeof(wpa->igtk.ipn_hi));

	memcpy(&wpa->igtk.ipn_lo, buff, sizeof(wpa->igtk.ipn_lo));
	memcpy(&wpa->igtk.ipn_hi, buff+sizeof(wpa->igtk.ipn_lo), sizeof(wpa->igtk.ipn_hi));
}
...
wpa_gen_igtk(wpa_t *wpa)
{
	...
	*(uint16 *)&data[data_len+sizeof(uint32)] = wpa->igtk.ipn_hi;
	...
	/* generate the GTK */
	(void)hmac_sha2_n(HASH_SHA256, wpa->gmk, sizeof(wpa->gmk), &pfx, 1,
		data, data_len, wpa->igtk.key, AES_TK_LEN);
...
static void
request_pkinit(bcm_timer_id timer, uintptr_t data)
{
	...
	if (wpa_set_itimer(nas->timer, &sta->td, (bcm_timer_cb)wpa_retransmission,
	    (uintptr_t)sta, sta->wpa_msg_timeout_s, sta->wpa_msg_timeout_ms) != ITIMER_OK)
...
wpa_request(wpa_t *wpa, nas_sta_t *sta)
{
	...
	*/
	...
	if (sta->suppl.pk_state == EAPOL_SUP_PK_UNKNOWN ||
		...
		request_pkinit(0, (uintptr_t)sta);
	}
	...
	if (bcm_timer_create(nas->timer, &sta->wds_td))
		ret = ITIMER_CREATE_ERROR;
	else if (bcm_timer_connect(sta->wds_td, (bcm_timer_cb)request_pkinit, (uintptr_t)sta))
		ret = ITIMER_CONNECT_ERROR;
...
static void
start_pkinit(bcm_timer_id timer, uintptr_t data)
{
...
wpa_start(wpa_t *wpa, nas_sta_t *sta)
{
	...
	if (bcm_timer_create(nas->timer, &sta->wds_td))
		ret = ITIMER_CREATE_ERROR;
	else if (bcm_timer_connect(sta->wds_td, (bcm_timer_cb)start_pkinit, (uintptr_t)sta))
		ret = ITIMER_CONNECT_ERROR;
	else if (bcm_timer_settime(sta->wds_td, &its))
		ret = ITIMER_SET_ERROR;
	if (ret)
...
#ifdef WLHOSTFBT
/* Insert PMKR1Name in EAPOL message */
static bool
wpa_insert_pmk_r1name(wpa_t *wpa, nas_sta_t *sta, eapol_header_t *eapol, uint16 *data_len)
{
	int len = *data_len;
	eapol_wpa_key_header_t *body = (eapol_wpa_key_header_t *)eapol->body;
	uint16 offset = 0;
	...
	/* Insert PMKR1Name in the end */
	/* Update RSNIE length to include PMKR1Name */
	body->data[1] += sizeof(wpa_pmkid_list_t);
	len += sizeof(wpa_pmkid_list_t);

	pmkid = (wpa_pmkid_list_t *)&body->data[offset];

	/* Insert PMKR1Name in the PMKID field of the RSNE */
	pmkid->count.low = 1;
	pmkid->count.high = 0;
	bcopy(wpa->fbt_info.pmk_r1name, &pmkid->list[0], WPA_PMK_NAME_LEN);
...
/* Insert MDIE in EAPOL message */
static bool
wpa_insert_mdie(wpa_t *wpa, nas_sta_t *sta, eapol_header_t *eapol, uint16 *data_len)
{
	int len = *data_len;
	eapol_wpa_key_header_t *body = (eapol_wpa_key_header_t *)eapol->body;
	bcm_tlv_t *mdie;
	...
	dbg(wpa->nas, "data len before mdie %d", len);

	/* insert MDIE into eapol message */
	mdie = (bcm_tlv_t *) (body->data + len);
	mdie->id = DOT11_MNG_MDIE_ID;
	mdie->len = MOBILITY_DOMAIN_ID_LEN + 1;
	bcopy(wpa->fbt_info.mobility_domain, mdie->data, MOBILITY_DOMAIN_ID_LEN);
	len += TLV_HDR_LEN + MOBILITY_DOMAIN_ID_LEN;
	...
	*(body->data + len) = capab;
	...
	dbg(wpa->nas, "data len including mdie %d", len);
...
/* Insert FTIE in EAPOL message */
static bool
wpa_insert_ftie(wpa_t *wpa, nas_sta_t *sta, eapol_header_t *eapol, uint16 *data_len,
		const unsigned char *anonce, const unsigned char *snonce,
		const unsigned char *subelem, size_t subelem_len)
{
	eapol_wpa_key_header_t *body = (eapol_wpa_key_header_t *)eapol->body;
	struct rsn_ftie *ftie;
	unsigned char *pos, *ielen, *buf;
	int len = *data_len;

	dbg(wpa->nas, "data len before ftie %d", len);

	pos = (unsigned char *) (body->data + len);
	buf = pos;
	...
	*pos++ = DOT11_MNG_FTIE_ID;
	...
	pos += sizeof(*ftie);
	WPA_PUT_LE16(ftie->mic_control, 0);
	if (anonce)
		memcpy(ftie->anonce, anonce, NONCE_LEN);
	if (snonce)
		memcpy(ftie->snonce, snonce, NONCE_LEN);
	...
	*pos++ = FT_R1KH_ID_LEN;
	...
	if (wpa->fbt_info.r0_key_holder) {
		*pos++ = FTIE_SUBELEM_R0KH_ID;
		*pos++ = wpa->fbt_info.r0_key_holder_len;
		memcpy(pos, wpa->fbt_info.r0_key_holder, wpa->fbt_info.r0_key_holder_len);
		pos += wpa->fbt_info.r0_key_holder_len;
	}
	...
	*ielen = pos - buf - 2;
	...
	dbg(wpa->nas, "data len including ftie %d", len);
