                                     ...
                                     tmctl_queueDropAlg_t* dropAlg_p)
{
   ...
   if (dropAlg_p->dropAlgorithm == TMCTL_DROP_RED)
   {
      ...
      if (dropAlg_p->queueProfileIdLo == 0)
         use_ext = 1;
   }
   else if (dropAlg_p->dropAlgorithm == TMCTL_DROP_WRED)
   {
      dropAlgorithm = FAP_IOCTL_TM_DROP_ALG_WRED;
      if ((dropAlg_p->queueProfileIdLo == 0) && (dropAlg_p->queueProfileIdHi == 0))
         use_ext = 1;
   }
   else
   {
   ...
   if (use_ext != 0)
   {
      if (fapCtlTm_queueDropAlgConfigExt((char *)ifname, qid, dropAlgorithm,
