...
#elif defined(BCM_PON_XRDP) || defined(BCM_DSL_XRDP)
...
#define tmctl_rate_size_t   uint64_t
#else
#define tmctl_rate_size_t   uint32_t
#endif /* KERNEL_64 */
...
typedef struct {
    ...
    tmctl_rate_size_t cir;                /**< Committed Information Rate (CIR) - bps (100K-1G/10G) */
    uint32_t cbs;                         /**< Committed Burst Size (CBS) - bytes (1K-100M) */
    uint32_t ebs;                         /**< Excess Burstsize (EBS) - bytes, used for srTCM */
    tmctl_rate_size_t pir;                /**< PEAK Information Rate (PIR) - bps (100K-1G/10G) */
