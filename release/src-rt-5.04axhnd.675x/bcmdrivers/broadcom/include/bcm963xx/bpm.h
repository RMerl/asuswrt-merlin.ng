...
static __inline__ int bpm_max_buf_pool_ix(void)
{
    int pktBufSize = bcm_pktbuf_size();
    return ((BPM_MAX_MEMSIZE * BPM_MAX_MEM_POOL_IX)/pktBufSize);
}
#else
...
#else
#if (defined(CONFIG_BCM94912) || defined(CONFIG_BCM96855)) && defined(CONFIG_BCM_JUMBO_FRAME)
static __inline__ int bpm_buf_size(void)
{
    return bcm_pktbuf_size();
}
#else
...
#if (defined(CONFIG_BCM94912) || defined(CONFIG_BCM96855)) && defined(CONFIG_BCM_JUMBO_FRAME)
static __inline__ int bpm_buf_to_end_offset(void)
{
    return BCM_DCACHE_ALIGN(bcm_max_pkt_len() + BCM_SKB_TAILROOM);
}
#else
...
{
    return ( (sk_buff_data_t) (BCM_PKT_HEADROOM + bpm_buf_to_end_offset()) );
}
#else
...
#if (defined(CONFIG_BCM94912) || defined(CONFIG_BCM96855)) && defined(CONFIG_BCM_JUMBO_FRAME)
static __inline__ struct skb_shared_info *bpm_buf_to_pshinfo(uint8_t* buf)
{
    return (struct skb_shared_info *)( (uint8_t*)(buf) + bpm_buf_to_end_offset());
}
#else
