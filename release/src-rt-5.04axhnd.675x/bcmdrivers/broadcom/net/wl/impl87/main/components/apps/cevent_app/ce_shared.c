...
#ifdef BCMDBG
#define CE_EVENT_UTILS_DBG(fmt, arg...) printf("%s: "fmt, __FUNCTION__ , ## arg)
#else
#define CE_EVENT_UTILS_DBG(fmt, arg...)
#endif /* BCMDBG */
...
#ifndef EAPD_WKSP_CEVENT_UDP_SPORT
#define EAPD_WKSP_CEVENT_UDP_SPORT	(52000 + (1 << 5))
#endif

/* send given 'data' of 'data_len' to the loopback interface over the mentioned UDP 'port' */
int
ce_send_to_port(uint32 port, unsigned char *data, int data_len)
{
	int reuse = 1;
	...
	if (sock < 0) {
		CE_EVENT_UTILS_DBG("Error: UDP Open failed.\n");
		return -1;
	}

	if (setsockopt(sock, SOL_SOCKET, SO_REUSEADDR, (char*)&reuse,
		sizeof(reuse)) < 0) {
		CE_EVENT_UTILS_DBG("Error: UDP setsockopt failed.\n");
		close(sock);
		return -1;
	}
	...
	if (bind(sock, (struct sockaddr *)&addr, sizeof(addr)) < 0) {
		CE_EVENT_UTILS_DBG("Error: UDP Bind failed, close eventd appSocket %d\n", sock);
		close(sock);
		return -1;
	}
	...
	if (sentBytes != data_len) {
		CE_EVENT_UTILS_DBG("Error: UDP send failed; sentBytes = %d\n", sentBytes);
	}
 ...
 */
void
ce_send_event_to_port(uint32 port, char *ifname,
		...
		void *ev_payload, uint32 ev_payload_len)
{
	...
	if ((!ev_payload) != (!ev_payload_len)) {
		CE_EVENT_UTILS_DBG("Error: Mismatching ev_payload ptr, ev_payload_len %d.\n",
				ev_payload_len);
		return;
	}
	...
	if (!(buf = (unsigned char *) calloc(buf_len, 1))) {
		CE_EVENT_UTILS_DBG("Error: memory allocation for bcm_event_t failed\n");
		return;
	}
	be = (bcm_event_t *) buf;
	ev = &be->event;
	...
	ev->version = htons(BCM_EVENT_MSG_VERSION);
	ev->flags = 0;
	ev->event_type = htonl(ev_type);
	ev->status = htonl(status);
	ev->reason = htonl(reason);
	ev->auth_type = htonl(auth_type);
	ev->datalen = htonl(ev_payload_len);
	if (sta_ea) {
	...
	be->bcm_hdr.subtype = htons(BCMILCP_SUBTYPE_VENDOR_LONG);
	/* Incorrect assignement of bcm_hdr.length! Picked it from driver code */
	be->bcm_hdr.length = htons(BCMILCP_BCM_SUBTYPEHDR_MINLENGTH + BCM_MSG_LEN);
	be->bcm_hdr.version = htons(BCMILCP_BCM_SUBTYPEHDR_VERSION);
	...
	be->eth.ether_type = htons(ETHER_TYPE_BRCM);

	ce_send_to_port(port, buf, buf_len);
 ...
 */
void
ce_send_cevent(char *ifname, struct ether_addr *lan_ea, struct ether_addr *sta_ea,
		...
		size_t ce_data_len)
{
	...
	if ((!ce_data) != (!ce_data_len)) {
		CE_EVENT_UTILS_DBG("Error: Mismatching ce_data ptr, ce_data_len %d.\n",
				ce_data_len);
		return;
	}
	...
	if (!(buf = (unsigned char *) calloc(buf_len, 1))) {
		CE_EVENT_UTILS_DBG("Error: memory allocation for wl_cevent_t failed\n");
		return;
	}
	ce = (wl_cevent_t *) buf;
	...
	ce_send_event_to_port(EAPD_WKSP_CEVENT_UDP_SPORT, ifname, lan_ea, sta_ea,
