...
/* use macro when title, header, spaces & newline must be avoided. Otherwise use ca_hexdump_ascii */
#define CA_HEXDUMP_FPRINTF(fd, buf, len) do { \
	unsigned int i; \
	for (i = 0; i < (len); ++i) \
		fprintf(fd, "%02x", ((uint8*) (buf))[i]); \
} while (0)

#define CA_HEXDUMP_SNPRINTF(to, to_len, from, from_len) do { \
	unsigned int i; \
	for (i = 0; (to_len) > 3 && i < (from_len) && (i*2) < ((to_len) - 3); ++i) \
		ca_snprintf(((char *)(to)) + (i*2), 3, "%02x", ((uint8*)(from))[i]); \
	((char *)(to))[(to_len) - 1] = '\0'; \
} while (0)
