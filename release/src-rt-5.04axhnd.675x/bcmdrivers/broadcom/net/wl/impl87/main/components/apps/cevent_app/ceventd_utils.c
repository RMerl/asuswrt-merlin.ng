...
#define CA_LOG_CE(cwksp, ca_ts, ifname, sta, flags, status, reason, auth, at, dir, hd, ce) do { \
	char *yhd = ((NULL != (void *)(hd)) && (hd)[0]) ? (hd) : \
			(CEVENT_MT2STR((ce)->type, (ce)->msgtype)); \
	char xhd[BUFSZ] = {0}, ce_data_hex[BUFSZ] = {0}; \
	uint16 ce_data_len = CEVENT_DATA_LEN(ce); \
	snprintf(xhd, sizeof(xhd), "%s%s", yhd, \
			CEVENT_D2C_D2CFLSTR((ce)->type, (ce)->msgtype, (ce)->flags)); \
	CA_HEXDUMP_SNPRINTF(ce_data_hex, sizeof(ce_data_hex), CEVENT_DATA(ce), \
			MIN(ce_data_len, sizeof(ce_data_hex)/2)); \
	if (cwksp->log_type == CA_LOG_TYPE_SSV) { \
		CA_LOG_EXT(cwksp, (ce)->timestamp, ifname, sta, flags, status, \
			reason, auth, at, dir, xhd, \
			"[CE: %2u %3u %3u/0x%04X/%-8s %2u " \
			"%3u/0x%04X/%-8s 0x%04X %08llu] {CE_DATA-%04uBytes:%s%s}", \
			(ce)->version, (ce)->length, (ce)->type, (ce)->type, \
			(CEVENT_TYPE2STR((ce)->type)), \
			(ce)->data_offset, (ce)->msgtype, (ce)->msgtype, \
			(CEVENT_MT2STR((ce)->type, (ce)->msgtype)), \
			(ce)->flags, (ce)->timestamp, ce_data_len, ce_data_hex, \
			(ce_data_len > ((BUFSZ)/2 - 1) ? "..." : "")); \
	} else if (cwksp->log_type == CA_LOG_TYPE_CSV) { \
		CA_LOG_EXT(cwksp, (ce)->timestamp, ifname, sta, flags, status, \
			reason, auth, at, dir, xhd, \
			"[CE: %2u, %3u, %3u/0x%04X/%-8s, %2u, " \
...
#define CEVENT_D2C_D2CFLSTR(T, MT, FL)	(((T) != CEVENT_TYPE_D2C || (MT) >= CEVENT_D2C_MT_TX_LAST) \
		? "" : CEVENT_D2C_TX_FLAG_STR((FL) & 0xFFu))

static char *cevent_a2c_mt_str[] = {
	"PTK_INSTALL",
	"GTK_INSTALL",
	"M1_RX",
	"M1_TX",
...
#define CEVENT_A2C_MT2STR(MT)	(((MT) >= 0 && (MT) < CEVENT_A2C_MT_LAST) ?	\
				cevent_a2c_mt_str[MT] : "--")

#define CEVENT_D2C_MT2STR(MT)	(((MT) >= 0 && (MT) < CEVENT_D2C_MT_LAST) ? \
				cevent_d2c_mt_str[MT] : "--")

#define CEVENT_MT2STR(T, MT) (((T) == CEVENT_TYPE_D2C) ? CEVENT_D2C_MT2STR(MT) : \
				(((T) == CEVENT_TYPE_A2C) ? CEVENT_A2C_MT2STR(MT) :	\
				("--")))
static char *cevent_subtype_str[] = {
	"NAS",
	"WPS",
	"WAI",
	"DCS",
...
#define CEVENT_ST2STR(ST)	(((ST) >= 0 && (ST) < CEVENT_ST_LAST) ?	\
				cevent_subtype_str[ST] : "--")
...
/* BTM response reason string */
char *ca_btm_resp_str[] = {
	"BTM_RESP_ACCEPT",		/* BTM respone accept */
	"BTM_RESP_REJECT",		/* BTM response reason reject unspecifed */
	"BTM_RESP_REJ_INSUFF_BCN",	/* BTM response reason reject insuff beacon/probe resp */
	"BTM_RESP_REJ_INSUFF_CAP",	/* BTM response reason reject insuff capacity from all */
	"BTM_RESP_REJ_BSS_TERM_UNDES",	/* BTM response reason reject BSS termination undesired */
	"BTM_RESP_REJ_BSS_TERM_DELAY",  /* BTM response reason reject BSS termination delay req */
	"BTM_RESP_REJ_STA_BSS_LIST",	/* BTM response reason reject STA BSS list provided */
	"BTM_RESP_REJ_NO_SUITABLE_BSS",	/* BTM response reason reject no suitable BSS trans list */
	"BTM_RESP_REJ_LEAVING_ESS",	/* BTM response reason reject leaving ESS */
};

static int ca_identify_a2d_d2a(ca_wksp_t *cwksp, wl_cevent_t  *ce, char *pheader, size_t hsz,
		char *pat, size_t asz);
static int ca_identify_a2c(ca_wksp_t *cwksp, wl_cevent_t *ce, uint32 reason, char *pheader,
		size_t hsz);
static int ca_identify_d2c_reason(ca_wksp_t *cwksp, wl_cevent_t *ce, uint32 reason, char *pheader,
	size_t hsz);
...
static int
ca_identify_a2c(ca_wksp_t *cwksp, wl_cevent_t *ce, uint32 reason, char *pheader, size_t hsz)
{
	CA_DBG("Checking for a2c by ce msgtype %u\n", ce->msgtype);
	switch (ce->msgtype)  {
		case CEVENT_A2C_MT_PTK_INSTALL:
			CA_MSG("PTK installation\n");
			snprintf(pheader, hsz, "PTK_INSTALL");
			return BCME_OK;
		case CEVENT_A2C_MT_GTK_INSTALL:
			CA_MSG("GTK installation\n");
			snprintf(pheader, hsz, "GTK_INSTALL");
			return BCME_OK;
		case CEVENT_A2C_MT_ACS_CH_SW:
			{
				char ch_str[CHANSPEC_STR_LEN] = {0};
				const char *acs_trig[] = {
					"INIT", "IOCTL", "CHANIM", "TIMER",
					"CISCAN", "TXOP", "NONACS", "DFS-REENTRY", "TXFAIL", "ZDFS"
				};
				if (CEVENT_DATA_LEN(ce) >= sizeof(chanspec_t)) {
					wf_chspec_ntoa(*((chanspec_t*)CEVENT_DATA(ce)), ch_str);
				}
				CA_MSG("Channel_Switch_Reason\n");
				ca_snprintf(pheader, hsz, "ACS_%s(%s)",
					((reason < ARRAY_SIZE(acs_trig)) ? acs_trig[reason] :
					 "UNKNOWN"), ch_str);
				return BCME_OK;
			}
		default:
			CA_MSG("Default. ce msgtype: %u\n", ce->msgtype);
			return BCME_UNSUPPORTED;
	}
}

static int
ca_identify_d2c_reason(ca_wksp_t *cwksp, wl_cevent_t *ce, uint32 reason, char *pheader, size_t hsz)
{
	CA_DBG("Checking for d2c by ce msgtype %u\n", ce->msgtype);
	switch (ce->msgtype)  {
		case CEVENT_D2C_MT_BTM_REQ:
			CA_MSG("ce msgtype: %u\n", ce->msgtype);
			break;
		case CEVENT_D2C_MT_BTM_RESP:
			if (reason < CEVENT_BTM_RESP_REASON_LAST) {
				CA_DBG("%s\n", ca_btm_resp_str[reason]);
...
ca_process_eapd_pkt(ca_wksp_t *cwksp, int bytes, uint64 ca_ts)
{
	...
	snprintf(at, asz, CEVENT_ST2STR(ce->subtype));
	switch (ce->type) {
		...
		case CEVENT_TYPE_A2C:
			header[0] = '\0';
			ca_identify_a2c(cwksp, ce, reason, header, hsz);
			break;
		case CEVENT_TYPE_D2C:
			ret = ca_identify_d2c_reason(cwksp, ce, reason, header, hsz);
			if (ret != BCME_OK) {
				header[0] = '\0';
			}
			break;
