...
print_TWTsessions(wifi_twt_sessions_t *ts, unsigned int numSess)
{
	...
	for (j = 0; j < numSess; j++, tsp++) {
		...
		printf("\nID    Interval (usec)  Duration (usec)  Unannounced  Trigger  Implicit"
			"     Agreement\n");
		tp = &(tsp->twtParameters);
		for (i = 0; i < tsp->numDevicesInSession; i++, tp++) {
			wakeInt = 0;
			wakeDur = 0;
			if (tp->agreement == wifi_twt_agreement_type_individual) {
				tpi = &(tp->params.individual);
...
print_countryCode(wifi_countrycode_type_t countrycode)
{
	...
	for (i = 0; countrycode_table[i].str_val != NULL; ++i) {
		if (countrycode == countrycode_table[i].enum_val)
...
(0/1)>" },
	...
	{ "wifi_csitest",				2, "<AP Index> <Client MACs>" },
...
INT wifi_RMBeaconReport_callback_test_func(UINT apIndex, wifi_BeaconReport_t *rm_bcnRep,
	UINT *out_array_size, UCHAR *out_DialogToken)
{
	UCHAR *ea;
	...
	ea = rm_bcnRep->bssid;
	printf("<802.11K Rx Rep> %s: opClass: %d channel: %d, dur: %d, frame info: %d, rcpi: %d,"
		"rsni: %d, bssid: %02x:%02x:%02x:%02x:%02x:%02x, antenna id: %d, parent tsf: %u\n",
		__FUNCTION__, rm_bcnRep->opClass, rm_bcnRep->channel,
		rm_bcnRep->duration, rm_bcnRep->frameInfo, rm_bcnRep->rcpi, rm_bcnRep->rsni,
		ea[0], ea[1], ea[2], ea[3], ea[4], ea[5],
		rm_bcnRep->antenna, rm_bcnRep->tsf);
	printf("\n---------------\n");
	return 0;
}
/* 802.11K api */
...
#define NUM_CH_ALL (NUM_CH_24 + NUM_CH_5) /* 1-165 */
		...
		if (argc > 3) {
			associated_dev_array =
				(wifi_associated_dev3_t *)malloc(sizeof(wifi_associated_dev3_t) * MAX_CSI_STA);
			printf("alloc: %p\n", associated_dev_array);
			...
			memset(associated_dev_array, 0, sizeof(wifi_associated_dev3_t) * MAX_CSI_STA);

			/*filter STA mac*/
			for (array_size = 0; (array_size + 3) < argc && (array_size < MAX_CSI_STA); array_size++) {
				parse_mac(argv[array_size + 3], &(associated_dev_array[array_size].cli_MACAddress));
				printf("Mac: 0x%02x\n", associated_dev_array[array_size].cli_MACAddress[0]);
			}

			printf("Total STA:%d for CSI data\n", array_size);
			for (i = 0, pt = associated_dev_array; i < array_size; i++, pt++) {
				printf("sta_%d: "MACF"\n", i, MAC_TO_MACF(pt->cli_MACAddress));
			}
		}
#endif /* WIFI_HAL_VERSION_GE_2_19 */
		...
		if (ret == RETURN_OK) {
			printf("Total_STA:%ld\n", array_size);
			for (i = 0, pt = associated_dev_array; i < array_size; i++, pt++) {
				printf("sta_%d: wifi_devMacAddress			:"MACF"\n",
	...
	to avoid RDKM compiling errors */
		...
		if (ret == RETURN_OK) {
			...
			*ptr = (unsigned short)atoi(argv[3]);
			printf("wifi_setFTMobilityDomainID(apIndex %d, 0x%x 0x%x 0x%x)\n",
				index, fbt_cfg[0], fbt_cfg[1], *ptr);
		}
	...
	/* 802.11r FBT HAL test ends */
		...
		* wifi_steering_eventRegister */
		...
		wifi_steering_apConfig_t cfg_array[MAX_NUM_RADIOS];
...
/* end of DPP */
	...
	(!defined(_CBR_PRODUCT_REQ_) && !defined(_XF3_PRODUCT_REQ_)) */
		...
		if (ret == RETURN_OK) {
			printf("ApAssociatedDevicesHighWatermarkThresholdReachedDate %d for apIndex=%d\n", hwm_value, index);
		}
#if WIFI_HAL_VERSION_GE_2_15 && (!defined(_CBR_PRODUCT_REQ_) && \
	!defined(_XF3_PRODUCT_REQ_))
	...
	(!defined(_CBR_PRODUCT_REQ_) && !defined(_XF3_PRODUCT_REQ_)) */
		...
		wifi_csi_data_t *csi_buf = NULL, *csi_data;
		wifi_frame_info_t *frame_info;
		wifi_csi_matrix_t *csi_matrix;

		/* set csi */
		printf("Enable CSI\n");
		for (n = 0; n < MAX_CSI_STA && (n + 3) < argc; n++) {
			parse_mac(argv[n + 3], &mac[n]);
		...
		/* to allow csi data collection from driver */
		sleep(2);

		cnt = n;
		assoc_buf = (wifi_associated_dev3_t *)malloc(sizeof(wifi_associated_dev3_t) * MAX_CSI_STA);
		printf("assoc_buf: 0x%p for STA:%d\n", assoc_buf, array_size);

		if (assoc_buf == NULL) {
			printf("%s Malloc assoc_buf Failure[size:%d]\n",
				__FUNCTION__, sizeof(wifi_associated_dev3_t) * MAX_CSI_STA );
			ret = -1;
			goto exit;
		}

		csi_buf = (wifi_csi_data_t *)malloc(sizeof(wifi_csi_data_t) * MAX_CSI_STA);
		if (csi_buf == NULL) {
			printf("%s Malloc csi_buf Failure[size: %d]\n",
				__FUNCTION__, sizeof(wifi_csi_data_t) * MAX_CSI_STA);
			ret = -2;
			goto exit;
		}
		...
		while (++loop < 10) {
			array_size = cnt;
			memset(assoc_buf, 0, sizeof(wifi_associated_dev3_t) * MAX_CSI_STA);
			memset(csi_buf, 0, sizeof(wifi_csi_data_t) * MAX_CSI_STA);

			for (n = 0; n < array_size; n++) {
				memcpy(assoc_buf[n].cli_MACAddress, mac[n], ETHER_ADDR_LEN);
				assoc_buf[n].cli_CsiData = csi_buf + n;
				printf("csi_data[%d]: size:%d loc:0x%p\n",
					n, sizeof(wifi_csi_data_t), (char *)assoc_buf[n].cli_CsiData);

				ptr = assoc_buf + n;
				csi_data = ptr->cli_CsiData;
				frame_info = &(csi_data->frame_info);
				csi_matrix = &(csi_data->csi_matrix);
			}

			printf("[%d] reading STA's record:%d\n", loop, array_size);
			ret = wifi_getApAssociatedDeviceDiagnosticResult3(index, &assoc_buf, &array_size);

			if (ret != RETURN_OK) {
				printf("%s:Read CSI data failure\n", __FUNCTION__);
				break;
			}

			printf("------print csi rec--------\n");
			for (n = 0, ptr = assoc_buf; n < array_size; n++, ptr++) {
				printf("[%d] "MACF"\n", n, MAC_TO_MACF(ptr->cli_MACAddress));
				csi_data = ptr->cli_CsiData;
				frame_info = &(csi_data->frame_info);
				csi_matrix = &(csi_data->csi_matrix);
				...
				printf("Nr:%d\n", frame_info->Nr);
				printf("Nc:%d\n", frame_info->Nc);
				printf("nr_rssi:%02x:%02x:%02x:%02x\n", frame_info->nr_rssi[0],
					frame_info->nr_rssi[1], frame_info->nr_rssi[2], frame_info->nr_rssi[3]);
				printf("valid_mask:0x%x\n", frame_info->valid_mask);
				printf("phy_bw:%d\n", frame_info->phy_bw);
				printf("cap_bw:%d\n", frame_info->cap_bw);
				printf("num_sc:%d\n", frame_info->num_sc);
				printf("decimation:%d\n", frame_info->decimation);
				printf("channel:%d\n", frame_info->channel);
				printf("time_stamp:0x%llx\n", frame_info->time_stamp);

				printf("CSI matrix:%p\n", raw_data);
				for (i = 0; i < 4; i++) {
					for (j = 0; j < 8; j++) {
						printf("0x%08x\t", raw_data[i * 8 + j]);
					}
					printf("\n");
				}

				printf("sta Info:\n");
				printf("cli_MACAddress = "MACF"\n", MAC_TO_MACF(ptr->cli_MACAddress));
				printf("cli_AuthenticationState = %d\n", ptr->cli_AuthenticationState);
				printf("cli_LastDataDownlinkRate = %d\n", ptr->cli_LastDataDownlinkRate);
				printf("cli_LastDataUplinkRate = %d\n", ptr->cli_LastDataUplinkRate);
				printf("cli_SignalStrength = %d\n", ptr->cli_SignalStrength);
				printf("cli_Retransmissions = %d\n", ptr->cli_Retransmissions);
				printf("cli_OperatingStandard = %s\n", ptr->cli_OperatingStandard);
				printf("cli_OperatingChannelBandwidth = %s\n", ptr->cli_OperatingChannelBandwidth);
				printf("cli_SNR = %d\n", ptr->cli_SNR);
				printf("cli_DataFramesSentAck = %lu\n", ptr->cli_DataFramesSentAck);
				printf("cli_DataFramesSentNoAck = %lu\n", ptr->cli_DataFramesSentNoAck);
				printf("cli_BytesSent = %lu\n", ptr->cli_BytesSent);
				printf("cli_BytesReceived = %lu\n", ptr->cli_BytesReceived);
				printf("cli_RSSI = %d\n", ptr->cli_RSSI);
				printf("cli_Disassociations = %d\n", ptr->cli_Disassociations);
				printf("cli_AuthenticationFailures = %d\n", ptr->cli_AuthenticationFailures);
				printf("cli_Associations = %llu\n", ptr->cli_Associations);
				printf("cli_PacketsSent = %lu\n", ptr->cli_PacketsSent);
				printf("cli_PacketsReceived = %lu\n", ptr->cli_PacketsReceived);
				printf("cli_ErrorsSent = %lu\n",  ptr->cli_ErrorsSent);
				printf("cli_RetransCount = %lu\n", ptr->cli_RetransCount);
				printf("cli_FailedRetransCount = %lu\n", ptr->cli_FailedRetransCount);
				printf("cli_RetryCount = %lu\n", ptr->cli_RetryCount);
				printf("cli_MultipleRetryCount = %lu\n", ptr->cli_MultipleRetryCount);
				printf("cli_MaxDownlinkRate = %d\n", ptr->cli_MaxDownlinkRate);
				printf("cli_MaxUplinkRate = %d\n", ptr->cli_MaxUplinkRate);
				printf("\n");
			}
			sleep(1);
		}
exit:
		...
		printf("\n   numRadios: %d", cap.wifi_prop.numRadios);
...
print_setApWpsConfiguration_usage:
		...
		if (ret == RETURN_OK) {
			...
			for (int i = 0; i < operationParam.numSecondaryChannels; i++) {
...
print_setApSecurity_usage:
		...
		if (ret == WIFI_HAL_SUCCESS) {
			for (i = 0; i < map.num_vaps; i++) {
				...
				wifi_InterworkingElement_t *output_struct;
				output_struct = &(interworking->interworking);
