...
#define WIFI_READY_IND_FILE	"/tmp/.brcm_wifi_ready"
...
do { \
	if ((radioIndex >= (HAL_GET_MAX_RADIOS)) || (radioIndex < 0)) { \
		HAL_WIFI_ERR(("%s: INCORRECT radioIndex = %d numRadios = %d\n", \
			__FUNCTION__, radioIndex, HAL_GET_MAX_RADIOS)); \
		return retcode; \
	} \
} while (0)
...
extern int wlcsm_mngr_update_stalist(unsigned int radio_idx, unsigned int sub_idx, void *value,
	int len);
extern int wlcsm_mngr_wifi_hal_started(unsigned int radio_idx,unsigned int source, int wait);
...
static void HAL_DBG_PRINT_BUF(const char *astr, unsigned char *aptr, int len)
{
	int i,j;
	unsigned char *ap=aptr;
	i = j = 0;
	printf("HAL: %s \n", astr);
	for (i=0; i<len; i++) {
		printf("%02x ", *ap);
...
{
	INT radioIdx;
	for (radioIdx = 0; radioIdx < HAL_RADIO_NUM_RADIOS; radioIdx ++) {
...
INT wifi_factoryResetRadios()
{
	...
	for (iIter = 0; iIter < HAL_RADIO_NUM_RADIOS; iIter++) {
		if ((iRet = wifi_factoryResetRadio(iIter)) != RETURN_OK) {
			//resetting both Radios to Factory defaults
			HAL_WIFI_DBG(("%s: failed wifi_factoryResetRadio for radio %d\n", __FUNCTION__, iIter));
			return iRet;
		}
	}
...
#endif
	return wldm_xbrcm_factory_reset(WLDM_NVRAM_FACTORY_RESET_RADIO, radioIndex, 0, 0);
}
...
void* wifi_lanBridgeConfigNvram_priv(void * data)
{
	...
	do {
		...
		if ((file = fopen("/sys/class/net/brlan0/flags", "r"))) {
	...
	wlcsm_mngr_wifi_hal_started(0, WLCSM_MNGR_CMD_WIFI_HAL_STARTED, 1);
	...
	while (1) {
		...
		for (radioIndex=0 ; radioIndex<HAL_RADIO_NUM_RADIOS; radioIndex++) {
...
INT wifi_init()		//RDKB
{
	...
	for (apIndex = 0; apIndex < HAL_AP_NUM_APS_PER_RADIO * HAL_RADIO_NUM_RADIOS; apIndex++) {
	...
	if (access(WIFI_READY_IND_FILE, F_OK) != 0) {
		if (system("wifi_setup.sh start")) {
			HAL_WIFI_DBG(("%s: system wifi_setup.sh start failed\n", __FUNCTION__));
		}
		HAL_WIFI_LOG(("%s: Start ACSD!\n", __FUNCTION__));
		restart_acsd();
	}
#endif /* WIFI_HAL_VERSION_3 */
	if (!wifi_apiThread) {
...
INT wifi_down()
{
	...
	for (radioCount = 0; radioCount < HAL_RADIO_NUM_RADIOS; radioCount++) {
...
INT wifi_getApATMSta(INT apIndex, UCHAR *output_sta_MAC_ATM_array, UINT  buf_size)
{
	...
	for (i = 0; i < num_stas; i++) {
		strncpy(wldm_staperc.macstr, stas[i], sizeof(wldm_staperc.macstr));
...
INT wifi_setApATMSta(INT apIndex, UCHAR *sta_MAC, UINT sta_AirTimePercent)
{
	...
	strncpy(wldm_staperc.macstr, sta_MAC, sizeof(wldm_staperc.macstr));
...
INT wl_setRadioMode(INT radioIndex, CHAR *channelMode, UINT pureMode)
{
	...
	int		ret, len;
	CHAR		nvValue[32] = {0};
	CHAR		chanspecName[32] = {0};
	PCHAR		pValue = NULL;
	int		radioAutoChannelEnable = FALSE;
...
INT wifi_setRadioMode(INT radioIndex, CHAR *channelMode, UINT pureMode)	//RDKB
{
	...
	if (NULL == channelMode)
	{
		HAL_WIFI_ERR(("%s: channelMode parameter error!!!\n", __FUNCTION__));
		return RETURN_ERR;
	}
	if ((radioIndex >= HAL_RADIO_NUM_RADIOS) || (radioIndex < 0)) {
		HAL_WIFI_ERR(("%s: INCORRECT radioIndex [%d] \n", __FUNCTION__, radioIndex));
...
INT wifi_getRadioOperatingChannelBandwidth(INT radioIndex, CHAR *output_string) //Tr181
{
	...
	len = OUTPUT_STRING_LENGTH_64 * sizeof(char);
	ret = wldm_Radio_OperatingChannelBandwidth(CMD_GET_NVRAM, radioIndex, output_string, &len, NULL, NULL);
	if (ret < 0) {
...
/*
 * libray function support "wl rateset"
 */
extern int wlcsm_get_current_rateset(char *interface, char* request, char *output);
...
INT wifi_getRadioSupportedDataTransmitRates(INT radioIndex, CHAR *output)
{
	...
	CHAR interface[OUTPUT_STRING_LENGTH_64] = {0};
	...
	int len = sizeof(supportedDataTransmitRates);
...
INT wifi_getRadioOperationalDataTransmitRates(INT radioIndex, CHAR *output)
{
	...
	if (NULL == output) {
		HAL_WIFI_ERR(("%s: output is null \n", __FUNCTION__));
		return RETURN_ERR;
	}
	int len = sizeof(operationalDataTransmitRates);
...
INT wifi_getRadioBasicDataTransmitRates(INT radioIndex, CHAR *output)
{
	...
	CHAR interface[OUTPUT_STRING_LENGTH_32] = {0};

	HAL_WIFI_DBG(("%s: radioIndex %d\n", __FUNCTION__, radioIndex));
	RADIO_INDEX_ASSERT(radioIndex);
	NULL_PTR_ASSERT(output);

	int len = sizeof(basicDataTransmitRates);
...
/* Per requirement on wifi_setApBeaconRate, user shall only select on of the following rate as beacon rate
"1Mbps"; "5.5Mbps"; "6Mbps"; "2Mbps"; "11Mbps"; "12Mbps"; "24Mbps"
*/
static BOOL isValidBeaconRate(char* rate)
{
	char* valideBeaconRateList[] = {"1", "2", "5.5", "6", "11", "12", "24" };
	UINT32 valideBeaconRateLen = sizeof(valideBeaconRateList) / sizeof(valideBeaconRateList[0]);
	UINT32 i;

	for (i = 0; i < valideBeaconRateLen; i++) {
		if (!strcmp(rate, valideBeaconRateList[i]))
			return TRUE;
	}
...
static BOOL isOfdmRate(char* rate)
{
	char* ofdmRateList[] = {"6", "9", "12", "18", "24", "36", "48", "54"};
	UINT32 ofdmRateListLen = sizeof(ofdmRateList) / sizeof(ofdmRateList[0]);
	UINT32 i;

	for (i = 0; i < ofdmRateListLen; i++ ) {
		if (!strcmp(rate, ofdmRateList[i]))
			return TRUE;
	}
...
static BOOL isValidRateInRateset(char* rate)
{
	char* validRateSetList[] = {"1", "2", "5.5", "6", "9", "11", "12", "18", "24", "36", "48", "54" };
	UINT32 len = sizeof(validRateSetList) / sizeof(validRateSetList[0]);
	UINT32 i;

	if (!rate) {
		HAL_WIFI_DBG(("%s: input rate is null.\n", __FUNCTION__));
		return FALSE;
	}

	for (i = 0; i < len; i++) {
		if (!strcmp(rate, validRateSetList[i])) {
			return TRUE;
		}
	}
...
INT wifi_getNeighboringWiFiDiagnosticResult2(INT radioIndex, BOOL scan, wifi_neighbor_ap2_t **neighbor_ap_array, UINT *output_array_size)
{
	...
	RADIO_INDEX_ASSERT(radioIndex);
	if (scan) {
		if (start_neighbor_scan(radioIndex, WIFI_RADIO_SCAN_MODE_FULL, 20, 0, NULL) != RETURN_OK) {
...
INT wifi_factoryResetAP(int apIndex)
{
	...
	snprintf(cmd, sizeof(cmd), "echo wl%d_ssid=$(awk -F \": \" '/%s:/ {print $2}' %s) > %s", apIndex, (apIndex == 0) ? "Default 2.4 GHz SSID" : "Default 5.0 GHz SSID", FACTORY_NVRAM_DATA, NVRAM_FACTORY_DEFAULT_TMP);
	if (system(cmd)) {
		HAL_WIFI_ERR(("%s: Default SSID not found\n", __FUNCTION__));
	}
	snprintf(cmd, sizeof(cmd), "echo wl%d_wpa_psk=$(awk -F \": \" '/%s:/ {print $2}' %s) >> %s", apIndex, "Default WIFI Password", FACTORY_NVRAM_DATA, NVRAM_FACTORY_DEFAULT_TMP);
	if (system(cmd)) {
		HAL_WIFI_ERR(("%s: Default password not found\n", __FUNCTION__));
	}

	snprintf(cmd, sizeof(cmd), "echo wl%d_wps_device_pin=$(awk -F \": \" '/%s:/ {print $2}' %s) >> %s", apIndex, "Default WPS Pin", FACTORY_NVRAM_DATA, NVRAM_FACTORY_DEFAULT_TMP);
	if (system(cmd)) {
...
INT wifi_addApAclDevice(INT apIndex, mac_address_t DeviceMacAddress)
{
	...
	if (do_apply && (wldm_apply_AccessPointObject(apIndex) != 0)) {
		HAL_WIFI_ERR(("%s: apply_AccessPointObject Failed apIndex [%d]\n", __FUNCTION__, apIndex));
		return RETURN_ERR;
	}
...
INT wifi_kickApAclAssociatedDevices(INT apIndex, BOOL enable)
{
	...
	while (devMac != NULL) {
		...
		*/
		if (bMacInACL == enable) {
			len = strlen(devMac);
			iResult =  wldm_AccessPoint_kickAssociatedDevice(CMD_DEL, apIndex, devMac, &len, NULL, NULL);
			if (iResult != RETURN_OK) {
...
INT wifi_setApSecurityKeyPassphrase(INT apIndex, CHAR *passPhrase)
{
	...
	if (do_apply && (wldm_apply_AccessPointSecurityObject(apIndex) != 0)) {
		HAL_WIFI_ERR(("%s: apply_AccessPointObject Failed apIndex [%d]\n", __FUNCTION__, apIndex));
		return RETURN_ERR;
	}
#ifdef RDKB_TIMER
...
static INT get_assoc_dev_max_tx_rx_rate(INT apIndex, wifi_associated_dev3_t *dev)
{
	char guard[OUTPUT_STRING_LENGTH_64], *interface,
		cmd[OUTPUT_STRING_LENGTH_256] = {0}, buff[OUTPUT_STRING_LENGTH_256] = {0};
...
{
	if (enable) {
		wifi_setLED_priv(radioIndex,WIFI_LED_ON_CONTINUOUS);
	} else {
		wifi_setLED_priv(radioIndex,WIFI_LED_OFF_CONTINUOUS);
	}
}
...
INT wifi_getRadioDCSScanTime(INT radioIndex, INT *output_interval_seconds, INT *output_dwell_milliseconds)
{
	...
	CHAR cmdBuf[OUTPUT_STRING_LENGTH_128] = {'\0'};
	CHAR outBuf[OUTPUT_STRING_LENGTH_128] = {'\0'};
...
{
	CHAR nv_ifname[OUTPUT_STRING_LENGTH_32], nv_name[OUTPUT_STRING_LENGTH_128];
	CHAR *p_nv_value = NULL;
...
{
	//store the paramters, and apply instantly
	CHAR nv_ifname[OUTPUT_STRING_LENGTH_64], nv_name[OUTPUT_STRING_LENGTH_128], nv_value[OUTPUT_STRING_LENGTH_128];
...
INT wifi_getApAuthenticatedDevices(INT ap_index, mac_address_t *mac_array, UINT maxNumDevices, UINT *output_numEntries)
{
	...
	if (wldm_xbrcm_ap(CMD_GET, ap_index, mac_str_array, &len, "authe_sta_list", NULL)
		!= 0) {
		HAL_WIFI_ERR(("%s: wldm_xbrcm_ap - authe_sta_list ERROR\n", __FUNCTION__));
		free(mac_str_array);
		return WIFI_HAL_ERROR;
	}
	mac_str = strtok_r(mac_str_array, ",", &rest);
	for (i = 0; (i < maxNumDevices) && (mac_str != NULL); ++i) {
		sscanf(mac_str, MACF, MAC_TO_MACF(&mac_array[i]));
		mac_str = strtok_r(NULL, ",", &rest);
	}
	*output_numEntries = i;
	free(mac_str_array);
...
INT wifi_getBandSteeringLog(INT record_index, ULONG *pSteeringTime, CHAR *pClientMAC, INT *pSourceSSIDIndex, INT *pDestSSIDIndex, INT *pSteeringReason) //if no steering or redord_index is out of boundary, return -1. pSteeringTime returns the UTC time in seconds. pClientMAC is pre allocated as 64bytes. pSteeringReason returns the predefined steering trigger reason
{
	...
	*/
	...
	fp = fopen(filename, "r");
	if (fp) {
		if (9 != fscanf(fp, "%u %11s %8s %40s RDKB_LOG_STEERED MAC=%17s FROM_BAND=%d TO_BAND=%d REASON=%d %25s\n",
...
{
	int len = sizeof(*output_timout_sec), ret = RETURN_OK;
...
INT wifi_steering_setGroup(UINT steeringgroupIndex, UINT numElements, wifi_steering_apConfig_t *cfgArray)
{
	...
	if ((numElements == 0) || (numElements > MAX_NUM_RADIOS)) {
...
#endif /* WIFI_HAL_VERSION_3_PHASE2 */
	int macAddrInt[6], k, len_assoc, len_disassoc;
	char encMode[64];
	memset(encMode, 0, sizeof(encMode));
	...
	if (dpkt->version == WIFI_HAL_EVT_VERSION) {
		...
		if (event_id == WIFI_HAL_CB_CH_CHG) {
			...
			if (radar_detected &&
				...
				if (radioIndex >= HAL_RADIO_NUM_RADIOS) {
					HAL_WIFI_ERR(("%s, INCORRECT radioIndex [%d] \n",
						__FUNCTION__, radioIndex));
					return RETURN_ERR;
				}
	...
	wifi_BeaconRequest_t *in_request, UCHAR *out_DialogToken)
{
	...
	char logStr[128];
	int ret, request_len = 0, inSubElemLen = 0;
...
INT wifi_setNeighborReportActivation(UINT apIndex, BOOL activate)
{
	...
	if (wldm_xbrcm_AccessPoint_RMCapabilities(CMD_GET, apIndex, (unsigned char *)(&rrm_val),
		&len, NULL, NULL) < 0) {
 ...
 */
int wlcsm_mngr_wifi_build_btmreq_option(char *buffer, wifi_NeighborReport_t *p_report)
{
	int len = 0, elen;
	char *ptr;
	...
	wifi_BTMRequest_t *in_struct)
{
	...
	for (i = 0; i < in_struct->numCandidates; i++) {
		...
		*(buf_ptr++) = 0x34; /* ID for candidate list */
		opt_sub_len = wlcsm_mngr_wifi_build_btmreq_option(buf_ptr+1, &in_struct->candidates[i]);
		*(buf_ptr++) = opt_sub_len;
		buf_ptr += opt_sub_len;
	...
	while (1) {
		...
		if (FD_ISSET(wifi_api_socket, &fdset)) {
			...
			if (!strcmp(pApiInfo->api_name, "wifi_setApEnable")) {
				...
				CHAR channelMode[32];
				BOOL gOnlyFlag, nOnlyFlag, acOnlyFlag;
				sscanf(pApiInfo->api_data, "%s %d %d %d\n", channelMode, &gOnlyFlag, &nOnlyFlag, &acOnlyFlag);
...
{
	int ret;
	unsigned int radioIndex = (unsigned int)radio_index;

	HAL_WIFI_DBG(("%s\n", __FUNCTION__));
	RADIO_INDEX_ASSERT(radioIndex);
	HAL_WIFI_DBG(("HAL %s radioIndex=%d\n", __FUNCTION__, radioIndex));

	ret = wldm_setGuardInterval(radioIndex, (unsigned int)guard_interval);
	HAL_WIFI_DBG(("HAL %s radioIndex=%d Set guard_interval=%d ret=%d \n", __FUNCTION__, radioIndex, (unsigned int)(guard_interval), ret));

	return ret;
}

INT wifi_getGuardInterval(INT radio_index, wifi_guard_interval_t *guard_intervalp)
{
	int ret;
	unsigned int radioIndex = (unsigned int)radio_index;
	...
	ret = wldm_getGuardInterval(radioIndex, (unsigned int *)(guard_intervalp));
	HAL_WIFI_DBG(("HAL %s radioIndex=%d Got guard_interval=%d ret=%d \n", __FUNCTION__, radioIndex, (unsigned int)(*guard_intervalp), ret));
...
INT wifi_setDownlinkDataAckType(INT radio_index, wifi_dl_data_ack_type_t ack_type)
{
	...
	ret = wldm_setDownlinkDataAckType(radioIndex, (unsigned int)ack_type);
	HAL_WIFI_DBG(("HAL %s radioIndex=%d Set ack_type=%d ret=%d \n", __FUNCTION__, radioIndex, (unsigned int)(ack_type), ret));
...
INT wifi_getDownlinkDataAckType(INT radio_index, wifi_dl_data_ack_type_t *ack_type)
{
	...
	ret = wldm_getDownlinkDataAckType(radioIndex, (unsigned int *)(ack_type));
	HAL_WIFI_DBG(("HAL %s radioIndex=%d Get ack_type=%d ret=%d \n", __FUNCTION__, radioIndex, (unsigned int)(*ack_type), ret));
...
INT wifi_getRadioVapInfoMap(wifi_radio_index_t index, wifi_vap_info_map_t *map)
{
	...
	for (i = 0; i < MAX_NUM_VAP_PER_RADIO; i++) {
		...
		len = sizeof(map->vap_array[i].u.bss_info.bssTransitionActivated);
		ret = wldm_11r_ft(WLDM_FT_GET_ACTIVATED_NVRAM, apIndex,
			&(map->vap_array[i].u.bss_info.bssTransitionActivated), &len, NULL);
		if (ret < 0) {
			HAL_WIFI_ERR(("%s: wldm_11r_ft WLDM_FT_GET_ACTIVATED Failed, apIndex=[%d]\n",
				__FUNCTION__, apIndex));
			map->vap_array[i].u.bss_info.bssTransitionActivated = FALSE;
			result = WIFI_HAL_ERROR;
		}
...
INT wifi_createVAP(wifi_radio_index_t index, wifi_vap_info_map_t *map)
{
	...
	for (i = 0; i < map->num_vaps; i++) {
		...
		ret = wldm_xbrcm_ap(CMD_SET_NVRAM, apIndex,
			&(map->vap_array[i].u.bss_info.mgmtPowerControl), &len,
			"bcnprs_txpwr_offset", NULL);
		if (ret < 0) {
		...
		len = sizeof(map->vap_array[i].u.bss_info.bssTransitionActivated);
		ret = wldm_11r_ft(WLDM_FT_SET_ACTIVATED_NVRAM, apIndex,
			&(map->vap_array[i].u.bss_info.bssTransitionActivated), &len, NULL);
		if (ret < 0) {
			HAL_WIFI_ERR(("%s: wldm_11r_ft WLDM_FT_SET_ACTIVATED_NVRAM Failed,"
				" apIndex=[%d]\n", __FUNCTION__, apIndex));
			result = WIFI_HAL_ERROR;
		}
		...
		len = DOT11_RRM_CAP_LEN;
		if (wldm_xbrcm_AccessPoint_RMCapabilities(CMD_GET_NVRAM, apIndex, (unsigned char *)(&rrm_val),
			&len, NULL, NULL) < 0) {
			HAL_WIFI_ERR(("%s: Error GET rmcap apIndex[%d]\n",
				__FUNCTION__, apIndex));
			result = WIFI_HAL_ERROR;
...
{
	int ret, i;
	char channels[OUTPUT_STRING_LENGTH_512], *pch, *saveptr;
...
static int hal_csi_rec_read(unsigned int RadioIndex, char *mac, char *buf)
{
	...
	wifi_csi_matrix_t *hal_csi_matrix;
	bcm_csimon_hdr_t *hdr;
	/* No. of sub-Carriers used in CSI data by Brcm Phy for bw: 20,40,80 and 160Mhz */
	unsigned int carrier[4] = {14, 28, 60, 120};
	char *raw_buf = NULL;
	...
	HAL_WIFI_CSI_DBG(("chaspec:%04X chan:%d\n", hdr->data.chanspec, CHSPEC_CHANNEL(hdr->data.chanspec)));
	...
	hal_csi_matrix = &(hal_csi_data->csi_matrix);

	HAL_WIFI_CSI_DBG(("%s Read sta_csi_rec[%d][%d]:hal_csi_matrix[%p] hal_frame_info{%p] hdr[%p] data[%p]\n",
		__FUNCTION__, RadioIndex, i, (void *)hal_csi_matrix, (void *)hal_frame_info, (void *)hdr, (void *)(hdr + 1)));

	if (CHSPEC_IS20(hdr->data.chanspec)) {
		hal_frame_info->bw_mode = 0;
		hal_frame_info->phy_bw = 20;
	}
	if (CHSPEC_IS40(hdr->data.chanspec)) {
		hal_frame_info->bw_mode = 1;
		hal_frame_info->phy_bw = 40;
	}
	if (CHSPEC_IS80(hdr->data.chanspec)) {
		hal_frame_info->bw_mode = 2;
		hal_frame_info->phy_bw = 80;
	}
	if (CHSPEC_IS160(hdr->data.chanspec)) {
		hal_frame_info->bw_mode = 3;
		hal_frame_info->phy_bw = 160;
	}

	hal_frame_info->Nr = hdr->data.rxstreams;
	hal_frame_info->Nc = hdr->data.txstreams;

	for (n = 0; n < MAX_NR; n++) {
		hal_frame_info->nr_rssi[n] = hdr->data.rssi[n];
	}

	hal_frame_info->cap_bw = hal_frame_info->phy_bw;
	hal_frame_info->num_sc = carrier[hal_frame_info->bw_mode];
	...
	memcpy(hal_csi_matrix, (char *)(hdr + 1), CSI_MAX_PAYLOAD);
	free(raw_buf);
...
INT wifi_enableCSIEngine(INT apIndex, mac_address_t sta, BOOL enable)
{
	...
	HAL_WIFI_CSI_DBG(("Start: %s wl:%d Mac:"MACF " enab:%d\n", __FUNCTION__, radioIndex, MAC_TO_MACF(sta), enable));
...
INT wifi_setRadioOperatingParameters(wifi_radio_index_t index, wifi_radio_operationParam_t *operationParam)
{
	...
	for (i = 0; i < tblSize; i++) {
		if (operationParam->variant & std2ieee80211Variant_infoTable[i].enum_val) {
	...
	if (i == tblSize) {
		HAL_WIFI_ERR(("%s: invalid operStd variant %d\n",
			__FUNCTION__, operationParam->variant));
		return WIFI_HAL_INVALID_ARGUMENTS;
	}
	...
	if (i == tblSize) {
		HAL_WIFI_ERR(("%s: invalid bandwidth %d\n",i __FUNCTION__, operationParam->channelWidth));
		return WIFI_HAL_INVALID_ARGUMENTS;
	}
	...
	HAL_WIFI_DBG(("%s: radioIndex = %d, channelMode = %s pureMode=%d\n",
				  __FUNCTION__, index, channelMode, pureMode));
		...
		wifi_radio_operationParam_t *operationParam)
{
	...
	HAL_WIFI_DBG(("%s:  radioIndex=%d radioEnabled=%d\n", __FUNCTION__, radioIndex, enable));
...
#define MAX_TOT_NUM_APS				(MAX_NUM_RADIOS * MAX_NUM_VAP_PER_RADIO)
#define TWT_SESS_INFO_PTR(apIndex, sessInd)	&(twt_sessions[apIndex][sessInd])

/* save bcast scheduleIDs in use */
static int twt_bcast_sched_idx[MAX_TOT_NUM_APS][WL_TWT_ID_BCAST_MAX + 1] = {0};

/* For individual twt links from associated STA, generate a unique sessionID and save it */
static wldm_twt_sess_info_t twt_sessions[MAX_TOT_NUM_APS][MAX_NUM_TWT_SESSION] = {0};
...
INT wifi_setSupportRatesBitmapControlFeature(BOOL enable)
{
	...
	if (enable) {
		...
		for (index = 0; index < HAL_RADIO_NUM_RADIOS; index++) {
