...
DEPENDS += "virtual/kernel broadcom-rgdriver sqlite3 libxml2 openssl bison-native libnl util-linux-native"
...
SRC_URI += "file://rdkb_dunfell.patch"
SRC_URI += "file://wifi.service"
SRC_URI += "file://wl_instances.txt"
...
RGOPTS += "BRCM_COMMON_CFLAGS='-Os -fno-strict-aliasing -Wall -Darm -Wno-error=cpp -Wno-error=misleading-indentation -g -fPIC'"
...
do_compile() {
    ...
    cp -rf ${S}/main/components/apps/wldm/nvram_default*.txt ${WORKDIR}/wifi_prebuilt_bin/etc/wlan/
...
do_install() {
    ...
    install -m 0644 ${INSTALL_DIR}/lib/modules/* ${D}/${base_libdir}/modules/${KERNEL_VERSION}/
    ...
    if [ -n "$(ls ${INSTALL_DIR}/etc/wlan/*_defaults.txt)"  ]; then
        install -m 0644 ${INSTALL_DIR}/etc/wlan/*_defaults.txt ${D}/usr/local/etc/wlan/
    fi
    ...
    install -m 0644 ${WORKDIR}/wl_instances.txt ${D}/usr/local/etc/wlan/
    # Install K80wifi
