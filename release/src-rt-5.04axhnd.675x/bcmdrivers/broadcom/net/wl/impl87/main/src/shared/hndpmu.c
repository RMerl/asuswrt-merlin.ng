 ...
 *
 * $Id: hndpmu.c 805932 2021-12-09 03:14:26Z $
 */
...
static const char BCMATTACHDATA(rstr_Invalid_Unsupported_xtal_value_D)[] =
	"Invalid/Unsupported xtal value ";
...
void si_pmu_avbtimer_enable(si_t *sih, osl_t *osh, bool set_flag)
{
	...
	if (((CHIPID(sih->chip) == BCM4366_CHIP_ID) &&
		...
		/*
		For Rev C0 4365 (chiprev 0x4) and 43684, bits 13:12 of chipcontrol_data
...
BCMATTACHFN(si_pmu_update_pllcontrol)(si_t *sih, osl_t *osh, uint32 xtal, bool update_required)
{
	...
	if (!update_required && pllctrlreg_update) {
		...
		if (xf != 0) {
			...
			write_en = FALSE;
			printf("%s%d\n", rstr_Invalid_Unsupported_xtal_value_D, xtal);
		}
	}
...
BCMATTACHFN(si_set_cpu_vcofreq_6715)(si_t *sih, osl_t *osh, pmuregs_t *pmu, uint32 freq)
{
	...
	if ((si_gci_chipstatus(sih, 0) & CST6715_CPUPLL_LOCKED) == 0) {
		...
		PMU_ERROR(("%s: PMU CPU PLL is locked!!\n", __FUNCTION__));
	}
	...
	CASE_BCM6715_CHIP:
		if (pllreg) {
			...
			*pllreg = ((si_pmu_pllcontrol(sih, PMU_PLL_CTRL_REG3, 0, 0) &
				PMU6715_PLL0_PC3_CH2_MDIV_MASK) >> PMU6715_PLL0_PC3_CH2_MDIV_SHIFT);
		}
		break;
	case BCM4347_CHIP_GRPID:
		if (pllreg) {
...
BCMINITFN(si_pmu_si_clock)(si_t *sih, osl_t *osh)
{
	...
	CASE_BCM6715_CHIP:
		{ /* BP clock is derived from BB PLL, channel 1 */
			...
			mdiv = (mdiv &
				PMU6715_PLL0_PC3_CH1_MDIV_MASK) >> PMU6715_PLL0_PC3_CH1_MDIV_SHIFT;
			ASSERT(mdiv != 0);
			clock = si_pmu1_pllfvco0(sih) / mdiv * 1000;
		}
		break;
...
si_pmu_get_resetcontrol(si_t *sih)
{
	uint32 rc;
	if (PMUREV(sih->pmurev) >= 35) {
		rc = pmu_corereg(sih, SI_CC_IDX, pmucontrol, 0, 0) & PCTL_RESETCTL_MASK;
	} else {
		rc = PCTL_RESETCTL_BP_ONLY;
		PMU_ERROR(("ERROR %s: version %d returning default\n", __FUNCTION__,
			PMUREV(sih->pmurev)));
	}
	return rc;
}
