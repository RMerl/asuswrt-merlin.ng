...
BCMATTACHFN(si_muxenab)(si_t *sih, uint32 w)
{
	...
	case BCM43570_CHIP_ID:
		if (w & MUXENAB4350_UART_MASK) {
			...
			uint8 uartopt_size = ARRAYSIZE(mux4350_uartopt);
...
si_gci_seci_init(si_t *sih)
{
	...
	if (CHIPID(sih->chip) == BCM7271_CHIP_ID) {
	...
	} else if (CHIPID(sih->chip) == BCM6710_CHIP_ID) {
		uint32 i;

		/* 6710 Configuration with 20704 BT chip -
		 * BT Chip gpio 6 - BT SECI <--> Wireless- SECO WIFI_GPIO 3 -> GCI GPIO 5
		 * BT Chip gpio 7 - BT SECO <--> Wireless- SECI WIFI_GPIO 4 -> GCI GPIO 6
		 *
		 * GCI rev 15: http://hwnbu-twiki.broadcom.com/bin/view/Mwgroup/GciRev15
		 ...
		 */
		si_gci_indirect(sih, 0,
			GCI_OFFSETOF(sih, gci_chipctrl), ALLONES_32, 0x00100000); //0x200
                       ...
                       uint bustype, void *sdh, char **vars, uint *varsz)
{
	...
	if (EMBEDDED_2x2AX_CORE(devid) || BCM6878_CHIP(devid)) {
		...
		if (sih->enum_base == DUALSLICE_DEV_B_PHYS_ADDR) {
	...
	if ((bustype == PCI_BUS) &&
	    ((OSL_PCI_READ_CONFIG(sii->osh, PCI_SPROM_CONTROL, sizeof(uint32)) == 0xffffffff) ||
	      (devid == EMBEDDED_2x2AX_ID) || (devid == EMBEDDED_2x2AX160_ID) ||
	      (BCM6878_CHIP(devid)) ||
	      FALSE)) {
...
{
	/* XXX we no longer support RSDB so we can safely
	 * return at most 1 here. Leaving the possibility
	 * to return 0 although something is clearly wrong
	 * in that case.
	 */
 ...
 *
 * @param[in] addr     Physical address from the perspective of the AXI backplane
 */
static uint
 ...
 */
uint
si_backplane_pci_config_bar0_core2_win2(si_t *sih, uint32 addr)
{
	si_info_t *sii = SI_INFO(sih);
	uint32 region = 0;
...
BCMNMIATTACHFN(si_devpath)(si_t *sih, char *path, int size)
{
	...
	case JTAG_BUS:
		...
		if (BCM53573_CHIP(sih->chip)) {
			...
			idx = (sih->enum_base == DUALSLICE_DEV_B_PHYS_ADDR) ? DUALSLICE_WLAN_DEV_B :
			                                                     DUALSLICE_WLAN_DEV_A;
...
si_srom_clk_set(si_t *sih)
{
	...
	if (BCM4365_CHIP(sih->chip)) {
		...
		divisor = CLKD2_SROMDIV_384; /* divide 320 by 384 -> SPROM clock ~ 0.83Mhz */
	} else {
		ASSERT(0);
	}
