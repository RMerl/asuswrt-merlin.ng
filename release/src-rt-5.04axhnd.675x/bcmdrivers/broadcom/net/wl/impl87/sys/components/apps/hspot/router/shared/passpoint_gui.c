...
validate_wl_hsflag(webs_t wp, char *value, struct variable *v, char *varname)
{
	...
	char* val = NULL, prefix[] = PREFIX_DEF;
	...
	/* Update akm for "osen" */
	if ((val = websGetVar(wp, "wl_osenen", NULL)) != NULL) {
		char nvname[NVRAM_MAX_PARAM_LEN] = {0}, *ptr = NULL;
		char akms[NVRAM_MAX_VALUE_LEN] = {0};
		int osen_val = atoi(val);

		ptr = nvram_safe_get(strcat_r(prefix, NVNM_AKM, nvname));
		strncpy_n(akms, ptr, sizeof(akms));

		/* If OSEN Value = 1, And wlX_akm missing "osen", Overwrite it */
		if ((osen_val > 0) && !find_in_list(akms, NVVAL_OSEN)) {
			nvram_set(nvname, NVVAL_OSEN);

		/* If OSEN Value = 0, And wlX_akm having "osen", Remove it */
		} else if ((osen_val == 0) && find_in_list(akms, NVVAL_OSEN)) {
			remove_from_list(NVVAL_OSEN, akms, sizeof(akms));
			nvram_set(nvname, akms);
		}
	}
