 ...
 *
 * $Id: dhd_linux_wq.c 780131 2019-10-16 07:43:41Z $
 */
...
dhd_deferred_work_init(void *dhd_info)
{
	...
	work->active_mask = DHD_WQ_EVENTMASK;
 ...
 */
int
dhd_deferred_schedule_work(void *workq, void *event_data, enum dhd_wq_event event,
	event_handler_t event_handler, u8 priority)
{
	...
	if (!(event_flag & DHD_WQ_EVENTMASK)) {
		DHD_ERROR(("%s: Unknown/invalid event: %08X (%08X)\n",
			__FUNCTION__, event_flag, DHD_WQ_EVENTMASK));
		return DHD_WQ_STS_UNKNOWN_EVENT;
	}

	if (!(deferred_wq->active_mask & event_flag)) {
		DHD_ERROR(("%s: event 0x%08X disabled (mask: 0x%08X)\n",
			__FUNCTION__, event_flag, deferred_wq->active_mask));
...
void
dhd_deferred_work_enable(void *work, u32 event_mask)
{
	struct dhd_deferred_wq *deferred_wq = (struct dhd_deferred_wq *)work;

	if (!deferred_wq || !event_mask || (event_mask & DHD_WQ_EVENTMASK) != event_mask) {
		DHD_ERROR(("%s: Invalid!!\n", __FUNCTION__));
		return;
	}

	deferred_wq->active_mask |= event_mask;
}

void
dhd_deferred_work_disable(void *work, u32 event_mask)
{
	struct dhd_deferred_wq *deferred_wq = (struct dhd_deferred_wq *)work;
