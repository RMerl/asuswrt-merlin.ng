 ...
 *
 * $Id: bcm_ring.h 787082 2020-05-18 09:21:51Z $
 */
...
/* Fetch the ring element pointer given an ring index in the circular ring */
#define BCM_RING_ELEM(ring_base, ring_index, elem_size) \
({ \
	(void*)((char*)(ring_base) + ((ring_index) * (elem_size))); \
})
...
#endif /* BCM_RING_DEBUG */
	int write;			/* WRITE index in a circular ring */
	int read;			/* READ index in a circular ring */
} bcm_ring_t;
