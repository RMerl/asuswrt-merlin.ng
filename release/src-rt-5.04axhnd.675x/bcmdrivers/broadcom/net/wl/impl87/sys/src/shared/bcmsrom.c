 ...
 *
 * $Id: bcmsrom.c 802241 2021-08-19 20:45:54Z $
 */
...
BCMATTACHFN(initvars_srom_pci)(si_t *sih, void *curmap, char **vars, uint *count)
{
	...
	if (si_is_sprom_available(sih)) {
		if (ISSIM_ENAB(sih) && (BCM43684_CHIP(sih->chip) ||
			...
			BS_ERROR(("skip srom read in 43684/6710/6715 veloce development\n"));
			err = 1;
		} else {
	...
	if (err || sromrev == 0) {
...
#if defined(BCA_SROMMAP)
		/* Precedence:
		 *     SROM -> nvram.nvm -> srommap -> default sromvars.
		 */
		printf("wl:srom/otp not programmed, using main memory mapped "
			"srom info(wombo board)\n");
		/* read in srom files during attach time */
		err = init_srom_sw_map(sih, CHIPID(sih->chip),
			sih->wl_srom_sw_map, sizeof(sih->wl_srom_sw_map));
		if (err) {
			printf("%s: init_srom_sw_map failed! err=%d\n",
				__FUNCTION__, err);
			goto defaultsrom;
		}

		read_sromfile(sih->wl_srom_sw_map, srom, 0,
			MIN(SROM_MAX, sizeof(sih->wl_srom_sw_map)));
			...
			(BUSCORETYPE(sih->buscoretype) == PCIE2_CORE_ID)))
		{
			...
			if (srom[SROM11_SIGN] == SROM13_SIGNATURE) {
				...
				/* srom 11  */
				sromrev = srom[SROM11_CRCREV] & 0xff;
				read_sromfile(sih->wl_srom_sw_map, srom, 0, SROM11_WORDS*2);
			} else if (srom[SROM10_SIGN] == SROM10_SIGNATURE) {
				/* srom 10  */
				sromrev = srom[SROM10_CRCREV] & 0xff;
				read_sromfile(sih->wl_srom_sw_map, srom, 0, SROM10_WORDS*2);
			} else {
...
#if defined(BCA_SROMMAP)
	/* update from nvram vars files if exist */
	if (srommap)
		init_nvramvars(sih, osh, &b);
#endif
	/* final nullbyte terminator */
	ASSERT(b.size >= 1);
	vp = b.buf;
...
BCMATTACHFN(flash_nvram_present)(si_t *sih, osl_t *osh, char **vars)
{
	...
	if ((value = si_getdevpathvar(sih, "sromrev"))) {
		...
		sr = 1 << sromrev;
		if ((sr & VALID_SROMREV) != 0) {
			if ((initvars(sih, osh, vars, MAXSZ_NVRAM_VARS)) == BCME_OK) {
				return TRUE;
			}
		}
	}
