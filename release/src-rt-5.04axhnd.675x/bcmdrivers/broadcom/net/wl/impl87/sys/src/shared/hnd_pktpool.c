 ...
 *
 * $Id: hnd_pktpool.c 794080 2020-12-21 03:36:52Z $
 */
...
#endif
#ifndef PQP_HEAP_BUFFER_LEN
#define PQP_HEAP_BUFFER_LEN	(SHARED_FRAG_POOL_LEN/2)
#endif
...
#ifdef HNDPQP
lfrag_buf_pool_t *pqp_heap_buf_pool = NULL;
#endif
...
BCMATTACHFN(lfbufpool_init)(osl_t *osh, lfrag_buf_pool_t *lfbufp, int *n_pkts, int max_buf_bytes)
{
	...
	for (i = 0; i < plen; i++) {
		...
		buf = (lfrag_buf_t *)MALLOC_ALIGN(osh, max_buf_bytes + LFBUFSZ,
			align_bits);
		if (buf == NULL) {
			/* Not able to allocate all requested pkts
			 * so just return what was actually allocated
...
lfbufpool_fill(osl_t *osh, lfrag_buf_pool_t *lfbufp, bool minimal)
{
	...
	for (len = (int)lfbufp->n_bufs; len < psize; len++) {
		lfbuf = (lfrag_buf_t *)MALLOC_ALIGN(osh, lfbufp->max_buf_bytes + LFBUFSZ,
			align_bits);
		if (lfbuf == NULL) {
			err = BCME_NOMEM;
			break;
...
#ifdef HNDPQP
	if (LFBUFHOMEPOOL(lfbuf) == pqp_heap_buf_pool)
		pqp_pgi_wake();
#endif /* HNDPQP */
}

void
lfbufpool_free_buf(osl_t *osh, void *p)
{
	ASSERT(p != NULL);

	/* free buffer */
	if (PKTISBUFALLOC(osh, p)) {
		if (PKTHASHEAPBUF(osh, p)) {
			MFREE(osh, PKTHEAD(osh, p), MAXPKTDATABUFSZ);
			PKTRESETHEAPBUF(osh, p);
		} else {
			lfbufpool_free(PKTHEAD(osh, p));
		}
...
#ifdef HNDPQP
	pqp_heap_buf_pool = MALLOCZ(osh, sizeof(lfrag_buf_pool_t));
	if (pqp_heap_buf_pool == NULL) {
		ASSERT(0);
		goto error2a;
	}
#endif /* HNDPQP */
...
#endif /* !BCM_DHDHDR_D11_DISABLE */
#ifdef HNDPQP
	/* PQP_BUFFER pool initialization */
	n = 1;
	if (lfbufpool_init(osh, pqp_heap_buf_pool, &n, MAXPKTDATABUFSZ) == BCME_ERROR) {
		ASSERT(0);
		goto error5a;
	}
	lfbufpool_setmaxlen(pqp_heap_buf_pool, PQP_HEAP_BUFFER_LEN);
#endif /* HNDPQP */
#endif /* BCM_DHDHDR && DONGLEBUILD */
...
#if defined(BCM_DHDHDR) && defined(DONGLEBUILD)
#if !defined(BCM_DHDHDR_D3_DISABLE) || !defined(BCM_DHDHDR_D11_DISABLE) || \
	defined(HNDPQP)
error5a:
#endif /* !BCM_DHDHDR_D3_DISABLE || !BCM_DHDHDR_D11_DISABLE || HNDPQP */
#if !defined(BCM_DHDHDR_D3_DISABLE)
...
#endif /* !BCM_DHDHDR_D11_DISABLE */
#ifdef HNDPQP
	if (pqp_heap_buf_pool)
		lfbufpool_deinit(osh, pqp_heap_buf_pool);
#endif /* HNDPQP */
#endif /* BCM_DHDHDR && DONGLEBUILD */
...
#ifdef HNDPQP
	if (pqp_heap_buf_pool) {
		hnd_free(pqp_heap_buf_pool);
		pqp_heap_buf_pool = (lfrag_buf_pool_t *)NULL;
	}
#endif
#endif /* BCM_DHDHDR && DONGLEBUILD */
...
#endif
#ifdef HNDPQP
	lfbufpool_fill(pktpool_osh, pqp_heap_buf_pool, minimal);
#endif
#endif /* BCM_DHDHDR && DONGLEBUILD */
...
#endif
#ifdef HNDPQP
	if (POOL_ENAB(pqp_heap_buf_pool)) {
		lfbufpool_fill(pktpool_osh, pqp_heap_buf_pool, minimal);
	}
#endif
#endif /* BCM_DHDHDR && DONGLEBUILD */
