 ...
 *
 * $Id: wl_cfg80211.c 808279 2022-02-15 05:56:45Z $
 */
...
rspec_to_cfg80211_rate_info(u32 rspec, u32 legacy_rate, struct rate_info *ri)
{
	...
	if (RSPEC_ISHT(rspec)) {
		...
		ri->mcs = RSPEC_VHT_MCS(rspec);
		ri->nss = RSPEC_VHT_NSS(rspec);
	} else if (RSPEC_ISHE(rspec)) {
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4, 19, 0))
		ri->flags |= RATE_INFO_FLAGS_HE_MCS;
		ri->mcs = RSPEC_HE_MCS(rspec);
		ri->nss = RSPEC_HE_NSS(rspec);
		ri->he_dcm = rspec & WL_RSPEC_DCM;
		if (RSPEC_ISHEGI(rspec)) {
		...
		struct station_info *sinfo)
{
	...
	if (sta_v7) {
		...
		for (i = 0; i < WL_STA_ANT_MAX; i++) {
			if (sta_v7->rssi[i]) {
				rssi_avg += sta_v7->rssi[i];
				rssi += sta_v7->rx_lastpkt_rssi[i];
				sinfo->chain_signal_avg[sinfo->chains] = sta_v7->rssi[i];
				sinfo->chain_signal[sinfo->chains] = sta_v7->rx_lastpkt_rssi[i];
				sinfo->chains++;
			}
		}
		if (sinfo->chains) {
		...
		sinfo->rx_bytes = dtoh64(sta_v7->rx_tot_bytes);
		sinfo->filled |= STA_INFO_BIT(INFO_TX_BYTES);
		sinfo->tx_bytes = dtoh64(sta_v7->tx_tot_bytes);
		sinfo->filled |= STA_INFO_BIT(INFO_RX_PACKETS);
		sinfo->rx_packets = dtoh64(sta_v7->rx_tot_pkts);
		sinfo->filled |= STA_INFO_BIT(INFO_TX_PACKETS);
		sinfo->tx_packets = dtoh64(sta_v7->tx_tot_pkts);
		if (sta_v7->tx_failures) {
			sinfo->filled |= STA_INFO_BIT(INFO_TX_FAILED);
			sinfo->tx_failed = dtoh32(sta_v7->tx_failures);
		}
		sinfo->filled |= STA_INFO_BIT(INFO_RX_BITRATE);
		rspec_to_cfg80211_rate_info(dtoh32(sta_v7->rx_rspec), dtoh32(sta_v7->rx_rate),
			&sinfo->rxrate);
		sinfo->filled |= STA_INFO_BIT(INFO_TX_BITRATE);
		rspec_to_cfg80211_rate_info(dtoh32(sta_v7->tx_rspec), dtoh32(sta_v7->tx_rate),
			&sinfo->txrate);
	}
	...
	struct net_device *dev)
{
	...
	wl_clr_drv_status(cfg, CONNECTED, dev);
#if !defined(BCA_HNDROUTER) || defined(WL_CFG80211_STRICT)
	if ((err = wl_cfg80211_bss_up(cfg, dev, bssidx, 0)) < 0) {
		WL_ERR(("bss down error %d\n", err));
	}
#endif /* !BCA_HNDROUTER || WL_CFG80211_STRICT */
...
wl_get_raw_epoch(u64 *timestamp)
{
		...
		*timestamp = do_div(*timestamp, 1000);
		...
		*timestamp = timespec_to_ns(&ts32);
		...
		struct timeval tv;
		do_gettimeofday(&tv);
		*timestamp = ((u64)tv.tv_sec*1000000)
				+ tv.tv_usec;
#endif
}
static s32
	...
	bcm_struct_cfgdev *cfgdev, const wl_event_msg_t *e, void *data)
{
	...
	WL_ERR(("In %s chanspec 0x%x DFS state %d\n", __FUNCTION__, dfs_sub_status->chanspec,
...
wl_cfg80211_alloc_2g_channel_bands(struct wireless_dev *wdev)
{
	...
	wl_2ghz_channels = kcalloc(MAX_NO_2G_CHANNELS, sizeof(struct ieee80211_channel), kflags);
	if (!wl_2ghz_channels) {
...
wl_cfg80211_alloc_5g_channel_bands(struct wireless_dev *wdev)
{
	...
	wl_5ghz_channels = kcalloc(MAX_NO_5G_CHANNELS, sizeof(struct ieee80211_channel), kflags);
	if (!wl_5ghz_channels) {
...
{
	struct ieee80211_channel *wl_6ghz_channels = NULL;
