...
bool wl_cfgp2p_find_gas_subtype(u8 subtype, u8* data, u32 len)
{
	...
	ie = bcm_find_tlv(ie, (int)len, DOT11_MNG_ADVERTISEMENT_ID);
...
#ifdef WL11U
#ifdef WL_DPP
bool wl_cfgp2p_is_allowed_gas_frame(struct net_device *dev, void *frame, u32 frame_len)
{
	const bcm_tlv_t *ie;
	const u8 *frm = NULL;
	u8 adv_proto_id;
	...
	bool gas_req = false;
	bool gas_resp = false;
	BCM_REFERENCE(adv_proto_id_offset);
	...
	if ((dev->ieee80211_ptr->iftype == NL80211_IFTYPE_AP) && (sd_act_frm->action ==
		GAS_COMEBACK_REQUEST_ACTION_FRAME || sd_act_frm->action ==
	...
	/* Skipped first ANQP Element, if frame has anqp elemnt */
	ie = bcm_find_tlv(ie, (int)frame_len, DOT11_MNG_ADVERTISEMENT_ID);

	if (ie == NULL)
		return false;

	frm = (const uint8 *)ie + TLV_HDR_LEN;
	adv_proto_id = frm[adv_proto_id_offset];
...
static void
wl_cfgp2p_escan_prep(struct bcm_cfg80211 *cfg, void *params,
	u16 active, u32 num_chans, u16 *channels, u16 action,
	struct ether_addr *tx_dst_addr, p2p_scan_purpose_t p2p_scan_purpose,
	wlc_ssid_t ssid, u16 version)
{
	u32 i = 0;
	...
	if (active)
		...
		params_v1->scan_type = DOT11_SCANTYPE_PASSIVE;

	if (tx_dst_addr == NULL)
		memcpy(&params_v1->bssid, &ether_bcast, ETHER_ADDR_LEN);
	else
		memcpy(&params_v1->bssid, tx_dst_addr, ETHER_ADDR_LEN);

	if (ssid.SSID_len)
		memcpy(&params_v1->ssid, &ssid, sizeof(wlc_ssid_t));

	params_v1->home_time = htod32(P2PAPI_SCAN_HOME_TIME_MS);

	switch (p2p_scan_purpose) {
		case P2P_SCAN_SOCIAL_CHANNEL:
		params_v1->active_time = htod32(P2PAPI_SCAN_SOCIAL_DWELL_TIME_MS);
			break;
		case P2P_SCAN_AFX_PEER_NORMAL:
		case P2P_SCAN_AFX_PEER_REDUCED:
			params_v1->active_time = htod32(P2PAPI_SCAN_AF_SEARCH_DWELL_TIME_MS);
			break;
		...
		params_v1->active_time = htod32(P2PAPI_SCAN_DWELL_TIME_MS);
			break;
	}

	if (p2p_scan_purpose == P2P_SCAN_CONNECT_TRY)
		params_v1->nprobes = htod32(params_v1->active_time /
			WL_SCAN_JOIN_PROBE_INTERVAL_MS);
	else
	params_v1->nprobes = htod32((params_v1->active_time /
		P2PAPI_SCAN_NPROBS_TIME_MS));

	if (params_v1->nprobes <= 0)
		params_v1->nprobes = 1;
	CFGP2P_DBG(("nprobes # %d, active_time %d\n",
		params_v1->nprobes, params_v1->active_time));
	...
	p2p_scan_purpose_t p2p_scan_purpose)
{
	...
	if ((err = wldev_iovar_getbuf(dev, "scan_ver", NULL, 0, cfg->escan_ioctl_buf,
	...
	if (version == WL_SCAN_VERSION_MAJOR_V2) {
		...
		eparams_size = (WL_SCAN_PARAMS_FIXED_SIZE_V2 +
			OFFSETOF(wl_escan_params_v2_t, params)) +
			num_chans *
			sizeof(((wl_escan_params_v2_t *)eparams)->params.channel_list[0]);
	} else {
		/* Allocate scan params which need space for 3 channels and 0 ssids */
		eparams_size = (WL_SCAN_PARAMS_FIXED_SIZE_V1 +
			OFFSETOF(wl_escan_params_v1_t, params)) +
			num_chans *
			sizeof(((wl_escan_params_v1_t *)eparams)->params.channel_list[0]);
	}
	...
	eparams = (void*) (p2p_params + 1);
	if (version == WL_SCAN_VERSION_MAJOR_V2) {
		wl_cfgp2p_escan_prep(cfg, &((wl_escan_params_v2_t *)eparams)->params,
			active, num_chans, channels, action,
			tx_dst_addr, p2p_scan_purpose, ssid, version);
	} else {
		wl_cfgp2p_escan_prep(cfg, &((struct wl_escan_params *)eparams)->params,
			active, num_chans, channels, action,
			tx_dst_addr, p2p_scan_purpose, ssid, version);
	}
	...
	((wl_escan_params_v1_t *)eparams)->action =  htod16(action);
	wl_escan_set_sync_id(((wl_escan_params_v1_t *)eparams)->sync_id, cfg);
	...
	WL_ERR(("P2P_SEARCH sync ID: %d, bssidx: %d\n",
		((wl_escan_params_v1_t *)eparams)->sync_id, bssidx));
	if (ret == BCME_OK)
	...
	s32 bssidx, s32 channel, struct ether_addr *tx_dst_addr)
{
	...
	if (bssidx == wl_to_p2p_bss_bssidx(cfg, P2PAPI_BSSCFG_PRIMARY))
		bssidx = wl_to_p2p_bss_bssidx(cfg, P2PAPI_BSSCFG_DEVICE);
	if (channel)
		n_channels = AF_PEER_SEARCH_CNT;
	else
		n_channels = SOCIAL_CHAN_CNT;
	...
	default_chan_list = kzalloc(n_channels * sizeof(*default_chan_list), GFP_KERNEL);
	...
	ret = wl_cfgp2p_escan(cfg, ndev, true, n_channels,
 ...
 */
/* TODO:6GHZ: should this function take a chspec parameter instead of a channel ? */
s32
	...
	if (wl_get_drv_status_all(cfg, SENDING_ACT_FRM)) {
		if (event_type == WLC_E_ACTION_FRAME_COMPLETE) {
			...
			if (status == WLC_E_STATUS_SUCCESS) {
				...
				if (!cfg->need_wait_afrx && cfg->af_sent_chspec) {
	...
	wl_af_params_t *af_params, s32 bssidx)
{
...
#ifdef WL_DPP
	u32 chanspec, chan;
	uint8 dpp_af_array[WL_DPP_AF_ARRAY_SIZE] = {0x04, 0x09, 0x50, 0x6f, 0x9a, 0x1a, 0x01, 0x00};
#endif /* WL_DPP */
	...
	cfg->af_sent_chspec =
		wf_create_20MHz_chspec(af_params->channel, BANDTYPE_CHSPEC(af_params->bandtype));
...
#ifdef WL_DPP
	ret = wldev_iovar_getint(dev, "chanspec", &chanspec);
	chan = wf_chspec_primary20_chan(wl_chspec_driver_to_host(chanspec));
	if ((chan != af_params->channel) &&
		(!memcmp(&af_params->action_frame.data, &dpp_af_array, WL_DPP_AF_ARRAY_SIZE))) {
		af_params->dwell_time = WL_DPP_AF_OFF_CHAN_DWELL_TIME;
	}
#endif /* WL_DPP */
...
#ifdef WL_DPP
	/* To avoid staying on current channel for more than 2s(dpp timeout in hostapd is 2s),
	 * modified the dwell time from 500ms to 50ms. In case of NOACK, glue layer is
	 * trying to send action frame for 5 times and for every retry glue layer is waiting
	 * for 250ms(WL_AF_TX_EXTRA_TIME + 50ms = 250ms) before proceeding to next retry.
	 * 5 retries timeout is still less than 2s and hence hostapd timeout is in sync
	 * with glue layer.
	 */
	if (!memcmp(&af_params->action_frame.data, &dpp_af_array, WL_DPP_AF_ARRAY_SIZE)) {
		af_params->dwell_time = WL_DPP_AF_RETRY_DWELL_TIME;
	}
#endif /* WL_DPP */
