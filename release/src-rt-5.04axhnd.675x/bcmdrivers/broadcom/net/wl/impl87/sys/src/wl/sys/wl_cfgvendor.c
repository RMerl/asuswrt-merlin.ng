 ...
 *
 * $Id: wl_cfgvendor.c 789214 2020-07-22 02:54:59Z $
 */
	...
	const void *data, int len)
{
	...
	struct bcm_cfg80211 *cfg = wiphy_priv(wiphy);
	dhd_pub_t *dhd_pub = cfg->pub;
	gfp_t kflags = in_atomic() ? (gfp_t)GFP_ATOMIC : (gfp_t)GFP_KERNEL;
	...
	nla_for_each_attr(iter, data, len, tmp) {
		...
		switch (type) {
			...
			case APF_ATTRIBUTE_PROGRAM:
				...
				kflags = in_atomic() ? (gfp_t)GFP_ATOMIC : (gfp_t)GFP_KERNEL;
