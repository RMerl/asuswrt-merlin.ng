 ...
 *
 * $Id: wl_dbg.h 805126 2021-11-17 03:06:23Z $
 */
...
#define WL_DBG_MSG_MAC_PFX_STRING	"wl%d.%d "MACF": %s: "

#define WL_DBG_MSG_MAC_PREFIX(scb) \
	WL_PRINT((WL_DBG_MSG_MAC_PFX_STRING, SCB_BSSCFG(scb)->wlc->pub->unit, \
		SCB_BSSCFG(scb)->_idx, ETHER_TO_MACF(scb->ea), __FUNCTION__));
 ...
 * WL_MSG_MACFILTER_EX(scb, (...)) - display messages
 *
 ...
 * WL_MSG_MACFILTER_EX(scb, (...)) - display messages when Mac filter matches
 *
 * With WL_MSG_MACFILTER_EX, the messages will get a fixed prefix
 * 'wl0.0 00:11:22:33:44:55: function_name: '
 * WL_MSG_MACFILTER_EX_NPFX can be used in case this is not needed/wanted.
 */
...
#define WL_MSG_MACFILTER_EX(msglvl, scb, args) \
	do { \
		if (msg_mac_filter_ex(msglvl, scb)) { \
			WL_DBG_MSG_MAC_PREFIX(scb); \
			printf args; \
		} \
	} while (0)

#define WL_MSG_MACFILTER_EX_NPFX(msglvl, scb, args) \
	do { \
		if (msg_mac_filter_ex(msglvl, scb)) { \
			printf args; \
		} \
...
#define WL_PS_EX(scb, args)	WL_MSG_MACFILTER_EX(WL_PS_VAL, (scb), args)

#define WL_AMPDU0(args)		WL_MSG_MACFILTER0(WL_AMPDU_VAL, args)
#define WL_AMPDU_EX(scb, args)	WL_MSG_MACFILTER_EX(WL_AMPDU_VAL, (scb), args)
#define WL_AMPDU_EX_NPFX(scb, args) WL_MSG_MACFILTER_EX_NPFX(WL_AMPDU_VAL, (scb), args)

#define WL_AMSDU_EX(scb, args)	WL_MSG_MACFILTER_EX(WL_AMSDU_VAL, (scb), args)
#define WL_AMSDU_EX_NPFX(scb, string, args...) \
	WL_MSG_MACFILTER_EX_NPFX(WL_AMSDU_VAL, (scb), (string, ##args))

#define WL_MSG_MACFILTER_PFX_LEN	80
typedef struct msg_macfilter_pfx {
	bool initialized;
	char prefix[WL_MSG_MACFILTER_PFX_LEN];
} msg_macfilter_pfx_t;

#define WL_MSG_MACFILTER_PFX_DECL	msg_macfilter_pfx_t msg_mac_filter_prefix
#define WL_MSG_MACFILTER_PFX_INIT(scb)	msg_mac_filter_prefix.initialized = FALSE

/* This MACRO should be used in combination with WL_DBG_MSG_MAC_PFX_DECL
 * and WL_DBG_MSG_MAC_PFX_INIT
 */
#define WL_MSG_MACFILTER_EX_GIVEN_PFX(msglvl, scb, string, args...) \
	do { \
		if (msg_mac_filter_ex_pfx(msglvl, scb, &msg_mac_filter_prefix, __FUNCTION__)) { \
			WL_PRINT(("%s"string, msg_mac_filter_prefix.prefix, ##args)); \
		} \
	} while (0)

#define WL_AMPDU_EX_PFX(scb, string, args...) \
	WL_MSG_MACFILTER_EX_GIVEN_PFX(WL_AMPDU_VAL, scb, string, ##args)

#define WL_AMSDU_EX_PFX(scb, string, args...) \
	WL_MSG_MACFILTER_EX_GIVEN_PFX(WL_AMSDU_VAL, scb, string, ##args)
...
#define WL_AMPDU_DBG_EX(dbg_val, scb, args) \
	do { \
		if (wl_ampdu_dbg & dbg_val) { \
			WL_AMPDU_EX(scb, args); \
		} \
	} while (0)

#define WL_AMPDU_DBG_EX_PFX(dbg_val, scb, string, args...) \
	do { \
		if (wl_ampdu_dbg & dbg_val) { \
			WL_AMPDU_EX_PFX(scb, string, ##args); \
		} \
	} while (0)

#define WL_AMPDU_UPDN_EX(scb, args)	WL_AMPDU_DBG_EX(WL_AMPDU_UPDN_VAL, scb, args)
#define WL_AMPDU_RX_EX(scb, args)	WL_AMPDU_DBG_EX(WL_AMPDU_RX_VAL, scb, args)
#define WL_AMPDU_ERR_EX(scb, args)	WL_AMPDU_DBG_EX(WL_AMPDU_ERR_VAL, scb, args)
#define WL_AMPDU_TX_EX(scb, args)	WL_AMPDU_DBG_EX(WL_AMPDU_TX_VAL, scb, args)
#define WL_AMPDU_CTL_EX(scb, args)	WL_AMPDU_DBG_EX(WL_AMPDU_CTL_VAL, scb, args)
#define WL_AMPDU_STAT_EX(scb, args)	WL_AMPDU_DBG_EX(WL_AMPDU_STAT_VAL, scb, args)

#define WL_AMPDU_UPDN_EX_PFX(scb, string, args...) \
	WL_AMPDU_DBG_EX_PFX(WL_AMPDU_UPDN_VAL, scb, string, ##args)
#define WL_AMPDU_RX_EX_PFX(scb, string, args...) \
	WL_AMPDU_DBG_EX_PFX(WL_AMPDU_RX_VAL, scb, string, ##args)
#define WL_AMPDU_ERR_EX_PFX(scb, string, args...) \
	WL_AMPDU_DBG_EX_PFX(WL_AMPDU_ERR_VAL, scb, string, ##args)
#define WL_AMPDU_TX_EX_PFX(scb, string, args...) \
	WL_AMPDU_DBG_EX_PFX(WL_AMPDU_TX_VAL, scb, string, ##args)
#define WL_AMPDU_CTL_EX_PFX(scb, string, args...) \
	WL_AMPDU_DBG_EX_PFX(WL_AMPDU_CTL_VAL, scb, string, ##args)
#define WL_AMPDU_STAT_EX_PFX(scb, string, args...) \
	WL_AMPDU_DBG_EX_PFX(WL_AMPDU_STAT_VAL, scb, string, ##args)
...
/* Msg MacFilter specific */
#ifndef BCMDBG
#define WL_MSG_MACFILTER_PFX_DECL
#define WL_MSG_MACFILTER_PFX_INIT(scb)

#define WL_PS0(args)		WL_PS(args)

#define WL_AMPDU0(args)					WL_AMPDU(args)
#define WL_AMPDU_EX(scb, args)				WL_AMPDU(args)
#define WL_AMPDU_EX_PFX(scb, string, args...)		WL_AMPDU((string, ##args))
#define WL_AMPDU_EX_NPFX(scb, args)			WL_AMPDU(args)
#define WL_AMPDU_UPDN_EX(scb, args)			WL_AMPDU_UPDN(args)
#define WL_AMPDU_UPDN_EX_PFX(scb, string, args...)	WL_AMPDU_UPDN((string, ##args))
#define WL_AMPDU_RX_EX(scb, args)			WL_AMPDU_RX(args)
#define WL_AMPDU_RX_EX_PFX(scb, string, args...)	WL_AMPDU_RX((string, ##args))
#define WL_AMPDU_ERR_EX(scb, args)			WL_AMPDU_ERR(args)
#define WL_AMPDU_ERR_EX_PFX(scb, string, args...)	WL_AMPDU_ERR((string, ##args))
#define WL_AMPDU_TX_EX(scb, args)			WL_AMPDU_TX(args)
#define WL_AMPDU_TX_EX_PFX(scb, string, args...)	WL_AMPDU_TX((string, ##args))
#define WL_AMPDU_CTL_EX(scb, args)			WL_AMPDU_CTL(args)
#define WL_AMPDU_CTL_EX_PFX(scb, string, args...)	WL_AMPDU((string, ##args))
#define WL_AMPDU_STAT_EX(scb, args)			WL_AMPDU_STAT(args)
#define WL_AMPDU_STAT_EX_PFX(scb, string, args...)	WL_AMPDU_STAT((string, ##args))

#define WL_AMSDU_EX(scb, args)			WL_AMSDU(args)
#define WL_AMSDU_EX_PFX(scb, string, args...)	WL_AMSDU((string, ##args))
#define WL_AMSDU_EX_NPFX(scb, string, args...)	WL_AMSDU((string, ##args))
#endif /* BCMDBG */
