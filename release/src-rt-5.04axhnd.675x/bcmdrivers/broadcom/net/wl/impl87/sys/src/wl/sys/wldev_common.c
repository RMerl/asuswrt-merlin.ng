 ...
 *
 * $Id: wldev_common.c 804699 2021-11-08 13:21:04Z $
 */
...
extern int dhd_ioctl_entry_local(struct net_device *dev, wl_ioctl_t *ioc, int cmd);
...
#else
	ret = wl_ioctl_entry_local(dev, &ioc, cmd);
...
static s32
wldev_mkxtlv(const s8 *iov_name, u16 id, const s8 *param, s32 paramlen, s8 *xtlv_buf, s32 buflen)
{
	s32 iolen, nmlen;

	nmlen = strlen(iov_name) + 1;
	iolen = nmlen + bcm_xtlv_size_for_data(paramlen, BCM_XTLV_OPTION_ALIGN32);
	...
	memcpy(xtlv_buf, iov_name, nmlen);
	bcm_xtlv_pack_xtlv((void *)(xtlv_buf + nmlen), id, paramlen, param,
...
s32
wldev_xtlv_getbuf(struct net_device *dev, s8 *iovar_name, u16 id,
	const void *param, s32 paramlen, void *buf, s32 buflen, struct mutex* buf_sync)
{
	s32 ret = 0;
	if (buf_sync) {
	...
	ret = wldev_mkxtlv(iovar_name, id, param, paramlen, buf, buflen);
	if (ret < 0) {
		goto exit;
	}
...
s32
wldev_xtlv_setbuf(struct net_device *dev, s8 *iovar_name, u16 id,
	const void *param, s32 paramlen, void *buf, s32 buflen, struct mutex* buf_sync)
{
	s32 ret;
	if (buf_sync) {
	...
	ret = wldev_mkxtlv(iovar_name, id, param, paramlen, buf, buflen);
	if (ret > 0)
		ret = wldev_ioctl_set(dev, WLC_SET_VAR, buf, ret);
...
s32
wldev_xtlv_setint(struct net_device *dev, s8 *iovar, u16 id, s32 val)
{
	s8 iovar_buf[WLC_IOCTL_SMLEN];

	val = htod32(val);
	memset(iovar_buf, 0, sizeof(iovar_buf));
	return wldev_xtlv_setbuf(dev, iovar, id, &val, sizeof(val), iovar_buf,
		sizeof(iovar_buf), NULL);
}

s32
wldev_xtlv_getint(struct net_device *dev, s8 *iovar, u16 id, s32 *pval, s32 mask)
{
	s8 iovar_buf[WLC_IOCTL_SMLEN];
	s32 err;

	memset(iovar_buf, 0, sizeof(iovar_buf));
	err = wldev_xtlv_getbuf(dev, iovar, id, pval, sizeof(*pval), iovar_buf,
		sizeof(iovar_buf), NULL);
	if (err == 0) {
		memcpy(pval, iovar_buf, sizeof(*pval));
		*pval = dtoh32(*pval);
