...
#else
static void fpm_isr_timer_timeout(struct timer_list *timer_p)
{
#if FPM_POOL_NUM > 1
	struct fpmdev *fdev = from_timer(fdev, timer_p, isr_timer);
#endif
	...
	if (fdev->init && FPM_GET_POOL_BASE_ADDR(0) == 0) {
		...
		if (fdev->irq > 0) {
			...
			setup_timer(&fdev->isr_timer, fpm_isr_timer_timeout,
				    (unsigned long)fdev);
#else
            timer_setup(&fdev->isr_timer, fpm_isr_timer_timeout, 0);
#endif
#if STOP_UBUS_CAPTURE
...
#ifdef CONFIG_BCM_FPM_DT
module_platform_driver(fpm_driver);
#else
