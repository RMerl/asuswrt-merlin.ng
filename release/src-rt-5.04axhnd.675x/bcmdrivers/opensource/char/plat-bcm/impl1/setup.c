...
int mtusize = 1500;
int bcm_fkb_inplace = sizeof(FkBuff_t);
EXPORT_SYMBOL(mtusize);
EXPORT_SYMBOL(bcm_fkb_inplace);
...
void __init get_mtusize(char *cmdline)
{
    char *cmd_ptr;
    int mtusize_input;

    cmd_ptr = strstr(cmdline, "mtusize");
    if (cmd_ptr != NULL) {
        sscanf(cmd_ptr, "mtusize=%d", &mtusize_input);
        if (mtusize < mtusize_input)
