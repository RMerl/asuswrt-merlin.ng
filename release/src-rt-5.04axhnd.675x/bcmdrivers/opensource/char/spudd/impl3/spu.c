...
static void spu_aead_fallback_cipher_comp_enc(struct crypto_async_request *areq, int err)
{
    ...
    subreq = container_of(req, struct aead_givcrypt_request, areq);
#else
                                 ...
                                 unsigned int authsize)
{
    ...
    if ((authsize != SPU_ICV_LENGTH_96) &&
        (authsize != SPU_ICV_LENGTH_128))
    {
        return -EINVAL;
    }
    ctx->icvsize = authsize;
...
static int spu_aead_givencrypt(struct aead_givcrypt_request *req)
{
    ...
    {
       pCtx->next_hdr  = *((u8 *)sg_virt(areq->src) + areq->cryptlen - 1);
    ...
    else
    {
       pTransReq->pAllocSkb = NULL;
       pSkb   = areq->base.data;
       offset = (unsigned char *) sg_virt(areq->assoc) - pSkb->data;
    }
    ...
    if ( crypto_aead_get_flags(aead) & CRYPTO_TFM_REQ_MAY_BLOG ) {
      pCtx->ivsize = crypto_aead_ivsize(aead);
...
#else
static int spu_aead_encrypt(struct aead_request *areq)
{
    struct crypto_aead       *aead;
    struct spu_trans_req     *pTransReq;
    int                       ret;
    ...
    spu_dump_data(areq, NULL, "spu_aead_encrypt", 1);
    ...
    if (SPU_FALLBACK_METHOD_CIPHER == fbMethod)
    {
        ...
        err = spu_aead_do_fallback_cipher(pCtx->fallback_cipher, NULL, 1);
        if ( 0 == err )
        {
            spuinfo->stats.encFallback++;
            spuinfo->stats.encSpuEgress++;
    ...
    if ( pCtx->direction == SPU_DIRECTION_INVALID )
    {
       pCtx->next_hdr  = *((u8 *)(sg_virt(areq->src) + areq->assoclen + areq->cryptlen - 1));
       pCtx->direction = SPU_DIRECTION_US;
       spu_runner_descr_config(pCtx);
    }
    pTransReq->context  = areq;
    ...
    else
    {
       pTransReq->pAllocSkb = NULL;
       pSkb   = areq->base.data;
       offset = (unsigned char *) sg_virt(areq->src) - pSkb->data;
    }

    pTransReq->giv    = areq->iv;
    pTransReq->pNBuf  = SKBUFF_2_PNBUFF(pSkb);

#if defined(CONFIG_BLOG)
    if ( areq->base.flags & CRYPTO_TFM_REQ_MAY_BLOG ) {
      pCtx->ivsize = crypto_aead_ivsize(aead);
      spu_blog_emit(pTransReq);
    }
#endif
...
static int spu_aead_decrypt (struct aead_request *areq)
{
    ...
    /* flush all data */
    cache_flush_len(pSkb->data, pSkb->len);
    ret = spu_runner_process_ipsec(pTransReq, pTransReq->pNBuf, 0);
    if ( 0 == ret )
    {
        ret = -EINPROGRESS;
        pCtx->update = 0;
...
#else
static void spu_cra_exit(struct crypto_aead *tfm)
{
    struct spu_ctx *ctx = crypto_aead_ctx(tfm);
...
static int spu_cra_init (struct crypto_tfm *tfm)
{
    ...
    ctx->icvsize   = crypto_aead_authsize(__crypto_aead_cast(tfm));
...
#else
static int spu_cra_init(struct crypto_aead *tfm)
{
    struct aead_alg *alg = crypto_aead_alg(tfm);
    struct spu_crypto_alg *spuAlg;
    struct spu_ctx *ctx = crypto_aead_ctx(tfm);

    spuAlg = container_of(alg, struct spu_crypto_alg, aeadAlg);
    ctx->direction = SPU_DIRECTION_INVALID;
    ctx->update    = 1;
    ctx->descAlg   = spuAlg->descAlg;
    ctx->pDescr    = spu_runner_descr_get();
    ...
    {
        ctx->fallback_cipher = crypto_alloc_aead(alg->base.cra_name, 0,
                                                 CRYPTO_ALG_ASYNC | 
                                                 CRYPTO_ALG_NEED_FALLBACK);
        if (IS_ERR(ctx->fallback_cipher))
        {
            SPU_TRACE(("%s() Error: failed to allocate fallback for %s\n", 
                       __func__, alg->base.cra_name));
            return PTR_ERR(ctx->fallback_cipher);
        }
    }
...
static struct spu_crypto_alg *spu_alg_alloc( struct spu_alg_template *algTemplate )
{
    ...
    alg->cra_priority = SPU_CRA_AEAD_PRIORITY;
    alg->cra_alignmask = 0;
    alg->cra_ctxsize = sizeof (struct spu_ctx);
    alg->cra_flags = CRYPTO_ALG_TYPE_AEAD | CRYPTO_ALG_ASYNC | 
                     CRYPTO_ALG_NEED_FALLBACK;
#if defined(CONFIG_BLOG)
    alg->cra_flags |= CRYPTO_ALG_BLOG;
#endif
    ...
    alg = &spuAlg->aeadAlg;
    memcpy(alg, &algTemplate->alg, sizeof(struct aead_alg));
    alg->base.cra_module = THIS_MODULE;
    alg->base.cra_priority = SPU_CRA_AEAD_PRIORITY;
    alg->base.cra_ctxsize = sizeof (struct spu_ctx);
    alg->base.cra_flags = CRYPTO_ALG_TYPE_AEAD | CRYPTO_ALG_ASYNC | 
                     CRYPTO_ALG_NEED_FALLBACK;
#if defined(CONFIG_BLOG)
    alg->base.cra_flags |= CRYPTO_ALG_BLOG; 
#endif
    alg->init = spu_cra_init;
    alg->exit = spu_cra_exit;
...
static void spu_unregister(unsigned long arg)
{
    ...
    do
    {
...
#else
            if ( refcount_read(&spuAlg->aeadAlg.base.cra_refcnt) != 1 )
            {
               printk("reference to alg %s still held - spu unregister incomplete\n", spuAlg->aeadAlg.base.cra_name);
               incomplete = 1;
               break;
            }
#endif
        ...
        {
#if (LINUX_VERSION_CODE < KERNEL_VERSION(4,19,0))
            if (crypto_unregister_alg(&spuAlg->cryptoAlg))
            {
               printk("unregister alg failed %s\n", spuAlg->cryptoAlg.cra_name);
            }
#else
            crypto_unregister_aead(&spuAlg->aeadAlg);
#endif
...
#else
            err = crypto_register_aead(&spuAlg->aeadAlg);
            if (err)
            {
                printk("spu_register: alg %s registration failed, err %d\n", spuAlg->aeadAlg.base.cra_driver_name, err);
            }
            else
            {
                list_add_tail(&spuAlg->entry, &spuinfo->algList);
                SPU_TRACE(("spu_register: alg %s registered\n", spuAlg->aeadAlg.base.cra_driver_name));
            }
#endif
        }
     } while ( 0 );
