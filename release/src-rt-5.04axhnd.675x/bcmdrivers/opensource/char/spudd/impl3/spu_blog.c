...
static void spu_blog_fc_crypt_done_us(struct spu_trans_req *pTransReq)
{
   ...
   else
   {
      ...
      if ( IS_SKBUFF_PTR(pTransReq->pNBuf) )
      {
         ...
         headroom = 0;
         skb->fc_ctxt = 0;
         blogAction = blog_sinit_args(skb, spuinfo->spu_dev_us,
      ...
      else
      {
         ...
         skb = NULL;
         fkb->fc_ctxt = 0;
         blogAction = blog_finit_args(fkb, spuinfo->spu_dev_us,
                                      TYPE_IP, 0, BLOG_SPU_US, &fcArgs);
      }
...
static void spu_blog_fc_crypt_done_ds(struct spu_trans_req *pTransReq)
{
   ...
   else
   {
      ...
      if ( IS_SKBUFF_PTR(pTransReq->pNBuf) )
      {
         ...
         headroom = 0;
         skb->fc_ctxt = 0;
         blogAction = blog_sinit_args(skb, spuinfo->spu_dev_ds,
      ...
      else
      {
         ...
         skb = NULL;
         fkb->fc_ctxt = 0;
         blogAction = blog_finit_args(fkb, spuinfo->spu_dev_ds,
                                      TYPE_IP, 0, BLOG_SPU_DS, &fcArgs);
      }
