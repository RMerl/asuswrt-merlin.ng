...
extern int pdc_get_max_channel(void);
#endif
#if (defined(CONFIG_BCM_FLEXRM) || defined(CONFIG_BCM_FLEXRM_MODULE))
extern int flexrm_send_data(int chan, struct bcmspu_message *msg);
extern int flexrm_select_channel(void);
...
#if defined(CONFIG_BLOG)
/* Enable blogging for SPU by default
 * To disable blog, set blog_enable in the insmod or
 * modprobe. Changing it after init time has no effect.
 */
static int blog_enable = 1;
module_param(blog_enable, int, 0644);
 ...
 */
static int
spu_ablkcipher_rx_sg_create(struct bcmspu_message *mssg,
			    ...
			    unsigned int chunksize, u32 stat_pad_len)
{
	...
	if (0 == dma_map_sg(&iproc_priv.pdev->dev, mssg->dst, sg_nents(mssg->dst), DMA_BIDIRECTIONAL)) {
		pr_err("%s(): Failed to DMA map dst sg with %u frags",
 ...
 */
static int
spu_ablkcipher_tx_sg_create(struct bcmspu_message *mssg,
			    struct iproc_reqctx_s *rctx,
			    u8 tx_frag_num, unsigned int chunksize, u32 pad_len)
{
	...
	if ( 0 == dma_map_sg(&iproc_priv.pdev->dev, mssg->src, sg_nents(mssg->src), DMA_BIDIRECTIONAL)) {
		pr_err("%s(): Failed to DMA map dst sg with %u frags",
...
static int handle_ablkcipher_req(struct iproc_reqctx_s *rctx)
{
	...
	/* number of entries in src and dst sg in message. */
	...
	mssg->rx_callback = spu_rx_callback;
	...
	err = spu->spu_send_data(ctx->chan_idx, mssg);
	if (unlikely(err < 0)) {
		atomic_inc(&iproc_priv.chan_send_fail);
		if (err == -ENOSPC)
			atomic_inc(&iproc_priv.chan_no_spc);
		return err;
	}
 ...
 */
static int
spu_ahash_rx_sg_create(struct bcmspu_message *mssg,
		       struct iproc_reqctx_s *rctx,
 ...
 */
static int
spu_ahash_tx_sg_create(struct bcmspu_message *mssg,
		       struct iproc_reqctx_s *rctx,
		       u8 tx_frag_num,
		       u32 spu_hdr_len,
 ...
 * @rctx:  Crypto request context
 *
 * Builds a SPU request message embedded in a message and submits the
 * message on a selected channel. The SPU request message is
 ...
 *   < 0 if an error
 */
static int spu_aead_rx_sg_create(struct bcmspu_message *mssg,
				 struct iproc_reqctx_s *rctx,
				 u8 rx_frag_num,
 ...
 *   < 0 if an error
 */
static int spu_aead_tx_sg_create(struct bcmspu_message *mssg,
				 struct iproc_reqctx_s *rctx,
				 u8 tx_frag_num,
				 u32 spu_hdr_len,
...
int handle_aead_req(struct iproc_reqctx_s *rctx)
{
	...
	aead_parms.assoc_size = rctx->assoc_len;
	 ...
	 * be included in this message. For dst sg, don't count space
	...
	if (ctx->cipher.mode == CIPHER_MODE_CCM) {
		...
		spu->spu_ccm_update_iv(digestsize, &cipher_parms, rctx->assoc_len,
	...
	INIT_LIST_HEAD(&mssg->list);
	mssg->error = 0;
	err = spu_aead_rx_sg_create(mssg, rctx, rx_frag_num,
				    aead_parms.assoc_size,
...
void handle_aead_resp(struct iproc_reqctx_s *rctx)
{
	 ...
	 */
	result_len = rctx->total_todo;
	if (rctx->is_encrypt) {
		icv_offset = rctx->assoc_len + rctx->total_sent;
		packet_dump("  ICV: ", rctx->msg_buf.digest, ctx->digestsize);
		flow_log("copying ICV to dst sg at offset %u\n", icv_offset);
		sg_copy_part_from_buf(rctx->dst_sg, rctx->msg_buf.digest,
 ...
 *
 * Scatterlists are allocated for each chunk. So free them after
 * processing each chunk.
 */
static void spu_chunk_cleanup(struct iproc_reqctx_s *rctx)
{
	/* message used to tx request */
	struct bcmspu_message *mssg = &rctx->mssg;

	kfree(mssg->src);
	kfree(mssg->dst);
	memset(mssg, 0, sizeof(struct bcmspu_message));
}
...
static void finish_req(struct iproc_reqctx_s *rctx, int err)
{
	...
	if (areq) {
                /* areq->complete may invoke gro_cells_receive
                   and calling gro_cells_receive requires disabling
                   bottom half first */
                local_bh_disable();
...
/**
 * spu_rx_callback() - Callback from dma framework with a SPU response.
 * @msg:	message containing SPU response
 */
static void spu_rx_callback(struct bcmspu_message *msg)
{
	...
	if (rctx->total_sent < rctx->total_todo) {
		...
		case CRYPTO_ALG_TYPE_AEAD:
			rctx->mssg.ctx = rctx;
			rctx->mssg.rx_callback = spu_rx_callback;
...
static int aead_enqueue(struct aead_request *req, bool is_encrypt)
{
	...
	rctx->ctx = ctx;
	memset(&rctx->mssg, 0, sizeof(struct bcmspu_message));
	rctx->mssg.ctx = rctx;	/* Will be returned in response */
	rctx->mssg.rx_callback = spu_rx_callback;
	...
	flow_log("  src sg: %px\n", rctx->src_sg);
	flow_log("  rctx->src_sg: %px, src_skip %u\n",
		 rctx->src_sg, rctx->src_skip);
	flow_log("  assoc:  %px, assoclen %u\n", rctx->assoc, rctx->assoc_len);
	flow_log("  dst sg: %px\n", rctx->dst_sg);
	flow_log("  rctx->dst_sg: %px, dst_skip %u\n",
...
#if defined(CONFIG_BLOG)
	if ( blog_enable && (req->base.flags & CRYPTO_TFM_REQ_MAY_BLOG) )
	{
			    ...
			    struct iproc_alg_s *cipher_alg)
{
	...
	ctx->chan_idx = spu->spu_select_channel();
...
#if (defined(CONFIG_BCM_FLEXRM) || defined(CONFIG_BCM_FLEXRM_MODULE))
		spu->spu_ops_in_transit = flexrm_ops_in_transit;
#endif
#endif
	}
#if (defined(CONFIG_BCM_PDC) || defined(CONFIG_BCM_PDC_MODULE))
...
static void spu_counters_init(void)
{
	...
	atomic_set(&iproc_priv.chan_no_spc, 0);
	atomic_set(&iproc_priv.chan_send_fail, 0);
	atomic_set(&iproc_priv.bad_icv, 0);

#if defined(CONFIG_BLOG)
	for (i = 0; i < SPU_STREAM_MAX; i++)
		atomic_set(&iproc_priv.blogged[i], 0);
#endif
}
...
static int spu_register_aead(struct iproc_alg_s *driver_alg)
{
	...
	if ( blog_enable )
	{
		aead->base.cra_flags |= CRYPTO_ALG_BLOG;
	}
#endif
...
static int spu_dt_read(struct platform_device *pdev)
{
	...
	/* Get SPU type from device tree */
	dt_val_ptr = of_get_property(dn, "brcm,spu_type", NULL);
	if (!dt_val_ptr) {
		/* assume it's SPUM if no information given */
		spu->spu_type = SPU_TYPE_SPUM;
		spu->spu_subtype = SPU_SUBTYPE_SPUM_NS2;
		dev_err(dev,
			"%s failed to get spu_type from device tree, assuming SPUM",
			__func__);
	} else {
		if (2 == be32_to_cpup(dt_val_ptr)) {
	...
	if (!dt_val_ptr) {
		...
		spu->num_chan = be32_to_cpup(dt_val_ptr);
		dev_dbg(dev, "Device has %d SPUs",
			 spu->num_chan);
	}

	if (SPU_TYPE_SPUM == spu->spu_type) {
		for (i = 0; (i < spu->num_chan) && ((spu_ctrl_regs =
...
static int bcm_spu_probe(struct platform_device *pdev)
{
	...
	if (spu->spu_type == SPU_TYPE_SPUM)
		iproc_priv.bcm_hdr_len = PDC_HDR_LEN;
	else if (spu->spu_type == SPU_TYPE_SPU2)
...
static int __init iproc_crypto_init(void)
{
	...
	rc = platform_driver_register(&bcm_spu_pdriver);
...
{
	platform_driver_unregister(&bcm_spu_pdriver);
}
