...
typedef union {
	...
	struct {
		...
		uint32_t vdsl_bpcm_soft_reset:1;
		uint32_t vdsl_ubus_soft_bpcm_reset:1;
		uint32_t sar_bpcm_soft_reset:1;
		uint32_t arm_ubus_bpcm_soft_reset:1;
		uint32_t sar_bbh_bpcm_soft_reset:1;         /* for B0 */
		uint32_t arm_axi_bpcm_cpu_clk_soft_reset:1; /* for B0 */
		uint32_t arm_axi_bpcm_arm_clk_soft_reset:1; /* for B0 */
		uint32_t gp:24;
	} Bits_vdsl;
	uint32_t Reg32;
} BPCM_SR_CONTROL;
...
typedef struct {
	...
	BPCM_VDSL_ARM_SR_CTL vdsl_arm_sr;	// offset = 0x20, actual offset = 8
