...
typedef union {
    struct {
        uint32_t threshold_low   : 10; // [09:00]
        uint32_t reserved        :  6; // [10:15]
        uint32_t threshold_hi    : 10; // [25:16]
...
typedef union {
    struct {
        uint32_t lock_addr       :  8; // [07:00]
        uint32_t lock_bit        :  1; // [08:08]
        uint32_t lock_mode       :  1; // [09:09]
...
typedef struct {
	...
	uint32_t reserved3;	// offset 0x5c, PMB reg index 23
