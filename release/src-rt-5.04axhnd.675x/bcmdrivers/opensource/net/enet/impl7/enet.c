...
static inline int rx_pkt_from_q(int hw_q_id, int budget)
{
    ...
    do
    {
        ...
        fc_args.mcast_fwd_exception = rx_info.is_mcast_fwd_exception;
...
#define DEV_ISWAN(dev) (dev ? (is_netdev_wan((struct net_device *)dev)) : 0)
...
static int bcmenet_is_wan_port(void *ctxt)
{
    ...
    /* For PON platform, using is_netdev_wan to make decsion, with input parameter net_device*. */
    struct net_device *dev = (struct net_device *)ctxt;
    is_wan_port = is_netdev_wan(dev);
#else
    int logical_port = *((int*)ctxt);
    enetx_port_t *port = port_by_unit_port(logical_port);
    is_wan_port = (port && !bcmenet_is_bonded_lan_wan_port(ctxt)) ? PORT_ROLE_IS_WAN(port) : 0;
#endif
    return is_wan_port;
}
...
static int bcmenet_handle_bonding_change(struct net_device *slave_dev)
{
    ...
    if (!err && netif_is_bridge_port(bond_dev))
    {
...
static inline int _enet_netdev_to_rdpa_if(struct net_device *dev, bcm_netdev_priv_info_out_t *info_out)
{
    int rc = -1;
#ifdef RUNNER
    enetx_port_t *port = NETDEV_PRIV(dev)->port;
    ...
    if (!port_obj)
        return rc;
    rc = rdpa_port_index_get(port_obj, (rdpa_if *)&info_out->bcm_netdev_to_rdpa_if.rdpa_if);
#endif
    return rc;
}
...
int enet_priv_info_get(struct net_device *dev, bcm_netdev_priv_info_type_t info_type, bcm_netdev_priv_info_out_t *info_out)
{
    switch (info_type)
    {
        case BCM_NETDEV_TO_RDPA_IF:
            _enet_netdev_to_rdpa_if(dev, info_out);
            break;
        default:
            return -1;
    }
...
static int enet_netdev_event(struct notifier_block *this, unsigned long event, void *ptr)
{
    ...
    switch (event) 
    {
        case NETDEV_CHANGEUPPER:
            ...
            if (netif_is_bridge_port(dev))
            {
                struct net_device *br_dev = netdev_get_bridge_master(dev, ptr);
                if (br_dev)
...
static int has_dt(void)
{
    ...
    if ((np = of_find_compatible_node(NULL, NULL, "brcm,enet")))
    {
        ...
        {
            if (of_get_available_child_count(child) > 0)
                has_dt = 1;
        }
    }
...
#ifdef ENET_BP
        enet_err("BP mode\n");
        /* XXX: This will show errors if phy_dt_parsing also compiled in */
        phy_drivers_set();
