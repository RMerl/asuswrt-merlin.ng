...
{
    char *usage = "Usage: init <mux_map>\nMux map: String, 1 char for every port (S|I|U|L|M|H|X|G|R)\nZ-Unavailabe\n"
    ...
    for (i = 0; i <  LPORT_NUM_OF_PORTS; i++)
        ...
        case 'I': init_params.prt_mux_sel[i] = PORT_SGMII_AN_IEEE_CL37; break;
        case 'U': init_params.prt_mux_sel[i] = PORT_SGMII_AN_USER_CL37; break;
        case 'L': init_params.prt_mux_sel[i] = PORT_SGMII_AN_SLAVE; break;
        case 'M': init_params.prt_mux_sel[i] = PORT_SGMII_AN_MASTER; break;
...
static int lport_proc_cmd_status(int argc, char *argv[])
{
    ...
    lport_port_mtu_get(port, &port_mtu);
...
static int serdes_proc_cmd_tx_cfg(int argc, char *argv[])
{
    ...
    char *serdes_proc_cmd_tx_cfg_usage = "Usage: txcfg <port (0-6)> [<pre (0-10)> <main (1-60)> <post1> <post2 (0-5)> <post2to1 (0-1)> <en_pre (0-1)> <en_post1 (0-1)> <en_post2 (0-1)> <tx_refcalshunt (0-15)> <hpf (0-15)]\n\
    \n\
    ...
    if (argc == 2)
    {
        lport_serdes_get_tx_cfg(port_id, &tx_cfg);
        ...
        pr_info("en_post1       %d\n", tx_cfg.en_post1);
        pr_info("en_post2       %d\n", tx_cfg.en_post2);
        pr_info("tx_refcalshunt %d\n", tx_cfg.tx_refcalshunt);
        pr_info("hpf            %d\n", tx_cfg.hpf);
    ...
    if (kstrtou16(argv[2], 10, &tx_cfg.pre))
        goto wrong_params;

    if (kstrtou16(argv[3], 10, &tx_cfg.main))
        goto wrong_params;

    if (kstrtou16(argv[4], 10, &tx_cfg.post1))
        goto wrong_params;

    if (kstrtou16(argv[5], 10, &tx_cfg.post2))
        goto wrong_params;

    if (kstrtou16(argv[6], 10, &tx_cfg.post2to1))
        goto wrong_params;

    if (kstrtou16(argv[7], 10, &tx_cfg.en_pre))
        goto wrong_params;

    if (kstrtou16(argv[8], 10, &tx_cfg.en_post1))
        goto wrong_params;

    if (kstrtou16(argv[9], 10, &tx_cfg.en_post2))
        goto wrong_params;

    if (kstrtou16(argv[10], 10, &tx_cfg.tx_refcalshunt))
        goto wrong_params;

    if (kstrtou16(argv[11], 10, &tx_cfg.hpf))
        goto wrong_params;

    return lport_serdes_set_tx_cfg(port_id, &tx_cfg);
