...
#ifdef LPORT_REMOTE_FAULT_HANDLER
static void *tx_dis_gpio_lane_priv[4];
static void (*tx_disable_gpio_ctrl)(void *priv, int value);

void lport_serdes_tx_disable_gpio_ctrl_set(void (*_tx_disable_gpio_ctrl)(void *priv, int value))
{
    tx_disable_gpio_ctrl = _tx_disable_gpio_ctrl;
}

int lport_serdes_tx_disable_gpio_assign(int port, void *priv)
{
    E_MERLIN_LANE lane_id;

    if (port >= sizeof(port_to_lane)/sizeof(port_to_lane[0]))
        return -1;

    lane_id = port_to_lane[port];
    tx_dis_gpio_lane_priv[lane_id] = priv;
...
{
    if (!tx_disable_gpio_ctrl)
        return;

    tx_disable_gpio_ctrl(tx_dis_gpio_lane_priv[lane_id], enable ? 0 : 1);
}
...
#else
void lport_serdes_tx_disable_gpio_ctrl_set(void (*_tx_disable_gpio_ctrl)(void *priv, int value)) { };
int lport_serdes_tx_disable_gpio_assign(int port, void *priv) { return 0; };
#endif
