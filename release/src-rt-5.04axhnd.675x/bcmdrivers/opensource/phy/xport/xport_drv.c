...
static int xport_init(xport_xlmac_port_info_s *init_params)
{
...
#if defined(CONFIG_BCM94912) || defined(CONFIG_BCM96813)
    {
        ...
        switch (PID_XPORT_NUM(init_params->xport_port_id))
        {
        case 0: rc = rc ? rc : ag_drv_xport_portreset_p0_ctrl_set(xlmac_num, 0); break;
        case 1: rc = rc ? rc : ag_drv_xport_portreset_p1_ctrl_set(xlmac_num, 0); break;
        case 2: rc = rc ? rc : ag_drv_xport_portreset_p2_ctrl_set(xlmac_num, 0); break;
        case 3: rc = rc ? rc : ag_drv_xport_portreset_p3_ctrl_set(xlmac_num, 0); break;
        }
    }
#endif
    __xportNotice("rc = %d; intf = %d port = %d spd = %s dup = %d\n",
...
static int xport_platform_init(xport_xlmac_port_info_s *init_params)
{
    ...
    if (xport_num == 0)
    {
        ...
        //reset SM setting
        link_down_rst_en = 0xf; enable_sm_run = 0; tick_timer_ndiv = 0;
...
int xport_port_eee_set(uint32_t portid, uint8_t enable)
{
    ...
    uint32_t bbhtx_id = XPORT_PORT_ID_2_BBHTX_ID(portid);
    ...
    if (rc == 0)
    {
        // BBH WAN register
#if defined(CONFIG_BCM94912) || defined(CONFIG_BCM96813)
        WANBBH->txBlk[bbhtx_id].unified_config.eee = enable ? 0x3f : 0;
#else
        WANBBH->txBlk[bbhtx_id].lan_config.eee = enable;
#endif
