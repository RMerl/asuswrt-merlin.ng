...
int xport_stats_cfg_set(uint32_t portid, xport_stats_cfg_s *stats_cfg)
{
    ...
    ag_drv_xport_mib_reg_control_get(xlmac_num, &eee_cnt_mode, &saturate_en, &cor_en, &cnt_rst);

    if(stats_cfg->saturate_en)
        saturate_en |= (1 << xport_num);
    else
        saturate_en &= ~(1 << xport_num);
    ...
    return ag_drv_xport_mib_reg_control_set(xlmac_num, eee_cnt_mode, saturate_en, cor_en, cnt_rst);
}
...
int xport_stats_cfg_get(uint32_t portid, xport_stats_cfg_s *stats_cfg)
{
    ...
    ag_drv_xport_mib_reg_control_get(xlmac_num, &eee_sym_mode, &saturate_en, &cor_en, &cnt_rst);

    stats_cfg->saturate_en = (saturate_en & (1 << xport_num)) != 0;
    stats_cfg->eee_sym_mode = (eee_sym_mode & (1 << xport_num)) != 0;
    stats_cfg->cor_en =( cor_en & (1 << xport_num)) != 0;
...
int xport_stats_reset(uint32_t portid)
{
    ...
    ag_drv_xport_mib_reg_control_set(xlmac_num, eee_cnt_mode, saturate_en, cor_en, cnt_rst );
