...
/* Index. Signed to allow special values. Long enough to enable
 * casting to pointer */
...
{
	return (!mac->b[0] && !mac->b[1] && !mac->b[2] &&
...
static inline int bdmf_ipv6_is_zero(const bdmf_ipv6_t *ipv6)
{
	...
	return (!ipv6_as_int[0] && !ipv6_as_int[1] &&
		!ipv6_as_int[2] && !ipv6_as_int[3]);
}
...
static inline int bdmf_ip_cmp(const bdmf_ip_t *ip, const bdmf_ip_t *ip2)
{
	...
	return (!(ipv6_as_int[0] == ipv6_as_int2[0] &&
		ipv6_as_int[1] == ipv6_as_int2[1] &&
		ipv6_as_int[2] == ipv6_as_int2[2] &&
		ipv6_as_int[3] == ipv6_as_int2[3]));
}
