...
#include <malloc.h>
#define CHAR_BIT	8
extern void bcm_sec_abort(void);
#define abort	bcm_sec_abort
...
gmp_default_alloc (size_t size)
{
  ...
  *(addr_t *)p = (addr_t)p+size;
  return (void*)((addr_t *)p + 1);
}
...
{
  void * p;
  addr_t *_blk = (addr_t*)old - 1;
   
  p = gmp_default_alloc (new_size);

  if (!p)
    gmp_die("gmp_default_realloc: Virtual memory exhausted.");
  /*debug("reallocating old size 0x%x new size 0x%x\n", (*_blk - (addr_t)_blk), new_size);*/
  memcpy(p, old, *_blk - (addr_t)_blk);

  gmp_default_free(old, 0);	
