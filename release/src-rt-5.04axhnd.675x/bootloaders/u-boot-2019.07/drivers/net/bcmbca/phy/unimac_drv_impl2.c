...
uintptr_t unimac_conf_base;
uintptr_t unimac_mib_base;
uintptr_t unimac_top_base;
...
static int unimac_probe(dt_device_t *pdev)
{
    ...
    if (mib_step == (uint32_t)(-1))
    {
        ret = -1;
        dev_err(&pdev->dev, "Missing mib_step entry\n");
        goto Exit;
    }

    dev_dbg(&pdev->dev, "unimac_conf_base=0x%lx\n", unimac_conf_base);
    dev_dbg(&pdev->dev, "unimac_mib_base=0x%lx\n", unimac_mib_base);
    dev_dbg(&pdev->dev, "unimac_top_base=0x%lx\n", unimac_top_base);
    dev_dbg(&pdev->dev, "conf_offset=0x%x\n", conf_offset);
    dev_dbg(&pdev->dev, "mib_offset=0x%x\n", mib_offset);
    dev_dbg(&pdev->dev, "top_offset=0x%x\n", top_offset);
    dev_dbg(&pdev->dev, "mib_step=0x%x\n", mib_step);
