...
#define bcm_seq_printf      seq_printf
...
#define bcm_pr_cont(fmt, ...)  bcm_printk(KERN_CONT fmt, ##__VA_ARGS__)
#define bcm_print(fmt, ...)  bcm_pr_cont(fmt, ##__VA_ARGS__)
#define bcm_pr_hex_dump_offset(r, g, b, l, a) \
        print_hex_dump(KERN_INFO, "", DUMP_PREFIX_OFFSET, r, g, b, l, a)
void bcm_seq_printf(struct seq_file *s, const char *fmt, ...);
#endif
