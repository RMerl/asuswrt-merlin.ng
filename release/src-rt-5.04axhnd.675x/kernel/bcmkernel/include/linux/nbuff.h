                                   ...
                                   uint8_t * pData, uint32_t len)
{
    ...
    fkb_dbg( 1, "fkb_p<%px> pBuf<%px> headroom<%u> pData<%px> len<%d>",
 ...
 *
 */
static inline FkBuff_t * _fkb_clone(FkBuff_t * fkb_p)
{
    FkBuff_t *fkbM_p;
    ...
    fkbM_p = _get_fkb_master_ptr_( fkb_p );

    if ( !is_dptr_tag_(fkbM_p->dirty_p ) && _is_in_skb_tag_(fkbM_p->ptr, fkbM_p->flags) )
    {
        /* FKB Alloc and copy */
        fkbM_p = fkb_alloc( FkbMaster_e );
        if (fkbM_p == FKB_NULL)
        {
            fkb_dbg(1, "fkb_clone FKB fkb_alloc failure");
            return FKB_NULL;
        }
        fkb_set_ref(fkbM_p, 1);

        fkbM_p->data = PFKBUFF_TO_PDATA(fkbM_p, BCM_PKT_HEADROOM);
        ...
        fkb_dbg(1, "fkbM_p<%p> ---> fkb_p<%p>", fkbM_p, fkb_p );

        return fkbM_p;
    }
    else
    {
        FkBuff_t *fkbC_p = fkb_alloc( FkbCloned_e );  /* Allocate FKB from Cloned pool */
