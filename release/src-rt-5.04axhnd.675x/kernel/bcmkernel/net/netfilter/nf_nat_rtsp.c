				...
				int matchoff, int matchlen, int *delta)
{
	...
	char buf2[sizeof("ffff:ffff:ffff:ffff:ffff:ffff:ffff:ffff")];
	int dir = CTINFO2DIR(ctinfo);
	int len;
	int ret;
	...
	if (nf_ct_l3num(ct) == AF_INET6) {
		len = sprintf(buf2, "%pI6",
		       ct->tuplehash[!dir].tuple.dst.u3.ip6);

		ret = rtsp_mangle_tcp_packet(skb, ct, ctinfo, protoff, matchoff, matchlen,
							  buf2, len);
	}
	else {
		len = sprintf(buf, "%pI4",
