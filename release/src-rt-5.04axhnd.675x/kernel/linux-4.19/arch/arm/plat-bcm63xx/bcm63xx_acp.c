		...
		bcm_acp_ubus_cfg_t *acp_ubus_cfg)
{
	...
	ret = ubus_cfg_entry_set(acp_cfg_tbl[ubus_pid].pmb_addr, idx, acp_ubus_cfg->addr_in,
...
void bcm63xx_acp_ubus_cfg_reset(uint8_t ubus_pid)
{
	...
	for (i = 0; i < 4; i++) {
		ubus_cfg_entry_set(acp_cfg_tbl[ubus_pid].pmb_addr, i, 0, 0, 0, 0, 0);
...
int bcm63xx_acp_enable(uint8_t ubus_pid)
{
	...
	for (i = 0; i < 4; i++) {
		ret |= ubus_cfg_entry_set(acp_cfg_tbl[ubus_pid].pmb_addr, i,
...
static ssize_t write_proc_addr_in(struct file *f, const char __user *buffer, size_t count, loff_t *off)
{
	...
	ret = ubus_cfg_entry_set(acp_cfg_tbl[ubus_pid].pmb_addr, cfg_id, (uint32_t)input_val,
...
static ssize_t write_proc_addr_out(struct file *f, const char __user *buffer, size_t count, loff_t *off)
{
	...
	ret = ubus_cfg_entry_set(acp_cfg_tbl[ubus_pid].pmb_addr, cfg_id,
