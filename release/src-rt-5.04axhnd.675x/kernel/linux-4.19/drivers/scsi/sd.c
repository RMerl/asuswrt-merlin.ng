...
#if defined(CONFIG_BCM_KF_SCSI_WRITE_PROTECT) && defined(CONFIG_BCM_SCSI_WRITE_PROTECT)
static ssize_t
write_protect_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct scsi_disk *sdkp = to_scsi_disk(dev);

	return sprintf(buf, "%u\n", sdkp->write_prot);
}
static DEVICE_ATTR_RO(write_protect);
#endif
