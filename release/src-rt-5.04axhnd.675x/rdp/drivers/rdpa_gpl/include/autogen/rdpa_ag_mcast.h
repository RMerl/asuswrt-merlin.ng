...
typedef enum {
    ...
    rdpa_mcast_attr_key_exclude_fields = 5, /* key_exclude_fields : W : enum_mask : List of fields to exclude from basic mcast class key, filter applies both to vlan0, vlan1 */
...
/** Get next mcast/flow attribute entry.
 *
 * Get next Multicast flow entry.
 * \param[in]   mo_ mcast object handle or mattr transaction handle
 * \param[in,out]   ai_ Attribute array index
 * \return 0 or error code < 0
 * The function can be called in task and softirq contexts.
 */
static inline int rdpa_mcast_flow_get_next(bdmf_object_handle mo_, bdmf_index * ai_)
{
    return bdmf_attrelem_get_next(mo_, rdpa_mcast_attr_flow, (bdmf_index *)ai_);
}
...
/** Get next mcast/flow_stat attribute entry.
 *
 * Get next Multicast flow entry statistics.
 * \param[in]   mo_ mcast object handle or mattr transaction handle
 * \param[in,out]   ai_ Attribute array index
 * \return 0 or error code < 0
 * The function can be called in task and softirq contexts.
 */
static inline int rdpa_mcast_flow_stat_get_next(bdmf_object_handle mo_, bdmf_index * ai_)
{
    return bdmf_attrelem_get_next(mo_, rdpa_mcast_attr_flow_stat, (bdmf_index *)ai_);
}


/** Set mcast/key_exclude_fields attribute.
 *
 ...
 * The function can be called in task context only.
 */
static inline int rdpa_mcast_key_exclude_fields_set(bdmf_object_handle mo_, uint32_t key_exclude_fields_)
{
    return bdmf_attr_set_as_num(mo_, rdpa_mcast_attr_key_exclude_fields, key_exclude_fields_);
}
