...
void dump_RDD_MCAST_LKP_WORD3_CRC(bdmf_session_handle session, unsigned char *p)
{
	...
	FIELD_MREAD_8((uint8_t *)p + 6, 4, 2, r);
	bdmf_session_print(session, "\tnum_of_vlans             = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	FIELD_MREAD_8((uint8_t *)p + 6, 0, 4, r);
...
void dump_RDD_FC_UCAST_FLOW_CONTEXT_WFD_NIC_ENTRY(bdmf_session_handle session, unsigned char *p)
{
	...
	FIELD_MREAD_16((uint8_t *)p + 12, 1, 15, r);
	bdmf_session_print(session, "\treserved4                = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");
...
void dump_RDD_FC_MCAST_FLOW_CONTEXT_ENTRY(bdmf_session_handle session, unsigned char *p)
{
	...
	FIELD_MREAD_8((uint8_t *)p + 129, 1, 2, r);
	bdmf_session_print(session, "\treserved5                = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");
...
void dump_RDD_FC_MCAST_MASTER_FLOW_CONTEXT_ENTRY(bdmf_session_handle session, unsigned char *p)
{
	...
	FIELD_MREAD_8((uint8_t *)p + 8, 1, 5, r);
	bdmf_session_print(session, "\tservice_queue_id         = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	FIELD_MREAD_8((uint8_t *)p + 8, 0, 1, r);
	bdmf_session_print(session, "\tfwd_and_trap             = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	FIELD_MREAD_32((uint8_t *)p + 8, 13, 11, r);
	bdmf_session_print(session, "\tmtu                      = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	FIELD_MREAD_8((uint8_t *)p + 10, 4, 1, r);
	bdmf_session_print(session, "\treserved1                = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	FIELD_MREAD_16((uint8_t *)p + 10, 4, 8, r);
	bdmf_session_print(session, "\tpathstat_idx             = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	FIELD_MREAD_8((uint8_t *)p + 11, 0, 4, r);
	bdmf_session_print(session, "\treserved2                = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");
	...
	FIELD_MREAD_8((uint8_t *)p + 13, 0, 3, r);
...
void dump_RDD_FC_NATC_MCAST_MASTER_FLOW_CONTEXT_ENTRY(bdmf_session_handle session, unsigned char *p)
{
	...
	{ uint32_t temp; FIELD_MREAD_32(((uint8_t *)p + 56), 0, 8, temp); r = temp << 8; FIELD_MREAD_32(((uint8_t *)p + 60), 24, 8, temp); r = r | temp; };
	bdmf_session_print(session, "\tcontext_continuation_table_index= 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	FIELD_MREAD_32((uint8_t *)p + 60, 8, 16, r);
	bdmf_session_print(session, "\tcommand_list_remaining_length= 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");
