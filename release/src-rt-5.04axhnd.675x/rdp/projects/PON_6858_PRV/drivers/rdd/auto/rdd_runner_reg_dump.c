...
{
	unsigned int r;
	bdmf_session_print(session, "  Register CODEL_QUEUE_DESCRIPTOR\n");
...
{
	unsigned int r;
	bdmf_session_print(session, "  Register CODEL_SQ_ENTRY\n");
...
void dump_RDD_RING_CPU_TX_DESCRIPTOR(bdmf_session_handle session, unsigned char *p)
{
	...
	FIELD_MREAD_8((uint8_t *)(p) + 10, 2, 6, (r));
...
void dump_RDD_POLICER_PARAMS_ENTRY(bdmf_session_handle session, unsigned char *p)
{
	...
	FIELD_MREAD_8((uint8_t *)(p) + 1, 4, 3, (r));
...
void dump_RDD_SYSTEM_CONFIGURATION_ENTRY(bdmf_session_handle session, unsigned char *p)
{
	...
	bdmf_session_print(session, "\tg9991_skip_parser_error  = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");
