...
int getScratchPadFile(const char *fname, char** ppBuf, UINT32 *bufLen)
{
    ...
    {
       if (statbuf.st_size != SP_MAX_LEN)
       {
          // file exists, but is wrong size (don't known when this actually
          // happens).  Delete it and start a new one.
          bcmuLog_error("scratchpad file is wrong size, delete it!");
          rc = unlink(fname);
          if (rc < 0)
          {
             bcmuLog_error("delete of %s failed, errno=%d", fname, errno);
             return -1;
          }
          newfile = 1;
       }
       else if ((statbuf.st_mode & 07777) != mode)
       {
          // file exists, is the right size, but wrong file mode (see inode 7).
          // fix it.
          bcmuLog_error("scratchpad file has wrong permissions, change it!");
          rc = chmod(fname, mode);
          if (rc < 0)
          {
             bcmuLog_error("chmod of %s failed, errno=%d", fname, errno);
             // wrong perms is not fatal, so just continue.
          }
       }
    }
    ...
    if (newfile)
    {
        printf("Creating new scratchpad file.\n");
    ...
    *ppBuf = malloc( *bufLen);
    ...
    {
        // This is a new file, so write all 0's into it and also return all
        // 0's as the data.
...
SINT32 devCtl_flashReadFile(const char *fname, char *buf,  UINT32 bufLen)
{
   ...
   if (rc != 0)
   {
       bcmuLog_debug("file %s not exist, stat rc=%d", fname, rc);
...
SINT32 devCtl_flashWriteFile(const char *fname, char *buf,  UINT32 bufLen)
{
   ...
   // Delete the existing file with possible bad mode bits.
   // The file might not be there, so don't treat rc < 0 as an error.
   rc = unlink(fname);
   bcmuLog_debug("unlink of %s returned rc=%d", fname, rc);
...
UINT64 devCtl_getSdramSize(void)
{
   ...
   ret = BCMRET_SUCCESS;
#else
   /* SDRAM size in MB from board driver */
   ret = devCtl_boardDriverIoctl(BOARD_IOCTL_GET_SDRAM_SIZE,
                           0, NULL,
                           0, 0, &size);
#endif
   return (ret == BCMRET_SUCCESS ? ((UINT64)size) << 20: 0);
}
                         ...
                         void *data)
{
    ...
    if ( boardFd != -1 )
    {
        ...
        /* Make sure our strlen is correct for SCRATCHPAD READ/WRITES */
        if (((boardIoctl == BOARD_IOCTL_FLASH_READ)  && (ioctlParms.action == SCRATCH_PAD)) ||
            ((boardIoctl == BOARD_IOCTL_FLASH_WRITE) && (ioctlParms.action == SCRATCH_PAD)) )
        {
            if( ioctlParms.string && strlen(ioctlParms.string) && (ioctlParms.strLen == 0) )
                ioctlParms.strLen = strlen(ioctlParms.string);
        }
