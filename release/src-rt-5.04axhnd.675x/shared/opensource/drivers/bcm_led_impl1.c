...
void bcm_common_led_init(void) {
    ...
    /* 63138 A0/A1 use old led controller*/
    if ( (CHIP_FAMILY_ID_HEX == 0x63138) && ((UtilGetChipRev()&0xf0) == 0xa0) ) {
        old_led_ctrl = 1;
        LedData = &LED->mask;
    } else {
        old_led_ctrl = 0;
...
static int bcm_common_led_linux_init(void)
{
    ...
    if(CHIP_FAMILY_ID_HEX == 0x4912 || CHIP_FAMILY_ID_HEX == 0x6813 || CHIP_FAMILY_ID_HEX == 0x63146)
        return 0;
