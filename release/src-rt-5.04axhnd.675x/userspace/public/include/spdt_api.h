 ...
 * Args:
 *   stream_idx
 *   mode - "SPDT_STATS_GET_BLOCKING_END_OF_TEST" blocking function call till end of test
 *          "SPDT_STATS_GET_CURRENT" return current statistics
 *   stat - statistics structure, will be filled according to speed test protocol the stream used during connection
 * Return: 0 - on success, other on failure
 * */
typedef enum {
    SPDT_STATS_GET_CURRENT = 0,
    SPDT_STATS_GET_BLOCKING_END_OF_TEST = 1
}spdt_stats_get_mode_t;
