...
bdmf_error_t ag_drv_xif_1588_ts_sync_offset_set(bdmf_boolean cfg_ts48_sync_ns_increment, uint16_t cfgtssyncoffset_161, uint16_t cfgtssyncoffset_125)
{
...
bdmf_error_t ag_drv_xif_1588_ts_sync_offset_get(bdmf_boolean *cfg_ts48_sync_ns_increment, uint16_t *cfgtssyncoffset_161, uint16_t *cfgtssyncoffset_125)
{
    uint32_t reg_1588_ts_sync_offset=0;

#ifdef VALIDATE_PARMS
    if(!cfg_ts48_sync_ns_increment || !cfgtssyncoffset_161 || !cfgtssyncoffset_125)
    {
        bdmf_trace("ERROR driver %s:%u| err=%s (%d)\n", __FILE__, __LINE__, bdmf_strerror(BDMF_ERR_PARM), BDMF_ERR_PARM);
        return BDMF_ERR_PARM;
    }
...
static int bcm_xif_cli_set(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    ...
    case BDMF_1588_ts_sync_offset:
        err = ag_drv_xif_1588_ts_sync_offset_set(parm[1].value.unumber, parm[2].value.unumber, parm[3].value.unumber);
        break;
...
static int bcm_xif_cli_get(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    ...
    case BDMF_1588_ts_sync_offset:
    {
        ...
        err = ag_drv_xif_1588_ts_sync_offset_get(&cfg_ts48_sync_ns_increment, &cfgtssyncoffset_161, &cfgtssyncoffset_125);
...
static int bcm_xif_cli_test(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    ...
    }
    {
        ...
        if(!err) bdmf_session_print(session, "ag_drv_xif_1588_ts_sync_offset_set( %u %u %u)\n", cfg_ts48_sync_ns_increment, cfgtssyncoffset_161, cfgtssyncoffset_125);
        if(!err) ag_drv_xif_1588_ts_sync_offset_set(cfg_ts48_sync_ns_increment, cfgtssyncoffset_161, cfgtssyncoffset_125);
        if(!err) ag_drv_xif_1588_ts_sync_offset_get( &cfg_ts48_sync_ns_increment, &cfgtssyncoffset_161, &cfgtssyncoffset_125);
        if(!err) bdmf_session_print(session, "ag_drv_xif_1588_ts_sync_offset_get( %u %u %u)\n", cfg_ts48_sync_ns_increment, cfgtssyncoffset_161, cfgtssyncoffset_125);
        if(err || cfg_ts48_sync_ns_increment!=gtmv(m, 1) || cfgtssyncoffset_161!=gtmv(m, 9) || cfgtssyncoffset_125!=gtmv(m, 9))
            return err ? err : BDMF_ERR_IO;
    }
    {
