...
bdmf_error_t ag_drv_xpcsrx_rx_fec_ctl_set(const xpcsrx_rx_fec_ctl *rx_fec_ctl)
{
    ...
    if((rx_fec_ctl->cfgxpcsrxidleinvldresen >= _1BITS_MAX_VAL_) ||
       (rx_fec_ctl->cfgxpcsrxfecinvldresen >= _1BITS_MAX_VAL_) ||
       (rx_fec_ctl->cfgxpcsrxfecstoponerr >= _1BITS_MAX_VAL_) ||
    ...
    reg_rx_fec_ctl = RU_FIELD_SET(0, XPCSRX, RX_FEC_CTL, CFGXPCSRXIDLEINVLDRESEN, reg_rx_fec_ctl, rx_fec_ctl->cfgxpcsrxidleinvldresen);
    reg_rx_fec_ctl = RU_FIELD_SET(0, XPCSRX, RX_FEC_CTL, CFGXPCSRXFECINVLDRESEN, reg_rx_fec_ctl, rx_fec_ctl->cfgxpcsrxfecinvldresen);
...
bdmf_error_t ag_drv_xpcsrx_rx_fec_ctl_get(xpcsrx_rx_fec_ctl *rx_fec_ctl)
{
    ...
    rx_fec_ctl->cfgxpcsrxidleinvldresen = RU_FIELD_GET(0, XPCSRX, RX_FEC_CTL, CFGXPCSRXIDLEINVLDRESEN, reg_rx_fec_ctl);
    rx_fec_ctl->cfgxpcsrxfecinvldresen = RU_FIELD_GET(0, XPCSRX, RX_FEC_CTL, CFGXPCSRXFECINVLDRESEN, reg_rx_fec_ctl);
...
bdmf_error_t ag_drv_xpcsrx_rx_test_ctl_set(uint8_t cfgxpcsrxtestpsudoham, bdmf_boolean cfgxpcsrxtestprbsdeten, bdmf_boolean cfgxpcsrxtestpsudodeten)
{
    uint32_t reg_rx_test_ctl=0;

#ifdef VALIDATE_PARMS
    if((cfgxpcsrxtestpsudoham >= _4BITS_MAX_VAL_) ||
       (cfgxpcsrxtestprbsdeten >= _1BITS_MAX_VAL_) ||
    ...
    reg_rx_test_ctl = RU_FIELD_SET(0, XPCSRX, RX_TEST_CTL, CFGXPCSRXTESTPSUDOHAM, reg_rx_test_ctl, cfgxpcsrxtestpsudoham);
...
bdmf_error_t ag_drv_xpcsrx_rx_test_ctl_get(uint8_t *cfgxpcsrxtestpsudoham, bdmf_boolean *cfgxpcsrxtestprbsdeten, bdmf_boolean *cfgxpcsrxtestpsudodeten)
{
    uint32_t reg_rx_test_ctl=0;

#ifdef VALIDATE_PARMS
    if(!cfgxpcsrxtestpsudoham || !cfgxpcsrxtestprbsdeten || !cfgxpcsrxtestpsudodeten)
    {
        bdmf_trace("ERROR driver %s:%u| err=%s (%d)\n", __FILE__, __LINE__, bdmf_strerror(BDMF_ERR_PARM), BDMF_ERR_PARM);
        return BDMF_ERR_PARM;
    }
    ...
    *cfgxpcsrxtestpsudoham = RU_FIELD_GET(0, XPCSRX, RX_TEST_CTL, CFGXPCSRXTESTPSUDOHAM, reg_rx_test_ctl);
...
bdmf_error_t ag_drv_xpcsrx_rx_test_psudo_pat_sh_set(uint8_t cfgxpcsrxtestpsudodetpatsh)
{
    uint32_t reg_rx_test_psudo_pat_sh=0;

#ifdef VALIDATE_PARMS
    if((cfgxpcsrxtestpsudodetpatsh >= _2BITS_MAX_VAL_))
    {
        bdmf_trace("ERROR driver %s:%u| err=%s (%d)\n", __FILE__, __LINE__, bdmf_strerror(BDMF_ERR_RANGE), BDMF_ERR_RANGE);
        return BDMF_ERR_RANGE;
    }
    ...
    reg_rx_test_psudo_pat_sh = RU_FIELD_SET(0, XPCSRX, RX_TEST_PSUDO_PAT_SH, CFGXPCSRXTESTPSUDODETPATSH, reg_rx_test_psudo_pat_sh, cfgxpcsrxtestpsudodetpatsh);

    RU_REG_WRITE(0, XPCSRX, RX_TEST_PSUDO_PAT_SH, reg_rx_test_psudo_pat_sh);

    return BDMF_ERR_OK;
}

bdmf_error_t ag_drv_xpcsrx_rx_test_psudo_pat_sh_get(uint8_t *cfgxpcsrxtestpsudodetpatsh)
{
    uint32_t reg_rx_test_psudo_pat_sh=0;

#ifdef VALIDATE_PARMS
    if(!cfgxpcsrxtestpsudodetpatsh)
    {
        bdmf_trace("ERROR driver %s:%u| err=%s (%d)\n", __FILE__, __LINE__, bdmf_strerror(BDMF_ERR_PARM), BDMF_ERR_PARM);
        return BDMF_ERR_PARM;
    }
    ...
    RU_REG_READ(0, XPCSRX, RX_TEST_PSUDO_PAT_SH, reg_rx_test_psudo_pat_sh);

    *cfgxpcsrxtestpsudodetpatsh = RU_FIELD_GET(0, XPCSRX, RX_TEST_PSUDO_PAT_SH, CFGXPCSRXTESTPSUDODETPATSH, reg_rx_test_psudo_pat_sh);

    return BDMF_ERR_OK;
}

bdmf_error_t ag_drv_xpcsrx_rx_test_psudo_pat1_low_set(uint32_t cfgxpcsrxtestpsudodetpat1low)
{
    uint32_t reg_rx_test_psudo_pat1_low=0;

#ifdef VALIDATE_PARMS
#endif

    reg_rx_test_psudo_pat1_low = RU_FIELD_SET(0, XPCSRX, RX_TEST_PSUDO_PAT1_LOW, CFGXPCSRXTESTPSUDODETPAT1LOW, reg_rx_test_psudo_pat1_low, cfgxpcsrxtestpsudodetpat1low);

    RU_REG_WRITE(0, XPCSRX, RX_TEST_PSUDO_PAT1_LOW, reg_rx_test_psudo_pat1_low);

    return BDMF_ERR_OK;
}

bdmf_error_t ag_drv_xpcsrx_rx_test_psudo_pat1_low_get(uint32_t *cfgxpcsrxtestpsudodetpat1low)
{
    uint32_t reg_rx_test_psudo_pat1_low=0;

#ifdef VALIDATE_PARMS
    if(!cfgxpcsrxtestpsudodetpat1low)
    {
        bdmf_trace("ERROR driver %s:%u| err=%s (%d)\n", __FILE__, __LINE__, bdmf_strerror(BDMF_ERR_PARM), BDMF_ERR_PARM);
        return BDMF_ERR_PARM;
    }
    ...
    RU_REG_READ(0, XPCSRX, RX_TEST_PSUDO_PAT1_LOW, reg_rx_test_psudo_pat1_low);

    *cfgxpcsrxtestpsudodetpat1low = RU_FIELD_GET(0, XPCSRX, RX_TEST_PSUDO_PAT1_LOW, CFGXPCSRXTESTPSUDODETPAT1LOW, reg_rx_test_psudo_pat1_low);

    return BDMF_ERR_OK;
}

bdmf_error_t ag_drv_xpcsrx_rx_test_psudo_pat1_high_set(uint32_t cfgxpcsrxtestpsudodetpat1high)
{
    uint32_t reg_rx_test_psudo_pat1_high=0;

#ifdef VALIDATE_PARMS
#endif

    reg_rx_test_psudo_pat1_high = RU_FIELD_SET(0, XPCSRX, RX_TEST_PSUDO_PAT1_HIGH, CFGXPCSRXTESTPSUDODETPAT1HIGH, reg_rx_test_psudo_pat1_high, cfgxpcsrxtestpsudodetpat1high);

    RU_REG_WRITE(0, XPCSRX, RX_TEST_PSUDO_PAT1_HIGH, reg_rx_test_psudo_pat1_high);

    return BDMF_ERR_OK;
}

bdmf_error_t ag_drv_xpcsrx_rx_test_psudo_pat1_high_get(uint32_t *cfgxpcsrxtestpsudodetpat1high)
{
    uint32_t reg_rx_test_psudo_pat1_high=0;

#ifdef VALIDATE_PARMS
    if(!cfgxpcsrxtestpsudodetpat1high)
    {
        bdmf_trace("ERROR driver %s:%u| err=%s (%d)\n", __FILE__, __LINE__, bdmf_strerror(BDMF_ERR_PARM), BDMF_ERR_PARM);
        return BDMF_ERR_PARM;
    }
    ...
    RU_REG_READ(0, XPCSRX, RX_TEST_PSUDO_PAT1_HIGH, reg_rx_test_psudo_pat1_high);

    *cfgxpcsrxtestpsudodetpat1high = RU_FIELD_GET(0, XPCSRX, RX_TEST_PSUDO_PAT1_HIGH, CFGXPCSRXTESTPSUDODETPAT1HIGH, reg_rx_test_psudo_pat1_high);

    return BDMF_ERR_OK;
}

bdmf_error_t ag_drv_xpcsrx_rx_test_psudo_pat2_low_set(uint32_t cfgxpcsrxtestpsudodetpat2low)
{
    uint32_t reg_rx_test_psudo_pat2_low=0;

#ifdef VALIDATE_PARMS
#endif

    reg_rx_test_psudo_pat2_low = RU_FIELD_SET(0, XPCSRX, RX_TEST_PSUDO_PAT2_LOW, CFGXPCSRXTESTPSUDODETPAT2LOW, reg_rx_test_psudo_pat2_low, cfgxpcsrxtestpsudodetpat2low);

    RU_REG_WRITE(0, XPCSRX, RX_TEST_PSUDO_PAT2_LOW, reg_rx_test_psudo_pat2_low);

    return BDMF_ERR_OK;
}

bdmf_error_t ag_drv_xpcsrx_rx_test_psudo_pat2_low_get(uint32_t *cfgxpcsrxtestpsudodetpat2low)
{
    uint32_t reg_rx_test_psudo_pat2_low=0;

#ifdef VALIDATE_PARMS
    if(!cfgxpcsrxtestpsudodetpat2low)
    {
        bdmf_trace("ERROR driver %s:%u| err=%s (%d)\n", __FILE__, __LINE__, bdmf_strerror(BDMF_ERR_PARM), BDMF_ERR_PARM);
        return BDMF_ERR_PARM;
    }
    ...
    RU_REG_READ(0, XPCSRX, RX_TEST_PSUDO_PAT2_LOW, reg_rx_test_psudo_pat2_low);

    *cfgxpcsrxtestpsudodetpat2low = RU_FIELD_GET(0, XPCSRX, RX_TEST_PSUDO_PAT2_LOW, CFGXPCSRXTESTPSUDODETPAT2LOW, reg_rx_test_psudo_pat2_low);

    return BDMF_ERR_OK;
}

bdmf_error_t ag_drv_xpcsrx_rx_test_psudo_pat2_high_set(uint32_t cfgxpcsrxtestpsudodetpat2high)
{
    uint32_t reg_rx_test_psudo_pat2_high=0;

#ifdef VALIDATE_PARMS
#endif

    reg_rx_test_psudo_pat2_high = RU_FIELD_SET(0, XPCSRX, RX_TEST_PSUDO_PAT2_HIGH, CFGXPCSRXTESTPSUDODETPAT2HIGH, reg_rx_test_psudo_pat2_high, cfgxpcsrxtestpsudodetpat2high);

    RU_REG_WRITE(0, XPCSRX, RX_TEST_PSUDO_PAT2_HIGH, reg_rx_test_psudo_pat2_high);

    return BDMF_ERR_OK;
}

bdmf_error_t ag_drv_xpcsrx_rx_test_psudo_pat2_high_get(uint32_t *cfgxpcsrxtestpsudodetpat2high)
{
    uint32_t reg_rx_test_psudo_pat2_high=0;

#ifdef VALIDATE_PARMS
    if(!cfgxpcsrxtestpsudodetpat2high)
    {
        bdmf_trace("ERROR driver %s:%u| err=%s (%d)\n", __FILE__, __LINE__, bdmf_strerror(BDMF_ERR_PARM), BDMF_ERR_PARM);
        return BDMF_ERR_PARM;
    }
    ...
    RU_REG_READ(0, XPCSRX, RX_TEST_PSUDO_PAT2_HIGH, reg_rx_test_psudo_pat2_high);

    *cfgxpcsrxtestpsudodetpat2high = RU_FIELD_GET(0, XPCSRX, RX_TEST_PSUDO_PAT2_HIGH, CFGXPCSRXTESTPSUDODETPAT2HIGH, reg_rx_test_psudo_pat2_high);
...
enum
{
    ...
    BDMF_rx_int_msk1,
    BDMF_rx_spare_ctl,
    BDMF_rx_test_psudo_pat_sh,
    BDMF_rx_test_psudo_pat1_low,
    BDMF_rx_test_psudo_pat1_high,
    BDMF_rx_test_psudo_pat2_low,
    BDMF_rx_test_psudo_pat2_high,
};
...
typedef enum
{
    ...
    bdmf_address_rx_test_psudo_pat_sh,
    bdmf_address_rx_test_psudo_pat1_low,
    bdmf_address_rx_test_psudo_pat1_high,
    bdmf_address_rx_test_psudo_pat2_low,
    bdmf_address_rx_test_psudo_pat2_high,
}
bdmf_address;
...
static int bcm_xpcsrx_cli_set(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    ...
    {
        xpcsrx_rx_fec_ctl rx_fec_ctl = { .cfgxpcsrxidleinvldresen=parm[1].value.unumber, .cfgxpcsrxfecinvldresen=parm[2].value.unumber, .cfgxpcsrxfecstoponerr=parm[3].value.unumber, .cfgxpcsrxfeccntnquecw=parm[4].value.unumber, .cfgxpcsrxfecnquerst=parm[5].value.unumber, .cfgxpcsrxfeconezeromode=parm[6].value.unumber, .cfgxpcsrxfecblkcorrect=parm[7].value.unumber, .cfgxpcsrxfecnquetestpat=parm[8].value.unumber, .cfgxpcsrxfecfailblksh0=parm[9].value.unumber, .cfgxpcsrxfecstrip=parm[10].value.unumber, .cfgxpcsrxfecbypas=parm[11].value.unumber, .cfgxpcsrxfecidleins=parm[12].value.unumber, .cfgxpcsrxfecen=parm[13].value.unumber};
    ...
    case BDMF_rx_test_ctl:
        err = ag_drv_xpcsrx_rx_test_ctl_set(parm[1].value.unumber, parm[2].value.unumber, parm[3].value.unumber);
        break;
    ...
    case BDMF_rx_spare_ctl:
        ...
        break;
    case BDMF_rx_test_psudo_pat_sh:
        err = ag_drv_xpcsrx_rx_test_psudo_pat_sh_set(parm[1].value.unumber);
        break;
    case BDMF_rx_test_psudo_pat1_low:
        err = ag_drv_xpcsrx_rx_test_psudo_pat1_low_set(parm[1].value.unumber);
        break;
    case BDMF_rx_test_psudo_pat1_high:
        err = ag_drv_xpcsrx_rx_test_psudo_pat1_high_set(parm[1].value.unumber);
        break;
    case BDMF_rx_test_psudo_pat2_low:
        err = ag_drv_xpcsrx_rx_test_psudo_pat2_low_set(parm[1].value.unumber);
        break;
    case BDMF_rx_test_psudo_pat2_high:
        err = ag_drv_xpcsrx_rx_test_psudo_pat2_high_set(parm[1].value.unumber);
        break;
    default:
...
static int bcm_xpcsrx_cli_get(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    ...
    case BDMF_rx_fec_ctl:
    {
        ...
        bdmf_session_print(session, "cfgxpcsrxidleinvldresen = %u = 0x%x\n", rx_fec_ctl.cfgxpcsrxidleinvldresen, rx_fec_ctl.cfgxpcsrxidleinvldresen);
        bdmf_session_print(session, "cfgxpcsrxfecinvldresen = %u = 0x%x\n", rx_fec_ctl.cfgxpcsrxfecinvldresen, rx_fec_ctl.cfgxpcsrxfecinvldresen);
    ...
    case BDMF_rx_test_ctl:
    {
        uint8_t cfgxpcsrxtestpsudoham;
        bdmf_boolean cfgxpcsrxtestprbsdeten;
        bdmf_boolean cfgxpcsrxtestpsudodeten;
        err = ag_drv_xpcsrx_rx_test_ctl_get(&cfgxpcsrxtestpsudoham, &cfgxpcsrxtestprbsdeten, &cfgxpcsrxtestpsudodeten);
        bdmf_session_print(session, "cfgxpcsrxtestpsudoham = %u = 0x%x\n", cfgxpcsrxtestpsudoham, cfgxpcsrxtestpsudoham);
    ...
    case BDMF_rx_test_psudo_pat_sh:
    {
        uint8_t cfgxpcsrxtestpsudodetpatsh;
        err = ag_drv_xpcsrx_rx_test_psudo_pat_sh_get(&cfgxpcsrxtestpsudodetpatsh);
        bdmf_session_print(session, "cfgxpcsrxtestpsudodetpatsh = %u = 0x%x\n", cfgxpcsrxtestpsudodetpatsh, cfgxpcsrxtestpsudodetpatsh);
        break;
    }
    case BDMF_rx_test_psudo_pat1_low:
    {
        uint32_t cfgxpcsrxtestpsudodetpat1low;
        err = ag_drv_xpcsrx_rx_test_psudo_pat1_low_get(&cfgxpcsrxtestpsudodetpat1low);
        bdmf_session_print(session, "cfgxpcsrxtestpsudodetpat1low = %u = 0x%x\n", cfgxpcsrxtestpsudodetpat1low, cfgxpcsrxtestpsudodetpat1low);
        break;
    }
    case BDMF_rx_test_psudo_pat1_high:
    {
        uint32_t cfgxpcsrxtestpsudodetpat1high;
        err = ag_drv_xpcsrx_rx_test_psudo_pat1_high_get(&cfgxpcsrxtestpsudodetpat1high);
        bdmf_session_print(session, "cfgxpcsrxtestpsudodetpat1high = %u = 0x%x\n", cfgxpcsrxtestpsudodetpat1high, cfgxpcsrxtestpsudodetpat1high);
        break;
    }
    case BDMF_rx_test_psudo_pat2_low:
    {
        uint32_t cfgxpcsrxtestpsudodetpat2low;
        err = ag_drv_xpcsrx_rx_test_psudo_pat2_low_get(&cfgxpcsrxtestpsudodetpat2low);
        bdmf_session_print(session, "cfgxpcsrxtestpsudodetpat2low = %u = 0x%x\n", cfgxpcsrxtestpsudodetpat2low, cfgxpcsrxtestpsudodetpat2low);
        break;
    }
    case BDMF_rx_test_psudo_pat2_high:
    {
        uint32_t cfgxpcsrxtestpsudodetpat2high;
        err = ag_drv_xpcsrx_rx_test_psudo_pat2_high_get(&cfgxpcsrxtestpsudodetpat2high);
        bdmf_session_print(session, "cfgxpcsrxtestpsudodetpat2high = %u = 0x%x\n", cfgxpcsrxtestpsudodetpat2high, cfgxpcsrxtestpsudodetpat2high);
        break;
    }
    default:
        err = BDMF_ERR_NOT_SUPPORTED;
...
static int bcm_xpcsrx_cli_test(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    ...
    {
        uint8_t cfgxpcsrxtestpsudoham=gtmv(m, 4);
        bdmf_boolean cfgxpcsrxtestprbsdeten=gtmv(m, 1);
        bdmf_boolean cfgxpcsrxtestpsudodeten=gtmv(m, 1);
        if(!err) bdmf_session_print(session, "ag_drv_xpcsrx_rx_test_ctl_set( %u %u %u)\n", cfgxpcsrxtestpsudoham, cfgxpcsrxtestprbsdeten, cfgxpcsrxtestpsudodeten);
        if(!err) ag_drv_xpcsrx_rx_test_ctl_set(cfgxpcsrxtestpsudoham, cfgxpcsrxtestprbsdeten, cfgxpcsrxtestpsudodeten);
        if(!err) ag_drv_xpcsrx_rx_test_ctl_get( &cfgxpcsrxtestpsudoham, &cfgxpcsrxtestprbsdeten, &cfgxpcsrxtestpsudodeten);
        if(!err) bdmf_session_print(session, "ag_drv_xpcsrx_rx_test_ctl_get( %u %u %u)\n", cfgxpcsrxtestpsudoham, cfgxpcsrxtestprbsdeten, cfgxpcsrxtestpsudodeten);
        if(err || cfgxpcsrxtestpsudoham!=gtmv(m, 4) || cfgxpcsrxtestprbsdeten!=gtmv(m, 1) || cfgxpcsrxtestpsudodeten!=gtmv(m, 1))
            return err ? err : BDMF_ERR_IO;
    }
    {
    ...
    {
        uint8_t cfgxpcsrxtestpsudodetpatsh=gtmv(m, 2);
        if(!err) bdmf_session_print(session, "ag_drv_xpcsrx_rx_test_psudo_pat_sh_set( %u)\n", cfgxpcsrxtestpsudodetpatsh);
        if(!err) ag_drv_xpcsrx_rx_test_psudo_pat_sh_set(cfgxpcsrxtestpsudodetpatsh);
        if(!err) ag_drv_xpcsrx_rx_test_psudo_pat_sh_get( &cfgxpcsrxtestpsudodetpatsh);
        if(!err) bdmf_session_print(session, "ag_drv_xpcsrx_rx_test_psudo_pat_sh_get( %u)\n", cfgxpcsrxtestpsudodetpatsh);
        if(err || cfgxpcsrxtestpsudodetpatsh!=gtmv(m, 2))
            return err ? err : BDMF_ERR_IO;
    }
    {
        uint32_t cfgxpcsrxtestpsudodetpat1low=gtmv(m, 32);
        if(!err) bdmf_session_print(session, "ag_drv_xpcsrx_rx_test_psudo_pat1_low_set( %u)\n", cfgxpcsrxtestpsudodetpat1low);
        if(!err) ag_drv_xpcsrx_rx_test_psudo_pat1_low_set(cfgxpcsrxtestpsudodetpat1low);
        if(!err) ag_drv_xpcsrx_rx_test_psudo_pat1_low_get( &cfgxpcsrxtestpsudodetpat1low);
        if(!err) bdmf_session_print(session, "ag_drv_xpcsrx_rx_test_psudo_pat1_low_get( %u)\n", cfgxpcsrxtestpsudodetpat1low);
        if(err || cfgxpcsrxtestpsudodetpat1low!=gtmv(m, 32))
            return err ? err : BDMF_ERR_IO;
    }
    {
        uint32_t cfgxpcsrxtestpsudodetpat1high=gtmv(m, 32);
        if(!err) bdmf_session_print(session, "ag_drv_xpcsrx_rx_test_psudo_pat1_high_set( %u)\n", cfgxpcsrxtestpsudodetpat1high);
        if(!err) ag_drv_xpcsrx_rx_test_psudo_pat1_high_set(cfgxpcsrxtestpsudodetpat1high);
        if(!err) ag_drv_xpcsrx_rx_test_psudo_pat1_high_get( &cfgxpcsrxtestpsudodetpat1high);
        if(!err) bdmf_session_print(session, "ag_drv_xpcsrx_rx_test_psudo_pat1_high_get( %u)\n", cfgxpcsrxtestpsudodetpat1high);
        if(err || cfgxpcsrxtestpsudodetpat1high!=gtmv(m, 32))
            return err ? err : BDMF_ERR_IO;
    }
    {
        uint32_t cfgxpcsrxtestpsudodetpat2low=gtmv(m, 32);
        if(!err) bdmf_session_print(session, "ag_drv_xpcsrx_rx_test_psudo_pat2_low_set( %u)\n", cfgxpcsrxtestpsudodetpat2low);
        if(!err) ag_drv_xpcsrx_rx_test_psudo_pat2_low_set(cfgxpcsrxtestpsudodetpat2low);
        if(!err) ag_drv_xpcsrx_rx_test_psudo_pat2_low_get( &cfgxpcsrxtestpsudodetpat2low);
        if(!err) bdmf_session_print(session, "ag_drv_xpcsrx_rx_test_psudo_pat2_low_get( %u)\n", cfgxpcsrxtestpsudodetpat2low);
        if(err || cfgxpcsrxtestpsudodetpat2low!=gtmv(m, 32))
            return err ? err : BDMF_ERR_IO;
    }
    {
        uint32_t cfgxpcsrxtestpsudodetpat2high=gtmv(m, 32);
        if(!err) bdmf_session_print(session, "ag_drv_xpcsrx_rx_test_psudo_pat2_high_set( %u)\n", cfgxpcsrxtestpsudodetpat2high);
        if(!err) ag_drv_xpcsrx_rx_test_psudo_pat2_high_set(cfgxpcsrxtestpsudodetpat2high);
        if(!err) ag_drv_xpcsrx_rx_test_psudo_pat2_high_get( &cfgxpcsrxtestpsudodetpat2high);
        if(!err) bdmf_session_print(session, "ag_drv_xpcsrx_rx_test_psudo_pat2_high_get( %u)\n", cfgxpcsrxtestpsudodetpat2high);
        if(err || cfgxpcsrxtestpsudodetpat2high!=gtmv(m, 32))
            return err ? err : BDMF_ERR_IO;
    }
    return err;
}
...
static int bcm_xpcsrx_cli_address(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    ...
    case bdmf_address_rx_test_psudo_pat_sh : reg = &RU_REG(XPCSRX, RX_TEST_PSUDO_PAT_SH); blk = &RU_BLK(XPCSRX); break;
    case bdmf_address_rx_test_psudo_pat1_low : reg = &RU_REG(XPCSRX, RX_TEST_PSUDO_PAT1_LOW); blk = &RU_BLK(XPCSRX); break;
    case bdmf_address_rx_test_psudo_pat1_high : reg = &RU_REG(XPCSRX, RX_TEST_PSUDO_PAT1_HIGH); blk = &RU_BLK(XPCSRX); break;
    case bdmf_address_rx_test_psudo_pat2_low : reg = &RU_REG(XPCSRX, RX_TEST_PSUDO_PAT2_LOW); blk = &RU_BLK(XPCSRX); break;
    case bdmf_address_rx_test_psudo_pat2_high : reg = &RU_REG(XPCSRX, RX_TEST_PSUDO_PAT2_HIGH); blk = &RU_BLK(XPCSRX); break;
    default :
...
bdmfmon_handle_t ag_drv_xpcsrx_cli_init(bdmfmon_handle_t driver_dir)
{
...

    {
        ...
        static bdmfmon_cmd_parm_t set_rx_fec_ctl[]={
            BDMFMON_MAKE_PARM("cfgxpcsrxidleinvldresen", "cfgxpcsrxidleinvldresen", BDMFMON_PARM_NUMBER, 0),
            BDMFMON_MAKE_PARM("cfgxpcsrxfecinvldresen", "cfgxpcsrxfecinvldresen", BDMFMON_PARM_NUMBER, 0),
        ...
        static bdmfmon_cmd_parm_t set_rx_test_ctl[]={
            BDMFMON_MAKE_PARM("cfgxpcsrxtestpsudoham", "cfgxpcsrxtestpsudoham", BDMFMON_PARM_NUMBER, 0),
        ...
        static bdmfmon_cmd_parm_t set_rx_test_psudo_pat_sh[]={
            BDMFMON_MAKE_PARM("cfgxpcsrxtestpsudodetpatsh", "cfgxpcsrxtestpsudodetpatsh", BDMFMON_PARM_NUMBER, 0),
            BDMFMON_PARM_LIST_TERMINATOR
        };
        static bdmfmon_cmd_parm_t set_rx_test_psudo_pat1_low[]={
            BDMFMON_MAKE_PARM("cfgxpcsrxtestpsudodetpat1low", "cfgxpcsrxtestpsudodetpat1low", BDMFMON_PARM_NUMBER, 0),
            BDMFMON_PARM_LIST_TERMINATOR
        };
        static bdmfmon_cmd_parm_t set_rx_test_psudo_pat1_high[]={
            BDMFMON_MAKE_PARM("cfgxpcsrxtestpsudodetpat1high", "cfgxpcsrxtestpsudodetpat1high", BDMFMON_PARM_NUMBER, 0),
            BDMFMON_PARM_LIST_TERMINATOR
        };
        static bdmfmon_cmd_parm_t set_rx_test_psudo_pat2_low[]={
            BDMFMON_MAKE_PARM("cfgxpcsrxtestpsudodetpat2low", "cfgxpcsrxtestpsudodetpat2low", BDMFMON_PARM_NUMBER, 0),
            BDMFMON_PARM_LIST_TERMINATOR
        };
        static bdmfmon_cmd_parm_t set_rx_test_psudo_pat2_high[]={
            BDMFMON_MAKE_PARM("cfgxpcsrxtestpsudodetpat2high", "cfgxpcsrxtestpsudodetpat2high", BDMFMON_PARM_NUMBER, 0),
            BDMFMON_PARM_LIST_TERMINATOR
        };
        static bdmfmon_enum_val_t selector_table[] = {
            ...
            { .name="rx_test_psudo_pat_sh", .val=BDMF_rx_test_psudo_pat_sh, .parms=set_rx_test_psudo_pat_sh },
            { .name="rx_test_psudo_pat1_low", .val=BDMF_rx_test_psudo_pat1_low, .parms=set_rx_test_psudo_pat1_low },
            { .name="rx_test_psudo_pat1_high", .val=BDMF_rx_test_psudo_pat1_high, .parms=set_rx_test_psudo_pat1_high },
            { .name="rx_test_psudo_pat2_low", .val=BDMF_rx_test_psudo_pat2_low, .parms=set_rx_test_psudo_pat2_low },
            { .name="rx_test_psudo_pat2_high", .val=BDMF_rx_test_psudo_pat2_high, .parms=set_rx_test_psudo_pat2_high },
            BDMFMON_ENUM_LAST
        };
    ...
    }
    {
        ...
        static bdmfmon_enum_val_t selector_table[] = {
            ...
            { .name="rx_test_psudo_pat_sh", .val=BDMF_rx_test_psudo_pat_sh, .parms=set_default },
            { .name="rx_test_psudo_pat1_low", .val=BDMF_rx_test_psudo_pat1_low, .parms=set_default },
            { .name="rx_test_psudo_pat1_high", .val=BDMF_rx_test_psudo_pat1_high, .parms=set_default },
            { .name="rx_test_psudo_pat2_low", .val=BDMF_rx_test_psudo_pat2_low, .parms=set_default },
            { .name="rx_test_psudo_pat2_high", .val=BDMF_rx_test_psudo_pat2_high, .parms=set_default },
            BDMFMON_ENUM_LAST
        };
    ...
    }
    {
        static bdmfmon_enum_val_t enum_table_address[] = {
            ...
            { .name="RX_TEST_PSUDO_PAT_SH" , .val=bdmf_address_rx_test_psudo_pat_sh },
            { .name="RX_TEST_PSUDO_PAT1_LOW" , .val=bdmf_address_rx_test_psudo_pat1_low },
            { .name="RX_TEST_PSUDO_PAT1_HIGH" , .val=bdmf_address_rx_test_psudo_pat1_high },
            { .name="RX_TEST_PSUDO_PAT2_LOW" , .val=bdmf_address_rx_test_psudo_pat2_low },
            { .name="RX_TEST_PSUDO_PAT2_HIGH" , .val=bdmf_address_rx_test_psudo_pat2_high },
            BDMFMON_ENUM_LAST
        };
