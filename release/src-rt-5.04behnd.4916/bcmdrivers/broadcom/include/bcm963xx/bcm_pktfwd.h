...
/** WLAN Radio Domains supports a max of 4 Radios */
#define PKTFWD_DOMAINS_WLAN         (4)
...
#define PKTFWD_DOMAIN_MASK          (0x0E00)
...
typedef union pktfwd_key                /* pktfwd_key_t */
{
    ...
    struct {
        LE_DECL(
            ...
            uint16_t incarn  : 1;       /* incarnation incremented on free */
            uint16_t domain  : 3;       /* endpoint domain */
        )
        BE_DECL(
            uint16_t domain  : 3;       /* used as wfd_id - bits 15,14,13 */
            uint16_t incarn  : 1;
            uint16_t index   : 12;
        )
    };
...
#define PKTFWD_KEY_INCARN_MASK      (0x1)
#define PKTFWD_KEY_INCARN_SHIFT     (12)

#define PKTFWD_KEY_DOMAIN_MASK      (0x7)
#define PKTFWD_KEY_DOMAIN_SHIFT     (13)
 ...
 * composed of a 3b domain (radio unit), a 1b incarnation, and a 12b global
...
struct pktlist                      /* pktlist_t */
{
    ...
    pktfwd_key_t    key;            /* 3b domain, 1b incarn, 12b index */
};
...
#define WLIF_UNIT_INVALID	(0xf)	/* d3fwd_wlif::unit 4bit */
...
struct d3fwd_wlif                       /* d3fwd_wlif_t */
{
    ...
    /* WDS peer mac */
    uint8_t     wds_peer_mac[6]; /*ETHER_ADDR_LEN*/

    /* netdev transmit handler */
    xmit_fn_t netdev_ops_xmit;
};

/* d3fwd_wlif->flags: 32bit bitwise flags */
#define D3FWD_WLIF_FLAGS_UP                 0x1
#define D3FWD_WLIF_FLAGS_DWDS_AP            0x2
#define D3FWD_WLIF_FLAGS_DWDS_CLIENT        0x4
#define D3FWD_WLIF_FLAGS_DWDS_DISCONNECTED  0x8
...
#if (D3LUT_POOL_TOT > 8)
#error "d3lut_key_t 3b key index limits to 8 domains"
#endif
 ...
 * retrieve a 16 bit key made of a <12 bit index, 1 bit incarn, 3 bit domain>
 *
