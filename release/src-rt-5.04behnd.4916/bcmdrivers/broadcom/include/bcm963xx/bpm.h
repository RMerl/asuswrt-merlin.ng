 ...
 * 
 * MIPS use Tx IuDMA channel to transmit packet to Ethernet switch.
...
struct bpm_hw_pool_ops {
	/* this function has to be registered or registration of the HW
	 * pool will fail
	 */
	bool (*is_hw_buffer)(void *buf_p);

	void *(*alloc_buf)(unsigned long ring_index);
	int (*alloc_mult_buf)(unsigned long ring_index, uint32_t num,
			      void **buf_p);
	void (*free_buf)(unsigned long ring_index, void *buf_p);
	void (*free_mult_buf)(unsigned long ring_index, uint32_t num, void **buf_p);

	void *(*alloc_skb_hdr)(unsigned long ring_index);
	void *(*alloc_skb_hdr_list)(unsigned long ring_index, uint32_t num);
	void *(*alloc_skb_data)(unsigned long ring_index, uint32_t datalen);
	void (*free_skb_hdr)(unsigned long ring_index, void *skbp);
	void (*free_skb_hdr_list)(unsigned long ring_index, void *skbp);
	void (*free_skb_data_list)(unsigned long ring_index, void *head, void *tail,
				   uint32_t num);
};

struct bpm_hw_pool_def_ring_id {
	unsigned long alloc;
	unsigned long alloc_mult;
	unsigned long alloc_mult_hiprio;
	unsigned long free;
	unsigned long free_mult;

	unsigned long skb_hdr_alloc;
	unsigned long skb_hdr_list_alloc;
	unsigned long skb_data_alloc;
	unsigned long skb_hdr_free;
	unsigned long skb_hdr_list_free;
	unsigned long skb_data_list_free;
};
#endif
