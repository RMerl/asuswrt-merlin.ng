...
static int dpp_conf_req_rx_map(struct dpp_authentication *auth, struct json_token *root);
#ifdef CONFIG_DRIVER_BRCM_MLO
static void dpp_wifi7_bsta_radio_list_free(struct dpp_wifi7_agent_bsta_capabilities *caps);
static void dpp_wifi7_aux_radio_list_free(struct dl_list *aux_list);
#endif /* CONFIG_DRIVER_BRCM_MLO */
#endif /* CONFIG_DRIVER_BRCM_MAP */
...
#if defined(CONFIG_DRIVER_BRCM_MAP) && defined(CONFIG_DRIVER_BRCM_MLO)
	dl_list_init(&conf->wifi7_bsta_capab.wifi7_bsta_radio_list);
#endif	/* CONFIG_DRIVER_BRCM_MAP && CONFIG_DRIVER_BRCM_MLO */
...
void dpp_configuration_free(struct dpp_configuration *conf)
{
	if (!conf)
		return;
#if defined(CONFIG_DRIVER_BRCM_MAP) && defined(CONFIG_DRIVER_BRCM_MLO)
	dpp_wifi7_bsta_radio_list_free(&conf->wifi7_bsta_capab);
#endif /* CONFIG_DRIVER_BRCM_MAP && CONFIG_DRIVER_BRCM_MLO */
...
#ifdef CONFIG_DRIVER_BRCM_MAP
	struct dpp_configuration *conf_map = NULL;
#endif /* CONFIG_DRIVER_BRCM_MAP */
...
#ifdef CONFIG_DRIVER_BRCM_MAP
	if (os_strstr(cmd, " conf=mapAgent") ||
		os_strstr(cmd, " conf=mapController")) {
		conf_map = os_zalloc(sizeof(*conf_map));
		if (!conf_map)
			goto fail;
		if (os_strstr(cmd, " conf=mapController"))
			conf_map->netrole = DPP_NETROLE_MAPCONTROLLER;
		else
			conf_map->netrole = DPP_NETROLE_MAPAGENT;
		conf_map->akm = DPP_AKM_DPP;
		if (auth->conf_map)
			dpp_configuration_free(auth->conf_map);
		auth->conf_map = conf_map;
		return 0;
	}

	pos = os_strstr(cmd, " conf=map-");
	if (pos) {
		conf_map = dpp_configuration_alloc(pos + 10);
		if (!conf_map)
			goto fail;
		conf_map->netrole = DPP_NETROLE_MAP_BH_STA;
		conf = conf_map;
	}

	pos = os_strstr(cmd, " conf=mapbh-");
	if (pos) {
		conf_map = dpp_configuration_alloc(pos + os_strlen(" conf=mapbh-"));
		if (!conf_map)
			goto fail;
		conf_map->netrole = DPP_NETROLE_MAP_BH_BSS;
		conf = conf_map;
	}
...
#ifdef CONFIG_DRIVER_BRCM_MAP
	dpp_configuration_free(conf_map);
#endif /* CONFIG_DRIVER_BRCM_MAP */
	return -1;
}
...
#endif /* CONFIG_TESTING_OPTIONS */
#ifdef CONFIG_DRIVER_BRCM_MAP
	wpabuf_free(auth->map_peer_pub_key);
#endif /* CONFIG_DRIVER_BRCM_MAP */
...
#ifdef CONFIG_DRIVER_BRCM_MAP
static struct wpabuf *
dpp_build_conf_start_map(struct dpp_authentication *auth,
		     struct dpp_configuration *conf, size_t tailroom)
{
	struct wpabuf *buf;

	buf = wpabuf_alloc(3000 + tailroom);
	if (!buf)
		return NULL;
	json_start_object(buf, NULL);
...
#ifdef CONFIG_DRIVER_BRCM_MLO
	if ((conf->netrole == DPP_NETROLE_MAP_BH_STA) &&
		(conf->wifi7_bsta_capab.num_radio > 0)) {
		int bsta_count = 0;
		struct dpp_wifi7_agent_bsta_radio_info *iter_bsta_radio_info;

		json_start_object(buf, "Backhaul_STA_MLD_Config");
		json_start_array(buf, "AffiliatedSTAList");

		/* Generate comma separated STAList based on Config Request data of Agent */
		dl_list_for_each(iter_bsta_radio_info,
				&conf->wifi7_bsta_capab.wifi7_bsta_radio_list,
				struct dpp_wifi7_agent_bsta_radio_info, list) {
			u8 wifi7_caps = iter_bsta_radio_info->wifi7_caps;
			char ruid_str[ETH_ALEN * 2 + 10];
			os_snprintf(ruid_str, sizeof(ruid_str), COMPACT_MACSTR,
				MAC2STR(iter_bsta_radio_info->ruid));

			if (bsta_count) {
				json_value_sep(buf);
			}
			json_start_object(buf, NULL);
			json_add_string(buf, "RUID", ruid_str);
			json_value_sep(buf);
			json_add_boolean(buf, "bSTA_STR_Support",
				DPP_IS_WIFI7_CAPAB_STR(wifi7_caps));
			json_value_sep(buf);
			json_add_boolean(buf, "bSTA_NSTR_Support",
				DPP_IS_WIFI7_CAPAB_NSTR(wifi7_caps));
			json_value_sep(buf);
			json_add_boolean(buf, "bSTA_EMLSR_Support",
				DPP_IS_WIFI7_CAPAB_EMLSR(wifi7_caps));
			json_value_sep(buf);
			json_add_boolean(buf, "bSTA_EMLMR_Support",
				DPP_IS_WIFI7_CAPAB_EMLMR(wifi7_caps));
			json_end_object(buf);
			bsta_count++;
		}
		json_end_array(buf);
...
skip_groups:
	...
	if (conf->netrole == DPP_NETROLE_MAP_BH_STA ||
		...
		buf = dpp_build_conf_start_map(auth, conf, tailroom);
	else
#endif /* CONFIG_DRIVER_BRCM_MAP */
...
#ifdef CONFIG_DRIVER_BRCM_MAP
	if ((conf->netrole == DPP_NETROLE_MAP_BH_STA) || (conf->netrole == DPP_NETROLE_MAP_BH_BSS))
		buf = dpp_build_conf_start_map(auth, conf, 1000);
	else
#endif /* CONFIG_DRIVER_BRCM_MAP */
...
#ifdef CONFIG_DRIVER_BRCM_MAP
	if (netrole == DPP_NETROLE_MAPAGENT || netrole == DPP_NETROLE_MAPCONTROLLER ||
		netrole == DPP_NETROLE_MAP_BH_BSS) {
		if (idx == 1) {
			memset(&conf_1905, 0, sizeof(conf_1905));
			conf_1905.netrole = netrole;
			conf_1905.akm = DPP_AKM_DPP;
			conf_1905.group_id = auth->conf_map->group_id;
			conf_1905.netaccesskey_expiry = auth->conf_map->netaccesskey_expiry;
			conf = &conf_1905;
		} else {
			conf = auth->conf_map;
		}
		...
		size_t attr_len)
{
	...
	if (os_strcmp(token->string, "map") == 0) {
		if (dpp_conf_req_rx_map(auth, root) < 0) {
			wpa_printf(MSG_DEBUG, "DPP: MAP configuration "
				"request parsing failed");
			goto fail;
		}
		netrole = auth->e_netrole;
		goto cont;
	} else if (os_strcmp(token->string, "infra") != 0) {
#else
...
#ifdef CONFIG_DRIVER_BRCM_MAP
#ifdef CONFIG_DRIVER_BRCM_MLO
/* Add new Radio Item in WiFi 7 Agent BSTA Capabilities Radio List */
static struct dpp_wifi7_agent_bsta_radio_info *
dpp_wifi7_agent_bsta_radio_info_add(struct dl_list *radio_list, const u8 *ruid)
{
	struct dpp_wifi7_agent_bsta_radio_info *new_radio_info;

	new_radio_info = os_zalloc(sizeof(*new_radio_info));
	if (!new_radio_info) {
		return NULL;
	}

	dl_list_add(radio_list, &new_radio_info->list);
	dl_list_init(&new_radio_info->aux_radio_list);
	os_memcpy(new_radio_info->ruid, ruid, ETH_ALEN);

	return new_radio_info;
}

/* Remove existing WiFi 7 Agent BSTA Capabilities */
static void
dpp_wifi7_bsta_radio_list_free(struct dpp_wifi7_agent_bsta_capabilities *caps)
{
	struct dpp_wifi7_agent_bsta_radio_info *iter_bsta_radio_info, *tmp_bsta_radio_info;

	if (caps->num_radio > 0) {
		dl_list_for_each_safe(iter_bsta_radio_info, tmp_bsta_radio_info,
				&caps->wifi7_bsta_radio_list,
				struct dpp_wifi7_agent_bsta_radio_info, list) {
			dpp_wifi7_aux_radio_list_free(&iter_bsta_radio_info->aux_radio_list);
			dl_list_del(&iter_bsta_radio_info->list);
			os_free(iter_bsta_radio_info);
		}
		caps->num_radio = 0;
	}
}

/* Free aux radio list of radio item */
static void
dpp_wifi7_aux_radio_list_free(struct dl_list *aux_list)
{
	struct dpp_wifi7_radio_capabilities *aux_item, *tmp_aux_item;

	dl_list_for_each_safe(aux_item, tmp_aux_item, aux_list,
		struct dpp_wifi7_radio_capabilities, list) {
		dl_list_del(&aux_item->list);
		os_free(aux_item);
	}
}

/* Return the aux radio item from the aux list with maching ruid for
 * nomatch case allocate the aux item */
static struct dpp_wifi7_radio_capabilities*
dpp_wifi7_get_aux_radio_item(struct dl_list *aux_list, u8 *aux_ruid)
{
	struct dpp_wifi7_radio_capabilities *aux_item, *tmp_aux_item;

	dl_list_for_each_safe(aux_item, tmp_aux_item, aux_list,
		struct dpp_wifi7_radio_capabilities, list) {
		if (!os_memcmp(aux_item->ruid, aux_ruid, ETH_ALEN)) {
			return aux_item;
		}
	}

	aux_item = os_zalloc(sizeof(*aux_item));
	if (!aux_item) {
		return NULL;
	}

	dl_list_add(aux_list, &aux_item->list);
	os_memcpy(aux_item->ruid, aux_ruid, ETH_ALEN);

	return aux_item;
}

/* Parse WiFi 7 Agent BSTA Capabilities per Radio : Freq Sep List */
static u8
dpp_conf_parse_freq_sep_info(struct json_token *parent_token,
		struct dpp_wifi7_agent_bsta_radio_info *radio_info)
{
	struct json_token *token, *list, *ruid, *sep;
	u8 sep_count;
	int idx, valid;
	enum wifi7_modes {STR = 0, NSTR = 1, EMLSR = 2, EMLMR = 3};
	char *json_arr_str, *ruid_tag, *freq_sep_tag;
	struct dl_list *sep_list;
	struct dpp_wifi7_radio_capabilities *aux_item;

	sep_list = &radio_info->aux_radio_list;

	for (idx = STR; idx <= EMLMR; idx++) {
		switch (idx) {
			case STR:
				json_arr_str = "STRFreqSeparation";
				ruid_tag = "bSTA_STR_RUID";
				freq_sep_tag = "bSTA_STR_Freq_Separation";
				valid = 1;
				break;

			case NSTR:
				json_arr_str = "NSTRFreqSeparation";
				ruid_tag = "bSTA_NSTR_RUID";
				freq_sep_tag = "bSTA_NSTR_Freq_Separation";
				valid = 1;
				break;

			case EMLSR:
				json_arr_str = "EMLSRFreqSeparation";
				ruid_tag = "bSTA_EMLSR_RUID";
				freq_sep_tag = "bSTA_EMLSR_Freq_Separation";
				valid = 1;
				break;

			case EMLMR:
				json_arr_str = "EMLMRFreqSeparation";
				ruid_tag = "bSTA_EMLMR_RUID";
				freq_sep_tag = "bSTA_EMLMR_Freq_Separation";
				valid = 1;
				break;
		...
		sep_count = 0;
		list = json_get_member(parent_token, json_arr_str);
		if (!list || list->type != JSON_ARRAY) {
			wpa_printf(MSG_WARNING, "DPP: No %s array found", json_arr_str);
			continue;
		}

		for (token = list->child; token; token = token->sibling) {
			u8 peer_ruid[ETH_ALEN];

			ruid = json_get_member(token, ruid_tag);
			if (!ruid || ruid->type != JSON_STRING) {
				wpa_printf(MSG_WARNING, "DPP: Missing RUID in %s", json_arr_str);
				continue;
			}
			if (hwaddr_compact_aton(ruid->string, peer_ruid) < 0) {
				wpa_printf(MSG_WARNING, "DPP: Invalid RUID %s in %s",
					ruid->string, json_arr_str);
				continue;
			}
			sep = json_get_member(token, freq_sep_tag);
			if (!sep || sep->type != JSON_NUMBER) {
				wpa_printf(MSG_WARNING, "DPP: Missing Separation in %s",
					json_arr_str);
				continue;
			}
			aux_item = dpp_wifi7_get_aux_radio_item(sep_list, peer_ruid);
			if (!aux_item) {
				wpa_printf(MSG_WARNING, "DPP: Error adding new RUID in %s",
					json_arr_str);
				continue;
			}
			if (idx == STR) {
				aux_item->str_freq_separation = sep->number;
				aux_item->flags |= DPP_WIFI7_CAPAB_STR;
			} else if (idx == NSTR) {
				aux_item->nstr_freq_separation = sep->number;
				aux_item->flags |= DPP_WIFI7_CAPAB_NSTR;
			} else if (idx == EMLSR) {
				aux_item->emlsr_freq_separation = sep->number;
				aux_item->flags |= DPP_WIFI7_CAPAB_EMLSR;
			} else if (idx == EMLMR) {
				aux_item->emlmr_freq_separation = sep->number;
				aux_item->flags |= DPP_WIFI7_CAPAB_EMLMR;
			}
			sep_count++;
		}
		wpa_printf(MSG_DEBUG, "DPP: %s List count = %u", json_arr_str, sep_count);
	}
}

/* Parse WiFi 7 Agent BSTA Capabilities per Radio : Capabilities Flags */
static void
dpp_conf_parse_wifi7_radio_capab_flags(struct json_token *parent_token, const char* cap_name,
	u8 cap_flag, struct dpp_wifi7_agent_bsta_radio_info *radio_info)
{
	struct json_token *support;

	support = json_get_member(parent_token, cap_name);
	if (!support || support->type != JSON_BOOLEAN) {
		wpa_printf(MSG_WARNING, "DPP: Missing %s BOOLEAN", cap_name);
	}
	DPP_WIFI7_CAPAB_UPDATE((support->number ? true : false),
		(radio_info->wifi7_caps), (cap_flag));
}

/* Parse WiFi 7 Agent BSTA Capabilities */
static int
dpp_conf_parse_wifi7_agent_bsta_capab(struct json_token *parent_token,
	struct dpp_configuration *conf)
{
	struct json_token *wifi7_agent_bsta_capab, *max_links;
	struct json_token *token, *list;
	struct json_token *ruid;
	struct dpp_wifi7_agent_bsta_radio_info *new_radio_info;
	struct dl_list *radio_list;
	u8 radio_ruid[ETH_ALEN];

	/* Handle wifi7 agent bsta capabilities */
	wifi7_agent_bsta_capab = json_get_member(parent_token, "WiFi7AgentBSTACapabilities");
	if (!wifi7_agent_bsta_capab || wifi7_agent_bsta_capab->type != JSON_OBJECT) {
		wpa_printf(MSG_DEBUG, "DPP: Missing WiFi7AgentBSTACapabilities Object");
		return -1;
	}
	radio_list = &conf->wifi7_bsta_capab.wifi7_bsta_radio_list;
	dpp_wifi7_bsta_radio_list_free(&conf->wifi7_bsta_capab);

	max_links = json_get_member(wifi7_agent_bsta_capab, "bSTA_Maximum_Links");
	if (!max_links || max_links->type != JSON_NUMBER) {
		wpa_printf(MSG_DEBUG, "DPP: Missing bSTA_Maximum_Links string");
		return -1;
	}
	conf->wifi7_bsta_capab.max_links = max_links->number;

	/* Handle wifi7 agent bsta capabilities radio list */
	list = json_get_member(wifi7_agent_bsta_capab, "RadioList");
	if (!list || list->type != JSON_ARRAY) {
		wpa_printf(MSG_DEBUG, "DPP: No RadioList array found");
		return -1;
	}
	dl_list_init(radio_list);
	conf->wifi7_bsta_capab.num_radio = 0;
	for (token = list->child; token; token = token->sibling) {
		ruid = json_get_member(token, "RUID");
		if (!ruid || ruid->type != JSON_STRING) {
			wpa_printf(MSG_WARNING, "DPP: Missing RUID in RadioList");
			continue;
		}
		if (hwaddr_compact_aton(ruid->string, radio_ruid) < 0) {
			wpa_printf(MSG_WARNING, "DPP: Invalid BSTA RUID in RadioList");
			continue;
		}
		new_radio_info = dpp_wifi7_agent_bsta_radio_info_add(radio_list, radio_ruid);
		if (!new_radio_info) {
			wpa_printf(MSG_DEBUG, "DPP: Error adding new BSTA RUID in RadioList");
			continue;
		}
		conf->wifi7_bsta_capab.num_radio++;

		/* Handle wifi7 agent bsta capabilities per radio capabilities flags */
		dpp_conf_parse_wifi7_radio_capab_flags(token, "bSTA_STR_Support",
			DPP_WIFI7_CAPAB_STR, new_radio_info);
		dpp_conf_parse_wifi7_radio_capab_flags(token, "bSTA_NSTR_Support",
			DPP_WIFI7_CAPAB_NSTR, new_radio_info);
		dpp_conf_parse_wifi7_radio_capab_flags(token, "bSTA_EMLSR_Support",
			DPP_WIFI7_CAPAB_EMLSR, new_radio_info);
		dpp_conf_parse_wifi7_radio_capab_flags(token, "bSTA_EMLMR_Support",
			DPP_WIFI7_CAPAB_EMLMR, new_radio_info);

		/* Handle wifi7 agent bsta capabilities per radio freq sep list */
		dpp_conf_parse_freq_sep_info(token, new_radio_info);
	}

	wpa_printf(MSG_DEBUG, "DPP: RadioList count = %d", conf->wifi7_bsta_capab.num_radio);
	return 0;
}
#endif /* CONFIG_DRIVER_BRCM_MLO */

static int
dpp_conf_req_rx_map(struct dpp_authentication *auth, struct json_token *root)
{
	struct json_token  *token, *list;
	struct dpp_configuration *conf = auth->conf_map;

	if (!conf) {
		wpa_printf(MSG_DEBUG,
			"DPP: No configuration available for Enrollee(mapAgent) - "
			"reject configuration request");
		return -1;
	}
	token = json_get_member(root, "netRole");
	if (!token || token->type != JSON_STRING) {
	...
	wpa_printf(MSG_DEBUG, "DPP: netRole = '%s'", token->string);
	if (os_strcmp(token->string, "mapAgent") == 0) {
		auth->e_netrole = DPP_NETROLE_MAPAGENT;
	} else {
		wpa_printf(MSG_DEBUG, "DPP: Unsupported netRole '%s'",
			   token->string);
		dpp_auth_fail(auth, "Unsupported netRole");
	...
	if (list && list->type == JSON_ARRAY) {
		...
		for (token = list->child; token; token = token->sibling) {
			...
			wpa_printf(MSG_DEBUG, "DPP: L2 netRole = '%s'", role->string);
			if (os_strcmp(role->string, "mapBackhaulSta") != 0) {
				wpa_printf(MSG_DEBUG, "DPP: Unsupported netRole '%s'",
					role->string);
				dpp_auth_fail(auth, "Unsupported netRole");
				return -1;
			...
			if (!chan_list || chan_list->type != JSON_STRING) {
				wpa_printf(MSG_DEBUG, "DPP: Missing channelList string");
			}
			array_size++;

#ifdef CONFIG_DRIVER_BRCM_MLO
			/* Parse WiFi 7 Agent BSTA Capabilities */
			(void)dpp_conf_parse_wifi7_agent_bsta_capab(token, conf);
#endif /* CONFIG_DRIVER_BRCM_MLO */
		}
		wpa_printf(MSG_DEBUG, "DPP: BSTAList count = %d", array_size);
	}
	return 0;
}
...
#ifdef CONFIG_DRIVER_BRCM_MAP
		if (os_strcmp(role->string, "mapAgent") == 0) {
			conf->netrole = DPP_NETROLE_MAPAGENT;
		}else if (os_strcmp(role->string, "mapController") == 0) {
			conf->netrole = DPP_NETROLE_MAPCONTROLLER;
		}
#endif /* CONFIG_DRIVER_BRCM_MAP */
		rules++;
			      ...
			      const u8 *conf_obj, u16 conf_obj_len)
{
...
#ifdef CONFIG_DRIVER_BRCM_MAP
	if ((os_strcmp(token->string, "map") != 0) &&
		 (os_strcmp(token->string, "infra") != 0)) {
		wpa_printf(MSG_DEBUG, "DPP: Unsupported wi-fi_tech value: '%s' for netrole '%s",
			   token->string, dpp_netrole_str(auth->e_netrole));
#else
	if (os_strcmp(token->string, "infra") != 0) {
	...
	if (conf->netrole == DPP_NETROLE_MAPAGENT ||
		...
		if (conf->akm != DPP_AKM_DPP) {
			dpp_auth_fail(auth, "MAP AKM not DPP, for ieee1905 security");
			wpa_printf(MSG_DEBUG, "DPP: AKM should be DPP, for netrole '%s'",
				...
				const char *curve, int ap)
{
...
#ifdef CONFIG_DRIVER_BRCM_MAP
	/* If public key is provided, then use that as peer key */
	if (auth->map_peer_pub_key) {
		auth->peer_protocol_key = crypto_ec_key_parse_pub(
			wpabuf_head(auth->map_peer_pub_key), wpabuf_len(auth->map_peer_pub_key));
		if (!auth->peer_protocol_key) {
			wpa_printf(MSG_ERROR, "Invalid Peer Public Key");
			return -1;
		}
	} else
#endif /* CONFIG_DRIVER_BRCM_MAP */
 ...
 #ifdef CONFIG_DRIVER_BRCM_MAP
	if (auth->conf_map)
		netrole = auth->conf_map->netrole;

	conf_obj = dpp_build_conf_obj(auth, netrole, 0, NULL);
#else
...
#ifdef CONFIG_DRIVER_BRCM_MAP
	/* If public is provided, then we generate the key and we are not using public key same
	 * as own key. So, needs to free it in auth_deinit.
	 */
	if (!auth->map_peer_pub_key)
#endif /* CONFIG_DRIVER_BRCM_MAP */
...
static int dpp_compatible_netrole(const char *role1, const char *role2)
{
	return (os_strcmp(role1, "sta") == 0 && os_strcmp(role2, "ap") == 0) ||
		...
		(os_strcmp(role2, "mapAgent") == 0 || os_strcmp(role2, "mapController") == 0) ||
