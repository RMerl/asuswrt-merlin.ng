...
dpp_relay_controller_get(struct dpp_global *dpp, const u8 *pkhash, void *cb_ctx)
#else
static struct dpp_relay_controller *
...
#endif /* CONFIG_DRIVER_BRCM_MAP */
{
	...
	dl_list_for_each(ctrl, &dpp->controllers, struct dpp_relay_controller,
		...
		if ((cb_ctx == ctrl->cb_ctx) &&
				(os_memcmp(pkhash, ctrl->pkhash, SHA256_MAC_LEN) == 0))
#else
			...
			void *cb_ctx)
{
	...
	if (type == DPP_PA_PRESENCE_ANNOUNCEMENT ||
		...
		if (!r_bootstrap)
			return -1;
#ifdef CONFIG_DRIVER_BRCM_MAP
		ctrl = dpp_relay_controller_get(dpp, r_bootstrap, cb_ctx);
#else
...
#ifdef CONFIG_DRIVER_BRCM_MAP
int dpp_relay_bootstrap_uri(struct dpp_global *dpp,
		u8 *src, int freq, void *cb_ctx, struct wpabuf *msg)
{
	struct dpp_relay_controller *ctrl;
	struct dpp_connection *conn;
	...
	ctrl = dpp_relay_controller_get_ctx(dpp, cb_ctx);
	if (!ctrl) {
		wpa_printf(MSG_ERROR, "DPP: Couldn't find relay controller");
		return -1;
	}

	conn = dpp_relay_new_conn(ctrl, src, freq);
	if (!conn) {
		wpa_printf(MSG_ERROR, "DPP: dpp_relay_bootstrap_uri failed to open connection");
		return -1;
	}
