					    ...
					    int show_errors)
{
	...
	case OUI_BROADCOM:
		...
		case VENDOR_MLO_OUI_TYPE:
			if (elen > 4) {
				wpa_printf(MSG_ERROR, "RX link_bmp:%d elen:%d\n", pos[5], elen);
				wpa_hexdump(MSG_MSGDUMP, "LINK_BMP IE", pos, elen);
				elems->vendor_mlo = pos;
				elems->vendor_mlo_len = elen;
			}
			break;
				      ...
				      int show_errors)
{
	...
	case WLAN_EID_EXT_MULTI_LINK:
		elems->multi_link = pos;
		elems->multi_link_len = elen;
		break;
#endif /* CONFIG_DRIVER_BRCM_MLO */
	default:
		if (show_errors) {
...
#ifdef CONFIG_DRIVER_BRCM_MLO
void ieee802_11_parse_subfragment(struct frag_ies_info *frag_ies,
				const u8 *pos, u8 elen)
{
	if (frag_ies->n_frags >= MAX_NUM_FRAG_IES_SUPPORTED) {
		wpa_printf(MSG_MSGDUMP, "Too many element fragments - skip");
...
/* This will alloc a wpabuf with the full MLIE.
 * Caller has to ensure Freeing of this wpa buffer
 */
struct wpabuf* ieee802_11_get_mlie(struct ieee802_11_elems *elems)
{
	struct wpabuf *mlie;

	/* MLIE in received assocreq can be Fragmented take care of it */
	mlie = ieee802_11_defrag(elems, WLAN_EID_EXTENSION,
					WLAN_EID_EXT_MULTI_LINK);

	if (!mlie) {
		wpa_printf(MSG_ERROR, "Assoc: MLIE: no mem for MLIE BUFF \n");
	}
	return mlie;
}

static ParseRes mlo_parse_persta_info(const u8 *start, size_t len,
		struct ieee802_11_subelems *subelems)
{
	const struct element *elem = NULL;

	if (!start)
		return ParseOK;
	os_memset(subelems, 0, sizeof(*subelems));
	...
	for_each_element(elem, start, len) {
		...
		switch (id) {
		case WLAN_SUBEID_PERSTA_PROF:
			subelems->persta_link_info = pos;
			subelems->persta_link_len = elen;
			wpa_printf(MSG_MSGDUMP, "Parsing PERSTA Prof id=%d elen=%d\n",
					id, elen);
			break;
		case WLAN_SUBEID_VENDOR_SPEC:
			break;
		case WLAN_SUBEID_FRAGMENTED:
			wpa_printf(MSG_MSGDUMP, "Parsing Fragmented subelement id=%d elen=%d\n",
					id, elen);
			ieee802_11_parse_subfragment(&subelems->frag_ies, pos, elen);
			break;
		default:
			wpa_printf(MSG_MSGDUMP, "IEEE 802.11 subelement parse "
				   "ignored unknown element (id=%d elen=%d)",
				   id, elen);
		}

		if (id != WLAN_SUBEID_FRAGMENTED && elen == 255)
			subelems->frag_ies.last_eid = id;
...
void ieee802_11_mlo_parse_ml_info(struct ml_info *mlsta_info,
	const u8 *multi_link, size_t multi_link_len)
{
	u16 ml_ctrl = 0, mld_cap = 0;
	u16 offset = 0, sta_linkid = 0;
	u8 comm_info_len = 0, numlinks = 0;
	u8 *persta = NULL;
	int i = 0;

	if (!multi_link || !mlsta_info) {
		return;
	}

	os_memcpy(&ml_ctrl, multi_link + offset, sizeof(ml_ctrl));
	mlsta_info->ml_ctrl = ml_ctrl;
	offset += ML_MLCNTRL_SZ;

	comm_info_len = *(multi_link + offset);
	mlsta_info->comm_info_len = comm_info_len;
	offset += ML_COMM_INFO_LEN_SZ;

	os_memcpy(mlsta_info->mld_addr, multi_link + offset, ML_COMM_MLDADDR_SZ);
	offset += ML_COMM_MLDADDR_SZ;

	if (ml_ctrl & ML_MLCNTRL_PRESN_LINKID_MASK) {
		os_memcpy(&mlsta_info->link_id, multi_link + offset, ML_COMM_LINKID_SZ);
		offset += ML_COMM_LINKID_SZ;
	}
	if (ml_ctrl & ML_MLCNTRL_PRESN_BSSPARM_MASK) {
		os_memcpy(&mlsta_info->bss_parm_change, multi_link + offset, ML_COMM_BSSPARM_SZ);
		offset += ML_COMM_BSSPARM_SZ;
	}
	if (ml_ctrl & ML_MLCNTRL_PRESN_MSD_MASK) {
		os_memcpy(&mlsta_info->msd_info, multi_link + offset, ML_COMM_MSD_SZ);
		offset += ML_COMM_MSD_SZ;
	}
	if (ml_ctrl & ML_MLCNTRL_PRESN_EMLCAP_MASK) {
		os_memcpy(&mlsta_info->eml_cap, multi_link + offset, ML_COMM_EMLCAP_SZ);
		offset += ML_COMM_EMLCAP_SZ;
	}
	if (ml_ctrl & ML_MLCNTRL_PRESN_MLDCAP_MASK) {
		os_memcpy(&mld_cap, multi_link + offset, ML_COMM_MLDCAP_SZ);
		offset += ML_COMM_MLDCAP_SZ;
		numlinks = (mld_cap & ML_MLDCAP_NUMLINKS_MASK) + 1;
		mlsta_info->numlinks = numlinks;
		mlsta_info->mld_cap = mld_cap;
	}
	if (ml_ctrl & ML_MLCNTRL_PRESN_MLDID_MASK) {
		os_memcpy(&mlsta_info->mld_id, multi_link + offset, ML_COMM_MLDID_SZ);
		offset += ML_COMM_MLDID_SZ;
	}

	wpa_printf(MSG_DEBUG, "%s STA MLDAddr "MACSTR" MLDCap 0x%x numlinks %d "
			"multi_link_len %d\n",
			__FUNCTION__, MAC2STR(mlsta_info->mld_addr),
			mld_cap, numlinks, multi_link_len);
	/* No PerSTA info in MLIE */
	if (multi_link_len <= (comm_info_len + ML_MLCNTRL_SZ)) {
		return;
	}

	/* Parse multi-link common info fields before this line */
	offset = (comm_info_len + ML_MLCNTRL_SZ);
	for (i = 0; i < (numlinks - 1); i++) {
		struct ieee802_11_subelems subelems = { 0 };
		struct wpabuf *perstabuf = NULL;
		const u8 *perstaprof = NULL;
		u16 sta_cntrl = 0;
		u8 sta_linkid = 0, sta_info_len = 0, sta_prof_offset = 0;

		if (numlinks <= 1)
			break;

		if (offset >= multi_link_len) {
			break;
		}

		perstaprof = (u8 *)(multi_link + offset);
		wpa_printf(MSG_DEBUG, "iter %d ieee802_11_mlo_parse_ml_info offset %d "
				"perstaprof 0x%x\n", i, offset, *perstaprof);

		mlo_parse_persta_info(perstaprof, (multi_link_len - offset), &subelems);
		if (subelems.persta_link_info) {

			/* PERSTA in received assocreq can be Fragmented take care of it */
			perstabuf = ieee802_11_subele_defrag(&subelems,
					WLAN_SUBEID_PERSTA_PROF);
			if (!perstabuf) {
				wpa_printf(MSG_DEBUG, "Assoc: MLIE: PerSTA Missing\n");
				return;
			}
			(mlsta_info->persta[i]).perstabuf = perstabuf;

			wpa_printf(MSG_DEBUG, "Assoc: MLIE: PerSTA len %d index %d frags %d\n",
					wpabuf_len(perstabuf), i, (subelems.frag_ies).n_frags);
			wpa_hexdump(MSG_ERROR, "###FUll Assoc Req PerSTA : ",
					wpabuf_head(perstabuf),
					wpabuf_len(perstabuf));
		}

		perstaprof = (u8 *)wpabuf_head(perstabuf);
		os_memcpy(&sta_cntrl, perstaprof, ML_PERSTA_CNTL_SZ);
		(mlsta_info->persta[i]).sta_cntrl = sta_cntrl;
		sta_linkid = sta_cntrl & ML_PERSTA_LINKID_MASK;
		perstaprof += ML_PERSTA_CNTL_SZ;
		sta_prof_offset += ML_PERSTA_CNTL_SZ;
		sta_info_len = *perstaprof;
		(mlsta_info->persta[i]).sta_info_len = sta_info_len;
		perstaprof += ML_PERSTA_STAINFOLEN_SZ;
		sta_prof_offset += ML_PERSTA_STAINFOLEN_SZ;
		wpa_printf(MSG_DEBUG, "sta_linkid %d sta_cntrl 0x%x\n", sta_linkid, sta_cntrl);
		if (sta_cntrl & ML_PERSTA_PRESN_STAMAC_MASK) {
			/* saving address and linkid */
			(mlsta_info->persta[i]).linkid = sta_linkid;
			os_memcpy(((mlsta_info->persta[i]).sta_addr),
				perstaprof, 6);
			perstaprof += ML_PERSTA_STAMAC_SZ;
			sta_prof_offset += ML_PERSTA_STAMAC_SZ;
			wpa_printf(MSG_DEBUG, "%s STA linkid %d "
				"STA LinkAddr "MACSTR" \n", __FUNCTION__, sta_linkid,
				MAC2STR((mlsta_info->persta[i]).sta_addr));
		}

		if (sta_cntrl & ML_PERSTA_PRESN_BCNITVL_MASK) {
			os_memcpy(&((mlsta_info->persta[i]).bcn_intvl), perstaprof,
					ML_PERSTA_BCNITVL_SZ);
			perstaprof += ML_PERSTA_BCNITVL_SZ;
			sta_prof_offset += ML_PERSTA_BCNITVL_SZ;
		}
		if (sta_cntrl & ML_PERSTA_PRESN_TSFOFF_MASK) {
			perstaprof += ML_PERSTA_TSFOFF_SZ;
			sta_prof_offset += ML_PERSTA_TSFOFF_SZ;
		}
		if (sta_cntrl & ML_PERSTA_PRESN_DTIM_MASK) {
			os_memcpy(&((mlsta_info->persta[i]).dtim_info), perstaprof,
					ML_PERSTA_DTIM_SZ);
			perstaprof += ML_PERSTA_DTIM_SZ;
			sta_prof_offset += ML_PERSTA_DTIM_SZ;
		}
		if (sta_cntrl & ML_PERSTA_PRESN_NSTR_MASK) {
			os_memcpy(&((mlsta_info->persta[i]).nstr_bmp), perstaprof,
					ML_PERSTA_NSTR_SZ);
			perstaprof += ML_PERSTA_NSTR_SZ;
			sta_prof_offset += ML_PERSTA_NSTR_SZ;
		}
		if (sta_cntrl & ML_PERSTA_PRESN_BSSPARM_MASK) {
			os_memcpy(&((mlsta_info->persta[i]).bss_parm_count), perstaprof,
					ML_PERSTA_BSSPARM_SZ);
			perstaprof += ML_PERSTA_BSSPARM_SZ;
			sta_prof_offset += ML_PERSTA_BSSPARM_SZ;
		}

		(mlsta_info->persta[i]).sta_prof_offset = sta_prof_offset;
		/* Include subelem ID and len and fragments if any */
		offset += wpabuf_len(perstabuf) + WLAN_SUBEID_FRAG_HDR + (WLAN_SUBEID_FRAG_HDR *
				(subelems.frag_ies.n_frags));
		wpa_printf(MSG_DEBUG, "iter %d ieee802_11_mlo_parse_ml_info offset %d \n",
				i, offset);
	}
	wpa_printf(MSG_ERROR, "STA Assoc with numlinks %d MLDAddr "MACSTR"\n",
			mlsta_info->numlinks, MAC2STR(mlsta_info->mld_addr));
	for (i = 0; i < (numlinks - 1); i++) {
		wpa_printf(MSG_ERROR, "Index %d STA linkid %d Linkaddr "MACSTR""
			" sta_prof_offset %d\n",
			i, (mlsta_info->persta[i]).linkid,
			MAC2STR((mlsta_info->persta[i]).sta_addr),
			(mlsta_info->persta[i]).sta_prof_offset);
	}
}
#endif /* CONFIG_DRIVER_BRCM_MLO */
...
#ifdef CONFIG_DRIVER_BRCM_MLO
		case WLAN_EID_EXT_MULTI_LINK:
			data = elems->multi_link;
			len = elems->multi_link_len;
			break;
#endif /* CONFIG_DRIVER_BRCM_MLO */
		default:
			wpa_printf(MSG_DEBUG,
...
#ifdef CONFIG_DRIVER_BRCM_MLO
struct wpabuf * ieee802_11_subelem_defrag_data(struct ieee802_11_subelems *subelems,
		u8 subeid, const u8 *data, u8 len)
{
	struct frag_ies_info *frag_ies = &subelems->frag_ies;
	struct wpabuf *buf;
	unsigned int i;

	if (!subelems || !data || !len)
		return NULL;
...
struct wpabuf * ieee802_11_subele_defrag(struct ieee802_11_subelems *subelems,
		u8 subeid)
{
	const u8 *data;
	u8 len;
	...
	/*
	 * TODO: Defragmentation mechanism can be supported for all Sub Element IEs.
	 * For now handle only those that are used (or use ieee802_11_subelem_defrag_data()).
	 */
	switch (subeid) {
	case WLAN_SUBEID_PERSTA_PROF:
		data = subelems->persta_link_info;
		len = subelems->persta_link_len;
		break;
	default:
		wpa_printf(MSG_DEBUG,
			"Defragmentation not supported. eid_ext=%u", subeid);
		return NULL;
	}

	return ieee802_11_subelem_defrag_data(subelems, subeid, data, len);
}
#endif /* CONFIG_DRIVER_BRCM_MLO */
