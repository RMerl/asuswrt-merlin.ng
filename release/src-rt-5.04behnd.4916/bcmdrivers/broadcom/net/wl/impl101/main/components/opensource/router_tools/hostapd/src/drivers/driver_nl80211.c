...
static void nl80211_refresh_nlmode(struct wpa_driver_nl80211_data *drv,
				int ifindex)
{
	struct i802_bss *bss;
	enum nl80211_iftype nlmode;
	...
	if (bss) {
		...
		wpa_printf(MSG_ERROR, "nl80211: re-configure driver mode to %d",
			nlmode);
		if (wpa_driver_nl80211_set_mode(bss, nlmode) < 0) {
			wpa_printf(MSG_ERROR, "nl80211: Could not re-configure"
				" driver mode");
		}
	}
}
...
#ifdef CONFIG_DRIVER_BRCM
			/* Re-configure the nlmode */
			nl80211_refresh_nlmode(drv, ifi->ifi_index);
#endif
...
static int wpa_driver_nl80211_del_beacon_bss(struct i802_bss *bss)
{
	...
	nl80211_put_wiphy_data_ap(bss);
	return send_and_recv_msgs(drv, msg, NULL, (void *) -1, NULL, NULL);
}
#endif /* CONFIG_DRIVER_BRCM */
/**
				     ...
				     struct wpa_driver_ap_params *params)
{
	...
	if (drv->device_ap_sme &&
	    (params->key_mgmt_suites & (WPA_KEY_MGMT_SAE |  WPA_KEY_MGMT_SAE_EXT_KEY)) &&
