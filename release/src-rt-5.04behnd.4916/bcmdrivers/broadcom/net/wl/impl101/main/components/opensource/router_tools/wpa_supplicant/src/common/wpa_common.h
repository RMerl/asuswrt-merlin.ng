...
#define RSN_KEY_DATA_MAP_GROUPKEY RSN_SELECTOR(0x50, 0x6f, 0x9a, 0)
...
#ifdef CONFIG_DRIVER_BRCM
#define WPA_MLO_GTK_LEN 16
#define WPA_MLO_GTK_MAX_LEN 32
#define WPA_MLO_IGTK_LEN 16
#define WPA_MLO_IGTK_MAX_LEN 32
#define WPA_MLO_BIGTK_LEN 16
#define WPA_MLO_BIGTK_MAX_LEN 32
#endif /* CONFIG_DRIVER_BRCM */
...
struct wpa_mlo_gtk_kde {
	u8 keyid; /* contains keyid, tx and link */
	u8 pn[6];
	u8 mlo_gtk[WPA_MLO_GTK_MAX_LEN];
} STRUCT_PACKED;
...
struct wpa_mlo_igtk_kde {
	...
	u8 ipn[6];
	u8 link;
	u8 mlo_igtk[WPA_MLO_IGTK_MAX_LEN];
} STRUCT_PACKED;
...
struct wpa_mlo_bigtk_kde {
	...
	u8 bipn[6];
	u8 link;
	u8 mlo_bigtk[WPA_MLO_BIGTK_MAX_LEN];
} STRUCT_PACKED;

#define WPA_MLO_LINK_KDE_PREFIX_LEN (1 + 6)
#define RSN_MLO_LINK_KDE_LI_RSNE_INFO	0x10 /* RSNE present Bit */
#define RSN_MLO_LINK_KDE_LI_RSNXE_INFO	0x20 /* RSNXE present Bit */
...
struct wpa_eapol_ie_parse {
	...
	const u8 *mlo_link;
	size_t mlo_link_len;
	const u8 *mlo_link_kde[MAX_NUM_LINKS];
	size_t mlo_link_count;
	const u8 *mlo_gtk[4];
	size_t mlo_gtk_len[4];
	size_t mlo_gtk_count;
	const u8 *mlo_igtk[4];
	size_t mlo_igtk_len[4];
	size_t mlo_igtk_count;
	const u8 *mlo_bigtk[4];
	size_t mlo_bigtk_len[4];
	size_t mlo_bigtk_count;
#endif /* CONFIG_DRIVER_BRCM */
};
