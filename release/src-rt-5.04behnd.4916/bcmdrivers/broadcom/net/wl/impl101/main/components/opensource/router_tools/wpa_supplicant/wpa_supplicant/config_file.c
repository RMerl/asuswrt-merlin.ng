...
struct wpa_config * wpa_config_read(const char *name, struct wpa_config *cfgp)
{
	...
	/* initializing default values to avoid issues with disabled mlo*/
	config->mld_unit = -1;
	config->link_id = -1;
#endif
...
static void wpa_config_write_global(FILE *f, struct wpa_config *config)
{
	...
	if (config->link_id != -1)
		fprintf(f, "link_id=%d\n", config->link_id);
	if (config->mld_unit != -1)
		fprintf(f, "mld_unit=%d\n", config->mld_unit);
	if (config->mld_link_pref)
		fprintf(f, "mld_link_pref=%d\n", config->mld_link_pref);
#endif /* CONFIG_DRIVER_BRCM_MLO */
#ifdef CONFIG_DRIVER_BRCM_MAP
	if (config->map_4_way_handshake)
		fprintf(f, "map_4_way_handshake=%d\n",
			config->map_4_way_handshake);
