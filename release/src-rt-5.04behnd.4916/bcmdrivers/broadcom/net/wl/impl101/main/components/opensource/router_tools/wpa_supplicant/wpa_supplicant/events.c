...
void wpas_network_reenabled(void *eloop_ctx, void *timeout_ctx)
{
...
#if 1
	wpa_dbg(wpa_s, MSG_DEBUG, "wpas_network_reenabled. check check! disconnected %d wpa_state %d wpa_s %p \n"
		, wpa_s->disconnected, wpa_s->wpa_state,wpa_s);
#endif
	...
	if (!wpa_s->disconnected && (wpa_s->conf->ap_scan == 1)) {
		if (wpa_s->wpa_state == WPA_DISCONNECTED) {
			...
			"wpas_network_reenabled. Ares try re-enable network and return" );
				/* copied from wpa_supplicant_ctrl_iface_enable_network */			
			wpa_supplicant_enable_network(wpa_s, NULL);
		} else if (!wpa_s->disconnected && (wpa_s->wpa_state >= WPA_SCANNING) &&
			(wpa_s->wpa_state <WPA_COMPLETED)) {

			wpa_dbg(wpa_s, MSG_DEBUG, "wpas_network_reenabled: Ares keep monitoring for 10 seconds.");
				eloop_register_timeout(3, 0, wpas_network_reenabled, wpa_s, NULL);
		} else
			wpa_dbg(wpa_s, MSG_DEBUG, "wpas_network_reenabled: Ares simply return, dont do anything.!!");
