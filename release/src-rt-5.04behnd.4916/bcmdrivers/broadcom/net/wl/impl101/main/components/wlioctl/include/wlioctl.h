 ...
 *
 * $Id: wlioctl.h 831328 2023-10-14 09:40:07Z $
 */
...
typedef struct wlc_scb_mlo_stats {
	uint16 ver;
	uint16 len;			/* length in bytes of this structure */
...
typedef struct wlc_scb_mlo_rssi_sample {
	int	rssi_window_sz;
	int8	rssi_window[MLO_MA_WINDOW_SZ];
	int8	rssi_chain_window[WL_RSSI_ANT_MAX][MLO_MA_WINDOW_SZ];
} wlc_scb_mlo_rssi_sample_t;

typedef struct wlc_scb_mlo_stats_v2 {
	struct ether_addr mld_addr;
	uint16 ver;
	uint16 len;				/* length in bytes of this structure */
	...
	uint32 rx_pkts[MAX_MLO_LINKS];
	int active_link_bmp;
	wlc_scb_mlo_rssi_sample_t rssi_perlink[MAX_MLO_LINKS - 1];
	uint32 rx_rspec[MAX_MLO_LINKS]; /* rx rspec of MAP and AAP's */
} wlc_scb_mlo_stats_v2_t;
