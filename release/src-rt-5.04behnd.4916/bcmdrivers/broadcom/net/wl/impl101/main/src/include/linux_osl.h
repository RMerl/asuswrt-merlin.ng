 ...
 *
 * $Id: linux_osl.h 833572 2023-11-29 11:59:26Z $
 */
...
/* ASSERT */
	...
	#define WL_ASSERT(exp, osh) \
		do { if (!(exp)) osl_assert(#exp, __FILE__, __LINE__, (osh)); } while (0)
	#define ASSERT(exp) WL_ASSERT(exp, NULL)
extern void osl_assert(const char *exp, const char *file, int line, osl_t *osh);
#else
	#ifdef __GNUC__
		#define GCC_VERSION \
 ...
 * the requirement)
 */
#define	DMA_ALLOC_CONSISTENT_BOUNDARY(osh, size, align, boundarybits, tot, pap, dmah) \
