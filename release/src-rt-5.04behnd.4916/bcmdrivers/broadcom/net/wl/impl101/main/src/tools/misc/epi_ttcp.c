 ...
 *
 * $Id: epi_ttcp.c 821234 2023-02-06 14:16:52Z $
 */
	...
	if (sockbufsize) {
		...
		*/
		...
		getsockopt(fd, SOL_SOCKET, SO_RCVBUF, (char *)&sockbufrcvsize,
		           &sockbuflen);
#else
		if (getsockopt(fd, SOL_SOCKET, SO_SNDBUF, &sockbufsndsize, &sockbuflen) !=0)
		{
			sockets_err("getsockopt: sndbuf");
		}
		sockbuflen = sizeof sockbufrcvsize;
		if (getsockopt(fd, SOL_SOCKET, SO_RCVBUF, &sockbufrcvsize, &sockbuflen) !=0)
		{
			sockets_err("getsockopt: rcvbuf");
		}
#endif /* __lynx__ __sun__ __NTVIS__ */
...
#if defined(sgi) || ( defined(__osf__) && !defined(_CFE) ) || defined(BCA_CPEROUTER)
			if( listen(fd,1) <0 )
#else
...
Nwrite( int s, void* bufp, int count )
{
	...
	again:
		...
		{
			sockets_err("Get file status flags failed");
		}
		if (fcntl(s,F_SETFL, val | O_NONBLOCK) == -1)
		{
			sockets_err("Set file status flags failed");
		}
