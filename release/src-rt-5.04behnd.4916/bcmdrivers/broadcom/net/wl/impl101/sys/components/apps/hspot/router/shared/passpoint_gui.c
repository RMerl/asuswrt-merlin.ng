 ...
 *
 * $Id: passpoint_gui.c 821338 2023-02-07 19:57:04Z $
 */
...
delete_icon_file(char *filename)
{
	...
	ret_code = 1;
	snprintf(fname, sizeof(fname), USERDEFINED_ICONPATH"%s", filename);
	if (remove(fname) == -1)
		ret_code = -1;
	return ret_code;
}
...
validate_wl_hsflag(webs_t wp, char *value, struct variable *v, char *varname)
{
	...
	if ((val = websGetVar(wp, "wl_osenen", NULL)) != NULL) {
		...
		if ((osen_val > 0) && !find_in_list(akms, NVVAL_OSEN)) {
			ret_code = nvram_set(nvname, NVVAL_OSEN);
		...
		/* If OSEN Value = 0, And wlX_akm having "osen", Remove it */
			...
			ret_code = nvram_set(nvname, akms);
		}
	}
...
validate_wl_netauthlist(webs_t wp, char *value, struct variable *v, char *varname)
{
	...
	ret_code = nvram_set(NVNM_WL_NETAUTHLIST, nvram_val);
...
validate_wl_realmlist(webs_t wp, char *value, struct variable *v, char *varname)
{
	...
	ret_code = nvram_set(NVNM_WL_REALMLIST, nvram_val);
...
validate_wl_venuelist(webs_t wp, char *value, struct variable *v, char *varname)
{
	...
	ret_code = nvram_set(NVNM_WL_VENUELIST, ptrUTF8);
...
validate_wl_ouilist(webs_t wp, char *value, struct variable *v, char *varname)
{
	...
	ret_code = nvram_set(NVNM_WL_OUILIST, nvram_val);
...
validate_wl_3gpplist(webs_t wp, char *value, struct variable *v, char *varname)
{
	...
	ret_code = nvram_set(NVNM_WL_3GPPLIST, nvram_val);
...
MAKE_DSCP_RANGE_FF:
	...
	ret_code = nvram_set(NVNM_WL_QOSMAPIE, nvram_val);
...
validate_wl_wanmetrics(webs_t wp, char *value, struct variable *v, char *varname)
{
	...
	ret_code = nvram_set(NVNM_WL_WANMETRICS, nvram_val);
...
validate_wl_oplist(webs_t wp, char *value, struct variable *v, char *varname)
{
	...
	ret_code = nvram_set("wl_oplist", nvram_val);
...
validate_wl_homeqlist(webs_t wp, char *value, struct variable *v, char *varname)
{
	...
	ret_code = nvram_set(NVNM_WL_HOMEQLIST, nvram_val);
...
validate_wl_concaplist(webs_t wp, char *value, struct variable *v, char *varname)
{
	...
	ret_code = nvram_set(NVNM_WL_CONCAPLIST, nvram_val);
...
validate_wl_osuplist(webs_t wp, char *value, struct variable *v, char *varname)
{
	...
	ret_code = nvram_set(v->name, nvram_val);
