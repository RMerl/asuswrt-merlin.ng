 ...
 *
 * $Id: dhd.h 833666 2023-11-30 13:10:21Z $
 */
...
#define       BCM_TRACE_RING_NAME "bcm_trace_ring"
#endif /* SHOW-LOGTRACE */
 ...
 * increase to 32K to support 6726 and 67263
 */
...
#define DHD_MAX_RADIO		(PCIE_IPC_AP_UNIT_MAX)
#define FWDER_MAX_RADIO         (DHD_MAX_RADIO)
#endif /* BCM_ROUTER */

/* Maximum STA per radio */
#if defined(BCM_ROUTER)
#define DHD_MAX_STA     512
#else
...
#define DHD_BUF_NAME_LEN	256u
typedef struct dhd_buf_info {
	char name[DHD_BUF_NAME_LEN];
	uint8 *buf;
	uint len;
} dhd_buf_info_t;
...
#endif /* for BCM_ROUTER || TRAFFIC_MGMT_DWM */
 ...
 *
 * DHD can be build to use a single perimeter lock, as opposed to a per queue lock.
 * Cummulative counters in parent objects may be updated without spinlocks.
 *
 * When using per queue lock, if a cummulative queue length is desired across all
 * flows belonging to either of (a station, or an interface or a dhd instance),
 * then an atomic operation is required using an atomic_t cummulative counters or
 * using a spinlock. BCM_ROUTER uses the Linux atomic_t construct when built using
 * per queue lock.
 */
#if defined(BCM_ROUTER)
...
typedef atomic_t cumm_ctr_t;       /* BCM_ROUTER Linux: atomic operations */
#define DHD_CUMM_CTR_PTR(clen)     ((cumm_ctr_t*)(clen))
...
/* r/w indices in TCM or host memory based on FW/Host agreement */
...
#define HEALTH_CHK_BUF_SIZE WL_HEALTH_CHECK_MODULE_BUF_MAXSIZE

/* Health check file will work like a circular buffer, should hold ~10 reports+its headers */
#define HEALTH_CHK_FILE_MAX_SIZE 4096

typedef enum log_dump_section_type {
...
typedef enum pcap_states
{
	PCAP_STATE_DEINIT = 0x0,
	PCAP_STATE_INIT = 0x1,
	PCAP_STATE_INTERFACE_ADDED = 0x2,
	PCAP_STATE_INTERFACE_DELETED = 0x4
} pcap_states_t;

enum pcap_pkt_type
{
	PCAP_PKT_RX_INVALID = 0,
	PCAP_PKT_RX_PROMISC,
	PCAP_PKT_RX_COPY,
	PCAP_PKT_TX_COPY,
	PCAP_PKT_TX_LOCAL_COPY,
	PCAP_PKT_TX_COPY_AMSDU_FIRST,
	PCAP_PKT_TX_COPY_AMSDU_INTRM,
	PCAP_PKT_TX_COPY_AMSDU_LAST
};

typedef struct pcap_skb_data {
	uint8	pkt_type;	/* pcap_pkt_type */
} pcap_skb_data_t;

#define PCAP_SKB_DATA(skb)		((pcap_skb_data_t *)((skb)->cb))
#define PCAP_SKB_DATASIZE		(sizeof(((struct sk_buff *)0)->cb))
#define PCAP_SKB_PKTTYPE(skb)		(PCAP_SKB_DATA(skb)->pkt_type)

typedef struct pcap_interface {
	int radiotap_enabled;
	struct net_device* real_ndev;	/* The real interface that the monitor is on */
	struct net_device* pcap_ndev;
} pcap_interface;

#if defined(PCIE_FULL_DONGLE) && defined(BCM_PCAP)
typedef struct {
	pcapxf_ipc_hme_t *ipc_hme;
	pcapxf_ring_elem_t *table;
	uint32 *wr_ptr;
	uint32 *rd_ptr;
	bcm_ring_t ring;
	uint32 num_recvd;
	uint32 polling_ticks;
} pcap_xfer_t;

/* Max headers and physts must be power of 2 */
#define MAX_PKT_HDRS	2048
#define D11HDR_IDX_MSK	(MAX_PKT_HDRS - 1)
#define MAX_PHYSTS_HDRS	512
#define PHYSTS_IDX_MSK	(MAX_PHYSTS_HDRS - 1)

typedef struct _pcap_frame {
	uint32 pktid;
	uint16 retries;
	void *frm_ptr;
} pcap_frame_t;

typedef struct _pcap_d11_hdr {
	uint32 pktid;
	uint32 phy_seq;
	uint16 pad_len;
	uint16 machdr_len;
	uint16 retries;
	uint8 d11hdr[D11RXHDR_LEN];
	uint8 mac_hdr[MAX_MACHDR_LEN];
} pcap_d11_hdr_t;

typedef struct _pcap_phy_hdr {
	uint32 phy_seq;
	uint16 physts_len;
	uint8 physts[PHYRXSTS_LEN];
} pcap_phy_hdr_t;

typedef struct {
	uint16	frm_list_rd;
	uint16	frm_list_wr;
	pcap_frame_t frm_list[MAX_PKT_HDRS];
	uint16	d11hdr_list_rd;
	uint16	d11hdr_list_wr;
	pcap_d11_hdr_t d11_hdr_list[MAX_PKT_HDRS];
#ifdef BCM_PCAP_PHYSTS
	uint16	phy_list_rd;
	uint16	phy_list_wr;
	pcap_phy_hdr_t phy_hdr_list[MAX_PHYSTS_HDRS];
#endif /* BCM_PCAP_PHYSTS */
} pcap_hdr_list_info_t;

/* PCAP packet packet tag - skb->cb */
typedef struct {
	uint16 fifo;
} pcap_pkttag_t;

#endif /* PCIE_FULL_DONGLE && BCM_PCAP */

typedef struct pcap_rx_stats {
	/* rx-copy stats */
	uint32 rxpkt_alloc;		/* rx-copy pkts allocated */
	uint32 rxpkt_tot;		/* rx-copy pkts */
	uint32 msdu_cnt;		/* rx-copy forwarded msdu */
	uint32 msdu_tot;		/* rx-copy total msdu */
	uint32 rxpkt_drop_alloc;	/* rx-copy pkts alloc failed */
	uint32 rxpkt_drop_list;		/* rx-copy pkts dropped: list full */
	uint32 msdu_bytes;		/* rx-copy pkt bytes */

	uint32 rxpkt_merge;		/* num of merging runs */
	uint32 rxpkt_retry_hdr;		/* rx-copy pkts retried: missing d11hdr */
	uint32 rxpkt_drop_hdr;		/* rx-copy pkts dropped: missing d11hdr */
	uint32 rxpkt_retry_phy;		/* rx-copy pkts retried: missing physts */
	uint32 rxpkt_drop_phy;		/* rx-copy pkts dropped: missing physts */
	uint32 rxpkt_drop_flush;	/* rx-copy pkts dropped: flushed */
	uint32 rxpkt_fwrd;		/* rx-copy pkts forwarded */
	uint32 rxpkt_enqd;		/* rx-copy pkts enqued */

	uint32 hdr_rcvd;		/* num d11hdr received */
	uint32 phy_rcvd;		/* num physts received */
	uint32 hdr_full;		/* d11hdr drops: d11hdr list full */
	uint32 phy_full;		/* physts drops: physts list full */
} pcap_rx_stats_t;

typedef struct dhd_linux_pcap {
	bool pcap_en;			/* Packet capture supported */
	bool inited;		/* PCAP first time init */
	/* max len in bytes to send to packet capture interface, per data MPDU */
	uint16 pcap_maxlen;
	uint64 pcap_filter;	/* Type and subtype frames to be captured */
	uint32 pcap_config;		/* configuration from dongle */
	uint16 h2d_pcap_subn_ringid;	/* Reserved PCAP ring id */
	uint16 d2h_pcap_cpln_ringid;	/* Reserved PCAP ring id */
	pcap_states_t pcap_state;
	pcap_interface pcap_if;
	struct list_head skb_list;
	void *skb_list_lock;
	uint32 last_scheduled;
#if defined(PCIE_FULL_DONGLE) && defined(BCM_PCAP)
	pcap_xfer_t pcap_xfer;
	pcap_hdr_list_info_t pcap_list_info;
	bool pcap_poll_mode;
#endif /* BCM_PCAP */
	/* Counters */
	uint32 tot_pkts_recd;		/* all pkts received by DHD with promisc capture */
	uint32 mgmt_pkts_recd;		/* mgmt pkts received by DHD with promisc capture */
	uint32 mgmt_pkts_sent;		/* mgmt pkts sent to dongle for mgmt proto handling */
	uint32 mgmt_pkts_acked;		/* mgmt pkts indicated by dongle to be handled */
	uint32 local_tx_pkts;		/* dongle generated tx pkts to be sent to radiotap */
	uint32 inv_pkttype_cpl;		/* completion received for an invalid pkt type */

	/* rx-copy stats */
#ifdef BCM_PCAP
	pcap_rx_stats_t rxstats;	/* pcap rx-copy stats */
#endif /* BCM_PCAP */
} dhd_linux_pcap_t;

#define DHD_PCAP_EN(dhd)		((dhd)->pcap.pcap_en)
#define DHD_PCAP_RXCPY_EN(dhd)		((dhd)->pcap.pcap_config & WLC_PCAP_RXCOPY_EN)
#define DHD_PCAP_RX_FSTMSDU(dhd)	((dhd)->pcap.pcap_config & WLC_PCAP_RX_FIRST_MSDU)
#define DHD_PCAP_RX_FSTMPDU(dhd)	((dhd)->pcap.pcap_config & WLC_PCAP_RX_FIRST_MPDU)
#define DHD_PCAP_H2D_RINGID(dhd)	((dhd)->pcap.h2d_pcap_subn_ringid)
#define DHD_PCAP_D2H_RINGID(dhd)	((dhd)->pcap.d2h_pcap_cpln_ringid)
#define DHD_PCAP_RXCPY_KEEP_BYTES(dhd)	((dhd)->pcap.pcap_config & WLC_PCAP_RX_KEEP_BYTES)
...
typedef struct dhd_pub {
	...
	struct eldbg_cmn *dbg;	/* Debugability module handle */
	...
	bool btrace_enable;	/* btrace enable flag */
	...
	*/
	...
	bool	halt_db_sent;		/* flag to tell whether a halt db was sent to dongle */
	int   hang_was_sent;
	...
	bool idma_inited;           /* implicit dma init */
	uint8 idma_set;             /* number of implicit dma set */
	uint8 h2d_idma_set;         /* number of h2d implicit dma set */
	...
	uint8 buzzz_event_state;			/* Check buzzz start/stop event state */
	bool buzzz_streaming_ext;			/* Check streaming support */
...
#endif /* SHOW_LOGTRACE */
#if defined(MLO_IPC)
	void	* dhd_mlo_ipc;		/* InterAP IPC for MLO */
	bool tmp_data_filter;
#endif /* MLO_IPC */
#ifdef DHD_SBF
	void *sbf;			/* SBF control block */
	uint32 sbf_tx_buff_inuse;	/* Tx buffers in use */
#endif /* DHD_SBF */
	bool tid_from_dongle;
#ifdef BCM_RXCPLE_RCH
	bool	rxcple_rch;
	uint16	max_rch_sdu_cnt;        /* Max SDU's in an RCH */
	uint16	bcmpcie_rxcple_rch_size; /* Max size of RCH in headroom */
#endif /* BCM_RXCPLE_RCH */
#if defined(BCMPCIE)
	bool  cpudbg_wl_support;	/* TRUE=dongle IPC revision >= 0x87 */
	bool  cpudbg_wk_scheduled;	/* TRUE=don't schedule new worker task */
	bool  cpudbg_syncreq;		/* need to sync up with seq # 0 */
	bool  cpudbg_smac_strs_valid;	/* FALSE=invalid smac dbgstrs detected */
	bool  cpudbg_mlo_strs_valid;	/* FALSE=invalid mlo dbgstrs detected */
	bool  cpudbg_wraparound;	/* TRUE=DMA buf WrIdx wrapped around */
	uint16 cpudbg_seqnum;		/* CPU DBG current sequence number */
	uint32 cpudbg_rdidx;		/* CPU DBG current entry read index */
	uint32 cpudbg_wridx;		/* CPU DBG last entry write index */
	char  *cpudbg_smac_strs;	/* ptr to smac dbgstrs */
	int   cpudbg_smac_strs_len;	/* length of smac dbgstrs */
	char  *cpudbg_mlo_strs;		/* ptr to mlo dbgstrs */
	int   cpudbg_mlo_strs_len;	/* length of mlo dbgstrs */
	psm_label_t *cpudbg_psmr_labels; /* ptr to psmr labels array */
	int   cpudbg_psmr_labels_num;	/* number of psmr labels */
	uint32 cpudbg_trace_lines;	/* number of lines in current CPU DBG trace file */
	uint32 cpudbg_trace_fidx;	/* current CPU DBG trace file index */
	uint32 cpudbg_config;		/* CPU DBG m2mdma xfer size, etc */
	uint32 cpudbg_max_fidx;		/* max CPU DBG trace file index */
	uint32 *cpudbg_hme;		/* pointer to CPU DBG HME memory */
	char   cpudbg_printbuf[CPUDBG_BUFSZ]; /* print buffer */
	daddr32_t cpudbg_rdidx_daddr32;  /* CPU DBG M2M DMA RdIdx reg access dev addr */
	cpudbg_ctrs_t cpudbg_ctrs;	/* Total counts of printed & dropped entries */
#endif /* defined(BCMPCIE) */
#ifdef BCM_IOCV_MEM_LOG
	void *dld;			/* Handle for logging to memory */
#endif /* BCM_IOCV_MEM_LOG */
#if defined(HNDUCLS)
	ucls_cap_t ucls_cap;
	uint16 ucls_ring_size;		/* UCLS ring buffer size */
	uint16 ucls_ring_depth;		/* UCLS ring buffer depth */
	uint32 ucls_daddr32;		/* UCLS registers emulation address */
	uint16 ucls_rdidx;		/* UCLS read index (local copy) */
	uint16 ucls_wridx;		/* UCLS write index (local copy) */
	char *ucls_buf;			/* Pointer to the HME UCLS buffer */
	uint32 ucls_cursor;		/* Index in the line buffer */
	char *ucls_line;		/* print buffer */
#endif /* HNDUCLS */
#if defined(BCM_HMO_EVENT)
	bool  hmo_event_up;
	uint16 hmo_event_ring_size;	/* HMOEVENT ring buffer size */
	uint16 hmo_event_ring_depth;	/* HMOEVENT ring buffer depth */
	uint32 hmo_event_rd_daddr32;	/* HMOEVENT read index address in dongle memory */
	uint32 hmo_event_wr_daddr32;	/* HMOEVENT write index address in dongle memory */
	uint16 hmo_event_rdidx;		/* HMOEVENT read index (local copy) */
	uint16 hmo_event_wridx;		/* HMOEVENT write index (local copy) */
	char *hmo_event_buf;		/* Pointer to the HME HMOEVENT buffer */
#endif /* BCM_HMO_EVENT */
#ifdef SHOW_LOGTRACE
	uint64 logset_prsrv_mask;
#endif /* SHOW_LOGTRACE */
	dhd_linux_pcap_t pcap;
#ifdef WL_MONITOR
	struct net_device *monitor_dev; /* monitor pseudo device */
	uint monitor_type;
	struct sk_buff *monitor_skb; /* rx AMSDU reassembly */
	struct sk_buff *tx_monitor_skb; /* tx AMSDU reassembly */
...
struct sk_buff* dhd_pcap_copy(dhd_pub_t *pub, void *pkt, uint8 pcap_pkt_type);
struct sk_buff* dhd_pcap_dup(dhd_pub_t *pub, void *pkt, uint16 size, uint8 pcap_pkt_type);
...
typedef struct dhd_pkttag_fd {
	...
	void      *secdma; /* secure dma sec_cma_info handle */
#ifdef MLO_BCMC
	uint16	  bcmc_seq; /* 802.11 seqno (12bits) for mlo bcmc pkt.
			     * bit15 is set after assigning seqno to pkttag
			     */
#endif /* MLO_BCMC */
#if defined(BCM_NBUFF)
...
#ifdef MLO_BCMC
#define DHD_PKTTAG_MLO_BCMC		0x0100	/* tag as MLO BCMC pkt */
#define DHD_PKT_SET_MLO_BCMC(pkt)	((DHD_PKTTAG_FD((pkt)))->flags |= DHD_PKTTAG_MLO_BCMC)
#define DHD_PKT_CLR_MLO_BCMC(pkt)	((DHD_PKTTAG_FD((pkt)))->flags &= ~DHD_PKTTAG_MLO_BCMC)
#define DHD_PKT_GET_MLO_BCMC(pkt)	\
	(((DHD_PKTTAG_FD((pkt)))->flags & DHD_PKTTAG_MLO_BCMC) ? TRUE : FALSE)
#define DHD_MLO_BCMC_ASSIGN_PKTTAG_SEQ(pkt, seq)	\
		((DHD_PKTTAG_FD((pkt))->bcmc_seq) = (seq))
#define DHD_MLO_BCMC_PKTTAG_SEQ(pkt)	\
		(DHD_PKTTAG_FD((pkt))->bcmc_seq)
#endif /* MLO_BCMC */
...
#else /* LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 27)) && defined(CONFIG_PM_SLEEP) */
 ...
 * "2" -> print event log messages as formatted
 * CONSOLE_E if logstrs.bin etc. files are available
 */
...
/* its defined in event_log_tag.h .. need to synch manually */
...
extern void dhd_dpc_disable(dhd_pub_t *dhdp);
...
#if defined(MLO_IPC)
extern int dhd_os_mlo_ipc_wait(dhd_pub_t * pub, uint * condition);
extern int dhd_os_mlo_ipc_wake(dhd_pub_t * pub);
#endif /* MLO_IPC */
extern unsigned int dhd_os_get_ioctl_resp_timeout(dhd_pub_t * pub);
extern void dhd_os_set_ioctl_resp_timeout(dhd_pub_t * pub, unsigned int timeout_msec);
...
extern int wl_host_event_get_data(void *pktdata, bcm_event_msg_u_t *out_event, void **data_ptr);
...
extern void dhd_segfull_event(dhd_pub_t *dhdp, uint32 seg);
#endif /* BCM_BUZZZ_STREAMING_BUILD */

#if defined(DHD_DEBUG)
extern int btrace_write_file(dhd_pub_t *dhdp, void *data, uint length, const char *filename,
	bool truncate);
#endif /* DHD_DEBUG */
...
extern int dhd_bus_start_try(dhd_pub_t *dhdp, const bool lock_taken);
static inline int dhd_bus_start(dhd_pub_t *dhdp) {
	return dhd_bus_start_try(dhdp, FALSE);
}
...
extern void dhd_wds_pktfwd_reset_keymap(dhd_pub_t * dhdp, int ifidx);
#else  /* ! BCM_ROUTER */
#define dhd_add_flowid(pub, ifidx, ac_prio, ea, flowid)  do {} while (0)
#define dhd_del_flowid(pub, ifidx, flowid)               do {} while (0)
#define dhd_wds_pktfwd_reset_keymap(pub, ifidx)          do {} while (0)
#endif /* ! BCM_ROUTER */

enum fwtrap_dump_type {
	FWTRAP_DUMP_FULL = 0,
	FWTRAP_DUMP_SKIPREG,
	FWTRAP_DUMP_SRAM_ONLY,
	FWTRAP_DUMP_MAX
};

extern void dhd_schedule_trap_log_dump(dhd_pub_t *dhdp, enum fwtrap_dump_type dump_type);
#ifdef MLO_IPC
extern void dhd_schedule_mlo_halt_log_dump(dhd_pub_t *dhdp, bool skip_regdumps);
#endif /* MLO_IPC */
extern void dhd_schedule_dtrace(dhd_pub_t *dhdp, uint8 *event_data, uint32 datalen);
extern int dhd_cpudbg_kconsole_get(uint32 *kconsole);
extern int dhd_cpudbg_kconsole_set(uint32 kconsole);
extern int dhd_cpudbg_filelines_get(uint32 *filelines);
extern int dhd_cpudbg_filelines_set(uint32 filelines);
extern int dhd_cpudbg_write_entry(dhd_pub_t *dhdp, uint32 *entry_p, uint32 *written,
	struct file *fp, struct bcmstrbuf *b);
extern uint32 dhd_cpudbg_write_entries(dhd_pub_t *dhdp, struct file *fp, struct bcmstrbuf *b,
	uint32 *skipped);
extern void *dhd_cpudbg_fileopen(dhd_pub_t *dhdp, bool logfile);
extern void dhd_cpudbg_fileclose(void *fp);
extern int dhd_schedule_cpudbg_consume(dhd_pub_t *dhdp);
extern void dhd_write_file(dhd_pub_t *dhdp, const char *file_name, uint8 *dumpbuf, uint len);
...
extern bool dhd_perim_audit(dhd_pub_t *dhdp);
extern bool dhd_perim_audit_ndev(struct net_device *ndev);
...
#define DHD_PERIM_ASSERT(dhdp)		ASSERT(dhd_perim_audit(dhdp))
#define DHD_PERIM_ASSERT_NDEV(ndev)	ASSERT(dhd_perim_audit_ndev(ndev))
...
#define DHD_PERIM_ASSERT(dhdp)            do {} while (0)
#define DHD_PERIM_ASSERT_NDEV(ndev)       do {} while (0)
#define DHD_LOCK      DHD_PERIM_LOCK
#define DHD_UNLOCK    DHD_PERIM_UNLOCK
...
#define DHD_PCAP_LOCK(lock, flags)       (flags) = dhd_os_spin_lock(lock)
#define DHD_PCAP_UNLOCK(lock, flags)     dhd_os_spin_unlock((lock), (flags))
...
#define DHD_RNR_FLAG_OFFL_BKUPQ            (1 << 4)
...
#define DHD_RNR_SET_BKUPQ(dhdp)         (dhdp)->rnr_offl |= DHD_RNR_FLAG_OFFL_BKUPQ
...
#define DHD_RNR_OFFL_BKUPQ(dhdp)                         \
	(((dhdp)->rnr_offl & DHD_RNR_FLAG_OFFL_BKUPQ) == DHD_RNR_FLAG_OFFL_BKUPQ)
#endif /* BCM_DHD_RUNNER */
...
#ifdef DHD_SBF
extern bool dhd_sbf_active(dhd_pub_t *dhdp);
#define SBF_ENABLED(dhdp)               ((dhdp)->sbf != NULL)
#define SBF_ACTIVE(dhdp)                dhd_sbf_active((dhdp))
#define DHD_CUMM_SYSBUF_CTR(dhdp)       (dhdp)->sbf_tx_buff_inuse
#define DHD_CUMM_SYSBUF_CTR_RESET(dhdp) (dhdp)->sbf_tx_buff_inuse = 0
#define DHD_CUMM_SYSBUF_CTR_INCR(dhdp, incr)  \
	(dhdp)->sbf_tx_buff_inuse += (incr)
#define DHD_CUMM_SYSBUF_CTR_DECR(dhdp, decr)  \
	(dhdp)->sbf_tx_buff_inuse -= (decr)
#else /* !DHD_SBF */
#define SBF_ENABLED(dhdp)               0
#define SBF_ACTIVE(dhdp)                0
#define DHD_CUMM_SYSBUF_CTR(dhdp)       0
#define DHD_CUMM_SYSBUF_CTR_RESET(dhdp)  \
	do {} while (0)
#define DHD_CUMM_SYSBUF_CTR_INCR(dhdp, incr)  \
	do {} while (0)
#define DHD_CUMM_SYSBUF_CTR_DECR(dhdp, decr)  \
...
#ifdef BCM_PKTFWD
/* DHD_NBUFF_PTR_TYPE definition, for compatibility with other releases. */
#ifndef DHD_NBUFF_PTR_TYPE
#define DHD_NBUFF_PTR_TYPE      FKBUFF_PTR
#endif /* ifndef DHD_NBUFF_PTR_TYPE */
#endif /* ifdef BCM_PKTFWD */

#ifdef BCM_IOCV_MEM_LOG
/* Returns current time as a string */
char* dhd_os_get_timestamp(void);
#endif /* BCM_IOCV_MEM_LOG */
#ifdef WLMLO_STA
dhd_pub_t *
...
#endif /* WLMLO_STA */
#if defined(PCIE_FULL_DONGLE) && defined(BCM_PCAP)
int dhd_pcapxf_frame_recv(dhd_pub_t *dhd, bool lock);
#endif /* BCM_PCAP */
