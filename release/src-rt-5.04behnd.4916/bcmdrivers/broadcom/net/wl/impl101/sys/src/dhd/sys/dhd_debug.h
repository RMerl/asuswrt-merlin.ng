 ...
 *
 * $Id: dhd_debug.h 828495 2023-08-10 18:44:21Z $
 */
...
#define DHD_PKT_PCAP_VAL	0x2000000
...
extern eldbg_ring_cmn_t *eldbg_get_ring_from_ring_id(eldbg_cmn_t *dbg, int ring_id);
...
int dhd_dbg_push_to_ring(dhd_pub_t *dhdp, int ring_id, eldbg_ring_entry_t *hdr,
		void *data);
int __eldbg_get_ring_status(eldbg_ring_cmn_t *ring, eldbg_ring_status_t *ring_status);
int dhd_dbg_get_ring_status(dhd_pub_t *dhdp, int ring_id,
		eldbg_ring_status_t *eldbg_ring_status);
#ifdef SHOW_LOGTRACE
...
extern int dhd_os_dbg_register_callback(int ring_id,
	void (*dbg_ring_sub_cb)(void *ctx, const int ring_id, const void *data,
		const uint32 len, const eldbg_ring_status_t eldbg_ring_status));
