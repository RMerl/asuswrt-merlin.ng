 ...
 *
 * $Id: dhd_flowring.c 832961 2023-11-15 21:40:29Z $
 */
...
#if defined(BCM_ROUTER) || defined(STB)
	...
	void *pktlist_head, void *pktlist_tail, uint16 pktlist_count)
{
	...
	/* Increment global system buffers in use */
	DHD_CUMM_SYSBUF_CTR_INCR(dhdp, pktlist_count);
 ...
 */
int BCMFASTPATH
dhd_pktfwd_pktlist_enqueue_sbf(dhd_pub_t *dhdp, uint16 flowid,
	pktlist_t *pktlist)
{
	int ret          = BCME_OK;
	flow_ring_node_t *flow_ring_node;
	flow_queue_t     *queue;
	pktlist_t        pktlist_enq;
	...
	if (SBF_ACTIVE(dhdp)) {
		ret = dhd_sbf_queue_throttle_pktlist(dhdp, queue, pktlist, &pktlist_enq);
	} else {
	    /* Use non-sbf enqueue method */
		ret = dhd_pktfwd_queue_throttle(queue, pktlist->len);

		if (ret) {
			pktlist_enq.len = 0;
		} else {
			/* pktlist_enq contains packets to be queued */
	...
	/* To be queued packets list */
	if (pktlist_enq.len) {
		if (queue->head) {
			FLOW_QUEUE_PKT_SETNEXT(queue->tail, pktlist_enq.head);
		} else {
			queue->head = pktlist_enq.head;
		}

		queue->tail = pktlist_enq.tail;
		FLOW_QUEUE_PKT_SETNEXT(queue->tail, NULL);
		queue->len += pktlist_enq.len;
		/* increment parent's cummulative length */
		DHD_CUMM_CTR(DHD_FLOW_QUEUE_CLEN_PTR(queue)) += pktlist_enq.len;
		/* increment grandparent's cummulative length */
		DHD_CUMM_CTR(DHD_FLOW_QUEUE_L2CLEN_PTR(queue)) += pktlist_enq.len;

		/* Increment global system buffers in use */
		DHD_CUMM_SYSBUF_CTR_INCR(dhdp, pktlist_enq.len);
	...
	/* To be throttled packets list */
	if (pktlist->len) {
		queue->failures++;
		ret = (*queue->cb)(queue, pktlist->head);
	}

	return pktlist_enq.len;
} /* dhd_pktfwd_pktlist_enqueue_sbf */
#endif /* DHD_SBF */
#endif /* BCM_PKTFWD */
...
/* Packets are enueued in the flow data structure as linked list. There is no container capacity
 * limit. queue->len variable keeps tract of the count of packets in the queue. Max of queue length
 * is same as the max of data type used to define the len variable which is uint16
 */
#define MAX_QUEUE_LEN	0xFFFF
...
dhd_flow_queue_enqueue(dhd_pub_t *dhdp, flow_queue_t *queue, void *pkt)
{
	...
	if (ETHER_ISMULTI(eh->ether_dhost) &&
		...
		flow_ring_node_t *flow_ring_node;
		uint16 flowid = DHD_PKT_GET_FLOWID(pkt);
		uint32 ifidx;

		flow_ring_node = DHD_FLOW_RING(dhdp, flowid);
		ifidx = flow_ring_node->flow_info.ifindex;

		if (DHD_IF_ROLE_AP(dhdp, ifidx) && dhd_is_mlo_map(dhdp) &&
			(dhd_get_mld_unit(dhdp, ifidx) != DHD_MLO_MLD_UNIT_INVALID)) {
			dhd_mlo_map_handle_bcmc(dhdp, pkt);
		}
	}
#endif /* MLO_BCMC */
...
#ifdef DHD_SBF
	if (SBF_ACTIVE(dhdp)) {
		ret = dhd_sbf_queue_throttle_pkt(dhdp, queue, pkt);
	} else
#endif /* DHD_SBF */
	{
		if ((ntoh16(eh->ether_type) == ETHER_TYPE_802_1X) ||
			...
			ret = dhd_flow_queue_throttle(queue);
		}
	}
	...
	/* Increment global system buffers in use */
	DHD_CUMM_SYSBUF_CTR_INCR(dhdp, 1);
...
#if defined(BCM_DHD_RUNNER)
	/* Get backup queue status from dhd_runner */
	rc = dhd_runner_do_iovar(dhdp->runner_hlp, DHD_RNR_IOVAR_STATUS,
			FALSE, (char*)&rnr_status, sizeof(rnr_status));
	if (rc == BCME_OK && rnr_status.en_features.bkupq) {
	    DHD_RNR_SET_BKUPQ(dhdp);
	}
#endif /* BCM_DHD_RUNNER  */
	DHD_INFO(("%s done\n", __FUNCTION__));
...
#endif
		if ((if_flow_lkup[ifindex].role == WLC_E_IF_ROLE_AP) && ETHER_ISMULTI(da)) {
			/* Override as per bss set priority */
			dhd_bss_get_mcast_prio_override(dhdp, ifindex, &prio);
		}
...
#ifdef DHD_SBF
	if (SBF_ACTIVE(dhdp) && !(ETHER_ISMULTI(da))) {
		flow_queue_t *queue = dhd_flow_queue(dhdp, id);
		dhd_sta_t *sta =  dhd_find_sta(dhdp, ifindex, da);

		/* Update queue threshold based on SBF weight */
		if (sta && queue) {
	        ...
	        if (DHD_FLOWRING_RNR_OFFL(flow_ring_node)) {
				dhd_runner_notify(dhdp->runner_hlp, H2R_FLRING_BQSIZE_NOTIF,
					id, (sta->sbf_threshold | FLRING_BQSIZE_NOTIF_GROUP));
	        }
...
typedef struct dhd_idma_flowmgr
{
	...
	dhd_idma_map_t *maps; /* idma map per Set */
	dhd_idma_map_t *map_max; /* highest idma map */
	uint8 h2d_idma_set; /* number of h2d idma set */
#ifdef DHD_IDMA_TEST
	int32 idma_test_next_group; /* next idma grp/map for flowring id alloc */
#endif /* DHD_IDMA_TEST */
} dhd_idma_flowmgr_t;

#ifdef DHD_IDMA_TEST
void
dhd_idma_test_next_flowgroup(dhd_pub_t *dhdp, bool set, int32 *int_val)
{
	dhd_idma_flowmgr_t *flowmgr = (dhd_idma_flowmgr_t *)(dhdp->flowid_allocator);
	if (set) {
		flowmgr->idma_test_next_group = (int32)(*int_val);
	} else {
		*int_val = flowmgr->idma_test_next_group;
	}
}
#endif /* DHD_IDMA_TEST */
...
dhd_idma_flowmgr_init(dhd_pub_t *dhdp, int max_h2d_rings)
{
	...
	/* Use idma set advertized by the dongle from bus layer */
	flowmgr->h2d_idma_set = dhdp->h2d_idma_set;
	ASSERT(flowmgr->h2d_idma_set);

	/* Allocate iDMA maps */
	flowmgr->maps = (dhd_idma_map_t *) MALLOCZ(dhdp->osh,
		sizeof(dhd_idma_map_t) * flowmgr->h2d_idma_set);
	if (flowmgr->maps == NULL) {
		MFREE(dhdp->osh, flowmgr, sizeof(dhd_idma_flowmgr_t));
		DHD_ERROR(("%s: flowmgr maps alloc failure\n", __FUNCTION__));
		return NULL;
	}
	...
	if (flowmgr->flow_ids_map == NULL) {
		...
		MFREE(dhdp->osh, flowmgr->maps, sizeof(dhd_idma_map_t) * flowmgr->h2d_idma_set);
		MFREE(dhdp->osh, flowmgr, sizeof(dhd_idma_flowmgr_t));
		return NULL;
	}
	...
	while (total_ids && index < (dhdp->h2d_idma_set-1)) {
	...
	/* Last one */
	if (total_ids && index == (dhdp->h2d_idma_set-1)) {
		max_flows = total_ids;
		map->max = max_flows;
		map->flow_id16_map = id16_map_init(dhdp->osh, max_flows, ID16_UNDEFINED);
		if (map->flow_id16_map == NULL) {
...
#ifdef DHD_IDMA_TEST
	flowmgr->idma_test_next_group = -1;
#endif /* DHD_IDMA_TEST */
	return flowmgr;
...
#ifdef DHD_IDMA_TEST
		if (flowmgr->idma_test_next_group != -1) {
			DHD_INFO(("flowmgr->idma_test_next_group = %d \n",
				flowmgr->idma_test_next_group));
			map_index = flowmgr->idma_test_next_group;
		}
#endif /* #if DHD_IDMA_TEST */

		/* Search from the second map */
		map = (!flowmgr->maps[map_index].flow_id16_map) ?
			&flowmgr->maps[0] : &flowmgr->maps[map_index];
		while (TRUE) {
...
#ifdef DHD_IDMA_TEST
			if (flowmgr->idma_test_next_group != -1) {
				DHD_ERROR(("Flowmgr->idma_test_next_group %d flowid %d"
					"sel_map @ %p avail %d \n",
					flowmgr->idma_test_next_group, flow_id,
					sel_map, sel_map->avail));

				/* idma_test_next_group or map 0(iDMA set 1) - H2D Common Rings,
				 * BCMC rings and Tx Flowrings (if map 1 to 14/62 are full)
				 * Note: iDMA set 0 is used for D2H common rings
				 * flowmgr map 1 to 14/62 (iDMA set 2 to 15/63) - Tx Flowrings
				 */
				if (flowmgr->idma_test_next_group >= (flowmgr->h2d_idma_set-1)) {
					flowmgr->idma_test_next_group = 0;
				} else {
					flowmgr->idma_test_next_group++;
				}
			}
...
dhd_idma_flowmgr_free(void *mgr, uint16 flow_id)
{
	...
	if (index >= flowmgr->h2d_idma_set)
		index = flowmgr->h2d_idma_set - 1;
...
/* See bcmpcie.h */
uint16
dhd_idma_flowmgr_ringid_2_index(dhd_pub_t *dhdp, bool h2d, uint16 flow_id)
{
	uint16 uindex;

	/* D2H:: CtrlCpl:2 TxCpl:3 RxCpl:4
	 * and dynamic D2H rings
	 */
	if (! h2d) {
	...
	if (uindex >= dhdp->idma_set)
		uindex = dhdp->idma_set-1;
...
dhd_flowid_map_init(dhd_pub_t *dhdp, int num_flow_rings)
{
	...
	if (BCM43602_CHIP(chipid))
		max_bss = 4;
	else if (BCM43684_CHIP(chipid))
		max_bss = 16;
...
#if defined(BCM_ROUTER)
int
dhd_get_flowid(dhd_pub_t *dhdp, uint8 ifindex,
                                uint8 prio, char *sa, char *da, uint16 *flowid)
{
