 ...
 *
 * $Id: dhd_flowring.h 832961 2023-11-15 21:40:29Z $
 */
...
extern uint16 dhd_idma_flowmgr_ringid_2_index(dhd_pub_t *dhdp, bool h2d, uint16 flow_id);
...
#if defined(DHD_SBF)
extern int dhd_pktfwd_pktlist_enqueue_sbf(dhd_pub_t *dhdp, uint16 flowid,
	pktlist_t *pktlist);
#endif /* DHD_SBF */
#endif /* BCM_PKTFWD */

#ifdef DHD_IDMA_TEST
void dhd_idma_test_next_flowgroup(dhd_pub_t *dhdp, bool set, int32 *int_val);
#endif /* DHD_IDMA_TEST */
...
extern int dhd_max_tx_frings(dhd_pub_t *dhd, int max_h2d_rings);
extern bool dhd_is_tx_fring(dhd_pub_t *dhd, uint16 ringid);
extern bool dhd_is_non_tx_fring(dhd_pub_t *dhd, uint16 ringid);
extern bool dhd_is_dyn_fring(dhd_pub_t *dhd, uint16 ringid);
#define MAX_TX_FLOWRINGS(dhdp, max_h2d_rings)	dhd_max_tx_frings((dhdp), (max_h2d_rings))
#define DHD_IS_TX_FLOWRING(dhdp, ringid)	dhd_is_tx_fring((dhdp), (ringid))
#define DHD_IS_NON_TX_FLOWRING(dhdp, ringid)	dhd_is_non_tx_fring((dhdp), (ringid))
#define DHD_IS_DYN_FLOWRING(dhdp, ringid)	dhd_is_dyn_fring((dhdp), (ringid))
#define MAX_DYN_FLOWRINGS(dhd)	dhd_bus_max_h2d_dynamic_rings((dhd)->bus)

#if defined(BCM_ROUTER)
extern int dhd_get_flowid(dhd_pub_t *dhdp, uint8 ifindex, uint8 prio, char *sa,
	char *da, uint16 *flowid);
#endif /* BCM_ROUTER */
