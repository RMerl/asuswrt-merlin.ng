 ...
 *
 * $Id: dhd_ife.c 821234 2023-02-06 14:16:52Z $
 */
...
#define DHD_IFE_DEFAULT_TIMEOUT_CNT 3
#ifdef CMWIFI_RDKB
#define DHD_IFE_DEFAULT_TIMEOUT         200000  // 10 min
#else
...
#ifdef CMWIFI
	uint32		    timeout_trigger; /* indication of the timeout fired */
#endif /* CMWIFI */
} dhd_ife_info_t;
...
#ifdef CMWIFI
	/* dhd_ife timer runs in softirq, on cmwifi platform we need it to run
	 * inside kernel thread to ensure flowmgr API(filp_open) works.
	 */
	ife_info->timeout_trigger = 1;
#else
...
#ifdef CMWIFI
	/* flowmgr: flush flowid and unlearn flow */
	dhd_flowmgr_flushwifi(dhdp, flow_ring_node);
#endif /* CMWIFI */
	...
	for (item = ife_info->iter_start;
		...
		if ((curr_wr_idx == evict_wr_idx) && (curr_wr_idx == curr_rd_idx) &&
			DHD_FLOW_QUEUE_EMPTY(&flow_ring_node->queue))
				flow_ring_node->evict_check_cnt++;
		else {
			flow_ring_node->evict_wr_idx = curr_wr_idx;
			flow_ring_node->evict_check_cnt = 0;
		}

		if (flow_ring_node->evict_check_cnt > DHD_IFE_DEFAULT_TIMEOUT_CNT) {
			char *da;
...
#ifdef CMWIFI
void
dhd_ife_watchdog(dhd_pub_t *dhdp)
{
	dhd_ife_info_t *ife_info = dhdp->ife_info;

	if (ife_info->timeout_trigger == 0)
		return;

	ife_info->timeout_trigger = 0;
