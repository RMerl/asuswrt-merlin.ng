 ...
 *
 * $Id: dhd_linux.h 828127 2023-08-02 10:59:48Z $
 */
...
#endif
#ifdef DHD_SBF
	uint16 sbf_index;		/* Client buffer fairness weight index */
	uint16 sbf_weight;		/* Client buffer fairness weight */
	uint16 sbf_threshold;   /* Client buffer fairness current queue threshold */
#endif /* DHD_SBF */
} dhd_sta_t;
...
int dhd_cpudbg_init(dhd_pub_t* dhdp);
...
void dhd_bss_get_mcast_prio_override(dhd_pub_t *dhdp, uint32 ifidx, uint8 *prio);
...
/* Invoke callback handler for all stations from station list iterator */
typedef int (*dhd_sta_list_iter_cb_t)(dhd_pub_t *dhd, dhd_sta_t *sta,
	void *arg);
extern int dhd_sta_list_invoke_func(void *pub, int ifidx,
	dhd_sta_list_iter_cb_t cb_func, void *arg);

/* Buffer len used in hc_txstall_ts.txt file creation */
#define DUMP_FILENAME_SZ		100
...
char *dhd_cpudbg_dumpfilename(dhd_pub_t *dhdp);

int dhd_create_file(const char * file_name, uint32 flags);

#ifdef WLMLO_STA
extern int dhd_mlo_get_mdev(dhd_pub_t * dhd_pub);
extern s32 dhd_is_mlo_supported(void);
extern int
dhd_mlo_get_mlink_unit(dhd_pub_t * dhd_pub);
#endif /* WLMLO_STA */

#ifdef MLO_IPC
#define DHD_MLO_MLD_UNIT_INVALID	((int8)-1)
extern int dhd_update_mld_unit_info(dhd_pub_t *dhd_pub, int ifidx, uint8 mld_unit);
extern int8 dhd_get_mld_unit(dhd_pub_t *dhd_pub, int ifidx);
extern int8 dhd_get_mld_unit_by_ndev(struct net_device *dev);
#endif /* MLO_IPC */

#ifdef MLO_BCMC
int dhd_mlo_map_handle_bcmc(dhd_pub_t *dhdp, void *pktbuf);
int dhd_mlo_aap_handle_bcmc(dhd_pub_t *dhdp, mlo_ipc_bcmc_xmit_msg_t *bcmc_xmit_msg);
uint16 dhd_mlo_get_bcmc_seq(dhd_pub_t *dhdp, int ifidx);
#endif /* MLO_BCMC */
