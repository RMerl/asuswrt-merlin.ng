 ...
 *
 * $Id: dhd_macdbg.h 831913 2023-10-27 20:17:35Z $
 */
...
#include <dhd.h>
#ifdef MLO_IPC
#define DUMPMAC_BUF_SZ (1024 * 1024)
#else
#define DUMPMAC_BUF_SZ (512 * 1024)
#endif /* MLO_IPC */
...
#define MACDBGDUMP_ENUMDEF_LIST \
	...
	MACDBGDUMP_ENUMDEF(LMAC, lmac) \
	MACDBGDUMP_ENUMDEF(PHYDTCM, phydtcm) \
	MACDBGDUMP_ENUMDEF(AQMBMP, aqmsts) \
	MACDBGDUMP_ENUMDEF(CPUDBGSTATE, cpudbgstate) \
	MACDBGDUMP_ENUMDEF(CPUDBG, cpudbg) \
	MACDBGDUMP_ENUMDEF(CPUDBFIFO, cpudbgfifo)
...
extern void dhd_macdbg_cpudbg_updt_rdidx(dhd_pub_t *dhdp);
#endif /* BCMPCIE */
extern bool dhd_d11_iscoreup(dhd_pub_t *dhdp);
uint32 dhd_macdbg_corerev(dhd_pub_t *dhdp);
uint32 dhd_macdbg_bmc_tpl_idx(uint32 corerev);
uint32 dhd_macdbg_puq_tx_fifos(uint32 corerev);
void dhd_macdbg_panic_dump(dhd_pub_t *dhdp);
