 ...
 *
 * $Id: dhd_msgbuf.c 831913 2023-10-27 20:17:35Z $
 */
...
#endif /* MLO_IPC */
#ifdef BCM_PCAP
int dhd_pcap_to_rtap_rxcpy(dhd_pub_t *dhd_pub, struct sk_buff *data_skb,
...
/* Maximum packet capture buffers to post */
#define DHD_H2D_PCAP_MAX_BUF_POST	1024 /* max outstanding posted PCAP buffers */
#define DHD_H2D_PCAP_BUF_BURST		32 /* Number of PCAP buffers posted at a time */
#define DHD_H2D_PCAP_POST_THRESHOLD	(8 * PCAP_AWI_AGGR) /* Refill threshold */

#define DHD_PROT_FUNCS	MSG_TYPE_API_MAX_RSVD
...
#define SWP_PAGEABLE_START		(0x800000)
 ...
 * BCMPCIE_H2D_COMMON_MSGRINGS = 2, and BCMPCIE_COMMON_MSGRINGS = 5, and the
...
#define RING_DIR_VALID(dir) ((dir) == H2D || (dir) == D2H)

#define RING_TYPE_VALID(type) ((type) < NUM_RING_TYPES && (type) > INVALID_TYPE)
#define RING_TYPE_STR(type) ((type) == COMMON ? "CMN" : \
			     (type) == TXPOST_FLOWRING ? "FR" : \
			     (type) == DYNAMIC ? "DYN" : "INVALID")
#define RING_DIR_STR(dir) ((dir) == H2D ? "H2D" : (dir) == D2H ? "D2H" : "uninit")

typedef struct {
	uint8 type;		/* type of ring: COMMON, TXPOST_FLOWRING, DYNAMIC, INVALID */
	uint8 dir;		/* direction: H2D or D2H */
	int16 flowid;
	int16 dma_idx_offset;
	char name[RING_NAME_MAX_LENGTH];
} ring_map_t;

typedef struct {
	uint16             ring_count;		/* size of map */
	uint16             d2h_offset_count;	/* size of d2h_offset2ring */
	ring_map_t        *map;			/* mapping of ringid to flowid, dma offset */
	int16             *d2h_offset2ring;	/* mapping of D2H DMA offset to ring id */
} ring_map_info_t;

/* Retrieve the ring mapping for a ringid */
#define _RINGMAP(prot, ringid) ((prot)->ringmap.map[(ringid)])
/* Validate the map entry */
#define RINGMAP_AUDIT(prot, ringid) \
	do { \
		ASSERT(ringid < (prot)->ringmap.ring_count); \
		ASSERT(RING_TYPE_VALID(_RINGMAP(prot, ringid).type)); \
		ASSERT(RING_DIR_VALID(_RINGMAP(prot, ringid).dir)); \
	} while (0)

/* Retrieve validated ringid mapping */
#define RINGMAP(prot, ringid) ({ \
	RINGMAP_AUDIT(prot, ringid); \
	_RINGMAP(prot, ringid); \
})

/* First TxPost Flowring Id */
#define DHD_FLOWRING_START_FLOWID   BCMPCIE_H2D_COMMON_MSGRINGS

/* Determine whether a ringid belongs to a TxPost flowring. Need to united wth DHD_IS_TX_FLOWRING */
#define DHD_IS_FLOWRING(prot, ringid)	(RINGMAP((prot), (ringid)).type == TXPOST_FLOWRING)

#define DHD_IS_H2D(prot, ringid)	(RINGMAP((prot), (ringid)).dir == H2D)
#define DHD_IS_D2H(prot, ringid)	(RINGMAP((prot), (ringid)).dir == D2H)

#define DHD_IS_COMMON(prot, ringid)	(RINGMAP((prot), (ringid)).type == COMMON)
#define DHD_IS_DYNAMIC(prot, ringid)	(RINGMAP((prot), (ringid)).type == DYNAMIC)
/* Determine whether a ringid is H2D/D2H dynamic ring */
#define DHD_IS_DYNAMIC_H2D(prot, ringid) \
	(DHD_IS_DYNAMIC((prot), (ringid)) && DHD_IS_H2D((prot), (ringid)))
#define DHD_IS_DYNAMIC_D2H(prot, ringid) \
	(DHD_IS_DYNAMIC((prot), (ringid)) && DHD_IS_D2H((prot), (ringid)))
/* Determine whether a ringid is H2D/D2H common ring */
#define DHD_IS_COMMON_H2D(prot, ringid) \
	(DHD_IS_COMMON((prot), (ringid)) && DHD_IS_H2D((prot), (ringid)))
#define DHD_IS_COMMON_D2H(prot, ringid) \
	(DHD_IS_COMMON((prot), (ringid)) && DHD_IS_D2H((prot), (ringid)))
...
#define DHD_RINGID_TO_FLOWID(prot, ringid)	(RINGMAP((prot), (ringid)).flowid)

/* Convert a H2D or D2H MsgBuf RingId to an offset index into the DMA indices array
 * This may be used for the D2H or H2D DMA WR index array, DMA RD index array or
 * any array of H2D or D2H rings.
 */
#define DHD_RING_OFFSET(prot, ringid)	(RINGMAP((prot), (ringid)).dma_idx_offset)

/* Convert a D2H DMA Indices Offset to a RingId */
#define DHD_D2H_RINGID(prot, offset) ({ \
	int16 ringid =	((prot)->ringmap.d2h_offset2ring[(offset)]); \
	ASSERT(offset == RINGMAP(prot, ringid).dma_idx_offset); \
	ringid; \
})

/* Dynamic ring allocation information
 * A dynamic ring is distinct from the common PCIE IPC rings and the
 * dynamic TX Post flowrings.
 *
 * Dynamic rings may be created based on dongle capabilities.
 */
typedef struct {
	uint16	dma_offset;	/* dma index offset */
	uint16	ringid;		/* ring id for allocation */
	uint16	alloc_cnt;	/* counts allocated dynamic rings */
	uint16  max_cnt;	/* maximum # dynamic rings */
} dynamic_ring_info_t;
...
typedef struct msgbuf_ring_params {
	char  *name;		/* pointer to the ring's string name */
	uint16 ringid;		/* unique ring id */
	uint16 item_type;	/* ring's work item type */
	uint16 item_size;	/* size of each work item */
	uint16 max_items;	/* max items/capacity of the ring */
	uint16 item_misc;	/* bit-shift for workitem aggregation */
	uint8  ring_dir;	/* ring direction: H2D or D2H */
	uint8  ring_type;	/* COMMON, TXPOST_FLOWRING, DYNAMIC */
} msgbuf_ring_params_t;
...
typedef struct msgbuf_ring {
	...
	uint32         create_req_id;  /* dynamic ring creation request id */
	bool           create_pending; /* dynamic ring requested */
...
typedef struct dhd_prot {
	...
	uint16        max_h2d_rings; /* total H2D (common rings + flowrings + dynamic rings) */
	uint16        max_h2d_dyn_rings; /* total H2D dynamic rings */

	dynamic_ring_info_t h2d_dyn_rings; /* allocation info for H2D dynamic rings */
	dynamic_ring_info_t d2h_dyn_rings; /* allocation info for D2H dynamic rings */
	bool                dyn_rings_inited; /* the above dyn ring info is inited */
	ring_map_info_t     ringmap;       /* mapping of H2D/D2H ring ids, flowids, offsets */

	uint32         dma_rxoffset;

	msgbuf_ring_t *h2d_pcap_subn;      /* H2D PCAP submission ring */
	msgbuf_ring_t *d2h_pcap_cpln;      /* D2H PCAP completion ring */

	/* Various state for PCAP Submission ring buffer posting */
	struct {
		bool	inprogress;	/* PCAP post in progress */
		bool	fail;		/* Last post failed */
		uint16	count;		/* Number of outstanding posted buffers */
		uint16	max;		/* Maximum number to post */
		uint16	cpl_count;	/* Pkt completion counter (per AWI) */
		uint32	pktgetfail;	/* Counter: failure of PKTGET() while posting */
		uint32	pktidfail;	/* Counter: packet id allocate failed */
	} pcap_post;
	...
	dhd_dma_buf_t   rsvd_h2d_dma_indx_buf;    /* WAR: Reserved space */
	...
	bool		rxpost_fail;
	dhd_dma_buf_t   cpudbg_dma_buf; /* CPU DBG entry circular buffer */
} dhd_prot_t;
...
static int dhd_prot_ring_alloc(dhd_pub_t *dhd, msgbuf_ring_t *ring, msgbuf_ring_params_t *params);
...
static int dhd_fwd_ioct_reqst(dhd_pub_t *dhd, uint16 len, uint cmd,
	void *buf, int ifidx);
...
static INLINE void dhd_msgbuf_rxbuf_post_refresh(dhd_pub_t *dhd, msgbuf_ring_t *ring);
...
#ifdef BCM_PCAP
static void dhd_prot_d2h_pcap_ring_cmplt_process(dhd_pub_t *dhd, void *msg);
#endif /* BCM_PCAP */
static inline void dhd_prot_pcap_d11pkt_resp_process(dhd_pub_t *dhd, void *msg);
...
/* msgbuf_ring_params_t initialization functions */
static void dhd_h2d_ctrl_submit_params_init(msgbuf_ring_params_t *params);
static void dhd_d2h_ctrl_completion_params_init(msgbuf_ring_params_t *params);
static int dhd_h2d_rxpost_params_init(dhd_prot_t *prot, msgbuf_ring_params_t *params,
	uint8 rxpost_format);
static int dhd_d2h_tx_completion_params_init(msgbuf_ring_params_t *params,
	uint8 txcpln_format);
static int dhd_d2h_rx_completion_params_init(dhd_pub_t *dhd, msgbuf_ring_params_t *params,
	uint8 rxcpln_format, uint16 ringid);
static void dhd_h2d_txpost_params_init(dhd_prot_t *prot, msgbuf_ring_params_t *params,
	uint16 ringid, char *ring_name);

static void dhd_prot_dynamic_rings_init(dhd_pub_t *dhd);
static int dhd_dynamic_ring_alloc(dhd_pub_t *dhd, uint8 ring_dir, uint16 *ringid,
	uint16 *dma_offset);
static void dhd_ring_map_attach(dhd_pub_t *dhd);
...
#ifdef BCM_PCAP
static void dhd_prot_detach_pcap_rings(dhd_pub_t * dhd);
static void BCMFASTPATH dhd_msgbuf_pcap_post(dhd_pub_t *dhd);
static int dhd_prot_pcap_post(dhd_pub_t *dhd);
static void dhd_pcap_frm_list_cleanup(dhd_pub_t *dhd);
#ifdef BCM_PCAP_PHYSTS
static void dhd_pcap_phy_list_cleanup(dhd_pub_t *dhd);
#endif /* BCM_PCAP_PHYSTS */
static void dhd_pcap_frm_merge(dhd_pub_t *dhd);
static uint16 dhd_pcap_msdu_cpy(uint8 *dst, uint8 *src, uint16 data_len, int sdu_cnt);
#endif /* BCM_PCAP */
...
static dhd_msgbuf_func_t table_lookup[DHD_PROT_FUNCS] = {
	[0]                               = dhd_prot_noop, /*  0  is  MSG_TYPE_INVALID  */
	[MSG_TYPE_GEN_STATUS]             = dhd_prot_genstatus_process,
	[MSG_TYPE_RING_STATUS]            = dhd_prot_ringstatus_process,
	[MSG_TYPE_FLOW_RING_CREATE_CMPLT] = dhd_prot_flow_ring_create_response_process,
	[MSG_TYPE_FLOW_RING_DELETE_CMPLT] = dhd_prot_flow_ring_delete_response_process,
	[MSG_TYPE_FLOW_RING_FLUSH_CMPLT]  = dhd_prot_flow_ring_flush_response_process,
	[MSG_TYPE_IOCTLPTR_REQ_ACK]       = dhd_prot_ioctack_process,
	[MSG_TYPE_IOCTL_CMPLT]            = dhd_prot_ioctcmplt_process,
	[MSG_TYPE_WL_EVENT]               = dhd_prot_event_process,
	[MSG_TYPE_TX_STATUS]              = dhd_prot_txstatus_process,
	[MSG_TYPE_RX_CMPLT]               = dhd_prot_rxcmplt_process,
	[MSG_TYPE_LPBK_DMAXFER_CMPLT]     = dhd_msgbuf_dmaxfer_process,
	[MSG_TYPE_H2D_RING_CREATE_CMPLT]  = dhd_prot_process_h2d_ring_create_complete,
	[MSG_TYPE_D2H_RING_CREATE_CMPLT]  = dhd_prot_process_d2h_ring_create_complete,
	[MSG_TYPE_D2H_RING_CONFIG_CMPLT]  = dhd_prot_d2h_ring_config_cmplt_process,
#ifdef BCM_PCAP
	[MSG_TYPE_PCAP_CMPLT]             = dhd_prot_d2h_pcap_ring_cmplt_process,
#endif /* BCM_PCAP */
	[MSG_TYPE_PCAP_D11PKT_RESP]       = dhd_prot_pcap_d11pkt_resp_process,
};
...
#endif /* !(defined(BCM_ROUTER) && defined(DHD_USE_PERIM)) */
	...
	for (tries = 0; tries < PCIE_D2H_SYNC_WAIT_TRIES; tries++) {
		...
		if (prot_checksum == 0U) { /* checksum is OK */
			if (msg->epoch == ring_seqnum) {
				ring->seqnum++; /* next expected sequence number */
				if (tries) {
					DHD_ERROR(("%s: Sync stall recovered after %d tries"
						"msg_type = %d\n",
						__FUNCTION__, tries, (int)msg->msg_type));
				}
				goto dma_completed;
			}
		}
		...
		if (tries == 0) {
			DHD_ERROR(("%s: Sync stall detected\n", __FUNCTION__));
		}
...
dhd_prot_hme_init(dhd_pub_t *dhd, uint32 users)
{
	...
	for (user_id = 0U; user_id < hme->users; ++user_id)
	{
		int i, pages;
		uint32 sgmts;
		dhd_dma_buf_t * sgmt_dma_buf;
		dhd_dma_buf_t * user_dma_buf = hme->user_dma_buf + user_id;
		pcie_ipc_hme_user_t * pcie_ipc_hme_user;

		pcie_ipc_hme_user = DHD_PCIE_IPC_HME_USER(pcie_ipc_hme, user_id,
			bus->pcie_ipc_hme_user_size);

		if (pcie_ipc_hme_user->pages == 0)
			continue; // HME user not configured

#ifdef DHD_BHM
		if ((pcie_ipc_hme_user->flags & PCIE_IPC_HME_FLAG_BHM) &&
			bus->dngl_bhm_offset) {

			dhd_dma_buf_t	*bhm_dma_buf = &bus->bhm_mem_dma_buf;
			uint32		bhm_offset = pcie_ipc_hme_user->bhm_offset;

			/* Memory is carved from BHM for this HME user. Fill pa, va and len in
			 * user_dma_buf with BHM details but don't touch OS allocated mem fields
			 */
			PHYSADDRLOSET(user_dma_buf->pa, PHYSADDRLO(bhm_dma_buf->pa) + bhm_offset);
			PHYSADDRHISET(user_dma_buf->pa, PHYSADDRHI(bhm_dma_buf->pa));
			user_dma_buf->va = (void *)((uintptr)bhm_dma_buf->va + bhm_offset);
			user_dma_buf->len = PCIE_IPC_HME_BYTES(pcie_ipc_hme_user->pages);

			/* Validate SWP HME user
			 * - DHD and Dongle should carve from start of BHM region
			 * - Addresses and Length should match
			 */
			if (user_id == PCIE_IPC_HME_USER_HMOSWP) // PCIE_IPC_HYBRIDFW_HME_USER
			{
				dhd_dma_buf_t * hybridfw_dma_buf = &bus->hybridfw_dma_buf;

				ASSERT(hybridfw_dma_buf->va != (void*)NULL);
				/* SWP should be carved from start of BHM region. */
				ASSERT(bhm_offset == 0);

				if ((PCIE_IPC_HME_PAGES(hybridfw_dma_buf->len) !=
						pcie_ipc_hme_user->pages) ||		/* pages */
					(PHYSADDRHI(hybridfw_dma_buf->pa) !=
						PHYSADDRHI(user_dma_buf->pa)) ||	/* Phy */
					(PHYSADDRLO(hybridfw_dma_buf->pa) !=
						PHYSADDRLO(user_dma_buf->pa)) ||
					(hybridfw_dma_buf->va != user_dma_buf->va) ||	/* Virt */
					(hybridfw_dma_buf->len != user_dma_buf->len)) { /* Length */

					DHD_ERROR(("%s: PCIe IPC PROTOCOL FAILURE: HME user "
						"HMOSWP dma_buf allocation\n", __FUNCTION__));
					DHD_ERROR(("hybridfw: " DHD_DMA_BUF_FMT "\n",
						DHD_DMA_BUF_VAL(*hybridfw_dma_buf)));
					DHD_ERROR(("SWP HME user: " DHD_DMA_BUF_FMT "\n",
						DHD_DMA_BUF_VAL(*user_dma_buf)));
					ASSERT(0);
					goto hme_init_failure;
				}

				/* Transferred the bus layer HYBRIDFW DMA buffer */
				memset((void*)hybridfw_dma_buf, 0, sizeof(*hybridfw_dma_buf));
			}

			DHD_PCIE_IPC(("\t Protocol: HME user %2u" DHD_DMA_BUF_FMT "\n",
		...
		/* The last sgmt save the table of HME haddr64 segments */
		sgmts = 0;
		sgmt_dma_buf = NULL;
		if (pages > PCIE_IPC_HME_NOBHM_PAGES_MAX) {
			if (pcie_ipc_hme_user->flags & PCIE_IPC_HME_FLAG_SGMT) {
				sgmts = CEIL(pages, PCIE_IPC_HME_NOBHM_PAGES_MAX);
				sgmt_dma_buf = (dhd_dma_buf_t*)user_dma_buf->va;
				user_dma_buf = sgmt_dma_buf + sgmts;
			}
		}
		...
		DHD_PCIE_IPC(("\t Protocol: HME user %2u" DHD_DMA_BUF_FMT "\n",
			user_id, DHD_DMA_BUF_VAL(*user_dma_buf)));
		for (i = 0; i < sgmts; i++) {
			DHD_PCIE_IPC(("\t               sgmt %u" DHD_DMA_BUF_FMT "\n",
				i, DHD_DMA_BUF_VAL(*sgmt_dma_buf)));
			sgmt_dma_buf++;
		}

		/* HTOL copy Physical Address into HME address table */
		dhd_base_addr_htolpa(hme_user_haddr64 + user_id, user_dma_buf->pa);
	...
	if ((hme->bytes == 0U) &&
#if defined(DHD_BHM)
		(dhd->bus->hme_from_bhm == FALSE) &&
#endif /* DHD_BHM */
		TRUE) {
		/* Oddly no request and no BHM allocs ... but yet dcap1 set? */
		goto hme_init_failure;
	}
...
dhd_prot_hme_reset(dhd_pub_t *dhd)
{
	...
	for (user_id = 0U; user_id < hme->users; ++user_id) {
		...
		dhd_hme_buf_free(dhd->bus, user_dma_buf, user_id);
	}
...
/**
 * Get HME user dma_buf
 *
 */
dhd_dma_buf_t *
dhd_prot_hme_get_dma_buf(dhd_pub_t *dhd, uint32 user_id)
{
	dhd_hme_t *hme = &dhd->prot->hme;

	ASSERT(hme->user_dma_buf != (dhd_dma_buf_t*)NULL);

	if (user_id < hme->users) {
		return (hme->user_dma_buf + user_id);
	}

	return NULL;
}

#define BHM_USERNAME_SEPARATOR	"_"
/*
 * Get HME user buffer info
 * * buf_info : output, list of HME user buffer info
 *	(name, buffer va and length).
 *	Caller should allocate enough memory for the buf_info
 * * count : input
 *	Number of members in buf_info
 */
int
dhd_prot_hme_get_buf_info(dhd_pub_t *dhdp, dhd_buf_info_t *buf_info, uint32 user_count)
{
	struct dhd_bus *bus = dhdp->bus;
	pcie_ipc_t *pcie_ipc = &bus->pcie_ipc;
	pcie_ipc_hme_t *pcie_ipc_hme = bus->pcie_ipc_hme;
	dhd_hme_t * hme = &dhdp->prot->hme;
	pcie_ipc_hme_user_t *ipc_hme_user;
	dhd_dma_buf_t *dma_buf;
	int ret = BCME_OK;
	uint i, j, bhm_index = 0;
	bool bhm = FALSE;
	uint hme_drop_user_list[] = {
		PCIE_IPC_HME_USER_SCRMEM,
		PCIE_IPC_HME_USER_CSIMON,
		PCIE_IPC_HME_USER_HMOSWP,
		PCIE_IPC_HME_USER_AIR_IQ
	};
	uint drop_list_count = ARRAYSIZE(hme_drop_user_list);

	BCM_REFERENCE(bhm_index);
	BCM_REFERENCE(bhm);

	if (!user_count || !buf_info) {
		DHD_ERROR(("%s arg invalid: user_count %d, user_info %p\n",
			__FUNCTION__, user_count, buf_info));
		return BCME_BADARG;
	}

	/* Clear output hme buffer info */
	bzero(buf_info, sizeof(dhd_buf_info_t) * user_count);

	/* HME is not enabled. To avoid error messge return OK */
	if ((pcie_ipc->hcap1 & PCIE_IPC_HCAP1_HOST_MEM_EXTN) == 0) {
		return BCME_OK;
	}

	/* Error check, alloced byte is zero and bhm is false */
	if (hme->bytes == 0 &&
#ifdef DHD_BHM
		bus->hme_from_bhm == FALSE &&
#endif /* DHD_BHM */
		TRUE) {
		return BCME_NOTUP;
	}
	...
	for (i = 0; i < user_count; i++) {
		/* Check this i th hme user is in the drop list
		 * If it is in the list, continue with the next one
		 */
		for (j = 0; j < drop_list_count; j++) {
			if (i == hme_drop_user_list[j]) {
				break;
			}
		}

		/* Hit the drop list, skip this one */
		if (j != drop_list_count) {
			continue;
		}

		/* Clear dma buf info at the start of loop */
		dma_buf = NULL;

		/* Get ipc info for the user */
		ipc_hme_user = DHD_PCIE_IPC_HME_USER(pcie_ipc_hme, i, bus->pcie_ipc_hme_user_size);

		if ((ipc_hme_user->flags & PCIE_IPC_HME_FLAG_BHM) == 0 &&
			ipc_hme_user->pages != 0) {
			/* DHD allocated hme buffer, so get the buffer info with API */
			dma_buf = dhd_prot_hme_get_dma_buf(dhdp, i);
		}
#ifdef DHD_BHM
		else if ((ipc_hme_user->flags & PCIE_IPC_HME_FLAG_BHM) &&
			(bus->dngl_bhm_offset == TRUE)) {
			/* Post PCIE IPC rev 0x8A, dongle sends BHM offset for BHM allocated buffer
			 * and DHD fills the dma_buf (pa, va, len) with BHM details
			 */
			if (ipc_hme_user->pages != 0) {
				dma_buf = dhd_prot_hme_get_dma_buf(dhdp, i);
			}
		} else if ((ipc_hme_user->flags & PCIE_IPC_HME_FLAG_BHM) &&
			ipc_hme_user->pages == 0 &&
			bus->hme_from_bhm == TRUE) {
			/* Buf info for bhm is already updated.
			 * So only append the new name to the end of the existing string
			 * Output string for bhm will be like PKTPGR_LCLPKT_HMOSCB
			 */
			if (bhm == TRUE)  {
				strncat(buf_info[bhm_index].name, BHM_USERNAME_SEPARATOR,
					strlen(BHM_USERNAME_SEPARATOR));
				strncat(buf_info[bhm_index].name, ipc_hme_user->name,
					PCIE_IPC_HME_USER_NAME_SIZE);
				continue;
			}

			/* BHM case (PCIe IPC rev <= 0x8A),
			 * DHD does not know about
			 * how much BHM is allocated to the hme user right now.
			 * So, capture whole bhm buffer here
			 * Postprocessing is done by gdb tool
			 */
			dma_buf = &bus->bhm_mem_dma_buf;
			bhm = TRUE;
			/* Backup bhm index to update string later */
			bhm_index = i;
		}
#endif /* DHD_BHM */

		if (!dma_buf) {
			DHD_ERROR(("%s dma_buf is null for HME user %d\n",
				__FUNCTION__, ipc_hme_user->user_id));
			/* When dma buf info is not available, try with the next one */
			continue;
		}

		buf_info[i].buf = dma_buf->va;
		buf_info[i].len = dma_buf->len;
		strncpy(buf_info[i].name, ipc_hme_user->name,
			PCIE_IPC_HME_USER_NAME_SIZE);
	}
...
void
dhd_prot_hme_get_sgmt_buf_info(dhd_pub_t *dhdp, uint32 user_id,
	dhd_buf_info_t *sgmt_buf_info, uint16 sgmt_idx)
{
	...
	ipc_hme_user = DHD_PCIE_IPC_HME_USER(pcie_ipc_hme, user_id, bus->pcie_ipc_hme_user_size);

	ASSERT(ipc_hme_user->sgmt_avail && sgmt_idx < ipc_hme_user->sgmt_avail);

	dma_buf = dhd_prot_hme_get_dma_buf(dhdp, user_id);
	dma_buf = (dhd_dma_buf_t*)dma_buf->va;
	dma_buf += sgmt_idx;

	sgmt_buf_info->buf = dma_buf->va;
	sgmt_buf_info->len = dma_buf->len;
	snprintf(sgmt_buf_info->name, sizeof(sgmt_buf_info->name),
		"%s_sgmt%d", ipc_hme_user->name, sgmt_idx);
}   /* dhd_prot_hme_get_sgmt_buf_info() */

/* Returns the number of HME users */
uint32
dhd_prot_hme_get_user_cnt(dhd_pub_t *dhdp)
{
	struct dhd_bus *bus = dhdp->bus;
	dhd_hme_t *hme = &dhdp->prot->hme;
	pcie_ipc_t *pcie_ipc = &bus->pcie_ipc;
...
/* Returns the number of HME users */
uint8
dhd_prot_hme_get_sgmt_cnt(dhd_pub_t *dhdp, uint32 user_id)
{
	struct dhd_bus *bus = dhdp->bus;
	pcie_ipc_hme_t *pcie_ipc_hme = bus->pcie_ipc_hme;
	pcie_ipc_hme_user_t *ipc_hme_user;
	...
	ipc_hme_user = DHD_PCIE_IPC_HME_USER(pcie_ipc_hme, user_id, bus->pcie_ipc_hme_user_size);
	return ipc_hme_user->sgmt_avail;
}
                  ...
                  uint32 buf_len, uint32 align_bits)
{
	...
	if (align_bits <= DHD_DMA_BUF_4KB_ALIGN_BITS) {
		...
		if (ISALIGNED(PHYSADDRLO(dma_buf->pa), (1 << align_bits))) {
			...
			dhd_dma_buf_free_audit(dhd, dma_buf, str,
				DHD_DMA_BUF_NO_AUDIT);
		}
	}
...
/* 64bit virtual and physical address */
#define DHD_PCIE_PKTID_PPI  /* Use both pointer and index based PktId mapping */
#if defined(BCM_NBUFF)
//#define DHD_PCIE_PKTID_NBUFFPTR  /* Use 33bit pkt pointer for PltId mapping */
#endif /* BCM_NBUFF */
#else /* BCMVADDR32OSL || !BCMDMA64OSL */
#undef DHD_PCIE_PKTID		/* Comment this undef, to reenable PKTIDMAP */
#endif /* BCMVADDR32OSL || !BCMDMA64OSL */
#endif /* BCM_ROUTER */

#if defined(CMWIFI_33940) && defined(BCMDMA64OSL)
/* 64b physical address space */
#undef DHD_PCIE_PKTID_PPI
#endif /* CMWIFI_33940 && BCMDMA64OSL */
...
#define PKTTYPE_IS_RX(type) (((type) == PKTTYPE_DATA_RX) || ((type) == PKTTYPE_PCAP_RX))
...
#define DHD_H2D_PCAP_REQ_PKTID          (0xFFFD << DHD_RUNNER_PKTID_SHIFT)
#define DHD_D2H_PCAP_REQ_PKTID          (0xFFFC << DHD_RUNNER_PKTID_SHIFT)
#define DHD_FAKE_PKTID                  (0xFACE << DHD_RUNNER_PKTID_SHIFT)
#else  /* !BCM_DHD_RUNNER */
#define DHD_IOCTL_REQ_PKTID             (0xFFFE)
#define DHD_H2D_PCAP_REQ_PKTID          (0xFFFD)
#define DHD_D2H_PCAP_REQ_PKTID          (0xFFFC)
...
#if defined(BCM_ROUTER) || defined(STBLINUX)
...
dhd_pktid_map_fini(dhd_pub_t *dhd, dhd_pktid_map_handle_t *handle)
{
	...
	for (i = 0; i < LOCKER_BLOCKS; i++) {
		if (map->locker_blocks[i] != NULL) {
			MFREE(dhd->osh, map->locker_blocks[i], LOCKER_BLOCK_SZ);
		}
	}
...
#else
		MFREE(dhd->osh, handle, dhd_pktid_map_sz);
#endif /* CONFIG_DHD_USE_STATIC_BUF && DHD_USE_STATIC_PKTIDMAP */
}
...
dhd_pktid_map_reset(dhd_pub_t *dhd, dhd_pktid_map_handle_t *handle)
{
	...
	for (nkey = 1; nkey <= map_items; nkey++) {
		dhd_pktid_item_t *locker = GET_LOCKER(map, nkey);
...
#if !defined(DHD_PCIE_PKTID) || defined(DHD_PCIE_PKTID_PPI)
#ifndef linux
...
/**
 * +---------------------------------------------------------------------------+
 * Packet to Packet Id mapper using direct packet pointer.
 *
 * usage of packet pointer for Packet Id based on the following assumptions
 * - HND router platforms has PKTTAG area to store packet metadata. No need of
 *   seperate lockers
 * - for 32bit router platforms, packet pointer is always 32bit
 * - for 64bit router platforms, upper 32bit of packet pointer is always same for
 *   DDR memory < 4GB
 *
 * dhd_native_to_pktid() may be used to save some packet metadata, insert the
 * packet in linked list and returns the lower 32bit of packet address as packet id
 * This unique packet id may be used to retrieve the previously saved packet metadata,
 * using dhd_pktid_to_native().
 *
 * Implementation Note:
 * - Lockless, Assumes usage of global DHD locks at DHD entry/exit
 * - packet metadata contains linked list for mapping/unmapping
 * - Seperate packet lists for TX/RX/CTRL
 *
 * +---------------------------------------------------------------------------+
 */
 ...
 */
#if defined(CONFIG_ARM64)
#if defined(DHD_PCIE_PKTID_NBUFFPTR)
#define PKTIDMAPHIGH_MASK       0xFFFFFFFE00000000
#define DHD_PKTID32_FKB         0x1
#define DHD_PKTID32(pktptr)     ((uint32)(((uint64)(pktptr)) >> 1))
#define DHD_PKTPTR32(pktid32)   ((void *)(((uint64)(pktid32)) << 1))
#else /* !DHD_PCIE_PKTID_NBUFFPTR */
#define PKTIDMAPHIGH_MASK       0xFFFFFFFF00000000
#define DHD_PKTID32(pktptr)	    ((uint32)((uint64)pktptr))
#define DHD_PKTPTR32(pktid32)	((void *)((uint64)pktid32))
#endif /* DHD_PCIE_PKTID_NBUFFPTR */
#else /* !CONFIG_ARM64 */
...
/* Construct a packet id mapping table, returning an opaque map handle */
static dhd_pktid_map_handle_t *dhd_pktid_map_pp_init(dhd_pub_t *dhd,
	uint32 num_items, uint32 index);

/* Destroy a packet id mapping table, freeing all packets active in the table */
static void dhd_pktid_map_pp_fini(dhd_pub_t *dhd, dhd_pktid_map_handle_t *map);

static void dhd_pktid_map_pp_reset(dhd_pub_t *dhd, dhd_pktid_map_handle_t *handle);

static INLINE uint32 dhd_native_to_pktid(dhd_pub_t *dhdp, dhd_pktid_map_handle_t *map, void *pktptr,
	dmaaddr_t pa, uint32 dma_len, void *dmah, void *secdma,
	dhd_pkttype_t pkttype);
static INLINE void * dhd_pktid_to_native(dhd_pub_t *dhdp, dhd_pktid_map_handle_t *map,
	uint32 pktid32, dmaaddr_t *pa, uint32 *dma_len, void **dmah, void **secdma,
	dhd_pkttype_t pkttype);

static dhd_pktid_map_handle_t *
dhd_pktid_map_pp_init(dhd_pub_t *dhd, uint32 num_items, uint32 index)
{
	osl_t *osh = dhd->osh;
...
#if defined(CONFIG_ARM64)
	{
	    ...
	    handle->pktidmaphigh = (uint64)pkt & PKTIDMAPHIGH_MASK;
...
/* map reset is a wrapper for router, only used for STB as of now */
static void
dhd_pktid_map_pp_reset(dhd_pub_t *dhd, dhd_pktid_map_handle_t *handle)
{
	dhd_pktid_map_pp_fini(dhd, handle);
	dhd->prot->pktid_map_handle = NULL;
}

static void
dhd_pktid_map_pp_fini(dhd_pub_t *dhd, dhd_pktid_map_handle_t *map)
{
	osl_t *osh = dhd->osh;
...
static INLINE uint32
dhd_native_to_pktid(dhd_pub_t *dhdp, dhd_pktid_map_handle_t *map, void *pktptr,
	dmaaddr_t pa, uint32 dma_len, void *dmah, void *secdma,
	dhd_pkttype_t pkttype)
{
	pktlists_t *handle = (pktlists_t *) map;
...
#if defined(CONFIG_ARM64)
	if (handle->pktidmaphigh != ((uint64)pktptr & PKTIDMAPHIGH_MASK)) {
	    DHD_ERROR(("Unexpected pktidmaphigh change [0x%llx] -> [0x%llx] for pkt [0x%p]\r\n",
	        handle->pktidmaphigh, (uint64)pktptr & PKTIDMAPHIGH_MASK, pktptr));
...
static INLINE void *
dhd_pktid_to_native(dhd_pub_t *dhdp, dhd_pktid_map_handle_t *map, uint32 pktid32,
	dmaaddr_t *pa, uint32 *dma_len, void **dmah, void **secdma,
	dhd_pkttype_t pkttype)
{
	pktlists_t *handle = (pktlists_t *) map;
...
#if defined(DHD_PCIE_PKTID_NBUFFPTR)
	pktptr = (void*) ((uint64)pktptr | (pktid32 & DHD_PKTID32_FKB));
#endif /* DHD_PCIE_PKTID_NBUFFPTR */
#endif /* CONFIG_ARM64 */
...
#endif /* !DHD_PCIE_PKTID || DHD_PCIE_PKTID_PPI */

#if defined(DHD_PCIE_PKTID_PPI)

/**
 * PPI: Packet Id mapper using Pointer or Index paradigms.
 *
 *  PPI paradigm implements a shim layer that enables both mapping mechanisms
 *  and selects one of them based packet address. For most packets, packet
 *  pointer is used as packet id, if packet memory allocation goes beyond 2GB,
 *  mapping by Index will be used (low possibility)
 *
 *  native_to_pktid
 *   addr_hi matches reference and addr_lo > MAX_PKTID_ITEMS, use packet pointer
 *   as packet id, otherwise use packet index
 *
 *  pktid_to_native
 *   packet Id < MAX_PKTID_ITEMS, use packet index unmapping, otherwise use
 *   use packet pointer unmapping
 *
 *  Configuration: Mode selection (auto/Index only/pointer only/both)
 *   selection by using nvram parameter, with default to auto(both).
 *
 */
#define DHD_PPI_MODE_AUTO                  0
#define DHD_PPI_MODE_PKTIDX                1
#define DHD_PPI_MODE_PKTPTR                2
#define DHD_PPI_MODE_MASK                  3

typedef struct dhd_pktid_map_ppi {
	dhd_pktid_map_handle_t *pi_map;      /* Packet Index mapping */
	dhd_pktid_map_handle_t *pp_map;      /* Packet Pointer mapping */
	uint32                 ref_addr_hi;  /* Reference Address Upper 32bit */
	uint32                 ppi_mode;     /* mode DHD_PPI_MODE_XXXX */
	long                   pi_hits;      /* #packets using Index mapping */
	long                   pp_hits;      /* #packets using Pointer mapping */
} dhd_pktid_map_ppi_t;

#define DHD_IS_PKTID_PKTPTR(pktid)         ((pktid) > MAX_PKTID_ITEMS)
#define DHD_IS_PKTPTR_PKTID(map, pktptr)   \
	((!(map)->pi_map) || \
	 (((map)->ref_addr_hi == (((uint64)(pktptr) & PKTIDMAPHIGH_MASK) >> 32)) && \
	  (((uint64)(pktptr)&(~PKTIDMAPHIGH_MASK)) > MAX_PKTID_ITEMS)))

/* Construct a packet id mapping table, returning an opaque map handle */
static dhd_pktid_map_handle_t *dhd_pktid_map_ppi_init(dhd_pub_t *dhd,
	uint32 num_items, uint32 index);

/* Destroy a packet id mapping table, freeing all packets active in the table */
static void dhd_pktid_map_ppi_fini(dhd_pub_t *dhd, dhd_pktid_map_handle_t *map);

static void dhd_pktid_map_ppi_reset(dhd_pub_t *dhd,
	dhd_pktid_map_handle_t *handle);

/* Determine number of pktids that are available */
static INLINE uint32 dhd_pktid_map_ppi_avail_cnt(
	dhd_pktid_map_handle_t *handle);

/* Allocate a unique pktid against which a pkt and some metadata is saved */
static INLINE uint32 dhd_pktid_map_ppi_reserve(dhd_pub_t *dhd,
	dhd_pktid_map_handle_t *handle, void *pkt);

static INLINE void dhd_pktid_map_ppi_save(dhd_pub_t *dhd,
	dhd_pktid_map_handle_t *handle, void *pkt, uint32 nkey, dmaaddr_t pa,
	uint32 len, uint8 dma, void *dmah, void *secdma, dhd_pkttype_t pkttype);

static uint32 dhd_pktid_map_ppi_alloc(dhd_pub_t *dhd,
	dhd_pktid_map_handle_t *map, void *pkt, dmaaddr_t pa, uint32 len, uint8 dma,
	void *dmah, void *secdma, dhd_pkttype_t pkttype);

/* Return an allocated pktid, retrieving previously saved pkt and metadata */
static void *dhd_pktid_map_ppi_free(dhd_pub_t *dhd, dhd_pktid_map_handle_t *map,
	uint32 id, dmaaddr_t *pa, uint32 *len, void **dmah, void **secdma,
	dhd_pkttype_t pkttype, bool rsv_locker);

static INLINE uint32 BCMFASTPATH
dhd_pktid_map_ppi_avail_cnt(dhd_pktid_map_handle_t *handle)
{
	dhd_pktid_map_ppi_t *map = (dhd_pktid_map_ppi_t *) handle;
	uint32 avail_cnt;

	if (map->pi_map)
		avail_cnt = dhd_pktid_map_avail_cnt(map->pi_map);
	else
		avail_cnt = (~0);
...
static INLINE uint32
dhd_pktid_map_ppi_reserve(dhd_pub_t *dhd, dhd_pktid_map_handle_t *handle,
	void *pkt)
{
	dhd_pktid_map_ppi_t *map = (dhd_pktid_map_ppi_t *) handle;
	uint32 pktid;

	if (DHD_IS_PKTPTR_PKTID(map, pkt))
		pktid = dhd_pktid_map_reserve(dhd, map->pi_map, pkt);
	else
		pktid = DHD_PKTID32(pkt);

	return pktid;
}

static INLINE void
dhd_pktid_map_ppi_save(dhd_pub_t *dhd, dhd_pktid_map_handle_t *handle,
	void *pkt, uint32 nkey, dmaaddr_t pa, uint32 len, uint8 dir, void *dmah,
	void *secdma, dhd_pkttype_t pkttype)
{
	...
	if (DHD_IS_PKTPTR_PKTID(map, pkt))
		dhd_native_to_pktid(dhd, map->pp_map, pkt, pa, len, dmah,
			secdma, pkttype);
	else
		dhd_pktid_map_save(dhd, map->pi_map, pkt, nkey, pa, len, dir, dmah,
			secdma, pkttype);
...
static void * BCMFASTPATH
dhd_pktid_map_ppi_free(dhd_pub_t *dhd, dhd_pktid_map_handle_t *handle,
	uint32 nkey, dmaaddr_t *pa, uint32 *len, void **dmah, void **secdma,
	dhd_pkttype_t pkttype, bool rsv_locker)
{
	dhd_pktid_map_ppi_t *map = (dhd_pktid_map_ppi_t *) handle;
	void *pktptr;

	if (DHD_IS_PKTID_PKTPTR(nkey))
		pktptr = dhd_pktid_to_native(dhd, map->pp_map, nkey, pa, len,
				dmah, secdma, pkttype);
	else {
		pktptr = dhd_pktid_map_free(dhd, map->pi_map, nkey, pa, len,
				dmah, secdma, pkttype, rsv_locker);
	}
...
static uint32 BCMFASTPATH
dhd_pktid_map_ppi_alloc(dhd_pub_t *dhd, dhd_pktid_map_handle_t *handle,
	void *pkt, dmaaddr_t pa, uint32 len, uint8 dir, void *dmah,
	void *secdma, dhd_pkttype_t pkttype)
{
	dhd_pktid_map_ppi_t *map = (dhd_pktid_map_ppi_t *) handle;
	uint32 pktid;
	...
	if (DHD_IS_PKTPTR_PKTID(map, pkt)) {
		pktid = dhd_native_to_pktid(dhd, map->pp_map, pkt, pa,
				len, dmah, secdma, pkttype);
		map->pp_hits++;
	} else {
		pktid = dhd_pktid_map_alloc(dhd, map->pi_map, pkt, pa,
				len, dir, dmah, secdma, pkttype);
		map->pi_hits++;
	}
...
static void
dhd_pktid_map_ppi_reset(dhd_pub_t *dhd, dhd_pktid_map_handle_t *handle)
{
	dhd_pktid_map_ppi_t *map = (dhd_pktid_map_ppi_t *) handle;

	if (map == (dhd_pktid_map_ppi_t *)NULL)
		return;

	if (map->pp_map)
		dhd_pktid_map_pp_reset(dhd, map->pp_map);
	else
		dhd_pktid_map_reset(dhd, map->pi_map);

	return;
}

static void
dhd_pktid_map_ppi_fini(dhd_pub_t *dhd, dhd_pktid_map_handle_t *handle)
{
	dhd_pktid_map_ppi_t *map = (dhd_pktid_map_ppi_t *) handle;
	...
	if (map->pp_map)
		dhd_pktid_map_pp_fini(dhd, map->pp_map);

	if (map->pi_map)
		dhd_pktid_map_fini(dhd, map->pi_map);

	MFREE(dhd->osh, map, sizeof(dhd_pktid_map_ppi_t));

	return;
}

static dhd_pktid_map_handle_t *
dhd_pktid_map_ppi_init(dhd_pub_t *dhd, uint32 num_items, uint32 index)
{
	osl_t *osh = dhd->osh;
	dhd_pktid_map_ppi_t *map = NULL;
	int size = sizeof(dhd_pktid_map_ppi_t);
	uint32 mode = DHD_PPI_MODE_AUTO;

	map = (dhd_pktid_map_ppi_t *) MALLOCZ(osh, size);
	if (map == NULL) {
		DHD_ERROR(("%s:%d: MALLOC failed for map allocation, size=%d\n",
			__FUNCTION__, __LINE__, size));
		return (dhd_pktid_map_handle_t *)NULL;
	}

	/*
	 * Configration through NVRAM
	 */
	{
		char *var;
		var = getvar(NULL, "dhd_ppi_mode");
		if (var) {
			mode = bcm_strtoul(var, NULL, 0);
			mode &= DHD_PPI_MODE_MASK;
		}
	}

	if ((mode == DHD_PPI_MODE_AUTO) || (mode & DHD_PPI_MODE_PKTIDX)) {
		map->pi_map = dhd_pktid_map_init(dhd, num_items, index);
		if (map->pi_map == NULL) {
			dhd_pktid_map_ppi_fini(dhd, (dhd_pktid_map_handle_t*)map);
			return (dhd_pktid_map_handle_t *)NULL;
		}
	}

	if ((mode == DHD_PPI_MODE_AUTO) || (mode & DHD_PPI_MODE_PKTPTR)) {
		uint64 pktidmaphigh;
		map->pp_map = dhd_pktid_map_pp_init(dhd, num_items, index);
		if (map->pp_map == NULL) {
			dhd_pktid_map_ppi_fini(dhd, (dhd_pktid_map_handle_t*)map);
			return (dhd_pktid_map_handle_t *)NULL;
		}
	        pktidmaphigh = ((pktlists_t*)map->pp_map)->pktidmaphigh;
		map->ref_addr_hi = (uint32)(pktidmaphigh >> 32);
	} else {
		map->ref_addr_hi = 0xffffffff;
	}
...
#endif /* ! DHD_PCIE_PKTID_PPI */

/*
 * Three different mapping modes
 *
 *  - DHD_PCIE_PKTID && DHD_PCIE_PKTID_PPI (pointer or index and pointer as host_pktid)
 *  - DHD_PCIE_PKTID (packet index as host_pktid)
 * -  None (packet pointer as host_pktid)
 */
#if defined(DHD_PCIE_PKTID_PPI)

/** Allocate and initialize host_pktid mapper */
#define DHD_NATIVE_TO_PKTID_INIT(dhd, items, index) dhd_pktid_map_ppi_init((dhd), (items), (index))

/** De-initialize and free host_pktid mapper */
#define DHD_NATIVE_TO_PKTID_FINI(dhd, map)   dhd_pktid_map_ppi_fini((dhd), (map))

/** Reset host_pktid mapper */
#define DHD_NATIVE_TO_PKTID_RESET(dhd, map)  dhd_pktid_map_ppi_reset((dhd), (map))

/* Convert a packet to a pktid, and save pkt pointer in busy locker */
#define DHD_NATIVE_TO_PKTID_RSV(dhd, map, pkt)    \
	dhd_pktid_map_ppi_reserve((dhd), (map), (pkt))
...
#define DHD_NATIVE_TO_PKTID_SAVE(dhd, map, pkt, nkey, pa, len, dir, dmah, secdma, pkttype) \
	dhd_pktid_map_ppi_save((dhd), (map), (void *)(pkt), (nkey), (pa), \
...
#define DHD_PKTID_AVAIL(map)                 dhd_pktid_map_ppi_avail_cnt(map)
#define DHD_PKTID_PPI_MODE(map) \
	((dhd_pktid_map_ppi_t*)(map))->ppi_mode

#elif defined(DHD_PCIE_PKTID)
...
#define DHD_NATIVE_TO_PKTID_INIT(dhd, items, index) dhd_pktid_map_pp_init((dhd), (items), (index))
#define DHD_NATIVE_TO_PKTID_FINI(dhd, map)   dhd_pktid_map_pp_fini((dhd), (map))
#define DHD_NATIVE_TO_PKTID_RESET(dhd, map)  dhd_pktid_map_pp_reset((dhd), (map))
...
#define DHD_NATIVE_TO_PKTID_SAVE(dhd, map, pkt, nkey, pa, len, dma_dir, dmah, secdma, pkttype) \
	({ BCM_REFERENCE(dhd); BCM_REFERENCE(nkey); BCM_REFERENCE(dma_dir); \
	   dhd_native_to_pktid((dhd), (dhd_pktid_map_handle_t *) map, (pkt), (pa), (len), \
...
#define DHD_PKTID_TO_NATIVE(dhd, map, pktid, pa, len, dmah, secdma, pkttype) \
	({ BCM_REFERENCE(dhd); BCM_REFERENCE(pkttype);	\
		dhd_pktid_to_native((dhd), (dhd_pktid_map_handle_t *) map, (uint32)(pktid), \
				(dmaaddr_t *)&(pa), (uint32 *)&(len), (void **)&(dmah), \
				(void **)&secdma, (dhd_pkttype_t)(pkttype)); \
	})
...
dhd_bcm_buzzz_attach(dhd_pub_t *dhd)
{
	...
	{
		/* dhd_dma_buf_alloc() adds bytes to the requested size in
		 * order to guarantee the returned buffer can be properly aligned.
		 * dma_buf_len = 4MB = maximum allocation size in Linux default
		 * configuration (MAX_ORDER=11). Therefore, set the requested
		 * alignment to 0 and don't try to allocate 4MB + 8B! kmalloc()
		 * is expected to allocate a buffer with size 2^n on a 2^n boundary.
		 * Bypassing explicit alignment request is OK here.
		 */
		...
		if (!ISALIGNED((uintptr)(dhd->bcm_buzzz_dma_buf + seg), 8)) {
			DHD_ERROR(("BUZZZ[%d] not aligned to 8B\n", seg));
		}

		DHD_ERROR(("\t segment[%d] = va<%px> pa <hi 0x%08x lo 0x%08x>\n",
			seg, (void*)(dhd->bcm_buzzz_dma_buf + seg)->va,
			PHYSADDRHI((dhd->bcm_buzzz_dma_buf + seg)->pa),
			(uint32)PHYSADDRLO((dhd->bcm_buzzz_dma_buf + seg)->pa)));
	}
...
{
	int seg;
	uint64 bcm_buzzz_pa[BCM_BUZZZ_SEGMENTS];
	...
	for (seg = 0; seg < BCM_BUZZZ_SEGMENTS; seg++) {
		bcm_buzzz_pa[seg] =
			(((uint64)htol32(PHYSADDRHI((dhd->bcm_buzzz_dma_buf + seg)->pa))) << 32) |
			htol32(PHYSADDRLO((dhd->bcm_buzzz_dma_buf + seg)->pa));
	}
...
dhd_prot_attach(dhd_pub_t *dhd)
{
	...
	/* create and allocate ring map, prepare for later initializtion for each ring */
	dhd_ring_map_attach(dhd);
	...
	dhd_h2d_ctrl_submit_params_init(&params);

	if (dhd_prot_ring_alloc(dhd, &prot->h2dring_ctrl_subn, &params) != BCME_OK)
	{
	...
	dhd_d2h_ctrl_completion_params_init(&params);

	if (dhd_prot_ring_alloc(dhd, &prot->d2hring_ctrl_cpln, &params) != BCME_OK)
	{
	...
	/* IPC rev >= 0x87: circular MAC CPU DBG entry buffer */
	if (dhd_dma_buf_alloc(dhd, &prot->cpudbg_dma_buf, "cpudbg",
		CPUDBG_HOSTBUF_SZ, DHD_DMA_BUF_ALIGN_BITS) == BCME_OK) {
		DHD_ERROR(("wl%d: %s: allocated cpudbg dma buf va %p\n", dhd->unit,
			__FUNCTION__, prot->cpudbg_dma_buf.va));
	} else {
		goto fail;
	}
	...
	uint32 flags)
{
	...
	if ((err = dhd_h2d_rxpost_params_init(prot, &params, rxpost_format)) != BCME_OK) {
		return err;
	}

	if (dhd_prot_ring_alloc(dhdp, &prot->h2dring_rxp_subn, &params) != BCME_OK) {
		DHD_ERROR(("%s: PCIe IPC PROTOCOL FAILURE: "
	...
	if ((!params.item_misc) && (params.max_items != prot->h2dring_rxp_subn.max_items)) {
		DHD_PCIE_IPC(("\t         : updated max %4u\n",
			prot->h2dring_rxp_subn.max_items));
	}
	/* H2D Rx Post Common Ring Construction Completed */

	/** Ring  3: D2H Transmit Complete Common Ring Construction */
#if defined(DHD_LB)
	if (txcpln_format == MSGBUF_WI_LEGACY) {
	...
	if ((err = dhd_d2h_tx_completion_params_init(&params, txcpln_format)) != BCME_OK) {
		return err;
	}

	if (dhd_prot_ring_alloc(dhdp, &prot->d2hring_tx_cpln, &params) != BCME_OK) {
		DHD_ERROR(("%s: PCIe IPC PROTOCOL FAILURE: "
	...
	if (params.max_items != prot->d2hring_tx_cpln.max_items) {
		DHD_PCIE_IPC(("\t         : updated max %4u\n",
			prot->d2hring_tx_cpln.max_items));
	}
...
#if defined(DHD_LB)
	if (rxcpln_format == MSGBUF_WI_LEGACY) {
		/* Generic Handler includes D2H DMA Sync and LoadBalancing */
	...
	{
		msgbuf_ring_t *ring = DHD_RING_IN_D2HRING_RX_CPLN(prot, i);

		dhd_d2h_rx_completion_params_init(dhdp, &params, rxcpln_format,
			BCMPCIE_D2H_MSGRING_RX_COMPLETE + i);

		if (dhd_prot_ring_alloc(dhdp, ring, &params) != BCME_OK)
		{
			DHD_ERROR(("%s: PCIe IPC PROTOCOL FAILURE: "
		...
		if (params.max_items != ring->max_items) {
			DHD_PCIE_IPC(("\t         : RxCpln [%u] updated max %4u\n",
				params.ringid, ring->max_items));
		}
	}
...
dhd_prot_init(dhd_pub_t *dhd)
{
	...
	/* Initialize non-flowring dynamic rings allocation information. */
	dhd_prot_dynamic_rings_init(dhd);
	...
	if ((prot->hme.bytes == 0U) &&
#if defined(DHD_BHM)
		(dhd->bus->hme_from_bhm == FALSE) &&
#endif /* DHD_BHM */
		TRUE) {
		// Legacy host_mem_dma_buf scheme
...
#ifdef BCM_PCAP
	dhd_pcapxf_hme_init(dhd);
#endif /* PCAP_BUILD */

	if (dhd->cpudbg_wl_support) {
		dhd->cpudbg_hme = (uint32 *)
			((prot->hme.user_dma_buf + PCIE_IPC_HME_USER_LOWMAC)->va);

		if (dhd->cpudbg_hme != NULL) {
			haddr64_t *base_addr_p = (haddr64_t *)
				((uint8*)(dhd->cpudbg_hme) +
				OFFSETOF(cpudbg_hme_t, cpudbg_dmamem_haddr64));
			uint32 *config_p = (uint32 *)((uint8*)(dhd->cpudbg_hme) +
				OFFSETOF(cpudbg_hme_t, cpudbg_config));

			dhd_base_addr_htolpa(&base_addr, prot->cpudbg_dma_buf.pa);
			*base_addr_p = base_addr;
			*config_p = htol32(dhd->cpudbg_config);
			DHD_ERROR(("wl%d: %s: wrote cpudbg hostaddr hi %x, lo %x, config 0x%x\n",
				dhd->unit, __FUNCTION__, base_addr.hi, base_addr.lo,
				dhd->cpudbg_config));
			dhd->cpudbg_syncreq = TRUE; /* need to sync up with seq number 0 */
			dhd->cpudbg_seqnum = 0xff;
		} else {
			DHD_ERROR(("wl%d: %s: PCIe IPC MEMORY FAILURE: "
				"LOWMAC HME buffer not allocated.\n", dhd->unit, __FUNCTION__));
			dhd->cpudbg_config = 0;
		}
	} else {
		DHD_ERROR(("%s: dongle firmware does not support CPU DBG feature\n",
			__FUNCTION__));
		dhd->cpudbg_config = 0;
	}
...
dhd_prot_detach(dhd_pub_t *dhd)
{
	...
	if (prot) {
		...
		/* free per radio prot resources for csimon */
		...
		dhd_dma_buf_free(dhd, &prot->cpudbg_dma_buf,     "cpudbg");
		...
		dhd_runner_notify(dhd->runner_hlp, H2R_DMA_BUF_NOTIF,
		                   ...
		                   (uintptr)(&prot->rsvd_h2d_dma_indx_buf));
#endif /* CMWIFI */
		dhd_runner_notify(dhd->runner_hlp, H2R_DMA_BUF_NOTIF,
		                   (DETACH_DMA_INDX_BUF + H2D_DMA_INDX_WR_BUF),
		...
		if (prot->ringmap.map) {
			MFREE(dhd->osh, prot->ringmap.map,
				sizeof(ring_map_t) * prot->ringmap.ring_count);
		}
		if (prot->ringmap.d2h_offset2ring) {
			MFREE(dhd->osh, prot->ringmap.d2h_offset2ring,
				sizeof(uint16) * prot->ringmap.d2h_offset_count);
		}
#ifdef BCM_PCAP
		dhd_prot_detach_pcap_rings(dhd);
#endif /* BCM_PCAP */
...
dhd_prot_reset(dhd_pub_t *dhd)
{
	...
	dhd_dma_buf_reset(dhd, &prot->cpudbg_dma_buf,      "cpudbg");
...
#ifdef BCM_PCAP
	if (prot->h2d_pcap_subn) {
		dhd_prot_ring_reset(dhd, prot->h2d_pcap_subn);
	}
	if (prot->d2h_pcap_cpln) {
		dhd_prot_ring_reset(dhd, prot->d2h_pcap_cpln);
	}
#endif /* BCM_PCAP */
...
#ifdef BCM_PCAP
	if (prot->pcap_post.fail) {
		dhd_msgbuf_pcap_post(dhd);
	}
#endif /* BCM_PCAP */
}
...
#if defined(OEM_ANDROID) && defined(DHD_DEBUG)
...
#else /* OEM_ANDROID && DHD_DEBUG */
	dhd->memdump_enabled = DUMP_MEMFILE;
#endif /* OEM_ANDROID && DHD_DEBUG */
...
static INLINE void BCMFASTPATH
dhd_msgbuf_rxbuf_post_refresh(dhd_pub_t *dhd, msgbuf_ring_t *ring)
{
	if (DMA_INDX_ENAB(dhd->dma_d2h_ring_upd_support)) {
...
dhd_msgbuf_rxbuf_post(dhd_pub_t *dhd, bool use_rsv_pktid)
{
	...
	dhd_msgbuf_rxbuf_post_refresh(dhd, &dhd->prot->h2dring_rxp_subn);
...
dhd_prot_rxbuf_post(dhd_pub_t *dhd, bool use_rsv_pktid)
{
...
#if defined(CMWIFI) && defined(CMWIFI_EROUTER)
		/* limit max data packet length to < 4K in monitor mode */
...
#ifdef BCM_RXCPLE_RCH
	/* Check the headroom size against the requrired headroom to support RCH */
	if (dhd->rxcple_rch) {
		if (extraheadroom > dhd->bcmpcie_rxcple_rch_size) {
			extraheadroom -= dhd->bcmpcie_rxcple_rch_size;
		} else {
			extraheadroom = 0;
		}
	}
#endif /* BCM_RXCPLE_RCH */
...

	{
		...
		for (i = 0, curpkt = &rxp_pktpool[0]; i < pkt_count; i++, curpkt++)
		{
			...
			if (wler_is_accelerated(0, OSL_PCIE_BUS(dhd->osh), false)) {
				curpkt->ptr = PKTGET_DATA(dhd->osh, pkt_tot_len, FALSE);
			} else
			...
			if (use_bpm_pool)
			{
				OSL_VIRT_TO_PHYSADDR(dhd->osh, PKTDATA(dhd->osh, curpkt->ptr),
					curpkt->buf_pa);
...
#ifdef BCM_RXCPLE_RCH
			/* Post Host buf address to dongle after RCH */
			if (dhd->rxcple_rch) {
				PHYSADDRLOSET(curpkt->buf_pa,
					PHYSADDRLO(curpkt->buf_pa) + dhd->bcmpcie_rxcple_rch_size);
				DHD_INFO(("%s: pktid 0x%x haddr0x%x\n", __FUNCTION__,
					curpkt->pktid, (uint32)PHYSADDRLO(curpkt->buf_pa)));
			}
#endif /* BCM_RXCPLE_RCH */
...
#if defined(CMWIFI) && defined(CMWIFI_EROUTER) && !defined(CMWIFI_33940)
		if (wler_is_accelerated(0, OSL_PCIE_BUS(dhd->osh), false)) {
			if ((p = PKTGET_DATA(dhd->osh, pktsz, FALSE)) == NULL) {
...
dhd_msgbuf_rxbuf_post_ioctlresp_bufs(dhd_pub_t *dhd)
{
	...
	if (prot == NULL) {
		/* Don't replenish the ioctlresp buffers if prot is freed or not inited. */
		DHD_ERROR(("%s: DHD prot is freed or not inited, state %d\n",
...
#ifdef BCM_PCAP
/* refill the PCAP post ring when DHD_H2D_PCAP_POST_THRESHOLD is exceeded.
 * decrement the posted count.
 */
static void
dhd_prot_pcap_refill(dhd_pub_t *dhd)
{
	dhd_prot_t *prot = dhd->prot;
	DHD_TRACE(("%s: cpl %d (posted %d max %d)\n", __FUNCTION__, prot->pcap_post.cpl_count,
		prot->pcap_post.count, prot->pcap_post.max));
	if (prot->pcap_post.count >= prot->pcap_post.cpl_count) {
		prot->pcap_post.count -= prot->pcap_post.cpl_count;
		prot->pcap_post.cpl_count = 0;
	} else {
		DHD_ERROR(("%s: %u > %u PCAP CPL's received\n", __FUNCTION__,
			prot->pcap_post.cpl_count, prot->pcap_post.count));
		prot->pcap_post.count = 0;
	}
	if (prot->pcap_post.count <= (prot->pcap_post.max - DHD_H2D_PCAP_POST_THRESHOLD)) {
		dhd_msgbuf_pcap_post(dhd);
	}
}

/** called when DHD needs to process PCAP completion messages
 */
int BCMFASTPATH
dhd_prot_process_pcap_completion(dhd_pub_t *dhd)
{
	dhd_prot_t *prot = dhd->prot;
	msgbuf_ring_t *ring = prot->d2h_pcap_cpln;
...
dhd_prot_ioctcmplt_process(dhd_pub_t *dhd, void *msg)
{
	...
	if (xt_id != prot->ioctl_trans_id) {
		...
		DHD_ERROR(("%s: cmd 0x%x pkt_id 0x%x xt_id %d trans_id %d "
			...
			ltoh16(ioct_resp->compl_hdr.status), ltoh16(ioct_resp->resp_len)));
		if (xt_id < prot->ioctl_trans_id)
			delta_xtid = prot->ioctl_trans_id - xt_id;
		else
			delta_xtid = 0xffff - xt_id + 1 + prot->ioctl_trans_id;
		ASSERT(delta_xtid <= dhd->rxcnt_timeout);
		goto exit;
	}
...
bool /* Returns TRUE if waiting for IOCTL response */
dhd_prot_ioctl_in_progress(dhd_pub_t * dhd)
{
	dhd_prot_t *prot = dhd->prot;
	return (prot->ioctl_state != 0);
}
 ...
 */
#define PCAP_MON_SCALE 6 /* Should be enough to get to at least 12K for max mpdu size */
	...
	uint8 *buf, uint32 buf_len)
{
...
#ifdef BCM_PCAP
				if ((pcap != PCAP_TXCPL_NONE) && DHD_PCAP_EN(dhd) &&
				    dhd->pcap.pcap_if.radiotap_enabled) {
					struct sk_buff *skb;
					uint8 pcap_type;
					uint16 dest_size;

					switch (pcap) {
						case PCAP_TXCPL_MPDU:
							pcap_type = PCAP_PKT_TX_COPY;
							dest_size = PCAP_PKT_BUFSZ;
							break;

						case PCAP_TXCPL_AMSDU_FIRST:
							/* Need to have an SKB that is big enough
							 * to reassemble a max sized amsdu frame
							 */
							pcap_type = PCAP_PKT_TX_COPY_AMSDU_FIRST;
							dest_size = PCAP_MON_SCALE * PCAP_PKT_BUFSZ;
							break;

						case PCAP_TXCPL_AMSDU_INTRM:
							pcap_type = PCAP_PKT_TX_COPY_AMSDU_INTRM;
							dest_size = PCAP_PKT_BUFSZ;
							break;

						case PCAP_TXCPL_AMSDU_LAST:
							pcap_type = PCAP_PKT_TX_COPY_AMSDU_LAST;
							dest_size = PCAP_PKT_BUFSZ;
							break;
						default:
							DHD_ERROR(("Unknow pcap value %d\n", pcap));
							pcap_type = PCAP_PKT_TX_COPY;
							dest_size = PCAP_PKT_BUFSZ;
							break;
					}

					skb = dhd_pcap_dup(dhd, pkt, dest_size, pcap_type);

					if (skb) {
						dhd_pcap_queue(dhd, skb);
					}
				}
#endif /* BCM_PCAP */
#if defined(BCMPCIE) && (defined(LINUX) || defined(OEM_ANDROID))
	...
	/* Decrement global system buffers in use */
	DHD_CUMM_SYSBUF_CTR_DECR(dhd, pkts_to_free_count);
...
dhd_prot_txstatus_process(dhd_pub_t *dhd, void *msg)
{
	...
	/* Decrement global system buffers in use */
	DHD_CUMM_SYSBUF_CTR_DECR(dhd, 1);
...
/* Simple index Macros */
#define PCAP_LIST_EMPTY(_wr, _rd, _mask)	((_wr) == (_rd))
#define PCAP_LIST_FULL(_wr, _rd, _mask)		((((_wr) + 1) & _mask) == _rd)
#define PCAP_LIST_INC(_idx, _mask)		do {(_idx) = ((_idx) + 1) & _mask; } while (0)

#ifdef BCM_PCAP
#define PCAP_MAX_MSDU ALIGN(ETHER_MAX_LEN + SNAP_HDR_LEN + 3, 4)
#define PCAP_MPDU_2K	2048
#define PCAP_MPDU_CRC	4

static void *
dhd_prot_pcap_get_rxcpy_frm(dhd_pub_t *dhd, int sdu_cnt, int *mpdu_len, int allow_sz)
{
	uint32 frame_alloc_len;
	uint32 headroom;
	void *curpkt = NULL;

	/* Mac header also include PLCP bytes */
	headroom = MAX_MACHDR_LEN + D11RXHDR_LEN + PHYRXSTS_LEN;

	/* If PCAP first msdu filter is enabled, allocate the smallest MPDU size */
	if (DHD_PCAP_RX_FSTMSDU(dhd)) {
		*mpdu_len = PCAP_MPDU_2K;
	} else {
		if (allow_sz > 0) {
			/* Frame buffer size, pad MSDU buffer to 4-byte alignment */
			uint16 mpdu_buf_len, buf_len;
			buf_len = MIN(PCAP_MAX_MSDU,
				ALIGN(((uint16)allow_sz + SNAP_HDR_LEN + 3), 4));
			mpdu_buf_len = (buf_len * sdu_cnt) + PCAP_MPDU_CRC;
			*mpdu_len = MAX(PCAP_MPDU_2K, mpdu_buf_len);
		} else {
			*mpdu_len = MAX(PCAP_MPDU_2K, (PCAP_MAX_MSDU * sdu_cnt + PCAP_MPDU_CRC));
		}
	}
	frame_alloc_len = MAX_MACHDR_LEN + D11RXHDR_LEN + PHYRXSTS_LEN + *mpdu_len;

	curpkt = PKTGET(dhd->osh, frame_alloc_len, FALSE);
	if (curpkt == NULL) {
		DHD_PKT_PCAP(("%s: PKTGET failed, req_sz %d\n", __FUNCTION__, frame_alloc_len));
		dhd->pcap.rxstats.rxpkt_drop_alloc++;
		return NULL;
	}
	dhd->pcap.rxstats.rxpkt_alloc++;
	PKTPULL(dhd->osh, curpkt, headroom);
	return curpkt;
}
static int
dhd_prot_pcap_set_rxcpy_frm(dhd_pub_t *dhd, void *curpkt, uint32 pktid)
{
	pcap_hdr_list_info_t *hdr_list = &dhd->pcap.pcap_list_info;
	uint16 frm_wr, frm_rd;

	frm_wr = hdr_list->frm_list_wr;
	frm_rd = hdr_list->frm_list_rd;
	if (PCAP_LIST_FULL(frm_wr, frm_rd, D11HDR_IDX_MSK)) {
		dhd_pcap_frm_list_cleanup(dhd);
		if (PCAP_LIST_FULL(frm_wr, frm_rd, D11HDR_IDX_MSK)) {
			DHD_PKT_PCAP(("DHD %s No resource to store frames wr %u rd %u, max %d\n",
					__FUNCTION__, frm_wr, frm_rd, MAX_PKT_HDRS));
			dhd->pcap.rxstats.rxpkt_drop_list++;
			PKTFREE(dhd->osh, curpkt, FALSE);
			return BCME_NORESOURCE;
		}
	}

	/* Set packet header room */
	hdr_list->frm_list[hdr_list->frm_list_wr].frm_ptr = curpkt;
	hdr_list->frm_list[hdr_list->frm_list_wr].pktid = pktid;
	hdr_list->frm_list[hdr_list->frm_list_wr].retries = 0;

	DHD_PKT_PCAP(("dhd%d: %s: frame %p data %p frame alloc rd %u wr %u pktid 0x%x\n",
		dhd->unit, __FUNCTION__, curpkt, PKTDATA(dhd->osh, curpkt),
		hdr_list->frm_list_rd, hdr_list->frm_list_wr, pktid));
	PCAP_LIST_INC(hdr_list->frm_list_wr, D11HDR_IDX_MSK);
...
/**
 * RCH messages in the RxCpln ring will be processed here.
 */
#ifdef BCM_RXCPLE_RCH
static int BCMFASTPATH
dhd_prot_process_rxcpln_rch(dhd_pub_t *dhd, msgbuf_ring_t *ring, uint8 *buf)
{
	dhd_prot_t *prot;
	hwa_rxcple_rch_t *rch = (hwa_rxcple_rch_t *) buf;
	bcmpcie_rxcple_rch_t *rxcple_rch_p;
	uint8	rxcple_rch_u8[BCMPCIE_RXCPLE_RCH_SIZE_MAX];
	unsigned long flags;

	uint8 ifidx, pktflags, prio = 0;
	uint16 data_offset, data_len;
	uint32 pktid, next_rch_host_pktid;
	void *pkt;
	int total_sdu_cnt = 0;
	int sdu_cnt = 0;	/* Number of SDUs in an RCH */
	int i;

#ifdef BCM_PCAP
	void *pcap_pkt = NULL;
	uint32 pcap_pktid;
	uint8 *pcap_data;	/* point to pcap packet data */
	int mpdu_sz, mpdu_len = 0;
	int pcap_allow_sz = -1;
	bool first_msdu = TRUE;
#endif /* BCM_PCAP */

	prot = dhd->prot;

	/* Only MSGBUF_WI_CWI is supported by RCH. */
	ASSERT(ring->item_type == MSGBUF_WI_CWI);

	ifidx      = BCM_GBF(rch->u8[0], HWA_RXCPLE_IFID);
	pktflags   = BCM_GBF(rch->u8[0], HWA_RXCPLE_FLAGS);
	pktid	   = ltoh32(rch->rch_host_pktid);
	if (dhd->tid_from_dongle) {
		prio = BCM_GBF(rch->u32[0], HWA_RXCPLE_PRIO);
	}

#ifdef BCM_PCAP
	pcap_pktid = pktid;
	if (DHD_PCAP_RXCPY_EN(dhd) && DHD_PCAP_RXCPY_KEEP_BYTES(dhd)) {
		/* Limit the PCAP content copy size */
		pcap_allow_sz = dhd->pcap.pcap_maxlen + ETHER_HDR_LEN;
	}
#endif /* BCM_PCAP */
	DHD_INFO(("dhd%d: %s: hwa_rxcple_rch: ifidx %d flags %x prio %d RCH pktid 0x%08x\n",
		dhd->unit, __FUNCTION__, ifidx, pktflags, prio, pktid));

	ASSERT(pktid != HWA_HOST_PKTID_NULL);
	ASSERT(pktflags & HWA_RXCPLE_FLAGS_RCH);

process_rxcpln_rch_continue:

	/* Convert pktid to a pkt and pass up to the OS layer */
#if defined(DHD_PKTID_AUDIT_RING)
	DHD_PKTID_AUDIT(dhd, prot->pktid_map_handle, pktid, DHD_DUPLICATE_FREE);
#endif /* DHD_PKTID_AUDIT_RING */
	DHD_GENERAL_LOCK(dhd, flags);
	...
	DHD_GENERAL_UNLOCK(dhd, flags);
	if (!pkt) {
		DHD_ERROR(("%s pdu pktid<%u> mapped to NULL pkt\n", __FUNCTION__, pktid));
		return total_sdu_cnt;
	}

	/* Copy RCH to a stack buffer and retain the headroom reserved for RCH */
	rxcple_rch_p = (bcmpcie_rxcple_rch_t *)PKTDATA(dhd->osh, pkt);
	memcpy(&rxcple_rch_u8, rxcple_rch_p, dhd->bcmpcie_rxcple_rch_size);
	PKTPULL(dhd->osh, pkt, dhd->bcmpcie_rxcple_rch_size);

	rxcple_rch_p = (bcmpcie_rxcple_rch_t *)&rxcple_rch_u8;
	sdu_cnt = rxcple_rch_p->sdu_cnt;
#ifdef BCM_PCAP
	if (DHD_PCAP_RXCPY_EN(dhd)) {
		dhd->pcap.rxstats.rxpkt_tot++;
		if (first_msdu && rxcple_rch_p->pcap_mpdu) {
			pcap_pkt = dhd_prot_pcap_get_rxcpy_frm(dhd, sdu_cnt, &mpdu_sz,
					pcap_allow_sz);
			if (pcap_pkt != NULL) {
				pcap_data = (uint8 *)PKTDATA(dhd->osh, pcap_pkt);
			}
		}
	}
#endif /* BCM_PCAP */
	next_rch_host_pktid = rxcple_rch_p->next_rch_host_pktid;
	DHD_INFO(("RCH: sdu_cnt %d next_rch_host_pktid 0x%08x\n",
		sdu_cnt, next_rch_host_pktid));

	ASSERT(sdu_cnt <= dhd->max_rch_sdu_cnt);

	for (i = 0; i < sdu_cnt; i++) {
		if (i == 0) {
			/* Processing the head pkt */
			ASSERT(pktid == ltoh32(rxcple_rch_p->sdu[i].host_pktid));
		}
		else
		{
			pktid = ltoh32(rxcple_rch_p->sdu[i].host_pktid);
			ASSERT(pktid != 0);
#if defined(DHD_PKTID_AUDIT_RING)
			DHD_PKTID_AUDIT(dhd, prot->pktid_map_handle, pktid, DHD_DUPLICATE_FREE);
#endif /* DHD_PKTID_AUDIT_RING */
			...
			DHD_GENERAL_UNLOCK(dhd, flags);
			if (!pkt) {
				DHD_ERROR(("%s sdu pktid<%u> mapped to NULL pkt\n",
					__FUNCTION__, pktid));
				continue;
			}

			/* RCH is not posted to Non-First MSDU of a PDU, Retain the headroom */
			PKTPULL(dhd->osh, pkt, dhd->bcmpcie_rxcple_rch_size);
		}

		data_offset = ltoh16(rxcple_rch_p->sdu[i].data_offset);
		data_len = ltoh16(rxcple_rch_p->sdu[i].data_len);

		DHD_INFO(("SDU[%d] pktid 0x%08x pkt %p data_offset %d data_len %d\n",
			i, pktid, pkt, data_offset, data_len));
#ifdef BCMHWA
		if (data_len == 0) {
		...
		PKTSETPRIO(pkt, prio);
#ifdef BCM_PCAP
		if (DHD_PCAP_RXCPY_EN(dhd) && pcap_pkt) {
			dhd->pcap.rxstats.msdu_tot++;
			if (!DHD_PCAP_RX_FSTMSDU(dhd) || (DHD_PCAP_RX_FSTMSDU(dhd) && first_msdu)) {
				int pcap_msdu_len, pcap_copy_len;
				uint8 *p_msdu = (uint8 *)PKTDATA(dhd->osh, pkt);

				if (pcap_allow_sz > 0) {
					pcap_copy_len = MIN(data_len, pcap_allow_sz);
				} else {
					pcap_copy_len = data_len;
				}
				first_msdu = FALSE;
				if (mpdu_len <= (mpdu_sz - pcap_copy_len - SNAP_HDR_LEN)) {
					pcap_msdu_len = dhd_pcap_msdu_cpy(pcap_data, p_msdu,
						pcap_copy_len, sdu_cnt);
					mpdu_len += pcap_msdu_len;
					pcap_data += pcap_msdu_len;
				}
				dhd->pcap.rxstats.msdu_cnt++;
				dhd->pcap.rxstats.msdu_bytes += pcap_copy_len;
				DHD_PKT_PCAP(("SDU[%d] pktid 0x%08x pkt %p data_offset %d "
					"data_len %d copy_len %d\n",
					i, pktid, pkt, data_offset, data_len, pcap_copy_len));
			}
		}
#endif /* BCM_PCAP */
	...
	if (DHD_PCAP_RXCPY_EN(dhd) && pcap_pkt) {
		...
		bzero(pcap_data, 4);
		mpdu_len += 4;
		PKTSETLEN(dhd->osh, pcap_pkt, mpdu_len);

		/* Add pcap_pkt into list */
		dhd_prot_pcap_set_rxcpy_frm(dhd, pcap_pkt, pcap_pktid);
	}
#endif /* BCM_PCAP */
	total_sdu_cnt += sdu_cnt;

	if (next_rch_host_pktid != HWA_HOST_PKTID_NULL) {
		ASSERT(sdu_cnt == dhd->max_rch_sdu_cnt);
		pktid = next_rch_host_pktid;
		DHD_INFO(("%s: processing next_rch_host_pktid %x\n", __FUNCTION__, pktid));
		next_rch_host_pktid = HWA_HOST_PKTID_NULL;
		/* Process the next RCH linked */
		goto process_rxcpln_rch_continue;
	}

	return total_sdu_cnt;
} /* dhd_prot_process_rxcpln_rch() */
#endif /* BCM_RXCPLE_RCH */
	...
	uint8 *buf, uint32 buf_len)
{
	...
	while (buf < buf_end) /* Process all RxCompletion workitems */
	{
		...
		if (dhd->rxcple_rch) {
			...
			if (pktflags & HWA_RXCPLE_FLAGS_RCH) {
				rx_compl_count += dhd_prot_process_rxcpln_rch(dhd, ring, buf);
				...
				if (rx_compl_count >= RX_BUF_BURST) {
					...
					if (prot->rxbufpost >= rx_compl_count) {
						...
						DHD_ERROR(("%s: %u extra RxCompletions received\n",
							__FUNCTION__,
							rx_compl_count - prot->rxbufpost));
						prot->rxbufpost = 0;
					}

					if (prot->rxbufpost <= (prot->max_rxbufpost -
						RXBUFPOST_THRESHOLD)) {
						/* alloc pkt ids */
						dhd_msgbuf_rxbuf_post(dhd, FALSE);
					}
		...
		switch (ring->item_type) {
			...
			case MSGBUF_WI_CWI: {
				...
				prio_data_len = ltoh32(cwi->u32[0]);
				break;
			}
			case MSGBUF_WI_ACWI: {
				hwa_rxcple_cwi_t *cwi = (hwa_rxcple_cwi_t *) buf;
		...
		if (dhd->tid_from_dongle) {
			if (isset(&prio_data_len, HWA_RXCPLE_JUMBO_FRAME_FLAG_SHIFT)) {
				data_len = BCM_GBF(prio_data_len, HWA_RXCPLE_JUMBO_DATA_LEN);
			} else {
				data_len = BCM_GBF(prio_data_len, HWA_RXCPLE_REG_DATA_LEN);
				prio = BCM_GBF(prio_data_len, HWA_RXCPLE_PRIO);
			}
		} else {
			data_len = BCM_GBF(prio_data_len, HWA_RXCPLE_DATA_LEN);
		}
...
#ifdef BCM_RXCPLE_RCH
		if (dhd->rxcple_rch) {
			/* Retain the headroom reserved for RCH */
			PKTPULL(dhd->osh, pkt, dhd->bcmpcie_rxcple_rch_size);
		}
#endif
		PKTPULL(dhd->osh, pkt, data_offset);
		...
		if (dhd_monitor_enabled(dhd, ifidx)) {
			if (pktflags & BCMPCIE_PKT_FLAGS_FRAME_802_11) {
				/* buf pointer already advanced for prefetch,
				 * we should use the prev item.
				 */
				dhd_rx_mon_pkt(dhd, (host_rxbuf_cmpl_t *)curr_wi, pkt, ifidx);
				continue;
			}
		}
#endif /* WL_MONITOR */
...
#if defined(BCM_PCAP) && !defined(PCAP_RXCPY_POLLING)
	if (DHD_PCAP_RXCPY_EN(dhd)) {
		dhd_pcapxf_frame_recv(dhd, FALSE);
	}
...
retry_ring_space:
	...
	for (nitems = 0; nitems < alloced; nitems++)
	{
...

		{
...
#ifdef BCM_NBUFF_WLMCAST
				/* mark to tell that this FKB is DHDHDR moved */
			...
			OSL_VIRT_TO_PHYSADDR(dhd->osh, pktdata, pa);
#else
		...
		switch (ring->item_type)
		{
...
#ifdef MLO_BCMC
				if (DHD_PKT_GET_MLO_BCMC(pkt)) {
					uint16 bcmc_seqno = DHD_MLO_BCMC_PKTTAG_SEQ(pkt);
					cwi32->u32[6] = BCM_CBF(cwi32->u32[6], HWA_TXPOST_FLOWOVRD);
					cwi32->u32[6] |= BCM_SBF(bcmc_seqno, HWA_TXPOST_FLOWOVRD);
					DHD_INFO(("dhd%d BCMCSEQ pktid:seq [0x%X:0x%X]\n",
						dhd->unit, pktid, bcmc_seqno));
			...
			case MSGBUF_WI_CWI64:
			{
				...
				if (DHD_PKT_GET_MLO_BCMC(pkt)) {
					uint16 bcmc_seqno = DHD_MLO_BCMC_PKTTAG_SEQ(pkt);
					cwi64->u32[7] = BCM_CBF(cwi64->u32[7], HWA_TXPOST_FLOWOVRD);
					cwi64->u32[7] |= BCM_SBF(bcmc_seqno, HWA_TXPOST_FLOWOVRD);
					DHD_INFO(("dhd%d BCMCSEQ pktid:seq [0x%X:0x%X]\n",
						dhd->unit, pktid, bcmc_seqno));
					ASSERT(MLO_BCMC_SEQNO_ISVALID(bcmc_seqno));
				}
...
dhd_prot_txdata(dhd_pub_t *dhd, void *PKTBUF, uint8 ifidx)
{
	...
	switch (ring->item_type)
	{
		...
		case MSGBUF_WI_CWI32:
		{
			...
			cwi32->host_pktid = pktid;
#ifdef MLO_BCMC
			if (DHD_PKT_GET_MLO_BCMC(PKTBUF)) {
				uint16 bcmc_seqno = DHD_MLO_BCMC_PKTTAG_SEQ(PKTBUF);
				cwi32->u32[6] = BCM_CBF(cwi32->u32[6], HWA_TXPOST_FLOWOVRD);
				cwi32->u32[6] |= BCM_SBF(bcmc_seqno, HWA_TXPOST_FLOWOVRD);
				DHD_INFO(("dhd%d BCMCSEQ pktid:seq [0x%X:0x%X]\n",
					dhd->unit, pktid, bcmc_seqno));
...
static int
dhd_prot_wlioctl_intercept(dhd_pub_t *dhd, int ifidx, wl_ioctl_t * ioc, void * buf)
{
	dhd_prot_t *prot = dhd->prot;
	int ret = BCME_OK;
...
#if defined(MLO_IPC)
	if (dhd_is_mlo_ap(dhd)) {
		ret = dhd_mlo_ipc_wlioctl_intercept(dhd, ifidx, ioc, buf);
	}
#endif /* MLO_IPC */
...
dhd_prot_ioctl(dhd_pub_t *dhd, int ifidx, wl_ioctl_t *ioc, void *buf, int len)
{
	...
	ret = dhd_prot_wlioctl_intercept(dhd, ifidx, ioc, buf);

#if defined(MLO_IPC)
	if (ret == -ETIMEDOUT) {
		DHD_ERROR(("%s: %s: cmd %d returns ETIMEDOUT\n", dhd_ifname(dhd, ifidx),
	...
	if (buf != NULL && len > 0) {
		int name_len = MIN(len, (sizeof(var_name) - 1));
		/* var name in buf may be overwriten by dhd_msgbuf_query_ioctl */
		memcpy(var_name, (char*)buf, name_len);
	...
	if (ret >= 0) {
		...
		if (((ioc->cmd == WLC_GET_VAR) || (ioc->cmd == WLC_SET_VAR)) && buf != NULL) {
			DHD_ERROR(("%s: %s: status ret value is %d iov=%s\n",
				dhd_ifname(dhd, ifidx), __FUNCTION__, ret, var_name));
		} else {
...
dhd_msgbuf_query_ioctl(dhd_pub_t *dhd, int ifidx, uint cmd, void *buf, uint len, uint8 action)
{
	...
	DHD_TRACE(("%s: Enter cmd=%d len=%d action=%d\n", __FUNCTION__, cmd, len, action));
	...
	if (prot->cur_ioctlresp_bufs_posted) {
		ret = dhd_fwd_ioct_reqst(dhd, (uint16)len, cmd, buf, ifidx);
...
dhd_msgbuf_wait_ioctl_cmplt(dhd_pub_t *dhd, uint32 len, void *buf)
{
	...
	if (timeleft == 0) {
		dhd->rxcnt_timeout++;
		dhd->rx_ctlerrs++;
		DHD_ERROR(("%s: resumed on timeout rxcnt_timeout %d [state %02x xtid %d]"
			", busstate %d.\n", __FUNCTION__, dhd->rxcnt_timeout, prot->ioctl_state,
			prot->ioctl_trans_id, dhd->busstate));
		if (dhd->busstate != DHD_BUS_DOWN) {
			dhd_bus_intr_reenable(dhd->bus);
		}
		if (dhd->rxcnt_timeout > MAX_CNTL_RX_TIMEOUT) {
			...
			/* Attempt to trigger a dongle trap and collect the crash dumps.
			 * The halt command can work only if the dongle CPU is still alive.
			 * A sysmem dump would be triggered at the next timeout if
			 * the dongle CPU hanged.
			 */
			if (!dhd->halt_db_sent) {
				DHD_ERROR(("%s: send bus:halt command\n", __FUNCTION__));
				dhd_bus_console_in(dhd, "halt", strlen("halt"));
				dhd->halt_db_sent = TRUE;
				ret = -ETIMEDOUT;
				goto out;
			}
#endif /* DHD_DEBUG && !BCMDBUS */
			if (!dhd->dongle_trap_occured) {
				DHD_ERROR(("%s: schedule dhd_schedule_trap_log_dump\n",
					__FUNCTION__));
				dhd->dongle_trap_occured = TRUE;
				dhd->busstate = DHD_BUS_DOWN;
				dhd_schedule_trap_log_dump(dhd, FWTRAP_DUMP_SRAM_ONLY);
			}
		}
		ret = -ETIMEDOUT;
		goto out;
...
dhd_prot_dump(dhd_pub_t *dhd, struct bcmstrbuf *b)
{
	...
	msgbuf_ring_t *rx_cpln;
	uint32 i;
	bool all_flowrings_ok = TRUE;
	int16 first_flr = -1, last_flr = -1;
	uint16 max_h2d_rings, max_d2h_rings, ring_cnt;
...
#ifdef BCM_PCAP
	if (prot->h2d_pcap_subn) {
		bcm_bprintf(b, " PCAP-Post=%u,%u", prot->h2d_pcap_subn->item_type,
			prot->h2d_pcap_subn->item_size);
	}
	if (prot->d2h_pcap_cpln) {
		bcm_bprintf(b, " PCAP-Cpl=%u,%u", prot->d2h_pcap_cpln->item_type,
			prot->d2h_pcap_cpln->item_size);
	}
#endif /* BCM_PCAP */
	bcm_bprintf(b, "\n");
	...
	/* dump ring map. sanity check flowrings (but don't print!) */
	max_h2d_rings = dhd_bus_max_h2d_rings(dhd->bus);
	max_d2h_rings = dhd_bus_max_d2h_rings(dhd->bus);
	ring_cnt = max_h2d_rings + max_d2h_rings;

	bcm_bprintf(b, "Ring map: count %d (H2D:%u D2H:%u) offset cnt %d\n",
		prot->ringmap.ring_count, max_h2d_rings, max_d2h_rings,
		prot->ringmap.d2h_offset_count);
	bcm_bprintf(b, "\tName\t\tRing ID\t\tDir\tDMA Idx Offset\tFlowID\n");
	for (i = 0; i < ring_cnt; i++) {
		ring_map_t * map = &prot->ringmap.map[i];
		bool flowring_ok;
		if (map->type != TXPOST_FLOWRING) {
			bcm_bprintf(b, "%s\t%8s\t%d\t\t%s\t%d\t\t%d\n",
				RING_TYPE_VALID(map->type) ? "" : "uninit",
				map->name, i,
				RING_DIR_STR(map->dir),
				map->dma_idx_offset,
				map->flowid);
		} else {
			if (first_flr < 0) {
			...
			last_flr = i;
			flowring_ok = RING_DIR_VALID(map->dir) &&
				(map->flowid == i - BCMPCIE_D2H_COMMON_MSGRINGS) &&
				(map->dma_idx_offset == map->flowid);
			if (! flowring_ok) {
				all_flowrings_ok = FALSE;
				bcm_bprintf(b, "BAD FLOWRING: %8s\t%5d\t\t%s\t%5d\t\t%d\n",
					map->name, i,
					RING_DIR_STR(map->dir),
					map->dma_idx_offset,
					map->flowid);
	...
	if (all_flowrings_ok) {
		bcm_bprintf(b, "All flowrings %d - %d OK\n", first_flr, last_flr);
	}
#if defined(BCM_DHD_RUNNER)
 ...
 *
 * Then forwards this request towards the dongle.
 *
 * @param[in] buf  buffer
 */
static int
dhd_fwd_ioct_reqst(dhd_pub_t *dhd, uint16 len, uint cmd, void* buf, int ifidx)
{
	dhd_prot_t *prot = dhd->prot;
...
/* Initialize a 'ring map' table entry.
 *
 * Sets up the mappings of ring id's, flowids and DMA RD/WR array indices.
 */
static int
dhd_prot_ring_map_init(dhd_pub_t *dhd, msgbuf_ring_params_t *params)
{
	dhd_prot_t *prot = dhd->prot;
	ring_map_t *map;
	int err;
	uint16 ringid = 0, dma_offset = 0;

	ASSERT(prot->ringmap.map);
	ASSERT(params->ring_type == DYNAMIC || params->ringid < MAX_DHD_RINGS);

	if (params->ring_type == DYNAMIC) {
		err = dhd_dynamic_ring_alloc(dhd, params->ring_dir, &ringid, &dma_offset);
		if (err != BCME_OK) {
			DHD_ERROR(("wl%d: error: alloc dynamic ring %s fail: %d\n",
				dhd->unit, params->name, err));
			return err;
		}
		params->ringid = ringid;
	}

	ringid = params->ringid;

	if (! RING_DIR_VALID(params->ring_dir)) {
		DHD_ERROR(("%s: error: ringid %d must be D2H or H2D\n", __FUNCTION__, ringid));
		ASSERT(0);
		return BCME_ERROR;
	}
	if (! RING_TYPE_VALID(params->ring_type)) {
		DHD_ERROR(("%s: error: ring type %d invalid\n", __FUNCTION__, params->ring_type));
		ASSERT(0);
		return BCME_ERROR;
	}
	if (params->ring_type == COMMON) {
		ASSERT(params->ringid < BCMPCIE_COMMON_MSGRINGS);
	}

	map = &prot->ringmap.map[ringid];

	/* initialize the ring's map info */
	map->type     = params->ring_type;
	map->dir      = params->ring_dir;
	strncpy(map->name, params->name, RING_NAME_MAX_LENGTH);
	map->name[RING_NAME_MAX_LENGTH - 1] = '\0';

	if (params->ring_type == TXPOST_FLOWRING) {
		/* H2D TX Post Flowring */
		ASSERT(map->dir == H2D);
		STATIC_ASSERT(BCMPCIE_COMMON_MSGRINGS ==
				(BCMPCIE_H2D_COMMON_MSGRINGS + BCMPCIE_D2H_COMMON_MSGRINGS));
		/* start at BCMPCIE_H2D_COMMON_MSGRINGS */
		map->flowid = ringid - BCMPCIE_D2H_COMMON_MSGRINGS;
	} else {
		/* flowid only applies to flowrings */
		map->flowid = 0;
	}
	/* host to dongle ring */
	if (params->ring_dir == H2D) {
		switch (params->ring_type) {
		case TXPOST_FLOWRING:
			/* H2D TX Post Flowring, flowid = dma idx offset */
			map->dma_idx_offset = map->flowid;
			break;
		case DYNAMIC:
			/* Use dma_offset returned from dhd_dynamic_ring_alloc */
			map->dma_idx_offset = dma_offset;
			break;
		case COMMON:
			map->dma_idx_offset = ringid;
			break;
		default:
			/* already handled */
			break;
	...
	/* dongle to host ring */
	} else if (params->ring_dir == D2H) {
		/* handle common D2H msg rings (RxCpl, TxCpl, Ctrl Cpl) */
		switch (params->ring_type) {
		case COMMON:
			map->dma_idx_offset = ringid - BCMPCIE_H2D_COMMON_MSGRINGS;
			break;
		case DYNAMIC:
			ASSERT(ringid > prot->max_h2d_rings);
			map->dma_idx_offset = dma_offset;
			break;
		default:
			/* already handled */
		...
		prot->ringmap.d2h_offset2ring[map->dma_idx_offset] = ringid;
	} else {
		/* already handled */
	}

	DHD_INFO(("%s: ringid %d dir:%d type:%s flowid %d offset %d\n",
		__FUNCTION__, ringid, map->dir, RING_TYPE_STR(map->type), map->flowid,
		map->dma_idx_offset));
 ...
 */
static int
dhd_prot_ring_alloc(dhd_pub_t *dhd, msgbuf_ring_t *ring, msgbuf_ring_params_t *params)
{
	int dma_buf_alloced = BCME_NOMEM;
	uint32 dma_buf_len = params->max_items * params->item_size;
	dhd_prot_t *prot = dhd->prot;
	int err;

	ASSERT(ring);
	ASSERT(params->name);
	ASSERT(params->item_type <= MSGBUF_WI_ACWI64);

	if (params->ring_type == TXPOST_FLOWRING) {
		// offset of ethhdr
		ASSERT((params->item_misc != 0) && (params->item_misc < params->item_size));
	}
	if (params->ringid == BCMPCIE_H2D_MSGRING_RXPOST_SUBMIT) {
		// lr_shift by 0 or 2
		ASSERT((params->item_misc == 0) || (params->item_misc == 2));
	}

	/* Init name */
	strncpy(ring->name, params->name, RING_NAME_MAX_LENGTH);
	ring->name[RING_NAME_MAX_LENGTH - 1] = '\0';

	ring->item_type = params->item_type;
	ring->item_size = params->item_size;
	ring->max_items = params->max_items;
	ring->item_misc = params->item_misc;

	if ((err = dhd_prot_ring_map_init(dhd, params)) != BCME_OK) {
		DHD_ERROR(("Failure to allocate %s ring\n", params->name));
		return BCME_ERROR;
	}

	/* Dynamic ring's ring id is written to params by dhd_prot_ring_map_init() */
	ring->id = params->ringid;

	/* A contiguous space may be reserved for all flowrings */
	if (params->ring_type == TXPOST_FLOWRING && (prot->flowrings_dma_buf.va)) {
		...
		base_offset = (DHD_RINGID_TO_FLOWID(prot, params->ringid) -
			BCMPCIE_H2D_COMMON_MSGRINGS) * dma_buf_len;
...
#if defined(BCM_DHD_RUNNER)
		if (((ring->id == BCMPCIE_H2D_MSGRING_RXPOST_SUBMIT) ||
			...
			dhd_runner_notify(dhd->runner_hlp,
				H2R_MSGRING_FORMAT_NOTIF, ring->id,
				((((unsigned long)ring->item_size) << 16) | ring->item_type));
		}
		...
		if (dma_buf_alloced == BCME_OK) {
			/* Update max_items based on latest dma_buffer length */
...
dhd_prot_ring_init(dhd_pub_t *dhd, msgbuf_ring_t *ring)
{
	uint16 ringid;
	dhd_prot_t *prot = dhd->prot;
	bool h2d = DHD_IS_H2D(prot, ring->id);
	...
	if (IDMA_ACTIVE(dhd)) {
		ring->idma_index = dhd_idma_flowmgr_ringid_2_index(
			dhd, h2d, DHD_RING_OFFSET(prot, ring->id));
	}
...
dhd_prot_ring_reset(dhd_pub_t *dhd, msgbuf_ring_t *ring)
{
...
#ifdef BCM_SECURE_DMA
	{
		...
		if (ring->dma_buf.secdma) {
			SECURE_DMA_UNMAP_ALL(dhd->osh, ring->dma_buf.secdma);
		}
	}
#endif /* BCM_SECURE_DMA */
...
dhd_prot_ring_free(dhd_pub_t *dhd, msgbuf_ring_t *ring)
{
	 ...
	 */
	if (DHD_IS_FLOWRING(prot, ring->id) && (prot->flowrings_dma_buf.va)) {
		(void)dhd_dma_buf_audit(dhd, &ring->dma_buf, ring->name);
...
#define	DHD_NON_DYN_H2D_RINGS(prot)	((prot)->max_h2d_rings - (prot)->max_h2d_dyn_rings)
...
#define FOREACH_RING_IN_FLOWRINGS_POOL(prot, ring, flowid) \
	for ((flowid) = DHD_FLOWRING_START_FLOWID, \
		 ...
		 (flowid) < DHD_NON_DYN_H2D_RINGS((prot));	\
		 (flowid)++, (ring)++)
...
dhd_prot_flowrings_pool_alloc(dhd_pub_t *dhd)
{
	...
	prot->max_h2d_dyn_rings = (uint16)dhd_bus_max_h2d_dynamic_rings(dhd->bus);
	...
	FOREACH_RING_IN_FLOWRINGS_POOL(prot, ring, flowid) {
		...
		dhd_h2d_txpost_params_init(prot, &params, DHD_FLOWID_TO_RINGID(flowid),
			ring_name);

		if (dhd_prot_ring_alloc(dhd, ring, &params) != BCME_OK)
		{
			goto attach_fail;
		}
	...
	uint16 nitems)
{
	...
	/* raise h2d interrupt */
	if (HWA_RXPOST_ACTIVE(ring)) {
		db_index = BCM_SBF(ring->wr, CH2_H2DEV_DB_INDEX_VAL) |
			BCM_SBF(DMA_TYPE_HWA_RXPOST, CH2_H2DEV_DB_DMA_TYPE);
		prot->db1_2_ring_fn(dhd->bus, db_index, TRUE);
	} else if (IDMA_ACTIVE(dhd)) {
		db_index = BCM_SBF(ring->idma_index, CH2_H2DEV_DB_FRG_ID) |
			BCM_SBF((ring->idma_index >> CH2_H2DEV_DB_FRG_ID_BITS),
				CH2_H2DEV_DB_FRG_ID_EXT) |
			BCM_SBF(DMA_TYPE_IDMA, CH2_H2DEV_DB_DMA_TYPE);
...
dhd_prot_upd_read_idx(dhd_pub_t *dhd, msgbuf_ring_t * ring)
{
	...
	if (HWA_TXCPL_ACTIVE(ring)) {
		db_index = BCM_SBF(ring->rd, CH2_H2DEV_DB_INDEX_VAL) |
			BCM_SBF(DMA_TYPE_HWA_TXCPL, CH2_H2DEV_DB_DMA_TYPE);
		prot->db1_2_ring_fn(dhd->bus, db_index, TRUE);
	} else if (HWA_RXCPL_ACTIVE(ring)) {
		db_index = BCM_SBF(ring->rd, CH2_H2DEV_DB_INDEX_VAL) |
			BCM_SBF((ring->id - BCMPCIE_D2H_MSGRING_TX_COMPLETE),
				CH2_H2DEV_DB_RINGINDEX_NUM) |
			BCM_SBF(DMA_TYPE_HWA_RXCPL, CH2_H2DEV_DB_DMA_TYPE);
		prot->db1_2_ring_fn(dhd->bus, db_index, TRUE);
	} else if (IDMA_ACTIVE(dhd)) {
		db_index = BCM_SBF(ring->idma_index, CH2_H2DEV_DB_FRG_ID) |
			BCM_SBF((ring->idma_index >> CH2_H2DEV_DB_FRG_ID_BITS),
...
static void
dhd_prot_dma_indx_memory_check(dhd_pub_t *dhd)
{
	...
	ptr = (uint32 *)(prot->rsvd_h2d_dma_indx_buf.va);
	u32_data = LTOH32(*(ptr));
	if ((u32_data != 0xdeadbeef)) {
		DHD_ERROR(("%s: h2d dma_indx buf is corrutped, from 0xdeadbeef to 0x%x\n",
				__FUNCTION__, u32_data));
		ASSERT(0);
	}
...
dhd_prot_dma_indx_set(dhd_pub_t *dhd, uint16 new_index, uint8 type, uint16 ringid)
{
	...
	switch (type) {
		case H2D_DMA_INDX_WR_UPD:
			dhd_prot_dma_indx_memory_check(dhd);
	...
	offset = DHD_RING_OFFSET(prot, ringid);
...
#if defined(BCM_DHD_RUNNER) && !defined(CMWIFI)
			dma_buf = &prot->rsvd_h2d_dma_indx_buf;
			if (dhd_prot_dma_indx_alloc(dhd, type, dma_buf, "h2d_wr", 4096)) {
				goto ret_no_mem;
			} else {
				// set label 0xdeadbeef
				uint32 *ptr = (*dma_buf).va;
				uint32 rd_val, val = 0xdeadbeef;
				*ptr = htol32(val);
				OSL_CACHE_FLUSH((void *)ptr, sizeof(uint32));
				rd_val = LTOH32(*(ptr));
				DHD_PCIE_IPC(("\t Protocol: "
					"[RSVD]H2D WR Indices buf set label 0x%x \n", rd_val));
			}
			DHD_PCIE_IPC(("\t Protocol: "
				"[RSVD]H2D WR Indices buf:" DHD_DMA_BUF_FMT " = %d\n",
				DHD_DMA_BUF_VAL(*dma_buf), dma_buf->len));
#endif /* BCM_DHD_RUNNER && !CMWIFI */
...
dhd_prot_get_read_addr(dhd_pub_t *dhd, msgbuf_ring_t *ring, uint32 *available_len)
{
	...
	*available_len = (uint32)(items * ring->item_size);
	...
	if ((PHYSADDRLO(ring->dma_buf.pa) & 0xFF000000) != 0xDA000000) {
...
extern int BCMFASTPATH
dhd_prot_flow_ring_avail_space(dhd_pub_t *dhdp, flow_ring_node_t *flow_ring_node)
{
	uint16 wr, rd, av;
	msgbuf_ring_t *ring = (msgbuf_ring_t*)(flow_ring_node->prot_info);

	if (DMA_INDX_ENAB(dhdp->dma_d2h_ring_upd_support))
		rd = dhd_prot_dma_indx_get(dhdp, H2D_DMA_INDX_RD_UPD, ring->id);
	else
		rd = ring->rd;

	if (DMA_INDX_ENAB(dhdp->dma_h2d_ring_upd_support))
		wr = dhd_prot_dma_indx_get(dhdp, H2D_DMA_INDX_WR_UPD, ring->id);
	else
		wr = ring->wr;

	av = WRITE_SPACE_AVAIL(rd, wr, ring->max_items);
...
#if defined(BCM_DHD_RUNNER)
	if (DHD_IS_FLOWRING(dhd->prot, flow_ring->id)) {
	    dhd_runner_notify(dhd->runner_hlp, H2R_FLRING_FLUSH_NOTIF,
	        DHD_RINGID_TO_FLOWID(dhd->prot, flow_ring->id), 0);
	}
#endif /* BCM_DHD_RUNNER */
	...
	struct bcmstrbuf *strbuf, const char * fmt)
{
	...
	/* H2D COMMON/DYNAMIC RING */
	if (DHD_IS_COMMON_H2D(prot, flow_ring->id) || DHD_IS_DYNAMIC_H2D(prot, flow_ring->id)) {
		if (DMA_INDX_ENAB(dhd->dma_d2h_ring_upd_support))
	...
	/* H2D FLOWRING  */
	} else if (DHD_IS_FLOWRING(prot, flow_ring->id)) {
		if (DMA_INDX_ENAB(dhd->dma_d2h_ring_upd_support))
			host_rd = dhd_prot_dma_indx_get(dhd, H2D_DMA_INDX_RD_UPD, flow_ring->id);
		else
			host_rd = flow_ring->rd;
	...
	if (PCIE_HBQD(dhd) && (DHD_IS_FLOWRING(prot, flow_ring->id))) {
		uint32 qd;
		uint32 host_qd;
...
#if defined(DHD_PCIE_PKTID_PPI)
	{
		dhd_pktid_map_ppi_t *map = (dhd_pktid_map_ppi_t*)prot->pktid_map_handle;

		bcm_bprintf(strbuf, "pktidmap_ppi: mode %d hi: 0x%x idx/ptr %ld/%ld\n",
			map->ppi_mode, map->ref_addr_hi, map->pi_hits, map->pp_hits);
	}
#endif /* DHD_PCIE_PKTID_PPI */

#ifdef BCM_PCAP
	if (prot->h2d_pcap_subn) {
		bcm_bprintf(strbuf, "PCAP-Post: ");
		dhd_prot_print_flow_ring(dhd, prot->h2d_pcap_subn, strbuf, NULL);
	}
	if (prot->d2h_pcap_cpln) {
		bcm_bprintf(strbuf, "PCAP-Cpl: ");
		dhd_prot_print_flow_ring(dhd, prot->d2h_pcap_cpln, strbuf, NULL);
	}
#endif /* BCM_PCAP */
}
...
dhd_prot_flow_ring_delete_response_process(dhd_pub_t *dhd, void *msg)
{
	...
	if ((dhd->fast_delete_ring_support) &&
		(flow_delete_resp->cmplt.status == BCME_OK)) {
...
dhd_msgbuf_ring_config_d2h_soft_doorbell(dhd_pub_t *dhd)
{
	...
	*/
	...
	for (ring_idx = 0; ring_idx < d2h_rings; ring_idx++) {
		...
		ring_config_req->ring_id = htol16(DHD_D2H_RINGID(prot, ring_idx));
...
static void
dhd_pcap_pkt_to_haddr64(dhd_pub_t *dhd, haddr64_t *h, void *p)
{
#ifndef BCM_SECURE_DMA
	dmaaddr_t pa = DMA_MAP(dhd->osh, PKTDATA(dhd->osh, p),
			PKTLEN(dhd->osh, p), DMA_TX, p, 0);
	dhd_base_addr_htolpa(h, pa);
#endif
}

static inline void
dhd_prot_pcap_send_pkt_to_dongle(dhd_pub_t *dhd, void *pkt, uint32 pktid)
{
	unsigned long flags;
	uint16 alloced = 0;
	pcap_d11pkt_req_t *req;
	msgbuf_ring_t *ring = &dhd->prot->h2dring_ctrl_subn;

	DHD_GENERAL_LOCK(dhd, flags);
	req = dhd_prot_alloc_ring_space(dhd, ring, 1, &alloced, FALSE);

	if (!req) {
		DHD_ERROR(("%s: Cannot allocate ring space\n", __FUNCTION__));
		goto send_to_dongle_exit;
	}

	req->msg.msg_type = MSG_TYPE_PCAP_D11PKT_REQ;
	req->msg.epoch = ring->seqnum % H2D_EPOCH_MODULO;
	ring->seqnum++;

	dhd_pcap_pkt_to_haddr64(dhd, &req->data_buf_haddr64, pkt);
	req->host_pktid = pktid;
	req->pktlen = (uint32) PKTLEN(dhd->osh, pkt);
	dhd_prot_ring_write_complete(dhd, ring, req, 1);

	dhd->pcap.mgmt_pkts_sent++;
...
static inline void
dhd_prot_pcap_d11pkt_resp_process(dhd_pub_t *dhd, void *msg)
{
	pcap_d11pkt_resp_t *resp = (pcap_d11pkt_resp_t *)msg;

	if (resp->host_pktid) {
		dhd->pcap.mgmt_pkts_acked++;
		dhd_prot_packet_free(dhd, resp->host_pktid, PKTTYPE_PCAP_RX);
	}
}
...
#ifdef BCM_PCAP
static inline bool
dhd_prot_pcap_is_ap_bound_pkt(dhd_pub_t *dhd, void *pkt)
{
	/* pkt:[ rxhdr ][ phy rxs ][ pad ][ plcp ][ mac header ... ] */
	struct dot11_header *h;
	uint16 hwrxoff;
	bool send_to_dongle;
	d11rxhdr_t *rxhdr = (d11rxhdr_t *)pkt;

	/* Broadcast frames also need to be sent do dongle */
	hwrxoff = sizeof(d11rxhdr_t) + D11PHYRXSTS_LEN(dhd->monitor_info->corerev) +
			RXHDR_GET_PAD_LEN(rxhdr);
	h = (struct dot11_header *)(((uint8 *)rxhdr) + hwrxoff + D11_RXPLCP_LEN_GE128);
	send_to_dongle = ETHER_ISMULTI(&h->a1);

	return send_to_dongle || !(RXHDR_GE129_PROMISC(rxhdr, dhd->monitor_info->corerev));
}
#else
static inline bool
dhd_prot_pcap_is_ap_bound_pkt(dhd_pub_t *dhd, void *pkt)
...
#ifdef BCM_PCAP
static void
dhd_prot_d2h_pcap_ring_cmplt_process(dhd_pub_t *dhd, void *msg)
{
	pcap_cpl_awi64_t *awi = (pcap_cpl_awi64_t *)msg;
	int i;
	unsigned long flags;
	void *pkt;
	uint8 *pktdata;
	uint32 pktid;
	uint16 len;
	bool send_to_dongle, flush = FALSE;
	uint32 pkttype = PKTTYPE_INVALID;

	if (awi->msg.flags & CMN_MSG_HDR_FLAG_FLUSH) {
		/* dongle is completing unused packet; recycle it */
		flush = TRUE;
	} else if (awi->msg.flags & CMN_MSG_HDR_FLAG_TX) {
		pkttype = PKTTYPE_PCAP_TX;
	} else if (awi->msg.flags & CMN_MSG_HDR_FLAG_PROMISC_RX) {
		pkttype = PKTTYPE_PCAP_RX;
	}
	/* iterated over each host packet in the aggregate work item */
	for (i = 0; i < PCAP_AWI_AGGR; i++) {
		pktid = awi->host_pktid[i];
		send_to_dongle = FALSE;
		...
		if (pktid) {
...
#endif /* DHD_PKTID_AUDIT_RING */
			DHD_GENERAL_LOCK(dhd, flags);
			pkt = dhd_prot_packet_get(dhd, pktid, pkttype, TRUE);
			if (!pkt) {
				DHD_GENERAL_UNLOCK(dhd, flags);
				DHD_ERROR(("%s: PCAP NULL pkt\n", __FUNCTION__));
				continue;
			}

			dhd->prot->pcap_post.cpl_count++;
			...
			pktdata = PKTDATA(dhd->osh, pkt);
			if (pkttype == PKTTYPE_PCAP_RX) {
				d11rxhdr_t *rxh;
				uint16 corerev = dhd_macdbg_corerev(dhd);
				/* TODO: After d11.h is split off, unify this */
				rxh = (d11rxhdr_t *) pktdata;

				len = D11RXHDR_FRAMELEN(corerev, rxh) +
					sizeof(d11rxhdr_t) +
					D11PHYRXSTS_LEN(corerev);

				if (len > PCAP_PKT_BUFSZ) {
					DHD_ERROR(("%s: Bad RX length %d. pkt=%p pktid=0x%x\n",
						__FUNCTION__, len, pkt, pktid));
					prhex("Packet:", pkt, PCAP_PKT_BUFSZ);
					PKTFREE(dhd->osh, pkt, FALSE);
					continue;
				}
				len = MIN(PCAP_PKT_BUFSZ, len);
				PKTSETLEN(dhd->osh, pkt, len);
				dhd->pcap.tot_pkts_recd++;

				if (awi->msg.request_id == DHD_D2H_PCAP_REQ_PKTID) {
					/* Test message pattern - wl pcap_test */
					PKTFREE(dhd->osh, pkt, FALSE);
					continue;
				}

				/* Set send_to_dongle only when RX Promisc is enabled */
				if (dhd->pcap.pcap_config & WLC_PCAP_PROMISC_EN) {
					send_to_dongle =
						dhd_prot_pcap_is_ap_bound_pkt(dhd, pktdata);
				}
				/* Only send mgmt pkts back to DHD and packets to the radiotap if
				 * if the radiotap intf is present and promisc capture is enabled.
				 */
				if (dhd->pcap.pcap_if.radiotap_enabled) {
					struct sk_buff *skb = NULL;
					if (dhd->pcap.pcap_config & WLC_PCAP_PROMISC_EN) {
						if (send_to_dongle) {
							skb = dhd_pcap_copy(dhd, pkt,
								PCAP_PKT_RX_PROMISC);
							dhd->pcap.mgmt_pkts_recd++;
						} else {
							skb = dhd_pcap_dup(dhd, pkt,
								PCAP_MON_SCALE * PCAP_PKT_BUFSZ,
								PCAP_PKT_RX_PROMISC);
						}
					} else if (dhd->pcap.pcap_config & WLC_PCAP_RXCOPY_EN) {
						skb = dhd_pcap_dup(dhd, pkt, len,
							PCAP_PKT_RX_PROMISC);
						dhd->pcap.mgmt_pkts_recd++;
					}
					if (skb) {
				...
				if (send_to_dongle) {
					/*
					 * ap-bound packets are freed after they are
					 * processed by the dongle.
					 */
					uint32 new_pktid;
					dmaaddr_t pa;

					DHD_GENERAL_LOCK(dhd, flags);
					pa = DMA_MAP(dhd->osh, pktdata, len, DMA_RX, pkt, 0);
					/* Need to generate a new pktid as earlier one is freed */
					new_pktid = DHD_NATIVE_TO_PKTID(dhd,
					                                dhd->prot->pktid_map_handle,
					                                pkt, pa, len, DMA_RX, NULL,
					                                NULL, PKTTYPE_PCAP_RX);
					DHD_GENERAL_UNLOCK(dhd, flags);

					dhd_prot_pcap_send_pkt_to_dongle(dhd, pkt, new_pktid);
				} else {
					PKTFREE(dhd->osh, pkt, FALSE);
				}
			} else if (pkttype == PKTTYPE_PCAP_TX) {
				if (dhd->pcap.pcap_if.radiotap_enabled &&
				    (dhd->pcap.pcap_config & WLC_PCAP_TXCOPY_EN)) {
					struct sk_buff *skb;
					pcap_tx_hdr_t *pcap_hdr;
					uint16 len;

					dhd->pcap.local_tx_pkts++;

					/*
					 * For local Tx Copy pkts, we need to pull the actual
					 * pkt length from the pcap_tx_hdr
					 */
					pcap_hdr = (pcap_tx_hdr_t *)PKTDATA(dhd->osh, pkt);

					/*
					 * The len in the pcap_tx_hdr is just the 802.11 frame
					 * len, so add on the size of the pcap_tx_hdr too
					 */
					len = pcap_hdr->pkt_size + sizeof(pcap_tx_hdr_t);

					skb = dhd_pcap_dup(dhd, pkt, len, PCAP_PKT_TX_LOCAL_COPY);
				...
				dhd->pcap.inv_pkttype_cpl++;
				DHD_ERROR(("%s: Invalid pkt type %d\r\n", __FUNCTION__, pkttype));
...
dhd_prot_ringupd_dump(dhd_pub_t *dhd, struct bcmstrbuf *b)
{
	...
	bcm_bprintf(b, "\nRPTR block H2D common rings, 0x%px\n", (void*)ptr);
	value = ltoh32(*ptr);
	...
	ptr++;
	bcm_bprintf(b, "RPTR block Flow rings , 0x%px\n", (void*)ptr);
	...
	bcm_bprintf(b, "\nWPTR block D2H common rings, 0x%px\n", (void*)ptr);
	value = ltoh32(*ptr);
...
dhd_prot_runner_txstatus_process(dhd_pub_t *dhd, void* msg)
{
	...
	if (prot->txpost_item_type == MSGBUF_WI_WI64) {
	    ...
	    if (DHD_FLOWRING_RNR_OFFL(flow_ring_node)) {
			...
			OSL_VIRT_TO_PHYSADDR(dhd->osh, PKTDATA(dhd->osh, pkt), pa);
...
dhd_prot_runner_preinit(dhd_pub_t *dhdp)
{
	...
	rc = dhd_runner_notify(dhdp->runner_hlp, H2R_MSGRING_FORMAT_NOTIF,
	    ID16_INVALID,
	    ((((unsigned long)prot->txpost_item_size) << 16) | prot->txpost_item_type));
	if (rc != BCME_OK) {
...
dhd_prot_rxpost_upd(dhd_pub_t *dhd)
{
	...
	if (prot->hwa_caps & PCIE_IPC_HCAP1_HWA_RXPOST_IDMA) {
		/* raise h2d interrupt */
		u32_db_index = BCM_SBF(wr, CH2_H2DEV_DB_INDEX_VAL) |
			BCM_SBF(DMA_TYPE_HWA_RXPOST, CH2_H2DEV_DB_DMA_TYPE);
...
dhd_prot_rxpost_reclaim(dhd_pub_t *dhd)
{
	...
	/* Refresh the rxpost submission common ring's rd index */
	dhd_msgbuf_rxbuf_post_refresh(dhd, ring);
...
dhd_prot_process_hwa_event(dhd_pub_t *dhd, uint32 reason)
{
	...
	switch (reason) {
		...
		case WLC_E_HWA_DHD_DUMP:
			...
			/* Update WR manually, for debug purpose */
			dhd_prot_rxpost_upd(dhd);
			break;
...
char *
dhd_prot_cpudbg_decode(dhd_pub_t *dhdp, wl_cpudbg_entry_t *cpudbg_entry, uint cpudbg_kconsole)
{
	char *dbgstrs_bin = NULL, *cp = NULL;
	dhd_bus_t *bus = dhdp->bus;
	bool labels_valid = (dhdp->cpudbg_psmr_labels_num != 0);
	char pfx_buf[32];
	char *str_p = "";
	dbgstr_id_t stringId = cpudbg_entry->stringId;

	if (stringId.version != CPU_DBG_SID_VER) {
		DHD_INFO(("wl%d: %s: Invalid stringId version (%x)\n", bus->dhd->unit,
			__FUNCTION__, stringId.u32));
		return NULL;
	}
	if (stringId.cpu_id < CPUC_ID_PSMR0 && stringId.link_id > PCIE_IPC_AP_UNIT_MAX) {
		DHD_INFO(("wl%d: %s: Invalid stringId link_id (%x)\n", bus->dhd->unit,
			__FUNCTION__, stringId.u32));
		return NULL;
	}

	/* make sure only one bit is set in msglevel */
	if (!stringId.msglevel || ((stringId.msglevel & (stringId.msglevel - 1)) != 0)) {
		DHD_INFO(("wl%d: %s: Invalid stringId msglevel (%x)\n", bus->dhd->unit,
			__FUNCTION__, stringId.u32));
		return NULL;
	}

	switch (stringId.cpu_id) {
	case CPUC_ID_PHY:
	case CPUC_ID_HWEVT:
		break;
	case CPUC_ID_SMAC:
		if (dhdp->cpudbg_smac_strs_len == 0 || !dhdp->cpudbg_smac_strs_valid ||
			stringId.offset >= dhdp->cpudbg_smac_strs_len) {
			return NULL;
		}
		dbgstrs_bin = dhdp->cpudbg_smac_strs;
		break;
	case CPUC_ID_MLO:
		if (dhdp->cpudbg_mlo_strs_len == 0 || !dhdp->cpudbg_mlo_strs_valid ||
			stringId.offset >= dhdp->cpudbg_mlo_strs_len) {
			return NULL;
		}
		dbgstrs_bin = dhdp->cpudbg_mlo_strs;
		break;
	case CPUC_ID_PSMR0:
	case CPUC_ID_PSMR1:
		if (labels_valid) {
			str_p = cpudbg_nearest_psmr_label(dhdp->cpudbg_psmr_labels,
				dhdp->cpudbg_psmr_labels_num,
				stringId.offset & ~(UCODE_FAULT_BIT | UTRACE_FLAG_BIT));
			if (str_p == NULL) {
				str_p = "LABEL NOT FOUND";
			}
		}
		break;
	default:
		DHD_INFO(("wl%d: %s: Invalid CPU DBG CPU Id: (%x)\n", bus->dhd->unit,
			__FUNCTION__, stringId.u32));
		return NULL;
	}

	/* dbgstrs.bin is a string vector: verify there is '\0' prior to the offset */
	if (dbgstrs_bin && !(stringId.offset == 4 || dbgstrs_bin[stringId.offset - 1] == '\0')) {
		DHD_INFO(("wl%d: %s: stringId 0x%x: Invalid %s dbgstrs input detected. Ensure "
			"dbgstrs binary is in sync with the fw\n", bus->dhd->unit, __FUNCTION__,
			stringId.u32, (stringId.cpu_id == CPUC_ID_SMAC) ? "SMAC" : "MLO"));
		return NULL;
	} else {
		uint32 *hwevt_msglevel_p = (uint32 *)((uint8*)(dhdp->cpudbg_hme) +
			OFFSETOF(cpudbg_hme_t, hwevt_msglevel));
		snprintf(pfx_buf, sizeof(pfx_buf), "MAC CONSOLE[wl%d]: ", bus->dhd->unit);
		if (dbgstrs_bin)
			str_p = &dbgstrs_bin[stringId.offset];
		cpudbg_dbglog_decode(dhdp->cpudbg_printbuf, CPUDBG_BUFSZ, cpudbg_entry, str_p,
			"", pfx_buf, *hwevt_msglevel_p & HWEVT_MASK);
		dhdp->cpudbg_printbuf[CPUDBG_BUFSZ - 1] = '\0';
		/* Force-print FAULT and ASSERT msgs to the kernel console */
		if (cpudbg_kconsole || (strstr(dhdp->cpudbg_printbuf, "FAULT:") ||
			strstr(dhdp->cpudbg_printbuf, "ASSERT "))) {
			printf("%s", dhdp->cpudbg_printbuf);
		}
		if (strlen(dhdp->cpudbg_printbuf) > 11) {
			cp = dhdp->cpudbg_printbuf + 11; /* strip "MAC CONSOLE" pfx from ret val */
		} else {
			cp = dhdp->cpudbg_printbuf;
		}
	}
...
/*
 * Read MAC CPU Cluster debug module entries, decode and print
 *
 * Each xfer contains a variable number of entries. The metadata for each xfer is recoded in the
 * xfer's 16-byte DMA header. See
 * https://confluence.broadcom.net/display/BCAWLAN/CPU+Cluster+Debug+Logging+Feature
 */
int
dhd_prot_cpudbg_process_entries(dhd_pub_t *dhdp, int *entry_ct, uint32 **entry_p, bool d11dump,
	cpudbg_dropcts_t *dropcts)
{
	...
	uint32 xfer_size;
	uint16 wrap_idx;
	uint32 max_subbuf_entry;

	*entry_ct = 0;
	bzero(dropcts, sizeof(cpudbg_dropcts_t));
	xfer_size = (1 << (dhdp->cpudbg_config & CM_CPU_DBG_CFG_SZ_MASK));
	wrap_idx = (CPUDBG_HOSTBUF_SZ / xfer_size) - 1;
	max_subbuf_entry = (xfer_size - CM_CPU_DBG_DMA_HDR_SIZE) / CM_CPU_DBG_ENTRY_SIZE;

	rdidx = dhdp->cpudbg_rdidx;
	wridx = (volatile uint16)dhdp->cpudbg_wridx;
	if (rdidx != wridx) {
		uint8 seq_num, total_entries, total_dropped;
		uint16 word0, word1, word2;
		uint16 cpu0_entry_ct, cpu1_entry_ct, cpu2_entry_ct, cpu3_entry_ct;
		uint16 cpu0_drop_ct, cpu1_drop_ct, cpu2_drop_ct, cpu3_drop_ct;
		uint8 *RdBase;
		uint32 *EntryBase;

		RdBase = (uint8 *)prot->cpudbg_dma_buf.va + (rdidx * xfer_size);
		word0 = *((uint16*)&RdBase[0]);
		word1 = *((uint16*)&RdBase[2]);
		word2 = *((uint16*)&RdBase[4]);
		cpu0_entry_ct = word0 & 0x0fff;
		cpu1_entry_ct = ((word1 & 0x00ff) << 8)  |
			(((word0 & 0xf000) >> 12) & 0x0fff);
		cpu2_entry_ct = ((word2 & 0x000f) << 12) |
			(((word1 & 0xff00) >> 8)  & 0x0fff);
		cpu3_entry_ct = ((word2 & 0xfff0) >> 4) & 0x0fff;
		total_entries =
			cpu0_entry_ct + cpu1_entry_ct + cpu2_entry_ct + cpu3_entry_ct;
		word0 = *((uint16*)&RdBase[6]);
		word1 = *((uint16*)&RdBase[8]);
		word2 = *((uint16*)&RdBase[10]);
		cpu0_drop_ct = word0 & 0x0fff;
		cpu1_drop_ct = ((word1 & 0x00ff) << 8)  |
			(((word0 & 0xf000) >> 12) & 0x0fff);
		cpu2_drop_ct = ((word2 & 0x000f) << 12) |
			(((word1 & 0xff00) >> 8)  & 0x0fff);
		cpu3_drop_ct = ((word2 & 0xfff0) >> 4) & 0x0fff;
		total_dropped = cpu0_drop_ct + cpu1_drop_ct + cpu2_drop_ct + cpu3_drop_ct;
		seq_num = RdBase[CM_CPU_DBG_M2M_HDR_SEQNUM_IDX];
		if (!d11dump) {
			DHD_TRACE(("wl%d: %s: %x: RdIdx %u; new WrIdx -> %u\n", dhdp->unit,
				__FUNCTION__, seq_num, rdidx, wridx));
		}
		if (!d11dump && RdBase[CM_CPU_DBG_M2M_HDR_TAG_IDX] == 0xff) {
			DHD_TRACE(("wl%d: %s: sequence number %u was not updated\n", dhdp->unit,
				__FUNCTION__, seq_num));
			return BCME_NOTFOUND;
		}
		/* check for sync required: need to sync up with the first entry? */
		if (dhdp->cpudbg_syncreq) {
			if (seq_num != 0) {
				DHD_INFO(("wl%d: %s: skipping seq_num %u\n", dhdp->unit,
					__FUNCTION__, seq_num));
				total_entries = 0;
			} else {
				DHD_ERROR(("wl%d: %s: sync'd @ %d\n", dhdp->unit, __FUNCTION__,
					seq_num));
				dhdp->cpudbg_syncreq = FALSE;
			}
		}
		if (!dhdp->cpudbg_syncreq) {
			uint16 next_seqnum;
			next_seqnum = (dhdp->cpudbg_seqnum + 1) & CPU_DBG_MAX_SEQNUM;
			if (next_seqnum != seq_num) {
				DHD_INFO(("wl%d: %s: gap between prev seqnum %u & curr seqnum %u\n",
					dhdp->unit, __FUNCTION__, next_seqnum, seq_num));
			}
			dhdp->cpudbg_seqnum = seq_num;
		}
		if (total_entries > max_subbuf_entry) {
			DHD_ERROR(("wl%d: %s: total_entries (%d) > max_subbuf_entry (%d)\n",
				dhdp->unit, __FUNCTION__, total_entries, max_subbuf_entry));
			total_entries = 0;
		} else if (!dhdp->cpudbg_syncreq && total_dropped != 0) {
			dropcts->cpu0_drop_ct = cpu0_drop_ct;
			dropcts->cpu1_drop_ct = cpu1_drop_ct;
			dropcts->cpu2_drop_ct = cpu2_drop_ct;
			dropcts->cpu3_drop_ct = cpu3_drop_ct;
			DHD_INFO(("wl%d: %s: seq num %u cpu0_drop_ct %u cpu1_drop_ct %u "
				"cpu2_drop_ct %u cpu3_drop_ct %u\n", dhdp->unit, __FUNCTION__,
				seq_num, cpu0_drop_ct, cpu1_drop_ct, cpu2_drop_ct, cpu3_drop_ct));
		}
		dhdp->cpudbg_ctrs.cpu0_entry_ct += cpu0_entry_ct;
		dhdp->cpudbg_ctrs.cpu1_entry_ct += cpu1_entry_ct;
		dhdp->cpudbg_ctrs.cpu2_entry_ct += cpu2_entry_ct;
		dhdp->cpudbg_ctrs.cpu3_entry_ct += cpu3_entry_ct;
		dhdp->cpudbg_ctrs.cpu0_drop_ct += cpu0_drop_ct;
		dhdp->cpudbg_ctrs.cpu1_drop_ct += cpu1_drop_ct;
		dhdp->cpudbg_ctrs.cpu2_drop_ct += cpu2_drop_ct;
		dhdp->cpudbg_ctrs.cpu3_drop_ct += cpu3_drop_ct;
		EntryBase = (uint32 *)(RdBase + CM_CPU_DBG_DMA_HDR_SIZE);
		*entry_ct = total_entries;
		*entry_p = EntryBase;
		++dhdp->cpudbg_rdidx;
		if (dhdp->cpudbg_rdidx > wrap_idx) {
			dhdp->cpudbg_rdidx = 0;
			dhdp->cpudbg_wraparound = TRUE;
		}
		if (!d11dump) {
			/* mark the entry as having been consumed */
			RdBase[CM_CPU_DBG_M2M_HDR_TAG_IDX] = 0xff;
		}
	} else {
		return BCME_NOTFOUND;
	}
...
/*
 * Fetch embedded dbgstrs and psmr label images from dongle SysMem if their
 * info has been configured by the dongle in the host LOWMAC HME region.
 * Also cache device address of CPUDBG M2M DMA RdIdx register for efficient
 * ACK of CPUDBG entries consumed by the host.
 * Must be called prior to "wl up" since the image data is RECLAIMable
 */
void
dhd_prot_cpudbg_load_hmedata(dhd_pub_t *dhd)
{
	dhd_prot_t *prot = dhd->prot;
	cpudbg_dbgstrs_info_t *cpudbg_hme_dbgstrs =
		(cpudbg_dbgstrs_info_t *)
		((uint8*)(dhd->cpudbg_hme) +
		 OFFSETOF(cpudbg_hme_t, dbgstrs_info));
	daddr32_t daddr32;
	char *cp;
	uint32 size;
	dhd_dma_buf_t *dma_buf = NULL;
	uint32 *cpudbg_hme_rdidx_daddr32;

	/* hybridfw_dma_buf is already handed off to the HME */
	dma_buf = dhd_prot_hme_get_dma_buf(dhd, PCIE_IPC_HME_USER_HMOSWP);

	if (dhd->cpudbg_wl_support && dhd->cpudbg_hme) {
		size = ltoh32(cpudbg_hme_dbgstrs->mlo_dbgstrs_size);
		daddr32 = ltoh32(cpudbg_hme_dbgstrs->mlo_dbgstrs_daddr32);
		DHD_INFO(("wl%d: %s: dongle mlo_dbgstrs size %u, daddr 0x%x\n", dhd->unit,
			__FUNCTION__, size, daddr32));
		if (size &&
#if defined(BCMQT)
		    /* time-consuming read: anything too big triggers kernel lock-up measures */
		    size < 25000 &&
#endif
		    dhd->cpudbg_mlo_strs_len == 0) {
			if ((cp = MALLOC(prot->osh, size))) {
				if (daddr32 < SWP_PAGEABLE_START) {
					dhdpcie_bus_read_sysmem(dhd->bus, daddr32,
						cp, size);
				} else {
					ASSERT(dma_buf && (dma_buf->va != NULL));
					memcpy(cp, dma_buf->va +
						(daddr32 - SWP_PAGEABLE_START), size);
				}
				dhd->cpudbg_mlo_strs = cp;
				dhd->cpudbg_mlo_strs_len = size;
				dhd->cpudbg_mlo_strs_valid = TRUE;
			}
		}
		size = ltoh32(cpudbg_hme_dbgstrs->smac_dbgstrs_size);
		daddr32 = ltoh32(cpudbg_hme_dbgstrs->smac_dbgstrs_daddr32);
		DHD_INFO(("wl%d: %s: dongle smac_dbgstrs size: %u, daddr 0x%x\n",
			dhd->unit, __FUNCTION__, size, daddr32));
		if (size &&
#if defined(BCMQT)
		    /* time-consuming read: anything too big triggers kernel lock-up measures */
		    size < 25000 &&
#endif
		    dhd->cpudbg_smac_strs_len == 0) {
			if ((cp = MALLOC(prot->osh, size))) {
				if (daddr32 < SWP_PAGEABLE_START) {
					dhdpcie_bus_read_sysmem(dhd->bus, daddr32,
						cp, size);
				...
				dhd->cpudbg_smac_strs = cp;
				dhd->cpudbg_smac_strs_len = size;
				dhd->cpudbg_smac_strs_valid = TRUE;
			}
		}
		size = ltoh32(cpudbg_hme_dbgstrs->psmr_label_size);
		daddr32 = ltoh32(cpudbg_hme_dbgstrs->psmr_label_daddr32);
		DHD_INFO(("wl%d: %s: dongle psmr_label size: %u, daddr 0x%x\n",
			dhd->unit, __FUNCTION__, size, daddr32));
		if (size &&
#if defined(BCMQT)
		    /* time-consuming read: anything too big triggers kernel lock-up measures */
			...
			if ((cp = MALLOC(prot->osh, size))) {
				...
				num_labels = size / PSM_LABEL_INPUT_LINE_LEN;
				labels_size = sizeof(psm_label_t) * num_labels;
				labels = MALLOC(prot->osh, labels_size);
				if (labels) {
					if (daddr32 < SWP_PAGEABLE_START) {
						dhdpcie_bus_read_sysmem(dhd->bus, daddr32,
							cp, size);
					...
					for (i = 0; i < num_labels; ++i) {
						sscanf(line, "%x %40s",
							&labels[i].addr,
							&(labels[i].label[0]));
						line += PSM_LABEL_INPUT_LINE_LEN;
					}
					dhd->cpudbg_psmr_labels = labels;
					dhd->cpudbg_psmr_labels_num = num_labels;
				}
				MFREE(prot->osh, cp, size);
			}
		}
		/* cache CPU DBG M2M DMA RdIdx reg device addr to speed consumption of entries */
		cpudbg_hme_rdidx_daddr32 = (uint32 *)
			((uint8*)(dhd->cpudbg_hme) + OFFSETOF(cpudbg_hme_t, rdidx_daddr32));
		dhd->cpudbg_rdidx_daddr32 = ltoh32(*cpudbg_hme_rdidx_daddr32);
		ASSERT(dhd->cpudbg_rdidx_daddr32);
	}
}

int
dhd_proto_get_ringmem_info(dhd_pub_t *dhd, int ring_id, dhd_buf_info_t *ring_buf)
{
	if (ring_id == BCMPCIE_H2D_MSGRING_CONTROL_SUBMIT) {
		ring_buf->len = dhd->prot->h2dring_ctrl_subn.dma_buf.len;
		ring_buf->buf = dhd->prot->h2dring_ctrl_subn.dma_buf.va;
		strncpy(ring_buf->name, "ctrlpost", sizeof(ring_buf->name));
	} else if (ring_id == BCMPCIE_D2H_MSGRING_CONTROL_COMPLETE) {
		ring_buf->len = dhd->prot->d2hring_ctrl_cpln.dma_buf.len;
		ring_buf->buf = dhd->prot->d2hring_ctrl_cpln.dma_buf.va;
		strncpy(ring_buf->name, "ctrlcpl", sizeof(ring_buf->name));
	} else {
		return BCME_ERROR;
	}
	return BCME_OK;
...
static void
dhd_h2d_ctrl_submit_params_init(msgbuf_ring_params_t *params)
{
	params->name = "h2dctrl";
	params->ringid = BCMPCIE_H2D_MSGRING_CONTROL_SUBMIT;
	params->item_type = MSGBUF_WI_WI64;
	params->item_size = H2DRING_CTRL_SUB_ITEMSIZE;
	params->max_items = H2DRING_CTRL_SUB_MAX_ITEM;
	params->item_misc = DHD_MSGBUF_RING_ITEM_MISC;
	params->ring_dir = H2D;
	params->ring_type = COMMON;
}

static void
dhd_d2h_ctrl_completion_params_init(msgbuf_ring_params_t *params)
{
	params->name = "d2hctrl";
	params->ringid = BCMPCIE_D2H_MSGRING_CONTROL_COMPLETE;
	params->item_type = MSGBUF_WI_WI64;
	params->item_size = D2HRING_CTRL_CMPLT_ITEMSIZE;
	params->max_items = D2HRING_CTRL_CMPLT_MAX_ITEM;
	params->item_misc = DHD_MSGBUF_RING_ITEM_MISC;
	params->ring_dir = D2H;
	params->ring_type = COMMON;
}
...
static int
dhd_h2d_rxpost_params_init(dhd_prot_t *prot, msgbuf_ring_params_t *params, uint8 rxpost_format)
{
	uint16 type, misc, size; /* work item configuration */
	...
	params->name = "h2drxp";
	params->ringid = BCMPCIE_H2D_MSGRING_RXPOST_SUBMIT;
	params->item_type = type;
	params->item_size = size;
	params->max_items = H2DRING_RXPOST_MAX_ITEM >> misc;
	params->item_misc = misc;
	params->ring_dir = H2D;
	params->ring_type = COMMON;

	DHD_PCIE_IPC(("\t Protocol: H2D RxPost %u: "
		"format %u [type %u size %2u max %4u shift %u]\n",
		params->ringid, rxpost_format, type, size, params->max_items, misc));

	return BCME_OK;
}

static int
dhd_d2h_tx_completion_params_init(msgbuf_ring_params_t *params, uint8 txcpln_format)
{
	uint16 type, misc, max, size; /* work item configuration */
	...
	params->name = "d2htxcpl";
	params->ringid = BCMPCIE_D2H_MSGRING_TX_COMPLETE;
	params->item_type = type;
	params->item_size = size;
	...
	DHD_PCIE_IPC(("\t Protocol: D2H TxCpln %u: "
		...
		params->ringid, txcpln_format, type, size, max));

	return BCME_OK;
}

static int
dhd_d2h_rx_completion_params_init(dhd_pub_t *dhd, msgbuf_ring_params_t *params,
	uint8 rxcpln_format, uint16 ringid)
{
	uint16 type, misc, max, size; /* work item configuration */
	misc = 0; /* default, single item per message, shift of 0 */
	...
	if (dhd->bus->pcie_ipc.hcap2 & PCIE_IPC_HCAP2_BA256_CFG) {
		max = D2HRING_RXCMPLT_BA256_MAX_ITEM;
	} else {
...
static void
dhd_h2d_txpost_params_init(dhd_prot_t *prot, msgbuf_ring_params_t *params, uint16 ringid,
	char *ring_name)
{
	params->name = ring_name;
	params->ringid = ringid;
	params->item_type = prot->txpost_item_type;
	params->item_size = prot->txpost_item_size;
	params->max_items = prot->txpost_max_items;
	params->item_misc = prot->txpost_item_misc;
	params->ring_dir = H2D;
	params->ring_type = TXPOST_FLOWRING;
}

static void
dhd_ring_map_attach(dhd_pub_t *dhd)
{
	dhd_prot_t *prot = dhd->prot;

	prot->ringmap.ring_count = MAX_DHD_RINGS;
	prot->ringmap.d2h_offset_count = BCMPCIE_D2H_COMMON_MSGRINGS; /* incr. for dyn rings */
	prot->ringmap.map = MALLOCZ(dhd->osh, sizeof(ring_map_t) * prot->ringmap.ring_count);
	prot->ringmap.d2h_offset2ring =
		MALLOCZ(dhd->osh, sizeof(uint16) * prot->ringmap.d2h_offset_count);
}

/* Initialize non-flowring dynamic rings allocation information. This is used for the
 * setup and allocation/initializion of the dynamic rings. Information such as starting
 * ring id's, dma offsets, and the total number of allocated D2H and H2D dynamic rings is
 * initialized. Called from dhd_prot_init()
 */
static void
dhd_prot_dynamic_rings_init(dhd_pub_t *dhd)
{
	uint32 max_h2d_rings, max_d2h_rings;
	uint32 max_h2d_dyn_rings, max_d2h_dyn_rings;
	dhd_prot_t *prot = dhd->prot;

	if (prot->dyn_rings_inited) {
		/* re-init case */
		return;
	}

	max_h2d_rings = dhd_bus_max_h2d_rings(dhd->bus);
	ASSERT(max_h2d_rings);

	max_d2h_rings = dhd_bus_max_d2h_rings(dhd->bus);
	ASSERT(max_d2h_rings);

	/* query max dynamic ring support */
	max_h2d_dyn_rings = dhd_bus_max_h2d_dynamic_rings(dhd->bus);
	max_d2h_dyn_rings = dhd_bus_max_d2h_dynamic_rings(dhd->bus);

	prot->h2d_dyn_rings.max_cnt = max_h2d_dyn_rings;
	prot->d2h_dyn_rings.max_cnt = max_d2h_dyn_rings;

	/* Ring ID organization:
	 * -------------------------------------|-----------------------
	 * 0					|
	 * 1					| Common Rings
	 * ...					|
	 * BCMPCIE_COMMON_MSGRINGS - 1		|
	 * -------------------------------------|-----------------------
	 * BCMPCIE_COMMON_MSGRINGS		|
	 * ...					| H2D Tx Post Flowrings
	 * BCMPCIE_COMMON_MSGRINGS +		|
	 *   pcie_ipc_rings->max_flowrings - 1	|
	 * -------------------------------------|-----------------------
	 * BCMPCIE_COMMON_MSGRINGS +		|
	 *   pcie_ipc_rings->max_flowrings	|
	 * ...					| H2D Dynamic Rings
	 * "  + dhd_bus_max_h2d_rings() - 1	|
	 * -------------------------------------|-----------------------
	 * " + dhd_bus_max_h2d_rings()		|
	 * ...					| D2H Dynamic Rings
	 * " + dhd_bus_max_h2d_rings +		|
	 *    dhd_bus_max_d2h_rings - 1		|
	 * -------------------------------------|-----------------------
	 */
	prot->h2d_dyn_rings.alloc_cnt = 0;
	prot->d2h_dyn_rings.alloc_cnt = 0;
	/* dynamic rings go at the end of the RD/WR DMA indices array */
	prot->h2d_dyn_rings.dma_offset = max_h2d_rings - max_h2d_dyn_rings;
	prot->d2h_dyn_rings.dma_offset = max_d2h_rings - max_d2h_dyn_rings;

	/* h2d_dyn_ringid, d2h_dyn_ringid -- starting ring id's for dynamic allocation */
	prot->h2d_dyn_rings.ringid =
		max_h2d_rings + max_d2h_rings - max_h2d_dyn_rings - max_d2h_dyn_rings;
	prot->d2h_dyn_rings.ringid = prot->h2d_dyn_rings.ringid + max_h2d_dyn_rings;

	DHD_PCIE_IPC(("\tProtocol: Dyn rings: H2D starting ringid %u (%u) D2H ringid %u (%u)\n",
		prot->h2d_dyn_rings.ringid, prot->h2d_dyn_rings.dma_offset,
		prot->d2h_dyn_rings.ringid, prot->d2h_dyn_rings.dma_offset));

	prot->dyn_rings_inited = TRUE;
}

/* dhd_dynamic_ring_alloc:allocates a ringid, dma offset, ..? for a dynamic ring.
 * Returns BCME_OK on success, error code otherwise.
 */
static int
dhd_dynamic_ring_alloc(dhd_pub_t *dhd, uint8 ring_dir, uint16 *ringid, uint16 *dma_offset)
{
	dhd_prot_t *prot = dhd->prot;
	dynamic_ring_info_t *dri;

	if (ring_dir == H2D) {
		dri = &prot->h2d_dyn_rings;
	} else {
		dri = &prot->d2h_dyn_rings;
	}

	if (dri->alloc_cnt >= dri->max_cnt) {
		DHD_ERROR(("wl%d: error: allocated %d of max %d dynamic rings\n",
			dhd->unit, dri->alloc_cnt, dri->max_cnt));
		return BCME_NORESOURCE;
	}

	*ringid		= dri->ringid++;
	*dma_offset	= dri->dma_offset++;
...
/* Handle a H2D dynamic ring creation message. For now only PCAP feature uses this,
 * however other dynamic ring users could be added.
 */
static void
dhd_prot_process_h2d_ring_create_complete(dhd_pub_t *dhd, void *buf)
{
	...
	/* PCAP support. Other dynamic ring types could be added */
	if (ltoh32(resp->cmn_hdr.request_id) != DHD_H2D_PCAP_REQ_PKTID) {
		DHD_ERROR(("H2D ring create cmplt: bad request ID 0x%x\n",
			ltoh32(resp->cmn_hdr.request_id)));
		return;
	}

	if (! DHD_PCAP_EN(dhd)) {
		DHD_ERROR(("Unexpected PCAP ring creation msg received\n"));
		ASSERT(0);
		return;
	}
#ifdef BCM_PCAP
	if (prot->h2d_pcap_subn &&
		prot->h2d_pcap_subn->create_req_id == ltoh32(resp->cmn_hdr.request_id) &&
		!prot->h2d_pcap_subn->create_pending) {
		DHD_ERROR(("error: H2D pcap ring create - not pending\n"));
	}

	if (ltoh16(resp->cmplt.status) != BCMPCIE_SUCCESS) {
		DHD_ERROR(("error: H2D pcap create failed with status %d\n",
			ltoh16(resp->cmplt.status)));
		return;
	}
	...
	if (prot->h2d_pcap_subn &&
		prot->h2d_pcap_subn->create_req_id == ltoh32(resp->cmn_hdr.request_id)) {
		prot->h2d_pcap_subn->create_pending = FALSE;
		prot->h2d_pcap_subn->inited = TRUE;
		DHD_PRINT(("H2D PCAP buffer post after ring create\n"));
		/* Post buffers to the PCAP subn ring */
		dhd_msgbuf_pcap_post(dhd);
	}
#endif /* BCM_PCAP */
}
...
dhd_prot_process_d2h_ring_create_complete(dhd_pub_t *dhd, void *buf)
{
	...
	/* PCAP support. Other dynamic ring types could be added */
	if (ltoh32(resp->cmn_hdr.request_id) != DHD_D2H_PCAP_REQ_PKTID) {
		DHD_ERROR(("D2H ring create cmplt: bad request ID 0x%x\n",
			ltoh32(resp->cmn_hdr.request_id)));
		return;
	}
	...
	if (!prot->d2h_pcap_cpln->create_pending) {
		DHD_ERROR(("D2H PCAP create status for not pending cpl ring\n"));
		return;
	}

	if (ltoh16(resp->cmplt.status) != BCMPCIE_SUCCESS) {
		DHD_ERROR(("D2H PCAP create failed with status %d\n",
			ltoh16(resp->cmplt.status)));
		return;
	}

	prot->d2h_pcap_cpln->create_pending = FALSE;
	prot->d2h_pcap_cpln->inited = TRUE;
}
	...
	uint16 ring_type, uint32 req_id)
{
	...
	ring_to_create->create_req_id = req_id;
	ring_to_create->create_pending = TRUE;

	/* Common msg buf hdr */
	d2h_ring->msg.msg_type = MSG_TYPE_D2H_RING_CREATE;
	d2h_ring->msg.if_id = 0;
	d2h_ring->msg.flags = 0;
	d2h_ring->msg.request_id = htol32(ring_to_create->create_req_id);
	d2h_ring->ring_id = htol16(DHD_RING_OFFSET(dhd->prot, ring_to_create->id));
	DHD_PRINT(("%s dma-offset: %d ringid: %d\n", __FUNCTION__, d2h_ring->ring_id,
			ring_to_create->id));

	d2h_ring->ring_type = ring_type;
	d2h_ring->max_items = htol16(ring_to_create->max_items);
	d2h_ring->item_size = htol16(ring_to_create->item_size);
	d2h_ring->haddr64.low_addr = ring_to_create->base_addr.low_addr;
	d2h_ring->haddr64.high_addr = ring_to_create->base_addr.high_addr;
	...
	/* Update the flow_ring's WRITE index */
	dhd_prot_ring_write_complete(dhd, ctrl_ring, d2h_ring, 1);
...
dhd_send_h2d_ringcreate(dhd_pub_t *dhd, msgbuf_ring_t *ring_to_create, uint8 ring_type, uint32 id)
{
	...
	/* Request for ring buffer space */
	h2d_ring = (h2d_ring_create_req_t *)dhd_prot_alloc_ring_space(dhd, ctrl_ring, 1,
		&alloced, FALSE);
	...
	ring_to_create->create_req_id = id;
	ring_to_create->create_pending = TRUE;
	...
	h2d_ring->msg.flags = 0;
	h2d_ring->ring_id = htol16(DHD_RING_OFFSET(dhd->prot, ring_to_create->id));
	h2d_ring->ring_type = ring_type;
	h2d_ring->max_items = htol16(ring_to_create->max_items);
	h2d_ring->item_size = htol16(ring_to_create->item_size);
	h2d_ring->haddr64.low_addr = ring_to_create->base_addr.low_addr;
	h2d_ring->haddr64.high_addr = ring_to_create->base_addr.high_addr;
	...
	/* Update the flow_ring's WRITE index */
	dhd_prot_ring_write_complete(dhd, ctrl_ring, h2d_ring, 1);

	return BCME_OK;
}

static void
dhd_h2d_pcap_subn_params_init(msgbuf_ring_params_t *params)
{
	params->name = "h2dpcap";
	params->ringid = 0; /* ringid is ignored for dynamic ring */
	params->item_type = MSGBUF_WI_LEGACY;
	params->item_size = sizeof(pcap_post_awi64_t);
	params->max_items = H2DRING_PCAP_SUB_MAX_ITEM;
	params->item_misc = 3; /* 3-bit shift, since PCAP_AWI_AGGR == 8 */
	STATIC_ASSERT(PCAP_AWI_AGGR == 8);
	params->ring_dir = H2D;
	params->ring_type = DYNAMIC;
}

static void
dhd_d2h_pcap_cpln_params_init(msgbuf_ring_params_t *params)
{
	params->name = "d2hpcap";
	params->ringid = 0; /* ringid is ignored for dynamic ring */
	params->item_type = MSGBUF_WI_LEGACY;
	params->item_size = sizeof(pcap_cpl_awi64_t);
	params->max_items = D2HRING_PCAP_CPLN_MAX_ITEM;
	params->item_misc = 3; /* 3-bit shift, since PCAP_AWI_AGGR == 8 */
	STATIC_ASSERT(PCAP_AWI_AGGR == 8);
	params->ring_dir = D2H;
	params->ring_type = DYNAMIC;
...
static int
dhd_prot_create_pcap_rings(dhd_pub_t *dhd)
{
	dhd_prot_t *prot = dhd->prot;
	int err = BCME_OK;
	msgbuf_ring_params_t params;
	...
	if (prot->d2h_pcap_cpln) {
		/* re-entry/re-init */
		prot->d2h_pcap_cpln->inited = FALSE;
	}

	if (prot->h2d_pcap_subn && prot->d2h_pcap_cpln) {
		return BCME_OK; /* dhd_prot_init re-entry after dhd_prot_reset */
	}
	/* Note: there is a dependency on dhdpcie_bus_init_pcie_ipc_rings() ring_mem
	 * init, reserving ring id for H2D PCAP Subn followed by D2H PCAP Cpln.
	 */
	if (prot->h2d_pcap_subn == NULL) {
		ring = prot->h2d_pcap_subn = MALLOCZ(prot->osh, sizeof(msgbuf_ring_t));

		if (prot->h2d_pcap_subn == NULL) {
			DHD_ERROR(("%s: out of memory\n", __FUNCTION__));
			err = BCME_NOMEM;
			goto create_pcap_err;
		}

		DHD_INFO(("%s: creating pcap submit ring\n", __FUNCTION__));

		dhd_h2d_pcap_subn_params_init(&params);

		err = dhd_prot_ring_alloc(dhd, prot->h2d_pcap_subn, &params);

		if (err != BCME_OK) {
			DHD_ERROR(("wl%d: %s: failure to allocate PCAP Submit ring\n",
				dhd->unit, __FUNCTION__));
			goto create_pcap_err;
		}
		/* Confirm previously reserved ringid matches */
		if (ring->id != DHD_PCAP_H2D_RINGID(dhd)) {
			DHD_ERROR(("wl%d: %s: H2D PCAP ring id %u doesn't match reserved %u\n",
				dhd->unit, __FUNCTION__, ring->id, DHD_PCAP_H2D_RINGID(dhd)));
			goto create_pcap_err;
		}
		if (IDMA_ACTIVE(dhd)) {
			ring->idma_index = dhd_idma_flowmgr_ringid_2_index(
				dhd, TRUE /* H2D */, DHD_RING_OFFSET(prot, ring->id));
			DHD_ERROR(("%s: Ring \"%s\" ringid %d flowid %d idma_index %d\n",
				__FUNCTION__, ring->name, ring->id,
				DHD_RING_OFFSET(prot, ring->id), ring->idma_index));
		}
	}

	if (prot->d2h_pcap_cpln == NULL) {
		ring = prot->d2h_pcap_cpln = MALLOCZ(prot->osh, sizeof(msgbuf_ring_t));
		...
		DHD_INFO(("%s: creating pcap completion ring\n", __FUNCTION__));

		dhd_d2h_pcap_cpln_params_init(&params);

		err = dhd_prot_ring_alloc(dhd, prot->d2h_pcap_cpln, &params);

		if (err != BCME_OK) {
			DHD_ERROR(("wl%d: %s: failure to allocate PCAP Completion ring\n",
				dhd->unit, __FUNCTION__));
			goto create_pcap_err;
		}

		/* Confirm previously reserved ringid matches */
		if (ring->id != DHD_PCAP_D2H_RINGID(dhd)) {
			DHD_ERROR(("wl%d: %s: D2H PCAP ring id %u doesn't match reserved %u\n",
				dhd->unit, __FUNCTION__, ring->id, DHD_PCAP_D2H_RINGID(dhd)));
			goto create_pcap_err;
		}

		if (IDMA_ACTIVE(dhd)) {
			ring->idma_index = dhd_idma_flowmgr_ringid_2_index(
				dhd, FALSE /* D2H */, DHD_RING_OFFSET(prot, ring->id));
			DHD_ERROR(("%s: Ring \"%s\" ringid %d flowid %d idma_index %d\n",
				__FUNCTION__, ring->name, ring->id,
				DHD_RING_OFFSET(prot, ring->id), ring->idma_index));
		}
...
create_pcap_err:
	if (prot->h2d_pcap_subn) {
		MFREE(prot->osh, prot->h2d_pcap_subn, sizeof(msgbuf_ring_t));
		prot->h2d_pcap_subn = NULL;
	}
	if (prot->d2h_pcap_cpln) {
		MFREE(prot->osh, prot->d2h_pcap_cpln, sizeof(msgbuf_ring_t));
		prot->d2h_pcap_cpln = NULL;
	}
	return err;
}

int
dhd_prot_pcap_init(dhd_pub_t *dhd)
{
	int err;
	dhd_prot_t *prot = dhd->prot;

	if ((err = dhd_prot_create_pcap_rings(dhd)) != BCME_OK) {
		DHD_ERROR(("%s: error: pcap ring create fail\n", __FUNCTION__));
		return err;
	}

	if (prot->d2h_pcap_cpln->inited || prot->d2h_pcap_cpln->create_pending) {
		DHD_INFO(("%s: PCAP completion ring was created\n", __FUNCTION__));
		return BCME_OK;
	}

	DHD_PRINT(("Creating PCAP rings...\n"));
	prot->pcap_post.max = DHD_H2D_PCAP_MAX_BUF_POST;
	prot->pcap_post.count = 0;

	err = dhd_send_d2h_ringcreate(dhd, prot->d2h_pcap_cpln,
		BCMPCIE_D2H_RING_TYPE_PCAP_CPL, DHD_D2H_PCAP_REQ_PKTID);

	if (err != BCME_OK) {
		DHD_ERROR(("%s: error creating D2H PCAP ring\n", __FUNCTION__));
		return err;
	}

	prot->h2d_pcap_subn->seqnum = H2D_EPOCH_INIT_VAL;
	prot->d2h_pcap_cpln->seqnum = D2H_EPOCH_INIT_VAL;

	err = dhd_send_h2d_ringcreate(dhd, prot->h2d_pcap_subn,
		BCMPCIE_H2D_RING_TYPE_PCAP_SUBMIT, DHD_H2D_PCAP_REQ_PKTID);

	return err;
}

static void
dhd_prot_detach_pcap_rings(dhd_pub_t * dhd)
{
	dhd_prot_t *prot = dhd->prot;

	if (prot->h2d_pcap_subn) {
		dhd_prot_ring_free(dhd, prot->h2d_pcap_subn);
		MFREE(prot->osh, prot->h2d_pcap_subn, sizeof(msgbuf_ring_t));
		prot->h2d_pcap_subn = NULL;
	}
	if (prot->d2h_pcap_cpln) {
		dhd_prot_ring_free(dhd, prot->d2h_pcap_cpln);
		MFREE(prot->osh, prot->d2h_pcap_cpln, sizeof(msgbuf_ring_t));
		prot->d2h_pcap_cpln = NULL;
	}
}
...
static int
dhd_prot_pcap_post_alloc_space(dhd_pub_t *dhd, pcap_post_awi64_t **wi_start, uint16 *pkt_count,
	uint16 *avail_count)
{
	dhd_prot_t *prot = dhd->prot;
	msgbuf_ring_t *pcap_ring = prot->h2d_pcap_subn;
	unsigned long flags;
	int req_items = DHD_H2D_PCAP_BUF_BURST >> pcap_ring->item_misc;

	DHD_GENERAL_LOCK(dhd, flags);

	if (prot->pcap_post.inprogress) {
		DHD_GENERAL_UNLOCK(dhd, flags);
		return BCME_BUSY;
	}

	/* Avoid re-entrancy: dhd_prot_alloc_ring_space is not re-entrant */
	prot->pcap_post.inprogress = TRUE;

	*wi_start = (pcap_post_awi64_t *)dhd_prot_alloc_ring_space(dhd, pcap_ring,
			req_items, avail_count, TRUE);

	if (*wi_start == NULL) {
		prot->pcap_post.inprogress = FALSE; /* End re-entrancy */
		DHD_GENERAL_UNLOCK(dhd, flags);

		DHD_INFO(("%s: No PCAP Post Ring space\n", __FUNCTION__));
		return BCME_NORESOURCE;
	}
	...
	/* Number of pkts that can fit into avail_count number of items */
	/* misc is a bit-shift.  0 = 1 packet per WI. Currently, using 3 for 8x aggregation
	 * of posted buffers per WI.
	 */
	*pkt_count = *avail_count << pcap_ring->item_misc;

	/* audit number of work items available to store pkts */
	ASSERT((*avail_count > 0) && (*pkt_count <= DHD_H2D_PCAP_BUF_BURST));

	return BCME_OK;
}

static int
dhd_prot_pcap_post_prep_buffers(dhd_pub_t *dhd, dhd_rxp_pkt_t *pcap_pktpool, uint16 pkt_count)
{
	dhd_prot_t *prot = dhd->prot;
	msgbuf_ring_t *pcap_ring = prot->h2d_pcap_subn;
	unsigned long flags;
	...
	/* Reserve extra headroom for router builds */
	pkt_tot_len = PCAP_PKT_BUFSZ;

	for (i = 0, curpkt = &pcap_pktpool[0]; i < pkt_count; i++, curpkt++) {
		curpkt->ptr = PKTGET(dhd->osh, pkt_tot_len, FALSE);

		if (curpkt->ptr == NULL) {
			DHD_ERROR(("%s: PKTGET failed\n", __FUNCTION__));
			prot->pcap_post.pktgetfail++;
			return BCME_NORESOURCE;
		}
...
#ifdef DHD_PCAP_DBG
		/* Poison memory if debugging */
		memset(pkt_buf_va, PCAP_POISON_VALUE, pkt_buf_len);
#endif /* DHD_PCAP_DBG */

		/* PHY RXS PKT Data initialization - needed for 6726 but harmless on other
		 * chips.
		 */
		rxstatus_len = (uint32 *)(pkt_buf_va + D11_RXHDR_PHYRXPKT_WAR_PHYRXS_OFFSET);
		*rxstatus_len = 0;

		/* Prepare the Rx packet's data buffer for DMA  */
		curpkt->buf_pa =
			DMA_MAP(dhd->osh, pkt_buf_va, pkt_buf_len, DMA_RX, curpkt->ptr, 0);
		...
		curpkt->pktid =
			DHD_NATIVE_TO_PKTID(dhd, prot->pktid_map_handle,
				curpkt->ptr, curpkt->buf_pa, pkt_buf_len,
				DMA_RX, NULL, pcap_ring->dma_buf.secdma, PKTTYPE_PCAP_RX);
#ifdef DHD_PCIE_PKTID
		DHD_GENERAL_UNLOCK(dhd, flags);

		if (curpkt->pktid == DHD_PKTID_INVALID) {
			prot->pcap_post.pktidfail++;
			DHD_ERROR(("Rx PktId alloc failed\n")); /* resize pktid pool? */
			return BCME_ERROR;
		}
#endif /* DHD_PCIE_PKTID */
	...
	} /* for pkt_count < AUX_BUF_BURST */

	/* Allocated and prepared AUX_BUF_BURST number of packets */
	ASSERT(i == pkt_count);
...
static int
dhd_prot_pcap_post_buffers(dhd_pub_t *dhd, dhd_rxp_pkt_t *pcap_pktpool,
	pcap_post_awi64_t *wi_start, uint16 pkt_count, uint16 avail_count)
{
	dhd_prot_t *prot = dhd->prot;
	msgbuf_ring_t *pcap_ring = prot->h2d_pcap_subn;
	dhd_rxp_pkt_t *curpkt;
	int i, j;
	pcap_post_awi64_t *awi = wi_start;

	ASSERT(sizeof(pcap_post_awi64_t) == pcap_ring->item_size);

	for (i = 0, curpkt = &pcap_pktpool[0]; i < pkt_count; i += PCAP_AWI_AGGR, awi++)
	{
		awi->msg.msg_type = MSG_TYPE_PCAP_POST;
		awi->msg.epoch = pcap_ring->seqnum++ % H2D_EPOCH_MODULO;
		/* Aggregated work items - batch submit */
		for (j = 0; j < PCAP_AWI_AGGR; j++, curpkt++) {
			ASSERT(curpkt->ptr && (curpkt->pktid != DHD_PKTID_INVALID));
			awi->host_pktid[j] = htol32(curpkt->pktid);
			HADDR64_LO_SET_HTOL(awi->data_buf_haddr64[j],
				PHYSADDRLO(curpkt->buf_pa));
			HADDR64_HI_SET_HTOL(awi->data_buf_haddr64[j],
				PHYSADDRHI(curpkt->buf_pa));
		}
	}   /* for loop: i < pkt_count */
	...
	dhd_prot_ring_write_complete(dhd, pcap_ring, wi_start, avail_count);

	prot->pcap_post.inprogress = FALSE;

	/* re-post rxbuffer when intermittent running out of BPM buffer  */
	prot->pcap_post.fail = FALSE;

	return BCME_OK;
}

static int
dhd_prot_pcap_post(dhd_pub_t *dhd)
{
	unsigned long flags;
	dhd_prot_t *prot = dhd->prot;
	msgbuf_ring_t *pcap_ring = prot->h2d_pcap_subn;
	int i, err;
	dhd_rxp_pkt_t *curpkt;
	pcap_post_awi64_t *wi_start; /* start address for space allocated in RxPost ring */
	uint16 avail_count = 0, pkt_count = 0;

	/* DHD_H2D_PCAP_BUF_BURST = 32. pcap_pktpool is 640-Byte on stack */
	dhd_rxp_pkt_t pcap_pktpool[DHD_H2D_PCAP_BUF_BURST]; /* pkt pool, for posting */

	/* Stage 1. Speculatively allocate space in ring */
	err = dhd_prot_pcap_post_alloc_space(dhd, &wi_start, &pkt_count, &avail_count);

	if (err != BCME_OK) {
		return err;
	}

	/* Stage 2. Alloc and prepare DHD_H2D_PCAP_BUF_BURST number of pkts */
	err = dhd_prot_pcap_post_prep_buffers(dhd, pcap_pktpool, pkt_count);

	if (err != BCME_OK) {
		goto error_free_pktpool;
	}

	/* Stage 3. Bulk post prepared packets */
	/* Post the prepared pkts from the pktpool into the PCAP Subn Ring */
	err = dhd_prot_pcap_post_buffers(dhd, pcap_pktpool, wi_start,
		pkt_count, avail_count);
...
error_free_pktpool: /* Restore ring's WR index without needing to flush */
	if (pcap_ring->wr == 0) {
		pcap_ring->wr = pcap_ring->max_items - avail_count;
	} else {
		pcap_ring->wr -= avail_count;
	}
	...
	for (i = 0, curpkt = &pcap_pktpool[0]; i < pkt_count; i++, curpkt++)
	{
		...
		if (PHYSADDRISZERO(curpkt->buf_pa)) {
			...
			DHD_GENERAL_LOCK(dhd, flags);
			SECURE_DMA_UNMAP(dhd->osh, curpkt->buf_pa, pktlen, DMA_RX,
					0, DHD_DMAH_NULL, pcap_ring->dma_buf.secdma, 0);
			DHD_GENERAL_UNLOCK(dhd, flags);
#else
			DMA_UNMAP(dhd->osh, curpkt->buf_pa, pktlen, DMA_RX,
					0, DHD_DMAH_NULL);
	...
	/* re-post rxbuffer when intermittent running out of BPM buffer  */
	prot->pcap_post.fail = TRUE;

	return -1;
} /* dhd_prot_pcap_post */

static void
dhd_msgbuf_pcap_post(dhd_pub_t *dhd)
{
	dhd_prot_t *prot = dhd->prot;
	int16 budget;
	int posted_count;
	...
	/* Refresh the rxpost submission common ring's rd index */
	dhd_msgbuf_rxbuf_post_refresh(dhd, dhd->prot->h2d_pcap_subn);

	budget = prot->pcap_post.max - prot->pcap_post.count;
	ASSERT(budget >= 0);

	while (budget >= DHD_H2D_PCAP_BUF_BURST) {
		/* Post bursts of DHD_H2D_PCAP_BUF_BURST number of buffers at a time */
		posted_count = dhd_prot_pcap_post(dhd);
		if (posted_count > 0) {
			prot->pcap_post.count += (uint16)posted_count;
			budget = prot->pcap_post.max - prot->pcap_post.count;
		} else {
			break;
		}
...
bool
dhd_is_tx_fring(dhd_pub_t *dhd, uint16 ringid)
{
	dhd_prot_t *prot = dhd->prot;
	return DHD_IS_FLOWRING(prot, ringid);
}

bool
dhd_is_non_tx_fring(dhd_pub_t *dhd, uint16 ringid)
{
	dhd_prot_t *prot = dhd->prot;
	return (DHD_IS_DYNAMIC(prot, ringid) || DHD_IS_COMMON(prot, ringid));
}

bool
dhd_is_dyn_fring(dhd_pub_t *dhd, uint16 ringid)
{
	dhd_prot_t *prot = dhd->prot;
	return DHD_IS_DYNAMIC(prot, ringid);
}

int
dhd_max_tx_frings(dhd_pub_t *dhd, int max_h2d_rings)
{
	return ((max_h2d_rings) - BCMPCIE_H2D_COMMON_MSGRINGS -
			dhd_bus_max_h2d_dynamic_rings((dhd)->bus));
}

#ifdef BCM_PCAP
/** Following 3 functions assume pcap xfer has been successfully initialized */
static inline uint32 // Fetch dongle posted WR index
__dhd_pcapxf_pull_wr(dhd_pub_t *dhd, pcap_xfer_t *pcapxf)
{
	OSL_CACHE_INV((void*)(pcapxf->wr_ptr), sizeof(uint32));

	return *(pcapxf->wr_ptr);
} /* __dhd_pcapxf_pull_wr() */

static inline uint32 // Fetch rd index in preamble shared with dongle
__dhd_pcapxf_pull_rd(dhd_pub_t *dhd, pcap_xfer_t *pcapxf)
{
	OSL_CACHE_INV((void*)(pcapxf->rd_ptr), sizeof(uint32));

	return *(pcapxf->rd_ptr);
} /* __dhd_pcapxf_pull_wr() */

static inline void // Commit rd pointer and push dongle via preamble
__dhd_pcapxf_push_rd(dhd_pub_t *dhd, pcap_xfer_t *pcapxf)
{
	*(pcapxf->rd_ptr) = (uint32)pcapxf->ring.read;
	OSL_CACHE_FLUSH((void*)(pcapxf->rd_ptr), sizeof(uint32));
} /* __dhd_pcapxf_push_rd() */

int
dhd_pcapxf_hme_init(dhd_pub_t *dhd)
{
	dhd_prot_t   *prot = dhd->prot;
	pcapxf_ipc_hme_t *ipc_hme;
	dhd_linux_pcap_t *pcap = &dhd->pcap;
	pcap_xfer_t *pcapxf = &pcap->pcap_xfer;

	/* reset data structure */
	memset(pcapxf, 0, sizeof(pcap_xfer_t));
	ASSERT(prot->hme.user_dma_buf != (dhd_dma_buf_t*)NULL);
	ipc_hme = (pcapxf_ipc_hme_t *)
	          ((prot->hme.user_dma_buf + PCIE_IPC_HME_USER_PCAPXF)->va);
	printf(" %s: pcapxf %p user_dma_buf %p ipc_hme %p\n",
		__FUNCTION__, pcapxf, prot->hme.user_dma_buf, ipc_hme);

	if (ipc_hme == NULL) {
		DHD_ERROR(("%s: PCIe IPC MEMORY FAILURE: "
			"PCAPXF DMA buffer not allocated.\n", __FUNCTION__));
		return BCME_UNSUPPORTED;
	}
	pcapxf->ipc_hme = ipc_hme;
	pcapxf->wr_ptr  = &ipc_hme->header.write_idx.u32;
	pcapxf->rd_ptr  = &ipc_hme->header.read_idx.u32;
	pcapxf->table   = &ipc_hme->table[0];
	pcapxf->polling_ticks = PCAPXF_POLL_160MSEC_TICKS;
#ifdef PCAP_RXCPY_POLLING
	pcap->pcap_poll_mode = TRUE;
#endif /* PCAP_RXCPY_POLLING */
	return BCME_OK;
}
...
static void BCMFASTPATH
dhd_pcap_list_print(pcap_hdr_list_info_t *pcap_list, uint16 frm_rd, uint16 d11hdr_rd,
	uint16 phy_rd, int loop_cnt, char *head_str, const char *func_str)
{
	uint16 frm_wr, d11hdr_wr;
	pcap_frame_t *frm_list, *frm_elem;
	pcap_d11_hdr_t *d11hdr_list, *d11hdr_elem;
#ifdef BCM_PCAP_PHYSTS
	uint16 phy_wr;
	pcap_phy_hdr_t *phy_list, *phy_elem;
#endif /* BCM_PCAP_PHYSTS */

	frm_list = pcap_list->frm_list;
	frm_wr = pcap_list->frm_list_wr;
	frm_elem = &frm_list[frm_rd];

	d11hdr_list = pcap_list->d11_hdr_list;
	d11hdr_wr = pcap_list->d11hdr_list_wr;
	d11hdr_elem = &d11hdr_list[d11hdr_rd];

#ifdef BCM_PCAP_PHYSTS
	phy_list = pcap_list->phy_hdr_list;
	phy_wr = pcap_list->phy_list_wr;
	phy_elem = &phy_list[phy_rd];

	DHD_PKT_PCAP((" %s %s pktid 0x%x phy_seq 0x%x frame %p "
		" frd %u fwr %u mrd %u mwr %u prd %u pwr %u lpcnt %d\n", head_str, func_str,
		frm_elem->pktid, phy_elem->phy_seq, frm_elem->frm_ptr, frm_rd, frm_wr,
		d11hdr_rd, d11hdr_wr, phy_rd, phy_wr, loop_cnt));
#else
	DHD_PKT_PCAP((" %s %s pktid 0x%x frame %p "
		" frd %u fwr %u mrd %u mwr %u lpcnt %d\n", head_str, func_str,
		frm_elem->pktid, frm_elem->frm_ptr, frm_rd, frm_wr,
		d11hdr_rd, d11hdr_wr, loop_cnt));
#endif /* BCM_PCAP_PHYSTS */
}
#else
#define dhd_pcap_list_print(pcap_list, frm_rd, d11hdr_rd, phy_rd, loop_cnt, head_str, func_str)
#endif /* defined(BCMDBG) || defined(DHD_DEBUG) */

#ifdef BCM_PCAP_PHYSTS
/**
 *  This function is used to clean up the physts list elements that are out of sync
 *  with d11hdr list.
 */
static void
dhd_pcap_phy_list_cleanup(dhd_pub_t *dhd)
{
	bool found;
	pcap_hdr_list_info_t *pcap_list;
	pcap_frame_t *frm_elem;
	pcap_d11_hdr_t *d11hdr_list, *d11hdr_elem;
	pcap_phy_hdr_t *phy_list, *phy_elem;
	uint16 frm_rd, frm_wr, d11hdr_rd, d11hdr_wr, phy_rd, phy_wr; /* current indices */
	int loop_cnt;

	pcap_list = &dhd->pcap.pcap_list_info;
	d11hdr_list = pcap_list->d11_hdr_list;
	phy_list = pcap_list->phy_hdr_list;
	...
	phy_rd = pcap_list->phy_list_rd;
	while (!PCAP_LIST_EMPTY(phy_wr, phy_rd, PHYSTS_IDX_MSK)) {
		phy_elem = &phy_list[phy_rd];

		/* Check d11hdr list */
		found = FALSE;
		loop_cnt = 0;
		d11hdr_rd = pcap_list->d11hdr_list_rd;
		while (!PCAP_LIST_EMPTY(d11hdr_wr, d11hdr_rd, D11HDR_IDX_MSK)) {
			d11hdr_elem = &d11hdr_list[d11hdr_rd];
			if (d11hdr_elem->phy_seq == phy_elem->phy_seq) {
				found = TRUE;
				break;
			}
			loop_cnt++;
			PCAP_LIST_INC(d11hdr_rd, D11HDR_IDX_MSK);
		}

		/* Cleanup is done if a matching elem is found */
		if (found) {
			/* Found d11hdr physeq, update d11hdr index and discard
			 * the elements before it
			*/
			pcap_list->phy_list_rd = phy_rd;
			break;
		}

		/* No matching d11hdr physeq is found, discard the phy elem */
		PCAP_LIST_INC(phy_rd, PHYSTS_IDX_MSK);
		pcap_list->phy_list_rd = phy_rd;
	}

	/* if no more elem in the physts list, reset frame and d11hdr lists */
	if (PCAP_LIST_EMPTY(phy_wr, phy_rd, PHYSTS_IDX_MSK)) {
		frm_wr = pcap_list->frm_list_wr;
		frm_rd = pcap_list->frm_list_rd;
		while (!PCAP_LIST_EMPTY(frm_wr, frm_rd, D11HDR_IDX_MSK)) {
			frm_elem = &pcap_list->frm_list[frm_rd];
			if (frm_elem->frm_ptr != NULL) {
				PKTFREE(dhd->osh, frm_elem->frm_ptr, FALSE);
				frm_elem->frm_ptr = NULL;
			}
			PCAP_LIST_INC(frm_rd, D11HDR_IDX_MSK);
		}
		pcap_list->frm_list_rd = pcap_list->frm_list_wr;
		pcap_list->frm_list_rd = pcap_list->phy_list_wr;
		pcap_list->d11hdr_list_rd = pcap_list->d11hdr_list_wr;
	}

	dhd_pcap_list_print(pcap_list, pcap_list->frm_list_rd, pcap_list->d11hdr_list_rd,
		pcap_list->phy_list_rd, 0, " Cleanup phy", __func__);
}
#endif /* BCM_PCAP_PHYSTS */

/**
 *  This function is used to clean up the frame list elements that are out of sync
 *  with d11hdr list.
 */
static void
dhd_pcap_frm_list_cleanup(dhd_pub_t *dhd)
{
	bool found;
	pcap_hdr_list_info_t *pcap_list;
	pcap_frame_t *frm_list, *frm_elem;
	pcap_d11_hdr_t *d11hdr_list, *d11hdr_elem;
	uint16 frm_rd, frm_wr, d11hdr_rd, d11hdr_wr; /* current indices */
	int loop_cnt;
	...
	while (!PCAP_LIST_EMPTY(frm_wr, frm_rd, D11HDR_IDX_MSK)) {
		...
		while (!PCAP_LIST_EMPTY(d11hdr_wr, d11hdr_rd, D11HDR_IDX_MSK)) {
			d11hdr_elem = &d11hdr_list[d11hdr_rd];
			if (frm_elem->pktid == d11hdr_elem->pktid) {
				found = TRUE;
				break;
			}
			loop_cnt++;
		...
		/* Cleanup is done if a matching elem is found */
		if (found) {
			/* Found d11hdr pkid, update mpdu index and discard
			 * the elements before it.
			*/
			pcap_list->d11hdr_list_rd = d11hdr_rd;
			break;
		}

		/* No matching d11hdr pktid is found, discard the frame */
		if (frm_elem->frm_ptr != NULL) {
			PKTFREE(dhd->osh, frm_elem->frm_ptr, FALSE);
			frm_elem->frm_ptr = NULL;
		}
		PCAP_LIST_INC(frm_rd, D11HDR_IDX_MSK);
		pcap_list->frm_list_rd = frm_rd;
	}

	/* if no more elem in the frame list, reset d11hdr and phy lists */
	if (PCAP_LIST_EMPTY(frm_wr, frm_rd, D11HDR_IDX_MSK)) {
		pcap_list->d11hdr_list_rd = pcap_list->d11hdr_list_wr;
#ifdef BCM_PCAP_PHYSTS
		pcap_list->phy_list_rd = pcap_list->phy_list_wr;
#endif /* BCM_PCAP_PHYSTS */
	}

#ifdef BCM_PCAP_PHYSTS
	dhd_pcap_list_print(pcap_list, pcap_list->frm_list_rd, pcap_list->d11hdr_list_rd,
		pcap_list->phy_list_rd, 0, " Cleanup frm", __func__);

	dhd_pcap_phy_list_cleanup(dhd);
#else
	dhd_pcap_list_print(pcap_list, pcap_list->frm_list_rd, pcap_list->d11hdr_list_rd,
		0, 0, " Cleanup frm", __func__);
#endif /* BCM_PCAP_PHYSTS */
}

/**
 *   PCAP rx-copy packet creation
 *   There are three lists (arrays) that store data packet, 802.11 header and phy metadata
 *      - frames: skb buffers that contain MPDU data with multiple MSDUs, referenced by pktid
 *      - d11 headers: 802.11 MAC and metadata, referenced by pktid and physeq
 *      - PHY status: referenced by physeq
 *  The goal is to search d11hdr and phy lists, find a frame (skb buffers) with matching
 *  pktid and physeq, copy the corresponding d11 and PHY status into frame buffer
 *  to form an 802.11 frame that can be used for radiotap header conversion.
 */
static void
dhd_pcap_frm_merge(dhd_pub_t *dhd)
{
	bool found;
	pcap_hdr_list_info_t *pcap_list;
	pcap_frame_t *frm_list, *frm_elem;
...
#ifdef BCM_PCAP_PHYSTS
	pcap_phy_hdr_t *phy_list, *phy_elem;
	uint16 phy_rd, phy_wr; /* current indices */
#else
	uint16 phy_rd = 0; /* used for list_print */
#endif /* BCM_PCAP_PHYSTS */
	int loop_cnt;

	BCM_REFERENCE(phy_rd);
	pcap_list = &dhd->pcap.pcap_list_info;
...
start_over:
	while (!PCAP_LIST_EMPTY(frm_wr, frm_rd, D11HDR_IDX_MSK)) {
		...
		if (!found) {
			frm_elem->retries++;
			if (frm_elem->retries > 1) {
				/* Discard the frame */
				dhd_pcap_list_print(pcap_list, frm_rd, pcap_list->d11hdr_list_rd,
					phy_rd, loop_cnt, " DROP  frm ", __func__);
				dhd->pcap.rxstats.rxpkt_drop_hdr++;
				if (frm_elem->frm_ptr != NULL) {
					PKTFREE(dhd->osh, frm_elem->frm_ptr, FALSE);
					frm_elem->frm_ptr = NULL;
				}
			...
			dhd_pcap_list_print(pcap_list, frm_rd, pcap_list->d11hdr_list_rd,
				phy_rd, loop_cnt, " RTRY  frm ", __func__);
			dhd->pcap.rxstats.rxpkt_retry_hdr++;
			break;	/* wait for new element */
		}

		/* Found d11hdr pkid, update d11hdr index and discuss the elements before it */
		pcap_list->d11hdr_list_rd = d11hdr_rd;
		d11hdr_elem = &d11hdr_list[d11hdr_rd];
		...
		if (!found) {
			d11hdr_elem->retries ++;
			if (d11hdr_elem->retries > 1) {
				/* discard frame and d11hdr elem */
				dhd_pcap_list_print(pcap_list, frm_rd, d11hdr_rd,
					pcap_list->phy_list_rd, loop_cnt,  " DROP d11hdr ",
					__func__);
				dhd->pcap.rxstats.rxpkt_drop_phy++;
				if (frm_elem->frm_ptr != NULL) {
					PKTFREE(dhd->osh, frm_elem->frm_ptr, FALSE);
					frm_elem->frm_ptr = NULL;
				}
			...
			dhd->pcap.rxstats.rxpkt_retry_phy++;
			break;	/* no more search */
		}

		/* Found a match, forward frame, update index and discard the elements before it */
		pcap_list->phy_list_rd = phy_rd;
		phy_elem = &phy_list[phy_rd];
#endif /* BCM_PCAP_PHYSTS */
		dhd_pcap_list_print(pcap_list, frm_rd, d11hdr_rd, phy_rd, loop_cnt,
			" FWRD  frm ", __func__);

		dhd->pcap.rxstats.rxpkt_fwrd++;
		if (frm_elem->frm_ptr != NULL) {
#ifdef BCM_PCAP_PHYSTS
			dhd_pcap_to_rtap_rxcpy(dhd, frm_elem->frm_ptr,
				d11hdr_elem->mac_hdr, d11hdr_elem->machdr_len,
				d11hdr_elem->d11hdr, D11RXHDR_LEN,
				phy_elem->physts, phy_elem->physts_len);
#else
			dhd_pcap_to_rtap_rxcpy(dhd, frm_elem->frm_ptr,
				d11hdr_elem->mac_hdr, d11hdr_elem->machdr_len,
				d11hdr_elem->d11hdr, D11RXHDR_LEN, NULL, 0);
#endif /* BCM_PCAP_PHYSTS */

			frm_elem->frm_ptr = NULL;
		}

		/* Advance frame and d11hdr index */
		PCAP_LIST_INC(frm_rd, D11HDR_IDX_MSK);
		PCAP_LIST_INC(d11hdr_rd, D11HDR_IDX_MSK);
...
/* This function copies the contents of the received pcap messages from dongle to
 * corresponding lists, either d11hdr_list or phy_list. The d11hdr_list and phy_list
 * are used with frame_list for constructing pcap frames for Radiotap header
 * conversaion.
 */
static inline void
dhd_pcap_hdr_list_fill(dhd_pub_t *dhd, pcap_comm_t *msghdr, bcm_ring_t *pcapxf_ring, bool lock)
{
	pcap_hdr_list_info_t *hdr_list = &dhd->pcap.pcap_list_info;
	if (msghdr->hdr.hdr_msgtype == PCAP_MSGTYPE_MPDU) {
		pcap_mpdu_t *mpdu = (pcap_mpdu_t *) msghdr;
		uint8 *h;
		dhd->pcap.rxstats.hdr_rcvd++;
		if (PCAP_LIST_FULL(hdr_list->d11hdr_list_wr, hdr_list->d11hdr_list_rd,
			D11HDR_IDX_MSK))
		{
			if (lock) {
				DHD_LOCK(dhd);
			...
			if (PCAP_LIST_FULL(hdr_list->d11hdr_list_wr,
				hdr_list->d11hdr_list_rd, D11HDR_IDX_MSK)) {
				DHD_PKT_PCAP(("DHD %s No resource to store pcap mpdu header "
					"wr %u rd %u max %d xfring wr %u rd %u\n",
					__FUNCTION__, hdr_list->d11hdr_list_wr,
					hdr_list->d11hdr_list_rd, MAX_PKT_HDRS,
					pcapxf_ring->write, pcapxf_ring->read));
				dhd->pcap.rxstats.hdr_full++;
				return;
			}
		}

		memcpy_s(hdr_list->d11_hdr_list[hdr_list->d11hdr_list_wr].d11hdr, D11RXHDR_LEN,
			mpdu->d11hdr, D11RXHDR_LEN);
		memcpy_s(hdr_list->d11_hdr_list[hdr_list->d11hdr_list_wr].mac_hdr, MAX_MACHDR_LEN,
			mpdu->mac_hdr, MAX_MACHDR_LEN);
		hdr_list->d11_hdr_list[hdr_list->d11hdr_list_wr].pktid = msghdr->hdr.pkt_id;
		hdr_list->d11_hdr_list[hdr_list->d11hdr_list_wr].phy_seq = msghdr->hdr.physeq;
		hdr_list->d11_hdr_list[hdr_list->d11hdr_list_wr].pad_len = mpdu->pad_len;
		hdr_list->d11_hdr_list[hdr_list->d11hdr_list_wr].machdr_len = mpdu->d11hdr_len;
		hdr_list->d11_hdr_list[hdr_list->d11hdr_list_wr].retries = 0;

		h = (uint8 *) hdr_list->d11_hdr_list[hdr_list->d11hdr_list_wr].mac_hdr;
		DHD_PKT_PCAP(("DHD %s: msg %d len %d mwr %u mrd %u pktid 0x%08x physeq 0x%x "
			"mac_len %u pad %u\n",
			__FUNCTION__, msghdr->hdr.hdr_msgtype, msghdr->hdr.hdr_len,
			hdr_list->d11hdr_list_wr, hdr_list->d11hdr_list_rd,
			hdr_list->d11_hdr_list[hdr_list->d11hdr_list_wr].pktid,
			hdr_list->d11_hdr_list[hdr_list->d11hdr_list_wr].phy_seq,
			hdr_list->d11_hdr_list[hdr_list->d11hdr_list_wr].machdr_len,
			hdr_list->d11_hdr_list[hdr_list->d11hdr_list_wr].pad_len));

		PCAP_LIST_INC(hdr_list->d11hdr_list_wr, D11HDR_IDX_MSK);

#ifdef BCM_PCAP_PHYSTS
	} else if (msghdr->hdr.hdr_msgtype == PCAP_MSGTYPE_PHYSTS) {
		pcap_physts_t *pcap_physts = (pcap_physts_t *) msghdr;
		dhd->pcap.rxstats.phy_rcvd++;
		if (PCAP_LIST_FULL(hdr_list->phy_list_wr, hdr_list->phy_list_rd, PHYSTS_IDX_MSK))
		{
			if (lock) {
				DHD_LOCK(dhd);
				dhd_pcap_frm_list_cleanup(dhd);
			...
			if (PCAP_LIST_FULL(hdr_list->phy_list_wr,
				hdr_list->phy_list_rd, PHYSTS_IDX_MSK)) {
				DHD_PKT_PCAP(("DHD %s No resource to store pcap physts "
					"header wr %u rd %d max %d xfring wr %u rd %u\n",
					__FUNCTION__, hdr_list->phy_list_wr,
					hdr_list->phy_list_rd, MAX_PHYSTS_HDRS,
					pcapxf_ring->write, pcapxf_ring->read));
				dhd->pcap.rxstats.phy_full++;
				return;
			}
		}

		memcpy_s(hdr_list->phy_hdr_list[hdr_list->phy_list_wr].physts, PHYRXSTS_LEN,
			pcap_physts->phyrxsts, pcap_physts->physts_len);
		hdr_list->phy_hdr_list[hdr_list->phy_list_wr].phy_seq = msghdr->hdr.physeq;
		hdr_list->phy_hdr_list[hdr_list->phy_list_wr].physts_len = pcap_physts->physts_len;

		DHD_PKT_PCAP(("DHD %s: msg %d len %d pwr %u prd %u physeq 0x%x phylen %d\n",
			__FUNCTION__, msghdr->hdr.hdr_msgtype, msghdr->hdr.hdr_len,
			hdr_list->phy_list_wr, hdr_list->phy_list_rd,
			hdr_list->phy_hdr_list[hdr_list->phy_list_wr].phy_seq,
			pcap_physts->physts_len));

		PCAP_LIST_INC(hdr_list->phy_list_wr, PHYSTS_IDX_MSK);
#endif /* BCM_PCAP_PHYSTS */
	}
}

static inline void
dhd_pcapxf_msg_proc(dhd_pub_t *dhd_pub, uint8 *pcap_pkt, bcm_ring_t *pcapxf_ring, bool lock)
{
	pcap_hdrpkt_t *pkt;
	pcap_hdr_t *hdr;
	uint8 *p;
	...
	pkt = (pcap_hdrpkt_t *) pcap_pkt;
	hdr = (pcap_hdr_t *) pkt->pcap_data;
	p = (uint8 *)hdr;
	DHD_PKT_PCAP(("DHD %s: pkt %p num_frames %d len %d xfring wr %u rd %u\n",
		__FUNCTION__, pkt, pkt->num_frames, pkt->frame_len,
		pcapxf_ring->write, pcapxf_ring->read));

	if (pkt->frame_len > 0) {
		for (i = 0; i < pkt->num_frames; i++) {
			hdr = (pcap_hdr_t *)p;
			dhd_pcap_hdr_list_fill(dhd_pub, (pcap_msdu_t *)p, pcapxf_ring, lock);
			p += hdr->hdr_len;
		}
	}
}

/* watchdog routine is for unit-test and debug */
int
dhd_pcapxf_watchdog(dhd_pub_t *dhd)
{
	if (dhd->pcap.pcap_xfer.polling_ticks &&
		((dhd->tickcnt % dhd->pcap.pcap_xfer.polling_ticks)) == 0) {
		return dhd_pcapxf_frame_recv(dhd, TRUE);
	}
	return BCME_OK;
}

int
dhd_pcapxf_frame_recv(dhd_pub_t *dhd, bool lock)
{
	pcap_xfer_t *pcapxf = &dhd->pcap.pcap_xfer;
	pcapxf_ring_elem_t *elem;
	int elem_idx;
	pcapxf_header_t *header;

	if (pcapxf->ipc_hme == NULL) {
		return BCME_UNSUPPORTED;
	}

	header = &pcapxf->ipc_hme->header;

	/* Check whether dongle has completed the configuration of the pcap header */
	if (pcapxf->ipc_hme->header.table_daddr32 == 0U) {
		OSL_CACHE_INV(&(pcapxf->ipc_hme->header), sizeof(pcapxf_header_t));
		if (pcapxf->ipc_hme->header.table_daddr32 == 0U) {
			return BCME_OK;
		}
	}
	...
	/* Refresh the ring's write index */
	pcapxf->ring.write = (int)__dhd_pcapxf_pull_wr(dhd, pcapxf);

	/* Process pcapxf packets transferred to host memory */
	while ((elem_idx = bcm_ring_cons(&pcapxf->ring, PCAPXF_RING_ITEMS_MAX))
	            != BCM_RING_EMPTY)
	{
		elem = PCAPXF_TABLE_IDX2ELEM(pcapxf->table, elem_idx);
		OSL_CACHE_INV((void*)(elem), PCAPXF_RING_ITEM_SIZE);

		/*  Process pcapxf messages */
		dhd_pcapxf_msg_proc(dhd, (uint8 *)(elem->data), &pcapxf->ring, lock);

		pcapxf->num_recvd++;
	}

	__dhd_pcapxf_push_rd(dhd, pcapxf);

	if (DHD_PCAP_RXCPY_EN(dhd)) {
		if (lock) {
			/* Merge pcap header lists */
			DHD_LOCK(dhd);
			dhd_pcap_frm_merge(dhd);
			DHD_UNLOCK(dhd);
		} else {
			dhd_pcap_frm_merge(dhd);
		}
	...
	return BCME_OK;
}  /* dhd_pcapxf_frame_recv() */

/** Copy msdu frame with LLC header if needed, return data length copied */
static uint16
dhd_pcap_msdu_cpy(uint8 *dst, uint8 *src, uint16 data_len, int sdu_cnt)
{
	struct dot11_llc_snap_header *d11_llc;

	uint16 eth_type = ((struct ether_header*)src)->ether_type, ether_type;
	uint tot_len;
	uint  cpy_len = data_len - ETHER_HDR_LEN;

	ether_type = NTOH16(eth_type);
	if (ether_type > ETHER_MAX_DATA) {

		tot_len = 0;

		/* src contains ETHER_HDR_LEN + playload */
		if (sdu_cnt > 1) {
			memcpy(dst, src, ETHER_HDR_LEN);

			/* MSDU in A-MSDU, payload consists of ETHER_HDR_LEN +
			 * DOT11_LLC_SNAP_HDR_LEN + playload content +
			 * padding to 4-byte boundry
			 */
			tot_len = (data_len + DOT11_LLC_SNAP_HDR_LEN + 3) & 0xfffc;
			((struct ether_header*)dst)->ether_type =
				HTON16(data_len - ETHER_HDR_LEN + DOT11_LLC_SNAP_HDR_LEN);
			dst += ETHER_HDR_LEN;
		} else {
			/* MPDU, payload consists of DOT11_LLC_SNAP_HDR_LEN +
			 * playload content + padding to 4-byte boundry
			 */
			tot_len = (data_len + DOT11_LLC_SNAP_HDR_LEN -
					ETHER_HDR_LEN + 3) & 0xfffc;
		}

		d11_llc = (struct dot11_llc_snap_header *)dst;

		/* Add SNAP header */
		memcpy(dst, _llc_snap_hdr, SNAP_HDR_LEN);

		/* Set OUI[2] for Bridge Tunnel */
		if (IS_BRIDGE_TUNNEL(ether_type)) {
			d11_llc->oui[2] = BRIDGE_TUNNEL_OUI2;
		}
		dst += SNAP_HDR_LEN;

		/* Set type */
		d11_llc->type = eth_type;
		dst += sizeof(uint16);
	} else {
		tot_len = cpy_len;
	}

	/* Skip ethernet header and copy data contents */
	memcpy(dst, &src[ETHER_HDR_LEN], cpy_len);
	return (tot_len);
}
