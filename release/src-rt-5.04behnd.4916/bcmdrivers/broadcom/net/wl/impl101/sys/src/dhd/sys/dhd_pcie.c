 ...
 *
 * $Id: dhd_pcie.c 833666 2023-11-30 13:10:21Z $
 */
...
#define MEMBLOCK	(2048)	/* 2K block size used for accessing SYSMEM */
...
extern int cmwifi_select_srom(unsigned char * nv_path, int nv_path_sz, int unit);
extern int cmwifi_get_fw_path(unsigned char * fw_path);
#endif /* CMWIFI */

#include <mac_cpudbg.h>
...
#if defined(DHD_DEBUG)
uint btrace_enable = 0;
module_param(btrace_enable, int, 0660);
#endif /* DHD_DEBUG */

int dhd_dongle_memsize;
int dhd_dongle_ramsize;
static int dhdpcie_checkdied(dhd_bus_t *bus, char *data, uint size, bool *ext_abort);
#ifdef DHD_DEBUG
static int dhdpcie_btrace_init(dhd_bus_t *bus, osl_t *osh);
static void dhdpcie_btrace_deinit(dhd_bus_t *bus, osl_t *osh);
static int dhdpcie_btrace_process(dhd_bus_t *bus);
...
static int dhd_hme_buf_alloc_try(dhd_bus_t *bus, uint16 pages,
...
static int dhdpcie_bus_write_sysmem_tlvs(dhd_bus_t *bus);
static int dhdpcie_bus_write_vars(dhd_bus_t *bus);
static bool dhdpcie_bus_process_mailbox_intr(dhd_bus_t *bus, uint32 intstatus);
#if defined(DHD_DEBUG) && defined(HNDUCLS)
static bool hndpci_bus_ucls_read(dhd_bus_t *bus);
#endif /* DHD_DEBUG && HNDUCLS */
#if defined(BCM_HMO_EVENT)
static bool dhdpcie_bus_hmo_event_ring_read(dhd_bus_t *bus);
#endif /* DHD_DEBUG && HNDUCLS */
...
#endif
#if defined(BCMEMBEDIMAGE) && defined(BCM_ROUTER)
...
#endif /* defined(BCMEMBEDIMAGE) && defined (BCM_ROUTER) */
...
static void dhdpcie_hw_adjust(dhd_bus_t *bus);
...
enum {
	...
	IOV_MLOAMGRSTART,
	IOV_MLOICC_OUT_SIZE,
	IOV_MLOICC_OUT_START,
	IOV_CPUDBG_KCONSOLE,
	IOV_CPUDBG_FILELINES,
	IOV_HWA_FIFOCTX_MEMSIZE,
	IOV_HWA_FIFOCTX_MEMSTART,
	IOV_PCAP,
...
const bcm_iovar_t dhdpcie_iovars[] = {
	...
	{"si_enum_base", IOV_SIENUMBASE,	0,	0,	IOVT_UINT32,	0 },
	{"smac_memsize", IOV_SMACMEMSIZE,	0,	0,  IOVT_UINT32,	0 },
	{"smac_memstart", IOV_SMACMEMSTART,	0,	0,  IOVT_UINT32,	0 },
	{"mlo_memsize", IOV_MLOMEMSIZE,		0,	0,  IOVT_UINT32,	0 },
	{"mlo_memstart", IOV_MLOMEMSTART,	0,	0,  IOVT_UINT32,	0 },
	{"mac_sramsize", IOV_MACSRAMSIZE,	0,	0,  IOVT_UINT32,	0 },
	{"mac_sramstart", IOV_MACSRAMSTART,	0,	0,  IOVT_UINT32,	0 },
	{"mlo_amgrsize", IOV_MLOAMGRSIZE,	0,	0,  IOVT_UINT32,	0 },
	{"mlo_amgrstart", IOV_MLOAMGRSTART,	0,	0,  IOVT_UINT32,	0 },
	{"icc_out_size", IOV_MLOICC_OUT_SIZE,	0,	0,  IOVT_UINT32,	0 },
	{"icc_out_start", IOV_MLOICC_OUT_START,	0,	0,  IOVT_UINT32,	0 },
	{"cpudbg_kconsole", IOV_CPUDBG_KCONSOLE, 0,	0,  IOVT_UINT32,	0 },
	{"cpudbg_filelines", IOV_CPUDBG_FILELINES, 0,	0,  IOVT_UINT32,	0 },
	{"hwa_fifoctx_memsize", IOV_HWA_FIFOCTX_MEMSIZE,	0,	0,  IOVT_UINT32,	0 },
	{"hwa_fifoctx_memstart", IOV_HWA_FIFOCTX_MEMSTART,	0,	0,  IOVT_UINT32,	0 },
	{NULL, 0, 0, 0, 0, 0 }
};

/* This address may be different for future chips. */
#define HWA_AXI_FIFOCTX_BASE_ADDR		0x2852c000
#define HWA_FIFOCTX_SZ				32
...
dhd_bus_t* dhdpcie_bus_attach(osl_t *osh,
	...
	haddr64_t bar2_haddr64, void *pci_dev)
{
	dhd_bus_t *bus;
...
#ifdef PCIEG3_EP
#if defined(LINUX) || defined(linux)
		/* attach/configure gen3 link on radio */
		if (bus->sih && pcieg3_support_en &&
		    (BCM6717_CHIP(si_chipid(bus->sih)) || BCM6726_CHIP(si_chipid(bus->sih)))) {
			bus->gen3ep = (void *) pcieg3_ep_attach(bus->cl_devid,
					bus->osh, bus->sih);
			if (bus->gen3ep) {
				/* gen3 ep: link retraining to switch to gen3 mode */
				if (pcieg3_ep_enable_gen3_support(
						(pcieg3_ep_t *) bus->gen3ep) == BCME_OK) {
					pcieg3_ep_link_retrain(bus->dev);
				}
			}
		}
#endif /* LINUX || linux */
...
/**
 * Placeholder for actions to prepare the endpoint before enum space registers are scanned
 * - Force Reset backplane for 6717/6726b0
 * - Clear AER_CORR_ERR_STATUS if needed
 */
int
dhdpcie_prepare_pcie_ep(osl_t *osh, struct pci_dev *pdev, bool force_bp_reset)
{
	uint32 corr_err_stat, revid, venid;
	uint16 device = pdev->device;
	bool bp_reset = FALSE;

	venid = OSL_PCI_READ_CONFIG(osh, PCI_CFG_VID, sizeof(uint32));
	if ((venid & 0xFFFF) != VENDOR_BROADCOM)
		return BCME_NODEVICE;

#ifdef BCMQT
	force_bp_reset = FALSE;
#endif

	corr_err_stat = OSL_PCI_READ_CONFIG(osh, PCI_CORR_ERR_STATUS, sizeof(uint32));

	if (force_bp_reset &&
		(BCM6717_CHIP(device) || BCM6717_DEVICE(device) ||
		BCM6726_CHIP(device) || BCM6726_DEVICE(device))) {
		bp_reset = TRUE;
	}

	if (corr_err_stat || bp_reset) {

		revid = OSL_PCI_READ_CONFIG(osh, PCI_REVID, sizeof(uint32));

		if ((revid & PCI_REVISION_MASK) >= 16) {
			DHD_ERROR(("%s: Resettting backplane for device 0x%x\n",
				__FUNCTION__, device));
			OSL_PCI_WRITE_CONFIG(osh, PCI_SPROM_CONTROL,
				sizeof(uint32), SPROM_CFG_TO_SB_RST);

			DHD_ERROR(("Polling for SB Reset state (PCI_SPROM_CONTROL)\n"));
			SPINWAIT((OSL_PCI_READ_CONFIG(osh, PCI_SPROM_CONTROL,
				sizeof(uint32)) & SPROM_CFG_TO_SB_RST), 500);

			DHD_ERROR(("SB reset bit is de-asserted by HW. "
				"Wait for 2ms for Backplane RESET to be completed\n"));
			OSL_DELAY(2000);

			if (corr_err_stat) {
				DHD_ERROR(("%s: correctable error detected (0x%08x)\n",
...
dhdpcie_dongle_attach(dhd_bus_t *bus)
{
	...
	/* Set bar0 window to si_enum_base_pa */
	dhdpcie_bus_cfg_set_bar0_win(bus, si_enum_base_pa(devid));

	if (dhdpcie_prepare_pcie_ep(osh, bus->dev, FALSE) != BCME_OK) {
	 ...
	 * dongle to a sane state (on host soft-reboot / watchdog-reboot). Leave PMU
	 * state untouched upon reset.
	 */
	PMU_REG(bus->sih, pmucontrol, PCTL_RESETCTL_MASK, PCTL_RESETCTL_BP_ONLY);

	si_corereg(bus->sih, SI_CC_IDX, OFFSETOF(chipcregs_t, watchdog), ~0, 4);
	OSL_DELAY(300000);

	dhdpcie_hw_adjust(bus);
	...
	if ((si_setcore(bus->sih, ARM7S_CORE_ID, 0)) ||
	    ...
	    (si_setcore(bus->sih, ARMCA53_CORE_ID, 0))) {
	...
	if (si_setcore(bus->sih, SYSMEM_CORE_ID, 0)) {
		...
		switch (si_chipid(bus->sih)) {
			...
			CASE_BCM6715_CHIP:
				...
				break;
			CASE_BCM6717_CHIP:
				bus->dongle_ram_base = CA53_6717_RAM_BASE;
				bus->orig_ramsize = 0xA80000; /* 10.5MB */
				break;
			CASE_BCM6726_CHIP:
				bus->dongle_ram_base = CA7_6726_RAM_BASE;
				bus->orig_ramsize = 0x480000; /* 4.5MB */
				break;
...
#ifdef BCM_ROUTER
	if (BCM4365_CHIP(si_chipid(bus->sih))) {
	...
	/* Use 1024B MRRS value for best performance */
	if (BCM6717_CHIP(si_chipid(bus->sih)) || BCM6726_CHIP(si_chipid(bus->sih))) {
		OSL_PCIE_MRRS_CONFIG(osh, PCIECFGREG_DEVCONTROL,
...
dhdpcie_bus_watchdog_reset(dhd_bus_t *bus)
{
	...
	if (BCM4365_CHIP(chip_id) ||
	    BCM43684_CHIP(chip_id) ||
	    BCM6715_CHIP(chip_id) ||
	    BCM6717_CHIP(chip_id) ||
	    BCM6726_CHIP(chip_id)) {
...
dhdpcie_bus_remove_prep(dhd_bus_t *bus)
{
	...
	/*
	 * disable the dpc outside the perimeter lock as it may
	 * sleep when the tasklet is running and may be meddling
	 * with PCIe interrupt mask.
	 */
	dhd_dpc_disable(bus->dhd);
...
#if defined(DHD_BHM)
			/* Free Boot Host Memory if there is any */
			dhdpcie_bhm_mem_free(bus);
#endif /* DHD_BHM */

#if defined(HNDUCLS) && defined(DHD_DEBUG)
			if (bus->dhd->ucls_line) {
				MFREE(bus->dhd->osh, bus->dhd->ucls_line, CONSOLE_LINE_MAX);
				bus->dhd->ucls_line = NULL;
			}
#endif /* HNDUCLS || DHD_DEBUG */
...
#ifdef PCIEG3_EP
#if defined(LINUX) || defined(linux)
		/* free gen3 ep resources */
		if (bus->gen3ep) {
			pcieg3_ep_detach((pcieg3_ep_t *)bus->gen3ep);
			bus->gen3ep = NULL;
		}
...
dhd_bus_stop(struct dhd_bus *bus, bool enforce_mutex)
{
	...
	dhd_os_set_ioctl_resp_timeout(bus->dhd, IOCTL_DISABLE_TIMEOUT);
...
{
#ifdef DHD_DEBUG
	uint32 dhd_console_ms = dhd->dhd_console_ms;
	dhd_bus_t *bus;
	bus = dhd->bus;

#if defined(DHD_DEBUG)
	dhdpcie_btrace_process(bus);
#endif /* DHD_DEBUG */

#if defined(HNDUCLS_DOORBELL)
	/* Don't read the dongle console buf if UCLS is working */
	if (dhd->ucls_cap > UCLS_CAP_NONE) {
		if (dhd->dongle_trap_occured == TRUE) {
			DHD_LOCK(dhd);
			hndpci_bus_ucls_read(bus);
			dhdpcie_bus_readconsole(bus);
			DHD_UNLOCK(dhd);
		}
		dhd_console_ms = 0;
	}
#endif /* HNDUCLS_DOORBELL */
...
#if defined(HNDUCLS) && !defined(HNDUCLS_DOORBELL)
				if (bus->dhd->ucls_cap > UCLS_CAP_NONE) {
					hndpci_bus_ucls_read(bus);
					/* UCLS stops transferring after a TRAP, so read the
					 * dongle console buffer after a UCLS read if a trap
					 * happens.
					 * Note that a TRAP signal could happen in the previous
					 * ucls read, with a budget limit. The following ucls
					 * read is with no budget limit to ensure all UCLS log
					 * messages are printed out first.
					 */
...
#if defined(BCM_HMO_EVENT) && !defined(HMO_EVENT_DOORBELL)
	DHD_LOCK(dhd);
	if (dhd->busstate == DHD_BUS_DATA &&
	    dhd->hmo_event_up &&
	    !dhd->dongle_trap_occured) {
		dhdpcie_bus_hmo_event_ring_read(dhd->bus);
	}
	DHD_UNLOCK(dhd);
#endif /* BCM_HMO_EVENT */
...
#define CHIP_6717_CHIPREV_A0    0
#define CHIP_6717_CHIPREV_A1    1
#define CHIP_6726_CHIPREV_B0	1
#define CHIP_43741_CHIPREV_A0	0
#define CHIP_43741_CHIPREV_A1	1
#define CHIP_43721_CHIPREV_A0	0
#define CHIP_43721_CHIPREV_A1	1
#define CHIP_43740_CHIPREV_B0	1
#define CHIP_43720_CHIPREV_B0	1
...
#define CHIPID_NONE            -1
	...
	} static chip_reqfw_map_table [] =
	{
		...
		"bcm6715b0", "bcm6715b0-firmware.bin", "bcm6715b0.nvm"},
		{BCM6717_CHIP_ID, CHIP_6717_CHIPREV_A0,
		"bcm6717a0", "bcm6717a0-firmware.bin", "bcm6717a0.nvm"},
		{BCM6717_CHIP_ID, CHIP_6717_CHIPREV_A1,
		"bcm6717a1", "bcm6717a0-firmware.bin", "bcm6717a1.nvm"},
#if defined(DHD_EAP)
		{BCM6726_CHIP_ID, CHIP_6726_CHIPREV_B0,
		"bcm43740b0", "bcm43740b0-firmware.bin", "bcm43740b0.nvm"},
#else
		{BCM6726_CHIP_ID, CHIP_6726_CHIPREV_B0,
		"bcm6726b0", "bcm6726b0-firmware.bin", "bcm6726b0.nvm"},
#endif
#if defined(DHD_EAP)
		{BCM43741_CHIP_ID, CHIP_43741_CHIPREV_A0,
		"bcm43741a0", "bcm43741a0-firmware.bin", "bcm43741a0.nvm"},
		{BCM43741_CHIP_ID, CHIP_43741_CHIPREV_A1,
		"bcm43741a1", "bcm43741a0-firmware.bin", "bcm43741a1.nvm"},
		{BCM43740_CHIP_ID, CHIP_43740_CHIPREV_B0,
		"bcm43740b0", "bcm43740b0-firmware.bin", "bcm43740b0.nvm"},
#else
		{BCM43741_CHIP_ID, CHIP_43741_CHIPREV_A0,
		"bcm6717a0", "bcm6717a0-firmware.bin", "bcm6717a0.nvm"},
		...
		"bcm6726b0", "bcm6726b0-firmware.bin", "bcm6726b0.nvm"},
#endif
		{BCM43721_CHIP_ID, CHIP_43721_CHIPREV_A0,
		"bcm43721a0", "bcm43741a0-firmware.bin", "bcm43721a0.nvm"},
		{BCM43721_CHIP_ID, CHIP_43721_CHIPREV_A1,
		"bcm43721a1", "bcm43741a0-firmware.bin", "bcm43721a1.nvm"},
		{BCM43720_CHIP_ID, CHIP_43720_CHIPREV_B0,
		"bcm43720b0", "bcm43740b0-firmware.bin", "bcm43720b0.nvm"},
	...
	char ini_path[64] = "/vendor/firmware/broadcom/dhd/firmware/";      /* initial path */
#else
	...
	while (chipid != CHIPID_NONE) {
		...
		if ((chipid == si_chipid(bus->sih)) && (chiprev == bus->sih->chiprev)) {
			break;
		}
		p_reqfw_index++;
	}
	...
	if (chipid != CHIPID_NONE) {
		...
		DHD_ERROR(("\n\n\n\033[1m\033[34m"
			...
			"##################################################################"
			"\033[0m\n",
			__FUNCTION__, si_chipid(bus->sih), bus->sih->chiprev));
		return 0;
	}
#elif defined(CMWIFI)
	char nv_path[64];	       /* path to nvram vars file */
	...
	bus->nv_path = nv_path;
	if (cmwifi_select_srom(bus->nv_path, sizeof(nv_path), dhd_get_instance(bus->dhd)) == 0) {
		DHD_ERROR(("%s: For Radio #%d, using SROM var file from %s\n\n",
			__FUNCTION__, dhd_get_instance(bus->dhd), bus->nv_path));
	}
...
dhdpcie_hybridfw_dma_buf_alloc(struct dhd_bus *bus, uint32 len)
{
	...
	if (bhm_dma_buf->len != 0) {
		/* Carve the SWP memory from BHM. Fill pa, va and len in user_dma_buf
		 * with BHM details but don't touch OS allocated mem fields
		 */
		PHYSADDRLOSET(hybridfw_dma_buf->pa, PHYSADDRLO(bhm_dma_buf->pa));
		PHYSADDRHISET(hybridfw_dma_buf->pa, PHYSADDRHI(bhm_dma_buf->pa));
		hybridfw_dma_buf->va = (void *)(uintptr)bhm_dma_buf->va;
		/* Roundup bytes to HME pages */
		hybridfw_dma_buf->len = PCIE_IPC_HME_BYTES(PCIE_IPC_HME_PAGES(len));
	...
	if (hybridfw_dma_buf->va == NULL)
	{
		...
		/* No dedicated S2P user registered and
		 * generic S2P services doesn't need boundary limits.
		 */
		pcie_ipc_hybridfw_hme.bound_bits = 0; // PCIE_IPC_HYBRIDFW_BOUND_BITS;
		pcie_ipc_hybridfw_hme.flags      &= ~PCIE_IPC_HME_FLAG_BOUNDARY;
		...
		dhd_hme_buf_alloc_try(bus, pcie_ipc_hybridfw_hme.pages,
			hybridfw_dma_buf, &pcie_ipc_hybridfw_hme);
	}
...
/*
 * Get type and length of the next portion from hybrid fw binary. It assumes
 * the file pointer is at the start of portion header and does sanity check on
 * the length that highest bit is not set.
 */
static int
dhdpcie_hybridfw_get_next_block(void *fptr, struct hybridfw_section_hdr *hdr)
{
	...
	hdr->type = ltoh32(hdr->type);
	hdr->len = ltoh32(hdr->len);

	if ((int)hdr->len < 0)
		return BCME_BADLEN;

	DHD_ERROR(("%s Found section %u with length %u\n", __FUNCTION__, hdr->type, hdr->len));

	return BCME_OK;
}

/*
 * Loads the dongle image into dongle memory in chunks. It either reads until the end-of-file
 * or when number of bytes specified by fwsize has been downloaded into dongle memory.
 */
static int dhdpcie_download_dongle_section(struct dhd_bus *bus, uint8 *memblock, int len,
	int fwsize, void *fp)
{
	bool eof = (fwsize == -1);
	int offset = 0;

	if (fwsize >= 0 && fwsize < len) {
		return BCME_ERROR;
	}
	if (len < sizeof(bus->resetinstr)) {
		DHD_ERROR(("%s: firmware too small (len=%d)\n", __FUNCTION__, len));
		return BCME_ERROR;
	}

	/* check if CR4/CA7/CA53 */
	if (si_setcore(bus->sih, ARMCR4_CORE_ID, 0) ||
		si_setcore(bus->sih, ARMCA7_CORE_ID, 0) ||
		si_setcore(bus->sih, ARMCA53_CORE_ID, 0)) {
		bus->resetinstr = *(((uint32*)memblock));
	...
	do {
		dhdpcie_bus_membytes(bus, PCIE_SYSMEM_WRITE, offset, memblock, len);
		offset += len;
		fwsize = eof ? MEMBLOCK : fwsize - len;
		if (fwsize > 0)
			len = dhd_os_get_image_block(memblock, MIN(fwsize, MEMBLOCK), fp);
	} while (fwsize && len > 0);
...
/*
 * Reads the firmware file specified by pfw_path and processes it as hybrid firmware if
 * it starts with hybrid firmware magic number or as regular firmware file. If hybrid_only
 * is true it will not attempt to process it as regular firmware file.
 */
static int
dhdpcie_download_code_file(struct dhd_bus *bus, char *pfw_path, bool hybrid_only)
{
	int bcmerror = BCME_ERROR;
	void *fp;
	uint8 *alloc_ptr = NULL, *align_ptr;
	int len, imgsize;
	uint32 magic;
	dhd_dma_buf_t * hybridfw_dma_buf;

	fp = dhd_os_open_image(bus->dhd, pfw_path);
	if (!fp)
		goto done;

	imgsize = dhd_os_get_image_size(fp);
	if (imgsize < sizeof(magic)) {
		DHD_ERROR(("%s: invalid firmware image size\n", __FUNCTION__));
		goto done;
	}

	alloc_ptr = align_ptr = MALLOC(bus->dhd->osh, MEMBLOCK + DHD_SDALIGN);
	if (alloc_ptr == NULL) {
		DHD_ERROR(("%s: Failed to allocate memory %d bytes\n", __FUNCTION__, MEMBLOCK));
		bcmerror = BCME_NOMEM;
		goto done;
	}
	if ((uint32)(uintptr)alloc_ptr % DHD_SDALIGN)
		align_ptr += (DHD_SDALIGN - ((uint32)(uintptr)alloc_ptr % DHD_SDALIGN));

	len = dhd_os_get_image_block(align_ptr, sizeof(magic), fp);
	if (len != sizeof(magic))
		goto done;

	imgsize -= sizeof(magic);
	magic = ltoh32(*(uint32 *)align_ptr);
	if (magic != PCIE_IPC_HYBRIDFW_MAGICNUM) {
		/* bail out is hybrid-only was requested */
		if (hybrid_only) {
			DHD_ERROR(("%s: not a hybrid firmware file\n", __FUNCTION__));
			goto done;
		}

		DHD_ERROR(("%s: process regular firmware file\n", __FUNCTION__));

		/* regular firmware so fill up the buffer */
		len = dhd_os_get_image_block(align_ptr + sizeof(magic),
			MEMBLOCK - sizeof(magic), fp);
		if (len > 0) {
			len += sizeof(magic);
			bcmerror = dhdpcie_download_dongle_section(bus, align_ptr, len, -1, fp);
		}
		goto done;
	}
	DHD_ERROR(("%s: process hybrid firmware file\n", __FUNCTION__));
	do {
	...
	KERNEL_VERSION(5, 4, 0)))
...
#endif
		struct hybridfw_section_hdr hdr;

		bcmerror = dhdpcie_hybridfw_get_next_block(fp, &hdr);
		if (bcmerror != BCME_OK)
			break;
		...
		if (!hdr.len) {
			DHD_ERROR(("%s: skip empty section %d\n", __FUNCTION__, hdr.type));
			continue;
		}

		if (imgsize < hdr.len) {
			DHD_ERROR(("%s: section length %d exceeding image size %d\n", __FUNCTION__,
				hdr.len, imgsize));
		...
		case PCIE_IPC_HYBRIDFW_TYPE_DNGL:
			len = dhd_os_get_image_block(align_ptr, MIN(hdr.len, MEMBLOCK), fp);
			if (len <= 0) {
				DHD_ERROR(("%s: failed reading dongle section\n", __FUNCTION__));
				bcmerror = BCME_ERROR;
				goto done;
			}
			bcmerror = dhdpcie_download_dongle_section(bus, align_ptr,
				len, hdr.len, fp);
			break;

		case PCIE_IPC_HYBRIDFW_TYPE_HOST:
			bcmerror = dhdpcie_hybridfw_dma_buf_alloc(bus, hdr.len);
			if (bcmerror != BCME_OK) {
				break;
			}
			...
			if (dhd_os_get_image_block(hybridfw_dma_buf->va, hdr.len, fp) <= 0) {
				bcmerror = BCME_ERROR;
				break;
			}

			OSL_CACHE_FLUSH((void *)hybridfw_dma_buf->va, hdr.len);

			DHD_ERROR(("%s: hybridfw" DHD_DMA_BUF_FMT "\n", __FUNCTION__,
				DHD_DMA_BUF_VAL(bus->hybridfw_dma_buf)));
			break;
	...
	} while (bcmerror == BCME_OK && imgsize > 0);

done:
	if (alloc_ptr) {
		MFREE(bus->dhd->osh, alloc_ptr, MEMBLOCK + DHD_SDALIGN);
	}
	if (fp) {
...
#if defined(BCM_ROUTER) && (defined(BCMEMBEDIMAGE) || defined(EXTFDIMGPATH))
 ...
 * Chipset 6717
 */
#define CHIPID_6717		BCM6717_CHIP_ID
#define CHIP_6717_CHIPREV_A0	0
#define CHIP_6717_CHIPREV_A1	1
#define CHIP_6717_PKG_OPT	0
#define CHIPIDSTR_6717		"6717"

/*
 * Chipset 6726
 */
#define CHIPID_6726		BCM6726_CHIP_ID
#define CHIPID_6716		BCM6716_CHIP_ID
#define CHIP_6726_CHIPREV_B0	1
#define CHIP_6726_PKG_OPT	0
#define CHIPIDSTR_6726		"6726"

/*
 * Chipset 43741
 */
#define CHIPID_43741		BCM43741_CHIP_ID
#define CHIP_43741_CHIPREV_A0	0
#define CHIP_43741_CHIPREV_A1	1
#define CHIP_43741_PKG_OPT	0
#define CHIPIDSTR_43741		"43741"

/*
 * Chipset 43721
 */
#define CHIPID_43721		BCM43721_CHIP_ID
#define CHIP_43721_CHIPREV_A0	0
#define CHIP_43721_CHIPREV_A1	1
#define CHIP_43721_PKG_OPT	0
#define CHIPIDSTR_43721		"43721"

/*
 * Chipset 43740
 */
#define CHIPID_43740		BCM43740_CHIP_ID
#define CHIP_43740_CHIPREV_B0	1
#define CHIP_43740_PKG_OPT	0
#define CHIPIDSTR_43740		"43740"

/*
 * Chipset 43720
 */
#define CHIPID_43720		BCM43720_CHIP_ID
#define CHIP_43720_CHIPREV_B0	1
#define CHIP_43720_PKG_OPT	0
#define CHIPIDSTR_43720		"43720"
...
enum chip_image_rev
{
	...
	CHIP_6717_A0_CHIP_IMAGE,
	CHIP_6726_B0_CHIP_IMAGE,
#if defined(DHD_EAP)
	CHIP_43741_A0_CHIP_IMAGE,
	CHIP_43721_A0_CHIP_IMAGE,
	CHIP_43740_B0_CHIP_IMAGE,
	CHIP_43720_B0_CHIP_IMAGE,
#endif
...
#else
/*
 * non-EAP build (i.e., "core" build) support for chipset 43794b0
 */
	{CHIPID_43794, CHIP_6715_CHIPREV_B0, CHIP_6715_PKG_OPT, CHIP_6715_B0_CHIP_IMAGE,
 ...
 * Chipset 6717a0
 */
	{CHIPID_6717, CHIP_6717_CHIPREV_A0, CHIP_6717_PKG_OPT, CHIP_6717_A0_CHIP_IMAGE,
	CHIPIDSTR_6717, CHIPREVSTR_A0},
	{CHIPID_6717, CHIP_6717_CHIPREV_A1, CHIP_6717_PKG_OPT, CHIP_6717_A0_CHIP_IMAGE,
	CHIPIDSTR_6717, CHIPREVSTR_A0},
/*
 * Chipset 6726b0
 */
	{CHIPID_6726, CHIP_6726_CHIPREV_B0, CHIP_6726_PKG_OPT, CHIP_6726_B0_CHIP_IMAGE,
	CHIPIDSTR_6726, CHIPREVSTR_B0},
	{CHIPID_6716, CHIP_6726_CHIPREV_B0, CHIP_6726_PKG_OPT, CHIP_6726_B0_CHIP_IMAGE,
	CHIPIDSTR_6726, CHIPREVSTR_B0},
 ...
 * Chipset 43741
 */
	{BCM43741_CHIP_ID, CHIP_43741_CHIPREV_A0, CHIP_43741_PKG_OPT, CHIP_43741_A0_CHIP_IMAGE,
	CHIPIDSTR_43741, CHIPREVSTR_A0},
	{BCM43741_CHIP_ID, CHIP_43741_CHIPREV_A1, CHIP_43741_PKG_OPT, CHIP_43741_A0_CHIP_IMAGE,
	CHIPIDSTR_43741, CHIPREVSTR_A0},
/*
 * Chipset 43721
 */
	{BCM43721_CHIP_ID, CHIP_43721_CHIPREV_A0, CHIP_43721_PKG_OPT, CHIP_43741_A0_CHIP_IMAGE,
	CHIPIDSTR_43741, CHIPREVSTR_A0},
	{BCM43721_CHIP_ID, CHIP_43721_CHIPREV_A1, CHIP_43721_PKG_OPT, CHIP_43741_A0_CHIP_IMAGE,
	CHIPIDSTR_43741, CHIPREVSTR_A0},

/*
 * Chipset 43740b0
 */
	{BCM43740_CHIP_ID, CHIP_43740_CHIPREV_B0, CHIP_43740_PKG_OPT, CHIP_43740_B0_CHIP_IMAGE,
	CHIPIDSTR_43740, CHIPREVSTR_B0},
/*
 * Chipset 43720b0
 */
	{BCM43720_CHIP_ID, CHIP_43720_CHIPREV_B0, CHIP_43720_PKG_OPT, CHIP_43740_B0_CHIP_IMAGE,
	CHIPIDSTR_43740, CHIPREVSTR_B0},

#else
/*
 * non-EAP build (i.e., "core" build) support for chipset 43741a0
 */
	{CHIPID_43741, CHIP_6717_CHIPREV_A0, CHIP_6717_PKG_OPT, CHIP_6717_A0_CHIP_IMAGE,
	CHIPIDSTR_6717, CHIPREVSTR_A0},
	{CHIPID_43741, CHIP_6717_CHIPREV_A1, CHIP_6717_PKG_OPT, CHIP_6717_A0_CHIP_IMAGE,
	CHIPIDSTR_6717, CHIPREVSTR_A0},
	{CHIPID_43740, CHIP_6726_CHIPREV_B0, CHIP_6726_PKG_OPT, CHIP_6726_B0_CHIP_IMAGE,
	CHIPIDSTR_6726, CHIPREVSTR_B0},
...
#endif
#ifdef DLIMAGE_6717a0
	{dlarray_6717a0, sizeof(dlarray_6717a0), dlimagename_6717a0,
	dlimagever_6717a0, dlimagedate_6717a0},
#endif
#ifdef DLIMAGE_6726b0
	{dlarray_6726b0, sizeof(dlarray_6726b0), dlimagename_6726b0,
	dlimagever_6726b0, dlimagedate_6726b0},
#endif
#ifdef DLIMAGE_43741a0
	{dlarray_43741a0, sizeof(dlarray_43741a0), dlimagename_43741a0,
	dlimagever_43741a0, dlimagedate_43741a0},
#endif
	...
	if (image_index != -1) {
		...
		*p_dlarray     = 0;
		DHD_ERROR(("\n\n\n\033[1m\033[34m"
			...
			"##################################################################"
			"\033[0m\n",
			__FUNCTION__, si_chip(bus->sih), bus->sih->chiprev, bus->sih->chippkg));
	}
}
#endif /* BCMEMBEDIMAGE */
...
select_chipidverstr(struct dhd_bus *bus, char *idverstr)
{
	...
	if (chipid != CHIPID_NONE) {
		...
		DHD_ERROR(("\n\n\n\033[1m\033[34m"
			...
			"##################################################################"
			"\033[0m\n",
			__FUNCTION__, si_chipid(bus->sih), bus->sih->chiprev, bus->sih->chippkg));
 ...
 * to the path based on dongle's chip id, version and flavor
 *
 */
static int
concate_extfdimg_name(dhd_bus_t *bus, char *fw_path)
{
...
#ifdef CMWIFI
	if ((cmwifi_get_fw_path(fw_path)) != 0) {
		DHD_ERROR(("%s: cmwifi_get_fw_path failed\n", __FUNCTION__));
		return BCME_ERROR;
	}
#endif
	...
	if (osl_os_get_image_stat(fw_path, &stat)) {
	 ...
	 *
	 * <dir>/<id><ver>/<flavor>/rtecdc.bin
	 */
	if (select_chipidverstr(bus, chipidver) == BCME_OK) {
		int idx = 0, len;
		bool firmware_found = FALSE;
		char var[32];
		char mfg_prebuild[64];
		char *nv_flavor = NULL;
		char *flavor_str[] = { "",
			"release", "debug", "mfgtest", NULL};

		/* Add final slash if missing in fw_path */
		if (fw_path[strlen(fw_path)-1] != '/') {
			strcat(fw_path, "/");
		}

		/* Save root of fw_path */
		strlcpy(mfg_prebuild, fw_path, sizeof(mfg_prebuild));

		/* Append path with chipset name, eg 6715b0 */
		strcat(fw_path, chipidver);
		strcat(fw_path, "/");

		/* Append as first option the flavor specified in nvram as flavor0 */
		snprintf(var, sizeof(var), "flavor%d", bus->dhd->unit);
		nv_flavor = getvar(NULL, var);
		if (nv_flavor && (strlen(nv_flavor) < 30) && (strlen(nv_flavor) > 0) &&
			(memcmp(nv_flavor, "custom", strlen("custom")))) {
			/*
			 * If nvram flavor0 is found (eg debug), we will use it to pick up a
			 * embedded rtecdc.bin. If not found, dhd will fail and nic driver
			 * will be used instead (on purpose). "custom" is reserved and
			 * can't be used.
			 */
			strcat(fw_path, nv_flavor);
			strcat(fw_path, "/rtecdc.bin");

			if (!memcmp(nv_flavor, "mfgtest", strlen("mfgtest"))) {
				/* If mfgtest is requested, it could be either the (a) prebuild
				 * from /etc/wlan/dhd/mfg/6715b0/mfgtest/rtecdc.bin or
				 * (b) build from /etc/wlan/dhd/6715b0/mfgtest/rtecdc.bin
				 * By default, will pick up (b) if found
				 */
				if (osl_os_get_image_stat(fw_path, &stat)) {
					/* (b) was not found, so let's use (a) instead */
					strcat(mfg_prebuild, "mfg/");
					strcat(mfg_prebuild, chipidver);
					strcat(mfg_prebuild, "/mfgtest/rtecdc.bin");

					/* Switch the path to (a) */
					strcpy(fw_path, mfg_prebuild);
				}
			}
		} else {
			/*
			 * We will go though the list of possible embedded rtecdc.bin and
			 * will pick up the first one found on the filesystem
			 */

			/* Get the path information */
			while ((flavor_str[idx] != NULL) && (firmware_found == FALSE)) {
				strcat(fw_path, flavor_str[idx]);
				strcat(fw_path,	"/rtecdc.bin");

				if (osl_os_get_image_stat(fw_path, &stat)) {
					/* non-existant or invalid path */
					len = strlen("/rtecdc.bin") + strlen(flavor_str[idx]);
					fw_path[strlen(fw_path) - len] = '\0';

					/* Try next firmware from the list */
					idx++;
				} else {
					firmware_found = TRUE;
				}
...
#endif /* EXTFDIMGPATH */
#endif /* BCM_ROUTER && (BCMEMBEDIMAGE || EXTFDIMGPATH) */
...
_dhdpcie_download_firmware(struct dhd_bus *bus)
{
	...
	/* External image takes precedence if specified */
	if ((bus->fw_path != NULL) && (bus->fw_path[0] != '\0')) {
		if (dhdpcie_download_code_file(bus, bus->fw_path, FALSE)) {
...
dhdpcie_bus_readconsole(dhd_bus_t *bus)
{
...
#if defined(HNDUCLS)
		/* Sync the read pointer after a trap */
		if (bus->dhd->ucls_cap > UCLS_CAP_NONE) {
			c->last = ltoh32(c->log.out_idx);
		}
#endif
	}
	...
	DHD_INFO(("dhd%u: conlog: addr=0x%x, idx=0x%x, last=0x%x\n", bus->dhd->unit,
...
static bool dhdpcie_is_ext_abort(dhd_bus_t *bus, trap_t *tr)
{
	uint32 fs, fsr, tr_type = ltoh32(tr->type);

	/* external abort is always data abort */
	if (tr_type != TR_DAB) {
		return FALSE;
	}
	/* obtain fault status from register value */
	fsr = ltoh32(tr->abort_fsr);
	ASSERT(ARM_CP15_FSR_FORMAT_SHORT(fsr));

	fs = ARM_CP15_FSR_FAULT_STATUS(fsr);

	return (fs == ARM_CP15_S_FSTAT_SEA || fs == ARM_CP15_S_FSTAT_SEA_TLB1 ||
		fs == ARM_CP15_S_FSTAT_SEA_TLB2 || fs == ARM_CP15_S_FSTAT_AEA);
}

static int
dhdpcie_checkdied(dhd_bus_t *bus, char *data, uint size, bool *ext_abort)
{
	...
	if (data == NULL) {
		 ...
		 */
		size = msize;
		mbuffer = data = MALLOC(dhd->osh, msize);
		if (mbuffer == NULL) {
	...
	if ((str = MALLOC(dhd->osh, maxstrlen)) == NULL) {
	...
	if ((pcie_ipc->flags & (PCIE_IPC_FLAGS_ASSERT | PCIE_IPC_FLAGS_TRAP)) == 0) {
		...
		if (console_size > CONSOLE_BUFFER_MAX ||
		        !(console_buffer = MALLOC(dhd->osh, console_size))) {
			goto printbuf;
		}
		...
		for (i = 0, n = 0; i < console_size; i += n + 1) {
			...
			if (n > 0) {
				...
				if (dhd_msg_level & DHD_ERROR_VAL)
				printf(CONSOLE_MSG, dhd->unit, line);
			}
		}
	}
...
printbuf:
	...
	if (mbuffer)
		MFREE(dhd->osh, mbuffer, msize);
	if (str)
		MFREE(dhd->osh, str, maxstrlen);

	if (console_buffer)
		MFREE(dhd->osh, console_buffer, console_size);
...
void
dhdpcie_bus_read_sysmem(dhd_bus_t *bus, daddr32_t daddr32, uint8 *data, uint size)
{
	dhdpcie_bus_membytes(bus, PCIE_SYSMEM_READ, daddr32, data, size);
}

void
dhdpcie_bus_write_sysmem(dhd_bus_t *bus, daddr32_t daddr32, uint8 *data, uint size)
{
	dhdpcie_bus_membytes(bus, PCIE_SYSMEM_WRITE, daddr32, data, size);
}
	...
	if (DHD_FLOWRING_RNR_OFFL(flow_ring_node))
	{
		...
		while ((txp = dhd_flow_queue_dequeue(bus->dhd, queue)) != NULL) {
			...
			if (dhd_runner_notify(bus->dhd->runner_hlp, H2R_TXPOST_NOTIF,
				...
				if (DHD_RNR_OFFL_BKUPQ(bus->dhd)) {
					DHD_INFO(("%s: Drop %d\n", __FUNCTION__, ret));
					PKTFREE(bus->dhd->osh, txp, TRUE);
				} else {
...
dhd_bus_txdata(struct dhd_bus *bus, void *txp, uint8 ifidx)
{
	...
	int ret = BCME_OK;
	void *txp_pend = NULL;
#if ((defined(BCM_ROUTER) && defined(PKTC)) || (defined(CMWIFI) && defined(PKTC)))
	void *ntxp = NULL;
	uint8 prio = PKTPRIO(txp);
#endif /* (BCM_ROUTER && PKTC) || (CMWIFI && PKTC ) */
...
#else  /* !((BCM_ROUTER && PKTC) || (CMWIFI && PKTC)) */
	if ((ret = dhd_flow_queue_enqueue(bus->dhd, queue, txp)) != BCME_OK) {
		txp_pend = txp;
	}
#endif /* (BCM_ROUTER && PKTC) || (CMWIFI && PKTC) */
...
#if (defined(BCM_ROUTER) && defined(PKTC))
...
#else  /* !(BCM_ROUTER && PKTC) */
		if ((ret = dhd_flow_queue_enqueue(bus->dhd, queue, txp_pend)) != BCME_OK) {
			DHD_FLOWRING_UNLOCK(flow_ring_node->lock, flags);
			txp = txp_pend;
			goto toss;
		}
#endif /* BCM_ROUTER && PKTC */
...
dhdpcie_bus_read_u64(dhd_bus_t *bus, uint32 daddr32)
{
	...
	data = R_REG(bus->dhd->osh, (volatile uint32 *)(bus->tcm + daddr32));
	datah = R_REG(bus->dhd->osh, (volatile uint32 *)(bus->tcm + daddr32 + sizeof(uint32)));
	data |= (datah << 32);
#endif /* !CONFIG_64BIT */
#else
...
dhd_bus_cmn_writeshared(dhd_bus_t *bus, void *data, uint8 type, uint16 ringid)
{
	...
	switch (type) {
		...
		case BUZZZ_HOSTMEM_SEGMENTS:
			...
			daddr32  += OFFSETOF(bcm_buzzz_t, seg_haddr_u64);
			dhdpcie_bus_membytes(bus, PCIE_SYSMEM_WRITE, daddr32, (uint8*)data,
				BCM_BUZZZ_SEGMENTS * sizeof(uint64));
	...
	uint32 *core, uint32 *indent_p)
{
	...
	static const char * fmt[] = BCM_BUZZZ_FMT_STRINGS;
...
dhd_buzzz_dump_bus(dhd_bus_t *bus)
{
	...
	if (bus->dhd->bcm_buzzz_dma_buf[BCM_BUZZZ_SEGMENTS-1].va == NULL) return BCME_NOTUP;
	...
	log_p = (bcm_buzzz_log_t *)(bus->dhd->bcm_buzzz_dma_buf[0].va);
	...
	printf("BUZZZ Streaming[%s] FWID<0x%x,0x%x> IPC rev %u, chip 0x%x %u\n"
		...
		si_chipid(bus->sih), bus->sih->chiprev, log_p->arg1,
	...
	for (seg = 0; seg < BCM_BUZZZ_SEGMENTS; seg++) {
		log_p = (bcm_buzzz_log_t *)((bus->dhd->bcm_buzzz_dma_buf[seg].va));
		...
		memset(bus->dhd->bcm_buzzz_dma_buf[seg].va, 0, BCM_BUZZZ_HOSTMEM_SEGSZ);
		OSL_CACHE_FLUSH(bus->dhd->bcm_buzzz_dma_buf[seg].va, BCM_BUZZZ_HOSTMEM_SEGSZ);
...
dhd_buzzz_file_bus(dhd_bus_t *bus)
{
	...
	for (seg = 0; seg < BCM_BUZZZ_SEGMENTS; seg++)
	{
		buf = bus->dhd->bcm_buzzz_dma_buf[seg].va;
		printk("\t%u.\t0x%p\t%u\n", seg, buf, (uint32)pos);
...
static uint32
dhdpcie_rc_get_regval(struct pci_bus *root_bus, uint32 regoffset)
{
	uint32 regval = 0;

	pci_bus_read_config_dword(root_bus, 0, regoffset, &regval);
	return regval;
} /* dhdpcie_get_regval */
 ...
 */
static int
dhd_hme_buf_alloc_try(dhd_bus_t *bus, uint16 pages,
	dhd_dma_buf_t *user_dma_buf, pcie_ipc_hme_user_t * pcie_ipc_hme_user)
{
	...
	ASSERT((pcie_ipc_hme_user->bound_bits == 0) ||
		PCIE_IPC_HME_BYTES(pages) <= (1 << pcie_ipc_hme_user->bound_bits));
	...
	for (try = 0; try < try_max; ++try)
	{
		...
		if (dhd_dma_buf_alloc(bus->dhd, cur_dma_buf, pcie_ipc_hme_user->name,
		        PCIE_IPC_HME_BYTES(pages), pcie_ipc_hme_user->align_bits))
		{
			DHD_ERROR(("%s: PCIe IPC MEMORY FAILURE: "
				"HME User %s pages %u align %u try %u\n",
				__FUNCTION__, pcie_ipc_hme_user->name,
				pages, (1 << pcie_ipc_hme_user->align_bits), try));
			break;
		}
...
static void
dhd_hme_buf_free_sgmt(dhd_bus_t *bus, dhd_dma_buf_t *user_dma_buf,
	pcie_ipc_hme_user_t *pcie_ipc_hme_user)
{
	uint32 sgmt, sgmts;
	dhd_dma_buf_t *sgmt_dma_buf = NULL;

	ASSERT(pcie_ipc_hme_user->flags & PCIE_IPC_HME_FLAG_SGMT);
	ASSERT(pcie_ipc_hme_user->pages > PCIE_IPC_HME_NOBHM_PAGES_MAX);

	/* How many segments */
	sgmts = CEIL(pcie_ipc_hme_user->pages, PCIE_IPC_HME_NOBHM_PAGES_MAX);

	if (user_dma_buf->va != NULL) {
		sgmt_dma_buf = (dhd_dma_buf_t*)user_dma_buf->va;
		for (sgmt = 0; sgmt < sgmts; sgmt++) {
			sgmt_dma_buf = sgmt_dma_buf + sgmt;
			if (sgmt_dma_buf->va != (void*)NULL) {
				dhd_dma_buf_free(bus->dhd, sgmt_dma_buf,
					pcie_ipc_hme_user->name);
			}
		}

		/* The last sgmt save the table of HME haddr64 segments */
		sgmt_dma_buf = sgmt_dma_buf + sgmt;
		if (sgmt_dma_buf->va != NULL) {
			dhd_dma_buf_free(bus->dhd, sgmt_dma_buf, "hme_sgmt_haddr64");
		}
	}
}   /* dhd_hme_buf_free_sgmt() */

static int
dhd_hme_buf_alloc_sgmt(dhd_bus_t *bus, dhd_dma_buf_t *user_dma_buf,
	pcie_ipc_hme_user_t *pcie_ipc_hme_user)
{
	int ret;
	uint16 pages, total_pages;
	uint32 user_id, bytes;
	uint32 sgmt, sgmts;
	dhd_dma_buf_t *sgmt_dma_buf = NULL;
	dhd_dma_buf_t *sgmts_haddr64_dma_buf = NULL;
	pcie_ipc_hme_sgmt_info_t *hme_sgmt_info = NULL;
	pcie_ipc_hme_sgmt_t *hme_sgmt;
	dhd_pub_t *dhd; /* dhd is required for OSL_CACHE_FLUSH in STB builds */

	ASSERT(pcie_ipc_hme_user->flags & PCIE_IPC_HME_FLAG_SGMT);
	ASSERT(pcie_ipc_hme_user->pages > PCIE_IPC_HME_NOBHM_PAGES_MAX);

	user_id = pcie_ipc_hme_user->user_id;
	total_pages = pcie_ipc_hme_user->pages;
	dhd = bus->dhd;

	/* How many segments we need */
	sgmts = CEIL(total_pages, PCIE_IPC_HME_NOBHM_PAGES_MAX);

	/* Allocate an array of dhd_dma_buf_t */
	/* One more segment to save the table of HME haddr64 segments */
	bytes = (uint32)((sgmts+1) * sizeof(dhd_dma_buf_t));
	if (dhd_dma_buf_alloc(dhd, user_dma_buf,
		"hme_sgmt", bytes, DHD_MEM_ALIGN_BITS_MIN)) {
		DHD_ERROR(("%s: PCIe IPC PROTOCOL FAILURE: "
			"HME segment users %u bytes %u align %u dma_buf allocation\n",
			__FUNCTION__, user_id, bytes, DHD_MEM_ALIGNMENT_MIN));
		return BCME_NOMEM;
	}
	sgmt_dma_buf = (dhd_dma_buf_t*)user_dma_buf->va;

	/* Allocate a dhd_dma_buf_t to hold the table of HME haddr64 segments */
	sgmts_haddr64_dma_buf = sgmt_dma_buf + sgmts;
	bytes = (uint32)(sizeof(pcie_ipc_hme_sgmt_info_t) +
		(sgmts * sizeof(pcie_ipc_hme_sgmt_t)));
	if (dhd_dma_buf_alloc(dhd, sgmts_haddr64_dma_buf,
		"hme_sgmt_haddr64", bytes, DHD_MEM_ALIGN_BITS_MIN)) {
		DHD_ERROR(("%s: PCIe IPC PROTOCOL FAILURE: "
			"HME Haddr64 segment users %u bytes %u align %u dma_buf allocation\n",
			__FUNCTION__, user_id, bytes, DHD_MEM_ALIGNMENT_MIN));
		goto sgmt_init_failure;
	}

	hme_sgmt_info = (pcie_ipc_hme_sgmt_info_t*)sgmts_haddr64_dma_buf->va;
	hme_sgmt_info->magic = HTOL32(PCIE_IPC_HME_SGMT_MAGIC);
	hme_sgmt_info->size = bytes;
	hme_sgmt_info->avail = 0;
	/* Allocate HME segment for the user */
	for (sgmt = 0; sgmt < sgmts; sgmt++) {
		hme_sgmt = &hme_sgmt_info->sgmt[sgmt];

		if (total_pages > PCIE_IPC_HME_NOBHM_PAGES_MAX)
			pages = PCIE_IPC_HME_NOBHM_PAGES_MAX;
		else
			pages = total_pages;
		...
		ret = dhd_hme_buf_alloc_try(bus, pages, sgmt_dma_buf,
			pcie_ipc_hme_user);
		if (ret <= 0) { // BCME_NOMEM
			DHD_ERROR(("%s: PCIe IPC PROTOCOL FAILURE: "
				"HME segment user %u dma_buf allocation\n",
				__FUNCTION__, user_id));
			goto sgmt_init_failure;
		}

		/* HTOL copy Physical Address into table of HME haddr64 segments */
		hme_sgmt->bytes = PCIE_IPC_HME_BYTES(pages);
		HADDR64_LO_SET_HTOL(hme_sgmt->haddr64, PHYSADDRLO(sgmt_dma_buf->pa));
		HADDR64_HI_SET_HTOL(hme_sgmt->haddr64, PHYSADDRHI(sgmt_dma_buf->pa));
	...
	OSL_CACHE_FLUSH((void *)sgmts_haddr64_dma_buf->va, sgmts_haddr64_dma_buf->len);

	/* Save segment count to pcie_ipc_hme_user */
	pcie_ipc_hme_user->sgmt_avail = hme_sgmt_info->avail;
...
sgmt_init_failure:
	...
	if (user_dma_buf->va != NULL) {
		dhd_hme_buf_free_sgmt(bus, user_dma_buf, pcie_ipc_hme_user);
		dhd_dma_buf_free(dhd, user_dma_buf, "hme_sgmt");
	}
...
dhd_hme_buf_alloc(dhd_bus_t *bus, dhd_dma_buf_t *user_dma_buf, uint32 hme_user_id)
{
	...
	pcie_ipc_hme_user = DHD_PCIE_IPC_HME_USER(pcie_ipc_hme, hme_user_id,
	...
	/* Special handle for a large memory allocation which allow segmentation */
	if (bytes > PCIE_IPC_HME_NOBHM_BYTES_MAX &&
		pcie_ipc_hme_user->flags & PCIE_IPC_HME_FLAG_SGMT) {
		return dhd_hme_buf_alloc_sgmt(bus, user_dma_buf, pcie_ipc_hme_user);
	}

	/* Allocate HME compliant DMA-able buffer using HME User memory profile */
	return dhd_hme_buf_alloc_try(bus, pcie_ipc_hme_user->pages,
		user_dma_buf, pcie_ipc_hme_user);
} /* dhd_hme_buf_alloc() */

int
dhd_hme_buf_free(dhd_bus_t *bus, dhd_dma_buf_t *user_dma_buf, uint32 hme_user_id)
{
	pcie_ipc_hme_t      * pcie_ipc_hme      = bus->pcie_ipc_hme;
	pcie_ipc_hme_user_t * pcie_ipc_hme_user;
	...
	if (pcie_ipc_hme_user->flags & PCIE_IPC_HME_FLAG_BHM) {
		/* BHM allocated memory for this user. Nothing to free */
		memset(user_dma_buf, 0, sizeof(*user_dma_buf));
		return BCME_OK;
	}
#endif /* DHD_BHM */
	...
	/* Free segment part */
	if (pcie_ipc_hme_user->pages > PCIE_IPC_HME_NOBHM_PAGES_MAX &&
		pcie_ipc_hme_user->flags & PCIE_IPC_HME_FLAG_SGMT) {
		dhd_hme_buf_free_sgmt(bus, user_dma_buf, pcie_ipc_hme_user);
	}

	dhd_dma_buf_free(bus->dhd, user_dma_buf, "hme_user");
...
/**
 * +---------------------------------------------------------------------------+
 *  Boot Host Memory (BHM):
 *
 *  As we now need very large memories (several MBytes) for HME users, a runtime
 *  kmalloc with a default PAGESIZE allowing a maximum of 4MB allocation is not
 *  a viable solution.
 *
 ...
 *
 *  DHD will reserve the memory from CMA region at attach phase and pass the
 *  BHM buffer to dongle using a TLV (bhm_location_tlv_t).
 *  Default of BHM size is set to 16MB and can be override with an NVRAM
 *  variable dhdX_bhm_size (X - dhd instance).
 *
 *  HME is then extended to manage host memory from pre-allocated BOOT region
 *  as well as runtime dhd_dma_buf allocated.
 *
 *  NOTE:
 *  Before enabling BHM, one has to make sure there is enough memory
 *  reserved in CMA.
 *  CMA memory size can be configured using
 *	- uboot params
 *	  setenv bootargs_append "cma=128M"
 *	- Linux kernel configuration parameter CMA_SIZE_MBYTES
 *	  (linux/drivers/base/Kconfig).
 *	- Linux kernel command line option.
 *	  cma=xyM
 * +---------------------------------------------------------------------------+
 */

#if !defined(DHDPCIE_BHM_MEM_SIZE_OVERRIDE)
/* Define default BHM memory size (in MB) based on chip */
#define DHDPCIE_BHM_MEM_SIZE_AX		(0x400000)	// AX			:  4MB - 43684
#define DHDPCIE_BHM_MEM_SIZE_AX_PP	(0xE00000)	// AX + PktPgr		: 14MB - 6715
#define DHDPCIE_BHM_MEM_SIZE_BE		(0x1400000)	// BE			: 20MB - 6717
#define DHDPCIE_BHM_MEM_SIZE_BE_PUQ	(0x1A00000)	// BE + PUQ		: 26MB - 6716
#define DHDPCIE_BHM_MEM_SIZE_BE_PUQ_MAP	(0x2800000)	// BE + PUQ + MAP	: 40MB - 6716
#endif /* ! DHDPCIE_BHM_MEM_SIZE_OVERRIDE */

int
dhdpcie_bhm_mem_alloc(dhd_bus_t *bus)
{
	dhd_dma_buf_t	*dma_buf = &bus->bhm_mem_dma_buf;
	uint32		alignment, align_bits;
	uint32		bhm_size = 0;
	uint16		chipid;
...
#ifdef BCMQT
	bhm_size = DHDPCIE_BHM_MEM_SIZE_QT;
#else /* ! BCMQT */

#if defined(BCM_ROUTER)
	{ /* Check and set BHM size with NVRAM (dhdX_bhm_size) setting */
		char *var;
		char varstr[32];

		snprintf(varstr, sizeof(varstr), "dhd%d_bhm_size", bus->dhd->unit);
		if ((var = getvar(NULL, varstr)) != NULL) {
			bhm_size = (uint32)bcm_strtoul(var, NULL, 0);
			/* Convert MB to bytes */
			bhm_size = bhm_size * 1024 * 1024;
		}
	}
#endif /* BCM_ROUTER */

	/* If NVRAM is not configured then set BHM size based on chip */
	if (bhm_size == 0) {
		if (BCM43684_CHIP(chipid)) {
			bhm_size = DHDPCIE_BHM_MEM_SIZE_AX;
		} else if (BCM6715_CHIP(chipid)) {
			bhm_size = DHDPCIE_BHM_MEM_SIZE_AX_PP;
		} else if (BCM6717_CHIP(chipid)) {
			bhm_size = DHDPCIE_BHM_MEM_SIZE_BE;
		} else if (BCM6726_CHIP(chipid)) {
			bhm_size = DHDPCIE_BHM_MEM_SIZE_BE_PUQ;
#if defined(MLO_IPC)
			if (dhd_is_mlo_map(bus->dhd)) {
				bhm_size = DHDPCIE_BHM_MEM_SIZE_BE_PUQ_MAP;
			}
#endif /* MLO_IPC */
		} else {
			bhm_size = DHDPCIE_BHM_MEM_SIZE_BE_PUQ;
	...
	if (bhm_size == 0) {
		DHD_ERROR(("%s: PCIe IPC BHM (CMA) is not configured\n", __FUNCTION__));
		/* BHM is mandatory */
		ASSERT(bhm_size != 0);
		return BCME_NOMEM;
	}

	/* Align BHM buffer to a DHD page unit (4K) */
	align_bits = DHD_DMA_BUF_4KB_ALIGN_BITS;
	alignment = (1 << align_bits);

	/* Do allocation without alignment to avoid memory waste,
	 * expect linux return at 4KB alignment already
	 */
	dma_buf->_mem = (uintptr) DMA_ALLOC_CONTIGUOUS(bus->dhd->osh, bhm_size,
		0, &dma_buf->_alloced, &dma_buf->pa, &dma_buf->dmah);
	if (dma_buf->_mem == (uintptr)NULL) {
		DHD_ERROR(("%s: PCIe IPC BHM ALLOC FAILURE: size %u\n",
			__FUNCTION__, bhm_size));
		DHD_ERROR(("<<<<< WARNING: Please review CMA memory allocation >>>>>\n"));
		ASSERT(0);
		return BCME_NOMEM;
	}
	...
	if (ISALIGNED(PHYSADDRLO(dma_buf->pa), (1 << align_bits))) {
		dma_buf->len = bhm_size; /* not including alignment or padded bytes */
		dma_buf->va  = (void*)dma_buf->_mem;
		goto dma_buf_alloc_done;
	} else {
		/* Free it and do allocation with SW alignment */
		dhdpcie_bhm_mem_free(bus);
	}

	dma_buf->_mem = (uintptr) DMA_ALLOC_CONTIGUOUS(bus->dhd->osh, bhm_size + alignment,
	...
	dma_buf->len = bhm_size; /* not including alignment or pad bytes */
...
dma_buf_alloc_done:
	...
	/* memory is flushed and zeroed during allocation on cmwifi platforms */
#ifndef CMWIFI
	/* Zero out the entire memory and cache flush */
	memset((void*)dma_buf->_mem, 0, dma_buf->_alloced);
	OSL_CACHE_FLUSH((void *)dma_buf->_mem, dma_buf->_alloced);
#endif /* CMWIFI */

	DHD_ERROR(("%s: PCIe IPC BHM ALLOC SUCCESS: size %dMB "DHD_DMA_BUF_FMT"\n",
		__FUNCTION__, (bhm_size >> 20), DHD_DMA_BUF_VAL(*dma_buf)));

	return BCME_OK;

} /* dhdpcie_bhm_mem_alloc() */

void
dhdpcie_bhm_mem_free(dhd_bus_t *bus)
{
	dhd_dma_buf_t	*dma_buf = &bus->bhm_mem_dma_buf;

	if (dma_buf->_mem != (uintptr)NULL) {
		DHD_INFO(("%s: PCIe IPC BHM FREE: "DHD_DMA_BUF_FMT"\n",
			__FUNCTION__, DHD_DMA_BUF_VAL(*dma_buf)));

		DMA_FREE_CONTIGUOUS(bus->dhd->osh, (void*)dma_buf->_mem,
			dma_buf->_alloced, dma_buf->pa, dma_buf->dmah);
	}

	memset(dma_buf, 0, sizeof(*dma_buf));

} /* dhdpcie_bhm_mem_free() */
#endif /* DHD_BHM */

static bool
dhd_is_puq_cap(dhd_pub_t *dhdp)
{
	char *token = NULL;
	...
	/* check if fw cap has puq */
	token = bcmstrstr(dhdp->wlcore->fw_capabilities, "puq");
	if (token) {
		return TRUE;
	}

exit:
	DHD_ERROR(("%s: PUQ isn't enabled\n", __FUNCTION__));
	return FALSE;
}
                ...
                void *params, int plen, void *arg, int len, int val_size)
{
	...
	case IOV_GVAL(IOV_SBREG):
	{
		...
		addr = sdreg.offset | SI_ENUM_BASE_PA(bus->sih);
		size = sdreg.func;
	...
	case IOV_SVAL(IOV_HALT_DONGLE): /* halts ARM and PSM(s) for debug inspection */
	{
		...
		} else if (si_setcore(bus->sih, ARMCA53_CORE_ID, 0)) {
			cpu_type = ARMCA53_CORE_ID;
		} else if (si_setcore(bus->sih, ARMCR4_CORE_ID, 0)) {
	...
	case IOV_SVAL(IOV_GDB_SERVER):
		...
		DHD_UNLOCK(bus->dhd);
		if (int_val > 0) {
			debugger_init((void *) bus, &bus_ops, int_val, SI_ENUM_BASE_PA(bus->sih));
		} else {
			debugger_close();
		}
		DHD_LOCK(bus->dhd);
	...
	case IOV_GVAL(IOV_BUZZZ_FILE):
		DHD_UNLOCK(bus->dhd);
		/* Unsupported case */
		if (bus->dhd->buzzz_event_state != BCM_BUZZZ_STOP_COMMAND) {
			bcmerror = BCME_NOTREADY;
		} else {
			/* Supported two different cases (Extension and Non-Extension mode) */
			if (bus->dhd->buzzz_streaming_ext == TRUE) {
				const char *fname = BCM_BUZZZ_STREAMING_FILE;
				printk(" STREAMING EXT mode: already Created %s \n", fname);
				printk("\tlabPC]$ nc -l 80 > %s\n", fname);
				printk("\t# cat %s | /usr/bin/nc <lab.pc.ip.addr> 80\n\n", fname);
			} else {
	...
	case IOV_SVAL(IOV_HYBRIDFW):
		bcmerror = dhdpcie_download_code_file(bus, params, TRUE);
		break;

	case IOV_GVAL(IOV_SIENUMBASE):
		int_val = (int32)SI_ENUM_BASE_PA(bus->sih);
		bcopy(&int_val, arg, val_size);
		break;

	case IOV_GVAL(IOV_SMACMEMSIZE):
		if (dhd_macdbg_corerev(bus->dhd) == 133) {
			int_val = SMAC_DATA_SIZE_REV133;
		} else {
			int_val = SMAC_DATA_SIZE;
		}
		bcopy(&int_val, arg, val_size);
		break;

	case IOV_GVAL(IOV_SMACMEMSTART):
		if (dhd_macdbg_corerev(bus->dhd) >= 135 && dhd_macdbg_corerev(bus->dhd) != 138) {
			int_val = CM_CLUSTER_HADDR_BASE_REV135 | SMAC_DATA_SEL;
		} else {
			int_val = CM_CLUSTER_HADDR_BASE | SMAC_DATA_SEL;
		}
		bcopy(&int_val, arg, val_size);
		break;

	case IOV_GVAL(IOV_MLOMEMSIZE):
		int_val = MLO_DATA_SIZE;
		bcopy(&int_val, arg, val_size);
		break;

	case IOV_GVAL(IOV_MLOMEMSTART):
		if (dhd_macdbg_corerev(bus->dhd) >= 135 && dhd_macdbg_corerev(bus->dhd) != 138) {
			int_val = CM_CLUSTER_HADDR_BASE_REV135 | MLO_DATA_SEL;
		} else {
			int_val = CM_CLUSTER_HADDR_BASE | MLO_DATA_SEL;
		}
		bcopy(&int_val, arg, val_size);
		break;

	case IOV_GVAL(IOV_MACSRAMSIZE):
		int_val = CM_SRAM_SIZE;
		bcopy(&int_val, arg, val_size);
		break;

	case IOV_GVAL(IOV_MACSRAMSTART):
		if (dhd_macdbg_corerev(bus->dhd) >= 135 && dhd_macdbg_corerev(bus->dhd) != 138) {
			int_val = CM_CLUSTER_HADDR_BASE_REV135 | CM_SHMEM_REGS_OFFST;
		} else {
			int_val = CM_CLUSTER_HADDR_BASE | CM_SHMEM_REGS_OFFST;
		}
		bcopy(&int_val, arg, val_size);
		break;

	case IOV_GVAL(IOV_MLOAMGRSIZE):
		int_val = CM_MLOAMGR_SIZE;
		bcopy(&int_val, arg, val_size);
		break;

	case IOV_GVAL(IOV_MLOAMGRSTART):
		if (dhd_macdbg_corerev(bus->dhd) >= 135 && dhd_macdbg_corerev(bus->dhd) != 138) {
			int_val = CM_CLUSTER_HADDR_BASE_REV135 | CM_MLO_AMGR_REGS_OFFSET;
		} else {
			int_val = CM_CLUSTER_HADDR_BASE | CM_MLO_AMGR_REGS_OFFSET;
		}
		bcopy(&int_val, arg, val_size);
		break;

	case IOV_GVAL(IOV_MLOICC_OUT_SIZE):
		int_val = CM_MLOICC_OUT_SIZE;
		bcopy(&int_val, arg, val_size);
		break;

	case IOV_GVAL(IOV_MLOICC_OUT_START):
		if (dhd_macdbg_corerev(bus->dhd) >= 135 && dhd_macdbg_corerev(bus->dhd) != 138) {
			int_val = CM_CLUSTER_HADDR_BASE_REV135 | CM_ICC_OUT_REGS_OFFSET;
		} else {
			int_val = CM_CLUSTER_HADDR_BASE | CM_ICC_OUT_REGS_OFFSET;
		}
		bcopy(&int_val, arg, val_size);
		break;

	case IOV_GVAL(IOV_CPUDBG_KCONSOLE):
		bcmerror = dhd_cpudbg_kconsole_get((uint32*)&int_val);
		bcopy(&int_val, arg, val_size);
		break;

	case IOV_SVAL(IOV_CPUDBG_KCONSOLE):
		bcmerror = dhd_cpudbg_kconsole_set((uint32)int_val);
		break;

	case IOV_GVAL(IOV_CPUDBG_FILELINES):
		bcmerror = dhd_cpudbg_filelines_get((uint32*)&int_val);
		bcopy(&int_val, arg, val_size);
		break;

	case IOV_SVAL(IOV_CPUDBG_FILELINES):
		bcmerror = dhd_cpudbg_filelines_set((uint32)int_val);
		break;

	case IOV_GVAL(IOV_HWA_FIFOCTX_MEMSIZE):
		{
		uint32 corerev = dhd_macdbg_corerev(bus->dhd);
		if (corerev >= 133) {
			uint32 num_fifos;
			if (dhd_is_puq_cap(bus->dhd)) {
				num_fifos = dhd_macdbg_puq_tx_fifos(corerev);
			} else {
				num_fifos = dhd_macdbg_bmc_tpl_idx(corerev);
			}
			/* sizeof(hwa_txfifo_fifoctx_t) * num_fifos roundup to multiple of 1024 */
			int_val = ((HWA_FIFOCTX_SZ * num_fifos) + 1023) & ~(0x3FF);
			bcopy(&int_val, arg, val_size);
		} else {
			bcmerror = BCME_UNSUPPORTED;
		}
	...
	case IOV_GVAL(IOV_HWA_FIFOCTX_MEMSTART):
		if (dhd_macdbg_corerev(bus->dhd) >= 133) {
			int_val = HWA_AXI_FIFOCTX_BASE_ADDR;
			bcopy(&int_val, arg, val_size);
		} else {
			bcmerror = BCME_UNSUPPORTED;
		}
...
dhdpcie_bus_suspend(struct dhd_bus *bus, bool state)
{
	...
	if (state) {
		...
		dhd_os_set_ioctl_resp_timeout(bus->dhd, DEFAULT_IOCTL_RESP_TIMEOUT);
		dhdpcie_send_mb_data(bus, PCIE_IPC_H2DMB_HOST_D3_INFORM);
		timeleft = dhd_os_d3ack_wait(bus->dhd, &bus->wait_for_d3_ack);
		dhd_os_set_ioctl_resp_timeout(bus->dhd, IOCTL_RESP_TIMEOUT);
...
dhdpcie_bus_download_state(dhd_bus_t *bus, bool enter)
{
	...
	if (enter) {
		...
		if (si_setcore(bus->sih, ARMCA53_CORE_ID, 0)) {
			/* Halt ARM & remove reset */
			si_core_reset(bus->sih, SICF_ARMCA53_CPUHALT, SICF_ARMCA53_CPUHALT);
			if (!(si_setcore(bus->sih, SYSMEM_CORE_ID, 0))) {
		...
		} else if (si_setcore(bus->sih, ARMCA7_CORE_ID, 0)) {
			uint armca7_corerev = si_corerev(bus->sih);

			if (bus->sih->chip == BCM6715_CHIP_ID) {
				si_pmu_6715_prevent_on_the_fly_arm_vco_change(bus->sih);
			}

			/* Halt ARM & remove reset */
			if (armca7_corerev >= 8) {
				si_core_reset(bus->sih, SICF_CPUHALT_CPU0, SICF_CPUHALT_CPU0);
			} else {
				si_core_reset(bus->sih, SICF_CPUHALT, SICF_CPUHALT);
			}
			if (!(si_setcore(bus->sih, SYSMEM_CORE_ID, 0))) {
			...
			*/
			/* Halt ARM & remove reset */
			si_core_reset(bus->sih, SICF_CPUHALT, SICF_CPUHALT);
			if (BCM43602_CHIP(si_chipid(bus->sih))) {
		...
		if (si_setcore(bus->sih, ARMCA53_CORE_ID, 0)) {
			...
			/* write SysMem TLV's */
			if ((bcmerror = dhdpcie_bus_write_sysmem_tlvs(bus))) {
				DHD_ERROR(("%s: could not write TLV's to RAM\n", __FUNCTION__));
				goto fail;
			}

			/* switch back to arm core again */
			if (!(si_setcore(bus->sih, ARMCA53_CORE_ID, 0))) {
				DHD_ERROR(("%s: Failed to find ARM CA53 core!\n", __FUNCTION__));
				bcmerror = BCME_ERROR;
				goto fail;
			}
			...
			bits = SICF_ARMCA53_CPU1HALT;
			resetbits = SICF_ARMCA53_CPU1HALT;
		...
		si_core_reset(bus->sih, bits, resetbits);
...
dhdpcie_bus_write_sysmem_tlvs(dhd_bus_t *bus)
{
	if (bus->hybridfw_dma_buf.va) {
		...
		host_location.tlv_size		= htol32(sizeof(host_location) -
			sizeof(host_location.tlv_size) - sizeof(host_location.tlv_signature));
		host_location.tlv_signature	= htol32(BCM_HOST_PAGE_LOCATION_SIGNATURE);
		...
		DHD_ERROR(("%s: HYBRIDFW Host page location info: 0x%08x%08x binary_size:%u\n",
			__FUNCTION__, host_location.addr_hi, host_location.addr_lo,
			host_location.binary_size));
	}

#if defined(MLO_IPC)
	if (bus->dhd->dhd_mlo_ipc != NULL) {
		pcie_ipc_mlo_t		* pcie_ipc_mlo;
		pcie_ipc_mlo_tlv_t	pcie_ipc_mlo_tlv; // Declared on stack
		uint8	ap_unit;

		ap_unit		= bus->dhd->unit;
		pcie_ipc_mlo	= mlo_ipc_sys_gp->pcie_ipc_mlo;
#ifdef CMWIFI_33940
		/* CMWIFI: PCIe DMA view of phys memory is not same as CPU view of phys memory */
		if (bus->dev) {
			dma_addr_t ipc_mlo_dma_addr;
			PHYSADDRTOULONG(mlo_ipc_sys_gp->pcie_ipc_mlo_pa, ipc_mlo_dma_addr);
			ipc_mlo_dma_addr = phys_to_dma(&bus->dev->dev, ipc_mlo_dma_addr);
			pcie_ipc_mlo_tlv.haddr64_lo	= lower_32_bits(ipc_mlo_dma_addr);
			pcie_ipc_mlo_tlv.haddr64_hi	= upper_32_bits(ipc_mlo_dma_addr);
		} else {
			ASSERT(bus->dev);
		}
#else /* !CMWIFI_33940 */
		pcie_ipc_mlo_tlv.haddr64_lo	= PHYSADDRLO(mlo_ipc_sys_gp->pcie_ipc_mlo_pa);
		pcie_ipc_mlo_tlv.haddr64_hi	= PHYSADDRHI(mlo_ipc_sys_gp->pcie_ipc_mlo_pa);
#endif /* CMWIFI_33940 */
		pcie_ipc_mlo_tlv.ap_unit	= (uint32)ap_unit;
		pcie_ipc_mlo_tlv.mlo_unit	= (uint32)pcie_ipc_mlo->sys.mlo_unit[ap_unit];
		pcie_ipc_mlo_tlv.tlv_size	= htol32(OFFSETOF(pcie_ipc_mlo_tlv_t, tlv_size));
		pcie_ipc_mlo_tlv.tlv_signature	= htol32(PCIE_IPC_MLO_TLV_SIGNATURE);

		bus->ramtop_addr -= sizeof(pcie_ipc_mlo_tlv_t);
		dhdpcie_bus_membytes(bus, PCIE_SYSMEM_WRITE, bus->ramtop_addr,
			(uint8*)&pcie_ipc_mlo_tlv, sizeof(pcie_ipc_mlo_tlv_t));

		DHD_ERROR(("%s: System-wide PCIE IPC MLO: 0x%08x%08x\n", __FUNCTION__,
			pcie_ipc_mlo_tlv.haddr64_hi, pcie_ipc_mlo_tlv.haddr64_lo));
	}
#endif /* MLO_IPC */

#if defined(DHD_BHM)
	/* Pass the BHM memory to dongle via SysMem TLV identified
	 * by the signature BHM_LOCATION_TLV_SIGNATURE.
	 */
	if (bus->bhm_mem_dma_buf.va != NULL) {
		bhm_location_tlv_t	bhm_location_tlv; // Declared on stack
		uint64			dma_buf_va_u64;

		dma_buf_va_u64 = (uint64)(uintptr)(bus->bhm_mem_dma_buf.va);

		bhm_location_tlv.haddr64_lo	= PHYSADDRLO(bus->bhm_mem_dma_buf.pa);
		bhm_location_tlv.haddr64_hi	= PHYSADDRHI(bus->bhm_mem_dma_buf.pa);
		bhm_location_tlv.haddr64_va_lo	= U64_TO_LO32(dma_buf_va_u64);
		/* coverity[Operands don't affect result] : valid only for 64bit platforms */
		bhm_location_tlv.haddr64_va_hi	= U64_TO_HI32(dma_buf_va_u64);
		bhm_location_tlv.bhm_size	= htol32(bus->bhm_mem_dma_buf.len);
		bhm_location_tlv.tlv_size	= htol32(BHM_LOCATION_TLV_SIZE);
		bhm_location_tlv.tlv_signature	= htol32(BHM_LOCATION_TLV_SIGNATURE);

		bus->ramtop_addr -= sizeof(bhm_location_tlv_t);
		dhdpcie_bus_membytes(bus, PCIE_SYSMEM_WRITE, bus->ramtop_addr,
			(uint8*)&bhm_location_tlv, sizeof(bhm_location_tlv_t));

		DHD_ERROR(("%s: Boot Host Memory location: 0x%08x%08x bhm_size:%u\n",
			__FUNCTION__, bhm_location_tlv.haddr64_hi, bhm_location_tlv.haddr64_lo,
			bhm_location_tlv.bhm_size));
	}
#endif /* DHD_BHM */

	return BCME_OK;
} /* dhdpcie_bus_write_sysmem_tlvs() */
...
void
dhd_bus_flowring_ids_dump(dhd_pub_t *dhdp, struct bcmstrbuf *strbuf)
{
	uint16 flowid;
	uint16 ix = 0;
	flow_ring_node_t *flow_ring_node;
	...
	uint16 frs_per_tid[NUMPRIO] = { 0 };

	bcm_bprintf(strbuf, "%4s:%4s %4s %s\n", "Num:", "Flow", "Prio",
		"Dest_MacAddress");
	...
	for (flowid = 0; flowid < dhdp->num_flow_rings; flowid++) {
		...
		bcm_bprintf(strbuf,
			"%4d:%4d %1d %s\n", ix++,
			flow_ring_node->flowid, flow_info->tid,
			bcm_ether_ntoa((struct ether_addr *)&flow_info->da, eabuf));
			frs_per_tid[flow_info->tid]++;
	}

	bcm_bprintf(strbuf, "------------------------\n");
	bcm_bprintf(strbuf, "TID | #Flowrings Per TID");
	for (ix = 0; ix < NUMPRIO; ix++) {
		bcm_bprintf(strbuf, "\n%3d | %4d", ix, frs_per_tid[ix]);
	}
	bcm_bprintf(strbuf, "\n");
...
dhd_bus_dump(dhd_pub_t *dhdp, struct bcmstrbuf *strbuf)
{
	...
	bcm_bprintf(strbuf, "%s %4s %4s %2s %4s %17s %4s %4s %4s %6s %10s ",
		"Num:", "Offl", "Flow", "If", "Prio", ":Dest_MacAddress:", "Qlen",
		"CThr", "CLen", "L2CLen", "Overflows");
	if (PCIE_HBQD(dhdp))
	...
	for (flowid = 0; flowid < dhdp->num_flow_rings; flowid++) {
		...
		flow_info = &flow_ring_node->flow_info;
		bcm_bprintf(strbuf,
			"%3d. %4d %4d %2d %4d %17s %4d %4d %4d %6d %10u ", ix++,
#if defined(BCM_DHD_RUNNER)
			...
			DHD_FLOW_QUEUE_THRESHOLD(&flow_ring_node->queue),
	...
	bcm_bprintf(strbuf, "TID from Dongle: %s\n", dhdp->tid_from_dongle == TRUE ? "Yes" : "No");
}
...
void
dhd_bus_update_cpudbg_wridx(dhd_pub_t *dhd)
{
	/* check for CPU DBG entry update */
	if (dhd->cpudbg_hme) {
		uint32 *cpudbg_hme_wridx = (uint32 *)
			((uint8*)(dhd->cpudbg_hme) + OFFSETOF(cpudbg_hme_t, wridx));
		dhd->cpudbg_wridx = (uint16)LTOH32(*cpudbg_hme_wridx);
	}
}

void
dhd_bus_update_cpudbg(dhd_pub_t *dhd)
{
	/* check for CPU DBG entry update */
	if (dhd->cpudbg_hme && !dhd->dongle_trap_occured) {
		dhd_bus_update_cpudbg_wridx(dhd);
		if (dhd->cpudbg_wridx != dhd->cpudbg_rdidx) {
			DHD_TRACE(("%s: got dhd->cpudbg_wridx %d\n",
				__FUNCTION__, dhd->cpudbg_wridx));
			dhd_schedule_cpudbg_consume(dhd);
		}
	}
}
...
#ifdef MLO_IPC
/* Notify the dongle to enter a new state */
int
dhdpcie_mlo_h2d_notify(dhd_bus_t *bus, uint8 state)
{
	uint32 h2d_mb_data;

	switch (state) {
	/* Experimental, should be invoked through the fsm */
	case mlo_ipc_mlc_bind_e:
		h2d_mb_data = PCIE_IPC_H2DMB_MLC_BIND;
		break;
	case mlo_ipc_mlc_link_e:
		h2d_mb_data = PCIE_IPC_H2DMB_MLC_LINK;
		break;
	case mlo_ipc_mlc_bridge_e:
		h2d_mb_data = PCIE_IPC_H2DMB_MLC_BRIDGE;
		break;
	case mlo_ipc_mlc_sync_e:
		h2d_mb_data = PCIE_IPC_H2DMB_MLC_SYNC;
		break;
	case mlo_ipc_mlc_ready_e:
		h2d_mb_data = PCIE_IPC_H2DMB_MLC_READY;
		break;
	case mlo_ipc_mlc_halt_e:
		h2d_mb_data = PCIE_IPC_H2DMB_MLC_HALT;
		break;
	case mlo_ipc_mlc_suspend_e:
		h2d_mb_data = PCIE_IPC_H2DMB_MLC_SUSPEND;
		break;
	case mlo_ipc_mlc_suspend_halt_e:
		h2d_mb_data = PCIE_IPC_H2DMB_MLC_SUSPEND_HALT;
		break;
	default:
		/* Unknown state */
		return BCME_UNSUPPORTED;
	...
	/* Send the mailbox data and generate mailbox intr. */
	dhdpcie_send_mb_data(bus, h2d_mb_data);
	return BCME_OK;
}
#endif /* MLO_IPC */
...
dhdpcie_send_mb_data(dhd_bus_t *bus, uint32 h2d_mb_data)
{
	...
	if (h2d_mb_data == PCIE_IPC_H2DMB_HOST_D3_INFORM) {
		DHD_INFO(("%s: send PCIE_IPC_H2DMB_HOST_D3_INFORM to dongle\n",
			__FUNCTION__));
		bus->d3_inform_cnt++;
	}
	if (h2d_mb_data == PCIE_IPC_H2DMB_HOST_D0_INFORM_IN_USE) {
		DHD_INFO(("%s: send PCIE_IPC_H2DMB_HOST_D0_INFORM_IN_USE to dongle\n",
			__FUNCTION__));
		bus->d0_inform_in_use_cnt++;
	}
	if (h2d_mb_data == PCIE_IPC_H2DMB_HOST_D0_INFORM) {
		DHD_INFO(("%s: send PCIE_IPC_H2DMB_HOST_D0_INFORM to dongle\n",
			__FUNCTION__));
		bus->d0_inform_cnt++;
	}
#ifdef MLO_IPC
	if (h2d_mb_data == PCIE_IPC_H2DMB_MLC_BIND) {
		DHD_ERROR(("%s: send PCIE_IPC_H2DMB_MLC_BIND to dongle\n",
			__FUNCTION__));
	}
	if (h2d_mb_data == PCIE_IPC_H2DMB_MLC_LINK) {
		DHD_ERROR(("%s: send PCIE_IPC_H2DMB_MLC_LINK to dongle\n",
			__FUNCTION__));
	}
	if (h2d_mb_data == PCIE_IPC_H2DMB_MLC_BRIDGE) {
		DHD_ERROR(("%s: send PCIE_IPC_H2DMB_MLC_BRIDGE to dongle\n",
			__FUNCTION__));
	}
	if (h2d_mb_data == PCIE_IPC_H2DMB_MLC_SYNC) {
		DHD_ERROR(("%s: send PCIE_IPC_H2DMB_MLC_SYNC to dongle\n",
			__FUNCTION__));
	}
	if (h2d_mb_data == PCIE_IPC_H2DMB_MLC_HALT) {
		DHD_ERROR(("%s: send PCIE_IPC_H2DMB_MLC_HALT to dongle\n",
			__FUNCTION__));
	}
#endif /* MLO_IPC */
}
...
dhdpcie_handle_mb_data(dhd_bus_t *bus)
{
	...
	DHD_INFO(("dhd%d: D2H_MB_DATA: 0x%08x\n", bus->dhd->unit, d2h_mb_data));
...
#ifdef MLO_IPC
	if (d2h_mb_data & PCIE_IPC_D2HMB_MLC_BIND)  {
		DHD_ERROR(("dhd%d: D2H_MB_DATA: dongle enters MLC_BIND\n", bus->dhd->unit));
		dhd_mlo_ipc_process_dongle_event(bus->dhd, mlo_ipc_mlc_bind_e);
	}
	if (d2h_mb_data & PCIE_IPC_D2HMB_MLC_LINK)  {
		DHD_ERROR(("dhd%d: D2H_MB_DATA: dongle enters MLC_LINK\n", bus->dhd->unit));
		dhd_mlo_ipc_process_dongle_event(bus->dhd, mlo_ipc_mlc_link_e);
	}
	if (d2h_mb_data & PCIE_IPC_D2HMB_MLC_BRIDGE)  {
		DHD_ERROR(("dhd%d: D2H_MB_DATA: dongle enters MLC_BRIDGE\n", bus->dhd->unit));
		dhd_mlo_ipc_process_dongle_event(bus->dhd, mlo_ipc_mlc_bridge_e);
	}
	if (d2h_mb_data & PCIE_IPC_D2HMB_MLC_SYNC)  {
		DHD_ERROR(("dhd%d: D2H_MB_DATA: dongle enters MLC_SYNC\n", bus->dhd->unit));
		dhd_mlo_ipc_process_dongle_event(bus->dhd, mlo_ipc_mlc_sync_e);
	}
	if (d2h_mb_data & PCIE_IPC_D2HMB_MLC_SUSPEND)  {
		DHD_ERROR(("dhd%d: D2H_MB_DATA: dongle enters MLC_SUSPEND\n", bus->dhd->unit));
		dhd_mlo_ipc_process_dongle_event(bus->dhd, mlo_ipc_mlc_suspend_e);
	}
	if (d2h_mb_data & PCIE_IPC_D2HMB_MLC_DOWN)  {
		DHD_ERROR(("dhd%d: D2H_MB_DATA: dongle enters MLC_DOWN\n", bus->dhd->unit));
		dhd_mlo_ipc_process_dongle_event(bus->dhd, mlo_ipc_mlc_down_e);
	}
#if defined(BCMMLO_GR)
	if (d2h_mb_data & PCIE_IPC_D2HMB_MLC_BH_REQ)  {
		DHD_ERROR(("dhd%d: D2H_MB_DATA: dongle requests MLC_BH\n", bus->dhd->unit));
		/* Schedule a Big Hammer Task */
		dhd_schedule_mlo_bh_req(bus->dhd, FALSE);
	}
	if (d2h_mb_data & PCIE_IPC_D2HMB_MLC_BH_HALT_REQ)  {
		DHD_ERROR(("dhd%d: D2H_MB_DATA: dongle requests MLC_BH_HALT\n", bus->dhd->unit));
		/* Schedule a Big Hammer Task with PSMWD */
		dhd_schedule_mlo_bh_req(bus->dhd, TRUE);
	}
	if (d2h_mb_data & PCIE_IPC_D2HMB_MLC_BH)  {
		DHD_ERROR(("dhd%d: D2H_MB_DATA: dongle enters MLC_BH\n", bus->dhd->unit));
		dhd_mlo_ipc_process_dongle_event(bus->dhd, mlo_ipc_mlc_bh_e);
	}
#endif /* BCMMLO_GR */
	if (d2h_mb_data & PCIE_IPC_D2HMB_MLC_DOWN_BH)  {
		DHD_ERROR(("dhd%d: D2H_MB_DATA: dongle enters MLC_DOWN_BH\n", bus->dhd->unit));
		dhd_mlo_ipc_process_dongle_event(bus->dhd, mlo_ipc_mlc_down_bh_e);
	}
	if (d2h_mb_data & PCIE_IPC_D2HMB_MLC_ACTIVE)  {
		DHD_ERROR(("dhd%d: D2H_MB_DATA: dongle enters MLC_ACTIVE\n", bus->dhd->unit));
		dhd_mlo_ipc_process_dongle_event(bus->dhd, mlo_ipc_mlc_active_e);
	}
#endif /* MLO_IPC */

	if ((
#ifdef MLO_IPC
		(d2h_mb_data & PCIE_IPC_D2HMB_MLC_HALT) ||
#endif
		(d2h_mb_data & PCIE_IPC_D2HMB_DEV_FWHALT)) &&
#ifdef BCM_ROUTER
		(!is_reboot) &&
#endif
		TRUE) {
...
#ifdef MLO_IPC
		if (d2h_mb_data & PCIE_IPC_D2HMB_MLC_HALT)  {
			DHD_ERROR(("D2H_MB_DATA: dongle fw halted\n"));
		} else
#endif /* MLO_IPC */
		{
			...
			/* DHD Dump */
			DHD_LOCK(bus->dhd);
			dhd_dump_to_kernelog(bus->dhd);
			DHD_UNLOCK(bus->dhd);
		}

		DHD_LOCK(bus->dhd);
		dhdpcie_checkdied(bus, NULL, 0, &ext_abort);
...
#ifdef MLO_IPC
		if (d2h_mb_data & PCIE_IPC_D2HMB_MLC_HALT)  {
			dhd_schedule_mlo_halt_log_dump(bus->dhd, ext_abort);
		} else
#endif /* MLO_IPC */
		{
#ifdef MLO_IPC
			if (dhd_mlo_ipc_process_dongle_event(bus->dhd,
				mlo_ipc_mlc_trap_e) == BCME_OK) {
				dhd_schedule_mlo_halt_log_dump(bus->dhd, ext_abort);
				return;
			}
			/* Proceed with the legacy dump for non-MLO */
#endif /* MLO_IPC */
			dhd_schedule_trap_log_dump(bus->dhd,
				ext_abort ? FWTRAP_DUMP_SKIPREG : FWTRAP_DUMP_FULL);
		}
	}
}
...
#define UCLS_BUDGET		16
#define UCLS_BUDGET_TRAP	64

static bool
hndpci_bus_ucls_read(dhd_bus_t *bus)
{
	m2m_core_regs_t *m2m_core_regs;
	volatile m2m_ucls_eng_regs_t *regs = NULL;
	uint32 wridx, rdidx;
	char *ring_buf;
	dhd_pub_t *dhd = bus->dhd;
	uint readlen = 0;
	uint8 ch;
	m2m_ucls_cmd_sts_t *slot_cmd;
	uint16 flags, id;
	uint32 n;
	int i;
	daddr32_t daddr32 = ltoh32(dhd->ucls_daddr32);
	static bool tr = FALSE;		/* last message is truncated? */
	bool nl;			/* newline */
	bool more = FALSE;
	int budget = UCLS_BUDGET;

	if (!dhd->ucls_line) {
		return more;
	}
	if (dhd->ucls_cap == UCLS_CAP_SW) {
		ASSERT(daddr32 != 0);
		/* Get dongle write index */
		dhdpcie_bus_membytes(bus, PCIE_SYSMEM_READ,
			daddr32 + OFFSETOF(m2m_ucls_eng_regs_t, wridx),
			(uint8 *)&wridx, sizeof(uint32));
	} else {
		ASSERT(dhd->ucls_cap == UCLS_CAP_HW);
		m2m_core_regs = si_setcore(bus->sih, M2MDMA_CORE_ID, M2MDMA_CORE_1);
		if (!m2m_core_regs)
			return FALSE;
		regs = &m2m_core_regs->ucls_regs;
		/* Get HW write index */
		wridx = R_REG(dhd->osh, &regs->wridx);
	}
	dhd->ucls_wridx = wridx;
	rdidx = dhd->ucls_rdidx;
	/* Have a larger budget limit when a trap happens */
	if (dhd->dongle_trap_occured) {
		budget = MIN(UCLS_BUDGET_TRAP, dhd->ucls_ring_depth);
	}

	while (rdidx != wridx && budget > 0) {
		ring_buf = (char *)dhd->ucls_buf + (dhd->ucls_ring_size * rdidx);
		OSL_CACHE_INV(ring_buf, dhd->ucls_ring_size);
		slot_cmd = (m2m_ucls_cmd_sts_t *)ring_buf;
		i = sizeof(m2m_ucls_cmd_sts_t);
		slot_cmd->uval32 = LTOH32(slot_cmd->uval32);
		flags = slot_cmd->flags;
		id = slot_cmd->id;
		readlen = slot_cmd->len;
		/* now output the read data from the local buffer to the host console */
#if defined(LINUX) || defined(linux)
		preempt_disable();
#endif /* LINUX || linux */
		while (i < readlen) {
			nl = FALSE;
			for (n = dhd->ucls_cursor; n < CONSOLE_LINE_MAX - 2 && i < readlen; n++) {
				ch = ring_buf[i];
				++i;
				if (ch == '\n')  {
					nl = TRUE;
			...
			if (n != dhd->ucls_cursor) {
				if (n == CONSOLE_LINE_MAX - 2) {
					nl = TRUE;
				}
				if (dhd->ucls_line[n - 1] == '\r') {
					n--;
				}
				dhd->ucls_line[n] = 0;
				if (nl) {
					if (dhd->dhd_console_ms)
						printf(CONSOLE_MSG, dhd->unit, dhd->ucls_line);
					dhd->ucls_cursor = 0;
					tr = FALSE;
				} else {
					/* message truncated */
		...
		/* Do not use UCLS_RING_MASK, as the dongle FW could change the ring depth */
		rdidx = (rdidx + 1) % dhd->ucls_ring_depth;
		budget--;
	}

	/* Reschedule if more messages or a message line is truncated */
	more = (rdidx != wridx) || (tr);

	/* Update dongle read index */
	if (rdidx != dhd->ucls_rdidx) {
		dhd->ucls_rdidx = rdidx;
		if (dhd->ucls_cap == UCLS_CAP_SW) {
			dhdpcie_bus_membytes(bus, PCIE_SYSMEM_WRITE,
				daddr32 + OFFSETOF(m2m_ucls_eng_regs_t, rdidx),
				(uint8_t *)&rdidx, sizeof(uint32));
		} else {
			/* UCLS_CAP_HW */
			W_REG(dhd->osh, &regs->rdidx, rdidx);
		}
	}
...
#if defined(BCM_HMO_EVENT)
static bool
dhdpcie_bus_hmo_event_ring_read(dhd_bus_t *bus)
{
	uint32 wridx, rdidx;
	char *ring_buf;
	dhd_pub_t *dhd = bus->dhd;
	...
	int ifidx = 0;
	void *pkt;
	daddr32_t wr_daddr32 = dhd->hmo_event_wr_daddr32;
	daddr32_t rd_daddr32 = dhd->hmo_event_rd_daddr32;

	DHD_PERIM_ASSERT(dhd);
	ASSERT(dhd->busstate == DHD_BUS_DATA);
	ASSERT((wr_daddr32 != 0) && (rd_daddr32 != 0));

	/* Get dongle write index */
	dhdpcie_bus_membytes(bus, PCIE_SYSMEM_READ, wr_daddr32,
		(uint8 *)&wridx, sizeof(uint32));
	dhd->hmo_event_wridx = wridx;
	rdidx = dhd->hmo_event_rdidx;
	while (rdidx != wridx)  {
		ring_buf = (char *)dhd->hmo_event_buf + (dhd->hmo_event_ring_size * rdidx);
		OSL_CACHE_INV(ring_buf, dhd->hmo_event_ring_size);
		msg = (cmn_msg_hdr_t *)ring_buf;
		evtmsg = (bcm_event_t *)&msg[1];
		buflen = hton16(evtmsg->bcm_hdr.length) + sizeof(struct ether_header) + 4;
		ifidx = dhd_ifname2idx(dhd->info, evtmsg->event.ifname);
		ASSERT(buflen <= dhd->hmo_event_ring_size);
		pkt = PKTGET(dhd->osh, buflen, FALSE);
		if (!pkt) {
			DHD_ERROR(("%s: PKTGET fail!\n", __FUNCTION__));
			return FALSE;
		}
		bcopy(&msg[1], PKTDATA(dhd->osh, pkt), buflen);
		PKTSETLEN(dhd->osh, pkt, buflen);
		/* DHD_INFO(("%s:%s wr %d rd %d len %d event_type %d ring_buf %p\n",
			__FUNCTION__, evtmsg->event.ifname, wridx, rdidx, buflen+8,
			hton32(evtmsg->event.event_type), ring_buf));
		*/
		dhd_bus_rx_frame(dhd->bus, pkt, ifidx, 1);
		rdidx = (rdidx + 1) % dhd->hmo_event_ring_depth;
	}

	/* Update dongle read index */
	if (rdidx != dhd->hmo_event_rdidx) {
		dhd->hmo_event_rdidx = rdidx;
		dhdpcie_bus_membytes(bus, PCIE_SYSMEM_WRITE, rd_daddr32,
			(uint8_t *)&rdidx, sizeof(uint32));
	}
...
dhdpci_bus_read_frames(dhd_bus_t *bus)
{
...
#ifdef BCM_PCAP
	if (DHD_PCAP_EN(bus->dhd)) {
		DHD_LOCK(bus->dhd);
		dhd_prot_process_pcap_completion(bus->dhd);
		DHD_UNLOCK(bus->dhd);
	}
#endif /* BCM_PCAP */
	...
	/* check for new CPU DBG entries */
	dhd_bus_update_cpudbg(bus->dhd);

#if defined(DHD_DEBUG) && defined(HNDUCLS_DOORBELL)
	if (bus->intstatus & PCIE_MB_TOPCIE_D2H2_DB1) {
		if (bus->dhd->ucls_cap > UCLS_CAP_NONE)
			more |= hndpci_bus_ucls_read(bus);
	}
#endif /* DHD_DEBUG && HNDUCLS_DOORBELL */

#if defined(HMO_EVENT_DOORBELL)
	if (bus->intstatus & PCIE_MB_TOPCIE_D2H2_DB0) {
		if (bus->dhd->busstate == DHD_BUS_DATA &&
		    bus->dhd->hmo_event_up &&
		    !bus->dhd->dongle_trap_occured) {
			more |= dhdpcie_bus_hmo_event_ring_read(bus);
		}
	}
#endif /* HMO_EVENT_DOORBELL */
...
#if defined(MLO_IPC)
	if (dhd_is_mlo_ap(bus->dhd)) {
		dhd_mlo_ipc_update_suspend_state(bus->dhd);
	}
#endif /* MLO_IPC */
	...
	pcie_ipc_hme_t **pcie_ipc_hme_p)
{
	...
	ASSERT(pcie_ipc_daddr32 != (daddr32_t*)NULL);
	...
	*pcie_ipc_daddr32 = 0U;
	 ...
	 * - PCIe IPC Rev >= 0x8B: pcie_ipc_hme_user_t is extended to carry bhm_offset
	...
	if (dngl_ipc_revision >= 0x84) // Discrete pcie_ipc_hme_t
	{
		...
		if (dngl_ipc_revision >= 0x8B) { // Post 0x8A, pcie_ipc_hme_user_t is extended
			ASSERT(pcie_ipc_hme_size == PCIE_IPC_HME_SIZE(pcie_ipc_hme_users));
			bus->pcie_ipc_hme_user_size = PCIE_IPC_HME_USER_SIZE;
#if defined(DHD_BHM)
			bus->dngl_bhm_offset = TRUE;
#endif /* DHD_BHM */
		} else {
			ASSERT(pcie_ipc_hme_size == PCIE_IPC_HME_REV_x8A_SIZE(pcie_ipc_hme_users));
			bus->pcie_ipc_hme_user_size = PCIE_IPC_HME_USER_REV_x8A_SIZE;
		}
...
#if defined(DHD_BHM)
			if (bus->dngl_bhm_offset) {
				_PCIE_IPC_HME_USER_LTOH32(bhm_offset);
			}
			/* Check if any HME user memory is carved from BHM */
			if (pcie_ipc_hme_user->flags & PCIE_IPC_HME_FLAG_BHM) {
		...
		/* Pre 0x84 only the max HME users are fixed to 8 (PCIE_IPC_HME_USERS_MAX), and
		 * use the same PCIE IPC HME user bytes pcie_ipc_hme_user_t as in Rev 0x8A
		 */
		pcie_ipc_hme_size = PCIE_IPC_HME_REV_x8A_SIZE(PCIE_IPC_HME_USERS_MAX);
		pcie_ipc_hme = (pcie_ipc_hme_t*)MALLOCZ(bus->dhd->osh, pcie_ipc_hme_size);
		if (pcie_ipc_hme == (pcie_ipc_hme_t*)NULL) {
			DHD_ERROR(("%s: PCIe IPC MEMORY FAILURE: "
				"malloc %u bytes. [pcie_ipc_hme users %u]\n",
	...
	*pcie_ipc_hme_p = pcie_ipc_hme; // store into bus
	...
	DHD_PCIE_IPC(("\t ID HMEUSR PAGES ALIGN   BOUNDARY  FLAGS  BHM\n"));
	for (user_id = 0; user_id < pcie_ipc_hme->users; ++user_id)
	{
		pcie_ipc_hme_user = DHD_PCIE_IPC_HME_USER(pcie_ipc_hme, user_id,
			bus->pcie_ipc_hme_user_size);
		DHD_PCIE_IPC(("\t %2u %6s %5u %5u 0x%08x 0x%04x  %s\n",
			...
			((pcie_ipc_hme_user->flags & PCIE_IPC_HME_FLAG_BHM) ? "YES" : "NO")));
	}
...
skip_pcie_ipc_hme:
	...
	if (pcie_ipc_rings != (pcie_ipc_rings_t*)NULL)
	{
		 ...
		 * MAX_DYNAMIC_RINGS = MAX_DHD_RINGS. Reduce max_flowrings if we cannot hold
		 * all of them.
		 */
		if (pcie_ipc_rings->max_h2d_rings > (MAX_DHD_RINGS)) {
			uint16 h2drings, flowrings, h2d_dyn_rings;

			ASSERT(pcie_ipc_rings->max_flowrings <=
				pcie_ipc_rings->max_h2d_rings - BCMPCIE_H2D_COMMON_MSGRINGS);

			/* Only reduce tx flow rings */
			h2d_dyn_rings = pcie_ipc_rings->max_h2d_rings -
				BCMPCIE_H2D_COMMON_MSGRINGS -
				pcie_ipc_rings->max_flowrings;
			flowrings = MAX_DHD_TX_FLOWS - h2d_dyn_rings;
	...
	if ((dngl_ipc_revision >= 0x88) && (pcie_ipc->dcap2 & PCIE_IPC_DCAP2_MAC_CPU_DBG)) {
		bus->dhd->cpudbg_wl_support = TRUE;
	}
...
dhdpcie_bus_init_pcie_ipc(dhd_bus_t *bus)
{
	...
	bus->btrace_daddr32            = pcie_ipc->btrace_daddr32;
	...
	bus->max_flowrings = pcie_ipc_rings->max_flowrings;
	bus->max_h2d_dynamic_rings = pcie_ipc_rings->max_h2d_rings -
		pcie_ipc_rings->max_flowrings - BCMPCIE_H2D_COMMON_MSGRINGS;
	bus->max_d2h_dynamic_rings = pcie_ipc_rings->max_d2h_rings -
	...
	DHD_PCIE_IPC(("\t Config  : Max Rings H2D %u Flowrings %u D2H %u\n",
		bus->max_h2d_rings, bus->max_flowrings, bus->max_d2h_rings));
	DHD_PCIE_IPC(("\t Config  : Max Dynamic Rings H2D %u D2H %u\n",
		dhd_bus_max_h2d_dynamic_rings(bus),
		dhd_bus_max_d2h_dynamic_rings(bus)));
	...
	if (IDMA_CAPABLE(bus) && bus->idma_enabled) {
		if (IDMA64_CAPABLE(bus) && (pcie_ipc->dcap2 & PCIE_IPC_DCAP2_IDMA64)) {
			pcie_ipc->hcap2 |= PCIE_IPC_HCAP2_IDMA64;
#if defined(BCM_DHD_RUNNER)
#if defined(H2R_PCIE_IPC_CAP_IDMA64)
			{
				uint32 rcap = H2R_PCIE_IPC_CAP_IDMA64;
				dhd_runner_notify(bus->dhd->runner_hlp, H2R_PCIE_IPC_CAP_NOTIF,
					(unsigned long)H2R_PCIE_IPC_CAP_IDMA64,
					(unsigned long)&rcap);
				/* If runner is not supporting then clear BA256 support flag */
				if ((rcap & H2R_PCIE_IPC_CAP_IDMA64) == 0) {
					pcie_ipc->hcap2 &= ~PCIE_IPC_HCAP2_IDMA64;
				}
			}
#else /* !H2R_PCIE_IPC_CAP_IDMA64 */
			/* If H2R_PCIE_IPC_CAP_IDMA64 is not defined then Runner/Archer
			 * code does not include IDMA64 support
			 */
			pcie_ipc->hcap2 &= ~PCIE_IPC_HCAP2_IDMA64;
#endif /* defined(H2R_PCIE_IPC_CAP_IDMA64) */
#endif /* BCM_DHD_RUNNER */
		}

		if (pcie_ipc->hcap2 & PCIE_IPC_HCAP2_IDMA64) {
			bus->dhd->idma_enable = TRUE;
			bus->dhd->idma_set = BCMPCIE_IDMA_SET64;
			/* "-1" because set 0 is used by D2H read indices */
			bus->dhd->h2d_idma_set = BCMPCIE_IDMA_SET64 - 1;
			pcie_ipc->hcap2 |= PCIE_IPC_HCAP2_IDMA64;
			DHD_PCIE_IPC(("\t Feature : Host iDMA64\n"));
		} else if (pcie_ipc->dcap1 & PCIE_IPC_DCAP1_IDMA) {
			bus->dhd->idma_enable = TRUE;
			bus->dhd->idma_set = BCMPCIE_IDMA_SET16;
			/* "-1" because set 0 is used by D2H read indices */
			bus->dhd->h2d_idma_set = BCMPCIE_IDMA_SET16 - 1;
			pcie_ipc->hcap1 |= PCIE_IPC_HCAP1_IDMA;
			DHD_PCIE_IPC(("\t Feature : Host iDMA16\n"));
		} else {
...
#ifdef BCM_PCAP
	/* Does the firmware support pcapxf? */
	if (pcie_ipc->dcap2 & PCIE_IPC_DCAP2_PCAPXF) {
		/* Enable CSI monitor support in firmware if supported */
		pcie_ipc->hcap2 |= PCIE_IPC_HCAP2_PCAPXF;
		DHD_PCIE_IPC(("\t Feature : Packet Capture PCAP\n"));
	} else {
		DHD_PCIE_IPC(("\t Feature : PCAP disabled\n"));
	}
#endif /* BCM_PCAP */
	...
	/* BA256 config exchange */
	if (pcie_ipc->dcap2 & PCIE_IPC_DCAP2_BA256_CFG) {
		/* Host set the enable flag. When BCM_DHD_RUNNER is defined it will
		 * check Runner/Archer's capability and clear it if needed.
		 */
		pcie_ipc->hcap2 |= PCIE_IPC_HCAP2_BA256_CFG;
#if defined(BCM_DHD_RUNNER)
#if defined(H2R_PCIE_IPC_CAP_BA256CFG)
		{
			uint32 rcap = H2R_PCIE_IPC_CAP_BA256CFG;
			dhd_runner_notify(bus->dhd->runner_hlp, H2R_PCIE_IPC_CAP_NOTIF,
				(unsigned long)H2R_PCIE_IPC_CAP_BA256CFG,
				(unsigned long)&rcap);
			/* If runner is not supporting then clear BA256 support flag */
			if ((rcap & H2R_PCIE_IPC_CAP_BA256CFG) == 0) {
				pcie_ipc->hcap2 &= ~PCIE_IPC_HCAP2_BA256_CFG;
			}
		}
#else /* !H2R_PCIE_IPC_CAP_BA256CFG */
			/* If H2R_PCIE_IPC_CAP_BA256CFG is not defined then Runner/Archer
			 * code does not include ba256 support
			 */
			pcie_ipc->hcap2 &= ~PCIE_IPC_HCAP2_BA256_CFG;
#endif /* defined(H2R_PCIE_IPC_CAP_BA256CFG) */
#endif /* BCM_DHD_RUNNER */
	}

	/* All dynamic ring initialization/capability checking must happen prior to
	 * dhdpcie_bus_init_pcie_ipc_rings(), for ring_mem init.
	 */
#ifdef BCM_PCAP
	/* Check packet capture (PCAP) support */
	if (pcie_ipc->dcap2 & PCIE_IPC_DCAP2_PCAP) {
		DHD_PCAP_EN(bus->dhd) = TRUE;
		pcie_ipc->hcap2 |= PCIE_IPC_HCAP2_PCAP;
		DHD_PCIE_IPC(("\t Feature : PCAP\n"));
	} else {
		DHD_PCAP_EN(bus->dhd) = FALSE;
		DHD_PCIE_IPC(("\t Feature : PCAP\n"));
	}
#endif /* BCM_PCAP */
	...
	host_physaddrhi = ~0U;
	use_haddr64 = TRUE;
#else  /* !(CMWIFI && BCMDMA64OSL) */
	/* All other Routers: haddr64.hi = 0 even on 64bit hosts */
	host_physaddrhi = 0U;
	use_haddr64 = FALSE;
	...
	/* Host will attempt to enable Rx Completion V2 (TID to host) if Dongle supports it */
	if (pcie_ipc->dcap2 & PCIE_IPC_DCAP2_RX_CMPL_V2) {
		/* Host set the enable flag. When BCM_DHD_RUNNER is defined it will
		 * check Runner/Archer's capability and clear it if needed.
		 */
		pcie_ipc->hcap2 |= PCIE_IPC_HCAP2_RX_CMPL_V2;
#if defined(BCM_DHD_RUNNER)
		/* If runner rx offl is enabled check runner capability. If not V2 will be used */
		if (DHD_RNR_OFFL_RXCMPL(bus->dhd)) {
			/* This compile check is needed if WLAN code is compiled with unsupported
			 * host code (P4 branch). If H2R_PCIE_IPC_CAP_RX_CMPL_V2 is defined check
			 * if it is enabled in the Runner.
			 */
#if defined(H2R_PCIE_IPC_CAP_RX_CMPL_V2)
			uint32 rcap = H2R_PCIE_IPC_CAP_RX_CMPL_V2;
			dhd_runner_notify(bus->dhd->runner_hlp, H2R_PCIE_IPC_CAP_NOTIF,
				(unsigned long)H2R_PCIE_IPC_CAP_RX_CMPL_V2,
				(unsigned long)&rcap);
			/* If runner is not supporting then clear V2 support flag */
			if ((rcap & H2R_PCIE_IPC_CAP_RX_CMPL_V2) == 0) {
				pcie_ipc->hcap2 &= ~PCIE_IPC_HCAP2_RX_CMPL_V2;
			}
#else /* !H2R_PCIE_IPC_CAP_RX_CMPL_V2 */
			/* If H2R_PCIE_IPC_CAP_RX_CMPL_V2 is not defined then Runner/Archer
			 * code does not include rx_cmpl_v2 support
			 */
			pcie_ipc->hcap2 &= ~PCIE_IPC_HCAP2_RX_CMPL_V2;
#endif /* defined(H2R_PCIE_IPC_CAP_RX_CMPL_V2) */
		}
#endif /* BCM_DHD_RUNNER */
		if (pcie_ipc->hcap2 & PCIE_IPC_HCAP2_RX_CMPL_V2) {
			bus->dhd->tid_from_dongle = TRUE;
		}
		DHD_PCIE_IPC(("\t Feature : TID from Donlge (rx_cmpl_v2) %s supported\n",
			bus->dhd->tid_from_dongle == TRUE ? "": "not"));
	}

#ifdef BCM_RXCPLE_RCH
	if (pcie_ipc->dcap2 & PCIE_IPC_DCAP2_RX_CMPL_RCH) {
		pcie_ipc->hcap2 |= PCIE_IPC_HCAP2_RX_CMPL_RCH;
#if defined(BCM_DHD_RUNNER)
		if (DHD_RNR_OFFL_RXCMPL(bus->dhd)) {
#if defined(H2R_PCIE_IPC_CAP_RX_CMPL_RCH)
			uint32 rcap = H2R_PCIE_IPC_CAP_RX_CMPL_RCH;
			dhd_runner_notify(bus->dhd->runner_hlp, H2R_PCIE_IPC_CAP_NOTIF,
				(unsigned long)H2R_PCIE_IPC_CAP_RX_CMPL_RCH,
				(unsigned long)&rcap);
			/* If runner/Archer is not supporting then clear RCH support flag */
			if ((rcap & H2R_PCIE_IPC_CAP_RX_CMPL_RCH) == 0) {
				pcie_ipc->hcap2 &= ~PCIE_IPC_HCAP2_RX_CMPL_RCH;
			}
#else /* !H2R_PCIE_IPC_CAP_RX_CMPL_RCH */
			/* If H2R_PCIE_IPC_CAP_RX_CMPL_RCH is not defined then Runner/Archer
			 * code does not include rx_cmpl_rch support
			 */
			pcie_ipc->hcap2 &= ~PCIE_IPC_HCAP2_RX_CMPL_RCH;
#endif /* defined(H2R_PCIE_IPC_CAP_RX_CMPL_RCH) */
		}
#endif /* BCM_DHD_RUNNER */
		if (pcie_ipc->hcap2 & PCIE_IPC_HCAP2_RX_CMPL_RCH) {
			bus->dhd->rxcple_rch = TRUE;
			ASSERT(pcie_ipc->max_rch_sdu_cnt <= BCMPCIE_RXCPLE_RCH_SDU_CNT_MAX);
			bus->dhd->max_rch_sdu_cnt = pcie_ipc->max_rch_sdu_cnt;
			bus->dhd->bcmpcie_rxcple_rch_size =
				BCMPCIE_RXCPLE_RCH_SIZE(pcie_ipc->max_rch_sdu_cnt);
			ASSERT(bus->dhd->bcmpcie_rxcple_rch_size <= BCMPCIE_RXCPLE_RCH_SIZE_MAX);
		}
		DHD_PCIE_IPC(("\t Feature : RXCPLE RCH%s supported; "
			"max_rch_sdu_cnt %d bcmpcie_rxcple_rch_size %d-bytes\n",
			bus->dhd->rxcple_rch == TRUE ? "": " not",
			bus->dhd->max_rch_sdu_cnt, bus->dhd->bcmpcie_rxcple_rch_size));
	}
...
#if defined(HNDUCLS)
	/* SW implemented UCLS using BME/d2h_xfer */
	if (pcie_ipc->dcap2 & PCIE_IPC_DCAP2_UCLS_SW) {
		bus->dhd->ucls_cap = UCLS_CAP_SW;
		pcie_ipc->hcap2 |= PCIE_IPC_HCAP2_UCLS_SW;
	}
	/* HW implemented UCLS using the M2MDMA/UCLS engine */
	if (pcie_ipc->dcap2 & PCIE_IPC_DCAP2_UCLS_HW) {
		bus->dhd->ucls_cap = UCLS_CAP_HW;
		pcie_ipc->hcap2 |= PCIE_IPC_HCAP2_UCLS_HW;
	}

	if (bus->dhd->ucls_cap > UCLS_CAP_NONE) {
		bus->dhd->ucls_ring_size = (uint32)pcie_ipc->ucls_ring_size;
		bus->dhd->ucls_ring_depth = (uint32)pcie_ipc->ucls_ring_depth;
		bus->dhd->ucls_daddr32 = pcie_ipc->ucls_daddr32;
		bus->dhd->ucls_buf = (char *)(dhd_prot_hme_get_dma_buf(bus->dhd,
			PCIE_IPC_HME_USER_CLSBUF)->va);
#if defined(DHD_DEBUG)
		bus->dhd->ucls_line = MALLOC(bus->dhd->osh, CONSOLE_LINE_MAX);
		if (!bus->dhd->ucls_line) {
			DHD_ERROR(("%s: ucls_line malloc failure\n",
				__FUNCTION__));
			return BCME_NOMEM;
		}
#endif /* DHD_DEBUG */
...
#if defined(BCM_HMO_EVENT)
	if (pcie_ipc->dcap2 & PCIE_IPC_DCAP2_HMO_EVENT) {
		bus->dhd->hmo_event_up = TRUE;
		pcie_ipc->hcap2 |= PCIE_IPC_HCAP2_HMO_EVENT;
		bus->dhd->hmo_event_ring_size = (uint32)pcie_ipc->hmo_event_ring_size;
		bus->dhd->hmo_event_ring_depth = (uint32)pcie_ipc->hmo_event_ring_depth;
		bus->dhd->hmo_event_wr_daddr32 = ltoh32(pcie_ipc->hmo_event_wr_daddr32);
		bus->dhd->hmo_event_rd_daddr32 = ltoh32(pcie_ipc->hmo_event_rd_daddr32);
		bus->dhd->hmo_event_buf = (char *)(dhd_prot_hme_get_dma_buf(bus->dhd,
			PCIE_IPC_HME_USER_HMOEVT)->va);
	}
#endif /* BCM_HMO_EVENT */
...
dhdpcie_bus_init_pcie_ipc_rings(dhd_bus_t *bus)
{
	...
	for (j = 0; j < bus->max_flowrings; i++, j++) {
		bus->ring_mem[i].wr_daddr32 = h2d_wr_daddr32;
		bus->ring_mem[i].rd_daddr32 = h2d_rd_daddr32;
	...
	/* Dynamic ring initialization happens here. The order of ring mem assignment here,
	 * specifically the H2D and D2H directions, needs to be consistent with the order
	 * that dynamic rings are set up in the proto layer. Saving DHD_PCAP_H2D_RINGID()/
	 * DHD_PCAP_D2H_RINGID() allows sanity-checking in proto layer.
	 */
#ifdef BCM_PCAP
	/* setup (1) H2D PCAP Submission and (2) D2H PCAP Completion rings. */
	if (DHD_PCAP_EN(bus->dhd)) {
		DHD_PRINT(("PCAP ringmem id's H2D:%d  D2H:%d\n", i, i + 1));
		/* H2D PCAP Submission */
		bus->ring_mem[i].wr_daddr32 = h2d_wr_daddr32;
		bus->ring_mem[i].rd_daddr32 = h2d_rd_daddr32;
		h2d_wr_daddr32 = h2d_wr_daddr32 + bus->rw_index_sz;
		h2d_rd_daddr32 = h2d_rd_daddr32 + bus->rw_index_sz;
		DHD_PCAP_H2D_RINGID(bus->dhd) = i;
		i++;
		/* D2H PCAP Completion */
		bus->ring_mem[i].wr_daddr32 = d2h_wr_daddr32;
		bus->ring_mem[i].rd_daddr32 = d2h_rd_daddr32;
...
dhd_bus_init(dhd_pub_t *dhdp, bool enforce_mutex)
{
	...
	if (ret < 0) {
		dhd_schedule_trap_log_dump(bus->dhd, FWTRAP_DUMP_SKIPREG);
		return ret;
	}
...
dhdpcie_chipmatch(uint16 vendor, uint16 device)
{
	...
	if ((device == BCM6717_D11BE_ID) || (device == BCM6717_D11BE2G_ID) ||
		(device == BCM6717_D11BE5G_ID) || (device == BCM6717_D11BE6G_ID) ||
		(device == BCM6717_D11BE2G6G_ID) || (device == BCM6717_D11BE5G6G_ID) ||
		(device == BCM6717_D11BE2G5G6G_ID) ||
		(device == BCM6717_CHIP_ID))
		return 0;

	if ((device == BCM6726_D11BE_ID) || (device == BCM6726_D11BE2G_ID) ||
		(device == BCM6726_D11BE5G_ID) || (device == BCM6726_D11BE6G_ID) ||
		(device == BCM6726_D11BE2G6G_ID) || (device == BCM6726_D11BE5G6G_ID) ||
		(device == BCM6726_D11BE2G5G6G_ID) ||
		(device == BCM6726_CHIP_ID) || (device == BCM6716_CHIP_ID))
		return 0;
	if ((device == BCM43741_CHIP_ID) || (device == BCM43721_CHIP_ID) ||
		(device == BCM43740_CHIP_ID) || (device == BCM43720_CHIP_ID))
		return 0;
...
dhdpcie_cc_nvmshadow(dhd_bus_t *bus, struct bcmstrbuf *b)
{
	...
	/* Check ChipID */
	if ((si_chipid(bus->sih) != BCM4350_CHIP_ID) && !BCM4345_CHIP(si_chipid(bus->sih))) {
		DHD_ERROR(("%s: cc_nvmdump cmd. supported for 4350/4345 only\n",
			__FUNCTION__));
		return BCME_UNSUPPORTED;
	}

	/* Check if UWIRE_PRESENT in SpromCtrl(0x190 in ChipCommon Regs) is set */
	if (chipcregs->sromcontrol & UWIRE_PRESENT) {
		/* SPROM Size: 1Kbits (0x0), 4Kbits (0x1), 16Kbits(0x2) */
		sprom_size = (1 << (2 * ((chipcregs->sromcontrol & UWIRE_SIZE_MASK)
					>> UWIRE_SIZE_SHIFT))) * 1024;
	...
	if (((chipcregs->sromcontrol & UWIRE_PRESENT) == 0) &&
	...
	if ((chipcregs->sromcontrol & SRC_OTPSEL) &&
		...
		(chipcregs->sromcontrol & UWIRE_PRESENT)) {
...
#define DHD_BTRACE_BUFSIZE_BYTES_MAX	(10 * 1024 * 1024)	/* For buffer size sanity check */
#define DHD_BTRACE_POLL_SLOW_MS		200			/* Polling interval if disabled */
#define DHD_BTRACE_POLL_FAST_MS		20			/* Polling interval if enabled */

/**
 * Initialize binary tracing.
 *
 * @param bus		Pointer to bus layer.
 * @param osh		Pointer to OS handle.
 * @return		BCME_OK on success.
 */

static int
dhdpcie_btrace_init(dhd_bus_t *bus, osl_t *osh)
{
	dhd_btrace_t *trace = &bus->btrace;

	memset(trace, 0, sizeof(dhd_btrace_t));
	bus->dhd->btrace_enable = btrace_enable;	/* Copy from module param */

	return BCME_OK;
}

/**
 * Deinitialize binary tracing.
 *
 * @param bus		Pointer to bus layer.
 * @param osh		Pointer to OS handle.
 */
...
static void
dhdpcie_btrace_deinit(dhd_bus_t *bus, osl_t *osh)
{
	dhd_btrace_t *trace = &bus->btrace;

	if (trace != NULL && trace->local_buf != NULL) {
		MFREE(osh, trace->local_buf, trace->buf_size);
		trace->local_buf = NULL;
	}
}

/**
 * Consume binary tracing data from shared buffer.
 *
 * Copies tracing data from shared memory into the local buffer, then updates shared memory
 * read offset.
 *
 * @param bus		Pointer to bus layer.
 * @param trace		Pointer to binary tracing context.
 * @param trace_ipc	Pointer to copy of shared tracing area.
 * @return		Number of bytes consumed.
 */

static uint
dhdpcie_btrace_read_buffer(dhd_bus_t *bus, dhd_btrace_t *trace, btrace_ipc_t *trace_ipc)
{
	uint rd_offset = ltoh32(trace_ipc->rd_offset);
	uint wr_offset = ltoh32(trace_ipc->wr_offset);
	uint available, part;

	/* Sanity check on read/write offsets */
	ASSERT(rd_offset < trace->buf_size);
	ASSERT(wr_offset < trace->buf_size);

	available = (rd_offset <= wr_offset) ?
		(wr_offset - rd_offset) : trace->buf_size - (rd_offset - wr_offset);

	if (available != 0 && available <= trace->buf_size) {
		part = MIN(trace->buf_size - rd_offset, available);
		dhdpcie_bus_read_sysmem(bus, trace->remote_buf + rd_offset,
			(uint8*)trace->local_buf, part);
		if (part != available) {
			dhdpcie_bus_read_sysmem(bus, trace->remote_buf,
				(uint8*)trace->local_buf + part, available - part);
		}

		rd_offset = (rd_offset + available) % trace->buf_size;
		trace_ipc->rd_offset = htol32(rd_offset);

		/* Update trace IPC data */
		dhdpcie_bus_write_sysmem(bus,
			bus->btrace_daddr32 + OFFSETOF(btrace_ipc_t, rd_offset),
			(uint8*)&trace_ipc->rd_offset, sizeof(uint16));
...
/**
 * Process binary tracing data if available.
 *
 * @param bus		Pointer to bus layer.
 * @return		BCME_OK on success.
 */
...
static int
dhdpcie_btrace_process(dhd_bus_t *bus)
{
	dhd_btrace_t *trace = &bus->btrace;
	btrace_ipc_t trace_ipc = { 0 };
	uint32 now;
	bool truncate = FALSE, enabled;
	uint read;
	int ret;

	if (bus->btrace_daddr32 == 0) {
		return BCME_OK;
	}
	...
	enabled = (trace->flags_h2d & BTRACE_IPC_H2D_FLAGS_ENABLED) != 0;
	if (bus->dhd->btrace_enable ^ enabled) {
		/* Signal enabled state to the dongle */
		trace->flags_h2d = (bus->dhd->btrace_enable ? BTRACE_IPC_H2D_FLAGS_ENABLED : 0);
		dhdpcie_bus_write_sysmem(bus,
			bus->btrace_daddr32 + OFFSETOF(btrace_ipc_t, flags_h2d),
			&trace->flags_h2d, sizeof(uint8));
	}

	if (bus->dhd->btrace_enable == FALSE) {
		return BCME_OK;
	}

	now = OSL_SYSUPTIME();
	if ((int32)(now - trace->process_last_ts) < trace->process_interval) {
		return BCME_OK;
	}
	trace->process_last_ts = now;
	trace->process_interval = DHD_BTRACE_POLL_SLOW_MS;

	/* Get trace IPC data */
	dhdpcie_bus_read_sysmem(bus, bus->btrace_daddr32, (uint8*)&trace_ipc, sizeof(btrace_ipc_t));

	/* Check if trace IPC is initialized */
	if ((trace_ipc.flags_d2h & BTRACE_IPC_D2H_FLAGS_INITIALIZED) == 0) {
		return BCME_OK;
	}

	/* Do lazy initialization if needed */
	if (trace->local_buf == NULL) {
		/* Get and validate remote buffer size */
		trace->buf_size = ltoh32(trace_ipc.buf_size);
		if (trace->buf_size == 0 || trace->buf_size > DHD_BTRACE_BUFSIZE_BYTES_MAX) {
			DHD_ERROR(("btrace: invalid bufsize (%u)\n", trace->buf_size));
			bus->dhd->btrace_enable = FALSE;
			return BCME_BADOPTION;
		}

		/* Allocate local buffer */
		trace->local_buf = MALLOC(bus->dhd->osh, trace->buf_size);
		if (trace->local_buf == NULL) {
			DHD_ERROR(("btrace: no memory (%u)\n", trace->buf_size));
			bus->dhd->btrace_enable = FALSE;
			return BCME_NOMEM;
		}
	...
	if ((trace_ipc.flags_d2h & BTRACE_IPC_D2H_FLAGS_ENABLED) == 0) {
		return BCME_OK;
	}

	/* Store location of trace buffer */
	if (trace->remote_buf == 0) {
		trace->remote_buf = ltoh32((uint32)trace_ipc.buf);
		if (trace->remote_buf == 0) {
			DHD_ERROR(("btrace: no remote buffer\n"));
			bus->dhd->btrace_enable = FALSE;
			return BCME_BADOPTION;
		}
	}

	ASSERT(trace->local_buf != NULL);
	ASSERT(trace->buf_size == ltoh32(trace_ipc.buf_size));
	ASSERT(trace->remote_buf == ltoh32((uint32)trace_ipc.buf));

	trace->process_interval = DHD_BTRACE_POLL_FAST_MS;

	/* Read and process trace buffer */
	read = dhdpcie_btrace_read_buffer(bus, trace, &trace_ipc);
	if (read != 0) {
		/* Monitor changes in trace sequence number for implementing trace file
		 * truncation and rotation
		 */
		if (trace->trace_seq != trace_ipc.trace_seq) {
			uint8 file_seq = 0;
			trace->trace_seq = trace_ipc.trace_seq;
			if (trace_ipc.flags_d2h & BTRACE_IPC_D2H_FLAGS_ROTATING_TRACE) {
				file_seq = trace_ipc.file_seq;
			}
			snprintf(trace->filename, sizeof(trace->filename)-1,
				BTRACE_FILE_NAME, bus->dhd->unit, file_seq);
			DHD_INFO(("btrace: opening %s\n", trace->filename));
			truncate = TRUE;
		}

		/* Write trace data to file */
		ret = btrace_write_file(bus->dhd,
			trace->local_buf, read, trace->filename, truncate);
		if (ret != BCME_OK) {
			bus->dhd->btrace_enable = FALSE;
			return ret;
		}
...
uint32
dhd_bus_max_d2h_rings(struct dhd_bus *bus)
{
	return bus->max_d2h_rings;
}

uint32
dhd_bus_max_h2d_dynamic_rings(struct dhd_bus *bus)
{
	return bus->max_h2d_dynamic_rings;
}

uint32
dhd_bus_max_d2h_dynamic_rings(struct dhd_bus *bus)
{
	return bus->max_d2h_dynamic_rings;
}
...
hnd_hw_coherent_disable(dhd_bus_t *bus)
{
	...
	if (BCM6715_CHIP(si_chipid(bus->sih)) || BCM6717_CHIP(si_chipid(bus->sih)) ||
		BCM6726_CHIP(si_chipid(bus->sih))) {
		OSL_PCI_WRITE_CONFIG(bus->osh, PCI_BAR0_WIN, 4,
			(SI_ENUM_BASE_PA(bus->sih) + SI_CCI500_S1_CTL_OFFSET));
	} else {
		OSL_PCI_WRITE_CONFIG(bus->osh, PCI_BAR0_WIN, 4,
			(SI_ENUM_BASE_PA(bus->sih) + SI_CCI400_S3_CTL_OFFSET));
	}
	regs = (void *)bus->regs;
...
static void
dhdpcie_hw_adjust(dhd_bus_t *bus)
{
	uint regval;
	uint regaddr;
	if (BCM6726_CHIP(si_chipid(bus->sih)))
	{
		/* RFP0_20712_XTAL7 */
		regaddr = 0x28014238;
		regval = 0xC722;
		si_backplane_access(bus->sih, regaddr, 2, &regval, false);
...
void
dhd_bus_intr_reenable(struct dhd_bus *bus)
{
	if (!bus->intr_enabled) {
		DHD_ERROR(("Re-enable bus interrupt\n"));
		dhdpcie_bus_intr_enable(bus);
	}
}
...
void
dhdpcie_bus_get_pcie_gen_lane(dhd_pub_t *dhd_pub, int *pcie_gen, int *pcie_lane)
{
	...
	if (bus->sih->buscoretype == PCIE2_CORE_ID) {
		*pcie_gen = 2;
		...
		v32 = OSL_PCI_READ_CONFIG(bus->osh, PCIECFGREG_LINKCAP, sizeof(uint32));
		*pcie_lane = BCM_GBF(v32, PCIECFGREG_LINKCAP_MW);

	} else if (bus->sih->buscoretype == PCIE_CORE_ID) {
		*pcie_gen = 1;

		v32 = OSL_PCI_READ_CONFIG(bus->osh, PCI_CFG_LINKCAP, sizeof(uint32));
		*pcie_lane = BCM_GBF(v32, PCI_CFG_LINKCAP_MW);

	} else {
		DHD_ERROR(("%s: dhd%d: Unsupported PCIE gen; buscoretype[%d]\n",
			__FUNCTION__, dhd_pub->unit, bus->sih->buscoretype));
		ASSERT(0);
	}

	DHD_INFO(("%s: dhd%d: pcie_gen %d pcie_lane %d\n", __FUNCTION__, dhd_pub->unit,
		*pcie_gen, *pcie_lane));
} /* dhd_bus_get_pcie_gen_lane() */

#ifdef MLO_IPC
haddr64_t
dhdpcie_bus_cfg_set_mlo_win(dhd_pub_t *dhd_pub)
{
	dhd_bus_t *bus = dhd_pub->bus;
	haddr64_t bar2_haddr64 = bus->bar2_haddr64;

	/* Set bar2_addr to map to si_emum_base */
	if (!HADDR64_IS_ZERO(bus->bar2_haddr64)) {
		OSL_PCI_WRITE_CONFIG(bus->osh, PCI_BAR2_WIN, 4,
			SI_ENUM_BASE_PA(bus->sih));
	}

#if defined(BCM_ROUTER_DHD)
	bar2_haddr64 = dhdpcie_get_bar2_win(bus);
...
struct pcie_reg_to_str {
	const char *fmt;
	uint32 offset;
};

/* PCIE CORE Registers */
struct pcie_reg_to_str pcie_cmn_regs[] = {
	{ "control       [0x%04x] : %08X\n", PCIE_CORE_REG_CONTROL},
	{ "iotatuts      [0x%04x] : %08X\n", PCIE_CORE_REG_IOSTATUS},
	{ "bitstatus     [0x%04x] : %08X\n", PCIE_CORE_REG_BITSTATUS},
	{ "goiosel       [0x%04x] : %08X\n", PCIE_CORE_REG_GPIO_SEL},
	{ "gpioout       [0x%04x] : %08X\n", PCIE_CORE_REG_GPIO_OUT_EN},
	{ "intstatus     [0x%04x] : %08X\n", PCIE_CORE_REG_INT_STATUS},
	{ "intmask       [0x%04x] : %08X\n", PCIE_CORE_REG_INT_MASK},
	{ "sbpciemb      [0x%04x] : %08X\n", PCIE_CORE_REG_SB_PCIE_MB},
	{ "errlog        [0x%04x] : %08X\n", PCIE_CORE_REG_ERRLOG},
	{ "erraddr       [0x%04x] : %08X\n", PCIE_CORE_REG_ERR_ADDR},
	{ "mb_intr       [0x%04x] : %08X\n", PCIE_CORE_REG_MB_INTR},
	{ "s2ptrans2     [0x%04x] : %08X\n", PCIE_CORE_REG_S2P_TRANS2},
	{ "s2ptrans2u    [0x%04x] : %08X\n", PCIE_CORE_REG_S2P_TRANS2_UPPER},
	{ "s2ptrans3     [0x%04x] : %08X\n", PCIE_CORE_REG_S2P_TRANS3},
	{ "s2ptrans3u    [0x%04x] : %08X\n", PCIE_CORE_REG_S2P_TRANS3_UPPER},
	{ "s2ptrans0     [0x%04x] : %08X\n", PCIE_CORE_REG_S2P_TRANS0},
	{ "s2ptrans1     [0x%04x] : %08X\n", PCIE_CORE_REG_S2P_TRANS1},
	{ "s2ptransbig   [0x%04x] : %08X\n", PCIE_CORE_REG_S2P_TRANS_BIG},
	{ "s2ptrans0u    [0x%04x] : %08X\n", PCIE_CORE_REG_S2P_TRANS0_UPPER},
	{ "s2ptrans1u    [0x%04x] : %08X\n", PCIE_CORE_REG_S2P_TRANS1_UPPER},
	{ "s2ptrans4     [0x%04x] : %08X\n", PCIE_CORE_REG_S2P_TRANS4},
	{ "s2ptrans4u    [0x%04x] : %08X\n", PCIE_CORE_REG_S2P_TRANS4_UPPER},
	{ "errheaderlog1 [0x%04x] : %08X\n", PCIE_CORE_REG_ERRORHEADERLOG1},
	{ "errheaderlog2 [0x%04x] : %08X\n", PCIE_CORE_REG_ERRORHEADERLOG2},
	{ "errheaderlog3 [0x%04x] : %08X\n", PCIE_CORE_REG_ERRORHEADERLOG3},
	{ "errheaderlog4 [0x%04x] : %08X\n", PCIE_CORE_REG_ERRORHEADERLOG4},
	{ "hmapvioaddrlo [0x%04x] : %08X\n", PCIE_CORE_REG_HMAPVIOLATIONADDRLOWER},
	{ "hmapvioaddrhi [0x%04x] : %08X\n", PCIE_CORE_REG_HMAPVIOLATIONADDRUPPER},
	{ "hmapvioinfo   [0x%04x] : %08X\n", PCIE_CORE_REG_HMAPVIOLATIONINFO},
	{ NULL, 0}
};

/* PCIE Config registers */
struct pcie_reg_to_str pcie_cmn_cfg_regs[] = {
	...
	{ "link_cap: %08X\n", PCIECFGREG_LINKCAP},
	{ "link_status_ctrl: %08X\n", PCIECFGREG_LINK_STATUS_CTRL},
	{ "link_status_ctrl2: %08X\n", PCIECFGREG_LINK_STATUS_CTRL2},
	{ "dev_ctrl: %08X\n", PCIECFGREG_DEV_STATUS_CTRL},
	{ "device_status_control_2: %08X\n", PCIECFGREG_DEV_STATUS_CTRL2},
	{ "adv_err_cap: %08X\n", PCIE_CFG_ADV_ERR_CAP},
	{ "uc_err_status: %08X\n", PCIE_CFG_UC_ERR_STS},
	{ "ucorr_err_mask: %08X\n", PCIE_CFG_UC_ERR_MASK},
	{ "ucorr_err_sevr: %08X\n", PCIE_CFG_UNCOR_ERR_SERV},
	{ "corr_err_status: %08X\n", PCIE_CFG_CORR_ERR_STS},
	{ "corr_err_mask: %08X\n", PCIE_CFG_CORR_ERR_MASK},
	{ "adv_err_cap_control: %08X\n", PCIE_CFG_ADV_ERR_CTRL},
	{ "header_log1: %08X\n", PCIE_CFG_HDR_LOG1},
	{ "header_log2: %08X\n", PCIE_CFG_HDR_LOG2},
	{ "header_log3: %08X\n", PCIE_CFG_HDR_LOG3},
	{ "header_log4: %08X\n", PCIE_CFG_HDR_LOG4},
	{ "rbar_ctrl: %08X\n", PCIECFGREG_RBAR_CTRL},
	{ "PML1_sub_capID: %08X\n", PCIE_CFG_PML1_SUB_CAP_ID},
	{ "PML1_sub_Cap_reg: %08X\n", PCIE_CFG_PML1_SUB_CAP_REG},
	{ "PML1_sub_control1: %08X\n",	PCIECFGREG_PML1_SUB_CTRL1},
	{ "PML1_sub_control2: %08X\n",	PCIECFGREG_PML1_SUB_CTRL2},
	{ "reg_bar2_config: %08X\n", PCIECFGREG_REG_BAR2_CONFIG},
	{ "reg_bar3_config: %08X\n", PCIECFGREG_REG_BAR3_CONFIG},
	{ "tl_control_0: %08X\n", PCIECFGREG_TL_CTRL0},
	{ "tl_control_5: %08X\n", PCIE_CFG_TL_CTRL_5},
	{ "pdl_ctrl1: %08X\n", PCIECFGREG_PDL_CTRL1},
	{ "pdl_iddq: %08X\n", PCIECFGREG_PDL_IDDQ},
	{ "reg_phy_ctrl7: %08X\n", PCIECFGREG_REG_PHY_CTL7},
	{ NULL, 0}
};

/* PCIE EXT Config registers */
struct pcie_reg_to_str pcie_ext_cfg_regs[] = {
	{ "phy_err_attn_vec: %08X\n", PCIE_CFG_PHY_ERR_ATT_VEC},
	{ "phy_err_attn_mask: %08X\n", PCIE_CFG_PHY_ERR_ATT_MASK},
	{ NULL, 0}
};

/* PCIE RC AER and TL (error logging) registers */
struct pcie_reg_to_str pcie_rc_aer_tl_regs[] = {
	{ "adv_err_cap: %08X\n", PCIE_CFG_ADV_ERR_CAP},
	{ "uc_err_status: %08X\n", PCIE_CFG_UC_ERR_STS},
	{ "ucorr_err_mask: %08X\n", PCIE_CFG_UC_ERR_MASK},
	{ "ucorr_err_sevr: %08X\n", PCIE_CFG_UNCOR_ERR_SERV},
	...
	{ "header_log3: %08X\n", PCIE_CFG_HDR_LOG3},
	{ "header_log4: %08X\n", PCIE_CFG_HDR_LOG4},
	{ "root_errror_command: %08X\n", PCIE_CFG_ROOT_ERROR_COMMAND},
	{ "root_error_status: %08X\n", PCIE_CFG_ROOT_ERROR_STATUS},
	{ "root_err_id: %08X\n", PCIE_CFG_ROOT_ERROR_ID},
	{ "tl_control_5: %08X\n", PCIE_CFG_TL_CTRL_5},
	{ "rc_bad_addr_low: %08X\n", PCIE_CFG_RC_BAD_ADDRESS_LOW},
	{ "rc_bad_addr_high: %08X\n", PCIE_CFG_RC_BAD_ADDRESS_HIGH},
	{ NULL, 0}
};

static int32
dhdpcie_get_core_regval(dhd_bus_t *bus, uint regoffset)
{
	/* dhd iovar uses si_corereg() to read values from pcie core registers */
	return si_corereg(bus->sih, bus->sih->buscoreidx, regoffset, 0, 0);
}

int dhdpcie_dump_pcie_regs(dhd_pub_t *dhdp, char *dumpbuf, int buf_len)
{
	struct bcmstrbuf bstrbuf;
	int i = 0;
	...
	if (!dhdp->bus) {
		DHD_ERROR(("%s: dhdp->bus is null\n", __FUNCTION__));
		return BCME_ERROR;
	}

	bcm_binit(&bstrbuf, dumpbuf, buf_len);

	bcm_bprintf(&bstrbuf, "PCIe CORE Registers\n");
	while (pcie_cmn_regs[i].fmt) {
		if (!(BCM6726_CHIP(si_chipid(dhdp->bus->sih))) &&
			pcie_cmn_regs[i].offset == PCIE_CORE_REG_S2P_TRANS4) {
			break;
		}
		bcm_bprintf(&bstrbuf, pcie_cmn_regs[i].fmt,
			pcie_cmn_regs[i].offset,
			dhdpcie_get_core_regval(dhdp->bus, (uint)pcie_cmn_regs[i].offset));
		i++;
	}

	bcm_bprintf(&bstrbuf, "\nPCIe CFG Registers\n");
	i = 0;
	while (pcie_cmn_cfg_regs[i].fmt) {
		/* dhdpcie_get_regval() is used in dhd iovar */
		bcm_bprintf(&bstrbuf, pcie_cmn_cfg_regs[i].fmt,
		        dhdpcie_get_regval(dhdp->bus, pcie_cmn_cfg_regs[i].offset));
		i++;
	}

	i = 0;
	while (pcie_ext_cfg_regs[i].fmt) {
		bcm_bprintf(&bstrbuf, pcie_ext_cfg_regs[i].fmt,
			dhdpcie_get_regval(dhdp->bus, pcie_ext_cfg_regs[i].offset));
		i++;
	}

	bcm_bprintf(&bstrbuf, "\nPCIe RC AER/TL Registers\n");

	pci_dev      = dhdp->bus->dev;
	root_bus     = pci_dev->bus;
	...
	i = 0;
	while (pcie_rc_aer_tl_regs[i].fmt) {
		/* dhdpcie_get_regval() is used in dhd iovar */
		bcm_bprintf(&bstrbuf, pcie_rc_aer_tl_regs[i].fmt,
		        dhdpcie_rc_get_regval(root_bus, pcie_rc_aer_tl_regs[i].offset));
		i++;
	}

	bcm_bprintf(&bstrbuf, "\nPCIe CFG Registers raw dump\n");
	bcm_bprintf(&bstrbuf, "[regoffset] : value\n");
	for (i = 0; i <= PCIE_CFG_PHY_ERR_ATT_MASK; i += 4) {
		bcm_bprintf(&bstrbuf, "[0x%04X] : %08X\n", i,
			dhdpcie_get_regval(dhdp->bus, i));
	}
...
void
dhd_bus_pcap_dump(dhd_pub_t *dhd, struct bcmstrbuf *strbuf)
{
	dhd_linux_pcap_t *pcap = &dhd->pcap;
#ifdef BCM_PCAP
	pcap_rx_stats_t *rxstats = &pcap->rxstats;
	uint32 msdus = 1;
#endif /* BCM_PCAP */

	bcm_bprintf(strbuf, "pcap_config 0x%x\n", dhd->pcap.pcap_config);
	bcm_bprintf(strbuf, "\ttotal_pkts_recd\t%u\n\tmgmt_pkts_recd\t%u\n\tmgmt_pkts_sent\t%u"
		   "\n\tmgmt_pkts_acked\t%u\n\tlocal_pkt_tx\t%u\n\tinv_pkttype_cpl\t%u\n",
		   pcap->tot_pkts_recd, pcap->mgmt_pkts_recd, pcap->mgmt_pkts_sent,
		   pcap->mgmt_pkts_acked, pcap->local_tx_pkts, pcap->inv_pkttype_cpl);

#ifdef BCM_PCAP
	bcm_bprintf(strbuf, "pcap rx-copy, poll %d\n", (pcap->pcap_poll_mode) ? 1 : 0);
	bcm_bprintf(strbuf, "\talloc     %8u tot       %8u drop_resrc %8u drop_list %8u\n",
		rxstats->rxpkt_alloc, rxstats->rxpkt_tot, rxstats->rxpkt_drop_alloc,
		rxstats->rxpkt_drop_list);
	bcm_bprintf(strbuf, "\tmerge     %8u pkts_fwrd %8u pkts_enqd  %8u flush     %8u\n",
		rxstats->rxpkt_merge, rxstats->rxpkt_fwrd, rxstats->rxpkt_enqd,
		rxstats->rxpkt_drop_flush);

	bcm_bprintf(strbuf, "\thdr_rcvd  %8u phy_rcfd  %8u hdr_full   %8u phy_full  %8u\n",
		rxstats->hdr_rcvd, rxstats->phy_rcvd, rxstats->hdr_full, rxstats->phy_full);
	bcm_bprintf(strbuf, "\tretry_hdr %8u drop_hdr  %8u retry_phy  %8u drop_phy  %8u\n",
		rxstats->rxpkt_retry_hdr, rxstats->rxpkt_drop_hdr, rxstats->rxpkt_retry_phy,
		rxstats->rxpkt_drop_phy);
	if (rxstats->msdu_cnt != 0) {
		msdus = rxstats->msdu_cnt;
	}
	bcm_bprintf(strbuf, "\tmsdu_cnt  %8u msdu_tot  %8u msdu_bytes %8u\n",
		rxstats->msdu_cnt, rxstats->msdu_tot, (rxstats->msdu_bytes/msdus));

	/* read on clear for rx-copy stats */
	bzero(rxstats, sizeof(pcap_rx_stats_t));
#endif /* BCM_PCAP */
} /* dhd_bus_pcap_dump() */
