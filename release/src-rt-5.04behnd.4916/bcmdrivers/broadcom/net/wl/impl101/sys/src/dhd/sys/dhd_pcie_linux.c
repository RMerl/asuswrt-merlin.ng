 ...
 *
 * $Id: dhd_pcie_linux.c 830014 2023-09-12 19:03:45Z $
 */
...
#define HADDR64_TO_PHYS_ADDR(ha64, pa) HADDR64_TO_U64(ha64, pa)
#define PHYS_ADDR_TO_HADDR64(pa, ha64) HADDR64_FROM_U64(ha64, pa)
#else /* !CONFIG_PHYS_ADDR_T_64BIT */
#define HADDR64_TO_PHYS_ADDR(ha64, pa) pa = HADDR64_LO(ha64)
#define PHYS_ADDR_TO_HADDR64(pa, ha64) HADDR64_LO_SET(ha64, pa);
...
{
	pci_unregister_driver(&dhdpcie_driver); /* calls dhd_detach */
}

#ifdef MLO_IPC
/* For MLO bilingual mode testing. Attach first interface as FD. */
static bool skip_enumeration = FALSE;
static int mlo_bl_en = 0;
module_param(mlo_bl_en, int, 0644);
#endif /* MLO_IPC */
...
{
#ifdef MLO_IPC
	/* For bilingual mode testig */
	if (mlo_bl_en && skip_enumeration) {
		printf("PCI_PROBE: [Bilingual MLO]: Skip bus %X, slot %X,vendor %X, device %X\n",
...
dhdpcie_pci_remove(struct pci_dev *pdev)
{
	...
	DHD_LOCK(bus->dhd);
	(void)dhd_wl_ioctl_cmd(bus->dhd, WLC_DOWN, NULL, 0, FALSE, 0);
	DHD_UNLOCK(bus->dhd);
...
#if defined(MLO_IPC)
		HADDR64_ZERO(dhdpcie_info->bar2_haddr64);

		/* Bar-2 mapped address */
		bar2_addr = pci_resource_start(pdev, 4);

		/* read Bar-2 mapped memory range */
		bar2_size = pci_resource_len(pdev, 4);

		if ((bar2_size == 0) || (bar2_addr == 0)) {
			printf("%s: BAR2 Not enabled for this device size(%ld),"
				" addr(0x"PRINTF_RESOURCE")\n",
				__FUNCTION__, bar2_size, bar2_addr);
			goto no_bar2_done;
		}

		if (bar2_size < PCIE_IPC_MLO_BAR_SIZE) {
			printf("%s: BAR2 size 0x%lx is too small, require 0x%x\n",
				__FUNCTION__, bar2_size, PCIE_IPC_MLO_BAR_SIZE);
			goto no_bar2_done;
		}

		/* no BAR2 ioremap for host cpu, it is for MLC IPC over PCIe */
		PHYS_ADDR_TO_HADDR64(bar2_addr, dhdpcie_info->bar2_haddr64);
...
dhdpcie_init(struct pci_dev *pdev)
{
	...
	do {
		...
		if (dhdpcie_prepare_pcie_ep(osh, pdev, TRUE) != BCME_OK) {
		...
		bus = dhdpcie_bus_attach(osh, dhdpcie_info->regs, dhdpcie_info->tcm,
			dhdpcie_info->bar2_haddr64, pdev);
		if (!bus) {
...
#if defined(DHD_BHM)
		if (dhdpcie_bhm_mem_alloc(bus) != BCME_OK) {
			DHD_ERROR(("%s: dhdpcie_bhm_mem_alloc() failed\n", __FUNCTION__));
			break;
		}
#endif /* DHD_BHM */
...
#if defined(MLO_IPC) && (defined(OEM_ANDROID) || defined(BCM_ROUTER))
		if (dhd_download_fw_on_driverload)
			dhd_mlo_ipc_start(bus->dhd);
#endif
...
dhdpcie_irq_enabled(dhd_bus_t *bus)
{
	...
	return TRUE;
#endif /* KERNEL_VERSION >= 3.0.0 && KERNEL_VERSION < 5.11.0 */
}
...
#if defined(MLO_IPC) && defined(BCM_ROUTER)
int
dhdpcie_bus_bridge_init(dhd_pub_t * dhd_pub)
{
	pcie_ipc_mlo_t  * pcie_ipc_mlo;
	struct pci_dev  * pci_dev;
	uint8             ap_unit, mlo_ap_bm;
	int               iter;
	haddr64_t         map_haddr64;
	uint32            map_size;
	...
	pci_dev      = dhd_pub->bus->dev;

	/* Config RC/ UBUS bridge */
	for (iter = 0; iter < PCIE_IPC_AP_UNIT_MAX; iter++) {
		DHD_INFO(("%s: dhd%d: iter[%d]: ap_unit[%d] mlo_ap_bm[%x]\n",
			__FUNCTION__, ap_unit, iter, ap_unit, mlo_ap_bm));
		if ((iter == ap_unit) || ((mlo_ap_bm & (1 << iter)) == 0)) {
			/* Don't setup bridge for self and non MLO AP */
			continue;
		}

		/* Get Bar-2 mapped address and size */
		pcie_ipc_mlo_ap = &pcie_ipc_mlo->ap[iter];
		map_haddr64 = pcie_ipc_mlo_ap->bar2_haddr64;
		map_size = (uint32)dhd_pub->bus->bar2_size;
		if (HADDR64_IS_ZERO(map_haddr64)) {
			DHD_ERROR(("%s: dhd%d: invalid BAR2 address\n",
				__FUNCTION__, ap_unit));
			continue;
		}

		if (map_size < PCIE_IPC_MLO_BAR_SIZE) {
			DHD_ERROR(("%s: dhd%d: invalid BAR2 size 0x%x\n",
				__FUNCTION__, ap_unit, map_size));
			continue;
		}

		HADDR64_TO_PHYS_ADDR(map_haddr64, map_addr);

		printk("%s: ipc_mlo 0x%px, ipc_mlo_ap 0x%px\n", __FUNCTION__,
			pcie_ipc_mlo, pcie_ipc_mlo_ap);

		/* Map address as incoming window for this AP */
	    /* Use any available BAR */
		rc = bcm_pcie_config_bar_addr(pci_dev, 0, map_addr, map_size);

		if (rc != 0) {
			if (rc == -ENOSPC) {
				DHD_ERROR(("%s: dhd%d: RC UBUS BAR is not enough\n",
					__FUNCTION__, ap_unit));
			} else {
				DHD_ERROR(("%s: dhd%d: Failed to config pcie bar address\n",
					__FUNCTION__, ap_unit));
			}
		}
	}

#else /* !BCM_PCIE_BARCFG */

/* Config RC/ UBUS BARx bridge */
/* Support for 8GB DDR memory size */
/* BAR registers assigment for different DDR sizes.
 * <=2GB: BAR1 (<=  2GB)
 *   4GB: BAR1 (0 - 2GB), BAR3 (2 - 4GB)
 *   8GB: BAR1 (0 - 2GB), BAR3 (2 - 6GB), BAR4 (6 -  8GB)
 *  16GB: BAR1 (0 - 2GB), BAR3 (2 - 6GB), BAR4 (6 - 14GB), BAR5 (14 - 16GB)
 *  32GB: BAR1 (0 - 2GB), BAR3 (2 - 6GB), BAR4 (6 - 14GB), BAR5 (14 - 30GB), BAR6 (30 - 32GB)
 */
/*
 * The 6813 has 6 Root Complex BARs.
 * RC BAR 1 --> DDR up to 2GB
 * RC BAR 2 --> Runner and any other register space we want to expose
 * RC BAR 3 --> DDR beyond 2GB
 * RC BAR 4 --> DDR beyond 0 - 2GB (used by above 8GB memory case)
 * RC BAR 5 --> We can use this for MLO destination #1
 * RC BAR 6 --> We can use this for MLO destination #2
 *
 * If we need this to work for 4912 which only has 3 Root Complex BARs,
 * we need to re-think. Perhaps squish it all into a large RC BAR 2.
 *
 * Note: Following piece of code does not support 4912.
*/
#define PCI_RC_UBUS_BAR_MAX	2	// 3 MLO APs maximum
#define PCI_BUS_WRITE_CONFIG(rb, off, val) \
	pci_bus_write_config_dword((rb), 0, (off), (val))

	struct pci_bus  * root_bus;
	uint32            rev, major, minor;
	uint8             bar_cnt;
	uint32            match_size;

	struct pci_rc_ubus_bar_offset {
		uint16 rc_lo;
		uint16 rc_hi;
		uint16 ubus_lo;
		uint16 ubus_hi;
	...
	/*
	 * CMWIFI 33940/3392 have 10 RC IB BARs.
	 * BAR7 is in use by GDO/DoR outside of linux/devtree
	 * Use BAR8, BAR9 for MLO destination #1, #2.
	 * TBD: remove programming once BAR configuration is pushed to device tree
	 */
	const struct pci_rc_ubus_bar_offset bar_offset[PCI_RC_UBUS_BAR_MAX] = {
		{0x40F4,  0x40F8, 0x412C, 0x4130},  // BAR8
		{0x40FC,  0x4100, 0x4134, 0x4138}   // BAR9
	};
#else /* !CMWIFI_33940 */
	// Start from BAR5
	const struct pci_rc_ubus_bar_offset bar_offset[PCI_RC_UBUS_BAR_MAX] = {
		{0x40D4,  0x40D8, 0x40EC, 0x40F0},  // BAR5
		{0x40DC,  0x40E0, 0x40F4, 0x40F8}   // BAR6
	};
#endif /* CMWIFI_33940 */
	...
	if (pci_bus_read_config_dword(root_bus, 0, 0x406C, &rev)) {
		DHD_ERROR(("%s: dhd%d: Failed to get pci revision informatonpci_dev\n",
			__FUNCTION__, ap_unit));
	    return BCME_ERROR;
	}

	/* Only support after bcm6813_a0 which has 6 RC BARs */
	major = (rev & 0x0000FF00) >> 8;
	minor = (rev & 0x000000FF);
	if (major < 4 || (major == 4 && minor < 4)) {
		DHD_ERROR(("%s: dhd%d: Unsupport PCIe RC reversion %d.%d\n",
			__FUNCTION__, ap_unit, major, minor));
		return BCME_ERROR;
	}
	...
	for (iter = 0; iter < PCIE_IPC_AP_UNIT_MAX; iter++) {
		...
		if (bar_cnt >= PCI_RC_UBUS_BAR_MAX) {
			DHD_ERROR(("%s: dhd%d: RC UBUS BAR is not enough\n",
				__FUNCTION__, ap_unit));
			continue;
		}
		...
		/*
		 * Configure MISC_RC_BAR5..6_CONFIG register with
		 * BAR szie range from map_haddr64
		 */
		/*
		 * 0x01 ...  64KB (MATCH_ADDRESS[63:16] used)
		 * 0x02 ... 128KB (MATCH_ADDRESS[63:17] used)
		 * 0x03 ... 256KB (MATCH_ADDRESS[63:18] used)
		 * 0x04 ... 512KB (MATCH_ADDRESS[63:19] used)
		 * 0x05 ...   1MB (MATCH_ADDRESS[63:20] used)
		 * 0x06 ...   2MB (MATCH_ADDRESS[63:21] used)
		 * 0x07 ...   4MB (MATCH_ADDRESS[63:22] used)
		 * 0x08 ...   8MB (MATCH_ADDRESS[63:23] used)
		 * 0x09 ...  16MB (MATCH_ADDRESS[63:24] used)
		 * 0x0a ...  32MB (MATCH_ADDRESS[63:25] used)
		 * 0x0b ...  64MB (MATCH_ADDRESS[63:26] used)
		 * 0x0c ... 128MB (MATCH_ADDRESS[63:27] used)
		 * 0x0d ... 256MB (MATCH_ADDRESS[63:28] used)
		 */
		/* PCIE_IPC_MLO_BAR_SIZE <= map_size */
		map_size = (map_size >> 16); // rebase to 64KB
		match_size = 0;
		while (map_size) {
			match_size++;
			map_size = (map_size >> 1);
		...
		PCI_BUS_WRITE_CONFIG(root_bus, bar_offset[bar_cnt].rc_lo,
			(HADDR64_LO(map_haddr64) | match_size));
		PCI_BUS_WRITE_CONFIG(root_bus, bar_offset[bar_cnt].rc_hi,
			HADDR64_HI(map_haddr64));

		/*
		 * Configure MISC_UBUS_BAR5..6_CONFIG_REMAP register with
		 * Enable UBUS access to BAR2 address space and offset(map_haddr64)
		 */
		PCI_BUS_WRITE_CONFIG(root_bus, bar_offset[bar_cnt].ubus_lo,
			(HADDR64_LO(map_haddr64) | 0x01)); // 0x1: enable
		PCI_BUS_WRITE_CONFIG(root_bus, bar_offset[bar_cnt].ubus_hi,
			HADDR64_HI(map_haddr64));

		bar_cnt++; // next
	}
#endif /* !BCM_PCIE_BARCFG */

	return BCME_OK;
}

void
dhdpcie_bus_bridge_deinit(dhd_pub_t * dhd_pub)
{
#if defined(BCM_PCIE_BARCFG)
	pcie_ipc_mlo_t  * pcie_ipc_mlo;
	struct pci_dev  * pci_dev;
	...
	for (iter = 0; iter < PCIE_IPC_AP_UNIT_MAX; iter++) {
		if (iter == ap_unit) {
			/* Don't setup bridge for self */
			DHD_INFO(("%s: dhd%d: Skipping ap_unit in iter[%d]\n",
				__FUNCTION__, ap_unit, iter));
			continue;
		}

		/*
		 * Checking BAR mapped address aginst 0 for non-MLO AP's instead of
		 * mlo_ipc_get_ap_bm() as this bitmap only shows active MLO AP's
		 */
		...
		map_size = (uint32)dhd_pub->bus->bar2_size;
		if (HADDR64_IS_ZERO(map_haddr64)) {
			DHD_INFO(("%s: dhd%d: skipping non-mlo AP [%d]\n",
				__FUNCTION__, ap_unit, iter));
			continue;
		}

		/* Invalidate unsuppoted sizes */
		if (map_size < PCIE_IPC_MLO_BAR_SIZE) {
			DHD_ERROR(("%s: dhd%d: invalid BAR2 size 0x%x\n",
				__FUNCTION__, ap_unit, map_size));
			continue;
		...
		/* setting size=0 will unconfigure the BAR */
		map_size = 0;
		HADDR64_TO_PHYS_ADDR(map_haddr64, map_addr);
		bcm_pcie_config_bar_addr(pci_dev, 0, map_addr, map_size);
	}
#else /* !BCM_PCIE_BARCFG */
	/* Not implemented */
#endif /* !BCM_PCIE_BARCFG */
...
haddr64_t
dhdpcie_get_bar2_win(dhd_bus_t *bus)
{
	haddr64_t bar2_win;

	ASSERT(bus && bus->dev);

	HADDR64_ZERO(bar2_win);

	if (bus) {
		bar2_win = bus->bar2_haddr64;

#if defined(BCM_PCIE_BARCFG)
		{
			phys_addr_t bar2_map_addr;

			HADDR64_TO_PHYS_ADDR(bar2_win, bar2_map_addr);

			/* bus->bar2_size is not populated yet, use '0' for default size */
			bar2_map_addr = bcm_pcie_map_bar_addr(bus->dev, bar2_map_addr,
				bus->bar2_size);

			if (bar2_map_addr != BCM_PCIE_MAP_ADDR_INVALID) {
				PHYS_ADDR_TO_HADDR64(bar2_map_addr, bar2_win);
			} else {
				DHD_ERROR(("%s: pcie_map_bar_addr returned invalid address\n",
					__FUNCTION__));
			}
		}
#endif /* !BCM_PCIE_BARCFG */
