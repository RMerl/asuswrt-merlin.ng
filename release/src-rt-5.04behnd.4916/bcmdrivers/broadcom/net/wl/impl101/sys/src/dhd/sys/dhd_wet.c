 ...
 *
 * $Id: dhd_wet.c 821234 2023-02-06 14:16:52Z $
 */
...
#define STA_OVERWRITE_INC(sta, ipver) \
	do { sta->overwrite_pos[ipver] ++; \
		if (sta->overwrite_pos[ipver] == wet_maxip_per_sta) \
			sta->overwrite_pos[ipver] = 0; \
	} while (0)

#define STA_OVERWRITE_DEC(sta, ipver) \
	do { if (sta->overwrite_pos[ipver] > 0) \
			sta->overwrite_pos[ipver] --; \
	} while (0)

typedef struct wet_iphash_node wet_iphash_node_t;
struct wet_iphash_node {
	wet_iphash_node_t *next;
	wet_sta_t *sta;
};
...
#define WET_STA_IP_SAME_HASH(ip, ipver, hash)	\
			(((ip)[iphashoff[ipver]]&(WET_STA_HASH_SIZE-1)) == hash) /* hash by IP */
...
struct dhd_wet_info {
	...
	wet_sta_t *stafree;
	/* Used sta hash by IP */
	wet_iphash_node_t *stahash_ip[IPTYPE][WET_STA_HASH_SIZE];
	wet_iphash_node_t *iphash_nodefree;
	wet_sta_iplink_t *iplink_nodefree;
...
static wet_sta_iplink_t * wet_iplink_node_alloc(dhd_wet_info_t *weth);
static inline void wet_iplink_node_free(dhd_wet_info_t *weth, wet_sta_iplink_t *node);
static wet_iphash_node_t * wet_iphash_node_alloc(dhd_wet_info_t *weth);
static inline void wet_iphash_node_free(dhd_wet_info_t *weth, wet_iphash_node_t *node);
...
static void wet_free_resources(dhd_wet_info_t *weth);
...
/* check if ip is in sta's iplink */
static inline int
wet_sta_hash_count(wet_sta_t *sta, uint8 ipver, int hash)
{
	wet_sta_iplink_t *p_sta = (sta->ip_link[ipver]);
	int count = WET_STA_IP_SAME_HASH(sta->ip[ipver], ipver, hash);
	while (p_sta) {
		if (WET_STA_IP_SAME_HASH(p_sta->ip, ipver, hash))
			count++;
		p_sta = p_sta->next;
	}
	return count;
...
static inline int
insert_sta_into_iphash(dhd_wet_info_t *weth, wet_sta_t *sta, uint8 *iaddr, uint ipver)
{
	wet_iphash_node_t  *node = NULL;
	int hash = WET_STA_HASH_IP(iaddr, ipver);
	node = weth->stahash_ip[ipver][hash];
	while (node && node->sta != sta)  {
		node = node->next;
	}
	if (node == NULL) {
		node = wet_iphash_node_alloc(weth);
		if (node) {
			node->sta = sta;
			node->next = weth->stahash_ip[ipver][hash];
			weth->stahash_ip[ipver][hash] = node;
		}
	}
	return hash;
}

static wet_iphash_node_t *
wet_iphash_node_alloc(dhd_wet_info_t *weth)
{
	wet_iphash_node_t *node = weth->iphash_nodefree;
	if (node) {
		weth->iphash_nodefree = node->next;
		node->next = NULL;
		node->sta = NULL;
	} else {
		node = MALLOCZ(weth->pub->osh, sizeof(wet_iphash_node_t));
	}
	return node;
}

static inline void
wet_iphash_node_free(dhd_wet_info_t *weth, wet_iphash_node_t *node)
{
	node->next = weth->iphash_nodefree;
	weth->iphash_nodefree = node;
}

static inline wet_sta_iplink_t *
wet_iplink_node_alloc(dhd_wet_info_t *weth)
{
	wet_sta_iplink_t *node = weth->iplink_nodefree;
	if (node) {
		weth->iplink_nodefree = node->next;
		node->next = NULL;
		memset(node->ip, 0, IPV6_ADDR_LEN);
	} else {
		node = MALLOCZ(weth->pub->osh, sizeof(wet_sta_iplink_t));
	}
	return node;
}

static inline void
wet_iplink_node_free(dhd_wet_info_t *weth, wet_sta_iplink_t *node)
{
	node->next = weth->iplink_nodefree;
	weth->iplink_nodefree = node;
}

static inline void
wet_free_iphash(dhd_wet_info_t *weth)
{
	wet_iphash_node_t *node = weth->iphash_nodefree, *next_node;
	while (node) {
		next_node = node->next;
		MFREE(weth->pub->osh, node, sizeof(wet_iphash_node_t));
		node = next_node;
	}
	weth->iphash_nodefree = NULL;
}

static inline void
wet_free_iplink(dhd_wet_info_t *weth)
{
	wet_sta_iplink_t *node = weth->iplink_nodefree, *next_node;
	while (node) {
		next_node = node->next;
		MFREE(weth->pub->osh, node, sizeof(wet_sta_iplink_t));
		node = next_node;
	}
	weth->iplink_nodefree = NULL;
}

static inline void
remove_sta_from_iphash(dhd_wet_info_t *weth, wet_sta_t *sta, uint8 *iaddr, uint ipver)
{
	wet_iphash_node_t *prev_node = NULL, *node;
	int hash = WET_STA_HASH_IP(iaddr, ipver);
	int count;
	node = weth->stahash_ip[ipver][hash];
	while (node && node->sta != sta)  {
	...
	if (node) {
		count = wet_sta_hash_count(node->sta, ipver, hash);
		if (count == 1) {
			if (!prev_node)
				weth->stahash_ip[ipver][hash] = node->next;
			else
				prev_node->next = node->next;
			wet_iphash_node_free(weth, node);
		}
	}
}

/* add ip to sta, first to primary IP, if assigned, then to iplink */
static int
wet_sta_add_ip(dhd_wet_info_t *weth, wet_sta_t *sta, uint8 ipver,
	...
	if (((ipver == IPVER_4) && IPV4_ADDR_NULL(sta->ip[ipver])) ||
		...
		sta->cached_ip[ipver] = sta->ip[ipver];
		...
		if (wet_sta_ip_inlink(sta, ipver, iaddr)) {
			...
			if (sta->depth[ipver] < wet_maxip_per_sta-1) {
				/* no ip assigned to this sta and still allow more ips, then
				 * assign a new ipnode from iplink free list or a new one
				 * if no free node.
				 */
				wet_sta_iplink_t *p_iplink_node =  wet_iplink_node_alloc(weth);
				if (p_iplink_node == NULL) {
					DHD_ERROR(("allocate ip space error!\n"));
					return WET_STA_HASH_OK;
				}
				bcopy(iaddr, p_iplink_node->ip, IP_ADDR_LEN[ipver]);
				p_iplink_node->next = sta->ip_link[ipver];
				sta->ip_link[ipver] = p_iplink_node;
				sta->depth[ipver] += 1;
			} else {
				/* No more new ip accepted, need to overwrite existing ips */
				char *overwrite_ip;
				if (sta->overwrite_pos[ipver] == 0) {
					overwrite_ip = &(sta->ip[ipver][0]);
				} else {
					wet_sta_iplink_t *p_iplink_node = sta->ip_link[ipver];
					int  i = 1;
					for (; i < sta->overwrite_pos[ipver]; i++) {
						p_iplink_node = p_iplink_node->next;
					}
					overwrite_ip = p_iplink_node->ip;
				}
				/* remove old ip from iphash first */
				remove_sta_from_iphash(weth, sta, overwrite_ip, ipver);
				bcopy(iaddr, overwrite_ip, IP_ADDR_LEN[ipver]);
				STA_OVERWRITE_INC(sta, ipver);
			}
			return  WET_STA_HASH_UNK;
		}
	}
...
wet_sta_remove_ip(dhd_wet_info_t *weth, wet_sta_t *sta, uint8 ipver,
		uint8 *iaddr, int hash) {
	wet_sta_iplink_t *p_iplink_node = sta->ip_link[ipver];
	wet_sta_iplink_t *prev_iplink_node = NULL;
	while (p_iplink_node) {
		if (!bcmp(p_iplink_node->ip, iaddr, IP_ADDR_LEN[ipver])) {

			if (wet_sta_hash_count(sta, ipver, hash) == 1)
				remove_sta_from_iphash(weth, sta, iaddr, ipver);

			if (prev_iplink_node)
				prev_iplink_node->next = p_iplink_node->next;
			else
				sta->ip_link[ipver] = p_iplink_node->next;
			if (sta->cached_ip[ipver] == (void *)p_iplink_node->ip)
				sta->cached_ip[ipver] = NULL;
			sta->depth[ipver] -= 1;
			STA_OVERWRITE_DEC(sta, ipver);
			wet_iplink_node_free(weth, p_iplink_node);
			return 0;
		}
		prev_iplink_node = p_iplink_node;
		p_iplink_node = p_iplink_node->next;
	}
	return 1;
}
...
{
	if (wet) {
		wet_free_resources((dhd_wet_info_t *)wet);
...
wlc_dhcp6_option_find(uint8 *dhcp, uint16 dhcplen, uint16 option_code)
{
	...
	while (offset < dhcplen) {
		ptr = dhcp + offset;
		code = NTOH16(*(uint16 *)(ptr + DHCP6_OPT_CODE_OFFSET));
		optlen =  NTOH16(*(uint16 *)(ptr + DHCP6_OPT_LEN_OFFSET));
		...
		uint8 *iaddr, struct ether_addr *eaddr, wet_sta_t **saddr)
{
	...
	if ((!wet_sta_find_mac(weth, eaddr, &sta))) {
		/* most of the time, should fall into this block when cached_ip/primary ip
		 * is the same as the incoming ip
		 */
	...
	/* add ip to sta and remove sta from old iphash */
	i = wet_sta_add_ip(weth, sta, ipver, iaddr);
	/* update IP and hash by new IP */
	if (i == WET_STA_HASH_UNK) {
		int hash = insert_sta_into_iphash(weth, sta, iaddr, ipver);
		sta->bss = BSSCFG_IDX_TO_WET_NWKID(ifidx);
		/* start here and look for other entries with same IP address */
		{
			wet_sta_t *sta2;
			wet_iphash_node_t *node;
			for (node = weth->stahash_ip[ipver][hash]; node; node = node->next) {
				sta2 = node->sta;
				if (sta2 == sta) continue;
				/* now handle the sta itself */
				if (!bcmp(iaddr, sta2->ip[ipver], IP_ADDR_LEN[ipver])) {
					/* remove from iphash if it has no other same hash ip */
					if (wet_sta_hash_count(sta2, ipver, hash) == 1)
						remove_sta_from_iphash(weth, sta2, iaddr, ipver);
					if (sta2->ip_link[ipver] == NULL) {
						...
						void *tmp_p;
						/* if cached ip point to first ip in iplink, point
						 * it to primary now
						 */
						...
						sta2->depth[ipver] -= 1;
						STA_OVERWRITE_DEC(sta2, ipver);
						wet_iplink_node_free(weth, tmp_p);
					}
				}  else {
					wet_sta_remove_ip(weth, sta2, ipver, iaddr, hash);
				}
			}
		}
	}
		...
		uint8 *iaddr, wet_sta_t **saddr)
{
	...
	for (node = weth->stahash_ip[ipver][i]; node; node = node->next) {
		sta = node->sta;
		if (!(sta->cached_ip[ipver] &&
				!bcmp(sta->cached_ip[ipver], iaddr, IP_ADDR_LEN[ipver]))) {
...
#else
			wl_mcast_blog_clone_handler(pkt, sdu);
#endif
...
static void
wet_free_resources(dhd_wet_info_t *weth)
{
	int i, ipver;
	wet_sta_t *sta;
	wet_sta_iplink_t *p_iplink_node, *next_node;
	wet_iphash_node_t *node, *cur_node;

	/* free all iphashnode */
	for (ipver = 0; ipver < IPTYPE; ipver++) {
		for (i = 0; i < WET_STA_HASH_SIZE; i ++) {
			node = weth->stahash_ip[ipver][i];
			while (node) {
				cur_node = node->next;
				MFREE(weth->pub->osh, node, sizeof(wet_iphash_node_t));
				node = cur_node;
			}
			weth->stahash_ip[ipver][i] = NULL;
		}
	}

	wet_free_iphash(weth);
	wet_free_iplink(weth);

	for (i = 0; i < WET_NUMSTAS; i++) {
		sta = &weth->sta[i];
		if (sta->next) continue;
		for (ipver = 0; ipver < IPTYPE; ipver++) {
			p_iplink_node = sta->ip_link[ipver];
			while (p_iplink_node) {
				next_node = p_iplink_node->next;
				MFREE(weth->pub->osh, p_iplink_node, sizeof(wet_sta_iplink_t));
				p_iplink_node = next_node;
			}
			sta->ip_link[ipver] = NULL;
		}
	}
}

static inline void
wet_dump_iphash(dhd_wet_info_t *weth, struct bcmstrbuf *b, int ipver)
{
	int i;
	wet_sta_t *sta;
	wet_iphash_node_t *node;
...
static inline void
wet_dump_link(dhd_pub_t *dhdp, struct bcmstrbuf *b)
{
	wet_sta_t *sta;
	...
	bcm_bprintf(b, "ipv4 hash:\n");
	wet_dump_iphash(weth, b, IPVER_4);
	bcm_bprintf(b, "ipv6 hash:\n");
	wet_dump_iphash(weth, b, IPVER_6);
}
		...
		sta = &weth->sta[i];
			if (!IPV4_ADDR_NULL(&sta->ip[IPVER_4])) {
				...
				if (sta->cached_ip[IPVER_4])
					bcm_bprintf(b, "\t\tcached(%d):\t\t%pI4\n",
							sta->overwrite_pos[IPVER_4],
			...
			if (!IPV6_ADDR_NULL(sta->ip[IPVER_6])) {
				...
				if (sta->cached_ip[IPVER_6])
					bcm_bprintf(b, "\t\tcached:(%d)\t\t%pI6\n",
							sta->overwrite_pos[IPVER_6],
							sta->cached_ip[IPVER_6]);
			}
	}
#ifdef BCMDBG
	wet_dump_link(dhdp, b);
#endif
}
