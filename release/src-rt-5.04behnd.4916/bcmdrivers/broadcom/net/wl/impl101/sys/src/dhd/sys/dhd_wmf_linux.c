 ...
 *
 * $Id: dhd_wmf_linux.c 832961 2023-11-15 21:40:29Z $
 */
#include <typedefs.h>
#include <linuxver.h>
#include <osl.h>
...
int32 dhd_wmf_mac_from_ifp(void *ifp, uint8 *out_mac)
{
	dhd_sta_t *sta = (dhd_sta_t*) ifp;
	memcpy(out_mac, sta->ea.octet, ETHER_ADDR_LEN);
...
#ifdef CMWIFI_WMF_IPV6
	wmf_emfc.mfdb_ifinfo_mac_map_fn = dhd_wmf_mac_from_ifp;
#endif /* CMWIFI_WMF_IPV6 */
...
#ifdef CMWIFI_WMF_IPV6
	emf_instance_append(wmf_inst->emfci, inst_id);
#endif /* CMWIFI_WMF_IPV6 */
...
#if defined(BCM_WLAN_MCAST_BLOG_SHARED_INFO)
	wl_mcast_blog_client_handler(p);
#endif
#endif /* BCM_WLAN_PER_CLIENT_FLOW_LEARNING && BCM_BLOG */

#if defined(CMWIFI) && defined(BCM_DHD_RUNNER) && defined(BCM_DHD_TX_ON_RUNNER) && \
	defined(DQNET_DOR_API_V2)
	ret = dhd_offload_sendpkt(dhdp, wmfh->bssidx, p);
#else
	ret = dhd_sendpkt(dhdp, wmfh->bssidx, p);
#endif /* CMWIFI && BCM_DHD_RUNNER && BCM_DHD_TX_ON_RUNNER && DQNET_DOR_API_V2 */
...
#ifdef CMWIFI_WMF_IPV6
	if (mgrp_ip == 0) {
		uint8 dhost[ETHER_ADDR_LEN] = {0x33, 0x33, 0x0, 0x0, 0x0, 0x1};
...
#if defined(CMWIFI) && defined(BCM_DHD_RUNNER) && defined(BCM_DHD_TX_ON_RUNNER) && \
	defined(DQNET_DOR_API_V2)
	ret = dhd_offload_sendpkt(dhdp, wmfh->bssidx, pkt);
#else
...
dhd_wmf_packets_handle(void *pub, void *p, void *sta_wmf, int ifidx, bool frombss)
{
	...
	if (ether_type == ETHER_TYPE_8021Q) {
		struct ethervlan_header *ehv = (struct ethervlan_header *)PKTDATA(dhdp->osh, p);
...
#if (defined(BCM_NBUFF_WLMCAST) && defined(DHD_WMF)) || defined(CMWIFI_WMF_IPV6)
...
#endif /* defined(BCM_NBUFF_WLMCAST) && defined(DHD_WMF) || CMWIFI_WMF_IPV6 */
