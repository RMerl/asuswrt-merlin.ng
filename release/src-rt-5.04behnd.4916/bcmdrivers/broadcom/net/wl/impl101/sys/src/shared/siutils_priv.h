 ...
 *
 * $Id: siutils_priv.h 827523 2023-07-14 13:12:06Z $
 */
...
#define AI_MAX_WRAPPERS_PERCORE		3
...
typedef struct si_cores_info {
	...
	uint32	coresba_pa[SI_MAXCORES];	/**< backplane physical address of each core */
	void	*regs2[SI_MAXCORES];	/**< virtual addr of each core 2nd register set (usbh20) */
	uint32	coresba2_pa[SI_MAXCORES];	/**< phys address of each core 2nd register set */
	uint32	coresba_size[SI_MAXCORES];	/**< backplane address space size */
	uint32	coresba2_size[SI_MAXCORES];	/**< second address space size */
	void	*wrappers[SI_MAXCORES][AI_MAX_WRAPPERS_PERCORE]; /**< core wrappers virt addr */
	uint32	wrapba_pa[SI_MAXCORES][AI_MAX_WRAPPERS_PERCORE];	/**< address of wrapper */
	uint32	cia[SI_MAXCORES];		/**< erom cia entry for each core */
	uint32	cib[SI_MAXCORES];		/**< erom cia entry for each core */

	uint32  csp2ba_pa[SI_MAXCORES];		/**< Second slave port base phys addr 0 */
...
typedef struct si_info {
	...
	uint32	oob_router_pa;		/**< oob router registers for axi */
	...
	uint32	br_wrapba_pa[SI_MAXBR];	/**< address of bridge controlling wrapper */
	uint32	xtalfreq;
	bool	device_removed;
	uint	axi_num_wrappers;
	axi_wrapper_t   * axi_wrapper;
	uint8	device_wake_opt; /* device_wake GPIO number */
	uint32	gicd_spisr;		/**< Shared Peripheral Interrupt Status Registers */
} si_info_t;
...
extern si_t *ai_attach(uint pcidev, osl_t *osh, void *regs, enum bustype_e bustype,
