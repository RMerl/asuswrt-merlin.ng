 ...
 *
 * $Id: wl_cfg80211.h 832872 2023-11-14 21:35:54Z $
 */
...
#define WL_SCAN_CONNECT_DWELL_TIME_MS		119
#define WL_SCAN_JOIN_PROBE_INTERVAL_MS		20
#define WL_SCAN_JOIN_ACTIVE_DWELL_TIME_MS	WL_SCAN_CONNECT_DWELL_TIME_MS
#define WL_SCAN_JOIN_PASSIVE_DWELL_TIME_MS	WL_SCAN_CONNECT_DWELL_TIME_MS
...
#ifndef WLAN_AKM_SUITE_SAE_EXT
#define WLAN_AKM_SUITE_SAE_EXT	       0x000FAC18
#define WLAN_AKM_SUITE_FT_OVER_SAE_EXT 0x000FAC19
#endif /* WLAN_AKM_SUITE_SAE_EXT */
...
struct parsed_ies {
	...
	const bcm_tlv_t *rrm_cap_ie;
	u32 rrm_cap_ie_len;
	const bcm_tlv_t *vht_cap_ie;
	const bcm_tlv_t *ht_cap_ie;
	const bcm_tlv_t *he_cap_ie;
#ifdef WL11U
...
#ifdef WL11U
#define IW_IES_MAX_BUF_LEN	9	/* Max length of IW IE */
#define RC_IES_MAX_BUF_LEN	17	/* Max length of RC IE - upto 3 OUI */
#endif
#ifdef WLFBT
...
#define MAX_MLDS 16
#define INVALID_MLD_ID -1
#define INVALID_LINK_ID -1
#define FIRST_AUX_DEV_LINK_ID 1
typedef struct wl_mlo_link_info {
	s8 mld_unit;
	s8 mld_link_id;
	u8 addr[ETHER_ADDR_LEN];
	struct net_device *ndev;
	struct ieee80211_supported_band *cur_bands[MAX_MLO_LINKS];
} wl_mlo_link_info_t;

typedef struct mlo_dev_info {
	u8 mld_unit;
	int link_count;
	u8 supported_bands;
	u8 *mld_addr;
	struct net_device *mlo_ndev;
	struct wl_mlo_link_info *mlo_linkinfo[MAX_MLO_LINKS];
} mlo_dev_info_t;
#endif /* WLMLO_STA */
...
#define ndev_to_wiphy(ndev) (ndev->ieee80211_ptr->wiphy)
...
extern s32 wl_cfg80211_notify_ifadd(struct net_device * dev, int ifidx, char *name,
	const uint8 *mac, uint8 bssidx, uint8 role);
...
extern struct net_device* wl_cfg80211_allocate_if(struct bcm_cfg80211 *cfg, int ifidx,
	const char *name, const uint8 *mac, uint8 bssidx, bool rntl_needed, const char *dngl_name);
...
int wl_cfg80211_cleanup_mismatch_status(struct net_device *dev, struct bcm_cfg80211 *cfg,
	bool disassociate);
void wl_cfg80211_ch_switch_notify(struct net_device *dev, chanspec_t chanspec,
...
struct net_device *wl_cfg80211_post_ifcreate(struct net_device *ndev,
	wl_if_event_info *event, const u8 *addr, const char *name, bool rtnl_lock_reqd);
...
#endif /* !BCMDONGLEHOST */
#ifdef WLMLO_STA
#define IS_MLO_STA_INTERFACE(wdev) (wdev && \
		(wdev->iftype == NL80211_IFTYPE_STATION) && \
		((wl_cfg80211_mlo_ndev_to_linkid(wdev_to_ndev(wdev))) != -1))
