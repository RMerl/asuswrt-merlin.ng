 ...
 *
 * $Id: wl_cfgp2p.c 820700 2023-01-23 11:40:06Z $
 *
 */
#include <typedefs.h>
#include <linuxver.h>
...
#ifdef WL11U
#ifdef WL_DPP
bool wl_cfgp2p_get_fwd_gas_rqst_status(struct bcm_cfg80211 *cfg, struct net_device *dev)
{
	s32 err;
	s32 bssidx;
	s32 is_fwd_gas_enabled = 0;
	...
	err = wldev_iovar_getint_bsscfg(dev, "fwd_gas_rqst_to_app", &is_fwd_gas_enabled, bssidx);
	WL_INFORM(("BSSIDX(%d) WLC_GET_FWD_GAS_RQST_TO_APP(%d)\n", bssidx, is_fwd_gas_enabled));
	if (unlikely(err < 0)) {
		WL_ERR(("WLC_GET_FWD_GAS_RQST_TO_APP error (%d)\n", err));
		goto end;
	}

end:
	return (is_fwd_gas_enabled) ? true : false;
}

bool wl_cfgp2p_is_allowed_gas_frame(struct bcm_cfg80211 *cfg,
	struct net_device *dev, void *frame, u32 frame_len)
{
	const bcm_tlv_t *ie;
	const u8 *frm = NULL;
	u8 adv_proto_id;
	...
	if ((dev->ieee80211_ptr->iftype == NL80211_IFTYPE_AP) && (sd_act_frm->action ==
		...
		if (wl_cfgp2p_get_fwd_gas_rqst_status(cfg, dev)) {
			return false;
		}
	}
...
wl_cfgp2p_set_p2p_mode(struct bcm_cfg80211 *cfg, u8 mode, u32 channel, u16 listen_ms, int bssidx)
{
	...
	bzero(&discovery_mode, sizeof(discovery_mode));
...
/*
 * In some customer's implementation "ifconfig wlan0 down" can get
...
wl_cfgp2p_add_p2p_disc_if(struct bcm_cfg80211 *cfg)
{
	...
	if (cfg->p2p_wdev) {
		...
		/*
		 * Some customer's design doesn't delete the p2p discovery
