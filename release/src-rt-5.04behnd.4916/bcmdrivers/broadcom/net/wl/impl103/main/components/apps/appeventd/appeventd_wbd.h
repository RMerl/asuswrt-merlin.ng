...
typedef enum {
	MAP_CLIENT_ADDED = 1,
	MAP_CLIENT_REMOVED = 2,
} map_client_status_t;

typedef enum {
	MAP_CONTROLLER_ADDED = 1,
	MAP_CONTROLLER_REMOVED = 2,
} map_controller_status_t;
...
/* Another Controller found event data */
typedef struct {
	struct ether_addr device_id;	/* Device AL MAC */
} app_event_wbd_controller_data_t;

/* Agent Status Changed in SmartMesh Network : event data */
typedef struct {
	struct ether_addr device_id;	/* AL_MAC of Agent in SmartMesh */
	map_agent_status_t status;	/* Added = 1, Removed = 2 */
} app_event_wbd_agent_status_changed_t;

/* Controller Status Changed in SmartMesh Network : event data */
typedef struct {
	struct ether_addr device_id;	/* AL_MAC of Controller in SmartMesh */
	map_controller_status_t status;	/* Added = 1, Removed = 2 */
} app_event_wbd_controller_status_changed_t;

/* Client Status Changed in SmartMesh Network : event data */
typedef struct {
	struct ether_addr sta_addr;	/* STA mac addr */
	map_client_status_t status;	/* Added = 1, Removed = 2 */
} app_event_wbd_client_status_changed_t;
