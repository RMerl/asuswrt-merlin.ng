 ...
 *
 * $Id: ieee1905_vendor.c 814145 2022-07-19 10:12:39Z $
 */
...
/* Add detach method for vendor specific TLVs. */
/* Remove callback item from the list that is given with a message type for receiving vendor data */
static void i5VendorRemoveRcvCbsFromList(ieee1905_glist_t *msg_cbs_list,
  const char *oui, i5VendorTLVReceiveCbfn cbfn)
{
   i5VendorRcvCbsList_t *node, *next_p;

  if (msg_cbs_list->count == 0) {
    i5Trace("Receive No CBs have been set for the given message.\n");
    return;
  }

  /* Traverse the callback list item. */
  foreach_safe_iglist_item(node, i5VendorRcvCbsList_t, *msg_cbs_list, next_p) {

    /* Need to keep next item incase we remove node in between */
    next_p = ((i5VendorRcvCbsList_t*)dll_next_p((dll_t*)(node)));

    /* Find list item that includes removed cbfn with the given oui. */
    if (!memcmp(node->oui, oui, sizeof(node->oui)) && node->cbfn == cbfn) {
      ieee1905_glist_delete(msg_cbs_list, (dll_t*)node);
      free(node);
      return;
    }
  }
...
/* De-Register the callback to stop recieving a vendor specific data. */
void i5VendorTLVReceiveCbsDeregister(const i5_message_types_t message_type,
  const char *oui, i5VendorTLVReceiveCbfn cbfn)
{
  i5VendorRegisteredRcvCbs_t *node = NULL;
  unsigned char binoui[I5_VNDR_OUI_LEN];
  int idx = I5_VNDR_MSG_TYPE_HASH(message_type);
  I5_VNDR_VALIDATE_HASH_MSG_TYPE_IDX(idx);

  node = i5_vndr_entry.cbs_to_rcv_vndr_tlvs_lst[idx];
  /* Check if meesage type node is already handling cbs. */
  if (node == NULL || node->message_type != message_type) {
    i5Trace("Receive Message is not registered to get CBs yet.\n");
    return;
  }

  i5String2OUI(oui, binoui, sizeof(binoui));
  i5VendorRemoveRcvCbsFromList(&node->cbs_list, (const char*)binoui, cbfn);
end:
    return;
}

void i5VendorTLVReceiveCbsDump()
{
  int i, j;
  i5Trace("Receive DUMPING vendor specific TLV subscribers:\n");
  for (i = 0; i < I5_VNDR_MSG_TYPE_HASH_SIZE; i++) {
    i5VendorRegisteredRcvCbs_t *node1 = NULL;
    i5VendorRcvCbsList_t *cbs1;
    if (i5_vndr_entry.cbs_to_rcv_vndr_tlvs_lst[i] != NULL) {
      node1 = i5_vndr_entry.cbs_to_rcv_vndr_tlvs_lst[i];
      i5Trace("%d. Message: %02x. Callbacks (%d):\n", i,
          node1->message_type, node1->cbs_list.count);
      j = 0;
      foreach_iglist_item(cbs1, i5VendorRcvCbsList_t, node1->cbs_list) {
        i5Trace("%d. OUI:%02x%02x%02x. handler:%p\n", j,
          cbs1->oui[0], cbs1->oui[1], cbs1->oui[2], cbs1->vndr_ctxt);
        j++;
      }
    }
  }
  ...
  const unsigned char *src_al_mac)
{
  ...
  if (node != NULL && (node->message_type == message_type)) {
    ...
    foreach_iglist_item(cbs, i5VendorRcvCbsList_t, node->cbs_list) {
      ...
      char keepTLVHeader = cbs->flags & I5_VNDR_RCV_FLAGS_KEEP_TLV_HEADER ?
                           i5MessageVendorTlvExtractKeepHeader :
                           i5MessageVendorTlvExtractRemoveHeader;
      ...
      if (i5MessageGetVendorSpecificTlvForOUI(pmsg, cbs->oui, keepTLVHeader, &tlvs, &tlv_len)) {
