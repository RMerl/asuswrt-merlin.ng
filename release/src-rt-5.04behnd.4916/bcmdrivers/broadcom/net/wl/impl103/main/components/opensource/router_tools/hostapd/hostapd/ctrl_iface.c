...
static int hostapd_ctrl_iface_wps_pbc(struct hostapd_data *hapd, char *txt)
{
	int ieee1905_tk = 0;
	...
	if (os_strncmp(txt, "ieee1905_tk=", 12) == 0)
		ieee1905_tk = atoi(txt + 12);

	return hostapd_wps_button_pushed_ex(hapd, ieee1905_tk);
}
...
static int hostapd_ctrl_iface_map_set_own_addr(struct hostapd_data *hapd, const char *txtaddr)
{
	u8 addr[ETH_ALEN];

	wpa_printf(MSG_DEBUG, "CTRL_IFACE MAP_SET_OWN_ADDR %s", txtaddr);

	if (hwaddr_aton(txtaddr, addr))
		return -1;

	os_memcpy(hapd->own_addr, addr, ETH_ALEN);
	wpa_auth_set_map_addr(hapd->wpa_auth, addr);
	wpa_auth_reconfig_group_keys(hapd->wpa_auth);

	return 0;
}

static int hostapd_ctrl_iface_map_add_dev(struct hostapd_data *hapd, const char *cmd)
{
	int ret = -1, used;
	size_t len, peer_disc_framelen;
	char *pos, *peer_disc_frame = NULL;
	u8 peer_al_mac[ETH_ALEN];
	struct sta_info *sta;
	struct wpa_state_machine *sm;

	wpa_printf(MSG_DEBUG, "CTRL_IFACE NEW_MAP_DEV %s", cmd);

	pos = (char *)cmd;

	/* Fetch Peer Device AL MAC, who sent DPP Peer Discovery Req Frame */
	used = hwaddr_aton2(pos, peer_al_mac);
	if (used < 0)
		goto end;
	pos += used;
	while (*pos == ' ')
	...
	/* Fetch DPP Peer Discovery Req Frame */
	len = os_strlen(pos);
	if (len == 0) {
		goto end;
	}
	...
	peer_disc_frame = os_malloc(len);
	if (peer_disc_frame == NULL) {
		goto end;
	}
	if (hexstr2bin(pos, peer_disc_frame, len) < 0) {
		goto end;
	}
	peer_disc_framelen = len;

	/* Process DPP Peer Discovery Req Frame */
	sta = ap_get_sta(hapd, peer_al_mac);
	if (sta) {
		wpa_printf(MSG_ERROR, "MAP Device " MACSTR "Already Exists. So, Remove it",
			MAC2STR(peer_al_mac));
		ap_free_sta(hapd, sta);
	}

	sta = ap_sta_add(hapd, peer_al_mac);
	if (!sta) {
		wpa_printf(MSG_ERROR, "Failed to add MAP Device " MACSTR,
			MAC2STR(peer_al_mac));
		goto end;
	}
	...
	if (sta->wpa_sm == NULL) {
		...
		if (sta->wpa_sm == NULL) {
			wpa_printf(MSG_ERROR, "Failed to initialize WPA state machine "
				"for MAP Device " MACSTR, MAC2STR(peer_al_mac));
			goto end;
		}
	}

	/* Process NIP, Process Peer Discovery Req Frame, Send Peer Discovery Resp */
	hostapd_dpp_rx_action(hapd, peer_al_mac, &peer_disc_frame[1], (peer_disc_framelen - 1), 0);

	wpa_auth_set_map_params(sta->wpa_sm, WPA_PARAM_PROTO, hapd->conf->wpa);
	wpa_auth_set_map_params(sta->wpa_sm, WPA_PARAM_PAIRWISE, hapd->conf->wpa_pairwise);
	wpa_auth_set_map_params(sta->wpa_sm, WPA_PARAM_KEY_MGMT, hapd->conf->wpa_key_mgmt);

	sta->flags |= (WLAN_STA_AUTH | WLAN_STA_ASSOC);

	wpa_auth_set_map_pksa_to_sm(sta->wpa_sm, peer_al_mac, hapd->wpa_auth);

	if (wpa_auth_sm_event(sta->wpa_sm, WPA_ASSOC) != 0) {
		goto end;
	}
	...
	if (peer_disc_frame) {
		os_free(peer_disc_frame);
		peer_disc_frame = NULL;
	}
	return ret;
}

static int hostapd_ctrl_iface_map_rx_eapol(struct hostapd_data *hapd, const char *cmd)
{
	int ret = -1, used;
	size_t len;
	u8 src[ETH_ALEN];
	...
	wpa_printf(MSG_DEBUG, "MAP External EAPOL RX: %s", cmd);
...
struct hostapd_data* hapd_get_master_hapd(struct hostapd_data *hapd) {
	struct hostapd_data *master_hapd = NULL;
	/* Always the first linki_unit in an MLD is master */
	struct hostapd_link_unit *plink =
		dl_list_entry(hapd->mld_link.mld->link_info.next,
			struct hostapd_link_unit, list);
	if (plink && (plink->link_id == 0)) {
		master_hapd = plink->hapd;
	}
	return master_hapd;
}

static int hostapd_ctrl_iface_get_mlo_config(struct hostapd_data *hapd,
					 char *buf, size_t buflen)
{
	int ret;
	char *pos, *end;
	...
	if (hapd->conf->mld_unit != -1)
	{
		ret = os_snprintf(pos, end - pos, "mld_unit=%d\n",
				  hapd->conf->mld_unit);
		if (os_snprintf_error(end - pos, ret))
			return pos - buf;
		pos += ret;
	}
	if (hapd->conf->link_id != -1)
	{
	...
	if (hapd->iface->interfaces->mlo_info)
	{
		int k = 0, l = 0;
		ret = os_snprintf(pos, end - pos, "interfaces_count:%d\n",
			  hapd->iface->interfaces->count);
		if (os_snprintf_error(end - pos, ret))
			return pos - buf;
		pos += ret;
		for (k=0; k < hapd->iface->interfaces->count; k++){
			struct hostapd_iface *local = hapd->iface->interfaces->iface[k];
			ret = os_snprintf(pos, end - pos, "phy:%s bss_count:%d\n",
				local->phy, local->num_bss);
			if (os_snprintf_error(end - pos, ret))
				return pos - buf;
			pos += ret;
			for (l = 0; l < local->num_bss; l++) {
				struct hostapd_data *lbss = local->bss[l];
				ret = os_snprintf(pos, end - pos, "iface:%s link_id:%d mld_unit:%d\n",
					lbss->conf->iface,
					lbss->mld_link.link->link_id,
					lbss->mld_link.mld->mld_id);
				if (os_snprintf_error(end - pos, ret))
					return pos - buf;
				pos += ret;
			}
	...
	if (hapd->iface->interfaces->mlo_info)
	{
		struct hostapd_data *mhapd = NULL;
		struct hostapd_mlo_global *pmlo = hapd->iface->interfaces->mlo_info;
		struct hostapd_mld_unit *pmld = NULL;
		struct hostapd_link_unit *plink = NULL;
		/*iterate and print mlo config */
		ret = os_snprintf(pos, end - pos, "interfaces_count:%d\n",
				  hapd->iface->interfaces->count);
		if (os_snprintf_error(end - pos, ret))
			return pos - buf;
		pos += ret;
		dl_list_for_each(pmld, &(pmlo->mld_info),
				struct hostapd_mld_unit, list) {
			ret = os_snprintf(pos, end - pos, "MLD_UNIT:%d\n",pmld->mld_id);
			if (os_snprintf_error(end - pos, ret))
				return pos - buf;
			pos += ret;
			dl_list_for_each(plink, &(pmld->link_info),
					struct hostapd_link_unit, list) {
				ret = os_snprintf(pos, end - pos,
					"bss:%s link:%d MAC addr:"
					MACSTR "\n",
					plink->hapd->conf->iface,
					plink->link_id,
					MAC2STR(plink->hapd->own_addr));
				if (os_snprintf_error(end - pos, ret))
					return pos - buf;
				pos += ret;
			}
			plink = dl_list_entry(pmld->link_info.next,
				struct hostapd_link_unit, list);
			if (plink && (plink->link_id == 0)) {
				mhapd = plink->hapd;
			}
			if (mhapd) {
				ret = os_snprintf(pos, end - pos,
					"MASTER ADDRESS:" MACSTR "\n",
					MAC2STR(mhapd->own_addr));
				if (os_snprintf_error(end - pos, ret))
					return pos - buf;
				pos += ret;
			}
...
static int hostapd_ctrl_iface_mlo_assoc_info(struct hostapd_data *hapd,
		const char *txtaddr, char *buf, size_t buflen)
{
	...
	wpa_printf(MSG_DEBUG, "CTRL_IFACE MLO ASSOC INFO %s\n", txtaddr);

	if (hwaddr_aton(txtaddr, addr))
		return -1;

	sta = ap_get_sta(hapd, addr);
	if (sta && sta->mlsta_info) {
		pos = buf;
		end = buf + buflen;

		ret = os_snprintf(pos, end - pos,
				"Numlinks %d MLDAddr "MACSTR" STAAddr "MACSTR"\n",
				sta->mlsta_info->numlinks, MAC2STR(sta->mlsta_info->mld_addr),
				MAC2STR(addr));
		...
		for (i = 0; i < (sta->mlsta_info->numlinks - 1); i++) {
			ret = os_snprintf(pos, end - pos,
					"Index %d STA Linkid %d ""STA Link Addr "MACSTR"\n",
					i, (sta->mlsta_info->persta[i]).linkid,
					MAC2STR((sta->mlsta_info->persta[i]).sta_addr));
			if (os_snprintf_error(end - pos, ret))
				return pos - buf;
			pos += ret;
		}
...
static int hostapd_ctrl_iface_set(struct hostapd_data *hapd, char *cmd)
{
	...
	*value++ = '\0';
	...
	if (0) {
		...
		if (val != hapd->conf->dpp_configurator_connectivity) {
			hapd->conf->dpp_configurator_connectivity = val;
				  ...
				  char *buf, size_t buflen)
{
	...
	if (os_strcmp(cmd, "version") == 0) {
		...
		return res;
#ifdef RDKB
	} else if (os_strcmp(cmd, "own_ip_addr") == 0) {
		buf[0] = '\0';
		hostapd_ip_txt(&hapd->conf->own_ip_addr, buf, buflen);
					      ...
					      socklen_t fromlen)
{
	...
	if (os_strcmp(buf, "PING") == 0) {
...
#ifdef CONFIG_DRIVER_BRCM_MAP
		if (hostapd_wps_button_pushed_ex(hapd, 0))
			reply_len = -1;
#else
		...
		if (hostapd_ctrl_iface_wps_pbc(hapd, buf + 8) < 0)
			reply_len = -1;
	} else if (os_strncmp(buf, "WPS_MAPBH_CONFIG ", 17) == 0) {
		reply_len = hostapd_ctrl_iface_wps_map_bh_creds(hapd, buf + 17,
				reply, reply_size);
	} else if (os_strncmp(buf, "MAP_SET_OWN_ADDR ", strlen("MAP_SET_OWN_ADDR ")) == 0) {
		if (hostapd_ctrl_iface_map_set_own_addr(hapd, buf + strlen("MAP_SET_OWN_ADDR ")) < 0) {
			reply_len = -1;
		}
	} else if (os_strncmp(buf, "MAP_ADD_DEV ", strlen("MAP_ADD_DEV ")) == 0) {
		if (hostapd_ctrl_iface_map_add_dev(hapd, buf + strlen("MAP_ADD_DEV ")) < 0) {
			reply_len = -1;
		}
	} else if (os_strncmp(buf, "MAP_RX_EAPOL ", strlen("MAP_RX_EAPOL ")) == 0) {
		if (hostapd_ctrl_iface_map_rx_eapol(hapd, buf + strlen("MAP_RX_EAPOL ")) < 0) {
			reply_len = -1;
		}
#endif /* CONFIG_DRIVER_BRCM_MAP */
#ifdef CONFIG_WPS_NFC
...
#endif /* ANDROID */
#ifdef	CONFIG_DRIVER_BRCM_MLO
	} else if (os_strcmp(buf, "MLO_CONFIG") == 0) {
		reply_len = hostapd_ctrl_iface_get_mlo_config(hapd, reply,
							  reply_size);
	} else if (os_strncmp(buf, "MLO_ASSOC_INFO ", 15) == 0) {
		reply_len = hostapd_ctrl_iface_mlo_assoc_info(hapd, buf + 15,
				reply, reply_size);
#endif /*CONFIG_DRIVER_BRCM_MLO */
	} else {
