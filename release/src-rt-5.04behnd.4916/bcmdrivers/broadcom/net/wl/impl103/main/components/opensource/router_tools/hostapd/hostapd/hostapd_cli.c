				   ...
				   char *argv[])
{
	...
	if (argc > 0) {
		snprintf(buf, sizeof(buf), "WPS_PBC %s", argv[0]);
		return wpa_ctrl_command(ctrl, buf);
	}
...
static int hostapd_cli_cmd_get_mlo_config(struct wpa_ctrl *ctrl, int argc,
				      char *argv[])
{
	return wpa_ctrl_command(ctrl, "MLO_CONFIG");
}

static int hostapd_cli_cmd_get_mlo_assoc_info(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
	char buf[64];
	if (argc < 1) {
		printf("Invalid 'mlo_assoc_info' command - exactly one "
			"argument, STA address, is required.\n");
		return -1;
	}

	os_snprintf(buf, sizeof(buf), "MLO_ASSOC_INFO %s", argv[0]);
...
#ifdef CONFIG_DRIVER_BRCM_INTERNAL_DEBUG
static int hostapd_cli_cmd_get_pmk(struct wpa_ctrl *ctrl, int argc,
					  char *argv[])
{
	char buf[64];
        if (argc != 1) {
                printf("Invalid 'get_pmk' command - exactly one argument, STA "
                       "address, is required.\n");
                return -1;
        }
	snprintf(buf, sizeof(buf), "GET_PMK %s", argv[0]);
	return wpa_ctrl_command(ctrl, buf);
}
#endif /* CONFIG_DRIVER_BRCM_INTERNAL_DEBUG */
...
#endif /* ANDROID */
#ifdef	CONFIG_DRIVER_BRCM_MLO
	{ "mlo_config", hostapd_cli_cmd_get_mlo_config, NULL,
	  "= show current mlo configuration" },
	{ "mlo_assoc_info", hostapd_cli_cmd_get_mlo_assoc_info, NULL,
	  "<addr> = show mlo assoc information of STA"},
#endif /* CONFIG_DRIVER_BRCM_MLO */
	{ NULL, NULL, NULL, NULL }
};
