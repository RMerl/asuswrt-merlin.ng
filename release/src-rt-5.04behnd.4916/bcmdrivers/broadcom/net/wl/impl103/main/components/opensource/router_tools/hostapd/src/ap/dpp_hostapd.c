...
/* If the Initiator is a Configurator, the state machine sets timer t to 10 seconds and awaits the
 * start of provisioning
 */
static void hostapd_dpp_config_req_wait_timeout(void *eloop_ctx, void *timeout_ctx);
			   ...
			   const u8 *data, size_t data_len, int ok)
{
	...
	if (hapd->dpp_auth->remove_on_tx_status) {
		...
		eloop_cancel_timeout(hostapd_dpp_auth_resp_retry_timeout, hapd,
				     NULL);
#ifdef CONFIG_DRIVER_BRCM
		eloop_cancel_timeout(hostapd_dpp_config_req_wait_timeout, hapd,
				     NULL);
#endif /* CONFIG_DRIVER_BRCM */
#ifdef CONFIG_DPP2
...
static void hostapd_dpp_config_req_wait_timeout(void *eloop_ctx, void *timeout_ctx)
{
	...
	wpa_printf(MSG_DEBUG,
		   "DPP: Terminate authentication exchange due to Configuration Request timeout");
	wpa_msg(hapd->msg_ctx, MSG_INFO, DPP_EVENT_FAIL
					  ...
					  struct dpp_config_obj *conf)
{
	...
	wpa_msg(hapd->msg_ctx, MSG_INFO, DPP_EVENT_CONFOBJ_NETROLE "%s",
...
#ifdef CONFIG_DRIVER_BRCM
	eloop_cancel_timeout(hostapd_dpp_config_req_wait_timeout, hapd, NULL);
	if ((auth->auth_success == 1) && (hapd->dpp_auth->configurator)) {
		eloop_register_timeout(10, 0, hostapd_dpp_config_req_wait_timeout, hapd, NULL);
	}
#endif /* CONFIG_DRIVER_BRCM */
}
...
#ifdef CONFIG_DRIVER_BRCM_MAP
	if (hapd->conf->multi_ap && !(hapd->conf->multi_ap & FRONTHAUL_BSS)) {
		wpa_printf(MSG_DEBUG,
			   "DPP: Ignore Reconfig Announcement from " MACSTR " on Non Fronthaul "
			   "BSS(%s)",
...
#ifdef CONFIG_DPP2
#ifdef CONFIG_DRIVER_BRCM_MAP
	if ((type == DPP_PA_PRESENCE_ANNOUNCEMENT) && hapd->conf->multi_ap &&
		!(hapd->conf->multi_ap & FRONTHAUL_BSS)) {
		wpa_printf(MSG_DEBUG,
			   "DPP: Ignore Presence Announcement from " MACSTR " on Non Fronthaul "
			   "BSS(%s)",
			   MAC2STR(src), hapd->conf->iface);
		return;
	}
...
#ifdef CONFIG_DRIVER_BRCM_MAP
	pub_key = get_param(cmd, " pubkey=");
	if (pub_key) {
		len = os_strlen(pub_key);
		len /= 2;
		auth->map_peer_pub_key = wpabuf_alloc(len);
		if (!auth->map_peer_pub_key) {
			return -1;
		}
		if (hexstr2bin(pub_key, wpabuf_put(auth->map_peer_pub_key, len), len) < 0) {
