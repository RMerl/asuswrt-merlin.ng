...
#else /* CONFIG_DRIVER_BRCM_RDKB_RADIUS_GREYLIST */
...
static int configured_fixed_chan_to_freq(struct hostapd_iface *iface)
{
	...
	for (j = 0; j < iface->num_hw_features; j++) {
		...
		for (i = 0; i < mode->num_channels; i++) {
			...
			/* TODO :6GHZ: Removed the check !is_6ghz_freq(chan->freq); revisit */
...
#ifdef CONFIG_DRIVER_BRCM_MLO
void
hostapd_mlo_remove(struct hostapd_data *hapd) {
	struct hostapd_mld_unit *pmld = NULL;
	struct hostapd_link_unit *plink = NULL;
	struct hostapd_mlo_global *pmlo = NULL;
	int mld_match, link_match;
	...
	if (hapd->iface->interfaces->mlo_info) {
		pmlo = hapd->iface->interfaces->mlo_info;
		mld_match = 0;
		dl_list_for_each(pmld, &(pmlo->mld_info),
				struct hostapd_mld_unit, list) {
			if (!hapd->mld_link.mld) {
				break;
			}
			if (pmld->mld_id == hapd->mld_link.mld->mld_id) {
				mld_match = 1;
				break;
			}
		}
		if (!mld_match) {
			wpa_printf(MSG_DEBUG, "NO MLD for %s",
				hapd->conf->iface);
			return;
		}
		link_match = 0;
		dl_list_for_each(plink, &(pmld->link_info),
			...
			if (!hapd->mld_link.link) {
				break;
			}
			if (plink->link_id == hapd->mld_link.link->link_id) {
				link_match = 1;
				break;
			}
		}
		...
		wpa_printf(MSG_DEBUG, "Deleting Link for %s",
			hapd->conf->iface);
		hapd->mld_link.mld = NULL;
		hapd->mld_link.link = NULL;
		...
		if (!pmld->link_count) {
			...
			wpa_printf(MSG_DEBUG, "Deleting mld for %s",
				hapd->conf->iface);
			dl_list_del(&pmld->list);
			os_free(pmld);
			pmlo->mld_count--;
		}
		if (!pmlo->mld_count) {
			/* remove mlo_info after last mld delete */
			wpa_printf(MSG_DEBUG, "Deleting mlo_info for %s",
				hapd->conf->iface);
			os_free(pmlo);
		}
	}
...
int
hostapd_mlo_add(struct hostapd_data *hapd, int mld_id, int link_id)
{
	int mld_add = 1; /* add before */
	int link_add = 1; /* add before */
	struct hostapd_mld_unit *pmld = NULL;
	struct hostapd_link_unit *plink = NULL;
	struct hostapd_mlo_global *pmlo = NULL;
	if (!hapd || !hapd->iface || !hapd->iface->interfaces) {
	...
	if (hapd->iface->interfaces->mlo_info == NULL) {
		/* add mlo struct if not present */
		pmlo = os_zalloc(sizeof(*pmlo));
		if (!pmlo) {
			wpa_printf(MSG_ERROR, "MLO memalloc failed");
			return -1;
		}
		dl_list_init(&(pmlo->mld_info));
		pmlo->mld_count = 0;
		hapd->iface->interfaces->mlo_info = pmlo;
	} else {
		pmlo = hapd->iface->interfaces->mlo_info;
	}

	dl_list_for_each(pmld, &(pmlo->mld_info), struct hostapd_mld_unit, list) {
		if (pmld->mld_id == mld_id) {
			mld_add = 0; /* element present */
			break;
		} else if (pmld->mld_id > mld_id) {
			break;
		}
	}
	if (mld_add) {
		/* add mld struct if not present */
		struct hostapd_mld_unit *lmld = os_zalloc(sizeof(*lmld));
		if (!lmld) {
			wpa_printf(MSG_ERROR, "MLD memalloc failed");
			return -1;
		}
		pmlo->mld_count++;
		dl_list_init(&(lmld->list));
		dl_list_init(&(lmld->link_info));
		lmld->link_count = 0;
		lmld->mld_id = mld_id;
		dl_list_add_tail(&(pmld->list), &(lmld->list));
		pmld = lmld;
	}
	hapd->mld_link.mld = pmld;

	dl_list_for_each(plink, &(pmld->link_info), struct hostapd_link_unit, list) {
	...
	if (link_add) {
		...
		struct hostapd_link_unit *llink = os_zalloc(sizeof(*llink));
		if (llink == NULL) {
	...
	hapd->mld_link.link = plink;
	return 0;
}
#endif /* CONFIG_DRIVER_BRCM_MLO */

#ifdef CONFIG_DRIVER_BRCM_MAP
int hostapd_new_dpp_bootstrap_uri(void *ctx, struct wpabuf *buf)
{
...
#ifdef CONFIG_DPP2
	return dpp_relay_bootstrap_uri(hapd->iface->interfaces->dpp, hapd->own_addr,
		hapd->iface->freq, hapd, buf);
#endif /* CONFIG_DPP2 */
...
#ifdef CONFIG_DRIVER_BRCM
#ifdef CONFIG_PASN
	if (bss->pasn_ftm_mode) {
		hapd_iface->drv_flags2 |=
...
#ifdef CONFIG_DRIVER_BRCM_MAP
int hostapd_driver_none_send_general_msg_on_ctrl_socket(struct hostapd_data *hapd, const u8 *addr,
	const char *cmd, const u8 *data, size_t data_len)
{
	...
	if (data && data_len) {
		...
		wpa_msg(hapd->msg_ctx, MSG_INFO, "%s " MACSTR " %s", cmd, MAC2STR(addr), hex);
		os_free(hex);
	} else {
		wpa_msg(hapd->msg_ctx, MSG_INFO, "%s " MACSTR, cmd, MAC2STR(addr));
	}
