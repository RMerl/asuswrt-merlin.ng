...
#ifdef CONFIG_DRIVER_BRCM
				if (hapd->wpa_auth->cb->psk_failure_report)
					hapd->wpa_auth->cb->psk_failure_report(hapd->wpa_auth->cb_ctx, sta->addr);

				wpa_printf(MSG_DEBUG,
					"SAE: %p sae_check_confirm failed from peer" MACSTR "\n",
					hapd->wpa_auth->cb->psk_failure_report,	MAC2STR(sta->addr));
#endif /* CONFIG_DRIVER_BRCM */
				goto reply;
			}
				    ...
				    int cipher, int akmp)
{
	...
	wpa_printf(MSG_DEBUG, "PASN: " MACSTR " present in PTKSA cache",
		   MAC2STR(sta_addr));
#ifdef CONFIG_DRIVER_BRCM
	brcm_set_secure_ranging_ctx(hapd, own_addr, sta_addr, cipher,
		entry->ptk.tk_len, entry->ptk.tk,
		entry->ptk.ltf_keyseed_len,
		entry->ptk.ltf_keyseed,
		entry->ptk.kdk_len,
		entry->ptk.kdk);
#else
			   ...
			   const u8 *ies, size_t ies_len, int reassoc)
{
	...
	/* Handling for WPA3 case */
	if (elems.multi_link) {
		const u8 *mlie = NULL;
		size_t mlielen = 0;
		struct wpabuf *mliebuf;

		sta->flags |= WLAN_STA_MULTI_LINK;
		/* MLIE in received assocreq can be Fragmented take care of it */
		mliebuf = ieee802_11_get_mlie(&elems);
		if (!mliebuf) {
			wpa_printf(MSG_ERROR, "Assoc: MLIE: Missing \n");
			goto override_mlie_parse;
		}
		wpa_hexdump(MSG_DEBUG, "In FUll Assoc Req MLIE for WPA3:",
				wpabuf_head(mliebuf),
				wpabuf_len(mliebuf));
		mlie = wpabuf_head(mliebuf);
		mlielen= wpabuf_len(mliebuf);

		if ((sta->mlsta_info = os_zalloc(sizeof(struct ml_info))) == NULL) {
			wpa_printf(MSG_ERROR, "Assoc: No mem for alloc \n");
			goto override_mlie_parse;
		}
		ieee802_11_mlo_parse_ml_info(sta->mlsta_info, mlie, mlielen);
		wpabuf_free(mliebuf);
	}
override_mlie_parse:
...
#ifdef CONFIG_DRIVER_BRCM_MLO
	if (elems.multi_link && elems.multi_link_len) {
		wpa_printf(MSG_DEBUG, "STA included ML IE in (Re)AssocReq");
		if (hapd->conf->ieee80211be) {
			wpa_auth_set_multi_link(sta->wpa_sm, 1);
			wpa_printf(MSG_DEBUG, "STA set has_ML in sm\n");
		}
		wpa_hexdump(MSG_DEBUG, "MULTI LINK FROM STATION",
			elems.multi_link, elems.multi_link_len);
		if (wpa_auth_get_link_from_mlie(sta->wpa_sm, (u8 *)elems.multi_link)) {
			wpa_printf(MSG_ERROR,"--------MLIE IS CORRUPTED-------");
			return WLAN_STATUS_UNSPECIFIED_FAILURE;
		}
	}
	if (elems.vendor_mlo && elems.vendor_mlo_len) {
		wpa_printf(MSG_ERROR, "STA ASSOC REQ BMP:%x", elems.vendor_mlo[5]);
		wpa_auth_set_sta_link_bmp(sta->wpa_sm, elems.vendor_mlo[5]);
	}
#endif /* CONFIG_DRIVER_BRCM_MLO */
...
#ifdef CONFIG_DRIVER_BRCM_MLO
		/* If MLO is enabled, do not send deauth if the assoc response status is
		 * Invalid PMKID. Deauth will be sent from firmware after assoc response is sent
		 * out.
		 */
		if (!(((hapd->conf->mld_unit != 255) && (hapd->conf->mld_unit != -1) &&
			(hapd->conf->link_id != 255) &&	(hapd->conf->link_id != -1)) &&
			(sta->flags & WLAN_STA_MULTI_LINK) && (resp == WLAN_STATUS_INVALID_PMKID)))
#endif /* CONFIG_DRIVER_BRCM_MLO */
		hostapd_drv_sta_remove(hapd, sta->addr);
		sta->added_unassoc = 0;
	}
...
#ifdef CONFIG_DRIVER_BRCM_MLO
			!((hapd->conf->mld_unit != 255 && hapd->conf->mld_unit != -1) &&
			!(hapd->conf->mld_unit & 0x40)) &&
#endif /* CONFIG_DRIVER_BRCM_MLO */
			1) {
