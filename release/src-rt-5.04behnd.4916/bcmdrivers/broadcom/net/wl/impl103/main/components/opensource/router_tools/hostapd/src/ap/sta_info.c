...
void mlo_ap_sta_freepersta(struct ml_info *mlsta_info)
{
	int i;

	if (!mlsta_info || mlsta_info->numlinks < 1) {
		return;
	}

	for (i = 0; i < (mlsta_info->numlinks - 1); i++) {
		if ((mlsta_info->persta[i]).perstabuf)
			wpabuf_free((mlsta_info->persta[i]).perstabuf);
	}
}
...
#ifdef CONFIG_DRIVER_BRCM_MLO
	mlo_ap_sta_freepersta(sta->mlsta_info);
	if (sta->mlsta_info) {
		os_free(sta->mlsta_info);
	}
#endif /* CONFIG_DRIVER_BRCM_MLO */
