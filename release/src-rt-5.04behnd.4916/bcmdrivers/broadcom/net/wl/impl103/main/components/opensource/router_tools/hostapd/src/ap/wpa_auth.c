...
static inline int wpa_auth_set_kdk(struct wpa_authenticator *wpa_auth,
				   const u8 *peer_addr, u32 cipher,
				   const u8 *kdk,
				   size_t kdk_len)
{
	if (!wpa_auth->cb->set_kdk)
		return -1;
	return wpa_auth->cb->set_kdk(wpa_auth->cb_ctx, peer_addr,
				     cipher, kdk, kdk_len);
}
			   ...
			   int keyidx, int encr)
{
	...
	if (wpa_auth->conf.map_4_way_handshake && wpa_auth->conf.map_eapol_key_timeout)
		timeout_ms = wpa_auth->conf.map_eapol_key_timeout;
...
#ifdef CONFIG_DRIVER_BRCM_MLO
	if (!sm->has_ML || (sm->wpa_auth->conf.link_id == 0)) {
#endif
...
{
#ifdef CONFIG_DRIVER_BRCM_MLO
	/* making space for MLD_MAC_ADDRESS */
	u8 buf[2 * (2 + RSN_SELECTOR_LEN) + PMKID_LEN + MAC_ADDR_LEN];
#else
	...
	if (sm->has_ML) {
		...
		wpa_add_kde(&buf[pmkid_len], RSN_KEY_DATA_MAC_ADDR,
			sm->wpa_auth->addr, MAC_ADDR_LEN,	NULL, 0);
		pmkid_len += 2 + MAC_ADDR_LEN + RSN_SELECTOR_LEN;
		wpa_printf(MSG_DEBUG, "Adding MAC KDE in M1\n");
	}
#endif
...
int fils_set_tk(struct wpa_state_machine *sm)
{
	...
	if (sm->wpa_auth->conf.secure_ltf &&
	    ieee802_11_rsnx_capab(sm->rsnxe, WLAN_RSNX_CAPAB_SECURE_LTF) &&
	    wpa_auth_set_kdk(sm->wpa_auth, sm->addr, sm->pairwise,
			     sm->PTK.kdk,
			     sm->PTK.kdk_len)) {
		wpa_printf(MSG_ERROR,
			   "FILS: Failed to set KDK to driver");
		return -1;
	}
#endif /* CONFIG_DRIVER_BRCM */
#endif /* CONFIG_PASN */
...
#ifdef CONFIG_DRIVER_BRCM_MAP
	/* Do not compare RSNIE while doing 4-way handshake for MAP security */
	if (!wpa_auth->conf.map_4_way_handshake)
#endif	/* CONFIG_DRIVER_BRCM_MAP */
	if (!sm->wpa_ie ||
...
#ifdef CONFIG_DRIVER_BRCM_MAP
	/* Do not check for RSNE while doing 4-way handshake for MAP security */
	if (!wpa_auth->conf.map_4_way_handshake)
#endif	/* CONFIG_DRIVER_BRCM_MAP */
...
#ifdef CONFIG_DRIVER_BRCM_MLO
static u8 * ieee80211w_mlo_kde_add(struct wpa_state_machine *sm,
		u8 *pos, u8 link, struct wpa_authenticator *wpa_auth)
{
	...
	if (gsm->wpa_group_state != WPA_GROUP_SETKEYSDONE ||
	    wpa_auth_get_seqnum(wpa_auth, NULL, gsm->GN_igtk, rsc) < 0)
		os_memset(igtk.ipn, 0, sizeof(igtk.ipn));
	else
		os_memcpy(igtk.ipn, rsc, sizeof(igtk.ipn));
	igtk.link = (link & 0x0f) << 4;
	wpa_printf(MSG_DEBUG,"Calculated igtk link:%02x\n", igtk.link);
	os_memcpy(igtk.mlo_igtk, gsm->IGTK[gsm->GN_igtk - 4], len);
	if (conf->disable_gtk || sm->wpa_key_mgmt == WPA_KEY_MGMT_OSEN) {
		/*
		 * Provide unique random IGTK to each STA to prevent use of
		 * IGTK in the BSS.
		 */
		if (random_get_bytes(igtk.mlo_igtk, len) < 0)
			return pos;
	}
	...
	if (gsm->wpa_group_state != WPA_GROUP_SETKEYSDONE ||
		...
		os_memset(bigtk.bipn, 0, sizeof(bigtk.bipn));
	else
		os_memcpy(bigtk.bipn, rsc, sizeof(bigtk.bipn));
	bigtk.link = (link & 0x0f) << 4;
	wpa_printf(MSG_DEBUG,"Calculated bigtk link:%02x\n", bigtk.link);
	os_memcpy(bigtk.mlo_bigtk, gsm->BIGTK[gsm->GN_bigtk - 6], len);
	if (sm->wpa_key_mgmt == WPA_KEY_MGMT_OSEN) {
		/*
		 * Provide unique random BIGTK to each OSEN STA to prevent use
		 * of BIGTK in the BSS.
		 */
		if (random_get_bytes(bigtk.mlo_bigtk, len) < 0)
			return pos;
	}
...
SM_STATE(WPA_PTK, PTKINITNEGOTIATING)
{
	...
	u8 hdr[2];
	struct wpa_auth_config *conf = &sm->wpa_auth->conf;
#if defined(CONFIG_DRIVER_BRCM_MLO)
	struct hostapd_link_unit *plink = NULL;
	unsigned int mlo_len = 0;
	u8 *mlo_pos = NULL, *mlo_kde = NULL;
#endif /* CONFIG_DRIVER_BRCM_MLO */
...
#if defined(CONFIG_DRIVER_BRCM_MLO)
	/* Send EAPOL(1, 1, 1, Pair, P, 0, ANonce, MIC(PTK), [OCI], MAC,
	 * MLO_LINKn, MLO_GTKn, MLO_IGTKn, [MLO_BIGTKn])
	 */
	if (sm->has_ML) {
		mlo_kde = os_malloc(1536);
		if (!mlo_kde)
		goto done;
...
#endif /* CONFIG_DRIVER_BRCM && CONFIG_DRIVER_BRCM_CEVENT */
	wpa_auth_logger(sm->wpa_auth, sm->addr, LOGGER_DEBUG,
			"sending 3/4 msg of 4-Way MLO Handshake");

	/* Cleaning RSC for MLO cases, RSC included per link */
	os_memset(rsc, 0, WPA_KEY_RSC_LEN);

	/* Add OCV_OCI element */
	if (ocv_oci_add(sm, &mlo_pos, conf->oci_freq_override_eapol_m3) < 0)
		goto done;

	/* MAC address KDE --need to fill mld_mac address*/
	mlo_pos = wpa_add_kde(mlo_pos, RSN_KEY_DATA_MAC_ADDR,
		sm->wpa_auth->addr, MAC_ADDR_LEN, NULL, 0);

	/* Tuple for each link contains
	 * MLO_LINK[N], MLO_GTL[N], MLO_IGTK[N], MLO_BIGTK[N] KDE
	 */

	if(conf) {
		wpa_printf(MSG_DEBUG,"link_info:%p ,count:%d, mld_unit:%d\n",
			conf->link_info, conf->link_count, conf->mld_unit);
	}
	dl_list_for_each(plink, conf->link_info, struct hostapd_link_unit, list)
	{
		struct hostapd_data *phapd = plink->hapd;
		struct wpa_authenticator *pwpa_auth = phapd->wpa_auth;
		struct wpa_group *pgsm = pwpa_auth->group;
		struct wpa_auth_config *pconf = &(pwpa_auth->conf);

		struct wpa_mlo_link_kde mlo_link;
		struct wpa_mlo_gtk_kde mlo_gtk;
		bool RSNE_PRSNT = 0;
		bool RSNXE_PRSNT = 0;
		if (!(sm->sta_link_bmp & (1 << plink->link_id))) {
			wpa_printf(MSG_ERROR," NO matching link found"
				"skip mlo_set for Link:%d sta_bmp:%x\n",
				plink->link_id, sm->sta_link_bmp);
			continue;
		}

		/* fill link_kde */
#if defined(CONFIG_DRIVER_BRCM) && defined(CONFIG_DRIVER_BRCM_CEVENT)
		wpa_printf(MSG_DEBUG,"bss:%s link:%d\n", ifname, plink->link_id);
		wpa_printf(MSG_DEBUG,"MAC addr:"MACSTR "\n", MAC2STR(pwpa_auth->addr));
#endif /* CONFIG_DRIVER_BRCM && CONFIG_DRIVER_BRCM_CEVENT */
		wpa_ie = pwpa_auth->wpa_ie;
		wpa_ie_len = pwpa_auth->wpa_ie_len;
		wpa_printf(MSG_DEBUG, "WPA_VERSION: %x, WPA_PROTO: %x,"
				" wpa_ie_len:%d, WPA_EID:%x\n",	sm->wpa,
				pconf->wpa, wpa_ie_len, wpa_ie[0]);
		if (sm->wpa == WPA_VERSION_WPA2 &&
				(pconf->wpa & WPA_PROTO_RSN) &&
				wpa_ie[0] == WLAN_EID_RSN) {
			u8 *lwpa_ie = wpa_ie;
			int lwpa_ie_len = wpa_ie_len;
			RSNE_PRSNT = 1;
			wpa_printf(MSG_DEBUG," RSNE is present\n");
			while (lwpa_ie_len > lwpa_ie[1] + 2U) {
				lwpa_ie_len -= (lwpa_ie[1] + 2);
				lwpa_ie = lwpa_ie +lwpa_ie[1] + 2;
				if(lwpa_ie[0] == WLAN_EID_RSNX) {
					RSNXE_PRSNT = 1;
					wpa_printf(MSG_DEBUG," RSNXE is present\n");
				}
				if (lwpa_ie[0] == WLAN_EID_MOBILITY_DOMAIN) {
					wpa_printf(MSG_ERROR,"EID is mobility\n");
				}
			}
		} else if (sm->wpa == WPA_VERSION_WPA &&
				(pconf->wpa & WPA_PROTO_RSN) &&
				wpa_ie_len > wpa_ie[1] + 2U &&
				wpa_ie[0] == WLAN_EID_RSN) {
		...
		wpa_printf(MSG_DEBUG,"LINE:%d RSNE:%d RSNXE:%d wpa_ie_len:%d wpa_ie:%p\n",
				__LINE__, RSNE_PRSNT, RSNXE_PRSNT, wpa_ie_len, wpa_ie);
#ifdef CONFIG_TESTING_OPTIONS
		if (pconf->rsne_override_eapol_set) {
			wpa_ie_buf2 = replace_ie(
				"RSNE", wpa_ie, &wpa_ie_len, WLAN_EID_RSN,
				pconf->rsne_override_eapol,
				pconf->rsne_override_eapol_len);
			if (!wpa_ie_buf2)
				goto done;
			wpa_ie = wpa_ie_buf2;
			RSNE_PRSNT = 1;
			wpa_printf(MSG_DEBUG,"LINE:%d RSNE:%d RSNXE:%d\n",
				__LINE__, RSNE_PRSNT, RSNXE_PRSNT);
		}
		if (pconf->rsnxe_override_eapol_set) {
			wpa_ie_buf = replace_ie(
				"RSNXE", wpa_ie, &wpa_ie_len, WLAN_EID_RSNX,
				pconf->rsnxe_override_eapol,
				pconf->rsnxe_override_eapol_len);
			if (!wpa_ie_buf)
				goto done;
			wpa_ie = wpa_ie_buf;
			RSNXE_PRSNT = 1;
...
#endif /* CONFIG_TESTING_OPTIONS */
		mlo_link.link = ((plink->link_id) | (RSNE_PRSNT << 4) |
			(RSNXE_PRSNT << 5)) ;
		wpa_printf(MSG_ERROR,"%s %d link_info: %08x\n", __FUNCTION__, __LINE__, mlo_link.link);
		os_memcpy(mlo_link.MAC, pwpa_auth->addr, 6);
		mlo_pos = wpa_add_kde(mlo_pos, RSN_KEY_DATA_MLO_LINK,
			(u8 *)&mlo_link, WPA_MLO_LINK_KDE_PREFIX_LEN,
			wpa_ie, wpa_ie_len);
		/* fill mlo_link_kde done */
		if (sm->wpa == WPA_VERSION_WPA2) {
			if (sm->use_ext_key_id && sm->TimeoutCtr == 1 &&
					wpa_auth_set_key(sm->wpa_auth, 0,
					wpa_cipher_to_alg(sm->pairwise),
					sm->addr, sm->keyidx_active, sm->PTK.tk,
			...
			gtk = pgsm->GTK[gsm->GN - 1];
			/* WPA2 send GTK in the 4-way handshake */
			secure = 1;
			gtk = pgsm->GTK[gsm->GN - 1];
			gtk_len = pgsm->GTK_len;
			if (pconf->disable_gtk ||
					sm->wpa_key_mgmt == WPA_KEY_MGMT_OSEN) {
				/*
				 * Provide unique random GTK to each STA to prevent use
				 * of GTK in the BSS.
		...
		/* fill MLO_GTK_KDE */
		mlo_gtk.keyid = ((gsm->GN & 0x03) | (plink->link_id<<4));
		/* added keyid and linkid */
		if (gsm->wpa_group_state != WPA_GROUP_SETKEYSDONE ||
				wpa_auth_get_seqnum(pwpa_auth, NULL, gsm->GN_igtk, rsc) < 0)
			os_memset(mlo_gtk.pn, 0, sizeof(mlo_gtk.pn));
		else
			os_memcpy(mlo_gtk.pn, rsc, sizeof(mlo_gtk.pn));

		mlo_pos = wpa_add_kde(mlo_pos, RSN_KEY_DATA_MLO_GTK,(u8 *)&mlo_gtk,
				WPA_MLO_GTK_KDE_PREFIX_LEN, gtk, gsm->GTK_len);
		mlo_pos = ieee80211w_mlo_kde_add(sm, mlo_pos, plink->link_id, pwpa_auth);
	}

	/* Cleaning RSC for MLO cases, RSC included per link */
	os_memset(rsc, 0, WPA_KEY_RSC_LEN);
	_rsc = rsc;
	mlo_len = (mlo_pos - mlo_kde);
	/*added the previously calculated MLO len */
	kde_len += mlo_len;
	} else {
#endif /* CONFIG_DRIVER_BRCM_MLO */
	...
	if (sm->wpa == WPA_VERSION_WPA2) {
		...
		if (sm->wpa_auth->conf.secure_ltf &&
		    ...
		    wpa_auth_set_kdk(sm->wpa_auth, sm->addr, sm->pairwise,
				     ...
				     sm->PTK.kdk_len)) {
			wpa_printf(MSG_ERROR,
				   "WPA: Failed to set KDK to driver");
			wpa_sta_disconnect(sm->wpa_auth, sm->addr,
					   WLAN_REASON_PREV_AUTH_NOT_VALID);
			return;
		}
...
#ifdef CONFIG_DRIVER_BRCM_MAP
	/* Do not include GTK in 3/4 Message if sending it from Agent */
	if (conf->map_4_way_handshake == MAP_4_WAY_HANDSHAKE_AGT) {
		gtk = NULL;
		gtk_len = 0;
		_rsc = NULL;
		secure = 1;
		wpa_printf(MSG_INFO, "Not adding MAP GTK in 3/4 Message when sending "
			"it from Agent");
	}
#endif	/* CONFIG_DRIVER_BRCM_MAP */
...
#ifdef CONFIG_DRIVER_BRCM_MLO
	if (sm->has_ML) {
		/* copied the filled mlo_kde in normal kde */
		memcpy(kde, mlo_kde, mlo_len);
		pos += mlo_len;
	} else {
#endif /* CONFIG_DRIVER_BRCM_MLO */
	...
	if (gtk) {
		hdr[0] = gtkidx & 0x03;
#ifdef CONFIG_DRIVER_BRCM_MAP
		if (conf->map_4_way_handshake) {
			pos = wpa_add_kde(pos, RSN_KEY_DATA_MAP_GROUPKEY, hdr, 1,
				  gtk, gtk_len);
		} else
#endif	/* CONFIG_DRIVER_BRCM_MAP */
	...
	if (sm->wpa == WPA_VERSION_WPA2) {
		...
		if (sm->has_ML) {
			...
			dl_list_for_each(plink, conf->link_info, struct hostapd_link_unit, list)
			{
				...
				pos = wpa_add_kde(pos, RSN_KEY_DATA_MLO_GTK,(u8 *)&mlo_gtk,
						WPA_MLO_GTK_KDE_PREFIX_LEN, gtk, gsm->GTK_len);
				pos = ieee80211w_mlo_kde_add(sm, pos, plink->link_id, pwpa_auth);
			}
		} else {
#endif /* CONFIG_DRIVER_BRCM_MLO */
		kde_len = 2 + RSN_SELECTOR_LEN + 2 + gsm->GTK_len +
			  ...
			  struct wpa_group *group)
{
	...
	if (conf->ieee80211w != NO_MGMT_FRAME_PROTECTION &&
		...
		inc_byte_array(group->Counter, WPA_NONCE_LEN);
#ifdef CONFIG_DRIVER_BRCM
		if (!is_6ghz_freq(hapd->iface->freq) || (hostapd_mac_comp(hapd->own_addr,
			hapd->iface->bss[0]->own_addr) == 0)) {
#endif /* CONFIG_DRIVER_BRCM */
		if (wpa_gmk_to_gtk(group->GMK, "BIGTK key expansion",
			...
			/* For 6G virtual interface, BIGTK is same as that of primary */
			struct hostapd_data *primary_hapd = hapd->iface->bss[0];
			os_memcpy(group->BIGTK[group->GN_bigtk - 6],
				primary_hapd->wpa_auth->group->BIGTK[group->GN_bigtk - 6], len);
		}
#endif /* CONFIG_DRIVER_BRCM */
...
#ifdef CONFIG_DRIVER_BRCM_RDKB_PMK_LIFETIME
		conf->dot11RSNAConfigPMKLifetime,
#else /* CONFIG_DRIVER_BRCM_RDKB_PMK_LIFETIME */
		dot11RSNAConfigPMKLifetime,
#endif /* CONFIG_DRIVER_BRCM_RDKB_PMK_LIFETIME */
...
#endif /* CONFIG_DPP2 */
#ifdef CONFIG_DRIVER_BRCM_MLO
void wpa_auth_set_multi_link(struct wpa_state_machine *sm, bool flag)
{
	if (sm)
		sm->has_ML = flag;
}
void wpa_auth_set_sta_multi_link(struct wpa_state_machine *sm, bool flag)
{
	if (sm)
		sm->sta_has_ML = flag;
}
void wpa_auth_set_sta_link_bmp(struct wpa_state_machine *sm, int val)
{
	if (sm)
		sm->sta_link_bmp = val;
}
int wpa_auth_get_link_from_mlie(struct wpa_state_machine *sm, u8 *mlie)
{
	if (!(mlie[1] & 0x01)) {
		wpa_printf(MSG_ERROR, "NO MLD_CAP IN MLIE, FORCE SINGLE LINK\n");
		wpa_auth_set_sta_multi_link(sm, 0);
	} else {
		u8 *parse_ptr;
		int exp_len = mlie[2];
		if (exp_len > 7) {
			...
			wpa_printf(MSG_ERROR, "ML IE COMMON INFO LEN FAIL :%d\n",
					mlie[2]);
			return -1;
		}
		if (mlie[0] & 0x10) {
		...
		if (exp_len >= 2) {
			wpa_printf(MSG_ERROR, "STA REPORTED NUM LINKS: %d\n",
					(parse_ptr[0] & 0x0f) + 1);
			wpa_auth_set_sta_multi_link(sm, !!(parse_ptr[0] & 0x0f));
		} else {
			wpa_printf(MSG_ERROR, "ML IE COMMON INFO LEN FAIL :%d\n",
					mlie[2]);
			return -1;
...
#ifdef CONFIG_DRIVER_BRCM_MAP
void wpa_auth_set_map_params(struct wpa_state_machine *sm, enum map_wpa_sm_conf_params param,
	unsigned int value)
{
	int ret = 0;
...
void wpa_auth_set_map_addr(struct wpa_authenticator *wpa_auth, u8 *addr)
{
	os_memcpy(wpa_auth->addr, addr, ETH_ALEN);
}

void wpa_auth_set_map_pksa_to_sm(struct wpa_state_machine *sm, u8 *sta_addr,
	struct wpa_authenticator *wpa_auth)
{
	sm->pmksa = pmksa_cache_auth_get(wpa_auth->pmksa, sta_addr, NULL);
	if (!sm->pmksa) {
		wpa_printf(MSG_ERROR, "WPA: PMKSA cache entry for MAP Device "
			   MACSTR " Not found", MAC2STR(sta_addr));
	}
}
#endif	/* CONFIG_DRIVER_BRCM_MAP */
