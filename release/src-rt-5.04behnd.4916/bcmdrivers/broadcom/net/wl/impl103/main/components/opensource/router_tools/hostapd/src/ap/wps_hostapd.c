...
static int hostapd_dpp_bootstrap_uri_cb(void *ctx, u8 *uri, u16 uri_len, u8 *bsta_mac, u8 *own_mac);
...
#ifdef CONFIG_DRIVER_BRCM_MAP
	cfg.new_dpp_bootstrap_uri_cb = hostapd_dpp_bootstrap_uri_cb;
#endif /* CONFIG_DRIVER_BRCM_MAP */
...
{
	struct wpabuf *buf;
	int buf_len = 4 /* total len */ + 12 /* 3 attr[2byte]+len[2byte] pair */ + uri_len +
		2 * ETH_ALEN /* bsta_mac and own_mac */;
	int ret;

	buf = wpabuf_alloc(buf_len);
	if (!buf) {
		wpa_printf(MSG_ERROR, "WPS: %s failed to allocate buf of size %d", __func__, buf_len);
		return -1;
	}

	/* Total len */
	wpabuf_put_be32(buf, buf_len - 4);
	/* Dpp Uri */
	wpabuf_put_be16(buf, ATTR_DPP_BOOTSTRAP_URI);
	wpabuf_put_be16(buf, uri_len);
	wpabuf_put_data(buf, uri, uri_len);
	/* BSTA Mac */
	wpabuf_put_be16(buf, ATTR_MAC_ADDR);
	wpabuf_put_be16(buf, ETH_ALEN);
	wpabuf_put_data(buf, bsta_mac, ETH_ALEN);
	/* Own Mac */
	wpabuf_put_be16(buf, ATTR_MAC_ADDR);
	wpabuf_put_be16(buf, ETH_ALEN);
	wpabuf_put_data(buf, own_mac, ETH_ALEN);

	ret = hostapd_new_dpp_bootstrap_uri(ctx, buf);
	if (ret != 0) {
		wpabuf_free(buf);
	}
...
int hostapd_wps_button_pushed_ex(struct hostapd_data *hapd, int ieee1905_tk)
{
	wps_registrar_update_ieee1905_tk(hapd->wps->registrar, ieee1905_tk);
	return hostapd_wps_button_pushed(hapd, NULL);
}
...
{
	struct hostapd_bss_config *conf = hapd->conf;
	size_t ssid_len, key_len;

	if (!(conf->multi_ap & FRONTHAUL_BSS) || !ssid || !key)
		return -1;
	...
	if (ssid_len <= 0 || ssid_len > SSID_MAX_LEN ||
		key_len < MIN_PASSPHRASE_LEN || key_len > MAX_PASSPHRASE_LEN)
		return -1;

	return wps_registrar_update_multi_ap(hapd->wps->registrar, ssid, ssid_len, key, key_len);
}
#endif /* CONFIG_DRIVER_BRCM_MAP */
