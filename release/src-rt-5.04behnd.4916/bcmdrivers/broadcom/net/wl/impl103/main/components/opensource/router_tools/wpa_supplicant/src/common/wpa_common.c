...
static int wpa_parse_generic(const u8 *pos, struct wpa_eapol_ie_parse *ie)
{
	...
	if (pos[1] > RSN_SELECTOR_LEN + 2 &&
	    RSN_SELECTOR_GET(pos + 2) == RSN_KEY_DATA_MAP_GROUPKEY) {
		ie->gtk = pos + 2 + RSN_SELECTOR_LEN;
		ie->gtk_len = pos[1] - RSN_SELECTOR_LEN;
		wpa_hexdump(MSG_DEBUG, "WPA: MAP KDE in EAPOL-Key",
				pos, pos[1] + 2);
		return 0;
	}
	...
	if (pos[1] > RSN_SELECTOR_LEN + 2 &&
		...
		ie->mlo_gtk[ie->mlo_gtk_count] = pos + 2 + RSN_SELECTOR_LEN;
		ie->mlo_gtk_len[ie->mlo_gtk_count] = pos[1] - RSN_SELECTOR_LEN;
	...
	if (pos[1] > RSN_SELECTOR_LEN + 2 &&
		...
		ie->mlo_igtk[ie->mlo_igtk_count] = pos + 2 + RSN_SELECTOR_LEN;
		ie->mlo_igtk_len[ie->mlo_igtk_count] = pos[1] - RSN_SELECTOR_LEN;
	...
	if (pos[1] > RSN_SELECTOR_LEN + 2 &&
		...
		ie->mlo_bigtk[ie->mlo_bigtk_count] = pos + 2 + RSN_SELECTOR_LEN;
		ie->mlo_bigtk_len[ie->mlo_bigtk_count] = pos[1] - RSN_SELECTOR_LEN;
	...
	if (pos[1] > RSN_SELECTOR_LEN + 2 &&
		...
		struct wpa_mlo_link_kde *l_link =
			(struct wpa_mlo_link_kde *)(pos + 2 + RSN_SELECTOR_LEN);
		u8 *l_pos = l_link->data;

		if (ie->mlo_link_count >= MAX_NUM_LINKS) {
			wpa_printf(MSG_MSGDUMP,
				"ERROR : MLO links parsed more than MAX_NUM_LINKS(4)");
			return -1;
		}

		/* Save MLO link KDE, which contain BSSID:RSNIE:RSNXE */
		ie->mlo_link_kde[ie->mlo_link_count] = (const u8 *)l_link;
		ie->mlo_link_count++;
