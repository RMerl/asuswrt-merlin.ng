...
#ifdef CONFIG_DRIVER_NL80211_BRCM
#ifdef CONFIG_PASN
static void brcm_nl80211_pasn_auth_start(struct wpa_driver_nl80211_data *drv,
				       const u8 *data, size_t len)
{
        int ret = -EINVAL;
        struct nlattr *attr;
	struct nlattr *tb[BRCM_WLAN_VENDOR_ATTR_PASN_MAX + 1];
        struct nlattr *cfg[BRCM_WLAN_VENDOR_ATTR_PASN_PEER_MAX + 1];
        unsigned int n_peers = 0, idx = 0;
        int rem_conf;
        enum brcm_wlan_vendor_pasn_action action;
        union wpa_event_data event;

        if (nla_parse(tb, BRCM_WLAN_VENDOR_ATTR_PASN_MAX,
                      (struct nlattr *) data, len, NULL) ||
            !tb[BRCM_WLAN_VENDOR_ATTR_PASN_PEERS] ||
            !tb[BRCM_WLAN_VENDOR_ATTR_PASN_ACTION]) {
                return;
        }

        os_memset(&event, 0, sizeof(event));
        action = nla_get_u32(tb[BRCM_WLAN_VENDOR_ATTR_PASN_ACTION]);
        switch (action) {
        case BRCM_WLAN_VENDOR_PASN_ACTION_AUTH:
                event.pasn_auth.action = PASN_ACTION_AUTH;
                break;
        case BRCM_WLAN_VENDOR_PASN_ACTION_DELETE_SECURE_RANGING_CONTEXT:
        ...
        nla_for_each_nested(attr, tb[BRCM_WLAN_VENDOR_ATTR_PASN_PEERS], rem_conf)
                n_peers++;
        ...
        nla_for_each_nested(attr, tb[BRCM_WLAN_VENDOR_ATTR_PASN_PEERS],
                            rem_conf) {
                struct nlattr *nl_src, *nl_peer;
                ret = nla_parse_nested(cfg, BRCM_WLAN_VENDOR_ATTR_PASN_PEER_MAX,
                                       attr, NULL);
                if (ret)
                        return;
                nl_src = cfg[BRCM_WLAN_VENDOR_ATTR_PASN_PEER_SRC_ADDR];
                nl_peer = cfg[BRCM_WLAN_VENDOR_ATTR_PASN_PEER_MAC_ADDR];
                if (nl_src) {
                ...
                if (cfg[BRCM_WLAN_VENDOR_ATTR_PASN_PEER_LTF_KEYSEED_REQUIRED])
        ...
        wpa_printf(MSG_DEBUG,
                   "nl80211: PASN auth action: %u, num_bssids: %d idx %d "
		   "Own addr " MACSTR " peer addr " MACSTR "\n",
                   event.pasn_auth.action,
                   event.pasn_auth.num_peers, idx,
		   MAC2STR(event.pasn_auth.peer[idx - 1].own_addr),
		   MAC2STR(event.pasn_auth.peer[idx - 1].peer_addr));
...
#ifdef CONFIG_DRIVER_NL80211_BRCM
#ifdef CONFIG_PASN
	case BRCM_VENDOR_EVENT_PASN:
		brcm_nl80211_pasn_auth_start(drv, data, len);
		break;
#endif /* CONFIG_PASN */
#endif /* CONFIG_DRIVER_NL80211_BRCM */
	default:
				 ...
				 struct nlattr **tb)
{
	...
	case NL80211_CMD_CONTROL_PORT_FRAME_TX_STATUS:
		...
		nl80211_control_port_frame_tx_status(drv,
						     ...
						     tb[NL80211_ATTR_COOKIE]);
		break;
#ifdef CONFIG_DRIVER_BRCM_SAE_FEATURE
