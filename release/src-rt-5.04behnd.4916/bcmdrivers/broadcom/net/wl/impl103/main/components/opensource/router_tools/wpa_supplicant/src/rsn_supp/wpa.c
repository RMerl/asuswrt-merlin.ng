...
static int wpa_supplicant_process_1_of_2_mlo(struct wpa_sm *sm,
		const struct wpa_eapol_key *key,
		const u8 *keydata,
		size_t keydatalen, u16 ver);
			  ...
			  const struct wpa_eapol_key *key, struct wpa_ptk *ptk)
{
	...
	ret =  wpa_pmk_to_ptk(sm->pmk, sm->pmk_len, "Pairwise key expansion",
			      sm->own_addr,
#ifdef CONFIG_DRIVER_BRCM_MLO
			      sm->mld_present? sm->ap_mld_addr : sm->bssid,
#else
			      sm->bssid,
#endif /* CONFIG_DRIVER_BRCM_MLO */
			      sm->snonce, key->key_nonce, ptk, akmp,
					  ...
					  size_t key_data_len)
{
...
#ifdef CONFIG_DRIVER_BRCM_MLO
	if (sm->has_ML && sm->bss_ml_capable) {
		if (sm->mlo_ie && sm->mlo_ie_len) {
			/* point to multi_link_control */
			u16 ml_ctrl = WPA_GET_LE16(sm->mlo_ie);
			u8 ml_len = sm->mlo_ie[2];
			if (ml_len >= 7) {
				os_memcpy(mld_mac_addr, &(sm->mlo_ie[3]),
					MAC_ADDR_LEN);
			}
			if ((ml_len >= 9) && (ml_ctrl & 0x0100)) {
				/* get info from mld_cap */
				num_links = sm->mlo_ie[9] & 0x0f;
			}
			/* move to link_info (ml_ctrl_size + ml_len)*/
			traverse = sm->mlo_ie + 2 + ml_len;
			for( i = 0 ; i < num_links; i++) {
				/*
				 * octets in per_sta sub-element
				 * ___________________________________________________________
				 * | psta_id[1] | psta_len[1] | sta_ctrl[2] | sta_inf_len[1] |
				 * _____________________________
				 * | stmac_addr[6] | *sta_prof |
				 * -----------------------------
				 */
				u8 persta_len = traverse[1];
				u16 sta_ctrl = WPA_GET_LE16(&traverse[2]);
				u8 sta_info_len = traverse[4];
				u8 *sta_mac_addr = &traverse[5]; //starts from traverse[5]
				u8 *sta_profile;
				u8 *sta_ie;
				uint sta_profile_len;
				struct ieee802_11_elems req_elems;
				if(sta_info_len >= 7) {
					wpa_hexdump(MSG_ERROR, "SELF MAC", sta_mac_addr,
							MAC_ADDR_LEN);
					os_memcpy(link_mac[i], sta_mac_addr, MAC_ADDR_LEN);
				}
				links[i] = sta_ctrl & 0x0f;
				link_len[i] += 7; //link_id_size + mac_addr_size
				/* Get sta_profile pointer by moving to
				 * per_sta_subelem_sz + persta_len_sz + sta_ctrl_sz + sta_info_len
				 */
				sta_profile = traverse + 2 + 2 + sta_info_len;
				sta_profile_len = persta_len - 2 - sta_info_len;
				sta_ie = sta_profile + 2; //remove fixed parameters
				link_data[i] = NULL;
				if (ieee802_11_parse_elems(sta_ie, sta_profile_len -2,
						&req_elems, 0) != ParseFailed) {
					bool RSNE_PRSNT = 0;
					bool RSNXE_PRSNT = 0;
					int req_len = 0;
					if(req_elems.rsn_ie && req_elems.rsn_ie_len) {
						RSNE_PRSNT = 1;
						req_len += req_elems.rsn_ie_len;
					}
					if(req_elems.rsnxe && req_elems.rsnxe_len) {
						RSNXE_PRSNT = 1;
						req_len += req_elems.rsnxe_len;
					}
					link_data[i] = os_malloc(req_len);
					os_memcpy(link_data[i], req_elems.rsn_ie,
						req_elems.rsn_ie_len);
					os_memcpy(link_data[i] + req_elems.rsn_ie_len,
						req_elems.rsnxe, req_elems.rsnxe_len);
					links[i] = links[i] | (RSNE_PRSNT << 4) |
						(RSNXE_PRSNT << 5);
					link_len[i] += req_len;
				} else {
					wpa_printf(MSG_ERROR,
						"For link %d ieee802_11_parse_elems failed. "
						"sta_profile_len %d persta_len %d link_len[i] %d "
						"sta_info_len %d ml_len %d num_links %d. Fix it.",
						i, sta_profile_len, persta_len, link_len[i],
						sta_info_len, ml_len, num_links);
				}
				traverse += 2 + persta_len;
			}
		}
		kde_buf = os_malloc(kde_len +
			    ...
			    2 + RSN_SELECTOR_LEN + MAC_ADDR_LEN +
			    num_links * (2 + RSN_SELECTOR_LEN) +
			    link_len[0] + link_len[1] + link_len[2] + link_len[3] +
	...
	if (sm->has_ML && sm->bss_ml_capable) {
		...
		*pos++ = RSN_SELECTOR_LEN + MAC_ADDR_LEN;
		RSN_SELECTOR_PUT(pos, RSN_KEY_DATA_MAC_ADDR);
		pos += RSN_SELECTOR_LEN;
		if (mld_mac_addr) {
			os_memcpy(pos, mld_mac_addr, MAC_ADDR_LEN);
		} else {
			os_memcpy(pos, sm->own_addr, MAC_ADDR_LEN);
		}
		kde_len += 2 + MAC_ADDR_LEN + RSN_SELECTOR_LEN;
		for (i = 0; i < num_links; i++) {
			if (link_data[i] == NULL) {
				continue;
			}
			...
			// dd:len
			*pos++ = WLAN_EID_VENDOR_SPECIFIC;
			*pos++ = RSN_SELECTOR_LEN + link_len[i];
			// 00:0f:ac:13
			RSN_SELECTOR_PUT(pos, RSN_KEY_DATA_MLO_LINK);
			pos += RSN_SELECTOR_LEN;
			// link_id + rsne_prsnt + rsnxe_prsnt
			*pos++ = links[i];
			// sta_mac_addr
			os_memcpy(pos, link_mac[i], MAC_ADDR_LEN);
			pos += MAC_ADDR_LEN;
			// variable length for rsne and rsnxe
			os_memcpy(pos, link_data[i], link_len[i] - 7);
			pos += link_len[i] - 7;
			kde_len += 2 + RSN_SELECTOR_LEN + link_len[i];
			os_free(link_data[i]);
		}
	}
	...
	/* Do not add DPP KDE if the 4-way handshake is for 1905 security */
	if (!sm->map_4_way_handshake)
...
#if defined(CONFIG_DRIVER_BRCM) && defined(CONFIG_DRIVER_BRCM_CEVENT)
	CE_SEND_CEVENT_A2C_EXT(sm->ifname, (struct ether_addr *)sm->bssid, 0, 0,
		CEVENT_ST_WPASUPP, CEVENT_A2C_MT_M2_TX, CEVENT_FRAME_DIR_TX, NULL, 0);
#endif /* CONFIG_DRIVER_BRCM && CONFIG_DRIVER_BRCM_CEVENT */
	os_free(kde_buf);
				      ...
				      enum key_flag key_flag)
{
	...
	if (wpa_sm_set_key(sm, alg,
#ifdef CONFIG_DRIVER_BRCM_MLO
			   sm->mld_present? sm->ap_mld_addr : sm->bssid,
#else
			   sm->bssid,
#endif /* CONFIG_DRIVER_BRCM_MLO */
			   sm->keyidx_active, 1, key_rsc,
...
static int wpa_supplicant_pairwise_mlo_gtk(struct wpa_sm *sm,
       const struct wpa_eapol_key *key,
       const u8 *mlo_gtk, size_t mlo_gtk_len,
       int key_info)
{
	...
	wpa_hexdump_key(MSG_DEBUG, "RSN: received MLO_GTK in pairwise handshake",
			mlo_gtk, mlo_gtk_len);

	if (mlo_gtk_len < 7 || mlo_gtk_len - 7 > sizeof(gd.gtk))
		return -1;

	gd.keyidx = mlo_gtk[0];
	gd.tx = wpa_supplicant_gtk_tx_bit_workaround(sm,
						     !!(mlo_gtk[0] & BIT(2)));
	mlo_gtk += WPA_MLO_GTK_KDE_PREFIX_LEN;
	mlo_gtk_len -= WPA_MLO_GTK_KDE_PREFIX_LEN;

	os_memcpy(gd.gtk, mlo_gtk, mlo_gtk_len);
	gd.gtk_len = mlo_gtk_len;
...
static int wpa_supplicant_install_mlo_igtk(struct wpa_sm *sm,
       const struct wpa_mlo_igtk_kde *mlo_igtk,
       int wnm_sleep, int link_id)
{
	size_t len = wpa_cipher_key_len(sm->mgmt_group_cipher);
	u16 keyidx = WPA_GET_LE16(mlo_igtk->keyid);

	keyidx = keyidx | (link_id << 4);

	/* Detect possible key reinstallation */
	if ((sm->igtk.igtk_len == len &&
	     os_memcmp(sm->igtk.igtk, mlo_igtk->mlo_igtk, sm->igtk.igtk_len) == 0) ||
	    (sm->igtk_wnm_sleep.igtk_len == len &&
	     os_memcmp(sm->igtk_wnm_sleep.igtk, mlo_igtk->mlo_igtk,
		       sm->igtk_wnm_sleep.igtk_len) == 0)) {
		wpa_dbg(sm->ctx->msg_ctx, MSG_DEBUG,
			"WPA: Not reinstalling already in-use MLO_IGTK to the driver (keyidx=%d)",
			keyidx);
		return  0;
	}

	wpa_dbg(sm->ctx->msg_ctx, MSG_DEBUG,
		"WPA: MLO_IGTK keyid %d pn " COMPACT_MACSTR,
		keyidx, MAC2STR(mlo_igtk->ipn));
	wpa_hexdump_key(MSG_DEBUG, "WPA: MLO_IGTK", mlo_igtk->mlo_igtk, len);
	if (keyidx > 4095) {
		wpa_msg(sm->ctx->msg_ctx, MSG_WARNING,
			"WPA: Invalid MLO_IGTK KeyID %d", keyidx);
		return -1;
	}
	if (wpa_sm_set_key(sm, wpa_cipher_to_alg(sm->mgmt_group_cipher),
			   broadcast_ether_addr,
			   keyidx, 0, mlo_igtk->ipn, sizeof(mlo_igtk->ipn),
			   mlo_igtk->mlo_igtk, len, KEY_FLAG_GROUP_RX) < 0) {
		if (keyidx == 0x0400 || keyidx == 0x0500) {
			wpa_msg(sm->ctx->msg_ctx, MSG_INFO,
				"WPA: Ignore MLO_IGTK configuration error due"
				" to invalid MLO_IGTK KeyID byte order");
		} else {
			wpa_msg(sm->ctx->msg_ctx, MSG_WARNING,
				"WPA: Failed to configure MLO_IGTK to the driver");
			return -1;
		}
	}
	...
	if (wnm_sleep) {
		sm->igtk_wnm_sleep.igtk_len = len;
		os_memcpy(sm->igtk_wnm_sleep.igtk, mlo_igtk->mlo_igtk,
			  sm->igtk_wnm_sleep.igtk_len);
	} else {
		sm->igtk.igtk_len = len;
		os_memcpy(sm->igtk.igtk, mlo_igtk->mlo_igtk, sm->igtk.igtk_len);
	}
	return 0;
}
static int wpa_supplicant_install_mlo_bigtk(struct wpa_sm *sm,
       const struct wpa_mlo_bigtk_kde *mlo_bigtk,
       int wnm_sleep)
{
	size_t len = wpa_cipher_key_len(sm->mgmt_group_cipher);
	u16 keyidx = WPA_GET_LE16(mlo_bigtk->keyid);

	/* Detect possible key reinstallation */
	if ((sm->bigtk.bigtk_len == len &&
	     os_memcmp(sm->bigtk.bigtk, mlo_bigtk->mlo_bigtk,
		       sm->bigtk.bigtk_len) == 0) ||
	    (sm->bigtk_wnm_sleep.bigtk_len == len &&
	     os_memcmp(sm->bigtk_wnm_sleep.bigtk, mlo_bigtk->mlo_bigtk,
		       sm->bigtk_wnm_sleep.bigtk_len) == 0)) {
		wpa_dbg(sm->ctx->msg_ctx, MSG_DEBUG,
			"WPA: Not reinstalling already in-use MLO_BIGTK to the driver (keyidx=%d)",
			keyidx);
		return  0;
	}

	wpa_dbg(sm->ctx->msg_ctx, MSG_DEBUG,
		"WPA: MLO_BIGTK keyid %d bipn " COMPACT_MACSTR,
		keyidx, MAC2STR(mlo_bigtk->bipn));
	wpa_hexdump_key(MSG_DEBUG, "WPA: MLO_BIGTK", mlo_bigtk->mlo_bigtk, len);
	if (keyidx < 6 || keyidx > 7) {
		wpa_msg(sm->ctx->msg_ctx, MSG_WARNING,
			"WPA: Invalid MLO_BIGTK KeyID %d", keyidx);
		return -1;
	}
	if (wpa_sm_set_key(sm, wpa_cipher_to_alg(sm->mgmt_group_cipher),
			   broadcast_ether_addr,
			   keyidx, 0, mlo_bigtk->bipn, sizeof(mlo_bigtk->bipn),
			   mlo_bigtk->mlo_bigtk, len, KEY_FLAG_GROUP_RX) < 0) {
		wpa_msg(sm->ctx->msg_ctx, MSG_WARNING,
			"WPA: Failed to configure MLO_BIGTK to the driver");
		return -1;
	}

	if (wnm_sleep) {
		sm->bigtk_wnm_sleep.bigtk_len = len;
		os_memcpy(sm->bigtk_wnm_sleep.bigtk, mlo_bigtk->mlo_bigtk,
			  sm->bigtk_wnm_sleep.bigtk_len);
	} else {
		sm->bigtk.bigtk_len = len;
		os_memcpy(sm->bigtk.bigtk, mlo_bigtk->mlo_bigtk, sm->bigtk.bigtk_len);
	}
...
#ifdef CONFIG_DRIVER_BRCM_MAP
	if ((sm->map_4_way_handshake) && (gtk_len < 1 || gtk_len - 1 > sizeof(gd.gtk)))
		return -1;
	else
#endif	/* CONFIG_DRIVER_BRCM_MAP */
...
#ifdef CONFIG_DRIVER_BRCM_MLO
	if (ie->mlo_igtk_count) {
		int k, link_id;
		const struct wpa_mlo_igtk_kde *mlo_igtk;
		for (k = 0; k < ie->mlo_igtk_count; k++) {
			len = wpa_cipher_key_len(sm->mgmt_group_cipher);
			if (ie->mlo_igtk_len[k] != WPA_MLO_IGTK_KDE_PREFIX_LEN + len)
				return -1;
			mlo_igtk = (const struct wpa_mlo_igtk_kde *) ie->mlo_igtk[k];
			link_id = (mlo_igtk->link >> 4) & 0x0f;
			if (wpa_supplicant_install_mlo_igtk(sm, mlo_igtk, 0, link_id) < 0)
				return -1;
		}
	}

	if (ie->mlo_bigtk_count && sm->beacon_prot) {
		const struct wpa_mlo_bigtk_kde *mlo_bigtk;
		int k;
		for (k = 0; k < ie->mlo_bigtk_count; k++) {
			len = wpa_cipher_key_len(sm->mgmt_group_cipher);
			if (ie->mlo_bigtk_len[k] != WPA_MLO_BIGTK_KDE_PREFIX_LEN + len)
				return -1;
			mlo_bigtk = (const struct wpa_mlo_bigtk_kde *) ie->mlo_bigtk[k];
			if (wpa_supplicant_install_mlo_bigtk(sm, mlo_bigtk, 0) < 0)
				return -1;
		}
	}
#endif /* CONFIG_DRIVER_BRCM_MLO */
				      ...
				      struct wpa_eapol_ie_parse *ie)
{
	...
	if (strstr(sm->ifname, "wds") != NULL) {
		if ((ie->wpa_ie && sm->ap_wpa_ie &&
			...
			wpa_compare_wds_rsn_ie(sm->ap_rsn_ie, sm->ap_rsn_ie_len,
					ie->rsn_ie, ie->rsn_ie_len))) {
			wpa_report_ie_mismatch(sm, "WDS IE in 3/4 msg does not match "
			       ...
			       struct wpa_ptk *ptk)
{
	...
	u8 *rbuf, *key_mic;
#ifdef CONFIG_DRIVER_BRCM_MLO
	size_t maclen = (2 + RSN_SELECTOR_LEN + MAC_ADDR_LEN);
	u8 *macbuf = NULL;
#endif
...
#ifdef CONFIG_DRIVER_BRCM_MLO
	if (sm->has_ML && sm->bss_ml_capable) {
		WPA_PUT_BE16(key_mic + mic_len, maclen);
		os_memcpy(key_mic + mic_len + 2, macbuf, maclen); /* Key Data */
		wpa_printf(MSG_DEBUG,
			"set mld_mac: " MACSTR "in 4/4 handshake\n",
			MAC2STR(macbuf));
		os_free(macbuf);
	} else
#endif
...
#ifdef CONFIG_DRIVER_BRCM_MLO
/**
 * wpa_supplicant_mlo_link_kde_parse_update_rsn_ie - Update M3's rsn ie with active link's rsn ie
 * @sm: Pointer to WPA state machine data from wpa_sm_init()
 * @bssid: active link ssid
 * Returns: 0 on success, -1 on failure
 */
static int wpa_supplicant_mlo_link_kde_parse_update_rsn_ie(struct wpa_sm *sm,
					struct wpa_eapol_ie_parse *ie)
{
	struct wpa_mlo_link_kde *l_link;
	u8 i;

	if ((sm == NULL) || (ie == NULL))
		return -1;

	/* return if no MLO Links present */
	if (ie->mlo_link_count == 0) {
		wpa_msg(sm->ctx->msg_ctx, MSG_INFO,
			"M3 doesn't have MLO Links KDE");
		return 0;
	}

	/* Fetch the BSSID from each MLO Links KDE from M3 message
	 * Compare the BSSID with active link's BSSID,
	 * When matched update the ie's rsn ie with active link's rsn ie
	 * from M3's MLO Links KDE.
	 * Note : M3 will have all links RSN IEs part of MLO.
	 */
	for (i = 0; i < ie->mlo_link_count && i < MAX_NUM_LINKS; i++) {
		l_link = (struct wpa_mlo_link_kde *)ie->mlo_link_kde[i];
		if (os_memcmp(l_link->MAC, sm->active_link_bssid, ETH_ALEN) == 0) {
			u8 *l_pos = l_link->data;
			wpa_msg(sm->ctx->msg_ctx, MSG_INFO,
				"WPA: Updating M3's RSN and RSNX IE with active link's rsn ie");
                        /* getting  sub elements from link_ie */
                        if (l_link->link & RSN_MLO_LINK_KDE_LI_RSNE_INFO) {
                                /* check for rsn present */
                                if (*l_pos == WLAN_EID_RSN) {
                                        ie->rsn_ie = l_pos;
                                        ie->rsn_ie_len = l_pos[1] + 2;
                                        wpa_hexdump(MSG_DEBUG, "WPA: RSN IE in EAPOL-Key",
                                                        ie->rsn_ie, ie->rsn_ie_len);
                                        l_pos += ie->rsn_ie_len;
                                }
                        }
                        if (l_link->link & RSN_MLO_LINK_KDE_LI_RSNXE_INFO) {
                                /* check for rsnx present */
                                if (*l_pos == WLAN_EID_RSNX) {
                                        ie->rsnxe = l_pos;
                                        ie->rsnxe_len = l_pos[1] + 2;
                                        wpa_hexdump(MSG_DEBUG, "WPA: RSNX IE in EAPOL-Key",
                                                        ie->rsnxe, ie->rsnxe_len);
                                        l_pos += ie->rsnxe_len;
					  ...
					  size_t key_data_len)
{
...
#ifdef CONFIG_DRIVER_BRCM_MLO
	if (wpa_supplicant_mlo_link_kde_parse_update_rsn_ie(sm, &ie) < 0) {
		wpa_msg(sm->ctx->msg_ctx, MSG_INFO,
			"WPA: Updating M3's rsn ie with active link's rsn ie is failed");
		goto failed;
	}
#endif /* CONFIG_DRIVER_BRCM_MLO */
...
#ifdef CONFIG_DRIVER_BRCM_MLO
	if (ie.mlo_gtk_count && !(key_info & WPA_KEY_INFO_ENCR_KEY_DATA)) {
		wpa_msg(sm->ctx->msg_ctx, MSG_WARNING,
			"WPA: MLO_GTK IE in unencrypted key data");
		goto failed;
	}
	if (ie.mlo_igtk_count && !(key_info & WPA_KEY_INFO_ENCR_KEY_DATA)) {
		wpa_msg(sm->ctx->msg_ctx, MSG_WARNING,
			"WPA: MLO_IGTK KDE in unencrypted key data");
		goto failed;
	}
	if (ie.mlo_igtk_count &&
	    sm->mgmt_group_cipher != WPA_CIPHER_GTK_NOT_USED &&
	    wpa_cipher_valid_mgmt_group(sm->mgmt_group_cipher) &&
	    ie.mlo_igtk_len[0] != WPA_MLO_IGTK_KDE_PREFIX_LEN +
	    (unsigned int) wpa_cipher_key_len(sm->mgmt_group_cipher)) {
		wpa_msg(sm->ctx->msg_ctx, MSG_WARNING,
			"WPA: Invalid MLO_IGTK KDE length %d",
			(unsigned long) ie.mlo_igtk_len[0]);
		goto failed;
	}
#endif /* CONFIG_DRIVER_BRCM_MLO */

#ifdef CONFIG_DRIVER_BRCM_MAP
	/* Do not validate RSNIE while doing 4-way handshake for MAP security */
	if (!sm->map_4_way_handshake)
#endif /* CONFIG_DRIVER_BRCM_MAP */
	...
	if (wpa_supplicant_send_4_of_4(sm,
#ifdef CONFIG_DRIVER_BRCM_MLO
				       sm->mld_present? sm->ap_mld_addr : sm->bssid,
#else
				       sm->bssid,
#endif /* CONFIG_DRIVER_BRCM_MLO */
				       key, ver, key_info, &sm->ptk) < 0) {
		goto failed;
	}
#if defined(CONFIG_DRIVER_BRCM) && defined(CONFIG_DRIVER_BRCM_CEVENT)
	CE_SEND_CEVENT_A2C_EXT(sm->ifname, (struct ether_addr *)sm->bssid, 0, 0,
		CEVENT_ST_WPASUPP, CEVENT_A2C_MT_M4_TX, CEVENT_FRAME_DIR_TX, NULL, 0);
#endif /* CONFIG_DRIVER_BRCM && CONFIG_DRIVER_BRCM_CEVENT */
	...
	if (sm->group_cipher == WPA_CIPHER_GTK_NOT_USED) {
...
#ifdef CONFIG_DRIVER_BRCM_MLO
	} else if (!ie.gtk && !ie.mlo_gtk_count && sm->proto == WPA_PROTO_RSN) {
#else
		...
		goto failed;
#ifdef CONFIG_DRIVER_BRCM_MLO
	} else if (ie.mlo_gtk_count) {
		int k;
		for (k = 0; k < ie.mlo_gtk_count; k++) {
			if (wpa_supplicant_pairwise_mlo_gtk(sm, key,
						ie.mlo_gtk[k], ie.mlo_gtk_len[k], key_info) < 0) {
				wpa_msg(sm->ctx->msg_ctx, MSG_INFO,
						"RSN: Failed to configure MLO_GTK");
				goto failed;
			}
		}
#endif /* CONFIG_DRIVER_BRCM_MLO */
...
#ifdef CONFIG_DRIVER_BRCM_MLO
	if (sm->group_cipher == WPA_CIPHER_GTK_NOT_USED || ie.gtk || ie.mlo_gtk_count)
#else
					  ...
					  size_t key_data_len, u16 ver)
{
	...
	if (sm->proto == WPA_PROTO_RSN || sm->proto == WPA_PROTO_OSEN) {
#ifdef CONFIG_DRIVER_BRCM_MLO
		ret = wpa_supplicant_process_1_of_2_mlo(sm, key, key_data, key_data_len, ver);
		if (ret == 0) {
			wpa_sm_set_state(sm, WPA_GROUP_HANDSHAKE);
			goto done;
		}
		...
		size_t keydatalen, u16 ver)
{
	...
	if (wpa_supplicant_parse_ies(keydata, keydatalen, &ie) < 0)
		return -1;
	if((ie.mlo_gtk_count == 0) || (ie.mlo_igtk_count == 0))
		return -1;

	wpa_hexdump_key(MSG_DEBUG, "MLO-RSN: msg 1/2 key data",
			keydata, keydatalen);
...
#if defined(CONFIG_DRIVER_BRCM) && defined(CONFIG_DRIVER_BRCM_CEVENT)
			CE_SEND_CEVENT_A2C_EXT(sm->ifname, (struct ether_addr *)sm->bssid, 0, 0,
				CEVENT_ST_WPASUPP, CEVENT_A2C_MT_M3_RX, CEVENT_FRAME_DIR_RX, NULL, 0);
#endif /* CONFIG_DRIVER_BRCM && CONFIG_DRIVER_BRCM_CEVENT */
			/* 3/4 4-Way Handshake */
...
#if defined(CONFIG_DRIVER_BRCM) && defined(CONFIG_DRIVER_BRCM_CEVENT)
			CE_SEND_CEVENT_A2C_EXT(sm->ifname, (struct ether_addr *)sm->bssid, 0, 0,
				CEVENT_ST_WPASUPP, CEVENT_A2C_MT_M1_RX, CEVENT_FRAME_DIR_RX, NULL, 0);
#endif /* CONFIG_DRIVER_BRCM && CONFIG_DRIVER_BRCM_CEVENT */
			/* 1/4 4-Way Handshake */
...
#ifdef CONFIG_DRIVER_BRCM_MLO
		sm->bss_ml_capable = config->bss_ml_capable;
#endif
	} else {
		sm->network_ctx = NULL;
...
#ifdef CONFIG_DRIVER_BRCM_MLO
		sm->bss_ml_capable = 0;
		os_memset(sm->active_link_bssid, 0, ETH_ALEN);
		sm->mld_present = false;
		os_memset(sm->ap_mld_addr, 0, ETH_ALEN);
#endif
	}
}

#ifdef CONFIG_DRIVER_BRCM_MLO
void wpa_sm_set_mlo_link(struct wpa_sm *sm, int link_id) {
	if (sm == NULL)
		return;
	sm->link_id = link_id;
}
void wpa_sm_set_mlo_mld(struct wpa_sm *sm, int mld_unit) {
	if (sm == NULL)
		return;
	sm->mld_unit = mld_unit;
}
...
void wpa_sm_save_multi_link(struct wpa_sm *sm, u8* ie, uint len) {
	if (sm == NULL)
		return;
	/* TODO enhance with fragmentation and defragmentation */
	if (sm->has_ML) {
		if (sm->mlo_ie) {
			os_free(sm->mlo_ie);
			sm->mlo_ie_len = 0;
		}
		sm->mlo_ie = os_malloc(len);
		if (!sm->mlo_ie) {
			return;
		}
		os_memcpy(sm->mlo_ie, ie, len);
		sm->mlo_ie_len = len;
		wpa_printf(MSG_DEBUG,
			"SAVED: multi link element for future use\n");
	}
}

/**
 * wpa_sm_save_ap_mld - Save AP MLD into sm.
 * @mld_addr: AP mld address
 * Returns: 0 on success, -1 on failure
 */
int wpa_sm_save_ap_mld(struct wpa_sm *sm, u8 *mld_addr)
{
	if ((sm == NULL) || (mld_addr == NULL))
		return -1;

	/*  Save active link's bssid into sm */
	os_memcpy(sm->ap_mld_addr, mld_addr, ETH_ALEN);
	sm->mld_present = true;
...
{
        return sm->mld_present ? sm->ap_mld_addr : sm->bssid;
}

/**
 * wpa_sm_set_active_link_bssid - Save active link's bssid into sm.
 * @bssid: active link ssid
 * Returns: 0 on success, -1 on failure
 */
int wpa_sm_save_active_link_bssid(struct wpa_sm *sm, u8 *bssid)
{
	if ((sm == NULL) || (bssid == NULL))
		return -1;

	/*  Save active link's bssid into sm */
	os_memcpy(sm->active_link_bssid, bssid, ETH_ALEN);
...
#endif /* CONFIG_DPP2 */
#ifdef CONFIG_DRIVER_BRCM_MAP
	case WPA_PARAM_MAP_4_WAY_HANDSHAKE:
		sm->map_4_way_handshake = value;
		break;
#endif /* CONFIG_DRIVER_BRCM_MAP */
	default:
		break;
...
{
	return pmksa_cache_get(sm->pmksa, bssid, own_addr, NULL, network_ctx, 0) != NULL;
}
...
void wpa_pasn_sm_set_caps(struct wpa_sm *sm, unsigned int flags2)
{
	...
	if (flags2 & WPA_DRIVER_FLAGS2_PROT_RANGE_NEG_STA)
		sm->prot_range_neg = 1;
#ifdef CONFIG_DRIVER_BRCM
	if (flags2 & WPA_DRIVER_FLAGS2_PROT_RANGE_ALLBW_NEG_STA)
		sm->urnm_mfpr = 2;
#endif /* CONFIG_DRIVER_BRCM */
}
...
#ifdef CONFIG_DRIVER_BRCM_MAP
void wpa_sm_get_driver_none_bssid(struct wpa_sm *sm, u8 *bssid)
{
	os_memcpy(bssid, sm->bssid, ETH_ALEN);
}

void wpa_sm_map_set_pmksa(struct wpa_sm *sm, u8 *addr)
{
	sm->cur_pmksa = pmksa_cache_get(sm->pmksa, addr, NULL, NULL, NULL, 0);
	wpa_sm_set_pmk_from_pmksa(sm);
}
#endif /* CONFIG_DRIVER_BRCM_MAP */
