...
static void wpas_dpp_get_mld_bsta_config(void *ctx, struct dpp_bsta_mld_config *out_config);
static int wpas_dpp_get_mld_bsta_config_evt_data(struct wpabuf *msg, struct wpa_supplicant *wpa_s,
			       ...
			       enum offchannel_send_action_result result)
{
	...
	if (auth->waiting_auth_conf &&
		...
		eloop_cancel_timeout(wpas_dpp_auth_conf_wait_timeout, wpa_s,
				     NULL);
#ifdef CONFIG_DRIVER_BRCM_MAP
		eloop_register_timeout(10, 0, wpas_dpp_auth_conf_wait_timeout,
				       wpa_s, NULL);
#else
...
#ifdef CONFIG_DRIVER_BRCM_MAP
#ifdef CONFIG_DRIVER_BRCM_MLO
	if (conf->bsta_mld_config.link_count) {
		struct wpabuf *attr = wpabuf_alloc(512);
		if (attr) {
			if (wpas_dpp_get_mld_bsta_config_evt_data(attr, wpa_s, conf) == 0) {
				size_t blen = wpabuf_len(attr) * 2 + 1;
				char *buf = os_malloc(blen);
				if (buf) {
					wpa_snprintf_hex(buf, blen,
						wpabuf_head_u8(attr), wpabuf_len(attr));
					wpa_msg(wpa_s, MSG_INFO,
						DPP_EVENT_CONFOBJ_MLO_BSTA_CONFIG "%s", buf);
					os_free(buf);
				}
			}
			wpabuf_free(attr);
...
#endif /* CONFIG_DRIVER_BRCM_MLO */
	wpa_msg(wpa_s, MSG_INFO, DPP_EVENT_CONFOBJ_NETROLE "%s",
...
#ifdef CONFIG_DRIVER_BRCM_MAP
	if (wpa_s->dpp_netrole == DPP_NETROLE_MAPAGENT) {
		buf = dpp_build_conf_req_helper_map(auth, wpa_s->conf->dpp_name,
						wpa_s->dpp_netrole);
	} else
#endif /* CONFIG_DRIVER_BRCM_MAP */
	{
...
int wpas_dpp_init(struct wpa_supplicant *wpa_s)
{
	...
	config.get_bsta_mld_config = wpas_dpp_get_mld_bsta_config;
...
#ifdef CONFIG_DRIVER_BRCM_MAP
	if (wpa_s->conf->map_4_way_handshake)
		config.netrole = DPP_NETROLE_MAPAGENT;
	else
#endif /* CONFIG_DRIVER_BRCM_MAP */
...
#ifdef CONFIG_DRIVER_BRCM_MAP
	wpa_printf(MSG_DEBUG, "DPP: No chirp response received netrole [%d] shadow_chirp_done [%d]",
		wpa_s->dpp_netrole, wpa_s->dpp_shadow_chirp_done);
#else
...
#ifdef CONFIG_DRIVER_BRCM_MAP
	/* Check dpp_shadow_chirp_done flag to send one more chirp on the same frequency
	 * to receive quicker auth request from proxy agent */
	if ((wpa_s->dpp_netrole == DPP_NETROLE_MAPAGENT) &&
		!wpa_s->dpp_shadow_chirp_done) { /* shadow chirp is not yet sent */
		wpa_s->dpp_shadow_chirp_done = true;
		wpas_dpp_chirp_start(wpa_s); /* send the chirp again */
	} else {
		wpa_s->dpp_shadow_chirp_done = false;
...
#ifdef CONFIG_DRIVER_BRCM_MAP
			/* Starting the dpp chirp scan while any other scan is running will
			 * override scan_res_handler callback fanction.
			 * Wait for other scans to get completed before starting chirp scan.
			 */
				|| (wpa_s->scanning)
#endif /* CONFIG_DRIVER_BRCM_MAP */
					) {
...
int wpas_dpp_chirp(struct wpa_supplicant *wpa_s, const char *cmd)
{
	...
	if (wpa_s->conf->ssid && wpa_s->conf->ssid->multi_ap_backhaul_sta) {
		wpa_s->dpp_netrole = DPP_NETROLE_MAPAGENT;
		wpa_s->dpp_shadow_chirp_done = false;
	}
#endif /* CONFIG_DRIVER_BRCM_MAP */
...
#if defined(CONFIG_DRIVER_BRCM_MAP) && defined(CONFIG_DRIVER_BRCM_MLO)
/**
 * wpas_dpp_get_wpa - Returns wpa_supplicant data corresponding to the matching mac address.
 * @wpa_s: Pointer to wpa_supplicant data
 * @mac_addr: Mac address for comparison
 * Returns: Wpa_supplicant data for matching mac or NULL
 */
static struct wpa_supplicant* wpas_dpp_get_wpa(struct wpa_supplicant *wpa_s,
	unsigned char *mac_addr)
{
	struct wpa_supplicant *tmp_wpa_s = wpa_s->global->ifaces;

	while (tmp_wpa_s) {
		if (!os_memcmp(tmp_wpa_s->own_addr, mac_addr, ETH_ALEN)) {
			return tmp_wpa_s;
		}
...
/**
 * wpas_dpp_get_mld_bsta_config_evt_data - Prepares event data based on the backhaul sta mld
 * config received in the dpp configuration response message.
 * @msg: Buffer to store event data.
 * @wpa_s: Pointer to wpa_supplicant data
 * @conf: Received DPP configuration object
 * Returns: 0 on success or -1 on failure
 */
static int wpas_dpp_get_mld_bsta_config_evt_data(struct wpabuf *msg, struct wpa_supplicant *wpa_s,
	struct dpp_config_obj *conf)
{
	struct dpp_bsta_mld_config self_config, *conf_bsta_mld_cfg, *self_bsta_mld_cfg;
	struct dpp_bsta_obj *self_bsta_obj, *conf_bsta_obj;
	struct wpa_supplicant *tmp_wpa_s;
	enum dpp_brcm_vs_attr_id {DPP_ATTR_MLD_BSTA_CONFIG = 0x2000, DPP_ATTR_INDEX = 0x2001,
		DPP_ATTR_IFNAME = 0x2002, DPP_ATTR_WIFI7_CAP = 0x2003};
	u8 wifi7_caps = 0, count = 0;
	int idx, jdx, found, ifname_len;
	struct wpabuf *wbuf;

	wpas_dpp_get_mld_bsta_config(wpa_s, &self_config);
	self_bsta_mld_cfg = &self_config;
	if (!self_bsta_mld_cfg->link_count) {
		wpa_printf(MSG_INFO, "DPP: device does not have any bsta mld config present");
		return -1;
	}

	conf_bsta_mld_cfg = &conf->bsta_mld_config;
	if (conf_bsta_mld_cfg->link_count != self_bsta_mld_cfg->link_count) {
		wpa_printf(MSG_INFO, "DPP: mismatch for link count received[%u]/supported[%u]",
			conf_bsta_mld_cfg->link_count, self_bsta_mld_cfg->link_count);
	}

	wbuf = wpabuf_alloc(256);
	if (wbuf == NULL) {
		return -1;
	}

	for (idx = 0; idx < conf_bsta_mld_cfg->link_count; idx++) {
		found = 0;
		conf_bsta_obj = &conf_bsta_mld_cfg->bsta_objs[idx];
		for (jdx = 0; jdx < self_bsta_mld_cfg->link_count; jdx++) {
			self_bsta_obj = &self_bsta_mld_cfg->bsta_objs[jdx];
			if (!os_memcmp(conf_bsta_obj->bsta_addr,
				self_bsta_obj->bsta_addr, ETH_ALEN)) {
				found = 1;
				break;
			}
		}
		...
		if (!found) {
			wpa_printf(MSG_INFO, "DPP: RUID " MACSTR " is not present in device",
				MAC2STR(conf_bsta_obj->bsta_addr));
			continue;
		}

		wifi7_caps = conf_bsta_obj->wifi7_caps & self_bsta_obj->wifi7_caps;
		if (!DPP_IS_CAPAB_WIFI7_STR(wifi7_caps)) {
			wpa_printf(MSG_INFO, "DPP: mismatch in wifi7 capabilities for " MACSTR " "
				"received[0x%x]/supported[0x%x]", MAC2STR(conf_bsta_obj->bsta_addr),
				conf_bsta_obj->wifi7_caps, self_bsta_obj->wifi7_caps);
		}

		tmp_wpa_s = wpas_dpp_get_wpa(wpa_s, conf_bsta_obj->bsta_addr);
		if (!tmp_wpa_s) {
			wpa_printf(MSG_INFO, "DPP: for RUID " MACSTR " %s",
				MAC2STR(conf_bsta_obj->bsta_addr), "couldn't find supplicant obj");
			continue;
		}

		wpabuf_put_be16(msg, DPP_ATTR_INDEX);
		wpabuf_put_be16(msg, 1);
		wpabuf_put_u8(msg, count);

		ifname_len = os_strlen(tmp_wpa_s->ifname);
		wpabuf_put_be16(msg, DPP_ATTR_IFNAME);
		wpabuf_put_be16(msg, ifname_len);
		wpabuf_put_data(msg, tmp_wpa_s->ifname, ifname_len);

		wpabuf_put_be16(msg, DPP_ATTR_WIFI7_CAP);
		wpabuf_put_be16(msg, 1);
		wpabuf_put_u8(msg, wifi7_caps);

		count++;
	}

	wpabuf_put_be16(msg, DPP_ATTR_MLD_BSTA_CONFIG);
...
/**
 * wpas_dpp_get_mld_bsta_config - Fetch mld backhaul sta configurations of multiap agent device.
 * Any radio having a ssid with multiap backhaul sta set to 1 and a valid link_id is considered
 * to be part of backhaul sta mld.
 * @ctx: Pointer to wpa_supplicant data
 * @out_config: Device's mld backhaul sta configuration
 */
static void wpas_dpp_get_mld_bsta_config(void *ctx, struct dpp_bsta_mld_config *out_config)
{
	struct wpa_supplicant *wpa_s = (struct wpa_supplicant*)ctx;
	struct wpa_supplicant *tmp_wpa_s = NULL;
	struct dpp_bsta_obj *bsta_obj;

	WPA_ASSERT(out_config != NULL);

	os_memset(out_config, 0, sizeof(*out_config));

	tmp_wpa_s = wpa_s->global->ifaces;
	while (tmp_wpa_s) {
		if (tmp_wpa_s->conf->link_id != -1 && tmp_wpa_s->conf->ssid &&
			 tmp_wpa_s->conf->ssid->multi_ap_backhaul_sta) {
			if (out_config->link_count >= DPP_MAX_BSTA_COUNT) {
				break;
			}
			bsta_obj = &out_config->bsta_objs[out_config->link_count];
			os_memcpy(bsta_obj->bsta_addr, tmp_wpa_s->own_addr, ETH_ALEN);
			bsta_obj->wifi7_caps |= DPP_CAPAB_WIFI7_STR;
			out_config->link_count++;
		}
		tmp_wpa_s = tmp_wpa_s->next;
	}
}
