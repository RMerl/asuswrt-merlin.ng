					     ...
					     struct wpa_ssid *ssid)
{
	...
	if (use_pk)
        ...
        sme_external_auth_build_buf(buf, resp, wpa_s->own_addr,
				    ...
				    bssid,
#endif /* CONFIG_DRIVER_BRCM_MLO */
                                    1,wpa_s->sme.seq_num, status);

	wpa_drv_send_mlme(wpa_s, wpabuf_head(buf), wpabuf_len(buf), 1, 0, 0);
#if defined(CONFIG_DRIVER_BRCM) && defined(CONFIG_DRIVER_BRCM_CEVENT)
	CE_SEND_CEVENT_A2C_EXT(wpa_s->ifname, wpa_s->current_bss->bssid, 0, 0,
		CEVENT_ST_WPASUPP, CEVENT_A2C_MT_AUTH_COMMIT_TX, CEVENT_FRAME_DIR_TX, NULL, 0);
#endif /* CONFIG_DRIVER_BRCM && CONFIG_DRIVER_BRCM_CEVENT */
	wpabuf_free(resp);
	wpabuf_free(buf);
					  ...
					  union wpa_event_data *data)
{
	...
	for (ssid = wpa_s->conf->ssid; ssid; ssid = ssid->next) {
		if (!wpas_network_disabled(wpa_s, ssid) &&
			...
			if((!ssid->bssid_set) || ((ssid->bssid_set) &&
			       ...
			       union wpa_event_data *data)
{
	...
	if (data->external_auth.action == EXT_AUTH_START) {
		...
		if (data->external_auth.mld_addr) {
			...
			wpa_sm_save_ap_mld(wpa_s->wpa, wpa_s->sme.ext_auth_ap_mld_addr);
		} else {
...
#if defined(CONFIG_DRIVER_BRCM) && defined(CONFIG_DRIVER_BRCM_CEVENT)
	CE_SEND_CEVENT_A2C_EXT(wpa_s->ifname, wpa_s->current_bss->bssid, 0, 0,
		CEVENT_ST_WPASUPP, CEVENT_A2C_MT_AUTH_COMMIT_RX, CEVENT_FRAME_DIR_RX, NULL, 0);
#endif /* CONFIG_DRIVER_BRCM && CONFIG_DRIVER_BRCM_CEVENT */
...
#if defined(CONFIG_DRIVER_BRCM) && defined(CONFIG_DRIVER_BRCM_CEVENT)
	CE_SEND_CEVENT_A2C_EXT(wpa_s->ifname, wpa_s->current_bss->bssid, 0, 0,
		CEVENT_ST_WPASUPP, CEVENT_A2C_MT_AUTH_CONFIRM_TX, CEVENT_FRAME_DIR_TX, NULL, 0);
#endif /* CONFIG_DRIVER_BRCM && CONFIG_DRIVER_BRCM_CEVENT */
		return 0;
...
#if defined(CONFIG_DRIVER_BRCM) && defined(CONFIG_DRIVER_BRCM_CEVENT)
	CE_SEND_CEVENT_A2C_EXT(wpa_s->ifname, wpa_s->current_bss->bssid, 0, 0,
		CEVENT_ST_WPASUPP, CEVENT_A2C_MT_AUTH_CONFIRM_RX, CEVENT_FRAME_DIR_RX, NULL, 0);
#endif /* CONFIG_DRIVER_BRCM && CONFIG_DRIVER_BRCM_CEVENT */
