...
INT wifi_setRadioOperatingParameters(wifi_radio_index_t index, wifi_radio_operationParam_t *operationParam)
{
	...
	char operStd[OUTPUT_STRING_LENGTH_32] = {0}, bwStr[32] = {0};
	unsigned int pureMode = PMODE_NONE, channelWidth, tempVariant = 0, bw;
	bool enable, radio_enabled;
	csa_t csaInfo;
	unsigned int radioIndex;
	wldm_xbrcm_radio_param_t param;
	...
	if (!operationParam->autoChannelEnabled) {
		...
		/* Get current channel and bandwidth */
		len = sizeof(param);
		ret = wldm_xbrcm_radio(CMD_GET, radioIndex, &param, (unsigned int *)&len,
			"chanspec", NULL);
		if (ret < 0) {
			HAL_WIFI_ERR(("%s: wldm_xbrcm_Radio_Chanspec CMD_GET radioIndex=%d failed\n",
				__FUNCTION__, radioIndex));
		...
		HAL_WIFI_DBG(("%s: radioIndex = %d wldm_xbrcm_Radio_Chanspec channel=%d bw=%s\n",
			__FUNCTION__, radioIndex, param.channel, param.bandwidth));

		/* No csa sent if csa channel/banwidth same as current chanspec */
		sscanf(param.bandwidth, "%d%32s", &bw, bwStr);
		if (param.channel == channel && bw == channelWidth) {
			HAL_WIFI_LOG(("%s: radioIndex = %d CSA not allowed to current chanspec "
				"%d/%d\n", __FUNCTION__, radioIndex, channel, channelWidth));
		} else if (operationParam->csa_beacon_count && operationParam->beaconInterval) {
			/* Send CSA */
		...
		HAL_WIFI_DBG(("%s: radioIndex = %d, channel=%d, channelWidth=%d, "
			"csa_beacon_count=%d beaconInterval=%d\n", __FUNCTION__, index,
			operationParam->channel, channelWidth, operationParam->csa_beacon_count,
