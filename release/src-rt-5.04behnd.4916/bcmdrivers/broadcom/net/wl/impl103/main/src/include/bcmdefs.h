 ...
 *
 * $Id: bcmdefs.h 836914 2024-02-23 12:01:04Z $
 */
...
/* Some magic so the macro STATIC_ASSERT() can be placed anywhere in source files */
#if !defined STATIC_ASSERT
#define STATIC_ASSERT(X) \
	_Pragma("GCC diagnostic push"); \
	_Pragma("GCC diagnostic ignored \"-Wdeclaration-after-statement\""); \
	_Static_assert((X), "Static assertion failure"); \
	_Pragma("GCC diagnostic pop");
...
#define BCMATTACHMLOFN(_fn)	ATTRIBUTE_NO_SANITIZE_ADDRESS \
	__attribute__ ((__section__ (".text_mlc_rebind." #_fn), noinline)) _fn
...
#define ULONGTOPHYSADDR(_ulong, _pa) \
	do { \
		unsigned long long _tmp = (unsigned long long)(_ulong);	\
		(_pa).loaddr = ((_tmp) &  DMADDR_LOWADDR_MASK);		\
		(_pa).hiaddr = (((_tmp) >> 32) & DMADDR_LOWADDR_MASK);	\
	} while (0)
#else
...
enum assert_type_e {
	ASSERT_T0_PANIC,
	ASSERT_T1_REINIT,	/**< Unused. No panic, reinits wl */
	ASSERT_T2_PROC_DIE,	/**< NIC: stack trace followed by death of current process */
	ASSERT_T3_WLC_OUT,	/**< FD:halt. NIC: macdbg dump followed by wlc_out */
	ASSERT_T4_REINIT,	/**< FD:halt. NIC: macdbg dump followed by re-initialize */
	ASSERT_T5_DMP_AND_HANG	/**< FD:halt. NIC: macdbg dump followed by hanging of kthread */
};
