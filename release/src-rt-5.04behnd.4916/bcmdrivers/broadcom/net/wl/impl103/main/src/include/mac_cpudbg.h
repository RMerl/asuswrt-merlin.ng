 ...
 *
 * $Id: mac_cpudbg.h 834750 2023-12-29 02:35:40Z $
 *
...
struct cpudbg_hme {
	...
	uint32    cpudbg_msglevel[CPUC_ID_MAX];    /* CPU[0..3] startup msglevel */
} __cpudbg_aligned;
...
typedef struct cpudbg_ctrs {
	uint32 cpu0_entry_ct;
	uint32 cpu1_entry_ct;
	uint32 cpu2_entry_ct;
	uint32 cpu3_entry_ct;
	uint32 cpu0_drop_ct;
	uint32 cpu1_drop_ct;
	uint32 cpu2_drop_ct;
	uint32 cpu3_drop_ct;
	uint32 hdr_no_update;
	uint32 skipped_seq;
	uint32 seq_gap;
	uint32 inval_entryct;
	uint32 inval_stringid;
	uint32 null_stringid;
	uint32 entry_ct_war;
	uint32 entry_ct_war_ts;
} cpudbg_ctrs_t;
...
cpudbg_dbglog_decode(char *outbuf, uint outbuflen, wl_cpudbg_entry_t *cpudbg_entry,
	const char *str_p, const char *tag, const char *pfx, uint32 hwevt_mask, bool valid)
{
	...
	if (!valid) {
		cpu_str =   " ----";
		trace_str = " ----";
		fmt = "0x%x 0x%x --> INVALID ENTRY";
	} else if (cpu_id == CPUC_ID_PHY) {
		cpu_str = "PHY";
		trace_str = "TRACE";
		fmt = "0x%x 0x%x";
	...
	if (cpysize <= DBGLOG_BUFSIZE) {
		...
		snprintf(outbuf, outbuflen - 1, dbglogfmt, ts, stringId.link_id,
			cpu_str, trace_str, stringId.u32, param1, param2, param3, param4);
	} else {
