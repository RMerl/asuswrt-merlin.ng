 ...
 *
 * $Id: mlo_ipc.h 836039 2024-02-02 07:43:18Z $
 * +--------------------------------------------------------------------------+
 */
...
typedef enum mlo_ipc_mlc_state
{
	...
	mlo_ipc_mlc_up_req_e           = 21,   // Notify MAP to start mlo up
	mlo_ipc_mlc_up_e               = 22,   // MLO IPC for mlo up

	mlo_ipc_mlc_active_e           = 23,   // MLO IPC for MLO_ACTIVE
...
typedef enum mlo_ipc_msg_id {
	...
	MLO_IPC_MSG_ID_UP_REQ		= 14,	// Request MAP for mlo up

	MLO_IPC_MSG_ID_ACTIVE		= 15,	// MLC_ACTIVE sync complete
...
#define MLO_IPC_LOCK()			spin_lock_bh(&(mlo_ipc_sys_gp->lock))
#define MLO_IPC_UNLK()			spin_unlock_bh(&(mlo_ipc_sys_gp->lock))
...
#define MLO_IPC_LOCK()			local_irq_disable()
#define MLO_IPC_UNLK()			local_irq_enable()
...
extern uint8 mlo_ipc_get_sys_mlo_down(int ap_unit);

extern struct device *mlo_ipc_get_dev(void);
extern bool mlo_ipc_bh_request(void);
extern void mlo_ipc_bh_complete(void);

extern void	mlo_ipc_dump(uint16 ap_unit, struct bcmstrbuf *b);
#endif /* _MLO_IPC_H_ */
