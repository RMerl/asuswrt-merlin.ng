 ...
 *
 * $Id: bcmutils.c 837206 2024-03-02 00:44:51Z $
 */
...
#include <linux/of.h>
#include <linux_pkt.h>
#endif /* BCM_ROUTER && !BCMDONGLEHOST && !DONGLEBUILD */
...
char acname[AC_COUNT][3] = {"BE", "BK", "VI", "VO"};
#endif

#ifdef BCMDRIVER

#define DCSP_MAX_VALUE		64
/* Default Priority : tos_tc >> IPV4_TOS_PREC_SHIFT(5) */
/* RFC8325 : 4.3 DSCP-to-UP Mapping Table */
/* 0:BE, 1:BK, 2: None, 3:EE(BE), 4:CL(VI), 5:VI, 6:VO, 7:NC */
static const uint8 def_dscp2priomap[DCSP_MAX_VALUE]=
{
	0, 0, 0, 0, 0, 0, 0, 0, /*  DF, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07 */
	1, 1, 3, 1, 3, 1, 3, 1, /* CS1, 0x09, AF11, 0x0B, AF12, 0x0D, AF13, 0x0F */
	0, 2, 3, 2, 3, 2, 3, 2, /* CS2, 0x11, AF21, 0x13, AF22, 0x15, AF23, 0x17 */
	4, 3, 4, 3, 4, 3, 4, 3, /* CS3, 0x19, AF31, 0x1B, AF32, 0x1D, AF33, 0x1F */
	4, 4, 4, 4, 4, 4, 4, 4, /* CS4, 0x21, AF41, 0x23, AF42, 0x25, AF43, 0x27 */
	5, 5, 5, 5, 6, 5, 6, 5, /* CS5, 0x29, 0x2A, 0x2B,   VA, 0x2D,   EF, 0x2F */
	7, 6, 6, 6, 6, 6, 6, 6, /* CS6, 0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37 */
	7, 7, 7, 7, 7, 7, 7, 7  /* CS7, 0x39, 0x3A, 0x3B, 0x3C, 0x3D, 0x3E, 0x3F */
};
...
#if defined(CONFIG_HAVE_SKBUFF_WL_SPC_V2)
	if (PKTISSPC(osh, p)) {
		return PKTTOTLEN(osh, p);
	}
#endif
...
pktsetprio(void *pkt, bool update_vtag)
{
	...
	if (eh->ether_type == hton16(ETHER_TYPE_8021Q)) {
		...
		} else if (evh->ether_type == hton16(ETHER_TYPE_1905_1)) {
			vlan_prio = vlan_prio ? vlan_prio : PRIO_8021D_VI;
		}
		...
		uint8 tos_tc = IP_TOS46(ip_body);
		uint8 dscp = ((tos_tc & IPV4_TOS_DSCP_MASK) >> IPV4_TOS_DSCP_SHIFT);
		priority = (int)def_dscp2priomap[dscp];
		rc |= PKTPRIO_DSCP;
	} else if (eh->ether_type == hton16(ETHER_TYPE_1905_1)) {
		priority = PRIO_8021D_VI;
	}
...
dscp2up(uint8 *up_table, uint8 dscp)
{
	...
	/* 802.11-2020 section 9.4.2.94 -
	 * When a non-AP STA begins transmission of a Data frame containing the Internet Protocol,
	 * it matches the DSCP field in the IP header to the corresponding DSCP value contained
	 * in this element.
	 * The non-AP STA will first attempt to match the DSCP value to a DSCP exception field and
	 * uses the UP from the corresponding UP in the same DSCP exception field if successful;
	 * if no match is found then the non-AP STA attempts to match the DSCP field to a UP n
	 * DSCP Range field, and uses the n as the UP if successful;
	 * and otherwise uses a UP of 0.
	 */
...
struct bcm_mwbmap {             /* Hierarchical multiword bitmap allocator    */
...
void
bcm_mwbmap_fini(osl_t * osh, bcm_mwbmap_t * mwbmap_hdl)
{
	bcm_mwbmap_t * mwbmap_p;
	...
	MFREE(osh, mwbmap_p, sizeof(bcm_mwbmap_t)
...
uint32 BCMFASTPATH
bcm_mwbmap_alloc(bcm_mwbmap_t * mwbmap_hdl)
{
	bcm_mwbmap_t * mwbmap_p;
...
void
bcm_mwbmap_force(bcm_mwbmap_t * mwbmap_hdl, uint32 bitix)
{
	bcm_mwbmap_t * mwbmap_p;
...
void BCMFASTPATH
bcm_mwbmap_free(bcm_mwbmap_t * mwbmap_hdl, uint32 bitix)
{
	bcm_mwbmap_t * mwbmap_p;
...
uint32
bcm_mwbmap_free_cnt(bcm_mwbmap_t * mwbmap_hdl)
{
	bcm_mwbmap_t * mwbmap_p;
...
bool
bcm_mwbmap_isfree(bcm_mwbmap_t * mwbmap_hdl, uint32 bitix)
{
	bcm_mwbmap_t * mwbmap_p;
	uint32 wordix, bitmap;
...
void
bcm_mwbmap_show(bcm_mwbmap_t * mwbmap_hdl)
{
	uint32 ix, count;
	bcm_mwbmap_t * mwbmap_p;
...
void
bcm_mwbmap_audit(bcm_mwbmap_t * mwbmap_hdl)
{
	bcm_mwbmap_t * mwbmap_p;
...
int
bcm_bputc(struct bcmstrbuf *b, int c)
{
	if (b->size) {
		*b->buf++ = c;
		b->size--;
 ...
 *
 * At function return, b->buf will not point at the NULL character, but instead one character
 * before that. Likewise, b->buflen does not take the NULL character into account.
 */
int
bcm_bprintf(struct bcmstrbuf *b, const char *fmt, ...)
{
	...
	/* vsnprintf returns the number of characters written, *excluding* the NULL character */
...
/**
 * @param[in] pad   Number of spaces to print before printing the val
 */
int
 ...
 *
 * Searches specified argument list for the specified option and extracts the string argument
 * if specified. Options are in the format [option]:[value].
 *
...
bool
bcm_has_option_str(const char *option, int argc, char *argv[], const char **value)
{
 ...
 * Helper function for parsing console commands.
 *
 * Searches specified argument list for the specified option and extracts the numeric argument
 * if specified. Options are in the format [option]:[value].
 *
 * @param option	Pointer to option to search for.
 * @param argc		Number or elements in arguments array.
...
bcm_has_option(const char *option, int argc, char *argv[], uint *value)
{
	...
	if (bcm_has_option_str(option, argc, argv, &argument)) {
		if (value && argument && *argument) {
			*value = bcm_atoi(argument);
		}
		return TRUE;
	}
 ...
 * Helper function for parsing console commands.
 *
 * Searches specified string for the specified option and extracts the numeric option
 * value if specified. Numeric options are in the format [option]:[value].
 *
 * @param option	Pointer to option to search for.
 * @param args		Pointer to null-terminated argument string.
 * @param value		Pointer to location for storing found value, can be NULL.
 * @return		TRUE if option was found.
 */

bool
bcm_has_option_ex(const char *option, char *args, uint *value)
{
	char *next;
	uint len = strlen(option);
	...
	if (args != NULL) {
		while (*args) {
			next = args;
			while (*next && !bcm_isspace(*next)) {
				++next;
			}

			if (!strncmp(args, option, len)) {
				if (value) {
					args += len;
					*value = (*args == ':' ? bcm_atoi(++args) : 0);
				}
				return TRUE;
			}

			args = next;
			while (*args && bcm_isspace(*args)) {
				++args;
			}
		}
	}
...
/**
 * Convert from zero-separated string to string pointer array.
 *
 * Input string is assumed to be in the format [elem0]\0[elem1]\0 ... [elemN]\0
 * Parsing of the input string stops when length is reached or a zero-sized element is found.
 *
 * Elements in the target string array will be pointed to the corresponding elements in the
 * input string. A final entry pointing to NULL will be added. When the return value of this
 * function indicates success, all strings pointed to by argv are guaranteed to be zero-terminated.
 *
 * @param buffer	Pointer to input buffer.
 * @param length	Input buffer length.
 * @param argv		Pointer to target string array. This array is assumed to have a
 *			minimum capacity of 'num' elements.
 * @param argc		Pointer for storing target string array element count.
 * @param num		Number of elements to convert, including the final NULL element.
 * @return		Return the number of bytes consumed, or -1 on error.
 */

int
bcm_tokenize_zsv(char *buffer, uint length, char **argv, uint *argc, uint num)
{
	char *offset = buffer, *end = buffer + length;
	...
	*argc = 0;
	while (offset < end && *offset && *argc < num - 1) {
		*argv++ = offset;
		(*argc)++;
		while (*offset++) {
			if (offset == end) {
...
/* Given an array of bitmaps (uint32 boundary), function will clear the bits from start_pos to
 * end_pos (both inclusive, indexed from 0).
 */
void
clr_bitrange_u32(void *array, uint start_pos, uint end_pos, uint maxbit_pos)
{
	uint32 *bitmap = (uint32 *)array;
	uint start_word_bit_pos;
	uint bit_cnt;

	ASSERT(((maxbit_pos + 1) % NUM_BITS_U32) == 0);	/* uint32 boundary */
	ASSERT(start_pos <= maxbit_pos);
	ASSERT(end_pos <= maxbit_pos);

	if (end_pos >= start_pos) {
		bitmap			+= (start_pos/NUM_BITS_U32);
		bit_cnt			= (end_pos - start_pos) + 1;
		start_word_bit_pos	= start_pos % NUM_BITS_U32;

		if (bit_cnt < (NUM_BITS_U32 - start_word_bit_pos)) {
			/* bit_cnt doesn't cross u32 boundary */
			*bitmap &= ~((uint32)(BITMASK(start_word_bit_pos, bit_cnt)));
			return;
		}

		/* Clear bits from start_pos position to end_pos of first word. */
		if (start_word_bit_pos) {
			*bitmap	&= (uint32)(MASK_32_BITS >> (NUM_BITS_U32 - start_word_bit_pos));
			bit_cnt -= (NUM_BITS_U32 - start_word_bit_pos);
			bitmap++;
		}

		/* clear bits by full words */
		while (bit_cnt/NUM_BITS_U32) {
			*bitmap = (uint32)0;
			bit_cnt -= NUM_BITS_U32;
			bitmap++;
		}

		/* Clear bits from start_pos of last word to end_pos position */
		if (bit_cnt) {
			*bitmap &= (uint32)(MASK_32_BITS << bit_cnt);
		}
	} else {
		clr_bitrange_u32(array, start_pos, maxbit_pos, maxbit_pos);
		clr_bitrange_u32(array, 0, end_pos, maxbit_pos);
	}
}
 ...
 */
int
bcm_tlv_dot11_defrag(void *buf, uint buf_len, uint16 id,
		uint8 *out, uint *out_len, uint *data_len)
{
	...
	if (id > DOT11_MNG_ID_EXT_ID) {
		ie = bcm_parse_tlvs_dot11(buf, buf_len, id - DOT11_MNG_ID_EXT_ID, TRUE);
	} else {
		ie = bcm_parse_tlvs_dot11(buf, buf_len, id, FALSE);
	}
	if (!ie) {
	...
	/* first fragment */
	tot_len = id > DOT11_MNG_ID_EXT_ID ? ie->len - 1u : ie->len;
	...
	if (out_left < tot_len) {
		...
		memcpy(out, &ie->data[id > DOT11_MNG_ID_EXT_ID ? 1u : 0], tot_len);
		out += tot_len;
		out_left -= tot_len;
	}
	...
	while ((ie = bcm_next_tlv(ie, (int *)&buf_len)) && (ie->id == DOT11_MNG_FRAGMENT_ID)) {
...
int
bcm_tlv_dot11_frag_tot_len(void *buf, uint buf_len,
	uint16 id, uint *ie_len)
{
	return  bcm_tlv_dot11_defrag(buf, buf_len, id, NULL, ie_len, NULL);
}

int
bcm_tlv_dot11_frag_tot_data_len(void *buf, uint buf_len,
	uint16 id, uint *ie_len, uint *data_len)
{
	return  bcm_tlv_dot11_defrag(buf, buf_len, id, NULL, ie_len, data_len);
}
 ...
 */
int
bcm_tlv_dot11_frag(uint16 id, const void *data, uint data_len,
	uint8 *buf, uint *buf_len)
{
	uint8 id_ext_len = id > DOT11_MNG_ID_EXT_ID ? 1u : 0;
	...
	if (is_write) {
		...
		if (id > DOT11_MNG_ID_EXT_ID) {
			ie_data = bcm_write_tlv_ext(DOT11_MNG_ID_EXT_ID, id - DOT11_MNG_ID_EXT_ID,
		...
		while (num_frags > 1u) {
			...
			len_to_copy = (num_frags > 2u) ? BCM_TLV_MAX_DATA_SIZE :
				(data_len - copied_len);
...
int
bcm_tlv_dot11_frag_subelement(uint16 subelement_id, const void *data, uint data_len,
	uint8 *buf, uint *buf_len)
{
	bool is_write = (buf != NULL);
	uint8 *ie_data = NULL;
	...
	if ((data_len) > BCM_TLV_MAX_DATA_SIZE) {
		num_frags = CEIL(data_len, BCM_TLV_MAX_DATA_SIZE);
	} else {
		num_frags = 1u;
	}
	ie_len = data_len + num_frags * BCM_TLV_HDR_SIZE;

	if (is_write) {
		uint len_to_copy = (num_frags > 1u) ? (uint)(BCM_TLV_MAX_DATA_SIZE) :
			data_len;
		uint copied_len = 0;
		if (ie_len > *buf_len) {
			err = BCME_BUFTOOSHORT;
		...
		ie_data = bcm_write_tlv(subelement_id, data, len_to_copy, buf);
		while (num_frags > 1u) {
			copied_len += len_to_copy;
			len_to_copy = (num_frags > 2u) ? BCM_TLV_MAX_DATA_SIZE :
				(data_len - copied_len);
			ie_data = bcm_write_tlv(EHT_ML_SE_FRAG_ID,
				(const uint8 *)data + copied_len,
				len_to_copy, ie_data);
			num_frags --;
		};
...
int
bcm_tlv_dot11_defrag_subelement(void *buf, uint buf_len, uint16 subelement_id,
		uint8 *out, uint *out_len, uint *data_len)
{
	int err = BCME_OK;
	bcm_tlv_t *ie;
	...
	ie = bcm_parse_tlvs_dot11(buf, buf_len, subelement_id, FALSE);
	if (!ie) {
		err = BCME_IE_NOTFOUND;
		goto done;
	}
	...
	/* update length from fragments, okay if no next ie */
	while ((ie = bcm_next_tlv(ie, (int *)&buf_len)) && (ie->id == EHT_ML_SE_FRAG_ID)) {
		/* note: buf_len starts at next ie and last frag may be partial */
		if (out_left < ie->len) {
			err = BCME_BUFTOOSHORT;
			out_left = 0;
...
int
bcm_tlv_dot11_frag_subelement_tot_len(void *buf, uint buf_len,
	uint16 id, uint *ie_len)
{
	return  bcm_tlv_dot11_defrag_subelement(buf, buf_len, id, NULL, ie_len, NULL);
}

int
bcm_tlv_dot11_frag_subelement_tot_data_len(void *buf, uint buf_len,
	uint16 id, uint *ie_len, uint *data_len)
{
	return  bcm_tlv_dot11_defrag_subelement(buf, buf_len, id, NULL, ie_len, data_len);
}

#if defined(BCMDBG) || defined(DHD_DEBUG) || defined(WLTEST)
/** Utility function to convert stall type to string */
char *stalltype2str(char stall_type)
{
	switch (stall_type)
	{
		case 0: /* Reference: WLC_DSTALL_NOTFOUND(wlioctl.h) */
			return "NOTFOUND";
		case 1: /* Reference: WLC_TXSTALL_ONCE(wlioctl.h) */
			return "ONCE";
		case 2: /* Reference: WLC_TXSTALL_PROLONG(wlioctl.h) */
			return "PROLONG";
		default:
			return "StallTypeErr";
	}
...
/** Util fn to return a simple running average across a sample size */
uint32
bcm_running_avg(uint32 cur_val, uint32 new_val, uint16 sample_size)
{
	uint32 new_avg;
	if (new_val > cur_val) {
		/* Round up the new average value */
		new_avg = ((cur_val * sample_size) - cur_val + new_val);
		new_avg = CEIL(new_avg, sample_size);
	} else {
		/* Round down the new average value */
		new_avg = ((cur_val * sample_size) - cur_val + new_val) / sample_size;
	}
	return new_avg;
}

#if defined(BCM_ROUTER) && defined(BCMDRIVER) && !defined(DONGLEBUILD)
#if !defined(BCMDONGLEHOST) && !defined(CMWIFI)
/*
 * Read potential WLAN relevant SOC info from OTP.
 */
int
bcm_get_soc_info(bcm_soc_info_t *soc_info)
{
	int err = 0;
	int rc;
	struct device_node *chip = of_find_node_by_path("/chip_info");

	ASSERT(soc_info != NULL);
	bzero(soc_info, sizeof(*soc_info));

	if (chip == NULL) {
		err = -1;
	} else {
		rc = of_property_read_u32(chip, "process", &soc_info->process);
		if (rc != 0) {
			err += 1;
		}
		rc = of_property_read_u32(chip, "substrate", &soc_info->substrate);
		if (rc != 0) {
			err += 2;
		}
		rc = of_property_read_u32(chip, "foundry", &soc_info->foundry);
		if (rc != 0) {
			err += 4;
		}
	}
