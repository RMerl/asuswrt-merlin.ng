 ...
 *
 * $Id: hnd_pktq.c 837403 2024-03-07 02:27:20Z $
 */
...
/*
 * if the incoming packet has multiple frags attached to it, we account for them
 * by adding or subtracting all frags from the queue size.
 */
static inline void spc_adjust(struct pktq_prio *q, struct pktq *pq, void *p, bool add)
{
	/*
	 * Only AMPDU queues count MSDUs and need to account for pre-aggregated
	 * SPC packets.
	 */
	if (pq->pktq_type != PKTQ_QUEUE_AMPDU) {
		return;
	}

	if (PKTISSPC(OSH_NULL, p)) {
		int16 nr_frags = PKT_FRAG_CNT(OSH_NULL, p);

		if (!add) {
			ASSERT(nr_frags <= q->n_pkts);
			nr_frags = -nr_frags;
		}
		...
		if (pq) {
			pq->n_pkts_tot += nr_frags;
		}
	}
}

static inline void spc_adjust_list(struct pktq_prio *list_q,
		struct pktq_prio *q, struct pktq *pq,  bool add)
{
	void *p = list_q->head;
	...
	while (p) {
		if (PKTISSPC(OSH_NULL, p)) {
			spc_adjust(q, pq, p, add);
		}
		p = PKTLINK(p);
	}
}

#else /* CONFIG_HAVE_SKBUFF_WL_SPC_V2 */
static inline void spc_adjust(struct pktq_prio *q, struct pktq *pq, void *p, bool add)
{
}
...
#endif /* CONFIG_HAVE_SKBUFF_WL_SPC_V2 */
...
pktqprio_penq(struct pktq *pq, int prio, void *p)
{
	...
	/* WAR for invalid q->tail */
	if (q->head && (((unsigned long)q->tail) > (RAMSIZE + RAMBASE))) {
		void *tail = q->head;

		/* search for the tail */
		while (tail && PKTLINK(tail)) {
			tail = PKTLINK(tail);
		}

		printf("%s: WAR: Set invalid q->tail %p to %p\n",
			__FUNCTION__, q->tail, tail);
...
/**
 * Append spktq 'list' to the tail of pktq 'pq', list has frags
 *
 * @param pq    Multi-priority packet queue
 * @param list  Single priority packet queue
 */
void
pktqprio_append_w_frags(struct pktq *pq, int prio, struct spktq *list, uint16 cnt)
{
	struct pktq_prio *q;				/**< single priority packet queue */
	struct pktq_prio *list_q;			/**< single priority packet queue */
...
pktqprio_prepend(struct pktq *pq, int prio, struct spktq *list)
{
	...
	q->n_pkts += list_q->n_pkts;
	pq->n_pkts_tot += list_q->n_pkts;
	spc_adjust_list(list_q, q, pq, TRUE);
