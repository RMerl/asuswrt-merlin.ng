 ...
 *
 * $Id: hndpci.c 821234 2023-02-06 14:16:52Z $
 */
...
static pci_config_regs si_pci_null =
#ifdef __GNUC__
        /* avoid a missing-field-initializer warning by using designated inits. */
		...
		if (pcie != NULL) {
			...
			if (off >= 256) {
				...
				*val = pcie_readreg(sih, pcie, PCIE_CONFIGREGS,
				...
				GCC_DIAGNOSTIC_PUSH_SUPPRESS_CAST();
				*addr = (uint32 *)&pcie->u.pcie1.pciecfg[func][off >> 2];
				GCC_DIAGNOSTIC_POP();
				*val = R_REG(osh, *addr);
			}
			ret = TRUE;
...
BCMATTACHFN(hndpci_init_pci)(si_t *sih, uint coreunit)
{
	...
	if (!host) {
		...
		if (pci) {
			...
			W_REG(osh, &pcie->sbtopcie0, SBTOPCIE_ACCESS_TYPE_IO);

			/* 64 MB configuration access window */
			W_REG(osh, &pcie->sbtopcie1, SBTOPCIE_ACCESS_TYPE_CFG0);

			/* 1 GB memory access window */
			W_REG(osh, &pcie->sbtopciebigmem, SBTOPCIE_ACCESS_TYPE_MEM |
...
BCMATTACHFN(hndpci_init_regions)(si_t *sih, uint func, pci_config_regs *cfg, si_bar_cfg_t *bar)
{
	...
	if ((si_coreid(sih) == USB20H_CORE_ID) ||
		...
		base = htol32(si_addrspace(sih, CORE_SLAVE_PORT_0, CORE_BASE_ADDR_0));
		if (issb) {
			base1 = base + 0x800;	/* OHCI/EHCI */
		} else {
			/* In AI chips EHCI is addrspace 0, OHCI is 1 */
			base1 = base;
			base = htol32(si_addrspace(sih, CORE_SLAVE_PORT_0,
				CORE_BASE_ADDR_1));
		}
		...
		for (i = 0; i < n; i++) {
			int size = si_addrspacesize(sih, CORE_SLAVE_PORT_0, i);

			if (size) {
				cfg->base[i] = htol32(si_addrspace(sih, CORE_SLAVE_PORT_0, i));
				bar->size[i] = size;
			}
		}
	}
...
BCMATTACHFN(hndpci_init_cores)(si_t *sih)
{
	...
	for (dev = 0; dev < SI_MAXCORES; dev ++) {
		...
		if (!(regs = (volatile void *)si_setcoreidx(sih, dev)))
			continue;
