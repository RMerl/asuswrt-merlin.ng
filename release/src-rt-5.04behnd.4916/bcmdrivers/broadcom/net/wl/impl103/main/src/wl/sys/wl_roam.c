 ...
 *
 * $Id: wl_roam.c 821234 2023-02-06 14:16:52Z $
 */
...
int
init_roam_cache(struct bcm_cfg80211 *cfg, int ioctl_ver)
{
	int err;
...
#ifdef WES_SUPPORT
int
get_roamscan_mode(struct net_device *dev, int *mode)
{
	*mode = roamscan_mode;
...
int
set_roamscan_mode(struct net_device *dev, int mode)
{
	int error = 0;
	roamscan_mode = mode;
	n_roam_cache = 0;
...
int
get_roamscan_channel_list(struct net_device *dev, unsigned char channels[])
{
	int n = 0;
...
int
set_roamscan_channel_list(struct net_device *dev,
...
#ifdef ESCAN_CHANNEL_CACHE
void
set_roam_band(int band)
{
	roam_band = band;
}

void
reset_roam_cache(struct bcm_cfg80211 *cfg)
{
	if (!cfg->rcc_enabled) {
		return;
	}
...
int
get_roam_channel_list(int target_chan,
...
void
print_roam_cache(struct bcm_cfg80211 *cfg)
{
	int i;
...
static void
add_roamcache_channel(wl_roam_channel_list_t *channels, chanspec_t ch)
{
	int i;
...
void
update_roam_cache(struct bcm_cfg80211 *cfg, int ioctl_ver)
{
...
void
wl_update_roamscan_cache_by_band(struct net_device *dev, int band)
{
