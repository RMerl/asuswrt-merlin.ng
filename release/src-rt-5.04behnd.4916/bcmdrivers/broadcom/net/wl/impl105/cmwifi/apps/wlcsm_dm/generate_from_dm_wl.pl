...
sub output_wlcsm_strmapper_file  {
    ...
    my $fileRef = shift;
    my $fileRef_h = shift;
    my (%rowHash, %prevRowHash, %prevObjRowHash );
    my $closePrevRow=0;
    my $oid;
    my $oidName;
    my $oidWlcsmName;
    ...
    while (parse_row($inputFileRef, \%rowHash)) {
		if(defined($prevRowHash{"name"})) {
			next if($rowHash{"name"} eq $prevRowHash{"name"});
		}
		%prevRowHash = %rowHash;
...
sub wlcsm_nvram_mngrname {
    ...
    elsif($varname eq "mngr_Dev2WifiIasStaObject") { $result2="WLCSM_WLAN_IAS_STA_STRUCT";}
...
my $result2;
    ...
    elsif ($varnames[0] eq "wifi") { $result2= "MNGR_POS_WIFI,"; }
    else { $result2="WRONGVALUE"; }
	...
	$mngr_oidname="WLMNGR_DATA_POS_".$mngr_oidname;
		...
		$entry_count=$entry_count+1;
                ...
                (defined ($rowHash{"gmapping"}) &&(( $rowHash{"gmapping"} eq "1" ) || ( $rowHash{"gmapping"} eq "2" ))))
            {
                ...
                print $fileLtc "/*! \\brief Obj struct for $rowHash{\"name\"}\n";
                print $fileLtc " *\n";
                print $fileLtc " * MDMOID_$oidName $oid\n";
                print $fileLtc " */\n";
                print $fileLtc "WLCSM_DM_WLMNGR_MAPPING  g_dm_lattice_mapping","_$oidName","[]= \n{\n";
                print $fileLtc_h "extern WLCSM_DM_WLMNGR_MAPPING  g_dm_lattice_mapping","_$oidName","[";
                $entry_count=0;
		$gmapping=1;
...
#
		...
		$lowermngrv=lc($mngrvar);
                print $fileRef_n1 sprintf("{%-30s%-80s%-30s%-10s},\n","\"$lowermngrv\",","WLCSM_DM_VAR_POSOFF($mngrvar_1,$mngrvar),","$convertedTypeName,",$nvrammap_index);
                ...
                print $fileLtc "{\t{\"$convertedFieldName\",\t0 },\n\t{\"$mngrvar\",\tWLCSM_DM_VAR_POSOFF(mngr_$oidWlcsmName,$mngrvar)},\n\t$convertedTypeName,\t$mappername},\n";
...
# print $fileRef "\t/**< $rowHash{\"name\"} */\n";
    ...
    print $fileLtc "} $objName;\n\n";
    print $fileLtc "/*! \\brief _$objName is used internally to represent $objName */\n";
    print $fileLtc "typedef $objName _$objName;\n\n\n\n";
	}
...
if ($ARGV[0] eq "wlcsm_mngr")
{
    ...
    $fileRef = open_filehandle_for_output("$build_dir/userspace/private/apps/wlan/wlcsm_dm/include/wlcsm_dm_generic.h");
    ...
    #print $fileRef "typedef mngr_Dev2WifiIasStaObject		        WLCSM_WLAN_IAS_STA_STRUCT;\n";
    ...
    print $fileRef "        WL_STATION_LIST_ENTRY  *assocailated_stas;\n";
...
elsif ($ARGV[0] eq "wlcsm")
{
    ...
    $fileLtc = open_filehandle_for_output("$build_dir/userspace/private/apps/wlan/wlcsm_dm/dm_impls/lattice/src/wlcsm_dm_lattice_mngr_mapping.c");
    ...
    $fileLtc_h = open_filehandle_for_output("$build_dir/userspace/private/apps/wlan/wlcsm_dm/dm_impls/lattice/include/wlcsm_dm_lattice_mngr_mapping.h");
    ...
    print $fileLtc_h "#ifndef __WLCSM_DM_LATTICE_MNGR_MAPPING_H__  \n";
    print $fileLtc_h "#define __WLCSM_DM_LATTICE_MNGR_MAPPING_H__ \n\n\n";
    ...
    autogen_warning($fileLtc);
    autogen_warning($fileLtc_h);
    ...
    doxygen_header($fileLtc, "wlcsm_dm_lattice_mngr_mapping.c");
    doxygen_header($fileLtc_h, "wlcsm_dm_lattice_mngr_mapping.h");
    ...
    print $fileLtc_h "/* DO NOT INCLUDE THIS HEADER FILE DIRECTLY, INSTEAD INCLUDE wlcsm_lib_dm.h */\n\n";
    ...
    print $fileRef "#include \"mdm_object.h\"\n\n";
    print $fileLtc "#include \"wlcsm_lib_dm.h\"\n\n";
    print $fileLtc "#include \"wlcsm_dm_lattice_mngr_mapping.h\"\n\n";
    print $fileRef_n "#include \"wlcsm_lib_dm.h\"\n\n";
    print $fileRef_h "#include \"wlcsm_lib_dm_datatype.h\"\n\n";
    print $fileLtc_h "#include \"wlcsm_lib_dm_datatype.h\"\n\n";
    ...
    $inputFileRef = open_filehandle_for_input("$build_dir/data-model/$inputFile1");
    output_wlcsm_mapping_file($inputFileRef, $fileRef,$fileRef_h,$fileLtc,$fileLtc_h,$fileRef_n,$fileRef_v,$fileRef_n1,$fileRef_v1,$fileRef_oid_mapper,$fileRef_oid_mapper_h);
    close $inputFileRef;
