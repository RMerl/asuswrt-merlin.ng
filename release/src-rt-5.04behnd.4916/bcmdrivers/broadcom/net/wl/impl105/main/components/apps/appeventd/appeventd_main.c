...
static appeventd_wksp_t *g_nwksp = NULL;
...
/* Parse the Agent Status Changed event data received from wbd. */
static void
appeventd_parse_wbd_agent_status_changed_data(char *pkt, int len)
{
	app_event_wbd_agent_status_changed_t *agent_data = NULL;
	int ulen = sizeof(*agent_data);

	if (len < ulen) {
		APPEVENTD_ERROR("WBD-Length mismatch for Agent Status Changed event "
			"rcvd len %d expected len %d\n", len, ulen);
		return;
	}

	agent_data = (app_event_wbd_agent_status_changed_t *)pkt;
	APPEVENTD_INFO("WBD-Agent : AL_MAC ["MACF"] %s SmartMesh\n\n",
		ETHER_TO_MACF(agent_data->device_id),
		((agent_data->status == MAP_AGENT_ADDED) ?
		"Added to" : "Removed from"));
}

/* Parse the Controller Status Changed event data received from wbd. */
static void
appeventd_parse_wbd_controller_status_changed_data(char *pkt, int len)
{
	app_event_wbd_controller_status_changed_t *controller_data = NULL;
	int ulen = sizeof(*controller_data);

	if (len < ulen) {
		APPEVENTD_ERROR("WBD-Length mismatch for Controller Status Changed event "
			"rcvd len %d expected len %d\n", len, ulen);
		return;
	}

	controller_data = (app_event_wbd_controller_status_changed_t *)pkt;
	APPEVENTD_INFO("WBD-Agent : AL_MAC ["MACF"] %s SmartMesh\n\n",
		ETHER_TO_MACF(controller_data->device_id),
		((controller_data->status == MAP_CONTROLLER_ADDED) ?
		"Added to" : "Removed from"));
}

/* Parse the Client Status Changed event data received from wbd. */
static void
appeventd_parse_wbd_client_status_changed_data(char *pkt, int len)
{
	app_event_wbd_client_status_changed_t *client_data = NULL;
	int ulen = sizeof(*client_data);

	if (len < ulen) {
		APPEVENTD_ERROR("WBD-Length mismatch for Client Status Changed event "
			"rcvd len %d expected len %d\n", len, ulen);
		return;
	}

	client_data = (app_event_wbd_client_status_changed_t *)pkt;
	APPEVENTD_INFO("WBD-Controller : STA["MACF"] %s SmartMesh\n\n",
		ETHER_TO_MACF(client_data->sta_addr),
		((client_data->status == MAP_CLIENT_ADDED) ?
		"Added to" : "Removed from"));
}
...
#ifdef BCM_QOSMGMT
static void
appeventd_qosmgmt_init(appeventd_app_t *app)
{
	memset(app->bitvec, 0, sizeof(app->bitvec));

	app->port_no = APPEVENTD_WKSP_QOSMGMT_UDP_SPORT;

	setbit(app->bitvec, APP_E_WBD_SLAVE_AGENT_STATUS_CHANGED);
	setbit(app->bitvec, APP_E_WBD_SLAVE_CLIENT_STATUS_CHANGED);
	setbit(app->bitvec, APP_E_WBD_SLAVE_CONTROLLER_STATUS_CHANGED);
	return;
}
#endif /* BCM_QOSMGMT */

static int
appeventd_app_sendup(appeventd_app_t *app, app_event_t *app_event,
	int sock, char *pdata, int bytes)
{
	int sentBytes = 0;
	struct sockaddr_in appSocket;

	/* Send Event to App, only if it has registered for it */
	if (!isset(app->bitvec, app_event->h.event_id)) {
		return APPEVENTD_OK;
	}

	/* Send to appSocket */
	memset(&appSocket, 0, sizeof(appSocket));
	appSocket.sin_family = AF_INET;
	appSocket.sin_addr.s_addr = htonl(INADDR_LOOPBACK);
	appSocket.sin_port = htons(app->port_no);
	sentBytes = sendto(sock, pdata, bytes, 0,
		(struct sockaddr *)&appSocket, sizeof(struct sockaddr_in));
	if (sentBytes != bytes) {
		APPEVENTD_ERROR("UDP send failed; bytes to send = %d, "
			"sentBytes = %d\n", bytes, sentBytes);
...
{
#ifdef BCM_QOSMGMT
	appeventd_qosmgmt_init(&g_nwksp->qosmgmt);
#endif /* BCM_QOSMGMT */
...
static int
appeventd_handle_event(app_event_t *app_event,
	int sock, char *pdata, int bytes)
{
#ifdef BCM_QOSMGMT
	/* Send to QoSMgmt appSocket */
	appeventd_app_sendup(&g_nwksp->qosmgmt, app_event, sock, pdata, bytes);
#endif /* BCM_QOSMGMT */
...
appeventd_main_loop(int sock)
{
	...
	if ((status > 0) && FD_ISSET(sock, &fdset)) {
		if ((bytes = recv(sock, pkt, APPEVENTD_BUFSIZE, 0)) > sizeof(app_event_h)) {
			...
			APPEVENTD_INFO("Received event %d, timestamp=%lld, status=%d, len=%d\n",
				app_event->h.event_id, (long long) app_event->h.timestamp,
			...
			switch (app_event->h.event_id) {
				...
				case APP_E_WBD_MASTER_AGENT_STATUS_CHANGED:
					APPEVENTD_DEBUG("Received event %s len = %d\n",
						"APP_E_WBD_MASTER_AGENT_STATUS_CHANGED", len);
					appeventd_parse_wbd_agent_status_changed_data(pkt, len);
					break;

				case APP_E_WBD_SLAVE_CONTROLLER_STATUS_CHANGED:
					APPEVENTD_DEBUG("Received event %s len = %d\n",
						"APP_E_WBD_SLAVE_CONTROLLER_STATUS_CHANGED", len);
					appeventd_parse_wbd_controller_status_changed_data(pkt, len);
					break;

				case APP_E_WBD_MASTER_CLIENT_STATUS_CHANGED:
					APPEVENTD_DEBUG("Received event %s len = %d\n",
						"APP_E_WBD_MASTER_CLIENT_STATUS_CHANGED", len);
					appeventd_parse_wbd_client_status_changed_data(pkt, len);
					break;

				case APP_E_WBD_SLAVE_AGENT_STATUS_CHANGED:
					APPEVENTD_DEBUG("Received event %s len = %d\n",
						"APP_E_WBD_SLAVE_AGENT_STATUS_CHANGED", len);
					appeventd_parse_wbd_agent_status_changed_data(pkt, len);
					break;

				case APP_E_WBD_SLAVE_CLIENT_STATUS_CHANGED:
					APPEVENTD_DEBUG("Received event %s len = %d\n",
						"APP_E_WBD_SLAVE_CLIENT_STATUS_CHANGED", len);
					appeventd_parse_wbd_client_status_changed_data(pkt, len);
					break;
			...
			/* send Appeventd data out */
			appeventd_handle_event(app_event, sock, buf, bytes);
		}
	}
}
...
main(int argc, char **argv)
{
	...
	/* Alloc Appevent workspace */
	g_nwksp = (appeventd_wksp_t *)malloc(sizeof(*g_nwksp));
	if (!g_nwksp) {
		APPEVENTD_ERROR("Err: Unable to allocate wksp memory\n");
		return APPEVENTD_FAIL;
	}
	...
	if ((sock = appeventd_socket_init()) < 0) {
        ...
        /* Set App specific bitvec & port_no */
	if (appeventd_app_init()) {
		APPEVENTD_ERROR("Err: fail to init app specific data\n");
		return APPEVENTD_FAIL;
	}
