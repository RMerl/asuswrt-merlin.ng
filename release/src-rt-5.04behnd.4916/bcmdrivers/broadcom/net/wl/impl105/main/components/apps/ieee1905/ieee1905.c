 ...
 *
 * $Id: ieee1905.c 839585 2024-04-29 07:08:00Z $
 */
...
/* Routine to fill the device's fronthaul and backhaul akm suite capabilities
 * XXX: FIXME these can be filled using callbacks from wbd for initial cut working with fixed
 * values */
  ...
  unsigned char *mld_addr, int8 mld_unit)
{
  ...
  /* Mark BSS as MLO if the mld_unit is not -1 else mark it as pseudo MLO. For pseudo MLO mld_unit
   * will be -1 here
   */
  if (mld_addr && !i5DmIsMacNull(mld_addr)) {
    if (mld_unit >= 0) {
      ...
      pdmbss->flags |= I5_BSS_FLAG_PSEUDO_MLO_ENAB;
    }

    /* Add SSID to Agent AP MLD Configuration */
    agent_ap_mld = i5DmFindAPMLDMACInApMLDConf(&i5_dm_network_topology.agent_ap_mld_conf, mld_addr);
