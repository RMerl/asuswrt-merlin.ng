 ...
 *
 * $Id: ieee1905_tlv_r3.c 839585 2024-04-29 07:08:00Z $
 */
...
int i5TlvAgentAPMLDConfigurationInsert(i5_message_type *pmsg, ieee1905_glist_t *agent_ap_mld_conf)
{
  ...
  foreach_iglist_item(agent_ap_mld, i5_agent_ap_mld_conf_t, *agent_ap_mld_conf) {
    ...
    /* Skip sending Pseudo MLO Configurations as AP MLDs from controller */
    if (I5_IS_MULTIAP_CONTROLLER(i5_config.flags) && I5_IS_AGENT_AP_MLD_PSEUDO_MLO(agent_ap_mld)) {
...
{
  unsigned char buf[sizeof(i5_tlv_t) + i5TlvAffiliatedSTAMetrics_Min_Len];
  i5_tlv_t *ptlv = (i5_tlv_t *)buf;
  ...
  ptlv->length = htons(i5TlvAffiliatedSTAMetrics_Min_Len);
  len += sizeof(i5_tlv_t);
  ...
  *((unsigned int *)&buf[len]) = htonl(bytes_sent);
  len += 4;

  *((unsigned int *)&buf[len]) = htonl(bytes_recv);
  len += 4;

  *((unsigned int *)&buf[len]) = htonl(affiliated_metric->packets_sent);
  len += 4;

  *((unsigned int *)&buf[len]) = htonl(affiliated_metric->packets_recv);
  len += 4;

  *((unsigned int *)&buf[len]) = htonl(affiliated_metric->tx_packet_err);
  len += 4;
