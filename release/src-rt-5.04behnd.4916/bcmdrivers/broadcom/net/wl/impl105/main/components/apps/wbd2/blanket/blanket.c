 ...
 *
 * $Id: blanket.c 841918 2024-07-02 12:57:47Z $
 *
 *  @author
 *  @bug No known bugs.
 */
...
#define BKT_MAX_TXPWR (33) /* Chanspec Txpwr Max */
...
/* Get Maximum Tx Power of Chanspec */
int
blanket_get_chanspec_txpwr_max(char *ifname, chanspec_t chanspec, int *txpower)
{
	wl_chanspec_txpwr_max_t params = {0}, *txpwrbuf = NULL;
	uint8 buf[BKT_TXPWR_BUF_LEN] = {0};
	int i = 0, ret = BKTE_OK, tx = 0;
	...
	ret = wl_iovar_getbuf(ifname, "chanspec_txpwr_max", &params, sizeof(params), buf, BKT_TXPWR_BUF_LEN);
	BKT_IOVAR_ASSERT(ifname, "chanspec_txpwr_max", ret);

	txpwrbuf = (wl_chanspec_txpwr_max_t *) buf;
	txpwrbuf->ver = dtoh16(txpwrbuf->ver);
	txpwrbuf->len = dtoh16(txpwrbuf->len);
	txpwrbuf->count = dtoh32(txpwrbuf->count);

	if (txpwrbuf->ver != WL_CHANSPEC_TXPWR_MAX_VER) {
		BKT_ERROR("Failed version check received %u != %u expected\n", txpwrbuf->ver, WL_CHANSPEC_TXPWR_MAX_VER);
		ret = -1;
		goto end;
	}

	if (txpwrbuf->count < 1) {
		BKT_ERROR("Failed count check received %u < 1 expected\n", txpwrbuf->count);
		ret = -1;
		goto end;
	}
...
{
	int ret = BKTE_OK;
	wl_mlo_info_basic_t *mlo_info = NULL;

	mlo_info = bkt_malloc(WLC_IOCTL_MAXLEN, &ret);
	BKT_ASSERT();

	ret = blanket_get_mlo_info(ifname, mlo_info, WLC_IOCTL_MAXLEN);
	if (ret != BKTE_OK) {
		goto end;
	}
	...
	ieee1905_mlo_sta_link_stats *out_link_stats, int arr_size)
{
	...
	for (i = 0; i < arr_size; i++) {
		...
		if (!(pstats->assoc_linkid_bmp & (1 << i))) {
			continue;
		}
...
/* Get MLO info */
int blanket_get_mlo_info(char *ifname, wl_mlo_info_basic_t *out_mlo_info, uint32 size)
{
	int ret = BKTE_OK;
	...
	ret = bcm_pack_xtlv_entry(&rem, &rem_len, WL_MLO_CMD_INFO,
		in_len, (uint8 *)&out_mlo_info, BCM_XTLV_OPTION_ALIGN32);
	if (ret != BKTE_OK) {
		goto end;
	}

	ret = wl_iovar_getbuf(ifname, "mlo", &mybuf, sizeof(mybuf), out_mlo_info, size);
	if (ret != BKTE_OK) {
		goto end;
	}

	out_mlo_info->len = dtoh16(out_mlo_info->len);

	if (out_mlo_info->ver > WL_MLO_INFO_VER) {
		BKT_ERROR("Supported mlo_info version: %d but received version: %d\n",
			WL_MLO_INFO_VER, out_mlo_info->ver);
		ret = BKTE_WL_ERROR;
		goto end;
	}

	if (out_mlo_info->len > WLC_IOCTL_MAXLEN || (out_mlo_info->len < (sizeof(*out_mlo_info) +
		out_mlo_info->no_of_mlo_scb * sizeof(*(out_mlo_info->msi))))) {
		ret = BKTE_WL_ERROR;
		goto end;
	}
...
/* Get MLO SCB info */
int blanket_get_mlo_scb(char *ifname, struct ether_addr *sta_mac, wl_mlo_scb_info_t *msi)
{
	int i, ret = BKTE_OK;
	wl_mlo_info_basic_t *mlo_info = NULL;
	uint8 found = FALSE;
	...
	for (i = 0; i < mlo_info->no_of_mlo_scb; i++) {
		if (!eacmp(sta_mac, &(mlo_info->msi[i].ea))) {
			found = TRUE;
			memcpy(msi, &mlo_info->msi[i], sizeof(*msi));
			break;
		}
	}

	if (!found) {
		BKT_ERROR("STA["MACF"] isn't found in the MLO SCB list\n",
...
/* Convert driver MLO mode to IEEE1905 */
int
blanket_driver_mlo_mode_to_ieee1905(uint8 mode, uint8 *out_mode)
{
	int ret = BKTE_OK;

	switch (mode) {
		case MLO_STR:
			*out_mode = I5_MLD_MODE_STR;
			break;
		case MLO_NSTR:
			*out_mode = I5_MLD_MODE_NSTR;
			break;
		case MLO_EMLSR:
			*out_mode = I5_MLD_MODE_EMLSR;
			break;
		case MLO_EMLMR:
			*out_mode = I5_MLD_MODE_EMLMR;
			break;
		default:
			/* STR mode is the default value when the 1905 client
			 * is created. For error cases, leave it untouched.
			 */
			*out_mode = I5_MLD_MODE_STR;
			ret = BKTE_INV_ARG;
			break;
...
/* Force olpc caliberation */
int
blanket_set_olpc_cal(char *ifname)
{
	int ret = BKTE_OK, val;
	BKT_ENTER();
	...
	BKT_INFO("ifname: %s Setting force olpc caliberation \n", ifname);

	ret = wl_iovar_getint(ifname, "olpc_cal_force", &val);
	BKT_IOVAR_ASSERT(ifname, "olpc_cal_force", ret);
