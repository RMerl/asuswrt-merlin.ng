...
/* round(cos(2*M_PI .* [0:128]/512.0) * pow(2.0, TWDL_SFT))  2^8 */
	256,
	256,
	256,
	256,
...
static int16 TWDL_cos_256[257] = {
/* round(cos(2*M_PI .* [0:257]/1024.0) * pow(2.0, TWDL_SFT))  2^8 */
	256,  256,  256,  256,  256,  256,  256,  256,  256,  256,  256,  255,  255,
	255,  255,  255,  255,  255,  254,  254,  254,  254,  254,  253,  253,  253,
	253,  252,  252,  252,  252,  251,  251,  251,  250,  250,  250,  249,  249,
	249,  248,  248,  248,  247,  247,  246,  246,  245,  245,  245,  244,  244,
	243,  243,  242,  242,  241,  241,  240,  239,  239,  238,  238,  237,  237,
	236,  235,  235,  234,  233,  233,  232,  231,  231,  230,  229,  229,  228,
	227,  227,  226,  225,  224,  224,  223,  222,  221,  220,  220,  219,  218,
	217,  216,  215,  215,  214,  213,  212,  211,  210,  209,  208,  207,  207,
	206,  205,  204,  203,  202,  201,  200,  199,  198,  197,  196,  195,  194,
	193,  192,  191,  190,  189,  188,  186,  185,  184,  183,  182,  181,  180,
	179,  178,  177,  175,  174,  173,  172,  171,  170,  168,  167,  166,  165,
	164,  162,  161,  160,  159,  157,  156,  155,  154,  152,  151,  150,  149,
	147,  146,  145,  144,  142,  141,  140,  138,  137,  136,  134,  133,  132,
	130,  129,  128,  126,  125,  123,  122,  121,  119,  118,  117,  115,  114,
	112,  111,  109,  108,  107,  105,  104,  102,  101,   99,   98,   97,   95,
	94,   92,   91,   89,   88,   86,   85,   83,   82,   80,   79,   77,   76,
	74,   73,   71,   70,   68,   67,   65,   64,   62,   61,   59,   58,   56,
	55,   53,   51,   50,   48,   47,   45,   44,   42,   41,   39,   38,   36,
	34,   33,   31,   30,   28,   27,   25,   24,   22,   20,   19,   17,   16,
	14,   13,   11,    9,    8,    6,    5,    3,    2,    0};
...
/**
 * Fixed point rounding routine
 *
 * @param[in] rnd_pos     MSB of the 'fractional' part of the fixed point number
 */
uint32
...
int32 math_fft_cos_256(int i)
{
	...
	if (i < 257) {
		...
		res = -1 * TWDL_cos_256[512 - i];
	} else if (i < 769) {
		res = -1 * TWDL_cos_256[i - 512];
	} else {
		res = TWDL_cos_256[1024 - i];
	}
...
int32 math_fft_sin_256(int i)
{
	int32 res;

	if (i < 257) {
		res = -1 * TWDL_cos_256[256 - i];
	} else if (i < 513) {
		res = -1 * TWDL_cos_256[i - 256];
	} else if (i < 769) {
		res = TWDL_cos_256[769 - i];
	} else {
		res = TWDL_cos_256[i - 769];
	}
