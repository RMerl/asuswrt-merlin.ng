...
#ifndef CONFIG_DRIVER_BRCM_RDKB_RADIUS_GREYLIST
	if (secret == NULL)
		return -1;
			       ...
			       const char *buf, char *pos, int line)
{
	if (os_strcmp(buf, "interface") == 0) {
		...
		*bss->network_auth_type = auth_type;
		...
		bss->hs20 = atoi(pos);
#ifdef CONFIG_DRIVER_BRCM_RDKB_PASSPOINT
	} else if (os_strcmp(buf, "brcm_hs20") == 0) {
		bss->brcm_hs20 = atoi(pos);
#endif /* CONFIG_DRIVER_BRCM_RDKB_PASSPOINT */
...
#define PARSE_TEST_PROBABILITY(_val)				\
	...
	} else if (os_strcmp(buf, "map_4_way_handshake") == 0) {
		int val = atoi(pos);

		if (val < 0 || val > 255) {
			wpa_printf(MSG_ERROR,
				"Line %d: Invalid map_4_way_handshake value",
				line);
			return 1;
		}
		conf->map_4_way_handshake = val;
	} else if (os_strcmp(buf, "map_eapol_key_timeout") == 0) {
		conf->map_eapol_key_timeout = (unsigned int)strtoul(pos, NULL, 0);
	...
	} else if (os_strcmp(buf, "mld_unit") == 0) {
		bss->mld_unit = atoi(pos);
	} else if (os_strcmp(buf, "link_id") == 0) {
		bss->link_id = atoi(pos);
	} else if (os_strcmp(buf, "ieee80211be") == 0) {
		/* generate mld_unit for pseudo */
		int unit, sub_unit = 0;
		bss->ieee80211be = atoi(pos);
		sscanf(bss->iface, "wl%d.%d", &unit, &sub_unit);
		if (!bss->ieee80211be)
			return 0;
		if (bss->mld_unit == -1) {
			/* formula to calculate pseudo unique mld_unit value
			 * using available data.
			 * Reserved is always 0.
			 * Pseudo is 1 for pseudo MLO else 0
			 * Unit is the radio index number from iface.
			 * Sub_unit is the vif taken from iface.
			 * _____________________________________________________________
			 * | Reserved:1 bit| Pseudo:1 bit| Unit:2 bits| Sub_unit:4 bits|
			 *--------------------------------------------------------------
			 */
			bss->mld_unit = 1 << 6 | unit << 4 | sub_unit ;
			bss->link_id = 0;
		}
	} else if (os_strcmp(buf, "mlo_aff_link_kde") == 0) {
		bss->mlo_aff_link_kde = atoi(pos);
...
#ifdef CONFIG_DRIVER_BRCM
	} else if (os_strcmp(buf, "ftm_mode") == 0) {
		bss->pasn_ftm_mode = atoi(pos);
