...
#ifdef CMWIFI
	pid_t pid = getpid();
	wpa_printf(MSG_DEBUG, "dm_unregister_app_restart_info(%d)", pid);
...
int main(int argc, char *argv[])
{
	...
	for (;;) {
		c = getopt(argc, argv, "b:Bde:f:hi:KP:sSTtu:v::g:G:");
		if (c < 0)
			break;
		switch (c) {
		case 'h':
...
#endif /* CONFIG_DEBUG_LINUX_TRACING */
		case 'v':
			if (optarg) exit(!has_feature(optarg));
			show_version();
			exit(1);
			break;
		case 'g':
	...
	for (i = 0; i < interfaces.count; i++) {
		...
		for (j = 0; j < iface->num_bss; j++) {
			...
			if ((hapd->conf->mld_unit != -1) && (hapd->conf->link_id != -1)) {
				if (hostapd_mlo_add(hapd,
						hapd->conf->mld_unit,
						hapd->conf->link_id)) {
					hostapd_interface_deinit_free(iface);
					goto out;
