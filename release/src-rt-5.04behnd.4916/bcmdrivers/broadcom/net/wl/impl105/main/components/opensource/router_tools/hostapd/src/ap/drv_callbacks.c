			...
			const u8 *req_ies, size_t req_ies_len, int reassoc)
{
	...
	if (elems.multi_link) {
		...
		if (!mliebuf) {
			wpa_printf(MSG_ERROR, "Assoc: No MLIE:\n");
			goto override_mlie_parse;
		}
		wpa_hexdump(MSG_DEBUG, "FUll Assoc Req MLIE: ", wpabuf_head(mliebuf),
				wpabuf_len(mliebuf));
...
#if (defined(CONFIG_DRIVER_BRCM) && LINUX_VERSION_CODE >= KERNEL_VERSION(4, 19, 0))
	if (!is_dfs && (hapd->iface->drv_flags & WPA_DRIVER_FLAGS_DFS_OFFLOAD)) {
		if (hapd->iface->state != HAPD_IFACE_ENABLED) {
			wpa_printf(MSG_DEBUG, "Hostapd state enable by CSA\n");
			hostapd_set_state(hapd->iface, HAPD_IFACE_ENABLED);
		}
	}
#endif /* CONFIG_DRIVER_BRCM && LINUX_VERSION_CODE >= KERNEL_VERSION(4, 19, 0) */
