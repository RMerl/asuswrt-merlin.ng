				   ...
				   const u8 *eap, size_t len)
{
	...
	else if (hapd->conf->brcm_hs20) {
		/**
		 * When Broadcom priorietary hotspot application is used for hotspot,
		 * hapd->conf->hs20 is not set, but some hotspot AVPs are required.
		 */
		u8 ver = hapd->conf->hs20_release - 1;
				...
				int remediation)
{
	...
	int dot11RSNAConfigPMKLifetime = hapd->conf->dot11RSNAConfigPMKLifetime;
