...
#ifdef CONFIG_DRIVER_BRCM_MLO
	if (((wpa_auth->conf.mld_unit >= 64) || (wpa_auth->conf.link_id == 0)) &&
...
SM_STATE(WPA_PTK, PTKINITNEGOTIATING)
{
	...
	dl_list_for_each(plink, conf->link_info, struct hostapd_link_unit, list)
	{
		...
		bool RSNE_PRSNT = 0;
		bool RSNXE_PRSNT = 0;
		if (!conf->mlo_aff_link_kde && !(sm->sta_link_bmp & (1 << plink->link_id))) {
		...
		/* Skip key update based on STA link bitmap */
                if (!(sm->sta_link_bmp & (1 << plink->link_id))) {
                        wpa_printf(MSG_DEBUG," NO matching link found"
                        "skip key update for Link:%d sta_bmp:%x\n",
                        plink->link_id, sm->sta_link_bmp);
                        continue;
                }
	...
	if (sm->wpa == WPA_VERSION_WPA2) {
		...
		if (sm->has_ML) {
			...
			dl_list_for_each(plink, conf->link_info, struct hostapd_link_unit, list)
			{
				...
				gtk = pgsm->GTK[pgsm->GN - 1];
				pos = wpa_add_kde(pos, RSN_KEY_DATA_MLO_GTK,(u8 *)&mlo_gtk,
						WPA_MLO_GTK_KDE_PREFIX_LEN, gtk, pgsm->GTK_len);
...
#ifdef CONFIG_DRIVER_BRCM_MLO
	/* Skip group key handshake for MLO STAs from aux links */
	if ((sm->wpa_auth->conf.link_id != 0) && (sm->has_ML))
		return 0;
#endif /* CONFIG_DRIVER_BRCM_MLO */
...
#ifdef CONFIG_DRIVER_BRCM_MLO
	/* Trigger key refresh and group key handshake on aux links when key interval is expired on main link */
	if (wpa_auth->conf.link_id == 0) {
		struct hostapd_link_unit *plink = NULL;
		dl_list_for_each(plink, wpa_auth->conf.link_info, struct hostapd_link_unit, list)
		{
			if (plink->link_id != 0)
			{
				struct hostapd_data *phapd = plink->hapd;
				struct wpa_authenticator *pwpa_auth = phapd->wpa_auth;
				struct wpa_group *pgroup = pwpa_auth->group;

				wpa_gtk_update(pwpa_auth, pgroup);
				if (pgroup->GKeyDoneStations) {
					wpa_printf(MSG_DEBUG,
					"wpa_group_setkeys: Unexpected GKeyDoneStations=%d when starting new GTK rekey",
						pgroup->GKeyDoneStations);
					pgroup->GKeyDoneStations = 0;
				}
				wpa_auth_for_each_sta(pwpa_auth, wpa_group_update_sta, pgroup);
				wpa_printf(MSG_DEBUG, "wpa_group_update_sta: GKeyDoneStations=%d",
						pgroup->GKeyDoneStations);
			}
		}
	}
				 ...
				 struct wpa_group *group)
{
	wpa_printf(MSG_DEBUG,
		   ...
		   group->vlan_id);
#ifdef CONFIG_DRIVER_BRCM_MLO
	/* Install keys on aux links */
	if ((group->wpa_group_state == WPA_GROUP_SETKEYS) && (wpa_auth->conf.link_id == 0)) {
		struct hostapd_link_unit *plink = NULL;
		dl_list_for_each(plink, wpa_auth->conf.link_info, struct hostapd_link_unit, list)
		{
			if (plink && plink->link_id != 0)
			{
				...
				struct wpa_authenticator *pwpa_auth = phapd->wpa_auth;
				struct wpa_group *pgroup = pwpa_auth->group;
				wpa_group_config_group_keys(pwpa_auth, pgroup);
			}
		}
	}
#endif /* CONFIG_DRIVER_BRCM_MLO */
