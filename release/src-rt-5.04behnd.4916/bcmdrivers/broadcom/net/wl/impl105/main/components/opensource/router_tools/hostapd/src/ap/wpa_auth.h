...
struct wpa_auth_config {
...
#if defined(CONFIG_PASN) && defined(CONFIG_DRIVER_BRCM)
	unsigned int urnm_mfpr;
#endif /* CONFIG_PASN && CONFIG_DRIVER_BRCM */
	...
	int mld_unit; /* MLD_UNIT */
	int link_id; /* LINK_ID */
	struct dl_list *link_info;
	int link_count;
	bool mlo_aff_link_kde;
#endif /* CONFIG_DRIVER_BRCM_MLO */
#ifdef CONFIG_DRIVER_BRCM_MAP
	int map_4_way_handshake;
	unsigned int map_eapol_key_timeout;	/* ms */
...
#ifdef CONFIG_DRIVER_BRCM
	int (*set_kdk)(void *ctx, const u8 *addr, u32 cipher, const u8 *kdk,
		       size_t kdk_len);
#endif /* CONFIG_DRIVER_BRCM */
#endif /* CONFIG_PASN */
