				  ...
				  struct wpa_auth_config *wconf)
{
	...
	wconf->dot11RSNAConfigPMKLifetime = conf->dot11RSNAConfigPMKLifetime;
#endif /* CONFIG_DRIVER_BRCM_RDKB_PMK_LIFETIME */
#ifdef CONFIG_DRIVER_BRCM_MLO
	wconf->mld_unit = conf->mld_unit;
	wconf->link_id = conf->link_id;
	wconf->mlo_aff_link_kde = conf->mlo_aff_link_kde;
#endif /* CONFIG_DRIVER_BRCM_MLO*/
#ifdef CONFIG_DRIVER_BRCM_MAP
	wconf->map_4_way_handshake = iconf->map_4_way_handshake;
	wconf->map_eapol_key_timeout = iconf->map_eapol_key_timeout;
...
#ifdef CONFIG_DRIVER_BRCM
static int hostapd_set_kdk(void *ctx, const u8 *peer_addr, u32 cipher,
			   const u8 *kdk,
			   size_t kdk_len)
{
	struct hostapd_data *hapd = ctx;
...
#ifdef CONFIG_DRIVER_BRCM
	return brcm_set_secure_ranging_ctx(hapd, hapd->own_addr, peer_addr,
					cipher, 0, NULL,
					0, NULL,
					kdk_len, kdk);
#else
...
int hostapd_setup_wpa(struct hostapd_data *hapd)
{
	...
	if ((hapd->conf->mld_unit != 255) && (hapd->conf->mld_unit != -1) &&
		...
		_conf.link_count = hapd->mld_link.mld->link_count;
		if(_conf.link_count) {
			_conf.link_info = &(hapd->mld_link.mld->link_info);
		}
		if(_conf.link_info) {
			wpa_printf(MSG_DEBUG,"Initializing MLO_LINK values link_info:%p"
				"link_count:%d \n", _conf.link_info, _conf.link_count);
		} else {
			wpa_printf(MSG_ERROR,"link_info is NULL \n");
		}
	}
#endif // endif
	...
	_conf.prot_range_neg =
		!!(hapd->iface->drv_flags2 & WPA_DRIVER_FLAGS2_PROT_RANGE_NEG_AP);

#if defined(CONFIG_PASN) && defined(CONFIG_DRIVER_BRCM)
	_conf.urnm_mfpr =  hapd->conf->urnm_mfpr;
...
void hostapd_reconfig_wpa(struct hostapd_data *hapd)
{
	...
	if ((hapd->conf->mld_unit != 255) && (hapd->conf->mld_unit != -1) &&
			(hapd->conf->link_id != 255) &&	(hapd->conf->link_id != -1)) {
		wpa_auth_conf.link_count = hapd->mld_link.mld->link_count;
		if (wpa_auth_conf.link_count) {
			wpa_auth_conf.link_info =
				&(hapd->mld_link.mld->link_info);
		}
		if(wpa_auth_conf.link_info) {
			wpa_printf(MSG_DEBUG,"Initializing MLO_LINK values link_info:%p"
				" link_count:%d\n", wpa_auth_conf.link_info,
				wpa_auth_conf.link_count);
		} else {
			wpa_printf(MSG_ERROR,"link_info is NULL \n");
		}
	}
