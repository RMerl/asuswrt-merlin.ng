...
#define DPP_WIFI7_CAPAB_STR	BIT(0)
#define DPP_WIFI7_CAPAB_NSTR	BIT(1)
#define DPP_WIFI7_CAPAB_EMLSR	BIT(2)
#define DPP_WIFI7_CAPAB_EMLMR	BIT(3)

#define DPP_IS_WIFI7_CAPAB_STR(flags)	((flags) & DPP_WIFI7_CAPAB_STR)
#define DPP_IS_WIFI7_CAPAB_NSTR(flags)	((flags) & DPP_WIFI7_CAPAB_NSTR)
#define DPP_IS_WIFI7_CAPAB_EMLSR(flags)	((flags) & DPP_WIFI7_CAPAB_EMLSR)
#define DPP_IS_WIFI7_CAPAB_EMLMR(flags)	((flags) & DPP_WIFI7_CAPAB_EMLMR)

#define DPP_WIFI7_CAPAB_UPDATE(b_cap, all_capab_flags, cap_flag) \
	do { \
		if ((b_cap)) { \
			(all_capab_flags) |= (cap_flag); \
		} else { \
			(all_capab_flags) &= ~(cap_flag); \
		} \
	} while (0)

/* Wi-Fi7 radio specific capabilities */
struct dpp_wifi7_radio_capabilities {
	struct dl_list list;
	u8 flags;			/* Of type DPP_WIFI7_RADIO_CAPAB_XYZ */
	u8 ruid[ETH_ALEN];		/* Affiliated radio unique identifier */
	u32 str_freq_separation;	/* Frequency separation for STR mode */
	u32 nstr_freq_separation;	/* Frequency separation for NSTR mode */
	u32 emlsr_freq_separation;	/* Frequency separation for EMLSR mode */
	u32 emlmr_freq_separation;	/* Frequency separation for EMLMR mode */
};

/* Wi-Fi7 bsta radio specific info */
struct dpp_wifi7_agent_bsta_radio_info {
	struct dl_list list;
	u8 ruid[ETH_ALEN];	/* Radio unique identifier */
	u8 wifi7_caps;		/* Wifi7 capabilities flags of type DPP_WIFI7_RADIO_CAPAB_XYZ */
	struct dl_list aux_radio_list; /* Of type struct dpp_wifi7_radio_capabilities */
};

/* Wi-Fi 7 Agent BSTA Capabilities */
struct dpp_wifi7_agent_bsta_capabilities {
	u8 max_links;				/* Maximum number of links supported by agent */
	u8 num_radio;				/* Number of bsta radio records present */
	struct dl_list wifi7_bsta_radio_list;	/* Of type struct dpp_wifi7_agent_bsta_radio_info */
};
...
#if defined(CONFIG_DRIVER_BRCM_MAP) && defined(CONFIG_DRIVER_BRCM_MLO)
	/* Wi-Fi7 Agent BSTA Capabilities */
	struct dpp_wifi7_agent_bsta_capabilities wifi7_bsta_capab;
#endif /* CONFIG_DRIVER_BRCM_MAP && CONFIG_DRIVER_BRCM_MLO */
};
...
#ifdef CONFIG_DRIVER_BRCM_MAP
	struct dpp_configuration *conf_map;
#endif /* CONFIG_DRIVER_BRCM_MAP */
...
#endif /* CONFIG_TESTING_OPTIONS */
#ifdef CONFIG_DRIVER_BRCM_MAP
	struct wpabuf *map_peer_pub_key;
#endif /* CONFIG_DRIVER_BRCM_MAP */
};
...
#ifdef CONFIG_DRIVER_BRCM_MAP
int dpp_relay_bootstrap_uri(struct dpp_global *dpp, u8 *src, int freq,
		void *cb_ctx, struct wpabuf *msg);
#endif /* CONFIG_DRIVER_BRCM_MAP */
#endif /* CONFIG_DPP */
#endif /* DPP_H */
