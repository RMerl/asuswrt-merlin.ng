...
struct wps_registrar {
	...
	/* To check whether the registrar has ieee1905 tk present or not */
	u8 ieee1905_tk_present;
	/**
	 * new_dpp_bootstrap_uri_cb - Callback for reporting dpp uri received in M7
	 * @ctx: Higher layer context data (cb_ctx)
	 * @uri: DPP BootStrap URI of the Enrollee
	 * @uri_len: Length of DPP URI
	 * @bsta_mac: Enrollee mac address
	 * @own_mac: Device own mac address
	 */
	int (*new_dpp_bootstrap_uri_cb)(void *ctx, u8 *uri, u16 uri_len, u8 *bsta_mac, u8 *own_mac);
					 ...
					 struct wps_parse_attr *attr)
{
	...
	if (!reg->ieee1905_tk_present ||
		attr->dpp_bootstrap_uri_len < 4 /* URI should atleast contain 'DPP:' */) {
		wpa_printf(MSG_INFO, "%s skip processing dpp uri 1905_tk=[%d] "
			"uri_len=[%d] uri[%s]",
			__func__, reg->ieee1905_tk_present, attr->dpp_bootstrap_uri_len,
			attr->dpp_bootstrap_uri ? (char*)attr->dpp_bootstrap_uri : "");
		return 0;
	}

	if (reg->new_dpp_bootstrap_uri_cb) {
		if (!reg->new_dpp_bootstrap_uri_cb(reg->cb_ctx,
			attr->dpp_bootstrap_uri, attr->dpp_bootstrap_uri_len,
			wps->mac_addr_e, wps->wps->dev.mac_addr)) {
			wps->skip_creds_in_m8 = true;
		}
	}

	wpa_printf(MSG_DEBUG, "%s 1905_tk=[%d] uri_len=[%d] uri[%s] skip_creds_in_m8[%d]",
		__func__, reg->ieee1905_tk_present, attr->dpp_bootstrap_uri_len,
		(char*)attr->dpp_bootstrap_uri, wps->skip_creds_in_m8);

	return 0;
}

int wps_registrar_update_ieee1905_tk(struct wps_registrar *reg, int ieee1905_tk)
{
	reg->ieee1905_tk_present = ieee1905_tk;
}
...
#ifdef CONFIG_DRIVER_BRCM_MAP
	reg->new_dpp_bootstrap_uri_cb = cfg->new_dpp_bootstrap_uri_cb;
#endif /* CONFIG_DRIVER_BRCM_MAP */
