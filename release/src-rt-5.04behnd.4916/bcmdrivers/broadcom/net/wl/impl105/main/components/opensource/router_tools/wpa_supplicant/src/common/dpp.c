...
static void dpp_build_freq_separation_array(struct wpabuf *json,
	struct dpp_bsta_obj *main_bsta_obj, struct dpp_bsta_mld_config *bsta_mld_cfg)
{
	int i, valid = 0, jdx, aux_count;
	enum wifi7_modes { STR = 0, NSTR = 1, EMLSR = 2, EMLMR = 3 };
	char *json_arr_str, *ruid_tag, *freq_sep_tag, tmp[32] = {0};
	struct dpp_bsta_obj *aux_bsta_obj;

	for (i = STR; i <= EMLMR; i++) {
		aux_count = 0;
		switch (i) {
			case STR:
				valid = DPP_IS_CAPAB_WIFI7_STR(main_bsta_obj->wifi7_caps);
		...
		if (!valid || (bsta_mld_cfg->link_count <= 1)) {
			continue;
		}

		json_value_sep(json);
		json_start_array(json, json_arr_str); /* FreqSeparation arr start */
		for (jdx = 0; jdx < bsta_mld_cfg->link_count; jdx++) {
			aux_bsta_obj = &bsta_mld_cfg->bsta_objs[jdx];
			if (main_bsta_obj == aux_bsta_obj) {
				continue;
			}
			...
			json_start_object(json, NULL);	/* Aux bsta obj start */
			os_snprintf(tmp, sizeof(tmp), COMPACT_MACSTR,
				MAC2STR(aux_bsta_obj->bsta_addr));
			json_add_string(json, ruid_tag, tmp);
			json_value_sep(json);
			json_add_int(json, freq_sep_tag, 0);
			json_end_object(json);	/* Aux bsta obj end */
			aux_count++;
		}
		json_end_array(json);	/* FreqSeparation arr end */
	}
}
#endif /* CONFIG_DRIVER_BRCM_MLO */

struct wpabuf * dpp_build_conf_req_helper_map(struct dpp_authentication *auth,
					  const char *name,
					  enum dpp_netrole netrole)
{
	size_t len, name_len = 0;
	const char *tech = "map";
	const char *l2_nw_role = "mapBackhaulSta";
	const char *akm = "dpp+psk+sae";
	const char *channel_list = "81/6,115/36,125/149";
	struct wpabuf *buf, *json;
#ifdef CONFIG_DRIVER_BRCM_MLO
	struct dpp_bsta_mld_config bsta_mld_cfg = {0};
	int idx;
	char tmp[32] = {0};

	if (auth->global->get_bsta_mld_config) {
		auth->global->get_bsta_mld_config(auth->global->cb_ctx, &bsta_mld_cfg);
	}
#endif /* CONFIG_DRIVER_BRCM_MLO */
	...
	json_start_object(json, NULL);
	if (name) {
		if (json_add_string_escape(json, "name", name, name_len) < 0) {
			wpabuf_free(json);
			return NULL;
		}
		json_value_sep(json);
	...
	json_add_string(json, "netRole", l2_nw_role);
	json_value_sep(json);

	json_add_string(json, "akm", akm);
...
#ifdef CONFIG_DRIVER_BRCM_MLO
	if (bsta_mld_cfg.link_count) {
		int idx;
		struct dpp_bsta_obj *bsta_obj;

		json_value_sep(json);
		json_start_object(json, "WiFi7AgentBSTACapabilities");
		json_add_int(json, "bSTA_Maximum_Links", bsta_mld_cfg.link_count);
		json_value_sep(json);
		json_start_array(json, "RadioList");
		for (idx = 0; idx < bsta_mld_cfg.link_count; idx++) {
			bsta_obj = &bsta_mld_cfg.bsta_objs[idx];

			if(idx) {
				json_value_sep(json);
			}
			json_start_object(json, NULL);
			os_snprintf(tmp, sizeof(tmp), COMPACT_MACSTR, MAC2STR(bsta_obj->bsta_addr));
			json_add_string(json, "RUID", tmp);
			json_value_sep(json);
			json_add_boolean(json, "bSTA_STR_Support",
				DPP_IS_CAPAB_WIFI7_STR(bsta_obj->wifi7_caps));
			json_value_sep(json);
			json_add_boolean(json, "bSTA_NSTR_Support",
				DPP_IS_CAPAB_WIFI7_NSTR(bsta_obj->wifi7_caps));
			json_value_sep(json);
			json_add_boolean(json, "bSTA_EMLSR_Support",
				DPP_IS_CAPAB_WIFI7_EMLSR(bsta_obj->wifi7_caps));
			json_value_sep(json);
			json_add_boolean(json, "bSTA_EMLMR_Support",
				DPP_IS_CAPAB_WIFI7_EMLMR(bsta_obj->wifi7_caps));
			dpp_build_freq_separation_array(json, bsta_obj, &bsta_mld_cfg);
			json_end_object(json);
		}
		json_end_array(json);	/* RadioList */
		json_end_object(json);	/* WiFi7AgentBSTACapabilities */
	}
#endif /* CONFIG_DRIVER_BRCM_MLO */
			       ...
			       u16 payload_len)
{
	...
	for (token = groups->child; token; token = token->sibling) {
		...
		} else if (os_strcmp(role->string, "mapBackhaulSta") == 0) {
			conf->netrole = DPP_NETROLE_MAP_BH_STA;
		}
#endif /* CONFIG_DRIVER_BRCM_MAP */
		rules++;
	}
			      ...
			      const u8 *conf_obj, u16 conf_obj_len)
{
...
#ifdef CONFIG_DRIVER_BRCM_MAP
	struct json_token *df_thresh;
#ifdef CONFIG_DRIVER_BRCM_MLO
	struct json_token *bsta_mld_obj, *bsta_list, *wifi7_mode;
#endif /* CONFIG_DRIVER_BRCM_MLO */
#endif /* CONFIG_DRIVER_BRCM_MAP */
	struct dpp_config_obj *conf;
	...
	if (conf->netrole == DPP_NETROLE_MAPAGENT) {
		if (conf->akm != DPP_AKM_DPP) {
			dpp_auth_fail(auth, "akm should be dpp, for netrole 'mapAgent'");
			goto fail;
		}
		df_thresh = json_get_member(root, "dfCounterThreshold");
		if (!df_thresh || df_thresh->type != JSON_NUMBER) {
			dpp_auth_fail(auth, "No Decrypt Failure Counter Threshold");
			goto fail;
		}
		wpa_printf(MSG_DEBUG, "DPP: dfCounterThreshold %d", df_thresh->number);
		//TODO: Save it in conf or somewhere else where it can be used
...
#ifdef CONFIG_DRIVER_BRCM_MLO
		bsta_mld_obj = json_get_member(root, "Backhaul_STA_MLD_Config");
		if (bsta_mld_obj && bsta_mld_obj->type == JSON_OBJECT) {
			struct dpp_bsta_mld_config *bsta_mld_cfg;
			struct dpp_bsta_obj *bsta_obj;

			os_memset(&conf->bsta_mld_config, 0, sizeof(*bsta_mld_cfg));
			bsta_mld_cfg = &conf->bsta_mld_config;

			bsta_list = json_get_member(bsta_mld_obj, "AffiliatedSTAList");
			if(!bsta_list || bsta_list->type != JSON_ARRAY) {
				wpa_printf(MSG_INFO, "DPP: JSON AffiliatedSTAList is %s",
					bsta_list ? " not received as array" : "not present");
				dpp_auth_fail(auth, "Unexpected discovery::AffiliatedSTAList data");
				goto fail;
			}
			for (token = bsta_list->child; token; token = token->sibling) {
				...
				if (bsta_mld_cfg->link_count >= DPP_MAX_BSTA_COUNT) {
					wpa_printf(MSG_DEBUG, "DPP: Skip RUID %s as max "
						"supported radio count in device is %d ",
						ruid->string, DPP_MAX_BSTA_COUNT);
					continue;
				}

				bsta_obj = &bsta_mld_cfg->bsta_objs[bsta_mld_cfg->link_count];
				if (!hwaddr_compact_aton(ruid->string, bsta_obj->bsta_addr)) {
					bsta_mld_cfg->link_count++;
				} else {
					wpa_printf(MSG_DEBUG, "DPP: unexpeced RUID %s "
						"format", ruid->string);
					continue;
				}

				wifi7_mode = json_get_member(token, "bSTA_STR_Support");
				if (wifi7_mode && wifi7_mode->type == JSON_BOOLEAN &&
						wifi7_mode->number) {
					bsta_obj->wifi7_caps |= DPP_CAPAB_WIFI7_STR;
				}

				wifi7_mode = json_get_member(token, "bSTA_NSTR_Support");
				if (wifi7_mode && wifi7_mode->type == JSON_BOOLEAN &&
						wifi7_mode->number) {
					bsta_obj->wifi7_caps |= DPP_CAPAB_WIFI7_NSTR;
				}

				wifi7_mode = json_get_member(token, "bSTA_EMLSR_Support");
				if (wifi7_mode && wifi7_mode->type == JSON_BOOLEAN &&
						wifi7_mode->number) {
					bsta_obj->wifi7_caps |= DPP_CAPAB_WIFI7_EMLSR;
				}

				wifi7_mode = json_get_member(token, "bSTA_EMLMR_Support");
				if (wifi7_mode && wifi7_mode->type == JSON_BOOLEAN &&
						wifi7_mode->number) {
					bsta_obj->wifi7_caps |= DPP_CAPAB_WIFI7_EMLMR;
				}
			}
		}
#endif /* CONFIG_DRIVER_BRCM_MLO */
...
struct dpp_global * dpp_global_init(struct dpp_global_config *config)
{
	...
	dpp->get_bsta_mld_config = config->get_bsta_mld_config;
