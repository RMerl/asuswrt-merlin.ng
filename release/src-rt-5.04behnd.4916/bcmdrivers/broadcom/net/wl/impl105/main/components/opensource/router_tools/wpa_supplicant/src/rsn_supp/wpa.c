       ...
       int wnm_sleep, int link_id)
{
	...
	if (wpa_sm_set_key(sm, wpa_cipher_to_alg(sm->mgmt_group_cipher),
		...
		if (keyidx == 0x0400 || keyidx == 0x0500) {
			 ...
			 * valid MLO_IGTK, so do not try to configure this key with
			 * swapped KeyID bytes. Instead, continue without
			 * configuring the MLO_IGTK so that the driver can drop any
...
/**
 * wpa_sm_clear_ap_mld - Clear AP MLD from sm.
 * Returns: 0 on success, -1 on failure
 */
int wpa_sm_clear_ap_mld(struct wpa_sm *sm)
{
	if (sm == NULL)
		return -1;

	/*  Clear ap_mld_addr from sm */
	os_memset(sm->ap_mld_addr, 0, ETH_ALEN);
	sm->mld_present = false;
