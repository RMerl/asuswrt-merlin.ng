...
int wps_build_dpp_bootstrp_uri(struct wps_data *wps, struct wpabuf *msg)
{
	char *uri = NULL;
	u16 len;
	...
	if (wps->wps->dpp_bootstrap_uri_cb)
		uri = wps->wps->dpp_bootstrap_uri_cb(wps->wps->cb_ctx);
	if (!uri)
		return 0;

	wpa_printf(MSG_INFO, "WPS:  * DPP Bootstrap URI %s\n", uri);

	len = os_strlen(uri);
	if (len < 4)	/* URI should atleast contain 'DPP:' */
		return 0;
	wpabuf_put_be16(msg, ATTR_DPP_BOOTSTRAP_URI);
	wpabuf_put_be16(msg, len);
	wpabuf_put_data(msg, uri, len);
	return 0;
}
