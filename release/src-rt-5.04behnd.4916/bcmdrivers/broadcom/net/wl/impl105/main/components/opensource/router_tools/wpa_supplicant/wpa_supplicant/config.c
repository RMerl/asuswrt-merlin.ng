...
static const struct parse_data ssid_fields[] = {
	...
	{ INT_RANGE(ftm_mode, 0, 1) },
	{ INT_RANGE(urnm_mfpr, 0, 2) },
...
#endif /* CONFIG_PASN */
#ifdef CONFIG_DRIVER_BRCM
	{ INT_RANGE(spp_amsdu, 0, 3), 0},
#endif /* CONFIG_DRIVER_BRCM */
#ifdef CONFIG_DRIVER_BRCM_MLO
	{ INT_RANGE(link_id, -1, 3), 0},
	{ INT_RANGE(mld_unit, -1, 3), 0},
	{ INT_RANGE(mld_link_pref, 0, 16), 0},
#endif /* CONFIG_DRIVER_BRCM_MLO */
#ifdef CONFIG_DRIVER_BRCM_MAP
	{ INT(map_4_way_handshake), 0 },
...
#endif /* CONFIG_AP */
#ifdef CONFIG_DRIVER_BRCM_MLO
		if (os_strncmp(pos, "mld_unit", 8) == 0) {
			char *tmp = os_strchr(pos, '=');
			if (tmp == NULL) {
				if (line < 0)
					return -1;
			...
			*tmp++ = '\0';
			config->mld_unit = atoi(tmp);
			wpa_printf(MSG_DEBUG,"%s:%d conf->mld_unit is :%d\n",
				__func__, __LINE__, config->mld_unit);
			return ret;
		}
		if (os_strncmp(pos, "link_id", 7) == 0) {
			char *tmp = os_strchr(pos, '=');
			if (tmp == NULL) {
				if (line < 0)
					return -1;
			...
			*tmp++ = '\0';
			config->link_id = atoi(tmp);
			wpa_printf(MSG_DEBUG,"%s:%d conf->link_id is :%d\n",
				__func__, __LINE__, config->link_id);
			return ret;
		}
#endif /* CONFIG_DRIVER_BRCM_MLO */
