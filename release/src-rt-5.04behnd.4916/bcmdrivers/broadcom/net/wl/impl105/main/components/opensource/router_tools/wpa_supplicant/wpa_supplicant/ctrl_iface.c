	...
	struct wpa_supplicant *wpa_s, char *cmd)
{
	...
	*value++ = '\0';
	...
	if (wpa_s->global->mlo_info && wpa_s->conf->ssid &&
		...
		os_strcmp(name, "bssid") == 0) {
		hwaddr_aton(value, wpa_s->global->bh_bssid);
		wpa_s->global->bh_bssid_set = 1;
		wpa_dbg(wpa_s, MSG_DEBUG, " Updated bh bssid");
	}
			   ...
			   char *reply, int reply_size, int *reply_len)
{
	...
	if (!wpa_s->sched_scanning && !wpa_s->scanning &&
		...
		wpa_s->known_wps_freq = 0;
#ifdef CONFIG_DRIVER_BRCM_MLO
		if(wpa_s->conf->mld_link_pref)
			wpa_s->mld_link_pref = wpa_s->conf->mld_link_pref;
#endif /* CONFIG_DRIVER_BRCM_MLO */
		wpa_supplicant_req_scan(wpa_s, 0, 0);
...
#ifdef CONFIG_DRIVER_BRCM_MLO
struct wpa_supplicant* wpa_get_master_wpa(struct wpa_supplicant *wpa_s) {
	struct wpa_supplicant *master_wpa = NULL;
	struct wpa_link_unit *plink = NULL;

	if (!wpa_s->mld_link.mld) {
		return NULL;
	}

	dl_list_for_each(plink, &(wpa_s->mld_link.mld->link_info),
		struct wpa_link_unit, list) {
		if (plink->link_id == 0) {
			master_wpa = plink->wpa_s;
			break;
		}
	}
	return master_wpa;
}
static int wpa_ctrl_iface_mlo_config(struct wpa_supplicant *wpa_s,
					 char *buf, size_t resp_len)
{
	...
	if (wpa_s->global->mlo_info) {
		struct wpa_supplicant *mwpa_s = NULL;
		struct wpa_mld_unit *pmld = wpa_s->global->mlo_info;
		struct wpa_link_unit *plink = NULL;
		ret = os_snprintf(pos, end - pos, "link_count:%d\n",
				  pmld->link_count);
		if (os_snprintf_error(end - pos, ret))
			return pos - buf;
		pos += ret;
		...
		dl_list_for_each(plink, &(pmld->link_info),
			...
			ret = os_snprintf(pos, end - pos,
					...
					plink->wpa_s->ifname,
					plink->link_id,
					MAC2STR(plink->wpa_s->own_addr));
			if (os_snprintf_error(end - pos, ret))
				return pos - buf;
			pos += ret;
		}
		mwpa_s = wpa_get_master_wpa(wpa_s);
		if (mwpa_s) {
			ret = os_snprintf(pos, end - pos,
				"MASTER ADDRESS:" MACSTR "\n",
				MAC2STR(mwpa_s->own_addr));
			if (os_snprintf_error(end - pos, ret))
				return pos - buf;
			pos += ret;
		}
...
static int wpa_supplicant_ctrl_iface_map_set_own_addr(struct wpa_supplicant *wpa_s,
	const char *txtaddr)
{
	u8 addr[ETH_ALEN];
	...
	wpa_sm_set_param(wpa_s->wpa, WPA_PARAM_MAP_4_WAY_HANDSHAKE,
		wpa_s->conf->map_4_way_handshake);
	wpa_s->auto_reconnect_disabled = 1;

	wpa_s->pairwise_cipher = wpa_s->conf->ssid->pairwise_cipher;
	wpa_sm_set_param(wpa_s->wpa, WPA_PARAM_PAIRWISE, wpa_s->pairwise_cipher);

	wpa_s->group_cipher = wpa_s->conf->ssid->group_cipher;

	wpa_s->key_mgmt = wpa_s->conf->ssid->key_mgmt;
	wpa_sm_set_param(wpa_s->wpa, WPA_PARAM_KEY_MGMT, wpa_s->key_mgmt);

	wpa_s->wpa_proto = wpa_s->conf->ssid->proto;
	wpa_sm_set_param(wpa_s->wpa, WPA_PARAM_PROTO, wpa_s->wpa_proto);

	wpa_sm_set_param(wpa_s->wpa, WPA_PARAM_DPP_PFS, wpa_s->conf->ssid->dpp_pfs);

	return 0;
}

static int wpa_supplicant_ctrl_iface_map_add_dev(struct wpa_supplicant *wpa_s,
	const char *cmd)
{
	int ret = -1, used, is_peer_controller = 0;
	...
	/* Fetch Peer Device AL MAC, who sent DPP Peer Discovery Resp Frame */
	...
	/* Fetch Peer Role: Controller/Agent, who sent DPP Peer Discovery Resp Frame */
	is_peer_controller = atoi(pos);

	wpa_printf(MSG_DEBUG, "Add New MAP Device " MACSTR " based on ctrl_iface "
		   "notification and Peer is %s", MAC2STR(peer_al_mac),
		   is_peer_controller ? "Controller" : "Agent");

	pos++;
	while (*pos == ' ')
		pos++;

	/* Fetch DPP Peer Discovery Resp Frame */
	len = os_strlen(pos);
	if (len == 0) {
		goto end;
	}
	...
	/* Process DPP Peer Discovery Resp Frame */
	wpa_supplicant_cancel_auth_timeout(wpa_s);

	os_memcpy(wpa_s->bssid, peer_al_mac, ETH_ALEN);

	wpa_supplicant_set_state(wpa_s, WPA_ASSOCIATED);

	wpa_sm_notify_assoc(wpa_s->wpa, peer_al_mac);

	if (is_peer_controller) {
		wpa_sm_set_param(wpa_s->wpa, WPA_PARAM_GROUP, wpa_s->group_cipher);
		wpa_sm_set_param(wpa_s->wpa, WPA_PARAM_MAP_4_WAY_HANDSHAKE,
			MAP_4_WAY_HANDSHAKE_CTRL);
	} else {
		wpa_sm_set_param(wpa_s->wpa, WPA_PARAM_GROUP, WPA_CIPHER_GTK_NOT_USED);
		wpa_sm_set_param(wpa_s->wpa, WPA_PARAM_MAP_4_WAY_HANDSHAKE,
			MAP_4_WAY_HANDSHAKE_AGT);
	}

	wpa_sm_set_assoc_wpa_ie_default(wpa_s->wpa, wpa_ie, &wpa_ie_len);

	os_memcpy(wpa_s->dpp_intro_bssid, peer_al_mac, ETH_ALEN);
	wpa_s->dpp_intro_network = wpa_s->conf->ssid;

	/* Process NIP, Process Peer Discovery Resp Frame */
	wpas_dpp_rx_action(wpa_s, peer_al_mac, &peer_disc_frame[1], (peer_disc_framelen - 1), 0);

	wpa_sm_map_set_pmksa(wpa_s->wpa, peer_al_mac);
...
static int wpa_supplicant_ctrl_iface_map_start_nip(struct wpa_supplicant *wpa_s,
	const char *cmd)
{
	int ret = -1;
	u8 peer_al_mac[ETH_ALEN];
	struct wpa_bss *temp_bss;

	wpa_printf(MSG_DEBUG, "CTRL_IFACE MAP_START_NIP %s", cmd);

	/* Fetch Peer Device AL MAC, to whom DPP Peer Discovery Req Frame is to be sent */
	if (hwaddr_aton2(cmd, peer_al_mac) < 0) {
		goto end;
	}

	/* Start NIP, Create Peer Discovery Req Frame and Send to Driver None */
	os_memcpy(wpa_s->bssid, peer_al_mac, ETH_ALEN);

	temp_bss = os_zalloc(sizeof(*temp_bss)); /* Create Dummy bss */
	os_memcpy(temp_bss->bssid, peer_al_mac, ETH_ALEN);

	wpas_dpp_check_connect(wpa_s, wpa_s->conf->ssid, temp_bss);
...
static int wpa_supplicant_ctrl_iface_map_rx_eapol(struct wpa_supplicant *wpa_s, const char *cmd)
{
	int ret = -1, used;
	size_t len;
	u8 src[ETH_ALEN];
					 ...
					 char *buf, size_t *resp_len)
{
	...
	if (os_strcmp(buf, "PING") == 0) {
		...
		if (wpa_supplicant_ctrl_iface_map_set_own_addr(wpa_s, buf + strlen("MAP_SET_OWN_ADDR ")) < 0) {
			reply_len = -1;
		}
	} else if (os_strncmp(buf, "MAP_ADD_DEV ", strlen("MAP_ADD_DEV ")) == 0) {
		if (wpa_supplicant_ctrl_iface_map_add_dev(wpa_s, buf + strlen("MAP_ADD_DEV ")) < 0) {
			reply_len = -1;
		}
	} else if (os_strncmp(buf, "MAP_START_NIP ", strlen("MAP_START_NIP ")) == 0) {
		if (wpa_supplicant_ctrl_iface_map_start_nip(wpa_s, buf + strlen("MAP_START_NIP ")) < 0) {
			reply_len = -1;
		}
	} else if (os_strncmp(buf, "MAP_RX_EAPOL ", strlen("MAP_RX_EAPOL ")) == 0) {
		if (wpa_supplicant_ctrl_iface_map_rx_eapol(wpa_s, buf + strlen("MAP_RX_EAPOL ")) < 0) {
			reply_len = -1;
		}
#endif /* CONFIG_DRIVER_BRCM_MAP */
#ifdef CONFIG_WPS_ER
	...
	} else if (os_strncmp(buf, WPA_CTRL_RSP, os_strlen(WPA_CTRL_RSP)) == 0)
	{
		...
		reply_len = wpa_ctrl_iface_mlo_config(wpa_s, reply, reply_size);
#endif /* CONFIG_DRIVER_BRCM_MLO */
	} else {
