...
void wpas_network_reenabled(void *eloop_ctx, void *timeout_ctx)
{
	...
	if (!wpa_s->disconnected && (wpa_s->conf->ap_scan == 1)) {
		if (wpa_s->wpa_state == WPA_DISCONNECTED) {
			wpa_supplicant_enable_network(wpa_s, NULL);
		} else if (!wpa_s->disconnected && (wpa_s->wpa_state >= WPA_SCANNING) &&
			(wpa_s->wpa_state <WPA_COMPLETED)) {
			eloop_register_timeout(3, 0, wpas_network_reenabled, wpa_s, NULL);
		}
			    ...
			    bool debug_print)
{
	...
	mlo_ie = wpa_bss_get_ie_ext(bss, WLAN_EID_EXT_MULTI_LINK);

	/* Get the BSS received Link Id */
	ie = wpa_bss_get_vendor_ie(bss, WPA_IE_BRCM_LINKID_VENDOR_TYPE);
	rx_linkid = ie ? ie[6] : 0; //dd 05 00 90 4c 9d <rx link_id>

	check_extmlo = mlo_ie && mlo_ie[1];
	if (wpa_s->mld_link_pref && !(wpa_s->mld_link_pref & BIT(rx_linkid))) {
		if (debug_print)
			wpa_dbg(wpa_s, MSG_DEBUG,
				"skip - MLO Non preferred AP (rx_linkid = %d)", rx_linkid);
		return false;
	}
	if (!check_extmlo) {
		struct wpa_channel_info *chan_info = os_malloc(sizeof(struct wpa_channel_info));
		if (chan_info) {
			wpa_drv_channel_info(wpa_s, chan_info);
			if (wpas_freq_to_band(chan_info->frequency) !=
					wpas_freq_to_band(bss->freq)) {
				wpa_dbg(wpa_s, MSG_DEBUG,
					"skip - BAND not matching for Non MLO AP");
				ret = false;
			}
			os_free(chan_info);
			chan_info = NULL;
	...
	if (wpa_s->mld_link_pref) {
		if (!check_extmlo) {
			if (debug_print)
				wpa_dbg(wpa_s, MSG_DEBUG, "   skip - MLO mismatch");
			return false;
		}
	}
#endif /* CONFIG_DRIVER_BRCM_MLO */
...
#ifdef CONFIG_DRIVER_BRCM_MLO
	if (wpa_s->global->mlo_info && wpa_s->conf->ssid &&
		wpa_s->conf->ssid->multi_ap_backhaul_sta &&
		wpa_s->global->bh_bssid_set) {
		if (os_memcmp(bss->bssid, wpa_s->global->bh_bssid, ETH_ALEN) != 0) {
			if (debug_print)
				wpa_dbg(wpa_s, MSG_DEBUG, " skip - MultiAP BH BSSID mismatch bssid:"MACSTR " bh_bssid "MACSTR" ",
					MAC2STR(bss->bssid), MAC2STR(wpa_s->global->bh_bssid));
			return false;
		} else {
			os_memset(wpa_s->global->bh_bssid, 0, ETH_ALEN);
			wpa_s->global->bh_bssid_set = 0;
		}
	}
#endif /* CONFIG_DRIVER_BRCM_MLO */
				     ...
				     int only_first_ssid, int debug_print)
{
	...
	if (bssid_ignore_count) {
		...
		if (wpa_s->conf->ssid && wpa_s->conf->ssid->multi_ap_backhaul_sta) {
			limit = 3;
		}
...
#ifdef CONFIG_DRIVER_BRCM_MLO
		} else if (selected == NULL && wpa_s->mld_link_pref) {
		/* Skipped one scan result. So if there is no MLO AP picked,
		 * do again without any filter
		 * */
		if (wpa_s->mld_link_pref != MLO_AP_NO_LINK_FILTER) {
			wpa_s->mld_link_pref = MLO_AP_NO_LINK_FILTER;
			wpa_dbg(wpa_s, MSG_DEBUG,
				"No APs found - clear ignore Mlo LINK select AP filter");
		} else {
			wpa_s->mld_link_pref = MLO_AP_NO_FILTER;
			wpa_dbg(wpa_s, MSG_DEBUG,
				"No APs found - clear ignore non mlo AP filter");
		}
#endif /* CONFIG_DRIVER_BRCM_MLO */
...
#ifdef CONFIG_DRIVER_BRCM_MAP
		/* Concurrent starting of DPP Authentication Confirmation
		 * with a trigger for new roam scan association with the existing BSSID
		 * is making Enrolle to not sent DPP Authentication Confirmation ACK
		 * is causing the failure to send DPP Configuration Request.
		 * This triggers starting a new DPP Presence Announcment to get
		 * a new DPP Authentication Confirmation.
		 * DPP Authentication Confirmation is set with a timeout of 10 seconds
		 * in wpas_dpp_auth_conf_wait_timeout()
		 * Assocation is set with a timeout of 20 seconds in
		 * wpa_supplicant_req_auth_timeout()
		 * This check avoids DPP Authentication Confirmation ACK failure
		 * to sync channels with Configurator and Enrolle
		 */
		if ((wpa_s->conf->ssid && wpa_s->conf->ssid->multi_ap_backhaul_sta) &&
			(wpa_s->dpp_auth && wpa_s->dpp_auth->waiting_auth_conf &&
			wpa_s->dpp_auth->auth_resp_status == DPP_STATUS_OK)) {
			wpa_msg(wpa_s, MSG_WARNING, "DPP authentication in progress. "
				" Aborting scan join");
			return 0;
		}
#endif /* CONFIG_DRIVER_BRCM_MAP */
...
#ifdef CONFIG_DRIVER_BRCM_MLO
	{
		/* In case of mlo, when wps is acive in any of the link than skip processing
		 * scan results for other supplicant instance.
		 */
		struct wpa_supplicant *wps_wpa_s;

		wps_wpa_s = wpas_get_wps_active_supplicant(wpa_s);
		if (wps_wpa_s != NULL && wps_wpa_s != wpa_s) {
			wpa_dbg(wpa_s, MSG_DEBUG, "wps is active on %s skip network selection "
					"for connect on %s\n", wps_wpa_s->ifname,
					wpa_s->ifname);
			ret = 0;
			goto scan_work_done;
...
#ifdef CONFIG_DRIVER_BRCM_MAP
#ifdef CONFIG_DPP2
			/* Check to ensure DPP chirp scan scheduled starts, skip
			 * starting the periodic scan request for NO SSID found case
			 */
			if (wpa_s->conf->ssid && wpa_s->conf->ssid->multi_ap_backhaul_sta &&
				wpa_s->dpp_chirp_bi && wpa_s->dpp_presence_announcement) {
				wpa_dbg(wpa_s, MSG_WARNING,
					"DPP : Already chirp scan scheduled skip new scan request");
				return 0;
			}
#endif /* CONFIG_DPP2 */
#endif /* CONFIG_DRIVER_BRCM_MAP */
					     ...
					     union wpa_event_data *data)
{
	...
	int res;
#ifdef CONFIG_DRIVER_BRCM_MLO
	struct wpa_supplicant *temp_wpa_s;
#endif // endif
...
#ifdef CONFIG_DRIVER_BRCM_MLO
	/* For valid wps active supplicant case if it did not find any suitable network where wps
	 * is active than before returning, request another scan using main wpa_s instance.
	 */
	temp_wpa_s = wpas_get_wps_active_supplicant(wpa_s);
	if (temp_wpa_s != NULL) {
		res = _wpa_supplicant_event_scan_results(temp_wpa_s, data, 0, res > 0);
		if (res == 1) {
			return 0;
		} else {
			wpa_dbg(wpa_s, MSG_DEBUG, "Since wps is active, continuing a scan request");
			wpa_s->scan_req = temp_wpa_s->scan_req;
	...
	/*
	 * In MLO environment, If the master STA has not picked any suitable network,
	 * loop through the other sta interfaces to find the matching network
	 */
	if (wpa_s->global->mlo_info) {
		for (temp_wpa_s = wpa_s->global->ifaces; temp_wpa_s;
					temp_wpa_s = temp_wpa_s->next) {
			if (temp_wpa_s->wpa_state >= WPA_ASSOCIATING &&
				!(wpa_s->conf->ssid && wpa_s->conf->ssid->multi_ap_backhaul_sta &&
				wpa_s->global->bh_bssid_set)) {
				continue;
			}
			res = _wpa_supplicant_event_scan_results(temp_wpa_s, data, 0,
				res > 0);

			/* If connect cmd sent successfully than return else try next wpa_s */
			if (res == 1) {
				return 0;
			}
		}
...
#else
	if ((ret = wpa_drv_set_4addr_mode(wpa_s, 1)) < 0) {
		/* If the ret is EBUSY , don't make it fail.
		 * WBD already enabled the bridge interface
		 */
		if (ret != -EBUSY) {
			wpa_printf(MSG_ERROR, "Failed to set 4addr mode");
			goto fail;
		}
		wpa_printf(MSG_ERROR, "Interface already added to the bridge");
	}
#endif /* CONFIG_DRIVER_BRCM_MAP */
...
#ifdef CONFIG_DRIVER_BRCM_MLO
			if (req_elems.multi_link, req_elems.multi_link_len) {
			       wpa_sm_set_has_ML(wpa_s->wpa);
			       /* save the multi_link ie for further use */
			       wpa_sm_save_multi_link(wpa_s->wpa,
					req_elems.multi_link,
					req_elems.multi_link_len);
			}
		}
	} else if (data->assoc_info.req_ies) {
...
#ifdef CONFIG_DRIVER_BRCM_MLO
			/* In case of mlo configuration, wps can be started in aux link so for
			 * failure case issue scan in main link (ap_scan is 1 only in main wpa_s).
			 */
			struct wpa_supplicant *main_wpa_s = NULL;
			struct wpa_supplicant *wps_wpa_s = wpas_get_wps_active_supplicant(wpa_s);
			if (wps_wpa_s == wpa_s) {
				if (wpa_s->global->mlo_info) {
					main_wpa_s = wpa_get_master_wpa(wpa_s);
				}
			}

			if (main_wpa_s) {
				wpa_supplicant_req_scan(main_wpa_s, 1, 0);
			} else if (wps_wpa_s == NULL)
#endif /* CONFIG_DRIVER_BRCM_MLO */
			  ...
			  union wpa_event_data *data)
{
	...
	case EVENT_SIGNAL_CHANGE:
		...
		if (!data->signal_change.above_threshold) {
			const char var[] = "simple";

#ifdef CONFIG_DRIVER_BRCM_MAP
			/* For SmartMesh, it creates two network blocks, one with BSSID and priority
			 * and another without it. So, we have to disable the network block
			 * with BSSID so that, supplicant can select any other network with
			 * good RSSI. Do this only if there are one or more network block without
			 * BSSID.
			 */
			if (wpa_s->conf->ssid && wpa_s->conf->ssid->multi_ap_backhaul_sta) {
				int count = 0;
				...
				if (count >= 1) {
					for (ssid = wpa_s->conf->ssid; ssid; ssid = ssid->next) {
						...
						wpa_dbg(wpa_s, MSG_DEBUG, "%s: Disabling the "
							"network block with BSSID "MACSTR"",
							wpa_s->ifname, MAC2STR(ssid->bssid));
						ssid->disabled = 1;
					}
				}
			}
#endif /* CONFIG_DRIVER_BRCM_MAP */
			wpa_s->current_ssid->bgscan = var;
			wpa_supplicant_reset_bgscan(wpa_s);
		} else {
#ifdef CONFIG_DRIVER_BRCM_MAP
			/* If the RSSI is good, enable the previously disabled network blocks */
			if (wpa_s->conf->ssid && wpa_s->conf->ssid->multi_ap_backhaul_sta) {
				struct wpa_ssid *ssid;
