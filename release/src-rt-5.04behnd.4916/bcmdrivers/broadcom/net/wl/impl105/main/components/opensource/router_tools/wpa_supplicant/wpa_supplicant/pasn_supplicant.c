					 ...
					 int cipher, int akmp)
{
	...
	wpa_printf(MSG_DEBUG, "PASN: " MACSTR " present in PTKSA cache",
		   MAC2STR(peer_addr));
#ifdef CONFIG_DRIVER_BRCM
	brcm_set_secure_ranging_ctx(wpa_s, own_addr, peer_addr, cipher,
		entry->ptk.tk_len, entry->ptk.tk,
		entry->ptk.ltf_keyseed_len, entry->ptk.ltf_keyseed,
		entry->ptk.kdk_len, entry->ptk.kdk);
#else
...
static void wpas_pasn_auth_start_cb(struct wpa_radio_work *work, int deinit)
{
	...
	} else if (wpa_s->conf->ssid->urnm_mfpr == 1)
#endif /* CONFIG_DRIVER_BRCM */
