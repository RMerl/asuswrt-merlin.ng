...
#ifdef CONFIG_DPP
	if (wpa_s->dpp_auth && (wpa_s->dpp_auth->conf_resp_status != DPP_STATUS_OK)) {
		wpa_printf(MSG_INFO, "DPP Authentication in progress - Reschedule Scan");
		wpa_scan_free_params(params);
		work->ctx = NULL;
		radio_work_done(work);
...
static void wpa_supplicant_scan(void *eloop_ctx, void *timeout_ctx)
{
	...
	if (ssid && max_ssids == 1) {
		...
		if((wpa_s->conf->ssid && wpa_s->conf->ssid->multi_ap_backhaul_sta) &&
			(params.num_ssids != 0)
#ifdef CONFIG_DRIVER_BRCM_MLO
			&& (wpas_get_wps_active_supplicant(wpa_s) == NULL)
#endif /* CONFIG_DRIVER_BRCM_MLO */
			)
		{
			...
			if (!((wpa_s->dpp_netrole == DPP_NETROLE_MAPAGENT) &&
				(wpa_s->dpp_chirp_bi || wpa_s->dpp_reconfig_ssid)))
#endif // endif
				goto ssid_list_set;
		}
#endif	/* CONFIG_DRIVER_BRCM_MAP */
				...
				struct scan_info *info, int new_scan)
{
...
#ifdef CONFIG_DRIVER_BRCM_MLO
	struct wpa_supplicant *master_wpa_s = NULL;
	...
	if (wpa_s->global->mlo_info) {
		dl_list_for_each(plink, &(pmld->link_info),
			...
			master_wpa_s = wpa_get_master_wpa(wpa_s);
			if (master_wpa_s)
				break;
		}
	}
	if (master_wpa_s) {
		scan_res = wpa_drv_get_scan_results2(master_wpa_s);
	} else
#endif // endif
