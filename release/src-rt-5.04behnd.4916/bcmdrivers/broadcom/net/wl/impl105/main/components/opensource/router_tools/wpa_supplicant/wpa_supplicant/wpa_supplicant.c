...
void
wpa_mlo_remove(struct wpa_supplicant *wpa_s)
{
	struct wpa_link_unit *plink = NULL;
	struct wpa_mld_unit *pmld = NULL;
	int link_match;
	if (!wpa_s || wpa_s->global) {
		wpa_printf(MSG_ERROR, "NULL reference ");
		return;
	}
	if (wpa_s->global->mlo_info) {
		pmld = wpa_s->global->mlo_info;
		link_match = 0;
		dl_list_for_each(plink, &(pmld->link_info),
				struct wpa_link_unit, list) {
			if (plink->link_id == wpa_s->mld_link.link->link_id) {
				link_match = 1;
				break;
			}
		}
		if (!link_match) {
			wpa_printf(MSG_DEBUG, "NO LINK for %s",
					wpa_s->ifname);
			return;
		}
		/* remove this link */
		wpa_printf(MSG_DEBUG, "Deleting Link for %s",
				wpa_s->ifname);
		wpa_s->mld_link.mld = NULL;
		wpa_s->mld_link.link = NULL;
		dl_list_del(&plink->list);
		plink->wpa_s = NULL;
		os_free(plink);
		pmld->link_count--;

		if (!pmld->link_count) {
			/* remove mld after last link delete */
			wpa_printf(MSG_DEBUG, "Deleting mld for %s",
					wpa_s->ifname);
			os_free(pmld);
		}
	}
...
int
wpa_mlo_add(struct wpa_supplicant *wpa_s, int mld_id, int link_id)
{
	int link_add = 1; /* add after */
	struct wpa_mld_unit *pmld = NULL;
	struct wpa_link_unit *plink = NULL;
	if (!wpa_s || !wpa_s->global) {
		wpa_printf(MSG_ERROR, "NULL reference to mld_unit:%d, link_id:%d",
				mld_id, link_id);
		return 0;
	}
	if (wpa_s->global->mlo_info == NULL) {
		/* add mlo struct if not present */
		pmld = os_zalloc(sizeof(*pmld));
		if (!pmld) {
			wpa_printf(MSG_ERROR, "MLO memalloc failed");
			return -1;
		}
		wpa_s->global->mlo_info = pmld;
		pmld->mld_id = mld_id;
		dl_list_init(&(pmld->link_info));
		pmld->link_count = 0;
	} else {
		pmld = wpa_s->global->mlo_info;
	}
	wpa_s->mld_link.mld = pmld;

	dl_list_for_each(plink, &(pmld->link_info), struct wpa_link_unit, list) {
		if (plink->link_id == link_id) {
			link_add = 0;
			break;
		} else if (plink->link_id > link_id) {
			break;
		}
	}
	if (link_add) {
		/* add link struct if not present */
		struct wpa_link_unit *llink = os_zalloc(sizeof(*llink));
		if (llink == NULL) {
			wpa_printf(MSG_ERROR, "LINK memalloc failed");
			return -1;
		}
		pmld->link_count++;
		dl_list_init(&(llink->list));
		llink->link_id = link_id;
		llink->wpa_s = wpa_s;
		dl_list_add_tail(&(plink->list), (&llink->list));
		plink = llink;
	}
	wpa_s->mld_link.link = plink;
	return 0;
}
...
#ifdef CONFIG_DRIVER_BRCM_MLO
	{
		/* In case wps authentication times out than schedule scan in the main wpa_s
		 * instance as wps can be issued in aux link also.
		 */
		struct wpa_supplicant *main_wpa_s = NULL;
		struct wpa_supplicant *wps_wpa_s = wpas_get_wps_active_supplicant(wpa_s);

		if (wps_wpa_s && wpa_s->global->mlo_info) {
			main_wpa_s = wpa_get_master_wpa(wpa_s);
		}

		wpa_supplicant_req_scan(main_wpa_s ? main_wpa_s : wpa_s, 1, 0);
	}
#else	/* CONFIG_DRIVER_BRCM_MLO */
	/*
				     ...
				     int sec, int usec)
{
	if (wpa_s->conf->ap_scan == 0 &&
		...
		return;
#ifdef CONFIG_DRIVER_BRCM_MLO
	/*TBD: Need to remove after testing */
	sec = 20;
#endif /* CONFIG_DRIVER_BRCM_MLO */
...
void set_wpas_nvram(struct wpa_supplicant *wpa_s, char *reason)
{
	char word[100], *next;
	...
	foreach (word, wl_ifnames, next) {
		if(strncmp(wpa_s->ifname, word, strlen(wpa_s->ifname)) == 0)
			break;
		unit++;
	}
	snprintf(prefix, sizeof(prefix), "wpas%d_", unit);

	nvram_set(strcat_r(prefix, "reason", tmp), reason);
}
			      ...
			      enum wpa_states state)
{
	...
	if (state == WPA_COMPLETED && wpa_s->new_connection) {
		...
		set_wpas_nvram(wpa_s, "completed");
...
#ifdef CONFIG_DRIVER_BRCM_MLO
	/* In MLO environment, set the state on other sta interfaces */
	if (wpa_s->global->mlo_info) {
			      ...
			      u8 *wpa_ie, size_t *wpa_ie_len)
{
...
#ifdef CONFIG_DRIVER_BRCM_MLO
	if (bss) {
		if (wpa_sm_save_active_link_bssid(wpa_s->wpa, bss->bssid) < 0)
			return -1;
	}
#endif /* CONFIG_DRIVER_BRCM_MLO */
	...
	if (strstr(wpa_s->ifname, "wds") != NULL) {
		wpa_s->group_cipher = WPA_CIPHER_GTK_NOT_USED;
		wpa_sm_set_param(wpa_s->wpa, WPA_PARAM_GROUP, wpa_s->group_cipher);
	}
	else
...
#ifdef CONFIG_DRIVER_BRCM
	spp_amsdu = wpa_s->conf->spp_amsdu;
	wpa_sm_set_param(wpa_s->wpa, WPA_PARAM_SPP_AMSDU, spp_amsdu);
#endif /* CONFIG_DRIVER_BRCM */
				     ...
				     const struct wpa_interface *iface)
{
...
#endif /* CONFIG_MATCH_IFACE */
#ifdef CONFIG_DRIVER_BRCM_MLO
	/* initialize the mlo here before running
	 * the complete initialization
	 */
	if ((wpa_s->conf->mld_unit != -1) && (wpa_s->conf->link_id != -1)) {
		wpa_mlo_add(wpa_s, wpa_s->conf->mld_unit, wpa_s->conf->link_id);
	}
#endif /* CONFIG_DRIVER_BRCM_MLO */
	...
	if (wpa_supplicant_init_wpa(wpa_s) < 0)
		return -1;
#ifdef CONFIG_DRIVER_BRCM_MLO
	wpa_sm_set_mlo_link(wpa_s->wpa, wpa_s->conf->link_id);
	wpa_sm_set_mlo_mld(wpa_s->wpa, wpa_s->conf->mld_unit);
#endif /* CONFIG_DRIVER_BRCM_MLO */
...
#ifdef CONFIG_DRIVER_BRCM
	if (wpa_s->conf->ssid) {
		if (wpa_s->conf->ssid->ftm_mode) {
			wpa_s->drv_flags2 |= WPA_DRIVER_FLAGS2_SEC_LTF_STA;
		}
		if (wpa_s->conf->ssid->urnm_mfpr == 1) {
			wpa_s->drv_flags2 |= WPA_DRIVER_FLAGS2_PROT_RANGE_NEG_STA;
		}
		if (wpa_s->conf->ssid->urnm_mfpr == 2) {
			wpa_s->drv_flags2 |= WPA_DRIVER_FLAGS2_PROT_RANGE_ALLBW_NEG_STA;
		}
	}
#endif /* CONFIG_DRIVER_BRCM */
...
#endif /* CONFIG_FST */
#ifdef CONFIG_DRIVER_BRCM_MLO
	/* Deinit the mlo here before
	 * the complete de-initialization
	 */
	wpa_mlo_remove(wpa_s);
#endif /* CONFIG_DRIVER_BRCM_MLO */
...
#ifdef CONFIG_DRIVER_BRCM_MAP
int wpa_supplicant_driver_none_get_bssid(void *ctx, u8 *bssid)
{
	struct wpa_supplicant *wpa_s = ctx;

	wpa_sm_get_driver_none_bssid(wpa_s->wpa, bssid);
}

int wpa_supplicant_driver_none_get_ssid(void *ctx, u8 *ssid)
{
	struct wpa_supplicant *wpa_s = ctx;

	os_memcpy(ssid, wpa_s->conf->ssid->ssid, wpa_s->conf->ssid->ssid_len);

	return wpa_s->conf->ssid->ssid_len;
}

int wpa_supplicant_driver_none_send_general_msg_on_ctrl_socket(void *ctx, const u8 *addr,
	const char *cmd, const u8 *data, size_t data_len)
{
	struct wpa_supplicant *wpa_s = ctx;
	...
	wpa_printf(MSG_DEBUG, "%s Len %zu for MAP Device " MACSTR, cmd, data_len, MAC2STR(addr));
	wpa_snprintf_hex(hex, hex_len, data, data_len);
	wpa_msg(wpa_s, MSG_INFO, "%s " MACSTR " %s", cmd, MAC2STR(addr), hex);
	os_free(hex);
