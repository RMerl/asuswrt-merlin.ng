				   ...
				   const struct wps_credential *cred)
{
	...
	case WPS_AUTH_WPA2PSK:
		...
		if (add_sae && cred->key_len != 2 * PMK_LEN) {
			...
			ssid->auth_alg = WPA_AUTH_ALG_NONE;
#endif // endif
		}
		ssid->proto = WPA_PROTO_RSN;
		break;
...
static char* wpa_supplicant_dpp_bootstrap_uri(void *ctx)
{
#ifdef CONFIG_DPP
	struct wpa_supplicant *wpa_s = ctx;
	return dpp_bootstrap_get_uri(wpa_s->dpp, 1);
#endif /* CONFIG_DPP */
			     ...
			     int freq)
{
	...
	if (main_wpa_s != NULL) {
		wpa_printf(MSG_DEBUG,
			"WPS: wps is active on %s but main link is %s hence starting scan on %s",
			wpa_s->ifname, main_wpa_s->ifname, main_wpa_s->ifname);
		if (main_wpa_s->current_ssid) {
			main_wpa_s->own_disconnect_req = 1;
			wpa_supplicant_deauthenticate(main_wpa_s, WLAN_REASON_DEAUTH_LEAVING);
		}
		main_wpa_s->disconnected = 0;
		main_wpa_s->reassociate = 1;
		main_wpa_s->normal_scans = 0;
		main_wpa_s->scan_req = wpa_s->scan_req;
		wpa_supplicant_cancel_sched_scan(main_wpa_s);
		wpa_supplicant_req_scan(main_wpa_s, 0, 0);
	} else {
#endif /* CONFIG_DRIVER_BRCM_MLO */
...
#ifdef CONFIG_DRIVER_BRCM_MAP
	wps->dpp_bootstrap_uri_cb = wpa_supplicant_dpp_bootstrap_uri;
#endif /* CONFIG_DRIVER_BRCM_MAP */
	wps->cb_ctx = wpa_s;
...
#ifdef CONFIG_DRIVER_BRCM_MLO
/* When mlo is active retruns the supplicant instance where wps is active otherwise NULL */
struct wpa_supplicant* wpas_get_wps_active_supplicant(struct wpa_supplicant *wpa_s)
{
	...
	for (temp_wpa_s = wpa_s->global->ifaces; temp_wpa_s; temp_wpa_s = temp_wpa_s->next) {
		if (wpas_wps_searching(temp_wpa_s) && !temp_wpa_s->wps_success) {
			return temp_wpa_s;
		}
	}
