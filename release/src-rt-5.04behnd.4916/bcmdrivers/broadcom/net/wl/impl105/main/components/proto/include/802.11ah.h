 ...
 *
 * $Id: 802.11ah.h 822350 2023-03-07 10:02:43Z $
 */
...
/* Control field (figure 9-687) 8 bits */
...
#define TWT_CTRL_WAKE_DURATION_256usec	0		/* Wake duration unit bit 5, not set */
#define TWT_CTRL_WAKE_DURATION_1msec	(1 << 5)	/* indicates 256 usec else 1 msec */
#define TWT_CTRL_LINK_ID_PRESENT	(1 << 6)	/* 11BE: Link ID Bitmap Present */

/* Control Negotiation Type subfield (table 9-298a) */
#define TWT_CTRL_NEGO_INDV_REQ_RSP	0		/* Negotiation 0 Individual Req/Rsp */
#define TWT_CTRL_NEGO_INDV_TBTT		1		/* Negotiation 1 Individual TBTT */
...
/* Individual TWT parameter Set - figure 9-687a */
...
/* Broadcast TWT parameter Set - figure 9-687b */
...
/* Request Type field (figure 9-688 and 9-688a) bit position and field width */
...
/* Setup Command field (table 9-299) - Values, store in TWT_REQ_TYPE_SETUP_CMD */
...
/* Broadcast TWT Info subfield (figure 9-682a) bit position and field width */
#define TWT_BCAST_INFO_RESERVED_IDX	0	/* Reserved */
#define TWT_BCAST_INFO_RESERVED_FSZ	3
...
/* Link ID Bitmap */
typedef uint16 twt_link_id_bitmap_t;
...
#define TWT_S1G_AF_HDR_SZ		S1G_AF_TWT_SETUP_TWT_IE_OFF
#define TWT_TOP_IE_SZ			(sizeof(twt_ie_top_t))
#define TWT_BCAST_IE_SZ			(sizeof(twt_ie_bcast_t))
#define TWT_INDV_IE_SZ			(sizeof(twt_ie_indv_t))
#define TWT_LINK_ID_BITMAP_SZ		(sizeof(twt_link_id_bitmap_t))
