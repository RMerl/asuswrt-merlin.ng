 ...
 *
 * $Id: bcmmsgbuf.h 842170 2024-07-08 19:31:33Z $
 */
...
#endif /* !STBLINUX */
#define H2DRING_CTRL_SUB_MAX_ITEM		512
#define D2HRING_TXCMPLT_MAX_ITEM		1024
#if defined(STBLINUX)
#define D2HRING_RXCMPLT_MAX_ITEM		512
#else /* !STBLINUX */
#define D2HRING_RXCMPLT_MAX_ITEM		2048
#endif /* !STBLINUX */
#define D2HRING_RXCMPLT_BA256_MAX_ITEM		4096

#define H2DRING_DYNAMIC_INFO_MAX_ITEM          32
#define D2HRING_DYNAMIC_INFO_MAX_ITEM          32

#ifdef BCM_ROUTER_DHD
#define D2HRING_CTRL_CMPLT_MAX_ITEM		512
#else /* !BCM_ROUTER_DHD */
#define D2HRING_CTRL_CMPLT_MAX_ITEM		64
#endif /* BCM_ROUTER_DHD */

#define H2DRING_PCAP_SUB_MAX_ITEM		1024
#define D2HRING_PCAP_CPLN_MAX_ITEM		2048
...
#define HMOEVENT_MAGIC	0x54564548	/* "HEVT" */

typedef struct hmoevent_msg_hdr {
	/* message type */
	uint8 msg_type;
	/* interface index this is valid for */
	uint8 if_id;
...
typedef enum bcmpcie_msgtype {
	...
	MSG_TYPE_PCAP_POST		= 0x2F,
	MSG_TYPE_PCAP_CMPLT		= 0x30,
	MSG_TYPE_PCAP_D11PKT_REQ	= 0x31,
	MSG_TYPE_PCAP_D11PKT_RESP	= 0x32,
...
/*
 * In pcap mode, management packets - along with the promiscuous packets - are
 * sent up directly to DDR and need to be sent back to the dongle to be
 * processed properly. Use these request/response message types for that cause.
 */
typedef struct pcap_d11pkt_req {
	cmn_msg_hdr_t   msg;		/* common message header */
	haddr64_t data_buf_haddr64;	/* location of the packet data */
	uint32 host_pktid;		/* use this id to tell dhd to free pkt */
	uint32 pktlen;			/* length of the d11 pkt with ucode+hw+phy hdrs */
	uint32 reserved[4];
} pcap_d11pkt_req_t;

typedef struct pcap_d11pkt_resp {
	cmn_msg_hdr_t msg;		/* common message header */
	compl_msg_hdr_t cmplt;		/* completion message header */
	uint32 host_pktid;		/* use this id to tell dhd to free pkt */
	uint32 rsvd[1];
	/* XOR checksum or a magic number to audit DMA done */
	dma_done_t marker;
...
typedef union ctrl_completion_item {
	...
	pcap_d11pkt_resp_t		pcap_d11pkt_resp;
...
#define MAX_REG_PKT_SIZE		(4*1024 - 1)
#define	PKT_REG_DATA_LEN_BITS		12
#define	PKT_JUMBO_DATA_LEN_BITS		14
#define	PKT_PRIO_BITS			3
...
typedef union hwa_rxcple_cwi
{
	...
	struct {
		...
		/* MSB of the following field is jumbo frame flag
		 * if b15 is set then b0-13 is frame length. It is a jumbo frame.
		 * if b15 is unset then b0-11 is frame length and b12-14 is prio/TID
		 */
		uint16 data_len;
...
typedef union hwa_rxcple_acwi
{
	...
	struct {
		...
		/* MSB of the following field is jumbo frame flag
		 * if b15 is set then b0-13 is frame length. It is a jumbo frame.
		 * if b15 is unset. then b0-11 is frame lenght and b12-14 is prio/TID
		 */
		uint16 data_len;
		uint32 host_pktid;                    // host pktid
	} aggr[HWA_AGGR_MAX];
...
#define HWA_RXCPLE_REG_DATA_LEN_SHIFT   16
#define HWA_RXCPLE_REG_DATA_LEN_MASK   \
	 (((1 << PKT_REG_DATA_LEN_BITS)-1) << HWA_RXCPLE_REG_DATA_LEN_SHIFT)
#define HWA_RXCPLE_JUMBO_DATA_LEN_SHIFT   16
#define HWA_RXCPLE_JUMBO_DATA_LEN_MASK   \
	 (((1 << PKT_JUMBO_DATA_LEN_BITS)-1) << HWA_RXCPLE_JUMBO_DATA_LEN_SHIFT)
#define HWA_RXCPLE_PRIO_SHIFT       (HWA_RXCPLE_REG_DATA_LEN_SHIFT + PKT_REG_DATA_LEN_BITS)
#define HWA_RXCPLE_PRIO_MASK        (((1 << PKT_PRIO_BITS)-1) << HWA_RXCPLE_PRIO_SHIFT)
#define HWA_RXCPLE_JUMBO_FRAME_FLAG_SHIFT	31

/* RxCompletion Flags - 3bits */
#define HWA_RXCPLE_FLAGS_FRAME_802_3		BCMPCIE_PKT_FLAGS_FRAME_802_3	// 0x1
#define HWA_RXCPLE_FLAGS_FRAME_802_11		BCMPCIE_PKT_FLAGS_FRAME_802_11	// 0x2
#define HWA_RXCPLE_FLAGS_RCH			0x04

/**
 * Subsection: RCH - RxCple Host info of a PDU in headroom
 * WLAN UMAC (6717) or D11MAC (6726) will DMA the RxCple Host info (bcmpcie_rxcple_rch_t)
 * of a PDU (AMSDU) to the predefined offset in the headroom of first MSDU of an AMSDU.
 * WLAN UMAC after Rxprocessing and AMPDU Reordering will send an RxCple RCH (hwa_rxcple_rch_t) to
 * Host using HWA 2a.
 * Max number of SDU in an RCH is configured using MAX_RCH_SDU_CNT chip specfic tunable and
 * is informed to DHD using PCI IPC (pcie_ipc_t::max_rch_sdu_cnt).
 * If number of MSDUs in an AMSDU exceeds the max_rch_sdu_cnt then WLAN U/LMAC uses daisy-chain of
 * RCHs linked using next_rch_host_pktid. The next_rch_host_pktid of last RCH is terminated by
 * HWA_HOST_PKTID_NULL.
 *
 * The Host driver (DHD) will process the RxCompletion of Rx packets using RCH
 * (bcmpcie_rxcple_rch_t) from headroom of first SDU received in hwa_rxcple_rch_t.
 * HWA 2a will send the RxCompletion only for the first RCH in a chain, DHD will check and
 * process all RCHs in the chain.
 *
 * Host Driver (DHD) will reserve the headroom for RCH in all Rx Buffers posted to RxPost ring and
 * retrieve the headroom in RxCompletion process.
 */

#define HWA_RXCPLE_RCH_WORDS			HWA_RXCPLE_CWI_WORDS
#define HWA_RXCPLE_RCH_BYTES			(HWA_RXCPLE_RCH_WORDS * NBU32)

typedef union hwa_rxcple_rch
{
	uint8   u8[HWA_RXCPLE_RCH_BYTES];	//  8 Bytes
	uint32 u32[HWA_RXCPLE_RCH_WORDS];	//  2 Words

	struct {
		uint8	ifid		:  5;	// rx packet interface id
		uint8	flags		:  3;	// flags - HWA_RXCPLE_FLAGS_RCH
		uint8	PAD;
		/* Priority of the PDU: b12-14 is prio/TID;
		 * Same bit position as in RxCple CWI (hwa_rxcple_cwi_t)
		 * All other bits are unused.
		 */
		uint16	PAD		: 12;
		uint16	prio		: 3;
		uint16	PAD		: 1;
		uint32	rch_host_pktid;		// Host pktid to which RCH is copied
	};

} hwa_rxcple_rch_t;

/** RxCpl Host info of a PDU in headroom */
#define BCMPCIE_RXCPLE_RCH_CMN_SIZE		8
#define BCMPCIE_RXCPLE_RCH_SDU_SIZE		8
#define BCMPCIE_RXCPLE_RCH_SDU_CNT_MAX		8

#define BCMPCIE_RXCPLE_RCH_SIZE_MAX		(BCMPCIE_RXCPLE_RCH_CMN_SIZE + \
	(BCMPCIE_RXCPLE_RCH_SDU_CNT_MAX * BCMPCIE_RXCPLE_RCH_SDU_SIZE))

#define BCMPCIE_RXCPLE_RCH_SIZE(sdu_cnt)	\
	(BCMPCIE_RXCPLE_RCH_CMN_SIZE + (sdu_cnt * BCMPCIE_RXCPLE_RCH_SDU_SIZE))

typedef struct bcmpcie_rxcple_rch {
	struct {
		uint8	sdu_cnt;		// No of SDU's filled
		union {
			uint8	PAD;		// SW: UNUSED
			uint8	prio;		// HW: TID(Optional) - Not Supported yet
		};
		union {
			uint8	PAD;		// HW: UNUSED
			struct {
				uint8	pcap_mpdu:2;	// SW: pcap mpdu flag
				uint8	unused:6;	// SW: UNUSED
			};
		};
		union {
			uint8	PAD;		// SW: UNUSED
			uint8	core_no;	// HW: HWA Rx Core generating the RCH
		};
		uint32	next_rch_host_pktid;	// Host pktid of next RCH for the PDU
	};
	/** Per SDU Host info */
	struct {
		uint16	data_offset;		// Start of packet data
		uint16	data_len;		// Host buffer data length
		uint32	host_pktid;		// Host buffer PKTID
	} sdu[0];				// Based on sdu_cnt
} bcmpcie_rxcple_rch_t;

/* RX Completion (RCH)/pcap_mpdu values (2 bits!) */
#define PCAP_RXCPL_NONE		0	/* Do not copy */
#define	PCAP_RXCPL_FIRST_SDU	1	/* Single MSDU */
#define	PCAP_RXCPL_ALL_SDU	2	/* All SDU's */
	...
	struct {
		...
		union {                                   // +----- word#1
			...
			struct {                              // word#1
				uint16 flow_ring_id;
				uint8 if_id;
				uint8 pcap:3;		// packet capture flags
				uint8 reserved1:5;
			};
		}; // u32[1]
	};
...
/* TX Completion/pcap values (3 bits!) */
#define PCAP_TXCPL_NONE		0	/* Do not copy */
#define	PCAP_TXCPL_MPDU		1	/* Single MPDU */
#define PCAP_TXCPL_AMSDU_FIRST	2	/* AMSDU: First MSDU */
#define PCAP_TXCPL_AMSDU_INTRM	3	/* AMSDU: Intermediate MSDU */
#define	PCAP_TXCPL_AMSDU_LAST	4	/* AMSDU: Last MSDU */

/* Packet capture aggregated work item size */
#define PCAP_AWI_AGGR	8

#define CMN_MSG_HDR_FLAG_TX		1 /* host pkt is TX */
#define CMN_MSG_HDR_FLAG_PROMISC_RX	2 /* host pkt is RX (promisc mode) */
#define CMN_MSG_HDR_FLAG_FLUSH		4 /* host pkt to recycle/flush */
#define CMN_MSG_HDR_FLAG_RX		8 /* host pkt is RX (dongle processed) */
#define CMN_MSG_HDR_FLAG_PENDING	0x80 /* host pkt still in use - do not free */

typedef struct pcap_cpl_awi64
{
	cmn_msg_hdr_t	msg;
	uint32		host_pktid[PCAP_AWI_AGGR]; /* array of host pkt id's */
	dma_done_t	marker;
} pcap_cpl_awi64_t;

typedef struct pcap_post_awi64
{
	cmn_msg_hdr_t msg;
	uint32 host_pktid[PCAP_AWI_AGGR]; /* host packet id's corresponding to data buffers */
	haddr64_t data_buf_haddr64[PCAP_AWI_AGGR]; /* data buffer physical addresses */
} pcap_post_awi64_t;
