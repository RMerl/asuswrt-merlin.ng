 ...
 *
 * $Id: fils.h 824562 2023-04-28 10:40:33Z $
 */
...
#define MLD_PARAM_FIELD_MLD_ID_MASK			0xFF
#define MLD_PARAM_FIELD_MLD_ID_SHIFT			0
#define MLD_PARAM_FIELD_LINK_ID_MASK			0X0F00
#define MLD_PARAM_FIELD_LINK_ID_SHIFT			8
#define MLD_PARAM_FIELD_BSS_PARAM_CHG_COUNT_MASK	0x0FF000
#define MLD_PARAM_FIELD_BSS_PARAM_CHG_COUNT_SHIFT	12

#define MLD_PARAM_FIELD_MLD_ID(field) \
	((field & MLD_PARAM_FIELD_MLD_ID_MASK) >> MLD_PARAM_FIELD_MLD_ID_SHIFT)

#define MLD_PARAM_FIELD_LINK_ID(field) \
	((field & MLD_PARAM_FIELD_LINK_ID_MASK) >> MLD_PARAM_FIELD_LINK_ID_SHIFT)

#define MLD_PARAM_FIELD_BSS_PARAM_CHANGE_COUNT(field) \
	((field & MLD_PARAM_FIELD_BSS_PARAM_CHG_COUNT_MASK) >>\
	 MLD_PARAM_FIELD_BSS_PARAM_CHG_COUNT_SHIFT)
/* 11be D11.0 9.4.2.170 MLD parameters subfield */
typedef BWL_PRE_PACKED_STRUCT struct mld_subfield {
	uint8 mldid;
	uint16 linkid:4;
	uint16 bss_chg:8;
	uint16 all_upd_inc:1;
	uint16 disabled_link_ind:1;
	uint16 pad:2;
} BWL_POST_PACKED_STRUCT mld_subfield_t;
...
typedef BWL_PRE_PACKED_STRUCT struct tbtt_info_field {
	...
	uint8		bss_params;
	int8            tpe_psd;
	mld_subfield_t  mld_info;
} BWL_POST_PACKED_STRUCT tbtt_info_field_t;

#define TBTT_INFO_FIELD_HDR_LEN	(OFFSETOF(tbtt_info_field_t, mld_info))
...
/* NBR_AP TBTT OFFSETfield(1) + MLD(3) 4Bytes */
#define NBR_AP_TBTT_MLD_LEN			4U
...
/* NBR_AP TBTT OFFSETfield(1) + BSSID(6) + BSS(1) + PSD(1) 9BYTES */
#define NBR_AP_TBTT_BSSID_BSS_PSD_LEN		9U

/* NBR_AP TBTT OFFSETfield(1) + BSSID(6) + MLD(3) 10BYTES */
#define NBR_AP_TBTT_BSSID_MLD_LEN		10U

/* NBR_AP TBTT OFFSETfield(1) + BSSID(6) + SHORTSSID(4) 11Bytes */
#define NBR_AP_TBTT_BSSID_SHORT_SSID_LEN	11U

/*  NBR_AP TBTT OFFSETfield(1) + BSSID(6) + SHORTSSID(4) + BSS(1) 12BYTES */
#define NBR_AP_TBTT_BSSID_SHORT_SSID_BSS_LEN	12U

/*  NBR_AP TBTT OFFSETfield(1) + BSSID(6) + SHORTSSID(4) + BSS(1) + PSD(1) 13BYTES */
#define NBR_AP_TBTT_BSSID_SHORT_SSID_BSS_PSD_LEN	13U

/*  NBR_AP TBTT OFFSETfield(1) + BSSID(6) + SHORTSSID(4) + BSS(1) + PSD(1) + MLD(3) 16BYTES */
