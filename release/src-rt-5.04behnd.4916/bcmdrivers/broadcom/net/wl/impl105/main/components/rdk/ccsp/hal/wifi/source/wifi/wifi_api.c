...
#define GET_INPUT_INT_INC_INDEX(argc, avInd, ival) \
	if ((avInd < argc) && (argv[avInd] != NULL)) \
		ival = atoi(argv[avInd]); \
	avInd++;

#define SCAN_INPUT_HEX4_INC_INDEX(argc, avInd, ivalp) \
	if ((avInd < argc) && (argv[avInd] != NULL)) \
		sscanf(argv[avInd], "0x%04hhx", (unsigned char *)(ivalp)); \
	avInd++;

#define GET_INPUT_STRING_INC_INDEX(argc, avInd, strval) \
	if ((avInd < argc) && (argv[avInd] != NULL)) \
		snprintf(strval, sizeof(strval), argv[avInd]); \
	avInd++;
...
extern int wl_map_str2enum_fromTable(char *istr, unsigned int *ivar, char *delim,
...
void printRadioOperatingParameters(int radioIndex, wifi_radio_operationParam_t *operationParamPtr)
{
	unsigned int i;

	printf("Radio %d Operating Parameters\n", radioIndex);
	printf("radio Enable: %d\n", operationParamPtr->enable);
	print_bands(operationParamPtr->band);
	printf("autoChannelEnabled: %d\n", operationParamPtr->autoChannelEnabled);
	printf("op_class: 0x%04x\n", operationParamPtr->op_class);
	printf("channel: %d\n", operationParamPtr->channel);
	printf("numSecondaryChannels: %d\n", operationParamPtr->numSecondaryChannels);
	printf("channelSecondary: ");
	for (i = 0; i < operationParamPtr->numSecondaryChannels; i++) {
		printf("%u ", operationParamPtr->channelSecondary[i]);
	}
	printf("\n");
	print_chBandwidths(operationParamPtr->channelWidth);
	print_80211_variants(operationParamPtr->variant);
	printf("csa_beacon_count: %d\n", operationParamPtr->csa_beacon_count);
	print_countryCode(operationParamPtr->countryCode);
	printf("DCSEnabled: %d\n", operationParamPtr->DCSEnabled);
	printf("dtimPeriod: %d\n", operationParamPtr->dtimPeriod);
	printf("beaconInterval: %d\n", operationParamPtr->beaconInterval);
	printf("operatingClass: %d (0x%04x)\n", operationParamPtr->operatingClass,
		operationParamPtr->operatingClass);
	print_bitrates(operationParamPtr->basicDataTransmitRates);
	printf("Operational ");
	print_bitrates(operationParamPtr->operationalDataTransmitRates);
	printf("fragmentationThreshold: %d\n", operationParamPtr->fragmentationThreshold);
	printf("guardInterval: %d\n", operationParamPtr->guardInterval);
	printf("transmitPower: %d\n", operationParamPtr->transmitPower);
	printf("rtsThreshold: %d\n", operationParamPtr->rtsThreshold);
	printf("factoryResetSsid: %d - na\n", operationParamPtr->factoryResetSsid);
	printf("radioStatsMeasuringRate: %d - na\n", operationParamPtr->radioStatsMeasuringRate);
	printf("radioStatsMeasuringInterval: %d - na\n",
		operationParamPtr->radioStatsMeasuringInterval);
	printf("ctsProtection: %d\n", operationParamPtr->ctsProtection);
	printf("obssCoex: %d\n", operationParamPtr->obssCoex);
	printf("stbcEnable: %d\n", operationParamPtr->stbcEnable);
	printf("greenFieldEnable: %d\n", operationParamPtr->greenFieldEnable);
	printf("userControl: %d - na\n", operationParamPtr->userControl);
	printf("adminControl: %d - na\n", operationParamPtr->adminControl);
	printf("chanUtilThreshold: %d - na\n", operationParamPtr->chanUtilThreshold);
	printf("chanUtilSelfHealEnable: %d - na\n", operationParamPtr->chanUtilSelfHealEnable);
...
(0/1)>" },
	...
	{ "wifi_setRadioOperatingParameters",		2, "<RadioIndex> <RadioEnable(0/1) \
	[<AutoChannelEnabled(0/1)> <Channel> <Bandwidth(1/2/4/8/16)> <OperatingMode(Bits: AX|0|AC|H|N|G|B|A)> \
	<CsaBeaconCount> <countryCode (2char string)> <DCSEnabled(0/1)> <dtimPeriod (unit8)> \
	<beaconInterval (uint16)> <basicDataTransmitRates> <fragmentationThreshold> \
	<guardInterval (Bits: Auto|3200nSec|1600nSec|800nSec|400nSec)> \
	<transmitPower> <rtsThreshold> <factoryResetSsid(0/1)> <ctsProtection(0/1)> \
	<obssCoex(0/1)> <stbcEnable(0/1)> <greenFieldEnable(0/1)>"
	"]" },
	...
	UINT *out_array_size, UCHAR *out_DialogToken)
{
	...
	printf("%s ap %d cnt=%d size=%zd *out_DialogToken=%d\n", __FUNCTION__, apIndex,
...
static int wifi_api_send_msg( wifi_api_info_t *p_apiInfo )
{
	...
	if (sentBytes != sizeof(wifi_api_info_t)) {
		printf("UDP send failed; sendingBytes[%zd], sentBytes[%d]\n",
			sizeof(wifi_api_info_t), sentBytes);
	}
	else {
		int ret = 0;
			...
			if (ret == RETURN_OK) {
				for (i = 0; i < (int)ARRAY_SIZE(ch_stats); i++) {
					...
					if (!s->ch_utilization_busy_tx
						...
						&& !s->ch_noise) continue;
					printf("  ch %3u tx %10llu rx %10llu busy %10llu total %10llu noise %d\n",
						s->ch_number,
		...
		if (argv[argc_idx] != NULL && argc >= (argc_idx + 1)) {
			...
			for (k = 0; k < count; k++) {
				...
				if (candidate_len != strlen(argv[argc_idx])/2) {
					printf("Error: len %d not match hex-string len %zd (divided by 2)\n",
...
print_setApWpsConfiguration_usage:
		...
		if (ret == RETURN_OK) {
			printRadioOperatingParameters(index, &operationParam);
		}
		...
		strncpy(apiInfo.api_name, argv[1], 1024);
		apiInfo.radioIndex = index;
		// printf("wifi_setRadioOperatingParameters index=%d argc=%d\n", index, argc);
		GET_INPUT_INT_INC_INDEX(argc, avInd, operationParam.enable);
		GET_INPUT_INT_INC_INDEX(argc, avInd, operationParam.autoChannelEnabled);
		// op_class - no input
		GET_INPUT_INT_INC_INDEX(argc, avInd, operationParam.channel);
		// numSecondaryChannels, channelSecondary - no input
		GET_INPUT_INT_INC_INDEX(argc, avInd, operationParam.channelWidth);
		SCAN_INPUT_HEX4_INC_INDEX(argc, avInd, &(operationParam.variant));
		GET_INPUT_INT_INC_INDEX(argc, avInd, operationParam.csa_beacon_count);
		GET_INPUT_STRING_INC_INDEX(argc, avInd, ccStr);
		if (wl_map_str2enum_fromTable(ccStr, &countryCode, "",
				countrycode_table) != RETURN_OK) {
			printf("Error bad ccode %s\n", ccStr);
			goto done;
		}
		operationParam.countryCode = countryCode;
		GET_INPUT_INT_INC_INDEX(argc, avInd, operationParam.DCSEnabled);
		GET_INPUT_INT_INC_INDEX(argc, avInd, operationParam.dtimPeriod);
		GET_INPUT_INT_INC_INDEX(argc, avInd, operationParam.beaconInterval);
		// operationParam.operatingClass - no input
		SCAN_INPUT_HEX4_INC_INDEX(argc, avInd, &(operationParam.basicDataTransmitRates));
		GET_INPUT_INT_INC_INDEX(argc, avInd, operationParam.fragmentationThreshold);
		GET_INPUT_INT_INC_INDEX(argc, avInd, operationParam.guardInterval);
		GET_INPUT_INT_INC_INDEX(argc, avInd, operationParam.transmitPower);
		GET_INPUT_INT_INC_INDEX(argc, avInd, operationParam.rtsThreshold);
		GET_INPUT_INT_INC_INDEX(argc, avInd, operationParam.factoryResetSsid); /* not used */
		// radioStatsMeasuringRate, radioStatsMeasuringInterval - no input
		GET_INPUT_INT_INC_INDEX(argc, avInd, operationParam.ctsProtection);
		GET_INPUT_INT_INC_INDEX(argc, avInd, operationParam.obssCoex);
		GET_INPUT_INT_INC_INDEX(argc, avInd, operationParam.stbcEnable);
		GET_INPUT_INT_INC_INDEX(argc, avInd, operationParam.greenFieldEnable);
		// chanUtilThreshold, chanUtilSelfHealEnable - no input
#if WIFI_HAL_VERSION_GE_3_0_3 && defined(WL11BE)
		GET_INPUT_INT_INC_INDEX(argc, avInd, operationParam.puncturingInfo.punct_bitmap);
		GET_INPUT_INT_INC_INDEX(argc, avInd, operationParam.puncturingInfo.punct_acs_threshold);
#endif /* WIFI_HAL_VERSION_GE_3_0_3 && WL11BE */
		printf("Setting Radio %d OperatingParameters as below:\n", index);
		printRadioOperatingParameters(index, &operationParam);
