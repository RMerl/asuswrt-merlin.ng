...
int wl_map_str2enum_fromTable(char *istr, unsigned int *ivar, char *delim,
	struct wifi_enum_to_str_map *aTable);
int wl_map_enum2str_fromTable(unsigned int ival, char *ostr, int ostr_len,
        struct wifi_enum_to_str_map *aTable);
int wl_map_enumBits2str_fromTable(unsigned int ivar, char *ostr, char *delim,
	int ostr_len, struct wifi_enum_to_str_map *aTable);
...
typedef struct chVar_info {
	...
	bool		basicRatesFlag;		/* 1 = OFDMonly, 0 = default */
} chVar_info_t;
	...
	UINT *output_array_size)
{
	...
	*neighbor_ap_array = NULL;
	...
	if (wldm_xbrcm_scan(CMD_GET, radioIndex, NULL, &num, "num_scan_results", NULL) < 0) {
		HAL_WIFI_DBG(("%s: radioIndex %d wldm_xbrcm_scan failed, errno [%d]\n",
	...
	unsigned int pureMode, chVar_info_t *chInfo)
{
	...
	chInfo->basicRatesFlag = 1;	/* OFDMonly */
	...
	defined(_CBR2_PRODUCT_REQ_))
			...
			chInfo->basicRatesFlag = ((pureMode == PMODE_G) || (pureMode == PMODE_N));
#else
				 "g,n";
#endif // endif
	...
	defined(_CBR2_PRODUCT_REQ_))
				...
				chInfo->basicRatesFlag = !(radioIndex == BAND_2G_INDEX);
...
#define	BASIC_RATES_OFDM		"6,12,24"
#define BASIC_RATES_2G_DEFAULT		"1,2,5.5,11"
static
INT wl_setRadioMode(INT radioIndex, CHAR *channelMode, UINT pureMode)
{
	...
	HAL_WIFI_DBG(("%s: channelMode=%s channel=%d bwCapStr=%s extChStr=%s basicRatesFlag=%d\n", __FUNCTION__,
		channelMode, chVars.curr_channel, chVars.channelBWStr, chVars.extensionChStr, chVars.basicRatesFlag));

	if (radioIndex == BAND_2G_INDEX) {
		basic_rates =  (chVars.basicRatesFlag ? BASIC_RATES_OFDM : BASIC_RATES_2G_DEFAULT);

		len = strlen(basic_rates) + 1;
		ret = wldm_Radio_BasicDataTransmitRates(CMD_SET, radioIndex, basic_rates, &len, NULL, NULL);
		if (ret != 0) {
			HAL_WIFI_LOG(("%s: wldm_Radio_BasicRates CMD SET basic rates to %s failed\n",
					__FUNCTION__, basic_rates));
			return RETURN_ERR;
		}
	}
...
wifi_params_mcs_rate_t non_brcm_mcs_rate_tbl[] = {
	/* Standard,	BW,	GI,	NSS,	Rate */
	{"ac",		"20",	1,	1,	86},
	{"ac",		"20",	1,	2,	173},
	{"ac",		"20",	1,	3,	260},
...
static INT get_assoc_dev_max_tx_rx_rate(INT apIndex, unsigned int nss, wifi_associated_dev3_t *dev, unsigned int is_brcm_sta)
{
	...
	if (strcmp(dev->cli_OperatingStandard, "a") == 0) {
		...
		if (is_brcm_sta) {
			...
			tbl_size = sizeof(non_brcm_mcs_rate_tbl) / sizeof(non_brcm_mcs_rate_tbl[0]);
#ifdef WIFI_HAL_VERSION_GE_3_0_1
			dev->cli_activeNumSpatialStreams = nss;
#endif /* WIFI_HAL_VERSION_GE_3_0_1 */
			for (i = 0; i < tbl_size; i++) {
				if (!strncmp(non_brcm_mcs_rate_tbl[i].oper_standard,
					dev->cli_OperatingStandard,
					sizeof(dev->cli_OperatingStandard)) &&
					!strncmp(non_brcm_mcs_rate_tbl[i].bw,
						dev->cli_OperatingChannelBandwidth,
						sizeof(dev->cli_OperatingChannelBandwidth) ) &&
					non_brcm_mcs_rate_tbl[i].gi == gi && non_brcm_mcs_rate_tbl[i].nss == nss) {
					dev->cli_MaxUplinkRate = non_brcm_mcs_rate_tbl[i].rate;
					dev->cli_MaxDownlinkRate = non_brcm_mcs_rate_tbl[i].rate;
					return RETURN_OK;
				}
			}
		}
	...
	wifi_associated_dev3_t **associated_dev_array, UINT *output_array_size)
{
	...
	*output_array_size = len;
	...
	for (i = 0; i < len; ++i) {
		...
		if ((strncmp(parray[i].cli_OperatingStandard, "ac",
			sizeof(parray[i].cli_OperatingStandard)) == 0) &&
			(strncmp(parray[i].cli_OperatingChannelBandwidth, "160",
				sizeof(parray[i].cli_OperatingChannelBandwidth)) != 0)) {
			if (p_wldm_array[i].cli_flags & WL_BRCM_STA) {
				get_assoc_dev_max_tx_rx_rate(apIndex, p_wldm_array[i].cli_Nss, &parray[i], 1);
			} else {
				HAL_WIFI_DBG(("%s NON BRCM STA \n", __FUNCTION__));
				get_assoc_dev_max_tx_rx_rate(apIndex, p_wldm_array[i].cli_Nss, &parray[i], 0);
			}
		}  else {
                        get_assoc_dev_max_tx_rx_rate(apIndex, p_wldm_array[i].cli_Nss, &parray[i], 1);
                }
...
INT wifi_setGuardInterval(INT radioIndex, wifi_guard_interval_t guard_interval)
{
	...
	ret = wldm_xbrcm_radio(CMD_SET_IOCTL, radioIndex, &guard_interval, (uint *)&len, "guardInterval", NULL);
	if (ret != 0) {
...
INT wifi_getGuardInterval(INT radioIndex, wifi_guard_interval_t *guard_intervalp)
{
	...
	ret = wldm_xbrcm_radio(CMD_GET, radioIndex, guard_intervalp, (uint *)&len, "guardInterval", NULL);
	if (ret != 0) {
...
INT wifi_createVAP(wifi_radio_index_t index, wifi_vap_info_map_t *map)
{
	...
	if (validPrimaryStatus) {
		...
		/* If primary BSS is disabled and needs to be enabled, then acs_restart radio */
		if (validPrimaryStatus && !prePrimaryEnabled && pstPrimaryEnabled) {
			/* Run acs_restart command to re-init radio params in acsd2 */
			if (wldm_xbrcm_acs(CMD_SET_IOCTL, index, NULL, NULL, "acs_restart",
				NULL) < 0) {
				HAL_WIFI_ERR(("%s: wldm_xbrcm_acs acs_restart CMD_SET_IOCTL "
					"Failed.\n", __FUNCTION__));
				result = WIFI_HAL_ERROR;
			}
		}
...
/* map single enum value to corresponding string */
int
wl_map_enum2str_fromTable(unsigned int ival, char *ostr, int ostr_len,
	struct wifi_enum_to_str_map *aTable)
{
	struct wifi_enum_to_str_map *aPtr = aTable;

	while (aPtr->str_val != NULL) {
		if (ival == aPtr->enum_val) {
			if (ostr_len > (int)strlen(aPtr->str_val)) {
				strcpy(ostr, aPtr->str_val);
				return RETURN_OK;
			}
			HAL_WIFI_ERR(("%s Error strlen need %d got %d\n",
				__FUNCTION__, (strlen(aPtr->str_val) + 1), ostr_len));
			return RETURN_ERR;
		}
		aPtr++;
	}
...
/* map multiple bits to a delimited string */
int
wl_map_enumBits2str_fromTable(unsigned int ivar, char *ostr, char *delim,
        int ostr_len, struct wifi_enum_to_str_map *aTable)
{
	int alen = 0;
	char aStr[OUTPUT_STRING_LENGTH_64] = {0};
	struct wifi_enum_to_str_map *aPtr = aTable;

	while (aPtr->str_val != NULL) {
		if (aPtr->enum_val & ivar) { // bit set
			alen += snprintf(aStr + alen,
					sizeof(aStr) - alen, "%s%c", aPtr->str_val, *delim);
		}
		aPtr++;
	}
	aStr[alen - 1] = '\0'; /* Remove the last delim */
	if (ostr_len > alen) {
		strncpy(ostr, aStr, ostr_len);
		return RETURN_OK;
	}
	HAL_WIFI_ERR(("%s aStr=%s out buffer too short need %d got %d\n",
			__FUNCTION__, aStr, (alen+1), ostr_len));
	return RETURN_ERR;
}
...
wl_getMLOCapabilities(wifi_multi_link_bands_t *mu_bands) {
	...
	len = sizeof(mlo_cap);
	ret = wldm_xbrcm_11be(CMD_GET_NVRAM, -1, mlo_cap, (uint *)&len, "wl_mlo_config", NULL);
	if (ret < 0) {
...
INT wifi_getHalCapability(wifi_hal_capability_t *cap)
{
	...
	int ret = 0;
#ifdef WIFI_HAL_VERSION_GE_3_0_3
	wifi_multi_link_bands_t mu_bands = WIFI_BAND_NONE;
#endif /* WIFI_HAL_VERSION_GE_3_0_3 */
...
INT wifi_setRadioOperatingParameters(wifi_radio_index_t index,
		wifi_radio_operationParam_t *operationParam)
{
	bool enable, changed, radio_enabled;
	int len, tblSize, i, ret = 0, ival, stbc;
	unsigned int pureMode = PMODE_NONE, tempVariant = 0, txrates, threshold, txpwr, channel;
	char channelMode[OUTPUT_STRING_LENGTH_32] = "11", variant[OUTPUT_STRING_LENGTH_32] = {0},
	     operStd[OUTPUT_STRING_LENGTH_32] = {0}, cbuf[OUTPUT_STRING_LENGTH_16] = {0},
	     ccStr[OUTPUT_STRING_LENGTH_16] = {0}, txratesStr[64] = {0},
	     channelWidthStr[OUTPUT_STRING_LENGTH_16] = {0};
	csa_t csaInfo;
	wifi_radio_operationParam_t getVal, *currParam;
	wifi_countrycode_type_t ccode;
	...
	/* If radio is switched from disabled to enabled, then radio params in ACSD need to be
	 * reinitialized in order to handle autochannel command
	 */
	len = sizeof(radio_enabled);
	if (wldm_Radio_Enable(CMD_GET_NVRAM, index, &radio_enabled, &len, NULL, NULL) != 0) {
		HAL_WIFI_ERR(("%s Fail, wldm_Radio_Enable get failed for index : %d\n",
				__FUNCTION__, index));
		return WIFI_HAL_ERROR;
	}
	HAL_WIFI_DBG(("%s: radioIndex=%d Radio Enabled=%d\n", __FUNCTION__, index, radio_enabled));

	currParam = &getVal;
	ret = wifi_getRadioOperatingParameters(index, currParam);
	if (ret != WIFI_HAL_SUCCESS) {
		HAL_WIFI_ERR(("%s Error wifi_getRadioOperatingParameters %d\n",
				__FUNCTION__, index));
		return WIFI_HAL_ERROR;
	}

	/* compare each field and SET if changed */
	if ((changed = (currParam->enable != operationParam->enable))) {
		len = sizeof(enable);
	...
	HAL_WIFI_LOG(("%s: radioIndex = %d, Enable=%d changed=%d\n", __FUNCTION__, index,
			operationParam->enable, changed));
	...
	/* IEEE80211h Enable from variant */
	changed = ((currParam->variant & WIFI_80211_VARIANT_H) !=
			(operationParam->variant & WIFI_80211_VARIANT_H));
	if (changed) {
		enable = (operationParam->variant & WIFI_80211_VARIANT_H) ? 1 : 0;
		ret = wldm_Radio_IEEE80211hEnabled(CMD_SET, index, &enable, &len, NULL, NULL);
		if (ret < 0) {
			HAL_WIFI_ERR(("%s: Error radio %d wldm_Radio_IEEE80211hEnabled %d ret=%d\n",
				__FUNCTION__, index, enable, ret));
			return WIFI_HAL_ERROR;
		}
		HAL_WIFI_DBG(("%s: radioIndex = %d, IEEE80211hEnabled=%d changed=%d\n",
				__FUNCTION__, index, enable, changed));
	}

	/* RadioMode Param */
	tempVariant = operationParam->variant & OPER_STANDS_MASK;
	changed = ((currParam->variant & OPER_STANDS_MASK) != tempVariant) ||
			(currParam->channelWidth != operationParam->channelWidth);
	...
	for (i = 0; i < tblSize; i++) {
		if (operationParam->channelWidth == wifi_bandwidth_infoTable[i].enum_val) {
			snprintf(channelWidthStr, sizeof(channelWidthStr), "%s",
					wifi_bandwidth_infoTable[i].str_val);
			break;
		}
	}
	if (i == tblSize) {
		HAL_WIFI_ERR(("%s: invalid bandwidth %d\n", __FUNCTION__, operationParam->channelWidth));
		return WIFI_HAL_INVALID_ARGUMENTS;
	}
	...
	if (changed) {
		...
		HAL_WIFI_DBG(("%s: radioIndex = %d, operStd=%s\n", __FUNCTION__, index, operStd));
		...
		for (i = 0; operStdPMode_infoTable[i].operStd != NULL; i++) {
			if (!(strcmp(operStdPMode_infoTable[i].operStd, operStd))) {
				...
				strncat(channelMode, variant,
						sizeof(channelMode) - strlen(channelMode));
				strncat(channelMode, channelWidthStr,
						sizeof(channelMode) - strlen(channelMode));
				pureMode = operStdPMode_infoTable[i].pmodeVal;
				break;
			}
		...
		HAL_WIFI_DBG(("%s: radioIndex = %d, operationParam channelWidth=%d variant=%d "
					"channelMode = %s pureMode=%d operStd %s\n",
					__FUNCTION__, index, operationParam->channelWidth,
					operationParam->variant, channelMode, pureMode, operStd));
	...
	if (!operationParam->autoChannelEnabled) {
		changed = 0;
		if (currParam->channel != operationParam->channel) {
			len = sizeof(operationParam->channel);
			channel = operationParam->channel;
			ret = wldm_Radio_Channel(CMD_SET, index, &channel, &len, 0, 0,
					NULL, NULL);
			if (ret < 0) {
				HAL_WIFI_ERR(("%s: Error radio %d wldm_Radio_Channel SET ret=%d\n",
						__FUNCTION__, index, ret));
				return WIFI_HAL_ERROR;
			}
			changed = 1;
			HAL_WIFI_DBG(("%s: radioIndex = %d channel=%d changed=%d\n",
			__FUNCTION__, index, channel, changed));

		}
		/* Set current channel and bandwidth */
		if (currParam->channelWidth != operationParam->channelWidth) {
			len = sizeof(channelWidthStr);
			ret = wldm_Radio_OperatingChannelBandwidth(CMD_SET, index,
					channelWidthStr, &len, NULL, NULL);
			if (ret < 0) {
				HAL_WIFI_ERR(("%s: wldm_Radio_OperatingChannel CMD_SET"
						"radioIndex=%d to %sMHz failed\n",
						__FUNCTION__, index, channelWidthStr));
				return WIFI_HAL_ERROR;
			}
			changed = 1;
			HAL_WIFI_DBG(("%s: radioIndex = %d bw=%s changed=%d\n",
			__FUNCTION__, index, channelWidthStr, changed));

		}

		/* No csa sent if csa channel/banwidth same as current chanspec */
		if (changed && (operationParam->csa_beacon_count > 0)) {
			/* Send CSA */
			csaInfo.channel = operationParam->channel;
			csaInfo.channel_width = atoi(channelWidthStr);
			csaInfo.csa_beacon_count = operationParam->csa_beacon_count;

			len = sizeof(csaInfo);
			ret = wldm_11h_dfs(CMD_SET_IOCTL, index, &csaInfo, (uint *)&len,
				"csa_bcast", NULL);
			/* Allow wldm_11h_dfs("csa_bcast") ret == -4, i.e., allow csa to chanspec
			 * with bw = bw_cap < requested bw; note that chanspec will have requested
			 * bw after wldm_apply() below.
			 */
			if (ret < 0 && ret != -4) {
				HAL_WIFI_ERR(("%s: Error csa radioIndex=%d\n", __FUNCTION__, index));
				return WIFI_HAL_ERROR;
			}
			/* Wait for CSA count down to allow some clients to reconnect smoothly */
			usleep(operationParam->csa_beacon_count * operationParam->beaconInterval *
				1000);
			HAL_WIFI_DBG(("%s: radioIndex = %d, channel=%d, channelWidth=%d, csa_beacon_count=%d"
					"changed=%d\n", __FUNCTION__, index, operationParam->channel,
					operationParam->channelWidth, operationParam->csa_beacon_count, changed));
		}
	}

	/* countryCode */
	if ((changed = (currParam->countryCode != operationParam->countryCode))) {
		ccode = operationParam->countryCode;
		ret = wl_map_enum2str_fromTable((unsigned int) ccode, ccStr, sizeof(ccStr),
				countrycode_table);
		if (ret < 0) {
			HAL_WIFI_ERR(("%s: Error radio %d ccode=0x%04x\n", __FUNCTION__, ccode));
			return WIFI_HAL_ERROR;
		}
		len = snprintf(cbuf, sizeof(cbuf), "%sI", ccStr) + 1;
		ret = wldm_Radio_RegulatoryDomain(CMD_SET, index, cbuf, &len, NULL, NULL);
		if (ret != WIFI_HAL_SUCCESS) {
			HAL_WIFI_ERR(("%s: Error radio %d wldm_Radio_RegulatoryDomain %s "
						"len %d SET ret = %d\n", __FUNCTION__, index,
						cbuf, len, ret));
			return WIFI_HAL_ERROR;
		}
		HAL_WIFI_DBG(("%s: radioIndex = %d, ccode=0x%04x (%s) changed=%d\n",
					__FUNCTION__, index, ccode, ccStr, changed));
	}
	/* DCSEnabled */
	if ((changed = (currParam->DCSEnabled != operationParam->DCSEnabled))) {
		enable = operationParam->DCSEnabled;
		HAL_WIFI_DBG(("%s: radioIndex = %d, DCSEnabled = %d changed=%d - no action\n",
					__FUNCTION__, index, enable, changed));
	}
	/* dtimPeriod */
	if ((changed = (currParam->dtimPeriod != operationParam->dtimPeriod))) {
		ival = operationParam->dtimPeriod;
		len = sizeof(ival);
		if ((ival < 0) || (ival > 255)) {
			HAL_WIFI_ERR(("%s: Error radio %d ival = %d ret = %d\n",
						__FUNCTION__, index, ival, ret));
			return WIFI_HAL_ERROR;
		}
		ret = wldm_Radio_DTIMPeriod(CMD_SET, index, &ival, &len, NULL, NULL);
		if (ret != WIFI_HAL_SUCCESS) {
			HAL_WIFI_ERR(("%s: Error radio %d wldm_Radio_DTIMPeriod SET ret = %d\n",
						__FUNCTION__, index, ret));
			return WIFI_HAL_ERROR;
		}
		HAL_WIFI_DBG(("%s: radioIndex = %d, dtimPeriod=%d changed=%d\n",
					__FUNCTION__, index, ival, changed));
	}
	/* beaconInterval */
	if ((changed = (currParam->beaconInterval != operationParam->beaconInterval))) {
		ival = operationParam->beaconInterval;
		len = sizeof(ival);
		ret = wldm_Radio_BeaconPeriod(CMD_SET, index, &ival, &len, NULL, NULL);
		if (ret != WIFI_HAL_SUCCESS) {
			HAL_WIFI_ERR(("%s: Error radio %d wldm_Radio_BeaconPeriod SET ret = %d\n",
						__FUNCTION__, index, ret));
			return WIFI_HAL_ERROR;
		}
		HAL_WIFI_DBG(("%s: radioIndex = %d, beaconInterval=%d changed=%d\n",
					__FUNCTION__, index, ival, changed));
	}
	/* operatingClass */
	/* basicDataTransmitRates */
	if ((changed = (currParam->basicDataTransmitRates !=
					operationParam->basicDataTransmitRates))) {
		txrates = operationParam->basicDataTransmitRates;
		ret = wl_map_enumBits2str_fromTable(txrates, txratesStr, " ", sizeof(txratesStr),
				wifi_bitrate_infoTable); /* e.g "6 12 24" for wldm */;
		if (ret < 0) {
			HAL_WIFI_ERR(("%s: Error radio %d txrates=0x%04x\n", __FUNCTION__, txrates));
			return WIFI_HAL_ERROR;
		}
		len = strlen(txratesStr);
		if (len) {
			len += 1;
			ret = wldm_Radio_BasicDataTransmitRates(CMD_SET, index,
					txratesStr, &len, NULL, NULL);
			if (ret < 0) {
				HAL_WIFI_ERR(("%s: Error radio %d wldm_Radio_BasicDataTransmitRates"
						" SET ret = %d\n", __FUNCTION__, index, ret));
				return WIFI_HAL_ERROR;
			}
		}
		HAL_WIFI_DBG(("%s: radioIndex = %d, txrates=0x%04x txratesStr=%s changed=%d\n",
					__FUNCTION__, index, txrates, txratesStr, changed));
	}
	/* operationalDataTransmitRates */
	/* fragmentationThreshold */
	if ((changed = (currParam->fragmentationThreshold !=
					operationParam->fragmentationThreshold))) {
		threshold = operationParam->fragmentationThreshold;
		len = sizeof(threshold);
		ret = wldm_Radio_FragmentationThreshold(CMD_SET, index, &threshold, &len,
				NULL, NULL);
		if (ret < 0) {
			HAL_WIFI_ERR(("%s: Error radio %d wldm_Radio_FragmentationThreshold SET "
					"ret = %d\n", __FUNCTION__, index, ret));
			return WIFI_HAL_ERROR;
		}
		HAL_WIFI_DBG(("%s: radioIndex = %d, fragmentationThreshold=%d changed=%d\n",
				__FUNCTION__, index, threshold, changed));
	}
	/* guardInterval */
	if ((changed = (currParam->guardInterval != operationParam->guardInterval))) {
		guard_interval = operationParam->guardInterval;
		len = sizeof(guard_interval);
		ret = wldm_xbrcm_radio(CMD_SET_IOCTL, index, &guard_interval, (uint *)&len,
				"guardInterval", NULL);
		if (ret < 0) {
			HAL_WIFI_ERR(("%s: Error radio %d guardInterval SET_IOCTL ret = %d\n",
						__FUNCTION__, index, ret));
			return WIFI_HAL_ERROR;
		}
		HAL_WIFI_DBG(("%s: radioIndex = %d, guardInterval=%d changed=%d\n",
					__FUNCTION__, index, guard_interval, changed));
	}
	/* transmitPower */
	if ((changed = (currParam->transmitPower != operationParam->transmitPower))) {
		txpwr = operationParam->transmitPower;
		len = sizeof(txpwr);
		ret = wldm_Radio_TransmitPower(CMD_SET, index, (int *)&txpwr, &len, NULL, NULL);
		if (ret < 0) {
			HAL_WIFI_ERR(("%s: Error radio %d wldm_Radio_TransmitPower SET "
						"ret = %d\n", __FUNCTION__, index, ret));
			return WIFI_HAL_ERROR;
		}
		HAL_WIFI_DBG(("%s: radioIndex = %d, transmitPower=%d changed=%d\n",
					__FUNCTION__, index, txpwr, changed));
	}
	/* rtsThreshold */
	if ((changed = (currParam->rtsThreshold != operationParam->rtsThreshold))) {
		threshold = operationParam->rtsThreshold;
		len = sizeof(threshold);
		ret = wldm_Radio_RTSThreshold(CMD_SET, index, &threshold, &len, NULL, NULL);
		if (ret < 0) {
			HAL_WIFI_ERR(("%s: Error radio %d wldm_Radio_RTSThreshold SET "
						"ret = %d\n", __FUNCTION__, index, ret));
			return WIFI_HAL_ERROR;
		}
		HAL_WIFI_DBG(("%s: radioIndex = %d, rtsThreshold=%d changed=%d\n",
					__FUNCTION__, index, threshold, changed));
	}
	/* factoryResetSsid */
	...
	/* ctsProtection */
	if ((changed = (currParam->ctsProtection != operationParam->ctsProtection))) {
		enable = operationParam->ctsProtection;
		len = sizeof(enable);
		ret = wldm_xbrcm_Radio_CtsProtectionEnable(CMD_SET, index, &enable,
				&len, NULL, NULL);
		if (ret  < 0) {
			HAL_WIFI_ERR(("%s: Error radio %d wldm_xbrcm_Radio_CtsProtectionEnable"
						" SET ret = %d\n", __FUNCTION__, index, ret));
			return WIFI_HAL_ERROR;
		}
		HAL_WIFI_DBG(("%s: radioIndex = %d, ctsProtection=%d changed=%d\n",
					__FUNCTION__, index, enable, changed));
	}
	/* obssCoex */
	if ((changed = (currParam->obssCoex != operationParam->obssCoex))) {
		enable = operationParam->obssCoex;
		len = sizeof(enable);
		ret = wldm_Radio_ObssCoexistenceEnable(CMD_SET_IOCTL | CMD_SET_NVRAM, index,
				&enable, &len, NULL, NULL);
		if (ret  < 0) {
			HAL_WIFI_ERR(("%s: Error radio %d wldm_Radio_ObssCoexistenceEnable"
						" SET_IOCTL ret = %d\n", __FUNCTION__, index, ret));
			return WIFI_HAL_ERROR;
		}
		HAL_WIFI_DBG(("%s: radioIndex = %d, obssCoex=%di changed=%d\n",
					__FUNCTION__, index, enable, changed));
	}
	/* stbcEnable */
	if ((changed = (currParam->stbcEnable != operationParam->stbcEnable))) {
		stbc = (operationParam->stbcEnable ? -1 : 0);
		len = sizeof(stbc);
		ret = wldm_xbrcm_Radio_STBCEnable(CMD_SET, index, &stbc, &len, NULL, NULL);
		if (ret  < 0) {
			HAL_WIFI_ERR(("%s: Error radio %d wldm_xbrcm_Radio_STBCEnable"
					" SET stbc = %d ret = %d\n", __FUNCTION__, index, stbc, ret));
			return WIFI_HAL_ERROR;
		}
		HAL_WIFI_DBG(("%s: radioIndex = %d, stbcEnable=%d changed=%d\n",
					__FUNCTION__, index, enable, changed));
	}
	/* greenFieldEnable */
	if ((changed = (currParam->greenFieldEnable != operationParam->greenFieldEnable))) {
		enable = operationParam->greenFieldEnable;
		len = sizeof(enable);
		ret = wldm_xbrcm_Radio_Greenfield11nEnable(CMD_SET, index, &enable, &len, NULL, NULL);
		if (ret  < 0) {
			HAL_WIFI_ERR(("%s: Error radio %d wldm_xbrcm_Radio_Greenfield11nEnable"
						" SET ret = %d\n", __FUNCTION__, index, ret));
			return WIFI_HAL_ERROR;
		}
		HAL_WIFI_DBG(("%s: radioIndex = %d, greenFieldEnable=%d changed=%d\n",
					__FUNCTION__, index, enable, changed));
	}
	/* userControl */
	/* adminControl */
...
#if !defined(WLDM_FOR_HUB4) && !defined(_CBR1_PRODUCT_REQ_)
	if (!radio_enabled && operationParam->enable) {
		/* Run acs_restart command to re-init radio params in acsd2 */
		if (wldm_xbrcm_acs(CMD_SET_IOCTL, index, NULL, NULL, "acs_restart", NULL)
				< 0) {
			HAL_WIFI_ERR(("%s: wldm_xbrcm_acs acs_restart CMD_SET_IOCTL "
					"failed for index : %d\n", __FUNCTION__, index));
			return WIFI_HAL_ERROR;
...
#endif /* !defined(WLDM_FOR_HUB4) && !defined(_CBR1_PRODUCT_REQ_) */
		...
		wifi_radio_operationParam_t *operationParam)
{
	...
	INT len, ret = WIFI_HAL_SUCCESS, stbc = 0;
	CHAR operatingStandards[OUTPUT_STRING_LENGTH_64] = {0};
	...
	/* UINT operatingClass */
	operationParam->operatingClass = param.operatingClass;
	operationParam->op_class = param.operatingClass;
	HAL_WIFI_DBG(("%s: radioIndex=%d  operationParam->operatingClass = 0x%04x\n",
			__FUNCTION__, radioIndex, operationParam->operatingClass));
	/* TBD check difference between op_class and operatingClass */
	...
	/* BOOL ctsProtection */
	len = sizeof(enable);
	ret = wldm_xbrcm_Radio_CtsProtectionEnable(CMD_GET, radioIndex, &enable,
			&len, NULL, NULL);
	if (ret < 0) {
		HAL_WIFI_ERR(("%s: Error radio %d wldm_xbrcm_Radio_CtsProtectionEnable GET "
					"ret = %d\n", __FUNCTION__, radioIndex, ret));
		return WIFI_HAL_ERROR;
	}
	operationParam->ctsProtection = enable;
	HAL_WIFI_DBG(("%s: radioIndex=%d ctsProtectionEnable=%d\n",
				__FUNCTION__, radioIndex, enable));
	/* BOOL obssCoex */
	len = sizeof(enable);
	ret = wldm_Radio_ObssCoexistenceEnable(CMD_GET, radioIndex,
			&enable, &len, NULL, NULL);
	if (ret < 0) {
		HAL_WIFI_ERR(("%s: Error radio %d wldm_Radio_ObssCoexistenceEnable GET "
					"ret = %d\n", __FUNCTION__, radioIndex, ret));
		return WIFI_HAL_ERROR;
	}
	operationParam->obssCoex = enable;
	HAL_WIFI_DBG(("%s: radioIndex=%d ObssCoexEnable=%d\n",
				__FUNCTION__, radioIndex, enable));
	/* BOOL stbcEnable */
	len = sizeof(stbc);
	ret = wldm_xbrcm_Radio_STBCEnable(CMD_GET, radioIndex, &stbc, &len, NULL, NULL);
	if (ret < 0) {
		HAL_WIFI_ERR(("%s: Error radio %d wldm_xbrcm_Radio_STBCEnable GET "
					"ret = %d\n", __FUNCTION__, radioIndex, ret));
		return WIFI_HAL_ERROR;
	}
	operationParam->stbcEnable = stbc ? 1 : 0;
	HAL_WIFI_DBG(("%s: radioIndex=%d stbcEnable=%d\n",
				__FUNCTION__, radioIndex, enable));
	/* greenFieldEnable */
	len = sizeof(enable);
	ret = wldm_xbrcm_Radio_Greenfield11nEnable(CMD_GET, radioIndex, &enable, &len, NULL, NULL);
	if (ret < 0) {
		HAL_WIFI_ERR(("%s: Error radio %d wldm_xbrcm_Radio_Greenfield11nEnable GET "
					"ret = %d\n", __FUNCTION__, radioIndex, ret));
		return WIFI_HAL_ERROR;
	}
	operationParam->greenFieldEnable = enable;
	HAL_WIFI_DBG(("%s: radioIndex=%d greenFieldEnable=%d\n", __FUNCTION__, radioIndex, enable));

#if WIFI_HAL_VERSION_GE_3_0_3 && defined(WL11BE)
	len = sizeof(puncturingInfo);
	if (wldm_xbrcm_Radio_BEpuncturing(CMD_GET, radioIndex, (void *)&puncturingInfo,
				(uint *)&len, NULL, NULL) != 0) {
		HAL_WIFI_ERR(("%s: wldm_xbrcm_radio CMD_GET puncturingInfo radinIndex=%d failed\n",
				__FUNCTION__, radioIndex));
		ret = WIFI_HAL_INTERNAL_ERROR;
	}
	operationParam->puncturingInfo.punct_bitmap = puncturingInfo.punct_bitmap;
	operationParam->puncturingInfo.punct_acs_threshold = puncturingInfo.punct_acs_threshold;
	HAL_WIFI_DBG(("%s: radioIndex=%d, punct_bitmap=%u, punct_acs_threshold=%u\n",
			__FUNCTION__, index, operationParam->puncturingInfo.punct_bitmap,
			operationParam->puncturingInfo.punct_acs_threshold));
#endif /* WIFI_HAL_VERSION_GE_3_0_3 && WL11BE */
