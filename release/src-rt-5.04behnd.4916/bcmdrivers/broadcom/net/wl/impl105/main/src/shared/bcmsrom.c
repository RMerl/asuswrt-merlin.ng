 ...
 *
 * $Id: bcmsrom.c 841375 2024-06-18 04:14:04Z $
 */
...
#endif /* !defined(BCMDONGLEHOST) */
uint srom_vars_len(char *vars, int vars_len);
 ...
 */
uint
BCMATTACHFN(srom_vars_len)(char *vars, int vars_len)
{
	uint pos = 0;
	uint len;
	char *s;
	...
	ASSERT(vars_len < MAXSZ_NVRAM_VARS);
#else
	ASSERT(vars_len < VARS_MAX);
 ...
 *
 * @param[in]  shadow Points at the start of the OTP/SPROM shadow RAM, located in the ChipCommon
 *                    core.
 *
 * @return   0 on success, nonzero on error.
...
{
	if (BCM43684_CHIP(sih->chip)) {
		defvarslen = srom_vars_len(defaultsromvars_43684, sizeof(defaultsromvars_43684));
		bcopy(defaultsromvars_43684, vp, defvarslen);
	} else if (BCM6710_CHIP(sih->chip)) {
		defvarslen = srom_vars_len(defaultsromvars_6710, sizeof(defaultsromvars_6710));
		bcopy(defaultsromvars_6710, vp, defvarslen);
	} else if (BCM6715_CHIP(sih->chip)) {
		defvarslen = srom_vars_len(defaultsromvars_6715, sizeof(defaultsromvars_6715));
		bcopy(defaultsromvars_6715, vp, defvarslen);
	} else if (BCM6717_CHIP(sih->chip)) {
		defvarslen = srom_vars_len(defaultsromvars_6717, sizeof(defaultsromvars_6717));
		bcopy(defaultsromvars_6717, vp, defvarslen);
	} else if (BCM6726_CHIP(sih->chip)) {
		defvarslen = srom_vars_len(defaultsromvars_6726, sizeof(defaultsromvars_6726));
		bcopy(defaultsromvars_6726, vp, defvarslen);
	} else if (BCM6711_CHIP(sih->chip)) {
		defvarslen = srom_vars_len(defaultsromvars_6711, sizeof(defaultsromvars_6711));
		bcopy(defaultsromvars_6711, vp, defvarslen);
	} else {
		defvarslen = srom_vars_len(defaultsromvars_wltest, sizeof(defaultsromvars_wltest));
...
BCMATTACHFN(initvars_srom_pci)(si_t *sih, void *curmap, char **vars, uint *count)
{
	...
	if (err || (sromrev == 0)) {
		...
		if (cmwifi_sromvars_map(sih, sih->chip,
			...
			len = srom_vars_len((char *)sromvars_map, MAXSZ_NVRAM_VARS - 1);
			vp += len;
			*vp++ = '\0';
			goto varsdone;
		}
...
/**
 * Called for for chips with an active PCIe interface *and* that use OTP for NVRAM storage.
 *
