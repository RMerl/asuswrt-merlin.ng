 ...
 *
 * $Id: bcmutils.c 837753 2024-03-14 05:14:08Z $
 */
...
int
bcm_bprintf_val_pcent_per(bcmstrbuf_t *b, uint32 val, uint32 per, uint32 pcent, int pad)
{
	int r = 1;

	if ((val) && (!per) && (!pcent)) {
		r = bcm_bprintf(b, "%*d(0%%)  ", pad, per);
	} else {
		r = bcm_bprintf_val_pcent(b, per, pcent, pad);
	}
	return r;
}
