 ...
 *
 * $Id: bcmwifi_rspec.h 841414 2024-06-18 16:05:54Z $
 */
...
#define RSPEC_CCK_IS_VALID(rspec) \
	(RSPEC_ISLEGACY(rspec) && \
	 ((rspec) & WL_RSPEC_LEGACY_RATE_MASK) >= WLC_RATE_1M && \
	 ((rspec) & WL_RSPEC_LEGACY_RATE_MASK) <= WLC_RATE_11M)

#define RSPEC_OFDM_IS_VALID(rspec) \
	(RSPEC_ISLEGACY(rspec) && \
	 ((rspec) & WL_RSPEC_LEGACY_RATE_MASK) >= WLC_RATE_6M && \
	 ((rspec) & WL_RSPEC_LEGACY_RATE_MASK) <= WLC_RATE_54M)
