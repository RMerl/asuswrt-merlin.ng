 ...
 *
 * $Id: bcmwifi_radiotap.c 835642 2024-01-24 01:15:40Z $
 */
...
/* Radiotap structure length, required alignment */
const struct rtap_field rtap_parse_info[] = {
	...
	{0, 0}, /* 26: */
	{0, 0}, /* 27: */
	{10, 4}, /* 28: IEEE80211_RADIOTAP_TLVS */
...
wl_radiotap_rx(struct dot11_header *mac_header,	wl_rxsts_t *rxsts, bsd_header_rx_t *bsd_header)
{
	...
	if (CHSPEC_IS2G(rxsts->chanspec)) {
		...
		channel_frequency = (uint16)wf_channel2mhz(wf_chspec_ctlchan(rxsts->chanspec),
...
wl_radiotap_tx(struct dot11_header *mac_header,	wl_txsts_t *txsts, bsd_header_tx_t *bsd_header)
{
	...
	if (CHSPEC_IS2G(txsts->chanspec)) {
		...
		channel_frequency = (uint16)wf_channel2mhz(wf_chspec_ctlchan(txsts->chanspec),
			WF_CHAN_FACTOR_2_4_G);
	} else {
...
wl_radiotap_rx_ht(struct dot11_header *mac_header, wl_rxsts_t *rxsts, wl_radiotap_ht_t *rtht)
{
	...
	rtht->rt_common.tsft_l = HTOL32(rxsts->mactime);
	rtht->rt_common.tsft_h = 0;
	rtht->rt_common.flags = flags;
	rtht->rt_common.u.pad1 = 0;
	rtht->rt_common.channel_freq = HTOL16(channel_frequency);
	rtht->rt_common.channel_flags = HTOL16(channel_flags);
	rtht->rt_common.signal = (int8)rxsts->signal;
	rtht->rt_common.noise = (int8)rxsts->noise;
	rtht->rt_common.antenna = (uint8)rxsts->antenna;
...
wl_radiotap_rx_vht(struct dot11_header *mac_header, wl_rxsts_t *rxsts, wl_radiotap_vht_t *rtvht)
{
	...
	rtvht->ieee_radiotap.it_present = HTOL32(WL_RADIOTAP_PRESENT_VHT);

	rtvht->rt_common.tsft_l = HTOL32(rxsts->mactime);
	rtvht->rt_common.tsft_h = 0;
	rtvht->rt_common.flags = flags;
	rtvht->rt_common.u.pad1 = 0;
	rtvht->rt_common.channel_freq = HTOL16(channel_frequency);
	rtvht->rt_common.channel_flags = HTOL16(channel_flags);
	rtvht->rt_common.signal = (int8)rxsts->signal;
	rtvht->rt_common.noise = (int8)rxsts->noise;
	rtvht->rt_common.antenna = (uint8)rxsts->antenna;
...
wl_radiotap_rx_he(struct dot11_header *mac_header, wl_rxsts_t *rxsts, wl_radiotap_he_t *rthe)
{
	...
	rthe->rt_common.tsft_l = HTOL32(rxsts->mactime);
	rthe->rt_common.tsft_h = 0;
	rthe->rt_common.flags = flags;
	rthe->rt_common.u.pad1 = 0;
	rthe->rt_common.channel_freq = HTOL16(channel_frequency);
	rthe->rt_common.channel_flags = HTOL16(channel_flags);
	rthe->rt_common.signal = (int8)rxsts->signal;
	rthe->rt_common.noise = (int8)rxsts->noise;
	rthe->rt_common.antenna = (uint8)rxsts->antenna;
...
/* populate EHT IEEE802.11 Radiotap */
uint
wl_radiotap_rx_eht(struct dot11_header *mac_header, wl_rxsts_t *rxsts, wl_radiotap_eht_t *rteht)
{
	uint16 channel_frequency;
	uint16 channel_flags;
	uint8 flags;
	...
	memset(rteht, 0, sizeof(wl_radiotap_eht_t));
	/* IEEE80211 Radiotap Header */
	rteht->ieee_radiotap.it_version = 0;
	rteht->ieee_radiotap.it_pad = 0;
	rteht->ieee_radiotap.it_len = HTOL16(sizeof(wl_radiotap_eht_t));

	/* IEEE80211 Radiotap Present flags */
	rteht->ieee_radiotap.it_present = HTOL32(WL_RADIOTAP_PRESENT_EHT);
	rteht->rt_common.tsft_l = HTOL32(rxsts->mactime);
	rteht->rt_common.tsft_h = 0;
	rteht->rt_common.flags = flags;
	rteht->rt_common.u.pad1 = 0;
	rteht->rt_common.channel_freq = HTOL16(channel_frequency);
	rteht->rt_common.channel_flags = HTOL16(channel_flags);
	rteht->rt_common.signal = (int8)rxsts->signal;
	rteht->rt_common.noise = (int8)rxsts->noise;
	rteht->rt_common.antenna = (uint8)rxsts->antenna;

	/* IEEE80211 Radiotap AMPDU Status */
	rteht->ampdu_ref_num = HTOL32(rxsts->ampdu_counter);

	rteht->ampdu_flags = HTOL16(IEEE80211_RADIOTAP_AMPDU_EOF_KNOWN |
		IEEE80211_RADIOTAP_AMPDU_LAST_KNOWN);
	if (!(rxsts->nfrmtype & WL_RXS_NFRM_AMPDU_FIRST) &&
			!(rxsts->nfrmtype & WL_RXS_NFRM_AMPDU_SUB)) {
		rteht->ampdu_flags |= HTOL16(IEEE80211_RADIOTAP_AMPDU_IS_LAST);
	}

	/* S-MPDU */
	if (rxsts->nfrmtype & WL_RXS_NFRM_SMPDU) {
		rteht->ampdu_flags |= HTOL16(IEEE80211_RADIOTAP_AMPDU_EOF);
	}

	/* EHT TLV */
	rteht->eht_t = HTOL16(IEEE80211_RADIOTAP_TLV_EHT);
	rteht->eht_l = HTOL16(IEEE80211_RADIOTAP_EHT_TLV_LEN);

	/* TODO:WL11BE:MONITOR fill EHT know and data fields */

	/* WL11BE:MONITOR fill EHT User info field */
	rteht->user_info = HTOL32(IEEE80211_RADIOTAP_EHT_HAVE_MCS |
		IEEE80211_RADIOTAP_EHT_HAVE_NSS |
		IEEE80211_RADIOTAP_EHT_HAVE_CODING);
	/* mcs */
	rteht->user_info |= HTOL32(((rxsts->mcs & (IEEE80211_RADIOTAP_EHT_MCS_USR_INF_MASK >>
		IEEE80211_RADIOTAP_EHT_MCS_USR_INF_SHIFT))
		<< IEEE80211_RADIOTAP_EHT_MCS_USR_INF_SHIFT));
	/* nss */
	rteht->user_info |= HTOL32(((rxsts->nss & (IEEE80211_RADIOTAP_EHT_NSS_USR_INF_MASK >>
		IEEE80211_RADIOTAP_EHT_NSS_USR_INF_SHIFT))
		<< IEEE80211_RADIOTAP_EHT_NSS_USR_INF_SHIFT));
	/* coding */
	rteht->user_info |= HTOL32(((rxsts->coding & (IEEE80211_RADIOTAP_EHT_CODING_MASK >>
		IEEE80211_RADIOTAP_EHT_CODING_SHIFT))
		<< IEEE80211_RADIOTAP_EHT_CODING_SHIFT));

	/* U-SIG TLV */
	rteht->usig_t = HTOL16(IEEE80211_RADIOTAP_TLV_U_SIG);
	rteht->usig_l = HTOL16(IEEE80211_RADIOTAP_USIG_TLV_LEN);

	/* U-SIG common, value/mask fields */
	rteht->common = HTOL32(rxsts->eht_usig_cmn);
