 ...
 *
 * $Id: bcmwifi_rates.c 842450 2024-07-16 20:26:09Z $
 */
 ...
 *
 * @param[in] mcs  A *single* spatial stream MCS (11ax)
 * @param[in] nss  Must be > 0
 *
 * XXX see [Confluence:11ax_rate_table.xlsx]
 * Symbol length = 12.8 usec [given as sym_len/10 below]
 * GI value = 0.8 or 1.6 or 3.2 usec [given as GI_value/10 below]
 * rate (Kbps) = (Nsd * Nbpscs * nss * (coding_n/coding_d) * 1000) / ((sym_len/10) + (GI_value/10))
 *	 Note that, for calculation purpose, following is used. [to be careful with overflows]
 * rate (Kbps) = (Nsd * Nbpscs * nss * (coding_n/coding_d) * 1000) / ((sym_len + GI_value) / 10)
 * rate (Kbps) = (Nsd * Nbpscs * nss * (coding_n/coding_d) * 1000) / (sym_len + GI_value) * 10
 */
uint
...
wf_he_plcp_to_rspec_rate(uint8 *plcp, uint8 ft, uint8 hefmt)
{
	...
	/* sanity check MCS and bound to sane values */
	if (mcs > WLC_MAX_HE_MCS || nss > 8) {
...
/* XXX: definitions based on:
 * https://docs.google.com/spreadsheets/d/1h8QQdMhn0R1Ia2_tfFQw8ENTSSIkxJvhTvEP9evcRa0/edit#gid=0
 */
