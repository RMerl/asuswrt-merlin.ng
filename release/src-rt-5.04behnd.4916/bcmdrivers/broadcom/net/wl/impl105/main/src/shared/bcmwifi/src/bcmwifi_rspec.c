 ...
 *
 * $Id: bcmwifi_rspec.c 842450 2024-07-16 20:26:09Z $
 */
	...
	switch (rspec & WL_RSPEC_ENCODING_MASK) {
		...
		case WL_RSPEC_ENCODE_HT:
			...
			if (mcs > 32) {
				...
				printf("%s: rspec=%x\n", __FUNCTION__, rspec);
#endif /* BCMDBG */
#ifdef BCMDBG_BAD_PLCP
				ASSERT(mcs <= 32);
#endif /* BCMDBG_BAD_PLCP */
				mcs = 0;
			}
			if (mcs == 32) {
...
/**
 * XXX from the column 'B7-B1 of the RU Allocation subfield' in the table 'Encoding of PS160 and
 * RU Allocation subfields in an EHT  variant User Info field' in the .11be standard.
 *
 * @param[in] ruidx_7b   7 bits wide ru index representing 'B7-B1 of the RU Allocation subfield'
 */
enum ru_type_e
