 ...
 *
 * $Id: hnd_pktpool.c 841321 2024-06-17 03:06:12Z $
 */
...
int
BCMATTACHFN(pktpool_attach)(uint32 total_pools)
{
	uint32 poolid;
...
int
BCMATTACHFN(pktpool_dettach)(void)
{
	uint32 poolid;
	...
	uint8 type)
{
	...
	if (type == PKTPOOL_TYPE_PCN) {
		pktp->maxlen = PKTPOOL_PCN_LEN_MAX;
	} else {
...
#else
		p = PKTGET(osh, pktp->max_pkt_bytes, TRUE);
#endif // endif
...
BCMATTACHFN(lfbufpool_init)(osl_t *osh, lfrag_buf_pool_t *lfbufp, int *n_pkts, int max_buf_bytes)
{
	...
	for (i = 0; i < plen; i++) {
		...
		buf = (lfrag_buf_t *)MALLOC(osh, max_buf_bytes + LFBUFSZ);
		if (buf == NULL) {
			/* Not able to allocate all requested pkts
			 * so just return what was actually allocated
			 * We can add to the pool later
...
BCMATTACHFN(lfbufpool_deinit)(osl_t *osh, lfrag_buf_pool_t *lfbufp)
{
	...
	while (lfbufp->freelist != NULL) {
		...
		MFREE(OSH_NULL, buf, lfbufp->max_buf_bytes + LFBUFSZ);
...
pktpool_get_ext(pktpool_t *pktp, uint8 type, void *bufp)
{
	...
	if (BCMSPLITRX_ENAB() && (type == lbuf_rxfrag)) {
		...
		do {
			...
			if (pktp->cbext.cb(pktp, pktp->cbext.arg, p, FALSE)) {
				if (head == NULL) {
					head = tail = p;
				} else {
...
BCMATTACHFN(hnd_pktpool_init)(osl_t *osh)
{
	...
	n = pktpool_attach(PKTPOOL_MAXIMUM_ID);
