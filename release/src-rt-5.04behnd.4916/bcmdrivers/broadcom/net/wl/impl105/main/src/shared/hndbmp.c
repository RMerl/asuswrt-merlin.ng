 ...
 *  XXX FIXME
 *
 *  Upon integration of ARQ and IVW,
 *
 *  0. Delete all BMP_UTEST_BUILD code after ARQ integration, bump HNDBMP rev
 *  1. BME Engine pool iterator sharing between H2D and D2H users, e.g. D2H
 *     may be used by both RCH and BMP, and cannot afford to have discrete
 *     iterators between these services.
 *  2. In case a BCMPOSTATTACH text segment is not created, then bmp_reg must
 *     be placed in paging text segment.
 */
...
{
	/*
	 * FIXME : Both RCH and BMP share the same BME engines
	 * Need a shared sync iterator for the parallel use of BME channels
	 * For now, sync on each pgo/pgi operation.
	 */
} bmp_dma_t;
...
_bmp_map(bmp_sys_t * bmp_sys, bmp_usr_t * bmp_usr, bmp_idx_t hbm_idx)
{
	...
	if (bme_eng != BCME_ERROR) {                        // Sync DBM PGO
		bme_sync_eng(bmp_sys->osh, bme_eng);            // FIXME use bmp_dma_t
	}
...
/*
 * XXX DELETE ME
 * BA64:  272B buffer size, HBM 512, DBM 64
 * BMP USR 0 ARQW0 buffer_sz 272 cb 0x000058a1(0x001594f8)
 * HBM  511 idle 511 clear 0 HME 139264 haddr64 hi 0x00000000 lo 0x2bdea000
 * DBM   63 memory [0x001b32ac 18404] base 0x001b3190 cache 0x001b7890
 *
 * BA256: 1064B buffer size, HBM 384, DBM 48
 * BMP USR 1 ARQW1 buffer_sz 1064 cb 0x000058a1(0x001594f8)
 * HBM  383 idle 383 clear 0 HME 408576 haddr64 hi 0x00000000 lo 0x2be0c000
 * DBM   47 memory [0x001a6a94 50956] base 0x001a6660 cache 0x001b3020
 */
#endif /* BMP_UTEST_BUILD */
