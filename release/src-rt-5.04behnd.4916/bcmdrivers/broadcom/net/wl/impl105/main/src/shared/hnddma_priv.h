 ...
 *
 * $Id: hnddma_priv.h 836975 2024-02-26 13:06:29Z $
 */
...
#define DMA_MAX_MLO_LINKS		PCIE_IPC_AP_UNIT_MAX
#define DMA_MAX_MLO_AUX_LINKS		(DMA_MAX_MLO_LINKS - 1)

typedef struct dma_mlo_aux_info {
...
#ifdef PP_XPM
	void		**rxpdata;	/* pointer to parallel array of pointers to pdata */
#endif /* PKTPULL_HW */
} dma_mlo_aux_info_t;
#endif /* BCM_HWALITE */
...
typedef struct dma_info {
	...
	bool		d11rx_war:1;	/* JIRA:CRWLDOT11M-1776; Rx DMA buffer cannot exceed
					 * 492 bytes
					 */
...
#ifdef BCM_PCAP
	bool		pcap_en:1;
	bool		rs0cd_reset:1;	/* RS0CD Reinit WAR */
	bool		pcap_reset:1;	/* RS0CD Reinit WAR */
	uint16		rs0cd_last;	/* Last CD prior to reset WAR */
#endif // endif
...
#ifdef BCM_HWALITE
	void		*mlc_dev;
	dma_mlo_aux_info_t aux_info[DMA_MAX_MLO_AUX_LINKS]; /* Aux RxDMA info */
	uint8		fifo;
#endif /* BCM_HWALITE */
	union {
		uint8 desc_wrap;
...
#ifdef PP_XPM
	void		**rxpdata;		/* pointer to parallel array of pointers to pdata */
	bool		pp_xpm_enab;		/* BPM/MPM based pktpool used */
#endif /* PP_XPM */
} dma_info_t;
...
/* MLO Aux RxFIFO might use different descriptor depths (nrxd) */
#define	RXD_MLO(x, n)			XXD(x, n)
#define	NEXTRXD_MLO(i, n)		RXD_MLO(((i) + 1), n)
#define	NRXDACTIVE_MLO(h, t, n)		RXD_MLO(((t) - (h)), n)
