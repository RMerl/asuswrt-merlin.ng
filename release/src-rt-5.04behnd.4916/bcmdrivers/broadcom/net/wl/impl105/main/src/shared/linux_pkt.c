 ...
 *
 * $Id: linux_pkt.c 840254 2024-05-17 05:16:05Z $
 */
...
/* PKTC_TAG_IN_SKB_CB: uses cb[32] to cb[47] for PKTC */
#define __OSL_PKTTAG_CLEAR(p) \
do { \
	struct sk_buff *s = (struct sk_buff *)(p); \
...
linux_pktfree_offload(osl_t *osh, struct sk_buff *skb, int line, const char *caller)
{
	...
	while (skb) {
		...
		if (PKTISSPC(osh, skb)) {
			...
			PKTSETTOTLEN(osh, skb, 0);
			PKTCLRSPC(osh, skb);

			/* zoom past the msdus in frag_list */
			if (skb_shinfo(skb)->frag_list) {
				struct sk_buff *head = skb;
				pktcnt += skb_shinfo(head)->nr_frags;
				head->prev = NULL;
...
#ifdef BPM_BULK_FREE
		if (!(skb->recycle_flags & SKB_RECYCLE)) {
			/* If freelist has host generated packets, free up the list immediately */
			commit_freelist = TRUE;
		}
#endif /* BPM_BULK_FREE */
