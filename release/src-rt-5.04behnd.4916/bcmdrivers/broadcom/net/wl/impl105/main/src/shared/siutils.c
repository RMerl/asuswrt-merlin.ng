 ...
 *
 * $Id: siutils.c 842271 2024-07-10 16:32:45Z $
 */
...
BCMATTACHFN(si_enum_base_pa)(uint devid)
{
	...
	switch (devid) {
		...
		CASE_BCM6711_CHIP: /* equals BCM6711_D11BE2G_ID, so don't add separately */
...
si_gci_seci_init(si_t *sih)
{
	...
	CASE_EMBEDDED_2x2BE_40MHZ_CORE:
		/* 6765/6766 Configuration
		 * Wireless- SECI_OUT -> GCI GPIO 5
		 * Wireless- SECI_IN  -> GCI GPIO 6
		 *
		...
		/*
		 * 6765/6766 SECI mode is enabled via SoC pinmux, ex: kernel/dts/6765/96765REF2.dts
		 */
	...
	CASE_BCM6711_CHIP:
		...
		/* 6711 GCI chip Control register bit 20, enable bt_seci_on_gpio_mode
		 */
		si_gci_indirect(sih, 0,
			GCI_OFFSETOF(sih, gci_chipctrl), ALLONES_32, (1u << 20)); //0x200
...
#if defined(BTCX_3WIRE_DEBUG)
	/*
	 * debug code to enable ucode ihr(BTCX_TRANSCTL) C_BTCXTRANSCTRL_MO_ANTSEL_NBIT
	 * to be used as a debug GPIO
	 */
	si_gci_indirect(sih, 1,
			GCI_OFFSETOF(sih, gci_gpioctl), ALLONES_32, 0x02);
	si_gci_indirect(sih, 4 << 16,
			GCI_OFFSETOF(sih, gci_gpiomask), 0x00000002, 0x00000002);
#endif // BTCX_3WIRE_DEBUG
...
si_sysmem_size(si_t *sih)
{
	...
	/* XXX: 6717A0 uses two contiguous 4KB register spaces for sysmem but doesn't separate it
	 * into two cores.
	 */
