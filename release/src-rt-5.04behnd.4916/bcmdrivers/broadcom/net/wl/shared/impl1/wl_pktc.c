...
static int wl_dump_pktc(void *ctx, struct bcmstrbuf *b);
...
{
	return (hndcrc16((uint8 *) da, 6, CRC16_INIT_VALUE) %
		(CHAIN_ENTRY_NUM / 2));
}
#else
uint8 pktc_tbl_hash(uint8 * da)
{
	return (hndcrc8((uint8 *) da, 6, CRC8_INIT_VALUE) %
		(CHAIN_ENTRY_NUM / 2));
}
#endif
...
void wl_write_to_chain_table(pktc_info_t *pktci, struct sk_buff *chead, struct sk_buff *ctail,
	uint8 fifo)
{
	c_pair_t *pPktc;
	...
	PKTCENQCHAINTAIL(pPktc[fifo].chead, pPktc[fifo].ctail, chead, ctail);
...
void BCMFASTPATH wl_start_txchain_txqwork(pktc_info_t * pktci)
{
	...
	while ((fifo = get_16bitmap_pos(pktci->prio_bitmap)) != -1) {
		...
		if ((cnt >= 0) && (cnt < PKTC_MAX_SIZE) && !mixed_chain) {
			/* less than threshold */
			pPktc[fifo].chead = pPktc[fifo].ctail = NULL;
			pktci->prio_bitmap &= ~(1 << fifo);
		}
		...
		if (chead_tmp != NULL) {
			...
			if (pktci->stats.txcurrchainsz > pktci->stats.txmaxchainsz)
				WLCNTSET(pktci->stats.txmaxchainsz, pktci->stats.txcurrchainsz);
...
void wl_txchainhandler(struct sk_buff *skb, uint32 chainIdx, int wl_radio_idx)
{
...
#ifdef DSLCPE_CACHE_SMARTFLUSH
	{
		...
		if (PKTDIRTYPISVALID(pktci->osh, skb)) {
			...
			uint8_t *deepest =
			    PKTDATA(NULL, skb) + dsl_tx_pkt_flush_len;
			if (dirty_p > deepest)
				deepest = dirty_p;
...
#if defined(PKTC_TBL) && defined(BCM_WFD) && defined(BCM_WLAN_NIC_RX_RNR_ACCEL)
...
wl_pktc_tbl_t *wl_pktc_attach(struct wl_info * wl, struct wl_if * wlif)
{
	...
	wlc_dump_add_fns(WLC_DUMP_INST(wl), "pktc", wl_dump_pktc, NULL, wl);
	...
	if (!pt->g_stats) {
		pt->g_stats =
		    (struct pktc_stats *)kmalloc(sizeof(struct pktc_stats), GFP_KERNEL);
		if (!pt->g_stats) {
...
void BCMFASTPATH wl_start_pktc(pktc_info_t * pktci, struct net_device *dev, struct sk_buff *skb)
{
	wl_if_t *wlif = (wl_if_t *) pktci->wlif;
	wl_info_t *wl = wlif->wl;
...
/* for packet chaining */
unsigned long
wl_pktc_req(int req_id, unsigned long param0, unsigned long param1, unsigned long param2)
{
	int i;
	wl_pktc_tbl_t *pt;
...
#if 0
			printk
			    ("Error : chain idx %ld is not in use or invalid handle 0x%lx\n",
	...
	case PKTC_TBL_UPDATE_WFD_IDX_BY_DEV:
...
#if 0
				printk
				    ("Found dev 0x%x updating wfd_idx %d i %d\n",
				     pktc_wldev[i].dev, pktc_wldev[i].wfd_idx,
				     i);
#endif
		...
		if (i == WLAN_DEVICE_MAX) {
			printk
			    ("ERROR, Matching WLAN device entry not found for dev(0x%lx)\n",
			     param0);
			return -1;
		}
		break;
	...
	case PKTC_TBL_UPDATE:
		{
			...
			pt = (wl_pktc_tbl_t *)
				PKTC_TBL_FN_UPDATE(g_pktc_tbl, (uint8_t *)param0,
			...
			/* if wl_handle is NULL and device is wlan, which means pkt is going
			 * to dhd drv, we should not create the chain entry for it, hence pkt
			 * won't be chained and sent to tx_dev directly but fcache. Same as wds.
			 */
			if ((pt->tx_dev == NULL) ||
				(((pktc_info_t *) pt->wl_handle == NULL ||
					(pktc_handle_t *) param2 == NULL) &&
...
#if 0
				printk("pt->wfd_idx 0x%x pt->idx 0x%x ret 0x%x wfd_idx "
					"after rotate 0x%x\n",
					pt->wfd_idx, pt->idx,
					((pt->idx & ~PKTC_WFD_IDX_BITMASK) |
						(wfd_idx << PKTC_WFD_IDX_BITPOS)),
					(wfd_idx << PKTC_WFD_IDX_BITPOS));
#endif
				return ((pt->idx & ~PKTC_WFD_IDX_BITMASK) |
					(wfd_idx << PKTC_WFD_IDX_BITPOS));
			}
#else
	...
	case PKTC_TBL_SET_STA_ASSOC:
		...
		/* param0 is STA addr, param1 is assoc status, 0: disassoc, 1: assoc,
		 * param2 is event type
		 */
...
unsigned long
wl_pktc_req_with_lock(int req_id, unsigned long param0, unsigned long param1, unsigned long param2)
{
...
unsigned long pktc_tbl_update_fn(wl_pktc_tbl_t * tbl, uint8_t * da,
	struct net_device *dev, pktc_handle_t * handle_p)
{
...
#if 0
	printk
	    ("Hash collision : Entry %d occupied [%02x:%02x:%02x:%02x:%02x:%02x]\n",
	     pktc_tbl_hash_idx, pt->ea.octet[0], pt->ea.octet[1],
	     pt->ea.octet[2], pt->ea.octet[3], pt->ea.octet[4],
	     pt->ea.octet[5]);
#endif
	return 0;
...
static int wl_dump_pktc(void *ctx, struct bcmstrbuf *b)
{
	...
	for (i = 0; i < CHAIN_ENTRY_NUM; i++) {
		if ((g_pktc_tbl[i].in_use) && (g_pktc_tbl[i].wl_handle == 0)) {
			printk
			    ("%02d  %02x:%02x:%02x:%02x:%02x:%02x   %s       %d\n",
			     g_pktc_tbl[i].idx, g_pktc_tbl[i].ea.octet[0],
			     ...
			     (g_pktc_tbl[i].tx_dev ==
			      NULL) ? "NULL" : g_pktc_tbl[i].tx_dev->name,
			     g_pktc_tbl[i].hits);
		}
	}
	...
	printk("\nTO WLAN: \n");
#if defined(BCM_WFD)
	printk("idx  prio  dest_MAC         dest_dev   wfd_idx   chainsz (cur/max)   "
		"pkts   drops\n");
#else
	...
	for (i = 0; i < CHAIN_ENTRY_NUM; i++) {
		if ((g_pktc_tbl[i].in_use) && (g_pktc_tbl[i].wl_handle)) {
			for (j = 0; j < 8; j++) {
				pktci =
				    (pktc_info_t *) (g_pktc_tbl[i].wl_handle);
				if (pktci->stats.total_pkts[j] != 0) {
					printk("%02d    %d  %02x:%02x:%02x:%02x:%02x:%02x    ",
					     g_pktc_tbl[i].idx, j,
					...
					printk("%lu    %d\n",
					       pktci->stats.total_pkts[j],
					       g_pktc_tbl[i].pktci->stats.
					       txdrop[j]);
				}
			}
		}
