...
#else
		 BE_DECL(uint32 idx:16;	/* table idx */
			 uint32 wfd_idx:2; uint32 sta_assoc:1; uint32 in_use:1;
			 uint32 reserved:12; )
		 LE_DECL(uint32 reserved:12; uint32 in_use:1;
			 uint32 sta_assoc:1; uint32 wfd_idx:2; uint32 idx:16; ) /* table idx */
#endif
	};
...
struct pktc_handle {
	...
	struct {
		...
		uint16 wfd_idx:2;
#else
		 BE_DECL(uint16 wfd_idx:2; uint16 reserved:14; )
		 LE_DECL(uint16 reserved:14; uint16 wfd_idx:2; )
#endif
	};
};
...
static inline int pktc_tbl_cmp_fn(wl_pktc_tbl_t * tbl, uint8_t * da, struct net_device *rxifp)
{
	...
	return (((_eacmp((pt)->ea.octet, (da)) == 0) && ((pt)->tx_dev != rxifp)) ||
		((_eacmp((pt2)->ea.octet, (da)) == 0) &&
		((pt2)->tx_dev != rxifp)));
}

void pktc_tbl_clear_fn(wl_pktc_tbl_t *brc, uint8 *da);
unsigned long
pktc_tbl_update_fn(wl_pktc_tbl_t *pt, uint8 *da, struct net_device *dev, pktc_handle_t *handle_p);
unsigned long pktc_tbl_lookup_fn(wl_pktc_tbl_t *pt, uint8_t *da);

/* PKTC requests */
#define PKTC_TBL_INIT			1	/* Initialize pktc table */
#define PKTC_TBL_GET_BY_DA		2	/* Get pktc_tbl for pkt chaining by dest addr */
#define PKTC_TBL_GET_BY_IDX		3	/* Get pktc_tbl for pkt chaining by table idx */
...
extern unsigned long wl_pktc_req(int rid, unsigned long p0, unsigned long p1,
		unsigned long p2);
extern unsigned long wl_pktc_req_with_lock(int rid, unsigned long p0,
...
wl_start_pktc(pktc_info_t * pktci, struct net_device *dev, struct sk_buff *skb);
...
extern int wl_check_fdb_expired(unsigned char *addr, struct net_device *net_device);
...
#if defined(PKTC_TBL)
extern BCMFASTPATH unsigned long dhd_pktc_req(int req_id, unsigned long param0,
...
extern int (*fdb_check_expired_wl_hook)(unsigned char *addr, struct net_device * net_device);

extern int (*send_nbuff_to_wfd)(pNBuff_t pNBuff, struct net_device * dev);
extern int (*send_packet_to_upper_layer)(struct sk_buff * skb);
extern int inject_to_fastpath;
extern void (*wfd_path_delay_calc)(struct sk_buff *head, int len, int type);
...
(*dhd_pktc_req_hook)(int req_id, unsigned long param0,
		unsigned long param1, unsigned long param2);
#endif
