...
/* SCB flowId space [1 .. MAXSCB] and PKTFWd LUT space [0 .. MAXSCB) */
#define WL_SCBID2LUTID(scb_flowid)  ((scb_flowid) - 1)
...
typedef struct wl_pktfwd_stats {	/* Global System Statistics */
	...
	uint32_t txf_mcast_pkts;  /* mcast fast path */
...
typedef enum wl_pktfwd_req {	/* external usage */
 ...
 */
extern unsigned long wl_pktfwd_request(int request, unsigned long param0,
	unsigned long param1, unsigned long param2);

/** PKTFWD 802.3 Address: LAN endpoint or WLAN Station insert/delete/hit/clr */
extern void *wl_pktfwd_lut_ins(uint8_t * d3addr, struct net_device *dev, bool is_wlan);
extern void wl_pktfwd_lut_del(uint8_t * d3addr, struct net_device *net_device);
extern int wl_pktfwd_lut_hit(uint8_t * d3addr, struct net_device *net_device);
...
extern void
wl_pktfwd_dnqueued(struct wl_info *wl, d3fwd_wlif_t * d3fwd_wlif, uint32_t * ucast_pkts);

#if defined(WL_PKTFWD_TXEVAL)
extern void wl_pktfwd_dispatch_pktlist(struct wl_info *wl, struct wl_if *wlif, uint8_t * d3addr,
...
extern void wl_pktfwd_pktqueue_add_pkt(struct wl_info *wl, struct net_device *rx_net_device,
...
extern void wl_pktfwd_update_link_credits(struct wl_info *wl, uint16_t scb_flowid, uint8_t *d3addr,
	uint32_t prio, int32_t credits, bool add);
