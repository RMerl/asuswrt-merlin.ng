...
/* Number of MSI interrupts to be tested */
int nr_msi = 0;
module_param(nr_msi, int, 0644);

/* enumerate based on vendor id */
int sup_vend = 0;
module_param(sup_vend, int, 0644);
 ...
 */
static int pcie_gepd_probe(struct pci_dev *pdev,
	const struct pci_device_id *ent)
{
	...
	if ((gepd_sup_vendor != 0) && (gepd_sup_vendor == pdev->vendor)) {
	    found++;
	    GEPD_LOG("found supported vendor 0x%x\r\n", pdev->vendor);
	}
	...
	for (res = 0; res < PCI_NUM_BAR_RESOURCES; res++) {
	    ...
	    if (epd->bar[res].phys && epd->bar[res].size) {
	        ...
	        epd->bar[res].virt = (volatile char*)ioremap(
	        ...
	        GEPD_LOG("bar %d, phys 0x%llx size 0x%lx, virt 0x%p\r\n",
	            res, epd->bar[res].phys, epd->bar[res].size,
	            epd->bar[res].virt);
	    }
	}
	...
	epd->irq_start = pdev->irq;
	if (gepd_nr_msi > 0) {
	    if (pci_alloc_irq_vectors(pdev, gepd_nr_msi, gepd_nr_msi,
	            PCI_IRQ_MSI) == gepd_nr_msi)
	    {
	        epd->irq_start = pdev->irq;
	    } else {
	        gepd_nr_msi = 0;
	...
	epd->irq_end = epd->irq_start + ((gepd_nr_msi) ? (gepd_nr_msi-1) : 0);

	GEPD_LOG("Using %s irqs [%d - %d]\r\n",
	    (gepd_nr_msi==0) ? "Regular" : "MSI", pdev->irq, epd->irq_end);

	i = 0;
	do {
	    if (request_irq(pdev->irq + i, pcie_gepd_isr, IRQF_SHARED,
	        GEPD_DRV_NAME, epd) < 0) {
	        GEPD_LOG("request_irq(%d) failed\n", pdev->irq);
	    } else {
	        GEPD_LOG("request_irq(%d) success\n", pdev->irq);
	    }
	    i++;
	} while (i < gepd_nr_msi);
 ...
 *
 *   Description:
 *    disable the pcie device and release all resources corresponding to the
 *    device
 *
 *  Return: 0 on success, -ve on failure
 */
