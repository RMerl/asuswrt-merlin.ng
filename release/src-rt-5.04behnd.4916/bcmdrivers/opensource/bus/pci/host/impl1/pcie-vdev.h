...
#define PCIE_VDEV_IRQ_IDX_CCM               0
#define PCIE_VDEV_IRQ_IDX_D11MAC            1
#define PCIE_VDEV_IRQ_IDX_M2MDMA            2
#define PCIE_VDEV_IRQ_IDX_WDRST             3
#define PCIE_VDEV_IRQ_IDX_M2MDMA1           4
#define PCIE_VDEV_IRQ_IDX_MLC               5
#define PCIE_VDEV_IRQ_IDX_PHY               6
#define PCIE_VDEV_IRQ_IDX_THERM_HIGH        7
#define PCIE_VDEV_IRQ_IDX_THERM_LOW         8
#define PCIE_VDEV_IRQ_IDX_THERM_SHUTDOWN    9
#define PCIE_VDEV_IRQ_IDX_MAX               10

#define PCIE_VDEV_IRQ_OFFSET(irq_idx)       (4 + sizeof(uint16) * (irq_idx))
...
#define PCIE_VDEV_LIST_ELEM(chip, rev, elem)                                                \
	{.chip_id = chip, .rev_id = rev,                                                    \
	 .size = sizeof(elem)/PCIE_VDEV_CFG_SIZE, .pcfg = (struct bcmvpcie_vdev_cfg*)(&elem)}
 ...
 * |----------------------------------------|-----|
 * |      mlc_irq      |    m2mdma1_irq     | 0ah |
 * |----------------------------------------|-----|
 * |  therm_high_irq   |      phy_irq       | 0ch |
 * |----------------------------------------|-----|
 * | therm_shutdn_irq  |  therm_low_irq     | 10h |
 * |----------------------------------------|-----|
 */
struct pcie_vdev_cap_vndr {
	uint8 cap_id;
	uint8 next_ptr;
	uint8 length;
	uint8 version;
	uint16 wifi_irq[PCIE_VDEV_IRQ_IDX_MAX];
	uint32 reserved[8];
};
 ...
 */
struct bcmvpcie_vdev_cfg_info {
	uint32               chip_id;
	uint32               rev_id;
	uint32               size;
	struct bcmvpcie_vdev_cfg *pcfg;
};
 ...
 */
const uint32 pcie_wifidefdev_cfg[2*PCIE_VDEV_CFG_SIZEW] = {
	/* 2 -  WIFI device, 1 - Virtual Core */
	/* 0:1:0.0 */ PCIE_VDEV_INIT_CFG(0xf6ca, 0x2, 0x00, 0x00, 0x00, 1, 0),
	/* 0:1:1.0 */ PCIE_VDEV_INIT_CFG(0xf6ca, 0x2, 0x00, 0x00, 0x00, 1, 1)
};
