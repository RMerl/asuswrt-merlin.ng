...
static int insert_slave(int bus, int slave_id, struct device *dev)
{
    ...
    if (bus < MAX_BUS && slave_id < MAX_SLAVES && !slaves[bus][slave_id])
    {
...
{
    return (bus < MAX_BUS && slave_id < MAX_SLAVES) ? slaves[bus][slave_id]: NULL ;
}
...
static void find_slave_dev_remove(struct device *dev)
{
    ...
    if (bus < MAX_BUS && spi->chip_select < MAX_SLAVES)
    {
