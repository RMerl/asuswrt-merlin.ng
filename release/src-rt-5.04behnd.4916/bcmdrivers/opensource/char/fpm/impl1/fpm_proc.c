...
static int fpm_proc_cmd_stats(int argc, char *argv[])
{
	...
	for (i = 0; i < FPM_POOL_NUM; i++) {
		...
		pr_info("Base Address (Phys)..............0x%px\n",
			(void *)fpm->pool_pbase[i]);
		pr_info("Base Address (Virt)..............0x%px\n",
			fpm->pool_vbase[i]);
...
#if (LINUX_VERSION_CODE > KERNEL_VERSION(5,10,0))
static const struct proc_ops isr_timer_fops = {
	.proc_open 		= fpm_isr_timer_open,
	.proc_read 		= seq_read,
	.proc_lseek 	= seq_lseek,
	.proc_release 	= single_release,
	.proc_write		= fpm_isr_timer_write,
};
#else
...
#if (LINUX_VERSION_CODE > KERNEL_VERSION(5,10,0))
static const struct proc_ops hist_fops = {
	.proc_open		= fpm_hist_proc_open,
	.proc_read		= seq_read,
	.proc_lseek		= seq_lseek,
	.proc_release	= seq_release,
};
