		       ...
		       uint8_t prio)
{
	...
	if (index_to_add < IQ_KEYMASKTBL_SIZE-1)
			    ...
			    int curr_offset)
{
	...
	case IQ_KEY_FIELD_SRC_MAC:
		memcpy(target_ptr, key_data->src_mac, ETH_ALEN);
		return ETH_ALEN;
	case IQ_KEY_FIELD_DST_MAC:
		memcpy(target_ptr, key_data->dst_mac, ETH_ALEN);
		return ETH_ALEN;
				 ...
				 int curr_offset)
{
	...
	case IQ_KEY_FIELD_INGRESS_DEVICE:
		...
		return 1;
	case IQ_KEY_FIELD_SRC_MAC:
		memcpy(target_ptr, blog->src_mac.u8, ETH_ALEN);
		return ETH_ALEN;
	case IQ_KEY_FIELD_DST_MAC:
		memcpy(target_ptr, blog->dst_mac.u8, ETH_ALEN);
		return ETH_ALEN;
	case IQ_KEY_FIELD_ETHER_TYPE:
		     ...
		     iq_key_data_t *key_data, iq_action_t *action)
{
	...
	for (i = 0; i < IQ_HASH_BIN_SIZE; i++) {
		p_iq_hash = &qos_htbl[IQ_HASHTBL_IDX(hash_value + i)];
			  ...
			  iq_action_t *action, int f_enable_bypass)
{
	...
	for (i = 0; i < atomic_read(&keymask_entry_cnt); i++) {
		...
		for (j = 0; j < IQ_HASH_BIN_SIZE; j++) {
			curr_hash = &qos_htbl[IQ_HASHTBL_IDX(hash_value + j)];
...
static int iq_drv_ioctl_subsys_keymask(iqctl_data_t *iq_p, unsigned long arg)
{
	...
	case IQCTL_OP_ADD:
		...
		if (copy_to_user((uint8_t *)arg, iq_p, sizeof(iqctl_data_t))) 
			ret = -EFAULT;
		break;
	case IQCTL_OP_REM:
		keymask = iq_drv_iqctl_keymask_convert(iq_p->key_data.key_field_mask);
	...
	case IQCTL_OP_GETNEXT:
		i = iq_p->nextIx;
		if (i < IQ_KEYMASKTBL_SIZE && keymask_tbl[i].key_opt.valid) {
			 ...
			 unsigned long arg)
{
	...
	if (copy_from_user(iq_p, (uint8_t *)arg, sizeof(iq)) != 0) {
		BCM_LOG_ERROR(BCM_LOG_ID_IQ, "copy_from_user error!");
		return -EINVAL;
	}
	if ((command > IQCTL_IOCTL_MAX) || (iq_p->subsys > IQCTL_SUBSYS_MAX) || (iq_p->op > IQCTL_OP_MAX)) {
		BCM_LOG_ERROR(BCM_LOG_ID_IQ, "parameter error! cmd=%d subsys=%d op=%d", command, iq_p->subsys, iq_p->op);
		return -EINVAL;
	}
	...
	if (cmd == IQCTL_IOCTL_SYS) {
		...
		case IQCTL_SUBSYS_STATUS:
			...
			case IQCTL_OP_GET:
				...
				if (copy_to_user((uint8_t *)arg, iq_p, sizeof(iq))) 
					ret = -EFAULT;
				break;
			case IQCTL_OP_SET:
...
static void iq_drv_deinit(void)
{
	if (!IS_ERR(iq_db.device)) {
		device_destroy(iq_db.class, MKDEV(iq_db.major, 0));
		cdev_del(&iq_db.cdev);
	}

	if (!IS_ERR(iq_db.class))
		class_destroy(iq_db.class);

	if (iq_db.major)
		unregister_chrdev_region(MKDEV(iq_db.major, 0),
					 IQ_NUM_DEVICES);
}
...
static int iq_drv_construct(void)
{
	...
	rc = alloc_chrdev_region(&dev, 0, IQ_NUM_DEVICES, IQ_DRV_NAME);
	if (rc < 0) {
		pr_err("%s:alloc_chrdev_region() failed\n", __func__);
		return -ENODEV;
	}
	iq_db.major = MAJOR(dev);

	/* create device and class */
	iq_db.class = class_create(THIS_MODULE, IQ_DRV_NAME);
	if (IS_ERR(iq_db.class)) {
		rc = PTR_ERR(iq_db.class);
		pr_err("%s:Fail to create class %s, rc = %d\n", __func__,
		       IQ_DRV_NAME, rc);
		goto fail;
	}

	devno = MKDEV(iq_db.major, 0);
	cdev_init(&iq_db.cdev, &iq_fops);
	iq_db.cdev.owner = THIS_MODULE;

	rc = cdev_add(&iq_db.cdev, devno, 1);
	if (rc) {
	...
	iq_db.device = device_create(iq_db.class, NULL, devno, NULL,
				     IQ_DRV_NAME);
	if (IS_ERR(iq_db.device)) {
		rc = PTR_ERR(iq_db.device);
...
{
	iq_drv_deinit();
	printk(IQ_MODNAME " Char Driver " IQ_VER_STR " Unregistered" CLRnl);
}
...
#define IQ_PROTO_ETHTYPE_PRIO	13	/* FIXME! Temporarily define here */
	bcm_iq_add_keymask(IQ_KEY_MASK_ETHER_TYPE, &key_option,
		       IQ_PROTO_ETHTYPE_PRIO);
#define IQ_PROTO_ETHTYPE_L3PROTO_PRIO   14  /* FIXME! Temporarily define here */
        bcm_iq_add_keymask(IQ_KEY_MASK_ETHER_TYPE + IQ_KEY_MASK_L3_PROTO, &key_option,
                       IQ_PROTO_ETHTYPE_L3PROTO_PRIO);
	...
	/* Init for DNS SRC and DST Port static */
	key_data.l4_src_port = 53;
	action.word = 0;
	action.type = IQ_ACTION_DEFAULT_DNS_RULE;
	action.is_static = 1;
	action.value = 1;
	key_data.ip_proto = IPPROTO_UDP;
	rc = bcm_iq_add_entry(IQ_KEY_MASK_IP_PROTO + IQ_KEY_MASK_SRC_PORT,
			  &key_option, &key_data, &action);
	BCM_LOG_DEBUG(BCM_LOG_ID_IQ, "default DNS UDP+SRC Port Entry  rc<%d>", rc);

	key_data.ip_proto = IPPROTO_TCP;
	rc = bcm_iq_add_entry(IQ_KEY_MASK_IP_PROTO + IQ_KEY_MASK_SRC_PORT,
			  &key_option, &key_data, &action);
	BCM_LOG_DEBUG(BCM_LOG_ID_IQ, "default DNS TCP+SRC Port Entry  rc<%d>", rc);

        key_data.l4_src_port = 0;
	key_data.l4_dst_port = 53;
	key_data.ip_proto = IPPROTO_UDP;
	rc = bcm_iq_add_entry(IQ_KEY_MASK_IP_PROTO + IQ_KEY_MASK_DST_PORT,
			  &key_option, &key_data, &action);
	BCM_LOG_DEBUG(BCM_LOG_ID_IQ, "default DNS UDP+DST Port Entry  rc<%d>", rc);

	key_data.ip_proto = IPPROTO_TCP;
	rc = bcm_iq_add_entry(IQ_KEY_MASK_IP_PROTO + IQ_KEY_MASK_DST_PORT,
			  &key_option, &key_data, &action);
	BCM_LOG_DEBUG(BCM_LOG_ID_IQ, "default DNS TCP+DST Port Entry  rc<%d>", rc);
	...
	/* init for PPP_SES_LCP high priority static, Note PPP_SES_DATA not included */
	key_data.eth_type = ETH_P_PPP_SES;
        key_data.l3_proto = 0;
	rc = bcm_iq_add_entry(IQ_KEY_MASK_ETHER_TYPE + IQ_KEY_MASK_L3_PROTO,
                              &key_option, &key_data, &action);
	BCM_LOG_DEBUG(BCM_LOG_ID_IQ, "default PPP_SES LCP high prio. rc<%d>", rc);
	...
	/* init for LACP Control high priority static */
	key_data.eth_type = ETH_P_SLOW;
	rc = bcm_iq_add_entry(IQ_KEY_MASK_ETHER_TYPE, &key_option, &key_data,
			      &action);
	BCM_LOG_DEBUG(BCM_LOG_ID_IQ, "default LACP Ctrl high prio. rc<%d>", rc);
...
int __init iq_init(void)
{
	...
	BCM_LOG_DEBUG(BCM_LOG_ID_IQ, "iqos_cpu_cong_g<0x%px>\n", &iqos_cpu_cong_g);
	...
	BCM_LOG_DEBUG(BCM_LOG_ID_IQ,
		      "iqos_enable_g<0x%px> qos_htbl<0x%px> keymask_tbl<0x%px>\n",
