...
static inline struct iphdr *bcm_mcast_ip_hdr(const struct sk_buff *skb, int iph_offset)
{
    struct iphdr *pip; 
    int len;
...
static inline struct ipv6hdr *bcm_mcast_ipv6_hdr(const struct sk_buff *skb, int iph_offset, int *len_p)
{
    struct ipv6hdr *pipv6;
    ...
    *len_p = skb_network_offset(skb) + iph_offset + sizeof(*pipv6);
    if ( *len_p > skb_headlen(skb) )
    {
        /* non-linear skb - stop processing */
        return NULL;
    }
   ...
   if ( is_multicast || (flags & BCM_MCAST_FLAG_PPP) )
   {
      ...
      do
      {
         if (proto == htons(ETH_P_IP) )
         {
            ...
            if ((pip = bcm_mcast_ip_hdr(skb, iph_offset)) == NULL)
            {
                break;
            }
         ...
         if (proto == htons(ETH_P_IPV6) )
         {
            ...
            if ((pipv6 = bcm_mcast_ipv6_hdr(skb, iph_offset, &len)) == NULL)
            {
               break;
            }
...
int bcm_mcast_should_deliver(int ifindex, const struct sk_buff *skb, struct net_device *dst_dev, bool dst_mrouter)
{
   ...
   if ( is_multicast || (flags & BCM_MCAST_FLAG_PPP) )
   {
      ...
      do
      {
         ...
         if (proto == htons(ETH_P_IP) )
         {
            ...
            /* no need to error check ip header in should deliver */
      ...
      if (rv && skb->blog_p && BLOG_RX_MCAST(skb->blog_p))
      {
   ...
   rcu_read_unlock();
   __logDebug("source device %s, dst device %s should_deliver %d client_cnt %d", 
              src_dev->name, dst_dev->name, rv, 
              (skb->blog_p && skb->blog_p->shared_info_p)?atomic_read(&skb->blog_p->shared_info_p->client_count):-1);
   return rv;
}
...
__init int bcm_mcast_init (void)
{
   ...
   /* No Whitelist needed for prv mode */
   if ( !bcm_mcast_mode_prv )
   {
       err = bcm_mcast_whitelist_init();
       if ( err )
       {
          printk("bcm_mcast_init: bcm_mcast_whitelist_init error\n");
          bcm_mcast_exit();
          return err;
       }  
   }
   ...
   err = bcm_mcast_hostclient_init();
   if ( err )
   {
      printk("bcm_mcast_init: bcm_mcast_hostclient_init error\n");
      bcm_mcast_exit();
      return err;
   }   
