...
bool bcm_mcast_is_gdxtx_in_root_path(struct net_device *dev)
{
   do 
   {
       if (is_netdev_accel_gdx_tx(dev))
       {
           return true;            
       }
...
{ 
   ...
   *info = 0;
   ...
   if( (BLOG_WLANPHY == BLOG_GET_PHYTYPE(phyType) && wlan_client_get_info_func_ptr && 
       !bcm_mcast_is_gdxtx_in_root_path(repDev)) )
   {
...
{
    struct net_device *root_wan_dev_p;
    uint32_t phyType;
    ...
    root_wan_dev_p = bcm_mcast_find_root_dev(from_dev);

    if (root_dev)
    {
        *root_dev = root_wan_dev_p;
    }

    __logInfo("from_dev %s root_wan_dev_p %s", from_dev->name, root_wan_dev_p->name);

    blog_p->rx.multicast = 1;
    phyType = netdev_path_get_hw_port_type(root_wan_dev_p);
    ...
    if(blog_p->rx.info.phyHdrType == BLOG_GPONPHY)
    {
        ...
        hw_subport_mcast_idx = netdev_path_get_hw_subport_mcast_idx(root_wan_dev_p);

        __logDebug("hw_subport_mcast_idx %d(0x%x)", hw_subport_mcast_idx, hw_subport_mcast_idx);
#if NETDEV_PATH_HW_SUBPORTS_MAX > 0
        if(hw_subport_mcast_idx < NETDEV_PATH_HW_SUBPORTS_MAX)
        { 
...
#endif
        {
            ...
            __logError("Not a GPON Multicast WAN device hw_subport_mcast_idx 0x%x from_dev %s", 
                       NETDEV_PATH_HW_SUBPORTS_MAX, from_dev->name);
            return -1;
        }
    }
    ...
    {
        blog_p->rx.info.channel = netdev_path_get_hw_port(root_wan_dev_p);
    }
                           ...
                           uintptr_t flowhdl)
{
   ...
   if ((lan_dev_p->rtnl_link_ops != NULL) && (!strcmp(lan_dev_p->rtnl_link_ops->kind, "veth")))
   {
       __logInfo("Ignore veth dst %s", lan_dev_p->name);
       return 0;
   }
   ...
   else
   {
      ...
      if (bcm_mcast_blog_fill_rx_info(from_dev, blog_p, &wan_dev_p) != 0)
      {
          blog_put(blog_p);
          return -1;
      }
   ...
   if(BCM_MCAST_PROTO_IPV6 == proto)
   {
      BCM_MCAST_IN6_ASSIGN_ADDR(&blog_p->tupleV6.saddr, &mld_fdb->src_entry.src);
      BCM_MCAST_IN6_ASSIGN_ADDR(&blog_p->tupleV6.daddr, &mld_fdb->grp);
   ...
   {
       blog_p->client_type = BLOG_MCAST_CLIENT_TYPE_TXDEV_WLINFO;
   }
   else
       blog_p->client_type = BLOG_MCAST_CLIENT_TYPE_TXDEV;

   __logInfo("client_type %d wl 0x%x src mac %pM wmf enabled %d", 
              blog_p->client_type, blog_p->wl, repMac, 
              blog_p->wl ? ((BlogWfd_t *)(&blog_p->wl))->mcast.is_wmf_enabled:0);

   blog_p->rx_dev_p = wan_dev_p;
   blog_p->tx_dev_p = dst_dev;
