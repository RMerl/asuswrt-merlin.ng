   ...
   for (i = 0; i < BCM_MCAST_HASH_SIZE; i++) 
   {
      ...
      hlist_for_each_entry_safe(mcast_group, n_group, &pif->mc_ipv4_hash[i], hlist) 
      {
         ...
         list_for_each_entry_safe(reporter_group, n_reporter, &mcast_group->rep_list, list)
         {
            ...
            {
                __logInfo("Snoopentry timeout rxgrp %pI4 txgrp %pI4 clientdev %s ", 
                           &(mcast_group->rxGrp.s_addr), &(mcast_group->txGrp.s_addr), 
                           mcast_group->dst_dev ? mcast_group->dst_dev->name:"NULL");
                       ...
                       uint32_t info)
{
   ...
   /* Check if there is a hostclient and set fwd_and_trap flag */
   bcm_mcast_hostclient_lkup_and_update_flow_igmp(rxGrp, src);
#endif
   return 0;
}
                                  ...
                                  int pkt_type)
{
   ...
   {
      /* accept packet */
      /*__logDebug("IGMP Control filter Accept pkt: src %s, dst %s "
                   "dstip %x", src_dev->name, dst_dev->name, ntohl(pip->daddr));*/
      return 1;
   }
      ...
      if (dst->txGrp.s_addr == pip->daddr)
      {
         if ((dst->src_entry.filt_mode != MCAST_INCLUDE) /* ASM, Exclude filter - exclude source not supported for now. Ref- RFC4607/5771 */
             ...
             ((dst->src_entry.filt_mode == MCAST_INCLUDE) &&
              (pip->saddr == dst->src_entry.src.s_addr))) /* SSM, if this is an include entry source must match */
         {
            grp_match = 1 ;
         }
   ...
   if (!should_deliver && (BCM_MCAST_SNOOPING_BLOCKING_MODE != pif->igmp_snooping))
   {
      if (((BCM_MCAST_SNOOPING_STANDARD_MODE == pif->igmp_snooping) && (grp_match == 0))
            ||
           ((BCM_MCAST_SNOOPING_DISABLED_FLOOD == pif->igmp_snooping)
            &&
            (BCM_MCAST_SNOOPING_DISABLED_HOSTCTL_ENABLED != pif->igmp_hostctl)))
          should_deliver = 1 ;
   }

   spin_unlock_bh(&pif->mc_igmp_lock);

   __logDebug("source device %s, dst device %s dstip %x forward: %s", 
              src_dev->name, dst_dev->name, ntohl(pip->daddr), should_deliver ? "Yes":"No");
...
int bcm_mcast_igmp_display(struct seq_file *seq, bcm_mcast_ifdata *pif)
{
   ...
   /* No whitelists in provisioning mode */
   if (!bcm_mcast_mode_prv)
   {
       bcm_mcast_dump_whitelist(seq);
   }
#endif

#if defined(CC_MCAST_HOST_CLIENT_SUPPORT)
   bcm_mcast_hostclient_entries_dump();
#endif
