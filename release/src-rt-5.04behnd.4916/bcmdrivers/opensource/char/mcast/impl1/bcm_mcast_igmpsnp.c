                             ...
                             uint32_t info)
{
   ...
   if ( bcm_mcast_whitelist_add_fn && (pif->dev != from_dev) ) 
   {
       bcm_mcast_ipaddr_t grpaddr;
       bcm_mcast_ipaddr_t srcaddr;

       grpaddr.is_ipv4 = 1;
       grpaddr.ipv4_addr.s_addr = rxGrp->s_addr;
       srcaddr.is_ipv4 = 1;
       srcaddr.ipv4_addr.s_addr = src->s_addr;

       if (bcm_mcast_whitelist_nodes_add(from_dev, grpVid, &grpaddr, &srcaddr, &mc_fdb->whitelist_info) != 0)
       {
...
#if defined(CONFIG_BLOG) 
      __logInfo("Invoking delete flow for snoopentry rxgrp %pI4 txgrp %pI4 clientdev %s udpexcludeport %d rtpseqflag %d ", 
                           &(igmp_fdb->rxGrp.s_addr), &(igmp_fdb->txGrp.s_addr), 
                igmp_fdb->dst_dev ? igmp_fdb->dst_dev->name : "NULL", 
                igmp_fdb->excludePort, igmp_fdb->enRtpSeqCheck);
      bcm_mcast_delete_flow(BCM_MCAST_PROTO_IPV4, &(igmp_fdb->flowhdl));
#endif
#if defined(CC_MCAST_WHITELIST_SUPPORT)
      /* Whitelist entries are not created if the RX interface is LAN */
      if ( bcm_mcast_whitelist_delete_fn && (pif->dev != igmp_fdb->from_dev) )
      {
          bcm_mcast_whitelist_nodes_del(&igmp_fdb->whitelist_info);
      }
#endif
