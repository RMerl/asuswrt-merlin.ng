                            ...
                            uint32_t info)
{
   ...
   BCM_MCAST_IN6_ASSIGN_ADDR(&mc_fdb->grp, grp);
   BCM_MCAST_IN6_ASSIGN_ADDR(&mc_fdb->src_entry, src);
   ...
   BCM_MCAST_IN6_ASSIGN_ADDR(&rep_entry->rep, rep);
   ...
   if ( bcm_mcast_whitelist_add_fn && (pif->dev != from_dev) ) 
   {
       ...
       grpaddr.is_ipv4 = 0;
       BCM_MCAST_IN6_ASSIGN_ADDR(&grpaddr.ipv6_addr, grp);
       srcaddr.is_ipv4 = 0;
       BCM_MCAST_IN6_ASSIGN_ADDR(&srcaddr.ipv6_addr, src);
                              ...
                              unsigned char    *repMac)
{
                            ...
                            rep_entry_n, &mld_fdb->rep_list, list) 
   {
      if (((NULL == rep) && (NULL == repMac)) ||
          (rep && BCM_MCAST_IN6_ARE_ADDR_EQUAL(&rep_entry->rep, rep)) ||
   ...
   if(list_empty(&mld_fdb->rep_list)) 
   {
      ...
      if ( bcm_mcast_whitelist_delete_fn && (pif->dev != mld_fdb->from_dev))
      {
          if ( bcm_mcast_whitelist_nodes_del(&mld_fdb->whitelist_info) )
          {
                                ...
                                uint32_t info)
{
   ...
   {
      if ((BCM_MCAST_IN6_ARE_ADDR_EQUAL(&mc_fdb->grp, grp)) && 
          (mode == mc_fdb->src_entry.filt_mode) &&
          (BCM_MCAST_IN6_ARE_ADDR_EQUAL(&mc_fdb->src_entry.src, src)) &&
          (mc_fdb->from_dev == from_dev) &&
          (mc_fdb->dst_dev == dst_dev) &&
                               ...
                               uint32_t info)
{
    ...
    {
        if ( BCM_MCAST_IN6_ARE_ADDR_EQUAL(&dst->grp, grp) )
        {
            if ( (BCM_MCAST_IN6_ARE_ADDR_EQUAL(src, &dst->src_entry.src)) &&
                 ...
                 (bcm_mcast_blog_cmp_wlan_reporter_info(dst_dev, dst->info, info) == 0) )
            {
                ...
                if ( reporter == NULL )
                {
                    ...
                    if ( reporter )
                    {
                        BCM_MCAST_IN6_ASSIGN_ADDR(&reporter->rep, rep);
                                              ...
                                              unsigned char *repMac)
{
   ...
   {
      if((rep && BCM_MCAST_IN6_ARE_ADDR_EQUAL(&rep_entry->rep, rep)) ||
         (repMac && (0 == memcmp(rep_entry->repMac, repMac, ETH_ALEN))))
      {
         return rep_entry;
      }
