...
static int bcm_mcast_netlink_process_igmp_snoop_entry(struct nlmsghdr *nlh, unsigned char *pdata)
{
       ...
       if(snoop_entry->wan_info[idx].if_ops)
       {
           ...
           else
           {
               ...
               bcm_mcast_igmp_add(from_dev,
                                  ...
                                  filter_mode, 
                                  snoop_entry->tci,
                                  snoop_entry->wan_info[idx].mcast_vlan,
                                  &snoop_entry->src,
                                  snoop_entry->lanppp,
   ...
   if (pif->igmp_hostctl)
   {
       __logInfo("Host control mode no need to handle LAN2LAN mcast!");
       goto igmp_snoop_exit;
   }
...
static int bcm_mcast_netlink_process_host_client_entry(struct nlmsghdr *nlh, unsigned char *pdata)
{
   t_BCM_MCAST_HOST_CLIENT_ENTRY *hc_entry;

   rcu_read_lock();
   /* make sure length is correct, read is not accepted */
   if ( (nlh->nlmsg_len != NLMSG_SPACE(sizeof(*hc_entry))) ||
        ((NLM_F_CREATE|NLM_F_REPLACE|NLM_F_REQUEST) != nlh->nlmsg_flags) )
   {
      rcu_read_unlock();
      return -EINVAL;
   ...
   hc_entry = (t_BCM_MCAST_HOST_CLIENT_ENTRY *)pdata;

   if (hc_entry->isAdd)
   {
       bcm_mcast_hostclient_entry_add(&hc_entry->grp,
                                      &hc_entry->src,
                                      hc_entry->wan_info);
   }
   else
   {
       bcm_mcast_hostclient_entry_remove(&hc_entry->grp, 
                                         &hc_entry->src);
   }
...
static int bcm_mcast_netlink_process_host_client_entry(struct nlmsghdr *nlh, unsigned char *pdata)
{
   __logError("Host Client support not available");
   return 0;
}
#endif
...
static int bcm_mcast_netlink_process_hostctl_cfg(struct nlmsghdr *nlh, unsigned char *pdata)
{
   t_BCM_MCAST_HOSTCTL_CFG *cfg;
   bcm_mcast_ifdata      *pif;
   int                    ret = 0;
   ...
   cfg = (t_BCM_MCAST_HOSTCTL_CFG *)pdata;
   rcu_read_lock();
   pif = bcm_mcast_if_lookup(cfg->ifi);
   if( NULL == pif )
   {
   ...
   do
   {
      ...
      if ( (NLM_F_CREATE|NLM_F_REPLACE|NLM_F_REQUEST) == nlh->nlmsg_flags )
      {
         if ( (cfg->enable > BCM_MCAST_SNOOPING_DISABLED_HOSTCTL_ENABLED) || 
              ((cfg->proto != BCM_MCAST_PROTO_IPV4) &&
               (cfg->proto != BCM_MCAST_PROTO_IPV6)) )
         {
            ret = -EINVAL;
      ...
      else if ( NLM_F_REQUEST == nlh->nlmsg_flags )
      {
         ...
         bcm_mcast_netlink_send(nlh->nlmsg_pid, BCM_MCAST_MSG_HOSTCTL_CFG, &rsp, sizeof(rsp));         
      }
      else
      {
         ret = -EINVAL;
...
process_rcv_func bcm_mcast_netlink_dispatch[BCM_MCAST_NR_MSGTYPES] =
{
   ...
   [BCM_MCAST_MSG_HOSTCTL_CFG          - BCM_MCAST_MSG_BASE] = bcm_mcast_netlink_process_hostctl_cfg,
   ...
   [BCM_MCAST_MSG_HOST_CLIENT_ENTRY_UPDATE - BCM_MCAST_MSG_BASE] = bcm_mcast_netlink_process_host_client_entry,
};
...
void bcm_mcast_notify_event(int event, int proto, void *grp_entry, void *rep_entry)
{
   ...
   if ( BCM_MCAST_PROTO_IPV6 == proto )
   {
           ...
           (grp6->dst_dev->name[1] == 'l') )
      {
         ...
         BCM_MCAST_IN6_ASSIGN_ADDR(&notify.ipv6grp, &grp6->grp);
         BCM_MCAST_IN6_ASSIGN_ADDR(&notify.ipv6src, &grp6->src_entry.src);
         BCM_MCAST_IN6_ASSIGN_ADDR(&notify.ipv6rep, &rep6->rep);
         raw_notifier_call_chain(&mcast_ctrl->mcast_snooping_chain, event, &notify);
      }
   }
#endif
