...
static int16_t mcast_key_add(uint32_t index, uint8_t vtag_num, Blog_t *blog_p)
{
    mcast_key_entry_t *entry, *tmp_entry;
    uint16_t try_num = 0;
...
static uint32_t runner_iptv_channel_create(Blog_t *blog_p)
{
    ...
    request.key.port_ingress_obj = blog_parse_ingress_port_get(blog_p);
    if (!request.key.port_ingress_obj)
        return FLOW_HW_INVALID;
#endif
...
Exit:
    ...
    if (rc < 0)
    {
        if (rc == BDMF_ERR_ALREADY)
            __logInfo("Failed to add channel request, BDMF_ERR_ALREADY");
        else
...
{
    int rc;
    BDMF_MATTR_ALLOC(iptv_attrs, rdpa_iptv_drv());
