...
pdc_tx_list_init(struct pdc_state *pdcs)
{
	...
	pdc_build_txd(pdcs, pdcs->bcm_header_dma, PDC_HDR_LEN, flags);
	return;
}
...
static int pdc_work_thread(void *cookie)
{
	...
	while ( 1 ) {
		...
		wait_event_interruptible(pdcg->wait_queue, test_bit(1, &pdcg->work_avail) || kthread_should_stop());
...
static int pdc_send_queued_data(struct pdc_state *pdcs)
{
	...
	if ( pdcs->msg_count )
	{
		msg = list_first_entry(&pdcs->msg_list, struct bcmspu_message, list);
...
int pdc_get_max_channel(void)
{
	return pdc_glob->num_chan;
}
EXPORT_SYMBOL(pdc_get_max_channel);

int pdc_send_data(int chan, struct bcmspu_message *msg)
{
