...
static char* dt_scan_mem_str[] = {
	...
	RNRMEM_BASE_ADDR_STR,
#endif
#if IS_ENABLED(CONFIG_BCM_DHD_RUNNER) || IS_ENABLED(CONFIG_BCM_DHD_ARCHER)
	DHD_BASE_ADDR_STR,
	DHD_BASE_ADDR_STR_1,
	DHD_BASE_ADDR_STR_2,
	DHD_BASE_ADDR_STR_3,
...
static reserve_mem_t reserve_mem[TOTAL_RESERVE_MEM_NUM];
static unsigned long reserved_mem_total = 0;
static int rsvd_mem_cnt = 0;
static bool is_memory_reserved = false;
...
static int bcm_rsvmem_addmem(char* rsv_name, void* virt_addr, phys_addr_t phys_addr, uint32_t rsv_size, int cached)
{
	...
	if (!bcm_rsvmem_entry_exist(rsv_name, &index)) {
		...
		pr_err("Reserved memory node %s already exist!!!", rsv_name);
...
static int bcm_rsvmem_get_cma_size(struct device_node *dn, uint32_t* size)
{
	struct device_node *node = NULL;
	struct reserved_mem *rmem = NULL;
	int ret = -1;

	node = of_parse_phandle(dn, "memory-region", 0);
	if (node) {
		rmem = of_reserved_mem_lookup(node);
		if (rmem) {
			*size = (uint32_t)rmem->size;
			ret = 0;
		}
	}
...
static int bcm_rsvmem_probe(struct platform_device *pdev)
{
	...
	char rsv_name[16];
	dma_addr_t dma_addr;
	uint32_t rsv_size, cma_size, dsl_rsv_size = 0;
	...
	if (dsl_rsv_size) {
		...
		if (bcm_rsvmem_get_cma_size(dn, &cma_size) == 0)
