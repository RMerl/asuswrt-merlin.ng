...
int pmd_op_sw_download(uint32_t *crc)
{
    ...
    int rc;
    uint16_t len;
    int i, loops, remainder, alloc_len, len_no_crc;
    uint32_t otp_val;
    ...
    RU_REG_WRITE(LD_LIA,LD_LIA_PARAM,len_no_crc);
    *crc = *(uint32_t*) (file+len_no_crc);
    loops = len_no_crc /SW_DOWNLOAD_CHUNK;
    remainder = len_no_crc % SW_DOWNLOAD_CHUNK;
