...
int rdpa_cmd_br_ioctl(unsigned long arg)
{
    ...
    if(copy_from_user(&br_para, userBr_p, sizeof(rdpa_drv_ioctl_br_t)))
    {
	
        CMD_BR_LOG_DEBUG("copy_from_user_failed\n");
...
ioctl_exit:
    ...
    bdmf_unlock();
    if(copy_to_user(userBr_p, &br_para, sizeof(rdpa_drv_ioctl_br_t)))
    {
        CMD_BR_LOG_DEBUG("copy_from_user_failed\n");
        rc=RDPA_DRV_ERROR;
    }
