...
int rdpa_cmd_ds_wan_udp_filter_ioctl(unsigned long arg)
{
    ...
    if(copy_from_user(&ds_wan_udp_filter, user_ds_wan_udp_filter_p, sizeof(rdpa_drv_ioctl_ds_wan_udp_filter_t)))
    {
        DS_WAN_UDP_FILTER_LOG_ERROR("ERROR : copy_from_user failed %s:%d\n", __FILE__, __LINE__);
        return -1;
    }
    ...
    switch(ds_wan_udp_filter.cmd)
    {
        case RDPA_IOCTL_DS_WAN_UDP_FILTER_CMD_ADD:
        {
            ...
            if(copy_to_user(&user_ds_wan_udp_filter_p->filter.index, &ds_wan_udp_filter.filter.index,
                         sizeof(ds_wan_udp_filter.filter.index)))
            {
                DS_WAN_UDP_FILTER_LOG_ERROR("ERROR : copy_to_user failed %s:%d\n", __FILE__, __LINE__);
                ret=-1;
            }
        }
        break;
        ...
        case RDPA_IOCTL_DS_WAN_UDP_FILTER_CMD_GET:
        {
            ...
            if(copy_to_user(user_ds_wan_udp_filter_p, &ds_wan_udp_filter, sizeof(rdpa_drv_ioctl_ds_wan_udp_filter_t)))
            {
                DS_WAN_UDP_FILTER_LOG_ERROR("ERROR : copy_to_user failed %s:%d\n", __FILE__, __LINE__);
                ret=-1;
            }
