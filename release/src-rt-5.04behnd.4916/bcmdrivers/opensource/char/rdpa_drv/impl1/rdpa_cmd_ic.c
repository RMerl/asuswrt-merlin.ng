...
#define COPY_FROM_USER(a, b, c) if(copy_from_user(a, b, c)){ printk("%s:%d: copy_from_user failed", __FUNCTION__, __LINE__ );}


static bdmf_object_handle ic_get_rdpa_port(const char *if_name)
{
    ...
    ret = rdpa_port_get(if_name, &port_obj);
    if (ret)
    {
        CMD_IC_LOG_ERROR("failed to get port for %s, ret=%d", if_name, ret);
        return NULL;
    }
...
static int create_ic_flow_result(uint8_t action, rdpactl_classification_rule_t *rule, rdpa_ic_info_t *flow, rdpa_traffic_dir dir)
{
    ...
    flow->result.port_egress_obj = ic_get_rdpa_port(rule->egress_port_name);
...
static void create_ic_flow_key(rdpactl_classification_rule_t *rule, rdpa_ic_info_t *flow)
{
    ...
    if (rule->field_mask & RDPA_IC_MASK_INGRESS_PORT)
        flow->key.port_ingress_obj = ic_get_rdpa_port(rule->ingress_port_name);
...
#define COMPARE(field) do \
    ...
    COMPARE(result.port_egress_obj);
...
static int rdpa_ic_add_class(rdpactl_classification_rule_t *pRule)
{
    ...
    CMD_IC_LOG_DEBUG("rdpa_ic_add_class: type(0x%x) field(0x%x) port_name(%s)", pRule->type, pRule->field_mask, pRule->ingress_port_name);
...
static int rdpa_ic_find_class(rdpactl_classification_rule_t *pRule)
{
    ...
    CMD_IC_LOG_DEBUG("rdpa_ic_find_class: type(0x%x) field(0x%x) port_name(%s)", pRule->type, pRule->field_mask, pRule->ingress_port_name);
...
static int rdpa_ic_del_class(rdpactl_classification_rule_t *pRule)
{
    ...
    CMD_IC_LOG_DEBUG("RDPA_IOCTL_IC_CMD_DEL: type(0x%x)field(0x%x) port_name(%s)",pRule->type, pRule->field_mask, pRule->ingress_port_name);
...
static int rdpa_ic_add_flow(rdpactl_classification_rule_t *pRule, uint8_t *pPriority)
{
    ...
    CMD_IC_LOG_DEBUG("rdpa_ic_add_flow: field(0x%x) port_name(%s)", pRule->field_mask, pRule->ingress_port_name);
...
static int rdpa_ic_del_flow(rdpactl_classification_rule_t *pRule)
{
    ...
    CMD_IC_LOG_DEBUG("rdpa_ic_del_flow: field(0x%x) port_name(%s)", pRule->field_mask, pRule->ingress_port_name);
...
int rdpa_iq_ingress_class_add(void *iq_param)
{
    ...
    memset(&rule, 0, sizeof(rdpactl_classification_rule_t));
    /* set default to a port name that exists on every board configuration
     * for every chip in order to prevent "port doesn't exist" errors */
    sprintf(rule.egress_port_name, "eth1");
    ...
    rule.type = RDPA_IC_TYPE_GENERIC_FILTER;
    rule.dir = rdpa_dir_ds; /* In FC there is 1 table for both US & DS, so US traffic will be catched as well */
#if defined(RDP_UFC)
    rule.generic_filter_location = RDPA_ALL_TRAFFIC;
#else
...
#else  /* RDP/63138/148/4908, XRDP_G4/G5 */
