...
#define RDPA_IQ_INGRESS_CLASS_SUPPORT_FIELD (IQ_KEY_MASK_INGRESS_DEVICE | \
            ...
            IQ_KEY_MASK_INNER_PBIT | IQ_KEY_MASK_DSCP | \
            IQ_KEY_MASK_L3_PROTO | IQ_KEY_MASK_IP_PROTO | \
            IQ_KEY_MASK_SRC_IP | IQ_KEY_MASK_DST_IP | \
            IQ_KEY_MASK_SRC_PORT | IQ_KEY_MASK_DST_PORT)

/*
 * 1) IQ_KEY_MASK_OFFSET_[0/1] are implemented in XRDP, but not enabled from
 *    IngressQoS driver/userspace.
 * 2) IQ_KEY_MASK_IPV6_FLOW_LABEL is not supported in newer XRDP (GEN6.0+). 
 *    We have it disabled for supporting on all RDP/XRDP.
 */
