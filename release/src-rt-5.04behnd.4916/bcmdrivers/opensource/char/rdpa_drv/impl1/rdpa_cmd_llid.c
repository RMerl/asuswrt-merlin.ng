...
#if defined(CONFIG_BCM_PON) || defined(CONFIG_BCM963158) || defined(CONFIG_BCM96813)
#include <rdpa_epon.h>
...
/*******************************************************************************/
#if (defined(CONFIG_BCM_XRDP) && !defined(CONFIG_BRCM_QEMU))
...
int rdpa_cmd_llid_ioctl(unsigned long arg)
{
    ...
    BDMF_MATTR_ALLOC(tm_mattrs, rdpa_egress_tm_drv());
    BDMF_MATTR_ALLOC(link_attrs, rdpa_llid_drv());

    if(copy_from_user(&llid, userLlid_p, sizeof(rdpa_drv_ioctl_llid_t)))
    {
    ...
    switch(llid.cmd)
    {
        case RDPA_IOCTL_LLID_CMD_NEW: 
        {
            ...
            port_obj = get_rdpa_epon_port_obj();
            if (!port_obj)
            {
            ...
            if (rc == 0 && egress_tm == NULL)
            {
                ...
                { 
                    ...
                    rdpa_egress_tm_num_queues_set(tm_mattrs, 16);
...
ioctl_exit:
    ...
    if(copy_to_user(userLlid_p, &llid, sizeof(rdpa_drv_ioctl_llid_t)))
    {
        pr_debug("%s:%d: copy_to_user failed", __FUNCTION__, __LINE__ );
        rc=RDPA_DRV_ERROR;
    }

    BDMF_MATTR_FREE(tm_mattrs);
    BDMF_MATTR_FREE(link_attrs);
