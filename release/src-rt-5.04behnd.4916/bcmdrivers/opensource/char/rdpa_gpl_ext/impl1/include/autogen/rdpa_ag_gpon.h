...
typedef enum {
    ...
    rdpa_gpon_attr_initial_equalization_delay = 11, /* initial_equalization_delay : R : number/4 : Equalization delay *on entry to o5 */
    rdpa_gpon_attr_encryption_key = 12, /* encryption_key : R : buffer/16 : Encryption key */
    rdpa_gpon_attr_tcont_counter_assigment = 13, /* tcont_counter_assigment : RW : number/4 : TCONT counter assigment */
    rdpa_gpon_attr_ds_fec_state = 14, /* ds_fec_state : R : bool/1 : DS FEC enable state */
    ...
    rdpa_gpon_attr_tcont_flush = 26, /* tcont_flush : W : bool/1[32(number)] : Flush TCONT Front End Buffer and BBH Queue */
    rdpa_gpon_attr_tcont_stop = 27, /* tcont_stop : W : bool/1[32(number)] : Stop traffic thru TCONT  */
    rdpa_gpon_attr_tcont_restart = 28, /* tcont_flush : W : bool/1[32(number)] : Restart traffic thru TCONT */    
    rdpa_gpon_attr_encryption_error_stat = 29, /* encryption_error_stat : R : number/4 : Encryption errors statistics */
    rdpa_gpon_attr_pon_id_parameters = 30, /* pon_id_parameters : R : aggregate/10 pon_id_param(rdpa_gpon_pon_id_param_t) : PON ID PLOAM paramters */
    rdpa_gpon_attr_mcast_enc_key_parameters = 31, /* multicast encryption keys : RW : aggregate/32 mcast_enc_key_param(rdpa_gpon_mcast_enc_key_param_t) : Mcast enc key paramters */
    rdpa_gpon_attr_stack_mode = 32, /* stack mode : RW : enum/4 stack_mode_param(rdpa_gpon_stack_mode_t) : GPON/NGPON2 stack mode */
    rdpa_gpon_attr_bw_record_cfg = 33, /* bw_record_cfg : RWF : aggregate/10 bw_record_cfg(rdpa_gpon_bw_record_cfg_t) : BW recording cfg */
    rdpa_gpon_attr_bw_record_result_gpon = 34, /* bw_record_result_gpon : R : aggregate/24[64] bw_record_result(rdpa_gpon_bw_record_result_gpon_t) : BW recording result */
    rdpa_gpon_attr_bw_record_result_ngpon = 35 /* bw_record_result_ngpon : R : aggregate/13[512] bw_record_result(rdpa_gpon_bw_record_result_ngpon_t) : BW recording result */
...
/** Get gpon/initial_equalization_delay attribute.
 *
 * Get Equalization delay on entry to o5.
 * \param[in]   mo_ gpon object handle or mattr transaction handle
 * \param[out]  equalization_delay_ Attribute value
 * \return 0 or error code < 0
 * The function can be called in task context only.
 */
static inline int rdpa_gpon_initial_equalization_delay_get(bdmf_object_handle mo_, bdmf_number *equalization_delay_)
{
    bdmf_number _nn_;
    int _rc_;
    _rc_ = bdmf_attr_get_as_num(mo_, rdpa_gpon_attr_initial_equalization_delay, &_nn_);
    *equalization_delay_ = (bdmf_number)_nn_;
    return _rc_;
}
