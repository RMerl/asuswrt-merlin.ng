...
typedef bdmf_boolean PON_SA_PLOAM_VAL;
...
typedef struct 
{
    ...
    uint32_t fs_hec_errors;     /**< FS HEC error count (HLEND)*/
    uint32_t fs_fixed_hec_errors;     /**< FS fixed HEC error count (HLEND) */
    uint32_t bwmap_hec_errors;        /**< BWMAP HEC error count */
    uint32_t bwmap_fixed_hec_errors;  /**< BWMAP fixed HEC error count */
    ...
    uint32_t rx_ranging_in_o5;  /**< Rx ranging PLOAMs in O5 counter */
    uint32_t rx_ranging_adj;    /**< Rx ranging adjustment PLOAMs counter (GPON) */
    ...
    uint32_t del_pass_pkt_counter; /**< XGEM packets with non zero PLI before Port-ID filtering */

    uint32_t success_act;       /**< successful activations counter*/
    uint32_t rogue_onu_counter; /**< rogue onu event counter */
} rdpa_gpon_stat_t;
...
typedef enum
{
    gpon_ploam_type_poll, /**< Dying Gasp */
