...
#define proto_log_debug bcmLog_logIsEnabled(BCM_LOG_ID_RDPA, BCM_LOG_LEVEL_DEBUG)
#define proto_debug(fmt, arg...) BCM_LOG_DEBUG(BCM_LOG_ID_RDPA, fmt, ##arg)
#define proto_info(fmt, arg...) BCM_LOG_INFO(BCM_LOG_ID_RDPA, fmt, ##arg)
...
#define TAGS_REMARK(tags, r) ((uint8_t)(tags + TAGS_MAX) | (r) << 8)
    ...
    proto_debug("num_tags_diff: %d tags_first_offset: %d num_tags_in_filter: %d, remark %x", num_tags_diff, tags->tags_first_offset, num_tags_in_filter, remark);
    ...
    switch (TAGS_REMARK(num_tags_diff, remark))
    {
        ...
        default:
            proto_info("vlan_action not supported remark<0x%x>", remark);
            if (proto_log_debug)
            {
                int i;

                for (i = 0; i < TAGS_MAX; i++)
                    proto_debug("%d) vid<%2d> pbit<%2d>", i, rdpa_mw_vlan_tag(tags, i)->vid, rdpa_mw_vlan_tag(tags, i)->pbit);
            }
            return -1;
    }
    ...
    proto_debug("result vlan cmd: %x", action->cmd);
    ...
    for (i = 0; i < blog_rule->actionCount; i++)
    {
        ...
        switch (cmd)
        {
            ...
            case BLOG_RULE_CMD_COPY_VID:
                proto_info("Copy VID action is not supported");
                break;
            case BLOG_RULE_CMD_COPY_PBITS:
                if ((blog_rule->action[i].toTag != 0) || (!rdpa_vtag_number_of_tags(tags)))
                {
                    proto_info("Copy pbit action not supported: number of VLAN tags = %d,"
                      "copy to VLAN tag %d", 
...
static int vlan_action_add(rdpa_vlan_action_cfg_t *action, rdpa_traffic_dir dir, bdmf_object_handle *vlan_action_obj)
{
    ...
    BDMF_MATTR_ALLOC(vlan_action_attr, rdpa_vlan_action_drv());
    ...
    BDMF_MATTR_FREE(vlan_action_attr);
#endif
    return rc;
}
...
exit:
    ...
    proto_debug("%s vlan_object %d", reuse ? "Reusing" : "Created", (int)idx);
    return 0;
}
#endif /* CONFIG_BLOG */
