...
void spu_blog_emit(struct spu_trans_req *pTransReq)
{
    ...
    if (SPU_DIRECTION_US == pTransReq->pSpuCtx->direction)
    {
        ...
        fcArgs.esp_ivsize = pTransReq->pSpuCtx->ivsize;
        fcArgs.esp_icvsize = pTransReq->pSpuCtx->icvsize;
        /* NOTE: impl3 only support blksize same as ivsize */
        fcArgs.esp_blksize = fcArgs.esp_ivsize;
        blog_emit_args(pTransReq->pNBuf, spuinfo->spu_dev_us, TYPE_IP, 0, BLOG_SPU_US, &fcArgs);
        pSkb->dev = skb_dev;
    ...
    else /* SPU_DIRECTION_DS */
    {
        ...
        fcArgs.esp_ivsize = pTransReq->pSpuCtx->ivsize;
        fcArgs.esp_icvsize = pTransReq->pSpuCtx->icvsize;
        blog_emit_args(pTransReq->pNBuf, spuinfo->spu_dev_ds, TYPE_IP, 0, BLOG_SPU_DS, &fcArgs);
        skb_pull(pSkb, BLOG_IPV4_HDR_LEN);
        pSkb->dev = skb_dev;
    }
}
...
static void spu_blog_fc_crypt_done_us(struct spu_trans_req *pTransReq)
{
   ...
   else
   {
      ...
      {
         /* PKT_DONE - packet was consumed */
         spuinfo->stats.encBlogged++;
      }
   }
...
static void spu_blog_fc_crypt_done_ds(struct spu_trans_req *pTransReq)
{
   ...
   else
   {
      ...
      icvsize = pTransReq->pSpuCtx->icvsize;
      fcArgs.esp_ivsize = pTransReq->pSpuCtx->ivsize;
      fcArgs.esp_icvsize = icvsize;
      ...
      if ( IS_SKBUFF_PTR(pTransReq->pNBuf) )
      {
         ...
         padlen  = pData[pktlen - icvsize - 2];
         nexthdr = pData[pktlen - icvsize - 1];
         headroom = 0;
      ...
      if ((blogAction == PKT_NORM) || (blogAction == PKT_BLOG))
      {
         ...
         hlen += BLOG_ESP_SEQNUM_LEN + pTransReq->pSpuCtx->ivsize;
         XFRM_SKB_CB(skb)->seq.input.low = seqno;
         XFRM_SKB_CB(skb)->seq.input.hi = seq_hi;

         /* trim padding and icv */
         __skb_trim(skb, skb->len - icvsize - padlen - 2);
      ...
      {
         spuinfo->stats.decBlogged++;
         hlen += BLOG_ESP_SEQNUM_LEN + pTransReq->pSpuCtx->ivsize;
         pktlen = pktlen - hlen - icvsize - padlen - 2;
...
static int spu_blog_xmit_us(pNBuff_t pNBuf, struct net_device *dev)
{
   ...
   do
   {
      ...
      icvsize = pCtx->icvsize;
      padlen  = pdata[nbuflen - icvsize - 2];
      datalen = nbuflen - BLOG_IPV4_HDR_LEN - padlen - pCtx->ivsize -
                BLOG_ESP_SPI_LEN - BLOG_ESP_SEQNUM_LEN - BLOG_ESP_PADLEN_LEN -
                BLOG_ESP_NEXT_PROTO_LEN - icvsize;
