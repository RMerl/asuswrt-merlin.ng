	...
	if (rctx->total_received == rctx->total_todo) {
		...
		atomic_inc(
		   &iproc_priv.cipher_cnt[ctx->cipher.alg][ctx->cipher.mode]);
        	...
        	/* implementing COP_FLAG_WRITE_IV for cryptodev */
		if ((ctx->cipher.mode == CIPHER_MODE_CBC) && rctx->iv_ctr_len
				&& (payload_len >= rctx->iv_ctr_len))
			sg_copy_part_to_buf(
				rctx->is_encrypt ? req->dst : req->src,
				req->info, rctx->iv_ctr_len,
				rctx->total_received - rctx->iv_ctr_len);
	}
}
 ...
 */
static int
spu_skcipher_rx_sg_create(struct bcmspu_message *mssg,
			    struct iproc_reqctx_s *rctx,
			    u8 rx_frag_num,
			    unsigned int chunksize, u32 stat_pad_len)
{
 ...
 */
static int
spu_skcipher_tx_sg_create(struct bcmspu_message *mssg,
			    struct iproc_reqctx_s *rctx,
			    u8 tx_frag_num, unsigned int chunksize, u32 pad_len)
{
	struct spu_hw *spu = &iproc_priv.spu;
...
static int handle_skcipher_req(struct iproc_reqctx_s *rctx)
{
	...
	u32 stat_pad_len;	/* num bytes to align status field */
	u32 pad_len;		/* total length of all padding */
	struct bcmspu_message *mssg;	/* mailbox message */
 ...
 *              any padding
 * @assoc_offset: offset of associated data from start of data sg
 *
 * The scatterlist that gets allocated here is freed in spu_chunk_cleanup()
 * when the request completes, whether the request is handled successfully or
 * there is an error.
...
int handle_aead_req(struct iproc_reqctx_s *rctx)
{
	...
	req_opts.bd_suppress = 0;
	req_opts.is_rfc4543 = 0;
	req_opts.ipsec_esn = rctx->esn_hdr;
	req_opts.seq_hi = rctx->seq_hi;
	...
	hash_parms.key_len = ctx->authkeylen;
	hash_parms.digestsize = digestsize;
	hash_parms.prebuf_len = 0;
	hash_parms.pad_len = 0;
	...
	err = spu_aead_tx_sg_create(mssg, rctx, tx_frag_num, spu_hdr_len,
				    ...
				    aead_parms.aad_pad_len, pad_len, incl_icv, assoc_offset);
	if (err)
		return err;
...
void handle_aead_resp(struct iproc_reqctx_s *rctx)
{
	...
	if (rctx->is_encrypt) {
		/* icv_offset needs to be adjusted by the difference between the src_skip and dst_skip,
		 * depending on how the iv is generated
		icv_offset = rctx->dst_skip + rctx->total_sent + (rctx->src_skip - rctx->dst_skip);
		 */
		icv_offset = rctx->total_sent + rctx->src_skip;
		packet_dump("  ICV: ", rctx->msg_buf.digest, ctx->digestsize);
		flow_log("copying ICV to dst sg at offset %u dst_skip %d src_skip %d total_sent %d\n",
			icv_offset, rctx->dst_skip, rctx->src_skip, rctx->total_sent);
...
static int ahash_init(struct ahash_request *req)
{
	...
	if (spu_no_incr_hash(ctx)) {
		...
		ctx->shash->tfm = hash;
		ctx->shash->flags = req->base.flags;
...
static int ahash_import(struct ahash_request *req, const void *in)
{
	...
	if (spu_no_incr_hash(ctx))
		return crypto_shash_import(ctx->shash, in);
...
static int aead_enqueue(struct aead_request *req, bool is_encrypt)
{
	...
	if (blog_enable && (req->base.flags & CRYPTO_TFM_REQ_MAY_BLOG) &&
	    rctx->is_encrypt) {
		if (spu_blog_emit_aead_us(rctx, 0)) {
			/* packet handled in the accelerated path
			 * and result will be returned asynchronously */
			spu_chunk_cleanup(rctx);
			return -EINPROGRESS;
		}
	} 
...
#else
			struct des_ctx dctx = {};
			if (des_expand_key(&dctx, keys.enckey, DES_KEY_SIZE) == 0) {
#endif
...
static int aead_esn_enqueue(struct aead_request *req, bool is_encrypt)
{
	struct iproc_reqctx_s *rctx = aead_request_ctx(req);
	struct crypto_aead *aead = crypto_aead_reqtfm(req);
	struct iproc_ctx_s *ctx = crypto_aead_ctx(aead);
	...
	scatterwalk_map_and_copy(tmp, req->src, 0, 12, 0);
	scatterwalk_map_and_copy(tmp, req->src, 4, 4, 1);

	spu_sg_at_offset(req->src, 4, &rctx->assoc, &skip);
	...
	if (blog_enable && (req->base.flags & CRYPTO_TFM_REQ_MAY_BLOG) &&
	    rctx->is_encrypt) {
		if (spu_blog_emit_aead_us(rctx, 1)) {
			/* packet handled in the accelerated path
			 * and result will be returned asynchronously */
			spu_chunk_cleanup(rctx);
			return -EINPROGRESS;
...
static int aead_esn_encrypt(struct aead_request *req)
{
	flow_log("%s() cryptlen:%u %08x\n", __func__, req->cryptlen,
		 req->cryptlen);
	dump_sg(req->src, 0, req->cryptlen + req->assoclen);
	...
	return aead_esn_enqueue(req, true);
}

static int aead_esn_decrypt(struct aead_request *req)
{
	flow_log("%s() cryptlen:%u\n", __func__, req->cryptlen);
	dump_sg(req->src, 0, req->cryptlen + req->assoclen);
	flow_log("  assoc_len:%u\n", req->assoclen);

	return aead_esn_enqueue(req, false);
}
...
static struct iproc_alg_s driver_algs[] = {
	 ...
	 },
	{
	 ...
	 .alg.aead = {
		 .base = {
			.cra_name = "authencesn(hmac(md5),cbc(des))",
			.cra_driver_name = "authenc-hmac-md5-cbc-des-iproc",
			.cra_blocksize = DES_BLOCK_SIZE,
			.cra_flags = CRYPTO_ALG_NEED_FALLBACK | CRYPTO_ALG_ASYNC | CRYPTO_ALG_ESN
		 },
	 ...
	 },
	{
	 ...
	 .alg.aead = {
		 .base = {
			.cra_name = "authencesn(hmac(sha224),cbc(des))",
			.cra_driver_name = "authenc-hmac-sha224-cbc-des-iproc",
			.cra_blocksize = DES_BLOCK_SIZE,
			.cra_flags = CRYPTO_ALG_NEED_FALLBACK | CRYPTO_ALG_ASYNC | CRYPTO_ALG_ESN
		 },
	 ...
	 },
	{
	 ...
	 .alg.aead = {
		 .base = {
			.cra_name = "authencesn(hmac(sha384),cbc(des))",
			.cra_driver_name = "authenc-hmac-sha384-cbc-des-iproc",
			.cra_blocksize = DES_BLOCK_SIZE,
			.cra_flags = CRYPTO_ALG_NEED_FALLBACK | CRYPTO_ALG_ASYNC | CRYPTO_ALG_ESN
		 },
	 ...
	 },
	{
	 ...
	 .alg.aead = {
		 .base = {
			.cra_name = "authencesn(hmac(sha512),cbc(des))",
			.cra_driver_name = "authenc-hmac-sha512-cbc-des-iproc",
			.cra_blocksize = DES_BLOCK_SIZE,
			.cra_flags = CRYPTO_ALG_NEED_FALLBACK | CRYPTO_ALG_ASYNC | CRYPTO_ALG_ESN
		 },
...
static int spu_register_aead(struct iproc_alg_s *driver_alg)
{
	...
	if ((aead->base.cra_flags & CRYPTO_ALG_ESN) && (spu->spu_type != SPU_TYPE_SPU2)) {
		pr_debug("ESN alg not supported on current spu type %d\n", spu->spu_type);
		return 0;
	}
	...
	if (aead->base.cra_flags & CRYPTO_ALG_ESN) {
		aead->encrypt = aead_esn_encrypt;
		aead->decrypt = aead_esn_decrypt;
	} else {
...
static int bcm_spu_probe(struct platform_device *pdev)
{
	...
	if (err < 0) {
		dev_err(dev, "dma_set_coherent_mask failure %d.\n", err);
		goto failure;	
	}
#if (LINUX_VERSION_CODE < KERNEL_VERSION(5,15,0))
