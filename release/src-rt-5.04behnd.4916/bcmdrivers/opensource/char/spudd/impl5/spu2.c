...
static void spu2_fmd_ctrl1_write(struct SPU2_FMD *fmd, bool is_inbound,
				 ...
				 bool return_payload, bool return_md, bool has_esn)
{
	u64 ctrl1 = 0;
			...
			bool is_encrypt)
{
	...
	if (is_encrypt && cipher_mode == CIPHER_MODE_GCM)
			...
			unsigned int data_size)
{
	...
	bool return_md = true, hash_iv = false;
	...
	if (req_opts->ipsec_esn)
		proto = SPU2_IPSEC_ESN;

	if (req_opts->is_aead)
		/*
		 * aead cipher first on encrypt, auth first on decrypt
		 */
	...
	if (!req_opts->is_inbound)
		hash_iv = (spu2_ciph_mode == SPU2_CIPHER_MODE_CBC);
	...
	spu2_fmd_ctrl1_write(fmd, req_opts->is_inbound, assoc_size,
			     ...
			     !req_opts->bd_suppress, return_md, req_opts->ipsec_esn);
	...
	if (req_opts->ipsec_esn) {
		memcpy(ptr, &req_opts->seq_hi, 4);
		ptr += 4;
		buf_len += 4;
	}
	if (cipher_parms->iv_len && cipher_parms->iv_buf) {
