...
#define SPU2_TEST_BATCHSIZE 127
...
static uint8_t *same_src_buffer[SPU2_MAX_CHANNEL][2] = {};
static uint8_t *same_big_buffer;
static dma_addr_t same_big_buffer_dma;
...
static struct timespec64 start_t, end_t;
...
extern int spu_test_same_ddr_bank;
...
static inline int spu2_select_next_test_chan(void)
{
	...
	uint8_t chan, numchan;
	get_random_bytes(&chan, 1);
	numchan = spu->spu_get_max_channel();
	return (chan % numchan);
#else
	static int count = 0;
	int chan = count++ & (spu->spu_get_max_channel() - 1);
	return chan;
#endif
}
...
static void spu2_test_callback(struct bcmspu_message *msg)
{
	...
	if (sample_size > SPU2_TEST_BATCHSIZE) {
		...
		div64_u64_rem(snapshot, 10000000, &remainder);
		if (0 == remainder )  {
			ktime_get_real_ts64(&end_t);
			time_to_tm(end_t.tv_sec, 0, &broken);
			printk("... %10lld of %lld processed at %d:%d:%d:%ld\n",
				snapshot, sample_size,
				broken.tm_hour, broken.tm_min, broken.tm_sec, end_t.tv_nsec);
		}
...
fail:
		...
		if (!atomic_read(&abort_test) &&
			...
			int next_idx = spu2_prep_request(chan);
			if (next_idx >= 0) {
	...
	if (completed == requested)
	{
		...
		if (sample_size <= SPU2_TEST_BATCHSIZE)
		{
			...
			if (atomic64_add_return(ATOMIC_READ(&num_response[chan]), &test_completed) == sample_size) {
				ktime_get_real_ts64(&end_t);
				...
				mbps =  div64_u64(mbps,  (last_packet_timestamp - first_packet_timestamp));
...
static int spu2_prep_request(uint8_t chan)
{
	...
	if (spu_test_same_src_buffer) {
		uint8_t *src_buffer = same_src_buffer[chan][idx&1];
		if (!src_buffer) {
			if (spu_test_dedicated_pool) {
				int offset;
				if (spu_test_same_ddr_bank)
					offset = 0;
				else
					offset = ((chan * 2) + (idx & 1)) * 2048;
				src_buffer = &same_big_buffer[offset];
			}
			else {
				src_buffer = (uint8_t *)kzalloc (2000, GFP_ATOMIC);;
			}
			if (!src_buffer) {
				printk("NO MEMORY!");
				return -ENOMEM;
			}
			printk("chan %d idx %d src_buff alloc 0x%px phy 0x%llx\n", chan, idx, src_buffer, (uint64_t)virt_to_phys(src_buffer));
			memcpy(src_buffer, test_buf, spu_hdr_len + BCM_HDR_LEN);
			if (HASH_ALG_NONE != hash_parms.alg) {
				memcpy(&src_buffer[spu_hdr_len + BCM_HDR_LEN],
					   assoc, aead_parms.assoc_size);
				memcpy(&src_buffer[spu_hdr_len + BCM_HDR_LEN + aead_parms.assoc_size],
					   src_data, chunksize+aead_parms.iv_len);
			} else {
				memcpy(&src_buffer[spu_hdr_len + BCM_HDR_LEN],
					   src_data, chunksize);
			}
			same_src_buffer[chan][idx&1] = src_buffer;
		}
		test_buff_src[chan][idx] = src_buffer;
	} else {
		if (spu_test_dedicated_pool) {
...
void spu2_test_raw_perf(long long test_size)
{
	...
	printk("\n");
	if ((test_size > SPU2_TEST_BATCHSIZE) && spu_test_check_output) {
		printk("Warning:\n"
			   "  The output of each cryto operation will be compared\n"
			   "  with the expected result on the fly, hence the measured\n"
			   "  throughput will be skewed. To get the true performance\n"
			   "  of the block, consider repeating the test with larger\n"
			   "  sample size and the 'check_output' option disabled\n"
			   "  (e.g. echo 0 > check_output).\n\n");
	}
	...
	if (spu_test_vector_id < 0 ||
		...
		if (spu_test_same_src_buffer) {
			...
			vec_id_sel = vec_id_sel % (sizeof(spu2_test_vectors) / sizeof(struct spu_test_vector_t));
			printk("Warning:\n"
				   "  Since test will be performed using the same source buffer,\n"
	...
	if (iproc_priv.test_src_pool == NULL) {
		...
		iproc_priv.test_src_pool =
			dma_pool_create("spu2_test_src_pool", &iproc_priv.pdev->dev, 2048*8, 64, 0);
		same_big_buffer = dma_pool_alloc(iproc_priv.test_src_pool,
					GFP_ATOMIC, &same_big_buffer_dma);
		if (same_big_buffer == NULL) {
			printk("NO MEMORY!");
			return;
		}
	}
	...
	if (spu_test_same_src_buffer) {
		if (spu_test_dedicated_pool) {
			memset(same_src_buffer, 0, sizeof(same_src_buffer));
		} else {
			int i, j;
			for (i=0; i < SPU2_MAX_CHANNEL; i++) {
				uint8_t *src_buffer;
				for (j=0; j < 2; j++) {
					src_buffer = same_src_buffer[i][j];
					if (src_buffer)
						kfree(src_buffer);
					same_src_buffer[i][j] = NULL;
				}
			}
		}
	}
	...
	for (i=0; i < spu->spu_get_max_channel(); i++)
	{
		...
		if (requested) {
			...
			if (spu->spu_send_data(i, test_msg[i])) {
				printk("chan #%lld spu_send_data failed!\n", i);
				return;
			}
