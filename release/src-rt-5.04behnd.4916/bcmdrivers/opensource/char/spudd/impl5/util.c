				...
				size_t count, loff_t *offp)
{
	...
	read_lock(&ipriv->ctxListLock[SPU_STREAM_US]);
	out_offset += scnprintf(buf + out_offset, out_count - out_offset,
				"US chan 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x\n",
				ipriv->blog_chan_bitmap[SPU_STREAM_US][0],
				ipriv->blog_chan_bitmap[SPU_STREAM_US][1],
				ipriv->blog_chan_bitmap[SPU_STREAM_US][2],
				ipriv->blog_chan_bitmap[SPU_STREAM_US][3],
				ipriv->blog_chan_bitmap[SPU_STREAM_US][4],
				ipriv->blog_chan_bitmap[SPU_STREAM_US][5],
				ipriv->blog_chan_bitmap[SPU_STREAM_US][6],
				ipriv->blog_chan_bitmap[SPU_STREAM_US][7]);
	{
		struct iproc_ctx_s *ctx;
		list_for_each_entry(ctx, &iproc_priv.ctxList[SPU_STREAM_US], entry)
		{
			out_offset += scnprintf(buf + out_offset, out_count - out_offset,
						"ctx spi 0x%x offload_id %d\n",
						ctx->spi, atomic_read(&ctx->offload_id));
		}
	}
	read_unlock(&ipriv->ctxListLock[SPU_STREAM_US]);

	read_lock(&ipriv->ctxListLock[SPU_STREAM_DS]);
	out_offset += scnprintf(buf + out_offset, out_count - out_offset,
				"DS chan 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x\n",
				ipriv->blog_chan_bitmap[SPU_STREAM_DS][0],
				ipriv->blog_chan_bitmap[SPU_STREAM_DS][1],
				ipriv->blog_chan_bitmap[SPU_STREAM_DS][2],
				ipriv->blog_chan_bitmap[SPU_STREAM_DS][3],
				ipriv->blog_chan_bitmap[SPU_STREAM_DS][4],
				ipriv->blog_chan_bitmap[SPU_STREAM_DS][5],
				ipriv->blog_chan_bitmap[SPU_STREAM_DS][6],
				ipriv->blog_chan_bitmap[SPU_STREAM_DS][7]);
	{
		struct iproc_ctx_s *ctx;
	...
	read_unlock(&ipriv->ctxListLock[SPU_STREAM_DS]);
#endif
...
int spu_test_same_ddr_bank = 0;
static ssize_t spu_debugfs_test_same_ddr_bank_write (struct file *file,
			    const char __user *user_buf, size_t count, loff_t *ppos)
{
	char buf[32];
	size_t buf_size;
	...
	buf[buf_size] = '\0';
	spu_test_same_ddr_bank = simple_strtol(buf, NULL, 10);
	return count;
}

static ssize_t spu_debugfs_test_same_ddr_bank_read (struct file *file,
			    char __user *user_buf, size_t count, loff_t *ppos)
{
	char *buf;
	ssize_t ret, out_offset = 0, out_count = 2048;
...
static const struct file_operations spu_debugfs_test_same_ddr_bank = {
	.owner = THIS_MODULE,
	.open = simple_open,
	.write = spu_debugfs_test_same_ddr_bank_write,
	.read = spu_debugfs_test_same_ddr_bank_read,
};

#endif

#if defined(CONFIG_BCM_SPU_HW_OFFLOAD)
int flow_offload_enable = 3; /* bit 0 for DS bit 1 for US */
static ssize_t spu_debugfs_offload_enable_write (struct file *file,
			    const char __user *user_buf, size_t count, loff_t *ppos)
{
	char buf[32];
	size_t buf_size;
	...
	buf[buf_size] = '\0';
	flow_offload_enable = simple_strtoul(buf, NULL, 10);
	printk("setting flow_offload_enable to %d\n", flow_offload_enable);
	return count;
}

static const struct file_operations spu_debugfs_offload_enable = { 
	.owner = THIS_MODULE,
	.open = simple_open,
	.write = spu_debugfs_offload_enable_write,
};

extern ssize_t spu_offload_debug_info(char *buf, ssize_t buf_size);

static ssize_t spu_debugfs_offload_session_info(struct file *file, char __user *ubuf,
						size_t count, loff_t *offp)
{
	char *buf;
	ssize_t ret, out_offset, out_count = 4096;

	buf = kmalloc(out_count, GFP_KERNEL);
	if (!buf)
		return -ENOMEM;

	out_offset = spu_offload_debug_info(buf, out_count);
...
static const struct file_operations spu_debugfs_offload_sessions = {
	.owner = THIS_MODULE,
	.open = simple_open,
	.read = spu_debugfs_offload_session_info,
};
...
void spu_setup_debugfs(void)
{
	...
	if (iproc_priv.spu.spu_test_raw_perf)
	{
		...
		if (!iproc_priv.debugfs_test_same_ddr_bank)
			/* Create file with permissions S_IRUSR */
			debugfs_create_file("same_ddr_bank", 0600, iproc_priv.debugfs_test_dir,
					    &iproc_priv, &spu_debugfs_test_same_ddr_bank);
	}
#endif
#if defined(CONFIG_BCM_SPU_HW_OFFLOAD)
	debugfs_create_file("offload_enable", 0200, iproc_priv.debugfs_dir,
			&iproc_priv, &spu_debugfs_offload_enable);
	/* Create file with permissions S_IRUSR */
	debugfs_create_file("offload_sessions", 0400, iproc_priv.debugfs_dir,
			    &iproc_priv, &spu_debugfs_offload_sessions);
#endif
}
