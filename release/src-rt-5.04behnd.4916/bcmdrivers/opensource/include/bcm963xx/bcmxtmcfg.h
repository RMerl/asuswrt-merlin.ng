...
typedef struct XtmHwInfo
{
    UINT32   sar_clk_freq;
    void     *sar_virt_base;
    void     *txpaf_virt_base;
    UINT32   sar_phy_base;
}XTM_HW_INFO, *PXTM_HW_INFO;
...
#define XTM_PORT_SHAPING_OFF   0
#define XTM_PORT_SHAPING_ON    1
...
#define XTM_THRESHOLD_PARM_GET  2   /* Print existing parameter value to console */
        ...
        UINT32  mgfastParam;    /* Indicates threshold for MGFAST */
    } sParams;
    ...
    UINT32        mgfastThreshold;
} XTM_THRESHOLD_PARMS, *PXTM_THRESHOLD_PARMS;


#define XTM_Q_SHAPING_OFF   0
#define XTM_Q_SHAPING_ON    1
...
typedef struct XtmXtmPStatus 
{
   UINT32 ulPendUpEventPort[MAX_BOND_PORTS] ;
   UINT32 ulPendDownEventPort[MAX_BOND_PORTS] ;
   UINT32 ulPendInUseEventPort[MAX_BOND_PORTS] ;
   UINT32 ulSRAEvents[MAX_BOND_PORTS] ;
   UINT32 ulSRAEventsSkipped[MAX_BOND_PORTS] ;
} XTM_XTMP_STATUS, *PXTM_XTMP_STATUS ;
...
typedef enum SarRegCategory {
    ...
    TMUEXT_REGS,
    ALL,
} XTM_SAR_REG_CATEGORY, *PXTM_SAR_REG_CATEGORY;

typedef enum SarDbgRegCategory {
    DBG_MIB_REGS = ALL + 1,
    DBG_TXBBH_FIFO,
    DBG_TXPKT_FIFO,
    DBG_TXUTO_FIFO,
    DBG_MAX,
} XTM_SAR_DBG_REG_CATEGORY, *PXTM_SAR_DBG_REG_CATEGORY;
...
typedef struct 
{
    ...
    UINT32 baseAddr;
    UINT32 numRegs;
} XTM_SAR_REG_DUMP, *PXTM_SAR_REG_DUMP; 
...
BCMXTM_STATUS BcmXtm_Initialize( PXTM_INITIALIZATION_PARMS pInitParms, PXTM_HW_INFO hwInfo );
#if defined(CONFIG_BCM_55153_DPU)
BCMXTM_STATUS BcmXtm_Initialize_DPU( PXTM_INITIALIZATION_PARMS pInitParms, PXTM_HW_INFO hwInfo );
...
BCMXTM_STATUS BcmXtm_AccessSAR( UINT8 bSarIO );
BCMXTM_STATUS BcmXtm_SarRegDump(UINT32 regCategory, void *regBuff, UINT32 *regBase, UINT32 *numRegs);
BCMXTM_STATUS BcmXtm_GetXTMPStatus( PXTM_XTMP_STATUS pXTMPStatus );
...
#define XTM_LINK_MODE_MGFAST                  0x10

#define XTM_CHECK_GFAST_MODES(linkmode)       ((linkmode)&(XTM_LINK_MODE_GFAST|XTM_LINK_MODE_MGFAST))
...
#define XTM_MGFAST_QUEUE_DROP_THRESHOLD      1024    /* MGFAST PTM, PTM_BONDED     */

#if defined(CONFIG_BCM963178)
/* This limit is associated with the ARCHER IUDMA ring size of XTM Transmit
   channels. If the ARCHER XTM TX IUDMA ring size is changed make sure the
   following definition is matched with that size. */
#define XTM_TX_QUEUE_SIZE_MAX                 512
#endif   //CONFIG_BCM63178
...
#if !defined(CONFIG_BCM963146)
#define XTM_MAX_MBS                           16383 /* Ref Errata - 158/178 */
#else
#define XTM_MAX_MBS                           32768 /* Ref Errata - 158/178 */
#endif

#define XTM_MIN_MBS                               0 /* Ref Errata - 158/178 */
...
#define XTM_MGFAST_BONDING_DELAY               96    /* MGFAST PTM_BONDED */
