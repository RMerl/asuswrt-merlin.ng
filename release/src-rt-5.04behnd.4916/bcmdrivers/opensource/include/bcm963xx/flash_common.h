...
*/                       
/*!\file flash_common.h
 * \brief This file contains definitions and prototypes used by kernel.
 *
 */
...
#define UBOOT_ENV_ADD_SUCCESS 0
#define UBOOT_ENV_ADD_FAILED -1
#define UBOOT_ENV_ADD_ALREADY_PRESENT -2
#define UBOOT_ENV_ADD_IVALID_FORMAT -3
#define UBOOT_ENV_NO_SPACE_LEFT -4
#define UBOOT_ENV_ADD_NOT_SUPPORTED -5
int uboot_env_add(char *param_value); //parameter=value formatted input
...
static inline int envram_add_locked(char *param_value)
{
   int ret=0;
   lock_inMemNvramData_mutex();
   ret=uboot_env_add(param_value);
   unlock_inMemNvramData_mutex();
   return ret;
}
