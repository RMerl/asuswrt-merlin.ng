...
typedef struct
{
    uint32_t has_sock_fd:1;
    uint32_t reserved:31;
    int sock_fd;
} __attribute__ ((packed)) udp_spdt_stream_options_t;

typedef struct
{
    union {
        udp_spdt_stream_options_t udp_options;
        /* Currently not in use for TCP */
    } proto;
} __attribute__ ((packed)) spdt_stream_options_t;
