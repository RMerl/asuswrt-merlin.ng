...
struct bca_legacy_led legacy_leds[] = {
    ...
    {.consumer_name = "usb1-led", .led_name = kLedUSB1, .led_type = kLedOK},
    {.consumer_name = "usb2-led", .led_name = kLedUSB2, .led_type = kLedOK},
...
static int bca_led_ctrl(BOARD_LED_NAME led_name, BOARD_LED_STATE led_state)
{
    ...
    if (!led)
        return -ENODEV;
    if(led->brightness_set == NULL || led->blink_set == NULL)
        return -ENODEV;
    ...
    case kLedStateOn:
        ...
        break;
    case kLedStateFail:
        if(led_fail && led_fail->brightness_set != NULL )
        {
