...
/*
 * Clock driver for bcm63158 based devices
 *
...
#define I2S_CLKDENOM_CFG                 ( 0x0070 ) /* 0x70 -- 0x20f0 clk divider Denominator  */
#define I2S_CLKNUMER_CFG                 ( 0x0074 ) /* 0x74 -- 0x20f4 clk numerator */
#define I2S_MCLK_DISABLE                 ( 0x0078 ) /* 0x78 -- 0x20f8 mclk disable */

#define I2S_TX_MCLK_RATIO_MASK           0x00F00000
#define I2S_TX_MCLK_RATIO_SHIFT          20
#define I2S_TX_SCLKS_PER_1FS_DIV32_SHIFT 4 
#define I2S_TX_64BITS_PERFRAME           ( 2<<I2S_TX_SCLKS_PER_1FS_DIV32_SHIFT )
#define I2S_TX_SCLKS_PER_1FS_DIV32_MASK  ( 0x0f<<I2S_TX_SCLKS_PER_1FS_DIV32_SHIFT )
#define CLK_DENOM                        0x0000FFFF

#define CLK_DENOM_MASK                   0xFFFFFFFF
#define CLK_NOMIN_MASK                   0xFFFFFFFF
#define CLK_NUMERATOR(x,y)              ( y <= 48000 ? ( x*CLK_DENOM/1024/y ):( x*CLK_DENOM/128/y ) )/* based on denominator=65535 */




struct clk_i2s_data {
  struct regmap *regmap_reset;
  struct regmap *regmap_numer;
  struct regmap *regmap_denom;
...
/* actual clock=sample_rate*64=(parent_rate*(numerator/dinominator))/(mclk_ratio*2)  */
static int bcm_i2s_clk_set_rate(struct clk_hw *hw, unsigned long rate, unsigned long parent_rate)
{
   ...
   regmap_update_bits(pdata->regmap_i2scfg,I2S_TX_CFG, I2S_TX_SCLKS_PER_1FS_DIV32_MASK, I2S_TX_64BITS_PERFRAME ); 
   regmap_update_bits(pdata->regmap_i2scfg,I2S_TX_CFG, I2S_TX_MCLK_RATIO_MASK, 
                                           (rate <= 48000 ? 8 : 1) << I2S_TX_MCLK_RATIO_SHIFT);    
   regmap_update_bits(pdata->regmap_i2scfg,I2S_CLKDENOM_CFG, CLK_DENOM_MASK,CLK_DENOM);
   regmap_update_bits(pdata->regmap_i2scfg,I2S_CLKNUMER_CFG, CLK_NOMIN_MASK,CLK_NUMERATOR(parent_rate,rate));
...
static unsigned long bcm_i2s_clk_recalc_rate(struct clk_hw *hw, unsigned long parent_rate)
{
   ...
   mclk_rate = ( mclk_rate & I2S_TX_MCLK_RATIO_MASK ) >> I2S_TX_MCLK_RATIO_SHIFT;

   return ( parent_rate * CLK_DENOM )/( 128*mclk_rate )/numerator ;
...
static void __init bcm_i2s_clk_init(struct device_node *node)
{
   ...
   data->regmap_reset = syscon_regmap_lookup_by_phandle(node, "clk-reset-syscon");
   if (IS_ERR(data->regmap_reset)) {
      goto out ;
   }

   data->regmap_numer = syscon_regmap_lookup_by_phandle(node, "clk-numerator-syscon");
   if (IS_ERR(data->regmap_numer)) {
      goto out ;
   }

   data->regmap_denom = syscon_regmap_lookup_by_phandle(node, "clk-denominator-syscon");
   if (IS_ERR(data->regmap_denom)) {
      goto out ;
   }
