...
#define I2S_TX_256BITS_PERFRAME		( 0x08 << I2S_TX_SCLK_1FS_DIV32_SHIFT )
...
#define I2S_RX_MCLK_COFF		2
#define I2S_RX_MCLK_RATIO		1

#define I2S_RX_CLKDENOM_CFG		( 0x0078 ) /* 0x78 -- 0x20F8  */
#define I2S_RX_CLKNUMER_CFG		( 0x007C ) /* 0x7C -- 0x20FC  */
...
#define CLK_DIVIDE(x,y) div_u64(div_u64((uint64_t)x*CLK_DENOM,\
		TOTAL_CH*SAMPLE_DEPTH*I2S_RX_MCLK_RATIO*I2S_RX_MCLK_COFF),\
		y)
...
/* parent_rate *(65535/divide) = mclk
	sclk = mclk/(mclk_datio*2)
	sclk = sample_rate * total_ch * sample_depth */
/* divide = parent_rate*65535/
(total_ch * sample_depth * mclk_ratio*mclk_coff)/sample_rate*/
				...
				unsigned long parent_rate)
{
	...
	regmap_update_bits(pdata->regmap_i2scfg, I2S_RX_CFG,
			   I2S_TX_SCLKS_PER_1FS_DIV32_MASK,
			   I2S_TX_256BITS_PERFRAME );
	regmap_update_bits(pdata->regmap_i2scfg,I2S_RX_CFG,
			   I2S_RX_MCLK_RATIO_MASK,
			   I2S_RX_MCLK_RATIO << I2S_RX_MCLK_RATIO_SHIFT);
	regmap_update_bits(pdata->regmap_i2scfg, I2S_RX_CLKDENOM_CFG,
			   CLK_DENOM_MASK,
			   CLK_DENOM);

	regmap_update_bits(pdata->regmap_i2scfg, I2S_RX_CLKNUMER_CFG, 
			   CLK_NOMIN_MASK,
			   CLK_DIVIDE(parent_rate,rate));
					     ...
					     unsigned long parent_rate)
{
	...
	regmap_read(pdata->regmap_i2scfg,I2S_RX_CLKNUMER_CFG, &numerator);

	regmap_read(pdata->regmap_i2scfg,I2S_RX_CFG, &mclk_rate);
	mclk_rate = ( mclk_rate & I2S_TX_MCLK_RATIO_MASK ) >>
		    I2S_TX_MCLK_RATIO_SHIFT;

	/* ( parent_rate * CLK_DENOM )/( 8*32*2*mclk_rate )/numerator */
	return div_u64(div_u64((uint64_t)parent_rate*CLK_DENOM,
		(TOTAL_CH*SAMPLE_DEPTH*I2S_RX_MCLK_COFF*mclk_rate)),
		numerator);
}
...
static struct clk *bcm_i2s_rx_clk_register(struct device *dev,
					const char *name,
...
void bcm_i2s_rx_clk_init(struct device_node *node)
{
	...
	of_property_read_string(node, "clock-rx-output-names", &clk_name);
	...
	clk = bcm_i2s_rx_clk_register(NULL, clk_name, parent_name, data);
	if (!IS_ERR(clk)) {
...
static void __init of_bcm_i2s_rx_clk_init(struct device_node *node)
{
   bcm_i2s_rx_clk_init(node);
}
CLK_OF_DECLARE(bcm_gate_clk, "brcm,i2s-rx-clock", of_bcm_i2s_rx_clk_init);
