...
static int bcm63xx_cpufreq_cpu_init(struct cpufreq_policy *policy)
{
	...
	struct device *cpu_dev;
	struct clk *cpu_clk;
	unsigned int transition_latency = 0, ret = 0;
	...
	ret =
#endif
		cpufreq_generic_init(policy, bcm63xx_freq_table, transition_latency);
...
void bcm_cpufreq_set_freq_max(unsigned maxdiv)
{
	...
	if (data) {
		max = data->cpuinfo.max_freq / (maxdiv ?: 1);
#if (LINUX_VERSION_CODE < KERNEL_VERSION(5,15,0))
		data->user_policy.max = max;
#else
		freq_qos_update_request(data->max_freq_req, max);
#endif
