...
typedef union {
	struct {
		uint32_t temp_threshold_hi:10;  // [09:00]
	  	uint32_t temp_threshold_lo:10;  // [19:10]
	  	uint32_t irq_enable_hi:1;       // [20]
	  	uint32_t irq_enable_lo:1;       // [21]
		uint32_t therm_ctl:6;           // [27:22]
...
typedef union {
	struct {
		uint32_t temp_threshold_hi:10;  // [09:00]
	  	uint32_t temp_threshold_lo:10;  // [19:10]
		uint32_t misc_irq_enable:1;     // [20]
		uint32_t reserved:11;           // [31:21]
	} Bits;
	uint32_t Reg32;
} TMON_THRESHOLD_CFG_REG;

typedef union {
	struct {
	  	uint32_t temp:10;               // [09:00]
		uint32_t reserved:21;           // [30:10]
		uint32_t valid:1;               // [31]
	} Bits;
	uint32_t Reg32;
} TMON_STATUS_REG;
...
typedef struct {
	...
	BPCM_CAPABILITES_REG capabilities;      // offset = 0x04
	BPCM_LINK_ADDR_REG link_address;        // offset = 0x08
	BPCM_SPARE_REG spare;                   // offset = 0x0c
	TMON_CTL_CFG_REG control;               // offset = 0x10
	TMON_THRESHOLD_CFG_REG threshold;       // offset = 0x14
	TMON_STATUS_REG status;                 // offset = 0x18
} TMON_BPCM_REGS;

#define TMONTEMP(status)         (45000000 - (54956 * status.Bits.temp))/100
#define TMONBPCMOffset(reg)      offsetof(TMON_BPCM_REGS, reg)
#define TMONBPCMRegOffset(reg)   (TMONBPCMOffset(reg) >> 2)
