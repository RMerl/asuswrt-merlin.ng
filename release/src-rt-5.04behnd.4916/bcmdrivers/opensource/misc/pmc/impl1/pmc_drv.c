...
#endif // #if defined(PMC_IMPL_3_X) || defined(PMC_ON_HOSTCPU) || defined(PMC_DIRECT_MODE_ONLY)
...
int GetRCalSetting(int resistor, int *rcal)
{
	...
	return rc;
#else
    printk("%s: SW RCAL is not needed on this platform\n", __func__);
    return kPMC_INVALID_COMMAND;
#endif
}
...
#else
    printk("%s: SW RCAL_1UM_VERT is not needed on this platform\n", __func__);
    return kPMC_INVALID_COMMAND;
#endif
}
...
int PowerOnDevice(int devAddr)
{
	if (PMC_ACCESS_BPCM_DIRECT || pmc_mode == PMC_MODE_PMB_DIRECT) {
		...
		for (ix = 0; status == kPMC_NO_ERROR && ix < capabilities.Bits.num_zones; ix++) {
...
int PowerOffDevice(int devAddr, int repower)
{
	if (PMC_ACCESS_BPCM_DIRECT || pmc_mode == PMC_MODE_PMB_DIRECT) {
		...
		for (ix = capabilities.Bits.num_zones - 1; status == kPMC_NO_ERROR && ix >= 0; ix--) {
...
#elif defined(PMC_IMPL_3_X)
	{
		volatile MaestroMisc *maestro;
		if (early_call)
			maestro = ioremap(EARLY_MAESTRO_BASE, EARLY_MAESTRO_SIZE);
		else
			maestro = (volatile MaestroMisc *)g_pmc->maestro_base;
...
int pmc_get_tmontemp(int devAddr, int *millitemp)
{
#if defined(PMB_TMON)
  	TMON_STATUS_REG tmon_status;
        int rc;

	rc = ReadBPCMRegister(devAddr, TMONBPCMRegOffset(status), &tmon_status.Reg32);
        if (rc != kPMC_NO_ERROR) {
                printk("TMON read error %x\n", rc);
		return -1;
	}

	if (tmon_status.Bits.valid) {
		*millitemp = TMONTEMP(tmon_status);
		return 0;
	}
#else
	printk("TMON not available\n");
#endif
	return -1;
}
