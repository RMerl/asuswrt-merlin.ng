...
{
    if (request_irq(virq, pmc_isr, 0, "pmc_mbox", NULL))
        pr_err("ERROR: failed to configure PMC interrupt\n");
}

static irqreturn_t pmc_temp_warn_isr(int irq, void *dev_id)
{
#if defined(PMB_ADDR_PVTMON)
	uint32_t val;
...
{
    if (request_irq(virq, pmc_temp_warn_isr, 0, "pmc_temp_warn", NULL))
...
static void pmc_init_rescal(void)
{
	/* only 28nm chip need HW RESCAL. This include 4908, 63158, 6756. 
	 * 16nm chips has RESCAL enabled by default. The old 28nm chip like 
	 * 63138, 63148 need call GetRCalSetting to read resister value
	 * and program to serdes/pcie manually
	 */
#ifdef PMC_RESCAL_SUPPORT
	void __iomem *rescal_base = NULL;
	uint32_t value;

	/* only do calibration when chip enable rescal feature */
	bcm_otp_is_rescal_enabled(&value);
	if (!value)
		return;

	rescal_base = ioremap(RESCAL_CTRL_REG_BASE, RESCAL_CTRL_REG_SIZE);
	if (!rescal_base)
		return;

	/* start the resistor calibrator by setting RSTB and then clearing the PWRDN bit */
	value = readl(rescal_base + RESCAL_CTRL_CFG_OFFSET);
	value |= RESCAL_RSTB;
	writel(value, rescal_base + RESCAL_CTRL_CFG_OFFSET);
	udelay(10);
	...
	writel(value, rescal_base + RESCAL_CTRL_CFG_OFFSET);
	udelay(10);
#ifdef RESCAL_CTRL_STATUS_OFFSET
	do {
		value = readl(rescal_base + RESCAL_CTRL_STATUS_OFFSET);
	} while(!(value&RESCAL_DONE));
#endif
