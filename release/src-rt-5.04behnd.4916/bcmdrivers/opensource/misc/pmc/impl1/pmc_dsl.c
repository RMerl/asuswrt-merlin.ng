...
int pmc_dsl_power_up(void)
{
	...
	ubus_master_remap_port(UBUS_PORT_ID_DSLCPU);
	ubus_master_remap_port(UBUS_PORT_ID_DSL);
#endif
...
int pmc_dsl_core_reset(void)
{
   ...
   if (g_powered_up)
      printk("=====>New %s %d dsl powered up (tentative)\n", __FUNCTION__, __LINE__);
   else
      printk("=====>New %s %d dsl not powered up\n", __FUNCTION__, __LINE__);
#endif

	/* apply the ubus setting. dsl reset core clear them on some chip like 63158 */
#if IS_BCMCHIP(63158)
	apply_ubus_credit_each_master(UBUS_PORT_ID_DSLCPU);
	apply_ubus_credit_each_master(UBUS_PORT_ID_DSL);
#endif
