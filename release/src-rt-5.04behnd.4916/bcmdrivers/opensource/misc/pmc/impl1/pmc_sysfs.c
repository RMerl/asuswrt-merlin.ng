...
static ssize_t power_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
        ...
        rc = ReadBPCMRegister(pmc->devno, BPCMZoneStsRegOffset(zone), &sts.Reg32);
        if (rc)
                return sprintf(buf, "name %s, dev %d, error %x\n", pmc->attr.name, pmc->devno, rc);
        if (sts.Reg32 == 0xdeaddead)
                return sprintf(buf, "%x\n", (unsigned) sts.Reg32);
        return sprintf(buf, "%d\n", sts.Bits.pwr_on_state);
}
...
static ssize_t volts_set (struct kobject *kobj, struct kobj_attribute *kattr, const char *buf, size_t count)
{
        ...
        if(kstrtouint (s_addr, 0, &address))
                printk("volts_set: scanning address failed\n");
        if(kstrtouint (s_val, 0, &value))
                printk("volts_set: scanning value failed\n");
        pvtmon = (pvtmon_regs *)ioremap(address, 0x100);
...
static ssize_t
pmc_cmd_help(struct kobject *kobj, struct kobj_attribute *kattr, char *buf)
{
    return sprintf(buf,
        "Usage: echo 0x<cmdw0> [0x<cmdw1> [0x<cmdw2> [0x<cmdw3>]]] > pmc_cmd\n");
}

static ssize_t
pmc_cmd_set(struct kobject *kobj, struct kobj_attribute *kattr,
                const char *buf, size_t count)
{
        unsigned int status, cmd[4] = { 0 }, rsp[4] = { 0 };

        if (sscanf(buf, "0x%x 0x%x 0x%x 0x%x",
                    &cmd[0], &cmd[1], &cmd[2], &cmd[3]) < 1) {
                pr_err("Error: invalid cmd words\n");
                return -EINVAL;
        }

	status = SendAndWait((TCommand *)cmd, (TCommand *)rsp);
	printk("cmd.word[0-3]=0x[%08x %08x %08x %08x] status=%u "
               "rsp.word[0-3]=0x[%08x %08x %08x %08x]\n",
               cmd[0], cmd[1], cmd[2], cmd[3], status,
               rsp[0], rsp[1], rsp[2], rsp[3]); 
        return count;
}
#if defined(PMB_TMON)
static ssize_t tmon_show(struct kobject *kobj, struct kobj_attribute *kattr, char *buf)
{
        struct bpcm_attribute *pmc = (struct bpcm_attribute *) kattr;
        int millitemp;

	if (pmc_get_tmontemp(pmc->devno, &millitemp))
		return sprintf(buf, "Wifi%dTemp: Invalid\n", pmc->regno);
	else
		return sprintf(buf, "Wifi%dTemp: %d.%03d C\n", pmc->regno, millitemp / 1000, abs(millitemp) % 1000);
}
#endif
...
#endif
        { { "pmc_cmd",    0644, }, pmc_cmd_help,      pmc_cmd_set, 0, 0, 0, 0 },
#if defined(PMB_TMON)
        { { "select12",    0644, }, tmon_show,     NULL, PMB_ADDR_WIFI0_TMON, 0, 0, 0 },
	{ { "select13",    0644, }, tmon_show,     NULL, PMB_ADDR_WIFI1_TMON, 0, 0, 1 },
#endif
};
...
static ssize_t avs_status_show(struct kobject *kobj, struct kobj_attribute *kattr, char *buf)
{
        int rc, avs_disabled = 1;

        rc = GetAvsDisableState(0, &avs_disabled);
        if (rc != kPMC_NO_ERROR)
                pr_err("failed to get avs %s, error=%d, assuming it's disabled\n",
                                kattr->attr.name, rc);
        return sprintf(buf, "%d\n", avs_disabled ? 0 : 1);
}

static struct kobj_attribute avs_attrs[] = {
        { { "status",    0444, }, avs_status_show,    NULL          },
};

static int __init pmc_sysfs_init(void)
{
        struct bpcm_attribute *pmcattr;
        struct kobject *bpcmkobj, *avs_kobj;
        unsigned d;
        int error;
...
#if defined(PMC_IMPL_3_X) || defined(PMC_ON_HOSTCPU) 
        if(sysfs_create_bin_file(bpcmkobj, &bin_attr_pmc_log) != 0)
	{
                pr_err("%s:%d failed sysfs_create_bin_file\n", __func__, __LINE__);
	}
#endif
        ...
        /* create avs subdirectory under bpcm */
        avs_kobj = kobject_create_and_add("avs", bpcmkobj);
        if (!avs_kobj) {
                pr_err("failed to create subdirectory avs under bpcm\n");
                return -ENOMEM;
        }
        d = ARRAY_SIZE(avs_attrs);
        while (d--) {
                struct attribute *pattr = &avs_attrs[d].attr;
                sysfs_attr_init(pattr);
                error = sysfs_create_file(avs_kobj, pattr);
                if (error) {
                        pr_err("failed to create avs attr %s, error=%d\n",
                                                pattr->name, error);
                        return error;
                }
        }
