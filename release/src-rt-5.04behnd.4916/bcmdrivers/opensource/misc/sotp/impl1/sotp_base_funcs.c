...
int sotp_get_info( SOTP_INFO_BLK * sotp_info_blk, int data_len, int * result )
{
	sotp_info_blk->version = SOTP_VERSION_SOTP;
	sotp_info_blk->max_rollback_lvl = SOTP_MAX_ROLLBACK_LVL;
	sotp_info_blk->num_words_in_keyslot = SOTP_NUM_WORDS_IN_KEYSLOT;
	*result = SOTP_S_KEY_SUCCESS;
	return 0;
}
...
int sotp_init ( void * base_ptr )
{
    ...
    if(sotp_base_ptr->sotp_otp_prog_ctrl == 0xDEADBEEF 
        && sotp_base_ptr->sotp_otp_ctrl_0 == 0xDEADBEEF)
    {
        printk("SOTP dev access not permitted. Not loading driver.\n");
        return -EPERM;
    }
