   ...
   
{
    ...
    if(master_addr)
       master_addr->decode_cfg.ctrl = val;
    else
       UBUS_REMAP_DEBUG_LOG("No node is found for port_id=%d \n", master_port_id);
    return 0;
}
EXPORT_SYMBOL(ubus_master_decode_cfg_set_ctrl);
...
unsigned int ubus_pcie_core_vs_portid[MAX_PCIE_CORES] = {
    ...
    0,
#endif
#if defined(UBUS_PORT_ID_PCIE2)
    UBUS_PORT_ID_PCIE2,
#else
    0,
#endif
#if defined(UBUS_PORT_ID_PCIE3)
    UBUS_PORT_ID_PCIE3,
#else
    0,
#endif
...
int ubus_master_decode_pcie_wnd_cfg(int core, int win, phys_addr_t base_addr, phys_addr_t remap_addr, unsigned int size)
{
#if defined(CONFIG_BCM_PCIE_SWAM)
    unsigned int master_port_id = 0, port_id = 0;
    unsigned int size_power_of_2 = 0;
    volatile MstPortNode *master_addr = NULL; 
    int ret = 0;
    u32 attributes;

    /* Convert PCIe cores to port_id's */
    if (core < MAX_PCIE_CORES) {
        master_port_id = ubus_pcie_core_vs_portid[core];
    }

    if (win < MAX_PCIE_CORES) {
        port_id = ubus_pcie_core_vs_portid[win];
        attributes = 0;
    } else {
        port_id = UBUS_PORT_ID_VPB;
        win = core;
        attributes = DECODE_CFG_CMD_DTA_MASK;
    }

    if ((master_port_id == 0) || (port_id == 0)) {
        printk("\x1b[35m Error: Unable to find port_id's core %d, master_port_id %d, win %d, port_id %d\x1b[0m\n",
           core, master_port_id, win, port_id);
        return -1;
    }
    ...
    if((win > 3) || (size_power_of_2 > 31)) {
        printk("\x1b[35m Parameters Error:  win[%d] size[%d] power_of_2[%d]\x1b[0m\n",
            win, size, size_power_of_2);
        return -1;
    }
    ...
    if((master_addr) && (size_power_of_2)) {
        ...
        attributes |= DECODE_CFG_ENABLE_ADDR_ONLY;

        master_addr->decode_cfg.window[win].base_addr =  (base_addr>>8);
        master_addr->decode_cfg.window[win].remap_addr = (remap_addr>>8);
        master_addr->decode_cfg.window[win].attributes = attributes;
...
EXPORT_SYMBOL(ubus_master_decode_pcie_wnd_cfg);
...
static int get_ddr_rate_width_idx(void)
{
    ...
    if (np && (val = of_get_property(np, "memcfg", NULL))) {
...
static int ubus_tokens_show(struct seq_file *m, void *v)
{
    int i = 1;

    seq_printf(m, "%9s | %s", " ", ubus_port_id_to_name(ub_mst_addr_map_tbl[0].port_id));
    while (ub_mst_addr_map_tbl[i].port_id != -1)
        seq_printf(m, " | %s", ubus_port_id_to_name(ub_mst_addr_map_tbl[i++].port_id));
    ...
    while (ub_mst_addr_map_tbl[i].port_id != -1)
    {
        ...
        seq_printf(m, "\n%9s", ubus_port_id_to_name(ub_mst_addr_map_tbl[i].port_id));
        while (ub_mst_addr_map_tbl[j].port_id != -1)
        {
            uint32_t port_id = ub_mst_addr_map_tbl[j].port_id;
            uint32_t field_len = strlen(ubus_port_id_to_name(ub_mst_addr_map_tbl[j].port_id));

            if (master_reg->token[port_id] > 256)
                seq_printf(m, " | %*s", field_len, "-");
            else
                seq_printf(m, " | %*d", field_len, master_reg->token[port_id]);
            j++;
        }
        i++;
    }
...
static int ubus_decode_show(struct seq_file *m, void *v)
{
    int i = 0;
...
static int create_ubus_proc(void)
{
    ...
    if(!proc_create_single(PROC_DIR "/" UBUS_DECODE_FILE, 0, NULL, ubus_decode_show))
        goto error;

    if(!proc_create_single(PROC_DIR "/" UBUS_TOKENS_FILE, 0, NULL, ubus_tokens_show))
        goto error;
...
int remap_ubus_masters_biu(void)
{
    ...
    while(ub_mst_addr_map_tbl[i].port_id != -1)
    {
        if (!ub_mst_addr_map_tbl[i].late_init)
        {
...
/* 
 * This function depends on fpm reserved memory info by
 * bcm_rsvmem driver. So defer it until rsvmem driver is initialized 
 */
#ifdef CONFIG_BCM_UBUS_DECODE_REMAP
subsys_initcall(remap_ubus_masters_biu);
#endif
