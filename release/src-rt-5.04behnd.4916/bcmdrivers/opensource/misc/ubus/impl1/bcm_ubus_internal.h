...
typedef struct ub_mst_addr_map_t {
    ...
    void __iomem  *base;
    /* 
     * This flag indicates a late init of the master port setting. Certain
     * blocks are powered off by the chip or their driver before reboot. 
     * If this flag is set, ubus driver will not initialize this port and the
     * correspoding driver should init it after power on the block.
     */
    int           late_init;
} ub_mst_addr_map_t;
