...
netdev_tx_t enet_xmit_ingress(pNBuff_t pNBuff, bdmf_object_handle port_obj, uint32_t flow_id)
{
    bdmf_boolean is_wan;
    rdpa_port_type type;    
    ...
    info.port_obj = port_obj;
    info.cpu_port = rdpa_cpu_host;
    info.drop_precedence = 0;
    info.flags = 0;

    if (rdpa_port_is_wan_get(port_obj, &is_wan))
        return BDMF_ERR_INTERNAL;

    if (rdpa_port_type_get(port_obj, &type))
        return BDMF_ERR_INTERNAL;

    if (is_wan && (type != rdpa_port_emac))
...
static inline struct sk_buff *rx_skb(FkBuff_t *fkb, enetx_rx_info_t *rx_info)
{
    ...
    nbuff_set_priority(skb, nbuff_get_priority(fkb));

    return skb;
}

static netdev_tx_t enet_xmit_egress(pNBuff_t pNBuff, bdmf_object_handle port_obj, uint32_t flow_id, int egress_queue)
{
    bdmf_boolean is_wan;
    rdpa_port_type type;    
    rdpa_cpu_tx_info_t info = {};
...
static int g_proxy_egress_flow_id, g_proxy_egress_egress_queue;
static int g_inject_flow_id;
static bdmf_object_handle g_inject_port_obj = NULL;
static bdmf_object_handle g_proxy_egress_port_obj = NULL;

static inline int proxy_egress(FkBuff_t *fkb, enetx_rx_info_t *rx_info)
{
    return enet_xmit_egress(fkb, g_proxy_egress_port_obj, g_proxy_egress_flow_id, g_proxy_egress_egress_queue);
}
...
static inline int rx_pkt_from_q(int hw_q_id, int budget)
{
    ...
    do
    {
        ...
        rcu_read_unlock();
        if (unlikely(!port) && (g_func == FUNC_PROXY_INGRESS))
        {
            printk("enet_ingress: %s: failed to get port\n", __FUNCTION__);
            _free_fkb(fkb);
            count++;
            continue;
        }
        ...
        switch (g_func)
        {
            case FUNC_PROXY_INGRESS:
                ...
                enet_xmit_ingress(skb, port->priv, rx_info.flow_id);
                }
                break;
            case FUNC_INJECT:
                skb = rx_skb(fkb, &rx_info);
            ...
            case FUNC_PROXY_EGRESS:
                ...
                break;
            default:
                printk("enet_ingress: %s: cmd not supported \n", __FUNCTION__);
                _free_fkb(fkb);
        }
...
static netdev_tx_t inject_xmit(struct sk_buff *skb, struct net_device *dev)
{
    ...
    rc = enet_xmit_ingress(skb, g_inject_port_obj, g_inject_flow_id);
    if (rc)
...
{
#define P(x) printk(x"\n")
    P("proxy_egress: Forward packets RXQ[1] -> TX egress <dest_port_name> <flow_id> <queue_id>");
    P("--------------------------------------------------------------");
    P("Example for proxy mirroring rx of eth1 to eth0:");
    P("  echo proxy_egress eth0 0 0 > /proc/driver/enet_ingress/cmd");
    P("  brctl delif br0 eth0.0");
    P("  bs /b/c system cpu_reason_to_tc[cpu_mirroring]=1");
    P("  bs /b/c system cfg={options=0x8}");
    P("  bs /b/c port/name=eth1 mirror_cfg={rx_dst_port={port/name=cpu}}");
    P("");
    P("");
    ...
    P("");
    P("");
    P("inject_ingress: Linux interface ingress -> TX ingress <rdpa_if> <src_port_name> <redirect>");
    P("proxy_inject: Forward RXQ[1] -> Linux interface ingress");
    P("--------------------------------------------------------------------");
    P("Example for injecting packets to from eth0:");
    P("  echo inject eth0 0 > /proc/driver/enet_ingress/cmd");
...
static int proc_cmd_inject(int argc, char *argv[])
{
    ...
    rc = rdpa_port_get(argv[1], &g_inject_port_obj);
    if (rc)
        goto Exit;
...
Exit:
    if (g_inject_port_obj)
        bdmf_put(g_inject_port_obj);

    if (rc)
        printk("Usage: inject <src_port_name> <flow_id>\n");
    return 0;
}

static int proc_cmd_proxy_egress(int argc, char *argv[])
{
    int flow_id, queue_id,  rc = -1;

    if (argc != 4)
        goto Exit;

    rc = rdpa_port_get(argv[1], &g_proxy_egress_port_obj);
    if (rc)
        goto Exit;
...
Exit:
    if (g_proxy_egress_port_obj)
        bdmf_put(g_proxy_egress_port_obj);

    if (rc)
        printk("Usage: proxy_egress <dest_port_name> <flow_id> <queue_id>\n");
...
int ptp_1588_cpu_send_sysb(bdmf_sysb sysb, rdpa_cpu_tx_info_t *info, char *ptp_header, enetx_port_t *enetx_port_p)
{
    return 0;
}
