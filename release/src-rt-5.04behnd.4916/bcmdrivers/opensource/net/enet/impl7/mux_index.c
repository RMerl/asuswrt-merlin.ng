...
static int mux_set_index(char *sw_name, enetx_port_t **map, int index, enetx_port_t *port)
{
    if (unlikely(index >= MUX_PORTS))
    {
        enet_err("cannot de/mux %s: %s:%d out of range\n",
            port->obj_name, sw_name, index);
        return -1;
    }

    if (port && rcu_dereference(map[index]))
    {
        enet_err("cannot de/mux %s: %s:%d already taken by %s\n",
            port->obj_name, sw_name, index, rcu_dereference(map[index])->obj_name);
        return -1;
    }
...
int mux_set_rx_index(enetx_port_t *sw, int index, enetx_port_t *port)
{
    ...
    if (!(rc = mux_set_index(sw->obj_name, sw->s.demux_map, index, port)))
    {
        if (!port)
        {
...
int mux_set_tx_index(enetx_port_t *from, enetx_port_t *sw, int index, enetx_port_t *to)
{
    ...
    if (!(rc = mux_set_index(sw->obj_name, sw->s.mux_map, index, to)))
    {
        if (!to)
        {
...
{
    if (unlikely(rx_info->src_port >= MUX_PORTS))
    {
        *out_port = NULL;
        return -1;
    }
