...
/* Add the TCP6/UDP6 pseudoheader addresses, basing on the existing checksum */

__sum16 csum_tcpudp_remagic_addrs(const struct in6_addr *saddr,
                                const struct in6_addr *daddr,
                                u16 csum) {
  u16 *pdata;
  int i;
...
/* Undo the IPv4 pseudoheader addresses inclusion into the checksum */
__sum16 csum_ipv4_unmagic_addrs(__be32 saddr, __be32 daddr,
                                       __sum16 csum) {
  u16 *pdata;
...
int xlate_payload6_to4(nat46_instance_t *nat46, void *pv6, void *ptrans_hdr, int v6_len, u16 *ul_sum, int *ptailTruncSize) {
  ...
  if (proto == NEXTHDR_FRAGMENT) {
    ...
    if(fh->frag_off == 0) {
      /* Atomic fragment */
      proto = fh->nexthdr;
      ipid = htons(ntohl(fh->identification) & 0xffff);
      v6_len -= 8;
...
int nat46_ipv6_input(struct sk_buff *old_skb) {
  ...
  if (proto == NEXTHDR_FRAGMENT) {
    ...
    if(fh->frag_off == 0) {
      ...
      frag_id = htons(ntohl(fh->identification) & 0xffff);
    ...
    u32 map_id = skbuff_bcm_ext_map_get(old_skb, map_id);
    if (map_id) {
      /* restore atomic fragment id - gone with the reassembly earlier */
      frag_id = htons(ntohl(map_id) & 0xffff);
    } else {
...
void ip6_update_csum(struct sk_buff * skb, struct iphdr * iph, struct ipv6hdr * ip6hdr, int do_atomic_frag)
{
  u32 sum1=0;
  u16 sum2=0;
  __sum16 oldsum = 0;
  ...
  switch (ip6hdr->nexthdr) {
    case IPPROTO_TCP: {
      ...
      oldsum = th->check;
      if (do_atomic_frag) {
        sum1 = csum_ipv4_unmagic_addrs(iph->saddr, iph->daddr, oldsum);
        sum2 = csum_tcpudp_remagic_addrs(&ip6hdr->saddr, &ip6hdr->daddr, sum1);
        th->check = sum2;
        break;
      }
    ...
    case NEXTHDR_ICMP: {
      ...
      sum1 = csum_partial((char*)icmp6h, icmp6len, 0); /* calculate checksum for ICMP hdr+payload */
...
int pairs_xlate_v4_to_v6_outer(nat46_instance_t *nat46, struct iphdr *hdr4, uint16_t *sport, uint16_t *dport, void *rule, void *v6saddr, void *v6daddr) {
  int ipair = 0;
  nat46_xlate_rulepair_t *apair = NULL;
  int xlate_src = -1;
  int xlate_dst = -1;
  ...
  if ( (xlate_src >= 0) && (xlate_dst >= 0) ) {
    memcpy(rule, &apair->local, sizeof(apair->local));
    return 1;
  }
...
int nat46_ipv4_input(struct sk_buff *old_skb) {
  ...
  if (check_for_l4) {
    ...
    __u32 map_id;
    int last_frag = !(hdr4->frag_off & htons(IP_MF)) ? 1 : 0;
    if (IPPROTO_ICMP == hdr4->protocol) {
      /* should not happen, we reassemble at beginning */
      goto done;
    }
    if (!restore_frag_l4_ports(hdr4->saddr, hdr4->daddr, hdr4->id, hdr4->protocol, &sport, &dport, &map_id, last_frag)) {
      /* never receive/forward first fragment, drop it */
      goto done;
    }
    skbuff_bcm_ext_map_get(old_skb, map_id) = map_id;
    having_l4 = 1;
  }

  if(!pairs_xlate_v4_to_v6_outer(nat46, hdr4, having_l4 ? &sport : NULL, having_l4 ? &dport : NULL, &rule, v6saddr, v6daddr)) {
  ...
  *(__be32 *)hdr6 = htonl(0x60000000 | (tclass << 20)) | flowlabel; /* version, priority, flowlabel */
  ...
  if (check_for_l4) {
    if (frag_id_rewrite) {
      /* RFC7599, section 10.3 */
      uint16_t v4_id = ntohs(hdr4->id);
      uint8_t psid_bits_len = rule.ea_len - (32 - rule.v4_pref_len);
      if (psid_bits_len) {
        uint16_t v6_id = 0;
        uint16_t psid = (ntohs(sport) >> (16 - psid_bits_len - rule.psid_offset)) & (0xffff >> (16 - psid_bits_len));
        if (rule.psid_offset) {
          v6_id = v4_id & (0xffff << (16 - rule.psid_offset));
          if (0 == v6_id) {
            int rand;
            while (1) { // we want to generate an integer between [1, 2^psid_offset - 1]
              get_random_bytes(&rand, sizeof(int));
              rand = (rand >= 0) ? rand : -rand;
              rand -= (rand >> rule.psid_offset) << rule.psid_offset;
              if (rand) {
                break;
              }
            }
            v6_id = rand << (16 - rule.psid_offset);
          }
          v6_id |= v4_id & (0xffff >> (psid_bits_len + rule.psid_offset));
        } else {
          v6_id = v4_id & (0xffff >> (psid_bits_len + rule.psid_offset));
          /* customer specific requirement, not for general use */
#if 0
          if ( (psid_bits_len < 6) && (v6_id < 1024) ) {
            v6_id = ((uint16_t)(~v6_id)) & (0xffff >> psid_bits_len);
          }
#endif
        }
        v6_id |= psid << (16 - psid_bits_len - rule.psid_offset); // use this packet id in fragmentation
        skbuff_bcm_ext_map_get(new_skb, map_id) = htonl(v6_id);
      } else {
        /* 1:1 mapping scenario */
        skbuff_bcm_ext_map_get(new_skb, map_id) = htonl(v4_id);
      }
    }
    if (add_frag_header) {
      __u32 map_id = skbuff_bcm_ext_map_get(new_skb, map_id);
      if (!backup_frag_l4_ports(hdr4->saddr, sport, hdr4->daddr, dport, hdr4->protocol, hdr4->id, map_id)) {
  ...
  if ((ntohs(hdr4->frag_off) & IP_DF)==0) {

  // FIXME: check if you can not fit the packet into the cached MTU
  // if (dst_mtu(skb_dst(new_skb))==0) { }

    skbuff_bcm_ext_map_get(new_skb, map_forward) = MAP_FORWARD_MODE2;
  } else {
    skbuff_bcm_ext_map_get(new_skb, map_forward) = MAP_FORWARD_MODE1;
  }
...
The below fragment list code is inspired by
...
void init_frag_list(time64_t timeout) {
  int i;
  spin_lock_init(&frag_list.lock);
  for (i = 0; i < NAT46_HTABLE_SIZE; i++) {
    INIT_HLIST_HEAD(&frag_list.chain[i]);
  }
  frag_list.size = 0;
  frag_list.timeout = timeout;
}

static int add_new_frag(__u32 v4saddr, __u16 sport, __u32 v4daddr, __u16 dport, __u16 proto, __u16 ipid, __u32 map_id) {
  frag_tuple_t *frag;
  int hash;
  frag = (frag_tuple_t *)kmalloc(sizeof(frag_tuple_t), GFP_ATOMIC);
  if (!frag) {
    return 0;
  }

  frag->v4saddr = v4saddr;
  frag->sport = sport;
  frag->v4daddr = v4daddr;
  frag->dport = dport;
  frag->proto = proto;
  frag->ipid = ipid;
  frag->map_id = map_id;
  ktime_get_real_ts64(&frag->timer);

  hash = ipid_hashfn(ipid);
  hlist_add_head(&frag->node, &frag_list.chain[hash]);
...
static void refresh_frag_list(void) {
  frag_tuple_t *iter;
  ...
  spin_lock_bh(&frag_list.lock);
  for (i = 0; i < NAT46_HTABLE_SIZE; i++) {
    hlist_for_each_entry_safe(iter, loop, &frag_list.chain[i], node) {
      delta = now.tv_sec - iter->timer.tv_sec;
      if (delta >= frag_list.timeout) {
        hlist_del(&iter->node);
        frag_list.size--;
        kfree(iter);
      }
...
int backup_frag_l4_ports(__u32 v4saddr, __u16 sport, __u32 v4daddr, __u16 dport, __u16 proto, __u16 ipid, __u32 map_id) {
  ...
  hash = ipid_hashfn(ipid);
  if (!hlist_empty(&frag_list.chain[hash])) {
    hlist_for_each_entry(iter, &frag_list.chain[hash], node) {
      if ( (iter->ipid == ipid) && (iter->proto == proto) &&
           (iter->v4saddr == v4saddr) && (iter->v4daddr == v4daddr) ) {
        ktime_get_real_ts64(&iter->timer);
        if ( (iter->sport == sport) && (iter->dport == dport) && (iter->map_id == map_id) ) {
          /* fragment already cached - do nothing with it */
        } else {
          // src&dest addr & id & protocol same, while src/dest port or map_id different: update the src/dest port or map_id
          if (iter->sport != sport) {
            iter->sport = sport;
          }
          if (iter->dport != dport) {
  ...
  if (!add_new_frag(v4saddr, sport, v4daddr, dport, proto, ipid, map_id)) {
    spin_unlock_bh(&frag_list.lock);
    return 0;
  }
...
int restore_frag_l4_ports(__u32 v4saddr, __u32 v4daddr, __u16 ipid, __u16 proto, __u16 *sport, __u16 *dport, __u32 *map_id, int last_frag) {
  ...
  hash = ipid_hashfn(ipid);
  hlist_for_each_entry_safe(iter, loop, &frag_list.chain[hash], node) {
    if ( (iter->ipid == ipid) && (iter->proto == proto) &&
         (iter->v4saddr == v4saddr) && (iter->v4daddr == v4daddr) ) {
      *sport = iter->sport;
      *dport = iter->dport;
