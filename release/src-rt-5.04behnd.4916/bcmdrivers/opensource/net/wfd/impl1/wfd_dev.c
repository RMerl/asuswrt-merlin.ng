...
#define WFD_FLCTL_SKB_EXHAUSTION_LO_PCNT 30 /* Favored Pkt threshold */
#define WFD_FLCTL_SKB_EXHAUSTION_HI_PCNT 15
...
#if (defined(CONFIG_BCM_RDPA)||defined(CONFIG_BCM_RDPA_MODULE))
#if defined(XRDP)
    spinlock_t          wfd_dev_lock[WIFI_MW_MAX_NUM_IF];
#endif
#endif
    int                 isValid;
...
void *wfd_dev_priv[WFD_MAX_OBJECTS]={};
#endif
...
int (*send_nbuff_to_wfd)(pNBuff_t pNBuff, struct net_device * dev) = NULL;
...
static ssize_t wfd_stats_file_read_proc(struct file *filep, char __user *page, size_t count, loff_t *data)
{
    ...
    for (wifi_index = 0 ; wifi_index < WFD_MAX_OBJECTS ;wifi_index++ )
    {
        if (wfd_objects[wifi_index].wfd_bulk_get)
        {
            ...
            len += sprintf((page+len), "\nwl_unicast_counters       =%d", wfd_objects[wifi_index].wl_chained_packets) ;
           ...
           {/* per queue status */          
                unsigned int queue_mask = 0x00;
                unsigned int qidx = 0, qid = 0;

                queue_mask = wfd_objects[wifi_index].wfd_queue_mask;
                qidx = __fls(queue_mask);
                while (queue_mask)
                {
                    qid = wfd_get_qid(qidx);
                    len += sprintf((page+len), "\nQueue[%d]:",qid);
...
static const struct proc_ops stats_fops = {
       .proc_read   = wfd_stats_file_read_proc,
};
#endif
...
static ssize_t wfd_flctl_file_write_proc(struct file *file, const char *buff, size_t len, loff_t *offset)
{
    ...
    if ((pkt_prio_favor > 7) && (pkt_prio_favor != 0xffff))
    { /* valid prio 0 .. 7 , 0xffff for ignore mode */
    ...
    if ((wfd_objects[radio_id].pktlist_context_p != PKTLIST_CONTEXT_NULL) &&
        (wfd_objects[radio_id].pktlist_context_p->fctable != PKTLIST_FCTABLE_NULL))
...
#else
static const struct proc_ops flctl_fops = {
    .proc_read   = wfd_flctl_file_read_proc,
...
static int wfd_dev_init(void)
{
...
#if defined(XRDP)
        int dev_if_idx;
        for (dev_if_idx = 0; dev_if_idx < WIFI_MW_MAX_NUM_IF; dev_if_idx++)
            spin_lock_init(&(wfd_objects[idx].wfd_dev_lock[dev_if_idx]));
#endif
#endif
    ...
    /* Initialize accelerator(Runner) specific data structures, Queues */
...
static inline int wfd_netdev_to_rdpa_port_obj(struct net_device *dev, bcm_netdev_priv_info_out_t *info_out)
{
    int i, j;

    for (i = 0; i < WFD_MAX_OBJECTS; i++)
    {
        for (j = 0; j < WIFI_MW_MAX_NUM_IF; j++)
        {
            if (wfd_objects[i].wl_if_dev[j] == dev)
            {
                info_out->bcm_netdev_to_rdpa_port_obj.rdpa_port_obj = wfd_dev_priv[i];
                return 0;
            }
        }
    }
...
int wfd_priv_info_get(struct net_device *dev, bcm_netdev_priv_info_type_t info_type,
    bcm_netdev_priv_info_out_t *info_out)
{
    ...
    case BCM_NETDEV_TO_RDPA_PORT_OBJ:
        rc = wfd_netdev_to_rdpa_port_obj(dev, info_out);
        break;
    default:
        break;
    }
             ...
             int wl_radio_idx)
{
    ...
    wfd_objects[tmp_idx].wfd_mcastHook_rx = wfd_mcastHook_rx;
...
int wfd_registerdevice(uint32_t wfd_idx, int ifidx, struct net_device *dev)
{
    ...
    bcm_netdev_ext_field_set(dev, bcm_netdev_cb_fn, wfd_priv_info_get);
...
int wfd_unregisterdevice(uint32_t wfd_idx, int ifidx)
{
    ...
    if (wfd_objects[wfd_idx].wl_if_dev[ifidx] == NULL) 
    {
        /* No device registered, nothing to do, bail out*/
        printk("%s Nothing to unregister ifidx %d wfd_idx %d\n", 
               __FUNCTION__, ifidx, wfd_idx);
        return 0;
    }

#if (defined(CONFIG_BCM_MCAST) || defined(CONFIG_BCM_MCAST_MODULE))
    bcm_mcast_if_handle_dev_up_or_down(wfd_objects[wfd_idx].wl_if_dev[ifidx], false);
#endif
#if (defined(CONFIG_BCM_RDPA)||defined(CONFIG_BCM_RDPA_MODULE))
#if defined(XRDP)
    spin_lock_bh(&(wfd_objects[wfd_idx].wfd_dev_lock[ifidx]));
#endif
#endif
    /* Uninitialize the bcm_netdevice callback function */
    bcm_netdev_ext_field_set(wfd_objects[wfd_idx].wl_if_dev[ifidx], bcm_netdev_cb_fn, NULL);

    wfd_objects[wfd_idx].wl_if_dev[ifidx] = NULL;
#if (defined(CONFIG_BCM_RDPA)||defined(CONFIG_BCM_RDPA_MODULE))
#if defined(XRDP)
    spin_unlock_bh(&(wfd_objects[wfd_idx].wfd_dev_lock[ifidx]));
#endif
#endif	
