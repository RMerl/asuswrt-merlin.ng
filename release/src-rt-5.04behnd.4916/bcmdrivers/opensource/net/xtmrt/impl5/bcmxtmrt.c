...
static const struct proc_ops data_path_dbg_proc_fops = {
   .proc_open = data_path_dbg_open,
   .proc_read = seq_read,
   .proc_lseek = seq_lseek,
   .proc_write = data_path_dbg_write,
...
static int __init bcmxtmrt_init(void)
{
   UINT16 usChipId  = UtilGetChipId();
   UINT16 usChipRev = UtilGetChipRev();
   ...
   g_GlobalInfo.ulChipRev = UtilGetChipRev();
...
static int speed_service_check (pNBuff_t pNBuf, UINT32 phyOverhead, struct net_device *dev, uint32_t *spdsvc_ctx)
{
   ...
   spdsvc_transmit.egress_type = SPDSVC_EGRESS_TYPE_DSL;
...
int bcmxtmrt_xmit(pNBuff_t pNBuf, struct net_device *dev)
{
   ...
   if (IS_SKBUFF_PTR(pNBuf))
   {
      ...
      if (pDevCtx->szMirrorIntfOut[0] != '\0' &&
         ...
         blog_hw_accel_force_disable (pNBuf, BLOG_HW_ACCEL_FORCE_DISABLE_ON) ;
      }
      else {
         blog_hw_accel_force_disable (pNBuf, BLOG_HW_ACCEL_FORCE_DISABLE_OFF) ;
      }
#endif /* HW_Accel_Mirroring */
#endif
   ...
   if (IS_SKBUFF_PTR(pNBuf))
   {
      // debug code for dumping the timestamp for vector packet
      dump_phy_error_vector_frame_info(skb);
   }
       ...
       hdrType == HT_VC_MUX_ETHERNET)
   {
      ...
      if (is_multicast_ether_addr(pData))
      {
         ...
         if (is_broadcast_ether_addr(pData))
         {
            ...
            pDevCtx->DevStats.tx_broadcast_packets++;
         }
         else
         {
                             ...
                             UINT8 is_group_fwd_exp)
{
   ...
   {
#if !defined(HW_ACCEL_MIRRORING)
      hw_accel_force_disable = BLOG_HW_ACCEL_FORCE_DISABLE_ON ;
#endif
      MirrorPacket(pDevCtx->szMirrorIntfIn, pNBuf);
   }
   else {
#if !defined(HW_ACCEL_MIRRORING)
      hw_accel_force_disable = BLOG_HW_ACCEL_FORCE_DISABLE_OFF ;
#endif
   }

#ifdef CONFIG_BLOG
   fcArgs.group_fwd_exception = is_group_fwd_exp;
...
#if !defined(HW_ACCEL_MIRRORING)
         blog_hw_accel_force_disable (pNBuf, hw_accel_force_disable) ;
#endif
