...
#else
#define PTM_MAX_TX_FRAME_LEN        1980  /* Per chip limitation, 63138/63148 SAR can only
...
/* If this definition is changed, please make sure accelerator (Runner)
** definitions are also in sync */
#if defined(CONFIG_BCM963158) || defined(CONFIG_BCM963146)
#define FSTAT_MATCH_ID_MASK         0x0000001f
#define FSTAT_MATCH_ID_SHIFT        0  /* Hw Shift-1, FW Shift-0 */
#define FSTAT_PACKET_CELL_MASK      0x00000020 /* Hw to FW/SW shift is by 1-bit */
#define FSTAT_PACKET                0x00000000
#define FSTAT_CELL                  0x00000020
#define FSTAT_ERROR                 0x00000040
#else
...
#define MAX_Q_LOGS              100
#define Q_SETUP                   0
#define Q_SHUTDOWN                1

typedef struct  _qDbgStatus {
   UINT32    qId ;
   int       status ;
   UINT32    occupancy ;
   UINT32    jiffies ;
   UINT32    operation ; /* setup/shutdown */
} BcmXtmQDbgStatus ;
#endif
...
#define XTMRT_BUDGET   32
    ...
    BcmXtmQDbgStatus      qDbgStatus [MAX_Q_LOGS] ;
    UINT32                qLogCurrIndex ;
#endif
