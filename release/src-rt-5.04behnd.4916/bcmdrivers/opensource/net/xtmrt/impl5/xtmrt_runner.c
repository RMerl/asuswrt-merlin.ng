...
bdmf_object_handle wan_port_obj = NULL;
...
static int runner_xtm_objects_init(void)
{
   ...
   BDMF_MATTR_ALLOC(xtm_attr,  rdpa_xtm_drv());
   BDMF_MATTR_ALLOC(port_attr, rdpa_port_drv());

   /* Create the xtm system object */   
   rc = bdmf_new_and_set(rdpa_xtm_drv(), NULL, xtm_attr, &xtm);
   if (rc)
   {
      BCM_LOG_NOTICE(BCM_LOG_ID_XTM, "Failed to create xtm object, rc %d", rc);
      goto exit;
   }
      
   /* Create logical ports object on top of the physical wan */
   rdpa_port_name_set(port_attr, "dsl");
   rdpa_port_type_set(port_attr, rdpa_port_dsl);
   rdpa_port_index_set(port_attr, 0);
   rdpa_port_is_wan_set(port_attr, 1);

   rc = bdmf_new_and_set(rdpa_port_drv(), xtm, port_attr, &wan_port_obj); 
   if (rc)
   {
   ...
   rc = bcmxapiex_runner_xtm_objects_init(wan_port_obj);

   if (rc)
   {
      BCM_LOG_NOTICE(BCM_LOG_ID_XTM, "Failed to init extended objects for WAN %s, error %d", bdmf_object_name(wan_port_obj), rc);
      goto exit;
   }
   ...
   g_GlobalInfo.bdmfWan = wan_port_obj;

   g_GlobalInfo.qLogCurrIndex = 0 ;
   memset (&g_GlobalInfo.qDbgStatus [0], 0, sizeof(BcmXtmQDbgStatus)*MAX_Q_LOGS) ;
      
exit:
   BDMF_MATTR_FREE(xtm_attr);
   BDMF_MATTR_FREE(port_attr);
   return rc;
...
static int runner_tx_queue_init(bdmf_object_handle xtm, int queueIdx)
{
   ...
   BDMF_MATTR_ALLOC(xtmchannel_attr, rdpa_xtmchannel_drv());
   ...
   if (rc) {
      BCM_XTM_ERROR( "Failed to create xtm channel for this tx queue %d", queueIdx);
      goto exit;
   }
   ...
   if (rc) {
      BCM_XTM_ERROR( "Failed to create XTM egress TM for this tx queue %d", queueIdx);
      if (xtmchannel != NULL) {
   ...
   if (rc) {
      BCM_XTM_ERROR( "Failed to bind XTM egress TM for this tx queue with xtm channel %d", queueIdx);
      if (xtmchannel != NULL) {
         bdmf_destroy(xtmchannel);
         pGi->txBdmfObjs[queueIdx].xtmchannel = NULL;
      }
...
exit:
   BDMF_MATTR_FREE(xtmchannel_attr);
...
static int cfg_xtmflows(UINT32 queueIdx, UINT32 trafficType, UINT32 hdrType, UINT8 vcid, UINT32 ulTxPafEnabled)
{
   ...
   BDMF_MATTR_ALLOC(packet_attr, rdpa_xtmflow_drv());
   
   if ((trafficType & TRAFFIC_TYPE_ATM_MASK) == TRAFFIC_TYPE_ATM)
   {
      BDMF_MATTR_ALLOC(f4_seg_attr, rdpa_xtmflow_drv());
      BDMF_MATTR_ALLOC(f4_e2e_attr, rdpa_xtmflow_drv());
      BDMF_MATTR_ALLOC(f5_seg_attr, rdpa_xtmflow_drv());
      BDMF_MATTR_ALLOC(f5_e2e_attr, rdpa_xtmflow_drv());
      BDMF_MATTR_ALLOC(asm_p0_attr, rdpa_xtmflow_drv());
      BDMF_MATTR_ALLOC(asm_p1_attr, rdpa_xtmflow_drv());
      BDMF_MATTR_ALLOC(asm_p2_attr, rdpa_xtmflow_drv());
      BDMF_MATTR_ALLOC(asm_p3_attr, rdpa_xtmflow_drv());
      ...
      BDMF_MATTR_FREE(f4_seg_attr);
      BDMF_MATTR_FREE(f4_e2e_attr);
      BDMF_MATTR_FREE(f5_seg_attr);
      BDMF_MATTR_FREE(f5_e2e_attr);
      BDMF_MATTR_FREE(asm_p0_attr);
      BDMF_MATTR_FREE(asm_p1_attr);
      BDMF_MATTR_FREE(asm_p2_attr);
      BDMF_MATTR_FREE(asm_p3_attr);
   }
   else
   {
      ...
      if (trafficType == TRAFFIC_TYPE_PTM_BONDED) {
         BCM_XTM_NOTICE("bcmxtmrt: Traffic Type is PTM Bonded.  TxPAF control-%d. ", (unsigned int) ulTxPafEnabled) ;
         rc = rc? rc : add_xtmflow(packet_attr, XCT_PTM, queueIdx, hdrType, packet_fstat, BC_PTM_BONDING_ENABLE) ;
      }
      else {
         BCM_XTM_NOTICE("bcmxtmrt: Traffic Type is Non-bonded-%d. ", (unsigned int) trafficType) ;
                                 ...
                                 UINT32 flag)
{
   ...
   BCM_XTM_DEBUG("XTM: hdrType:%x, fstat:%x",hdrType,fstat);
...
xtm_exit:
   if (!rc) 
      BCM_XTM_NOTICE("Created XTM CPU-RUnner Rx queue %d with size %d", queue_id, (int)queue_size);
   else 
      BCM_XTM_ERROR( "Failed to configure XTM CPU-Runner Rx queue %d with size %d ", queue_id, (int) queue_size);
...
static void cpu_rxq_isr(int queue_id)
{
   ...
   rdpa_cpu_int_disable(XTM_RDPA_CPU_PORT, RDPA_XTM_CPU_HW_FIREWALL_RX_QUEUE_ID);
   rdpa_cpu_int_clear(XTM_RDPA_CPU_PORT, RDPA_XTM_CPU_HW_FIREWALL_RX_QUEUE_ID);
#endif
...
int bcmxapi_module_init(void)
{
   ...
   if (rc == 0)
   {
      ...
      {
        bdmf_put(system_obj);
        BCM_XTM_ERROR("Failed to get high prio tc threshold");
        return -1;
      }
   ...
   {
      BCM_XTM_ERROR( CARDNAME ": Error(%d) getting system object", rc);
      return -EFAULT;
   }
   ...
   if (rc)
   {
      BCM_XTM_ERROR( CARDNAME ": Error(%d) cfg CPU low priority rx queue", rc);
      return -EFAULT;
   }
   ...
   if (rc)
   {
      BCM_XTM_ERROR( CARDNAME ": Error(%d) cfg CPU high priority rx queue", rc);
      return -EFAULT;
   }
   ...
   rc = cfg_cpu_rx_queue(RDPA_XTM_CPU_HW_FIREWALL_RX_QUEUE_ID, RDPA_XTM_CPU_RX_QUEUE_SIZE,
                         &cpu_rxq_isr);
   if (rc)
   {
      BCM_XTM_ERROR( CARDNAME ": Error(%d) cfg CPU high priority rx queue", rc);
   ...
   for (reason = rdpa_cpu_reason_min; reason < rdpa_cpu_reason__num_of; reason++) {
...
#if IS_ENABLED(CONFIG_BCM_HW_FIREWALL)
      else if (reason == rdpa_cpu_rx_reason_hw_firewall_miss) {
         /* XRDP-only feature. HW_FIREWALL should be lowest priority */
         tc = XTM_RDPA_CPU_TC_HW_FIREWALL;
         queue_id = RDPA_XTM_CPU_HW_FIREWALL_RX_QUEUE_ID;
      }
#endif
      else {
      ...
      if (rc)
      {
         BCM_XTM_ERROR( CARDNAME ": Error(%d) cfg CPU reason to TC to priority rx queue", rc);
         BCM_XTM_ERROR( CARDNAME "failed to set Map TC to RXQ, error: %d, RDPA reason %d, TC %d, CPU RXQ %d", rc,
...
#if IS_ENABLED(CONFIG_BCM_HW_FIREWALL)
   rdpa_cpu_int_enable(XTM_RDPA_CPU_PORT, RDPA_XTM_CPU_HW_FIREWALL_RX_QUEUE_ID);
#endif
...
#if IS_ENABLED(CONFIG_BCM_HW_FIREWALL)
   rdpa_cpu_int_disable(XTM_RDPA_CPU_PORT, RDPA_XTM_CPU_HW_FIREWALL_RX_QUEUE_ID);
   rdpa_cpu_rxq_flush_set(rdpa_cpu_xtm_obj, RDPA_XTM_CPU_HW_FIREWALL_RX_QUEUE_ID, TRUE);
   rdpa_cpu_int_clear(XTM_RDPA_CPU_PORT, RDPA_XTM_CPU_HW_FIREWALL_RX_QUEUE_ID);
#endif
   return 0;
   ...
   */
   do
   {
      ...
      if (rc == BDMF_ERR_NO_MORE)
      {
          ...
          {
#if IS_ENABLED(CONFIG_BCM_HW_FIREWALL)
             /* Receive packet from the lowest priority cpu rx queue for firewall miss */
             rc = bcmxapiex_get_pkt_from_ring (RDPA_XTM_CPU_HW_FIREWALL_RX_QUEUE_ID, &pFkb, &info);
             if (rc == BDMF_ERR_NO_MORE)
             {
                ...
                g_xtm_rxq_stats_received[XTM_RDPA_CPU_HW_FIREWALL_RX_QUEUE_IDX]++;
             else
             {
                spin_unlock_irqrestore(&pGi->xtmlock_rx, irqFlags);
                g_xtm_rxq_stats_dropped[XTM_RDPA_CPU_HW_FIREWALL_RX_QUEUE_IDX]++;
                BCM_XTM_ERROR( "Error in bcmxapiex_get_pkt_from_ring() qid %d rc (%d)", 
                        RDPA_XTM_CPU_HW_FIREWALL_RX_QUEUE_ID, rc);
                goto drop_pkt;
             }
#else
             ulRxPktGood |= XTM_POLL_DONE;
      ...
      BCM_XTM_RX_DEBUG("Rcvd packet ReasonData:%x ulVcId:%u ulCell:%u pDevCtx:%pk\n",info.reason_data,ulVcId,ulCell,pDevCtx);
          ...
          ((!ulCell) && (pDevCtx == NULL)))   /* packet */
      {
         if (ulVcId == TEQ_DATA_VCID && pGi->pTeqNetDev)
         {
            ...
            skb = bdmf_sysb_header_alloc(bdmf_sysb_skb, (uint8_t *)info.data, info.data_offset, info.size, 0, 0);
            if (!skb)
            {
                /* free the data buffer */
                bdmf_sysb_databuf_free((uint8_t *)info.data, 0);

                BCM_XTM_ERROR("sysb_header allocation failed");
                goto drop_pkt;
            }
         ...
         else
         {
            ...
            /* free the data buffer */
            BCM_XTM_DEBUG("Freeing data buffer (Error status or invalid context) ");
            bdmf_sysb_databuf_free((uint8_t *)info.data, 0);
            if (pDevCtx)
      ...
      else if (!ulCell) /* process packet, pDev != NULL */
      {
         ...
         {
             BCM_XTM_DEBUG("Pkt_len=%d, offset=%x, reason=%d", info.size, info.data_offset, info.reason);
             ...
             info.data += sizeof(flow_key);
             BCM_XTM_DEBUG("Flw_key=%x, data=%p, Pkt_len=%d", flow_key, info.data, info.size);
         }
#endif
         bcmxtmrt_processRxPkt(pDevCtx, NULL, (UINT8 *)info.data,
                               (UINT16)info.reason_data, info.size, flow_key,
                               info.reason == rdpa_cpu_rx_reason_group_exception? 1 : 0);
         ulRxPktGood++;
         ulBudget--;
      }
      else  /* process cell */
...
int bcmxapi_add_proc_files(struct proc_dir_entry *dir)
{
   ...
   return (bcmxapiex_add_proc_files(dir));
...
{
   return (bcmxapiex_del_proc_files(dir));
...
int bcmxapi_DoGlobInitReq(PXTMRT_GLOBAL_INIT_PARMS pGip)
{
   ...
   BCM_XTM_NOTICE("E-RXIntr");
   bcmxapi_enable_rx_interrupt();

   /* create and initialize xtm objects */
   rc = runner_xtm_objects_init();
   if (rc) {
      BCM_XTM_ERROR("Failed to initialize runner objects rc:%u",rc);
      rc = -EFAULT;
   }
   else
...
int bcmxapi_DoGlobUninitReq(void)
{
   ...
   if (pGi->ulDrvState == XTMRT_UNINITIALIZED) {
      BCM_XTM_ERROR("Driver state already uninitialized");
      return -EPERM;
   }
                         ...
                         PXTMRT_TRANSMIT_QUEUE_ID pTxQId)
{
   ...
   if (txdma == NULL) {
      BCM_XTM_ERROR( "Unable to allocate memory for xtm tx dma info");
      return -ENOMEM;
   }
   ...
   if ((ulPort < MAX_PHY_PORTS) && (pTxQId->ucSubPriority < MAX_SUB_PRIORITIES)) {
      ...
      if (egress_tm != NULL) {
         ...
         BCM_XTM_NOTICE(" bcmxtmrt:  Tx Q(%d) Actual MBR = %d \n", queueIdx, queue_cfg.reserved_packet_buffers) ;
         ...
         BCM_XTM_NOTICE("bcmxtmrt: Egress TM Q %d Setup. Buffering/Q - %d ", (unsigned int)queueIdx, 
               (unsigned int) queue_cfg.drop_threshold );
         BCM_XTM_DEBUG("ulTrafficType:%u, ulHdrType:%u, ucTxVcid:%u, ulTxPafEnabled:%u",
                                                          pDevCtx->ulTrafficType,
                                                          pDevCtx->ulHdrType,
                                                          pDevCtx->ucTxVcid,
                                                          pDevCtx->ulTxPafEnabled);
         rc = rdpa_egress_tm_queue_cfg_set(egress_tm, 0, &queue_cfg);
         if (rc)
         {
            BCM_XTM_ERROR(CARDNAME "DoSetTxQueue: rdpa_egress_tm_queue_cfg_set error rc=%d", rc);
            goto _End;
         }
         ...
         if (cfg_xtmflows(queueIdx, pDevCtx->ulTrafficType, pDevCtx->ulHdrType,
                  pDevCtx->ucTxVcid, pDevCtx->ulTxPafEnabled) != 0)
         {
            BCM_XTM_ERROR(CARDNAME "DoSetTxQueue: Failed to create xtmflows for Q %d",
         ...
         pQDbgStatus->occupancy = queue_cfg.drop_threshold ;
         pQDbgStatus->jiffies = jiffies ;
         pQDbgStatus->operation = Q_SETUP ;
                         ...
                         volatile BcmPktDma_XtmTxDma *txdma)
{
   ...
   BCM_XTM_DEBUG("StopTxQueue: queueIdx:%u",queueIdx);
   ...
   if (egress_tm != NULL)
   {
      ...
      {
         BCM_XTM_ERROR(CARDNAME "StopTxQueue: rdpa_egress_tm_queue_cfg_set error rc=%d", rc);
      }
                          ...
                          volatile BcmPktDma_XtmTxDma *txdma)
{
   ...
   queueIdx  = txdma->ulDmaIndex;
   BCM_XTM_DEBUG("StartTxQueue queueIdx:%u",queueIdx);
   ...
   if (egress_tm != NULL)
   {
      ...
      BCM_XTM_DEBUG(" bcmxtmrt:  DropAlg = %d ", txdma->ucDropAlg) ;
      ...
      if ((rc = rdpa_egress_tm_queue_cfg_set(egress_tm, 0, &queue_cfg)))
      {
         BCM_XTM_ERROR(CARDNAME "StartTxQueue: rdpa_egress_tm_queue_cfg_set error rc=%d", rc);
      }
...
void bcmxapi_XtmGetStats(PBCMXTMRT_DEV_CONTEXT pDevCtx, UINT32 *rxDropped, UINT32 *txDropped)
{
    ...
    *txDropped = 0;
    ...
    for (index=0;index < pDevCtx->ulTxQInfosSize; index++)
    {
        ...
        queue_id = 0;
        rc = rdpa_egress_tm_queue_stat_get(g_GlobalInfo.txBdmfObjs[pDevCtx->txdma[index]->ulDmaIndex].egress_tm,
                                           queue_id,
                                           &egress_tm_stat);
        if(!rc)
        {
...
void bcmxapi_XtmResetStats(UINT8 vport)
{
    ...
    if (rc)
    {
        BCM_XTM_ERROR("rdpa_port_stat_set returned error rc %d", rc);
    }
