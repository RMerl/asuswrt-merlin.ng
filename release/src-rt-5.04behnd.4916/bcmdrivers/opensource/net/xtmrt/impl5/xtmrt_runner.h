...
#define XTM_RDPA_CPU_TC_HW_FIREWALL 0
#define XTM_RDPA_CPU_TC_LOW      1
#define XTM_RNR_CPU_RX_QUEUES    3
#else
...
/**** Externs ****/
extern bdmf_object_handle wan_port_obj;
...
int bcmxapi_add_proc_files(struct proc_dir_entry *ptr);
int bcmxapi_del_proc_files(struct proc_dir_entry *ptr);
...
#ifdef CONFIG_BLOG
static inline BlogAction_t bcmxapi_blog_emit (pNBuff_t pNBuf, struct net_device *dev,
...
#ifdef CONFIG_BLOG
/*---------------------------------------------------------------------------
 * BlogAction_t bcmxapi_blog_emit (pNBuff_t pNBuf, struct net_device *dev,
 *                         PBCMXTMRT_DEV_CONTEXT pDevCtx,
 *                         BcmPktDma_XtmTxDma *txdma,
 *                         UINT32 rfc2684Type, UINT16 bufStatus)
 * Description:
                                      ...
                                      UINT32 rfc2684Type, UINT16 bufStatus)
{
   ...
   return blog_emit(pNBuf, dev, pDevCtx->ulEncapType, wanChannelFlow,
                                      ...
                                      void *pTcpSpdTstInfo)
{
   ...
   extra_info.so_mark = *(uint32_t *)pTcpSpdTstInfo;

   BCM_XTM_TX_DEBUG("ctType:%04x wanFlow:%04x",ctType,wanFlow);
   
   rc = rdpa_cpu_tx_port_enet_or_dsl_wan(pDataBuf, txdmaIdx, (rdpa_flow) wanFlow, wan_port_obj, extra_info);
   if (rc != 0)
   {
      BCM_XTM_ERROR("rdpa_cpu_tx_port_enet_or_dsl_wan() for XTM port "
