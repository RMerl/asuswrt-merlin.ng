...
*/
static inline int crossbar_plat_select(int crossbar_id, int int_ep, int ext_ep, phy_dev_t *phy_dev)
{
    uint32_t val32 = 0;
    int i, old_ext_ep;
...
static inline int crossbar_plat_finalize(void)
{
    const sw_port_t *sp;
    int id, endpoint;
    ...
    {   // find max internal endpoint
        ...
        /* If we are not using Crossbar structure in DT file like in 47622, skip touching hardware */
        if (crossbar_is_not_initialized(id))    
            continue;
