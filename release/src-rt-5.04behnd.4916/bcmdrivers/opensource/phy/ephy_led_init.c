...
static int ephyled_probe(dt_device_t *pdev)
{
    ...
    of_property_read_u32(pdev->dev.of_node, "led_reg_max", (u32 *)&PEPHY_LED->max_led_regs);
    ...
    do
    {
        ...
        if (res == NULL || IS_ERR((void*)(uintptr_t)res->start))
            continue;
        PEPHY_LED->led_reg[index] = devm_ioremap_resource(&pdev->dev, res);
    } while(++index < PEPHY_LED->max_led_regs);

    PEPHY_LED->workarround_5g = of_property_read_bool(pdev->dev.of_node, "5g-led-workarround");
...
static const struct of_device_id ephy_led_match_table[] = {
    { .compatible = "brcm,ephy-led" },
    { /* end of list */ },
};

static struct platform_driver ephy_led_platform_driver = {
    .driver = {
        .name = "brcm-ephyled",
        .of_match_table = ephy_led_match_table,
    },
    .probe = ephyled_probe,
};
module_platform_driver(ephy_led_platform_driver);
...
int ephy_leds_init(void *_leds_info)
{
    ...
    printk("[%s:%d]: SW %d Port %d led idx %d configured as:\n",
        __FUNCTION__, __LINE__,sw, port, led_idx);
    printk("[%s:%d]:led ctrl 0x%08x encode 0x%08x encode sel 0x%08x\n",__FUNCTION__, __LINE__,
