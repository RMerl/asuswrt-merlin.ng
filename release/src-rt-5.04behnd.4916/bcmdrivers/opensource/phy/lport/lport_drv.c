...
static void __iomem *lport_phys_base;
static void __iomem *lport_virt_base;
static void __iomem *misc_virt_base;
static void __iomem *xlif_virt_base;
#ifdef LPORT_INTERRUPTS
static int lport_irq0, lport_irq1;
#endif
...
#ifdef LPORT_INTERRUPTS
    lport_irq0 = platform_get_irq(pdev, 0);
    if (lport_irq0 < 0) 
    {
        dev_err(&pdev->dev, "Failed to get lport0 irq\n");
        ret = -1;
        goto Exit;
    }

    lport_irq1 = platform_get_irq(pdev, 1);
    if (lport_irq1 < 0) 
    {
        dev_err(&pdev->dev, "Failed to get lport1 irq\n");
        ret = -1;
        goto Exit;
    }
#endif
...
int lport_init_driver(lport_init_s *init_params)
{
    ...
    {
#ifdef LPORT_INTERRUPTS
        if (lport_intr_init(lport_irq0, lport_irq1))
        {
