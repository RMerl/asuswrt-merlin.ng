...
static lport_rx_stats_s rxCountersLast[LPORT_NUM_OF_PORTS];
static lport_tx_stats_s txCountersLast[LPORT_NUM_OF_PORTS];
#define LPORT_CURRENT_COUNTERS(portid, cur, last, bits, param) \
    do{\
...
int lport_stats_get_rx(uint32_t portid, lport_rx_stats_s *rx_stats)
{
    ...
    rc = rc ? rc :ag_drv_lport_mib_grxlpi_get(portid,&cnt32);  if (!rc) rx_stats->GRXLPI = (uint64_t) cnt32;
    rc = rc ? rc :ag_drv_lport_mib_grxdlpi_get(portid,&cnt32); if (!rc) rx_stats->GRXDLPI = (uint64_t) cnt32;
    ...
    if (!rc)
    {
        LPORT_CURRENT_COUNTERS(portid, rx_stats, rxCountersLast, 40, GRX64);
        LPORT_CURRENT_COUNTERS(portid, rx_stats, rxCountersLast, 40, GRX127);
        LPORT_CURRENT_COUNTERS(portid, rx_stats, rxCountersLast, 40, GRX255);
        LPORT_CURRENT_COUNTERS(portid, rx_stats, rxCountersLast, 40, GRX511);
        LPORT_CURRENT_COUNTERS(portid, rx_stats, rxCountersLast, 40, GRX1023);
        LPORT_CURRENT_COUNTERS(portid, rx_stats, rxCountersLast, 40, GRX1518);
        LPORT_CURRENT_COUNTERS(portid, rx_stats, rxCountersLast, 40, GRX1522);
        LPORT_CURRENT_COUNTERS(portid, rx_stats, rxCountersLast, 40, GRX2047);
        LPORT_CURRENT_COUNTERS(portid, rx_stats, rxCountersLast, 40, GRX4095);
        LPORT_CURRENT_COUNTERS(portid, rx_stats, rxCountersLast, 40, GRX9216);
        LPORT_CURRENT_COUNTERS(portid, rx_stats, rxCountersLast, 40, GRX16383);
        LPORT_CURRENT_COUNTERS(portid, rx_stats, rxCountersLast, 40, GRXPKT);
        LPORT_CURRENT_COUNTERS(portid, rx_stats, rxCountersLast, 40, GRXUCA);
        LPORT_CURRENT_COUNTERS(portid, rx_stats, rxCountersLast, 40, GRXMCA);
        LPORT_CURRENT_COUNTERS(portid, rx_stats, rxCountersLast, 40, GRXBCA);
        LPORT_CURRENT_COUNTERS(portid, rx_stats, rxCountersLast, 40, GRXFCS);
        LPORT_CURRENT_COUNTERS(portid, rx_stats, rxCountersLast, 40, GRXCF);
        LPORT_CURRENT_COUNTERS(portid, rx_stats, rxCountersLast, 40, GRXPP);
        LPORT_CURRENT_COUNTERS(portid, rx_stats, rxCountersLast, 40, GRXPF);
        LPORT_CURRENT_COUNTERS(portid, rx_stats, rxCountersLast, 40, GRXUDA);
        LPORT_CURRENT_COUNTERS(portid, rx_stats, rxCountersLast, 40, GRXWSA);
        LPORT_CURRENT_COUNTERS(portid, rx_stats, rxCountersLast, 40, GRXALN);
        LPORT_CURRENT_COUNTERS(portid, rx_stats, rxCountersLast, 40, GRXFLR);
        LPORT_CURRENT_COUNTERS(portid, rx_stats, rxCountersLast, 40, GRXFRERR);
        LPORT_CURRENT_COUNTERS(portid, rx_stats, rxCountersLast, 40, GRXFCR);
        LPORT_CURRENT_COUNTERS(portid, rx_stats, rxCountersLast, 40, GRXOVR);
        LPORT_CURRENT_COUNTERS(portid, rx_stats, rxCountersLast, 40, GRXJBR);
        LPORT_CURRENT_COUNTERS(portid, rx_stats, rxCountersLast, 40, GRXMTUE);
        LPORT_CURRENT_COUNTERS(portid, rx_stats, rxCountersLast, 40, GRXMCRC);
        LPORT_CURRENT_COUNTERS(portid, rx_stats, rxCountersLast, 40, GRXPRM);
        LPORT_CURRENT_COUNTERS(portid, rx_stats, rxCountersLast, 40, GRXVLN);
        LPORT_CURRENT_COUNTERS(portid, rx_stats, rxCountersLast, 40, GRXDVLN);
        LPORT_CURRENT_COUNTERS(portid, rx_stats, rxCountersLast, 40, GRXTRFU);
        LPORT_CURRENT_COUNTERS(portid, rx_stats, rxCountersLast, 40, GRXPOK);
        LPORT_CURRENT_COUNTERS(portid, rx_stats, rxCountersLast, 40, GRXSCHCRC);
        LPORT_CURRENT_COUNTERS(portid, rx_stats, rxCountersLast, 48, GRXBYT);
        LPORT_CURRENT_COUNTERS(portid, rx_stats, rxCountersLast, 40, GRXRPKT);
        LPORT_CURRENT_COUNTERS(portid, rx_stats, rxCountersLast, 40, GRXUND);
        LPORT_CURRENT_COUNTERS(portid, rx_stats, rxCountersLast, 40, GRXFRG);
        LPORT_CURRENT_COUNTERS(portid, rx_stats, rxCountersLast, 48, GRXRBYT);
        LPORT_CURRENT_COUNTERS(portid, rx_stats, rxCountersLast, 32, GRXLPI);
        LPORT_CURRENT_COUNTERS(portid, rx_stats, rxCountersLast, 32, GRXDLPI);
        LPORT_CURRENT_COUNTERS(portid, rx_stats, rxCountersLast, 40, GRXPTLLFC);
        LPORT_CURRENT_COUNTERS(portid, rx_stats, rxCountersLast, 40, GRXLTLLFC);
        LPORT_CURRENT_COUNTERS(portid, rx_stats, rxCountersLast, 40, GRXLLFCFCS);
        LPORT_CURRENT_COUNTERS(portid, rx_stats, rxCountersLast, 40, GRXUO);
    }
    return rc;
}
...
int lport_stats_get_tx(uint32_t portid, lport_tx_stats_s *tx_stats)
{
    ...
    rc = rc ? rc :ag_drv_lport_mib_gtxlpi_get(portid, &cnt32);  if (!rc) tx_stats->GTXLPI = (uint64_t) cnt32;
    rc = rc ? rc :ag_drv_lport_mib_gtxdlpi_get(portid, &cnt32); if (!rc) tx_stats->GTXDLPI = (uint64_t) cnt32;
    rc = rc ? rc :ag_drv_lport_mib_gtxltllfc_get(portid, &tx_stats->GTXLTLLFC);
    
    if (!rc)
    {
        LPORT_CURRENT_COUNTERS(portid, tx_stats, txCountersLast, 40, GTX64);
        LPORT_CURRENT_COUNTERS(portid, tx_stats, txCountersLast, 40, GTX127);
        LPORT_CURRENT_COUNTERS(portid, tx_stats, txCountersLast, 40, GTX255);
        LPORT_CURRENT_COUNTERS(portid, tx_stats, txCountersLast, 40, GTX511);
        LPORT_CURRENT_COUNTERS(portid, tx_stats, txCountersLast, 40, GTX1023);
        LPORT_CURRENT_COUNTERS(portid, tx_stats, txCountersLast, 40, GTX1518);
        LPORT_CURRENT_COUNTERS(portid, tx_stats, txCountersLast, 40, GTX1522);
        LPORT_CURRENT_COUNTERS(portid, tx_stats, txCountersLast, 40, GTX2047);
        LPORT_CURRENT_COUNTERS(portid, tx_stats, txCountersLast, 40, GTX4095);
        LPORT_CURRENT_COUNTERS(portid, tx_stats, txCountersLast, 40, GTX9216);
        LPORT_CURRENT_COUNTERS(portid, tx_stats, txCountersLast, 40, GTX16383);
        LPORT_CURRENT_COUNTERS(portid, tx_stats, txCountersLast, 40, GTXPOK);
        LPORT_CURRENT_COUNTERS(portid, tx_stats, txCountersLast, 40, GTXPKT);
        LPORT_CURRENT_COUNTERS(portid, tx_stats, txCountersLast, 40, GTXUCA);
        LPORT_CURRENT_COUNTERS(portid, tx_stats, txCountersLast, 40, GTXMCA);
        LPORT_CURRENT_COUNTERS(portid, tx_stats, txCountersLast, 40, GTXBCA);
        LPORT_CURRENT_COUNTERS(portid, tx_stats, txCountersLast, 40, GTXPF);
        LPORT_CURRENT_COUNTERS(portid, tx_stats, txCountersLast, 40, GTXPFC);
        LPORT_CURRENT_COUNTERS(portid, tx_stats, txCountersLast, 40, GTXJBR);
        LPORT_CURRENT_COUNTERS(portid, tx_stats, txCountersLast, 40, GTXFCS);
        LPORT_CURRENT_COUNTERS(portid, tx_stats, txCountersLast, 40, GTXCF);
        LPORT_CURRENT_COUNTERS(portid, tx_stats, txCountersLast, 40, GTXOVR);
        LPORT_CURRENT_COUNTERS(portid, tx_stats, txCountersLast, 40, GTXDFR);
        LPORT_CURRENT_COUNTERS(portid, tx_stats, txCountersLast, 40, GTXEDF);
        LPORT_CURRENT_COUNTERS(portid, tx_stats, txCountersLast, 40, GTXSCL);
        LPORT_CURRENT_COUNTERS(portid, tx_stats, txCountersLast, 40, GTXMCL);
        LPORT_CURRENT_COUNTERS(portid, tx_stats, txCountersLast, 40, GTXLCL);
        LPORT_CURRENT_COUNTERS(portid, tx_stats, txCountersLast, 40, GTXXCL);
        LPORT_CURRENT_COUNTERS(portid, tx_stats, txCountersLast, 40, GTXFRG);
        LPORT_CURRENT_COUNTERS(portid, tx_stats, txCountersLast, 40, GTXERR);
        LPORT_CURRENT_COUNTERS(portid, tx_stats, txCountersLast, 40, GTXVLN);
        LPORT_CURRENT_COUNTERS(portid, tx_stats, txCountersLast, 40, GTXDVLN);
        LPORT_CURRENT_COUNTERS(portid, tx_stats, txCountersLast, 40, GTXRPKT);
        LPORT_CURRENT_COUNTERS(portid, tx_stats, txCountersLast, 40, GTXUFL);
        LPORT_CURRENT_COUNTERS(portid, tx_stats, txCountersLast, 40, GTXNCL);
        LPORT_CURRENT_COUNTERS(portid, tx_stats, txCountersLast, 48, GTXBYT);
        LPORT_CURRENT_COUNTERS(portid, tx_stats, txCountersLast, 32, GTXLPI);
        LPORT_CURRENT_COUNTERS(portid, tx_stats, txCountersLast, 32, GTXDLPI);
        LPORT_CURRENT_COUNTERS(portid, tx_stats, txCountersLast, 40, GTXLTLLFC);
    }
    return rc;
}
