...
#include "merlin_mptwo_field_access.c"
#include "merlin_mptwo_internal.c"
#include "merlin_mptwo_dv_functions.c"
...
err_code_t merlin_mptwo_display_eye_scan(void) {
    uint32_t   stripe[64];
    uint16_t   status = 0;
    int8_t     y;

    EFUN(merlin_mptwo_display_eye_scan_header(1));
    /* start horizontal acquisition */
    {   err_code_t err_code = merlin_mptwo_meas_eye_scan_start(EYE_SCAN_HORIZ);
        if (err_code) {
            EFUN((merlin_mptwo_meas_eye_scan_done(), err_code));
        }
    }
    ...
    for (y = 31;y>=-31;y=y-1)
    {
        {   err_code_t err_code = merlin_mptwo_read_eye_scan_stripe(&stripe[0], &status);
            if (err_code) {
                EFUN((merlin_mptwo_meas_eye_scan_done(), err_code));
            }
        }
        EFUN(merlin_mptwo_display_eye_scan_stripe(y,&stripe[0]));
        EFUN_PRINTF(("\n"));
    }
    /* stop acquisition */
    EFUN(merlin_mptwo_meas_eye_scan_done());
    EFUN(merlin_mptwo_display_eye_scan_footer(1));

    return(ERR_CODE_NONE);
}

/* This function is for Falcon and is configured for passive mode */
err_code_t merlin_mptwo_meas_lowber_eye(const struct merlin_mptwo_eyescan_options_st eyescan_options, uint32_t *buffer) {
  int8_t y,x;
  int16_t i;
  uint16_t status;
  uint32_t errors = 0;
  ...
  for (y = eyescan_options.vert_max;y>=eyescan_options.vert_min;y=y-eyescan_options.vstep) {
    for (x=eyescan_options.horz_min;x<=eyescan_options.horz_max;x=x+eyescan_options.hstep) {
      /* acquire sample */
        EFUN(merlin_mptwo_pmd_uc_cmd_with_data(CMD_DIAG_EN,CMD_UC_DIAG_GET_EYE_SAMPLE,((uint16_t)x)<<8 | (uint8_t)y,200));
        /* wait for sample complete */
#if 0
        do {
            EFUN(merlin_mptwo_delay_us(1000));
            ESTM(status=rdv_usr_diag_status());
            EFUN_PRINTF(("status=%04x\n",status));
        } while((status & 0x8000) == 0);
#else
        EFUN(merlin_mptwo_poll_diag_done(&status, (((uint32_t)timeout)<<7)*10 + 20000));
#endif
        {
            merlin_mptwo_osr_mode_st osr_mode;
            ENULL_MEMSET(&osr_mode, 0, sizeof(merlin_mptwo_osr_mode_st));
            EFUN(_merlin_mptwo_get_osr_mode(&osr_mode));
            if(osr_mode.tx_rx == MERLIN_MPTWO_OSX1) {
                EFUN(merlin_mptwo_prbs_err_count_ll(&errors));
            } else if(osr_mode.tx_rx == MERLIN_MPTWO_OSX2) {
...
err_code_t merlin_mptwo_display_lowber_eye(const struct merlin_mptwo_eyescan_options_st eyescan_options, uint32_t *buffer) {
    int8_t x,y,z;
    uint8_t i;
    uint16_t j; /* buffer pointer */
    uint32_t val;
    uint8_t overflow;
...
err_code_t merlin_mptwo_meas_eye_scan_start(uint8_t direction) {
    uint8_t lock;
    ...
    if(direction == EYE_SCAN_VERTICAL) {
        EFUN(merlin_mptwo_pmd_uc_diag_cmd(CMD_UC_DIAG_START_VSCAN_EYE,200));
    } else {
        EFUN(merlin_mptwo_pmd_uc_diag_cmd(CMD_UC_DIAG_START_HSCAN_EYE,200));
    }
    return(ERR_CODE_NONE);
}

err_code_t merlin_mptwo_read_eye_scan_stripe(uint32_t *buffer,uint16_t *status){
    uint32_t val[2] = {0,0};
    uint16_t sts = 0;
    int8_t   i;
    ...
    *status = 0;
    for(i=0;i<32;i++) {
        {   err_code_t err_code = merlin_mptwo_poll_diag_eye_data(&val[0], &sts, 200);
            *status |= sts & 0xF000;
            if (err_code) {
                return (err_code);
            }
...
err_code_t merlin_mptwo_display_eye_scan_stripe(int8_t y,uint32_t *buffer) {
    ...
    for (x=-31;x<32;x++) {
        for (i=0;i<7;i++) {
            if (buffer[(uint8_t)(x+31)]>=limits[i]) {
                EFUN_PRINTF(("%c", '0'+i+1));
                break;
            }
        }
...
err_code_t merlin_mptwo_meas_eye_density_data(const struct merlin_mptwo_eyescan_options_st eyescan_options, int32_t *buffer,uint16_t *buffer_size) {
  int8_t y,x,z;
  uint16_t i;
  int8_t hzcnt;
  /*uint32_t errcnt; */
  ...
  EFUN(merlin_mptwo_pmd_uc_diag_cmd(CMD_UC_DIAG_DENSITY,2000));
  i = 0;hzcnt=0;
  ...
  for (y = eyescan_options.vert_max;y>=eyescan_options.vert_min;y=y-eyescan_options.vstep) {
    ...
    for (x=eyescan_options.horz_min;x<=eyescan_options.horz_max;x=x+eyescan_options.hstep) {
         ...
         EFUN(merlin_mptwo_poll_dsc_state_equals_uc_tune(2000));
...
err_code_t merlin_mptwo_display_eye_density_data(const struct merlin_mptwo_eyescan_options_st eyescan_options, int32_t *buffer,uint16_t buffer_size) {
    int8_t x,y,z;
    uint8_t i;
    uint16_t j; /* buffer pointer */
    int32_t val;
...
err_code_t merlin_mptwo_display_eye_scan_header(int8_t i) {
int8_t x;
    EFUN_PRINTF(("\n"));
...
err_code_t merlin_mptwo_display_eye_scan_footer(int8_t i) {
int8_t x;
    for(x=1;x<=i;x++) {
        EFUN_PRINTF(("         : -|----|----|----|----|----|----|----|----|----|----|----|----|-"));
    }
...
/*eye_scan_status_t merlin_mptwo_read_eye_scan_status(void) */
err_code_t merlin_mptwo_read_eye_scan_status(uint16_t *status) {
...
err_code_t merlin_mptwo_meas_eye_scan_done(void) {
  EFUN(merlin_mptwo_pmd_uc_diag_cmd(CMD_UC_DIAG_DISABLE,200));
  return(ERR_CODE_NONE);
}


err_code_t merlin_mptwo_start_ber_scan_test(uint8_t ber_scan_mode, uint8_t timer_control, uint8_t max_error_control) {
    uint8_t lock,sts;
    ESTM(lock = rd_pmd_rx_lock());
    if(lock == 0) {
    ...
    EFUN(merlin_mptwo_pmd_uc_cmd(CMD_CAPTURE_BER_START, ber_scan_mode,500));
    return(ERR_CODE_NONE);
}

err_code_t merlin_mptwo_read_ber_scan_data(uint32_t *errors, uint32_t *timer_values, uint8_t *cnt, uint32_t timeout) {
 ...
 ESTM(sts = rdv_usr_diag_status());
    ...
    *cnt = (sts & 0x00FF)/3;
    for(i=0;i < *cnt;i++) {
        ...
        EFUN(merlin_mptwo_pmd_uc_cmd( CMD_READ_DIAG_DATA_WORD, 0, timeout));
        ...
        EFUN(merlin_mptwo_pmd_uc_cmd( CMD_READ_DIAG_DATA_BYTE, 0, timeout));
...
err_code_t merlin_mptwo_eye_margin_proj(USR_DOUBLE rate, uint8_t ber_scan_mode, uint8_t timer_control, uint8_t max_error_control) {
    ...
    EFUN(merlin_mptwo_start_ber_scan_test(ber_scan_mode, timer_control, max_error_control));
    ...
    EFUN(merlin_mptwo_poll_diag_done(&sts,timer_control*2000));
    
    if(verbose > 2) EFUN_PRINTF(("delay done\n"));

    EFUN(merlin_mptwo_read_ber_scan_data( &errs[0], &time[0], &cnt, 2000));

    if(verbose > 2) EFUN_PRINTF(("read done cnt=%d\n",cnt));

    EFUN(merlin_mptwo_pmd_uc_cmd(CMD_CAPTURE_BER_END,0x00,50));
 ...
 /* }                                                                  */
    ...
    EFUN(merlin_mptwo_display_ber_scan_data(rate, ber_scan_mode, &errs[0], &time[0],(uint8_t)_abs(offset_start)));
...
err_code_t merlin_mptwo_display_ber_scan_data (USR_DOUBLE rate, uint8_t ber_scan_mode, uint32_t *total_errs, uint32_t *total_time, uint8_t max_offset) {
    ...
    ********************************************* */
    ...
    if (verbose > 2) EFUN_PRINTF(("first small errcnt idx at %d, errors = %d\n", first_small_errcnt_idx, ((first_small_errcnt_idx>=0) ? total_errs[(uint8_t)first_small_errcnt_idx] : -1)));
...
err_code_t merlin_mptwo_display_lane_state_hdr(void) {
  EFUN_PRINTF(("LN "));
...
err_code_t merlin_mptwo_display_lane_state_legend(void) {
  EFUN_PRINTF(("\n"));
  EFUN_PRINTF(("**********************************************************************************************\n"));
...
err_code_t merlin_mptwo_display_lane_state(void) {
  err_code_t err_code = _merlin_mptwo_display_lane_state_no_newline();
  EFUN_PRINTF(("\n"));
  return (err_code);
}

err_code_t merlin_mptwo_get_eye_margin_est(int *left_eye_mUI, int *right_eye_mUI, int *upper_eye_mV, int *lower_eye_mV) {
...
err_code_t merlin_mptwo_display_core_state_hdr(void) {
    char core_type[20] = "merlin_mptwo";
...
err_code_t merlin_mptwo_display_core_state_line(void) {
  err_code_t err_code = _merlin_mptwo_display_core_state_no_newline();
  EFUN_PRINTF(("\n"));
  return (err_code);
}

err_code_t merlin_mptwo_display_core_state_legend(void) {
  EFUN_PRINTF(("\n"));
  EFUN_PRINTF(("**************************************************************************************************************\n"));
...
err_code_t merlin_mptwo_display_core_config(void) {
    EFUN_PRINTF(("\n\n***********************************\n"  ));
    ...
    EFUN_PRINTF((    "***********************************\n\n"));
    {
        ...
        ENULL_MEMSET(&core_cfg, 0, sizeof(core_cfg));
            EFUN(merlin_mptwo_get_uc_core_config(&core_cfg));
        {
...
err_code_t merlin_mptwo_display_lane_config(void) {
    struct merlin_mptwo_uc_lane_config_st lane_cfg;

    ENULL_MEMSET(&lane_cfg, 0, sizeof(lane_cfg));

    EFUN_PRINTF(("\n\n*************************************\n"                                          ));
    ESTM_PRINTF((   "**** SERDES LANE %d CONFIGURATION ****\n"  , merlin_mptwo_get_lane()                     ));
    EFUN_PRINTF((    "*************************************\n\n"                                        ));
    EFUN(                                                         merlin_mptwo_get_uc_lane_cfg(&lane_cfg)      );
...
err_code_t merlin_mptwo_display_core_state(void) {
    EFUN(merlin_mptwo_display_core_state_hdr());
    EFUN(merlin_mptwo_display_core_state_line());
    EFUN(merlin_mptwo_display_core_state_legend());
    return ERR_CODE_NONE;
}

err_code_t merlin_mptwo_display_lane_debug_status(void) {
    /* startup */
    struct merlin_mptwo_usr_ctrl_disable_functions_st     ds;
    struct merlin_mptwo_usr_ctrl_disable_dfe_functions_st dsd;
    /* steady state */
    struct merlin_mptwo_usr_ctrl_disable_functions_st     dss;
    struct merlin_mptwo_usr_ctrl_disable_dfe_functions_st dssd;
    ...
    EFUN(merlin_mptwo_get_usr_ctrl_disable_startup(&ds));
    ...
    EFUN(                                                            merlin_mptwo_get_usr_ctrl_disable_startup_dfe(&dsd) );
    ...
    EFUN(                                                           merlin_mptwo_get_usr_ctrl_disable_steady_state(&dss) );
    ...
    EFUN(                                                      merlin_mptwo_get_usr_ctrl_disable_steady_state_dfe(&dssd) );
...
err_code_t merlin_mptwo_stop_uc_lane(uint8_t enable) {

  if (enable) {
    return(merlin_mptwo_pmd_uc_control(CMD_UC_CTRL_STOP_GRACEFULLY,100));
  }
  else {
    return(merlin_mptwo_pmd_uc_control(CMD_UC_CTRL_RESUME,50));
  }
}


err_code_t merlin_mptwo_stop_uc_lane_status(uint8_t *uc_lane_stopped) {
...
err_code_t merlin_mptwo_stop_rx_adaptation(uint8_t enable) {
...
err_code_t merlin_mptwo_request_stop_rx_adaptation(void) {

  return(merlin_mptwo_pmd_uc_control_return_immediate(CMD_UC_CTRL_STOP_GRACEFULLY));
}
...
err_code_t merlin_mptwo_ucode_crc_verify(uint16_t ucode_len,uint16_t expected_crc_value) {
    uint16_t calc_crc;

    EFUN(merlin_mptwo_pmd_uc_cmd_with_data(CMD_CALC_CRC,0,ucode_len,200));
...
err_code_t merlin_mptwo_start_ucode_crc_calc(uint16_t ucode_len) {

    EFUN(merlin_mptwo_pmd_uc_cmd_with_data_return_immediate(CMD_CALC_CRC, 0, ucode_len)); /* Invoke Calculate CRC command and return control immediately */
    return(ERR_CODE_NONE);
}

err_code_t merlin_mptwo_check_ucode_crc(uint16_t expected_crc_value, uint32_t timeout_ms) {
    uint16_t calc_crc;
    err_code_t err_code;

    err_code = merlin_mptwo_poll_uc_dsc_ready_for_cmd_equals_1(timeout_ms);     /* Poll for uc_dsc_ready_for_cmd = 1 to indicate merlin_mptwo ready for command */
    if (err_code) {
...
err_code_t merlin_mptwo_pmd_uc_cmd_return_immediate(enum srds_pmd_uc_cmd_enum cmd, uint8_t supp_info) {
  err_code_t err_code;
  uint16_t   cmddata;

  err_code = merlin_mptwo_poll_uc_dsc_ready_for_cmd_equals_1(1); /* Poll for uc_dsc_ready_for_cmd = 1 to indicate merlin_mptwo ready for command */
  if (err_code) {
...
/*EFUN(wr_uc_dsc_ready_for_cmd(0x0)  ); */                     /* Issue command, by clearing "ready for command" field */
  ...
  EFUN(merlin_mptwo_pmd_wr_reg(DSC_A_DSC_UC_CTRL, cmddata));         /* This address is same for Eagle, and all Merlin */

  return(ERR_CODE_NONE);
}

err_code_t merlin_mptwo_pmd_uc_cmd(enum srds_pmd_uc_cmd_enum cmd, uint8_t supp_info, uint32_t timeout_ms) {

  EFUN(merlin_mptwo_pmd_uc_cmd_return_immediate(cmd, supp_info));    /* Invoke merlin_mptwo_uc_cmd and return control immediately */

  EFUN(merlin_mptwo_poll_uc_dsc_ready_for_cmd_equals_1(timeout_ms)); /* Poll for uc_dsc_ready_for_cmd = 1 to indicate merlin_mptwo ready for command */
  return(ERR_CODE_NONE);
}


err_code_t merlin_mptwo_pmd_uc_cmd_with_data_return_immediate(enum srds_pmd_uc_cmd_enum cmd, uint8_t supp_info, uint16_t data) {
  uint16_t   cmddata;
  err_code_t err_code;
...
err_code_t merlin_mptwo_pmd_uc_cmd_with_data(enum srds_pmd_uc_cmd_enum cmd, uint8_t supp_info, uint16_t data, uint32_t timeout_ms) {

    EFUN(merlin_mptwo_pmd_uc_cmd_with_data_return_immediate(cmd, supp_info, data)); /* Invoke merlin_mptwo_uc_cmd and return control immediately */

    EFUN(merlin_mptwo_poll_uc_dsc_ready_for_cmd_equals_1(timeout_ms)); /* Poll for uc_dsc_ready_for_cmd = 1 to indicate merlin_mptwo ready for command */
    return(ERR_CODE_NONE);
}

err_code_t merlin_mptwo_pmd_uc_control_return_immediate(enum srds_pmd_uc_ctrl_cmd_enum control) {
  return(merlin_mptwo_pmd_uc_cmd_return_immediate(CMD_UC_CTRL, (uint8_t) control));
}

err_code_t merlin_mptwo_pmd_uc_control(enum srds_pmd_uc_ctrl_cmd_enum control, uint32_t timeout_ms) {
  return(merlin_mptwo_pmd_uc_cmd(CMD_UC_CTRL, (uint8_t) control, timeout_ms));
}

err_code_t merlin_mptwo_pmd_uc_diag_cmd(enum srds_pmd_uc_diag_cmd_enum control, uint32_t timeout_ms) {
  return(merlin_mptwo_pmd_uc_cmd(CMD_DIAG_EN, (uint8_t) control, timeout_ms));
}
...
uint8_t merlin_mptwo_rdbl_uc_var(err_code_t *err_code_p, uint16_t addr) {
    uint8_t rddata;
    ...
    EPSTM(rddata = merlin_mptwo_rdb_uc_ram(err_code_p, (LANE_VAR_RAM_BASE+addr+(merlin_mptwo_get_lane()*LANE_VAR_RAM_SIZE)))); /* Use Micro register interface for reading RAM */

    return (rddata);
}

/* Micro RAM Lane Byte Signed Read */
int8_t merlin_mptwo_rdbls_uc_var(err_code_t *err_code_p, uint16_t addr) {
  return ((int8_t) merlin_mptwo_rdbl_uc_var(err_code_p, addr));
}

/* Micro RAM Lane Word Read */
uint16_t merlin_mptwo_rdwl_uc_var(err_code_t *err_code_p, uint16_t addr) {
  uint16_t rddata;
  ...
  EPSTM(rddata = merlin_mptwo_rdw_uc_ram(err_code_p, (LANE_VAR_RAM_BASE+addr+(merlin_mptwo_get_lane()*LANE_VAR_RAM_SIZE)))); /* Use Micro register interface for reading RAM */
...
int16_t merlin_mptwo_rdwls_uc_var(err_code_t *err_code_p, uint16_t addr) {
  return ((int16_t) merlin_mptwo_rdwl_uc_var(err_code_p, addr));
}

/* Micro RAM Lane Byte Write */
err_code_t merlin_mptwo_wrbl_uc_var(uint16_t addr, uint8_t wr_val) {


    if(addr < LANE_VAR_RAM_SIZE) {
        return (merlin_mptwo_wrb_uc_ram((LANE_VAR_RAM_BASE+addr+(merlin_mptwo_get_lane()*LANE_VAR_RAM_SIZE)), wr_val));    /* Use Micro register interface for writing RAM */
    } else {
...
err_code_t merlin_mptwo_wrbls_uc_var(uint16_t addr, int8_t wr_val) {
  return (merlin_mptwo_wrbl_uc_var(addr, wr_val));
}

/* Micro RAM Lane Word Write */
err_code_t merlin_mptwo_wrwl_uc_var(uint16_t addr, uint16_t wr_val) {
    ...
    if(addr < LANE_VAR_RAM_SIZE) {
        return (merlin_mptwo_wrw_uc_ram((LANE_VAR_RAM_BASE+addr+(merlin_mptwo_get_lane()*LANE_VAR_RAM_SIZE)), wr_val));    /* Use Micro register interface for writing RAM */
    } else {
        return(_error(ERR_CODE_INVALID_RAM_ADDR));
    }
}
...
err_code_t merlin_mptwo_wrwls_uc_var(uint16_t addr, int16_t wr_val) {
  return (merlin_mptwo_wrwl_uc_var(addr,wr_val));
}
...
uint8_t merlin_mptwo_rdbc_uc_var(err_code_t *err_code_p, uint8_t addr) {
  ...
  EPSTM(rddata = merlin_mptwo_rdb_uc_ram(err_code_p, (CORE_VAR_RAM_BASE+addr)));                      /* Use Micro register interface for reading RAM */
  return (rddata);
}

/* Micro RAM Core Byte Signed Read */
int8_t merlin_mptwo_rdbcs_uc_var(err_code_t *err_code_p, uint8_t addr) {
  return ((int8_t) merlin_mptwo_rdbc_uc_var(err_code_p, addr));
}

/* Micro RAM Core Word Read */
uint16_t merlin_mptwo_rdwc_uc_var(err_code_t *err_code_p, uint8_t addr) {
  ...
  EPSTM(rddata = merlin_mptwo_rdw_uc_ram(err_code_p, (CORE_VAR_RAM_BASE+addr)));                  /* Use Micro register interface for reading RAM */
  return (rddata);
}

/* Micro RAM Core Word Signed Read */
int16_t merlin_mptwo_rdwcs_uc_var(err_code_t *err_code_p, uint8_t addr) {
  return ((int16_t) merlin_mptwo_rdwc_uc_var(err_code_p, addr));
}

/* Micro RAM Core Byte Write  */
err_code_t merlin_mptwo_wrbc_uc_var(uint8_t addr, uint8_t wr_val) {


    if(addr < CORE_VAR_RAM_SIZE) {
    return (merlin_mptwo_wrb_uc_ram((CORE_VAR_RAM_BASE+addr), wr_val));                                /* Use Micro register interface for writing RAM */
    } else {
        return(_error(ERR_CODE_INVALID_RAM_ADDR));
    }
}
...
err_code_t merlin_mptwo_wrbcs_uc_var(uint8_t addr, int8_t wr_val) {
  return (merlin_mptwo_wrbc_uc_var(addr, wr_val));
}

/* Micro RAM Core Word Write  */
err_code_t merlin_mptwo_wrwc_uc_var(uint8_t addr, uint16_t wr_val) {
    if (addr%2 != 0) {                                                                /* Validate even address */
        return (_error(ERR_CODE_INVALID_RAM_ADDR));
    }
    if(addr < CORE_VAR_RAM_SIZE) {
        return (merlin_mptwo_wrw_uc_ram((CORE_VAR_RAM_BASE+addr), wr_val));                                 /* Use Micro register interface for writing RAM */
    } else {
        return(_error(ERR_CODE_INVALID_RAM_ADDR));
    }
}
...
err_code_t merlin_mptwo_wrwcs_uc_var(uint8_t addr, int16_t wr_val) {
  return(merlin_mptwo_wrwc_uc_var(addr,wr_val));
}
...
err_code_t merlin_mptwo_set_uc_core_config(struct merlin_mptwo_uc_core_config_st struct_val) {
    {   
#ifndef ATE_LOG
        uint8_t reset_state;
        ESTM(reset_state = rdc_core_dp_reset_state());
        if(reset_state < 7) {
            EFUN_PRINTF(("ERROR: merlin_mptwo_set_uc_core_config(..) called without core_dp_s_rstb=0 Lane=%d reset_state=%d\n",merlin_mptwo_get_lane(),reset_state));
...
err_code_t merlin_mptwo_set_usr_ctrl_core_event_log_level(uint8_t core_event_log_level) {
  return(wrcv_usr_ctrl_core_event_log_level(core_event_log_level));
}

err_code_t merlin_mptwo_set_uc_lane_cfg(struct merlin_mptwo_uc_lane_config_st struct_val) {
#ifndef ATE_LOG
    uint8_t reset_state;

    ESTM(reset_state = rd_lane_dp_reset_state());
    if(reset_state < 7) {
        EFUN_PRINTF(("ERROR: merlin_mptwo_set_uc_lane_cfg(..) called without ln_dp_s_rstb=0 Lane=%d reset_state=%d\n",merlin_mptwo_get_lane(),reset_state));
...
err_code_t merlin_mptwo_set_usr_ctrl_lane_event_log_level(uint8_t lane_event_log_level) {
  return(wrv_usr_ctrl_lane_event_log_level(lane_event_log_level));
}

err_code_t merlin_mptwo_set_usr_ctrl_disable_startup(struct merlin_mptwo_usr_ctrl_disable_functions_st set_val) {
  EFUN(_update_usr_ctrl_disable_functions_byte(&set_val));
  return(wrv_usr_ctrl_disable_startup_functions_byte(set_val.byte));
}

err_code_t merlin_mptwo_set_usr_ctrl_disable_startup_dfe(struct merlin_mptwo_usr_ctrl_disable_dfe_functions_st set_val) {
...
err_code_t merlin_mptwo_set_usr_ctrl_disable_steady_state(struct merlin_mptwo_usr_ctrl_disable_functions_st set_val) {
...
err_code_t merlin_mptwo_set_usr_ctrl_disable_steady_state_dfe(struct merlin_mptwo_usr_ctrl_disable_dfe_functions_st set_val) {
  EFUN(_update_usr_ctrl_disable_dfe_functions_byte(&set_val));
...
/******************************************************************/
err_code_t merlin_mptwo_get_uc_core_config(struct merlin_mptwo_uc_core_config_st *get_val)
{
...
err_code_t merlin_mptwo_get_usr_ctrl_core_event_log_level(uint8_t *core_event_log_level) {
...
err_code_t merlin_mptwo_get_uc_lane_cfg(struct merlin_mptwo_uc_lane_config_st *get_val) {
...
err_code_t merlin_mptwo_get_usr_ctrl_lane_event_log_level(uint8_t *lane_event_log_level) {
...
err_code_t merlin_mptwo_get_usr_ctrl_disable_startup(struct merlin_mptwo_usr_ctrl_disable_functions_st *get_val) {
...
err_code_t merlin_mptwo_get_usr_ctrl_disable_startup_dfe(struct merlin_mptwo_usr_ctrl_disable_dfe_functions_st *get_val) {
...
err_code_t merlin_mptwo_get_usr_ctrl_disable_steady_state(struct merlin_mptwo_usr_ctrl_disable_functions_st *get_val) {
...
err_code_t merlin_mptwo_get_usr_ctrl_disable_steady_state_dfe(struct merlin_mptwo_usr_ctrl_disable_dfe_functions_st *get_val) {
...
err_code_t merlin_mptwo_get_clk90_offset_adjust(int8_t *adjust) {
  if (!adjust) {
    return(_error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
  }
...
err_code_t merlin_mptwo_set_clk90_offset_adjust(int8_t adjust) {
...
err_code_t merlin_mptwo_get_clk90_offset_override(uint8_t *override) {
  if (!override) {
    return(_error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
  }
...
err_code_t merlin_mptwo_set_clk90_offset_override(uint8_t enable, uint8_t override) {        
...
err_code_t merlin_mptwo_get_clk90_offset(int8_t *offset) {
    int8_t clk90_offset;
...
err_code_t merlin_mptwo_poll_diag_done(uint16_t *status, uint32_t timeout_ms) {
 uint8_t loop;
 ...
 for(loop=0;loop < 100; loop++) {
     ...
     if(loop>10) {
         EFUN(merlin_mptwo_delay_us(10*timeout_ms));
     }
 }
...
/* poll for microcontroller to populate the dsc_data register */
err_code_t merlin_mptwo_poll_diag_eye_data(uint32_t *data,uint16_t *status, uint32_t timeout_ms) {
 uint8_t loop;
 uint16_t dscdata;
 if(!data || !status) {
     return(_error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
 ...
 for(loop=0;loop < 100; loop++) {
     ...
     if(((*status & 0x00FF) > 2) || ((*status & 0x8000) > 0)) {
        EFUN(merlin_mptwo_pmd_uc_cmd( CMD_READ_DIAG_DATA_WORD, 0, 200));
...
err_code_t merlin_mptwo_poll_uc_dsc_ready_for_cmd_equals_1(uint32_t timeout_ms) {
#ifndef ATE_LOG
  uint16_t loop;
  ...
  for (loop = 0; loop < 100; loop++) {
    uint16_t rddata;
    EFUN(merlin_mptwo_pmd_rdt_reg(DSC_A_DSC_UC_CTRL, &rddata));
    if (rddata & 0x0080) {    /* bit 7 is uc_dsc_ready_for_cmd */
      if (rddata & 0x0040) {  /* bit 6 is uc_dsc_error_found   */
        ...
        EFUN(merlin_mptwo_pmd_wr_reg(DSC_A_DSC_UC_CTRL,0x80));
...
#else
  {   uint16_t rddata;
      EFUN_PRINTF(("// ATE_LOG merlin_mptwo_poll_uc_dsc_ready_for_cmd_equals_1 begin\n"));
      if(timeout_ms < 100) {
          EFUN(merlin_mptwo_delay_us(1000 * timeout_ms));
      } else {
          EFUN(merlin_mptwo_delay_ms(timeout_ms));
      }
      EFUN(merlin_mptwo_pmd_rdt_reg(DSC_A_DSC_UC_CTRL, &rddata));
      if (rddata & 0x0080) {    /* bit 7 is uc_dsc_ready_for_cmd */
          if (rddata & 0x0040) {  /* bit 6 is uc_dsc_error_found   */
          ...
          EFUN_PRINTF(("// ATE_LOG merlin_mptwo_poll_uc_dsc_ready_for_cmd_equals_1 end\n"));
          return (ERR_CODE_NONE);
      }
      EFUN_PRINTF(("// ATE_LOG merlin_mptwo_poll_uc_dsc_ready_for_cmd_equals_1 timeout\n"));
  }
#endif
...
err_code_t merlin_mptwo_poll_dsc_state_equals_uc_tune(uint32_t timeout_ms) {
  uint16_t loop;
  uint16_t dsc_state;
...
err_code_t merlin_mptwo_poll_st_afe_tx_fifo_resetb_equals_1(uint32_t timeout_ms) {
  uint16_t loop;
...
err_code_t merlin_mptwo_tx_ull_config(uint8_t enable) {
    EFUN(merlin_mptwo_tx_rptr_mode_timing(enable)); /* Enable jitter filter enable */
    EFUN(_ull_tx_mode_setup(enable));
    return (ERR_CODE_NONE);
}

/* Configure ULL mode for RX */
err_code_t merlin_mptwo_rx_ull_config(uint8_t enable) {
    EFUN(merlin_mptwo_rx_rptr_mode_timing(enable));
    EFUN(_ull_rx_mode_setup(enable));
    return (ERR_CODE_NONE);
}

/* TX PI setup for Repeater Mode */
err_code_t merlin_mptwo_tx_rptr_mode_timing(uint8_t enable) {
        ...
        EFUN(merlin_mptwo_delay_us(25));                            /* Wait for TX_PI to settle */
    return (ERR_CODE_NONE);
}

/* RX setup for Repeater Mode timing */
err_code_t merlin_mptwo_rx_rptr_mode_timing(uint8_t enable) {
    (void)enable;
...
err_code_t merlin_mptwo_init_pram_for_uc_load(uint16_t ucode_len) {
    uint8_t result;
    ...
    EFUN(merlin_mptwo_delay_us(500));
    ...
    /* Wait 8 pram clocks */
    /* call merlin_mptwo_finish_pram_load */
    return(ERR_CODE_NONE);
}

err_code_t merlin_mptwo_finish_pram_load(void) {
...
err_code_t merlin_mptwo_reg_dump(void) {
  ...
  for (addr = 0x0; addr < 0x10; addr++) {
    ...
    EFUN(merlin_mptwo_pmd_rdt_reg(addr,&rddata));
...
err_code_t merlin_mptwo_uc_core_var_dump(void) {
  uint8_t addr;
  ...
  for (addr = 0x0; addr < CORE_VAR_RAM_SIZE; addr++) {
    ...
    ESTM_PRINTF(("%02x ", merlin_mptwo_rdbc_uc_var(__ERR, addr)));
  }
  return (ERR_CODE_NONE);
}


err_code_t merlin_mptwo_uc_lane_var_dump(void) {
  uint8_t     rx_lock, uc_stopped = 0;
  uint16_t    addr;


  EFUN_PRINTF(("\n**** SERDES UC LANE %d RAM VARIABLE DUMP ****",merlin_mptwo_get_lane()));
  ...
  if (rx_lock == 1) {
      ...
      if (!uc_stopped) {
          EFUN(merlin_mptwo_stop_rx_adaptation(1));
      }
  } else {
      EFUN(merlin_mptwo_pmd_uc_control(CMD_UC_CTRL_STOP_IMMEDIATE,200));
  }
    ...
    for (addr = 0x0; addr < LANE_VAR_RAM_SIZE; addr++) {
      ...
      ESTM_PRINTF(("%02x ", merlin_mptwo_rdbl_uc_var(__ERR, addr)));
    }

  if (rx_lock == 1) {
      if (!uc_stopped) {
          EFUN(merlin_mptwo_stop_rx_adaptation(0));
      }
  } else {
      EFUN(merlin_mptwo_stop_rx_adaptation(0));
  }
...
err_code_t merlin_mptwo_display_diag_data(int32_t  diag_level) {
    ...
    ESTM_PRINTF(("Core = %d; LANE = %d\n",merlin_mptwo_get_core(),merlin_mptwo_get_lane()));
    EFUN(merlin_mptwo_version(&api_version));
    ...
    if (rx_lock == 1) {
        ...
        EFUN(merlin_mptwo_display_lane_state_hdr());
        EFUN(merlin_mptwo_display_lane_state());
    ...
    if(diag_level & SRDS_DIAG_EVENT) {
        ...
        EFUN(merlin_mptwo_read_event_log(trace_mem,EVENT_LOG_DECODED));
    }
    if(diag_level & SRDS_DIAG_EYE) {
        EFUN(merlin_mptwo_display_eye_scan());
    }
    if(diag_level & SRDS_DIAG_REG_CORE) {
        EFUN(merlin_mptwo_reg_dump());
    }
    ...
    if(diag_level & SRDS_DIAG_UC_CORE) {
        EFUN(merlin_mptwo_uc_core_var_dump());
    }
    if(diag_level & SRDS_DIAG_UC_LANE) {
        EFUN(merlin_mptwo_uc_lane_var_dump());
    }
    if(diag_level & SRDS_DIAG_LANE_DEBUG) {
        EFUN(merlin_mptwo_display_lane_debug_status());
    }
    if(diag_level & SRDS_DIAG_BER_VERT) {
        ...
        EFUN(_merlin_mptwo_display_ber_scan_data(ber_mode, timer_control,  err_threshold));
...
/* Required Diagnostic Functions */
err_code_t merlin_mptwo_diag_access(enum srds_diag_access_enum type, uint16_t addr, uint16_t data, uint16_t param) {
    ...
    case SRDS_REG_READ:      {
                             ...
                             if(data > 1) {
                                 ...
                                 for (i = 0; i < data; i++) {
                                     ...
                                     EFUN(merlin_mptwo_pmd_rdt_reg(i+addr,&tmp));
                                     ESTM_PRINTF(("%04x ", tmp));
                                 }
                                 EFUN_PRINTF(("\n"));
                             } else {
                                 EFUN(merlin_mptwo_pmd_rdt_reg(addr,&tmp));
    ...
    case SRDS_REG_RMW:       {
                             ...
                             EFUN(merlin_mptwo_pmd_wr_reg(addr,tmp2));
    ...
    case SRDS_CORE_RAM_READ_BYTE: {
                             if(data > 1) {
                                 ...
                                 for (i = 0; i < (uint8_t)data; i++) {
                                     ...
                                     ESTM_PRINTF(("%02x ", merlin_mptwo_rdbc_uc_var(__ERR, i+(uint8_t)addr)));
                                 }
                                 EFUN_PRINTF(("\n"));
                             } else {
                                 ESTM_PRINTF(("Core RAM Read byte: x%04x = x%02x\n",(uint8_t)addr,merlin_mptwo_rdbc_uc_var(__ERR, (uint8_t)addr)));
                             }
                             } break;
    case SRDS_LANE_RAM_READ_BYTE: {
                             if(data > 1) {
                                 ...
                                 for (i = 0; i < data; i++) {
                                     ...
                                     ESTM_PRINTF(("%02x ", merlin_mptwo_rdbl_uc_var(__ERR,i+addr)));
                                 }
                                 EFUN_PRINTF(("\n"));
                             } else {
                                 ESTM_PRINTF(("Lane RAM Read byte: x%04x = x%02x\n",addr,merlin_mptwo_rdbl_uc_var(__ERR,addr)));
                             }
                             } break;
    case SRDS_CORE_RAM_READ_WORD: {
                             if(data > 1) {
                                 ...
                                 for (i = 0; i < (uint8_t)data; i+=2) {
                                     ...
                                     ESTM_PRINTF(("%04x ", merlin_mptwo_rdwc_uc_var(__ERR, i+(uint8_t)addr)));
                                 }
                                 EFUN_PRINTF(("\n"));
                             } else {
                                 ESTM_PRINTF(("Core RAM Read word: x%04x = x%04x\n",addr,merlin_mptwo_rdwc_uc_var(__ERR, (uint8_t)addr)));
                             }
                             } break;
    case SRDS_LANE_RAM_READ_WORD: {
                             if(data > 1) {
                                 ...
                                 for (i = 0; i < data; i+=2) {
                                     ...
                                     ESTM_PRINTF(("%04x ", merlin_mptwo_rdwl_uc_var(__ERR,i+addr)));
                                 }
                                 EFUN_PRINTF(("\n"));
                             } else {
                                 ESTM_PRINTF(("Lane RAM Read word: x%04x = x%04x\n",addr,merlin_mptwo_rdwl_uc_var(__ERR, addr)));
                             }
                             } break;
    case SRDS_CORE_RAM_RMW_BYTE:  {
                             uint8_t tmp, tmp2;
                             ESTM(tmp = merlin_mptwo_rdbc_uc_var(__ERR, (uint8_t)addr));
                             tmp2 = (tmp & (uint8_t)~param) | ((uint8_t)param & data);
                             EFUN(merlin_mptwo_wrbc_uc_var((uint8_t)addr,tmp2));
                             EFUN_PRINTF(("Core RAM RMW byte: x%04x = x%02x -> x%02x\n",addr,tmp,tmp2));
                             } break;
    case SRDS_LANE_RAM_RMW_BYTE:  {
                             uint8_t tmp, tmp2;
                             ESTM(tmp = merlin_mptwo_rdbl_uc_var(__ERR,addr));
                             tmp2 = (tmp & (uint8_t)~param) | ((uint8_t)param & data);
                             EFUN(merlin_mptwo_wrbl_uc_var(addr,tmp2));
                             EFUN_PRINTF(("Lane RAM RMW byte: x%04x = x%02x -> x%02x\n",addr,tmp,tmp2));
                             } break;
    case SRDS_CORE_RAM_RMW_WORD:  {
                             uint16_t tmp, tmp2;
                             ESTM(tmp = merlin_mptwo_rdwc_uc_var(__ERR, (uint8_t)addr));
                             tmp2 = (tmp & ~param) | (param & data);
                             EFUN(merlin_mptwo_wrwc_uc_var((uint8_t)addr,tmp2));
                             EFUN_PRINTF(("Core RAM RMW word: x%04x = x%04x -> x%04x\n",addr,tmp,tmp2));
                             } break;
    case SRDS_LANE_RAM_RMW_WORD:  {
                             uint16_t tmp, tmp2;
                             ESTM(tmp = merlin_mptwo_rdwl_uc_var(__ERR,addr));
                             tmp2 = (tmp & ~param) | (param & data);
                             EFUN(merlin_mptwo_wrwl_uc_var(addr,tmp2));
    ...
    case SRDS_GLOB_RAM_READ_BYTE: {
                             if(data > 1) {
                                 ...
                                 for (i = 0; i < data; i++) {
                                     ...
                                     ESTM_PRINTF(("%02x ", _merlin_mptwo_rdb_uc_var(__ERR,i+addr)));
                                 }
                                 EFUN_PRINTF(("\n"));
                             } else {
                                 ESTM_PRINTF(("Glob RAM Read byte: x%04x = x%02x\n",addr,_merlin_mptwo_rdb_uc_var(__ERR,addr)));
                             }
                             } break;
    case SRDS_GLOB_RAM_RMW_BYTE:  {
                             uint8_t tmp, tmp2;
                             ESTM(tmp = _merlin_mptwo_rdb_uc_var(__ERR,addr));
                             tmp2 = (tmp & (uint8_t)~param) | ((uint8_t)param & data);
                             EFUN(_merlin_mptwo_wrb_uc_var(addr,tmp2));
    ...
    case SRDS_GLOB_RAM_READ_WORD: {
                             if(data > 1) {
                                 ...
                                 for (i = 0; i < data; i+=2) {
                                     ...
                                     ESTM_PRINTF(("%04x ", _merlin_mptwo_rdw_uc_var(__ERR,i+addr)));
                                 }
                                 EFUN_PRINTF(("\n"));
                             } else {
                                 ESTM_PRINTF(("Glob RAM Read word: x%04x = x%04x\n",addr,_merlin_mptwo_rdw_uc_var(__ERR,addr)));
                             }
                             } break;
    case SRDS_GLOB_RAM_RMW_WORD:  {
                             uint16_t tmp, tmp2;
                             ESTM(tmp = _merlin_mptwo_rdw_uc_var(__ERR,addr));
                             tmp2 = (tmp & ~param) | (param & data);
                             EFUN(_merlin_mptwo_wrw_uc_var(addr,tmp2));
                             EFUN_PRINTF(("Glob RAM RMW word: x%04x = x%04x -> x%04x\n",addr,tmp,tmp2));
                             } break;
    case SRDS_UC_CMD:        {
                             uint16_t tmp;
                             EFUN(merlin_mptwo_pmd_uc_cmd_with_data((enum srds_pmd_uc_cmd_enum)addr, (uint8_t)param, data, 100));
                             ESTM(tmp = rd_uc_dsc_data());
                             EFUN_PRINTF(("uC Command: cmd=x%02x supp=x%02x data=x%04x returned=x%04x\n",addr,param,data,tmp));
                             } break;
    case SRDS_BER_PROJ_DATA: {
                             /* display ber projections for all channels */
                             EFUN(_merlin_mptwo_display_ber_scan_data((uint8_t)addr, (uint8_t)data,  (uint8_t)(param>>4)));
                             } break;
    ...
    default: EFUN_PRINTF(("Invalid request type merlin_mptwo_diag_access\n"));
    }
...
err_code_t merlin_mptwo_version(uint32_t *api_version) {

    if(!api_version) {
        return(_error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
    }
    *api_version = 0xA10220;
    return (ERR_CODE_NONE);
}
...
err_code_t merlin_mptwo_read_event_log(uint8_t *trace_mem,enum srds_event_log_display_mode_enum display_mode) {
    ...
    EFUN(merlin_mptwo_event_log_stop());

    EFUN(merlin_mptwo_event_log_readmem(trace_mem));

    EFUN(merlin_mptwo_event_log_display(trace_mem, display_mode));

    return(ERR_CODE_NONE);
}

err_code_t merlin_mptwo_event_log_stop(void) {
    ...
    EFUN(merlin_mptwo_pmd_uc_cmd(CMD_EVENT_LOG_READ, CMD_EVENT_LOG_READ_START, 1000));

    return(ERR_CODE_NONE);
}

err_code_t merlin_mptwo_event_log_readmem(uint8_t *trace_mem) {
    ...
    do {
        /* Read Next */
        EFUN(merlin_mptwo_pmd_uc_cmd(CMD_EVENT_LOG_READ, CMD_EVENT_LOG_READ_NEXT, 1000));
    ...
    EFUN(merlin_mptwo_pmd_uc_cmd(CMD_EVENT_LOG_READ, CMD_EVENT_LOG_READ_DONE, 1000));

    return(ERR_CODE_NONE);
}

err_code_t merlin_mptwo_event_log_display(uint8_t *trace_mem,enum srds_event_log_display_mode_enum display_mode) {
    ...
    if (display_mode == EVENT_LOG_DECODED || display_mode == EVENT_LOG_HEX_AND_DECODED) {
        ...
        while (addr < trace_mem_size-8) {                                                            /* last 8-byte reserved for trace memory wraparound */
            ...
            if (*(trace_mem+addr) == 0xff) {                                                         /* timestamp wraparound event handling */
                ...
                EFUN(merlin_mptwo_uc_lane_idx_to_system_id(uc_lane_id_str, cc));
...
err_code_t merlin_mptwo_display_state (void) {
    EFUN(merlin_mptwo_display_core_state());
    EFUN(merlin_mptwo_display_lane_state_hdr());
    EFUN(merlin_mptwo_display_lane_state());
    EFUN(merlin_mptwo_display_lane_state_legend());
    return(ERR_CODE_NONE);
}

err_code_t merlin_mptwo_display_config (void) {
    EFUN(merlin_mptwo_display_core_config());
    EFUN(merlin_mptwo_display_lane_config());
    return(ERR_CODE_NONE);
}
...
err_code_t merlin_mptwo_force_die_temperature (int16_t die_temp) {
...
err_code_t merlin_mptwo_read_die_temperature (int16_t *die_temp) {

    uint16_t die_temp_sensor_reading;

    EFUN(merlin_mptwo_pmd_uc_cmd(CMD_UC_DBG,CMD_UC_DBG_DIE_TEMP,100));
...
err_code_t merlin_mptwo_read_die_temperature_double (USR_DOUBLE *die_temp) {
#ifdef SERDES_API_FLOATING_POINT
    /* Formula for PVTMON: T = 410.04-0.48705*reg10bit (from PVTMON Analog Module Specification v5.0 section 6.2) */
    uint16_t die_temp_sensor_reading;
    ...
    *die_temp = 410.04-0.48705* ((USR_DOUBLE)die_temp_sensor_reading);
    ...
    return(ERR_CODE_NONE);
#else
    EFUN_PRINTF((" Function 'merlin_mptwo_read_die_temperature_double' needs SERDES_API_FLOATING_POINT defined to operate \n"));
    return(_error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
#endif
}
