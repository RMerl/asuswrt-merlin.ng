...
static err_code_t _ull_tx_mode_setup(uint8_t enable) {
    ...
    EFUN(merlin_mptwo_poll_st_afe_tx_fifo_resetb_equals_1(1));  /* Check if FIFO reset is reported to be released within 1ms time interval */
...
static uint8_t _merlin_mptwo_rdb_uc_var(err_code_t *err_code_p, uint16_t addr) {
    ...
    EPSTM(rddata = merlin_mptwo_rdb_uc_ram(err_code_p, addr)); /* Use Micro register interface for reading RAM */
...
#endif
  EPSTM(rddata = merlin_mptwo_rdw_uc_ram(err_code_p, (addr))); /* Use Micro register interface for reading RAM */
...
static err_code_t _merlin_mptwo_wrb_uc_var(uint16_t addr, uint8_t wr_val) {
    ...
    return (merlin_mptwo_wrb_uc_ram(addr, wr_val));                                /* Use Micro register interface for writing RAM */
}
...
static err_code_t _merlin_mptwo_wrw_uc_var(uint16_t addr, uint16_t wr_val) {
    ...
    return (merlin_mptwo_wrw_uc_ram(addr, wr_val));                                 /* Use Micro register interface for writing RAM */
}
...
static err_code_t _display_event(uint8_t event_id,
    ...
    case EVENT_CODE_FLR_DEACTIVATED_DUE_TO_TIMEOUT:
        ...
        break;
    case EVENT_CODE_RESTART_PMD_ON_EYE_CHECK_FAIL:
        EFUN_PRINTF(("  Restart Rx PMD due to Eye Opening Check Fail"));
        break;
    default:
...
SDK_STATIC err_code_t _merlin_mptwo_get_osr_mode(merlin_mptwo_osr_mode_st *imode) {
...
SDK_STATIC err_code_t _merlin_mptwo_read_lane_state(merlin_mptwo_lane_state_st *istate) {
  ...
  EFUN(_merlin_mptwo_pmd_lock_status(&state.rx_lock, &state.rx_lock_chg));
  ...
  EFUN(_merlin_mptwo_sigdet_status(&state.sig_det, &state.sig_det_chg));
  ESTM(state.rx_ppm = rd_cdr_integ_reg()/84);
  EFUN(merlin_mptwo_get_clk90_offset(&state.clk90));
...
static err_code_t _merlin_mptwo_display_lane_state_no_newline(void) {
  ...
  EFUN(_merlin_mptwo_read_lane_state(&state));

  lane_idx = merlin_mptwo_get_lane();
...
SDK_STATIC err_code_t _merlin_mptwo_read_core_state(merlin_mptwo_core_state_st *istate) {
  ...
  EFUN(merlin_mptwo_version(&state.api_version));
  ESTM(state.afe_hardware_version = rdcv_afe_hardware_version());
  ESTM(state.temp_idx             = rdcv_temp_idx());
  {  int16_t                           die_temp = 0;
     EFUN(merlin_mptwo_read_die_temperature(&die_temp));
  ...
  EFUN(_merlin_mptwo_pll_lock_status(&state.pll_lock, &state.pll_lock_chg));
...
static err_code_t _merlin_mptwo_display_core_state_no_newline(void) {
    ...
    ENULL_MEMSET(&state     , 0, sizeof(state     ));
        EFUN(_merlin_mptwo_read_core_state(&state));
    EFUN_PRINTF((" %02d "              ,  merlin_mptwo_get_core()));
    ...
    EFUN(_merlin_mptwo_display_pll_to_divider(  state.pll_div));
...
SDK_STATIC err_code_t _merlin_mptwo_pll_lock_status(uint8_t *pll_lock, uint8_t *pll_lock_chg) {
    ...
    uint16_t rddata;
    EFUN(merlin_mptwo_pmd_rdt_reg(PLL_STATUS_ADDR, &rddata));
...
SDK_STATIC err_code_t _merlin_mptwo_pmd_lock_status(uint8_t *pmd_lock, uint8_t *pmd_lock_chg) {
    ...
    uint16_t rddata;
    EFUN(merlin_mptwo_pmd_rdt_reg(PMD_LOCK_STATUS_ADDR, &rddata));
...
SDK_STATIC err_code_t _merlin_mptwo_sigdet_status(uint8_t *sig_det, uint8_t *sig_det_chg) {
    uint16_t rddata;
    EFUN(merlin_mptwo_pmd_rdt_reg(SIGDET_STATUS_ADDR, &rddata));
...
static err_code_t _merlin_mptwo_display_ber_scan_data(uint8_t ber_scan_mode, uint8_t timer_control, uint8_t max_error_control) {
    ...
    EFUN(merlin_mptwo_get_uc_core_config(&core_config));
    EFUN(_merlin_mptwo_get_osr_mode(&osr_mode));
    ...
    for(i=0;i < cnt;i++) {
        ...
        EFUN(merlin_mptwo_pmd_uc_cmd( CMD_READ_DIAG_DATA_WORD, 0, 100));
        ...
        EFUN(merlin_mptwo_pmd_uc_cmd( CMD_READ_DIAG_DATA_BYTE, 0, 100));
    ...
    EFUN_PRINTF(("\n"));
    EFUN(merlin_mptwo_pmd_uc_cmd(CMD_CAPTURE_BER_END,0x00,2000));
