...
/***********************************************************************************
 ***********************************************************************************
 *  File Name     :  merlin16_shasta_debug_functions.c                                  *
...
/** @file merlin16_shasta_debug_functions.c
...
static err_code_t _merlin16_shasta_reg_print_no_buf(srds_access_t *sa__, uint8_t reg_section);
static err_code_t _merlin16_shasta_reg_print_with_buf(srds_access_t *sa__, uint8_t reg_section, char reg_buffer[][250], uint8_t *buf_index);
static err_code_t _merlin16_shasta_reg_print(srds_access_t *sa__, uint8_t reg_section, char reg_buffer[][250], uint8_t *buf_index);
...
err_code_t merlin16_shasta_stop_uc_lane(srds_access_t *sa__, uint8_t enable) {
    ...
    return(merlin16_shasta_stop_rx_adaptation(sa__,enable));
}
...
err_code_t merlin16_shasta_set_usr_ctrl_disable_startup(srds_access_t *sa__, struct merlin16_shasta_usr_ctrl_disable_functions_st set_val) {
  EFUN(merlin16_shasta_INTERNAL_update_usr_ctrl_disable_functions_byte(&set_val));
...
err_code_t merlin16_shasta_set_usr_ctrl_disable_startup_dfe(srds_access_t *sa__, struct merlin16_shasta_usr_ctrl_disable_dfe_functions_st set_val) {
  EFUN(merlin16_shasta_INTERNAL_update_usr_ctrl_disable_dfe_functions_byte(&set_val));
...
err_code_t merlin16_shasta_get_usr_ctrl_disable_startup(srds_access_t *sa__, struct merlin16_shasta_usr_ctrl_disable_functions_st *get_val) {
  ...
  EFUN(merlin16_shasta_INTERNAL_update_usr_ctrl_disable_functions_st(get_val));
  return (ERR_CODE_NONE);
}
...
err_code_t merlin16_shasta_get_usr_ctrl_disable_startup_dfe(srds_access_t *sa__, struct merlin16_shasta_usr_ctrl_disable_dfe_functions_st *get_val) {
  ...
  EFUN(merlin16_shasta_INTERNAL_update_usr_ctrl_disable_dfe_functions_st(get_val));
  return (ERR_CODE_NONE);
}
...
static err_code_t _merlin16_shasta_reg_print_no_buf(srds_access_t *sa__, uint8_t reg_section)
{
    ...
    for (addr = merlin16_shasta_reg_dump_arr[reg_section][0]; addr < merlin16_shasta_reg_dump_arr[reg_section][1]; addr++) 
    {
        if (!(addr & 0x000F))  
        {
            EFUN_PRINTF(("\n%04x ",addr));
        }
        EFUN(merlin16_shasta_acc_rdt_reg(sa__, addr,&rddata));
...
static err_code_t _merlin16_shasta_reg_print(srds_access_t *sa__, uint8_t reg_section, char reg_buffer[][250], uint8_t *buf_index)
{
    if(reg_buffer == NULL)
    {
         EFUN(_merlin16_shasta_reg_print_no_buf(sa__, reg_section));
    }
    else
    {
        EFUN(_merlin16_shasta_reg_print_with_buf(sa__, reg_section, reg_buffer, buf_index));
    }
    return (ERR_CODE_NONE);
}
...
{    
    ...
    while(reg_section < MERLIN16_SHASTA_REG_DUMP_SECTIONS) 
    {
        if(merlin16_shasta_reg_dump_arr[reg_section][0] == MERLIN16_SHASTA_REG_PLL_START)
        {
                while( reg_section < MERLIN16_SHASTA_REG_DUMP_SECTIONS && merlin16_shasta_reg_dump_arr[reg_section][0] != MERLIN16_SHASTA_REG_UC_START)
                {
                    EFUN(_merlin16_shasta_reg_print(sa__, reg_section, reg_buffer, &buf_index));
                    reg_section++;
                }
        }
        else if (merlin16_shasta_reg_dump_arr[reg_section][0] == MERLIN16_SHASTA_REG_UC_START)
        {
            ...
            ESTM(micro_orig = merlin16_shasta_get_micro_idx(sa__));
            for(micro_idx = 0; micro_idx < num_micros; micro_idx++)
            {
                reg_section = uc_start_section;
                EFUN(merlin16_shasta_set_micro_idx(sa__,micro_idx));
#endif
                while(reg_section < MERLIN16_SHASTA_REG_DUMP_SECTIONS)
                {
                    EFUN(_merlin16_shasta_reg_print(sa__, reg_section, reg_buffer, &buf_index));
            ...
            EFUN(merlin16_shasta_set_micro_idx(sa__,micro_orig));
#endif
        }
        else
        {
...
err_code_t merlin16_shasta_uc_core_var_dump(srds_access_t *sa__, uint8_t core, char ram_buffer[][250]) {
  ...
  if (ram_buffer == NULL) {
      ...
      for (addr = 0x0; addr < core_var_ram_size; addr++) {
          ...
          ESTM_PRINTF(("%02x ", merlin16_shasta_rdbc_uc_var(sa__, __ERR, addr)));
      }
  }
  else {
      uint8_t index=0;
      ...
      for (addr = 0x0; addr < core_var_ram_size; addr++) {
          ...
          ESTM(rddata = merlin16_shasta_rdbc_uc_var(sa__, __ERR, addr));
...
err_code_t merlin16_shasta_uc_lane_var_dump(srds_access_t *sa__, uint8_t core, uint8_t lane, char ram_buffer[][250]) {
...

  {
      ...
      uc_stopped = merlin16_shasta_INTERNAL_stop_micro(sa__,rx_lock,&err_code);
  ...
  if (ram_buffer == NULL) {
      ...
      for (addr = 0x0; addr < lane_var_ram_size; addr++) {
         ...
         ESTM_PRINTF(("%02x ", merlin16_shasta_rdbl_uc_var(sa__, __ERR, addr)));
      }
  }
  else {
      ...
      for (addr = 0x0; addr < lane_var_ram_size; addr++) {
         ...
         ESTM(rddata = merlin16_shasta_rdbl_uc_var(sa__, __ERR, addr));
         USR_SPRINTF(tempBuf,"%02x ", rddata);
         USR_STRNCAT(ram_buffer[index], tempBuf, 6);
      }
      ram_buffer[index+1][0] = 0xA;
...
err_code_t merlin16_shasta_read_event_log(srds_access_t *sa__) {
    ...
    uint8_t micro_num = 0;
    {
        ...
        EFUN(merlin16_shasta_INTERNAL_read_event_log_with_callback(sa__, micro_num, 0, &state, merlin16_shasta_INTERNAL_event_log_dump_callback));
...
err_code_t merlin16_shasta_tx_pi_jitt_gen(srds_access_t *sa__, uint8_t enable, int16_t freq_override_val, enum srds_tx_pi_freq_jit_gen_enum jit_type, uint8_t tx_pi_jit_freq_idx, uint8_t tx_pi_jit_amp) {
    ...
    EFUN(merlin16_shasta_tx_pi_freq_override(sa__, enable, freq_override_val));
...
err_code_t merlin16_shasta_isolate_ctrl_pins(srds_access_t *sa__, uint8_t enable) {
    uint8_t lane, lane_orig, num_lanes;
        EFUN(merlin16_shasta_isolate_core_ctrl_pins(sa__, enable));

    ESTM(lane_orig = merlin16_shasta_get_lane(sa__));
    ...
    for(lane = 0; lane < num_lanes; lane++) {
        ...
        EFUN(merlin16_shasta_isolate_lane_ctrl_pins(sa__, enable));
    }
    EFUN(merlin16_shasta_set_lane(sa__, lane_orig));
...
/*  Note: No EFUNs or ESTMs should be used in this function as this print routine is called by _error() handler. */
    merlin16_shasta_triage_info info;
    ...
    memset(&info, 0, sizeof(merlin16_shasta_triage_info));
    info.error = err_code;
    info.line = line;
    ...
    if (merlin16_shasta_version(sa__, &info.api_ver)) {
    ...
    if(merlin16_shasta_INTERNAL_sigdet_status(sa__, &info.sig_det, &info.sig_det_chg)) {
        error_seen = 1;
    }
    if (merlin16_shasta_INTERNAL_pmd_lock_status(sa__, &info.pmd_lock, &info.pmd_lock_chg)) {
        error_seen = 1;
    }
    ...
    info.core = merlin16_shasta_get_core(sa__);
    info.lane = merlin16_shasta_get_lane(sa__);
    if (print_data) {
        if ((err_code == ERR_CODE_UC_CMD_POLLING_TIMEOUT) || (err_code == ERR_CODE_UC_NOT_STOPPED)) {
            USR_PRINTF(("%4d, %4d,  %X_%X,    %X_%X, %17d, %7d,%d       , %11d,   0x%04x, %8d, %6d,    0x%x,0x%x   , %s\n",
                        ...
                        info.dsc_one_hot[0],
                        info.dsc_one_hot[1],
                        merlin16_shasta_INTERNAL_e2s_err_code(info.error) ));
        } else {
