...
/**************************************************************************************
 **************************************************************************************
 *  File Name     :  merlin16_shasta_prbs.c                                                *
...
err_code_t merlin16_shasta_config_shared_tx_pattern(srds_access_t *sa__, uint8_t patt_length, const char pattern[]) {
  ...
  uint8_t    zero_pad_len    = 0; /* suppress warnings, changed by merlin16_shasta_INTERNAL_calc_patt_gen_mode_sel() */
  uint16_t   patt_gen_wr_val = 0;
  uint8_t    mode_sel        = 0; /* suppress warnings, changed by merlin16_shasta_INTERNAL_calc_patt_gen_mode_sel() */

  EFUN(merlin16_shasta_INTERNAL_calc_patt_gen_mode_sel(sa__, &mode_sel,&zero_pad_len,patt_length));
  ...
  if ((str_len > 2) && ((USR_STRNCMP(pattern, "0x", 2)) == 0)) {
    ...
    for (i=2; i < str_len; i++) {
      if (pattern[i] != '_') {
        EFUN(merlin16_shasta_INTERNAL_compute_bin(sa__, pattern[i],bin));
        ...
        if (actual_patt_len > 240) {
          ...
          return (merlin16_shasta_error(sa__, ERR_CODE_CFG_PATT_PATTERN_BIGGER_THAN_MAXLEN));
        }
      }
    }
    ...
    if ((offset_len > 3)  || (actual_patt_len < patt_length)) {
      ...
      return (merlin16_shasta_error(sa__, ERR_CODE_CFG_PATT_LEN_MISMATCH));
    }
    else if (offset_len) {
    ...
    for (i=0; i < str_len; i++) {
      if ((pattern[i] == '0') || (pattern[i] == '1')) {
        ...
        return (merlin16_shasta_error(sa__, ERR_CODE_CFG_PATT_INVALID_PATTERN));
      }
    }
  ...
  for (i=0; i < 15; i++) {
    ...
    for (j=0; j < 4; j++) {
      ...
      bin[4] = '\0';
      EFUN(merlin16_shasta_INTERNAL_compute_hex(sa__, bin, &hex));
    ...
    switch (i) {
      ...
      default: EFUN_PRINTF(("ERROR: Invalid write to patt_gen_seq register\n"));
               return (merlin16_shasta_error(sa__, ERR_CODE_CFG_PATT_INVALID_SEQ_WRITE));
    }
  }
...
err_code_t merlin16_shasta_tx_shared_patt_gen_en(srds_access_t *sa__, uint8_t enable, uint8_t patt_length) {
  ...
  if (enable) {
    if ((mode_sel < 1) || (mode_sel > 6)) {
      return (merlin16_shasta_error(sa__, ERR_CODE_PATT_GEN_INVALID_MODE_SEL));
    }
...
err_code_t merlin16_shasta_prbs_err_count_state(srds_access_t *sa__, uint32_t *prbs_err_cnt, uint8_t *lock_lost) {
    ...
    EFUN(merlin16_shasta_prbs_err_count_ll(sa__, prbs_err_cnt));
...
err_code_t merlin16_shasta_config_prbs_chk_hw_timer(srds_access_t *sa__, uint16_t time_ms, uint16_t *time_ms_adjusted) {
    ...
    struct merlin16_shasta_prbs_chk_hw_timer_ctrl_st prbs_chk_hw_timer_ctrl;
    USR_MEMSET(&prbs_chk_hw_timer_ctrl, 0, sizeof(struct merlin16_shasta_prbs_chk_hw_timer_ctrl_st));

    EFUN(wr_prbs_chk_burst_err_cnt_en(0));

    /* Figure out timer mode and timer count */
    EFUN(merlin16_shasta_INTERNAL_get_prbs_timeout_count_from_time(time_ms, time_ms_adjusted, &prbs_chk_hw_timer_ctrl));
...
/* Comment out display of PRBS burst error if BLACKHAWK */
  ...
  const char* merlin16_shasta_e2s_prbs_mode_enum[8] = {
    " PRBS_7",
    " PRBS_9",
    "PRBS_11",
    "PRBS_15",
...
err_code_t merlin16_shasta_display_detailed_prbs_state(srds_access_t *sa__) {
    ...
    ESTM_PRINTF(("  %d ",merlin16_shasta_get_lane(sa__)));
    ...
    if(enabled) {
        ...
        uint8_t prbs_inv = 0;
        char *prbs_string;
        EFUN(merlin16_shasta_get_tx_prbs_config(sa__, &prbs_poly_mode, &prbs_inv));
        prbs_string = (char *)merlin16_shasta_e2s_prbs_mode_enum[prbs_poly_mode];
    ...
    if(enabled) {
        ...
        uint8_t prbs_inv = 0;
        char *prbs_string;
        EFUN(merlin16_shasta_get_rx_prbs_config(sa__, &prbs_poly_mode, &prbs_checker_mode, &prbs_inv));
        prbs_string = (char *)merlin16_shasta_e2s_prbs_mode_enum[prbs_poly_mode];
        ESTM_PRINTF((" %s",prbs_string));
        ESTM_PRINTF(("      %1d     ",prbs_inv));
    } else {
    ...
    EFUN(merlin16_shasta_prbs_err_count_state(sa__,&err_cnt,&lock_lost));
