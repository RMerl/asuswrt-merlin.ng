...
struct merlin16_shortfin_ber_data_st {
#if defined(CONFIG_BCM96766) || defined(CONFIG_BCM96764)  /* 6766's 32 bit compiler missing 64 built-in devider now */
    uint32_t num_errs;
    uint32_t num_bits;
#else
    uint64_t num_errs;
...
struct merlin16_shortfin_check_platform_info_st {
    char c;
    uint32_t d;
};
