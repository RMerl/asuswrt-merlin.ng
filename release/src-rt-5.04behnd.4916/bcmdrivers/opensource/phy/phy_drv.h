...
#define SERDES_INTER_TYPE_STRING \
...
typedef struct phy_dev_s
{
    ...
    int an_enabled;     /* AN Status */
    int configured_an_enable;     /* AN Configuration Status; Auto; On; Off */
    void *descriptor; /* For some PHY familiy workaround */
#define PHY_NAME_LEN 32
    char name[PHY_NAME_LEN];
    phy_txfir_t txfir[4];
    int txfir_reg_speed_idx; /* The speed of TXFIR values currently set in register */
    int shared_ref_clk_mhz;
    ethcd_t ethcd;
    struct /* Link Flapping statistics */
    {
        uint32_t link_flaps;
        uint32_t link_flaps_per_min;
        uint64_t link_flaps_jiffies;
    };
} phy_dev_t;
...
#define INTER_PHY_TYPE_AUTO_M           (1<<INTER_PHY_TYPE_AUTO)
...
#define INTER_PHY_TYPE_AN_AND_FORCED_SPEED(inter_type) \
    (((1<<inter_type)&(INTER_PHY_TYPE_AUTO_M | INTER_PHY_TYPE_1000BASE_X_M | INTER_PHY_TYPE_1GBASE_R_M | INTER_PHY_TYPE_2P5GBASE_X_M | \
...
typedef struct phy_drv_s
{
    ...
    int (*wol_enable)(phy_dev_t *phy_dev, wol_params_t *wol_params);
    ...
    int (*cable_diag_run)(phy_dev_t *phy_dev);
    ...
    int (*prbs)(phy_dev_t *phy_dev, int param);
    ...
    int (*phy_temp_get)(phy_dev_t *phy_dev, int16_t *temp);
} phy_drv_t;
...
static inline int phy_is_pon_wan_ae_serdes(phy_dev_t *phy_dev)
{
    if (!phy_dev || !(phy_dev->phy_drv))
        return 0;

    if (phy_dev->phy_drv->phy_type == PHY_TYPE_WAN_AE)
        return 1;
...
static inline int phy_dev_wol_enable(phy_dev_t *phy_dev, wol_params_t *wol_params)
{
    if (!phy_dev->phy_drv->wol_enable)
        return 0;

    return phy_dev->phy_drv->wol_enable(phy_dev, wol_params);
}
...
{
    if (!phy_dev->phy_drv->silent_start_set)
        return -1;
...
static inline void phy_dev_cable_diag_init(phy_dev_t *phy_dev)
{
	ethcd_t *ethcd = &phy_dev->ethcd;

    ethcd->phy_addr = phy_dev->addr; 
    ethcd->cable_code = ETHCD_INVALID;
}

static inline int phy_dev_cable_diag_run(phy_dev_t *phy_dev)
{
	ethcd_t *ethcd = &phy_dev->ethcd;

    if (!phy_dev->phy_drv->cable_diag_run)
    {
        ethcd->return_value = ethcd->cable_code = ETHCD_NOT_SUPPORTED;
        return -1;
    }
    return phy_dev->phy_drv->cable_diag_run(phy_dev);
}
...
{
    return (phy_dev->ethcd.flag & ETHCD_FLAG_AUTORUN_ENABLED) > 0;
}
...
static inline int phy_dev_cable_diag_set(phy_dev_t *phy_dev, int enable)
{
    ...
    ethcd->flag &= ~ETHCD_FLAG_AUTORUN_ENABLED;
    ethcd->flag |= enable? ETHCD_FLAG_AUTORUN_ENABLED: 0;
    ethcd->return_value = enable? ETHCD_AUTORUN_ENABLED: ETHCD_AUTORUN_DISABLED;
...
static inline int phy_dev_cable_diag_get(phy_dev_t *phy_dev, int *enable)
{
    ...
    {
        ethcd->return_value = ethcd->cable_code = ETHCD_NOT_SUPPORTED;
        *enable = ETHCD_NOT_SUPPORTED;
        return -1;
    }

    ethcd->return_value = (ethcd->flag & ETHCD_FLAG_AUTORUN_ENABLED)? ETHCD_AUTORUN_ENABLED: ETHCD_AUTORUN_DISABLED;
    *enable = ethcd->return_value;
    return 0;
}

static inline int phy_dev_cable_diag_query(phy_dev_t *phy_dev)
{
    ethcd_t *ethcd = &phy_dev->ethcd;
    ...
    if (!(ethcd->flag & ETHCD_FLAG_DATA_VALID))
        ethcd->return_value = ETHCD_INVALID;
...
static inline int phy_dev_eee_support(phy_dev_t *phy_dev, int *eee_support)
{
    *eee_support = phy_dev->phy_drv->eee_set > 0;
    return 0;
}
...
static inline int phy_dev_prbs(phy_dev_t *phy_dev, int param)
{
    if (!phy_dev->phy_drv->prbs)
        return -1;

    return phy_dev->phy_drv->prbs(phy_dev, param);
}
...
static inline int phy_dev_inter_phy_types_get_with_caps_type(phy_dev_t *phy_dev, inter_phy_types_dir_t if_dir, int caps_type, uint32_t *types)
{
    int rc;
    *types = INTER_PHY_TYPE_UNKNOWN_M;
    ...
    if (!phy_dev)
        return 0;
    if (caps_type == CAPS_TYPE_ADVERTISE && phy_dev->configured_current_inter_phy_type != INTER_PHY_TYPE_AUTO)
    {
        *types = (1 << phy_dev->configured_current_inter_phy_type);
        return 0;
    }
...
{
    return phy_dev_inter_phy_types_get_with_caps_type(phy_dev, if_dir, CAPS_TYPE_ADVERTISE, types);
}
...
#define DOES_PHY_SUPPORT_XFI_CONFIG(phy_dev)    (phy_dev->phy_drv->configured_inter_phy_speed_type_set > 0)
...
static inline int phy_dev_is_broadcom_phy(phy_dev_t *phy_dev)
{
    ...
    return ((phy_id & 0xffff0000) == 0xae020000 || (phy_id & 0xffff0000) == 0x35900000 || (phy_id & 0xffff0000) == 0xf7a60000);
}
...
static inline int phy_dev_init(phy_dev_t *first_phy)
{
    ...
    for (phy_dev = cascade_phy_get_first(first_phy); phy_dev; phy_dev = phy_dev->cascade_next)
    {
        ...
        ret |= phy_drv_init(phy_dev->phy_drv);
        if (phy_dev->phy_drv->init)
            ret |= phy_dev->phy_drv->init(phy_dev);
        ...
        if (phy_dev->phy_drv->phy_type != PHY_TYPE_CROSSBAR)
        {
            ...
            if (phy_is_pon_wan_ae_serdes(phy_dev->cascade_prev))
#endif
            ret |= phy_dev_apd_set(phy_dev, 1);
            ret |= phy_dev_eee_set(phy_dev, 1);
        }
    }
        ...
        int type, int cfg_an)
{
    if (is_cascade_phy(phy_dev))
    {
        ...
        for (cascade = cascade_phy_get_last(phy_dev); cascade; cascade = cascade_phy_get_prev(cascade))
        {
            if ((rc = phy_dev_configured_inter_phy_speed_type_set(cascade, phy_adv_caps, duplex,
...
static inline int cascade_phy_dev_caps_get(phy_dev_t *phy_dev, int caps_type, uint32_t *caps)
{
    ...
    phy_dev_t *active_phy = cascade_phy_get_last_active(phy_dev);
#if defined(DSL_DEVICES)
    ...
    if (is_cascade_phy(phy_dev))
    {
        for (cascade = cascade_phy_get_first(phy_dev), *caps = 0; cascade; cascade = cascade_phy_get_next(cascade))
        {
            if (cascade->phy_drv->caps_get)
            {
                ...
                if (cascade_caps)
                {
                    *caps = (*caps) ? (*caps & cascade_caps) : cascade_caps;
                    
                    if (cascade == active_phy) /* Only the active PHY will determin non speed caps like PAUSE caps */
                        *caps |= cascade_caps & PHY_CAP_NON_SPEED_CAPS;

#if defined(DSL_DEVICES)
                    if (!inter_phy_types_last)
                    {
                        rc |= phy_dev_inter_phy_types_get_with_caps_type(cascade, INTER_PHY_TYPE_DOWN, caps_type, &inter_phy_types);
                        inter_phy_types_last = inter_phy_types;
                    }
                    else
                    {
                        rc |= phy_dev_inter_phy_types_get_with_caps_type(cascade, INTER_PHY_TYPE_UP, caps_type, &inter_phy_types);
                        inter_phy_types &= inter_phy_types_last;
                    }

                    rc |= get_inter_phy_supported_speed_caps(inter_phy_types, &inter_phy_supported_speed_caps);
                    *caps = (*caps & inter_phy_supported_speed_caps) | (*caps & PHY_CAP_NON_SPEED_CAPS);
#endif
                }
            }
        }
...
static inline phy_speed_t cascade_phy_max_adv_speed_get(phy_dev_t *phy_dev)
{
    uint32_t caps;
...
static inline time64_t get_epoch_time64(void)
{
