...
#define SWITCH_REG_SINGLE_SERDES_CNTRL     ((volatile uint32_t *)(serdes_base+4))
...
#define SWITCH_SINGLE_SERDES_STAT      ((volatile uint32_t *)(serdes_base+8))
...
#define SGMII_FIBER_DETECT             ((volatile uint32_t *)sgmii_fiber_detect)
    #define MISC_SGMII_FIBER_GPIO36     1

static void __iomem *sgmii_fiber_detect;
#endif
...
#if defined(CONFIG_BCM963138) || defined(CONFIG_BCM963148)
    sgmii_fiber_detect = dt_dev_remap(pdev, 1);
    if (IS_ERR(sgmii_fiber_detect))
    {
        ret = PTR_ERR(sgmii_fiber_detect);
        sgmii_fiber_detect = NULL;
        dev_err(&pdev->dev, "Missing SGMII fiber detect entry\n");
        goto Exit;
    }
#endif
...
int phy_drv_serdes_138class_init_lock(phy_dev_t *phy_dev)
{
    ...
    if (phy_serdes->signal_detect_gpio == 36)
        *SGMII_FIBER_DETECT = MISC_SGMII_FIBER_GPIO36;
    else
        *SGMII_FIBER_DETECT = 0;
#endif
...
static int phy_138_priv_fun_lock(phy_dev_t *phy_dev, int op_code, va_list ap)
{
    int ret = 0;
    ...
    switch (op_code)
    {
        ...
        case PHY_OP_WR_MDIO:
            if ((ret = dsl_serdes_check_power(phy_dev)))
                goto end;
            dsl_phy_exp_op(phy_dev, op_code, ap);
            break;
        default:
...
phy_drv_t phy_drv_serdes_138class =
{
    ...
    .priv_fun = phy_138_priv_fun_lock,
