...
/*
 * Merlin Shortfin SerDes PHY driver
 */
...
#define prog_seq_tbl prog_entry_ext_t

#include "common/srds_api_err_code.h"
#include "merlin16_shortfin_interface.h"
#include "public/merlin16_shortfin_fields_public.h"
#include "merlin_shortfin/merlin16_shortfin_ucode_image.h"
#include "merlin_shortfin/M1_merlin.h"
#include <linux/delay.h>

#define LANES_PER_CORE   1
#define PORTS_PER_CORE   4

static int core_enabled[SHORTFIN_CORES];
...
static inline int is_usxgmii_m(phy_dev_t *phy_dev)
{
    return is_inter_phy_type_supported(phy_dev, INTER_PHY_TYPE_USXGMII_MP);
}
...
static int _serdes_core_init(phy_dev_t *phy_dev, int enable)
{
    ...
    struct merlin16_shortfin_uc_core_config_st core_conf;
    ...
    if (is_usxgmii_m(phy_dev))
        ret |= phy_dev_prog_ext(phy_dev, usxgmii_switch_pcs_from_A_to_B_4port_10g);
    else
        ret |= phy_dev_prog_ext(phy_dev, usxgmii_switch_pcs_from_B_to_A);

    ret |= wrc_merlin16_shortfin_mdio_multi_prts_en(phy_dev, 1);
    ...
    ret |= phy_dev_prog_ext(phy_dev, PMD_setup_50_10p3125_VCO);

    ret |= merlin16_shortfin_get_uc_core_config(phy_dev, &core_conf);
    core_conf.field.vco_rate = (10.3125 * 4.0) - 22.0; //19.25
    ret |= merlin16_shortfin_INTERNAL_set_uc_core_config(phy_dev, core_conf);

    /* workaround for 10G-DXGMII 100Mbps, okay to apply to all speeds */
    if (is_usxgmii_m(phy_dev))
        ret |= phy_dev_prog_ext(phy_dev, usxgmii_mac_credit_enable);

    ret |= phy_dev_prog_ext(phy_dev, datapath_reset_core);
    ret |= phy_dev_c45_write_mask(phy_dev, 0x1, 0xd0f2, MASK_BIT(12), 12, 0x1); /* clock select */
...
{
    struct merlin16_shortfin_uc_lane_config_st lane_conf = {};
    ...
    switch (serdes_mode)
    {
    case SERDES_MODE_AN_USXGMII_MP_MASTER:
    case SERDES_MODE_AN_USXGMII_MP_SLAVE:
    case SERDES_MODE_AN_USXGMII_MASTER:
    case SERDES_MODE_AN_USXGMII_SLAVE:
        lane_conf.field.dfe_on = 1;
        break;
    case SERDES_MODE_FORCE_10G_USXGMII_MP:
    case SERDES_MODE_FORCE_5G_USXGMII_MP:
    ...
    merlin16_shortfin_set_uc_lane_cfg(phy_dev, lane_conf);
...
static int _serdes_lane_txfir_cfg(phy_dev_t *phy_dev, serdes_mode_t serdes_mode)
{
    ...
    case SERDES_MODE_FORCE_5G:
    case SERDES_MODE_FORCE_2P5G:
        ret |= merlin16_shortfin_apply_txfir_cfg (phy_dev, 0, 60, 0, 0);
        ret |= merlin16_shortfin_config_tx_hpf(phy_dev, 3);
        break;
    case SERDES_MODE_FORCE_1G:
    case SERDES_MODE_FORCE_100M:
        ret |= merlin16_shortfin_apply_txfir_cfg (phy_dev, 0, 38, 0, 0);
        ret |= merlin16_shortfin_config_tx_hpf(phy_dev, 0);
        break;
    default:
        return -1;
    }
...
static int _serdes_lane_speed_cfg(phy_dev_t *phy_dev, serdes_mode_t serdes_mode)
{
    ...
    {
    case SERDES_MODE_AN_USXGMII_MP_MASTER:
        ret |= phy_dev_prog_ext(phy_dev, usxgmii_mp_an_master);
        ret |= phy_dev_prog_ext(phy_dev, set_usxgmii_mp_an_enable);
        break;
    case SERDES_MODE_AN_USXGMII_MP_SLAVE:
        ret |= phy_dev_prog_ext(phy_dev, usxgmii_mp_an_slave);
        ret |= phy_dev_prog_ext(phy_dev, set_usxgmii_mp_an_enable);
        break;
    case SERDES_MODE_AN_USXGMII_MASTER:
        ret |= phy_dev_prog_ext(phy_dev, usxgmii_an_master);
        ret |= serdes_access_wait_link_status(phy_dev);
        ret |= phy_dev_prog_ext(phy_dev, set_usxgmii_an_enable);
        break;
    case SERDES_MODE_AN_USXGMII_SLAVE:
        ret |= phy_dev_prog_ext(phy_dev, usxgmii_an_slave);
        ret |= serdes_access_wait_link_status(phy_dev);
        ret |= phy_dev_prog_ext(phy_dev, set_usxgmii_an_enable);
        break;
    case SERDES_MODE_FORCE_10G_USXGMII_MP:
        ret |= phy_dev_prog_ext(phy_dev, usxgmii_force_speed_10g);
        break;
    case SERDES_MODE_FORCE_5G_USXGMII_MP:
        ret |= phy_dev_prog_ext(phy_dev, usxgmii_force_speed_5g);
        break;
    case SERDES_MODE_FORCE_2P5G_USXGMII_MP:
        ret |= phy_dev_prog_ext(phy_dev, usxgmii_force_speed_2p5g);
        break;
    case SERDES_MODE_FORCE_1G_USXGMII_MP:
        ret |= phy_dev_prog_ext(phy_dev, usxgmii_force_speed_1g);
        break; 
    case SERDES_MODE_FORCE_100M_USXGMII_MP:
    ...
    case SERDES_MODE_FORCE_10G_USXGMII:
        ...
        break;
    case SERDES_MODE_FORCE_5G_USXGMII:
        ret |= phy_dev_prog_ext(phy_dev, force_speed_5g_usxgmii);
        break;
    case SERDES_MODE_FORCE_2P5G_USXGMII:
        ret |= phy_dev_prog_ext(phy_dev, force_speed_2p5g_usxgmii);
        break;
    case SERDES_MODE_FORCE_1G_USXGMII:
        ret |= phy_dev_prog_ext(phy_dev, force_speed_1g_usxgmii);
        break; 
    case SERDES_MODE_FORCE_100M_USXGMII:
    ...
    case SERDES_MODE_FORCE_10G_R:
        ...
        break;
    case SERDES_MODE_FORCE_5G_R:
        ret |= phy_dev_prog_ext(phy_dev, force_speed_5g_R);
        break;
    case SERDES_MODE_FORCE_2P5G_R:
        ret |= phy_dev_prog_ext(phy_dev, force_speed_2p5g_R);
        break;
    case SERDES_MODE_FORCE_1G_R:
        ret |= phy_dev_prog_ext(phy_dev, force_speed_1g_R);
        break;
    case SERDES_MODE_FORCE_5G:
        ret |= phy_dev_prog_ext(phy_dev, force_speed_5g);
        break;
    case SERDES_MODE_FORCE_2P5G:
        ...
        ret |= phy_dev_prog_ext(phy_dev, force_speed_2p5g_x);
        break;
    case SERDES_MODE_FORCE_1G:
        ret |= phy_dev_prog_ext(phy_dev, force_speed_1g);
        break;
    case SERDES_MODE_FORCE_100M:
        ret |= phy_dev_prog_ext(phy_dev, force_speed_100m);
        break;
    default:
        return -1;
    }
...
static int _serdes_lane_init(phy_dev_t *phy_dev, int core_enabled, serdes_mode_t serdes_mode)
{
    ...
    if (lane_mode[core_id] == serdes_mode)
        return 0;
    ...
    ret |= phy_dev_prog_ext(phy_dev, en_datapath_reset_lane);
    ret |= _serdes_lane_uc_cfg(phy_dev, serdes_mode);
    ret |= phy_dev_prog_ext(phy_dev, datapath_reset_lane);

    if (is_usxgmii_m(phy_dev))
        ret |= phy_dev_prog_ext(phy_dev, usxgmii_pcs_reset_and_enable);

    ret |= serdes_access_wait_pll_lock(phy_dev);

Exit:
    lane_mode[core_id] = serdes_mode;

    return ret;
}

static int _serdes_port_init(phy_dev_t *phy_dev, int core_enabled, serdes_mode_t serdes_mode)
{
    int ret = 0;
    uint8_t core_id = phy_dev->core_index - SHORTFIN_BASE_CORE;
    uint8_t port_id = phy_dev->lane_index;

    if (port_mode[core_id][port_id] == serdes_mode)
        return 0;
    ...
    ret |= phy_dev_prog_ext(phy_dev, change_speed);

#if defined(SERDES_LINK_POWER_DOWN)
    ret |= phy_dev_prog_ext(phy_dev, powerdn_lane);
#else
    ret |= phy_dev_prog_ext(phy_dev, en_datapath_reset_lane);
#endif
...
#if defined(SERDES_LINK_POWER_DOWN)
    ret |= phy_dev_prog_ext(phy_dev, powerup_lane);
#endif

    ret |= phy_dev_prog_ext(phy_dev, en_datapath_reset_lane);
    ret |= _serdes_lane_txfir_cfg(phy_dev, serdes_mode);
    ret |= _serdes_lane_speed_cfg(phy_dev, serdes_mode);
    ret |= phy_dev_prog_ext(phy_dev, lpi_enable);
    if (is_usxgmii_m(phy_dev))
        ret |= phy_dev_prog_ext(phy_dev, usxgmii_port_reset);
    ret |= phy_dev_prog_ext(phy_dev, datapath_reset_lane);

Exit:
    port_mode[core_id][port_id] = serdes_mode;

    return ret;
}

#if defined(SERDES_LINK_POWER_DOWN)
static int _is_other_port_active(phy_dev_t *phy_dev)
{
    int i;
    uint8_t core_id = phy_dev->core_index - SHORTFIN_BASE_CORE;
    uint8_t port_id = phy_dev->lane_index;
    ...
    for (i = 0; i < PORTS_PER_CORE; i++)
    {
        if (i == port_id)
            continue;

        if (port_mode[core_id][i] != SERDES_MODE_UNKNOWN)
            enabled_ports++;
    }

    return enabled_ports ? 1 : 0;
}
#endif
...
#if defined(SERDES_LINK_POWER_DOWN)
    enable_core = _is_other_port_active(phy_dev) || serdes_mode != SERDES_MODE_UNKNOWN;
#endif

    if ((ret = _serdes_core_init(&base_phy, enable_core)))
        return ret;

    if ((ret = _serdes_lane_init(&base_phy, enable_core, serdes_mode)))
        return ret;

    if ((ret = _serdes_port_init(phy_dev, enable_core, serdes_mode)))
        return ret;
...
static int _phy_speed_set(phy_dev_t *phy_dev, phy_speed_t speed, phy_duplex_t duplex)
{
    ...
    else if (is_inter_phy_type_supported(phy_dev, INTER_PHY_TYPE_USXGMII_MP))
    {
        if (speed == PHY_SPEED_10000)
            serdes_mode = SERDES_MODE_FORCE_10G_USXGMII_MP;
        else if (speed == PHY_SPEED_5000)
            serdes_mode = SERDES_MODE_FORCE_5G_USXGMII_MP;
        else if (speed == PHY_SPEED_2500)
            serdes_mode = SERDES_MODE_FORCE_2P5G_USXGMII_MP;
    ...
    configured_mode[core_id][port_id] = serdes_mode;
...
static int _serdes_enable(phy_dev_t *phy_dev, int8_t module_detect)
{
    uint8_t core_id = phy_dev->core_index - SHORTFIN_BASE_CORE;
    uint8_t port_id = phy_dev->lane_index;
    serdes_mode_t serdes_mode = configured_mode[core_id][port_id];
...
static int _phy_prbs(phy_dev_t *phy_dev, int param)
{
    ...
    {
        printk("PRBS disable\n");
        ret |= merlin16_shortfin_rx_prbs_en(phy_dev, 0);
        ret |= merlin16_shortfin_tx_prbs_en(phy_dev, 0);
        return ret;
    }
    ...
    wr_merlin16_shortfin_osr_mode_frc(phy_dev, 1);
    wr_merlin16_shortfin_osr_mode_frc_val(phy_dev, val);

    printk("PRBS mode %d enable, speed %d\n", param, speed);
    ret |= merlin16_shortfin_config_tx_prbs(phy_dev, prbs_mode, 0);
    ret |= merlin16_shortfin_config_rx_prbs(phy_dev, prbs_mode, PRBS_SELF_SYNC_HYSTERESIS, 0);

    ret |= merlin16_shortfin_tx_prbs_en(phy_dev, 1);
    ret |= merlin16_shortfin_rx_prbs_en(phy_dev, 1);
    msleep(10);

Exit:
    ret |= merlin16_shortfin_prbs_chk_lock_state(phy_dev, &lock);
    ret |= merlin16_shortfin_prbs_err_count_state(phy_dev, &errors, &lock_lost);
...
{
    return merlin16_shortfin_read_tx_afe(phy_dev, TX_AFE_PRE, pre) || 
        ...
        merlin16_shortfin_rd_tx_hpf_config(phy_dev, (uint8_t *)hpf);
}

static int _phy_tx_cfg_set(phy_dev_t *phy_dev, int8_t pre, int8_t main, int8_t post1, int8_t post2, int8_t hpf)
{
    return merlin16_shortfin_apply_txfir_cfg(phy_dev, pre, main, post1, post2) ||
        merlin16_shortfin_config_tx_hpf(phy_dev, hpf);
}

phy_drv_t phy_drv_shortfin =
{
    .phy_type = PHY_TYPE_SHORTFIN,
