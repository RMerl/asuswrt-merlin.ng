...
struct bcm_macsec_ops longfin_macsec_ops;
...
int phy_macsec_pu_init(phy_dev_t *phy_dev)
{
    ...
    else
    {
        ...
        macsec_dev->tx_active_sa_index = SA_NUM_MAX;
        memset(&macsec_dev->secy_devices[0], 0, sizeof(secy_device_t) * 2);
        phy_dev->macsec_ops = &longfin_macsec_ops;
        phy_dev->macsec_dev = macsec_dev;
        phy_macsec_log(LOG_INFO, "macsec device allocated\n");
    }
    ...
    /* should read scratchpad for value */
...
int phy_macsec_oper(phy_dev_t *phy_dev, void *data)
{
    ...
    switch (macsec_data->op)
    {
        ...
        {
            macsec_log_level = macsec_data->data1;
            break;
        }
        default:
        {
...
static int macsec_xlmac_restart(phy_dev_t *phy_dev, macsec_port_loc_t port_loc)
{
    ...
    macsec_xlmac_reg64_read(phy_dev, port_loc, XLMAC_TX_CTRLr, &(mac_tx_ctrl._xlmac_tx_ctrl));
    XLMAC_TX_CTRLr_PAD_ENf_SET(mac_tx_ctrl, 1);
    macsec_xlmac_reg64_write(phy_dev, port_loc, XLMAC_TX_CTRLr, mac_tx_ctrl._xlmac_tx_ctrl);
...
static int macsec_device_init(secy_device_access_t *secy_device_access, const macsec_settings_t * const Settings_p, macsec_Capabilities_t *Capabilities)
{
    ...
    temp_log = macsec_log_level;
    macsec_log_level = LOG_INFO;
...
static uint32_t macsec_SC_SA_Count_Get(secy_device_t* secy_device, uint32_t sc_index)
{
    secy_vport_descriptor_t *vport_desc;
    ...
    return vport_desc->InUse.SACount;
...
uint32_t macsec_get_hkey(uint8_t *key, uint8_t *hkey, uint32_t keySize)
{
    uint8_t src_buf[16]={};
    int ret = 0;
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 10, 0)
    ...
    if (aes_expandkey(&ctx, key, keySize) != 0)
    {
        phy_macsec_log(LOG_ERROR, "aes_expandkey failed\n");
        return -1;
    }

    aes_encrypt(&ctx, hkey, src_buf);
#else
    ...
    if (!tfm)
    {
        phy_macsec_log(LOG_ERROR, "crypto_alloc_cipher failed\n");
        return -1;
    }

    if (crypto_cipher_setkey(tfm, key, keySize) != 0)
    {
        phy_macsec_log(LOG_ERROR, "crypto_cipher_setkey failed\n");
        return -1;
    }

    crypto_cipher_encrypt_one(tfm, hkey, src_buf);
...
int phy_macsec_mdo_dev_open(struct macsec_context *ctx, void *private)
{
    int ret = 0;

    phy_macsec_log(LOG_DEBUG, "phy_macsec_mdo_dev_open\n");
...
int phy_macsec_mdo_dev_stop(struct macsec_context *ctx, void *private)
{
    int ret = 0;

    phy_macsec_log(LOG_DEBUG, "phy_macsec_mdo_dev_stop\n");
...
int phy_macsec_mdo_add_secy(struct macsec_context *ctx, void *private)
{
    int ret = 0;
    phy_dev_t *phy_dev = (phy_dev_t*)private;
    phy_macsec_dev *macsec_dev = (phy_macsec_dev*)phy_dev->macsec_dev;
    macsec_rule_t ruleparams = {};
    macsec_settings_t Settings;

    phy_macsec_log(LOG_DEBUG, "phy_macsec_mdo_add_secy\n");
    ...
    Settings.fAutoStatCntrsReset = 1;

    Settings.ControlDropBypass.Untagged.fBypass = 1;

    Settings.NonControlDropBypass.Untagged.DropType = macsec_SECY_SA_DROP_PKT_ERROR;
    Settings.NonControlDropBypass.Tagged.DropType = macsec_SECY_SA_DROP_PKT_ERROR;
    Settings.NonControlDropBypass.BadTag.DropType = macsec_SECY_SA_DROP_PKT_ERROR;
    Settings.NonControlDropBypass.KaY.DropType = macsec_SECY_SA_DROP_PKT_ERROR;
    Settings.ControlDropBypass.Untagged.DropType = macsec_SECY_SA_DROP_PKT_ERROR;
    Settings.ControlDropBypass.Tagged.DropType = macsec_SECY_SA_DROP_PKT_ERROR;
    Settings.ControlDropBypass.BadTag.DropType = macsec_SECY_SA_DROP_PKT_ERROR;
    Settings.ControlDropBypass.KaY.DropType = macsec_SECY_SA_DROP_PKT_ERROR;

    Settings.SAFControl.fEnable         = 1;
    Settings.SAFControl.fDropClass      = 1;
    Settings.SAFControl.fDropPP         = 1;
    Settings.SAFControl.fDropSecFail    = 1;
    Settings.SAFControl.fDropMTU        = 1;
    Settings.SAFControl.fDropMACCRC     = 1;
    Settings.SAFControl.fDropMACErr     = 1;
    Settings.SAFControl.LowWatermark    = 0x400;
    Settings.SAFControl.HighWatermark   = 0x600;

    Settings.CPMatchEnableMask = 0x80000;

    ret = phy_macsec_init(phy_dev, &Settings);

    ret = phy_macsec_vPort_add(&(macsec_dev->secy_devices[0]), 0);

    ret = phy_macsec_rule_add(&(macsec_dev->secy_devices[0]), 0, 0, &ruleparams);
    ret = phy_macsec_rule_enable(&(macsec_dev->secy_devices[0]), 0, 1, 1);

    return ret;
}

int phy_macsec_mdo_upd_secy(struct macsec_context *ctx, void *private)
{
    int ret = 0;
    phy_dev_t *phy_dev = (phy_dev_t*)private;
    phy_macsec_dev *macsec_dev = (phy_macsec_dev*)phy_dev->macsec_dev;

    phy_macsec_log(LOG_DEBUG, "phy_macsec_mdo_upd_secy\n");
    
    if (!ctx->prepare)
        return 0;

    if (macsec_dev->tx_active_sa_index == SA_NUM_MAX)
    {
        phy_macsec_log(LOG_DEBUG, "txsa not configured\n");
        return 0;
    }

    if (ctx->secy->tx_sc.encoding_sa != macsec_dev->tx_active_sa_index)
    {
        ret = phy_macsec_sa_switch(&(macsec_dev->secy_devices[0]), macsec_dev->tx_active_sa_index, ctx->secy->tx_sc.encoding_sa);
        if (!ret)
        {
            phy_macsec_log(LOG_DEBUG, "active sa changed from: %u to: %u\n", macsec_dev->tx_active_sa_index, ctx->secy->tx_sc.encoding_sa);
            macsec_dev->tx_active_sa_index = ctx->secy->tx_sc.encoding_sa;
        }
    }
...
int phy_macsec_mdo_del_secy(struct macsec_context *ctx, void *private)
{
    int ret = 0;
    phy_dev_t *phy_dev = (phy_dev_t*)private;
    phy_macsec_dev *macsec_dev = (phy_macsec_dev*)phy_dev->macsec_dev;

    phy_macsec_log(LOG_DEBUG, "phy_macsec_mdo_del_secy\n");
    
    if (!ctx->prepare)
        return 0;

    ret = phy_macsec_rule_enable(&(macsec_dev->secy_devices[0]), 0, 0, 1);
    ret = phy_macsec_rule_remove(&(macsec_dev->secy_devices[0]), 0, 0);
    ret = phy_macsec_vPort_remove(&(macsec_dev->secy_devices[0]), 0);

    return ret;
}

int phy_macsec_mdo_add_rxsc(struct macsec_context *ctx, void *private)
{
    int ret = 0;
    phy_dev_t *phy_dev = (phy_dev_t*)private;
    phy_macsec_dev *macsec_dev = (phy_macsec_dev*)phy_dev->macsec_dev;
    ...
    phy_macsec_log(LOG_DEBUG, "phy_macsec_mdo_add_rxsc\n");
    
    if (!ctx->prepare)
        return 0;
    
    ret = phy_macsec_vPort_add(&(macsec_dev->secy_devices[1]), 0);

    ruleparams.key.macsec_tagged = 1;
    ruleparams.mask.macsec_tagged = 1;
    ret = phy_macsec_rule_add(&(macsec_dev->secy_devices[1]), 0, 0, &ruleparams);
    ret = phy_macsec_rule_enable(&(macsec_dev->secy_devices[1]), 0, 1, 1);

    return ret;
}

int phy_macsec_mdo_upd_rxsc(struct macsec_context *ctx, void *private)
{
    int ret = 0;

    phy_macsec_log(LOG_DEBUG, "phy_macsec_mdo_upd_rxsc\n");
...
int phy_macsec_mdo_del_rxsc(struct macsec_context *ctx, void *private)
{
    int ret = 0;
    phy_dev_t *phy_dev = (phy_dev_t*)private;
    phy_macsec_dev *macsec_dev = (phy_macsec_dev*)phy_dev->macsec_dev;

    phy_macsec_log(LOG_DEBUG, "phy_macsec_mdo_del_rxsc\n");
    
    if (!ctx->prepare)
        return 0;

    ret = phy_macsec_rule_enable(&(macsec_dev->secy_devices[1]), 0, 0, 1);
    ret = phy_macsec_rule_remove(&(macsec_dev->secy_devices[1]), 0, 0);

    ret = phy_macsec_vPort_remove(&(macsec_dev->secy_devices[1]), 0);

    return ret;
}

int phy_macsec_mdo_add_rxsa(struct macsec_context *ctx, void *private)
{
    int ret = 0;
    phy_dev_t *phy_dev = (phy_dev_t*)private;
    phy_macsec_dev *macsec_dev = (phy_macsec_dev*)phy_dev->macsec_dev;
    ...
    phy_macsec_log(LOG_DEBUG, "phy_macsec_mdo_add_rxsa\n");
    
    if (!ctx->prepare)
        return 0;

    phy_macsec_log(LOG_DEBUG, "sci=%llx, validate_frames=%d, replay_protect=%d, an=%d, replay_window=%d, ctx->secy->xpn=%d\n", 
        ctx->secy->sci, ctx->secy->validate_frames, ctx->secy->replay_protect, ctx->sa.assoc_num, ctx->secy->replay_window, ctx->secy->xpn);


    sa_params.action_type = MACSEC_SA_ACTION_INGRESS; 
    sa_params.dest_port = MACSEC_PORT_TXMAC;
    sa_params.drop_type = MACSEC_SA_DROP_INTERNAL;

    sa_params.params.ingress.fsa_inuse = 0;
    sa_params.params.ingress.validate_frames_tagged = ctx->secy->validate_frames;
    sa_params.params.ingress.freplay_protect = ctx->secy->replay_protect;
    sa_params.params.ingress.sci_p = (uint8_t *)&(ctx->sa.rx_sa->sc->sci);
    ...
    transform_record_basic.operation = MACSEC_SAB_OP_MACSEC;
    transform_record_basic.direction = MACSEC_SAB_DIRECTION_INGRESS;
    transform_record_basic.seq_num_lo = 1;
    transform_record_basic.an = ctx->sa.assoc_num;
    transform_record_basic.flags = 0; //(SAB_MACSEC_FLAG_ROLLOVER | SAB_MACSEC_FLAG_UPDATE_ENABLE | SAB_MACSEC_FLAG_SA_EXPIRED_IRQ);
    transform_record_basic.key_p = ctx->sa.key;
    transform_record_basic.key_byte_count = ctx->secy->key_len;
    ret = macsec_get_hkey(ctx->sa.key, hkey, ctx->secy->key_len);
    transform_record_basic.h_key_p = hkey;
    if (ctx->secy->xpn)
    {
        transform_record_basic.salt_p = ctx->sa.rx_sa->key.salt.bytes;
        transform_record_basic.ssci_p = ctx->sa.rx_sa->key.salt.bytes; //&(ctx->sa.rx_sa->ssci);
    }
    else
    {
        transform_record_basic.salt_p = NULL; //api_sa->salt;
    ...
    transform_record_basic.sci_p = (uint8_t *)&(ctx->sa.rx_sa->sc->sci);
    transform_record_basic.seq_num_hi = 0x00000000;
    transform_record_basic.window_size = ctx->secy->replay_window == 0 ? 0x00000080 : ctx->secy->replay_window;
    transform_record_basic.icv_byte_count = 0x0;

    ret = macsec_build_transform_record(&transform_record_basic, transform_record);
    if (ret) 
    {
        phy_macsec_log(LOG_ERROR, "FAILED: MACSec build_transform_record failed for macsec-direction ingress\n");
        return ret;
    } 
    else 
    {
        phy_macsec_log(LOG_DEBUG, "PASSED: MACSec build_transform_record passed for macsec-direction ingress\n");
    }  
                 
    memcpy(sa_params.transform_record, transform_record, sizeof(transform_record));

    ret = phy_macsec_sa_add(&(macsec_dev->secy_devices[1]), 0, ctx->sa.assoc_num, &sa_params);

    return ret;     
}

int phy_macsec_mdo_upd_rxsa(struct macsec_context *ctx, void *private)
{
    int ret = 0;

    phy_macsec_log(LOG_DEBUG, "phy_macsec_mdo_upd_rxsa\n");
...
int phy_macsec_mdo_del_rxsa(struct macsec_context *ctx, void *private)
{
    int ret = 0;
    phy_dev_t *phy_dev = (phy_dev_t*)private;
    phy_macsec_dev *macsec_dev = (phy_macsec_dev*)phy_dev->macsec_dev;

    phy_macsec_log(LOG_DEBUG, "phy_macsec_mdo_del_rxsa\n");
    
    if (!ctx->prepare)
        return 0;

    ret = phy_macsec_sa_remove(&(macsec_dev->secy_devices[1]), 0, ctx->sa.assoc_num);

    return ret;
}

int phy_macsec_mdo_add_txsa(struct macsec_context *ctx, void *private)
{
    int ret = 0;
    phy_dev_t *phy_dev = (phy_dev_t*)private;
    phy_macsec_dev *macsec_dev = (phy_macsec_dev*)phy_dev->macsec_dev;
    ...
    phy_macsec_log(LOG_DEBUG, "phy_macsec_mdo_add_txsa\n");
    
    if (!ctx->prepare)
        return 0;

    sa_params.action_type = MACSEC_SA_ACTION_EGRESS; 
    sa_params.dest_port = MACSEC_PORT_TXMAC;
    sa_params.drop_type = MACSEC_SA_DROP_INTERNAL;

    phy_macsec_log(LOG_DEBUG, "sci=%llx, protect_frames=%d, send_sci=%d, encrypt=%d, es=%d, scb=%d, an=%d, replay_window=%d, ctx->secy->xpn=%d\n", 
        ctx->secy->sci, ctx->secy->protect_frames, ctx->secy->tx_sc.send_sci, ctx->secy->tx_sc.encrypt, ctx->secy->tx_sc.end_station, ctx->secy->tx_sc.scb, ctx->sa.assoc_num, ctx->secy->replay_window, ctx->secy->xpn);

    sa_params.params.egress.fsa_inuse = 0;
    sa_params.params.egress.fprotect_frames = ctx->secy->protect_frames;
    sa_params.params.egress.finclude_sci = ctx->secy->tx_sc.send_sci;
    sa_params.params.egress.fconf_protect = ctx->secy->tx_sc.encrypt;
    sa_params.params.egress.fallow_data_pkts = 1;
    sa_params.params.egress.confidentiality_offset = 0;
    sa_params.params.egress.fuse_es = ctx->secy->tx_sc.end_station;
    sa_params.params.egress.fuse_scb = ctx->secy->tx_sc.scb;
    sa_params.params.egress.fsl_pad_strip_enb = 0;
    sa_params.params.egress.pre_sectag_auth_start = 0;
    sa_params.params.egress.pre_sectag_auth_length = 12;
    sa_params.params.egress.sectag_offset = 12;
    ...
    phy_macsec_log(LOG_DEBUG, "keylen=%d key=%llx%llx", ctx->secy->key_len, (uint64_t)(ctx->sa.key[0]), (uint64_t)(ctx->sa.key[8]));

    transform_record_basic.operation = MACSEC_SAB_OP_MACSEC;
    transform_record_basic.direction = MACSEC_SAB_DIRECTION_EGRESS;
    transform_record_basic.seq_num_lo = 0;
    transform_record_basic.an = ctx->sa.assoc_num;   
    transform_record_basic.flags = 0; //(SAB_MACSEC_FLAG_ROLLOVER | SAB_MACSEC_FLAG_UPDATE_ENABLE | SAB_MACSEC_FLAG_SA_EXPIRED_IRQ);
    transform_record_basic.key_p = ctx->sa.key; 
    transform_record_basic.key_byte_count = ctx->secy->key_len;
    ret = macsec_get_hkey(ctx->sa.key, hkey, ctx->secy->key_len);
    phy_macsec_log(LOG_DEBUG, "hkey=%llx%llx", (uint64_t)hkey[0], (uint64_t)hkey[8]);
    transform_record_basic.h_key_p = hkey;
    if (ctx->secy->xpn)
    {
        transform_record_basic.salt_p = ctx->sa.rx_sa->key.salt.bytes;
    ...
    transform_record_basic.sci_p = (uint8_t *)&(ctx->secy->sci);
    transform_record_basic.seq_num_hi = 0x00000000;
    transform_record_basic.window_size = ctx->secy->replay_window == 0 ? 0x00000080 : ctx->secy->replay_window;
    transform_record_basic.icv_byte_count = 0x0;
    ...
    if (ret) 
    {
        phy_macsec_log(LOG_DEBUG, "FAILED: MACSec build_transform_record failed for macsec-direction egress\n");
        return ret;
    } 
    else 
    {
        phy_macsec_log(LOG_DEBUG, "PASSED: MACSec build_transform_record passed for macsec-direction egress\n");
    }  
                 
    memcpy(sa_params.transform_record, transform_record, sizeof(transform_record));

    if (macsec_SC_SA_Count_Get(&(macsec_dev->secy_devices[0]), 0) == 0)
    {
        ret = phy_macsec_sa_add(&(macsec_dev->secy_devices[0]), 0, ctx->sa.assoc_num, &sa_params);
        if (!ret)
        {
            macsec_dev->tx_active_sa_index = ctx->sa.assoc_num;
            phy_macsec_log(LOG_DEBUG, "tx_active_sa = %u\n", macsec_dev->tx_active_sa_index);
        }
    }
    else
    {
        phy_macsec_log(LOG_DEBUG, "sa chaining %u to %u\n", ctx->sa.assoc_num, macsec_dev->tx_active_sa_index);
        ret = phy_macsec_sa_chain(&(macsec_dev->secy_devices[0]), macsec_dev->tx_active_sa_index, ctx->sa.assoc_num, &sa_params);
    }
...
int phy_macsec_mdo_upd_txsa(struct macsec_context *ctx, void *private)
{
    int ret = 0;
    
    phy_macsec_log(LOG_DEBUG, "phy_macsec_mdo_upd_txsa\n");
...
int phy_macsec_mdo_del_txsa(struct macsec_context *ctx, void *private)
{
    int ret = 0;
    phy_dev_t *phy_dev = (phy_dev_t*)private;
    phy_macsec_dev *macsec_dev = (phy_macsec_dev*)phy_dev->macsec_dev;
    
    phy_macsec_log(LOG_DEBUG, "phy_macsec_mdo_del_txsa\n");
    
    if (!ctx->prepare)
        return 0;

    ret = phy_macsec_sa_remove(&(macsec_dev->secy_devices[0]), 0, ctx->sa.assoc_num);
...
int phy_macsec_mdo_get_dev_stats(struct macsec_context *ctx, void *private)
{
    int ret = 0;
    phy_dev_t *phy_dev = (phy_dev_t*)private;
    phy_macsec_dev *macsec_dev = (phy_macsec_dev*)phy_dev->macsec_dev;
    macsec_api_secy_e_stats sc_e_stats;
    macsec_api_secy_i_stats sc_i_stats;
    macsec_api_secy_sa_e_stats sa_e_stats;

    phy_macsec_log(LOG_REG_DEBUG, "phy_macsec_mdo_get_dev_stats\n");

    if (!ctx->prepare)
        return 0;

    phy_macsec_vport_e_statistics_get(&(macsec_dev->secy_devices[0]), 0, &sc_e_stats, 1);
    phy_macsec_vport_i_statistics_get(&(macsec_dev->secy_devices[0]), 0, &sc_i_stats, 1);
    phy_macsec_sa_e_statistics_get(&(macsec_dev->secy_devices[0]), 0, &sa_e_stats, 1);

    ctx->stats.dev_stats->OutPktsUntagged = *((uint64_t *)&(sc_e_stats.OutPktsUntagged));
    ctx->stats.dev_stats->InPktsUntagged = *((uint64_t *)&(sc_i_stats.InPktsUntagged));
    ctx->stats.dev_stats->OutPktsTooLong = *((uint64_t *)&(sa_e_stats.OutPktsTooLong));
    ctx->stats.dev_stats->InPktsNoTag = *((uint64_t *)&(sc_i_stats.InPktsNoTag));
    ctx->stats.dev_stats->InPktsBadTag = *((uint64_t *)&(sc_i_stats.InPktsBadTag));
    ctx->stats.dev_stats->InPktsUnknownSCI = *((uint64_t *)&(sc_i_stats.InPktsUnknownSCI));
    ctx->stats.dev_stats->InPktsNoSCI = *((uint64_t *)&(sc_i_stats.InPktsNoSCI));
    ctx->stats.dev_stats->InPktsOverrun = 0;

    return ret;
}

int phy_macsec_mdo_get_tx_sc_stats(struct macsec_context *ctx, void *private)
{
    int ret = 0;
    phy_dev_t *phy_dev = (phy_dev_t*)private;
    phy_macsec_dev *macsec_dev = (phy_macsec_dev*)phy_dev->macsec_dev;
    ...
    phy_macsec_log(LOG_REG_DEBUG, "phy_macsec_mdo_get_tx_sc_stats\n");

    if (!ctx->prepare)
        return 0;

    phy_macsec_sa_e_statistics_get(&(macsec_dev->secy_devices[0]), 0, &stats, 1);

    ctx->stats.tx_sc_stats->OutPktsProtected = *((uint64_t *)&(stats.OutPktsEncryptedProtected));
    ctx->stats.tx_sc_stats->OutPktsEncrypted = *((uint64_t *)&(stats.OutPktsEncryptedProtected));
    ctx->stats.tx_sc_stats->OutOctetsProtected = *((uint64_t *)&(stats.OutOctetsEncryptedProtected));
    ctx->stats.tx_sc_stats->OutOctetsEncrypted = *((uint64_t *)&(stats.OutOctetsEncryptedProtected));

    return ret;
}

int phy_macsec_mdo_get_tx_sa_stats(struct macsec_context *ctx, void *private)
{
    int ret = 0;
    phy_dev_t *phy_dev = (phy_dev_t*)private;
    phy_macsec_dev *macsec_dev = (phy_macsec_dev*)phy_dev->macsec_dev;
    ...
    phy_macsec_log(LOG_REG_DEBUG, "phy_macsec_mdo_get_tx_sa_stats\n");
    ...
    ctx->stats.tx_sa_stats->OutPktsProtected = *((uint64_t *)&(stats.OutPktsEncryptedProtected));
    ctx->stats.tx_sa_stats->OutPktsEncrypted = *((uint64_t *)&(stats.OutPktsEncryptedProtected));

    return ret;
}

int phy_macsec_mdo_get_rx_sc_stats(struct macsec_context *ctx, void *private)
{
    int ret = 0;
    phy_dev_t *phy_dev = (phy_dev_t*)private;
    phy_macsec_dev *macsec_dev = (phy_macsec_dev*)phy_dev->macsec_dev;
    ...
    phy_macsec_log(LOG_REG_DEBUG, "phy_macsec_mdo_get_rx_sc_stats\n");

    if (!ctx->prepare)
        return 0;

    phy_macsec_sa_i_statistics_get(&(macsec_dev->secy_devices[1]), 0, &stats, 1);

    ctx->stats.rx_sc_stats->InOctetsValidated = *((uint64_t *)&(stats.InOctetsValidated));
    ctx->stats.rx_sc_stats->InOctetsDecrypted = *((uint64_t *)&(stats.InOctetsDecrypted));
    ctx->stats.rx_sc_stats->InPktsUnchecked = *((uint64_t *)&(stats.InPktsUnchecked));
    ctx->stats.rx_sc_stats->InPktsDelayed = *((uint64_t *)&(stats.InPktsDelayed));
    ctx->stats.rx_sc_stats->InPktsOK = *((uint64_t *)&(stats.InPktsOK));
    ctx->stats.rx_sc_stats->InPktsInvalid = *((uint64_t *)&(stats.InPktsInvalid));
    ctx->stats.rx_sc_stats->InPktsLate = *((uint64_t *)&(stats.InPktsLate));
    ctx->stats.rx_sc_stats->InPktsNotValid = *((uint64_t *)&(stats.InPktsNotValid));
    ctx->stats.rx_sc_stats->InPktsNotUsingSA = *((uint64_t *)&(stats.InPktsNotUsingSA));
    ctx->stats.rx_sc_stats->InPktsUnusedSA = *((uint64_t *)&(stats.InPktsUnusedSA));

    return ret;
}

int phy_macsec_mdo_get_rx_sa_stats(struct macsec_context *ctx, void *private)
{
    int ret = 0;
    phy_dev_t *phy_dev = (phy_dev_t*)private;
    phy_macsec_dev *macsec_dev = (phy_macsec_dev*)phy_dev->macsec_dev;
    ...
    phy_macsec_log(LOG_REG_DEBUG, "phy_macsec_mdo_get_rx_sa_stats\n");
    ...
    ctx->stats.rx_sa_stats->InPktsOK = *((uint64_t *)&(stats.InPktsOK));
    ctx->stats.rx_sa_stats->InPktsInvalid = *((uint64_t *)&(stats.InPktsInvalid));
    ctx->stats.rx_sa_stats->InPktsNotValid = *((uint64_t *)&(stats.InPktsNotValid));
    ctx->stats.rx_sa_stats->InPktsNotUsingSA = *((uint64_t *)&(stats.InPktsNotUsingSA));
    ctx->stats.rx_sa_stats->InPktsUnusedSA = *((uint64_t *)&(stats.InPktsUnusedSA));
...
{
    /* Device wide */
    .bcm_mdo_dev_open = phy_macsec_mdo_dev_open,
    .bcm_mdo_dev_stop = phy_macsec_mdo_dev_stop,
    /* SecY */
    .bcm_mdo_add_secy = phy_macsec_mdo_add_secy,
    .bcm_mdo_upd_secy = phy_macsec_mdo_upd_secy,
    .bcm_mdo_del_secy = phy_macsec_mdo_del_secy,
    /* Security channels */
    .bcm_mdo_add_rxsc = phy_macsec_mdo_add_rxsc,
    .bcm_mdo_upd_rxsc = phy_macsec_mdo_upd_rxsc,
    .bcm_mdo_del_rxsc = phy_macsec_mdo_del_rxsc,
    /* Security associations */
    .bcm_mdo_add_rxsa = phy_macsec_mdo_add_rxsa,
    .bcm_mdo_upd_rxsa = phy_macsec_mdo_upd_rxsa,
    .bcm_mdo_del_rxsa = phy_macsec_mdo_del_rxsa,
    .bcm_mdo_add_txsa = phy_macsec_mdo_add_txsa,
    .bcm_mdo_upd_txsa = phy_macsec_mdo_upd_txsa,
    .bcm_mdo_del_txsa = phy_macsec_mdo_del_txsa,
    /* Statistics */
    .bcm_mdo_get_dev_stats = phy_macsec_mdo_get_dev_stats,
    .bcm_mdo_get_tx_sc_stats = phy_macsec_mdo_get_tx_sc_stats,
    .bcm_mdo_get_tx_sa_stats = phy_macsec_mdo_get_tx_sa_stats,
    .bcm_mdo_get_rx_sc_stats = phy_macsec_mdo_get_rx_sc_stats,
    .bcm_mdo_get_rx_sa_stats = phy_macsec_mdo_get_rx_sa_stats,
};
