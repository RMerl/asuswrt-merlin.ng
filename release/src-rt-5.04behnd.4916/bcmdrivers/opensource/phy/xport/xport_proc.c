...
extern void __iomem *xport_virt_base;

#define XPORT_BASE      (const uintptr_t)xport_virt_base
...
{
    int port;
    xport_port_status_s port_status = {0};
#if defined(CONFIG_BCM94912) || defined(CONFIG_BCM96813)
    char *xport_status_usage = "Usage: status <port id (0-7)>";

    if ((port = xport_port_validate(argc, argv, 1, 0, 7)) == -1)
#elif defined(CONFIG_BCM96888) || defined(CONFIG_BCM968880) || defined(CONFIG_BCM96837)
    char *xport_status_usage = "Usage: status <port id (0-11)>";

    if ((port = xport_port_validate(argc, argv, 1, 0, 11)) == -1)
...
#error undefined chip
    ...
    if (xport_get_port_status(port, &port_status)) return -1;
...
void xport_proc_xlmac_core_ctrl_dump(uint8_t port_id)
{
    ...
    pr_info("\n");
    pr_info("XPORT_XLMAC_CORE_%d_PORT%d_XLMAC_CTRL                :\n",PID_XLMAC_NUM(port_id), PID_XPORT_NUM(port_id));
...
void xport_proc_xlmac_core_mode_dump(uint8_t port_id)
{
    ...
    pr_info("\n");
    pr_info("XPORT_XLMAC_CORE_%d_PORT%d_XLMAC_MODE                :\n",PID_XLMAC_NUM(port_id), PID_XPORT_NUM(port_id));
...
void xport_proc_xlmac_core_tx_ctrl_dump(uint8_t port_id)
{
    ...
    if (ag_drv_xport_xlmac_core_tx_ctrl_get(port_id,&ctrl)) return;
    pr_info("\n");
    pr_info("XPORT_XLMAC_CORE_%d_PORT%d_TX_CTRL                   :\n",PID_XLMAC_NUM(port_id), PID_XPORT_NUM(port_id));
...
void xport_proc_xlmac_core_tx_mac_sa_dump(uint8_t port_id)
{
    ...
    pr_info("\n");
    pr_info("XPORT_XLMAC_CORE_%d_PORT%d_TX_MAC_SA                 = <0x%016llu>\n",PID_XLMAC_NUM(port_id), PID_XPORT_NUM(port_id),*(uint64_t*)&ctrl_sa);
}
void xport_proc_xlmac_core_rx_ctrl_dump(uint8_t port_id)
{
    ...
    pr_info("\n");
    pr_info("XPORT_XLMAC_CORE_%d_PORT%d_RX_CTRL                   :\n",PID_XLMAC_NUM(port_id), PID_XPORT_NUM(port_id));
...
void xport_proc_xlmac_core_rx_mac_sa_dump(uint8_t port_id)
{
    ...
    pr_info("\n");
    pr_info("XPORT_XLMAC_CORE_%d_PORT%d_RX_MAC_SA                 = <0x%016llu>\n",PID_XLMAC_NUM(port_id), PID_XPORT_NUM(port_id),*(uint64_t*)&rx_sa);
}
void xport_proc_xlmac_core_rx_max_size_dump(uint8_t port_id)
{
    ...
    pr_info("\n");
    pr_info("XPORT_XLMAC_CORE_%d_PORT%d_RX_MAX_SIZE               = <%d>\n",PID_XLMAC_NUM(port_id), PID_XPORT_NUM(port_id),rx_max_size);
...
void xport_proc_xlmac_core_rx_vlan_tag_dump(uint8_t port_id)
{
    ...
    pr_info("\n");
    pr_info("XPORT_XLMAC_CORE_%d_PORT%d_RX_VLAN_TAG               :\n",PID_XLMAC_NUM(port_id), PID_XPORT_NUM(port_id));
...
void xport_proc_xlmac_core_rx_lss_ctrl_dump(uint8_t port_id)
{
    ...
    pr_info("\n");
    pr_info("XPORT_XLMAC_CORE_%d_PORT%d_RX_LSS_CTRL               :\n",PID_XLMAC_NUM(port_id), PID_XPORT_NUM(port_id));
...
void xport_proc_xlmac_core_rx_lss_status_dump(uint8_t port_id)
{
    ...
    pr_info("\n");
    pr_info("XPORT_XLMAC_CORE_%d_PORT%d_RX_LSS_STATUS             :\n",PID_XLMAC_NUM(port_id), PID_XPORT_NUM(port_id));
...
void xport_proc_xlmac_core_clear_rx_lss_status_dump(uint8_t port_id)
{
    ...
    pr_info("\n");
    pr_info("XPORT_XLMAC_CORE_%d_PORT%d_CLEAR_RX_LSS_STATUS       :\n",PID_XLMAC_NUM(port_id), PID_XPORT_NUM(port_id));
...
void xport_proc_xlmac_core_pause_ctrl_dump(uint8_t port_id)
{
    ...
    pr_info("\n");
    pr_info("XPORT_XLMAC_CORE_%d_PORT%d_PAUSE_CTRL                :\n",PID_XLMAC_NUM(port_id), PID_XPORT_NUM(port_id));
...
void xport_proc_xlmac_core_pfc_ctrl_dump(uint8_t port_id)
{
    ...
    pr_info("\n");
    pr_info("XPORT_XLMAC_CORE_%d_PORT%d_PFC_CTRL                  :\n",PID_XLMAC_NUM(port_id), PID_XPORT_NUM(port_id));
...
void xport_proc_xlmac_core_pfc_type_dump(uint8_t port_id)
{
    ...
    pr_info("\n");
    pr_info("XPORT_XLMAC_CORE_%d_PORT%d_PFC_TYPE                  = <0x%04u>\n",PID_XLMAC_NUM(port_id), PID_XPORT_NUM(port_id),pfc_eth_type);
}
void xport_proc_xlmac_core_pfc_opcode_dump(uint8_t port_id)
{
    ...
    pr_info("\n");
    pr_info("XPORT_XLMAC_CORE_%d_PORT%d_PFC_OPCODE                = <0x%04u>\n",PID_XLMAC_NUM(port_id), PID_XPORT_NUM(port_id),pfc_opcode);
}
void xport_proc_xlmac_core_pfc_da_dump(uint8_t port_id)
{
    ...
    pr_info("\n");
    pr_info("XPORT_XLMAC_CORE_%d_PORT%d_PFC_DA                    = <0x%016llu>\n",PID_XLMAC_NUM(port_id), PID_XPORT_NUM(port_id),*(uint64_t*)&pfc_macda);
}
void xport_proc_xlmac_core_llfc_ctrl_dump(uint8_t port_id)
{
    ...
    pr_info("\n");
    pr_info("XPORT_XLMAC_CORE_%d_PORT%d_LLFC_CTRL                 :\n",PID_XLMAC_NUM(port_id), PID_XPORT_NUM(port_id));
...
void xport_proc_xlmac_core_tx_llfc_msg_fields_dump(uint8_t port_id)
{
    ...
    pr_info("\n");
    pr_info("XPORT_XLMAC_CORE_%d_PORT%d_TX_LLFC_MSG_FIELDS        :\n",PID_XLMAC_NUM(port_id), PID_XPORT_NUM(port_id));
...
void xport_proc_xlmac_core_rx_llfc_msg_fields_dump(uint8_t port_id)
{
    ...
    pr_info("\n");
    pr_info("XPORT_XLMAC_CORE_%d_PORT%d_RX_LLFC_MSG_FIELDS        :\n",PID_XLMAC_NUM(port_id), PID_XPORT_NUM(port_id));
...
void xport_xlmac_core_tx_timestamp_fifo_data_dump(uint8_t port_id)
{
    ...
    pr_info("\n");
    pr_info("XPORT_XLMAC_CORE_%d_PORT%d_TX_TIMESTAMP_FIFO_DATA    :\n",PID_XLMAC_NUM(port_id), PID_XPORT_NUM(port_id));
...
void xport_proc_xlmac_core_tx_timestamp_fifo_status_dump(uint8_t port_id)
{
    ...
    pr_info("\n");
    pr_info("XPORT_XLMAC_CORE_%d_PORT%d_TX_TIMESTAMP_FIFO_STATUS  :\n",PID_XLMAC_NUM(port_id), PID_XPORT_NUM(port_id));
...
void xport_proc_xlmac_core_fifo_status_dump(uint8_t port_id)
{
    ...
    pr_info("\n");
    pr_info("XPORT_XLMAC_CORE_%d_PORT%d_FIFO_STATUS               :\n",PID_XLMAC_NUM(port_id), PID_XPORT_NUM(port_id));
...
void xport_proc_xlmac_core_clear_fifo_status_dump(uint8_t port_id)
{
    ...
    pr_info("\n");
    pr_info("XPORT_XLMAC_CORE_%d_PORT%d_CLEAR_FIFO_STATUS         :\n",PID_XLMAC_NUM(port_id), PID_XPORT_NUM(port_id));
...
void xport_proc_xlmac_core_lag_failover_status_dump(uint8_t port_id)
{
    ...
    pr_info("\n");
    pr_info("XPORT_XLMAC_CORE_%d_PORT%d_LAG_FAILOVER_STATUS       :\n",PID_XLMAC_NUM(port_id), PID_XPORT_NUM(port_id));
    pr_info("rsvd                                               = 0x%02x\n",rsvd);
...
void xport_proc_xlmac_core_eee_ctrl_dump(uint8_t port_id)
{
    ...
    pr_info("\n");
    pr_info("XPORT_XLMAC_CORE_%d_PORT%d_EEE_CTRL                  :\n",PID_XLMAC_NUM(port_id), PID_XPORT_NUM(port_id));
    pr_info("rsvd                                               = 0x%02x\n",rsvd);
...
void xport_proc_xlmac_core_eee_timers_dump(uint8_t port_id)
{
    ...
    pr_info("\n");
    pr_info("XPORT_XLMAC_CORE_%d_PORT%d_EEE_TIMERS                :\n",PID_XLMAC_NUM(port_id), PID_XPORT_NUM(port_id));
...
void xport_proc_xlmac_core_eee_1_sec_link_status_timer_dump(uint8_t port_id)
{
    ...
    pr_info("\n");
    pr_info("XPORT_XLMAC_CORE_%d_PORT%d_EEE_1_SEC_LINK_STATUS_TIMER:\n",PID_XLMAC_NUM(port_id), PID_XPORT_NUM(port_id));
...
void xport_proc_xlmac_core_higig_hdr_0_dump(uint8_t port_id)
{
    ...
    pr_info("\n");
    pr_info("XPORT_XLMAC_CORE_%d_PORT%d_HIGIG_HDR_0               = <0x%016llu>\n",PID_XLMAC_NUM(port_id), PID_XPORT_NUM(port_id),*(uint64_t*)&higig_hdr_0);
}
void xport_proc_xlmac_core_higig_hdr_1_dump(uint8_t port_id)
{
    ...
    pr_info("\n");
    pr_info("XPORT_XLMAC_CORE_%d_PORT%d_HIGIG_HDR_1               = <0x%016llu>\n",PID_XLMAC_NUM(port_id), PID_XPORT_NUM(port_id),*(uint64_t*)&higig_hdr_1);
}
void xport_proc_xlmac_core_gmii_eee_ctrl_dump(uint8_t port_id)
{
    ...
    pr_info("\n");
    pr_info("XPORT_XLMAC_CORE_%d_PORT%d_GMII_EEE_CTRL             :\n",PID_XLMAC_NUM(port_id), PID_XPORT_NUM(port_id));
...
void xport_proc_xlmac_core_timestamp_adjust_dump(uint8_t port_id)
{
    ...
    pr_info("\n");
    pr_info("XPORT_XLMAC_CORE_%d_PORT%d_TIMESTAMP_ADJUST          :\n",PID_XLMAC_NUM(port_id), PID_XPORT_NUM(port_id));
...
void xport_proc_xlmac_core_timestamp_byte_adjust_dump(uint8_t port_id)
{
    ...
    pr_info("\n");
    pr_info("XPORT_XLMAC_CORE_%d_PORT%d_TIMESTAMP_BYTE_ADJUST     :\n",PID_XLMAC_NUM(port_id), PID_XPORT_NUM(port_id));
...
void xport_proc_xlmac_core_tx_crc_corrupt_ctrl_dump(uint8_t port_id)
{
    ...
    pr_info("\n");
    pr_info("XPORT_XLMAC_CORE_%d_PORT%d_TX_CRC_CORRUPT_CTRL       :\n",PID_XLMAC_NUM(port_id), PID_XPORT_NUM(port_id));
...
void xport_proc_xlmac_core_e2e_ctrl_dump(uint8_t port_id)
{
    ...
    pr_info("\n");
    pr_info("XPORT_XLMAC_CORE_%d_PORT%d_E2E_CTRL                  :\n",PID_XLMAC_NUM(port_id), PID_XPORT_NUM(port_id));
...
void xport_proc_xlmac_core_e2ecc_module_hdr_0_dump(uint8_t port_id)
{
    ...
    pr_info("\n");
    pr_info("XPORT_XLMAC_CORE_%d_PORT%d_E2ECC_MODULE_HDR_0        = <0x%016llu>\n",PID_XLMAC_NUM(port_id), PID_XPORT_NUM(port_id),*(uint64_t*)&e2ecc_module_hdr_0);
}
void xport_proc_xlmac_core_e2ecc_module_hdr_1_dump(uint8_t port_id)
{
    ...
    pr_info("\n");
    pr_info("XPORT_XLMAC_CORE_%d_PORT%d_E2ECC_MODULE_HDR_1        = <0x%016llu>\n",PID_XLMAC_NUM(port_id), PID_XPORT_NUM(port_id),*(uint64_t*)&e2ecc_module_hdr_1);
}
void xport_proc_xlmac_core_e2ecc_data_hdr_0_dump(uint8_t port_id)
{
    ...
    pr_info("\n");
    pr_info("XPORT_XLMAC_CORE_%d_PORT%d_E2ECC_DATA_HDR_0          = <0x%016llu>\n",PID_XLMAC_NUM(port_id), PID_XPORT_NUM(port_id),*(uint64_t*)&e2ecc_data_hdr_0);
}
void xport_proc_xlmac_core_e2ecc_data_hdr_1_dump(uint8_t port_id)
{
    ...
    pr_info("\n");
    pr_info("XPORT_XLMAC_CORE_%d_PORT%d_E2ECC_DATA_HDR_1          = <0x%016llu>\n",PID_XLMAC_NUM(port_id), PID_XPORT_NUM(port_id),*(uint64_t*)&e2ecc_data_hdr_1);
}
void xport_proc_xlmac_core_e2efc_module_hdr_0_dump(uint8_t port_id)
{
    ...
    pr_info("\n");
    pr_info("XPORT_XLMAC_CORE_%d_PORT%d_E2EFC_MODULE_HDR_0        = <0x%016llu>\n",PID_XLMAC_NUM(port_id), PID_XPORT_NUM(port_id),*(uint64_t*)&e2efc_module_hdr_0);
}
void xport_proc_xlmac_core_e2efc_module_hdr_1_dump(uint8_t port_id)
{
    ...
    pr_info("\n");
    pr_info("XPORT_XLMAC_CORE_%d_PORT%d_E2EFC_MODULE_HDR_1        = <0x%016llu>\n",PID_XLMAC_NUM(port_id), PID_XPORT_NUM(port_id),*(uint64_t*)&e2efc_module_hdr_1);
}
void xport_proc_xlmac_core_e2efc_data_hdr_0_dump(uint8_t port_id)
{
    ...
    pr_info("\n");
    pr_info("XPORT_XLMAC_CORE_%d_PORT%d_E2EFC_DATA_HDR_0          = <0x%016llu>\n",PID_XLMAC_NUM(port_id), PID_XPORT_NUM(port_id),*(uint64_t*)&e2efc_data_hdr_0);
}
void xport_proc_xlmac_core_e2efc_data_hdr_1_dump(uint8_t port_id)
{
    ...
    pr_info("\n");
    pr_info("XPORT_XLMAC_CORE_%d_PORT%d_E2EFC_DATA_HDR_1          = <0x%016llu>\n",PID_XLMAC_NUM(port_id), PID_XPORT_NUM(port_id),*(uint64_t*)&e2efc_data_hdr_1);
}
void xport_proc_xlmac_core_txfifo_cell_cnt_dump(uint8_t port_id)
{
    ...
    pr_info("\n");
    pr_info("XPORT_XLMAC_CORE_%d_PORT%d_TXFIFO_CELL_CNT           = <0x%02u>\n",PID_XLMAC_NUM(port_id), PID_XPORT_NUM(port_id),cell_cnt);
}
void xport_proc_xlmac_core_txfifo_cell_req_cnt_dump(uint8_t port_id)
{
    ...
    pr_info("\n");
    pr_info("XPORT_XLMAC_CORE_%d_PORT%d_TXFIFO_CELL_REQ_CNT       = <0x%02u>\n",PID_XLMAC_NUM(port_id), PID_XPORT_NUM(port_id),cell_req_cnt);
}
void xport_proc_xlmac_core_mem_ctrl_dump(uint8_t port_id)
{
    ...
    pr_info("\n");
    pr_info("XPORT_XLMAC_CORE_%d_PORT%d_MEM_CTRL                  :\n",PID_XLMAC_NUM(port_id), PID_XPORT_NUM(port_id));
...
void xport_proc_xlmac_core_ecc_ctrl_dump(uint8_t port_id)
{
    ...
    pr_info("\n");
    pr_info("XPORT_XLMAC_CORE_%d_PORT%d_ECC_CTRL                  :\n",PID_XLMAC_NUM(port_id), PID_XPORT_NUM(port_id));
...
void xport_proc_xlmac_core_ecc_force_double_bit_err_dump(uint8_t port_id)
{
    ...
    pr_info("\n");
    pr_info("XPORT_XLMAC_CORE_%d_PORT%d_ECC_FORCE_DOUBLE_BIT_ERR  :\n",PID_XLMAC_NUM(port_id), PID_XPORT_NUM(port_id));
...
void xport_proc_xlmac_core_ecc_force_single_bit_err_dump(uint8_t port_id)
{
    ...
    pr_info("\n");
    pr_info("XPORT_XLMAC_CORE_%d_PORT%d_ECC_FORCE_single_BIT_ERR  :\n",PID_XLMAC_NUM(port_id), PID_XPORT_NUM(port_id));
...
void xport_proc_xlmac_core_rx_cdc_ecc_status_dump(uint8_t port_id)
{
    ...
    pr_info("\n");
    pr_info("XPORT_XLMAC_CORE_%d_PORT%d_RX_CDC_ECC_STATUS         :\n",PID_XLMAC_NUM(port_id), PID_XPORT_NUM(port_id));
...
void xport_proc_xlmac_core_tx_cdc_ecc_status_dump(uint8_t port_id)
{
    ...
    pr_info("\n");
    pr_info("XPORT_XLMAC_CORE_%d_PORT%d_tx_CDC_ECC_STATUS         :\n",PID_XLMAC_NUM(port_id), PID_XPORT_NUM(port_id));
...
void xport_proc_xlmac_core_clear_ecc_status_dump(uint8_t port_id)
{
    ...
    pr_info("\n");
    pr_info("XPORT_XLMAC_CORE_%d_PORT%d_CLEAR_ECC_STATUS          :\n",PID_XLMAC_NUM(port_id), PID_XPORT_NUM(port_id));
...
void xport_proc_xlmac_core_intr_status_dump(uint8_t port_id)
{
    ...
    pr_info("\n");
    pr_info("XPORT_XLMAC_CORE_%d_PORT%d_INTR_STATUS               :\n",PID_XLMAC_NUM(port_id), PID_XPORT_NUM(port_id));
...
void xport_proc_xlmac_core_intr_enable_dump(uint8_t port_id)
{
    ...
    pr_info("\n");
    pr_info("XPORT_XLMAC_CORE_%d_PORT%d_INTR_ENABLE               :\n",PID_XLMAC_NUM(port_id), PID_XPORT_NUM(port_id));
...
void xport_proc_xlmac_core_version_id_dump(uint8_t port_id)
{
    ...
    pr_info("\n");
    pr_info("XPORT_XLMAC_CORE_%d_PORT%d_VERSION_ID                = <0x%04u>\n",PID_XLMAC_NUM(port_id), PID_XPORT_NUM(port_id),xlmac_version);
}
...
static void xport_proc_cmd_xlmac_core_reg_dump(uint8_t port_id, char *reg_str)
{
    ...
    pr_info("\n**** XPORT_XLMAC_CORE_%d_PORT%d block\n", port_id >> 2, port_id & 3);
...
#define XPORT_PHYS_BASE         0x837f0000
#define XPORT1_PHYS_BASE        0x837f4000
#define XPORT2_PHYS_BASE        0x837f8000

#define XPORT_XLMAC_CORE_OFFSET 0x00000
#define XPORT_XLMAC_CORE_SIZE   0x00DC0
#define XPORT_MIB_CORE_OFFSET   0x01000
#define XPORT_MIB_CORE_SIZE     0x00F68
#define XPORT_REG_OFFSET        0x02000
#define XPORT_REG_SIZE          0x00010
#define XPORT_XLMAC_REG_OFFSET  0x03000
#define XPORT_XLMAC_REG_SIZE    0x0004C
#define XPORT_MIB_REG_OFFSET    0x03100
#define XPORT_MIB_REG_SIZE      0x00068
#define XPORT_MAB_REG_OFFSET    0x03300
#define XPORT_MAB_REG_SIZE      0x00010
#define XPORT_PORTRESET_OFFSET  0x03400
#define XPORT_PORTRESET_SIZE    0x000f4

#include <xport_proc_4912.c>
...
void xlmac_reg_rmt_lpbk_cntrl_dump(void)
{
    ...
    ag_drv_xport_xlmac_reg_rmt_lpbk_cntrl_get(0, &rmt_lpbk_cntrl);
    pr_info("\n");
...
static int xport_proc_cmd_reg_dump(int argc, char *argv[])
{
    ...
    if (blk_type == eADDR && argc == 3) {
        ...
        uint32_t v32;
        uint64_t v64;
        if(kstrtou32(argv[2], 16, &reg_phy_addr))
		reg_phy_addr=0;
        ...
        reg_addr = (uintptr_t)XPORT_BASE + (reg_phy_addr - XPORT_PHYS_BASE);

        switch ( blk_type )
        {
        case eXLMAC_CORE:
            READ_64(reg_addr, v64);
...
static int xport_proc_cmd_reg_set(int argc, char *argv[])
{
    ...
    pr_info ("Address <0x%08x : 0x%px> Val <0x%08x> Pos <%u> width <%u> mask <0x%016llx>\n",reg_phy_addr,(void*)reg_addr, v32_Lo, field_pos, width_tmp, field_mask);
