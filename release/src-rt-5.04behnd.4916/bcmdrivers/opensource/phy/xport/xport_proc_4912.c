...
{
    int instance = 0;
    char xport_reg_dump_usage[256];
    int instance_max = XPORT_NUM_OF_XLMACS - 1;
    int blk_idx = 1;
    char *blk_p = argv[1];
    char *q_str_p = NULL;
    ...
    char usage_format[] = 
        ...
        "       reg_dump XLMAC_CORE <port_id 0..%d>\n"
        "       reg_dump XLMAC_WRAP | MIB_WRAP | XPORT_TOP | MAB | PORTRESET  <xlmac_id 0..%d>\n"
        "       reg_dump addr <Qualifying string>\n\n";

    sprintf(xport_reg_dump_usage, usage_format, XPORT_NUM_OF_PORTS - 1, instance_max);
    ...
    if (!strcmp(blk_p, "XLMAC_CORE")) { blk_type = eXLMAC_CORE; instance_max = XPORT_NUM_OF_PORTS - 1; } 
