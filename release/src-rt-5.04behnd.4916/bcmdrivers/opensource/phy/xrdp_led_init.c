...
static void __iomem *xrdpled_base;
...
static int xrdpled_probe(dt_device_t *pdev)
{
    ...
    dev_dbg(&pdev->dev, "xrdpled_base=0x%px\n", xrdpled_base);
...
int xrdp_leds_init(void *_leds_info)
{
    ...
    for (j = 0; j < 3; j++)
    {
        ...
        if ((led_mux & LED_SPEED_10) || (led_mux & LED_SPEED_5G))
        ...
        if ((led_activity & LED_SPEED_10) || (led_activity & LED_SPEED_5G))
