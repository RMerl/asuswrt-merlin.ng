...
extern int ba_svc_boot_secondary(u32 cpu_mask, u32 vector);
/*
 * set specified CPU start address in the BIUCFG address space.
 */
#ifndef PLATFORM_USING_SMC
...
void biu_set_cpu_ep(int cpu, long ep)
{
...
static int brcm_pwr_domain_on(u_register_t mpidr)
{
	...
	/*
	 * here it is assumed that boot loader didn't start the CPUs. Set the
	 * CPU entry point and power the CPU up.
	 */
#if defined PLATFORM_USING_SMC
	u32 jump_addr;

	jump_addr = (u32)(long)bl31_warm_entrypoint;
	ba_svc_boot_secondary(1 << ((mpidr >> MPIDR_AFF1_SHIFT) & 0xFF), jump_addr);
#else
#ifdef AARCH32
...
#else
#if defined PLATFORM_USING_SMC
	WDTIMER0->WatchDogDefCount = MICROSECS_TO_WDOG_TICKS(1);
	WDTIMER0->WatchDogCtl = WDT_START_1;
	WDTIMER0->WatchDogCtl = WDT_START_2;
#else
