...
#if !defined(BPCM_ZONE_HAS_STATUS)
#define BPCM_PWR_ZONE_N_STATUS		BPCM_PWR_ZONE_N_CONTROL

#define BPCMZoneCtrlOffset(zone)	BPCMOffset(zones[zone].control)
#define BPCMZoneCtrlRegOffset(zone)	(BPCMZoneCtrlOffset(zone) >> 2)

#define BPCMZoneStsOffset		BPCMZoneCtrlOffset
#define BPCMZoneStsRegOffset		BPCMZoneCtrlRegOffset
#endif
...
static int SendAndWait(TCommand * cmd, TCommand * rsp)
{
	...
	PMC->ctrl.gpTmr0Ctl = (((uint32_t)1 << 31) | (1 << 29) |
			       ((400000 << 1) & 0x1fffffff));	// 400ms

	while (!(PMC->dqm.notEmptySts & PMC_DQM_RPL_STS) &&
	       (PMC->ctrl.gpTmr0Ctl & ((uint32_t)1 << 31))) ;
#else
	PMC->ctrl.gpTmr2Ctl = (((uint32_t)1 << 31) | (1 << 29) | 400000);	// 400ms

	while (!(PMC->dqm.notEmptySts & PMC_DQM_RPL_STS) &&
	       (PMC->ctrl.gpTmr2Ctl & ((uint32_t)1 << 31))) {
...
int PowerOnZone(int devAddr, int zone)
{
	...
	status =
	    ReadBPCMRegister(devAddr, BPCMRegOffset(zones[zone].control), &reg.Reg32);
	status +=
...
int PowerOffDevice(int devAddr, int repower)
{
	...
	if (PMC_ACCESS_BPCM_DIRECT || pmc_mode == PMC_MODE_PMB_DIRECT) {
		...
		status =
		    ReadBPCMRegister(devAddr, BPCMRegOffset(zones[0].control), &reg.Reg32);
		status +=
		   ReadBPCMRegister(devAddr, BPCMZoneStsRegOffset(0), &sts.Reg32);
		if (status == kPMC_NO_ERROR && sts.Bits.pwr_off_state == 0) {
