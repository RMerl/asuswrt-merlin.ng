...
#                                                              #
# NOTE: Script must be run from                                #
#                       SDK_root_directory/bootloaders         #
#                                                              #
# ./gen4_delg_package.sh <output folder>                       #
#                                                              #
# eg:                                                          #
# ./gen4_delg_package.sh build/work/delg_package               #
#                                                              #
################################################################
...
then
  ...
  echo "Usage: ./gen4_delg_package.sh <output folder>"
  exit 1;
fi
...
# Key-owner's Key Name Hint to Key Mapping
OWNER_KEY_MAP=$KEY_PATH/GEN4/$SOC_ID/key_map.cfg

# Key-owner's Encryption key
ROE_AES_EK=a1k1
ROE_AES_IV=a1k1
ROE_AES_OPTIONS="-aes-128-cbc -salt"

# Key-owner's Signing Key
OWNER_ROT_RSA_PRV=r2k1
...
DELG_SEC_POL=build/configs/sec_policy_template.its
...
if [ -n $DELG_KEY_RING ]; then
	if [ $FORCE_KEY_RING_GEN -eq 1 ]; then
		...
		./build/scripts/gen4_encrypt_openssl_simple.sh $OWNER_KEY_MAP "$ROE_AES_OPTIONS" $ROE_AES_EK $ROE_AES_IV $DELG_KEY1_SRC ${DELG_KEY1_SRC}.enc
		DELG_ENCKEY1_HEX=`hexdump -v -e '1/1 "%02X"' ${DELG_KEY1_SRC}.enc`
		./build/scripts/gen4_encrypt_openssl_simple.sh $OWNER_KEY_MAP "$ROE_AES_OPTIONS" $ROE_AES_EK $ROE_AES_IV $DELG_KEY2_SRC ${DELG_KEY2_SRC}.enc
...
./build/work/gen_sec_delg_obj --delg_id=$DELEGATE_ID --rollback=$ROLLBACK --secits=$DELG_SEC_POL --kencaesdelg=$DELG_ENC_AES --krsadelgpub=$DELG_RSA_PUB_MOD_BIN --krsadbgpub=$DBG_RSA_PUB_MOD_BIN --out_path=$OUTDIR $LEG_SDR_OPT $KEY_RING_OPT
...
./build/scripts/gen4_sign_openssl_simple.sh $OWNER_KEY_MAP pkcs1 $OWNER_ROT_RSA_PRV ${OUTDIR}/${SEC_POL_ITB} ${OUTDIR}/${SEC_POL_ITB_SIG}
./build/scripts/gen4_sign_openssl_simple.sh $OWNER_KEY_MAP pkcs1 $OWNER_ROT_RSA_PRV ${OUTDIR}/${SDR_BIN} ${OUTDIR}/${SDR_BIN_SIG}
