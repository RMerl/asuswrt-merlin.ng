...
dpfe_fix_load_offset=${CONFIG_BOOT_BLOB_JTAG_LOAD_MAX_DDR_SIZE}+0x2000;
...
do 
	...
	do
		...
		echo d.load.binary ~~~~/${j##*/} 0x${hex_addr} /noclear >> ${cmm_file}
...
do
    ...
    ddrfile=`cd ${cmm_out} && ls ${dpfe_prefix}${BRCM_CHIP}_*DDR*${mcb}.cmm 2> /dev/null`
    ddrgdb=`cd ${cmm_out} && ls ${dpfe_prefix}${BRCM_CHIP}_*DDR*${mcb}.gdb 2> /dev/null` 
    if [ "x${ddrfile}" == "x" ]; then
	m=$(printf "%d" 0x${mcb})
        let m=${m}+0x20000
	m=$(printf "%x" ${m})
        ddrfile=`cd ${cmm_out} && ls ${dpfe_prefix}${BRCM_CHIP}_*DDR*${m}.cmm 2> /dev/null`
        ddrgdb=`cd ${cmm_out} && ls ${dpfe_prefix}${BRCM_CHIP}_*DDR*${m}.gdb 2> /dev/null` 
    fi
    if [ "x${ddrfile}" == "x" ]; then
	m_temp=`echo ${mcb}|sed -e 's/^4//'`
        ddrfile=`cd ${cmm_out} && ls ${dpfe_prefix}${BRCM_CHIP}_*DDR*${m_temp}.cmm 2> /dev/null`
        ddrgdb=`cd ${cmm_out} && ls ${dpfe_prefix}${BRCM_CHIP}_*DDR*${m_temp}.gdb 2> /dev/null`
    fi 
    if [ "x${ddrfile}" == "x" ]; then
	m_temp=`echo ${mcb}|sed -e 's/^4//'`
