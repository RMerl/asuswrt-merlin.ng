...
typedef union {
	struct {
		...
		uint32_t ndiv_frac_mode_sel:1;	// 30
...
typedef struct {
	BPCM_ID_REG id_reg;	// offset = 0x00
	BPCM_CAPABILITES_REG capabilities;	// offset = 0x04
	uint32_t reserved0[6];	// offset = 0x08..0x1c
	PLL_CTRL_REG resets;	// offset = 0x20
	uint32_t reserved1[5];	// offset = 0x24
	PLL_NDIV_REG ndiv;	// offset = 0x38
	PLL_PDIV_REG pdiv;	// offset = 0x3c
	...
	RDPPLL_DECPDIV_REG decpdiv;// offset = 0x58
	RDPPLL_DECCH25_REG decch25;// offset = 0x5c
} RDPPLL_BPCM_REGS;
