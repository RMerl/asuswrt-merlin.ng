...
/*
	op - operation type
		...
		2 - poll read, read until the value read & mask equal to the value in entry
*/
...
reg_entry reg_table[] = {
#ifdef CONFIG_BCMBCA_DDR_SIMINIT
	/* 
	 * For XIP boot, sim code runs the ddr init so ARM just
	 * wait for its finish. 
	 */
#if defined(CONFIG_BCM4912) || defined(CONFIG_BCM6813) || defined(CONFIG_BCM6765)
	{2, 0x80040008, 0x100, 0x100},
	{0, 0, 0, 0x9},   /* randomly pick 512MB total size*/	  
#endif
#else
	/*
	 * For BootROM boot, only ARM can run DDR init. Need to obtain
	 * the ddr init reg sequence from the memory team that match IKOS
	 * test environment 
	 */
...
#define DDR3_8BIT_512MB_x4
#ifdef DDR3_8BIT_512MB_x4
        /* 4 x 4096Mbit x8  DDR3 1067Mz */	
        {1, 0x800400b8, 0, 0x12345678},
        {0, 0x800400b8, 0, 0x12345678},
        {0, 0x80040e88, 0, 0x00230003},
        {1, 0x80040e88, 0, 0x80230003},
        {0, 0x8004093c, 0, 0x00000011},
        {0, 0x8004097c, 0, 0x008d011a},
        {1, 0x8004097c, 0, 0x808d011a},
        {0, 0x80040008, 0, 0x01000000},
        {1, 0x80040008, 0, 0x01000000},
        {1, 0x80040944, 0, 0x00000307},
        {1, 0x80060364, 0, 0x00000001},
        ...
        {1, 0x80040310, 0, 0x00000001},
#endif
        {1, 0x80040954, 0, 0x000e0a0e},
        {1, 0x80040958, 0, 0x00040006},
        {1, 0x8004095c, 0, 0x00000000},
        {1, 0x80040960, 0, 0x1b000e24},
        {1, 0x80040964, 0, 0x00081000},
        {1, 0x80040968, 0, 0x00000000},
        {1, 0x8004096c, 0, 0x00000408},
        {1, 0x80040970, 0, 0x0002000b},
        {1, 0x80040974, 0, 0x01560056},
        {1, 0x80040978, 0, 0x40001100},
        {1, 0x8004097c, 0, 0x008d0118},
        {0, 0x80040048, 0, 0x04100002},
        {1, 0x80040048, 0, 0x04110002},
        {1, 0x8004004c, 0, 0x00000380},
        {1, 0x80040050, 0, 0x00000000},
        {1, 0x80040054, 0, 0x20210808},
        {1, 0x80040058, 0, 0x000000e4},
        {0, 0x80040908, 0, 0x00071000},
        {1, 0x80040908, 0, 0x00071001},
        {0, 0x80040008, 0, 0x01000600},
        {1, 0x80040008, 0, 0x81000700},
        {0, 0x80040008, 0, 0x81000700},
        {0, 0, 0, 0xb},   /* 2GB hijack last entry value for total ddr size */
#endif
#ifdef DDR3_16BIT_512MB_x2	
        /* 2 4096Mbit x16 DDR3 1067Mz */
        {1, 0x800400b8, 0, 0x12345678},
        {0, 0x800400b8, 0, 0x12345678},
        {0, 0x80040e88, 0, 0x00030003},
        {1, 0x80040e88, 0, 0x80030003},
...
static uint32_t run_reg_init(void)
{
	...
	while (entry->addr) {
		switch(entry->op) {
			...
			case REG_OP_POLL:
				while (1) {
				/*printf("ddr_regs         poll  : addr=0x00%08x, mask=0x%08x expect 0x08x\n", entry->addr, entry->mask, entry->value); */
