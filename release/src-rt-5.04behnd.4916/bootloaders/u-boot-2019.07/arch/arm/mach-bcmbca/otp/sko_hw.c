...
static otp_hw_cmn_err_t set_perm(otp_hw_cmn_t *dev,
			u32 addr, 
			...
			u32 lock)
{
	...
	u32	sko_sect;
        otp_hw_cmn_row_t  *row;
	u32 perm_mask = SKO_PERM_UNLOCK_ALL,
			perm_curr = 0;
	...
	_DPRT("%s %s pre_mask 0x%02x perm ctrl 0x%08x\n",__FUNCTION__,(lock?"locking":"unlocking"), (SKO_PAC_MASK&perm_mask), perm);
	/* Retrieve current permissions */
	perm_curr = (sko_hw_readl(dev, SKO_KEYN_PAC_CTRL_OFFSET+sko_sect*SKO_SIZE)&SKO_PAC_MASK);
	if (lock) {	
		_DPRT(" w/o current 0x%x\n",
			((perm_mask&SKO_KEY_PERM_MASK)|(perm_mask&SKO_PAC_PERM_MASK)));
		perm_mask = 
			(((perm_curr|perm_mask)&SKO_KEY_PERM_MASK)|
			((perm_curr&perm_mask)&SKO_PAC_PERM_MASK));
	...
	_DPRT("\t %s  perm curr 0x%02x final mask 0x%02x \n",__FUNCTION__, perm_curr, (SKO_PAC_MASK&perm_mask));
	if ( perm ) {
...
#define	CMN_CTL_LOCK_NS_MASK	(OTP_HW_CMN_CTL_LOCK_PAC_NSRD|		\
					OTP_HW_CMN_CTL_LOCK_PAC_NSW|	\
					OTP_HW_CMN_CTL_LOCK_NSRD|	\
					OTP_HW_CMN_CTL_LOCK_NSW)


#define	CMN_CTL_LOCK_S_MASK	(OTP_HW_CMN_CTL_LOCK_PAC_SRD|		\
					OTP_HW_CMN_CTL_LOCK_PAC_SW|	\
					OTP_HW_CMN_CTL_LOCK_SRD|	\
					OTP_HW_CMN_CTL_LOCK_SW)

#define	CMN_CTL_LOCK_KEY_NS_MASK	(OTP_HW_CMN_CTL_LOCK_NSRD|OTP_HW_CMN_CTL_LOCK_NSW)
#define	CMN_CTL_LOCK_PAC_NS_MASK	(OTP_HW_CMN_CTL_LOCK_PAC_NSRD|OTP_HW_CMN_CTL_LOCK_PAC_NSW)

#define	CMN_CTL_LOCK_KEY_S_MASK	(OTP_HW_CMN_CTL_LOCK_SRD|OTP_HW_CMN_CTL_LOCK_SW)
#define	CMN_CTL_LOCK_PAC_S_MASK	(OTP_HW_CMN_CTL_LOCK_PAC_SRD|OTP_HW_CMN_CTL_LOCK_PAC_SW)

#define	CMN_CTL_LOCK_SEL_ALL_MASK	(CMN_CTL_LOCK_S_MASK|CMN_CTL_LOCK_NS_MASK)	

static otp_hw_cmn_err_t sko_hw_lock(otp_hw_cmn_t *dev,
			u32 addr,	
			otp_hw_cmn_perm_t perm,
			u32 lock)
{
	otp_hw_cmn_err_t rc = OTP_HW_CMN_OK; 
	u32 perm_mask = 0x0;
	u32 res = 0;
	if (perm & (OTP_HW_CMN_CTL_LOCK_NS|OTP_HW_CMN_CTL_LOCK_NS_PROV)) {
		if (!lock ) {
			/* Unlock non-secure access */

			/* unlock non-secure access for provisioning - Check if slot is empty */
			if (perm & OTP_HW_CMN_CTL_LOCK_NS_PROV) { 
				/* res will be 0 if slot is unpopulated */
				rc = status(dev, addr, 
					OTP_HW_CMN_STATUS_ROW_DATA_VALID, &res); 
				_DPRT("handling prov! rc:0x%08x res:0x%08x 0x%08x\n", rc, res, addr);
				/* res will be 0 if slot is unpopulated */
			} 
			if ((perm & OTP_HW_CMN_CTL_LOCK_NS) || ((rc == OTP_HW_CMN_OK) && !res)) {
				_DPRT("unlocking for NS access!\n");
				/* Set mask for unlocking non-secure access. Access
				 * will be granted based on value of 'lock' flag */
				perm_mask = CMN_CTL_LOCK_SEL_ALL_MASK; 
			}	  
			else
			{
				_DPRT("locking NS access!\n");
				/* We are in PROV mode and slot IS populated, we should
				 * lock non-secure access for this slot as it has data */
				set_perm(dev, addr, CMN_CTL_LOCK_KEY_NS_MASK, 1);
				set_perm(dev, addr, CMN_CTL_LOCK_PAC_NS_MASK, 1);
			}
		} else if (perm & OTP_HW_CMN_CTL_LOCK_NS) {
			/* Lock non-secure access */

			/* unlock secure access */
			set_perm(dev, addr, CMN_CTL_LOCK_PAC_S_MASK, 0);

			/* Set mask for locking non-secure access. Access
			 * will be granted based on value of 'lock' flag */
			perm_mask = CMN_CTL_LOCK_KEY_NS_MASK;
		}	
	} else if (perm & OTP_HW_CMN_CTL_LOCK_ALL) {
		/* Set mask for locking all access. Access
	         * will be granted based on value of 'lock' flag */
	  	perm_mask = CMN_CTL_LOCK_SEL_ALL_MASK;
			...
			if (lock) {
				...
				/* Set mask for locking secure access. Access
	 		 	 * will be granted based on value of 'lock' flag */
	  			perm_mask = CMN_CTL_LOCK_SEL_ALL_MASK;
			} else {
				/* Unlock secure access */

				/* Lock non-secure access. This will have no 
				 * effect until reset if set before this call */
				set_perm(dev, addr, CMN_CTL_LOCK_KEY_NS_MASK, 1);

				/* Set mask for unlocking secure access. Access
			 	 * will be granted based on value of 'lock' flag */
	  			perm_mask = CMN_CTL_LOCK_PAC_S_MASK;
			}
	} else {
		perm_mask = perm;
	}
	...
	if (perm_mask) {
		/* Configure secure/non-secure access based on 
		 * selected mask and value of 'lock' flag */
		set_perm(dev, addr, perm_mask, lock);		
	}
	return rc;
}
			...
			u32* res)
{
	...
	switch ((u32)(cmd->ctl)) {
   		...
   		case OTP_HW_CMN_CTL_LOCK: {
				...
				if ( ctl_data->addr != SKO_0_DATA_ROW && ctl_data->addr != SKO_1_DATA_ROW ) {
					ctl_data->addr = SKO_1_DATA_ROW;
				}
				rc = sko_hw_lock(dev, ctl_data->addr, ctl_data->perm, 
					((u32)(cmd->ctl) == OTP_HW_CMN_CTL_LOCK));
			}
			break;
