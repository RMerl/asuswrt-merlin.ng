						...
						u32 fsr_offs)
{
	...
	do {
		_DPRT("Fsr status 0x%x\n",fsr_hw_readl(dev, FSR_OTP_STATUS_OFFSET+fsr_offs));
			...
			u32 size)
{
	...
	if (!FSR_PORTAL_VALID(fsr_hw_readl(dev, FSR_PORTAL_STATUS_OFFSET+fsr_offs))) {
		...
		if (FSR_PORTAL_READY(fsr_hw_readl( dev, FSR_PORTAL_STATUS_OFFSET+fsr_offs))) {
						...
						u32 ksr_offs)
{
	...
	do {
		_DPRT("Ksr status 0x%x\n", ksr_hw_readl(dev, KSR_OTP_STATUS_OFFSET+ksr_offs));
			...
			u32 size)
{
	...
	if(KSR_PORTAL_KEY_LOADED(ksr_hw_readl( dev, KSR_PORTAL_STATUS_OFFSET+ksr_offs)) ||
           KSR_PORTAL_DESC_VALID(ksr_hw_readl( dev, KSR_PORTAL_STATUS_OFFSET+ksr_offs))){
...
static otp_hw_cmn_err_t fsr_set_populated_pacs(otp_hw_cmn_t *dev, u32 pac_val)
{
	int i;
	otp_hw_cmn_err_t rc = OTP_HW_CMN_ERR_FAIL; 
	for (i = 0; i < FSR_MAX_ROW_ADDR - FSR_START_ROW_ADDR; i++) { 
		/* If portal is valid and descriptor is good, we have valid data so lock NS */
		if ( FSR_PORTAL_VALID(fsr_hw_readl(dev, FSR_PORTAL_STATUS_OFFSET+i*FSR_SIZE)) ) {
				fsr_hw_writel(dev, FSR_PAC_OFFSET+i*FSR_SIZE, pac_val);
		}
	}
	rc = OTP_HW_CMN_OK;
	return rc;
...
static otp_hw_cmn_err_t fsr_set_all_pacs(otp_hw_cmn_t *dev, u32 pac_val)
{
	int i;
	otp_hw_cmn_err_t rc = OTP_HW_CMN_ERR_FAIL; 
	for (i = 0; i < FSR_MAX_ROW_ADDR - FSR_START_ROW_ADDR; i++) { 
...
static otp_hw_cmn_err_t portal_lock(otp_hw_cmn_t *dev,
			u32 addr, 
			...
			u8 lock)
{
	...
	if (perm_mask) {
		...
		if( IS_KSR_ADDR(row->addr)) {
			curr_perm_mask = ksr_hw_readl(dev, KSR_PAC_OFFSET + (row->addr-KSR_START_ROW_ADDR)*KSR_SIZE);
			if( lock ) {
				ksr_hw_writel(dev, KSR_PAC_OFFSET + (row->addr-KSR_START_ROW_ADDR)*KSR_SIZE, 
			...
			curr_perm_mask = fsr_hw_readl(dev, FSR_PAC_OFFSET + (row->addr-FSR_START_ROW_ADDR)*FSR_SIZE);
			if( lock ) {
				fsr_hw_writel(dev, FSR_PAC_OFFSET + (row->addr-FSR_START_ROW_ADDR)*FSR_SIZE, 
					curr_perm_mask|perm_mask);
			} else {
				fsr_hw_writel(dev, FSR_PAC_OFFSET + (row->addr-FSR_START_ROW_ADDR)*FSR_SIZE, 
					curr_perm_mask&(~perm_mask));
...
static otp_hw_cmn_err_t multi_portal_lock(otp_hw_cmn_t *dev,
			otp_hw_cmn_perm_t perm,
			u8 lock)
{
	otp_hw_cmn_err_t rc = OTP_HW_CMN_ERR_FAIL; 
	if( lock ) {
		...
		if (perm & OTP_HW_CMN_CTL_LOCK_NS || perm & OTP_HW_CMN_CTL_LOCK_NS_PROV ) {
			...
			if( perm & OTP_HW_CMN_CTL_LOCK_NS_PROV) {
				...
				rc |= fsr_set_populated_pacs(dev, KSR_PERM_LOCK_NS);
			}
		} else if (perm & OTP_HW_CMN_CTL_LOCK_S) {
	  		rc = ksr_set_all_pacs(dev, KSR_PERM_UNLOCK_S);
	  		rc |= fsr_set_all_pacs(dev, KSR_PERM_UNLOCK_S);
		} else if (perm & OTP_HW_CMN_CTL_LOCK_ALL) {
	  		rc = ksr_set_all_pacs(dev, KSR_PERM_UNLOCK_ALL);
	  		rc |= fsr_set_all_pacs(dev, KSR_PERM_UNLOCK_ALL);
		} 
	}
	return rc;
}

static otp_hw_cmn_err_t portal_status(otp_hw_cmn_t *dev,
			u32 addr,
			...
			u32* res)
{
	...
	if( IS_KSR_ADDR(row->addr)) {
		perm_mask = ksr_hw_readl(dev, KSR_PAC_OFFSET + 
			(row->addr-KSR_START_ROW_ADDR)*KSR_SIZE);
	} else if(IS_FSR_ADDR(row->addr)) {
		perm_mask = fsr_hw_readl(dev, FSR_PAC_OFFSET + 
	...
	if( IS_KSR_ADDR(row->addr)) {
		if ( !KSR_PORTAL_VALID(ksr_hw_readl(dev, KSR_PORTAL_STATUS_OFFSET+(row->addr-KSR_START_ROW_ADDR)*KSR_SIZE)) ||
			!KSR_RD_DGOOD(ksr_hw_readl(dev, KSR_RD_DESC_OFFSET+(row->addr-KSR_START_ROW_ADDR)*KSR_SIZE))) {
			status &= ~OTP_HW_CMN_STATUS_ROW_DATA_VALID;
		}
		if (KSR_PORTAL_READY(ksr_hw_readl(dev, KSR_PORTAL_STATUS_OFFSET+(row->addr-KSR_START_ROW_ADDR)*KSR_SIZE))) {
			status &= ~OTP_HW_CMN_STATUS_ROW_RD_LOCKED;
		}
	} else if(IS_FSR_ADDR(row->addr)) {
		if ( !FSR_PORTAL_VALID(fsr_hw_readl(dev, FSR_PORTAL_STATUS_OFFSET+(row->addr-FSR_START_ROW_ADDR)*FSR_SIZE))) {
			status &= ~OTP_HW_CMN_STATUS_ROW_DATA_VALID;
		}
	}
	*res = status;
			...
			u8 lock)
{
	...
	switch ( perm ) {
		...
		case OTP_HW_CMN_CTL_LOCK_S:
			rc = multi_portal_lock(dev, perm, lock);
		break;
		default:
			rc = portal_lock(dev, addr, perm, lock);
	}
	return rc;
}
			...
			u32* res)
{
	...
	otp_hw_cmn_err_t rc = OTP_HW_CMN_ERR_FAIL; 
	rc = portal_status(dev, addr, status, res);
	return rc;
}
