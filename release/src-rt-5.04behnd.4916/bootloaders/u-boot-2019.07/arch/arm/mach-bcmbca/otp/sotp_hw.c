...
{
	if ((s32)s >= SOTP_SECT_ID_RSVD  && s < SOTP_SECT_ID_MAX) {
...
static SotpRowStatus sotpIsRegionFuseLocked(otp_hw_cmn_t *dev,u32 region)
{
   ...
   if ((region >= (SOTP_NUM_REG_IN_FUSELOCK_ROW - 1)) && (region < (SOTP_NUM_ROWS/SOTP_ROWS_IN_REGION)))	 
   {
   ...
   for( i=startRegion; i<SOTP_NUM_ROWS/SOTP_ROWS_IN_REGION; i++) 
   {
      for( j=0; j<SOTP_ROWS_IN_REGION; j++) 
      {
         ...
         if( j+(i*SOTP_ROWS_IN_REGION) < SOTP_FIRST_ECC_CONFIG_ROW )
           sotpReadNonEccRow( dev, j+(i*SOTP_ROWS_IN_REGION), &value);
         else
           sotpReadEccRow(dev, j+(i*SOTP_ROWS_IN_REGION), (u32*)&value, 1);
...
static SotpRowStatus sotpKeySlotReadLockStatus(otp_hw_cmn_t *dev, u32 section)
{
   u32 startRegion;
   int i;
