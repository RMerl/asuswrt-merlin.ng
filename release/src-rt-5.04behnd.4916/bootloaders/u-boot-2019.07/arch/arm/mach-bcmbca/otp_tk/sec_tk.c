                                ...
                                bcm_sec_state_t cur_state)
{
        int res, rc = 0;
        int mid_size;
	u32 arch_ver = (ks_get_arch_info() >> 28);
        switch(cur_state) {
				...
				u32 val = 0x1, size = 4;
#if 0 
				if (sec_tk_fuse_verify(OTP_MAP_BRCM_PRODUCTION_MODE, &val, size )) {
					goto err;
				}
#endif
                ...
                case SEC_STATE_GEN3_MFG:
                        if (req_state == KS_REQ_TRANSIT_FLD) {
                                int res;
				u32 aes_key_size = 0, aes_iv_size = KS_AES_128_CBC_SZ;
                        	/*program otp bits accordingly */
                                u8 ek_iv[KS_AES_256_CBC_SZ+KS_AES_128_CBC_SZ],
                                                hash[KS_HASH_SZ];
                                u32 mid;
				ks_data_type_t aes_type = 
					(arch_ver == KS_ARCH_SEC_VERv1? KS_DATA_TYPE_KEY_AES_CBC_192_EK:
						(arch_ver == KS_ARCH_SEC_VERv2? 
							KS_DATA_TYPE_KEY_AES_CBC_256_EK : KS_DATA_TYPE_KEY_AES_CBC_128_EK));
                                if (ks_get_data_info(aes_type, 
                                        KS_DATA_STATE_FLD_ENCR, ek_iv) != KS_ERR_SUCC) {
                                        goto err;
                                }
				if (ks_type2size(aes_type, &aes_key_size)  != KS_ERR_SUCC) {
                                        goto err;
				}
                                if (ks_get_data_info(KS_DATA_TYPE_KEY_AES_CBC_IV, 
                                        KS_DATA_STATE_FLD_ENCR, ek_iv + aes_key_size) != KS_ERR_SUCC) {
                                        goto err;
                                }
				...
				/*
                                ...
                                */
				mid = ntohl(mid);
                                ntohl_array((u32*)ek_iv,(aes_key_size + aes_iv_size)/sizeof(u32)); 
                                ntohl_array((u32*)hash,(KS_HASH_SZ)/sizeof(u32)); 
                                mid_size = sizeof(u32);
				rc = sec_tk_fuse_verify(OTP_MAP_CUST_MFG_MRKTID, &mid, mid_size);
				if (rc) {
                                        goto err;
				}
				...
				if (arch_ver == KS_ARCH_SEC_VER) {
					rc = sec_tk_fuse_verify(SOTP_MAP_FLD_ROE, ek_iv, aes_key_size+aes_iv_size);
					if (rc) {
                                        	goto err;
					}
				} else if (arch_ver == KS_ARCH_SEC_VERv1 || arch_ver == KS_ARCH_SEC_VERv2 ) {
					rc = sec_tk_fuse_verify(SOTP_MAP_FLD_ROE, ek_iv, aes_key_size);
					if (rc) {
                                        	goto err;
					}
					rc = sec_tk_fuse_verify(SOTP_MAP_FLD_ROE1, ek_iv + aes_key_size, aes_iv_size);
					if (rc) {
                                        	goto err;
					}
				} else {
				...
				if (ks_type2size(KS_DATA_TYPE_DEV_KEY_256, &aes_key_size)  != KS_ERR_SUCC) {
                                        goto err;
				}
                                if (ks_get_data_info(KS_DATA_TYPE_DEV_KEY_256, 
                                        KS_DATA_STATE_GEN_RAND, ek_iv) == KS_ERR_SUCC) {
                                	ntohl_array((u32*)ek_iv, aes_key_size/sizeof(u32)); 
					/* KRNG gen random key */
