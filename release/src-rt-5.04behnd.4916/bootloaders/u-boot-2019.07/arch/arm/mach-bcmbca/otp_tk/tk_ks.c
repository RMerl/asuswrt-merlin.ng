		...
		const u8* aes_key)
{
	...
	hdr = (ks_hdr_t*)keystore;
        ...
        */
        ...
        if (sec_arch != (hdr->sec_arch&0xfffffff)) {
                printf("EARC\n");
                goto err;
        }
	//BRKPT;
	...
	ks_hdr_t  *hdr ; 
        ...
        switch(KS_DATA_GET_STATE(key_info->type_state)) {
			...
			/*in place decryption encrypted - */
                        aes_cbc_decrypt( (u32*)&key_info->data[0],/*src*/
			...
			void* data)
{
	...
	sig_size =   (arch_ver == KS_ARCH_SEC_VERv1? KS_SIG1_SIZE:
						(arch_ver == KS_ARCH_SEC_VERv2? 
							KS_SIG2_SIZE : KS_SIG_SIZE));
        key_info = (ks_key_info_t*)((uintptr_t)(&((ks_t*)ks_obj.mem)->key.info) + sig_size - KS_SIG_SIZE);
        key_info_max = (u8*)key_info + hdr->info_size - (sizeof(u32) + sig_size);
        ...
        if (ks_get_data(key_info, ks_obj.ek, ks_obj.ek_len, data) != KS_ERR_SUCC) {
               goto err;
        }
        rc =  KS_ERR_SUCC; 
err:
