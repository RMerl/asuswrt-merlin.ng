...
#endif
#if defined(CONFIG_BCMBCA_DDR4) && defined(CONFIG_BCMBCA_DDR4_DEF_MCBSEL)
	if ((mcb_mode&SPL_DDR_INIT_MCB_OVRD) && (mcb_mode&SPL_DDR_INIT_DDR4_SAFE_MODE))
		mcb_sel = CONFIG_BCMBCA_DDR4_DEF_MCBSEL;
#endif
#if defined(CONFIG_BCMBCA_LPDDR4) && defined(CONFIG_BCMBCA_LPDDR4_DEF_MCBSEL)
	if (mcb_mode&SPL_DDR_INIT_MCB_OVRD) {
		if (mcb_mode&SPL_DDR_INIT_LPDDR4_SAFE_MODE)
			mcb_sel = CONFIG_BCMBCA_LPDDR4_DEF_MCBSEL;
		else if (mcb_mode&SPL_DDR_INIT_LPDDR4X_SAFE_MODE)
			mcb_sel = CONFIG_BCMBCA_LPDDR4_DEF_MCBSEL | BP1_DDR_TYPE_LPDDR4X;
	}
#endif
#if defined(CONFIG_BCMBCA_LPDDR5) && defined(CONFIG_BCMBCA_LPDDR5_DEF_MCBSEL)
	if (mcb_mode&SPL_DDR_INIT_MCB_OVRD) {
		if (mcb_mode&SPL_DDR_INIT_LPDDR5_SAFE_MODE)
			mcb_sel = CONFIG_BCMBCA_LPDDR5_DEF_MCBSEL;
		else if (mcb_mode&SPL_DDR_INIT_LPDDR5X_SAFE_MODE)
			mcb_sel = CONFIG_BCMBCA_LPDDR5_DEF_MCBSEL | BP1_DDR_TYPE_LPDDR5X;
	}
#endif
#if !defined(CONFIG_BCMBCA_DDR_DEF_MCBSEL) && !defined(CONFIG_BCMBCA_DDR4_DEF_MCBSEL) && \
	!defined(CONFIG_BCMBCA_LPDDR4_DEF_MCBSEL) && !defined(CONFIG_BCMBCA_LPDDR5_DEF_MCBSEL)
...
#ifdef CONFIG_BCMBCA_DDR_DEFAULT
			mcb_sel = CONFIG_BCMBCA_DDR_DEFAULT;
#else
			mcb_sel = CONFIG_BCMBCA_DDR_DEF_MCBSEL;
#endif
			safe_mode = 1;
...
static int load_ddr_stdalone(uint32_t mcb_sel, void *ddr_bin, int *len)
{
	uint32_t table_magic;

	if (BP_DDR_IS_DDR3(mcb_sel))
		table_magic = DDR3_TABLE_MAGIC;
#if defined(CONFIG_BCMBCA_DDR4)
	else if (BP_DDR_IS_DDR4(mcb_sel))
		table_magic = DDR4_TABLE_MAGIC;
#endif
#if defined(CONFIG_BCMBCA_LPDDR4)
	else if (BP1_DDR_IS_LPDDR4(mcb_sel) || BP1_DDR_IS_LPDDR4X(mcb_sel))
		table_magic = LPDDR4_TABLE_MAGIC;
#endif
#if defined(CONFIG_BCMBCA_LPDDR5)
	else if (BP1_DDR_IS_LPDDR5(mcb_sel))
		table_magic = LPDDR5_TABLE_MAGIC;
#endif
	else
		return 1;

	return load_boot_blob(table_magic, 0, ddr_bin, len);
}
#endif
...
#else
		if (load_ddr_stdalone(mcb_sel, (void *)ddr_stdalone, &ddr_stdsize)) {
...
static int run_ddrinit(ddr_init_func ddr_init, uint32_t* mcb_ptr)
{
	...
	if (ddr_gen_scramber_seed(seed))
		return -1;
#endif
	mcb = mcb_ptr[MCB_CHECKSUM_WORD];
	printf("MCB Selector 0x%X Checksum 0x%X Safe Mode %d\n",
		mcb_sel, mcb, safe_mode);
	mcb = mcb_ptr[MCB_VERSION_WORD];
	printf("MCB Rev %d.%d.%d ",
		(mcb >> MCB_VERSION_MAJOR_SHIFT) & MCB_VERSION_MASK,
		(mcb >> MCB_VERSION_MINOR_SHIFT) & MCB_VERSION_MASK,
		(mcb >> MCB_VERSION_BUILD_SHIFT) & MCB_VERSION_MASK);
	mcb = mcb_ptr[MCB_SN_WORD];
	printf("Ref ID 0x%05X Sub Bld 0x%03X\n",
		(mcb >> MCB_SN_REFID_SHIFT) & MCB_SN_REFID_MASK,
		(mcb >> MCB_SN_BLDID_SHIFT) & MCB_SN_BLDID_MASK);
	...
	printf("dpfe stdalone addr 0x%x, seg buf 0x%x\n",
		(uint32_t)((uintptr_t)param.dpfe_stdalone),
		(uint32_t)((uintptr_t)param.dpfe_segbuf));
#endif
...
int spl_ddrinit(uint32_t mcb_mode, uint32_t sel)
{
	...
	if (IS_JTAG_LOADED(boot_params)) {
		...
		dcache_disable();
#endif
	//add this code to appease compiler [-Wunused-label]
	// this label is used only by the board recovery jtag script
        if(0 == 1) {
		goto jtag_spl_done;
	}
