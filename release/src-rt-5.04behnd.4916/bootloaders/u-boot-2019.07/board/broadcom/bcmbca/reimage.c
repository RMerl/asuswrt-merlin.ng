...
#define ERASE_COUNT_ATTEMPTS 0
#define ERASE_COUNT_SUCCESS 1

static struct reimager rei = { 0 };
static struct hsearch_data rei_envht;
...
#endif
	size = 1 << shift;	// in MB
	printf("DDR size from controller %lldMB\n", size);
...
#define NAND_MTD_ID		"brcmnand.0"
#define SPINAND_MTD_ID	"spi1.0"

static char *get_mtd_id(void)
{
	int bdev;
	char *mtd_id;
	...
	if (bdev ==  BOOT_DEVICE_NAND)
		...
		mtd_id = SPINAND_MTD_ID;
	else {
		printf("unsupported boot device for reimage %d\n", bdev);
		mtd_id = NULL;
	}
...
static int check_nv_crc(struct nvram_s *nv)
{
	...
	new = crc32_le(0xffffffff, (unsigned char *)nv, 1024);
...
static int erase(struct mtd_info *mtd, int first, int blocks, int mode);
...
static int do_prepare(cmd_tbl_t * cmdtp, int flag, int argc, char *const argv[])
{
	...
	if (rei.pure_payload_image) {
		...
		cp = (unsigned char *)CONFIG_SYS_LOAD_ADDR;
		for (leb = 0;
		...
		erase(mtd, rei.erase_first_start, rei.erase_first_blocks, 
			ERASE_COUNT_ATTEMPTS);
		put_mtd_device(mtd);
		while (count < rei.payload_blocks * rei.blocksizeK * 1024) {
			cp = (unsigned char *)CONFIG_SYS_LOAD_ADDR;
			while ((cp < (unsigned char *)CONFIG_SYS_LOAD_ADDR + SZ_16M)
			       && (count <
			...
			if (cp > (unsigned char *)CONFIG_SYS_LOAD_ADDR + SZ_16M) {
				offset =
				    s - (cp -
					 ((unsigned char *)CONFIG_SYS_LOAD_ADDR + SZ_16M));
				leb--;
				// printf
		...
		while (bp <
			...
			i = mtd_read(mtd, rblk * mtd->erasesize, mtd->erasesize,
				     ...
				     mtd->erasesize * (rblk -
						       rei.payload_start));
			printf("read loader -> mem %zd bytes ret %d\n", sz, i);
			rblk++;
			bp += sz;
		}
		printf("loader in memory buffer at 0x%p and size 0x%x\n",
		...
		while (block < (rei.loader_blocks + rei.payload_blocks)) {
			i = mtd_read(mtd, rblk * mtd->erasesize, mtd->erasesize,
				     &sz, (unsigned char *)CONFIG_SYS_LOAD_ADDR);
			printf("R");
			if (sz) {
				block++;
				i = 1;
	...
	cp = (unsigned char *)more_env;

	cp += sprintf((char *)cp, "ethaddr=%x:%x:%x:%x:%x:%x",
	...
	*(cp++) = '\0';
	...
	cp += sprintf((char *)cp, "once=sdk metadata 1 1;setenv once true;saveenv");
	*(cp++) = '\0';

	*(cp++) = '\0';
	more_env_size = (char *)cp - more_env + 1;
	...
	for (bp = rei.loader_payload;
		...
		uint32_t new, crc;
		tenv = (int *)bp;
		printf("env check at %lx\r", (long)(bp - rei.loader_payload));
...
static int do_finish(cmd_tbl_t * cmdtp, int flag, int argc, char *const argv[])
{
	...
	erase(mtd, 0, rei.loader_blocks, ERASE_COUNT_ATTEMPTS);
	for (i = 0; i < rei.loader_blocks; i++) {
		ret = mtd_write(mtd,
				i * mtd->erasesize,
				mtd->erasesize, &sz,
				&rei.loader_payload[i * mtd->erasesize]);
		printf("w mem->ldr %zd b ret %d\n", sz, ret);
	}
	...
	if (rei.preserved_data_max_len) {
		...
		run_command(cmd, 0);
		sprintf(cmd, "ubi write 0x%p transition 0x%x",
			  ...
			  char *const argv[])
{
	...
	if (mtd_id) {
		sprintf(mtd_id_full, "%s=%s", mtd->name, mtd_id);
	} else {
		put_mtd_device(mtd);
		return 1;
	}
  	env_set("mtdids", mtd_id_full);
	put_mtd_device(mtd);
	...
	for (i = 1; i < 3; i++) {
		...
		if (IS_ERR_OR_NULL(mtd)) {
			printf("failed to get mtd dev %s\n", tmp);
			return (-1);
		}
		for (j = 0; j < 0x1000000/blocksize; j++) {
			// printf("read i=%d j=%d\n",i,j);
			ret = mtd_read(mtd, j * blocksize, 2048, &sz, (unsigned char *)tmp);
			// printf("ret %d read %d\n",ret,sz);
		...
		if (rei.pure_payload_image) {
			...
			err =
			    ubi_eba_read_leb(rei.ubi, rei.ubi->volumes[m],
					     0, (char *)CONFIG_SYS_LOAD_ADDR, 0, s, 0);
			cp = (char *)CONFIG_SYS_LOAD_ADDR;
			t = simple_strtoul(cp, (char **)&cp, 0);
				...
				char *const argv[])
{
	...
	printf("allocated 0x%x bytes at %p\n", len, cp);
	return (0);
}
			    ...
			    char *const argv[])
{
	...
	rei.recovery_chunks_list = (struct recovery_chunks *)CONFIG_SYS_LOAD_ADDR;
	rei.recovery_data_buf =
	    (unsigned char *)CONFIG_SYS_LOAD_ADDR + (blocks * pages +
	...
	while (block < blocks) {
		...
		oob_ops.ooboffs = 0;
		oob_ops.datbuf = bp;
		oob_ops.oobbuf = (unsigned char *)spare;
			...
			char *const argv[])
{
	...
	mtd = bcmbca_get_image_mtd_device();
	blocks = (mtd->size / mtd->erasesize);
	erase(mtd, 0, blocks, ERASE_COUNT_ATTEMPTS);
	chunk = 0;
			      ...
			      char *const argv[])
{
	...
	if (rei.recovery_data_len) {
		sprintf(cmd, "ubi create recovery 0x%lx dynamic 34",
			(long)(rei.recovery_data_len + (void *)rei.recovery_data_buf -
			(void *)rei.recovery_chunks_list));
		run_command(cmd, 0);
		sprintf(cmd, "ubi write 0x%p recovery 0x%lx",
...
/* 
 * Merge the env blob in ep from the null seperated name=value more_env list.
 * Add if the name variable is new. Update if it already exists in ep
 */
static int reimage_merge_env(env_t *ep, int env_len, char *more_env,
				 int more_env_size)
{
	int rc = 0;
	char *res, *name, *value, *env, *env_start;
	ENTRY e, *entry;

	env_start = malloc(more_env_size);
	if (!env_start)
		return -1;
	memcpy(env_start, more_env, more_env_size);

	if (himport_r(&rei_envht, (char *)ep->data, env_len, '\0', 0, 0,
			0, NULL) == 0) {
		printf("Cannot import reimage env: errno = %d\n", errno);
		free(env_start);
		return -1;
	}
	env = env_start;
	while (*env && (env - env_start) < more_env_size) {
		name = env;
		value = strchr(env, '=');
		if (!value)
			break;
		*value++ = 0;
		e.key	= name;
		e.data	= value;
		hsearch_r(e, ENTER, &entry, &rei_envht, H_PROGRAMMATIC);
		if (!entry) {
			printf("## Error inserting \"%s\" variable, errno=%d\n",
				name, errno);
			break;
		}
		env = value + strlen(value) + 1;
	}
	res = (char *)ep->data;
	rc = hexport_r(&rei_envht, '\0', 0, &res, env_len, 0, NULL);
	if (rc < 0)
		printf("Cannot export reimage env: errno = %d\n", errno);

	hdestroy_r(&rei_envht);
	free(env_start);
	
	ep->crc = the_env_crc32(0, ep->data, env_len);
...
void reimage_splice_env(struct reimager *r, char *more_env, int more_env_size)
{
	...
	for (bp = r->loader_payload;
		...
		uint32_t new, crc;
		tenv = (int *)bp;
		printf("env check at %lx\r", (long)(bp - r->loader_payload));
		if (tenv[0] == BOOT_MAGIC_MAGIC) {
			printf("\nGOT IT\n");
			ep = (env_t *) & tenv[2];
			memcpy(&crc, &ep->crc, sizeof(crc));
			...
			if (new != crc) {
				...
				printf("good\n");
				reimage_merge_env(ep, tenv[1] - 4, more_env, more_env_size);
			}
		}
	}
}
