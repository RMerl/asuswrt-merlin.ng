...
struct reimager {
	...
	// bookkeeping
	int blocksizeK;
	unsigned char *remaining_payload;	// buffer for EBs that are part of payload and didn't fit in burn_first
	int remaining_payload_len;	// length of payload held in remaining_payload[]
	unsigned char *loader_payload;	// in-memory copy of loader
	...
	unsigned char *recovery_data_buf;
