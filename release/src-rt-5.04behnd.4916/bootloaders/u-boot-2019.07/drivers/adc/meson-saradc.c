...
meson_saradc_get_fifo_count(struct meson_saradc_priv *priv)
{
	...
	return RDP_FIELD_GET(MESON_SAR_ADC_REG0_FIFO_COUNT_MASK, regval);
}
...
static int meson_saradc_wait_busy_clear(struct meson_saradc_priv *priv)
{
	...
	do {
		udelay(1);
		regmap_read(priv->regmap, MESON_SAR_ADC_REG0, &regval);
	} while (RDP_FIELD_GET(MESON_SAR_ADC_REG0_BUSY_MASK, regval) && timeout--);
					...
					unsigned int channel, uint *val)
{
	...
	fifo_chan = RDP_FIELD_GET(MESON_SAR_ADC_FIFO_RD_CHAN_ID_MASK, regval);
	...
	fifo_val = RDP_FIELD_GET(MESON_SAR_ADC_FIFO_RD_SAMPLE_VALUE_MASK, regval);
