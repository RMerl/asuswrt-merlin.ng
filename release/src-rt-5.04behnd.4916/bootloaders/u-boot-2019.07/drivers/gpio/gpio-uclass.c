...
/*
 * On boot up, Some of the pins need to be set to gpio pinmux and tied to a specific high/low output value or gpio input state.
 * The earliest this can be achieved is in u-boot proper. Recently, gpio-hog driver was backported which can read the device tree and set the "gpio-hog" pins to defined gpio state.
 * Unfortunately, When linux boots up, it also "hogs" these gpios and other drivers can no longer use it. e.g. in 63138 certain *MII clock pins have to be set to the gpio out but then later when the mii driver starts it can configure these to clock pinmux.
 * Add a new gpio-hog-uboot boolean property in the device tree, and only u-boot code will search and set the assigned gpio to "hog-uboot".
 * Later when linux boots up, it will only look for gpio-hog nodes ignore gpio-hog-uboot nodes.
 * */
#define GPIO_HOG_STR "gpio-hog"
#define GPIO_HOG_UBOOT_STR "gpio-hog-uboot"

struct gpio_hog_priv {
	struct gpio_desc gpiod;
};

#define UBOOT_HOG_ONLY 0xef
...
static int gpio_hog_ofdata_to_platdata(struct udevice *dev)
{
	...
	dev_read_prop(dev, "gpios", &len);
	plat->gpio_nums =  len/sizeof(u32);
	plat->val = calloc(plat->gpio_nums, sizeof(u32));
	if (plat->val != NULL)
	{
		ret = dev_read_u32_array(dev, "gpios", plat->val, plat->gpio_nums);
		if (ret) {
			free(plat->val);
...
static int gpio_hog_probe(struct udevice *dev)
{
	...
	int ret,i;
	char *str=GPIO_HOG_STR;


	if (plat->flag == UBOOT_HOG_ONLY)
		str=GPIO_HOG_UBOOT_STR;
	for(i=0;i<plat->gpio_nums;i+=2) {
		ret = gpio_dev_request_index(dev->parent, dev->name, str, 
					     plat->val[i], plat->gpiod_flags,
					     plat->val[i+1], &priv->gpiod);
		if (ret < 0) {
...
static int gpio_post_bind(struct udevice *dev)
{
	...
	dev_for_each_subnode(node, dev) {
		if (ofnode_read_bool(node, GPIO_HOG_STR)) {
		...
		else if (ofnode_read_bool(node, GPIO_HOG_UBOOT_STR)) {
			const char *name = ofnode_get_name(node);

			device_bind_driver_to_node(dev, "gpio_hog", name,
						   node, &child);
			plat->flag=UBOOT_HOG_ONLY;
		}
	}
#endif
	return 0;
