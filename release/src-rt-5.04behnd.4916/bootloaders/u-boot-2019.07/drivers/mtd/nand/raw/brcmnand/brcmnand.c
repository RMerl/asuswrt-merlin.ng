...
static inline u32 brcmnand_ecc_level_mask(struct brcmnand_controller *ctrl)
{
	...
	/* v7.2 includes additional ECC levels, but 7.3 return it back */
	if (ctrl->nand_version == 0x0702)
...
static int brcmnand_setup_dev(struct brcmnand_host *host)
{
	...
	if ((chip->ecc.strength == 0) && (chip->ecc.mode != NAND_ECC_ON_DIE))
		dev_err(ctrl->dev, "WARNING!!! ECC disabled, this is typically only for on-die ECC NAND devices\n");
	...
	if ((chip->ecc.mode != NAND_ECC_HW) && (chip->ecc.mode != NAND_ECC_ON_DIE)) {
		dev_err(ctrl->dev, "only HW ECC or on-die ECC supported; selected: %d\n",
			chip->ecc.mode);
		return -EINVAL;
	}
	...
	offs1 = brcmnand_cs_offset(ctrl, host->cs, BRCMNAND_CS_TIMING1);
	offs2 = brcmnand_cs_offset(ctrl, host->cs, BRCMNAND_CS_TIMING2);

	nand_timing_1 = nand_readreg(ctrl, offs1);
	nand_timing_2 = nand_readreg(ctrl, offs2);

	if (dev_read_u32(ctrl->dev, "timing_1", &timing_1) || !timing_1)
		timing_1 = 0x00320000;

	if (dev_read_u32(ctrl->dev, "timing_2", &timing_2) || !timing_2)
		timing_2 = 0x00000004;

	if (timing_1 & BRCMNAND_TIMING_1_tWP_MASK) {
		nand_timing_1 &= ~BRCMNAND_TIMING_1_tWP_MASK;
		nand_timing_1 |= (timing_1 & BRCMNAND_TIMING_1_tWP_MASK);
	}
	if (timing_1 & BRCMNAND_TIMING_1_tWH_MASK) {
		nand_timing_1 &= ~BRCMNAND_TIMING_1_tWH_MASK;
		nand_timing_1 |= (timing_1 & BRCMNAND_TIMING_1_tWH_MASK);
	}
	if (timing_1 & BRCMNAND_TIMING_1_tRP_MASK) {
		nand_timing_1 &= ~BRCMNAND_TIMING_1_tRP_MASK;
		nand_timing_1 |= (timing_1 & BRCMNAND_TIMING_1_tRP_MASK);
	}
	if (timing_1 & BRCMNAND_TIMING_1_tREH_MASK) {
		nand_timing_1 &= ~BRCMNAND_TIMING_1_tREH_MASK;
		nand_timing_1 |= (timing_1 & BRCMNAND_TIMING_1_tREH_MASK);
	}
	if (timing_1 & BRCMNAND_TIMING_1_tCS_MASK) {
		nand_timing_1 &= ~BRCMNAND_TIMING_1_tCS_MASK;
		nand_timing_1 |= (timing_1 & BRCMNAND_TIMING_1_tCS_MASK);
	}
	if (timing_1 & BRCMNAND_TIMING_1_tCLH_MASK) {
		nand_timing_1 &= ~BRCMNAND_TIMING_1_tCLH_MASK;
		nand_timing_1 |= (timing_1 & BRCMNAND_TIMING_1_tCLH_MASK);
	}
	if (timing_1 & BRCMNAND_TIMING_1_tALH_MASK) {
		nand_timing_1 &= ~BRCMNAND_TIMING_1_tALH_MASK;
		nand_timing_1 |= (timing_1 & BRCMNAND_TIMING_1_tALH_MASK);
	}
	if (timing_1 & BRCMNAND_TIMING_1_tADL_MASK) {
		nand_timing_1 &= ~BRCMNAND_TIMING_1_tADL_MASK;
		nand_timing_1 |= (timing_1 & BRCMNAND_TIMING_1_tADL_MASK);
	}

	nand_writereg(ctrl, offs1, nand_timing_1);

	if (timing_2 & BRCMNAND_TIMING_2_tWB_MASK) {
		nand_timing_2 &= ~BRCMNAND_TIMING_2_tWB_MASK;
		nand_timing_2 |= (timing_2 & BRCMNAND_TIMING_2_tWB_MASK);
	}
	if (timing_2 & BRCMNAND_TIMING_2_tWHR_MASK) {
		nand_timing_2 &= ~BRCMNAND_TIMING_2_tWHR_MASK;
		nand_timing_2 |= (timing_2 & BRCMNAND_TIMING_2_tWHR_MASK);
	}
	if (timing_2 & BRCMNAND_TIMING_2_tREAD_MASK) {
		nand_timing_2 &= ~BRCMNAND_TIMING_2_tREAD_MASK;
		nand_timing_2 |= (timing_2 & BRCMNAND_TIMING_2_tREAD_MASK);
	}

	nand_writereg(ctrl, offs2, nand_timing_2);
