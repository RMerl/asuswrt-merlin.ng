...
static void denali_detect_max_banks(struct denali_nand_info *denali)
{
	...
	denali->max_banks = 1 << RDP_FIELD_GET(FEATURES__N_BANKS, features);
			       ...
			       unsigned long *uncor_ecc_flags)
{
	...
	max_bitflips = RDP_FIELD_GET(ECC_COR_INFO__MAX_ERRORS, ecc_cor);
			       ...
			       unsigned long *uncor_ecc_flags, uint8_t *buf)
{
	...
	do {
		...
		err_sector = RDP_FIELD_GET(ECC_ERROR_ADDRESS__SECTOR, err_addr);
		err_byte = RDP_FIELD_GET(ECC_ERROR_ADDRESS__OFFSET, err_addr);

		err_cor_info = ioread32(denali->reg + ERR_CORRECTION_INFO);
		err_cor_value = RDP_FIELD_GET(ERR_CORRECTION_INFO__BYTE,
					  err_cor_info);
		err_device = RDP_FIELD_GET(ERR_CORRECTION_INFO__DEVICE,
				       err_cor_info);
