...
/* 
 * For the old sf2 based chips(63138, 63148, 4908, 63158), they do not use dt
 * based PHY framework so no phy led auto init. Need to manually init the led
 */
#if defined(MANUAL_LED_INIT)
#include "bcm_bca_leds.h"
#define DT_NUM_OF_PORTS  9
static bca_leds_info_t port_leds_info[DT_NUM_OF_PORTS];
void ephy_leds_init(bca_leds_info_t*);
void ephy_init(void);
#endif
...
#if defined(MANUAL_LED_INIT)
	while(port_index <  DT_NUM_OF_PORTS)
	{
		/* sf2 switch is the second switch */
		port_leds_info[port_index].sw_id = 1;
...
static int sf2_eth_probe(struct udevice *dev)
{
	...
	if (!ret) {
		sf2_base_init((uintptr_t)devm_ioremap(dev, res.start, resource_size(&res)), (uintptr_t)(void*)priv->switch_core);
#endif
	}
...
#if defined(MANUAL_LED_INIT)
	ephy_init();
	memset(port_leds_info, 0xff, sizeof(port_leds_info));
#endif
	...
	dev_for_each_subnode(subnode, dev) {
		...
		ofnode_for_each_subnode(port_node, subnode) {
			...
			{
				bca_led_request_network_leds(port_node, &port_leds_info[port_index]);
				port_index++;
			}
#endif
		}
