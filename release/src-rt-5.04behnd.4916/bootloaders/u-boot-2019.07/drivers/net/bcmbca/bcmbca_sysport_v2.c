...
static int __sp_enable_tdma(volatile sys_port_tdma *sysport_tdma)
{
	...
	sysport_tdma->SYSTEMPORT_TDMA_CONTROL |= SYSPORT_TDMA_CONTROL_TDMA_EN_M | SYSPORT_TDMA_CONTROL_TPD_16b_EN;
...
//	v32 |= SYSPORT_RDMA_CTRL_DDR_DESC_SWAP_M;     /* Both Byte and word swap enabled for Desc - TBD - need to understand */
	...
	// set RD_MAX_BURST_SIZE RD_MIN_BURST_SIZE to 8 (for 16 byte TX descriptor)
	sysport_tdma->SYSTEMPORT_TDMA_DDR_DESC_RING_CTRL[0]= 0x0808;
...
#define XBAR_CNTRL  (volatile uint32_t *) 0x80411004
			...
			if (i==0) {
#define LFH_P0_CTRL (volatile uint32_t *) 0x80474B00
    #define LFH_XGMII_SEL                       0x20
    #define LFH_XGMII_SEL_OVRD                  0x40

				if (phy_dev->link && !phy_dev_is_xgmii_mode(phy_dev)) {
					*LFH_P0_CTRL &= ~LFH_XGMII_SEL;
				} else {
					*LFH_P0_CTRL |= LFH_XGMII_SEL;
				}
				*LFH_P0_CTRL |= LFH_XGMII_SEL_OVRD;
			}
#endif
...
static int sysport_init(struct udevice *dev)
{
	...
	{
		if (priv->xbow_ipa)
			priv->xbow_ipa->XBOW_IPA_RDS_CFG = SYSPORT_XBOW_IPA_RDS_DISABLE;  // 6765 set crossbow in pass-through mode
		...
		if (priv->xbow_ipa)
			priv->xbow_ipa->XBOW_IPA_SIB_CTRL = SYSPORT_XBOW_IPA_SIB_CTL_RX_EN | SYSPORT_XBOW_IPA_SIB_CTL_TX_EN;
...
static int __sp_send(volatile sys_port_tdma *sysport_tdma, unsigned char* tx_dma_bufaddr, uint8_t *buffer, uint32_t length)
{
	...
	txdesc_lo = (uint32_t)(uintptr_t)(tx_dma_bufaddr);

	// write tx desc  word0,word1 to hw
	...
	// write tx desc word2,word3 to hw
	sysport_tdma->SYSTEMPORT_TDMA_DESCRIPTOR_WRITE_PORT[0].SYSTEMPORT_TDMA_DESCRIPTOR_XX_WRITE_PORT_LO=0;

        barrier();

	sysport_tdma->SYSTEMPORT_TDMA_DESCRIPTOR_WRITE_PORT[0].SYSTEMPORT_TDMA_DESCRIPTOR_XX_WRITE_PORT_HI=0;
	...
	if(sysport_poll(priv, 2) == 0)
	{
		*packetp=(unsigned char *) static_buff;//memalign(sizeof(int), MAX_PKT_LEN);
		if (*packetp == NULL) {
			...
			{
				__sp_recv(priv->sys_port_rdma[0], *packetp, (uint32_t*)&length);
			}
			else if(priv->sys_port_rdma[1] != NULL) 
			{
				__sp_recv(priv->sys_port_rdma[1], *packetp, (uint32_t*)&length);
			}
			rc = length;
		}
	}
...
{
	if(packet != NULL && packet != (uchar *)static_buff)
		free(packet);
	return 0;
}
...
static int sysport_probe(struct udevice *dev)
{
	...
	priv->sphy_ctrl=NULL;
	if (!ret && priv->phy_test_ctrl != NULL) {
	...
	ret = dev_read_resource_byname(dev, "systemport-ipa", &res);
	priv->xbow_ipa=NULL;
	if (!ret) {
		priv->xbow_ipa=devm_ioremap(dev, res.start, resource_size(&res));
	}
