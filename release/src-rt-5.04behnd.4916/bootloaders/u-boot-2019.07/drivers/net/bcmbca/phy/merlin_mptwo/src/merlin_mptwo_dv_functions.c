...
err_code_t merlin_mptwo_prbs_err_count_ll(uint32_t *prbs_err_cnt) {
    ...
    EFUN(merlin_mptwo_pmd_rdt_reg(TLB_RX_PRBS_CHK_ERR_CNT_MSB_STATUS, &rddata));
...
err_code_t merlin_mptwo_prbs_err_count_state(uint32_t *prbs_err_cnt, uint8_t *lock_lost) {
    ...
    EFUN(merlin_mptwo_prbs_err_count_ll(prbs_err_cnt));
...
err_code_t merlin_mptwo_dig_lpbk_rptr(uint8_t enable, enum srds_rptr_mode_enum mode) {
  switch(mode) {
    case DATA_IN_SIDE:
        ...
        if (enable) {
            EFUN(merlin_mptwo_loop_timing(0));
            EFUN(merlin_mptwo_loop_timing(1));     /* Program DATA_IN_SIDE side TX_PI for loop-timing mode to lock SYS TX to SYS TX clock freq. */
        } else {
            EFUN(merlin_mptwo_loop_timing(0));
        }
        break;
    ...
    case DIG_LPBK_SIDE:
        ...
        EFUN(merlin_mptwo_tx_rptr_mode_timing(1));          /* Program DIG_LPBK_SIDE side TX_PI for repeater mode to lock LINE TX to SYS RX clock freq. */
        EFUN(wr_dig_lpbk_en(enable));                  /* 0 = diabled, 1 = enabled */
...
err_code_t merlin_mptwo_ull_from_dig_lpbk(enum srds_rptr_mode_enum mode) {
   switch(mode) {
    case DATA_IN_SIDE:
        EFUN(merlin_mptwo_dig_lpbk_rptr(0,mode));
        EFUN(merlin_mptwo_tx_ull_config(1));
        break;
    case DIG_LPBK_SIDE:
        EFUN(merlin_mptwo_dig_lpbk_rptr(0,mode));
        EFUN(merlin_mptwo_rx_ull_config(1));
        break;
    default : return(_error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
        break;
...
err_code_t merlin_mptwo_rmt_lpbk(uint8_t enable) {
    if (enable) {
        EFUN(merlin_mptwo_loop_timing(enable));
...
err_code_t merlin_mptwo_rmt_lpbk_from_ull(void) {
    EFUN(merlin_mptwo_rmt_lpbk(0)); /* Reset TX_PI */
    EFUN(merlin_mptwo_rmt_lpbk(1)); /* Enable RMT LPBK */
    return (ERR_CODE_NONE);
}
...
err_code_t merlin_mptwo_ull_from_rmt_lpbk(enum srds_rptr_mode_enum mode) {
  switch(mode) {
    case DATA_IN_SIDE:
        EFUN(merlin_mptwo_rx_rptr_mode_timing(1));  /* Enable jitter filter enable */
        EFUN(_ull_rx_mode_setup(1));
        break;
    case RMT_LPBK_SIDE:
        EFUN(merlin_mptwo_rmt_lpbk(0));         /* Reset TX_PI */
        EFUN(merlin_mptwo_tx_rptr_mode_timing(1));  /* Enable jitter filter enable */
        EFUN(_ull_tx_mode_setup(1));
        break;
    default : return(_error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
        break;
...
err_code_t merlin_mptwo_nl_from_rmt_lpbk(enum srds_rptr_mode_enum mode) {
  switch(mode) {
    case DATA_IN_SIDE:
        EFUN(merlin_mptwo_rx_rptr_mode_timing(1));  /* Enable RX PI transfer */
        break;
    case RMT_LPBK_SIDE:
        EFUN(merlin_mptwo_rmt_lpbk(0));                 /* Reset TX_PI */
        EFUN(merlin_mptwo_tx_rptr_mode_timing(1));      /* Enable jitter filter enable */
...
err_code_t merlin_mptwo_tx_pi_jitt_gen(uint8_t enable, int16_t freq_override_val, enum srds_tx_pi_freq_jit_gen_enum jit_type, uint8_t tx_pi_jit_freq_idx, uint8_t tx_pi_jit_amp) {
    ...
    EFUN(merlin_mptwo_tx_pi_freq_override(enable, freq_override_val));
...
err_code_t merlin_mptwo_uc_reset(uint8_t enable) {
  if (enable) {
    ...
    EFUN(merlin_mptwo_pmd_wr_reg(0xD200, 0x0000));
    EFUN(merlin_mptwo_pmd_wr_reg(0xD201, 0x0000));
    EFUN(merlin_mptwo_pmd_wr_reg(0xD202, 0x0000));
    EFUN(merlin_mptwo_pmd_wr_reg(0xD203, 0x0000));
    EFUN(merlin_mptwo_pmd_wr_reg(0xD207, 0x0000));
    EFUN(merlin_mptwo_pmd_wr_reg(0xD208, 0x0000));
    EFUN(merlin_mptwo_pmd_wr_reg(0xD20A, 0x080f));
    EFUN(merlin_mptwo_pmd_wr_reg(0xD20C, 0x0002));
    EFUN(merlin_mptwo_pmd_wr_reg(0xD20D, 0x0000));
  }
  else {
...
err_code_t merlin_mptwo_core_pwrdn(enum srds_core_pwrdn_mode_enum mode) {
    switch(mode) {
    case PWR_ON:
        EFUN(_merlin_mptwo_core_clkgate(0));
    ...
    case PWRDN_DEEP:
        ...
        EFUN(merlin_mptwo_delay_ns(   500)); /* wait >50 comclk cycles  */
                EFUN(wrc_afe_s_pll_pwrdn(0x1));
        EFUN(_merlin_mptwo_core_clkgate(1));
        break;
    default:
...
err_code_t merlin_mptwo_lane_pwrdn(enum srds_core_pwrdn_mode_enum mode) {
  ...
  */
    ...
    case PWR_ON:
        ...
        EFUN(_merlin_mptwo_lane_clkgate(0));
        break;
    case PWRDN:
    ...
    case PWRDN_DEEP:
        ...
        EFUN(wr_ln_rx_s_pwrdn(0x1));
        EFUN(wr_ln_tx_s_pwrdn(0x1));
        EFUN(_merlin_mptwo_lane_clkgate(1));
...
err_code_t merlin_mptwo_isolate_ctrl_pins(uint8_t enable) {

    EFUN(merlin_mptwo_isolate_lane_ctrl_pins(enable));
    EFUN(merlin_mptwo_isolate_core_ctrl_pins(enable));

  return (ERR_CODE_NONE);
}

err_code_t merlin_mptwo_isolate_lane_ctrl_pins(uint8_t enable) {
...
err_code_t merlin_mptwo_isolate_core_ctrl_pins(uint8_t enable) {
...
err_code_t merlin_mptwo_wrw_uc_ram(uint16_t addr, uint16_t wr_val) {
...
err_code_t merlin_mptwo_wrb_uc_ram(uint16_t addr, uint8_t wr_val) {
    EFUN(wrc_micro_mdio_ram_access_mode(0x2));          /* Select Data RAM access through MDIO Register interface mode */
...
/* Micro RAM Word Read */
uint16_t merlin_mptwo_rdw_uc_ram(err_code_t *err_code_p, uint16_t addr) {
    uint16_t rddata;
...
/* Micro RAM Byte Read */
uint8_t merlin_mptwo_rdb_uc_ram(err_code_t *err_code_p, uint16_t addr) {
    uint8_t rddata;
...
err_code_t merlin_mptwo_wrws_uc_ram(uint16_t addr, int16_t wr_val) {
    return (merlin_mptwo_wrw_uc_ram(addr, wr_val));
}

/* Micro RAM Byte Signed Write */
err_code_t merlin_mptwo_wrbs_uc_ram(uint16_t addr, int8_t wr_val) {
    return (merlin_mptwo_wrb_uc_ram(addr, wr_val));
}

/* Micro RAM Word Signed Read */
int16_t merlin_mptwo_rdws_uc_ram(err_code_t *err_code_p, uint16_t addr) {
    return ((int16_t)merlin_mptwo_rdw_uc_ram(err_code_p, addr));
}

/* Micro RAM Byte Signed Read */
int8_t merlin_mptwo_rdbs_uc_ram(err_code_t *err_code_p, uint16_t addr) {
    return ((int8_t)merlin_mptwo_rdb_uc_ram(err_code_p, addr));
}


/* Micro RAM Byte Read */
err_code_t merlin_mptwo_rdblk_uc_ram(uint8_t *mem, uint16_t addr, uint16_t cnt) {
    if(!mem) {
        return(_error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
    }
    while (cnt--) {
        err_code_t error_code;
        uint8_t    tmp = merlin_mptwo_rdb_uc_ram(&error_code, addr++); /* Value read from the required Data RAM Address */
        if (error_code) {
...
err_code_t merlin_mptwo_configure_fast_link_recovery(uint8_t enable, uint8_t enable_timeout, enum merlin_mptwo_flr_timeout_enum timeout_time, uint8_t enable_2ms_yield, uint8_t filter_count) {
    ...
    if(reset_state < 7) {
        EFUN_PRINTF(("ERROR: merlin_mptwo_configure_fast_link_recovery(..) called without ln_dp_s_rstb=0 Lane=%d reset_state=%d\n",merlin_mptwo_get_lane(),reset_state));
        return (_error(ERR_CODE_LANE_DP_NOT_RESET));
    }
#endif

    if (timeout_time_val > 3) {
        ESTM_PRINTF(("timeout_time value for merlin_mptwo_configure_fast_link_recovery() is too large:  %d\n", timeout_time_val));
        return (_error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
    }
    if (filter_count > 7) {
        ESTM_PRINTF(("filter_count value for merlin_mptwo_configure_fast_link_recovery() is too large:  %d\n", filter_count));
        return (_error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
    }
