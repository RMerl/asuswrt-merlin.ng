...
/** @file merlin16_shasta_dependencies.h
 * Dependencies to be provided by IP User
 */

#ifndef MERLIN16_SHASTA_API_DEPENDENCIES_H
#define MERLIN16_SHASTA_API_DEPENDENCIES_H
 ...
 * If using Serdes MDIO controller to access the registers, implement this function using merlin16_shasta_pmd_mdio_mwr_reg(..)
 *
 * If NOT using a Serdes MDIO controller or the Serdes PMI Masked write feature, please use the following code to
 * implement this function
 *
 *    merlin16_shasta_pmd_wr_reg(addr, ((merlin16_shasta_pmd_rd_reg(addr) & ~mask) | (mask & (val << lsb))));
 *
 * @param sa__ is an opaque state vector passed through to device access functions.
 ...
 * @return Error code generated by write function (returns ERR_CODE_NONE if no errors)
 */
err_code_t merlin16_shasta_pmd_mwr_reg(srds_access_t *sa__, uint16_t addr, uint16_t mask, uint8_t lsb, uint16_t val);
 ...
 */
#ifdef SRDS_API_ALL_FUNCTIONS_HAVE_ACCESS_STRUCT
err_code_t merlin16_shasta_delay_ns(srds_access_t *sa__, uint16_t delay_ns);
#else
 ...
 */
#ifdef SRDS_API_ALL_FUNCTIONS_HAVE_ACCESS_STRUCT
err_code_t merlin16_shasta_delay_us(srds_access_t *sa__, uint32_t delay_us);
#else
 ...
 */
#ifdef SRDS_API_ALL_FUNCTIONS_HAVE_ACCESS_STRUCT
err_code_t merlin16_shasta_delay_ms(srds_access_t *sa__, uint32_t delay_ms);
#else
...
#define merlin16_shasta_get_physical_lane merlin16_shasta_get_lane
...
#define merlin16_shasta_set_physical_lane merlin16_shasta_set_lane



#if defined(SERDES_EXTERNAL_INFO_TABLE_EN)
/** Returns address of merlin16_shasta info table stored in upper level software.
 * @param sa__ is an opaque state vector passed through to device access functions.
 */
srds_info_t *merlin16_shasta_get_info_table_address(srds_access_t *sa__);
#endif
