...
/** @file merlin16_shasta_internal.h
 * Internal API functions
 */

#ifndef MERLIN16_SHASTA_API_INTERNAL_H
#define MERLIN16_SHASTA_API_INTERNAL_H
...
#include "merlin16_shasta_types.h"
...
/*---------------------------------------*/
enum merlin16_shasta_prbs_chk_timer_selection_enum {
    USE_HW_TIMERS = 0,
    USE_SW_TIMERS = 1
};
...
typedef struct {
  ...
  /** PLL Divider value.  (Same encoding as enum #merlin16_shasta_pll_div_enum.) */
  uint32_t pll_div;
  /** PLL Lock */
  uint8_t  pll_lock;
  /** PLL Lock Change */
...
/** Checks to see if the stored ucode_version in merlin16_shasta_info is matching with the current thread ucode
 * @param sa__ is an opaque state vector passed through to device access functions.
...
/** Verify the internal merlin16_shasta_info.
 * @param *merlin16_shasta_info_ptr Info_table to be verified
 * @param sa__ is an opaque state vector passed through to device access functions.
 * @return Error code, if generated (returns ERR_CODE_NONE if no errors)
 */
...
/** Return whether the fraction portion of a #merlin16_shasta_pll_div_enum value is nonzero. */
#define SRDS_INTERNAL_IS_PLL_DIV_FRACTIONAL(div_) (((uint32_t)(div_) & 0xFFFFF000UL) != 0)

/** Extract the integer portion of a #merlin16_shasta_pll_div_enum value. */
#define SRDS_INTERNAL_GET_PLL_DIV_INTEGER(div_) ((uint32_t)(((uint32_t)(div_)) & 0x00000FFFUL))

/** Extract the fraction portion of a #merlin16_shasta_pll_div_enum value.
...
/** Compose a value in the same format as a #merlin16_shasta_pll_div_enum value.
 ...
 * @param sa__ is an opaque state vector passed through to device access functions.
 * @param refclk Reference clock frequency (enumerated), or MERLIN16_SHASTA_PLL_REFCLK_UNKNOWN to auto-compute.
 * @param *refclk_freq_hz Resultant reference clock frequency, in Hz.
 * @param *srds_div Divider value (enumerated), or MERLIN16_SHASTA_PLL_DIV_UNKNOWN to auto-compute.
 *             The final divider value will be filled in if MERLIN16_SHASTA_PLL_DIV_UNKNOWN.
 * @param *vco_freq_khz VCO output frequency, in kHz, or 0 to auto-compute.
 *                      The final VCO output frequency will be filled in if 0.
 * @param pll_option Select PLL configuration option from enum #merlin16_shasta_pll_option_enum.
 * @return Error Code, if generated (returns ERR_CODE_NONE if no errors)
 */
 ...
 * @param sa__ is an opaque state vector passed through to device access functions.
 * @param *srds_div Divider value, in the same encoding as enum #merlin16_shasta_pll_div_enum.
 * @return Error Code, if generated (returns ERR_CODE_NONE if no errors)
 */
 ...
 * as part of merlin16_shasta_display_core_state_line()
 * @param sa__ is an opaque state vector passed through to device access functions.
 * @param srds_div Divider value, in the same encoding as enum #merlin16_shasta_pll_div_enum.
 * @return Error Code, if generated (returns ERR_CODE_NONE if no errors)
 */
 ...
 * @return Error Code, if generated (returns ERR_CODE_NONE if no errors)
 */
err_code_t merlin16_shasta_INTERNAL_get_vco_from_refclk_div(srds_access_t *sa__,
                                                   uint32_t refclk_freq_hz,
                                                   enum merlin16_shasta_pll_div_enum srds_div,
                                                   uint32_t *vco_freq_khz,
                                                   enum merlin16_shasta_pll_option_enum pll_option);
...
} merlin16_shasta_INTERNAL_event_log_dump_state_t;

/** Callback function for dumping event log, intended to be called by merlin16_shasta_INTERNAL_read_event_log_with_callback.
 *  It must also be called once after, with a byte count of 0, to finish printing.
 * @param *arg is a merlin16_shasta_INTERNAL_event_log_dump_state_t pointer for holding state.
...
/*-----------------------------*/
/** Read current merlin16_shasta core status.
 * @param sa__ is an opaque state vector passed through to device access functions.
 * @param *istate Current merlin16_shasta core status read back and populated by the API
 * @return Error Code generated by API (returns ERR_CODE_NONE if no errors)
 */
...
/*-----------------------------*/
/** Read current merlin16_shasta lane status.
 * @param sa__ is an opaque state vector passed through to device access functions.
 * @param *istate Current merlin16_shasta lane status read back and populated by the API
 * @return Error Code generated by API (returns ERR_CODE_NONE if no errors)
 */
 ...
 * @return Error Code generated by API (returns ERR_CODE_NONE if no errors)
 */
err_code_t merlin16_shasta_INTERNAL_get_osr_mode(srds_access_t *sa__, merlin16_shasta_osr_mode_st *mode);
 ...
 * @param sa__ is an opaque state vector passed through to device access functions.
 * @param *pmd_lock Current merlin16_shasta lane pmd_rx_lock status read back and populated by the API
 * @param *pmd_lock_chg Current merlin16_shasta lane pmd_rx_lock_change status read back and populated by the API
 * @return Error Code generated by API (returns ERR_CODE_NONE if no errors)
 */
 ...
 * @param sa__ is an opaque state vector passed through to device access functions.
 * @param *sig_det Current merlin16_shasta lane signal_detect status read back and populated by the API
 * @param *sig_det_chg Current merlin16_shasta lane signal_detect_change status read back and populated by the API
 * @return Error Code generated by API (returns ERR_CODE_NONE if no errors)
 */
 ...
 * @param sa__ is an opaque state vector passed through to device access functions.
 * @param *pll_lock Current merlin16_shasta lane pll_lock status read back and populated by the API
 * @param *pll_lock_chg Current merlin16_shasta lane pll_lock_change status read back and populated by the API
 * @return Error Code generated by API (returns ERR_CODE_NONE if no errors)
 */
 ...
 * Along with merlin16_shasta_core_clkgate(), all lanes should also be clock gated using merlin16_shasta_lane_clkgate() to complete a Core Clockgate
 * @param sa__ is an opaque state vector passed through to device access functions.
 ...
 * Define macro CUSTOM_REG_POLLING to replace the default implementation provided in merlin16_shasta_internal.c.
 *
 * @param sa__ is an opaque state vector passed through to device access functions.
...
typedef struct {
    uint8_t *mem_ptr;
    srds_access_t *sa;
} merlin16_shasta_INTERNAL_rdblk_callback_arg_t;

/** Callback function for reading uc RAM memory by merlin16_shasta_INTERNAL_rdblk_uc_generic_ram().
 * @param *arg is a merlin16_shasta_INTERNAL_rdblk_callback_arg_t pointer for holding state.
 ...
 *         indefinitely, until cnt bytes total are read.
 *  In other words, this is the same as merlin16_shasta_INTERNAL_rdblk_uc_generic_ram(), but reading in descending address order.
 * @param sa__ is an opaque state vector passed through to device access functions.
 * @param block_addr Address of RAM block to be read.
 *                   For program RAM, the upper 16 bits are 0x0000.  Otherwise, the upper 16 bits are 0x2000.
 * @param block_size Length of RAM block to read.
