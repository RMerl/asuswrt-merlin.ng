...
/** @file merlin16_shasta_types.h
 * Common types used by Serdes API functions
 */

#ifndef MERLIN16_SHASTA_API_TYPES_H
#define MERLIN16_SHASTA_API_TYPES_H

#include "merlin16_shasta_ipconfig.h"
#include "merlin16_shasta_usr_includes.h"

typedef struct merlin16_shasta_triage_info_st {
   uint32_t     api_ver;
   uint32_t     ucode_ver;
   uint8_t      core;
   uint8_t      lane;
...
struct merlin16_shasta_uc_lane_config_field_st {
...
struct merlin16_shasta_uc_core_config_field_st {
...
/** Lane Config Struct */
struct  merlin16_shasta_uc_lane_config_st {
  struct merlin16_shasta_uc_lane_config_field_st field;
  uint16_t word;
};

/** Core Config Struct */
struct  merlin16_shasta_uc_core_config_st {
  struct merlin16_shasta_uc_core_config_field_st field;
  uint16_t word;
...
struct merlin16_shasta_usr_ctrl_func_st {
...
struct merlin16_shasta_usr_ctrl_dfe_func_st {
...
struct merlin16_shasta_usr_ctrl_disable_functions_st {
  struct merlin16_shasta_usr_ctrl_func_st field;
  uint8_t  byte;
};

/** Lane User Control Disable DFE Function Struct */
struct merlin16_shasta_usr_ctrl_disable_dfe_functions_st {
  struct merlin16_shasta_usr_ctrl_dfe_func_st field;
  uint8_t  byte;
};
struct merlin16_shasta_ber_data_st {
    uint64_t num_errs;
    uint64_t num_bits;
    uint8_t lcklost;
    uint8_t prbs_chk_en;
...
struct merlin16_shasta_check_platform_info_st {
    char c;
    uint32_t d;
};
