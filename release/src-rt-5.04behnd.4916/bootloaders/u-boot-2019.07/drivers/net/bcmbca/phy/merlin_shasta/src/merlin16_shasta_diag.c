...
/***********************************************************************************
 ***********************************************************************************
 *  File Name     :  merlin16_shasta_diag.c                                             *
...
err_code_t merlin16_shasta_test_uc_timestamp(srds_access_t *sa__) {
    ...
    EFUN(merlin16_shasta_pmd_uc_cmd(sa__, CMD_UC_DBG,CMD_UC_DBG_TIMESTAMP,100));
...
err_code_t merlin16_shasta_display_eye_scan(srds_access_t *sa__) {
    ...
    ESTM(core=merlin16_shasta_get_core(sa__));
    ESTM(lane=merlin16_shasta_get_lane(sa__));
    EFUN_PRINTF(("\n\n****  SERDES EYE SCAN CORE %d LANE %d   ****\n",core,lane));
    EFUN(merlin16_shasta_display_eye_scan_header(1));

    /* start horizontal acquisition */
    {   err_code_t err_code = merlin16_shasta_meas_eye_scan_start(sa__, EYE_SCAN_HORIZ);
        if (err_code)
        {
            ...
            {
                EFUN((merlin16_shasta_meas_eye_scan_done(sa__), err_code));
            }
        }
    }
    ...
    {
        {   err_code_t err_code = merlin16_shasta_read_eye_scan_stripe(sa__, &stripe[0], &status);
            if (err_code) {
                EFUN((merlin16_shasta_meas_eye_scan_done(sa__), err_code));
            }
        }
        EFUN(merlin16_shasta_display_eye_scan_stripe(sa__, y,&stripe[0]));
        EFUN_PRINTF(("\n"));
    }
    /* stop acquisition */
    EFUN(merlin16_shasta_meas_eye_scan_done(sa__));
    EFUN(merlin16_shasta_display_eye_scan_footer(1));

    return(ERR_CODE_NONE);
}

err_code_t merlin16_shasta_meas_eye_scan_start(srds_access_t *sa__, uint8_t direction) {
    uint8_t lock;
    ...
    if(direction == EYE_SCAN_VERTICAL) {
        EFUN(merlin16_shasta_pmd_uc_diag_cmd(sa__, CMD_UC_DIAG_START_VSCAN_EYE,GRACEFUL_STOP_TIME));
    } else if(direction == EYE_SCAN_HORIZ) {
        EFUN(merlin16_shasta_pmd_uc_diag_cmd(sa__, CMD_UC_DIAG_START_HSCAN_EYE,GRACEFUL_STOP_TIME));
    } else if(direction == EYE_SCAN_SLICE) {
        EFUN(merlin16_shasta_pmd_uc_diag_cmd(sa__, CMD_UC_DIAG_START_EYE_SLICE,GRACEFUL_STOP_TIME));
    }
    return(ERR_CODE_NONE);
}

err_code_t merlin16_shasta_INTERNAL_poll_diag_data(srds_access_t *sa__, srds_info_t *merlin16_shasta_info_ptr, uint16_t *status, uint8_t *diag_rd_ptr, uint8_t byte_count, uint32_t timeout_ms) {
    uint32_t lane_diag_size;
    uint16_t loop;

    CHECK_AND_RETURN_IF_NULL(merlin16_shasta_info_ptr, ERR_CODE_INVALID_INFO_TABLE_ADDR);
    lane_diag_size = merlin16_shasta_info_ptr->diag_mem_ram_size;
    *diag_rd_ptr = 0;
    ...
    if (byte_count > (lane_diag_size / 2)) {
        ESTM_PRINTF(("\nERROR : merlin16_shasta_INTERNAL_poll_diag_data() has excessive byte count of %d.\n", byte_count));
...
typedef struct {
    uint32_t *buffer_ptr;
    } merlin16_shasta_read_eye_scan_stripe_callback_arg_t;

static err_code_t merlin16_shasta_read_eye_scan_stripe_callback(void *arg, uint8_t byte_count, uint16_t data) {
    merlin16_shasta_read_eye_scan_stripe_callback_arg_t * const cast_arg = (merlin16_shasta_read_eye_scan_stripe_callback_arg_t *)arg;
    *(cast_arg->buffer_ptr++) = merlin16_shasta_INTERNAL_float8_to_int32((float8_t)(data & 0xFF));
    if (byte_count > 1) {
        *(cast_arg->buffer_ptr++) = merlin16_shasta_INTERNAL_float8_to_int32((float8_t)(data >> 8));
    }
    return (ERR_CODE_NONE);
}

err_code_t merlin16_shasta_read_eye_scan_stripe(srds_access_t *sa__, uint32_t *buffer, uint16_t *status) {
    srds_info_t * merlin16_shasta_info_ptr = merlin16_shasta_INTERNAL_get_merlin16_shasta_info_ptr_with_check(sa__);
    const uint8_t lane = merlin16_shasta_get_lane(sa__);
    const uint8_t stripe_size = 64;

    merlin16_shasta_read_eye_scan_stripe_callback_arg_t arg;
    ...
    EFUN(merlin16_shasta_INTERNAL_verify_merlin16_shasta_info(merlin16_shasta_info_ptr, sa__));
        ...
        lane_diag_base = merlin16_shasta_info_ptr->diag_mem_ram_base + ((lane%merlin16_shasta_info_ptr->lane_count) * merlin16_shasta_info_ptr->diag_mem_ram_size) +
            (uint32_t)(merlin16_shasta_info_ptr->grp_ram_size*merlin16_shasta_INTERNAL_grp_idx_from_lane(merlin16_shasta_get_physical_lane(sa__)));

    EFUN(merlin16_shasta_INTERNAL_poll_diag_data(sa__, merlin16_shasta_info_ptr, status, &diag_rd_ptr, stripe_size, 400));

    arg.buffer_ptr = buffer;
    EFUN(merlin16_shasta_INTERNAL_rdblk_uc_generic_ram(sa__,
                                              lane_diag_base,
                                              (uint16_t)merlin16_shasta_info_ptr->diag_mem_ram_size,
                                              diag_rd_ptr,
                                              stripe_size,
                                              &arg,
                                              merlin16_shasta_read_eye_scan_stripe_callback));

    diag_rd_ptr = (uint8_t)(((uint32_t)diag_rd_ptr + stripe_size) % merlin16_shasta_info_ptr->diag_mem_ram_size);
    EFUN(wrv_usr_diag_rd_ptr(diag_rd_ptr));
    ESTM(*status = rdv_usr_diag_status() & 0xFF);
    return(ERR_CODE_NONE);
}

err_code_t merlin16_shasta_display_eye_scan_stripe(srds_access_t *sa__, int8_t y,uint32_t *buffer) {
    ...
    level = merlin16_shasta_INTERNAL_ladder_setting_to_mV(sa__, y,data_thresh);
...
err_code_t merlin16_shasta_display_eye_scan_header(int8_t i) {
int8_t x;
    EFUN_PRINTF(("\n"));
...
err_code_t merlin16_shasta_display_eye_scan_footer(int8_t i) {
int8_t x;
    for(x=1;x<=i;x++) {
        EFUN_PRINTF(("         : -|----|----|----|----|----|----|----|----|----|----|----|----|-"));
    }
...
err_code_t merlin16_shasta_read_eye_scan_status(srds_access_t *sa__, uint16_t *status) {
...
err_code_t merlin16_shasta_meas_eye_scan_done(srds_access_t *sa__) {
  EFUN(merlin16_shasta_pmd_uc_diag_cmd(sa__, CMD_UC_DIAG_DISABLE,GRACEFUL_STOP_TIME));
  return(ERR_CODE_NONE);
}


err_code_t merlin16_shasta_start_ber_scan_test(srds_access_t *sa__, uint8_t ber_scan_mode, uint8_t timer_control, uint8_t max_error_control) {
    uint8_t lock,sts;
    ESTM(lock = rd_pmd_rx_lock());
    if(lock == 0) {
    ...
    EFUN(merlin16_shasta_pmd_uc_cmd(sa__, CMD_CAPTURE_BER_START, ber_scan_mode,GRACEFUL_STOP_TIME));
    return(ERR_CODE_NONE);
}

err_code_t merlin16_shasta_read_ber_scan_data(srds_access_t *sa__, uint32_t *errors, uint32_t *timer_values, uint8_t *cnt, uint32_t timeout) {
 ...
 ESTM(sts = rdv_usr_diag_status());
    if((sts & 0x8000) == 0) {
        return(merlin16_shasta_error(sa__, ERR_CODE_DATA_NOTAVAIL));
    }
    *cnt = (uint8_t)((sts & 0x00FF)/3);
    for(i=0;i < *cnt;i++) {
        /* Read 2 bytes of data */
        EFUN(merlin16_shasta_pmd_uc_cmd(sa__, CMD_READ_DIAG_DATA_WORD, 0, timeout));
        ...
        EFUN(merlin16_shasta_pmd_uc_cmd(sa__, CMD_READ_DIAG_DATA_BYTE, 0, timeout));
        ESTM(prbs_byte = (uint8_t)rd_uc_dsc_data());
        errors[i] = merlin16_shasta_INTERNAL_float12_to_uint32(prbs_byte,prbs_multi); /* convert 12bits to uint32 */
        timer_values[i] = (merlin16_shasta_INTERNAL_float12_to_uint32(time_byte,time_multi)<<3);
...
err_code_t merlin16_shasta_eye_margin_proj(srds_access_t *sa__, USR_DOUBLE rate, uint8_t ber_scan_mode, uint8_t timer_control, uint8_t max_error_control) {
    ...
    EFUN(merlin16_shasta_start_ber_scan_test(sa__, ber_scan_mode, timer_control, max_error_control));
    ESTM(offset_start = (int16_t)rd_uc_dsc_data());
    if(DIAG_VERBOSE > 2) EFUN_PRINTF(("offset_start = %d:%dmV\n",offset_start,merlin16_shasta_INTERNAL_ladder_setting_to_mV(sa__, (int8_t)offset_start,0)));
    ...
    EFUN(merlin16_shasta_INTERNAL_poll_diag_done(sa__, &sts,(uint32_t)(timer_control*2000)));

    if(DIAG_VERBOSE > 2) EFUN_PRINTF(("delay done\n"));

    EFUN(merlin16_shasta_read_ber_scan_data(sa__, &errs[0], &time[0], &cnt, 2000));

    if(DIAG_VERBOSE > 2) EFUN_PRINTF(("read done cnt=%d\n",cnt));

    EFUN(merlin16_shasta_pmd_uc_cmd(sa__, CMD_CAPTURE_BER_END,0x00,200));
 ...
 /* }                                                                  */
    ...
    EFUN(merlin16_shasta_display_ber_scan_data(sa__, rate, ber_scan_mode, &errs[0], &time[0],(uint8_t)SRDS_ABS(offset_start)));
...
err_code_t merlin16_shasta_display_ber_scan_data (srds_access_t *sa__, USR_DOUBLE rate, uint8_t ber_scan_mode, uint32_t *total_errs, uint32_t *total_time, uint8_t max_offset) {
    ...
    do {
        if(heye == 1) {
            ...
            if(intrusive) {
                ...
                margins[i] = direction*merlin16_shasta_INTERNAL_ladder_setting_to_mV(sa__, (int16_t)offset[i], range250);
            }
#else
            margins[i] = info_out->margins[i];
#endif
...
err_code_t merlin16_shasta_display_lane_state_hdr(void) {
...
err_code_t merlin16_shasta_display_lane_state_legend(void) {
  EFUN_PRINTF(("\n"));
  EFUN_PRINTF(("**********************************************************************************************\n"));
...
err_code_t merlin16_shasta_display_lane_state(srds_access_t *sa__) {
  err_code_t err_code = merlin16_shasta_INTERNAL_display_lane_state_no_newline(sa__);
  EFUN_PRINTF(("\n"));
  return (err_code);
}

err_code_t merlin16_shasta_display_core_state_hdr(void) {
    char core_type[20] = "merlin16_shasta";
...
err_code_t merlin16_shasta_display_core_state_line(srds_access_t *sa__) {
  err_code_t err_code = merlin16_shasta_INTERNAL_display_core_state_no_newline(sa__);
  EFUN_PRINTF(("\n"));
  return (err_code);
}

err_code_t merlin16_shasta_display_core_state_legend(void) {
  EFUN_PRINTF(("\n"));
  EFUN_PRINTF(("**************************************************************************************************************\n"));
...
/**********************************************/
err_code_t merlin16_shasta_display_core_config(srds_access_t *sa__) {
    EFUN_PRINTF(("\n\n***********************************\n"  ));
...
err_code_t merlin16_shasta_display_core_state(srds_access_t *sa__) {
    EFUN(merlin16_shasta_display_core_state_hdr());
    EFUN(merlin16_shasta_display_core_state_line(sa__));
    EFUN(merlin16_shasta_display_core_state_legend());
    return ERR_CODE_NONE;
}


err_code_t merlin16_shasta_display_lane_debug_status(srds_access_t *sa__) {
    /* startup */
    struct merlin16_shasta_usr_ctrl_disable_functions_st     ds;
    struct merlin16_shasta_usr_ctrl_disable_dfe_functions_st dsd;
    /* steady state */
    struct merlin16_shasta_usr_ctrl_disable_functions_st     dss;
    struct merlin16_shasta_usr_ctrl_disable_dfe_functions_st dssd;
    uint8_t core,lane;
    ...
    EFUN(merlin16_shasta_get_usr_ctrl_disable_startup(sa__, &ds));
    ...
    EFUN(                                                            merlin16_shasta_get_usr_ctrl_disable_startup_dfe(sa__, &dsd));
    ...
    EFUN(                                                           merlin16_shasta_get_usr_ctrl_disable_steady_state(sa__, &dss));
    ...
    EFUN(                                                      merlin16_shasta_get_usr_ctrl_disable_steady_state_dfe(sa__, &dssd));
...
static err_code_t _display_ber_scan_data(srds_access_t *sa__, uint8_t ber_scan_mode, uint8_t timer_control, uint8_t max_error_control) {
    ...
    if((osr_mode.rx > MERLIN16_SHASTA_OSX4) && (osr_mode.rx != MERLIN16_SHASTA_OSX2P5)) {
#else
    if(osr_mode.rx > MERLIN16_SHASTA_OSX4) {
#endif
        EFUN_PRINTF(("ERROR DIAG display_ber_data: osr mode too high\n"));
        return(ERR_CODE_BAD_PTR_OR_INVALID_INPUT);
    }
    if (osr_mode.rx > MERLIN16_SHASTA_OSX4) { /* BER supported for only MERLIN16_SHASTA_OSX2P5 mode when osr_mode > osx4 */
    ...
    if(ber_scan_mode & DIAG_BER_HORZ) {
        ...
        EFUN_PRINTF(("STARTING OFFSET = %d : %d mV\n",offset_start,merlin16_shasta_INTERNAL_ladder_setting_to_mV(sa__, (int8_t)offset_start, range250)));
    }
    if(DIAG_VERBOSE > 2) EFUN_PRINTF(("start done\n"));
    ...
    for(i=0;i < cnt;i++) {
        /* Read 2 bytes of data */
        EFUN(merlin16_shasta_pmd_uc_cmd(sa__, CMD_READ_DIAG_DATA_WORD, 0, 200));
        ESTM(dataword = rd_uc_dsc_data());           /* LSB contains 2 -4bit nibbles */
        time_byte = (uint8_t)(dataword>>8);          /* MSB is time byte */
        prbs_multi = (uint8_t)dataword & 0x0F;       /* split nibbles */
        time_multi = (uint8_t)dataword>>4;
        /* Read 1 bytes of data */
        EFUN(merlin16_shasta_pmd_uc_cmd(sa__, CMD_READ_DIAG_DATA_BYTE, 0, 200));
        ESTM(prbs_byte = (uint8_t)rd_uc_dsc_data());
        errors = merlin16_shasta_INTERNAL_float12_to_uint32(prbs_byte,prbs_multi); /* convert 12bits to uint32 */
        timer_values = (merlin16_shasta_INTERNAL_float12_to_uint32(time_byte,time_multi)<<3);
        if(DIAG_VERBOSE < 5) {
            if (!(i % 4))  {
                EFUN_PRINTF(("\n"));
            }
            if(ber_scan_mode & DIAG_BER_HORZ) {
                ...
                EFUN_PRINTF(("%d %d %d ",direction*merlin16_shasta_INTERNAL_ladder_setting_to_mV(sa__, (int8_t)(SRDS_ABS(offset_start)-i), range250),errors,timer_values));
            }
    ...
    EFUN_PRINTF(("\n"));
    EFUN(merlin16_shasta_pmd_uc_cmd(sa__, CMD_CAPTURE_BER_END,0x00,2000));
...
err_code_t merlin16_shasta_display_diag_data(srds_access_t *sa__, uint16_t  diag_level) {
    ...
    ESTM_PRINTF(("Core = %d; LANE = %d\n",merlin16_shasta_get_core(sa__),merlin16_shasta_get_lane(sa__)));
    EFUN(merlin16_shasta_version(sa__, &api_version));
    ...
    ESTM(rx_lock = rd_pmd_rx_lock());
    {
        ...
        micro_stop = merlin16_shasta_INTERNAL_stop_micro(sa__,rx_lock,&err_code);
    ...
    EFUN(merlin16_shasta_display_lane_state_hdr());
    EFUN(merlin16_shasta_display_lane_state(sa__));


    if(diag_level & SRDS_DIAG_TIMESTAMP) {
        EFUN(merlin16_shasta_test_uc_timestamp(sa__));
    }
    if(diag_level & SRDS_DIAG_EVENT_SAFE) {
        merlin16_shasta_INTERNAL_event_log_dump_state_t state;
        uint8_t micro_num = 0;
        {
            state.index = 0;
            state.line_start_index = 0;
            EFUN(merlin16_shasta_INTERNAL_read_event_log_with_callback(sa__, micro_num, 1, &state, merlin16_shasta_INTERNAL_event_log_dump_callback));
            EFUN(merlin16_shasta_INTERNAL_event_log_dump_callback(&state, 0, 0));
        }
    }
    if(diag_level & SRDS_DIAG_EVENT) {
        EFUN(merlin16_shasta_read_event_log(sa__));
    }
    /* currently REG_CORE and REG_LANE dump same data. */
    core = merlin16_shasta_get_core(sa__);
    lane = merlin16_shasta_get_lane(sa__);

    if((diag_level & SRDS_DIAG_REG_CORE)
       || (diag_level & SRDS_DIAG_REG_LANE)) {
        EFUN(merlin16_shasta_reg_dump(sa__, core, lane, NULL));
    }
    if(diag_level & SRDS_DIAG_UC_CORE) {
        EFUN(merlin16_shasta_uc_core_var_dump(sa__, core, NULL));

    }
    if(diag_level & SRDS_DIAG_UC_LANE) {

        EFUN(merlin16_shasta_uc_lane_var_dump(sa__, core, lane, NULL));
    }
    if(diag_level & SRDS_DIAG_EYE) {
        EFUN(merlin16_shasta_display_eye_scan(sa__));
    }
    if(diag_level & SRDS_DIAG_LANE_DEBUG) {
        EFUN(merlin16_shasta_display_lane_debug_status(sa__));
    }
    ...
    if(diag_level & SRDS_DIAG_INFO_TABLE) {
        EFUN(merlin16_shasta_INTERNAL_display_info_table(sa__));
    }

    /* re enable micro */
    if (!micro_stop) {
        EFUN(merlin16_shasta_stop_rx_adaptation(sa__, 0));
    }
...
} merlin16_shasta_diag_access_callback_state_t;

static err_code_t merlin16_shasta_diag_access_read_byte_callback(void *arg, uint8_t byte_count, uint16_t data) {
    merlin16_shasta_diag_access_callback_state_t * const state_ptr = (merlin16_shasta_diag_access_callback_state_t *)arg;
    while (byte_count > 0) {
...
static err_code_t merlin16_shasta_diag_access_read_word_callback(void *arg, uint8_t byte_count, uint16_t data) {
    merlin16_shasta_diag_access_callback_state_t * const state_ptr = (merlin16_shasta_diag_access_callback_state_t *)arg;
    UNUSED(byte_count);
...
/* Required Diagnostic Functions */
err_code_t merlin16_shasta_diag_access(srds_access_t *sa__, enum srds_diag_access_enum type, uint16_t addr, uint16_t data, uint16_t param) {
    srds_info_t * merlin16_shasta_info_ptr = merlin16_shasta_INTERNAL_get_merlin16_shasta_info_ptr_with_check(sa__);
    merlin16_shasta_diag_access_callback_state_t state;
    ...
    case SRDS_REG_READ:      {
                             ...
                             if(data > 1) {
                                 ...
                                 for (i = 0; i < data; i++) {
                                     ...
                                     EFUN(merlin16_shasta_acc_rdt_reg(sa__, (uint16_t)(i+addr),&tmp));
                                     ESTM_PRINTF(("%04x ", tmp));
                                 }
                                 EFUN_PRINTF(("\n"));
                             } else {
                                 EFUN(merlin16_shasta_acc_rdt_reg(sa__, addr,&tmp));
    ...
    case SRDS_REG_RMW:       {
                             ...
                             EFUN(merlin16_shasta_acc_wr_reg(sa__, addr,tmp2));
    ...
    case SRDS_CORE_RAM_READ_BYTE: {
                             if(data > 1) {
                                 ...
                                 EFUN(merlin16_shasta_INTERNAL_rdblk_uc_generic_ram(sa__,
                                                                           addr + merlin16_shasta_info_ptr->core_var_ram_base,
                                                                           data,
                                                                           0,
                                                                           data,
                                                                           &state,
                                                                           merlin16_shasta_diag_access_read_byte_callback));
                                 EFUN_PRINTF(("\n"));
                             } else {
                                 ESTM_PRINTF(("Core RAM Read byte: x%04x = x%02x\n",(uint8_t)addr,merlin16_shasta_rdbc_uc_var(sa__, __ERR, (uint8_t)addr)));
                             }
                             } break;
    case SRDS_LANE_RAM_READ_BYTE: {
                             if(data > 1) {
                                 EFUN_PRINTF(("\n****  SERDES BLK LANE RAM READ BYTE   ****"));
                                 EFUN(merlin16_shasta_INTERNAL_rdblk_uc_generic_ram(sa__, (addr + merlin16_shasta_info_ptr->lane_var_ram_base + (merlin16_shasta_get_physical_lane(sa__) * merlin16_shasta_info_ptr->lane_var_ram_size)),
                                                                           data,
                                                                           0,
                                                                           data,
                                                                           &state,
                                 ...
                                 ESTM_PRINTF(("Lane RAM Read byte: x%04x = x%02x\n",addr,merlin16_shasta_rdbl_uc_var(sa__, __ERR,addr)));
                             }
                             } break;
    case SRDS_CORE_RAM_READ_WORD: {
                             if(data > 1) {
                                 ...
                                 EFUN(merlin16_shasta_INTERNAL_rdblk_uc_generic_ram(sa__,
                                                                           ...
                                                                           (uint16_t)(((data+1)>>1)<<1), /* Round up to nearest word count */
                                                                           &state,
                                                                           merlin16_shasta_diag_access_read_word_callback));
                                 EFUN_PRINTF(("\n"));
                             } else {
                                 ESTM_PRINTF(("Core RAM Read word: x%04x = x%04x\n",addr,merlin16_shasta_rdwc_uc_var(sa__, __ERR, (uint8_t)addr)));
                             }
                             } break;
    case SRDS_LANE_RAM_READ_WORD: {
                             if(data > 1) {
                                 ...
                                 ESTM_PRINTF(("Lane RAM Read word: x%04x = x%04x\n",addr,merlin16_shasta_rdwl_uc_var(sa__, __ERR, addr)));
                             }
                             } break;
    case SRDS_CORE_RAM_RMW_BYTE:  {
                             uint8_t tmp, tmp2;
                             ESTM(tmp = merlin16_shasta_rdbc_uc_var(sa__, __ERR, (uint8_t)addr));
                             tmp2 = (uint8_t)((tmp & (uint8_t)~param) | ((uint8_t)param & data));
                             EFUN(merlin16_shasta_wrbc_uc_var(sa__, (uint8_t)addr,tmp2));
                             EFUN_PRINTF(("Core RAM RMW byte: x%04x = x%02x -> x%02x\n",addr,tmp,tmp2));
                             } break;
    case SRDS_LANE_RAM_RMW_BYTE:  {
                             uint8_t tmp, tmp2;
                             ESTM(tmp = merlin16_shasta_rdbl_uc_var(sa__, __ERR,addr));
                             tmp2 = (uint8_t)((tmp & (uint8_t)~param) | ((uint8_t)param & data));
                             EFUN(merlin16_shasta_wrbl_uc_var(sa__, addr,tmp2));
                             EFUN_PRINTF(("Lane RAM RMW byte: x%04x = x%02x -> x%02x\n",addr,tmp,tmp2));
                             } break;
    case SRDS_CORE_RAM_RMW_WORD:  {
                             uint16_t tmp, tmp2;
                             ESTM(tmp = merlin16_shasta_rdwc_uc_var(sa__, __ERR, (uint8_t)addr));
                             tmp2 = (uint16_t)((tmp & ~param) | (param & data));
                             EFUN(merlin16_shasta_wrwc_uc_var(sa__, (uint8_t)addr,tmp2));
                             EFUN_PRINTF(("Core RAM RMW word: x%04x = x%04x -> x%04x\n",addr,tmp,tmp2));
                             } break;
    case SRDS_LANE_RAM_RMW_WORD:  {
                             uint16_t tmp, tmp2;
                             ESTM(tmp = merlin16_shasta_rdwl_uc_var(sa__, __ERR,addr));
                             tmp2 = (uint16_t)((tmp & ~param) | (param & data));
                             EFUN(merlin16_shasta_wrwl_uc_var(sa__, addr,tmp2));
    ...
    case SRDS_GLOB_RAM_READ_BYTE: {
                             if(data > 1) {
                                 ...
                                 ESTM_PRINTF(("Glob RAM Read byte: x%04x = x%02x\n",addr,merlin16_shasta_INTERNAL_rdb_uc_var(sa__, __ERR,addr)));
                             }
                             } break;
    case SRDS_GLOB_RAM_RMW_BYTE:  {
                             uint8_t tmp, tmp2;
                             ESTM(tmp = merlin16_shasta_INTERNAL_rdb_uc_var(sa__, __ERR,addr));
                             tmp2 = (uint8_t)((tmp & (uint8_t)~param) | ((uint8_t)param & data));
                             EFUN(merlin16_shasta_INTERNAL_wrb_uc_var(sa__, addr,tmp2));
    ...
    case SRDS_GLOB_RAM_READ_WORD: {
                             if(data > 1) {
                                 ...
                                 ESTM_PRINTF(("Glob RAM Read word: x%04x = x%04x\n",addr,merlin16_shasta_INTERNAL_rdw_uc_var(sa__, __ERR,addr)));
                             }
                             } break;
    case SRDS_GLOB_RAM_RMW_WORD:  {
                             uint16_t tmp, tmp2;
                             ESTM(tmp = merlin16_shasta_INTERNAL_rdw_uc_var(sa__, __ERR,addr));
                             tmp2 = (uint16_t)((tmp & ~param) | (param & data));
                             EFUN(merlin16_shasta_INTERNAL_wrw_uc_var(sa__, addr,tmp2));
                             EFUN_PRINTF(("Glob RAM RMW word: x%04x = x%04x -> x%04x\n",addr,tmp,tmp2));
                             } break;
    case SRDS_UC_CMD:        {
                             uint16_t tmp;
                             EFUN(merlin16_shasta_pmd_uc_cmd_with_data(sa__, (enum srds_pmd_uc_cmd_enum)addr, (uint8_t)param, data, GRACEFUL_STOP_TIME));
    ...
    default: EFUN_PRINTF(("Invalid request type merlin16_shasta_diag_access\n"));
    }

    return(ERR_CODE_NONE);
}

err_code_t merlin16_shasta_display_state (srds_access_t *sa__) {
    EFUN(merlin16_shasta_display_core_state(sa__));
    EFUN(merlin16_shasta_display_lane_state_hdr());
    EFUN(merlin16_shasta_display_lane_state(sa__));
    EFUN(merlin16_shasta_display_lane_state_legend());
    return(ERR_CODE_NONE);
}

err_code_t merlin16_shasta_display_config (srds_access_t *sa__) {
    EFUN(merlin16_shasta_display_core_config(sa__));
    EFUN(merlin16_shasta_display_lane_config(sa__));
    return(ERR_CODE_NONE);
}
...
err_code_t merlin16_shasta_force_die_temperature (srds_access_t *sa__, int16_t die_temp) {
    /* disable force */
    if(die_temp == -255) {
...
err_code_t merlin16_shasta_display_cl72_status(srds_access_t *sa__) {
  EFUN_PRINTF(("\n\n************************\n"                                                                                                                              ));
