...
/***********************************************************************************
 ***********************************************************************************
 *  File Name     :  merlin16_shasta_internal.c                                         *
...
/** @file merlin16_shasta_internal.c
...
#include "merlin16_shasta_prbs.h"
#include "merlin16_shasta_debug_functions.h"
...
uint32_t merlin16_shasta_INTERNAL_mult_with_overflow_check(uint32_t a, uint32_t b, uint8_t *of) {
    uint16_t c,d;
    uint32_t r,s;
    if (a > b) return merlin16_shasta_INTERNAL_mult_with_overflow_check(b, a, of);
    *of = 0;
    c = (uint16_t)(b >> 16);
    d = UINT16_MAX & b;
    r = a * c;
...
static srds_info_t merlin16_shasta_info[NUM_SERDES_INFO_TABLES];
#endif


srds_info_t *merlin16_shasta_INTERNAL_get_merlin16_shasta_info_ptr(srds_access_t *sa__) {    
#if defined(SERDES_EXTERNAL_INFO_TABLE_EN)
    return merlin16_shasta_get_info_table_address(sa__);
#else
    uint8_t index = 0;
    index = 0;
    return &merlin16_shasta_info[index];
#endif
}

srds_info_t *merlin16_shasta_INTERNAL_get_merlin16_shasta_info_ptr_with_check(srds_access_t *sa__) {
    ...
    CHECK_AND_RETURN_IF_NULL(merlin16_shasta_info_ptr, 0);
    if (merlin16_shasta_info_ptr->signature != SRDS_INFO_SIGNATURE) {
        err_code = merlin16_shasta_init_merlin16_shasta_info(sa__);
        if (err_code != ERR_CODE_NONE){
            EFUN_PRINTF(("ERROR:  %s Failed: 0x%04X \n", API_FUNCTION_NAME, err_code));
            return 0;
        }
    } 
    return merlin16_shasta_info_ptr;
}

err_code_t merlin16_shasta_INTERNAL_match_ucode_from_info(srds_access_t *sa__) {
    uint16_t ucode_version_major;
    uint8_t ucode_version_minor;
    uint32_t ucode_version;

    srds_info_t * merlin16_shasta_info_ptr = merlin16_shasta_INTERNAL_get_merlin16_shasta_info_ptr(sa__);
    CHECK_AND_RETURN_IF_NULL(merlin16_shasta_info_ptr, ERR_CODE_INVALID_INFO_TABLE_ADDR);
    ...
    if (ucode_version == merlin16_shasta_info_ptr->ucode_version) {
        return(ERR_CODE_NONE);
    } else {
        EFUN_PRINTF(("ERROR:  ucode version of the current thread not matching with stored merlin16_shasta_info->ucode_version, Expected 0x%08X, but received 0x%08X.\n",
                    merlin16_shasta_info_ptr->ucode_version, ucode_version));
        return(ERR_CODE_UCODE_VERIFY_FAIL);
    }
}

err_code_t merlin16_shasta_INTERNAL_verify_merlin16_shasta_info(srds_info_t *merlin16_shasta_info_ptr, srds_access_t *sa__)
{
    err_code_t err_code = ERR_CODE_NONE;
    ...
    if (merlin16_shasta_info_ptr->signature != SRDS_INFO_SIGNATURE) {
        EFUN_PRINTF(("ERROR:  Mismatch in merlin16_shasta_info signature.  Expected 0x%08X, but received 0x%08X.\n",
                     SRDS_INFO_SIGNATURE, merlin16_shasta_info_ptr->signature));
        return (merlin16_shasta_error(sa__, ERR_CODE_INFO_TABLE_ERROR));
    }
...
err_code_t merlin16_shasta_INTERNAL_get_afe_hw_version(srds_access_t *sa__, uint8_t *afe_hw_version) {
...
int merlin16_shasta_osr_mode_enum_to_int_x1000(uint8_t osr_mode) {
    switch(osr_mode) {
    case MERLIN16_SHASTA_OSX1:      return 1000;
    case MERLIN16_SHASTA_OSX2:      return 2000;
    case MERLIN16_SHASTA_OSX4:      return 4000;
    case MERLIN16_SHASTA_OSX3:      return 3000;
    case MERLIN16_SHASTA_OSX3P3:    return 3300;
    case MERLIN16_SHASTA_OSX5:      return 5000;
    case MERLIN16_SHASTA_OSX7P5:    return 7500;
    case MERLIN16_SHASTA_OSX8P25:   return 8250;
    case MERLIN16_SHASTA_OSX10:     return 10000;
    case MERLIN16_SHASTA_OSX8:      return 8000;
#ifdef wr_ams_rx_rxclk_div2p5
    case MERLIN16_SHASTA_OSX2P5:    return 2500;
#endif
    default:                    return 1000;
    }
}

err_code_t merlin16_shasta_INTERNAL_get_num_bits_per_ms(srds_access_t *sa__, uint32_t *num_bits_per_ms) {
    uint8_t osr_mode = 0;
    ...
    { 
       merlin16_shasta_osr_mode_st osr_mode_st;
       ENULL_MEMSET(&osr_mode_st, 0, sizeof(merlin16_shasta_osr_mode_st));
       EFUN(merlin16_shasta_INTERNAL_get_osr_mode(sa__, &osr_mode_st));
    ...
    ESTM(pam4_mode = rd_rx_pam4_mode());
        ...
        *num_bits_per_ms = (uint32_t)((((uint32_t)core_config.vco_rate_in_Mhz * 100000UL) / (uint32_t)(merlin16_shasta_osr_mode_enum_to_int_x1000(osr_mode))*10));
...
err_code_t merlin16_shasta_INTERNAL_display_BER(srds_access_t *sa__, uint16_t time_ms) {
    char string[32];
      EFUN(merlin16_shasta_INTERNAL_get_BER_string(sa__,time_ms,string));
      USR_PRINTF(("%s",string));
    return(ERR_CODE_NONE);
}

err_code_t merlin16_shasta_INTERNAL_get_BER_string(srds_access_t *sa__, uint16_t time_ms, char *string) {
    char string2[3];
    struct merlin16_shasta_ber_data_st ber_data_local = {0, 0, 0, 0};

    if(string == NULL) {
        return(ERR_CODE_BAD_PTR_OR_INVALID_INPUT);
    }

    EFUN(merlin16_shasta_INTERNAL_get_BER_data(sa__, time_ms, &ber_data_local, USE_HW_TIMERS));
...
err_code_t merlin16_shasta_INTERNAL_get_BER_data(srds_access_t *sa__, uint16_t time_ms, struct merlin16_shasta_ber_data_st *ber_data, enum merlin16_shasta_prbs_chk_timer_selection_enum timer_sel) {
    uint8_t lcklost = 0;
    uint32_t err_cnt= 0;
    ...
    if((time_ms <= PRBS_MAX_HW_TIMER_TIMEOUT) && (timer_sel == USE_HW_TIMERS)) {
        ...
        struct merlin16_shasta_prbs_chk_hw_timer_ctrl_st prbs_chk_hw_timer_ctrl_bak;
        USR_MEMSET(&prbs_chk_hw_timer_ctrl_bak, 0, sizeof(struct merlin16_shasta_prbs_chk_hw_timer_ctrl_st));
        ...
        EFUN(merlin16_shasta_get_prbs_chk_hw_timer_ctrl(sa__, &prbs_chk_hw_timer_ctrl_bak));
        
        /* STEP 4: Configure hardware timer to count prbs errors */
        EFUN(merlin16_shasta_config_prbs_chk_hw_timer(sa__, time_ms, &time_ms_adjusted));
        
        /* STEP 5: Clear prbs error counters and lock lost status */
        EFUN(merlin16_shasta_prbs_err_count_state(sa__, &err_cnt,&lcklost)); /* clear error counters */
        
        /* STEP 6: toggle checker enable to start hardware timers and error counters */
        EFUN(merlin16_shasta_prbs_chk_en_toggle(sa__));
        ...
        if(!prbs_chk_lock) {
            ...
            EFUN(merlin16_shasta_prbs_err_count_state(sa__, &err_cnt,&lcklost));
            
            /* STEP 11: Update the BER data structure */
            EFUN(merlin16_shasta_INTERNAL_get_num_bits_per_ms(sa__,&num_bits_per_ms));
        ...
        EFUN(merlin16_shasta_set_prbs_chk_hw_timer_ctrl(sa__, &prbs_chk_hw_timer_ctrl_bak));
...
#if defined(MERLIN16_SHASTA_PRBS_CHK_HARDWARE_TIMERS)
err_code_t merlin16_shasta_INTERNAL_get_prbs_timeout_count_from_time(uint16_t time_ms, uint16_t * time_ms_adjusted, struct merlin16_shasta_prbs_chk_hw_timer_ctrl_st * const prbs_chk_hw_timer_ctrl ) {
...
err_code_t merlin16_shasta_INTERNAL_get_p1_threshold(srds_access_t *sa__, int8_t *val) {
 ESTM(*val = (int8_t)(-rd_p1_eyediag_bin())); 
 return (ERR_CODE_NONE);
}


int16_t merlin16_shasta_INTERNAL_ladder_setting_to_mV(srds_access_t *sa__, int8_t ctrl, uint8_t range_250) {
...
err_code_t merlin16_shasta_INTERNAL_compute_bin(srds_access_t *sa__, char var, char bin[]) {
    if(!bin) {
        return(merlin16_shasta_error(sa__, ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
    }
  ...
  switch (var) {
    ...
    default :  ENULL_STRCPY(bin,"");
               ...
               return (merlin16_shasta_error(sa__, ERR_CODE_CFG_PATT_INVALID_HEX));
  }
  return (ERR_CODE_NONE);
}


err_code_t merlin16_shasta_INTERNAL_compute_hex(srds_access_t *sa__, char bin[],uint8_t *hex) {
  if(!hex) {
    return(merlin16_shasta_error(sa__, ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
  }
  ...
  if (!USR_STRCMP(bin,"0000")) {
    ...
    *hex = 0x0;
    return (merlin16_shasta_error(sa__, ERR_CODE_CFG_PATT_INVALID_BIN2HEX));
  } 
  return (ERR_CODE_NONE);
}

uint8_t merlin16_shasta_INTERNAL_stop_micro(srds_access_t *sa__, uint8_t graceful, err_code_t *err_code_p) {
   ...
   if (!(stop_state & 0x7F)) {
       if (graceful) {
           EPFUN2((merlin16_shasta_stop_rx_adaptation(sa__, 1)),0xFF);
       }
       else {
           EPFUN2((merlin16_shasta_pmd_uc_control(sa__, CMD_UC_CTRL_STOP_IMMEDIATE,GRACEFUL_STOP_TIME)),0xFF);
       }
   }
...
uint8_t merlin16_shasta_INTERNAL_rdb_uc_var(srds_access_t *sa__, err_code_t *err_code_p, uint16_t addr) {
    uint8_t rddata;

    if(!err_code_p) {
        return(0);
    }
    EPSTM(rddata = merlin16_shasta_rdb_uc_ram(sa__, err_code_p, addr)); /* Use Micro register interface for reading RAM */
    return (rddata);
}

/* Micro Global RAM Word Read */
uint16_t merlin16_shasta_INTERNAL_rdw_uc_var(srds_access_t *sa__, err_code_t *err_code_p, uint16_t addr) {
  uint16_t rddata;
  ...
  EPSTM(rddata = merlin16_shasta_rdw_uc_ram(sa__, err_code_p, (addr))); /* Use Micro register interface for reading RAM */
  return (rddata);
}

/* Micro Global RAM Byte Write  */
err_code_t merlin16_shasta_INTERNAL_wrb_uc_var(srds_access_t *sa__, uint16_t addr, uint8_t wr_val) {

    return (merlin16_shasta_wrb_uc_ram(sa__, addr, wr_val));          /* Use Micro register interface for writing RAM */
}

/* Micro Global RAM Word Write  */
err_code_t merlin16_shasta_INTERNAL_wrw_uc_var(srds_access_t *sa__, uint16_t addr, uint16_t wr_val) {
    if (addr%2 != 0) {                                       /* Validate even address */
      USR_PRINTF(("Error incorrect addr x%04x\n",addr));
        return (merlin16_shasta_error(sa__, ERR_CODE_INVALID_RAM_ADDR));
    }
    return (merlin16_shasta_wrw_uc_ram(sa__, addr, wr_val));          /* Use Micro register interface for writing RAM */
}
...
/*  Event Log Display  */
/***********************/
err_code_t merlin16_shasta_INTERNAL_event_log_dump_callback(void *arg, uint8_t byte_count, uint16_t data) {
    merlin16_shasta_INTERNAL_event_log_dump_state_t * const state_ptr = (merlin16_shasta_INTERNAL_event_log_dump_state_t *)arg;
...
err_code_t merlin16_shasta_INTERNAL_read_event_log_with_callback(srds_access_t *sa__,
                                                        uint8_t micro_num,
    ...
    srds_info_t * merlin16_shasta_info_ptr = merlin16_shasta_INTERNAL_get_merlin16_shasta_info_ptr_with_check(sa__);
    CHECK_AND_RETURN_IF_NULL(merlin16_shasta_info_ptr, ERR_CODE_INVALID_INFO_TABLE_ADDR);

    EFUN(merlin16_shasta_INTERNAL_verify_merlin16_shasta_info(merlin16_shasta_info_ptr, sa__));

    if (micro_num >= merlin16_shasta_info_ptr->micro_count) {
        return (merlin16_shasta_error(sa__, ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
    }
    /* Read Current lane so that it can be restored at the end of function */
    current_lane = merlin16_shasta_get_lane(sa__);
    EFUN_PRINTF(("\n\n********************************************\n"));
    ...
    if (bypass_micro) {
        ...
        if (merlin16_shasta_info_ptr->trace_memory_descending_writes) {
            ++rd_idx;
            if (rd_idx >= merlin16_shasta_info_ptr->trace_mem_ram_size) {
                rd_idx = 0;
            }
        } else {
            if (rd_idx == 0) {
                rd_idx = (uint16_t)merlin16_shasta_info_ptr->trace_mem_ram_size;
            }
            --rd_idx;
        }
    } else {
        /* Start Read to stop uC logging and read the word at last event written by uC */
        EFUN(merlin16_shasta_pmd_uc_cmd(sa__, CMD_EVENT_LOG_READ, CMD_EVENT_LOG_READ_START, GRACEFUL_STOP_TIME));
        ESTM(rd_idx = rdcv_trace_mem_rd_idx());
    }
    
    EFUN_PRINTF(( "\n  DEBUG INFO: trace memory read index = 0x%04x\n", rd_idx));
    
    EFUN_PRINTF(("  DEBUG INFO: trace memory size = 0x%04x\n\n", merlin16_shasta_info_ptr->trace_mem_ram_size));

    if (merlin16_shasta_info_ptr->trace_memory_descending_writes) {
        /* Micro writes trace memory using descending addresses.
         * So read using ascending addresses using block read
         */
        EFUN(merlin16_shasta_INTERNAL_rdblk_uc_generic_ram(sa__,
                                                  merlin16_shasta_info_ptr->trace_mem_ram_base + (uint32_t)(merlin16_shasta_info_ptr->grp_ram_size*micro_num),
                                                  (uint16_t)merlin16_shasta_info_ptr->trace_mem_ram_size,
                                                  rd_idx,
                                                  (uint16_t)merlin16_shasta_info_ptr->trace_mem_ram_size,
                                                  arg,
                                                  callback));
         ...
         * So read using ascending addresses using block read
         */
        EFUN(merlin16_shasta_INTERNAL_rdblk_uc_generic_ram_descending(sa__,
                                                             merlin16_shasta_info_ptr->trace_mem_ram_base + (uint32_t)(merlin16_shasta_info_ptr->grp_ram_size*micro_num),
                                                             (uint16_t)merlin16_shasta_info_ptr->trace_mem_ram_size,
                                                             rd_idx,
                                                             (uint16_t)merlin16_shasta_info_ptr->trace_mem_ram_size,
    ...
    if (!bypass_micro) {
        ...
        EFUN(merlin16_shasta_pmd_uc_cmd(sa__, CMD_EVENT_LOG_READ, CMD_EVENT_LOG_READ_DONE, 50));
    }
    EFUN(merlin16_shasta_set_lane(sa__,current_lane));
    return(ERR_CODE_NONE);
}

#ifdef TO_FLOATS
/* convert uint32_t to float8 */
float8_t merlin16_shasta_INTERNAL_int32_to_float8(uint32_t input) {
    int8_t cnt;
    uint8_t output;
...
uint32_t merlin16_shasta_INTERNAL_float8_to_int32(float8_t input) {
    uint32_t x;
    if(input == 0) return(0);
...
uint32_t merlin16_shasta_INTERNAL_float12_to_uint32(uint8_t input, uint8_t multi) {
...
uint8_t merlin16_shasta_INTERNAL_uint32_to_float12(uint32_t input, uint8_t *multi) {
    int8_t cnt;
    uint8_t output;
    if(!multi) {
      return(merlin16_shasta_error(sa__, ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
...
err_code_t merlin16_shasta_INTERNAL_set_rx_pf_main(srds_access_t *sa__, uint8_t val) {
    if (val > 15) 
    {
      return (merlin16_shasta_error(sa__, ERR_CODE_PF_INVALID));
    }
    EFUN(WR_RX_PF_CTRL(val));
    return(ERR_CODE_NONE); 
}

err_code_t merlin16_shasta_INTERNAL_get_rx_pf_main(srds_access_t *sa__, uint8_t *val) {
    ESTM(*val = RD_RX_PF_CTRL());
    return (ERR_CODE_NONE);
}

err_code_t merlin16_shasta_INTERNAL_set_rx_pf2(srds_access_t *sa__, uint8_t val) {
    if (val > 7) {
      return (merlin16_shasta_error(sa__, ERR_CODE_PF_INVALID));
    }
...
err_code_t merlin16_shasta_INTERNAL_get_rx_pf2(srds_access_t *sa__, uint8_t *val) {
    ESTM(*val = RD_RX_PF2_CTRL());
    return (ERR_CODE_NONE);
}


err_code_t merlin16_shasta_INTERNAL_set_rx_vga(srds_access_t *sa__, uint8_t val) {

    EFUN(merlin16_shasta_INTERNAL_check_uc_lane_stopped(sa__));  /* make sure uC is stopped to avoid race conditions */

    if (val > RX_VGA_CTRL_VAL_MAX) {
      return (merlin16_shasta_error(sa__, ERR_CODE_VGA_INVALID));
    }
...
err_code_t merlin16_shasta_INTERNAL_get_rx_vga(srds_access_t *sa__, uint8_t *val) {
    ESTM(*val = rd_vga_bin());
    return (ERR_CODE_NONE);
}


err_code_t merlin16_shasta_INTERNAL_set_rx_dfe1(srds_access_t *sa__, int8_t val) {
    EFUN(merlin16_shasta_INTERNAL_check_uc_lane_stopped(sa__));  /* make sure uC is stopped to avoid race conditions */

    {
        ...
        if (val > 63) {
            return (merlin16_shasta_error(sa__, ERR_CODE_DFE1_INVALID));  
        }
...
err_code_t merlin16_shasta_INTERNAL_get_rx_dfe1(srds_access_t *sa__, int8_t *val) {
    ESTM(*val = (int8_t)(rd_dfe_1_e() + rd_dfe_1_cmn()));
    return (ERR_CODE_NONE);
}


err_code_t merlin16_shasta_INTERNAL_set_rx_dfe2(srds_access_t *sa__, int8_t val) {
    int8_t tap_eo;

    if ((val > 31) || (val < -31)) {
      return (merlin16_shasta_error(sa__, ERR_CODE_DFE2_INVALID));  
    }
...
err_code_t merlin16_shasta_INTERNAL_get_rx_dfe2(srds_access_t *sa__, int8_t *val) {
    ESTM(*val = (int8_t)(rd_dfe_2_se() ? -(rd_dfe_2_e()+rd_dfe_2_cmn()) : (rd_dfe_2_e()+rd_dfe_2_cmn())));
    return (ERR_CODE_NONE);
}

err_code_t merlin16_shasta_INTERNAL_set_rx_dfe3(srds_access_t *sa__, int8_t val) {
    if ((val > 31) || (val < -31)) {
      return (merlin16_shasta_error(sa__, ERR_CODE_DFE3_INVALID));  
    }
...
err_code_t merlin16_shasta_INTERNAL_get_rx_dfe3(srds_access_t *sa__, int8_t *val) {
    ESTM(*val = rd_dfe_3_cmn());
    return (ERR_CODE_NONE);
}

err_code_t merlin16_shasta_INTERNAL_set_rx_dfe4(srds_access_t *sa__, int8_t val) {
    if ((val > 15) || (val < -15)) {
      return (merlin16_shasta_error(sa__, ERR_CODE_DFE4_INVALID));  
    }
...
err_code_t merlin16_shasta_INTERNAL_get_rx_dfe4(srds_access_t *sa__, int8_t *val) {
    ESTM(*val = rd_dfe_4_cmn());
    return (ERR_CODE_NONE);
}

err_code_t merlin16_shasta_INTERNAL_set_rx_dfe5(srds_access_t *sa__, int8_t val) {
    if ((val > 15) || (val < -15)) {
      return (merlin16_shasta_error(sa__, ERR_CODE_DFE5_INVALID));  
    }
...
err_code_t merlin16_shasta_INTERNAL_get_rx_dfe5(srds_access_t *sa__, int8_t *val) {
...
err_code_t merlin16_shasta_INTERNAL_get_tx_pre(srds_access_t *sa__, int8_t *val) {
  ESTM(*val = (int8_t)rd_cl72_txfir_pre());
  return (ERR_CODE_NONE);
}


err_code_t merlin16_shasta_INTERNAL_get_tx_main(srds_access_t *sa__, int8_t *val) {
  ESTM(*val = (int8_t)rd_cl72_txfir_main());
  return (ERR_CODE_NONE);
}


err_code_t merlin16_shasta_INTERNAL_get_tx_post1(srds_access_t *sa__, int8_t *val) {
  ESTM(*val = (int8_t)rd_cl72_txfir_post());
  return (ERR_CODE_NONE);
}


err_code_t merlin16_shasta_INTERNAL_get_tx_post2(srds_access_t *sa__, int8_t *val) {
...
err_code_t merlin16_shasta_INTERNAL_core_clkgate(srds_access_t *sa__, uint8_t enable) {
  UNUSED(sa__);
...
err_code_t merlin16_shasta_INTERNAL_lane_clkgate(srds_access_t *sa__, uint8_t enable) {
...
/*-----------------------------------------------*/
err_code_t merlin16_shasta_INTERNAL_get_eye_margin_est(srds_access_t *sa__, uint16_t *left_eye_mUI, uint16_t *right_eye_mUI, uint16_t  *upper_eye_mV, uint16_t *lower_eye_mV) {
  uint8_t ladder_range = 0;
  ESTM(ladder_range = rd_p1_thresh_sel());
  ESTM(*left_eye_mUI  = merlin16_shasta_INTERNAL_eye_to_mUI(sa__, rdv_usr_sts_heye_left()));
  ESTM(*right_eye_mUI = merlin16_shasta_INTERNAL_eye_to_mUI(sa__, rdv_usr_sts_heye_right()));
  ESTM(*upper_eye_mV = merlin16_shasta_INTERNAL_eye_to_mV(sa__, rdv_usr_sts_veye_upper(), ladder_range));
  ESTM(*lower_eye_mV = merlin16_shasta_INTERNAL_eye_to_mV(sa__, rdv_usr_sts_veye_lower(), ladder_range));

  return (ERR_CODE_NONE);
}


uint16_t merlin16_shasta_INTERNAL_eye_to_mUI(srds_access_t *sa__, uint8_t var) 
{   
    UNUSED(sa__);
...
uint16_t merlin16_shasta_INTERNAL_eye_to_mV(srds_access_t *sa__, uint8_t var, uint8_t ladder_range) 
{       
    /* find nearest two vertical levels */
    uint16_t vl = (uint16_t)(merlin16_shasta_INTERNAL_ladder_setting_to_mV(sa__, (int8_t)(var/8), ladder_range));    
    uint16_t vu = (uint16_t)(merlin16_shasta_INTERNAL_ladder_setting_to_mV(sa__, (int8_t)(SRDS_MIN(31,var/8+1)), ladder_range));
    return ((uint16_t)(vl + (vu-vl)*(var&7)/8));
}

err_code_t merlin16_shasta_INTERNAL_get_osr_mode(srds_access_t *sa__, merlin16_shasta_osr_mode_st *imode) {
    merlin16_shasta_osr_mode_st mode;

     ENULL_MEMSET(&mode, 0, sizeof(merlin16_shasta_osr_mode_st));
...
#ifdef wr_ams_rx_rxclk_div2p5    /* Analog 2p5 OSR mode */
  {
      ...
      if ((mode.tx == MERLIN16_SHASTA_OSX1) && (osr_2p5 == 1)  && (osr_2p5_pwrup == 1)) {
          mode.tx = MERLIN16_SHASTA_OSX2P5;
      }
      ESTM(osr_2p5       = rd_ams_rx_rxclk_div2p5());
      ESTM(osr_2p5_pwrup = rd_ams_rx_rxclk_div2p5_pwrup());
      if ((mode.rx == MERLIN16_SHASTA_OSX1) && (osr_2p5 == 1)  && (osr_2p5_pwrup == 1)) {
          mode.rx = MERLIN16_SHASTA_OSX2P5;
      }
  }
#endif
  *imode = mode;
...
err_code_t merlin16_shasta_INTERNAL_pmd_lock_status(srds_access_t *sa__, uint8_t *pmd_lock, uint8_t *pmd_lock_chg) {
    uint16_t rddata;
...
#else
#error "Unknown core for merlin16_shasta_INTERNAL_pmd_lock_status()"    
#endif
    return(ERR_CODE_NONE);
}

err_code_t merlin16_shasta_INTERNAL_sigdet_status(srds_access_t *sa__, uint8_t *sig_det, uint8_t *sig_det_chg) {
    uint16_t rddata;
...
err_code_t merlin16_shasta_INTERNAL_pll_lock_status(srds_access_t *sa__, uint8_t *pll_lock, uint8_t *pll_lock_chg) {
...
err_code_t merlin16_shasta_INTERNAL_read_lane_state(srds_access_t *sa__, merlin16_shasta_lane_state_st *istate) {
       
  merlin16_shasta_lane_state_st state;
  uint8_t ladder_range = 0;

  ENULL_MEMSET(&state, 0, sizeof(merlin16_shasta_lane_state_st));

  if(!istate) 
    return(merlin16_shasta_error(sa__, ERR_CODE_BAD_PTR_OR_INVALID_INPUT));

  EFUN(merlin16_shasta_INTERNAL_pmd_lock_status(sa__,&state.rx_lock, &state.rx_lock_chg));
  {
      err_code_t err_code = 0;
      state.stop_state = merlin16_shasta_INTERNAL_stop_micro(sa__,state.rx_lock,&err_code);
      if(err_code) USR_PRINTF(("WARNING: Unable to stop microcontroller,  following data is suspect\n"));
  }

  {  merlin16_shasta_osr_mode_st osr_mode;
  ENULL_MEMSET(&osr_mode, 0, sizeof(merlin16_shasta_osr_mode_st));
  EFUN(merlin16_shasta_INTERNAL_get_osr_mode(sa__, &osr_mode));
  state.osr_mode = osr_mode;
  }
  {
      struct merlin16_shasta_uc_lane_config_st lane_cfg;
      EFUN(merlin16_shasta_get_uc_lane_cfg(sa__, &lane_cfg));
  ...
  EFUN(merlin16_shasta_INTERNAL_sigdet_status(sa__,&state.sig_det, &state.sig_det_chg));
  ...
  EFUN(merlin16_shasta_read_rx_afe(sa__, RX_AFE_PF, &state.pf_main));
  ESTM(state.pf_hiz = (int8_t)rd_pf_hiz());
  ESTM(state.pf2_ctrl = (int8_t)rd_pf2_lowp_ctrl());
  EFUN(merlin16_shasta_read_rx_afe(sa__, RX_AFE_VGA, &state.vga));

  ESTM(state.dc_offset = rd_dc_offset_bin());
  ESTM(ladder_range = rd_p1_thresh_sel());
  EFUN(merlin16_shasta_INTERNAL_get_p1_threshold(sa__, &state.p1_lvl_ctrl));         
  state.p1_lvl = merlin16_shasta_INTERNAL_ladder_setting_to_mV(sa__, state.p1_lvl_ctrl, ladder_range);
  state.m1_lvl = 0;

  EFUN(merlin16_shasta_INTERNAL_get_rx_dfe1(sa__, &state.dfe1));
  EFUN(merlin16_shasta_INTERNAL_get_rx_dfe2(sa__, &state.dfe2));  
  EFUN(merlin16_shasta_INTERNAL_get_rx_dfe3(sa__, &state.dfe3));  
  EFUN(merlin16_shasta_INTERNAL_get_rx_dfe4(sa__, &state.dfe4));
  EFUN(merlin16_shasta_INTERNAL_get_rx_dfe5(sa__, &state.dfe5));
  ...
  EFUN(merlin16_shasta_INTERNAL_get_tx_pre(sa__, &state.txfir_pre));
  EFUN(merlin16_shasta_INTERNAL_get_tx_main(sa__, &state.txfir_main));
  EFUN(merlin16_shasta_INTERNAL_get_tx_post1(sa__, &state.txfir_post1));
  EFUN(merlin16_shasta_INTERNAL_get_tx_post2(sa__, &state.txfir_post2));
  EFUN(merlin16_shasta_INTERNAL_get_eye_margin_est(sa__, &state.heye_left, &state.heye_right, &state.veye_upper, &state.veye_lower));
/* TODO: KD, For Blackhawk PAM4 mode, Eye margin display should show minimum value for each of the above 4 variables among the 3 eyes */ 
  ...
  ESTM(state.link_time = (uint16_t)((((uint32_t)rdv_usr_sts_link_time())*8)/10));     /* convert from 80us to 0.1 ms units */
  {
      ...
      EFUN(merlin16_shasta_INTERNAL_pmd_lock_status(sa__,&rx_lock_at_end, &rx_lock_chg_at_end));
  ...
  if (!state.stop_state || (state.stop_state == 0xFF)) {
      /* manually check error code instead of EFUN*/
      {
          ...
          resume_status = merlin16_shasta_stop_rx_adaptation(sa__, 0);
          if (resume_status) {
...
err_code_t merlin16_shasta_INTERNAL_display_lane_state_no_newline(srds_access_t *sa__) {     
  uint16_t lane_idx;
  merlin16_shasta_lane_state_st state;
  ...
  EFUN(merlin16_shasta_INTERNAL_read_lane_state(sa__, &state));
 
  lane_idx = merlin16_shasta_get_lane(sa__); 
  ...
  EFUN(merlin16_shasta_INTERNAL_display_BER(sa__,100));

  
  return (ERR_CODE_NONE);
}

err_code_t merlin16_shasta_INTERNAL_read_core_state(srds_access_t *sa__, merlin16_shasta_core_state_st *istate) {  
  merlin16_shasta_core_state_st state;
  struct merlin16_shasta_uc_core_config_st core_cfg;

  ENULL_MEMSET(&state, 0, sizeof(merlin16_shasta_core_state_st));
  ENULL_MEMSET(&core_cfg, 0, sizeof(struct merlin16_shasta_uc_core_config_st));

  if(!istate) 
    return(merlin16_shasta_error(sa__, ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
  EFUN(merlin16_shasta_get_uc_core_config(sa__, &core_cfg));
  ...
  EFUN(merlin16_shasta_version(sa__, &state.api_version));
  ...
  {  int16_t                           die_temp = 0;
     EFUN(merlin16_shasta_read_die_temperature(sa__, &die_temp));
  ...
  EFUN(merlin16_shasta_INTERNAL_read_pll_div(sa__, &state.pll_div));
  EFUN(merlin16_shasta_INTERNAL_pll_lock_status(sa__, &state.pll_lock, &state.pll_lock_chg));
...
err_code_t merlin16_shasta_INTERNAL_display_core_state_no_newline(srds_access_t *sa__) {
  merlin16_shasta_core_state_st state;
    ENULL_MEMSET(&state     , 0, sizeof(state     ));
  EFUN(merlin16_shasta_INTERNAL_read_core_state(sa__, &state));
    ...
    EFUN_PRINTF((" %02d "              ,  merlin16_shasta_get_core(sa__)));
    ...
    EFUN(merlin16_shasta_INTERNAL_display_pll_to_divider(sa__, state.pll_div));
...
err_code_t merlin16_shasta_INTERNAL_update_uc_lane_config_st(struct  merlin16_shasta_uc_lane_config_st *st) {
  uint16_t in = st->word;
...
err_code_t merlin16_shasta_INTERNAL_update_usr_ctrl_disable_functions_st(struct merlin16_shasta_usr_ctrl_disable_functions_st *st) {
  uint8_t in = st->byte;
...
err_code_t merlin16_shasta_INTERNAL_update_usr_ctrl_disable_dfe_functions_st(struct merlin16_shasta_usr_ctrl_disable_dfe_functions_st *st) {
  uint8_t in = st->byte;
...
err_code_t merlin16_shasta_INTERNAL_update_uc_core_config_st(struct merlin16_shasta_uc_core_config_st *st) {
  uint16_t in = st->word;
...
err_code_t merlin16_shasta_INTERNAL_update_uc_core_config_word(struct merlin16_shasta_uc_core_config_st *st) {
  uint16_t in = 0;
...
err_code_t merlin16_shasta_INTERNAL_update_uc_lane_config_word(struct merlin16_shasta_uc_lane_config_st *st) {
  uint16_t in = 0;
...
err_code_t merlin16_shasta_INTERNAL_update_usr_ctrl_disable_functions_byte(struct merlin16_shasta_usr_ctrl_disable_functions_st *st) {
  uint8_t in = 0;
...
err_code_t merlin16_shasta_INTERNAL_update_usr_ctrl_disable_dfe_functions_byte(struct  merlin16_shasta_usr_ctrl_disable_dfe_functions_st *st) {
  uint8_t in = 0;
...
err_code_t merlin16_shasta_INTERNAL_check_uc_lane_stopped(srds_access_t *sa__) {

  uint8_t is_micro_stopped;
  ESTM(is_micro_stopped = rdv_usr_sts_micro_stopped() || (rd_rx_lane_dp_reset_state() > 0));
  if (!is_micro_stopped) {
      return(merlin16_shasta_error(sa__, ERR_CODE_UC_NOT_STOPPED));
  } else {
      return(ERR_CODE_NONE);
  }
}

err_code_t merlin16_shasta_INTERNAL_calc_patt_gen_mode_sel(srds_access_t *sa__, uint8_t *mode_sel, uint8_t *zero_pad_len, uint8_t patt_length) {
  ...
  if (!(140 % patt_length)) {
    ...
    *zero_pad_len = 0;
    ...
    return (merlin16_shasta_error(sa__, ERR_CODE_CFG_PATT_INVALID_PATT_LENGTH));
  }
  return(ERR_CODE_NONE);
}
...
err_code_t merlin16_shasta_INTERNAL_set_uc_core_config(srds_access_t *sa__, struct merlin16_shasta_uc_core_config_st struct_val) {
    {   uint8_t reset_state;
        ESTM(reset_state = rdc_core_dp_reset_state());
        if(reset_state < 7) {
            EFUN_PRINTF(("ERROR: merlin16_shasta_INTERNAL_set_uc_core_config(..) called without core_dp_s_rstb=0\n"));
    ...
    EFUN(merlin16_shasta_INTERNAL_update_uc_core_config_word(&struct_val));
...
/*  PLL Configuration  */
/*---------------------*/
/** Extract the refclk frequency in Hz, based on a merlin16_shasta_pll_refclk_enum value. */
err_code_t _merlin16_shasta_get_refclk_in_hz(srds_access_t *sa__, enum merlin16_shasta_pll_refclk_enum refclk, uint32_t *refclk_in_hz) {
    switch (refclk) {
        case MERLIN16_SHASTA_PLL_REFCLK_50MHZ:           *refclk_in_hz =  50000000; break;
        case MERLIN16_SHASTA_PLL_REFCLK_54MHZ:           *refclk_in_hz =  54000000; break;
        case MERLIN16_SHASTA_PLL_REFCLK_125MHZ:          *refclk_in_hz = 125000000; break;
        case MERLIN16_SHASTA_PLL_REFCLK_156P25MHZ:       *refclk_in_hz = 156250000; break;
        case MERLIN16_SHASTA_PLL_REFCLK_161P1328125MHZ:  *refclk_in_hz = 161132813; break;
        default:
            EFUN_PRINTF(("ERROR: Unknown refclk frequency:  0x%08X\n", (uint32_t)refclk));
            *refclk_in_hz = 0xFFFFFFFF;
            return (merlin16_shasta_error(sa__, ERR_CODE_REFCLK_FREQUENCY_INVALID));
    }
    return (ERR_CODE_NONE);
}
 ...
 */
static err_code_t _merlin16_shasta_get_divisor_ratio(srds_access_t *sa__, enum merlin16_shasta_pll_div_enum srds_div, uint16_t *numerator, uint16_t *denominator)
{
    switch (srds_div) {
        case MERLIN16_SHASTA_PLL_DIV_190P97222:  *denominator =  55;  *numerator =  288; break;
        case MERLIN16_SHASTA_PLL_DIV_231P48148:  *denominator =  25;  *numerator =  108; break;
        case MERLIN16_SHASTA_PLL_DIV_250:        *denominator =   1;  *numerator =    4; break;
        case MERLIN16_SHASTA_PLL_DIV_52P751515:  *denominator = 162;  *numerator = 3071; break;
        case MERLIN16_SHASTA_PLL_DIV_58P181818:  *denominator =  16;  *numerator =  275; break;
        case MERLIN16_SHASTA_PLL_DIV_62P060606:  *denominator =  53;  *numerator =  854; break;
        case MERLIN16_SHASTA_PLL_DIV_67P878788:  *denominator =  56;  *numerator =  825; break;
        case MERLIN16_SHASTA_PLL_DIV_66P460703:  *denominator =  43;  *numerator =  647; break;
        case MERLIN16_SHASTA_PLL_DIV_66P743079:  *denominator =  58;  *numerator =  869; break;
        case MERLIN16_SHASTA_PLL_DIV_68P570764:  *denominator =  84;  *numerator = 1225; break;
        case MERLIN16_SHASTA_PLL_DIV_68P856242:  *denominator =  65;  *numerator =  944; break;
        case MERLIN16_SHASTA_PLL_DIV_69P152458:  *denominator = 102;  *numerator = 1475; break;
        case MERLIN16_SHASTA_PLL_DIV_69P389964:  *denominator = 124;  *numerator = 1787; break;
        case MERLIN16_SHASTA_PLL_DIV_69P818182:  *denominator =  96;  *numerator = 1375; break;
        case MERLIN16_SHASTA_PLL_DIV_71P112952:  *denominator = 177;  *numerator = 2489; break;
        case MERLIN16_SHASTA_PLL_DIV_54P4:       *denominator =  34;  *numerator =  625; break;
        case MERLIN16_SHASTA_PLL_DIV_60:         *denominator =   3;  *numerator =   50; break;
        case MERLIN16_SHASTA_PLL_DIV_64:         *denominator =   8;  *numerator =  125; break;
        case MERLIN16_SHASTA_PLL_DIV_66:         *denominator =  33;  *numerator =  500; break;
        case MERLIN16_SHASTA_PLL_DIV_68:         *denominator =  17;  *numerator =  250; break;
        case MERLIN16_SHASTA_PLL_DIV_68P537598:  *denominator = 127;  *numerator = 1853; break;
        case MERLIN16_SHASTA_PLL_DIV_68P828796:  *denominator = 104;  *numerator = 1511; break;
        case MERLIN16_SHASTA_PLL_DIV_70:         *denominator =   7;  *numerator =  100; break;
        case MERLIN16_SHASTA_PLL_DIV_70P713596:  *denominator = 106;  *numerator = 1499; break;
        case MERLIN16_SHASTA_PLL_DIV_71P008:     *denominator = 205;  *numerator = 2887; break;
        case MERLIN16_SHASTA_PLL_DIV_71P31347:   *denominator = 177;  *numerator = 2482; break;
        case MERLIN16_SHASTA_PLL_DIV_71P5584:    *denominator = 118;  *numerator = 1649; break;
        case MERLIN16_SHASTA_PLL_DIV_72:         *denominator =   9;  *numerator =  125; break;
        case MERLIN16_SHASTA_PLL_DIV_73P335232:  *denominator = 250;  *numerator = 3409; break;
        case MERLIN16_SHASTA_PLL_DIV_73P6:       *denominator =  46;  *numerator =  625; break;
        case MERLIN16_SHASTA_PLL_DIV_75:         *denominator =   3;  *numerator =   40; break;
        case MERLIN16_SHASTA_PLL_DIV_80:         *denominator =   2;  *numerator =   25; break;
        case MERLIN16_SHASTA_PLL_DIV_82P5:       *denominator =  33;  *numerator =  400; break;
        case MERLIN16_SHASTA_PLL_DIV_85P671997:  *denominator =  58;  *numerator =  677; break;
        case MERLIN16_SHASTA_PLL_DIV_86P036:     *denominator = 130;  *numerator = 1511; break;
        case MERLIN16_SHASTA_PLL_DIV_87P5:       *denominator =   7;  *numerator =   80; break;
        case MERLIN16_SHASTA_PLL_DIV_88P392:     *denominator =  83;  *numerator =  939; break;
        case MERLIN16_SHASTA_PLL_DIV_88P76:      *denominator = 199;  *numerator = 2242; break;
        case MERLIN16_SHASTA_PLL_DIV_89P141838:  *denominator = 243;  *numerator = 2726; break;
        case MERLIN16_SHASTA_PLL_DIV_89P447998:  *denominator = 128;  *numerator = 1431; break;
        case MERLIN16_SHASTA_PLL_DIV_90:         *denominator =   9;  *numerator =  100; break;
        case MERLIN16_SHASTA_PLL_DIV_91P669037:  *denominator = 296;  *numerator = 3229; break;
        case MERLIN16_SHASTA_PLL_DIV_92:         *denominator =  23;  *numerator =  250; break;
        case MERLIN16_SHASTA_PLL_DIV_100:        *denominator =   1;  *numerator =   10; break;
        case MERLIN16_SHASTA_PLL_DIV_170:        *denominator =  17;  *numerator =  100; break;
        case MERLIN16_SHASTA_PLL_DIV_187P5:      *denominator =   3;  *numerator =   16; break;
        case MERLIN16_SHASTA_PLL_DIV_200:        *denominator =   1;  *numerator =    5; break;
        case MERLIN16_SHASTA_PLL_DIV_206P25:     *denominator =  33;  *numerator =  160; break;
        default:
            EFUN_PRINTF(("ERROR: Unknown divider value:  0x%08X\n", (uint32_t)srds_div));
            *numerator = 0;
            *denominator = 0;
            return (merlin16_shasta_error(sa__, ERR_CODE_PLL_DIV_INVALID));
    }
    return (ERR_CODE_NONE);
}

/** Get the Refclk frequency in Hz, based on the merlin16_shasta_pll_div_enum value and VCO frequency. */
static err_code_t _merlin16_shasta_get_refclk_from_div_vco(srds_access_t *sa__, uint32_t *refclk_freq_hz, enum merlin16_shasta_pll_div_enum srds_div, uint32_t vco_freq_khz, enum merlin16_shasta_pll_option_enum pll_option) {
    uint16_t numerator, denominator;
    EFUN(_merlin16_shasta_get_divisor_ratio(sa__, srds_div, &numerator, &denominator));
    *refclk_freq_hz = ((vco_freq_khz+(denominator>>1)) / denominator) * numerator;
    if (pll_option == MERLIN16_SHASTA_PLL_OPTION_REFCLK_DOUBLER_EN) *refclk_freq_hz /= 2;
    if (pll_option == MERLIN16_SHASTA_PLL_OPTION_REFCLK_DIV2_EN)    *refclk_freq_hz *= 2;
    if (pll_option == MERLIN16_SHASTA_PLL_OPTION_REFCLK_DIV4_EN)    *refclk_freq_hz *= 4;
    return (ERR_CODE_NONE);
}

/** Get the VCO frequency in kHz, based on the reference clock frequency and merlin16_shasta_pll_div_enum value. */
err_code_t merlin16_shasta_INTERNAL_get_vco_from_refclk_div(srds_access_t *sa__, uint32_t refclk_freq_hz, enum merlin16_shasta_pll_div_enum srds_div, uint32_t *vco_freq_khz, enum merlin16_shasta_pll_option_enum pll_option) {
    uint16_t numerator, denominator;
    EFUN(_merlin16_shasta_get_divisor_ratio(sa__, srds_div, &numerator, &denominator));
    if (pll_option == MERLIN16_SHASTA_PLL_OPTION_REFCLK_DOUBLER_EN) refclk_freq_hz *= 2;
    if (pll_option == MERLIN16_SHASTA_PLL_OPTION_REFCLK_DIV2_EN)    refclk_freq_hz /= 2;
...
static const enum merlin16_shasta_pll_div_enum _merlin16_shasta_div_candidates[] = {
    MERLIN16_SHASTA_PLL_DIV_190P97222,
    MERLIN16_SHASTA_PLL_DIV_231P48148,
    MERLIN16_SHASTA_PLL_DIV_250,
    MERLIN16_SHASTA_PLL_DIV_52P751515,
    MERLIN16_SHASTA_PLL_DIV_54P4,
    MERLIN16_SHASTA_PLL_DIV_58P181818,
    MERLIN16_SHASTA_PLL_DIV_60,
    MERLIN16_SHASTA_PLL_DIV_62P060606,
    MERLIN16_SHASTA_PLL_DIV_64,
    MERLIN16_SHASTA_PLL_DIV_66,
    MERLIN16_SHASTA_PLL_DIV_66P460703,
    MERLIN16_SHASTA_PLL_DIV_66P743079,
    MERLIN16_SHASTA_PLL_DIV_67P878788,
    MERLIN16_SHASTA_PLL_DIV_68,
    MERLIN16_SHASTA_PLL_DIV_68P537598,
    MERLIN16_SHASTA_PLL_DIV_68P570764,
    MERLIN16_SHASTA_PLL_DIV_68P828796,
    MERLIN16_SHASTA_PLL_DIV_68P856242,
    MERLIN16_SHASTA_PLL_DIV_69P152458,
    MERLIN16_SHASTA_PLL_DIV_69P389964,
    MERLIN16_SHASTA_PLL_DIV_69P818182,
    MERLIN16_SHASTA_PLL_DIV_70,
    MERLIN16_SHASTA_PLL_DIV_70P713596,
    MERLIN16_SHASTA_PLL_DIV_71P008,
    MERLIN16_SHASTA_PLL_DIV_71P112952,
    MERLIN16_SHASTA_PLL_DIV_71P31347,
    MERLIN16_SHASTA_PLL_DIV_71P5584,
    MERLIN16_SHASTA_PLL_DIV_72,
    MERLIN16_SHASTA_PLL_DIV_73P335232,
    MERLIN16_SHASTA_PLL_DIV_73P6,
    MERLIN16_SHASTA_PLL_DIV_75,
    MERLIN16_SHASTA_PLL_DIV_80,
    MERLIN16_SHASTA_PLL_DIV_82P5,
    MERLIN16_SHASTA_PLL_DIV_85P671997,
    MERLIN16_SHASTA_PLL_DIV_86P036,
    MERLIN16_SHASTA_PLL_DIV_87P5,
    MERLIN16_SHASTA_PLL_DIV_88P392,
    MERLIN16_SHASTA_PLL_DIV_88P76,
    MERLIN16_SHASTA_PLL_DIV_89P141838,
    MERLIN16_SHASTA_PLL_DIV_89P447998,
    MERLIN16_SHASTA_PLL_DIV_90,
    MERLIN16_SHASTA_PLL_DIV_91P669037,
    MERLIN16_SHASTA_PLL_DIV_92,
    MERLIN16_SHASTA_PLL_DIV_100,
    MERLIN16_SHASTA_PLL_DIV_170,
    MERLIN16_SHASTA_PLL_DIV_187P5,
    MERLIN16_SHASTA_PLL_DIV_200,
    MERLIN16_SHASTA_PLL_DIV_206P25
};

static const uint8_t _merlin16_shasta_div_candidates_count = sizeof(_merlin16_shasta_div_candidates) / sizeof(_merlin16_shasta_div_candidates[0]);

static err_code_t _merlin16_shasta_check_div(srds_access_t *sa__, enum merlin16_shasta_pll_div_enum srds_div) {
    uint8_t i, found = 0;
    for (i=0; i<_merlin16_shasta_div_candidates_count; i++) {
        found = (uint8_t)(found | (srds_div == _merlin16_shasta_div_candidates[i]));
    }
    if (!found) {
...
static err_code_t _merlin16_shasta_check_vco_freq_khz(srds_access_t *sa__, uint32_t vco_freq_khz) {
    if (vco_freq_khz < SERDES_VCO_FREQ_KHZ_MIN - SERDES_VCO_FREQ_KHZ_TOLERANCE) {
        ...
        return (merlin16_shasta_error(sa__, ERR_CODE_VCO_FREQUENCY_INVALID));
    }
...
/** Find the entry out of _merlin16_shasta_div_candidates that is closest to matching refclk_freq_hz and vco_freq_khz. */
static err_code_t _merlin16_shasta_get_div(srds_access_t *sa__, uint32_t refclk_freq_hz, uint32_t vco_freq_khz, enum merlin16_shasta_pll_div_enum *srds_div, enum merlin16_shasta_pll_option_enum pll_option) {
    int32_t vco_freq_khz_min_error = 0x7FFFFFFF;
    uint8_t i;
    for (i=0; i<_merlin16_shasta_div_candidates_count; i++) {
        uint32_t actual_vco_freq_khz = 0;
        int32_t  vco_freq_khz_error;
        EFUN(merlin16_shasta_INTERNAL_get_vco_from_refclk_div(sa__, refclk_freq_hz, _merlin16_shasta_div_candidates[i], &actual_vco_freq_khz, pll_option));
        ...
        if (vco_freq_khz_min_error > vco_freq_khz_error) {
            ...
            *srds_div = _merlin16_shasta_div_candidates[i];
        }
    }
    if (vco_freq_khz_min_error == (int32_t)0xFFFFFFFF) {
      return (merlin16_shasta_error(sa__, ERR_CODE_CONFLICTING_PARAMETERS));
    }
    return (ERR_CODE_NONE);
}

err_code_t merlin16_shasta_INTERNAL_resolve_pll_parameters(srds_access_t *sa__,
    ...
    const uint8_t given_param_count = (uint8_t)(((refclk == MERLIN16_SHASTA_PLL_REFCLK_UNKNOWN) ? 0 : 1)
                                       + ((*srds_div == MERLIN16_SHASTA_PLL_DIV_UNKNOWN) ? 0 : 1)
                                       + ((*vco_freq_khz == 0) ? 0 : 1));
    const uint32_t original_vco_freq_khz = *vco_freq_khz;
    enum merlin16_shasta_pll_div_enum auto_div = MERLIN16_SHASTA_PLL_DIV_UNKNOWN;
    ...
    if (given_param_count < 2) {
        return (merlin16_shasta_error(sa__, ERR_CODE_INSUFFICIENT_PARAMETERS));
    }

    /* Skip verification if option is selected. Error if all parameters not given. */
    if ((pll_option & MERLIN16_SHASTA_PLL_OPTION_DISABLE_VERIFY) == MERLIN16_SHASTA_PLL_OPTION_DISABLE_VERIFY) {
        if (given_param_count < 3) {
            return (merlin16_shasta_error(sa__, ERR_CODE_INSUFFICIENT_PARAMETERS));
        } else {
            EFUN(_merlin16_shasta_get_refclk_in_hz(sa__, refclk, refclk_freq_hz));
            return(ERR_CODE_NONE);
        }
    }
    ...
    if (*srds_div != MERLIN16_SHASTA_PLL_DIV_UNKNOWN) {
        EFUN(_merlin16_shasta_check_div(sa__, *srds_div));
    }

    /* Verify that the requested VCO frequency is allowed. */
    if (*vco_freq_khz != 0) {
        EFUN(_merlin16_shasta_check_vco_freq_khz(sa__, *vco_freq_khz));
    }

    if (refclk == MERLIN16_SHASTA_PLL_REFCLK_UNKNOWN) {
        /* Determine refclk from vco frequency and div */
        EFUN(_merlin16_shasta_get_refclk_from_div_vco(sa__, refclk_freq_hz, *srds_div, *vco_freq_khz, pll_option));
    } else {
        EFUN(_merlin16_shasta_get_refclk_in_hz(sa__, refclk, refclk_freq_hz));
    }
     ...
     */
    EFUN(_merlin16_shasta_get_div(sa__, *refclk_freq_hz, *vco_freq_khz, &auto_div, pll_option));
    if (*srds_div == MERLIN16_SHASTA_PLL_DIV_UNKNOWN) {
...
err_code_t merlin16_shasta_INTERNAL_display_pll_to_divider(srds_access_t *sa__, uint32_t srds_div) {
...
err_code_t merlin16_shasta_INTERNAL_print_uc_dsc_error(srds_access_t *sa__, enum srds_pmd_uc_cmd_enum cmd) {
    uint32_t supp_info;
...
err_code_t merlin16_shasta_INTERNAL_poll_diag_done(srds_access_t *sa__, uint16_t *status, uint32_t timeout_ms) {
 uint8_t loop;
 ...
 return(merlin16_shasta_error(sa__, ERR_CODE_DIAG_TIMEOUT));
}
/** Poll for field "uc_dsc_ready_for_cmd" = 1 [Return Val => Error_code (0 = Polling Pass)] */
err_code_t merlin16_shasta_INTERNAL_poll_uc_dsc_ready_for_cmd_equals_1(srds_access_t *sa__, uint32_t timeout_ms, enum srds_pmd_uc_cmd_enum cmd) {
    ...
    for (loop = 0; loop < 100; loop++) {
        ...
        if (rddata & 0x0080) {    /* bit 7 is uc_dsc_ready_for_cmd */
            if (rddata & 0x0040) {  /* bit 6 is uc_dsc_error_found   */
                EFUN(merlin16_shasta_INTERNAL_print_uc_dsc_error(sa__, cmd));
                return(merlin16_shasta_error(sa__, ERR_CODE_UC_CMD_RETURN_ERROR));
            }
            return (ERR_CODE_NONE);
        }
        if(loop>10) {
    ...
    }
    {
        ...
        merlin16_shasta_INTERNAL_print_triage_info(sa__, ERR_CODE_UC_CMD_POLLING_TIMEOUT, 1, 1, __LINE__);
...
err_code_t merlin16_shasta_INTERNAL_poll_dsc_state_equals_uc_tune(srds_access_t *sa__, uint32_t timeout_ms) {
  uint16_t loop;
  uint16_t dsc_state;
  ...
  return (merlin16_shasta_error(sa__, ERR_CODE_POLLING_TIMEOUT));          /* Error Code for polling timeout */
}


/* Poll for field "micro_ra_initdone" = 1 [Return Val => Error_code (0 = Polling Pass)] */
err_code_t merlin16_shasta_INTERNAL_poll_micro_ra_initdone(srds_access_t *sa__, uint32_t timeout_ms) {
  uint16_t loop;
...
uint8_t merlin16_shasta_INTERNAL_is_big_endian(void)
{
...
int merlin16_shasta_INTERNAL_get_endian_offset(uint8_t *addr) {
...
err_code_t merlin16_shasta_INTERNAL_rdblk_callback(void *arg, uint8_t byte_count, uint16_t data) {
    srds_access_t *sa__;
    merlin16_shasta_INTERNAL_rdblk_callback_arg_t * const cast_arg = (merlin16_shasta_INTERNAL_rdblk_callback_arg_t *)arg;
    int endian_offset = 0;
    sa__ = cast_arg->sa;

    ESTM(endian_offset = merlin16_shasta_INTERNAL_get_endian_offset(cast_arg->mem_ptr));
...
err_code_t merlin16_shasta_INTERNAL_rdblk_uc_generic_ram(srds_access_t *sa__,
                                                uint32_t block_addr,
                                                uint16_t block_size,
...
err_code_t merlin16_shasta_INTERNAL_rdblk_uc_generic_ram_descending(srds_access_t *sa__,
                                                           uint32_t block_addr,
                                                           uint16_t block_size,
                                                           uint16_t start_offset,
                                                           uint16_t cnt,
...
uint8_t merlin16_shasta_INTERNAL_grp_idx_from_lane(uint8_t lane) {
    return(0);
}


err_code_t merlin16_shasta_INTERNAL_display_info_table (srds_access_t *sa__) {
    srds_info_t * const merlin16_shasta_info_ptr = merlin16_shasta_INTERNAL_get_merlin16_shasta_info_ptr_with_check(sa__);
    EFUN(merlin16_shasta_INTERNAL_verify_merlin16_shasta_info(merlin16_shasta_info_ptr, sa__));

    EFUN_PRINTF(("\n********** SERDES INFO TABLE DUMP **********\n"));
    EFUN_PRINTF(("signature                 = 0x%X\n", merlin16_shasta_info_ptr->signature));
    EFUN_PRINTF(("diag_mem_ram_base         = 0x%X\n", merlin16_shasta_info_ptr->diag_mem_ram_base));
    EFUN_PRINTF(("diag_mem_ram_size         = 0x%X\n", merlin16_shasta_info_ptr->diag_mem_ram_size));
    EFUN_PRINTF(("core_var_ram_base         = 0x%X\n", merlin16_shasta_info_ptr->core_var_ram_base));
    EFUN_PRINTF(("core_var_ram_size         = 0x%X\n", merlin16_shasta_info_ptr->core_var_ram_size));
    EFUN_PRINTF(("lane_var_ram_base         = 0x%X\n", merlin16_shasta_info_ptr->lane_var_ram_base));
    EFUN_PRINTF(("lane_var_ram_size         = 0x%X\n", merlin16_shasta_info_ptr->lane_var_ram_size));
    EFUN_PRINTF(("trace_mem_ram_base        = 0x%X\n", merlin16_shasta_info_ptr->trace_mem_ram_base));
    EFUN_PRINTF(("trace_mem_ram_size        = 0x%X\n", merlin16_shasta_info_ptr->trace_mem_ram_size));
    EFUN_PRINTF(("micro_var_ram_base        = 0x%X\n", merlin16_shasta_info_ptr->micro_var_ram_base));
    EFUN_PRINTF(("lane_count                = 0x%X\n", merlin16_shasta_info_ptr->lane_count));
    EFUN_PRINTF(("trace_memory_descending_writes = 0x%X\n", merlin16_shasta_info_ptr->trace_memory_descending_writes));
    EFUN_PRINTF(("micro_count               = 0x%X\n", merlin16_shasta_info_ptr->micro_count));
    EFUN_PRINTF(("micro_var_ram_size        = 0x%X\n", merlin16_shasta_info_ptr->micro_var_ram_size));
    EFUN_PRINTF(("grp_ram_size              = 0x%X\n", merlin16_shasta_info_ptr->grp_ram_size));
    EFUN_PRINTF(("ucode_version             = 0x%X\n", merlin16_shasta_info_ptr->ucode_version));
