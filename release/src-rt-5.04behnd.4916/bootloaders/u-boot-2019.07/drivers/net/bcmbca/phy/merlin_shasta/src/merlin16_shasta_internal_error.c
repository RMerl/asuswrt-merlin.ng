...
/***********************************************************************************
 ***********************************************************************************
 *  File Name     :  merlin16_shasta_internal_error.c                                   *
...
/** @file merlin16_shasta_internal_error.c
...
err_code_t merlin16_shasta_INTERNAL_print_err_msg(srds_access_t *sa__, uint16_t err_code, const char *filename, const char *func_name, uint16_t line_num)
{
    if (err_code != ERR_CODE_NONE) {
        USR_PRINTF(("ERROR:%s->%s() SerDes err_code = %s : %d\n", filename, func_name, merlin16_shasta_INTERNAL_e2s_err_code(err_code),err_code));
    }
    return err_code;
}

err_code_t merlin16_shasta_INTERNAL_print_err_msg_and_triage_info(srds_access_t *sa__, uint16_t err_code, const char *filename, const char *func_name, uint16_t line_num)
{
    if (err_code != ERR_CODE_NONE) {
        USR_PRINTF(("ERROR:%s->%s() SerDes err_code = %s : %d\n", filename, func_name, merlin16_shasta_INTERNAL_e2s_err_code(err_code),err_code));
        merlin16_shasta_INTERNAL_print_triage_info(sa__, err_code, 1, 1, line_num);
    }
    return err_code;
}
