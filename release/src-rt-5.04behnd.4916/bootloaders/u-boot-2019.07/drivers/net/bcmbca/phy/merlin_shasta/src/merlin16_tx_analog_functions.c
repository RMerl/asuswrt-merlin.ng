...
err_code_t merlin16_shasta_validate_txfir_cfg(srds_access_t *sa__, int8_t pre, int8_t main, int8_t post1, int8_t post2) {
  ...
  return (merlin16_shasta_error(sa__, failcode));
}
...
err_code_t merlin16_shasta_apply_txfir_cfg(srds_access_t *sa__, int8_t pre, int8_t main, int8_t post1, int8_t post2) {

  uint8_t osr_mode;  
  err_code_t failcode = merlin16_shasta_validate_txfir_cfg(sa__, pre, main, post1, post2);
  ...
  if (!failcode) {
    ...
    if (osr_mode == MERLIN16_SHASTA_OSX2) {
      EFUN(wr_ams_tx_enable_os_2(1));
      EFUN(wr_ams_tx_enable_os_4(0));
    }
    else if (osr_mode == MERLIN16_SHASTA_OSX4) {
...
err_code_t merlin16_shasta_read_tx_afe(srds_access_t *sa__, enum merlin16_shasta_tx_afe_settings_enum param, int8_t *val) {
  if(!val) {
  ...
  switch(param) {
    case TX_AFE_PRE:
      EFUN(merlin16_shasta_INTERNAL_get_tx_pre(sa__, val));
      break;
    case TX_AFE_MAIN:
      EFUN(merlin16_shasta_INTERNAL_get_tx_main(sa__, val));
      break;
    case TX_AFE_POST1:
      EFUN(merlin16_shasta_INTERNAL_get_tx_post1(sa__, val));
      break;
    case TX_AFE_POST2:
      EFUN(merlin16_shasta_INTERNAL_get_tx_post2(sa__, val));
      break;
    default:
      return(merlin16_shasta_error(sa__, ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
  }
...
err_code_t merlin16_shasta_config_tx_hpf(srds_access_t *sa__, uint8_t val) {  /* KD */
...
err_code_t merlin16_shasta_rd_tx_hpf_config(srds_access_t *sa__, uint8_t *pval) {  /* KD */
