...
{
    int ret;
    void * unimac_conf_base2;
    void * unimac_mib_base2;
    void * unimac_top_base2;
    ...
    conf_offset = (uint32_t)(uintptr_t)unimac_conf_base2 - (uint32_t)(uintptr_t)unimac_conf_base;
    mib_offset  = (uint32_t)(uintptr_t)unimac_mib_base2  - (uint32_t)(uintptr_t)unimac_mib_base;
    top_offset  = (uint32_t)(uintptr_t)unimac_top_base2  - (uint32_t)(uintptr_t)unimac_top_base;
...
static int unimac_probe(dt_device_t *pdev)
{
    ...
    unimac_ver = (int) pdev->driver_data;

    if (unimac_ver == UNIMAC_VER2)
    {
        ret = unimac2_init(pdev);
        if (ret) goto Exit; else goto Done;
    }
    unimac_conf_base = dt_dev_remap_resource(pdev, 0);
...
static const struct udevice_id unimac_ids[] = {
    { .compatible = "brcm,unimac",  .data = UNIMAC_VER1 },
    { .compatible = "brcm,unimac2", .data = UNIMAC_VER2 },
    { /* end of list */ },
};
...
void mac_hwapi_set_eee(rdpa_emac emacNum, int32_t enable)
{
    ...
    if (enable)
    {
#if defined(CONFIG_BCM963158) || defined(CONFIG_BCM96846) || defined(CONFIG_BCM96856) || defined(CONFIG_BCM947622) || defined(CONFIG_BCM96878)
