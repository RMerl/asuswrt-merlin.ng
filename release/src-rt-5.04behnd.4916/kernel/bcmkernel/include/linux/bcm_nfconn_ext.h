...
struct bcm_nf_conn_ext {
	...
	uint8_t blog_learned; /* indicates CT has completed blog/fc learning atleast once */
	uint8_t blog_unused[3];
#endif
...
#define CT_SCS_INITED(ct)  (ct)->bcm_ext.scs.lock
	struct nf_scs {
	    struct list_head    entry;
	    spinlock_t          *lock;  /* per rule */
	} scs;
#endif

#if IS_ENABLED(CONFIG_NF_MSCS)
#define CT_MSCS_INITED(ct)  (ct)->bcm_ext.mscs.lock
	struct nf_mscs {
	    struct list_head    entry;
	    spinlock_t          *lock;  /* per rule */
	    u_int8_t            priority;
...
#define NF_CT_EXPECT_RELATED(e)     nf_ct_expect_related(e,0)
#else
#define NF_CT_EXPECT_RELATED(e)     nf_ct_expect_related(e)
#endif
