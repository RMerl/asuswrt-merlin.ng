...
#define FKBM_EXTEND_SIZE_ENGG       32
#define FKBM_EXTEND_MAX_ENGG        \
    (((CONFIG_BCM_MAX_MCAST_GROUPS * (CONFIG_BCM_MAX_MCAST_CLIENTS+1))/FKBM_EXTEND_SIZE_ENGG) + 1)
...
#define fkb_dbg(lvl, fmt, arg...) \
    if (nbuff_dbg >= lvl) printk( "FKB %s :" fmt "[<%pS>]\n", \
...
/*
 * FIXME: Currently 6837/6888 works with CPU_RX from FPM and it configured to
 * work with CC_NBUFF_FLUSH_OPTIMIZATION.
 *
 * Removing CC_NBUFF_FLUSH_OPTIMIZATION will require additional test and code
 * changes.
 */
#if !defined(CONFIG_BCM_GLB_COHERENCY) || defined(CONFIG_CPU_RX_FROM_XPM)
...
#if defined(__KERNEL__)     /* Linux Cache Specific */
/*
 *------------------------------------------------------------------------------
...
struct fkbuff
{
    ...
    union {
        ...
        struct{
            BE_DECL(
                ...
                uint32_t  spdtst:1;
                uint32_t  data_hw_recycle_capable:1;
                uint32_t  data_hw_recycle_done:1;
                uint32_t  svc_q_en:1; 
                uint32_t  svc_q_id:5;   /* WAR for DHD when mark is overwitten with queue */
                uint32_t  reserved:2;
                uint32_t  len:20;              /* Packet length               */
            )
            LE_DECL(
                uint32_t  len:20;
                uint32_t  reserved:2;
    ...
    union {
		...
		/*TODO define wl as  just uint32_t */
    ...
    union {
	      ...
	      * BE CAREFULL when using this recylce_context for free etc....  
	      */ 
...
#define FKB_CLEAR_LEN_WORD_FLAGS(len_word) (len_word &= 0x000FFFFF)
...
static inline uint8_t * _to_kptr_from_dptr_(uint8_t * dirty_ptr)
{
    FKB_AUDIT(
        if ( dirty_ptr && !is_dptr_tag_(dirty_ptr) )
            printk("FKB ASSERT %s !is_dptr_tag_(%lu)\n",
 ...
 *    If CONFIG_BCM_NBUFF_FKB_POOL is enabled then a Free Pool of Master 
 *    FKB objects is maintained. Master FKB may be alocated and recycled from 
 *    this Master FKB Pool.The Master FKB Pool may also be used for 
 *    replinishing a network device driver's rx buffer ring.
 *
...
{
    uint32_t users;
    users = atomic_long_read(&fkbM_p->users);

    FKB_AUDIT(
        if ( users == 0 )
            printk("FKB ASSERT cpu<%u> %s(%p) users == 0, recycle<%pS>\n",
        ...
        FKB_AUDIT(
            ...
            if ( _is_kptr_(fkb_p->master_p->list) )
            {
                printk("FKB ASSERT cpu<%u> %s :"
                       ...
                       fkb_p->master_p, fkb_p->master_p->list,
                       fkb_p->master_p,
 ...
 */
extern void fkb_preinit(uint8_t * pBuf, RecycleFuncP recycle_hook,
 ...
 */
extern FkBuff_t *_fkb_init(uint8_t * pBuf, uint32_t headroom,
 ...
 */
static inline void _fkb_release_blog(FkBuff_t * fkb_p)
{
    fkb_dbg(1, "fkb_p<%p> fkb_p->blog_p<%px>", fkb_p, fkb_p->blog_p );
#if defined(CONFIG_BLOG)
...
FKB_FN( fkb_release_blog,
        void fkb_release_blog(FkBuff_t * fkb_p),
        _fkb_release_blog(fkb_p) )
 ...
 * same packet. fkbInSkb case allocates a new master FKB and copy the data
 * (basically a FKB copy).
 *
 */
extern FkBuff_t * _fkb_clone(FkBuff_t *fkb_p);
                                     ...
                                     uint32_t * mark_p, uint32_t *priority_p)
{
    ...
    fkb_dbg(1, "pNBuff<%px> pBuf<%px> data_p<%px>",
            pNBuff, pBuf, *data_p );
    return pBuf;
}
...
static inline uint8_t * nbuff_push(pNBuff_t pNBuff, uint32_t len)
{
    ...
    fkb_dbg(1, "pNBuff<%px> pBuf<%px> data<%px> len<%u>",
...
static inline uint8_t * nbuff_put(pNBuff_t pNBuff, uint32_t len)
{
    ...
    fkb_dbg(1, "pNBuff<%px> pBuf<%px> tail<%px> len<%u>",
...
static inline pNBuff_t nbuff_unshare(pNBuff_t pNBuff)
{
 ...
 /* Function   : nbuff_is_shared
 * Description: check if there are more than one references to the data buffer
 * associated with the network buffer
 */
static inline bool nbuff_is_shared(pNBuff_t pNBuff)
{
    if ( IS_SKBUFF_PTR(pNBuff) )
    {
        struct sk_buff *skb_p = PNBUFF_2_SKBUFF(pNBuff);
        return (skb_shared(skb_p) || skb_cloned(skb_p));
    }
    else
    {
        FkBuff_t *fkb_p = PNBUFF_2_FKBUFF(pNBuff);
        if ( unlikely(_is_fkb_cloned_pool_(fkb_p)) )     /* Cloned FKB */
            return atomic_long_read(&(fkb_p->master_p->users)) > 1;
        else
            return atomic_long_read(&fkb_p->users) > 1;
    }
}

 /* Function   : nbuff_is_hw_recycle_capable
 * Description: check if the data buffer associated with the network buffer
 * can be recycled by HW/Accelerator 
 */
static inline bool nbuff_is_hw_recycle_capable(pNBuff_t pNBuff)
{
    if ( IS_SKBUFF_PTR(pNBuff) )
    {
        struct sk_buff *skb_p = PNBUFF_2_SKBUFF(pNBuff);

        if(skb_p->recycle_flags & SKB_DATA_HW_RECYCLE_CAPABLE)
            return true;
    }
    else
    {
...
/* Function   : nbuff_is_mark_hw_recycle_done
 * Description:  Mark the network buffer to indicate the associated data buffer
 * is recyled by HW/Accelerator, i.e after this data buffer should not be 
 * accessed any more
 */
static inline void nbuff_mark_hw_recycle_done(pNBuff_t pNBuff)
{
    if ( IS_SKBUFF_PTR(pNBuff) )
    {
        struct sk_buff *skb_p = PNBUFF_2_SKBUFF(pNBuff);
        skb_p->recycle_flags |= SKB_DATA_HW_RECYCLE_DONE;
    }
    else
    {
        FkBuff_t *fkb_p = PNBUFF_2_FKBUFF(pNBuff);
        fkb_p->data_hw_recycle_done=1;
    }
}

/* Function   : nbuff_data_hw_recycle_prep
 * Description:  If data buffer associated with the netwrok buffer is capable  
 * HW recyle  then free th network buffer now and the data buffer will be
 * recycled by HW/Accelerator later 
 */
static inline bool nbuff_data_hw_recycle_prep(pNBuff_t pNBuff)
{
    if(nbuff_is_hw_recycle_capable(pNBuff) && !nbuff_is_shared(pNBuff))
    {
        nbuff_mark_hw_recycle_done(pNBuff);
        /*this will free only skb or fkb structure */
        nbuff_free(pNBuff);
        return true;
    }
    return false;

    /*TODO do we need an optimized version of this i.e reduce number of skbuff/fkbuff checks
     * or the compiler otimization is good enough */
}
...
static inline void nbuff_invalidate_headroom(pNBuff_t pNBuff, uint8_t * data)
{
     ...
     */
    int32_t inv_len = 0;
    fkb_dbg(1, "pNBuff<%p> data<%p>", pNBuff, data);
...
extern void nbuff_invalidatefree(pNBuff_t pNBuff);
