				  ...
				  const struct net_bridge_fdb_entry *fdb)
{
...
#endif
    {
		...
		if (fdb->blog_fdb_key != BLOG_FDB_KEY_INVALID)
			blog_query(QUERY_BRIDGEFDB, (void*)&fdb->fdbid, fdb->blog_fdb_key, 0, 0);
		blog_unlock();
#endif
		       ...
		       bool swdev_notify)
{
	...
	if(type == RTM_DELNEIGH)
	{
...
#if defined(CONFIG_BLOG)
		blog_lock();
		blog_notify_async(DESTROY_BRIDGEFDB, (void*)&fdb->fdbid, fdb->blog_fdb_key, 0, NULL, NULL);
		bcm_fdbid_event(BCM_FDBID_EVT_DELETE, (struct net_bridge_fdb_entry *) fdb);
		blog_unlock();
#endif /* CONFIG_BLOG */
		...
		blog_lock();
		blog_notify_async(DESTROY_BRIDGEFDB, (void*)&fdb->fdbid, fdb->blog_fdb_key, 0, NULL, NULL);
		bcm_fdbid_event(BCM_FDBID_EVT_CREATE, (struct net_bridge_fdb_entry *) fdb);
		blog_unlock();
#endif /* CONFIG_BLOG */
	}
...
int bcm_br_fdb_init(struct net_bridge *br, struct net_bridge_fdb_entry *fdb)
{
		...
		blog_notify_async(CREATE_BRIDGEFDB, (void*)fdb, (unsigned long)fdb->key.addr.addr, (unsigned long)br, NULL, NULL);
        blog_unlock();
#endif
		return 0;
}
