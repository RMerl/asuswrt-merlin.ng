...
blogRuleTcHook_t blogRuleTcHook = NULL;
 ...
 *------------------------------------------------------------------------------
 */
int blog_rule_free_list(void *rule_p)
{
...
/*
 *------------------------------------------------------------------------------
 * Function     : blog_rule_hw_formatted_dump
 * Description  : Dump the contents of a blogRule object specific to the hw_accel.
 *  blogRule_p  : Pointer to the blogRule_t object
 *------------------------------------------------------------------------------
 */
void blog_rule_hw_formatted_dump(blogRule_t *blogRule_p)
{
    int action_idx = 0;

    printk("filter={");
    printk("nbr_of_vlan_tags=%d,",    blogRule_p->filter.nbrOfVlanTags);
    printk("has_pppoe_header=%d,",    blogRule_p->filter.hasPppoeHeader);
    printk("ipv4_tos_mask=%d,",       blogRule_p->filter.ipv4.mask.tos);
    printk("ipv4_ip_proto_mask=%d,",  blogRule_p->filter.ipv4.mask.ip_proto);
    printk("ipv4_tos_value=%d,",      blogRule_p->filter.ipv4.value.tos);
    printk("ipv4_ip_proto_value=%d,", blogRule_p->filter.ipv4.value.ip_proto);
    printk("ipv6_tclass_mask=%d,",    blogRule_p->filter.ipv6.mask.tclass);
    printk("ipv6_next_hdr_mask=%d,",  blogRule_p->filter.ipv6.mask.nxtHdr);
    printk("ipv6_tclass_value=%d,",   blogRule_p->filter.ipv6.value.tclass);
    printk("ipv6_next_hdr_value=%d,", blogRule_p->filter.ipv6.value.nxtHdr);

    printk("skb_priority=%d,",        blogRule_p->filter.skb.priority);
    printk("skb_mark_flow_id=%d,",    blogRule_p->filter.skb.markFlowId);
    printk("skb_mark_port=%d,",       blogRule_p->filter.skb.markPort);
    printk("flags=%d},",              blogRule_p->filter.flags);

    printk("action_count=%d",         blogRule_p->actionCount);

    if (action_idx < blogRule_p->actionCount)
    {
        printk(",action=");
        while(action_idx < blogRule_p->actionCount)
        {
            printk("%02X", blogRule_p->action[action_idx].cmd);
            printk("%02X", blogRule_p->action[action_idx].toTag);

            switch(blogRule_p->action[action_idx].cmd)
            {
                ...
                case BLOG_RULE_CMD_SET_STA_MAC_ADDRESS:
                    ...
                    printk("%pm", blogRule_p->action[action_idx].macAddr);
                    break;
                ...
                case BLOG_RULE_CMD_SET_SKB_MARK_QUEUE:
                    ...
                    printk("%04X", blogRule_p->action[action_idx].arg);
                    break;
            }
...
/*
 *------------------------------------------------------------------------------
 * Function     : blog_rule_list_hw_formatted_dump
 * Description  : Dump the contents of a linked list of blogRule objects specific to the hw_accel.
 *  rule_p      : void Pointer to the blogRule_t object
 *------------------------------------------------------------------------------
 */
void blog_rule_list_hw_formatted_dump(void *rule_p)
{
	blogRule_t *blogRule_p = (blogRule_t*) rule_p;
	int count = 0;
    ...
    while (blogRule_p)
    {
		if (count++ > 0)
			printk(",");
        printk("blog_rule={");
        blog_rule_hw_formatted_dump(blogRule_p);
        printk("}");
...
int blog_rule_add_action(blogRule_t *blogRule_p, blogRuleAction_t *action_p)
{
    ...
    {
        printk("ERROR : Maximum number of actions reached for blogRule_p <%px>\n",
               blogRule_p);
