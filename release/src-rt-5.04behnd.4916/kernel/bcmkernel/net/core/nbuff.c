...
#if defined(CC_CONFIG_FKB_STATS) || defined(CONFIG_BCM_NBUFF_FKB_POOL) 
...
void fkb_free(FkBuff_t * fkb_p)
{
    ...
    {
        bool data_hw_recycle_done = fkb_p->data_hw_recycle_done;
        fkbM_p = fkb_p->master_p;
        fkbC_recycle(fkb_p);
        if(data_hw_recycle_done)
            return;
    }
    else
    {
        fkbM_p = fkb_p;
        if(fkbM_p->data_hw_recycle_done)
            return;
    }
...
#if IS_ENABLED(CONFIG_BCM_BPM)
        if(fkbM_p->data_hw_recycle_capable)
        {
            gbpm_recycle_hw_buf(fkbM_p, fkbM_p->recycle_context);
            return;
        }
#endif
...
#if IS_ENABLED(CONFIG_BCM_BPM)
            fkbM_p->data_hw_recycle_capable = gbpm_is_buf_hw_recycle_capable(fkbM_p);
#endif
 ...
 *------------------------------------------------------------------------------
 */
void fkb_preinit(uint8_t * pBuf, RecycleFuncP recycle_hook,
                 unsigned long recycle_context)
{
    ...
    fkb_p->len_word = 0;
#if IS_ENABLED(CONFIG_BCM_BPM)
    fkb_p->data_hw_recycle_capable = gbpm_is_buf_hw_recycle_capable(fkb_p);
#endif
    fkb_p->mark = 0;
    fkb_p->priority = 0;
    fkb_set_ref(fkb_p, 0);
 ...
 *------------------------------------------------------------------------------
 */
FkBuff_t * _fkb_init(uint8_t * pBuf, uint32_t headroom,
...
/*
 * Function   : nbuff_invalidatefree
 * Description: Invalidate and free/recycle a network buffer.
 * If the data buffer was referenced by a single network buffer, then the data
 * buffer will also be freed/recycled. 
 * For SKBs, the invalidate is performed by the recycle function. 
 * Assumption: The nbuff ref count/users is 1 and is not shared as the invalidate 
 * can reload the data from DDR. 
 */
static inline void nbuff_invalidatefree_ex(pNBuff_t pNBuff, int in_thread)
{
    void * pBuf = PNBUFF_2_PBUF(pNBuff);
    fkb_dbg(1, "pNBuff<%px> pBuf<%px>", pNBuff, pBuf);
    if ( IS_SKBUFF_PTR(pNBuff) )
    ...
    {
        FkBuff_t * fkb_p = (FkBuff_t *)pBuf;
        fkb_flush(fkb_p, fkb_p->data, fkb_p->len, FKB_CACHE_INV);
        fkb_free(fkb_p);
    }
    fkb_dbg(2, "<<");
}
...
void nbuff_invalidatefree(pNBuff_t pNBuff)
{
#if defined(CONFIG_BCM_PON)
    nbuff_invalidatefree_ex(pNBuff, 0);
#else
    nbuff_invalidatefree_ex(pNBuff, 1);
#endif
}
 ...
 *
 */
FkBuff_t * _fkb_clone(FkBuff_t *fkb_p)
{
    FkBuff_t *fkbM_p;
    ...
    if ( !is_dptr_tag_(fkb_p->dirty_p ) && _is_in_skb_tag_(fkb_p->ptr, fkb_p->flags) )
    {
        /* FKB Alloc and copy */
        fkbM_p = fkb_alloc( FkbMaster_e );
        if (fkbM_p == FKB_NULL)
...
EXPORT_SYMBOL(fkb_preinit);
EXPORT_SYMBOL(_fkb_init);
...
EXPORT_SYMBOL(nbuff_invalidatefree);
EXPORT_SYMBOL(_fkb_clone);
