                     ...
                     unsigned int hooknum)
{
   ...
   esphdr = skb_header_pointer(skb, dataoff, sizeof(_esphdr), &_esphdr);
   if (esphdr == NULL) {
      nf_l4proto_log_invalid(skb, net, pf, IPPROTO_ESP, "null esp header");
      return -NF_ACCEPT;
   }
   return NF_ACCEPT;
}
...
{
   return nf_ct_l4proto_register_one(&nf_conntrack_l4proto_esp4);
}

void __exit nf_ct_proto_esp_fini(void)
{
   nf_ct_l4proto_unregister_one(&nf_conntrack_l4proto_esp4);
}
