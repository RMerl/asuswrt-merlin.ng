...
# echo a 1 100 200 2 300 400 eth0.1 > /proc/net/nf_nat_pt
# echo a 1 6000 6000 3 8000 8001 ppp0 > /proc/net/nf_nat_pt

// Read entry
# cat /proc/net/nf_nat_pt
...
ssize_t nat_pt_read_proc(struct file *file, char __user *buf, size_t size, loff_t *offset)
{
    ...
    p = np = buf;
    np += sprintf(str1, "idx: trigger port(protocol,start,end) : open port(protocol,start,end)        intf\n");
    if (copy_to_user(p, str1, sizeof(str1)))
    ...
    for (i = 0; i < max_idx_used; i++) {
        ...
        {
            np += sprintf(str1, "%3d:             %2d %6d %6d              %2d %6d %6d                %s\n", 
    ...
    np += sprintf(str1, "total_outport_cnt=%d, total_inport_cnt=%d\n", total_outport_cnt, total_inport_cnt);
...
static struct proc_ops proc_fops = {
	 .proc_read    = nat_pt_read_proc,
	 .proc_write   = nat_pt_write_proc,
};
#else
