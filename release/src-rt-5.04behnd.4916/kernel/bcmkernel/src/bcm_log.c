...
static struct proc_ops log_proc_fops = {
    .proc_read = log_proc_read,
...
int exp_gpio_request(int gpio, char *name)
{
    return gpio_request(gpio, name);
}

int exp_gpio_dir_out(int gpio, int val)
{
...
int __init bcmLog_init( void ) {
    ...
    bcmFun_reg(BCM_FUN_ID_GPIO_REQUEST, (bcmFun_t *)exp_gpio_request);
    bcmFun_reg(BCM_FUN_ID_GPIO_DIR_OUT, (bcmFun_t *)exp_gpio_dir_out);
