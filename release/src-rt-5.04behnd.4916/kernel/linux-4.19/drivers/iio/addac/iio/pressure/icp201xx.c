...
static irqreturn_t icp201xx_read_measurement(int irq, void *p)
{
	...
	ret = icp201xx_get_fifo_count(st, &fifo_cnt);
	//pr_info("ICP201XX, fifo_cnt : %d", fifo_cnt);

	if(fifo_cnt > 0) {
		ret = icp201xx_get_fifo_data(st, fifo_cnt, fifo_data);
	...
	if (ret) //bus transfer error, set pressure/temperature = 0
	{
		data.pressure = 0;
		data.temp = 0;
	}
	...
	//pr_info("ICP201XX, pressure raw : %d", (int)data.pressure);
	//pr_info("ICP201XX, temperature raw : %d", (int)data.temp);
