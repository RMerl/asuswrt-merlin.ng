...
bdmf_error_t ag_drv_dma_config_max_otf_set(uint8_t dma_id, uint16_t max_ddr, uint16_t max_sram)
{
    ...
    reg_config_max_otf = RU_FIELD_SET(dma_id, DMA, CONFIG_MAX_OTF, MAX_DDR, reg_config_max_otf, max_ddr);
    reg_config_max_otf = RU_FIELD_SET(dma_id, DMA, CONFIG_MAX_OTF, MAX_SRAM, reg_config_max_otf, max_sram);
...
bdmf_error_t ag_drv_dma_config_max_otf_get(uint8_t dma_id, uint16_t *max_ddr, uint16_t *max_sram)
{
    ...
    *max_ddr = RU_FIELD_GET(dma_id, DMA, CONFIG_MAX_OTF, MAX_DDR, reg_config_max_otf);
    *max_sram = RU_FIELD_GET(dma_id, DMA, CONFIG_MAX_OTF, MAX_SRAM, reg_config_max_otf);
...
static int ag_drv_dma_cli_set(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    ...
    case cli_dma_config_max_otf:
        ag_err = ag_drv_dma_config_max_otf_set(parm[1].value.unumber, parm[2].value.unumber, parm[3].value.unumber);
        break;
...
int bcm_dma_cli_get(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    ...
    case cli_dma_config_max_otf:
    {
        uint16_t max_ddr;
        uint16_t max_sram;
        ag_err = ag_drv_dma_config_max_otf_get(parm[1].value.unumber, &max_ddr, &max_sram);
...
static int ag_drv_dma_cli_test(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
...

    {
        ...
        bdmf_session_print(session, "ag_drv_dma_config_max_otf_set(%u %u %u)\n", dma_id,
            max_ddr, max_sram);
        ag_err = ag_drv_dma_config_max_otf_set(dma_id, max_ddr, max_sram);
        if (!ag_err)
            ag_err = ag_drv_dma_config_max_otf_get(dma_id, &max_ddr, &max_sram);

        if (!ag_err)
        {
            bdmf_session_print(session, "ag_drv_dma_config_max_otf_get(%u %u %u)\n", dma_id,
                max_ddr, max_sram);
        }
        else
        {
...
bdmfmon_handle_t ag_drv_dma_cli_init(bdmfmon_handle_t root_dir)
{
    ...
    }
    {
        ...
        static bdmfmon_enum_val_t selector_table[] = {
            ...
            { .name = "config_max_otf", .val = cli_dma_config_max_otf, .parms = get_default },
