...
bdmf_error_t ag_drv_rnr_quad_general_config_aqm_random_test_inc_get(uint8_t quad_idx, bdmf_boolean *random_inc)
{
    uint32_t reg_general_config_aqm_random_test_inc;
    ...
    RU_REG_READ(quad_idx, RNR_QUAD, GENERAL_CONFIG_AQM_RANDOM_TEST_INC, reg_general_config_aqm_random_test_inc);

    *random_inc = RU_FIELD_GET(quad_idx, RNR_QUAD, GENERAL_CONFIG_AQM_RANDOM_TEST_INC, RANDOM_INC, reg_general_config_aqm_random_test_inc);
...
int bcm_rnr_quad_cli_get(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    ...
    case cli_rnr_quad_general_config_aqm_random_test_inc:
    {
        bdmf_boolean random_inc;
        ag_err = ag_drv_rnr_quad_general_config_aqm_random_test_inc_get(parm[1].value.unumber, &random_inc);
        bdmf_session_print(session, "random_inc = %u = 0x%x\n", random_inc, random_inc);
        break;
    }
    case cli_rnr_quad_general_config_multi_psel_cfg:
    {
...
static int ag_drv_rnr_quad_cli_test(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
...

    {
        ...
        if (!ag_err)
            ag_err = ag_drv_rnr_quad_general_config_aqm_random_test_inc_get(quad_idx, &random_inc);

        if (!ag_err)
        {
            bdmf_session_print(session, "ag_drv_rnr_quad_general_config_aqm_random_test_inc_get(%u %u)\n", quad_idx,
                random_inc);
        }
        else
        {
...

    {
        ...
        bdmf_session_print(session, "ag_drv_rnr_quad_debug_fifo_config_set(%u %u %u %u %u %u %u)\n", quad_idx,
            debug_fifo_config.psram_hdr_sw_rst, debug_fifo_config.psram_data_sw_rst, debug_fifo_config.ddr_hdr_sw_rst, debug_fifo_config.psram_hdr_sw_rd_addr, 
        ...
        if (!ag_err)
        {
            bdmf_session_print(session, "ag_drv_rnr_quad_debug_fifo_config_get(%u %u %u %u %u %u %u)\n", quad_idx,
                debug_fifo_config.psram_hdr_sw_rst, debug_fifo_config.psram_data_sw_rst, debug_fifo_config.ddr_hdr_sw_rst, debug_fifo_config.psram_hdr_sw_rd_addr, 
                debug_fifo_config.psram_data_sw_rd_addr, debug_fifo_config.ddr_hdr_sw_rd_addr);
        }
        else
        ...
        if (debug_fifo_config.psram_hdr_sw_rst != gtmv(m, 1) || debug_fifo_config.psram_data_sw_rst != gtmv(m, 1) || debug_fifo_config.ddr_hdr_sw_rst != gtmv(m, 1) || debug_fifo_config.psram_hdr_sw_rd_addr != gtmv(m, 4) || debug_fifo_config.psram_data_sw_rd_addr != gtmv(m, 4) || debug_fifo_config.ddr_hdr_sw_rd_addr != gtmv(m, 4))
        {
            test_failure_cnt++;
            bdmf_session_print(session, "  register failed match; failures=%u\n", test_failure_cnt);        }
        else
...
bdmfmon_handle_t ag_drv_rnr_quad_cli_init(bdmfmon_handle_t root_dir)
{
...

    {
        ...
        static bdmfmon_cmd_parm_t set_debug_fifo_config[] = {
            BDMFMON_MAKE_PARM("quad_idx", "quad_idx", BDMFMON_PARM_UNUMBER, 0),
            BDMFMON_MAKE_PARM("psram_hdr_sw_rst", "psram_hdr_sw_rst", BDMFMON_PARM_UNUMBER, 0),
            BDMFMON_MAKE_PARM("psram_data_sw_rst", "psram_data_sw_rst", BDMFMON_PARM_UNUMBER, 0),
            BDMFMON_MAKE_PARM("ddr_hdr_sw_rst", "ddr_hdr_sw_rst", BDMFMON_PARM_UNUMBER, 0),
    ...
    }
    {
        ...
        static bdmfmon_enum_val_t selector_table[] = {
            ...
            { .name = "general_config_aqm_random_test_inc", .val = cli_rnr_quad_general_config_aqm_random_test_inc, .parms = get_default },
