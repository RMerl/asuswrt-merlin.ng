...
static int ag_drv_rnr_pred_cli_set(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    bdmf_error_t ag_err;
    ...
    case cli_rnr_pred_mem_entry:
        ag_err = ag_drv_rnr_pred_mem_entry_set(parm[1].value.unumber, parm[2].value.unumber, parm[3].value.unumber);
        break;
    default:
...
int bcm_rnr_pred_cli_get(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    ...
    {
        uint16_t pred_mem;
        ag_err = ag_drv_rnr_pred_mem_entry_get(parm[1].value.unumber, parm[2].value.unumber, &pred_mem);
...
static int ag_drv_rnr_pred_cli_test(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
...

    {
        ...
        bdmf_session_print(session, "ag_drv_rnr_pred_mem_entry_set(%u %u %u)\n", rnr_id, word_index,
            pred_mem);
        ag_err = ag_drv_rnr_pred_mem_entry_set(rnr_id, word_index, pred_mem);
        if (!ag_err)
            ag_err = ag_drv_rnr_pred_mem_entry_get(rnr_id, word_index, &pred_mem);

        if (!ag_err)
        {
            bdmf_session_print(session, "ag_drv_rnr_pred_mem_entry_get(%u %u %u)\n", rnr_id, word_index,
                pred_mem);
        }
        else
        {
...
#ifdef HAL_DRV_TEST_ENABLE
static int ag_drv_rnr_pred_cli_ext_test(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
...
#ifdef HAL_DRV_TEST_ENABLE
static int ag_drv_rnr_pred_cli_address(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
...
bdmfmon_handle_t ag_drv_rnr_pred_cli_init(bdmfmon_handle_t root_dir)
{
    ...
    dir = bdmfmon_dir_add(root_dir, "rnr_pred", "rnr_pred", BDMF_ACCESS_ADMIN, NULL);
    BUG_ON(dir == NULL);

    {
        static bdmfmon_cmd_parm_t set_mem_entry[] = {
            ...
            BDMFMON_MAKE_PARM("pred_mem", "pred_mem", BDMFMON_PARM_UNUMBER, 0),
        ...
        BDMFMON_MAKE_CMD(dir, "set", "set", ag_drv_rnr_pred_cli_set,
    ...
    }
    {
        ...
        static bdmfmon_enum_val_t selector_table[] = {
            { .name = "mem_entry", .val = cli_rnr_pred_mem_entry, .parms = get_mem_entry },
            BDMFMON_ENUM_LAST
        };
...
#ifdef HAL_DRV_TEST_ENABLE
    {
        ...
        BDMFMON_MAKE_PARM(dir, "test", "test", ag_drv_rnr_pred_cli_test,
...
#ifdef HAL_DRV_TEST_ENABLE
    {
        ...
        static bdmfmon_enum_val_t selector_table[] = {
            { .name = "mem_entry", .val = cli_rnr_pred_mem_entry, .parms = ext_test_default},
            BDMFMON_ENUM_LAST
        };
        static bdmfmon_enum_val_t enum_table_test_method[] = {
            { .name = "low", .val = ag_drv_cli_test_method_low },
        ...
        BDMFMON_MAKE_PARM(dir, "ext_test", "ext_test", ag_drv_rnr_pred_cli_ext_test,
...
#ifdef HAL_DRV_TEST_ENABLE
    {
        ...
        BDMFMON_MAKE_PARM(dir, "address", "address", bcm_rnr_pred_cli_address,
