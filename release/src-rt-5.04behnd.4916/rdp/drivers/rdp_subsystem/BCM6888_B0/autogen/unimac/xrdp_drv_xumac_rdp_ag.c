...
bdmf_error_t ag_drv_xumac_rdp_ipg_hd_bkp_cntl_set(uint8_t xumac_id, bdmf_boolean hd_fc_ena, bdmf_boolean hd_fc_bkoff_ok, uint8_t ipg_config_rx, bdmf_boolean hd_random_jam)
{
    ...
    reg_ipg_hd_bkp_cntl = RU_FIELD_SET(xumac_id, XUMAC_RDP, IPG_HD_BKP_CNTL, HD_RANDOM_JAM, reg_ipg_hd_bkp_cntl, hd_random_jam);
...
bdmf_error_t ag_drv_xumac_rdp_ipg_hd_bkp_cntl_get(uint8_t xumac_id, bdmf_boolean *hd_fc_ena, bdmf_boolean *hd_fc_bkoff_ok, uint8_t *ipg_config_rx, bdmf_boolean *hd_random_jam)
{
...
#ifdef VALIDATE_PARMS
    if (!hd_fc_ena || !hd_fc_bkoff_ok || !ipg_config_rx || !hd_random_jam)
    {
    ...
    *hd_random_jam = RU_FIELD_GET(xumac_id, XUMAC_RDP, IPG_HD_BKP_CNTL, HD_RANDOM_JAM, reg_ipg_hd_bkp_cntl);
...
bdmf_error_t ag_drv_xumac_rdp_umac_eee_ctrl_set(uint8_t xumac_id, const xumac_rdp_umac_eee_ctrl *umac_eee_ctrl)
{
    ...
    if ((xumac_id >= BLOCK_ADDR_COUNT) ||
       ...
       (umac_eee_ctrl->lpi_clock_gating_en >= _1BITS_MAX_VAL_))
    {
        bdmf_trace("ERROR driver %s:%u| err=%s (%d)\n", __FILE__, __LINE__, bdmf_strerror(BDMF_ERR_RANGE), BDMF_ERR_RANGE);
        return BDMF_ERR_RANGE;
    }
    ...
    reg_umac_eee_ctrl = RU_FIELD_SET(xumac_id, XUMAC_RDP, UMAC_EEE_CTRL, LPI_CLOCK_GATING_EN, reg_umac_eee_ctrl, umac_eee_ctrl->lpi_clock_gating_en);
...
bdmf_error_t ag_drv_xumac_rdp_umac_eee_ctrl_get(uint8_t xumac_id, xumac_rdp_umac_eee_ctrl *umac_eee_ctrl)
{
    ...
    umac_eee_ctrl->lpi_clock_gating_en = RU_FIELD_GET(xumac_id, XUMAC_RDP, UMAC_EEE_CTRL, LPI_CLOCK_GATING_EN, reg_umac_eee_ctrl);
...
bdmf_error_t ag_drv_xumac_rdp_tx_idle_stuffing_control_set(uint8_t xumac_id, uint8_t tx_idle_stuffing_ctrl, bdmf_boolean idle_stuffing_mode, uint8_t tx_start_threshold_offset, bdmf_boolean tx_start_on_eop)
{
    ...
    if ((xumac_id >= BLOCK_ADDR_COUNT) ||
       ...
       (idle_stuffing_mode >= _1BITS_MAX_VAL_) ||
       (tx_start_threshold_offset >= _3BITS_MAX_VAL_) ||
       (tx_start_on_eop >= _1BITS_MAX_VAL_))
    {
        bdmf_trace("ERROR driver %s:%u| err=%s (%d)\n", __FILE__, __LINE__, bdmf_strerror(BDMF_ERR_RANGE), BDMF_ERR_RANGE);
        return BDMF_ERR_RANGE;
    }
    ...
    reg_tx_idle_stuffing_control = RU_FIELD_SET(xumac_id, XUMAC_RDP, TX_IDLE_STUFFING_CONTROL, IDLE_STUFFING_MODE, reg_tx_idle_stuffing_control, idle_stuffing_mode);
    reg_tx_idle_stuffing_control = RU_FIELD_SET(xumac_id, XUMAC_RDP, TX_IDLE_STUFFING_CONTROL, TX_START_THRESHOLD_OFFSET, reg_tx_idle_stuffing_control, tx_start_threshold_offset);
    reg_tx_idle_stuffing_control = RU_FIELD_SET(xumac_id, XUMAC_RDP, TX_IDLE_STUFFING_CONTROL, TX_START_ON_EOP, reg_tx_idle_stuffing_control, tx_start_on_eop);
...
bdmf_error_t ag_drv_xumac_rdp_tx_idle_stuffing_control_get(uint8_t xumac_id, uint8_t *tx_idle_stuffing_ctrl, bdmf_boolean *idle_stuffing_mode, uint8_t *tx_start_threshold_offset, bdmf_boolean *tx_start_on_eop)
{
...
#ifdef VALIDATE_PARMS
    if (!tx_idle_stuffing_ctrl || !idle_stuffing_mode || !tx_start_threshold_offset || !tx_start_on_eop)
    {
        bdmf_trace("ERROR driver %s:%u| err=%s (%d)\n", __FILE__, __LINE__, bdmf_strerror(BDMF_ERR_PARM), BDMF_ERR_PARM);
        return BDMF_ERR_PARM;
    }
    ...
    *idle_stuffing_mode = RU_FIELD_GET(xumac_id, XUMAC_RDP, TX_IDLE_STUFFING_CONTROL, IDLE_STUFFING_MODE, reg_tx_idle_stuffing_control);
    *tx_start_threshold_offset = RU_FIELD_GET(xumac_id, XUMAC_RDP, TX_IDLE_STUFFING_CONTROL, TX_START_THRESHOLD_OFFSET, reg_tx_idle_stuffing_control);
    *tx_start_on_eop = RU_FIELD_GET(xumac_id, XUMAC_RDP, TX_IDLE_STUFFING_CONTROL, TX_START_ON_EOP, reg_tx_idle_stuffing_control);
...
static int ag_drv_xumac_rdp_cli_set(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    ...
    case cli_xumac_rdp_ipg_hd_bkp_cntl:
        ag_err = ag_drv_xumac_rdp_ipg_hd_bkp_cntl_set(parm[1].value.unumber, parm[2].value.unumber, parm[3].value.unumber, parm[4].value.unumber, parm[5].value.unumber);
        break;
    ...
    {
        xumac_rdp_umac_eee_ctrl umac_eee_ctrl = { .eee_en = parm[2].value.unumber, .rx_fifo_check = parm[3].value.unumber, .eee_txclk_dis = parm[4].value.unumber, .dis_eee_10m = parm[5].value.unumber, .lp_idle_prediction_mode = parm[6].value.unumber, .lpi_clock_gating_en = parm[7].value.unumber};
    ...
    case cli_xumac_rdp_tx_idle_stuffing_control:
        ag_err = ag_drv_xumac_rdp_tx_idle_stuffing_control_set(parm[1].value.unumber, parm[2].value.unumber, parm[3].value.unumber, parm[4].value.unumber, parm[5].value.unumber);
        break;
...
int bcm_xumac_rdp_cli_get(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    ...
    case cli_xumac_rdp_ipg_hd_bkp_cntl:
    {
        ...
        bdmf_boolean hd_random_jam;
        ag_err = ag_drv_xumac_rdp_ipg_hd_bkp_cntl_get(parm[1].value.unumber, &hd_fc_ena, &hd_fc_bkoff_ok, &ipg_config_rx, &hd_random_jam);
        ...
        bdmf_session_print(session, "hd_random_jam = %u = 0x%x\n", hd_random_jam, hd_random_jam);
        break;
    }
    case cli_xumac_rdp_command_config:
    {
    ...
    case cli_xumac_rdp_umac_eee_ctrl:
    {
        ...
        bdmf_session_print(session, "lpi_clock_gating_en = %u = 0x%x\n", umac_eee_ctrl.lpi_clock_gating_en, umac_eee_ctrl.lpi_clock_gating_en);
        break;
    }
    case cli_xumac_rdp_mii_eee_delay_entry_timer:
    {
    ...
    case cli_xumac_rdp_tx_idle_stuffing_control:
    {
        ...
        bdmf_boolean idle_stuffing_mode;
        uint8_t tx_start_threshold_offset;
        bdmf_boolean tx_start_on_eop;
        ag_err = ag_drv_xumac_rdp_tx_idle_stuffing_control_get(parm[1].value.unumber, &tx_idle_stuffing_ctrl, &idle_stuffing_mode, &tx_start_threshold_offset, &tx_start_on_eop);
        bdmf_session_print(session, "tx_idle_stuffing_ctrl = %u = 0x%x\n", tx_idle_stuffing_ctrl, tx_idle_stuffing_ctrl);
        bdmf_session_print(session, "idle_stuffing_mode = %u = 0x%x\n", idle_stuffing_mode, idle_stuffing_mode);
        bdmf_session_print(session, "tx_start_threshold_offset = %u = 0x%x\n", tx_start_threshold_offset, tx_start_threshold_offset);
        bdmf_session_print(session, "tx_start_on_eop = %u = 0x%x\n", tx_start_on_eop, tx_start_on_eop);
        break;
    }
    case cli_xumac_rdp_actual_data_rate:
    {
...
static int ag_drv_xumac_rdp_cli_test(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
...

    {
        ...
        bdmf_boolean hd_random_jam = gtmv(m, 1);
        bdmf_session_print(session, "ag_drv_xumac_rdp_ipg_hd_bkp_cntl_set(%u %u %u %u %u)\n", xumac_id,
            hd_fc_ena, hd_fc_bkoff_ok, ipg_config_rx, hd_random_jam);
        ag_err = ag_drv_xumac_rdp_ipg_hd_bkp_cntl_set(xumac_id, hd_fc_ena, hd_fc_bkoff_ok, ipg_config_rx, hd_random_jam);
        if (!ag_err)
            ag_err = ag_drv_xumac_rdp_ipg_hd_bkp_cntl_get(xumac_id, &hd_fc_ena, &hd_fc_bkoff_ok, &ipg_config_rx, &hd_random_jam);

        if (!ag_err)
        {
            bdmf_session_print(session, "ag_drv_xumac_rdp_ipg_hd_bkp_cntl_get(%u %u %u %u %u)\n", xumac_id,
                hd_fc_ena, hd_fc_bkoff_ok, ipg_config_rx, hd_random_jam);
        }
        else
        {
        ...
        if (hd_fc_ena != gtmv(m, 1) || hd_fc_bkoff_ok != gtmv(m, 1) || ipg_config_rx != gtmv(m, 5) || hd_random_jam != gtmv(m, 1))
        {
            test_failure_cnt++;
    ...
    {
        xumac_rdp_umac_eee_ctrl umac_eee_ctrl = {.eee_en = gtmv(m, 1), .rx_fifo_check = gtmv(m, 1), .eee_txclk_dis = gtmv(m, 1), .dis_eee_10m = gtmv(m, 1), .lp_idle_prediction_mode = gtmv(m, 1), .lpi_clock_gating_en = gtmv(m, 1)};
        bdmf_session_print(session, "ag_drv_xumac_rdp_umac_eee_ctrl_set(%u %u %u %u %u %u %u)\n", xumac_id,
            umac_eee_ctrl.eee_en, umac_eee_ctrl.rx_fifo_check, umac_eee_ctrl.eee_txclk_dis, umac_eee_ctrl.dis_eee_10m, 
            umac_eee_ctrl.lp_idle_prediction_mode, umac_eee_ctrl.lpi_clock_gating_en);
        ...
        if (!ag_err)
        {
            bdmf_session_print(session, "ag_drv_xumac_rdp_umac_eee_ctrl_get(%u %u %u %u %u %u %u)\n", xumac_id,
                umac_eee_ctrl.eee_en, umac_eee_ctrl.rx_fifo_check, umac_eee_ctrl.eee_txclk_dis, umac_eee_ctrl.dis_eee_10m, 
                umac_eee_ctrl.lp_idle_prediction_mode, umac_eee_ctrl.lpi_clock_gating_en);
        }
        else
        ...
        if (umac_eee_ctrl.eee_en != gtmv(m, 1) || umac_eee_ctrl.rx_fifo_check != gtmv(m, 1) || umac_eee_ctrl.eee_txclk_dis != gtmv(m, 1) || umac_eee_ctrl.dis_eee_10m != gtmv(m, 1) || umac_eee_ctrl.lp_idle_prediction_mode != gtmv(m, 1) || umac_eee_ctrl.lpi_clock_gating_en != gtmv(m, 1))
        {
            test_failure_cnt++;
            bdmf_session_print(session, "  register failed match; failures=%u\n", test_failure_cnt);        }
        else
...

    {
        ...
        bdmf_boolean idle_stuffing_mode = gtmv(m, 1);
        uint8_t tx_start_threshold_offset = gtmv(m, 3);
        bdmf_boolean tx_start_on_eop = gtmv(m, 1);
        bdmf_session_print(session, "ag_drv_xumac_rdp_tx_idle_stuffing_control_set(%u %u %u %u %u)\n", xumac_id,
            tx_idle_stuffing_ctrl, idle_stuffing_mode, tx_start_threshold_offset, tx_start_on_eop);
        ag_err = ag_drv_xumac_rdp_tx_idle_stuffing_control_set(xumac_id, tx_idle_stuffing_ctrl, idle_stuffing_mode, tx_start_threshold_offset, tx_start_on_eop);
        if (!ag_err)
            ag_err = ag_drv_xumac_rdp_tx_idle_stuffing_control_get(xumac_id, &tx_idle_stuffing_ctrl, &idle_stuffing_mode, &tx_start_threshold_offset, &tx_start_on_eop);

        if (!ag_err)
        {
            bdmf_session_print(session, "ag_drv_xumac_rdp_tx_idle_stuffing_control_get(%u %u %u %u %u)\n", xumac_id,
                tx_idle_stuffing_ctrl, idle_stuffing_mode, tx_start_threshold_offset, tx_start_on_eop);
        }
        else
        {
        ...
        if (tx_idle_stuffing_ctrl != gtmv(m, 4) || idle_stuffing_mode != gtmv(m, 1) || tx_start_threshold_offset != gtmv(m, 3) || tx_start_on_eop != gtmv(m, 1))
        {
            test_failure_cnt++;
            bdmf_session_print(session, "  register failed match; failures=%u\n", test_failure_cnt);        }
        else
...
bdmfmon_handle_t ag_drv_xumac_rdp_cli_init(bdmfmon_handle_t root_dir)
{
...

    {
        static bdmfmon_cmd_parm_t set_ipg_hd_bkp_cntl[] = {
            ...
            BDMFMON_MAKE_PARM("hd_random_jam", "hd_random_jam", BDMFMON_PARM_UNUMBER, 0),
        ...
        static bdmfmon_cmd_parm_t set_umac_eee_ctrl[] = {
            ...
            BDMFMON_MAKE_PARM("lpi_clock_gating_en", "lpi_clock_gating_en", BDMFMON_PARM_UNUMBER, 0),
            BDMFMON_PARM_LIST_TERMINATOR
        };
        ...
        static bdmfmon_cmd_parm_t set_tx_idle_stuffing_control[] = {
            ...
            BDMFMON_MAKE_PARM("idle_stuffing_mode", "idle_stuffing_mode", BDMFMON_PARM_UNUMBER, 0),
            BDMFMON_MAKE_PARM("tx_start_threshold_offset", "tx_start_threshold_offset", BDMFMON_PARM_UNUMBER, 0),
            BDMFMON_MAKE_PARM("tx_start_on_eop", "tx_start_on_eop", BDMFMON_PARM_UNUMBER, 0),
            BDMFMON_PARM_LIST_TERMINATOR
        };
