...
typedef enum {
    ...
    rdpa_ip_class_attr_key_type = 7, /* key_type : RW : enum : IP class key type */
    rdpa_ip_class_attr_pathstat = 8, /* pathstat : RF : aggregate[] rdpa_stat(rdpa_stat_t) : Ip class path entry statistics */
    rdpa_ip_class_attr_tcp_ack_prio = 9, /* tcp_ack_prio : RW : bool : TCP pure ACK prioritization (common for L3 and L2) */
    rdpa_ip_class_attr_tos_mflows = 10, /* tos_mflows : RW : bool : TOS multi-flows (common for L3 and L2) */
    rdpa_ip_class_attr_pbit_key_mode = 11, /* pbit_key_mode : RI : bool : create ucast keys with VLAN's pbit - enable only via scratchpad key PbitKeyMode */
...
static inline int rdpa_ip_class_tos_mflows_get(bdmf_object_handle mo_, bdmf_boolean *tos_mflows_)
{
    bdmf_number _nn_;
    int _rc_;
    _rc_ = bdmf_attr_get_as_num(mo_, rdpa_ip_class_attr_tos_mflows, &_nn_);
...
{
    return bdmf_attr_set_as_num(mo_, rdpa_ip_class_attr_tos_mflows, tos_mflows_);
}
...
static inline int rdpa_ip_class_pbit_key_mode_get(bdmf_object_handle mo_, bdmf_boolean *pbit_key_mode_)
{
    bdmf_number _nn_;
    int _rc_;
    _rc_ = bdmf_attr_get_as_num(mo_, rdpa_ip_class_attr_pbit_key_mode, &_nn_);
    *pbit_key_mode_ = (bdmf_boolean)_nn_;
    return _rc_;
}
...
{
    return bdmf_attr_set_as_num(mo_, rdpa_ip_class_attr_pbit_key_mode, pbit_key_mode_);
}
