...
typedef enum {
    ...
    rdpa_tcont_attr_pbit_to_queue = 6, /* pbit_to_queue : R : object : Handle to linked pbit to queue object */
...
/** Get tcont/pbit_to_queue attribute.
 *
 ...
 * The function can be called in task context only.
 */
static inline int rdpa_tcont_pbit_to_queue_get(bdmf_object_handle mo_, bdmf_object_handle *pbit_to_queue_)
{
    bdmf_number _nn_;
    int _rc_;
    _rc_ = bdmf_attr_get_as_num(mo_, rdpa_tcont_attr_pbit_to_queue, &_nn_);
