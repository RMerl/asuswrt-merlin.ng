                                         ...
                                         uint32_t  ddr_packet_headroom_size)
{
    ...
    MWRITE_32(bpm_ddr_optimized_base_ptr, RDD_RSV_VIRT_TO_PHYS(g_runner_ddr_base_addr, g_runner_ddr_base_addr_phys, 
        runner_ddr_pool_ptr + ddr_packet_headroom_size + RDD_PACKET_DDR_OFFSET));
...
int rdd_max_pkt_size_cfg(uint16_t max_pkt_size)
{
    ...
    if (max_pkt_size <= RDD_IPV6_HEADER_SIZE)
        return BDMF_ERR_PARM;

    RDD_SYSTEM_CONFIGURATION_MTU_MINUS_40_WRITE((max_pkt_size - RDD_IPV6_HEADER_SIZE), system_cfg);
