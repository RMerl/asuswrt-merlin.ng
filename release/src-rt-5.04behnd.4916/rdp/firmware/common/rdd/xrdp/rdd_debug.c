...
static void rdd_debug_prints_runner_init(uint32_t addr_hi, uint32_t addr_lo, uint32_t length)
{
    ...
    /* reset wr_ptr  */
    drv_cntr_counter_clr(CNTR_GROUP_SYNC_CTR, SYNC_CTR_GROUP_WRITE_PTR_0_DEBUG_PRINT);
    drv_cntr_counter_clr(CNTR_GROUP_SYNC_CTR, SYNC_CTR_GROUP_WRITE_PTR_1_DEBUG_PRINT);
...
{
    uint64_t cntr_arr[MAX_NUM_OF_COUNTERS_PER_READ] = {};
    ...
    drv_cntr_counter_read(CNTR_GROUP_SYNC_CTR, (SYNC_CTR_GROUP_WRITE_PTR_0_DEBUG_PRINT+rdd_debug_prints_info.double_buf_num), cntr_arr);
    write_idx = (uint32_t)cntr_arr[0];
    if (write_idx == 0)
      return;
    ...
    /* reset wr_ptr  */
    drv_cntr_counter_clr(CNTR_GROUP_SYNC_CTR, SYNC_CTR_GROUP_WRITE_PTR_0_DEBUG_PRINT+rdd_debug_prints_info.double_buf_num);
    ...
    while (buf_start_addr < buf_end_addr)
    {
      ...
      if (mark == C_STRING_PRINT) /* print from C code */
      {
        ...
        strncat(runner_debug_string_format,
          (char *)rdd_debug_all_runners_c_strings[image_id] + (swap_short(p) - BASE_ADDR),
        ...
        for (i = params_num-1; i >= 0; i--)
        {
          ...
          case 1:
            ...
            break;
          case 2:
            params[i] = (uint32_t)swap_short(p);
            p += 2;
            break;
          case 4:
            params[i] = (uint32_t)swap_int(p);
            p += 4;
            break;
          case 8:
            params[i] = (uint32_t)swap_long(p);
            p += 8;
            break;
          }
        }
      ...
      else if (mark == ASM_STRING_PRINT)
      {
        ...
        strncat(runner_debug_string_format,
          rdd_debug_get_debug_string(image_id, string_id, params_num),
          sizeof(runner_debug_string_format) - strlen(runner_debug_string_format));
      }
