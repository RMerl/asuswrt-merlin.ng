...
#else
#if (BBH_TX_WAN_CONFIGURATIONS_Q2RNR_Q0_FIELD_WIDTH == 1)
    return ag_drv_bbh_tx_wan_configurations_q2rnr_get(bbh_id_tx, q_2_rnr_index, q0, q1);
#else
    return ag_drv_bbh_tx_wan_configurations_q2rnr_get(bbh_id_tx, q_2_rnr_index, (uint8_t*)q0, (uint8_t*)q1);
#endif
#endif
}
...
#else
    return ag_drv_bbh_tx_unified_configurations_q2rnr_get(bbh_id_tx, q_2_rnr_index, (uint8_t*)q0, (uint8_t*)q1);
#endif
#endif
}
...
int drv_bbh_tx_configuration_set(uint8_t bbh_id, bbh_tx_config *config)
{
    ...
    rc = rc ? rc : ag_drv_bbh_tx_rnr_src_id_set(bbh_id, config->rnr_cfg[0].rnr_src_id,  config->rnr_cfg[1].rnr_src_id,
                                                        config->rnr_cfg[2].rnr_src_id,  config->rnr_cfg[3].rnr_src_id);
#else
    rc = rc ? rc : ag_drv_bbh_tx_rnr_src_id_set(bbh_id, config->rnr_cfg[0].rnr_src_id,  config->rnr_cfg[1].rnr_src_id);
#endif
...
#if defined(BCM4912) || defined(BCM6813)
    rc = rc ? rc : ag_drv_bbh_tx_common_configurations_rnrcfg_1_set(bbh_id, 2, config->rnr_cfg[2].tcont_addr,
        config->rnr_cfg[2].skb_addr);
    rc = rc ? rc : ag_drv_bbh_tx_common_configurations_rnrcfg_2_set(bbh_id, 2, config->rnr_cfg[2].ptr_addr,
        config->rnr_cfg[2].task_number);
    rc = rc ? rc : ag_drv_bbh_tx_common_configurations_rnrcfg_1_set(bbh_id, 3, config->rnr_cfg[3].tcont_addr,
        config->rnr_cfg[3].skb_addr);
    rc = rc ? rc : ag_drv_bbh_tx_common_configurations_rnrcfg_2_set(bbh_id, 3, config->rnr_cfg[3].ptr_addr,
        config->rnr_cfg[3].task_number);
#endif
#ifndef XRDP_BBH_PER_LAN_PORT
    rc = rc ? rc : ag_drv_bbh_tx_common_configurations_perqtask_set(bbh_id, &config->per_q_task);
#endif
#if (CHIP_VER >= RDP_GEN_61) && !defined(XRDP_MANAGE_SBPM)
    /* Needed for free truncated packets. Default should be changed in HW in the next chips */
    rc = rc ? rc : ag_drv_bbh_tx_common_configurations_sbpmcfg_get(bbh_id, &config->sbpmcfg);
    config->sbpmcfg.use_second_bn_from_pd_in_free_len_mis = 1;
    rc = rc ? rc : ag_drv_bbh_tx_common_configurations_sbpmcfg_set(bbh_id, &config->sbpmcfg);
#endif
...
int drv_bbh_tx_configuration_get(uint8_t bbh_id, bbh_tx_config *config)
{
    ...
    rc = rc ? rc : ag_drv_bbh_tx_rnr_src_id_get(bbh_id, &config->rnr_cfg[0].rnr_src_id,
        &config->rnr_cfg[1].rnr_src_id, &config->rnr_cfg[2].rnr_src_id, &config->rnr_cfg[3].rnr_src_id);
#else
    rc = rc ? rc : ag_drv_bbh_tx_rnr_src_id_get(bbh_id, &config->rnr_cfg[0].rnr_src_id,
    ...
    rc = rc ? rc : ag_drv_bbh_tx_common_configurations_rnrcfg_2_get(bbh_id, 1, &config->rnr_cfg[1].ptr_addr,
        &config->rnr_cfg[1].task_number);
#if defined(BCM4912) || defined(BCM6813)
    rc = rc ? rc : ag_drv_bbh_tx_common_configurations_rnrcfg_1_get(bbh_id, 2, &config->rnr_cfg[2].tcont_addr,
        &config->rnr_cfg[2].skb_addr);
    rc = rc ? rc : ag_drv_bbh_tx_common_configurations_rnrcfg_2_get(bbh_id, 2, &config->rnr_cfg[2].ptr_addr,
        &config->rnr_cfg[2].task_number);
    rc = rc ? rc : ag_drv_bbh_tx_common_configurations_rnrcfg_1_get(bbh_id, 3, &config->rnr_cfg[3].tcont_addr,
        &config->rnr_cfg[3].skb_addr);
    rc = rc ? rc : ag_drv_bbh_tx_common_configurations_rnrcfg_2_get(bbh_id, 3, &config->rnr_cfg[3].ptr_addr,
        &config->rnr_cfg[3].task_number);
#endif
    ...
    if(IS_WAN_TX_PORT(bbh_id))
    {
        ...
        rc = rc ? rc : ag_drv_bbh_tx_wan_configurations_stsrnrcfg_2_get(bbh_id, 1, &config->sts_rnr_cfg[1].ptr_addr,
            &config->sts_rnr_cfg[1].task_number);
#if defined(BCM4912) || defined(BCM6813)
        rc = rc ? rc : ag_drv_bbh_tx_wan_configurations_stsrnrcfg_1_get(bbh_id, 2, &config->sts_rnr_cfg[2].tcont_addr);
        rc = rc ? rc : ag_drv_bbh_tx_wan_configurations_stsrnrcfg_2_get(bbh_id, 2, &config->sts_rnr_cfg[2].ptr_addr,
            &config->sts_rnr_cfg[2].task_number);
        rc = rc ? rc : ag_drv_bbh_tx_wan_configurations_stsrnrcfg_1_get(bbh_id, 3, &config->sts_rnr_cfg[3].tcont_addr);
        rc = rc ? rc : ag_drv_bbh_tx_wan_configurations_stsrnrcfg_2_get(bbh_id, 3, &config->sts_rnr_cfg[3].ptr_addr,
            &config->sts_rnr_cfg[3].task_number);
#endif
    }
    else
    {
...
#if defined(BCM63158)
#define COMPILE_TIME_ASSERT(cond)       ({ __attribute__((unused)) int dummy = 1/(cond); })

static inline int rdd_us_channel_offset_tcont_in_range(uint32_t i)
{
    COMPILE_TIME_ASSERT(RDD_US_CHANNEL_OFFSET_TCONT == 0); /* avoid checking unsigned >= 0 */
    return (/* (i >= RDD_US_CHANNEL_OFFSET_TCONT) && */ (i <= RDD_US_CHANNEL_OFFSET_TCONT_END));
}
#endif
...
static int drv_bbh_tx_cli_occupancy_get(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    ...
    if(IS_WAN_TX_PORT(bbh_id))
    {
        ...
        else
        {
            ...
            {
#if defined(BCM63158)
                if (rdd_us_channel_offset_tcont_in_range(i))
                {
