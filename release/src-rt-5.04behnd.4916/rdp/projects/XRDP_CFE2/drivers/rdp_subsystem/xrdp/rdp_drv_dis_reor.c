...
#define DSPTCHR_CONJ_INGR (RU_BLK(DSPTCHR).addr[0] +  RU_REG_OFFSET(DSPTCHR,CONGESTION_INGRS_CONGSTN))
#define DSPTCHR_CONJ_EGRS (RU_BLK(DSPTCHR).addr[0] +  RU_REG_OFFSET(DSPTCHR,CONGESTION_EGRS_CONGSTN))
#if !defined(BCM6858) && !defined(BCM6846) && !defined(BCM63158)
#define DSPTCHR_BDRAM_NEXT (RU_BLK(DSPTCHR).addr[0] +  RU_REG_OFFSET(DSPTCHR,BDRAM_NEXT_DATA))
#else
#define DSPTCHR_BDRAM_NEXT (RU_BLK(DSPTCHR).addr[0] +  RU_REG_OFFSET(DSPTCHR,BDRAM_DATA))
...
{
#if (!defined(CONFIG_GPL_RDP_GEN)  || defined(BCM6858))
    uint8_t i;
#endif
...
#if ((CHIP_VER < RDP_GEN_60) || (CHIP_VER >= RDP_GEN_62))
...
#if defined(CONFIG_GPL_RDP_GEN) && !defined(BCM6858)
    MEMSET_32((uint32_t *)DSPTCHR_CONJ_INGR, __swap4bytes(0x3f0ff0ff), DSPTCHR_VIRTUAL_QUEUE_NUM);
    MEMSET_32((uint32_t *)DSPTCHR_CONJ_EGRS, __swap4bytes(0x3f0ff0ff), DSPTCHR_VIRTUAL_QUEUE_NUM);
#else
...
#if defined(CONFIG_GPL_RDP_GEN) && !defined(BCM6858)
    ACCESS_LOG(ACCESS_LOG_OP_MEMSET_ADAPTIVE_32, DSPTCHR_BDRAM_NEXT, 0x40004, DIS_REOR_LINKED_LIST_BUFFER_NUM - 1);
	p_addr = (volatile uint32_t *)DSPTCHR_BDRAM_NEXT;
	data = 4;

	for (i = 0; i < DIS_REOR_LINKED_LIST_BUFFER_NUM - 1; i++)
	{
		p_addr[i] = (uint32_t)data;
		data+=4;
	}
