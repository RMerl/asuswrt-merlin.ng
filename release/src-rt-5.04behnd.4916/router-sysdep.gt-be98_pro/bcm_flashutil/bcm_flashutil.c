...
int getBootPartition( void )
{
    ...
    bcmret = sysUtil_readFileToDynamicBuf("/sys/firmware/fdt", (UINT8 **) &blob, &blobLen);
    if (bcmret == BCMRET_SUCCESS)
    {
        // This check is not needed, but uses blobLen to keep compiler happy.
        if (blobLen == 0)
        {
            fprintf(stderr, "%s: sysUtl_readFileToDynamicBuf returned blob=%p blobLen=%d\n",
                    __FUNCTION__, blob, blobLen);
        }
    ...
    // If we could not get the boot partition from fdt above, use this
    // method to get boot partition.
    UPDATE_FLASH_INFO();
    switch( gFlashInfo )
    {
        case FLASH_INFO_FLAG_NAND:
