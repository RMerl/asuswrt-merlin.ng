...
static char * metadata_dup ( int imageNumber, int index , int * metadata_size)
{
    ...
    if ((*metadata_size = ftell(fp)) < 0)
    {
        printf("%s: Error finding file position\n", __FUNCTION__);
        goto CLOSE;
    }
    ...
    if( (fread(metadata_ptr, 1, *metadata_size, fp) < *metadata_size) || ferror(fp) )
    {
...
int emmcGetBootPartition(void)
{
    ...
    rbuf = fgets(rootfs_devname, sizeof(rootfs_devname), fp);
    if (rbuf != NULL)
    {
...
static uint64_t emmcGetPartSize( char * full_dev_path )
{
	...
	/* Read the output a line at a time - output it. */
	rbuf = fgets(size, sizeof(size), fp);
	if (rbuf != NULL)
	{
...
int emmcGetBootPartIndex( void ) {
	...
	/* Read the output a line at a time - output it. */
	rbuf = fgets(part_conf_str, sizeof(part_conf_str), fp);
	if (rbuf != NULL)
	{
...
{ 
    ...
    *psize = (512*(mmcblk0_size+mmcblk0boot0_size+mmcblk0boot1_size)/1024);
    return 0;
}
...
int getEmmcMetadata( char * data, int size , int mdata_idx)
{
    ...
    if( mdata_fh )
    {
        ...
        if( (num_bytes < size) || ferror(mdata_fh) )
        {
