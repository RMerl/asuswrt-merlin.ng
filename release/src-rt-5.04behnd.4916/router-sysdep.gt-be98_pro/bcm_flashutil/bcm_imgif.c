        ...
        switch( imgif_ctx.flash_type )
        {
            ...
            case FLASH_INFO_FLAG_NAND:
                if( !nandIsLegacyFlashLayout() )
                {
                    ...
                    printf("imgif: Enabling NAND image flashing functions!\n");
                }
                else
                    printf("imgif: Error! image flashing not supported for legacy NAND flash layout!\n");
            break;
            ...
            case FLASH_INFO_FLAG_EMMC:
                if( !emmcIsLegacyFlashLayout() )
                {
                    ...
                    imgif_ctx.imgif_set_image_info_cb = imgif_pkgtb_set_image_info;
                    imgif_ctx.imgif_get_flash_info_cb = imgif_pkgtb_get_flash_info;
                     printf("imgif: Enabling EMMC image flashing functions!\n");
                }
                else
                    printf("imgif: Error! image flashing not supported for legacy EMMC flash layout!\n");
            break;

#ifdef BCM_FLASH_VFBIO
            case FLASH_INFO_FLAG_VFBIO:
                /* VFBIO interface doesn't support legacy layout */
                imgif_ctx.imgif_open_cb = imgif_pkgtb_open;
                imgif_ctx.imgif_write_cb = imgif_pkgtb_write;
                imgif_ctx.imgif_close_cb = imgif_pkgtb_close;
                imgif_ctx.imgif_set_image_info_cb = imgif_pkgtb_set_image_info;
                imgif_ctx.imgif_get_flash_info_cb = imgif_pkgtb_get_flash_info;
                printf("imgif: Enabling VFBIO image flashing functions!\n");
            break;
#endif
            ...
            case FLASH_INFO_FLAG_NOR:
                if( norIsNewFlashLayout() )
                {
                    ...
                    imgif_ctx.imgif_set_image_info_cb = imgif_pkgtb_set_image_info;
                    imgif_ctx.imgif_get_flash_info_cb = imgif_pkgtb_get_flash_info;
                    printf("imgif: Enabling SPI NOR image flashing functions!\n");
                }
                else
                    printf("imgif: Error! image flashing not supported for legacy NOR flash layout!\n");
            break;
#endif
            default:
                printf("imgif: Error! image flashing not supported!\n");
            break;
        }
    }
    else
...
*  PURPOSE:   Check whether image flashing is supported and enabled.
*  PARAMETERS:
*      None.
*  RETURNS:
*      TRUE - supported and enabled. FALSE - else.
