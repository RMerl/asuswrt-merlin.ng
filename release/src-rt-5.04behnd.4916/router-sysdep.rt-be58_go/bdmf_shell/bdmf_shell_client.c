/***********************************************************************
 * <:copyright-BRCM:2011-2015:DUAL/GPL:standard
 * 
 *    Copyright (c) 2011-2015 Broadcom 
 *    All Rights Reserved
 * 
...
/* Receive handler */
static void *bdmfmons_client_thread_handler(void *arg __attribute__((unused)))
{
    char ch;
...
static void *bdmfmons_cli_output_thread_handler(void *arg __attribute__((unused)))
{
    char ch;
    int rc;
...
static int send_to_session(char *sbuf)
{
    ...
    if (serial_file)
    {
        ...
        iop = (struct io_param *)malloc(sizeof(struct io_param));
        if (!iop)
            return print_help(0, "send: Cound not allocate memory for command buffer\n");
        memset(iop, 0, sizeof(struct io_param));
        iop->session_id = session_id;
        strncpy(iop->command, sbuf, BDMF_CHRDEV_MAX_CMD_LENGTH-1);
