...
UINT32 devCtl_getNumMacAddresses(void)
{
   UINT32 num;
   BcmRet ret;
   
   ret = devCtl_boardDriverIoctl(BOARD_IOCTL_GET_NUM_MAC_ADDRESSES,
                           0, NULL,
                           0, 0, &num);
   return (ret == BCMRET_SUCCESS ? num : 0);
}
...
SINT32 devCtl_scratchPadList(const char *fileName, char *tokBuf, int bufLen)
{
    ...
    if( fd < 0 )
        // if fd < 0, pbuf is failed to alloc or get freed in getScratchPadFile()
        /* coverity[leaked_storage] */ 
        return 0;

    if(memcmp(((PSP_HEADER)pBuf)->SPMagicNum, MAGIC_NUMBER, MAGIC_NUM_LEN) != 0) 
    {
        bcmuLog_notice("Scratchpad not found");
        if (putScratchPadFile(fd, pBuf, spDataLen, 0) < 0)
        {
            bcmuLog_error("putScratchPadFile() failed");
        }

        // Return 0 for now.  Scratchpad will be initialized on first set.
        return 0;
    }
    ...
    {
        tokenNameLen = strnlen(pToken->tokenName, sizeof(pToken->tokenName)-1);
        pToken->tokenName[sizeof(pToken->tokenName)-1] = '\0';
        needLen += tokenNameLen + 1;
        if (needLen <= bufLen)
        {
            strncpy(&tokBuf[copiedLen], pToken->tokenName, tokenNameLen);
...
SINT32 devCtl_scratchPadSet(const char *fileName, char *tokenId, char *tokBuf, int bufLen)
{
    ...
    else  
    {
        ...
        while( isalnum(pToken->tokenName[0]) && isascii(pToken->tokenName[0]) &&
            strnlen(pToken->tokenName, sizeof(pToken->tokenName)-1) < TOKEN_NAME_LEN &&
            pToken->tokenLen > 0 &&
        ...
        if( putAtEnd )
        {
            if( tokBuf )
            {
                // length check was done when Calculate the used length. Won't overrun here.
        ...
        { 
           ...
           if (ret == BCMRET_SUCCESS)
           {
              if ((boardIoctl == BOARD_IOCTL_GET_PSI_SIZE) ||
                  ...
                  (boardIoctl == BOARD_IOCTL_GET_NUM_MAC_ADDRESSES) ||                  
