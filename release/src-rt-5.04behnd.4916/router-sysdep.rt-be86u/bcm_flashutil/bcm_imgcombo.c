...
static char board_id[BOARD_ID_LEN+5];
imgutil_ret_e bcmImg_GetBoardId(){
    ...
    if(fp)
    {
        ...
        int count;
        /* create %xs format specifier for fscanf so we don't read more characters than the buffer size */
        char fmt[10];
        int i = sprintf(&fmt[1], "%ds", BOARD_ID_LEN+4);
        fmt[0] = '%';
        fmt[i+1] = '\0';

        count = fscanf(fp, fmt, board_id);
        if (count <= 0)
        {
