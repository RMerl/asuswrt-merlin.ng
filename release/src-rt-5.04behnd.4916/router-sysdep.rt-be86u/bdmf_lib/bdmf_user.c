                    ...
                    bdmf_object_handle *pmo)
{
    ...
    ret = call_ioctl(&ba, BDMF_USER_NEW_AND_SET);
    if (ret)
        return -EINVAL;
...
int bdmf_destroy(bdmf_object_handle mo)
{
    ...
    ret = call_ioctl(&ba, BDMF_USER_DESTROY);
    if (ret)
        return -EINVAL;    
...
bdmf_mattr_handle bdmf_mattr_alloc(bdmf_type_handle drv)
{
    ...
    ret = call_ioctl(&ba, BDMF_USER_MATTR_ALLOC);
    if (ret)
        return 0; 
...
void bdmf_mattr_free(bdmf_mattr_handle mattr)
{
    ...
    call_ioctl(&ba, BDMF_USER_MATTR_FREE);
}
                                 ...
                                 bdmf_object_handle mo, const char *filter)
{
    ...
    ret = call_ioctl(&ba, BDMF_USER_GET_NEXT);
    if (ret)
        return 0;
              ...
              bdmf_object_handle us, const char *attrs)
{
    ...
    ret = call_ioctl(&ba, BDMF_USER_LINK);
    if (ret)
        return -EINVAL; 
...
int bdmf_unlink(bdmf_object_handle ds, bdmf_object_handle us)
{
    ...
    ret = call_ioctl(&ba, BDMF_USER_UNLINK);
    if (ret)
        return -EINVAL;     
                                          ...
                                          bdmf_link_handle prev)
{
    ...
    ret = call_ioctl(&ba, BDMF_USER_GET_NEXT_US_LINK);
    if (ret)
        return 0;
                                        ...
                                        bdmf_link_handle prev)
{
    ...
    ret = call_ioctl(&ba, BDMF_USER_GET_NEXT_DS_LINK);
    if (ret)
        return 0;
...
bdmf_object_handle bdmf_us_link_to_object(bdmf_link_handle us_link)
{
    ...
    ret = call_ioctl(&ba, BDMF_USER_US_LINK_TO_OBJECT);
    if (ret)
        return 0;     
...
bdmf_object_handle bdmf_ds_link_to_object(bdmf_link_handle ds_link)
{
    ...
    ret = call_ioctl(&ba, BDMF_USER_DS_LINK_TO_OBJECT);
    if (ret)
        return 0;     
                    ...
                    bdmf_object_handle *owner)
{
    ...
    call_ioctl(&ba, BDMF_USER_GET_OWNER);
