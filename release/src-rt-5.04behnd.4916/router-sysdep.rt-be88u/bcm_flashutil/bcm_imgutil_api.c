  ...
  imgutil_img_info_t *imgInfoOutP)
{
    ...
    /* dump out version info now that upgrade is complete */
    if ( ctxP->pending_version_info && imgInfoOutP )
    {
        ctxP->pending_version_info = 0;
        if (ctxP->wholeFlashB == TRUE)
        {
            ret = bcmImg_getImageVersion((char*)ctxP->imgBufP,
              ctxP->accImgSize, (char*)imgInfoOutP->version,
              IMGUTIL_VERSION_LEN);
            if (ret == IMGUTIL_PROC_OK)
            {
                imgInfoOutP->bitmask |= IMGUTIL_IMGINFO_BIT_VERSION;
            }
            else
            {
        ...
        {
            devCtl_getImageVersion(getUpgradePartition(), (char*)imgInfoOutP->version,
              IMGUTIL_VERSION_LEN);
        }
    }
  ...
  UINT32 bitmask, imgutil_img_info_t *imgInfoP)
{
    ...
    else if (src == IMGINFO_SRC_BLTIN)
    {
        if (bitmask & IMGUTIL_IMGINFO_BIT_VERSION)
        {
            if( ctxP->upgradeState == IMGUTIL_STATE_IMG )
            {
                /* We are in the middle of an upgrade, the upgrade hasnt finished
                 * yet, and new image has not been committed. Postpone the image
                 * version retrieval until img_util_close is called */
                ctxP->pending_version_info = 1;
            } 
            else 
            {
                ctxP->pending_version_info = 0;
                ...
                {
                    devCtl_getImageVersion(getUpgradePartition(), (char*)imgInfoP->version,
                      IMGUTIL_VERSION_LEN);
                }
            }
        }
...
int validateAndFlashConfigFile(imgutil_ctx_t *ctxP)
{
    ...
    // Typically, this function points to cmsImg_ConfigFileValidate, which 
    // returns a CmsImageFormat enum defined in bcm_imgutil_def.h.  Note that
    // CMS_IMAGE_FORMAT_INVALID is 0.
    rc = ctxP->clientValidateFuncP(ctxP->imgStartP, ctxP->accImgSize,
                                   ctxP->clientCtxP);
    if (rc == CMS_IMAGE_FORMAT_INVALID)
    {
        imgutil_error("clientValidateFuncP() failed, rc=%d", rc);
        return -1;
    }

    // Typically, this function points to cmsImg_ConfigFileWrite, which 
    // returns 0 on success or -1 on error.
    rc = ctxP->clientFlashFuncP(ctxP->imgStartP, ctxP->accImgSize,
                                ctxP->clientCtxP);
    if (rc != 0)
    {
        imgutil_error("clientFlashFuncP() failed, rc=%d", rc);
        return -1;
    }
...
static int validateAndFlashWholeImage(imgutil_ctx_t *ctxP)
{
    ...
    printf("validateAndFlashWholeImage: Done! (noReboot=%d)\n", noReboot);
    fflush(stdout);
    if (noReboot == 0)
    {
        // This is appropriate if we are flashing a single image NOR flash.
        // Once we write the new image, we need to reboot immmediately because
        // the image in the flash do not match what is running anymore.
        bcmImg_reboot();
    }
...
#else      
    if ((ctxP->forceWholeFlashB == TRUE) ||
        ((ctxP->flashType == FLASH_INFO_FLAG_NOR) && (0 == norIsNewFlashLayout())))
#endif        
    {
        ctxP->wholeFlashB = TRUE;
    }
