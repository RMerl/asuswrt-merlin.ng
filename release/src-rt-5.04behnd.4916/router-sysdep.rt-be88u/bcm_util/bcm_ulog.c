...
UINT32 uTraceDestMask = (BCMULOG_DESTMASK_SYSLOG|BCMULOG_DESTMASK_STDERR);
...
void bcmuLog_formatInfo(FILE *stream, BcmuLogFormatInfo *info)
{
   ...
   if (info->logHeaderMask & BCMULOG_HDRMASK_TIMESTAMP)
   {
      ...
      fprintf(stream, "%lld.%03lu", (long long) ts.tv_sec%1000, ts.tv_nsec/1000000);
   }
   else if (info->logHeaderMask & BCMULOG_HDRMASK_DATETIME)
   {
      char buf[128]={0};
      bcmTms_getXSIDateTimeGmt(0, buf, sizeof(buf));
...
void ulog_log(BcmuLogLevel level, const char *funcName, UINT32 lineNum, const char *pFmt, ... )
{
   ...
   if (level > (bcmuLog_getLevelEx(threadId, 0) & BCMULOG_LEVEL_MASK))
   {
      return;
   }
   ...
   if (stream == NULL)
   {
       printf("ulog_trace: failed to open memory stream!\n");
       return;
   }
...
void ulog_trace(const char *funcName, UINT32 lineNum, const char *anchor, const char *pFmt, ... )
{
   ...
   threadId = sysUtl_getThreadId();
   if ((bcmuLog_getLevelEx(threadId, 0) & BCMULOG_LEVEL_PERFTRACE) == 0)
   {
      // If perftrace bitflag is not set, do not log.
      return;
   }
...
static void outputKmsgBuf(BcmuLogLevel level, char *buf)
{
   int fd;
   int flag = 0;
   char buf2[BCMULOG_MAX_LINE_LEN] = {0};

   // Lines logged to /dev/dmsg should start with <%d> to indicate
   // "facility" and log level.  We pick LOCAL5 as the facility, but this
   // can be changed.
   flag = LOG_LOCAL5 | (level & 0x7);
   snprintf(buf2, sizeof(buf2), "<%d>%s", flag, buf);
   ...
   // write buffer to fd
   if (write(fd, buf2, strlen(buf2)) < (SINT32) strlen(buf2))
       printf("ulog_trace: write failed!\n");
...
void ulog_kmsg(BcmuLogLevel level, const char *funcName, UINT32 lineNum, const char *pFmt, ... )
{
   ...
   fillFormatInfo(level, funcName, lineNum, &info);
   // Used a simplified header format compared to normal debug lines.
   // also, use DATETIME which is more user friendly instead of timestamp.
   info.logHeaderMask = BCMULOG_HDRMASK_APPNAME|BCMULOG_HDRMASK_LEVEL|BCMULOG_HDRMASK_DATETIME;
   va_start(ap, pFmt);
   bcmuLog_formatLine(stream, &info, pFmt, ap);
   va_end(ap);
