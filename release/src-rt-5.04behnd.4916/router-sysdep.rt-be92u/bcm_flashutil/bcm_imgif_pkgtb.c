...
#define imgif_debug(fmt, arg...) \
  _cprintf("%s.%u: " fmt , __FUNCTION__, __LINE__, ##arg)
                                 ...
                                 const UINT8 *imgseg_sha)
{
    ...
    if (fp == NULL)
    {
        _cprintf("Failed to run command %s\n", cmdBuf);
        return -1;
    }
    ...
    {
        _cprintf("Could not read results of cmd %s (errno=%d)\n", cmdBuf, errno);
        pclose(fp);
        return -1;
    }
    ...
    // Compare output sha256 with expected value in imgseg_sha
    {
        ...
        for (i=0; i < 32; i++)
        {
            ...
            if (rc != 1)
            {
                _cprintf("%s: parse failed at [%d] (output=%s)\n",
                       __FUNCTION__, i*2, output);
                ret = -1;
            }
            else if (byte != imgseg_sha[i])
            {
                _cprintf("%s: compare failed at [%d]: got %2hhx expected %2hhx\n",
...
static uint64_t nand_get_part_extra_bytes( char * part_prefix )
{
    ...
    if( access( filename, F_OK ) != -1 )
    {
        ...
        {
              _cprintf("could not open %s", filename);
              return 0;
        }

        if( NULL == fgets(value, 10, fp) )
        {
           _cprintf("Could not read %s", filename);
           fclose(fp);
           return 0;
        }
        fclose(fp);
        ...
        {
           _cprintf("Partition 0x%llx larger than available space 0x%x\n", extra_bytes * size_mult, size * 1024);
           return 0;
        }
    ...
    char* dev_name_prefix, int seg_index, imgif_ctx_t *ctxP)
{
    ...
    {
        _cprintf("%s: Error: Ubi volume %s not deleted properly!\n", __FUNCTION__, cmd);
        ret = -1;
    }
    ...
    else
    {
        ...
        _cprintf("%s: Error: Ubi volume %s does not exist!\n", __FUNCTION__, cmd);
        ret = -1;
    }
    return ret;
}
    ...
    char* dev_name_prefix, int seg_index, imgif_ctx_t *ctxP )
{
    char cmd[128];
    int ret= -1;
    _cprintf("Flashing %s from /var/%s.tmp ....\n", dev_name_prefix, dev_name_prefix);
    ...
    char* dev_name_prefix, int seg_index, imgif_ctx_t *ctxP )
{
    ...
    _cprintf("Flashing %s to ", dev_name_prefix);
    if( ctxP->pkgtb_img_segments[seg_index].type == IMGIF_PKGTB_IMG_SEGMENT_LOADER )
    {
    ...
    {
        _cprintf("%s: Error: GPT partition %s does not exist!\n", __FUNCTION__, cmd);
        ret = -1;
    }
    ...
    char *dev_name_prefix, int seg_index, imgif_ctx_t *ctxP )
{
    ...
    {
        _cprintf("Unexpected dev_name_prefix=%s\n", dev_name_prefix);
        return -1;
    }
    _cprintf("Flashing %s to %s. Size %llu bytes\n", 
    ...
    if (is_dynamic)
    {
        ...
        if (vfbio_lun_get_id(lun_name, &old_lun_id) == VFBIO_ERROR_OK)
        {
            ...
            if (ret)
                _cprintf("Unable to delete LUN %s. Error %s\n", lun_name, vfbio_error_str(ret));
        }
        ...
        {
            _cprintf("Unable to create LUN %s with id %d. Error %s\n", lun_name, lun_id, vfbio_error_str(ret));
            return -1;
        }
    }
    ...
    {
        _cprintf("Unable to retrieve info of LUN %s. Error %s\n", lun_name, vfbio_error_str(ret));
        return ret;
    }
    ...
    {
        _cprintf("LUN %s is too small for the new %s image. Image size: %lu, LUN size %lu bytes\n", 
...
static int finalize_tmpfile_write( imgif_ctx_t *ctxP, int seg_index )
{
    ...
    if ( !ctxP->pkgtb_img_segments[seg_index].delay_commit )
    {
        ...
        if( child_pid >= 0 )
        {
            ...
            {
                int i;
                _cprintf("%s: Child PID %ld launched for flashing seg %d (%s) forked_flash_writes=%d\n",
                ...
                {
                    _cprintf("%s: overflowed forked_writes_array, forked_flash_writes=%d\n",
        ...
        {
            _cprintf("%s: Error launching Child for flashing segment %d \n",__FUNCTION__, seg_index);
            return -1;
        }
    }
    ...
    if( child_pid == 0 )
    {
        ...
        if( ret )
        {
            ...
            if ( !ctxP->pkgtb_img_segments[seg_index].delay_commit )
            {
                _cprintf("%s: Failed to flash segment %d. Child PID %ld, ret %d!!\n", 
                    __FUNCTION__, seg_index, (long)getpid(), ret);
            }
            else
#endif
            {
                _cprintf("%s: Failed to flash segment %d. ret %d!!\n", 
                ...
                (ctxP->pkgtb_img_segments[seg_index].type != IMGIF_PKGTB_IMG_SEGMENT_ROOTFS))
        {
            ...
            if (rcs < 0)
            {
                _cprintf("%s: system(%s) failed, rcs=%d\n",
        ...
        if ( !ctxP->pkgtb_img_segments[seg_index].delay_commit )
        {
            ...
            _cprintf("Forked flashing completed for segment: %s (ret=%d)\n", dev_name_prefix, ret);
            fflush(stdout);
            _exit(ret);
        }
#endif
...
static void print_imgupdate_progress( imgif_ctx_t * ctxP)
{
    ...
    _cprintf("\rImage Update Progress: %d/%d bytes\n", bytes_done, size);
#else
    fprintf(stderr, ".");
#endif
}
...
static int process_pkgt_hdr( imgif_ctx_t *ctxP)
{
    ...
    if(ret)
    {
        _cprintf("Invalid FDT hdr check failed:%d! Aborting!\n", ret);
    }
    else
    {
...
static int process_pkgt_img_segments( imgif_ctx_t *ctxP )
{
    ...
    if (images_noffset < 0) {
        _cprintf("Can't find images parent node '%s' (%s)\n",
...
static void dump_img_segments( imgif_ctx_t * ctxP )
{
    ...
    _cprintf("PKGTB update image detected. Image Segments found:\n");
    for( i=0; i<ctxP->num_img_segments; i++ )
    {
        _cprintf("    [%d] %s --> offset:0x%08x, size:0x%08x, type:%d\n",
            i,
    ...
    _cprintf("Current_img_idx: %d, Update_img_idx: %d\n", ctxP->booted_img_idx, ctxP->update_img_idx);
}
...
static int select_pkgt_img_segments( imgif_ctx_t *ctxP, char * name, IMGIF_PKGTB_IMG_SEGMENTS type )
{
    ...
    _cprintf("%s: Error! Could not find image segment!\n", __FUNCTION__);
...
static FILE * init_storage_dev( const char * dev_name_prefix, int seg_index, imgif_ctx_t *ctxP)
{
    ...
    {
        case IMGIF_PKGTB_IMG_SEGMENT_LOADER:
            _cprintf("ERROR: Loader cannot be flashed incrementally! Check delay_commit flag!\n");
            ret = -1;
        break;
        ...
        case IMGIF_PKGTB_IMG_SEGMENT_PAKL:
            ...
            _cprintf("ERROR: No incremental flashing support for VFBIO binaries ");
            ret = -1;
#endif
        break;
        ...
        switch( flashInfo.flashType )
        {
            case FLASH_INFO_FLAG_NAND:
                ...
                if( ret == 0 ) 
                {
                    ...
                    _cprintf("Incrementally Flashing %s to /dev/ubi0_%d ...\n", 
            ...
            case FLASH_INFO_FLAG_EMMC:
                ...
                if( flashDevNodeExists(cmd, 1) )
                {
                    ...
                    _cprintf("Incrementally Flashing %s to /dev/%s%d ...\n", 
            ...
            break;
            default:
                _cprintf("ERROR: No incremental flashing support for flash type %d!!\n", 
                    flashInfo.flashType );
                ret = -1;
            break;
        }
...
static int check_compat_prop(char * prop, char * values)
{
    ...
    if( strcmp(prop, "chip") == 0 )
    {
        ...
        if( ret )
            _cprintf("%s: Error! chipid check failed\n", __FUNCTION__);
    }
    else if( strcmp(prop, "flash") == 0 )
    {
        ...
        switch( flashInfo.flashType )
        {
            case FLASH_INFO_FLAG_NAND:
                ...
                if( ret )
                    _cprintf("%s: Error! Invalid img flash format:%s, supported format:nand%d\n",
            ...
            case FLASH_INFO_FLAG_EMMC:
                if( strstr(values, "emmc") )
                    ret = 0;
                if( ret )
                    _cprintf("%s: Error! Invalid img flash format:%s, supported format:emmc\n",
                        __FUNCTION__, values);
                break;
#ifdef BCM_FLASH_VFBIO
            case FLASH_INFO_FLAG_VFBIO:
                if( strstr(values, "vflash") )
                    ret = 0;
                if( ret )
                    _cprintf("%s: Error! Invalid img flash format:%s, supported format:vflash\n",
                        __FUNCTION__, values);
                break;
#endif
            case FLASH_INFO_FLAG_NOR:
                ...
                if( ret )
                    _cprintf("%s: Error! Invalid img flash format:%s, supported format:nor\n",
                        __FUNCTION__, values);
                break;
...
int check_pkgtb_compat_string( char* compat_str )
{
    ...
    if( comp_str_buf )
    {
        ...
        if( ret )
            _cprintf("%s: Error! Image compatibility check FAILS! Aborting upgrade!\n", __FUNCTION__);

        free(comp_str_buf);
    }
    else
            _cprintf("%s: Error! Cannot allocate memory for compat string! Aborting upgrade!\n", __FUNCTION__);
...
static int auth_pkgt( char * pkgt_hdr, int hdr_size, int total_size )
{
    ...
    ip = (int *)pkgt_hdr;
    fdt_len = ntohl(ip[1]);
    _cprintf("pkgt header length is %d\n",fdt_len);
    if (fdt_len > PKGTB_AUTH_MAX_HDR_SIZE) {
        _cprintf("pkgtb header exceeds max size\n");
        return(-1);
    }
    ...
    if (dp == NULL) {
        _cprintf("couldnt readdir authorized_pkg_keys\n");
        return(-1);
    }
    while ( (ep = readdir(dp)) != NULL )
    {
        if ((strlen(ep->d_name) > 4) && (strcmp(".pem",ep->d_name + strlen(ep->d_name) - 4) == 0)) {
            _cprintf("processing key %s\n",ep->d_name);
    ...
    FILE **fhP, char **bufP)
{
    ...
    *fhP = init_storage_dev(volume, seg_index, ctxP);
    ...
    {
        if( bufP && ! *bufP )
            _cprintf("%s: Error! Cant alloc memory for %s flashing!\n", __FUNCTION__, volume);
        else
        {
            _cprintf("%s: Error! Cant open file for %s flashing!\n", __FUNCTION__, volume);
            if (bufP && *bufP)
            {
                free(*bufP);
                *bufP = NULL;
...
static int process_pkgt( imgif_ctx_t *ctxP )
{
    ...
    if(desc_ptr)
    {
        ...
        if (conf_noffset < 0) {
            _cprintf("Can't find configuration node '%s' (%s)\n",
        ...
        {
            _cprintf("%s: Error! Missing compatibility string! Cannot determine SoC name!\n", __FUNCTION__);
            return -1;
        }
        ...
        if( flashInfo.flashType == FLASH_INFO_FLAG_NOR )
        {
            if(rootfs_size)
            {
                ...
                {
                    _cprintf("%s: Error! rootfs MTD partition too small for new rootfs!\n",
                        __FUNCTION__);
                    _cprintf("%s: Required space:%llu, Available space:%llu\n", __FUNCTION__,
            ...
            if(bootfs_size)
            {
                ...
                {
                    _cprintf("%s: Error! bootfs MTD partition too small for new bootfs !\n",
                         __FUNCTION__);
                    _cprintf("%s: Required space:%llu, Available space:%llu\n", __FUNCTION__,
        ...
        else
        {
            if( bootfs_rootfs_size )
            {
                ...
                {
                    _cprintf("%s: Error! image partition(s) too small for new bootfs and rootfs!\n",
                        __FUNCTION__);
                    _cprintf("%s: Required space:%llu, Available space:%llu\n", __FUNCTION__,
        ...
        if( loader_size )
        {
            ...
            {
                _cprintf("%s: Error! loader MTD partition too small for new loader binary!\n",
                    __FUNCTION__);
                return -1;
            }
        }
        ...
        if( flashInfo.flashType != FLASH_INFO_FLAG_NOR)
        {
            ...
            if( flashInfo.flashType == FLASH_INFO_FLAG_EMMC)
            {
                 ...
                 if( ret )
                 {
                     _cprintf("%s: Bootfs repartitioning FAILED! MANDATORY REBOOT initiated!\n", __FUNCTION__);
                     system("sync");
                     system("reboot");
                     sleep(10);
                     return ret;
    ...
    {
        _cprintf("%s: Error Default configurations not found in upgrade image!\n", __FUNCTION__);
        return -1;
    }
...
static int finalize_serial_segment( imgif_ctx_t *ctxP, int seg_index) 
{
            ...
            ctxP->pkgtb_img_segments[seg_index].seg_fh )
    {
        ...
        {
            _cprintf("%s: Error writing segment %d\n",  __FUNCTION__, seg_index);
            ret = -1;
        }
    ...
    if (ret)
    {
        _cprintf("%s: Error writing %s segment %d\n",
...
static int init_ctx(imgif_ctx_t *ctxP)
{
    ...
    else
    {
        ...
        if( ret == -1 )
        {
            _cprintf("%s: Error while retrieving boot partition number!\n", __FUNCTION__);
            return ret;
        }
        else
        {
...
static int write_bytes_to_pkgtb_device( imgif_ctx_t *ctxP, char * data, unsigned int pktbuf_num_bytes )
{
    ...
    while( pktbuf_num_bytes )
    {
        ...
        else
        {
            if( ctxP->current_img_segment_idx >= ctxP->num_img_segments )
            {
                ...
                */
                ...
                _cprintf("%s: Error: Attempt to write %d unknown img data bytes to invalid segment!\n",
        ...
        {
            _cprintf("%s:ERROR! no destination specified for segment %d!\n",  
                    __FUNCTION__, ctxP->current_img_segment_idx);
            return -1;
        }
...
static int check_image_partitions(UINT32 flashType, imgif_ctx_t *ctxP)
{
    ...
    if( ret ) 
    {
        _cprintf("%s: Error: %s %s for image %d does not exist!\n", __FUNCTION__, 
                ((flashType==FLASH_INFO_FLAG_NAND)?"UBI Volume":"GPT partition"),
                   cmd, ctxP->update_img_idx);
    }
    else
    {
        _cprintf("%s: All %ss for image %d are present\n", __FUNCTION__, 
                ((flashType==FLASH_INFO_FLAG_NAND)?"UBI Volume":"GPT partition"),
                   ctxP->update_img_idx);
    }
...
static int finalize_pkgtb_img_write(imgif_ctx_t *ctxP, unsigned char abortFlag)
{
    ...
    if (forked_flash_writes > 0)
    {
        ...
        for (i=0; i < MAX_FORKED_WRITES; i++)
        {
            if (forked_writes_array[i] != 0)
            {
                _cprintf("%s: waiting for child PID %ld---",
                ...
                if (child_pid > 0)
                {
                    ...
                    {
                        _cprintf("%s: Child with PID %ld exited with status 0x%x, forked_flash_writes=%d\n",__FUNCTION__, 
                            (long)child_pid, WEXITSTATUS(child_ret), forked_flash_writes-1);
                    }
                    else if (WIFSIGNALED(child_ret)) {
                        _cprintf("%s: Child with PID %ld exited via signal %d, forked_flash_writes=%d\n",__FUNCTION__, 
                ...
                else
                {
                    ...
                    write_fails++;
                    _cprintf("%s: Unexpected error, forked_flash_writes=%d\n", __FUNCTION__, forked_flash_writes-1);
                }
                ...
                {
                    _cprintf("%s: forked_flash_writes already at 0!\n", __FUNCTION__);
                }
        ...
        if (forked_flash_writes != 0)
        {
            _cprintf("%s: still need to collect more children, forked_flash_writes=%d!\n",
                   __FUNCTION__, forked_flash_writes);
        }
    }
    if(write_fails)
    {
        _cprintf("%s: Error! Segment flashing failed.\n",__FUNCTION__);
        goto cleanup;
    }
#endif /* PKGTB_FORKED_WRITES */
    ...
    if((ctxP->calCrc32Cb != NULL) && (ctxP->imgInfoExt.bitmask & IMG_INFO_BITMASK_CRC))
    {
        ...
        {
            _cprintf("%s: Whole Image CRC check failed! expected:0x%08x, calculated:0x%08x\n",
                __FUNCTION__,
                ctxP->imgInfoExt.crc,
                ctxP->crc_image_calc);
            goto cleanup;
    ...
    {
        _cprintf("Unmatched file size values, ext=0x%08x, cal=0x%llx",
    ...
    {
        _cprintf("%s: ERROR: img segments in header=%d complete=%d\n",
               __FUNCTION__,
    ...
    if (ret )
    {
        _cprintf("%s: Error committing upgrade image to flash! sync() failure!\n", __FUNCTION__);
        goto cleanup;
    }
    ...
    if( ret )
    {
        _cprintf("%s: ERROR: Invalid flash image partitions detected after upgrade!\n", __FUNCTION__);
        goto cleanup;
    }
    ...
    for( seg_index = 0; seg_index < ctxP->num_img_segments; seg_index++ )
    {
        ...
        if( ctxP->pkgtb_img_segments[seg_index].delay_commit )
        {
            ...
            if( ctxP->pkgtb_img_segments[seg_index].bytesdone )
            {
                ...
                {
                    _cprintf("%s: Error writing delay_commit segment: %s\n", __FUNCTION__, ctxP->pkgtb_img_segments[seg_index].dev_prefix);
                    goto cleanup;
                }
                _cprintf("Delayed commit completed for segment: %s\n", ctxP->pkgtb_img_segments[seg_index].dev_prefix);
            }
        }
    }
    ...
    if( delayed_commits ) 
    {
        ...
        if (ret )
        {
            _cprintf("%s: Error committing FULL upgrade image to flash! sync() failure!\n", __FUNCTION__);
            goto cleanup;
        }
    }
    /* Set image as valid */
...
int imgif_pkgtb_get_flash_info(imgif_flash_info_t *flashInfoP)
{
    ...
    if( ret == 0 )
    {
        ...
        else if ( flags == FLASH_INFO_FLAG_VFBIO )
        {
            ...
            {
                     _cprintf("%s: Error while retrieving upgrade partition number!\n", __FUNCTION__);
                     return -1;
            }
#else
            flashInfoP->flashSize = 0x1000000;
