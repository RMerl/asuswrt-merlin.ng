 ...
 */
 ...
 typedef enum
 {
    ...
    BCMULOG_LEVEL_PERFTRACE  = 0x100  /**< Message at trace level.  This is a bit flag in the level number. */
 } BcmuLogLevel;

#define BCMULOG_DEFAULT_LEVEL   BCMULOG_LEVEL_ERR

// the real levels are 0-7 (first 3 bits).  Everything above that are
// special flags.
#define BCMULOG_LEVEL_MASK      0x7
...
/* Special performance tracing log commands.  They can be processed and
 * visualized offline using a tool such as Jaeger.
 */
...
/** Special logging command to log very important events.
 *  This will be sent to the kernel printk buffer, which may affect performance,
 *  so use sparingly and keep the line short.  If the system crashes, the
 *  contents of the printk buffer will be preserved.  These log lines also
 *  show up in dmesg and syslog.
 */
#define bcmuLog_knotice(args...) ulog_kmsg(BCMULOG_LEVEL_NOTICE, __FUNCTION__, __LINE__, args)
 ...
 *  The timestamp has millisecond resolution and is synchronized across all
 *  ARM cores.  This option is incompatible with (has precedence over)
 *  HDRMASK_DATETIME
 */
...
/** Show date and time in the log line.
 *  This option shows the time to 1 second resolution only, but is more
 *  user friendly.  This option is incompatible with HDRMASK_TIMESTAMP.
 */
#define BCMULOG_HDRMASK_DATETIME   0x0020
...
void ulog_kmsg(BcmuLogLevel level, const char *funcName, UINT32 lineNum, const char *pFmt, ... );
