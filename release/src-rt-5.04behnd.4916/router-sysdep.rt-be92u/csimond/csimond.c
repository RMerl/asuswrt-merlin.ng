...
csi_rec_continuous_read(int sock_fd, unsigned int rec_size, unsigned int msg_freq)
{
	...
	while (1) {
		...
		/* The difftime call is used to check the approx
		...
		if (rec_size != 0) {
			...
			printf("Recd msg %u payld sz %d nl id %d msgflags %d "
				"recvmsg execution time:%.2f seconds\n",
				rec_cnt, sz, nl_socket_id, msg.msg_flags, difftime(end, start));
		}
...
int main(int argc, char **argv)
{
	...
	if (nl_socket_id != 0)
	{
		char nv_ifname[IFNAMSIZ] = {0}, nvram_name[IFNAMSIZ] = {0};
		char os_ifname[IFNAMSIZ] = {0};
		int idx  = -1;
		int csimon_driver = FALSE;
		char caps[WLC_IOCTL_MEDLEN], cap[80], *next;
		char cap_needle[80];

		memset(caps, 0, sizeof(caps));
	memset(cap, 0, sizeof(cap));
	memset(cap_needle, 0, sizeof(cap_needle));
	strcpy(cap_needle, "csimon");

	switch (nl_socket_id)
	{
		case NETLINK_CSIMON1:
			idx = 1;
			break;

		case NETLINK_CSIMON0:
			idx = 0;
			break;

		case NETLINK_CSIMON2:
			idx = 2;
			break;

		default:
			printf("%d:Invalid socket id %d; should be between %d and %d\n",
					__LINE__, nl_socket_id, NETLINK_CSIMON1, NETLINK_CSIMON2);
			exit(-1);

	}
	snprintf(nvram_name, sizeof(nvram_name),  "wl%d_radio", idx);
	if ((nvram_match(nvram_name, "0")) == TRUE) {
		printf("Err: csimon: %d: radio %d disabled\n", __LINE__, idx);
		exit(-1);
	}
	memset(nv_ifname, 0, IFNAMSIZ);
	snprintf(nv_ifname, sizeof(nv_ifname), "wl%d", idx);
	if (nvifname_to_osifname(nv_ifname, os_ifname, sizeof(os_ifname))) {
		printf("Err: csimon: %d: converting %s to os name \n", __LINE__, nv_ifname);
		exit(-1);
	}
	if (wl_iovar_get(os_ifname, "cap", (void *)caps, sizeof(caps)) < 0) {
		printf("Err: csimon: %d cap: iovar failed for %s\n", __LINE__, os_ifname);
		exit(-1);
	}
	...
	if (!csimon_driver) {
		printf("csimon is not ENABLED in driver\n");
		exit(-1);
	}
}
