...
int processDumpCmd(const char *key, PspDumpMode mode);
...
void usage(int exitCode)
{
   ...
   printf("   dump <key>  : dump the contents of the specified key (uses heuristic to determine binary or ascii)\n");
   printf("   dumpbinary <key> : dump the contents of the specified key in binary format\n");
   printf("   dumpascii <key>  : dump the contents of the specified key in ascii format\n");
   printf("   adump <key>      : same as dumpascii\n");
...
int main(int argc, char *argv[])
{
   ...
   if (!strcmp(cmd, "list"))
   {
      ...
      rv = processListCmd();
            ...
            !strcmp(cmd, "dumpbinary") ||
            !strcmp(cmd, "dumpascii") ||
            !strcmp(cmd, "adump"))
   {
      PspDumpMode mode = PSP_DUMP_AUTO;  // use heuristics to detect binary or ascii
      ...
      if (!strcmp(cmd, "dumpascii") ||
          !strcmp(cmd, "adump"))
      {
         mode = PSP_DUMP_ASCII;
      }
      else if (!strcmp(cmd, "dumpbinary"))
      {
         mode = PSP_DUMP_BINARY;
      }

      rv = processDumpCmd(argv[2], mode);
   }
...
int isBinaryBuf(const char *buf, int len)
{
   ...
   for (i=0; i < len; i++)
   {
      // We've gotten to the last character, and so far everything has been
      // ascii, and the last character is a null terminating character, so
      // this is an ascii string (not binary).
      if ((i == len-1) && (buf[i] == '\0'))
      {
         return 0;
      }
...
int processDumpCmd(const char *key, PspDumpMode mode)
{
   ...
   {
      if ((mode == PSP_DUMP_BINARY) ||
          ((mode == PSP_DUMP_AUTO) && isBinaryBuf(buf, rc)))
      {
         int i=0, j;
