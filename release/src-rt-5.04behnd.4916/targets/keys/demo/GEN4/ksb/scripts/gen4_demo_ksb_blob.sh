...
if [[ $# != 2 ]]; then
	...
	echo " "
	echo "    Description:"
	echo "        This script generate Key Store Box acording to specified Configuration"
	echo "        Should be executed from"
	echo "            SDK_root_directory/targets/keys/demo/GEN4/<SOC_FOLDER>/ksb"
	echo " "
	echo "    Example:"
	echo "        ../../scripts/gen4_demo_ksb_blob.sh ./ksb.cfg ../ksb.bin"
	echo " "
	...
	echo "    Return Value:"
	echo "        0 - error"
	echo "        N - number of entries in the ksb.bin"
	echo " "
	echo "    PAY ATTENTION !!!"
...
function ks_entry_padding() {
	...
	dd if=/dev/random ibs=1 count="$padcount" >> $1
	if [ $? -ne 0 ]; then
		clean_up
	fi
}
