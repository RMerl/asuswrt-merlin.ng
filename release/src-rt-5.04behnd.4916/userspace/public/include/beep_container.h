...
#define CONT_MOUNT_MAX_ENTRIES  8
#define CONT_MOUNT_OPT_MAX_ENTRIES  8
...
#define CONT_PROCESS_ENV_MAX      8
#define CONT_PROCESS_ENV_LEN_MAX  255

#define CONT_BUSNAME_LEN_MAX       255

#define LXC_VERSION_2_0_4 "2.0.4"
#define LXC_VERSION_3_1_0 "3.1.0"
#define LXC_VERSION_5_0_2 "5.0.2"
#define LXC_VERSION_5_0_3 "5.0.3"
...
typedef struct
{
   ...
   char source[32];
   char type[16];
   char options[CONT_MOUNT_OPT_MAX_ENTRIES][32];
} ContMounts_t;

typedef struct
{
   char apparmorProfile[256];
   char args[CONT_PROCESS_ARGS_MAX][CONT_PROCESS_ARGS_LEN_MAX+1];
   char env[CONT_PROCESS_ENV_MAX][CONT_PROCESS_ENV_LEN_MAX+1];
} ContProcess_t;
...
typedef struct
{
   ...
   union 
   {
      struct
      {
         char busName[CONT_BUSNAME_LEN_MAX+1];
         char busObject[CONT_BUSNAME_LEN_MAX+1];
         char busInterface[CONT_BUSNAME_LEN_MAX+1];
      } dbus;
   };
...
int contStopContainer(const char *container_name, int needNetwork);
...
int contIPv6NetworkAddress(const char *container_name, char *addr, int addrLen);
int contGetContainerDataDir(const char *path, int limitFlash, int isEE,
                            char *dataDir, int dataDirLen, int fullFS);
int contGetContainerCrashInfo(const char *path, int limitFlash);
int contGetEeContainerDuDir(const char *path, int limitFlash, int isEE,
                            char *duDir, int duDirLen, int fullFS);
int contCleanupContainerMountPoint(const char *container_name, const char *path,
                                   int limitFlash, int fullFS);
int contGetContainerBinDir(const char *path, int limitFlash, 
                           char *dataDir, int dataDirLen);
int contGetContainerRootfsDir(const char *path, int limitFlash, int isEE,
                              char *rootfs, int rootfsLen, int fullFS);
...
int contGetContainerVersion(char *containerVersion, int containerVersionSize);
