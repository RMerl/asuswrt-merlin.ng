...
/** Max values defined statically so that cms_util and other apps can
 *  use them without having a hard dependency on the data model.
 *  mdm_init will verify the values defined here matches what the data model
 *  uses (mdm_params.h).
 */

/** buffer length for an Alias buffer (64+null byte) */
#define MAX_MDM_ALIAS_BUFLEN               65

/** buffer length for a single fullpath reference (256+null byte) */
#define MAX_MDM_SINGLE_FULLPATH_BUFLEN    257

/** buffer length for multiple fullpath references (2048+null byte) */
#define MAX_MDM_MULTI_FULLPATH_BUFLEN    2049
