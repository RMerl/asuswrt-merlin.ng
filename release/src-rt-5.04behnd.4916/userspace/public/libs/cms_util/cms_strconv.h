...
/***************************************************************************
 *
 * The following syslog conversion functions have been moved to
 * cms_core/linux/rut_validstrings.c so that cms_util will not have a
 * dependency on the data model.  The prototypes are still left here for now.
 *
 */

#define cmsUtl_syslogModeToNum(x)          rutVStr_syslogModeToNum(x)
#define cmsUtl_numToSyslogModeString(x)    rutVStr_numToSyslogModeString(x)
#define cmsUtl_isValidSyslogMode(x)        rutVStr_isValidSyslogMode(x)
#define cmsUtl_syslogLevelToNum(x)         rutVStr_syslogLevelToNum(x)
#define cmsUtl_numToSyslogLevelString(x)   rutVStr_numToSyslogLevelString(x)
#define cmsUtl_isValidSyslogLevel(x)       rutVStr_isValidSyslogLevel(x)
#define cmsUtl_isValidSyslogLevelString(x) rutVStr_isValidSyslogLevelString(x)
 ...
 */
UBOOL8 rutVStr_isValidSyslogLevelString(const char *level);


/*
 * End syslog conversion functions.
 *
 ***************************************************************************/
...
/** Split a string into an array of substrings using the delim as token.
 *
 * @param str (IN) the string to be splited.
 * @param delim (IN) the delim used as the token.
 * @param size (OUT) the number of substrings returned.
 *
 * returns an array of substrings. On error, NULL is returned.
 * The caller is responsible to free the returned array by iterating it to
 * free each substring, and afterwards free the array itself.
 */
char** cmsUtl_strsplit(const char *str, const char *delim, size_t *size);
...
/***************************************************************************
 * The following two pppAuth conversion functions, and
 * logLevelStringToEnum, and logDestinationStringToEnum have been moved to
 * cms_core/linux/rut_validstrings.c
 */

#define cmsUtl_pppAuthToNum(x)         rutVStr_pppAuthToNum(x)
#define cmsUtl_numToPppAuthString(x)   rutVStr_numToPppAuthString(x)
#define cmsUtl_logLevelStringToEnum(x) rutVStr_logLevelStringToEnum(x)
#define cmsUtl_logDestinationStringToEnum(x) rutVStr_logDestinationStringToEnum(x)
...
/*
 * The following function has been moved to cms_core/linux/rut_validstrings.c
 */
#define cmsUtl_connectionModeStrToNum  rutVStr_connectionModeStrToNum
...
/** Generate a jabberID based on input params.
 *
 * @param username (IN) must be specified.
 * @param domain (IN)   may be NULL.
 * @param domain (IN)   may be NULL.
 * @param jabberID (OUT) caller must supply a buffer of at least 1024 bytes.
 */
 ...
 *
 * @return Number of bytes written to destination buffer (Not including NULL
 *         character).
 */
...
/** Set bit in a string (string represents bits).
 *
 * @param pszBITS (IN) NULL terminated string pointer containing bits
 *                     Example: "0A86" represents 0000 1010 1000 0110
 * @param bit     (IN) Which bit to set to 1 (leftmost bit = 0)
 *                     So leftmost bit are bits 0-3, next is bits 4-7, etc.
 *
 * @return 0 if successful, -1 if error.
 */
int cmsUtl_BITSSetBit(char *pszBITS, int bit);

/** Clears bit in a string (string represents bits).
 *
 * @param pszBITS (IN) NULL terminated string pointer containing bits
 *                     Example: "0A86" represents 0000 1010 1000 0110
 * @param bit     (IN) Which bit to clear or set to 0 (leftmost bit = 0)
 *                     So leftmost bit are bits 0-3, next is bits 4-7, etc.
 *
 * @return 0 if successful, -1 if error.
 */
...
/** Gets bit in a string (string represents bits).
 *
 * @param pszBITS (IN) NULL terminated string pointer containing bits
 *                     Example: "0A86" represents 0000 1010 1000 0110
 * @param bit     (IN) Which bit to get (leftmost bit = 0)
 *                     So leftmost bit are bits 0-3, next is bits 4-7, etc.
 *
 * @return 0 if bit is not set, 1 if bit is set, -1 if error.
 *
 */
int cmsUtl_BITSGetBit(char *pszBITS, int bit);

/** Determine if string is UTF-8 multi-byte
 *
 * @param str (IN) to string to check if it's UTF-8 multi-byte
 *
 * @return 1 if multi-byte, 0 otherwise
 *
 */
int isStrMultibyte(char *str);

/** Given a decimal string, return true if it is a valid decimal string
 *
 * @param decimal (IN) decimal string.
 *
 * @return TRUE if the string is a valid decimal address.
 */
