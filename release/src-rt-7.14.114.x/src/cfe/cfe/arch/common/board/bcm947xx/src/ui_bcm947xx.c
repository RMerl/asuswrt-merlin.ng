...
#ifdef RTK3
#define PWR_LED_GPIO	(1 << 2)	// GPIO 2
#define RST_BTN_GPIO	(1 << 17)	// GPIO 17
...
#else
		if (nvram_match("asus_trx_test", "1")) {
#endif
			...
			if(nvram_match("asus_mfg", "1")){	// goto cmd mode if during ATE mode
				...
				LEDOFF();
				/* wait for cmd input */
				xprintf("\n1. Wait 10 secs to enter tftp mode\n   or push RESET-BTN to enter cmd mode\n");
				for(i=0; i<3; ++i){
					if (DETECT())
						cfe_command_loop();
					else
