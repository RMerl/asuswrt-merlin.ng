...
export GT-AX6000 += BUILD_NAME="GT-AX6000" EXT_PHY="BCM84880" NVSIZE="128" DHDAP=y DPSTA=n REPEATER=y IPV6SUPP=y \
		...
		NFS=y SNMPD=y TOR=y UPNPIGD2=n NANO=y NTPD=y IPSET=y VPN_FUSION_MERLIN=y
...
export XT12 += BUILD_NAME="XT12" EXT_PHY="BCM84880" NVSIZE="128" DHDAP=y REPEATER=y IPV6SUPP=y HTTPS=y ARM=y AUTODICT=y \
		...
		NFS=n SNMPD=y TOR=y UPNPIGD2=n NANO=y NTPD=y IPSET=y VPN_FUSION_MERLIN=y
...
export HND-96813_EMMC_BASE := HND_ROUTER=y HND_ROUTER_BE_4916=y PROFILE="96813GW" SW_HW_AUTH=y AMAS=y UTF8_SSID=y ETHOBD=y DWB=y \
		CONNDIAG=y SAMBA3="3.6.x" OOKLA=y WL_SCHED_V3=y PSISTLOG=y FLASH_TYPE="EMMC"
export GT-BE19000AI := $(HND-96813_EMMC_BASE)
export GT-BE19000AI += BUILD_NAME="GT-BE19000AI" EXT_PHY="BCM84880" NVSIZE="192" DHDAP=y DPSTA=n REPEATER=y IPV6SUPP=y HTTPS=y ARM=y \
		AUTODICT=y BBEXTRAS=y USBEXTRAS=y EBTABLES=y MEDIASRV=y MODEM=y PARENTAL2=y ACCEL_PPTPD=y PRINTER=y USB="USB" APP="network" \
		PROXYSTA=y DNSMQ=y SHP=n BCMWL6=y BCMWL6A=y DISK_MONITOR=y BTN_WIFITOG=y OPTIMIZE_XBOX=y ODMPID=y BCMSMP=y XHCI=y DUALWAN=y \
		NEW_USER_LOW_RSSI=y OPENVPN=y TIMEMACHINE=y MDNS=y VPNC=y JFFS2LOG=y BWDPI=y DUMP_OOPS_MSG=n LINUX_MTD="64" DEBUGFS=y \
		TEMPROOTFS=n SSH=y EMAIL=y FRS_FEEDBACK=n SYSSTATE=y ROG=y STAINFO=y CLOUDCHECK=n NATNL_AICLOUD=y REBOOT_SCHEDULE=y \
		MULTICASTIPTV=y QUAGGA=y WLCLMLOAD=n BCM_MUMIMO=y LAN50="all" ATCOVER=y GETREALIP=y CFEZ=y ETLAN_LED=y TFAT=y NTFS="tuxera" \
		HFS="tuxera" NEWSSID_REV4=y NEW_APP_ARM=y NETOOL=y TRACEROUTE=y FORCE_AUTO_UPGRADE=n ALEXA=y SW_HW_AUTH=y HD_SPINDOWN=y \
		ASPMD=n BCM_MEVENT=n BCMEVENTD=y BCM_APPEVENTD=n LETSENCRYPT=y VPN_FUSION=y JFFS_NVRAM=y NVRAM_ENCRYPT=y NATNL_AIHOME=y \
		BCM_CEVENTD=y UTF8_SSID=y I2CTOOLS=y AMAS=y DBLOG=y ETHOBD=y CFGSYNC_LOCSYNC=y CONNDIAG=y IPSEC=STRONGSWAN \
		IPSEC_SRVCLI_ONLY=SRV DWB=y OPEN_NAT=y ROG_UI=y AMASDB=n CRASHLOG=y WATCH_REINIT=n BW160M=y BRCM_HOSTAPD=y UUPLUGIN=n \
		IPERF3=y INFO_EXAP=y BCN_RPT=y BTM_11V=y INTERNETCTRL=y ASD=y AHS=n FRS_LIVE_UPDATE=y ASUSCTRL=y CAPTCHA=y OUTFOX=y \
		GAME_MODE=y AMAS_ETHDETECT=y INSTANT_GUARD=y AVBLCHAN=y FRONTHAUL_DWB=y BCM_CLED=y BONDING=y BONDING_WAN=n ACL96=y \
		LACP=y OWE_TRANS=y DHD_6715B0=y EXTEND_LIMIT=y GOOGLE_ASST=y BTN_TURBO=y WIREGUARD=y BCMBSD_V2=y SW_CTRL_ALLLED=y COMFW=y \
		NFCM=n DNSQUERY_INTERCEPT=y WIFI7=y BW320M=y MTLANCFG=y CAPTIVE_PORTAL=y CP_FREEWIFI=y WPA3_ENTERPRISE=y \
		EXTRA_KERNEL_CONFIGS="BCM_JUMBO_FRAME=y BCM_MAX_MTU_SIZE=10240 CONFIG_BCM_IGNORE_BRIDGE_MTU=y" MAX_VPNF=4 WTFAST_V2=y \
		AMAS_OB_DISABLE_DSAP=y USB_WAN_BACKUP=y AUTO_WANPORT=y PRESSURE_SENSOR=y ENERGY_SAVE=y AISRV=y HAS_6G=y \
		MULTISERVICE_WAN=y NO_AIDISK=y GTBOOSTER=y UI4=y AFC_POSITIONING=y BCM_AFC=y AURALED_NIGHT=y AURALED=y \
		NFS=n SNMPD=y TOR=y UPNPIGD2=n NANO=y NTPD=y IPSET=y VPN_FUSION_MERLIN=y
