...
/**
 * Get TX/RX incremental bytes from QCA9531 embedded switch.
 * @bus:
 * @port:
 * @rx:
 * @tx:
...
{
	unsigned int base = AR7240_REG_STATS_BASE(port);

	/* Take lower 32-bit only. */
	*rx = ar7240sw_reg_read(bus, base + AR7240_STATS_RXGOODBYTE);
	*tx = ar7240sw_reg_read(bus, base + AR7240_STATS_TXBYTE);
}
...
int swports_reset_check_traffic(struct bled_priv *bp)
{
	...
	/* QCA9531 embedded switch can't report total RX/TX bytes. Reset RX/TX bytes. */
