...
#define DUMP_PARSE_CLIENTLIST_PATH 0
...
bool cli_list_find_mac_by_ip(struct list_head *list, bool isv4, char *ip, char *mac)
{
    bool ret = false;
    cli_node_t *cli;
    struct in_addr ipv4;
    ...
    list_for_each_entry(cli, list, list) {
        if(isv4 && cli->isv4 && cli->ipv4.s_addr == ipv4.s_addr) {
          memcpy(mac, cli->mac, ETHER_ADDR_LENGTH);
          cli->lru_time = now;
          return true; 
        }
        else if (!isv4 && !cli->isv4 && !memcmp(&cli->ipv6, &ipv6, sizeof(struct in6_addr))) {
          memcpy(mac, cli->mac, ETHER_ADDR_LENGTH);
          cli->lru_time = now;
          return true; 
        }
