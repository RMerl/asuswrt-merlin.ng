...
static int get_backup_db_path_by_datetime(const char *db_name, const char *db_version, int query_datetime, char *db_file_path)
{
    ...
    codbg("Get db name [%d] from query_datetime [%d]", timestamp_day, gmt_time);
    ...
    while (pch_db_version != NULL)
    {
        ...
        codb_safe_strncpy(db_file_path, db_bakcup_path, MAX_FILE_PATH);
        codb_safe_strncat(db_file_path, "/", MAX_FILE_PATH);
        codb_safe_strncat(db_file_path, file_name, MAX_FILE_PATH);
    ...
    while (pch != NULL)
    {
        ...
        while (pch2 != NULL)
        {
            ...
            if (idx == 0)
            {
                ...
                codb_safe_strncpy(query_match_columns_idx->name, pch2, len + 1);
            }
            else if (idx == 1)
            {
                //- column type
            ...
            else if (idx == 2)
            {
                    ...
                    query_match_columns_idx->type == COLUMN_TYPE_TEXT_JSON)
                {
                    ...
                    codb_safe_strncpy(query_match_columns_idx->value.t, pch2, len + 1);
                }
...
static sql_column_prototype_t *parse_query_columns_data(char *data, int data_count)
{
    ...
    while (pch != NULL)
    {
        ...
        codb_safe_strncpy(query_columns_idx->name, pch, len + 1);
...
int codb_test()
{
    ...
    // res = codb_latest_content_query_json_field_ex("wifi_detect", column_count, content, filter_count, filter_data, &retObj);
...
int query_database(void *pdb,
                   ...
                   int columns_count, char **query_columns,
                   const char *between_column_name, unsigned long query_start, unsigned long query_end,
                   const char *order,
                   int sampling_interval_sec, int limit,
                   json_object *resultValueArrayObj, int *total_ret_rows)
{
    ...
    if (pdb != NULL)
    {
        int ret = cosql_get_column_values_ex(pdb,
                                             match_and_columns_count, match_and_columns,
...
{
    return codb_content_query_json_field_ex(db_name, columns_count, columns_name, filter_count, filter_data, "data_time", start, end, order_by, 0, limit, retJsonObj);
}

int codb_content_query_json_field_ex(char *db_name, int columns_count, char *columns_name, int filter_count, char *filter_data, const char *between_column_name, int start, int end, char *order_by, int sampling_interval_sec, int limit, json_object **retJsonObj)
{
    ...
    if (order_desc == 1)
    {
        query_start = get_zero_time_on_day(end); //- ex. 2022-01-09 00:00:00
    ...
    else
    {
        ...
        query_end = get_zero_time_on_day(start) + 86399; //- ex. 2022-01-09 23:59:59
        if (query_end > end)
            query_end = end;
    }
    ...
    while (1)
    {
        ...
        if (order_desc == 1)
        {
            ...
            ret_rows = query_database(pdb_tmp,
                                      ...
                                      0, NULL,
                                      columns_count, query_columns,
                                      between_column_name, query_start, query_end,
    ...
    NEXT_PERIOD:
        ...
        if (order_desc == 1)
        {
            ...
            query_start = get_zero_time_on_day(query_end); //- ex. 2022-01-08 00:00:00
            if (query_start < start)
                query_start = start;
        }
        else
        {
            ...
            query_end = get_zero_time_on_day(query_start) + 86399; //- ex. 2022-01-10 23:59:59
            if (query_end > end)
                query_end = end;
        }
    }
