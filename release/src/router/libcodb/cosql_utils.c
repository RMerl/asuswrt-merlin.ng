...
static char *gen_match_query_string(int match_columns_count, sql_column_match_t *match_columns, char *match_query_string, const char *split_string)
{
	...
	match_query_string_size = match_query_string_size + 1;

	match_query_string = (char *)malloc(sizeof(char) * (match_query_string_size));
	if (match_query_string == NULL)
	{
		return NULL;
	}

	memset(match_query_string, 0, match_query_string_size);
	...
	for (i = 0; i < match_columns_count; i++)
	{
		...
		{
			codb_safe_strncat(match_query_string, split_string, match_query_string_size);
		}

		codb_safe_strncat(match_query_string, buff, match_query_string_size);
...
int format_sql_between_data_time(const char *between_column_name, int start_data_time, int end_data_time, char *ret_sql)
{
	if (ret_sql == NULL)
	{
		return COSQL_ERROR;
	...
	if (between_column_name == NULL || strlen(between_column_name) <= 0 || strlen(between_column_name) > MAX_BUF_LEN)
	{
		snprintf(ret_sql, MAX_BUF_LEN, " AND (data_time BETWEEN %d AND %d)", start_data_time, end_data_time);
	}
	else
	{
		snprintf(ret_sql, MAX_BUF_LEN, " AND (%s BETWEEN %d AND %d)", between_column_name, start_data_time, end_data_time);
	}
...
int cosql_create_table(sqlite3 *pdb, const char *db_version, int columns_count, sql_column_prototype_t *columns)
{
	...
	sql_create_table_size = sql_create_table_size + 1;

	//- malloc() allocate the memory for sql_create_table_size+1 chars
	char *sql_create_table = (char *)malloc(sizeof(char) * (sql_create_table_size));
	...
	memset(sql_create_table, 0, sql_create_table_size);
	/////////////////////////////////////////////////////////////
	...
	for (i = 0; i < columns_count; i++)
	{
		...
		{
			codb_safe_strncat(sql_create_table, ",", sql_create_table_size);
		}

		codb_safe_strncat(sql_create_table, buff, sql_create_table_size);
										   ...
										   int *ret_count)
{
	...
	format_sql_between_data_time("data_time", start_data_time, end_data_time, sql_between_data_time);
...
int cosql_insert_table(sqlite3 *pdb, int columns_count, sql_column_t *columns)
{
	...
	sql_insert_columns_size = sql_insert_columns_size + 1;

	//- malloc() allocate the memory for sql_insert_columns_size+1 chars
	char *sql_column_names = (char *)malloc(sizeof(char) * (sql_insert_columns_size));
	...
	memset(sql_column_names, 0, sql_insert_columns_size);

	//- malloc() allocate the memory for sql_insert_values_size+1 chars

	sql_insert_values_size = sql_insert_values_size + 1;

	char *sql_insert_values = (char *)malloc(sizeof(char) * (sql_insert_values_size));
	...
	memset(sql_insert_values, 0, sql_insert_values_size);
	/////////////////////////////////////////////////////////////
	...
	for (i = 0; i < columns_count; i++)
	{
		...
		{
			codb_safe_strncat(sql_column_names, ",", sql_insert_columns_size);
		}

		if (strlen(sql_insert_values) > 0)
		{
			codb_safe_strncat(sql_insert_values, ",", sql_insert_values_size);
		}

		codb_safe_strncat(sql_column_names, buff_column_name, sql_insert_columns_size);
		codb_safe_strncat(sql_insert_values, buff_insert_value, sql_insert_values_size);
...
int cosql_get_column_values(sqlite3 *pdb,
							...
							char ***ret_result)
{
	return cosql_get_column_values_ex(pdb,
...
int cosql_get_column_values_ex(sqlite3 *pdb,
							   int match_and_columns_count, sql_column_match_t *match_and_columns,
							   int match_or_columns_count, sql_column_match_t *match_or_columns,
							   int query_columns_count, sql_column_prototype_t *query_columns,
							   ...
							   char ***ret_result)
{
	...
	sql_query_columns_size = sql_query_columns_size + 1;

	char *sql_column_names = (char *)malloc(sizeof(char) * sql_query_columns_size);
	if (sql_column_names == NULL)
	{
		codbg("fail to alloc sql buffer");
		return COSQL_ERROR;
	...
	memset(sql_column_names, 0, sql_query_columns_size);
	...
	for (i = 0; i < query_columns_count; i++)
	{
		...
		if (column_name != NULL && strlen(column_name) > 0)
		{
			...
			if (strlen(sql_column_names) > 0)
			{
				codb_safe_strncat(sql_column_names, ",", sql_query_columns_size);
			}

			codb_safe_strncat(sql_column_names, column_name, sql_query_columns_size);
		}
	...
	{
		//- JOIN (SELECT (data_time/300) AS slot, MIN(data_time) AS first_time FROM DATA_INFO WHERE data_time BETWEEN 1756310400 AND 1756370671 GROUP BY slot) t ON (DATA_INFO.data_time/300)=t.slot AND DATA_INFO.data_time=t.first_time 
		char the_between_column_name[15];

		char sampling_interval[5];
		snprintf(sampling_interval, sizeof(sampling_interval), "%d", sampling_interval_sec);

		if (between_column_name == NULL || strlen(between_column_name) <= 0)
		{
			codb_safe_strncpy(the_between_column_name, "data_time", sizeof(the_between_column_name));
		}
		else 
		{
			codb_safe_strncpy(the_between_column_name, between_column_name, sizeof(the_between_column_name));
		}
		
		codb_safe_strncat(sql_query, " JOIN (SELECT (", sql_query_size);
		codb_safe_strncat(sql_query, the_between_column_name, sql_query_size);
		codb_safe_strncat(sql_query, "/", sql_query_size);
		codb_safe_strncat(sql_query, sampling_interval, sql_query_size);
		codb_safe_strncat(sql_query, ") AS slot", sql_query_size);
		codb_safe_strncat(sql_query, ",", sql_query_size);
		codb_safe_strncat(sql_query, "MIN(", sql_query_size);
		codb_safe_strncat(sql_query, the_between_column_name, sql_query_size);
		codb_safe_strncat(sql_query, ") AS first_time", sql_query_size);
		codb_safe_strncat(sql_query, " ", sql_query_size);
		codb_safe_strncat(sql_query, "FROM DATA_INFO", sql_query_size);
		
		if (start_data_time > 0 && end_data_time > 0)
		{
			char where_clause[MAX_BUF_LEN];
			snprintf(where_clause, MAX_BUF_LEN, " WHERE (%s BETWEEN %d AND %d)", the_between_column_name, start_data_time, end_data_time);
			codb_safe_strncat(sql_query, where_clause, sql_query_size);
		}

		codb_safe_strncat(sql_query, " GROUP BY slot) t", sql_query_size);
		codb_safe_strncat(sql_query, " ON (DATA_INFO.", sql_query_size);
		codb_safe_strncat(sql_query, the_between_column_name, sql_query_size);
		codb_safe_strncat(sql_query, "/", sql_query_size);
		codb_safe_strncat(sql_query, sampling_interval, sql_query_size);
		codb_safe_strncat(sql_query, ")=t.slot", sql_query_size);
		codb_safe_strncat(sql_query, " AND DATA_INFO.", sql_query_size);
		codb_safe_strncat(sql_query, the_between_column_name, sql_query_size);
		codb_safe_strncat(sql_query, "=t.first_time", sql_query_size);
	}
	
	if (sql_and_where != NULL && sql_or_where != NULL)
	{
		codb_safe_strncat(sql_query, " WHERE (", sql_query_size);
		codb_safe_strncat(sql_query, sql_and_where, sql_query_size);
		codb_safe_strncat(sql_query, ") AND (", sql_query_size);
		codb_safe_strncat(sql_query, sql_or_where, sql_query_size);
		codb_safe_strncat(sql_query, ")", sql_query_size);
	}
	else if (sql_and_where != NULL)
	{
	...
	{
		codb_safe_strncat(sql_query, " WHERE 1", sql_query_size);
	}
	...
	if (start_data_time > 0 && end_data_time > 0)
	{
		char sql_between_data_time[MAX_BUF_LEN];
		memset(sql_between_data_time, 0, MAX_BUF_LEN);
		format_sql_between_data_time(between_column_name, start_data_time, end_data_time, sql_between_data_time);
		codb_safe_strncat(sql_query, sql_between_data_time, sql_query_size);
	}

#if 0
	if (sampling_interval_sec > 0)
	{
		char group_by_clause[50];
		if (between_column_name == NULL || strlen(between_column_name) <= 0)
		{
			snprintf(group_by_clause, sizeof(group_by_clause), " GROUP BY (data_time / %d)", sampling_interval_sec);
		}
		else
		{
			snprintf(group_by_clause, sizeof(group_by_clause), " GROUP BY (%s / %d)", between_column_name, sampling_interval_sec);
		}

		codb_safe_strncat(sql_query, group_by_clause, sql_query_size);
	}
#endif

	if (order_column_name != NULL && order_by != NULL)
	{
		codb_safe_strncat(sql_query, " ORDER BY ", sql_query_size);
		codb_safe_strncat(sql_query, order_column_name, sql_query_size);
		codb_safe_strncat(sql_query, " ", sql_query_size);
		codb_safe_strncat(sql_query, order_by, sql_query_size);
	}

	if (limit > 0)
	{
		codb_safe_strncat(sql_query, " LIMIT ", sql_query_size);

		char buf_limit[12];
		snprintf(buf_limit, sizeof(buf_limit), "%d", limit);
		codb_safe_strncat(sql_query, buf_limit, sql_query_size);
	}
...
int cosql_remove_data_between_time(sqlite3 *pdb, int start_data_time, int end_data_time)
{
	...
	if (start_data_time>end_data_time) {
		return COSQL_ERROR;
	}
...
int cosql_remove_data_keep_one_per_group_by_columns(sqlite3 *pdb, int group_columns_count, sql_column_prototype_t *group_columns)
{
	if (pdb == NULL)
	{
		return COSQL_ERROR;
	...
	sql_column_prototype_t *org_columns = group_columns;
	for (i = 0; i < group_columns_count; i++)
	{
		char *column_name = group_columns->name;
		if (strlen(buff)>0) strcat(buff, ", ");
		strcat(buff, column_name);
		group_columns++;
	}

	//- ex. DELETE FROM data_info WHERE data_id NOT IN ( SELECT data_id FROM (SELECT data_id, ROW_NUMBER() OVER (PARTITION BY node_mac, band ORDER BY data_time DESC) AS rn FROM data_info) WHERE rn = 1);
	ret = cosql_exec(pdb, "DELETE FROM %s WHERE data_id NOT IN ( SELECT data_id FROM (SELECT data_id, ROW_NUMBER() OVER (PARTITION BY %s ORDER BY data_time DESC) AS rn FROM %s) WHERE rn = 1)", DATA_TABLE_NAME, buff, DATA_TABLE_NAME);
	if (ret != COSQL_OK)
	{
		codbg("fail to remove data");
		return COSQL_ERROR;
	}
...
int cosql_free_match_columns(sql_column_match_t *match_columns, int match_columns_count)
{
	...
	for (i = 0; i < match_columns_count; i++)
	{
		...
		if ((match_columns_idx->type == COLUMN_TYPE_TEXT ||
			 ...
			 match_columns_idx->type == COLUMN_TYPE_TEXT_JSON) &&
			match_columns_idx->value.t != NULL)
		{
