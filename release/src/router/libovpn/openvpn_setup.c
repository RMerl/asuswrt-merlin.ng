...
int ovpn_write_server_config(ovpn_sconf_t *sconf, int unit) {
	...
	if (!fp) {
		logmessage("openvpn", "ERROR: Failed to write OpenVPN server config!");
		return -1;
	}
...
void ovpn_write_server_keys(ovpn_sconf_t *sconf, int unit) {
	...
	ovpn_write_static_key(sconf, unit);
}


void ovpn_write_server_client_config(ovpn_sconf_t *sconf, int unit) {
	char buffer[64], buffer2[8000];
	FILE *fp_client;
	...
	if (!fp_client) {
		logmessage("openvpn", "WARNING: Failed to create OpenVPN client config for exportation.");
		return;
	}

	if (sconf->tlscrypt == OVPN_TLS_CRYPT_V2)
		min_version = "2.5.0";
	else
		min_version = "2.4.0";

	fprintf(fp_client, "# Config generated by Asuswrt-Merlin %s, requires OpenVPN %s or newer.\n\n", min_version, nvram_safe_get("buildno"));
...
#ifdef RTCONFIG_IPV6
	if (ipv6_enabled()) {
		strlcpy(buffer, nvram_safe_get(ipv6_nvname("ipv6_wan_addr")), sizeof(buffer));
		if (*buffer) {
			fprintf(fp_client, ";Server IPv6 address\n;remote %s %d\n", strtok(buffer,"/"), sconf->port);
		}
	}
#endif
...
// Insert key/certs in client config
	...
	// Static key (for tls-auth/tls-crypt)
	if (sconf->direction >= 0) {
...
void ovpn_write_static_key(ovpn_sconf_t *sconf, int unit) {
	...
	get_ovpn_key(OVPN_TYPE_SERVER, unit, OVPN_SERVER_STATIC, buffer, sizeof(buffer));
	if (*buffer) {
	...
	if (generate) {
		...
		ovpn_get_runtime_filename(OVPN_TYPE_SERVER, unit, OVPN_SERVER_STATIC, filename, sizeof(filename));
		if (eval("openvpn", "--genkey", keytype, filename) != 0) {
			logmessage("openvpn", "Failed to generate tls-crypt-v2 server key for OpenVPN server %d", unit);
			return;
		}

		set_ovpn_key(OVPN_TYPE_SERVER, unit, OVPN_SERVER_STATIC, NULL, filename);
		logmessage("openvpn","Generated new %s static key for OpenVPN server %d.", keytype, unit);
	}
