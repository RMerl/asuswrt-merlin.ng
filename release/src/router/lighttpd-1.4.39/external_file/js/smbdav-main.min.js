...
for(f=0;f<p.length;f++)if(!(p[f].childNodes.length<=0))if(p[f].nodeName=="D:getcontenttype")h=String(p[f].childNodes[0].nodeValue);else if(p[f].nodeName=="D:getuniqueid")l=String(p[f].childNodes[0].nodeValue);else if(p[f].nodeName=="D:getonline")v=String(p[f].childNodes[0].nodeValue);else if(p[f].nodeName=="D:getlastmodified")y=(new Date(String(p[f].childNodes[0].nodeValue))).toLocaleString().toLocaleString();else if(p[f].nodeName=="D:getcontentlength")s=String(size_format(parseInt(p[f].childNodes[0].nodeValue)));
else if(p[f].nodeName=="D:getmac")k=String(p[f].childNodes[0].nodeValue);else if(p[f].nodeName=="D:getip")o=String(p[f].childNodes[0].nodeValue);else if(p[f].nodeName=="D:gettype")w=String(p[f].childNodes[0].nodeValue);else if(p[f].nodeName=="D:getattr"){var E=p[f].childNodes;for(j=0;j<E.length;j++)if(E[j].nodeName=="D:readonly")z=E[j].childNodes[0].nodeValue;else if(E[j].nodeName=="D:hidden")u=E[j].childNodes[0].nodeValue}else if(p[f].nodeName=="D:getuseragent")j=String(p[f].childNodes[0].nodeValue),
g_storage.set("user_agent",j);else if(p[f].nodeName=="D:getroutersync")x=String(p[f].childNodes[0].nodeValue);else if(p[f].nodeName=="D:getmetadata"){E=p[f].childNodes;for(j=0;j<E.length;j++)if(E[j].nodeName=="D:title")B=E[j].childNodes[0].nodeValue;else if(E[j].nodeName=="D:thumb")C=E[j].childNodes[0].nodeValue}}}else D[c].nodeType==1&&D[c].nodeName=="D:href"&&(g=String(D[c].childNodes[0].nodeValue),e="",g.match(/^http/)&&(e=g_storage.get("request_host_url")),f=e+addPathSlash(b),g!=f?(g=g.replace(e,
""),n=g.substring(g.lastIndexOf("/")+1,g.length),n!=""?(t=n,q=n=mydecodeURI(n)):g=""):g="");g!=""&&(h=="httpd/unix-directory"?g_folder_array.push({contenttype:h,furl:g,name:n,uname:t,shortname:q,online:v,time:y,size:s,ip:o,mac:k,uid:l,type:w,freadonly:z,fhidden:u,routersyncfolder:x,matadatatitle:B,thumb:C}):g_file_array.push({contenttype:h,furl:g,name:n,uname:t,shortname:q,online:v,time:y,size:s,ip:o,mac:k,uid:l,type:w,freadonly:z,fhidden:u,routersyncfolder:x,matadatatitle:B,thumb:C}))}}}
...
"D:getuniqueid")q=String(r[g].childNodes[0].nodeValue);else if(r[g].nodeName=="D:getonline")s=String(r[g].childNodes[0].nodeValue);else if(r[g].nodeName=="D:getlastmodified")o=(new Date(String(r[g].childNodes[0].nodeValue))).toLocaleString();else if(r[g].nodeName=="D:getcontentlength")k=String(size_format(parseInt(r[g].childNodes[0].nodeValue)));else if(r[g].nodeName=="D:getmac")z=String(r[g].childNodes[0].nodeValue);else if(r[g].nodeName=="D:getip")w=String(r[g].childNodes[0].nodeValue);else if(r[g].nodeName==
"D:gettype")u=String(r[g].childNodes[0].nodeValue);else if(r[g].nodeName=="D:getattr"){var p=r[g].childNodes;for(h=0;h<p.length;h++)if(p[h].nodeName=="D:readonly")t=p[h].childNodes[0].nodeValue;else if(p[h].nodeName=="D:hidden")x=p[h].childNodes[0].nodeValue}}}else C[j].nodeType==1&&C[j].nodeName=="D:href"&&(l=String(C[j].childNodes[0].nodeValue),d="",l.match(/^http/)&&(d=g_storage.get("request_host_url")),g=d+addPathSlash("/"),l!=g?(l=l.replace(d,""),v=l.substring(l.lastIndexOf("/")+1,l.length),
v!=""?(B=v,y=v=mydecodeURI(v),d=g_list_view.get()==0?12:45,y.length>d&&(y=y.substring(0,d)+"...")):l=""):l="");l!=""&&n=="httpd/unix-directory"&&a.push({contenttype:n,href:l,name:v,uname:B,shortname:y,online:s,time:o,size:k,ip:w,mac:z,uid:q,type:u,freadonly:t,fhidden:x})}a.sort(sortByName);for(B=0;B<a.length;B++)a[B].type=="usbdisk"&&arraymove(a,B,0);createHostList(e,a)}},null,1)}catch(a){alert("catch error: "+a)}}
...
g_storage.set("HostList","");doPROPFIND("/",function(){c!=void 0&&c!="/"&&c.indexOf("https://")==0&&doPROPFIND(c)},0)}else c=g_storage.get("openurl"),doPROPFIND(c);initAudioPlayer();g_window_height=$(window).height();$("#modalWindow").jqm({overlay:70,modal:!0,target:"#jqmContent",onHide:function(a){var b=$(a.w);a.o.remove();b.fadeOut("200",function(){g_show_modal=0;if(g_reload_page==1){var a=addPathSlash(g_storage.get("openurl"));doPROPFIND(a)}})},onShow:function(a){var a=$(a.w),b=$("#jqmContent",
a);$("#jqmTitle",a);var c=(getPageSize()[0]-g_modal_window_width)/2;g_show_modal=1;navigator.userAgent.toLowerCase();var d=function(){var a=-1;navigator.appName=="Microsoft Internet Explorer"&&/MSIE ([0-9]{1,}[.0-9]{0,})/.exec(navigator.userAgent)!=null&&(a=parseFloat(RegExp.$1));return a};g_modal_url!=""&&(b.empty(),$("<iframe src='' frameborder='0' width='100%' height='100%'>").appendTo(b),$("iframe",a).attr("src",g_modal_url));isBrowser("msie")&&d()<=8&&(g_modal_window_height+=25);a.css({width:g_modal_window_width,
