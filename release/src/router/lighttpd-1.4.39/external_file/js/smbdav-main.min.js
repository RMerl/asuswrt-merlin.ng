...
200?alert(m.getString("wol_ok_msg")):alert(m.getString("wol_fail_msg"))}));else if(e=="1")doPROPFIND(b,null,null);else{c=getFileExt(b);e=g_storage.get("webdav_mode");if(c=="mp4"||c=="m4v"||c=="wmv"||c=="avi"||c=="rmvb"||c=="rm"||c=="mpg"||c=="mpeg"||c=="mkv"||c=="mov"||c=="flv")if((e==0||e==2)&&isWinOS()){if(isIE()&&getInternetExplorerVersion()<=8)alert(m.getString("msg_vlcsupport"));else{var g=$("div#modalWindow"),d=myencodeURI(d);this_url=j.substring(0,j.lastIndexOf("/"));var h=g_storage.get("request_host_url");
h.indexOf("://")!=-1&&(h=h.substr(h.indexOf("://")+3));h.indexOf(":")!=-1&&(h=h.substring(0,h.indexOf(":")));h="http://"+h+":"+g_storage.get("http_port")+"/";g_webdav_client.OPENSTREAMINGPORT("/",1,function(e){if(e==200){var b=a.attr("matadatatitle");g_webdav_client.GSL(this_url,this_url,d,0,0,function(a,e,c){a==200&&(a=parseXml(c),a=$(a).find("sharelink").text(),e="",e="/smb/css/vlc_video.html?v="+h+a+"&u="+this_url,e+="&t="+b,e+="&showbutton=1",g_modal_url=e,g_modal_window_width=655,g_modal_window_height=
580,$("#jqmMsg").css("display","none"),$("#jqmTitleText").text(m.getString("title_videoplayer")),g&&g.jqmShow())})}})}return}if(c=="mp3")openAudioPlayer(b);else{if(c=="doc"||c=="docx"||c=="ppt"||c=="pptx"||c=="xls"||c=="xlsx"||c=="pdf")if((isWinOS()||isMacOS())&&!isPrivateIP()){d=myencodeURI(d);this_url=j.substring(0,j.lastIndexOf("/"));g_webdav_client.GSL(this_url,this_url,d,0,0,function(a,e,b){a==200&&(a=parseXml(b),a=decodeURI($(a).find("sharelink").text()),e="",a=g_storage.get("request_host_url")+
"/"+a,e="https://docs.google.com/viewer?url="+encodeURI(a),window.open(e))});return}c=="jpg"||c=="jpeg"||c=="png"||c=="gif"?(j=getPageSize(),g_image_player.show(b,j[0],j[1],g_file_array)):window.open(b)}}}
...
g_select_array[b].title,d=g_select_array[b].isdir,c=g_storage.get("request_host_url")+"/"+c;d==1?a.push(c):download_file(c)}download_folder(a)}});$("div#btnRename").click(function(){if($(this).hasClass("disable"))return!1;g_select_array.length==1&&open_rename_window(g_select_array[0].title,g_select_array[0].uhref,g_select_array[0].isdir)});$("div#btnLock").click(function(){if($(this).hasClass("disable"))return!1;g_select_array.length==1&&g_webdav_client.LOCK(g_select_array[0].uhref,"",function(a,
...
"none");$("div#boxSearch").css("display","none");$("#upload_panel").css("display","block");$("#upload_panel").css("left","1999px");$("#upload_panel").animate({left:"0px"},"slow");var a=addPathSlash(g_storage.get("openurl"));$("#upload_panel iframe").attr("src","/smb/css/upload.html?u="+a);$("#function_help").text(m.getString("msg_uploadmode_help"));adjustLayout()});$("#btnCancelUpload").click(closeUploadPanel);$(".mediaListDiv").mousedown(function(){var a=$(this).attr("qtype"),b=g_storage.get("openurl");
g_storage.set("data-id","");$(".mediaListDiv").removeClass("down");$(this).addClass("down");g_ui_mode.set(a);doPROPFINDMEDIALIST(b,!1,function(){},a,"0","50",null,"TIMESTAMP","DESC")});$("div#btnAiMusicPopupMenux").click(function(){var a="<div class='popupmenu' id='popupmenu'>";a+="<div class='menuitem' id='classifyByAlbum'>";a+="<div class='menuitem-content' style='-webkit-user-select: none;'>";a+="<span class='menuitem-icon a-inline-block' style='-webkit-user-select: none;'>&nbsp;</span>";a+="<span class='menuitem-container a-inline-block' style='-webkit-user-select: none;'>";
...
c);else if(a=="share2weibo")open_sharelink_window("weibo",c);else if(a=="share2qq")open_sharelink_window("qq",c);else if(a=="sharelink"){a=[];for(c=0;c<g_select_array.length;c++)a.push(g_select_array[c].uhref);open_sharelink_window("other",a);a=null}},items:{"delete":{name:m.getString("func_delete"),disabled:function(){if(g_storage.get("aimode")==1||g_storage.get("aimode")==2||g_storage.get("aimode")==3||g_query_type!=0)return!0}},submenu_copymove:{name:m.getString("title_copymove"),items:{copy:{name:m.getString("func_copy")},
...
$("#fileview").css("height",c);$("#function_help").css("left",$("#main_left_region").width());if(c=$("div#modalWindow"))a=(b-g_modal_window_width)/2,c.css("left",a);a=(b-$(".select-window").width())/2;$(".select-window").css("left",a);$("#header-fixed").css("width",$("#ntl").width());b=$("#header_region").height()+$("#main_right_region #infobar").height()+2;$("#main_right_container #hintbar").css("display")=="block"&&(b+=$("#main_right_container #hintbar").height());$("#header-fixed").css("top",b);
g_fileview_only==1&&($("#button_panel").hide(),$("#main_right_region").css("left","0px"),$("#main_right_region").css("width","100%"),$("#main_right_region").css("height","100%"));$("#audio_player_panel").show();adjustUploadLayout();g_image_player&&g_image_player.adjustLayout()}
