...
static void ListRedirectionsToFile(struct UPNPUrls * urls,
                             struct IGDdatas * data, const char *file)
{
	int r;
	int i = 0;
	char index[6];
	...
	UPNP_GetPortMappingNumberOfEntries(urls->controlURL, data->servicetype, &num);
	printf("PortMappingNumberOfEntries : %u\n", num);*/
	fprintf(fp, " i protocol exPort->inAddr:inPort description remoteHost leaseTime\n");
	do {
		snprintf(index, 6, "%d", i);
		rHost[0] = '\0'; enabled[0] = '\0';
		duration[0] = '\0'; desc[0] = '\0';
...
int main(int argc, char ** argv)
{
	   ...
	   || (command == 'D' && commandargc<1))
	{
		...
		fprintf(stderr, "       \t%s [options] -f file_path\n\t\tStore the list of redirections to the file\n", argv[0]);
	                             ...
	                             localport, ipv6, ttl, &error)))
	{
		  ...
		  || (i = UPNP_GetValidIGD(devlist, &urls, &data, lanaddr, sizeof(lanaddr))))
		{
			...
			case 'f':
				if(commandargc > 0)
					ListRedirectionsToFile(&urls, &data, commandargv[0]);
				break;
			default:
