...
struct json_object* nf_node_to_json(nf_node_t *nn, struct list_head *arlist)
{
    ...
    if (nn->layer1_info.eth_type == PHY_UNKNOWN) get_eth_type_from_arp(nn, arlist);
...
int nf_list_to_json(struct list_head *iplist, struct list_head *arlist)
{
    ...
    list_for_each_entry(nn, iplist, list) {
        if (nn->isv4 && !is_in_lanv4(&nn->srcv4)) continue;
...
struct json_object* nf_node_statistics_to_json(nf_node_t *nn, struct list_head *arlist)
{
    ...
    json_object_object_add(node_obj, "src_ip", json_object_new_string(ipstr));
    json_object_object_add(node_obj, "src_mac", json_object_new_string(nn->src_mac));

    json_object_object_add(node_obj, "up_ttl_bytes", json_object_new_int64(nn->up_ttl_bytes));
    json_object_object_add(node_obj, "up_dif_bytes", json_object_new_int64(nn->up_dif_bytes));
    json_object_object_add(node_obj, "up_speed", json_object_new_int64(nn->up_speed / nf_conntrack_period));

    json_object_object_add(node_obj, "dn_ttl_bytes", json_object_new_int64(nn->dn_ttl_bytes));
    json_object_object_add(node_obj, "dn_dif_bytes", json_object_new_int64(nn->dn_dif_bytes));
    json_object_object_add(node_obj, "dn_speed", json_object_new_int64(nn->dn_speed / nf_conntrack_period));

    json_object_object_add(phy_obj, "ifname", json_object_new_string(nn->layer1_info.ifname));
    json_object_object_add(phy_obj, "is_guest", json_object_new_int(nn->layer1_info.is_guest));
...
struct json_object* nf_node_tcp_to_json(nf_node_t *nn)
{
    char ipstr[INET6_ADDRSTRLEN];
    ...
    json_object_object_add(node_obj, "tcp_state", json_object_new_string(tcp_state_str[nn->state]));
    json_object_object_add(node_obj, "time", json_object_new_int64(nn->time_in));

    return node_obj;
}

int nf_list_tcp_to_json(struct list_head *tcplist)
{
    nf_node_t *nn;
    struct json_object *json_obj;
    ...
    list_for_each_entry(nn, tcplist, list) {
        json_obj = nf_node_tcp_to_json(nn);
        json_object_array_add(ary_obj, json_obj);
    }

    /* print it */
    json_object_to_file(JSON_OUTPUT_TCP_FILE, ary_obj);
