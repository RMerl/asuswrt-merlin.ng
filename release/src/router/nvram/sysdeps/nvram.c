...
#ifdef NVRAM_FILE
int nvram_save_new(char *file, char *buf, unsigned int buf_size)
{
	FILE *fp;
	char *name;
	unsigned long count, filelen, i;
	...
	for (name = buf; *name && *name != 0 ; name += strlen(name) + 1)
	{
		count = count + strlen(name) + 1;
	}
	count = buf_size;
	...
	filelen = count + (1024 - count % 1024);
	rand = get_rand() % 30;
	//printf("random number: %x, count = %ld\n", rand, count);
...
int nvram_restore_new(char *file, char *buf)
{
	FILE *fp;
	char header[8], *p, *v;
	unsigned long count, i, len, *lenptr;
	...
	count = fread(header, 1, 8, fp);
	// if (count>=4)
	if (count>=8 && strncmp(header, PROFILE_HEADER_NEW, 4) == 0)
	{
		lenptr = (unsigned long *)(header+4);
		len = *lenptr & 0xffffff;
		randptr = (unsigned char *)(header + 7);
		rand = *randptr;
		count = fread(buf, 1, len, fp);
		//printf("random number: %x, count = %ld, len = %ld\n", rand, count, len);
		for (i = 0; i < count; i++)
		{
	...
	while (*p || p-buf<=count)
	{
		...
		if (v != NULL)
		{
			*v++ = '\0';
			...
			//if (issyspara(p))	// old default.c is not in use
			nvram_set(p, v);
	...
	for (; *argv; argv++) {
		if (!strncmp(*argv, "get", 3)) {
			...
			if (*++argv) {
				...
				if(ret == 2)
					puts("Write illegal nvram to CFE!");
#else
		...
		else if (!strncmp(*argv, "save", 4)) 
		{
			if (*++argv) 
			{
				l_buf_size = nvram_getall(buf, MAX_NVRAM_SPACE);
				nvram_save_new(*argv, buf, l_buf_size);
			}
		...
		else if (!strncmp(*argv, "restore", 7)) 
		{
			...
			for (name = buf; *name; name += strlen(name) + 1)
				puts(name);
			size = (unsigned int) name - (unsigned int) buf;
