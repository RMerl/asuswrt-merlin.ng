...
#define FIELD_WIFISYS "WIFISYS" // FIELD_WIFISYS>node type>IP>MAC>2G's ifname,2G's chip>5G's ifname,5G's chip>2G's country/rev>5G's country/rev
...
#define FIELD_ROAMING "ROAMING" // FIELD_ROAMING>node type>IP>MAC>STA's MAC>STA's RSSI>candidate_rssi_criteria>candidate's MAC>candidate's RSSI
...
extern int detect_wifi_noise(char *ifname, char *output, int len);
extern int detect_wifi_mcs(char *ifname, char *output, int len);
