...
static void free_sched_v2_list(sched_v2_t **sched_v2_list){
...
static sched_v2_type get_sched_v2_etype(const char c_type) {
	sched_v2_type e_type = SCHED_V2_TYPE_TIMESTAMP;
	//SCHED_DBG("%d", c_type);
	if (c_type == 'D')
...
static char get_sched_v2_ctype(const sched_v2_type e_type) {
	char c_type = 'T';
	//SCHED_DBG("%d", c_type);
	if (e_type == SCHED_V2_TYPE_DAY)
		c_type = 'D';
	else if (e_type == SCHED_V2_TYPE_WEEK)
		c_type = 'W';
	return c_type;
}
...
static int expand_sched_v2_same_w_period(sched_v2_t **sched_v2_list) {
	...
	for(curr = tmp_sched_v2_list; curr != NULL; curr = curr->next) {
		...
		if (is_merged_day_of_week(curr->value_w.day_of_week)) {
			SCHED_DBG("Meged day of week detected %u", curr->value_w.rule_to_number);
			for (i = 6; i >= 0; i--) {
...
static int parse_str_v2_to_sched_v2_list(const char *str_sched_v2, sched_v2_t **sched_v2_list, int merge_same_period) {
	int ret = -1, i;
	char tmp_str_sched[MAX_NVRAM_SCHED_LEN];
	char word[MAX_NVRAM_SCHED_LEN], *next_word;
			...
			if ((*tmp_sched_v2)->type == SCHED_V2_TYPE_TIMESTAMP) {
				...
				switch(i) {
					case SCHED_V2_D_IDX_ENABLE:
						...
						if (!(*tmp_sched_v2)->value_d.enable) { // Skip disabled rules
							free((*tmp_sched_v2));
							*tmp_sched_v2 = NULL;
							SCHED_DBG("Skip disabled rule. rule=%s", word);
							goto BYPASS;
						}
						break;
				...
				switch(i) {
					case SCHED_V2_W_IDX_ENABLE:
						...
						if (!(*tmp_sched_v2)->value_w.enable) { // Skip disabled rules
							free((*tmp_sched_v2));
							*tmp_sched_v2 = NULL;
							SCHED_DBG("Skip disabled rule. rule=%s", word);
							goto BYPASS;
	...
	if (!parse_str_v2_to_sched_v2_list(sched_str, &sched_v2_list, 1)) {
		...
		for (sched_v2 = sched_v2_list; sched_v2 != NULL; sched_v2 = sched_v2->next) {
			...
			if (sched_v2->type == SCHED_V2_TYPE_DAY && 
				...
				((sched_v2->value_w.day_of_week & (1 << ptm->tm_wday)) > 0) &&
				(start_min <= curr_min) && (curr_min < end_min)) {
				ret = 0;
				break;
			}
...
void convert_pc_sched_v1_to_sched_v2() {
	...
	if (!nvram_get("MULTIFILTER_MACFILTER_DAYTIME"))
		return;
	...
	if (!nvram_get("MULTIFILTER_MACFILTER_DAYTIME_V2")) {
		foreach_62(word, str_sched_v1, next_word) {
		...
		if (changed) {
			...
			nvram_unset("MULTIFILTER_MACFILTER_DAYTIME");
			nvram_commit();
		}
	}
}

static void get_event_day_limits(sched_v2_t *sched_v2, int *start_day, int *end_day) {
	int i = 0;
	int s_found = 0;

	if (!sched_v2 || !start_day || !end_day)
		return;

	for (i = 0; i < 7; i++) {
		if ((sched_v2->value_w.day_of_week & (1 << i)) > 0) {
			if (!s_found) {
				*start_day = i;
				s_found = 1;
			}
...
pc_event_s *get_event_list_by_sched_v2(pc_event_s **target_list, char *sched_v2_str) {
	...
	if (!parse_str_v2_to_sched_v2_list(sched_v2_str, &sched_v2_list, 0)) {
		sched_v2_t *sched_v2;
		//SCHED_DBG("now=%ld", now);
