...
//#ifdef RTCONFIG_RTK_SWITCH_VLAN
	case 299:	/* reset rtk vlan */
	case 300:	/* init rtk vlan */
	...
	case GET_TMODE:	/* phyTestMode_get */
//#endif			
		p = (void*)&value;
...
unsigned int rtkswitch_wanPort_phyStatus(int wan_unit)
{
	...
	{
		value = 0;
		if (rtkswitch_ioctl(0, value, -1) < 0)
			return -1;
		return value;
	}
...
unsigned int rtkswitch_lanPorts_phyStatus(void)
{
	...
	if(rtkswitch_ioctl(3, value, -1) < 0)
		return -1;
...
unsigned int __rtkswitch_WanPort_phySpeed(int wan_unit)
{
	...
	{
		value = 0;
		if (rtkswitch_ioctl(13, value, -1) < 0)
			return -1;
		return value;
	}
