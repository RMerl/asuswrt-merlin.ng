...
#ifdef RTCONFIG_AMAS
/* unit: start from 0, it should be 0 only in bluecave */
uint32_t get_amas_eth_phy_status(int unit)
{
	char tmp[100];
	char *wan_ifname;
	char buf[32];
	...
	wan_ifname = nvram_safe_get("eth_ifnames");
	if(strlen(wan_ifname) <= 0 || strlen(wan_ifname) > 4){
...
int check_wave_ready(int action)
{
	...
	while(nvram_get_int("wave_ready") == 0){
		_dprintf("[%s][%d][%d] current action:[%d],"
			" waiting wave_ready=1\n",
			__func__, __LINE__,
			action, nvram_get_int("wave_action_cur"));
		sleep(3);
		wait_count++;
		if(wait_count > 20){
			_dprintf("[%s][%d] wait wave_ready=1 over 60 seconds,"
				" please check wave_monitor status "
				"(current action:[%d]), skip [%d]\n",
				__func__, __LINE__,
				nvram_get_int("wave_action_cur"), action);
			ret = -1;
...
int trigger_wave_monitor_and_wait(char *caller, int line, int action, int wait)
{
	if(check_wave_ready(action) == -1){
		return;
	}
	_dprintf("[%s][%d] %s(%d) trigger action:[%d]\n",
			__func__, __LINE__, caller, line, action);
...
int trigger_wave_monitor(char *caller, int line, int action)
{
	trigger_wave_monitor_and_wait(caller, line, action, 5);
}
