...
/* GT-AX6000N (AP.HK01 demo board) mapping
 ...
 *     -2:	invalid parameter
 */
int qca8075_vlan_set(int idx, int vid, int prio, int mbr, int untag)
{
	dbg("%s(): idx %d vid %d prio %d mbr 0x%x untag 0x%x\n", __func__, idx, vid, prio, mbr, untag);
	return 0;
}
 ...
 *  otherwise:	fail
 */
static int get_qca8075_port_info(unsigned int port, unsigned int *link, unsigned int *speed)
{
	dbg("%s: port %d\n", __func__, port);
 ...
 */
static void get_qca8075_phy_linkStatus(unsigned int mask, unsigned int *linkStatus)
{
	int i;
	...
	for (i = 0; m > 0 && !value; ++i, m >>= 1) {
		...
		get_qca8075_port_info(i, &value, NULL);
		value &= 0x1;
	}
	*linkStatus = value;
}
...
static void config_qca8075_LANWANPartition(int type)
{
	...
	// LAN 
	qca8075_vlan_set(1, 1, 0, (lan_mask | CPU_PORT_LAN_MASK), (lan_mask | CPU_PORT_LAN_MASK));

	// WAN & DUALWAN
	if (sw_mode == SW_MODE_ROUTER) {
		switch (wanscap_wanlan) {
		case WANSCAP_WAN | WANSCAP_LAN:
			qca8075_vlan_set(2, 2, 0, (wan_mask      | CPU_PORT_WAN_MASK), wan_mask);
			qca8075_vlan_set(3, 3, 0, (wans_lan_mask | CPU_PORT_WAN_MASK), wans_lan_mask);
			eval("swconfig", "dev", MII_IFNAME, "set", "enable_vlan", "1"); // enable vlan
			break;
		case WANSCAP_LAN:
			qca8075_vlan_set(2, 2, 0, (wans_lan_mask | CPU_PORT_WAN_MASK), (wans_lan_mask | CPU_PORT_WAN_MASK));
			break;
		case WANSCAP_WAN:
			qca8075_vlan_set(2, 2, 0, (wan_mask      | CPU_PORT_WAN_MASK), (wan_mask      | CPU_PORT_WAN_MASK));
			break;
		default:
...
static void get_qca8075_WAN_Speed(unsigned int *speed)
{
	...
	for (i = 0; m; ++i, m >>= 1) {
		...
		get_qca8075_port_info(i, NULL, (unsigned int*) &t);
		t &= 0x3;
		if (t > v)
			v = t;
	}
...
static void link_down_up_qca8075_PHY(unsigned int mask, int status)
{
	int i;
	char idx[2];
	unsigned int m;
...
int get_qca8075_PHY_power(int port)
{
	FILE *fp;
	int rlen;
	char buf[64];
...
static void initialize_Vlan(int stb_bitmask)
{
	...
	// DUALWAN
	if (wanscap_lan) {
		qca8075_vlan_set(wans_lan_vid, wans_lan_vid, 0, (wans_lan_mask | CPU_PORT_WAN_MASK), wans_lan_mask);
		if(wans_lan_vid == 3)
			eval("swconfig", "dev", MII_IFNAME, "set", "enable_vlan", "1"); // enable vlan
	}
...
static void create_Vlan(int bitmask)
{
	...
	if (mbr & 0x200) {	//Internet && WAN port
		char pvid[8];
		qca8075_vlan_set(2, vid, prio, mbr_qca, untag_qca);
		snprintf(pvid, sizeof(pvid), "%d", vid);
		eval("swconfig", "dev", MII_IFNAME, "port", "set", "pvid", pvid);
	}
	else {	//IPTV, VoIP port
		qca8075_vlan_set(-1, vid, prio, mbr_qca, untag_qca);
	}
	eval("swconfig", "dev", MII_IFNAME, "set", "enable_vlan", "1"); // enable vlan
	eval("swconfig", "dev", MII_IFNAME, "set", "apply"); // apply changes
}
...
void ATE_port_status(void)
{
	...
	for (i = 0; i < NR_WANLAN_PORT; i++) {
		pS.link[i] = 0;
		pS.speed[i] = 0;
		get_qca8075_port_info(lan_id_to_port_nr(i), &pS.link[i], &pS.speed[i]);
	}
