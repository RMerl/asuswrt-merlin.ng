...
#endif /* WIFI7_SDK_20250122, WIFI7_SDK_20250506, WIFI8_SDK_20251126, WIFI8_SDK_20260204 || WIFI8_SDK_20260402 */
...
#endif /* WIFI8_SDK_20251126 || WIFI8_SDK_20260204 || WIFI8_SDK_20260402 */
...
wl_uhriovar_setint(char *ifname, char *iovar, char *subcmd, int val)
{
	...
	if (strcmp(subcmd, "features") == 0) {
		v32.id = WL_UHR_CMD_FEATURES;
		v32.len = 4;
		v32.val = (uint32)val;

		subcmd_len = sizeof(v32.id) + sizeof(v32.len) + v32.len;
	} else if (strcmp(subcmd, "enab") == 0) {
		v32.id = WL_UHR_CMD_ENAB;
		v32.len = 4;
		v32.val = (uint32)val;

		subcmd_len = sizeof(v32.id) + sizeof(v32.len) + v32.len;
	} else if (strcmp(subcmd, "dps_assist") == 0) {
		v32.id = WL_UHR_CMD_DPS_ASSIST;
		v32.len = 4;
		v32.val = (uint32)val;

		subcmd_len = sizeof(v32.id) + sizeof(v32.len) + v32.len;
	} else if (strcmp(subcmd, "bssuhrmode") == 0) {
		v32.id = WL_UHR_CMD_BSSUHRMODE;
		v32.len = 4;
		v32.val = (uint32)val;

		subcmd_len = sizeof(v32.id) + sizeof(v32.len) + v32.len;
	} else if (strcmp(subcmd, "msglevel") == 0) {
		v32.id = WL_UHR_MSGLEVEL;
		v32.len = 4;
		v32.val = (uint32)val;

		subcmd_len = sizeof(v32.id) + sizeof(v32.len) + v32.len;
	} else if (strcmp(subcmd, "sbt") == 0) {
		v32.id = WL_UHR_CMD_SBT;
		v32.len = 4;
		v32.val = (uint32)val;

		subcmd_len = sizeof(v32.id) + sizeof(v32.len) + v32.len;
	} else if (strcmp(subcmd, "npca_enab") == 0) {
		v32.id = WL_UHR_CMD_NPCA_ENAB;
		v32.len = 4;
		v32.val = (uint32)val;
...
/*
 * Set uhr subcommand with buffer data
 */
int
wl_uhriovar_set_buf(char *ifname, char *iovar, char *subcmd, void *buf, uint16 buf_len)
{
	char smbuf[WLC_IOCTL_SMLEN] = {0};
	bcm_xtlv_t *xtlv;
	char *p = smbuf;
	int namelen, xtlv_len, iolen;
	uint16 cmd_id;

	if (ifname == NULL || iovar == NULL || subcmd == NULL || buf == NULL) {
		return BCME_BADARG;
	}
	...
	/* Determine command ID from subcmd string */
	if (strcmp(subcmd, "sbt_smd_id") == 0) {
		cmd_id = WL_UHR_CMD_SBT_SMD_ID;
	} else {
		/* Other uhr subcommands not yet supported */
		return BCME_UNSUPPORTED;
	}
	...
	/* XTLV header size + buffer length */
	xtlv_len = BCM_XTLV_HDR_SIZE + buf_len;

	iolen = namelen + xtlv_len;
	...
	/* copy iovar name including null */
	(void)memcpy_s(p, sizeof(smbuf), iovar, namelen);
	p += namelen;

	/* build XTLV structure */
	xtlv = (bcm_xtlv_t *)p;
	xtlv->id = htol16(cmd_id);
	xtlv->len = htol16(buf_len);
	(void)memcpy_s(xtlv->data, buf_len, buf, buf_len);
...
#if defined(WIFI8_SDK_20260204) || defined(WIFI8_SDK_20260402)
/* Get the first and second network block IDs from wpa_supplicant */
int
wl_wlif_wpa_supp_get_network_ids(char *ifname, int *first_id, int *second_id)
{
	FILE *fp;
	char cmd[2*WLIF_BUF_512] = {0};
	char line[256] = {0x0};
	char *token;
	int network_id = -1;
	...
	*first_id = -1;
	...
	snprintf(cmd, sizeof(cmd), "wpa_cli -i %s list_networks", ifname);
	dprintf("Info: rc: %d: %s\n", __LINE__, cmd);
	fp = popen(cmd, "r");
	if (fp == NULL) {
		dprintf("Err: %s popen failed for %s\n", __func__, cmd);
		return -1;
	}

	/* Parse output to get first and second network IDs */
	while (fgets(line, sizeof(line), fp) != NULL) {
		...
		/* Skip header line */
		if (!header_skipped) {
			if (strstr(line, "network id") != NULL) {
				header_skipped = TRUE;
			}
			continue;
		}
		...
		/* Extract network ID from first column */
		token = strtok(line, "\t ");
		if (token != NULL) {
			network_id = (int)strtoul(token, NULL, 0);
			if (count == 0) {
				*first_id = network_id;
				count++;
			} else if (count == 1 && second_id != NULL) {
				*second_id = network_id;
				break;
			}
		}
	}
	...
	dprintf("Info: rc: %d: first network_id %d, second network_id %d for %s\n",
		__LINE__, *first_id, (second_id ? *second_id : -1), ifname);
	return (*first_id >= 0) ? 0 : -1;
}
#endif /* WIFI8_SDK_20260204 || WIFI8_SDK_20260402 */
...
wl_ehtiovar_setint(char *ifname, char *iovar, char *subcmd, int val)
{
	...
	if (strcmp(subcmd, "features") == 0) {
		...
		subcmd_len = sizeof(v32.id) + sizeof(v32.len) + v32.len;
                ...
                subcmd_len = sizeof(v32.id) + sizeof(v32.len) + v32.len;
#if defined(WIFI8_SDK_20260204) || defined(WIFI8_SDK_20260402) || defined(WIFI7_SDK_20250506)
...
/* get unit */
	for (i = 0; i < len; i++) {
		if (mlo_config_og[wl[i]] >= 0) {
			...
			/* Skip 2.4G interface */
			if (wl[i] == WL_2G_BAND) {
				MLO_API_DBG("Skip 2.4G interface wl%d\n", wl[i]);
				continue;
			}
#endif
			if (first)
...
int apply_mlo_rp_settings(int mlo_client_mode)
{
	...
	foreach (word, mld0_ifnames, next) {
		...
		nvram_set(strlcat_r(prefix2, "wifi_mode", tmp, sizeof(tmp)), "bn");
#endif
#endif
	}
...
#if defined(RTCONFIG_WIFI_MODE)
#if defined(RTCONFIG_WIFI8)
		_dprintf("chk convert %s=[%s]\n", strlcat_r(prefix2, "wifi_mode", tmp, sizeof(tmp)), nvram_safe_get(strlcat_r(prefix2, "wifi_mode", tmp, sizeof(tmp))));
#endif
#endif
	}
