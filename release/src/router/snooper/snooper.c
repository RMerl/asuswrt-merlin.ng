...
static int init_socket(char *ifname, int rcvsize, unsigned char *ifhwaddr, in_addr_t *ifaddr)
{
...
#ifdef SOCK_CLOEXEC
	fd = socket(PF_PACKET, SOCK_DGRAM | SOCK_CLOEXEC | SOCK_NONBLOCK, htons(ETH_P_IP));
	if (fd < 0) {
...
int main(int argc, char *argv[])
{
	...
	fd = init_socket(ifbridge ? : ifswitch, sizeof(packet) * 16, ifhwaddr, &ifaddr);
	if (fd < 0)
		goto socket_error;
	if (switch_init(ifswitch, ifvid, cputrap) < 0)
		goto switch_error;
	if (init_timers() < 0)
		goto timer_error;
	if (init_cache() < 0)
