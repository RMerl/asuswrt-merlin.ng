...
#define path "./example.db3"


int sqlite_getTable(int table, int *row, int *col, char ***result, char *Name, int Direction)
{
	char sql_str[128];
	char *errMsg;
	sqlite3 *db;
...
int sqlite_update(int table, char *para[])
{
	char sql_str[256];
	sqlite3 *db;
	int ret=0;
...
int sqlite_delete(int table, char *Name)
{
	char sql_str[256];
	char sql_str1[256];
	sqlite3 *db;
...
void main(void)
{
  ...
  // char *key="12345";	
   ...
   SQLITEAPI(update(TABLE_ACC_GROUP, parameters));   
   ...
   SQLITEAPI(update(TABLE_ACC_USER, parameters));   
   ...
   SQLITEAPI(getTable(TABLE_ACC_USER, &rows, &cols, &result, "*", 0)); 
   ...
   SQLITEAPI(getTable(TABLE_ACC_GROUP, &rows, &cols, &result, "*", 0)); 
   ...
   SQLITEAPI(freeTable(result));
   SQLITEAPI(delete(TABLE_ACC_USER, "John"));
