...
backup_file_types BACK_FILE_TYPES[] = {
	...
	{TYPE_IPSEC, IPSEC_BACKUP_TYPE, NULL, 0, CLOUD_FILE_PREFIX_NAME_IPSEC, CLOUD_FILE_EXT_COMPRESS, 3},
	{
		...
		NULL,
		0,
		CLOUD_FILE_PREFIX_NAME_OPENVPN,
		CLOUD_FILE_EXT_COMPRESS,
		3,
	},
...
char *attach_extra_options(char *options_buffer)
{

	DECLARE_CLEAR_MEM(char, tmp, 10);

	//- extra options[tag_name: tag_value]
	//- a: app_mnt
	int app_mnt = nvram_get_int("app_mnt");
	if (app_mnt > 0)
	{
		snprintf(tmp, sizeof(tmp), "_a%d", app_mnt);
		strncat(options_buffer, tmp, strlen(tmp));
	}
...
int generate_backup_file(backup_file_types *handler, char *filename, int update_time)
{
	...
	DECLARE_CLEAR_MEM(char, options_buffer, 128);

	//- format: {type}_{formated router mac}_{uploader reset time}_{upload offset time}_{extra_options}.{file_ext}
	snprintf(filename, MAX_FILENAME_LEN, BACKUP_FILE_FORMAT, handler->bf_name, g_formated_router_mac, uploader_reset_time, offset_time, attach_extra_options(options_buffer), handler->bf_ext);
...
backup_files *insert_backup_file(backup_files *backup_file, const int file_len, const char *filename)
{
	...
	DECLARE_CLEAR_MEM(char, s_options, 128);
	DECLARE_CLEAR_MEM(char, s_file_ext, MAX_FILE_EXT_LEN);

	if (sscanf(filename, "%[^_]_%[^_]_%[^_]_%[^_]_%[^.].%s", s_type, s_mac, s_reset_time, s_upload_time, s_options, s_file_ext) == 6)
	{
		//- e.g. setting_0492266A7060_1680862400_1680862430_[options].cfg
		// Cdbg(API_DBG, "filename=%s, type=%s, mac=%s, reset_time=%s, upload_time=%s, s_options=%s, s_file_ext=%s",
		// 	filename, s_type, s_mac, s_reset_time, s_upload_time, s_options, s_file_ext);
	}
	else if (sscanf(filename, "%[^_]_%[^_]_%[^_]_%[^.].%s", s_type, s_mac, s_reset_time, s_upload_time, s_file_ext) == 5)
	{
	...
	{
		Cdbg(API_DBG, "Invalid file format(%s)!", filename);
		return NULL;
	}
...
int get_cloud_access_token(char *access_token)
{
	...
	do
	{
		...
		if (status != 0)
		{
			UPLOADER_DBG(API_DBG, "Fail to get cloud access token, status=%d", status);
			return -1;
		}
		else if (strncmp(rt.status, "0", 1) != 0)
		{
			UPLOADER_DBG(API_DBG, "Fail to get cloud access token, rt.status=%s", rt.status);
			...
			if (retry_count > 5)
			{
				UPLOADER_DBG(API_DBG, "Maximum number of errors exceeded, retry_count=%d", retry_count);
				return -1;
			}
...
int list_cloud_file(const char *access_token)
{
	...
	if (status != 0)
	{
		UPLOADER_DBG(API_DBG, "Fail to list cloud file, status=%d", status);
		return -1;
	}
	else if (strncmp(lf.status, "0", 1) != 0)
	{
		UPLOADER_DBG(API_DBG, "Fail to list cloud file, status=%s", lf.status);
		return -1;
	}

	if (strlen(lf.files) <= 0)
	{
		UPLOADER_DBG(API_DBG, "The file data is empty.");
		return -1;
	}
	...
	if (files_obj)
	{
		...
		Cdbg(API_DBG, "files_len=%d", files_len);
	...
	{
		UPLOADER_DBG(API_DBG, "Fail to parse file data!");
		return -1;
	}
...
void reorganize_cloud_file(const char *access_token)
{
	...
	for (handler = &BACK_FILE_TYPES[0]; handler->id > 0; handler++)
	{
		...
		{
			UPLOADER_DBG(API_DBG, "%s: Delete the locked files(%d) exceeding the maximum number(%d)", handler->bf_name, count2, MAX_RESET_TIME_BACKUP_NUM);
...
int delete_cloud_file(const char *access_token, const char *file_name)
{
	...
	if (status == 0)
	{
		UPLOADER_DBG(API_DBG, "Success to delete cloud file[%s]", file_name);
	}
	else
	{
		UPLOADER_DBG(API_DBG, "Fail to delete cloud file[%s], status = %d", file_name, status);
	}
...
int upload_cloud_file(const char *access_token, const char *file_name)
{
	...
	if (status != 204)
	{
		UPLOADER_DBG(API_DBG, "Fail to upload file[%s] to S3, status=%d", file_name, status);
		return -1;
	}
...
SEND_UPLOAD_FILE_AGAIN:
	...
	UPLOADER_DBG(API_DBG, "Success to upload backup file[%s].", file_name);
...
PROC_JSON_DATA get_proc_json_fn(WS_ID wi)
{
	...
	if (wi == e_router_check_token)
		return proc_router_check_token_json;
	if (wi == e_router_list_file)
		return proc_router_list_file_json;
	if (wi == e_router_del_file)
		return proc_router_del_file_json;
	if (wi == e_router_get_upload_file_url)
		return proc_router_get_upload_file_url_json;
	if (wi == e_router_upload_file)
		return proc_router_upload_file_json;
	if (wi == e_router_get_debug_token)
		return proc_router_get_debug_token_json;
}
