...
void *cm_ipcPacketHandler(void *args) {
    ...
    if (root) {
        ...
        char msg[512] = {0};
        snprintf(msg, sizeof(msg) ,"{\"api\":\"%s\", \"file_path\":\"%s\",\"file_name\":\"router_%s_%s.cfg\"}", api, UPLOADER_FOLDER, g_formated_router_mac, timestamp_str);

        Cdbg(APC_DBG, "cm_ipcPacketHandler write msg = %s", msg);

        char upload_file[128] = {0};

        snprintf(upload_file, sizeof(upload_file), "%srouter_%s_%s.cfg", UPLOADER_FOLDER, g_formated_router_mac, timestamp_str);
