...
try_again:
	...
	if (left < 0) {
		if (errno != EINTR && errno != EAGAIN)
			DBGPRINT(RT_DEBUG_ERROR, "netlink: recvfrom failed: %s",
				   strerror(errno));
		return;
	}
	...
	if (left > 0) {
		DBGPRINT(RT_DEBUG_TRACE, "netlink: %d extra bytes in the end of "
...
struct netlink_data * netlink_init(struct netlink_config *cfg)
{
	...
	if (netlink->sock < 0) {
		DBGPRINT(RT_DEBUG_ERROR, "netlink: Failed to open netlink "
	...
	{
		DBGPRINT(RT_DEBUG_ERROR, "netlink: Failed to bind netlink "
			   "socket: %s", strerror(errno));
		netlink_deinit(netlink);
		return NULL;
	}
			   ...
			   int linkmode, int operstate)
{
	...
	DBGPRINT(RT_DEBUG_ERROR, "netlink: Operstate: linkmode=%d, operstate=%d",
		   linkmode, operstate);

	ret = send(netlink->sock, &req, req.hdr.nlmsg_len, 0);
	if (ret < 0) {
		DBGPRINT(RT_DEBUG_ERROR, "netlink: Sending operstate IFLA "
