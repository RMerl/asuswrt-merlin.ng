				...
				int result)
{
	...
	if (gas->current == NULL) {
		wpa_printf(MSG_ERROR, "GAS: Unexpected TX status: freq=%u dst="
	...
	if (os_memcmp(dst, query->addr, ETH_ALEN) != 0) {
		wpa_printf(MSG_ERROR, "GAS: TX status for unexpected destination");
		return;
	}
...
static void gas_query_timeout(void *eloop_data, void *user_ctx)
{
	...
	wpa_printf(MSG_INFO1, "GAS: No response received for query to " MACSTR
		   " dialog token %u",
			    ...
			    struct gas_query_pending *query)
{
	...
	if (query->wdev)
		...
		printf("wdev is null, failed to get the mac address\n");
