...
struct gas_server_handler {
	...
	void (*status_cb)(void *ctx, u8 *da, int ok);
	void *ctx;
...
struct gas_server {
	...
	void (*tx)(void *ctx, struct wapp_dev *wdev, int freq, const u8 *da, struct wpabuf *resp,
...
static void gas_server_response_timeout(void *eloop_ctx, void *user_ctx)
{
	...
	response->handler->status_cb(response->handler->ctx, response->dst,
				     0);
	response->resp = NULL;
...
static void
gas_server_send_resp(struct gas_server *gas, struct wapp_dev *wdev,
		     struct gas_server_handler *handler,
		     ...
		     struct wpabuf *query_resp)
{
	...
	gas->tx(gas->ctx, wdev, freq, da, resp, comeback_delay ? 2000 : 0);
	wpabuf_free(resp);
...
static int
gas_server_rx_initial_req(struct gas_server *gas, struct wapp_dev *wdev, const u8 *da, const u8 *sa,
			  ...
			  const u8 *data, size_t len)
{
	...
	dl_list_for_each(handler, &gas->handlers, struct gas_server_handler,
		...
		resp = handler->req_cb(handler->ctx, sa, query_req,
				       query_req_len, NULL, 0);
		wpa_hexdump_buf(MSG_MSGDUMP, "GAS: Response from the handler",
				resp);
		gas_server_send_resp(gas, wdev, handler, sa, freq, dialog_token,
				     resp);
		return 0;
	}
...
void gas_serv_req_dpp_processing(struct gas_server *gas,
				 struct wapp_dev *wdev,
       				 const u8 *sa, u8 dialog_token,
                                 int prot, struct wpabuf *resp)
{
	struct gas_server_handler *handler;
	dl_list_for_each(handler, &gas->handlers, struct gas_server_handler,
			 list) {
		/* Write this check when registered for multiple */
#if 0
		if (adv_proto_len < 1 + handler->adv_proto_id_len ||
		    os_memcmp(adv_proto + 1, handler->adv_proto_id,
			      handler->adv_proto_id_len) != 0)
		...
		wpa_hexdump_buf(MSG_MSGDUMP, "GAS: Response from the handler",
				resp);
		gas_server_send_resp(gas, wdev, handler, sa, 0, dialog_token,
				     resp);
		return;
	}
}
...
static void
gas_server_handle_rx_comeback_req(struct gas_server_response *response, struct wapp_dev *wdev)
{
	...
	gas->tx(gas->ctx, wdev, response->freq, response->dst, resp,
...
static int
gas_server_rx_comeback_req(struct gas_server *gas, struct wapp_dev *wdev,
			   ...
			   const u8 *bssid, int freq, u8 dialog_token)
{
	...
	dl_list_for_each(response, &gas->responses, struct gas_server_response,
		...
		if (response->dialog_token != dialog_token ||
			...
			continue;
		gas_server_handle_rx_comeback_req(response, wdev);
		return 0;
	}
		  ...
		  int freq)
{
	...
	wpa_printf(MSG_DEBUG, "GAS: Received GAS %s Request frame DA=" MACSTR
		   " SA=" MACSTR " freq=%d dialog_token=%u len=%u",
		   action == WLAN_PA_GAS_INITIAL_REQ ? "Initial" : "Comeback",
		   MAC2STR(da), MAC2STR(sa) , freq, dialog_token,
		   (unsigned int) len);

	if (action == WLAN_PA_GAS_INITIAL_REQ)
		return gas_server_rx_initial_req(gas, wdev, da, sa, bssid,
						 freq, dialog_token,
						 pos, end - pos);
	return gas_server_rx_comeback_req(gas, wdev, da, sa, bssid,
					  freq, dialog_token);
}
			  ...
			  size_t data_len, int ack)
{
	...
	dl_list_for_each(response, &gas->responses, struct gas_server_response,
			 list) {
		if (os_memcmp(dst, response->dst, ETH_ALEN) != 0)
			continue;
		gas_server_handle_tx_status(response, ack);
		return;
	}

	wpa_printf(MSG_ERROR, "GAS: No pending response matches TX status");
}
