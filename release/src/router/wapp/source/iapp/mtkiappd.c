	...
	IAPP_IN		INT32				Flags)
{
	...
	if ((strlen(pCtrlBK->IfNameWlanIoctl[ApIdx]) >= sizeof(IfName)) ||
		...
		snprintf(IfName, IFNAMSIZ, "%s", FT_KDP_DEFAULT_IF_WLAN_IOCTL);
	else
		snprintf(IfName, sizeof(IfName), "%s", pCtrlBK->IfNameWlanIoctl[ApIdx]);
	/* End of if */
    ...
    VOID)
{
	...
	if(ret)
		DBGPRINT(RT_DEBUG_OFF, "iapp> system cmd ret=%d\n", ret);
	/* sleep for a while to kill old daemon */
	sleep(2);
#endif // IAPP_OS_LINUX //
...
#if 1 /* Money: BC UDP could be sent to another band directly */
{
		...
		case FT_KDP_SIG_FT_REASSOCIATION:
		{
			...
			*/
			...
			DBGPRINT(RT_DEBUG_TRACE, "iapp> Reassoc Intra AP OldAP: %02x:%02x:%02x:%02x:%02x:%02x ",
						pEvtReAssoc->OldApMacAddr[0], pEvtReAssoc->OldApMacAddr[1],
						pEvtReAssoc->OldApMacAddr[2], pEvtReAssoc->OldApMacAddr[3],
						pEvtReAssoc->OldApMacAddr[4], pEvtReAssoc->OldApMacAddr[5]);
			...
			if (Intf_idx >= 0) {		/* Detect Direct Intra AP roaming Case */
				...
				if (Ap_Idx < 0) {
					DBGPRINT(RT_DEBUG_TRACE, "iapp> The STA %02x:%02x:%02x:%02x:%02x:%02x ",
								pEvtReAssoc->MacAddr[0], pEvtReAssoc->MacAddr[1],
								pEvtReAssoc->MacAddr[2], pEvtReAssoc->MacAddr[3],
								pEvtReAssoc->MacAddr[4], pEvtReAssoc->MacAddr[5]);
					...
					if (!is_all_zero_mac(pCtrlBK->RecvDrvInfMac) &&
						...
						DBGPRINT(RT_DEBUG_TRACE, "pCtrlBK->RecvDrvInfMac %02x:%02x:%02x:%02x:%02x:%02x\n",
								pCtrlBK->RecvDrvInfMac[0], pCtrlBK->RecvDrvInfMac[1],
								pCtrlBK->RecvDrvInfMac[2], pCtrlBK->RecvDrvInfMac[3],
								pCtrlBK->RecvDrvInfMac[4], pCtrlBK->RecvDrvInfMac[5]);
	...
	IAPP_IN		UCHAR				*pRspBuf)
{
	...
	*/
	...
	DBGPRINT(RT_DEBUG_TRACE, "iapp> Delete the STA %02x:%02x:%02x:%02x:%02x:%02x\n",
				pNotify->MacAddr[0], pNotify->MacAddr[1],
				pNotify->MacAddr[2], pNotify->MacAddr[3],
				pNotify->MacAddr[4], pNotify->MacAddr[5]);
	/*
...
/* #endif */
	...
	if (SizeRcvMsg > 0)
	{
		...
		*/
		...
		DBGPRINT(RT_DEBUG_TRACE, "iapp> Receive a FT action frame from %02X:%02X:%02X:%02X:%02X:%02X\n",
	...
	IAPP_IN		INT32				if_idx)
{
	...
	DBGPRINT(RT_DEBUG_TRACE,
			"iapp> Forward a RRB packet (len = %d) from %02x:%02x:%02x:%02x:%02x:%02x!\n",
			pEvtHdr->EventLen,
			pMacAp[0], pMacAp[1], pMacAp[2],
			pMacAp[3], pMacAp[4], pMacAp[5]);

	DBGPRINT(RT_DEBUG_TRACE,
			"iapp> SA is %02x:%02x:%02x:%02x:%02x:%02x!\n",
			pMacSa[0], pMacSa[1], pMacSa[2],
			pMacSa[3], pMacSa[4], pMacSa[5]);
