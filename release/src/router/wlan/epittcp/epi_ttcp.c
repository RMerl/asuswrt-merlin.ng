...
#endif
{
	...
	if (verbose) {
		...
		snprintf(time_str, sizeof(time_str), "%s %s %d %02d:%02d:%02d %d\n",
...
outfmt(char format, double b)
{
	...
	switch (format) {
	case 'G':
		snprintf(obuf, sizeof(obuf), "%.3f GB", b / giga);
		break;
	case 'M':
		snprintf(obuf, sizeof(obuf), "%.3f MB", b / mega);
		break;
	case 'K':
		snprintf(obuf, sizeof(obuf), "%.3f KB", b / kilo);
		break;
	case 'B':
		snprintf(obuf, sizeof(obuf), "%4f B", b);
		break;
	case 'g':
		snprintf(obuf, sizeof(obuf), "%.3f Gbit", b * 8.0 / giga);
		break;
	case 'm':
		snprintf(obuf, sizeof(obuf), "%.3f Mbit", b * 8.0 / mega);
		break;
	case 'k':
		snprintf(obuf, sizeof(obuf), "%.3f Kbit", b * 8.0 / kilo);
		break;
	case 'b':
		snprintf(obuf, sizeof(obuf), "%4f b", b * 8.0);
		break;
	default:
		snprintf(obuf, sizeof(obuf), "default.........."); 
	}
	return obuf;
}
/*--------------------------------------------------------------------------*/
...
open_log()
{
	...
	SYSTEMTIME SystemTime;
#endif
	snprintf(logfile, sizeof(logfile), "%s_%s",logfile_head,trans?"t":"r");
