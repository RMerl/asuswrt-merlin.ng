...
class QisPolicyPageComponent {
    constructor(props) {
        ...
        if (policy === "EULA") {
            if (policy_status.Policy_lang === "CN") {
                title = title.replace(/(.{4})/g, '<div>$1</div>');
            } else if (policy_status.Policy_lang === "TW") {
        ...
        function changeLanguage(lang) {
            httpApi.nvramSet({
                action_mode: "apply", rc_service: "email_info", flag: "set_language", preferred_lang: lang, "skip_modfiy_flag":"1"
            }, function () {
                setTimeout(function () {
                    location.reload();
                }, 10);
