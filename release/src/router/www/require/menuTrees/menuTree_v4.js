        ...
        list: [
            ...
            */
            ...
            },
            {
                ...
                tab: [
                    {
                        url: (isSupport("mtlancfg") ? "SDN.asp" : "Guest_network.asp"),
                        tabName: isSupport("sdn_mwl") ? `<#Network#>` : Guest_Network_naming
                    },
...
/*                    {url: "TrafficAnalyzer_Statistic.asp", tabName: "<#Statistic#>"}, */
            ...
            },
            {
                ...
                tab: [
                    ...
                    {
                        url: "Advanced_AiDisk_samba.asp",
                        tabName: "<#menu5_4_1#>".concat(WebDav_support ? " / <#Cloud_Disk#>" : "")
                    },
        ...
        exclude: {
            ...
            tabs: function () {
                ...
                if (!isSupport("traffic_analyzer") && !isSupport("dns_dpi")) {
                    ...
                    for (i = 0; i < menuTree.list.length; i++) {
                        if (menuTree.list[i].menuName == '<#Adaptive_QoS#>') {
                            ...
                            menuTree.list[i].tab[index] = {
                                url: "Main_TrafficMonitor_realtime.asp",
                                tabName: "<#traffic_monitor#>"
                            };
                ...
                if (isSupport("multiwan"))
                    ...
                    retArray.push("MultiGroup_WAN.asp");
                ...
                if (isSupport("noWiFi")) {
                    ...
                    retArray.push("Main_WStatus_Content.asp");
                }
    ...
    function moveTabToNewMenu(menuArray, tabUrl, menuIdx, sourceMenuIdx = null) {
        if (!menuArray?.length) return false;

        let targetTab, sourceMenuIndex, sourceTabIndex;

        if (sourceMenuIdx) {
            sourceMenuIndex = menuArray.findIndex(m => m.index === sourceMenuIdx);
            if (sourceMenuIndex === -1) return false;

            sourceTabIndex = menuArray[sourceMenuIndex].tab.findIndex(t => t.url === tabUrl);
            if (sourceTabIndex === -1) return false;

            targetTab = menuArray[sourceMenuIndex].tab[sourceTabIndex];
        } else {
            for (let i = 0; i < menuArray.length; i++) {
                sourceTabIndex = menuArray[i].tab.findIndex(t => t.url === tabUrl);
                if (sourceTabIndex !== -1) {
                    targetTab = menuArray[i].tab[sourceTabIndex];
                    sourceMenuIndex = i;
                    break;
                }
            }
        ...
        const targetMenuIndex = menuArray.findIndex(m => m.index === menuIdx);
        if (targetMenuIndex === -1) return false;

        menuArray[sourceMenuIndex].tab.splice(sourceTabIndex, 1);

        const targetMenu = menuArray[targetMenuIndex];
        const nullIndex = targetMenu.tab.findIndex(t => t.url === "NULL");

        nullIndex !== -1
            ? targetMenu.tab.splice(nullIndex, 0, targetTab)
            : targetMenu.tab.push(targetTab, {url: "NULL", tabName: "__INHERIT__"});
    ...
    if (isSupport("gtbooster") && isSupport("ark_qoe")) {
        moveTabToNewMenu(menuTree.list, "AdaptiveQoS_InternetSpeed.asp", "menu_NekworkTool", "menu_BandwidthMonitor")
    }
