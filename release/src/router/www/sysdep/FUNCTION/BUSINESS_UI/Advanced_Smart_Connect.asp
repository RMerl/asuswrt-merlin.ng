...
function gen_bsd_steering_div(flag){
	...
	for(i = start_band_idx; i < wl_info.wl_if_total; i++){	
		...
		code +='<span id="wl'+i+'_bsd_steering_bandutil_t" name="wl0_bsd_steering_bandutil_t" style=""></span>%';
	...
	for(i = start_band_idx; i < wl_info.wl_if_total; i++){
		...
		code +='<input type="text" onkeypress="return validator.isNegativeNumber(this,event)" value="100" class="short_input input_3_table" id="wl'+i+'_bsd_steering_rssi" name="wl'+i+'_bsd_steering_rssi" maxlength="4">';
	...
	for(i = start_band_idx; i < wl_info.wl_if_total; i++){
		...
		code +='<td style="border:0px">';
		code +='<div id="wl'+i+'_bsd_steering_phy_l0_t">< <span style="_INHTML;" name="wl'+i+'_bsd_steering_phy_l_t" id="wl'+i+'_bsd_steering_phy_l_t">300</span> Mbps</div>';
	...
	for(i = start_band_idx; i < wl_info.wl_if_total; i++){
		...
		code +='</td>';
		code +='<td style="border:0px">';
		code +='<div id="wl'+i+'_bsd_steering_phy_g0_t">> <span style="_INHTML;" name="wl'+i+'_bsd_steering_phy_g_t" id="wl'+i+'_bsd_steering_phy_g_t">300</span> Mbps</div>';
...
function gen_bsd_sta_select_div(){
	...
	for(i = start_band_idx; i < wl_info.wl_if_total; i++){
		...
		code +='<td style="border:0px; padding-left:0px;">';
		code +='<select class="input_option biz_sm_rssi" name="wl'+i+'_bsd_sta_select_policy_rssi_s">';
		code +='<option selected="" value="0" class="content_input_fd"><#option_less#></option>';
		code +='<option value="1" class="content_input_fd"><#option_greater#></option>';
		code +='</select></td>';
		code +='<td style="border:0px; padding-left:0px">';
		code +='<input type="text" onkeypress="return validator.isNegativeNumber(this,event)" value="100" class="short_input input_3_table" id="wl'+i+'_bsd_sta_select_policy_rssi" name="wl'+i+'_bsd_sta_select_policy_rssi" maxlength="4">';
	...
	for(i = start_band_idx; i < wl_info.wl_if_total; i++){
		...
		code +='<div id="wl'+i+'_bsd_sta_select_policy_phy_l0_t">< <span style="_INHTML;" name="wl'+i+'_bsd_sta_select_policy_phy_l_t" id="wl'+i+'_bsd_sta_select_policy_phy_l_t">300</span> Mbps</div>';
	...
	for(i = start_band_idx; i < wl_info.wl_if_total; i++){
		...
		code +='</td><td style="border:0px;">';
		code +='<div id="wl'+i+'_bsd_sta_select_policy_phy_g0_t">> <span style="_INHTML;" name="wl'+i+'_bsd_sta_select_policy_phy_g_t" id="wl'+i+'_bsd_sta_select_policy_phy_g_t">300</span> Mbps</div>';
...
function gen_bsd_if_select_div(){
	...
	for(i = start_band_idx; i < wl_info.wl_if_total; i++){
		...
		code +='</td><td style="border:0px;">';
		code +='<span id="wl'+i+'_bsd_if_qualify_policy_t" name="wl'+i+'_bsd_if_qualify_policy_t" style=""></span>%';
		code +='</td></tr></table></div></td>';
	}
	code +='</tr>';
...
function register_event(){
		...
		$( "#slider_wl0_bsd_steering_phy_l" ).slider({
			...
			max: (based_modelid == "RT-AC5300" || 
				...
				based_modelid == "RT-BE58U" || based_modelid == "TUF-BE3600" || based_modelid == "RT-BE92U" || based_modelid == "RT-BE95U" || based_modelid == "RT-BE82U" || based_modelid == "TUF-BE82" || based_modelid == "RT-BE82M" || based_modelid == "RT-BE58U_PRO" || based_modelid == "GS-BE18000"
				)?1000:600,
			value:1,
			slide:function(event, ui){
		...
		$( "#slider_wl2_bsd_steering_phy_l" ).slider({
			...
			max: (based_modelid == "RT-AC5300" || 
				...
				based_modelid == "GT-AXE11000" ||
				based_modelid == "GT-AX6000" ||
				based_modelid == "GT-AX11000_PRO" || based_modelid == "ET12" || based_modelid == "XT12" || based_modelid == "GT-AXE16000" || based_modelid == "RT-BE96U"
				)?2167:1300,
			value:1,
			slide:function(event, ui){
		...
		$( "#slider_wl1_bsd_sta_select_policy_phy_g" ).slider({
			...
			max: (based_modelid == "RT-AC5300" || 
				...
				based_modelid == "GT-AXE16000" ||
				based_modelid == "RT-AX88U_PRO" || based_modelid == "RT-BE96U" || based_modelid == "RT-BE88U" || based_modelid == "RT-BE86U" ||
				based_modelid == "RT-BE58U" || based_modelid == "TUF-BE3600" || based_modelid == "RT-BE92U" || based_modelid == "RT-BE95U" || based_modelid == "RT-BE82U" || based_modelid == "TUF-BE82" || based_modelid == "RT-BE58U_PRO" || based_modelid == "GS-BE18000"
				)?2167:1300,
			value:1,
			slide:function(event, ui){
...
$(document).ready(function() {
    $('#clientListModal').on('shown.bs.modal', function (e) {
        if($(e.target).find('iframe').attr('src')===undefined) {
            $(e.target).find('iframe').attr('src', '/device-map/clientlist.html');
        }
    });
});
...
<form method="post" name="form" id="ruleForm" action="/start_apply.htm" target="hidden_frame">
    ...
    <input type="hidden" name="wl1_bsd_if_select_policy_x" value="<% nvram_get(" wl1_bsd_if_select_policy_x"); %>"
    disabled>
    <input type="hidden" name="wl2_bsd_if_select_policy_x" value="<% nvram_get(" wl2_bsd_if_select_policy_x"); %>"
    disabled>
                            ...
                            <table width="760px" border="0" cellpadding="4" cellspacing="0" class="FormTitle"
                                ...
                                <tr>
                                    <td valign="top">
                                        ...
                                        <table cellspacing="0" cellpadding="4" bordercolor="#6b8fa3" border="1"
                                               align="center" width="100%" class="FormTable" id="MainTable2"
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    <input type="text"
                                                           ...
                                                           value="100" class="input_6_table" name="windows_time_sec"
                                                           maxlength="4" autocorrect="off" autocapitalize="off">
                                                    <#Second#>
                                                </td>
                                            </tr>
                                            <tr>
                                                ...
                                                <td>
                                                    <input type="text"
                                                           onkeypress="return validator.isNumber(this,event)"
                                                           value="100" class="input_6_table" name="bsd_counts"
                                                           maxlength="4" autocorrect="off" autocapitalize="off">
                                                </td>
                                            </tr>
                                            <tr>
                                                ...
                                                <td>
                                                    <input type="text"
                                                           onkeypress="return validator.isNumber(this,event)"
                                                           value="100" class="input_6_table" name="dwell_time_sec"
                                                           maxlength="4" autocorrect="off" autocapitalize="off">
                                                    <#Second#>
                                                </td>
                                            </tr>
                                        ...
                                        <div class="apply_gen">
                                            <input type="button" id="restoreButton" class="button_gen_pre"
                                                   value="<#Setting_factorydefault_value#>" onclick="restoreRule();">
                                            <input type="button" id="applyButton" class="button_gen"
