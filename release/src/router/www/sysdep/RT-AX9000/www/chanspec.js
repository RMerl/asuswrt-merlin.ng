...
if(band2g_support){
	...
	wl_info['2'].dfs_support = false;
	wl_info.bw_160_support = false;
}

if(band5g_support){
	wl_info['0'] = new Object;
	wl_info['0'].dfs_support = (function(){
	...
	wl_info['0'].bw_160_support = (function(){
		var count = 0;
		for(i=0;i<_chanspecs_5g.length;i++){
			if(_chanspecs_5g[i].indexOf('/160') != -1){
				count++;
...
function wl_chanspec_list_change(){
	...
	if (phytype == "a") {	// a mode
		...
		else if(band == "1"){	// 5 GHz-2 - high band
...
function change_channel(obj){
	...
	if(band == 0 || band == 1){
		if((band == 0 && (_chanspecs_5g.indexOf('56') != -1 || _chanspecs_5g.indexOf('100') != -1)) 
		|| (band == 1 && _chanspecs_5g_2.indexOf('100') != -1)){
		...
		if(unii4Support){
			if(wl_unit == '0' && !band5g2_support && document.form.wl_channel.value  == 0){
				document.getElementById('acs_unii4_field').style.display = "";
				// document.getElementById('acs_unii4_checkbox').disabled = false;
			}
			else if(wl_unit == '1' && wl_nband_array[1] =='1' && document.form.wl_channel.value == 0){
				document.getElementById('acs_unii4_field').style.display = "";
			}
			else{
